/*
 * Copyright 2010, 2024 Sony Corporation
 */
...
static void remove_file(const char* fname_p)
{
    if (unlink(fname_p) == 0)
    {
      IDD_LOGI("Deleted %s", fname_p);
    }
    else
    {
      IDD_LOGW("Failed to delete %s : %s", fname_p, strerror(errno));
    }
}
...
void user_consent_handle(int user_consent)
{
  ...
  else
  {
    ...
    {
      remove_report(0);
      remove_file(SDATA_TXT_FNAME);
      remove_file(COUNTERS_TXT_FNAME);
