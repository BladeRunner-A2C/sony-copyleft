 ...
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Include/SpmiBusCfg.h#1 $
 * $Change: 53083464 $ 
 * 
 *                              Edit History
 * Date     Description
...
// Constants
//******************************************************************************
#define SPMI_OWNER_ADSP SPMI_OWNER_LPASS
...
typedef enum 
{
    ...
    SPMI_OWNER_TME      = SPMI_LSHIFT(7),
    SPMI_OWNER_UEFI     = SPMI_LSHIFT(0) | SPMI_LSHIFT(14), //UEFI is a Auxiliary Owner and is mapped to APPS, reserving bit 14 to recognize UEFI
...
typedef enum 
{
    ...
    SPMI_CHNL_INVALID_OPERATION,
}SpmiBusCfg_ChnlOperation;

typedef enum
{
   SPMI_CHNL_OEMCFG_RESTRICTED,
   SPMI_CHNL_OEMCFG_ALLOWED,
   SPMI_CHNL_OEMCFG_UEFI,
   SPMI_CHNL_OEMCFG_INVALID
}SpmiBusCfg_OemCfgFlag;
...
{
    uint8   busId:4;
    uint8   slaveId:4;     /**< Slave id of the SPMI device */
    uint8   periphId:8;    /**< Peripheral id on the slave device */
    uint16  irqOwnerMask:16;    /**< The EE id's mask that will receive this peripheral's interrupt (refer to SpmiBusCfg_OwnerMask enum) */
    uint16  periphOwnerMask:16; /**< The EE id's mask that has write access to this peripheral (refer to SpmiBusCfg_OwnerMask enum) */
    SpmiBusCfg_OemCfgFlag oemConfigurable:2;  /**< 1 = oem configurable  ; 0 = oem cannot configure ( refer to SpmiBusCfg_OemCfgFlag enum )*/
...
typedef struct
{
    ...
    uint16 ownerMask;    /**< The EE id's mask that will receive this peripheral's interrupt (refer to SpmiBusCfg_OwnerMask enum) */
    SpmiBusCfg_ChnlOperation operation;
} SpmiBusCfg_OemChannelCfg;

typedef struct
{
    uint8 slaveId;  /**< Slave id of the SPMI device */
    uint16 address; /**< Address (Periph id + offset) in the device */
    uint8 busId;    /**< Bus id of the SPMI device */
} SpmiBusCfg_Ppid;
...
typedef struct 
{
    ...
    boolean muxSelEn;        /**< MUX Select Enable Flag */
    uint8 muxSelVal;         /**< MUX Select Value */
...
typedef struct 
{
    uint8   busId;                  /**< SPMI BUS ID*/
    uint8   vioCtlId;               /**< VIOCTL port ID*/
    uint8   selfClearEn;            /**< Self clear enable*/  
    boolean posEdgeEn;              /**< Positive edge enable*/
    uint8   posEdgeSlaveId;         /**< Positive Edge Slave id*/
    uint8   posEdgePeriphId;        /**< Positive edge periph ID*/
    uint8   posEdgeIrqMask;         /**< Positive edge periph interrupt mask */
    boolean negEdgeEn;              /**< Negative edge enable */
    uint8   negEdgeSlaveId;         /**< Negative Edge Slave id*/
    uint8   negEdgePeriphId;        /**< Negative edge periph ID*/
...
/**
 * @brief Assigns SPMI Channel Ownership and IRQs OEM Defined peripherals
 * 
 * This function will map logical SW Channels to the peripherals and set ownership.  
 * The mapping will exist for both the owner and the observer SW channels.
 * 
