...
typedef enum {
   ...
   PCIE_RESOURCE_PREFETCH_MEM_SPACE_64          = 3,
   PCIE_RESOURCE_NON_PREFETCH_MEM_SPACE_32      = 4,
...
/**
 * @brief
 * Determines whether a PCIe segment should be considered hot pluggable.
 * It is hot pluggable if the hot pluggable flag is set on the root port,
 * and the platform policy api has not been used to enable the root port.
 *
 * @param rp_index The segment number
 * @return true if the segment should be considered as hot pluggable
 *         false otherwise
 */
uint32 pcie_rp_is_hot_pluggable(uint32 port_index);

/* Reload all the config after a HW port config change */
pcie_status_t pcie_rp_cfg_reload_config (void);

/* Enable port configuration by index */
