...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Include/Library/Clock.h#1 $
...
/**
  ...
  Example: The CLOCK_QUERY_PERF_LEVEL_* queries use the nIndex argument
  to determine which performance level to query.
...
** =========================================================================*/
/**
  Register for a callback when a clock driver controlled resource changes
  state.

  NOTE:
  - Only clients from root PD can register for the callbacks.
  - All callbacks are invoked with a mutex held. It is not recommended to
    lock another mutex within the callback.
  - A client is not expected to modify the state of the resource it has
    registered a callback for.
  - Any callback will only be invoked when the resource is actually enabled or
    disable in hardware.

  @param hClient        [in]  Handle to the client.
  @param nResourceId    [in]  Resource ID (ClockPowerDomainIdType)
  @param eEvent         [in]  Event type
  @param pFn            [in]  Callback function to be registered.
  @param pCtxt          [in]  Parameter associated with the callback function.
  @param *pnCallbackId  [out] Pointer to the callback ID to be filled in.
                              This can be used during de-registration.

  @return
  CLOCK_SUCCESS -- Registration successful
  CLOCK_ERROR -- Registration unsuccessful
...
(
  ...
  ClockCallbackIdType   *pnCallbackId
);
...
** =========================================================================*/
/**
  Deregister for a registered resource state change callback.

  @param hClient      [in]  Handle to the client.
  @param nCallbackId  [in]  Callback ID

  @return
  CLOCK_SUCCESS -- De-registration successful
  CLOCK_ERROR -- De-registration unsuccessful
