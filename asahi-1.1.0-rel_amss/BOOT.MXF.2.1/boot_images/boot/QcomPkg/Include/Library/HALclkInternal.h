...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Include/Library/HALclkInternal.h#1 $
 ...
 */
#define ENCODE_NOT_2D(n2DVal)      (~(n2DVal))
 ...
 * HAL-specific flags
 *
 * DIVIDER_SUPPORTS_HID - Indicates that the divider supports half
 *   integer division.
 */
#define HAL_CLK_FLAG_DIVIDER_SUPPORTS_HID       0x01
...
typedef struct
{
  ...
  boolean (*IsCalibrated)       ( HAL_clk_SourceDescType *pSource );
  boolean (*SetCalConfig)       ( HAL_clk_SourceDescType *pSource, const HAL_clk_PLLConfigType *pmConfig);
 ...
 * nGDSHWCtrlAddr       - Global distributed switch controller (GDSC) HW CTRL register address.
 * VoterRegister        - Register/mask for HW votable configuration setup.
...
boolean HAL_clk_WaitForPowerDomainOff ( HAL_clk_PowerDomainDescType *pmPowerDomainDesc );
