/** @file QcomLib.h
  ...
  Copyright (c) 2011-2021,2022 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 08/08/22   cm      Added GetFileSizeOverPxe and ReadFileOverPxe
 04/12/21   bpk     Added ConnectAllDrivers()
 03/05/21  visver   Added New API GetECID
 15/03/21   bpk      move part of the macro and struct def to 
                    QcomEnvType.h in QcomSdkPkg, in order to expose some functions 
                    by QcomEnv Protocol
 10/31/19   shl     Added KEYMASTER_LOAD_FLAG
...
#define         KEYMASTER_LOAD_FLAG                 0x80         
#define         ENABLE_QSEE_LOGS_FLAG               0x100 
#define         ENABLE_QSEE_DIAG_LOGS_FLAG          0x200     
#define         MS_SEC_APP_FLAG                     0x400 
#define         ENABLE_TCG_DXE_FLAG                 0x800
#define         ENABLE_SECURITY_STUB_DXE_FLAG       0x1000
#define         ENABLE_MEASUREBOOT_EVENT_LOG_FLAG   0x2000
...
#ifdef FEATURE_XBOOT
#define NO_TIMEOUT_WAIT_FOR_KEY 0x00
#endif /* FEATURE_XBOOT */
#define TIMEOUT_WAIT_FOR_KEY    0x01

/* Maximum length of a file path */
#define MAX_FILE_PATH_LENGTH_IN_NETWORK      256

/* Server IP index*/
#define DHCP_SERVER_OPTION_INDEX 5  /*DHCP ACK OPTION 5th INDEX*/
...
/**
  Return the size of a file, that is in a network path
...
/**
  Load given file name from network path

  @param  Path                File path/network path
  @param  FileSize            Size of file
  @param  BufferPtr           Buffer pointer
  @return EFI_SUCCESS         On successfully download file
  @return OUT_OF_RESOUCE      If file path is not available, memory is not available
  @return FAIL                If file fail to download
**/
...
/**
  Read a file Over network to a  buffer
...
/**
  Wrapper around WriteToPartition to enable setting default
  partition attributes

  @param[in]  PartitionGUID     GUID of the partition
  @param[in]  PartitionLabel    Partition name as in partition.xml
  @param[in]  Buffer            Data Buffer
                                Data from buffer will be copied into storage partition.
**/
...
/* This function gets the Block IO protocol instance from the handle */
EFI_STATUS GetBlkIO (
                          EFI_GUID              *PartitionGuid   OPTIONAL,
                          CHAR16                *PartitionLabel  OPTIONAL,
...
/**
  This function returns a concatenation of Die and Device HW serial number
