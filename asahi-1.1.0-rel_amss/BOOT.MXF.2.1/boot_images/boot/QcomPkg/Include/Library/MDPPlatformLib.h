...
typedef enum
{
  ...
  MDPPLATFORM_CONFIG_DPGPIOCONFIG,                       /* Configure DP related GPIO */
...
typedef struct
{
    ...
    uint32                          uPMICId;               /* PMIC chip number. This is set to zero if it is a single PMIC chip.*/
    uint32                          uBrightnessMax;        /* Maximum luminance of the display in millinits (1/1000 of a nit) */
    uint32                          uBrightnessMin;        /* Minimum luminance of the display in millinits (1/1000 of a nit). */ 
    uint32                          uPwmDesiredFreqHz;     /* PMIC PWM desired frequency in Hz*/
    union {
...
/* DP GPIO config information
 */
typedef struct
{
  MDP_DP_ControllerModeType eDPControllerModeType;          /* cable plug orientation: only valid for DP over USB-type C */
} DPGPIOConfigInfo;
...
typedef union
{
    ...
    DPGPIOConfigInfo           sDPGPIOConfigInfo;   /* MDPPLATFORM_CONFIG_DPGPIOCONFIG */
} MDPPlatformParams;
