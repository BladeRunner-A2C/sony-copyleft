...
DESCRIPTION
  ...
  Copyright (c) 2004-2009, 2012-2013, 2019-2020,2022, 2023 Qualcomm Technologies Incorporated. 
  All Rights Reserved.
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Include/Library/flash_miparti.h#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $

   
when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/25/23   tvra    revert FLASH_NUM_PART_ENTRIES count to 64.
04/25/23   tvra    Updated FLASH_NUM_PART_ENTRIES count to 80.
10/27/22   gk      Updated MIBIB offset for Pinnacles and Kuno 
10/14/22   gml     Added sbl_b partition and updated mibib partition offsets
09/22/22   tvra    Updated FLASH_NUM_PART_ENTRIES count to 54.
06/08/20   sa      Ported for Olympic.
10/25/19   sa      Changes to allow 'FLASH_NUM_PART_ENTRIES' to be set from inf file.
08/29/19   sa      Updated FLASH_NUM_PART_ENTRIES count.
...
#define FLASH_MI_BLOCK_MIBIB_AUTO 0x14
...
/*  This define can be changed from the inf file.
  If 'FLASH_NUM_PART_ENTRIES' is not defined in inf files, the value will
  be set to '64'. Else the value of 'FLASH_NUM_PART_ENTRIES' will
  be taken from the inf files.
 */

#ifndef FLASH_NUM_PART_ENTRIES
#define FLASH_NUM_PART_ENTRIES  64
#endif

/* The maximum number of partitions that we support is defined by 
 * "FLASH_NUM_PART_ENTRIES". We create handles for each of these partitions. 
 * Apart from that we also create handle for the whole device using "all"
 * partition name. So we need space to store a extra handle.
 */
