...
// --------   ---      ----------------------------------------------------------
// 07/01/21   pw       add qusb_dci_eud_recovery
...
// ===========================================================================
/**
 * @function    qusb_dci_cancel_unstall_requeue
 * 
 * @brief       This function handle unstall ep with active transfer queued
 * 
 * @details
 *      On ClearFeature (ENDPOINT_HALT), software must first remove all pending 
 *      transfers for the endpoint through the End Transfer command. 
 *      It may then issue a Clear Stall command on the endpoint followed by Start
 *      Transfer to start transfers again. 
 *      For a description of the Clear Stall endpoint command and its effects.
 *      After cancel transfer and unstall endpoint also requeue the urb, so client
 *      does not require to handle clear feature.
 *   
 * @note : Any write to the ENDPTCTRLx register during operational mode must preserve the endpoint type field (i.e. perform a readmodifywrite).
 *  
 * @note : Reset Data-Toggle as mentioned in Chapter 8.3.3.1 Data Toggle Reset
 *
 * @note : Section "Commands 4 and 5: Set Stall and Clear Stall (DEPSSTALL, DEPCSTALL)" on page 320.   
 *
 * @param ep    
 *          endpoint number
 * 
 ...
 *
 * @brief       This API will wait and detect the Super Speed USB Lane.
 *               
 *
 * @param   None
 *
...
// ===========================================================================
/**
 * @function    qusb_dci_handle_charger_cdp
 *
 * @brief       This API will wait and detect charger type.
 *              If the charger type is Charging Downstream Port (CDP),
 *              USB2PHY D+ will be toggled (pulled high, then low) to 
 *              satifiy CDP requirement.
 *
 * @param   None
 *
 * @return  None
...
//============================================================================
/** 
 * API is used to get CC OUT super speed lane
 * 
 * @param  None
 *
 * @return returns lane A, B, or NONE
 */ 
 //============================================================================
