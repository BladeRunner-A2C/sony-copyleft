...
*/
/*=============================================================================
  Copyright (c) 2011-2013,2022 Qualcomm Technologies, Incorporated.
  All rights reserved.
...
/** Protocol version. */
#define ACPIPLATFORM_REVISION EFI_ACPIPLATFORM_REVISION_4
#define EFI_ACPIPLATFORM_REVISION_4 0x0000000000010004
/** TableSignature macro for all tables in AcpiTableRegister  **/
#define ACPI_TABLE_SIGNATURE_ALL 0x0

/** Max device name size in ACPI **/
#define ACPI_MAX_DEVICE_NAME_SIZE ( 64 )

/** Max dsdt device nodes to patch in ACPI **/
#define ACPI_MAX_DSDT_DEVICES ( 64 )
/*===========================================================================
...
/* ACPIGetTableCallbackEx */
/**
  @par Summary
  This callback function is called when a table with the registered signature
  is read. The client driver can use this callback to save a copy of the table.
  ACPIGetTableCallbackEx differs from the ACPITableGetCallbackPtr in terms of 
  the capability to carry the context as part of the callback
  ...
  @param[in] IsPrefix        Whether to get the prefix tables or the postfix 
                             tables.
  @param[in] CallbackContext Context which will return back to callback function
...
/* ACPISetTableCallbackEx */
/**
  @par Summary
  In this callback function, the client updates the ACPI table obtained from 
  ACPIPlatformDXE and passes it back.ACPISetTableCallbackEx differs from the 
  ACPITableSetCallbackPtr in terms of the capability to carry the context as 
...
/**
 AMLGetVariableCallbackEx differs from the AMLVariableGetCallbackPtr in terms of the 
 capability to carry the context as part of the callback
 This callback function is called when a variable with the registered name
 is read. The client driver can use this callback to obtain the variable's value.
 Since loading of ACPI tables into system memory is blocked till the callback functions 
 complete, client drivers should return from callback functions as soon as possible.
 ...
 and the size indicates total size of all AmlVariableEntry. Client driver will have to
 traverse through buffer correctly using the AmlVariableEntry->Length to determine size of
 the value, since string variables have variable length. This call back also support Context
 parameter which will be returned back to callback.
...
typedef
EFI_STATUS
(EFIAPI *AMLGetVariableCallbackEx )
(
  IN VOID           *AmlVariableBuffer,
  IN UINTN           AmlVariableBufferSize,
  IN BOOLEAN         IsPrefix,
...
/**
 AMLSetVariableCallbackEx differs from the AMLVariableSetCallbackPtr in terms of the 
 capability to carry the context as part of the callback
 In this callback function, client should update AML global variables obtained from 
 ACPIPlatformDXE and pass it back. Since loading of ACPI tables into system 
 memory is blocked till the callback functions complete, client drivers should 
 ...
 updates. Also, if the data does not adhere to the variable type, it will be truncated (i.e. variable type is
 byte, but buffer contains a word). ACPIPlatformDXE has no way of knowing if buffer was modified to be larger
 so it will only copy the expected size, thus truncating any exceeding data.This call back also support Context
 parameter which will be returned back to callback.
...
typedef
EFI_STATUS
(EFIAPI *AMLSetVariableCallbackEx )
(
  IN OUT  VOID         **AmlVariableBuffer,
  IN      UINTN          AmlVariableBufferSize,
  IN      VOID          *CallbackContext    OPTIONAL
...
/*EFI_ACPI_PLATFORM_REGISTER_EX*/
/** @ingroup efi_acpi_platform_register
  @par Summary
  Registers the callback function to support ACPI table query and update 
  services.EFI_ACPI_PLATFORM_REGISTER_EX differs from the 
  EFI_ACPI_PLATFORM_REGISTER in terms of the capability to carry the context
  ...
  @param[in] SetTableCallback  Pointer to the ACPITableSetCallbackPtr() 
                               callback function; can be NULL.
  @param[in] CallbackContext   Context which will return back to callback functions
  @return
  EFI_SUCCESS           -- Registration succeeded.
  @par
  EFI_OUT_OF_RESOURCES  -- Request could not be completed due to lack of 
...
typedef
EFI_STATUS
(EFIAPI *EFI_ACPI_PLATFORM_REGISTER_EX)(
  IN  EFI_QCOM_ACPIPLATFORM_PROTOCOL    *This,
  IN  UINT32                             TableSignature,
  IN  GETMODE                            GetMode,
...
/* EFI_ACPI_AML_VARIABLE_REGISTER_EX */
/** 
  Register callback function to support ACPI table query/update services.
  EFI_ACPI_AML_VARIABLE_REGISTER_EX differs from the EFI_ACPI_AML_VARIABLE_REGISTER in terms of the capability to 
  carry the context as part of the callback
...
typedef
EFI_STATUS
(EFIAPI *EFI_ACPI_AML_VARIABLE_REGISTER_EX )
(
  IN  EFI_QCOM_ACPIPLATFORM_PROTOCOL    *This,
  IN  CHAR8                              VariableName[AML_NAMESTRING_LENGTH],
  IN  GETMODE                            GetMode,
...
/* EFI_ACPI_REGISTER_DEVICES_FOR_DSDT_STA_PATCH */
/** 
...
typedef
EFI_STATUS
(EFIAPI *EFI_ACPI_REGISTER_DEVICES_FOR_DSDT_STA_PATCH )
(
  IN  EFI_QCOM_ACPIPLATFORM_PROTOCOL    *This,
...
struct _EFI_QCOM_ACPIPLATFORM_PROTOCOL {
   ...
   EFI_ACPI_PLATFORM_REGISTER_EX                     AcpiTableRegisterEx;					/* Added in EFI_ACPIPLATFORM_REVISION_4*/
   EFI_ACPI_AML_VARIABLE_REGISTER_EX                 AmlVariableRegisterEx;					/* Added in EFI_ACPIPLATFORM_REVISION_4*/
}; 
