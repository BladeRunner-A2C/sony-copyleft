/**
  @file  EFIRmVm.h
  @brief UEFI Resource Manager EFI protocol interface.
*/
/*=============================================================================
...
/** @cond */
typedef struct _EFI_RMVMINTF_PROTOCOL EFI_RMVM_PROTOCOL;
/** @endcond */
...
#define EFI_RMVM_PROTOCOL_GUID \
  { 0X0F987C66, 0X6694, 0X4663, { 0X93, 0X63, 0X10, 0X6B, 0X57, 0XB4, 0XFD, 0XBD } }
...
{
  UINT16 vmid;         /**< Participant VMID to whom memparcel is shared/donated */
  UINT8  rights;       /**< Participant VMID access rights read/write/execute */
  UINT8 reserved;
} RmMemAclEntry;

typedef struct __attribute__((packed)) _RmMemAcl 
{
  UINT16 acl_entries_count;   /**< Total number of ACL entries*/
  UINT16 reserved;
...
{
  UINT16 sgl_entries_count;   /**< Total number of SGL entries*/
  UINT16 reserved;
...
{
  UINT16 attributes;      /**< Memory region attributes */
  UINT16 attrVmid;        /**< Participant VMID affected by attribute entry */
} RmMemAttributeEntry;

typedef struct __attribute__((packed)) _RmMemAttributes 
{
  UINT16 attr_entries_count;          /**< Total number of entries*/
  UINT16 reserved;
  RmMemAttributeEntry AttrEntries[];  /**< Start of entries */
} RmMemAttributes;
...
/** @ingroup efi_rm_fw_set_vm_firmware
  ...
  @param[in]      MemHandle     Handle to a memory parcel, which must have previously
                                been donated to RM.
  @param[in]      FwOffset      Firmware image's offset into the specified memory parcel.
