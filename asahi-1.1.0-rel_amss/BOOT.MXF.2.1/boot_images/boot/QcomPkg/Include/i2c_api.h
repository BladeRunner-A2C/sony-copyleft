 ...
 */
/*=============================================================================
         Copyright (c) 2017,2019,2020,2022 Qualcomm Technologies, Incorporated.
                              All rights reserved.
...
#define SMBUS_FLAG_CMD       0x00001000 /**< To be set to indicate Command is to be appeneded to write transfer. */
#define SMBUS_FLAG_LEN       0x00002000 /**< To be set to indicate Length is to be appended to write transfer, 
                                             for read transfers setting this compares byte[0] to length of packet read*/
...
/**
  sets slave config as per client request and stored in t_ctxt -> config
  (specially added to support different frequencies other than 400kHz in pmbus/smbus)
  
  @param[in] i2c_handle  Handle to the I2C instance.
             config      slave config structure
  
**/
i2c_status 
i2c_set_slave_config
(
    void *i2c_handle, 
	i2c_slave_config *config
);
