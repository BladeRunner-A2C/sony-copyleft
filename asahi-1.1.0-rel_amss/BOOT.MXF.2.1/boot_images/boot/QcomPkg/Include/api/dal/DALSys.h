...
@file DALSys.h
        ...
        Copyright (c) 2008,2010,2014,2022,2023 Qualcomm Technologies, Inc.
        All Rights Reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 03/15/23   ck     Added com_dtypes.h
...
#define DALSYS_memcpy( pDest, pSrc, len ) \
   DALSYS_memscpy((VOID *)pDest,(UINTN)len,(CONST VOID *)pSrc,(UINTN)len)
...
/*
  @brief Allocate memory from local heap with a specific alignment

  This is similar to Malloc from the C library, but this will allocate memory
  within the DAL Environment Context with a certain alignment. 
  Memory alloc'd through this call should be freed by calling DALSYS_Free.

  @param dwSize   : Requested size of memory to be alloced
         alignment: Requested alignment of memory to be alloced
         ppMem    : Return parameter, callee will populate this to point to the
                    block of alloced memory, valid only upon DAL_SUCCESS return
                    code
  @see DALSYS_Free
 ...
 */
UINTN DALSYS_memscpy(VOID * pDest, UINTN iDestSz, CONST VOID * pSrc, UINTN iSrcSize);
