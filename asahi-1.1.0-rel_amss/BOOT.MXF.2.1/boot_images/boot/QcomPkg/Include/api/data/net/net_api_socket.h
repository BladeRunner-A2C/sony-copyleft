...
/** @file net_api_socket.h
 * @addtogroup net_api_socket_datatypes
 * @{
 *
...
#ifndef _NET_API_SOCKET_H_
#define _NET_API_SOCKET_H_

#include <stdint.h>


#undef htonl
#define htonl(l) (((((l)>>24) & 0x000000ff))| ((((l)>>8) & 0x0000ff00)) | (((l) & 0x0000ff00)<<8) | (((l) & 0x000000ff)<<24))
#undef htons
...
typedef int32_t net_api_Status_t;


/*
 * Address families (AF) for "domain" or "family" argument in net_api_socket()
 */
...
#define     SO_RST_SEM     0x1018      /* reset so_semaphore to socket's my_semaphore */
...
/** @name Flags for net_api_shutdown()
@{ */
...
/** Infinite time for the timeout_ms argument in net_api_select(). */
#define     NET_API_NET_WAIT_FOREVER   (0xFFFFFFFF)

/** Pre-pending net_api to internal socket structures' names to avoid conflict with internal
 *  structures. */
#define in6_addr   net_api_in6_addr
#define ip6_addr   net_api_ip6_addr
#define ip46addr_n net_api_ip46addr_n
...
/** @} */ /* end_addtogroup net_api_socket_datatypes */
...
#undef  fd_set
#define FD_SETSIZE_V2 150

/**
 * File descriptor sets for net_api_select().
 */
typedef struct
{
   uint32_t fd_count;
...
typedef struct
{
   uint32_t fd_count;
   /**< File descriptor count. */
   uint32_t fd_array[FD_SETSIZE_V2];
   /**< File descriptor array. */
} fd_set_v2;
...
#undef FD_SET
#undef FD_ISSET
#define FD_ZERO(set)            net_api_fd_zero((set))           /**< Clears a set. */
#define FD_CLR(handle, set)     net_api_fd_clr((handle), (set))    /**< Removes a given file descriptor from a set. */
#define FD_SET(handle, set)     net_api_fd_set((handle), (set))    /**< Adds a given file descriptor from a set. */
#define FD_ISSET(handle, set)   net_api_fd_isset((handle), (set))  /**< Tests to see if a file descriptor is part of the set after select() returns. */

/** @} */ /* end_namegroup */

/** @} */ /* end_addtogroup net_api_socket_datatypes */

/** @ingroup net_api_lib_init
 *
 * Initializes net_api_library.
 *
...
/** @ingroup net_api_lib_deinit
 *
 * De-Initializes net_api_library.
 *
 * @return On success, a handle for the new socket is returned. \n
 * On error, -1 is returned.
 */
...
/** @ingroup net_api_bind
 *
 * Assigns an address to the socket created by net_api_socket().
 *
 * @param[in] handle    Socket handle returned from net_api_socket().
 ...
 * @param[in] addrlen   Specifies the size, in bytes, of the address pointed to by 
 *       address.
 *
...
/** @ingroup net_api_listen
 *
 * Marks the socket as a passive socket.
 *
 * @param[in] handle    Handle (returned from net_api_socket()) that refers to a SOCK_STREAM socket.
 ...
 *  EINVAL: Invalid Arguments Passed. \n
 *  EADDRNOTAVAIL: Address not available. \n
 *  EADDRINUSE: Address is in use, port is in use, and SO_REUSEADDR is not set.
 */

net_api_Status_t net_api_listen(int32_t handle, int32_t backlog);

/** @ingroup net_api_accept
 *
 ...
 * as nonblocking, net_api_accept() blocks the caller until a connection is present.
 * If the socket is marked nonblocking and no pending connections are present on the queue,
 * net_api_accept() fails with the error EAGAIN or EWOULDBLOCK.
 *
 * @param[in] handle    Socket handle that has been created with net_api_socket(), bound to a
 *                      local address with net_api_bind(), and listens for connections after
 *                      net_api_listen().
 ...
 * On error, -1 is returned.
 * Errors:
 *  EINVAL: The socket provided is not in listen state. Need to call net_api_listen(). \n
 ...
 *  ENP_PARAM: Trying to set a socket option that is either not supported or not present. \n
 *  ENOBUFFS: No Memory.
 */

net_api_Status_t net_api_setsockopt(int32_t handle, int32_t level, int32_t optname, void *optval, int32_t optlen);

/** @ingroup net_api_getsockopt
 *
 ...
 * @return On success, 0 is returned. On error, -1 is returned.
 *  EINVAL: Invalid arguments passed to set the value. \n 
 *  ENOTCONN: Trying to set socket options that require a socket
 *   to be in a connected state for setting the same. \n
 *             
 ...
 *  EWOULDBLOCK: If the socket is in the non-blocking state, a receive timeout is set, 
 *  and no data is received for that duration. \n
 *                
 *  ENOBUFS: No memory.
 */
...
/** @ingroup net_api_recv
 *
 * Receives a message from a socket.
 *
 * The net_api_recv() call is normally used only on a connected socket
 * and is identical to net_api_recvfrom(handle, buf, len, flags, NULL, NULL)
 *
 * @param[in] handle    Socket handle returned from net_api_socket().
 * @param[in] buf       Pointer to a buffer for the received message.
 * @param[in] len       Number of bytes to receive.       
 ...
 *  ENOTCONN: Socket is in disconnected state. \n
 *  EWOULDBLOCK: If the socket is in the non-blocking state, a send timeout is set, 
 *  and no data is received for that duration. \n
 *  EPIPE: For a TCP Socket this shows that peer has disconnected the socket. \n
 *  ENOBUFS: No memory.
 */
 ...
 * net_api_sendto(handle, buf, len, flags, NULL, 0)
 *
 * @param[in] handle    Socket handle returned from net_api_socket().
 ...
 *                      diagnostic or routing programs. @vertspace{-14} 
 *
 * @return The number of bytes sent, or -1 if an error occurred and error number is set appropriately. \n
 *  Errors: \n
 *    ENOTCONN: Socket is in disconnected state. \n
 *    EWOULDBLOCK: If the socket is in non-blocking state, a send timeout is set, and
 *                no data is received for that duration. \n
 *    EPIPE: For a TCP socket this shows that a peer has disconnected the socket. \n
 *    ENOBUFS: No memory.
 */

int net_api_send(int32_t handle, char *buf, int32_t len, int32_t flags);

/** @ingroup net_api_select
 *
 ...
 * } 
 *@ingroup net_api_select_v2
 *
 * Enhanced net_api_select with increased number of socket file descriptors.
 * Monitors multiple socket handles, waiting until one or more of them become "ready"
 * for some class of I/O operation (e.g., read, write, etc.). 
 *
 * The call causes the calling process to block waiting for activity on any of a
 ...
 * Data\_Services 1.10.0  &  Introduced. @tblendline
 * } 
 *@ingroup net_api_fd_zero_v2
 *
 * Enhanced net_api_fd_zero with with increased number of socket file descriptors.
 * Initializes a socket that is set to zero.
 *
 * @param[in] set   Pointer to a list of sockets.
 *
 ...
 * Data\_Services 1.10.0  &  Introduced. @tblendline
 * }
 * @ingroup net_api_fd_clr_v2
 *
 * Enhanced net_api_fd_clr with with increased number of socket file descriptors.
 *
 * Removes a socket from the socket set.
 *
 * @param[in] handle    Socket handle returned from net_api_socket().
 ...
 * Data\_Services 1.10.0  &  Introduced. @tblendline
 * }
 * @ingroup net_api_fd_set_v2
 *
 * Enhanced net_api_fd_set with with increased number of socket file descriptors.
 *
 * Adds a socket to the socket set.
 *
 * @param[in] handle    Socket handle returned from net_api_socket().
