...
-------------------------------------------------------------------------------
  ...
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Include/api/systemdrivers/icbarb.h#1 $
 ...
 --------   ---     -----------------------------------------------------------
 12/03/18   sds     Add slave and master info queries
 06/26/18   sds     Add query for slave scaling unit
 02/02/18   sds     Add client query
 06/26/17   sds     Add support for Deinit/Reinit
...
typedef enum
{
   ...
   /* DEPRECATED: Get information about a specific slave */
...
/*
 * Slave query data for ICBARB_QUERY_SLAVE_INFO
 */
typedef struct
{
   ICBId_SlaveType eSlaveId; /* The slave to query */
   uint32 uNumPorts;         /* Number of ports for the slave */
   uint32 uPortWidth;        /* The width of ports for the slave */
   uint32 uScaleUnit;        /* Hardware scaling unit for this node */
   uint32 uEstFreqKHz;       /* An estimated frequency requirement for this node, if applicable. */
} ICBArb_QuerySlaveType;
...
/*
 * Master query data for ICBARB_QUERY_MASTER_INFO
 */
typedef struct
{
   ICBId_MasterType eMasterId; /* The master to query */
   uint32 uNumPorts;           /* Number of ports for the master */
   uint32 uPortWidth;          /* The width of ports for the master */
   uint32 uScaleUnit;          /* Hardware scaling unit for this node */
   uint32 uEstFreqKHz;         /* An estimated frequency requirement for this node, if applicable. */
} ICBArb_QueryMasterType;
...
typedef struct
{
   npa_client_handle client;         /**< Client to use for query */
   boolean           is_xo_blocking; /**< Will this client currently block XO? */
} ICBArb_QueryClientType;
...
typedef union
{
   ...
   ICBArb_QuerySlaveType          slaveInfo;
   ICBArb_QueryBCMType            bcmRequest;
   ICBArb_QueryClientType         clientInfo;
   ICBArb_QueryMasterType         masterInfo;
...
/*============================================================================
                   DATA TYPES - Client attributes
============================================================================*/
typedef enum
{
   /* Nothing special about this client. */
   ICBARB_CLIENT_ATTR_DEFAULT       = 0,

   /* Ignore this client for purposes of active counting */
   ICBARB_CLIENT_ATTR_IGNORE_ACTIVE = 1<<0,

   /* Maximum allowed mask. */
   ICBARB_CLIENT_ATTR_MAX           = ICBARB_CLIENT_ATTR_IGNORE_ACTIVE,
   ICBARB_CLIENT_ATTR_PLACEHOLDER   = 0x7FFFFFFF /**< Force 32-bit */
} ICBArb_ClientAttrEnumType;
...
/**=============================================================================
   ...
   @brief icbarb_deinit - De-initialize and destroy the ICB arbiter nodes

   This function is for deinitializing the driver for a hibernate/power down
   scenario. It will clean-up old state.

   @pre Requires that all clients and nodes be torn down by NPA framework prior
        to this call.
...
/**=============================================================================
   ...
   @brief icbarb_reinit - Re-initialize and define the ICB arbiter node
...
/**=============================================================================
   ...
   @brief icbarb_set_client_attr - Set client attributes
   
   Set a client attribute
   
   @param client : the client to operate on
   @param attr : the mask of attributes to set
...
/**=============================================================================
   ...
   @brief icbarb_clear_client_attr - Clear client attributes
   
   Clear a client attribute
   
   @param client : the client to operate on
   @param attr : the mask of attributes to clear
