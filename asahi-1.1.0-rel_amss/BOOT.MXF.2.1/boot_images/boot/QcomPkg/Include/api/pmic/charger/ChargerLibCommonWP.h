...
/*===========================================================================
 ...
 01/10/19   vr     support battery missing error handling, either shutdown or booting to HLOS
 01/10/19   vr      support fg ship mode
 07/20/18   sb      Changed USB port type to match hardware
 07/05/18   sb      Added DISABLEBATTERY UEFI variable
 06/29/18   dc      Support TypeC Charger Type
 06/29/18   dc      Move PON reading to Library Level
 06/29/18   dc      Support Input Current Status API & Log
 06/21/18   cs      need to include EFIPlatformInfoTypes.h
...
#define DEFAULT_LOGFILE_SIZE   0x400000 // 4MB
...
#define _ULOG(Expression)   ULogPrint Expression
  ...
  #define CHARGER_LOG_DEBUG(Expression)        \
    do {                           \
      if (ChargerLib_PrintDebugMsg ()) {  \
        _DEBUG (Expression);       \
      }                            \
...
typedef enum
{
  ...
  CHARGERLIB_CHARGING_FW_LINK_ERROR,
  /**< Charger lib FW link Error*/
...
typedef enum
{
  ...
  CHARGERLIB_ERROR_ACTION_LOAD_CHARGER_FW,
  /**< Action required for Try Loading Charger FW in case its needed to recover */

  CHARGERLIB_ERROR_ACTION_CONNECT_CHARGER_FW,
  /**< Action required for estblishing Link to Charger FW */

  CHARGERLIB_ERROR_ACTION_RESET_COLD,
  /**< Action required for resetting device */
...
typedef enum _EFI_BATTERY_GAUGE_HW_TYPE {
  ...
  EfiBatteryGaugeExternal,

  EfiBatteryGaugeVirtual
  /**< External FG   */
...
typedef struct
{
  ...
  BOOLEAN DISABLEBATTERY;
  /**< Flag to Enable/Disable DISABLEBATTERY flag.  */
...
typedef enum
{
  ...
  CHARGERLIB_PLATFORM_HW_CONFIG_SMB_EXTFG,
...
{
  BOOLEAN   enable;
  UINT32    vbatThLightLoad;
  UINT32    vbatThDeltaHeavyLoad;
}chargerlib_offmodesrc_cfg;


typedef struct
{
    uint32 soc_centipct;
    /**<Battery SOC in 0.01% at which device will boot to HLOS with certain level of USB adapter power */

    uint32 adpt_power_w;
    /**<USB adapter power at which device is allowed to boot to HLOS with certain level of battery soc */
    
}CHARGERLIB_BOOT_THRESHOLD_TYPE;

#define BOOT_TH_SIZE 3
...
typedef struct
{
  ...
  //uint32   barrel_chg_boot_threshold_in_soc;
  /**<Battery SOC at which device will boot to HLOS with barrel charger*/

  //uint32   barrel_chg_min_power_in_w;
  /**<Mininum power for barrel charger to have different soc boot threshold than USBC charger */

  //uint32   dead_batt_usb_chg_boot_soc;
  /**<Mininum USB input power to boot with a dead battery */

  //uint32   dead_batt_usb_chg_boot_min_power_w;  
  /**<Minimum USB input power to boot with a dead battery */

  CHARGERLIB_BOOT_THRESHOLD_TYPE boot_th_usb_adpt[BOOT_TH_SIZE];
  /**<Boot threshold combination (Battery SOC, USB adapter power) to allow boot to HLOS */

  CHARGERLIB_BOOT_THRESHOLD_TYPE boot_th_dc_adpt[BOOT_TH_SIZE];
  /**<Boot threshold combination (Battery SOC, DC adapter power) to allow boot to HLOS */
  ...
  BOOLEAN  enable_charger_fw;
  /**<  Enable Charger FW */

  UINT32  glink_down_action;
  /**< Action to take when GLink communication goes down */
  UINT32  fw_missing_action;
  /**< Action to take when failing to load Charger FW */

  UINT32  chargerfw_missing_display_time;
  /**< No. of seconds to display charger firmware missing message */
  ...
  BOOLEAN  ForceOffmodePowerTest;
  /**< force mtp, cdp to stay in uefi power testing mode, do not boot to HLOS */

  BOOLEAN  FwUpdateSupport;
  /**<  FW Update support */

  chargerlib_offmodesrc_cfg  off_mode_src_cfg;
  /**<  Off mode sourcing support */
  
  BOOLEAN connect_glink_logging_channel;
  /**< Enable/disable pmic glink channel connection for ChargerPD Logging based on logging timer*/

  uint32 vbatt_threshold_margin_in_mv;
  /**<adds some hysteresis(in mV) to boot_to_hlos_threshold_in_mv above which device will boot to HLOS */

  uint32 barrel_chg_pmic_index;  
  /**<pmic index paring with barrel charger */

  uint32 barrel_chg_dft_voltage;
  /**<barrel charger default voltage in mv */

  uint32 barrel_chg_dft_current;
  /**<barrel charger default current in mA */

  uint32 vbatt_threshold_load_fw_in_mv;
  /**< Load FW if vbatt is above this threshold */  

  BOOLEAN enable_boot_after_threshold_charging;
  /**< Enable boot directly to HLOS after threshold charging completes */
  
  BOOLEAN enable_charging_limit;
  /**< enable a charging limit for the battery, will only charge to certain target*/

  uint8 target_soc_charging_limit;
  /**< the target soc for the above feature. */

  uint8 delta_soc_charging_limit;
  /**< the recharge delta soc for the above feature. */

  BOOLEAN enable_battery_authentication;
  /**< enable/disable battery authentication. */

  BOOLEAN unauthenticated_battery_charging_enable;
  /**< enable/disable unauthenticated battery charging */

  BOOLEAN offmode_charging_in_chargerApp;
  /**< to do offmode charging loop in chargerApp or chargerDxe */

  uint32 offmode_charging_sleep_time;
   /**< configure offmode charging sleep time*/
...
typedef enum {
  CHARGERLIB_EVENT_DISP_IMAGE_NONE,
...
typedef enum
{
  CHARGERLIB_CHG_USBC_SINK_NONE,           /**< Not a valid sink port */
  CHARGERLIB_CHG_USBC_SINK_DFAULT,         /**< sink port for default USB power */
  CHARGERLIB_CHG_USBC_SINK_1P5A,           /**< sink port for 5V/1.5A USB power */
  CHARGERLIB_CHG_USBC_SINK_3P0A,           /**< Standard Downstream Port */
}CHARGERLIB_CHG_USBC_SINK_TYPE;
...
typedef struct
{
  ...
  UINT32  Voltage;
  UINT32  Current;
}CHARGERLIB_CHG_PD_CONTRACT;

typedef enum
{
  CHARGERLIB_CHG_POWER_MODE_NONE,
  CHARGERLIB_CHG_POWER_MODE_BC1P2,
  CHARGERLIB_CHG_POWER_MODE_TYPEC,
  CHARGERLIB_CHG_POWER_MODE_PD,
}CHARGERLIB_CHG_POWER_MODE_TYPE;

typedef struct
{
  CHARGERLIB_CHGR_PORT_TYPE       BC1p2;
  CHARGERLIB_CHG_USBC_SINK_TYPE   TypeC;
  CHARGERLIB_CHG_PD_CONTRACT      PD;
}CHARGERLIB_CHG_USB_CHARGER_DATA;
...
typedef struct
{
  ...
  UINT32  ICLMaxMa;
  UINT32  ICLfinalMa;
}CHARGERLIB_CHARGER_INPUT_STATUS;
...
}CHARGERLIB_PLATFORM_PWRON_REASON_TYPE;

typedef enum
{
  SHIP_MODE_NONE,
  SHIP_MODE_FG_ENTER,
  SHIP_MODE_PMIC_ENTER,
  SHIP_MODE_FG_EXIT = 4,
  SHIP_MODE_PMIC_EXIT = 8,
}CHARGERLIB_PLATFORM_SHIP_MODE_TYPE;

extern UINT8 ShipModeExit;
...
typedef union _CHARGERLIB_PLATFORM_CHGR_MAX_PWR_REQ_CFG_STS_TYPE
{
  UINT8 AsUint8;
  struct
  {
    UINT8 rsvd          : 4;    /* bit [ 0 : 3 ] reserved */
    UINT8 skip_smb_reset: 1;    /* bit [ 4 : 4 ] skip smb reset */
    UINT8 barrel_active : 1;    /* bit [ 5 : 5 ] barrel charger is attached and charging */
    UINT8 batt_dead     : 1;    /* bit [ 6 : 6 ] battery dead flag */
    UINT8 batt_missing  : 1;    /* bit [ 7 : 7 ] batery missing flag */
  };
}CHARGERLIB_PLATFORM_CHGR_MAX_PWR_REQ_CFG_STS_TYPE;

typedef enum
{
  CHGRLIB_BATT_MISSING_ACTION_BOOT_TO_HLOS,
  CHGRLIB_BATT_MISSING_ACTION_SHUTDOWN,
  CHGRLIB_BATT_MISSING_ACTION_INVALID,
}CHGRLIB_BATT_MISSING_ACTION_TYPE;

typedef enum
{
  CHGRLIB_GLINK_DOWN_ACTION_BOOT_TO_HLOS,
  CHGRLIB_GLINK_DOWN_ACTION_SHUTDOWN,
  CHGRLIB_GLINK_DOWN_ACTION_INVALID,
}CHGRLIB_GLINK_DOWN_ACTION_TYPE;

typedef enum
{
  CHGRLIB_FW_MISSING_ACTION_BOOT_TO_HLOS,
  CHGRLIB_FW_MISSING_ACTION_COLD_REBOOT,
  CHGRLIB_FW_MISSING_ACTION_INVALID,
}CHGRLIB_FW_MISSING_ACTION_TYPE;

// enable off mode sourcing only when charger is present (UEFI in off-mode charging loop)
#define CHARGERLIB_OFF_MODE_SRC_EN_CHG_PRESENCE     0x0
// disable off mode sourcing 
#define CHARGERLIB_OFF_MODE_SRC_DIS                 0x40
// enable off mode sourcing regardless of charger presence (HW autonomous)
#define CHARGERLIB_OFF_MODE_SRC_EN_ALWAYS           0x80
...
UINT64  ChargerLib_CalculateTime(void);

UINT64  ChargerLib_CalculateTimeMSec(void);
...
EFI_STATUS ChargerLib_GetShipMode(CHARGERLIB_PLATFORM_SHIP_MODE_TYPE* shipModeType);

EFI_STATUS ChargerLib_IsChargingSupported(BOOLEAN *pChargingSupported);

EFI_STATUS ChargerLib_GetDefectiveParts(BOOLEAN *pIsDefective);

EFI_STATUS ChargerLib_LoadChargerFW(void);

EFI_STATUS ChargerLib_LinkChargerFW(void);

EFI_STATUS ChargerLib_DisconnectChargerFW(void);

EFI_STATUS ChargerLib_GetVphPowerVoltage(UINT32 *VphVoltage);

EFI_STATUS ChargerLib_HandleRecovery(void);
...
EFI_STATUS ChargerLib_GetPlatformPwrOnReason(CHARGERLIB_PLATFORM_PWRON_REASON_TYPE *pwron_reason_type );

EFI_STATUS ChargerLib_EnableShipMode(CHARGERLIB_PLATFORM_SHIP_MODE_TYPE shipModeType);

EFI_STATUS ChargerLib_GetDisplayImageType(CHARGERLIB_EVENT_DISP_IMAGE_TYPE *pDispImage);

EFI_STATUS ChargerLib_CpdlogSetProperties(UINT64 LogCategories, UINT32 LogLevel);

EFI_STATUS ChargerLib_CpdlogGetBuffer(UINT32 LogMaxSize);

EFI_STATUS ChargerLib_CpdlogGetInitBuffer(void);

EFI_STATUS ChargerLib_CpdlogGetQbgDump(void);

EFI_STATUS ChargerLib_GetChargingStateForLed(BOOLEAN *pToggleLed);

EFI_STATUS ChargerLib_GetBatteryType(ChgBattType *pBatteryType);

EFI_STATUS ChargerLib_SetOffModeSrc(UINT8 offModeSrcCfg);

EFI_STATUS ChargerLib_GetPlatformMaxPwrReqCfgSts(CHARGERLIB_PLATFORM_CHGR_MAX_PWR_REQ_CFG_STS_TYPE *pSts, UINT8* pNegotiatedPwr);

EFI_STATUS ChargerLib_GetBarrelChargerStatus(BOOLEAN* connected, UINT16* charger_max_voltage_mv, UINT16* charger_max_current_ma);

EFI_STATUS ChargerLib_SetChargerInput(UINT8 pmic_index, UINT16 charger_max_voltage_mv, UINT16 charger_max_current_ma);

EFI_STATUS ChargerLib_GetControlAction(EFI_CHARGER_CONTROL_ACTION_TYPE* pControlAction);

EFI_STATUS ChargerLib_SetControlAction(EFI_CHARGER_CONTROL_ACTION_TYPE controlAction);

EFI_STATUS ChargerLib_SetMaxPwrReqSkipChargerReset(BOOLEAN skip);

EFI_STATUS ChargerLib_EnableAicl(UINT8 pmic_index, BOOLEAN enable);

EFI_STATUS ChargerLib_GetActivePortPmicIndex(UINT8* pmicIndex);

EFI_STATUS ChargerLib_GetBatteryEOCStatus(BOOLEAN *p_isEoC);

EFI_STATUS ChargerLib_SetHardReset(void);

EFI_STATUS ChargerLib_EnablePD(BOOLEAN enable);

EFI_STATUS ChargerLib_ChkChgFwLoadRequired_BattDead(CHARGERLIB_ERROR_ACTION_TYPE *pActionType, chargerlib_batt_status_info* pBattSts, BOOLEAN* b_keepChecking);

EFI_STATUS ChargerLib_ChkChgFwLoadRequired_BattMissing(CHARGERLIB_ERROR_ACTION_TYPE *pActionType);

BOOLEAN ChargerLib_GetDCINSupported(void);

EFI_STATUS ChargerLib_SetOS(CHARGERLIB_OS_TYPE OS);

EFI_STATUS ChargerLib_SetUsbPDFeatures(void);

EFI_STATUS ChargerLibTarget_SetPowerState(CHARGERLIB_POWER_STATE_TYPE pwr_state);

//this function will return if uefi should enter offmode charging that allow Cxpc to happen.
EFI_STATUS ChargerLib_IsLPMOffmode(BOOLEAN *b_lpm_offmode, BOOLEAN* b_offmode);

EFI_STATUS ChargerLib_GetNumSerialBattery(UINT8* num_serial_battery);

EFI_STATUS ChargerLib_GetBootSOCTh(UINT32* soc_th);

EFI_STATUS ChargerLib_GetTadShutdownDecision(BOOLEAN* tad_shutdown);
