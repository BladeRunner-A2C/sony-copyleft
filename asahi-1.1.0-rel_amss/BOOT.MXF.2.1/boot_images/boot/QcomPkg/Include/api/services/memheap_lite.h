...
===========================================================================*/
/*
Copyright (c) 2015-2018,2022 Qualcomm Technologies, Inc.  All Rights Reserved.
*/
/*===========================================================================
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Include/api/services/memheap_lite.h#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
...
#define MEM_MAX_ALIGNED_CHUNKS 64
...
/**< this is random number used in XOR(guard_bytes)
  ...
  unsigned int alignment; /*alignemnt set for aligned allocation*/
  uint64 aligned_mem_bits_array; /*each bit will represent the pre decided alignment at 
                           that bit position index from the aligned_mem buffer */
  void *aligned_mem_buffer;
  void *aligned_mem_buffer_end;
} mem_heap_type;
...
/**
  mem_init_heap_with_resrv_alignd_mem:
  Initializes the I/O heap object and sets up heap_mem_ptr for use with
  the I/O heap object in addition to that it can reserve memory(from the 
  same heap) for future memory alignemnent requests with fixed alignment 
  size if client provides the alignment and number_of_chunks that can 
  help reduce fragmentations in the heap caused by multiple alignment 
  requests.
  ...
  @param[in] alignment,       Number of bytes to be aligned with and a chunk size
  @param[in] number_of_chunks Number of chunks with the alignment
...
/**
  ...
  @param[in] heap_ptr   Pointer to the heap from which to allocate.
  @param[in] alignment  Number of bytes to be aligned with.
  @param[in] size       Number of bytes to allocate.

  @return
  Returns a pointer to the newly allocated, aligned block, or NULL if the block
  could not be allocated.

  @dependencies
  heap_ptr must not be NULL.
  size must not be 0.
  alignment must be power-of-2 greater than 8.
*/
void* mem_memalign
(
  mem_heap_type* heap_ptr,
