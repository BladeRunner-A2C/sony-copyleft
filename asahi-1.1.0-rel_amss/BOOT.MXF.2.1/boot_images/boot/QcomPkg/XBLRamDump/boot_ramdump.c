...
struct coldplug_guid ram_dump_partition_id = { 0x66C9B323, 0xF7FC, 0x48B6, { 0xBF, 0x96, 0x6F, 0x32, 0xE3, 0x35, 0xA4, 0x28 } };
...
static bl_error_boot_type boot_ramdump_media_if_write(void *buf_addr, uint64 offset, uint32 write_size)
{
  ...
  do
  {
    ...
    status = boot_config_context_set_ptr(config_context_handle, CONFIG_CONTEXT_PARTITION_ID, &ram_dump_partition_id);
    if(status != BL_ERR_NONE)
    {
...
static void boot_process_raw_ram_dump()
{
  ...
  bl_error_boot_type status = BL_ERR_NONE;
  boot_visual_indicator_if_type *visual_indicator_if=NULL;
  uint32 pmic_device_index = PMIC_B; //PMIC_B(PM8550)
  uint8 flash_led_enable = 1;
  ...
  {
    /* Enable Flash LED for Visual Indication of dump start */
    status = boot_config_context_get_ptr(config_context_handle, CONFIG_CONTEXT_VISUAL_INDICATOR_INTERFACE, (void **)&visual_indicator_if);
    if(status == BL_ERR_NONE)
    {
      if(!boot_check_ramdump_fail() && boot_is_ramdump_to_rddata_allowed(config_context_handle))
        visual_indicator_if->flash_led_module_control(pmic_device_index, flash_led_enable);
    }
...
static void boot_ramdump_to_raw_partition(void)
{
...
static void boot_ramdump_partition_size(struct coldplug_guid *guid)
{
  ...
  chdl = coldplug_open_gpt_partition(COLDPLUG_DEVICE_UFS, 1 /* is_embedded */, guid);
  if (NULL == chdl)
  {
...
boot_boolean boot_is_ramdump_to_rddata_allowed(boot_handle config_context_handle)
{
  ...
  if (debug_allow)
  {
    /* Get partition size */
    if(!partition_size)
      boot_ramdump_partition_size(&ram_dump_partition_id);
