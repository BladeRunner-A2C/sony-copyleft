...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 09/06/23   nnaram  Added CONFIG_CONTEXT_PARTITION_INTERFACE
 03/22/23   ng      Shifting chipinfo init before logger init.
 02/24/23   krd     For kuno, disabled the hfatlib
 02/15/23   ck      Renamed edk2 interfaces
 02/14/23   rama    fix wdog bite during delayed USB enumeration for flashless dumps
 01/13/23   tmt     CR-3307011 security fixes
 12/08/22   rama    Remove extern references to BootMemMap lib APIs
 10/06/22   rhy     Moved media_init after chipinfo Init. Registered MHI logger
 ...
 08/19/22   rhy     Added AOSS Reset status in config_context_global_values
...
#define BOOT_DTB_NODE "/sw/boot"
#define XBL_RAMDUMP_WDOG_FLAG "xbl_ramdump_wdog"
...
{
  /*boot_media      ramdump_media   ramdump_format      post_ramdump_action                 status */
...
boot_config_context_populate_table config_context_global_values[] =
{
  ...
  {CONFIG_CONTEXT_CACHE_INTERFACE,             PARAMETER_TYPE_PTR,   {},                     {&cache_if,                   0,                                     NULL},                                },
  {CONFIG_CONTEXT_MMU_INTERFACE,               PARAMETER_TYPE_PTR,   {},                     {&mmu_if,                     0,                                     NULL},                                },
  {CONFIG_CONTEXT_ARM_INTERFACE,               PARAMETER_TYPE_PTR,   {},                     {&arm_if,                     0,                                     NULL},                                },
  ...
  {CONFIG_CONTEXT_AOSS_RESET_STATUS_REGISTER,  PARAMETER_TYPE_VALUE, {0x0, NULL},            {}                                                                                                         },
  {CONFIG_CONTEXT_BAM_INTERFACE,               PARAMETER_TYPE_PTR,   {},                     {&bam_if,                     0,                                     NULL},                                },
  {CONFIG_CONTEXT_PARTITION_INTERFACE,         PARAMETER_TYPE_PTR,   {},                     {&partition_if,               0,                                     NULL},                                }
};

void xbl_ramdump_enable_non_secure_watchdog(void)
{
  /* check if JATG connections status and enable wdog if not connected */
  if(!(HWIO_INF(AOSS_CC_CDBGPWRUPREQ, STATUS)))
  {
    /*set wdog time to 30 secs and enable watchdog */
    WatchDog_Timer2_SetBite_MSec(30000);
    WatchDog_Timer2_Enable_NSec();
    WatchDog_Timer2_Pet();
  }
}

void xbl_ramdump_disable_non_secure_watchdog(void)
{
  WatchDog_Timer2_Disable();
}

static bl_error_boot_type xbl_ramdump_watchdog_disable_check(void)
{
  bl_error_boot_type status = BL_ERR_NONE;
  fdt_node_handle node;
  int ret_val = 0;
  boot_boolean xbl_rd_wdog_enable = TRUE;

  do
  {
    ret_val = fdt_get_node_handle(&node, NULL, BOOT_DTB_NODE);
    if(ret_val != 0)
    {
      /* todo fix error code */
      status = GET_ERROR_CODE(BL_ERROR_GROUP_XBL_CONFIG, 0);
      break;
    }

    ret_val = fdt_get_boolean_prop(&node, XBL_RAMDUMP_WDOG_FLAG, &xbl_rd_wdog_enable);
    if(ret_val != 0 || (xbl_rd_wdog_enable > ENABLE || xbl_rd_wdog_enable < DISABLE))
    {
      break;
    }
    
    if(xbl_rd_wdog_enable == ENABLE)
    {
      xbl_ramdump_enable_non_secure_watchdog();
    }
    if(xbl_rd_wdog_enable == DISABLE)
    {
      xbl_ramdump_disable_non_secure_watchdog();
    }
  }while(FALSE);
...
#define BATT_DISPLAY_THRESHOLD 3700
  ...
  /* Here battery voltage value >= required threshold value */
  else if(pmic_ret == PM_ERR_FLAG_FEATURE_NOT_SUPPORTED)
  {
    boot_log_message("Couldn't read battery voltage, setting threshlod TRUE");
    return_status = TRUE;
  }
...
bl_error_boot_type system_init(boot_handle *config_context_handle)
{
  ...
  boot_registered_logger_if_type * logger_mhi_if =  NULL;
  boot_mmu_if_type *mmu_if =                        NULL;
  bl_error_boot_type status =                       BL_ERR_NONE;
  boot_platforminfo_if_type *platforminfo_if =      NULL;
  const void * boot_device_tree_blob_address =      NULL;
  ...
  do
  {
    ...
    status = xbl_ramdump_watchdog_disable_check();
    if(status != BL_ERR_NONE)
    {
      break;
    }
    ...
    if (chipinfo_if == NULL)
    {
      status = GET_ERROR_CODE(BL_ERROR_GROUP_CONFIG_CONTEXT, BL_ERR_INTERFACE_FAILURE);
      break;
    }
    ...
    /* register mhi it with the main logger */
    if (boot_device == PCIE_FLASHLESS)
    {
      status = boot_logger_mhi_get_interface(&logger_mhi_if);
      if (status != BL_ERR_NONE)
      {
        break;
      }
    
      status = logger_if->register_logger(logger_mhi_if);
      if (status != BL_ERR_NONE)
      {
        break;
      }
    ...
    boot_log_message("RAMDUMP BUILD @ " __TIME__ " on " __DATE__);
...
bl_error_boot_type XBLRamDumpInit (boot_handle config_context_handle)
{
  ...
  do
  {
    ...
    status = boot_config_context_get_ptr(config_context_handle, CONFIG_CONTEXT_CACHE_INTERFACE, (void**)&cache_if);
    if ((status != BL_ERR_NONE) || (cache_if == NULL))
    {
    ...
    cache_if->WriteBackInvalidateDataCacheRange((void *)Image$$RAMDUMP_DATA_ZI$$Base, (uintnt)Image$$RAMDUMP_DATA_ZI$$Length);
...
bl_error_boot_type xbl_ramdump_process_post_dump (bl_error_boot_type closed_status, post_ramdump_action_type rd_action)
{
  ...
  status = boot_config_context_get_ptr(CONFIG_CONTEXT_CRT_HANDLE, CONFIG_CONTEXT_CACHE_INTERFACE, (void**)&cache_if);
  if (status != BL_ERR_NONE)
  {
    return status;
  }
...
bl_error_boot_type ReenterRamdump(boot_handle config_context_handle)
{
  ...
  do
  {
    ...
    cache_if->WriteBackInvalidateDataCacheRange((void *)xbl_get_external_cached_heap_addr(), (uintnt)xbl_get_external_cached_heap_size());
    dal_if->DeInitMod();
    ...
    if (mmu_if->ArmSetMemoryAttributes((uintnt)external_uncached_heap_addr,
    ...
    cache_if->InvalidateDataCacheRange((void *)Image$$RAMDUMP_DATA_ZI$$Base, (uintnt)Image$$RAMDUMP_DATA_ZI$$Length);
    el1_entry((void*)NULL);
  } while (FALSE);
  return status;
}
...
/* SONY_END(Ramdumper support) */
      ...
      if(xbl_ramdump_config_table[current_dump_entry].ramdump_media == PCIE_FLASHLESS ||
         xbl_ramdump_config_table[current_dump_entry].ramdump_media == USB_FLASHLESS)
      {
        xbl_ramdump_disable_non_secure_watchdog();
      }
      else
      {
        /* check if watchdog needs to be disabled based on Device Tree configuration */
        status = xbl_ramdump_watchdog_disable_check();
        if(status != BL_ERR_NONE)
        {
          boot_log_message("wdog disable check failed");
          goto skip_to_next;
        }
      }
      ...
      {
        xbl_ramdump_config_table[current_dump_entry].status = BL_ERR_RAMDUMP_COLLECTION_NOT_SUPPORTED;
        if(status == BL_ERR_NO_SDCARD)
        {
          boot_log_message("SD Card not present");
        }
        else
        {
