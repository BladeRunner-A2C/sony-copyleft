...
//============================================================================
/**
  Copyright (c) 2016-2019, 2020-2023 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 07/27/20   ps      Added Auto Launch Testapp support
...
/*
 * Handler to set variables for run any testapp on repeat for no. of iteration.
 */

EFI_STATUS
AutoLaunch(CHAR8* Arg)
{
  EFI_STATUS    Status = EFI_NOT_FOUND;
  UINT32        AutoLaunchCnt = 0;
  UINTN         VarSize;
  CHAR8 AutoLaunchName[64] = {0}, AutoLaunchCntString[64] = {0};

  do
  {
    AsciiPrint("Enter Partition Name & TestApp Name\neg: toolsfv fv3:\\memtest\r\n");
    FlushKeys();
    GetAutoLaunchUserInput(AutoLaunchName);
    if(AutoLaunchName == NULL)
      break;
    FlushKeys();
    AsciiPrint("Enter No of iteration [Decimal]\r\n",AutoLaunchName);
    GetAutoLaunchUserInput(AutoLaunchCntString);

    if(AutoLaunchCntString == NULL)
      break;
    AutoLaunchCnt = AsciiStrDecimalToUintn(AutoLaunchCntString);
    ...
    if(Status != EFI_SUCCESS)
      break;
    VarSize = sizeof(AutoLaunchName);
    Status = gRT->SetVariable (L"AutoLaunch", &gQcomTokenSpaceGuid,
                               EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                               VarSize, &AutoLaunchName);
  }while(0);
...
/*
 * Handler to reset variables for run any testapp on repeat for 5000 iteration.
 */

EFI_STATUS
AutoLaunchDisable(CHAR8* Arg)
{
  ...
  if(Status == EFI_SUCCESS)
    AsciiPrint("Reboot/PowerCycle device gracefully for setting to take effect. %d\r\n", AutoLaunchCnt);
  else
    AsciiPrint("ERROR: Unable to change/store setting\r\n");
  return Status;
}
...
MountFvCommand (UINTN  Argc, CHAR8  **Argv)
{
  ...
  switch (Argc)
  {
    ...
    case 2:
        ...
        {
            AsciiStrnCpyS(GuidStr, Argv[2], GUID_STRING_LENGTH+1);
            Ch = &GuidStr[0];
            while (*Ch != '\0') {
                if (*Ch == '-')
                    *Ch = ' ';
...
ToggleUnauthenticatedBatteryCharging(VOID)
{
    ...
    if (QueryToggleFlag("Toggle Battery Authentication", &ChargingEnable, NON_INVERTED_MEANING) != EFI_SUCCESS)
        return EFI_SUCCESS;
...
/*
 * Toggle MinidumpTALoad, variable read by MinidumpTADxe
 */
EFI_STATUS
ToggleMinidumpTALoad(VOID)
{
  AsciiPrint("Entered MD_TA_LOAD\r\n");
  EFI_STATUS Status = EFI_NOT_FOUND;
  UINT8 MinidumpTALoad = 0;
  UINTN VarSize = sizeof (MinidumpTALoad);

  Status = gRT->GetVariable (L"MinidumpTALoad", &gQcomTokenSpaceGuid,NULL,
             &VarSize, &MinidumpTALoad);

  if (Status == EFI_NOT_FOUND)
    AsciiPrint("ERROR: Variable does  not exist. Added to token space. \r\n");
  else if (EFI_ERROR(Status)) {
    AsciiPrint("ERROR: Unable to locate MinidumpTALoad variable. \r\n");
    return Status;
  }
  
  MinidumpTALoad = (MinidumpTALoad ^ 0x1) & 0x1;

  Status = gRT->SetVariable (L"MinidumpTALoad", &gQcomTokenSpaceGuid,
              EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
              VarSize, &MinidumpTALoad);
			  
  if(EFI_ERROR(Status))
    AsciiPrint("ERROR: Unable to create MinidumpTALoad variable. Status: 0x%x \r\n", Status);
  else
    AsciiPrint("MinidumpTALoad Variable state: %d. Reboot device to take effect. \r\n", MinidumpTALoad);	
  return Status;
}
...
#define DEFAULT_USBDPBONDEDPRIMARY            (0)
#define DEFAULT_USBDPBONDEDSECONDARY          (0)
#define DEFAULT_USBDPBONDEDTERTIARY           (0)
...
/*
 * Handler for EnableUsbDPBondedPrimary Command for Secondary Port only.
 */
EFI_STATUS
EnableUsbDPBondedPrimary (CHAR8* Arg)
{
  EFI_STATUS    Status;
  UINT8         Flag = DEFAULT_USBDPBONDEDPRIMARY;
  UINTN         VarSize;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"UsbDPBondedPrimary", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QueryToggleFlag ("Skip GCC Reset for DP Bonded Mode for Primary Port", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"UsbDPBondedPrimary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler for EnableUsbDPBondedSecondary Command for Secondary Port only.
 */
EFI_STATUS
EnableUsbDPBondedSecondary (CHAR8* Arg)
{
  EFI_STATUS    Status;
  UINT8         Flag = DEFAULT_USBDPBONDEDSECONDARY;
  UINTN         VarSize;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"UsbDPBondedSecondary", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QueryToggleFlag ("Skip GCC Reset for DP Bonded Mode for Secondary Port", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"UsbDPBondedSecondary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler for EnableUsbDPBondedTertiary Command for Tertiary Port only.
 */
EFI_STATUS
EnableUsbDPBondedTertiary (CHAR8* Arg)
{
  EFI_STATUS    Status;
  UINT8         Flag = DEFAULT_USBDPBONDEDTERTIARY;
  UINTN         VarSize;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"UsbDPBondedTertiary", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QueryToggleFlag ("Skip GCC Reset for DP Bonded Mode for Tertiary Port", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"UsbDPBondedTertiary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler to Set UsbMultiPortDisableBitfield Uefi variable to modify
 * the configuration of the USB MP ports enablement.
 */
...
ResetUsbUefiVariable()
{
  ...
  Flag = DEFAULT_USBDPBONDEDPRIMARY;
  Status = gRT->SetVariable (L"UsbDPBondedPrimary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USBDPBONDEDSECONDARY;
  Status = gRT->SetVariable (L"UsbDPBondedSecondary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USBDPBONDEDTERTIARY;
  Status = gRT->SetVariable (L"UsbDPBondedTertiary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
