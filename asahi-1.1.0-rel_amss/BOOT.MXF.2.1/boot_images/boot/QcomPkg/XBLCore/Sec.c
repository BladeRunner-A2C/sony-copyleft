...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 04/10/22   rbv     Initalising DT early on to parse Memory map and config entries
 10/11/22   sree    Introduce EarlyInitCoreMask for presil targets to enable multicore in RUMI phase
 08/12/22   sree    Convert Imageoffsets into PCD's and remove Imageoffsets.h
...
#define MB_TO_BYTES        0x100000
#define SMP_MAX_CPU        12
...
BOOLEAN EFIAPI
AddUEFIMinidumpRegion (
  CHAR8  *RegionName,
  UINTN  *RegionBaseAddr,
  UINTN  RegionSize
  );
...
{
  EFI_STATUS  Status;
  UINT64      SerialLogBuffer, SerialBufferOffset, SerialBufferSize;

  SerialBufferOffset = FixedPcdGet64 (PcdSerialBufferOffset);
  SerialBufferSize   = FixedPcdGet64 (PcdSerialBufferSize);

  SerialLogBuffer = FixedPcdGet64 (PcdEmbeddedFdBaseAddress) + SerialBufferOffset;

  SerialPortInitialize ();
  Status = SerialBufferInit ((UINT8 *)SerialLogBuffer, SerialBufferSize);
  ...
  )
{
  ...
  UefiInfoBlkPtr->UartLogBufferLen = SerialBufferSize;
...
/** Create a structure which has mapping of Functional cores with
  Logical CPU ID and MPIDR values
*/

VOID
GetCoreClusterMapping (
  VOID
  )
{
  DALResult        DalStatus = DAL_ERROR;
  EFI_STATUS       Status = EFI_NOT_READY;
  UINT32 Cluster = 0, MaskPerCluster = 0;
  UINT32 Cpu = 0, LogicalCoreId = 0, FuntionalCoreCnt = 0, FunctionalCoreCountToOverride = 0;

  Status = GetConfigValue("IsFunctionalCoreCountToOverrideFlag", &FunctionalCoreCountToOverride);

  if ((Status == EFI_SUCCESS) && (FunctionalCoreCountToOverride == 1)) 
  {
    /* If in presil time as fuses are not blown for finding non-disabled CPU exactly based on Hardware changes, so using this configuration for RUMI phase */
    SerialPrint(" WARN: Functional cores hardcoded for target \n");
    for (Cpu = 0; Cpu < CoreInfo.MaxCoreCount; Cpu++)
    {
      CoreInfo.CoreClusterInfo[Cpu].Functional = 1;

      CoreInfo.CoreClusterInfo[Cpu].LogicalCpuId = LogicalCoreId;
      LogicalCoreId++;
    }
  } 
  else
  {
    ...
    while ((DalStatus != DAL_ERROR_OUT_OF_RANGE_PARAMETER) && (&MaskPerCluster != NULL)) 
    {
      DalStatus = ChipInfo_GetDisabledCPUs(Cluster, &MaskPerCluster); /* Returns Mask setting disabled CPU's to 1 */
      if ((DalStatus == DAL_SUCCESS))
      {
        MaskPerCluster = ~MaskPerCluster;        /* Convert mask to make disabled CPU to 0 and functional CPU to 1 */
        for (Cpu = 0; Cpu < CoreInfo.CoresPerCluster; Cpu++) 
        {
          UINT32 Index = Cpu  + Cluster * CoreInfo.CoresPerCluster;
          CoreInfo.CoreClusterInfo[Index].Functional = (1U & (MaskPerCluster >> Cpu));

          if (CoreInfo.CoreClusterInfo[Index].Functional) 
            FuntionalCoreCnt++;

          CoreInfo.CoreClusterInfo[Index].LogicalCpuId = LogicalCoreId;
          LogicalCoreId++;
        }
        Cluster++;
      }
    }
    if (CoreInfo.EarlyInitCoreCnt > FuntionalCoreCnt)
       CoreInfo.EarlyInitCoreCnt  = FuntionalCoreCnt;
  }
  return;
}
  ...
  )
{
  EFI_STATUS       Status = EFI_NOT_READY;
  UefiInfoBlkType  *UefiInfoBlkPtr = NULL;
  UINT64           UefiFdBase = 0, SecHeapMemBase = 0, SecHeapMemOffset = 0, SecHeapMemSize = 0, UefiInfoBlkOffset = 0, UefiInfoBlkSize = 0;
  UINT64           UefiEarlyInitSize = 0, UefiFdSize = 0, CPUvectorsSize = 0, MMUpagetablesSize = 0, SerialBufferSize = 0;
  ...
  CoreClusterMapping  CoreClusterMapInfo[SMP_MAX_CPU];
  CoreInfo.CoreClusterInfo = CoreClusterMapInfo;
  CoreInfo.Revision        = CORE_CLUSTER_MAP_INIT_REVISION;

  UefiEarlyInitSize = FixedPcdGet64 (PcdUefiEarlyInitSize);
  UefiFdSize        = FixedPcdGet64 (PcdUefiFdSize);
  CPUvectorsSize    = FixedPcdGet64 (PcdCPUvectorsSize);
  UefiInfoBlkOffset = FixedPcdGet64 (PcdUefiInfoBlkOffset);
  UefiInfoBlkSize   = FixedPcdGet64 (PcdUefiInfoBlkSize);
  MMUpagetablesSize = FixedPcdGet64 (PcdMMUpagetablesSize);
  SerialBufferSize  = FixedPcdGet64 (PcdSerialBufferSize);
  SecHeapMemOffset  = FixedPcdGet64 (PcdSecHeapMemOffset);
  SecHeapMemSize    = FixedPcdGet64 (PcdSecHeapMemSize);

  gReset         = GetTimeInNanoSecond (GetPerformanceCounter ());
  UefiFdBase     = FixedPcdGet64 (PcdEmbeddedFdBaseAddress);
  SecHeapMemBase = UefiFdBase + SecHeapMemOffset;
  ...
  InitHobList (
               SecHeapMemBase,
               SecHeapMemSize - (SecHeapAslrVal*ASLR_HEAP_ALIGN),
               UefiInfoBlkPtr
               );
  ...
  UefiInfoBlkPtr = InitInfoBlock (UefiFdBase + UefiInfoBlkOffset, UefiInfoBlkSize);
  ...
  ASSERT (UefiEarlyInitSize >= (UefiFdSize + CPUvectorsSize + UefiInfoBlkSize + MMUpagetablesSize + SerialBufferSize + SecHeapMemSize));
  Status = EarlyCacheInit (UefiFdBase, UefiEarlyInitSize);
  if (EFI_ERROR (Status)) {
  ...
  Status = GetMemRegionInfoByName ("DXE_Heap", &DxeHeapMemInfo);
  if (Status != EFI_SUCCESS) {
  ...
  CoreInfo.MaxCoreCount = FixedPcdGet64(PcdMaxCoreCnt);
  if ((CoreInfo.MaxCoreCount == 0) || (CoreInfo.MaxCoreCount > SMP_MAX_CPU)) {
    DEBUG ((EFI_D_ERROR, "Error while fetching MaxCoreCnt\n"));
    CpuDeadLoop ();
  }

  Status = GetConfigValue ("EarlyInitCoreCnt", &(CoreInfo.EarlyInitCoreCnt));
  if ((Status != EFI_SUCCESS) || (CoreInfo.EarlyInitCoreCnt > CoreInfo.MaxCoreCount)) {
    CoreInfo.EarlyInitCoreCnt = 0x2;
  }

  CoreInfo.CoresPerCluster = FixedPcdGet64(PcdCorePerCluster);
  if (( CoreInfo.CoresPerCluster == 0 )) {
    DEBUG ((EFI_D_ERROR, "Error while fetching CoresPerCluster\n"));
    CpuDeadLoop ();
  }

  DalStatus = ChipInfo_Init();
  if (DalStatus != DAL_SUCCESS) 
     CoreInfo.EarlyInitCoreCnt = 0x1;
  else 
     GetCoreClusterMapping();
   
  InitSplitBoot (&UefiBootContinue, &CoreInfo);  // Doesn't return if succeeds
  ...
  )
{
  ...
  */
  Status = StartFvDecompression (
                                 (void *)UefiFdBase,
                                 UefiFdSize,
                                 &gCompressedDxeCoreContainerFvGuid,
                                 SYNCHRONOUS,
                                 NOMOUNT
                                 );
  if (Status != EFI_SUCCESS) {
  ...
  if ((Status != EFI_SUCCESS) || (EnableMultiCoreFvDecompression == 0)) {
    ...
    if (!SEC_RETAIL) {
      if (CoreInfo.EarlyInitCoreCnt < 0x2) {
        DEBUG ((EFI_D_ERROR, "AuxCores are not enabled or non operational"));
        LaunchMode = SYNCHRONOUS;
      }
    }
  }
