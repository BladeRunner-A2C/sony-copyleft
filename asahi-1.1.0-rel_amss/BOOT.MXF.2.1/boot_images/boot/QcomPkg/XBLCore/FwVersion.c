...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 07/28/21   cm       Minor changes to print statements
 06/12/20   cm       Added support to check boot_from_flashless
 09/21/18   kpa      Added IsSecFuseLibNull and updated log message
 04/24/18   kpa      Add sd boot support
  ...
  )
{
  ...
  switch (CHAR_TO_INT (DateString[0], DateString[1], DateString[2], DateString[3])) {
    case JAN:
      ...
      FinalVersionString[i++] = '1';
      break;
    case FEB:
      FinalVersionString[i++] = '0';
      FinalVersionString[i++] = '2';
      break;
    case MAR:
      FinalVersionString[i++] = '0';
      FinalVersionString[i++] = '3';
      break;
    case APR:
      FinalVersionString[i++] = '0';
      FinalVersionString[i++] = '4';
      break;
    case MAY:
      FinalVersionString[i++] = '0';
      FinalVersionString[i++] = '5';
      break;
    case JUN:
      FinalVersionString[i++] = '0';
      FinalVersionString[i++] = '6';
      break;
    case JUL:
      FinalVersionString[i++] = '0';
      FinalVersionString[i++] = '7';
      break;
    case AUG:
      FinalVersionString[i++] = '0';
      FinalVersionString[i++] = '8';
      break;
    case SEP:
      FinalVersionString[i++] = '0';
      FinalVersionString[i++] = '9';
      break;
    case OCT:
      FinalVersionString[i++] = '1';
      FinalVersionString[i++] = '0';
  ...
  if (boot_from_ufs ()) {
    ...
    // TODO:HAMOA
    // do not assert for now during ramboot when boot device is not found.
    // ASSERT (FALSE);
    // CpuDeadLoop();
#else
    ASSERT (FALSE);
  ...
  if (RETAIL) {
    ...
    if (IsSecFuseLibNull () == TRUE) {
      DEBUG ((EFI_D_ERROR, "Retail        : FALSE [WARNING: SecFuseLibNULL used]\n"));
    } else {
