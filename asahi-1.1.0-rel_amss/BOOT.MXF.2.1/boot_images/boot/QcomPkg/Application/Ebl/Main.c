/** @file
  ...
  Copyright (c) 2013, 2016, 2022 Qualcomm Technologies, Inc. All rights reserved.<BR>
  ...
  )
{
  ...
  *Argc = 0;
  ...
  //Remove any space if it prepends before the start of args
  while(*Char == ' '){
    Char++;
  }

  //Assign start char to Argv[0]
  if (*Char != ' '){
    Argv[Arg++] = Char;
  }
  ...
  while(*Char != ' '){
    ...
    if (*Char == CMD_SEPARATOR) {
      *Argc = Arg;
      // Replace the previous command delimiter with null and return pointer to current command line
      *Char = '\0';
      return ++Char;
    }
    ...
    if(*Char == '\0'){
      *Argc = Arg;
      return NULL; //if only one arg is passed, retrun from the function
    }
  }
  ...
  *Char = '\0';
  ...
  // If cmd starts with 'st' or 'start', then populate only 3 Args ie., Argv[0], Argv[1] and Argv[2]
  if((AsciiStrCmp (Argv[0], "st") == 0) || (AsciiStrCmp (Argv[0], "start") == 0)){
    Populate3Args = TRUE;
  }

  //Iterate through rest of the CmdLine String
  for(Arg = 1; *Char != '\0'; Char++){
    ...
    if(Populate3Args){
      ...
      //if Argv[2] is populated, then stop parsing
      if(Arg == MAX_POPULATE_ARGS){
        break;
      }
  ...
  if(Populate3Args && InQuote){
    //Traverse until End of String
    while (*Char != '\0'){
      Char++;
    }
     //Strip off the last double quotes
    if((*(Char-1) == '"') && (*(Char-2) != '\\')){
      *(Char-1) = '\0';
      InQuote = FALSE;
    }
