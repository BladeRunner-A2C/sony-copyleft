/** @file
  ...
  Copyright (c) 2011,2016, 2022, 2023 Qualcomm Technologies, Inc. All rights reserved.
  ...
  if (!EFI_ERROR (Status)) {
    if (Argc >= 3) {
      ...
      if(ImageInfo->LoadOptions == NULL){
        AsciiPrint("AllocatePool out of resource.\n");
  ...
  )
{
  ...
  if ((DestFileName[FileNameLen-1] == '\\') || (DestFileName[FileNameLen-1] == ':')) {
    ...
    DestFilePathLength = FileNameLen + AsciiStrLen (SrcFileName) + 1;
    DestFileName = (CHAR8*)AllocatePool (DestFilePathLength);
    if (DestFileName == NULL){
      AsciiPrint("AllocatePool out of resource.\n");
      return EFI_OUT_OF_RESOURCES;
    }
    AsciiStrCpyS (DestFileName, DestFilePathLength,  Argv[2]);
    AsciiStrCatS (DestFileName, DestFilePathLength, SrcFileName);
  }
