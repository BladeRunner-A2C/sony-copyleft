...
/*=============================================================================
 ...
 --------     ---     -----------------------------------------------------------
 09/05/19     wek     Add NVMe LPM
 05/20/19     mr      Added support to move display functionality from Chargerlib to ChargerApp
 08/22/17     ivy     Fixed display not turning back on with key press
...
#ifdef FEATURE_XBOOT
#define CHGAPP_SEVEN_SEC_WAIT_IN_US                60000000 /*tick is 100ns; 100 * 60000000 = 6sec*/
#else /* !FEATURE_XBOOT */
...
extern BOOLEAN                    ForcePowerTest;
extern BOOLEAN                    EnableImageDisplay;
...
QcomChargerAppEvent_KeyPressHandler( QCOMCHARGERAPP_EVENT_ANIMATION_TYPE AnimationType )
{
  ...
  if(NULL == EventAnimImg)
  {
    if(QCOMCHARGERAPP_EVENT_ANIMATION__LOW_BATTERY_NO_ANIM == AnimationType)
    {
      CHARGERAPP_DEBUG((EFI_D_WARN, "QcomChargerApp:: %a CHGAPP_DISP_LOW_BATTERY with clearnscreen\r\n", __FUNCTION__));
          QcomChargerAppDisplay_DispBattSymbol( EFI_QCOM_CHARGER_DISP_IMAGE_LOWBATTERY, TRUE);
    }
    else if (QCOMCHARGERAPP_EVENT_ANIMATION__THERMAL_NO_ANIM == AnimationType)
    {
      CHARGERAPP_DEBUG((EFI_D_WARN, "QcomChargerApp:: %a CHGAPP_DISP_TSENS HIGH with clearnscreen\r\n", __FUNCTION__));
          QcomChargerAppDisplay_DispBattSymbol( EFI_QCOM_CHARGER_DISP_IMAGE_TSENS_THERMAL_SYMBOL, TRUE);
    }
    else if (QCOMCHARGERAPP_EVENT_ANIMATION__DEBUG_BOARD_NO_ANIM == AnimationType)
    {
      CHARGERAPP_DEBUG((EFI_D_WARN, "QcomChargerApp:: %a CHGAPP_DISP_DEBUG_BOARD with clearnscreen\r\n", __FUNCTION__));
          QcomChargerAppDisplay_DispBattSymbol( EFI_QCOM_CHARGER_DISP_IMAGE_DEBUG_LOW_SYMBOL, TRUE);
    }
    else
    {
      CHARGERAPP_DEBUG((EFI_D_WARN, "QcomChargerApp:: %a CHGAPP_DISP_LOW_BATTERY_CHARGING with clearnscreen\r\n", __FUNCTION__));
          QcomChargerAppDisplay_DispBattSymbol( EFI_QCOM_CHARGER_DISP_IMAGE_LOWBATTERYCHARGING, TRUE);
    }
  ...
  QcomChargerAppEvent_DisplayPanelOn();
...
VOID QcomChargerAppEvent_EnterLPM( VOID )
{
  if ((FALSE == SystemInLPM) )
  {
    ...
    QcomChargerAppEvent_ReportStatusCode(gEfiUfsLU0Guid, LPM_ENTRY);

    QcomChargerAppEvent_ReportStatusCode(gEfiNvme0Guid, LPM_ENTRY);

    //QcomChargerAppEvent_ReportStatusCode(gEfiWlanGuid, LPM_ENTRY);
    QcomChargerAppEvent_ReportStatusCode(gQcomDDRLpmGuid, LPM_ENTRY);

    //QcomChargerAppEvent_ReportStatusCode(gEfiWwanGuid, LPM_ENTRY);

    SystemInLPM = TRUE;
  }
  else
  {
    CHARGERAPP_DEBUG((EFI_D_INFO, "QcomChargerApp:: %a Skipping Enter LPM \r\n", __FUNCTION__));
  }
}
...
VOID QcomChargerAppEvent_ExitLPM( VOID )
{
  if(TRUE == SystemInLPM)
  {
    ...
    QcomChargerAppEvent_ReportStatusCode(gQcomDDRLpmGuid, LPM_EXIT);

    //QcomChargerAppEvent_ReportStatusCode(gEfiWlanGuid, LPM_EXIT);

    //QcomChargerAppEvent_ReportStatusCode(gEfiWwanGuid, LPM_EXIT);

    SystemInLPM = FALSE;
  }
  else
  {
    CHARGERAPP_DEBUG((EFI_D_INFO, "QcomChargerApp:: %a Skipping Exit LPM \r\n", __FUNCTION__));
  }
}


//This function gets called before exiting the charging loop to boot to HLOS.
VOID QcomChargerAppEvent_DisplayPanelOn( VOID )
{

    CHARGERAPP_DEBUG((EFI_D_ERROR, "QcomChargerApp:: %a turning ON display panel \r\n", __FUNCTION__));
    /* Turn on the display */
...
EFI_STATUS QcomChargerAppEvent_DispBattSymbol(EFI_QCOM_CHARGER_DISP_IMAGE_TYPE DispImageType)
{
  ...
  if(EFI_SUCCESS != Status)
  {
    DEBUG((EFI_D_ERROR, "QcomChargerApp:: %a Error getting platform type = %r \r\n", __FUNCTION__, Status));
    return EFI_DEVICE_ERROR;
  }

  if ((!ForcePowerTest) && EnableImageDisplay)
  {
    Status = QcomChargerAppDisplay_DispBattSymbol(DispImageType, TRUE);
...
EFI_STATUS QcomChargerAppEvent_HandleDispBattSymbol(VOID)
{
  EFI_STATUS Status = EFI_SUCCESS;
  EFI_QCOM_CHARGER_DISP_IMAGE_TYPE DispImageType = EFI_QCOM_CHARGER_DISP_IMAGE_NONE;
  ...
  if(!pQcomChargerProtocol)
  {
    ...
    {
      CHARGERAPP_DEBUG((EFI_D_ERROR, "QcomChargerApp:: %a  Locate QcomChargerProtocol Protocol failed = %r \r\n", __FUNCTION__, Status));
      return Status;
    }
  }

  Status = pQcomChargerProtocol->GetDisplayImageType(&DispImageType);
  if ((EFI_SUCCESS == Status) && (DispImageType != EFI_QCOM_CHARGER_DISP_IMAGE_NONE))
  {
    Status = QcomChargerAppEvent_DispBattSymbol(DispImageType);
  }
