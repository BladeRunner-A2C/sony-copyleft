...
BOOLEAN                    EnableImageDisplay = TRUE;
...
)
{
  ...
  {
    CHARGERAPP_FILE_UART_DEBUG(( EFI_D_ERROR, "QcomChargerApp:: %a Charging is not supported. Exiting app. = %r \r\n", __FUNCTION__, Status));
    return EFI_SUCCESS;
  }
  ...
  if(FALSE == bChargingSupported && EFI_PLATFORMINFO_TYPE_CLS == PlatformType)
  {
  ...
  Status = QcomChargerAppEvent_HandleDispBattSymbol();
  if(EFI_SUCCESS != Status)
  {
    CHARGERAPP_DEBUG((EFI_D_ERROR, "QcomChargerApp:: %a  HandleDispBattSymbol failed = %r \r\n", __FUNCTION__, Status));
  }
  ...
  if((EFI_QCOM_CHARGER_ACTION_DEBUG_BOARD_GOOD_TO_BOOT != ChargingAction ) && (EFI_QCOM_CHARGER_ACTION_GOOD_TO_BOOT != ChargingAction ))
  {
    ...
    /* This action will be returned only on CLS platform */
    ...
    {
#define QCOM_CHARGER_IDLE_WAIT_DURATION 6000 /* 6 sec in milliseconds */
      QcomChargerAppEvent_DispBattSymbol(EFI_QCOM_CHARGER_DISP_IMAGE_NOCHARGER);
      WaitForTimeout (QCOM_CHARGER_IDLE_WAIT_DURATION, NO_TIMEOUT_WAIT_FOR_KEY, NULL);
      /* Required panel turning off before shutdown from Display Team */
    ...
    //todo have connected charger FW but failed treat it same as recovery
      ...
      case EFI_QCOM_CHARGER_ACTION_RECOVERY:
        ...
        /* Starts required timer to display LowBattery and LowBatteryCharging images */
        Status |= QcomChargerApp_Initialize(ChargingAction);
        if(EFI_SUCCESS == Status)
        {
          /* if initialization success then start charging upto target SoC*/
...
#ifdef FEATURE_XBOOT
    else if (ChargingAction == EFI_QCOM_CHARGER_ACTION_RECOVERY)
    {
      QcomChargerAppEvent_DispBattSymbol(EFI_QCOM_CHARGER_DISP_IMAGE_LOWBATTERYCHARGING);
    }
#endif /* FEATURE_XBOOT */
    else
    {
...
EFI_STATUS QcomChargerApp_MonitorCharging( VOID )
{
  ...
  DEBUG((EFI_D_ERROR, "QcomChargerApp::%a TimeStamp,StateOfCharge,Voltage,ChargeCurrent,Temp \r\n",__FUNCTION__));
  ...
  do
  {
...
#ifdef FEATURE_XBOOT
    /* Do not display by shutdown from charging */
    if (ChargingAction != EFI_QCOM_CHARGER_ACTION_SHUTDOWN)
    {
#endif /* FEATURE_XBOOT */
    /*Get Image to be displayed */
    ...
    switch(ChargingAction)
    {
      ...
      default:
           ...
           if(EFI_ERROR (Status))
           {
             DEBUG((EFI_D_ERROR, "QcomChargerApp:: %a MazeStart() failed Status = %r \r\n", __FUNCTION__, Status));
           }
#endif /* FEATURE_XBOOT */
           break;
    }
...
EFI_STATUS QcomChargerApp_DisplaySignOfLifeOnVBat(EFI_QCOM_CHARGER_ACTION_INFO *ChargerActionInfo)
{
  ...
  if(ChargerActionInfo->BatteryStatusInfo.BatteryVoltage <= (ChargerActionInfo->DispSignOfLifeMaxThresholdMv ))
  {
    ...
    {
#ifdef FEATURE_XBOOT
#define BOOT_TO_HLOS_TH_WITH_CHGR	3200 /* gChargerLibCfgData.boot_to_hlos_threshold_in_mv */
      if(ChargerActionInfo->BatteryStatusInfo.BatteryVoltage <= BOOT_TO_HLOS_TH_WITH_CHGR)
#endif /* FEATURE_XBOOT */
...
EFI_STATUS QcomChargerApp_ContinueCharging(EFI_QCOM_CHARGER_ACTION_TYPE ChgAction, EFI_QCOM_CHARGER_ACTION_INFO *pChgActionInfo)
{
  ...
  else
  {
    ...
    WaitForTimeout(QCOM_CHARGER_IDLE_WAIT_DURATION_IN_MS, QCOM_CHARGER_TIMEOUT_WAIT_FOR_KEY, &UserKey);
    if (EFI_PLATFORMINFO_TYPE_CLS == PlatformType )
    {  /*Check if WaitForTimeout returned due to power key press */
      if ((UserKey.UnicodeChar == 0x00) && (UserKey.ScanCode == 0x102))
      {
