...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ICBLib/icbcfg.c#1 $ 
...
----------  ---    -----------------------------------------------------------
2022/06/29  sds    Add support for a variable number of regions and split LLCC mappings
2020/07/08  sds    Fix remap/mapping handling for complicated mappings
2020/06/30  sds    Fix handling of empty config segments in nested lists
2019/05/01  sds    Add support for multiple config segments
2018/07/17  sds    Add support for variant check via fuse/register
2017/11/15  sds    Add support for split address remap
...
2012/02/07  av     Created
        ...
        Copyright (c) 2012-2022 QUALCOMM Technologies, Incorporated.
               All Rights Reserved.
...
static icbcfg_error_type ICB_Configure_Settings( icbcfg_prop_type *prop_data )
{
   ...
   uint32 seg_idx, i, cnt = 0, data_32 = 0;
   uint16 data_16 = 0;
   uint8 data_8 = 0;
   ...
   for(seg_idx = 0;
       seg_idx < prop_data->len && ICBCFG_SUCCESS == ret_val;
       seg_idx++)
   {
      icbcfg_prop_seg_type *seg = prop_data->segs[seg_idx];
      ...
      icbcfg_data_type *cfg_data = seg->cfg_data;
...
)
{
   if( !dev_config_valid )
   {
      do
      {
         ...
         family  = ChipInfo_GetChipFamily();
         version = ChipInfo_GetChipVersion();
         ...
         for( i = 0; i < icbcfg_info->num_configs; i++ )
         {
                ...
                (false == device_config->match && version >= device_config->version) )
            {
               ...
               * We succeed if:
               *   We've no register to read (reg addr == NULL) *OR*
               *   The masked register read equals register_val. */
               if( NULL == device_config->reg_addr ||
                   device_config->reg_val == 
                     (in_dword(device_config->reg_addr) & device_config->reg_mask) )
               {
                  /* If num_llccs_per_channel is zero, then update default llccs per channel i.e 1 */
                  if(!device_config->num_llccs_per_channel)
                  {
                    device_config->num_llccs_per_channel = 1;
                  }
                 if((device_config->chipid == CHIPINFO_ID_UNKNOWN) ||
                      (chipid == device_config->chipid) )
                  {
...
/**
@brief
    Find matching constraint region.
 
@param[in]  dev_config  a pointer to the device configuration data to use
@param[in]  start_addr  start address of the region to match
@param[in]  end_addr    end address of the region to match

@returns A pointer to the matched region, NULL if not found.
*/ 
//*============================================================================*/
icbcfg_mem_region_type *get_constraint_region
(
  icbcfg_device_config_type *dev_config,
  uint64                     start_addr,
  uint64                     end_addr
...
{
   uint32 ridx;
   icbcfg_mem_region_type *region = NULL;

   for( ridx = 0; ridx < dev_config->map_ddr_region_count; ridx++ )
   {
      /* If the start address of either region lies in the other, select it. */
      if( (start_addr >= dev_config->map_ddr_regions[ridx].start_addr &&
            start_addr < dev_config->map_ddr_regions[ridx].end_addr) ||
            (dev_config->map_ddr_regions[ridx].start_addr >= start_addr &&
            dev_config->map_ddr_regions[ridx].start_addr < end_addr) )
      {
         region = &dev_config->map_ddr_regions[ridx];
         break;
      }
   }
...
)
{
   ...
   for( channel = 0; channel < dev_config->num_channels; channel++ )
   {
      ...
      for( i = 1; i < dev_config->num_segments; i++ )
      {
      ...
      for( pass = 0; pass < (dev_config->num_segments - 1); pass++ )
      {
         for( i = 0; i < (dev_config->num_segments - 1 - pass); i++ )
         {
            /* Swap if:
   ...
   for( channel = 0; channel < dev_config->num_channels; channel++ )
   {
      ...
      for( comp_region = 0, region = 0; region < dev_config->num_segments; region++ )
      { 
         ...
         * share the same interleaving mode, and are address contiguous. */
         ...
         if( constrain )
         {
            ...
            icbcfg_mem_region_type *map_ddr_region =
               get_constraint_region(dev_config,
                                     channel_regions[comp_region].base_addr,
                                     channel_regions[comp_region].base_addr + channel_regions[comp_region].size);

            /* If we didn't find a region that matches, it lies outside of the allowable address space.
             * Remove it. */
            if( NULL == map_ddr_region )
            {
            ...
            if( channel_regions[comp_region].base_addr < map_ddr_region->start_addr )
            {
               channel_regions[comp_region].size -= map_ddr_region->start_addr - channel_regions[comp_region].base_addr;
               channel_regions[comp_region].base_addr = map_ddr_region->start_addr;
            }
            if( channel_regions[comp_region].base_addr + channel_regions[comp_region].size > map_ddr_region->end_addr )
            {
               channel_regions[comp_region].size = map_ddr_region->end_addr - channel_regions[comp_region].base_addr;
            }
         }
      }
      ...
      for( used_region = 0; used_region < dev_config->num_segments; used_region++ )
      {
      ...
      if( 0 != used_region )
      {
         memsmove(&channel_regions[0],
                 sizeof(channel_regions[0]) * (dev_config->num_segments - used_region),
      ...
      for( comp_region++; comp_region < dev_config->num_segments; comp_region++ )
      {
         channel_regions[comp_region].interleaved = false;
         channel_regions[comp_region].base_addr = 0;
         channel_regions[comp_region].size = 0;
...
)
{
   ...
   *match = REGION_MATCH_NONE;
   ...
   for( uint32 i = 0; i < dev_config->num_segments; i++ )
   {
...
)
{
        ...
        channel++ )
   {
      ...
      while( map_idx < dev_config->num_segments && hw_idx < dev_config->num_segments )
      {
         ...
         if( hw_idx >= dev_config->num_segments )
         {
            break;
         }
         ...
         find_remap_match( dev_config, sorted_remaps, curr_region, &remap, &match );
         switch(match)
         {
            ...
            {
               icbcfg_mem_region_type *map_ddr_region;
               uint64 size = remap->size, curr_end;
               next_start_addr = curr_region->base_addr + remap->size;

               /* Make sure we don't exceed the valid range for the allowed memory map. */
               map_ddr_region = get_constraint_region(dev_config,
                                                      curr_region->base_addr,
                                                      next_start_addr);
               if( NULL != map_ddr_region &&
                   next_start_addr > map_ddr_region->end_addr )
               {
                  size = map_ddr_region->end_addr - curr_region->base_addr;
                  next_start_addr = map_ddr_region->end_addr;
               }

               /* Make sure we also don't exceed the valid range for current region 
                * we're operating on. */
               curr_end = curr_region->base_addr + curr_region->size;
               if( next_start_addr > curr_end )
               {
                  size = curr_end - curr_region->base_addr;
                  next_start_addr = curr_end;
               }

               hw_map->region[channel][hw_idx].offset =
                  (int64)remap->src_addr - (int64)remap->dest_addr;
               curr_region->size -= size;
               curr_region->base_addr += size;
               break;
            }
      ...
      /* Check to see if our last populated region is not the last supported
       * region AND the region is adjacent to ADDR_MAX (or in a unexpected case greater than)
       * If it is, it must be the last region we populate.
       * Additionally, protect against the case that something is really wrong with the
       * input ddr_info (no DDR!) and avoid accessing before the start of the array.
       */
      const uint64 ADDR_MAX = 1ULL << dev_config->addr_width;
      if( hw_idx < dev_config->num_segments && hw_idx >= 1 &&
          hw_map->region[channel][hw_idx].start_addr >= ADDR_MAX )
      {
         /* Copy the previous segment data.
          * Then zero previous segment offset and mark as error.
          */
         for( ; hw_idx < dev_config->num_segments; hw_idx++ )
         {
            uint32 prev = hw_idx - 1;
            hw_map->region[channel][hw_idx].start_addr = hw_map->region[channel][prev].start_addr;
            hw_map->region[channel][hw_idx].offset = hw_map->region[channel][prev].offset;
            hw_map->region[channel][prev].offset = 0;
            hw_map->error_mask[channel] |= 1 << prev;
         }
      }

      /* Fill in any unused regions with the last base and mark error. */
      for( uint32 last = hw_idx; hw_idx < dev_config->num_segments; hw_idx++ )
      {
   ...
   switch(dev_config->trans_type)
   {
      ...
      case ICBCFG_ADDR_TRANS_LLCC:
         ...
         for( uint32 channel = 0;
              ...
              channel++ )
         {
            uint32 llccs_start = channel * dev_config->num_llccs_per_channel;
			
            for(uint32 llccs_per_ch = llccs_start; 
                 llccs_per_ch < (dev_config->num_llccs_per_channel 
                      + llccs_start); llccs_per_ch++)
            {
               HWIO_OUTXF(dev_config->trans_bases[llccs_per_ch],
                      ...
                      + llccs_start) ; llccs_per_ch++)
            {
	            ...
	            for( uint32 region = 0; region < dev_config->num_segments; region++ )
	            {
	               ...
	               * The register doesn't exist and hardware assumes zero. */
	               if( region != 0 )
	               {
	                  /* Hardware has added an additional set of REGIONn registers
	                   * but they didn't make them contiguous with the original set.
	                   * So we'll use some special HWIO to make this mapping less awful. */
	                  if( region < ADDL_REGION_START )
	                  {
	                     HWIO_OUTXI(dev_config->trans_bases[llccs_per_ch],
	                              LLCC_BEAC_ADDR_REGIONn_CFG2,
	                              region,
	                              ADDR_LOW(hw_addr->start_addr));
	                     HWIO_OUTXFI(dev_config->trans_bases[llccs_per_ch],
	                                 LLCC_BEAC_ADDR_REGIONn_CFG3,
	                                 region,
	                                 BASE_HIGH,
	                                 ADDR_HIGH(hw_addr->start_addr));
	                  }
	                  /* Regions ADDL_REGION_START and higher */
	                  else
	                  {
	                     HWIO_OUTXI(dev_config->addl_trans_bases[llccs_per_ch],
	                              LLCC_BEAC_ADDR_ADDL_REGIONn_CFG2,
	                              region,
	                              ADDR_LOW(hw_addr->start_addr));
	                     HWIO_OUTXFI(dev_config->addl_trans_bases[llccs_per_ch],
	               ...
	               /* Rememeber that the additional region applies to the offset regs too. */
	               if( region < ADDL_REGION_START )
	               {
	                  /* Set new offset. */
	                  HWIO_OUTXI(dev_config->trans_bases[llccs_per_ch],
      ...
      case ICBCFG_ADDR_TRANS_MC:
         ...
         for( uint32 channel = 0;
              ...
              channel++ )
         {
            uint32 mcs_start = channel * dev_config->num_llccs_per_channel;
			
            for(uint32 mcs_per_ch = mcs_start; 
                 mcs_per_ch < (dev_config->num_llccs_per_channel 
                      + mcs_start); mcs_per_ch++)
            {
            HWIO_OUTXF(dev_config->trans_bases[mcs_per_ch],
                       MC_ISU_ADDR_TRANSLATOR_CFG, ENABLE,
                       0);
           }
         }
              ...
              channel++ )
         {
            ...
            for( uint32 region = 0; region < dev_config->num_segments; region++ )
            {
               ...
               * The register doesn't exist and hardware assumes zero. */
               if( region != 0 )
               {
                  HWIO_OUTXI(dev_config->trans_bases[mcs_per_ch],
                             MC_ISU_ADDR_REGIONn_CFG2,
                             region,
                                ADDR_LOW(hw_addr->start_addr));
                  HWIO_OUTXFI(dev_config->trans_bases[mcs_per_ch],
              ...
              channel++ )
         {
            ...
            HWIO_OUTXF(dev_config->trans_bases[mcs_per_ch],
                       MC_ISU_ADDR_TRANSLATOR_CFG, ENABLE,
                       HWIO_FMSK(MC_ISU_ADDR_TRANSLATOR_CFG,ENABLE));
           }
         }
		 }
         break;
   ...
   for( region = 0; region < dev_config->num_segments; region++ )
   {
               ...
               input_region->base_addr < (channel_regions[region].base_addr + channel_regions[region].size) )
      {
             ...
             free_region < dev_config->num_segments )
         {
            ...
            channel_regions[free_region].interleaved = input_region->interleaved;
         }
...
{
   uint32 channel;
   const uint32 LAST_REGION = dev_config->num_segments - 1;
...
)
{
   ...
   for( uint32 i = start_idx; i < dev_config->num_segments; i++ )
   {
      /* Skip unused entries. */
      ...
      if( (elem->dest_addr >= remaps[i].src_addr &&
           elem->dest_addr < (remaps[i].src_addr + remaps[i].size)) ||
...
/**
@brief 
      ICB configuration data for very early configurations (ARCG, etc)
      It reads ICB configuration data (register address and config data) from
      DAL Device Property file and configures ICB registers.
...
/*============================================================================*/
icbcfg_error_type
ICB_Config_EarlyInit( const char * dev )
{
   icbcfg_error_type ret_val = ICBCFG_ERROR_INIT_FAILURE;
   ...
   do
   {
      ...
      ret_val = ICB_Configure_Settings(dev_config->early_prop_data);
   } while( false );
...
)
{
   ...
   do
   {
      ...
      else
      {
         ...
         for(channel = 0; channel < dev_config->num_channels; channel++)
         {
            src_regions[channel] = dev_config->num_segments;
            for(region = 0; region < dev_config->num_segments; region++)
            {
      ...
      memscpy(sorted_remaps,sizeof(sorted_remaps), req_remaps, sizeof(sorted_remaps));
      for( uint32 i = 0; i < dev_config->num_segments - 1; i++ )
      {
         for( uint32 j = 0; j < dev_config->num_segments - (i + 1); j++ )
         {
            /* Swap if the current entry will overlap with something after it
             * or if it's an empty entry. */
            if( (sorted_remaps[j].src_addr == sorted_remaps[j].dest_addr || sorted_remaps[j].size == 0) ||
                will_overlap( dev_config, &sorted_remaps[j], sorted_remaps, j+1 ) )
            {
      ...
      for(i = 0; i < dev_config->num_segments; i++)
      {
         ...
         for(channel = 0; channel < dev_config->num_channels && !found; channel++)
         {
            ...
            if(src_regions[channel] >= dev_config->num_segments)
            {
               continue;
            }
