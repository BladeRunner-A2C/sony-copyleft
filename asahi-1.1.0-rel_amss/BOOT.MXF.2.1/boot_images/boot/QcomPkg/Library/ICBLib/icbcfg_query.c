...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ICBLib/icbcfg_query.c#1 $
...
----------  ---    -----------------------------------------------------------
2022/06/29  sds    Support more than 6 regions
2018/07/17  sds    Add support for variant check via fuse/register
2017/06/22  sds    Added support for SoC addr fetch
...
static addr_trans_seg_t           addr_trans_segs[MAX_REGIONS+1];
...
)
{
   ...
   switch(dev_config->trans_type)
   {
      ...
      case ICBCFG_ADDR_TRANS_LLCC:
         ...
         for( i = 0; i < dev_config->num_segments; i++ )
         {
            ...
            else
            {
               ...
               {
                  addr_trans_segs[i].base = ((uint64)HWIO_INXI(dev_config->addl_trans_bases[0],LLCC_BEAC_ADDR_ADDL_REGIONn_CFG2,i)) |
                                    (((uint64)HWIO_INXI(dev_config->addl_trans_bases[0],LLCC_BEAC_ADDR_ADDL_REGIONn_CFG3,i)) << 32);
               }
            }
            ...
            {
               addr_trans_segs[i].offset = ((uint64)HWIO_INXI(dev_config->addl_trans_bases[0],LLCC_BEAC_ADDR_ADDL_REGIONn_CFG0,i)) |
                                    (((uint64)HWIO_INXI(dev_config->addl_trans_bases[0],LLCC_BEAC_ADDR_ADDL_REGIONn_CFG1,i)) << 32);
            }
      ...
      case ICBCFG_ADDR_TRANS_MC:
         error = HWIO_INXF(dev_config->trans_bases[0], MC_ISU_ADDR_TRANSLATOR_CFG, ERROR);
         for( i = 0; i < dev_config->num_segments; i++ )
         {
            /* Base register does not exist for first segment. Forced to 0x0. */
            if( i == 0 )
            {
               addr_trans_segs[0].base = 0ULL;
               addr_trans_segs[i].offset = ((uint64)HWIO_INXI(dev_config->trans_bases[0],MC_ISU_ADDR_REGION_n_CFG0,i)) |
                                    (((uint64)HWIO_INXI(dev_config->trans_bases[0],MC_ISU_ADDR_REGION_n_CFG1,i)) << 32);
            }
            else
            {
               addr_trans_segs[i].base = ((uint64)HWIO_INXI(dev_config->trans_bases[0],MC_ISU_ADDR_REGIONn_CFG2,i)) |
                                 (((uint64)HWIO_INXI(dev_config->trans_bases[0],MC_ISU_ADDR_REGIONn_CFG3,i)) << 32);
               addr_trans_segs[i].offset = ((uint64)HWIO_INXI(dev_config->trans_bases[0],MC_ISU_ADDR_REGIONn_CFG0,i)) |
                                    (((uint64)HWIO_INXI(dev_config->trans_bases[0],MC_ISU_ADDR_REGIONn_CFG1,i)) << 32);
            }
...
)
{
   ...
   for( i = 0, region = 0; i < ARRAY_SIZE(addr_trans_segs) ; i++ )
   {
      ...
      if( !addr_trans_segs[i].error )
      {
          ...
          * Always assume interleaved if more than one channel. */
         if( 0 == regions[region].base_addr )
         {
            regions[region].base_addr = addr_trans_segs[i].base;
            regions[region].interleaved = dev_config->num_channels > 1;
  ...
  do
  {
    ...
    for( i = 0; i < dev_config->num_segments - 1; i++ )
    {
      ...
      if(addr_trans_segs[i].base < addr_trans_segs[i+1].base)
      {
        ...
        region1.interleaved = dev_config->num_channels > 1;
      }
      else
      {
        return ICBCFG_ERROR;
      ...
      for( j = i+1; j < dev_config->num_segments; j++ )
      {
        ...
        /* addr_trans_segs size is MAX_REGIONS +1,so accessing j+1 is OK
         * Always assume interleaved if more than one channel. */
        if(addr_trans_segs[j].base < addr_trans_segs[j+1].base)
        {
          ...
          region2.interleaved = dev_config->num_channels > 1;
        }
        else
        {
          return ICBCFG_ERROR;
...
/**
@brief
    Compute the soc address from a memory controller address

@param[in]    dev_config   device config information to use
@param[in]    mc_addr      The mc address to translate
@param[inout] soc_addr     Pointer to store soc address

*/
//*============================================================================*/
icbcfg_error_type translate_mc_address
(
 icbcfg_device_config_type *dev_config,
 uint64                     mc_addr,
 uint64 *                   soc_addr 
)
{
   ...
   {
      /* Check if our address lies between the two segments we're looking at. */
      if( (addr_trans_segs[i].base + addr_trans_segs[i].offset)   <= mc_addr &&
          (addr_trans_segs[i+1].base + addr_trans_segs[i].offset) >  mc_addr )
      {
         /* We have a match.
          * If it's not to an error segment, translate the address. */
         if( !addr_trans_segs[i].error )
         {
            /* This is only safe to do because we sign extended
             * the offset earlier. */
            *soc_addr = mc_addr - addr_trans_segs[i].offset;
            ret = ICBCFG_SUCCESS;
            break;
         }
         /* Mapped to error segment, skip.
          * With remapping, it's possible to map to more than one. */
         else
         {
            continue;
         }
...
/**
@brief 
      Translate memory controller addr space to SoC addr space

@param[in]    dev      DAL string device ID.
@param[in]    mc_addr  The address in memory controller space
@param[inout] soc_addr The address in SoC space
...
*/ 
/*============================================================================*/
icbcfg_error_type ICB_Get_SoCAddr( const char * dev, uint64 mc_addr, uint64 * soc_addr )
{
   icbcfg_error_type ret_val = ICBCFG_ERROR;
   ...
   do
   {
      ...
      ret_val = translate_mc_address( dev_config, mc_addr, soc_addr );
   } while( false );
