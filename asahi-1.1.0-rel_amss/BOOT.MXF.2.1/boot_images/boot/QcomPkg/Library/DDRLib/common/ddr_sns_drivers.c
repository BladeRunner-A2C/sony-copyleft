...
--------   ---     --------------------------------------------------------
02/24/23   ck      Moved dt_entries instantiation from ddr_drivers.h to here
11/14/22   sym     remove void *blob variable
...
void ddr_devicetree_helper_uint32(const char *entry_name, char *dt_name, uint32 *retval)
{
	...
	fdt_get_node_handle(&node, NULL, dt_name);
...
void ddr_extract_common_dt_entries(void){
	...
	ddr_devicetree_helper_uint32("ddr_log_level", "/soc/ddr", &temp_ddr_log);
	ddr_devicetree_helper_uint32("print_severity", "/soc/ddr", &temp_print_severity);
