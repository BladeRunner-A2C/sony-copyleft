...
--------   ---     --------------------------------------------------------
10/17/22   yps     Remove unused variable
03/09/21   mkma    abnormal sdi use AOP code ram
01/28/21   mkma    updated cold boot condition check
11/21/20   kns     Add shub freq data to SMEM
...
/*==============================================================================
                                      DATA
==============================================================================*/
#define LLCC_AS_TCM_BASE_ADDR  0x15800000
...
{   
    ...
    fptr_HAL_EXT_CORE_SW_strlcpy = ddr_external_strlcpy;
...
#ifdef  SECLIB_API
    fptr_HAL_EXT_CORE_SW_Populate_LCP_DARE_Regions_Cfg = seclib_initiate_dare_sequence;
#endif
	...
	fptr_HAL_DDR_FW_IOCTL = DDR_FW_IOCTL;
    fptr_HAL_EXT_CORE_SW_Get_Chromebook_Sku_Info = EXT_CORE_SW_Get_Chromebook_Sku_Info;
    fptr_HAL_EXT_CORE_SW_Get_Warm_Mission_Mode_Info = EXT_CORE_SW_Get_Warm_Mission_Mode_Info;
...
void ddr_get_shub_shrm_freq(void)
{
	...
	uint32 shub_init_freq = 0;
	uint32 shrm_init_freq = 0;
	ClockHandle nClkHandle;
	ClockIdType nClkId;

	if (Clock_Attach(&nClkHandle, "DDRLib") != CLOCK_SUCCESS)
	{
	 	return;
	}

	if (Clock_GetId(nClkHandle, "gcc_memnoc_clk", &nClkId) != CLOCK_SUCCESS)
	{
	 	return;
	}

	if(Clock_GetFrequency(nClkHandle, nClkId, &shub_init_freq) == CLOCK_SUCCESS)
	{
	...
	if (Clock_GetId(nClkHandle, "gcc_shrm_clk", &nClkId) != CLOCK_SUCCESS)
	{
		return;
	}

	if(Clock_GetFrequency(nClkHandle, nClkId, &shrm_init_freq) == CLOCK_SUCCESS)
	{
...
{
  uint8 *ddr_fw_buffer_ptr = NULL;
  HAL_Set_Function_Pointers();
  ddr_set_sdi_struct();
  
  if(dload_mode == 0x0) 
  {
	ddr_fw_buffer_ptr = (uint8 *)SCL_SBL1_DDR_DSF_CFG_BUF_BASE; //0x0B000000;//aop code ram
	if(clock_configured == FALSE)
		HAL_DDR_FW_Enter((void*)ddr_fw_buffer_ptr, SIZE_68KB);//
  }
  else if ((HWIO_INF(AOSS_CC_RESET_FSM_CTRL, FIRST_PASS_COMPLETE) && HWIO_AOSS_CC_ALT_RESET_ENABLE_INM(HWIO_AOSS_CC_ALT_RESET_ENABLE_ALLOW_BIMC_ALT_ARES_BMSK)) ||
            HWIO_INF(AOSS_CC_RESET_FSM_STATUS, FIRST_PASS_EXPIRE)||(!HWIO_INF(AOSS_CC_RESET_FSM_CTRL, FIRST_PASS_COMPLETE)))
  {

	ddr_fw_buffer_ptr = (uint8 *)SCL_SBL1_DDR_DSF_CFG_BUF_BASE;//0x0B000000; //aop code ram 
	if(clock_configured == FALSE)
		HAL_DDR_FW_Enter((void*)ddr_fw_buffer_ptr, SIZE_68KB);//SCL_SBL1_DDR_DSF_CFG_BUF_BASE
  }	

  ddr_shared_data = HAL_DDR_FW_Get_DDR_SHARED_STRUCT_Pointer(); //call after FW_Enter, do confirm with earlier implementation  
...
boolean ddr_get_shared_info_details(ddr_details_entry_ptr_type ddr_details_entry_ptr)
{
	  ...
	  uint8 num_of_ranks;
	  uint8 clk_idx=0;
      shub_freq_plan_entry shub_freq_tbl;
	  ddr_misc_info ddr_misc_data;
    ...
    /* Copy data to SMEM */
    memscpy(((UINT8 *)ddr_details_entry_ptr + DDR_REGIONS_ENTRY_SMEM_OFFSET), sizeof(DDR_REGIONS_ENTRY), &ddr_driver_struct->ddr_regions_info, sizeof(DDR_REGIONS_DATA));
    if(ddr_get_boot_mode() != BOOT_MODE_RAMDUMP)
	{
 	  //do not Move acceessing of AOP PARAMS in ram dump mode
	  ...
	  ddr_details_entry_ptr->ddr_freq_tbl.max_nom_ddr_freq = ddr_search_nominal_freq();
	  ddr_details_entry_ptr->ddr_freq_tbl.clk_period_address = (UINT32*)get_clk_period_regaddr();
	  
      memset(&shub_freq_tbl, 0, sizeof(shub_freq_plan_entry));
      for (clk_idx = 0; clk_idx < ddr_shared_data->num_shub_levels; clk_idx++)
      {
        shub_freq_tbl.shub_freq[clk_idx].freq_khz = ddr_driver_struct->aop_params->shub_states[clk_idx].freq_khz;
        shub_freq_tbl.shub_freq[clk_idx].enable = ddr_driver_struct->aop_params->shub_states[clk_idx].freq_enable;
      }
      shub_freq_tbl.num_shub_freqs = ddr_shared_data->num_shub_levels;

      memset(&ddr_misc_data, 0, sizeof(ddr_misc_info));
      memscpy(&ddr_misc_data, (sizeof(uint32)*3), ddr_shared_data, (sizeof(uint32)*3));
	  //ddr_misc_data.dsf_version = ddr_shared_data->dsf_code_version;

      /* Copy data to SMEM */
      memscpy(((UINT8 *)ddr_details_entry_ptr + SHUB_FREQ_PLAN_ENTRY_SMEM_OFFSET), sizeof(shub_freq_plan_entry), &shub_freq_tbl, sizeof(shub_freq_plan_entry));
      memscpy(((UINT8 *)ddr_details_entry_ptr + DDR_MISC_INFO_SMEM_DATA_OFFSET), sizeof(ddr_misc_info), &ddr_misc_data, sizeof(ddr_misc_info));
	  /* Indicating End of Boot and cannot access SHRM memory after this in XBL */
	  HAL_DDR_FW_IOCTL(DDR_FW_IO_End_of_Boot, NULL, 0);
	}
    return status;
}
...
uint32 ddr_search_nominal_freq(void)
{
	...
	{
		if (MCCfg[NewLvlIdx].vddcx == TURBO || MCCfg[NewLvlIdx].vddcx == TURBO_L1)
		{
			TURBOIdx = NewLvlIdx;
		}
