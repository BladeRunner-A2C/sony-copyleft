...
Copyright (c) 2012, 2014, 2016, 2022 Qualcomm Technologies Inc. All rights reserved
  ...
  )
{
  ...
  if (!VolumeNameMatch) {
    if (FileStart == StrLen) {
      ...
      CwdPlusPathNameLen = AsciiStrSize (gCwd) + AsciiStrSize (PathName);
      CwdPlusPathName = AllocatePool (CwdPlusPathNameLen);
      ...
      if ((PathName[0] == '/') || (PathName[0] == '\\')) {
        ...
        AsciiStrCpyS (CwdPlusPathName, CwdPlusPathNameLen + 1,  gCwd);
        StrLen = AsciiStrLen (gCwd);
        if ((*PathName != '/') && (*PathName != '\\') && (gCwd[StrLen-1] != '/') && (gCwd[StrLen-1] != '\\')) {
          AsciiStrCatS (CwdPlusPathName, CwdPlusPathNameLen + 1, "\\");
        }
      }

      AsciiStrCatS (CwdPlusPathName,CwdPlusPathNameLen + 1,  PathName);
  ...
  AsciiStrCpyS (File->DeviceName, StrLen + 1,  PathName);
  ...
  )
{
  ...
  NewPathLen = AsciiStrSize (Cwd) + StrLen + 1;
  NewPath = AllocatePool (NewPathLen);
  if (NewPath == NULL) {
    return NULL;
  }
  AsciiStrCpyS (NewPath, NewPathLen,  Cwd);
  ...
  for (Start = Path ;;) {
    ...
    AsciiStrnCatS (NewPath, NewPathLen, Start, Work - Start);
  ...
  AsciiStrnCatS (NewPath, NewPathLen, Start, End - Start);
  ...
  )
{
  ...
  DeviceNameLength = AsciiStrSize (File->DeviceName);
  FileNameLength = AsciiStrSize (File->FileName);
  gCwd = AllocatePool (DeviceNameLength + FileNameLength + 10);
  if (gCwd == NULL) {
    return EFI_INVALID_PARAMETER;
  }

  AsciiStrCpyS (gCwd, DeviceNameLength, File->DeviceName);
  if (File->FileName == NULL) {
    AsciiStrCatS (gCwd, DeviceNameLength+AsciiStrSize(":\\"),  ":\\");
  } else {
    AsciiStrCatS (gCwd, DeviceNameLength+AsciiStrSize(":"), ":");
    AsciiStrCatS (gCwd, DeviceNameLength+FileNameLength,  File->FileName);
  }
