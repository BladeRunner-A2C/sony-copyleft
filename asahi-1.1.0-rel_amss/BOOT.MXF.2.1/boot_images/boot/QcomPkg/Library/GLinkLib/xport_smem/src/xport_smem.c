/*===========================================================================
   ...
   Copyright (c) 2014-2015, 2019-2020,2022, 2023 Qualcomm Technologies, Inc.
   All Rights Reserved.
...
#define XPORT_SMEM_READ_BACK(addr) \
  do { \
    volatile uint64 read_back = *((uint64 *)(addr)); \
    read_back = read_back; \
  } while(0)
...
static EFI_HWIO_PROTOCOL *xport_smem_hwio_protocol_iface;
...
===========================================================================*/
/*===========================================================================
FUNCTION      xport_smem_read_back_on_4k_boundary
===========================================================================*/
void xport_smem_read_back_on_4k_boundary(uint64 address_limit, uint64 address_boundary)
{
  
  while(1)
  {
    if( address_boundary >= address_limit )
    {
      /*break out of the while loop*/
      break;
    }

    XPORT_SMEM_READ_BACK(address_boundary);
    
    /*Calculate the next 4K boundary after checking for OVERFLOW*/
    address_boundary = glink_os_add64(address_boundary, 0x1000);
  }
  return;
}
...
*/
/*=========================================================================*/
int32 xport_smem_smem_alloc_ex
(
...
int32 xport_smem_hwio_map_region(const char* module, void** addr)
{
	EFI_STATUS ret;

   if (xport_smem_hwio_protocol_iface)
   {
     ret = xport_smem_hwio_protocol_iface->MapRegion(xport_smem_hwio_protocol_iface, module, (UINT8**)addr); 
   }
   else
   {
     ret = SMEM_STATUS_FAILURE;
...
)
{
  ...
  *(uint64*)&ctx_ptr->tx_fifo[write_ind] = value;
  ...
  XPORT_SMEM_READ_BACK((uint64)&ctx_ptr->tx_fifo[write_ind]);
...
)
{
  ...
  if (tx_last <= data_size)
  {
    ...
    XPORT_SMEM_READ_BACK((uint64)(&ctx_ptr->tx_fifo[write_ind]));
    xport_smem_read_back_on_4k_boundary(glink_os_add((uint64)&ctx_ptr->tx_fifo[write_ind], tx_last), ((uint64)(&ctx_ptr->tx_fifo[write_ind]) + 0x1000) & 0xFFFFF000);
    write_ind = 0;
    data_size -= tx_last;
    data_size64 -= tx_last;
    buffer_ptr += tx_last;
  ...
  if (data_size != 0)
  {
    ...
    XPORT_SMEM_READ_BACK((uint64)(&ctx_ptr->tx_fifo[write_ind]));	
    xport_smem_read_back_on_4k_boundary(glink_os_add((uint64)&ctx_ptr->tx_fifo[write_ind], data_size), ((uint64)(&ctx_ptr->tx_fifo[write_ind]) + 0x1000) & 0xFFFFF000);
  }
...
)
{
  ...
  /* Read back the write index to ensure latest value is reflected onto shared memory */
  XPORT_SMEM_READ_BACK((uint64)&ctx_ptr->tx_desc->write_ind);
...
)
{
  ...
  glink_os_cs_acquire(&ctx->tx_link_lock);
  ...
  glink_os_cs_release(&ctx->tx_link_lock);

  return GLINK_STATUS_SUCCESS;
}

boolean xport_smem_is_link_down
(
  glink_transport_if_type *if_ptr
)
{
  ...
  if(ctx_ptr->tx_desc->read_ind == 0  &&  ctx_ptr->rx_desc->write_ind == 0)
  {
	xport_smem_ssr(if_ptr);
	return TRUE;
  }
  return FALSE;
}
...
void xport_smem_check_link(xport_smem_ctx_type *ctx_ptr)
{
  ...
  {
    if(ctx_ptr->pcfg->alloc_fifo == NULL)
    {
      smem_alloc_params_type smem_params;
    ...
    {
      /*use alloc_fifo itself to refill all details again and update spss register as well.
      link_up is not called till spss interrupt comes from other side as modem will be 
      allocating fifo for both and if we say link_up and if spss is not ready it will be mismatch*/
      ctx_ptr->pcfg->alloc_fifo(ctx_ptr);
    }
  }
...
/*===========================================================================
FUNCTION      xport_smem_mask_interrupt
===========================================================================*/
/**
...
glink_err_type xport_smem_mask_interrupt(glink_transport_if_type *if_ptr, boolean mask)
{
  xport_smem_ctx_type *ctx_ptr = (xport_smem_ctx_type *)if_ptr;
  ...
  if (ctx_ptr->irq_mask != mask)
  {
    if (mask)
    { 
      glink_os_deregister_isr(&ctx_ptr->pcfg->irq_out);
    }
    else
    {
      glink_os_register_isr(&ctx_ptr->pcfg->irq_out, 
                                (os_isr_cb_fn)xport_smem_isr, 
                                ctx_ptr);
    }
...
glink_err_type xport_smem_init(void *arg)
{
  ...
  EFI_GUID hwio_guid = EFI_HWIO_PROTOCOL_GUID;
  
  if(xport_smem_config_init())
  {
  ...
  /* get HWIO protocol interface */
  status = gBS->LocateProtocol(&hwio_guid, NULL,
                               (void**)&xport_smem_hwio_protocol_iface);
  if(status != EFI_SUCCESS)
  {
    return GLINK_STATUS_NOT_INIT;
  }
  ...
  for (ind = 0; ind < xport_smem_config_num; ind++)
  {
	...
	xport_smem_ctx[ind].xport_if.mask_rx_irq = &xport_smem_mask_interrupt;
    xport_smem_ctx[ind].xport_if.qos_functbl = xport_smem_ctx[ind].pcfg->qos_api;
    xport_smem_ctx[ind].xport_if.is_link_down = (is_link_down_fn)&xport_smem_is_link_down;
	xport_smem_ctx[ind].xport_if.remote_down = FALSE;
	xport_smem_ctx[ind].xport_if.remote_down_cs = glink_os_cs_create();
    ...
    // if (!glink_os_register_isr((os_ipc_intr_type *)&xport_smem_ctx[ind].pcfg->irq_out, (os_isr_cb_fn)xport_smem_isr, 
                           // &xport_smem_ctx[ind]))
    // {
