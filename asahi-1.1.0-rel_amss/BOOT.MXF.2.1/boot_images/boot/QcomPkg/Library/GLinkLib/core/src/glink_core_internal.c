/*===========================================================================
 ...
 Copyright (c) 2015, 2018-2019, 2022-2023 by Qualcomm Technologies, Inc.
 All Rights Reserved.
===========================================================================*/
/*===========================================================================
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/GLinkLib/core/src/glink_core_internal.c#1 $
 ...
 * @param[in]    link_status_mask   XPORT Link status mask
 * @param[in]    out                unused
 * 
...
(
  glink_transport_if_type *if_ptr,
  void                    *xport_name,
  uint32                   link_status_mask,
  void                   **out
)
{
 GLINK_OS_UNREFERENCED_PARAM( out );
  
  if (!(link_status_mask & (1 << if_ptr->glink_core_priv->status)))
  {
    return FALSE;
  }
...
)
{
  ...
  ch_ctx->magic = GLINK_CHANNEL_CTX_MAGIC;
...
(
  glink_handle_type          client_ch_handle
)
{
  struct glink_channel_ctx* handle = (struct glink_channel_ctx*)client_ch_handle;
...
/*===========================================================================
  FUNCTION      glink_is_link_down
===========================================================================*/
/** 
 * Transport to check if remote is down. 
 * This is used only during UEFI exit.
 * Supported only by smem transport.
 *
 * @param[in]    remote_ss      name of remote subsystem
 * @param[in]    transport_name name of transport
 * 
 * @return       TRUE if link is down
 *               FALSE if not supported or link is not down
 * 
 * @sideeffects  NONE
 * @comment      check looks at read and wite indices. This check is reliable only 
 *               in UEFI exit scenario.
 */
/*=========================================================================*/

boolean glink_is_link_down(const char *remote_ss, const char *transport_name)
{
  ...
  {
	  glink_core_xport_ctx_type  *xport_ctx = if_iter_ptr->glink_core_priv;
	  if(xport_ctx && 0 == glink_os_string_compare(xport_ctx->xport, transport_name) && if_iter_ptr->is_link_down)
		  return if_iter_ptr->is_link_down(if_iter_ptr);
  }
  return FALSE;
}
/*===========================================================================
...
)
{
  ...
  /* Get a pointer to if_ptr before checking the magic in the
   * channel context. That way, we can be confidant that this
   * pointer is valid when we acquire the lock in it. */
  if_ptr = ch_ctx->if_ptr;
  if(ch_ctx->magic != GLINK_CHANNEL_CTX_MAGIC)
  {
    return FALSE;
  }

  glink_os_cs_acquire( &if_ptr->glink_core_priv->channel_q_cs );
  /* The channel context might have been freed before acquiring
   * the lock. Hence check it again before trying to take a
   * reference */
  if(ch_ctx->magic == GLINK_CHANNEL_CTX_MAGIC)
  {
  if(opened_only)
  ...
  glink_os_cs_release( &if_ptr->glink_core_priv->channel_q_cs );
  return ch_opened;
}
...
{
  glink_transport_if_type *if_ptr = ch_ctx->if_ptr;

  glink_os_cs_acquire( &if_ptr->glink_core_priv->channel_q_cs );
  ASSERT(ch_ctx->ref_count > 0);
  ASSERT(ch_ctx->magic == GLINK_CHANNEL_CTX_MAGIC);
  ch_ctx->ref_count--;
  if(ch_ctx->ref_count == 0)
  {
    ch_ctx->magic = 0;
    glink_clean_channel_ctx(if_ptr->glink_core_priv, ch_ctx);
...
{
  glink_transport_if_type *best_xport = glinki_find_best_xport(remote_ss, GLINK_XPORT_LINK_STATUS_MASK_UP);
  ...
  *suggested_priority = GLINK_INVALID_PRIORITY;
  ...
  xport_found = glinki_xports_find(remote_ss,
                                   ...
                                   (void *)xport_name,
                                   GLINK_XPORT_LINK_STATUS_MASK_UP,
                                   NULL);
...
/*===========================================================================
  FUNCTION      glinki_find_xport_by_name
===========================================================================*/
/**
 * Find requested transport depending on client's request
 *
 * @param[in]    xport_name         name of transport
 * @param[in]    remote_ss          remote subsystem name
 * @param[in]    link_status_mask   XPORT Link status mask
...
glink_transport_if_type *glinki_find_xport_by_name
(
  const char           *xport,
  const char           *remote_ss,
  uint32               link_status_mask
...
)
{
  ...
  if (xport == NULL)
  {
    if_ptr = glinki_find_best_xport(remote_ss, link_status_mask);
  }
  else
  {
    if_ptr = glinki_xports_find (remote_ss,
                                 glinki_client_requested_xport_check,
                                 (void *)xport, link_status_mask, NULL);
  }
...
/*===========================================================================
  FUNCTION      glink_notify_link_down_to_clients
===========================================================================*/
/** 
 * Notify clients about LINK_DOWN and CHANNEL_DISCONNECTED
 *
 * @param[in]    remote_ss      name of remote subsystem
 * @param[in]    transport_name name of transport
 * 
 ...
 */
/*=========================================================================*/
void glink_notify_link_down_to_clients(const char *remote_ss, const char *transport_name)
{
  glink_transport_if_type *if_iter_ptr;
  uint32 remote_host = glinki_find_remote_host(remote_ss);
  ...
  {
	  glink_core_xport_ctx_type  *xport_ctx = if_iter_ptr->glink_core_priv;
	  if(xport_ctx && 0 == glink_os_string_compare(xport_ctx->xport, transport_name) && if_iter_ptr->glink_core_if_ptr->link_down)
	  {	      
        if_iter_ptr->glink_core_if_ptr->link_down(if_iter_ptr);
		return;
	  }
  }
}
...
/*===========================================================================
  FUNCTION      glink_set_remote_down
===========================================================================*/
/** 
 * Set remote_down flag to TRUE
 *
 * @param[in]    remote_ss      name of remote subsystem
 * @param[in]    transport_name name of transport
 * 
 ...
 */
/*=========================================================================*/
void glink_set_remote_down(const char *remote_ss, const char *transport_name)
{
  glink_transport_if_type *if_iter_ptr;
  uint32 remote_host = glinki_find_remote_host(remote_ss);
  ...
  {
	  glink_core_xport_ctx_type  *xport_ctx = if_iter_ptr->glink_core_priv;
	  if(xport_ctx && 0 == glink_os_string_compare(xport_ctx->xport, transport_name))
	  {	     
        glink_os_cs_acquire(&if_iter_ptr->remote_down_cs);
        if_iter_ptr->remote_down = TRUE;
		glink_os_cs_release(&if_iter_ptr->remote_down_cs);
		return;
	  }
  }
}