...
Copyright (c) 2014-2016,2022-2023 by QUALCOMM Technologies, Incorporated.  All Rights
Reserved.
===========================================================================*/
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/GLinkLib/core/src/glink_core_intent.c#1 $
...
)
{
  ...
  boolean single_thread = options & GLINK_TX_SINGLE_THREADED;
  boolean force_poll = options & GLINK_TX_FORCE_POLL;
  
  if(open_ch_ctx->rm_intent_timeout && single_thread)
  ...
  if (rm_intent == NULL)
  {
    ...
    else
    {
      ...
      {
         if(single_thread) /* cannot wait on an event in single-threaded mode */
         {
            while (!rm_intent)
            {
               glink_rx_poll_internal((glink_handle_type)open_ch_ctx, force_poll);
               rm_intent = glinki_get_rm_intent(open_ch_ctx, pctx->size);
            }
         }
         else
