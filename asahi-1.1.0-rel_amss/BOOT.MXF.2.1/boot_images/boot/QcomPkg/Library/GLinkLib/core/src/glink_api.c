/*===========================================================================
 ...
 Copyright (c) 2014-2016, 2018,2022, 2023 by QUALCOMM Technologies, Incorporated.  All Rights
 Reserved.
===========================================================================*/
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/GLinkLib/core/src/glink_api.c#1 $
...
)
{
  ...
  /* acquire channel reference before using handle */
  ...
  status = xport_ctx->channel_submit_pkt(handle, pctx, options);
 ...
 * This allows the clients to poll the transport for any new
 * data that might have come in.
 *
 * @param[in]   handle      GLink handle associated with the logical channel
 *
 * @return      Standard GLink error codes
 ...
 * @sideeffects  None
 */
glink_err_type glink_rx_poll_internal
(
  glink_handle_type client_ch_handle,
  boolean           force_poll
)
{
  ...
  if (!glinki_channel_ref_acquire(handle, FALSE))
  {
    return GLINK_STATUS_CH_NOT_FULLY_OPENED;
  }

  glink_os_cs_acquire(&handle->if_ptr->remote_down_cs);

  if((!force_poll) && ((handle->if_ptr->remote_down == TRUE) 
	  || (handle->if_ptr->glink_core_priv->status == GLINK_XPORT_LINK_DOWN)))
  {
	status = GLINK_STATUS_SUCCESS;
  }
  else if(handle->if_ptr->poll)
  {
    status = handle->if_ptr->poll(handle->if_ptr);
  }
  else
  {
    status = GLINK_STATUS_API_NOT_SUPPORTED;
  }
  
  glink_os_cs_release(&handle->if_ptr->remote_down_cs);  
  glinki_channel_ref_release(handle);
  return status;
}

/** 
 * This API allows the clients to poll the transport for any new
 * data that might have come in. It would *usually* be used when incoming
...
{
  return glink_rx_poll_internal(client_ch_handle, FALSE);
}

glink_err_type glink_mask_interrupt
(
...
)
{
  ...
  /* Return the if ptr if the LINK is UP for the remote and transport */
  if_ptr = glinki_find_xport_by_name (xport, remote_ss, GLINK_XPORT_LINK_STATUS_MASK_UP);
  if(if_ptr == NULL)
  {
    return GLINK_STATUS_NO_TRANSPORT;
  }

  if (if_ptr->mask_rx_irq != NULL)
  {
    return if_ptr->mask_rx_irq(if_ptr, mask);
  }
...
{
  glink_transport_if_type *if_ptr;
  glink_link_notif_data_type * hdl = (glink_link_notif_data_type *)handle;


  if (!hdl || hdl->remote_ss == NULL)
  {
    return GLINK_STATUS_INVALID_PARAM;
  }

  if_ptr = glinki_find_xport_by_name (hdl->xport, hdl->remote_ss, GLINK_XPORT_LINK_STATUS_MASK_ANY);
  ASSERT(if_ptr != NULL);

  if (if_ptr->glink_core_priv->status != GLINK_XPORT_LINK_UP &&
      if_ptr->poll != NULL)
  {
    if_ptr->poll(if_ptr);
  }

  *state = (if_ptr->glink_core_priv->status == GLINK_XPORT_LINK_UP ?
             GLINK_LINK_STATE_UP : GLINK_LINK_STATE_DOWN);
