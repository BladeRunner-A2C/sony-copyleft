...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 02/05/19	akeshan	Addition of capsule storage type
 11/06/18   prema   KW fixes
 09/27/18   prema   Changes done for CalculateCrc32 handling
...
/**
  Gets the capsule value type which can be either EFIESP (default) or capsule RAW_PARTITION.

  @retval CAPSULE_STORAGE_TYPE_CAPSULE_RAW_FILE                     Capsule storage type is EFIESP.
  @retval CAPSULE_STORAGE_TYPE_CAPSULE_PARTITION                    Capsule storage type is RAW_PARTITION.

**/
CapsuleStorageType GetCapsuleStorageType(VOID) {
	EFI_STATUS Status;
	UINTN      VarSize;
	CapsuleStorageType        CapsuleStorage = CAPSULE_STORAGE_TYPE_UNKNOWN;

	VarSize = sizeof(CapsuleStorage);
	Status = gRT->GetVariable (L"EnableCapsulePartition", &gQcomTokenSpaceGuid,
							   NULL, &VarSize, &CapsuleStorage);

	if(Status != EFI_SUCCESS || CapsuleStorage == CAPSULE_STORAGE_TYPE_MASS_STORAGE || CapsuleStorage == CAPSULE_STORAGE_TYPE_RAM || CapsuleStorage == CAPSULE_STORAGE_TYPE_UNKNOWN) {
		CapsuleStorage = (CapsuleStorageType)PcdGet16(PcdCapsuleStorageType);   //get the capsule storage type from PCD which by default is EFIESP
		DEBUG((DEBUG_INFO, "DxeCapsuleQcom.c GetCapsuleStorageType cpasule storage variable was not set correctly or has an invalid value \n"));
		DEBUG((DEBUG_INFO, "DxeCapsuleQcom.c GetCapsuleStorageType capsule storage type is 0x%X\n", (UINTN) CapsuleStorage));
		return CapsuleStorage;
	}
	
	DEBUG((DEBUG_ERROR, "DxeCapsuleQcom.c GetCapsuleStorageType capsule storage type is 0x%X\n", (UINTN) CapsuleStorage));
	return CapsuleStorage;
}
  ...
  )
{
  ...
  //Doesn't reach here, added just to fix MISRA violation.
  return 0 ;
}
    ...
    if (Files != 0) {
      ...
      for (File = 0; File < Files; File++) {
        ...
        if (!EFI_ERROR(Status)) {
          ...
          if ((IsValidCapsuleHeader((EFI_CAPSULE_HEADER *)FileData, Size)) &&
              (EFI_SUCCESS == SupportCapsuleImage (FileData))) {
            (*CapsuleCount)++;
    ...
    if (Files != 0) {
      ...
      for (File = 0; File < Files; File++) {
        ...
        if (!EFI_ERROR(Status)) {
          ...
          DEBUG ((DEBUG_ERROR, "Failed to read FMP capsule file. Skipping and deleting current file! ResetNeeded:d\n", ResetNeeded));
          FileHandle->Delete(FileHandle);
        }
      }
    }
  ...
  )
{
  ...
  mScatterGatherHeader->Crc32 = CalculateCrc32(mScatterGatherHeader,
  ...
  )
{
  ...
  CapsuleStorage = GetCapsuleStorageType();
  DEBUG ((DEBUG_INFO, "DxeCapsuleQcom.c PersistCapsules capsule storage type is 0x%X\n", (UINTN) CapsuleStorage));
  ...
  if (!EFI_ERROR (Status)) {
    MarkCapsulePendingInPersistedMedia();
  }
  return Status;
}
  ...
  )
{
  ...
  CapsuleStorage = GetCapsuleStorageType();
  DEBUG ((DEBUG_INFO, "DxeCapsuleQcom.c GetMaxCapsuleSize capsule storage type is 0x%X\n", (UINTN) CapsuleStorage));
  ...
  )
{
  ...
  ScatterGatherArrayHeader->Crc32 = CalculateCrc32(ScatterGatherArrayHeader,
                                                   sizeof(SCATTERGATHERLIST_ARRAY_HEADER)); 
...
DoneValidating:
  ...
  //Restore SavedCrc32 if something was backedup earlier and original was altered.
  if ((ScatterGatherArrayHeader != NULL) &&
    (SavedCrc32 != ScatterGatherArrayHeader->Crc32) &&
    (SavedCrc32 != (UINT32)(-1))) // Check if default value to identify the saved value
  {
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "Starting to flush any pending persisted capsules from media\n"));
  ...
  )
{
  ...
  CapsuleStorage = GetCapsuleStorageType();
  DEBUG ((DEBUG_INFO, "DxeCapsuleQcom.c GetCapsuleSizeInStorageMedia capsule storage type is 0x%X\n", (UINTN) CapsuleStorage));
...
**/
BOOLEAN
IsMassStorageCapsuleFlagEnabled (VOID
  )
{
  UINT64 *OsIndications = NULL;
  UINTN   Size          = 0;
...
/**
  Mark persisted media capsule processing pending.
This flag will be checked to decide boot mode in MsEnvDxe
...
{
  EFI_STATUS Status = EFI_SUCCESS;
  BOOLEAN     IsCapsulePendingInPersistedMedia = TRUE;
  ...
  if (EFI_ERROR(Status))
  {
    DEBUG ((EFI_D_ERROR, "Failed to set PendingCapsuleProcess variable\n"));
  }
}
