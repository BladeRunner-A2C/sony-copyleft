/** @file
  ...
  Portions Copyright (c) 2016 - 2018, Intel Corporation. All rights reserved.<BR>
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 03/09/23   dnkg    Porting support for ImageCapsuleSupport Variable & security
                    violation check from EDK2 202202.
 02/01/19   rajb    Merged latest EDK2 code changes from MdeModulePkg
 07/17/18   prema   Synced DisplayCapsuleImage and IsNestedFmpCapsule with UDK2018 changes
 01/31/18   prema   Synced DisplayCapsuleImage function implementation from EDK2
                    as RenderBGRTImage function is decpricated.
 05/22/17   gparam  Removed the code that changed the return code from
                    EFI_NOT_FOUND to EFI_NOT_READY.
 04/18/17   gparam  Added UX capsule display code from legacy implementation
 03/24/17   gparam  Memory leak fixes + Additional logs
...
EDKII_FIRMWARE_MANAGEMENT_PROGRESS_PROTOCOL  *gFmpProgress = NULL;
...
/**
  ...
  @param[in] CapsuleFileName In case of capsule loaded from disk, the zero-
  ...
  )
{
  ...
  //
  // Clear gFmpProgress after SetImage() returns
  //
  gFmpProgress = NULL;
  ...
  )
{
  ...
  if (!IsFmpCapsuleGuid(&CapsuleHeader->CapsuleGuid)) {
    return ProcessFmpCapsuleImage ((EFI_CAPSULE_HEADER *)((UINTN)CapsuleHeader + CapsuleHeader->HeaderSize), CapsuleFileName, ResetRequired);
  }
  ...
  //
  // TODO: Check if we need to do this only if FmpCapsuleHeader->EmbeddedDriverCount > 0
  BdsLibConnectAll();
  ...
  for (Index = FmpCapsuleHeader->EmbeddedDriverCount; Index < ItemNum; Index++) {
    ...
    if (EFI_ERROR(Status) ||
      ...
      RecordFmpCapsuleStatus (
        NULL,
        CapsuleHeader,
        Status, // Changed from EFI_NOT_READY. Use status directly as we do not process capsules in multipass as in EDK2 
        Index - FmpCapsuleHeader->EmbeddedDriverCount,
        ImageHeader,
        CapsuleFileName
        );
      DEBUG ((DEBUG_WARN, "Failed to find FMP handle corresponding to the payload %d. Status = %r\n", Index, Status));
      continue;
    }
    ...
    for (Index2 = 0; Index2 < NumberOfHandles; Index2++) {
      if (Abort) {
        ...
        DEBUG ((DEBUG_WARN, "Update ABORTED...\n"));
        continue;
      }
      ...
      if (Status != EFI_SUCCESS) {
        DEBUG ((DEBUG_WARN, "Failed to set the firmware payload %d. Status = %r .NotReady = %d\n", Index, Status, NotReady));
        Abort = TRUE;
      }

      // In EDK2, this is condition is part of the else condition of the
      // error check of SetFmpImageData. We moved it outside as we want device
      // to reset even for a failed update to make sure that we dont continue
      // the boot after a failure where we did a rollback. This is to make sure
      // that the partition contents are picked from the original location when
      // any modules that operate on bootchain partitions (like HLOS) operates
      // on the right partition and not the backup partition that might the one
...
#if 0
  // TODO: Enable if we are implementing gEfiEndOfDxeEventGroupGuid and
  // calling ProcessCapsules after this event is triggered
  if (NotReady) {
  ...
  )
{
  ...
  if (IsFmpCapsule(CapsuleHeader)) {
    ...
    //
    // Qualcomm's FMP implementation does not allow embedded drivers within
    // FMP capsules as Windows desktop devices allow end users to disable
    // secure boot and thus we cannot always rely on secure boot to get the
    // drivers authenticated.
    //
    // So even a valid FMP capsule which contain embedded drivers will be
    // reported as UNSUPPORTED.
    // 
    if (!EFI_ERROR (Status) &&
      (0 != EmbeddedDriverCount)) {
  ...
  )
{
  ...
  if (NULL != CapsuleFileName) {
    DEBUG ((DEBUG_WARN, "Starting to process mass-storage capsule file '%s'!\n", CapsuleFileName));
  }
  ...
  if (IsFmpCapsule (CapsuleHeader)) {
    ...
    Status = ProcessFmpCapsuleImage(CapsuleHeader, CapsuleFileName, ResetRequired);
...
{
  DEBUG((DEBUG_INFO, "DxeCapsuleLibEndOfDxe Event received\n"));
