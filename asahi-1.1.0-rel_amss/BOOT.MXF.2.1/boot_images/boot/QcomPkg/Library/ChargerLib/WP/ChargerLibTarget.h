...
EFI_STATUS ChargerLibTarget_ChargerEnable(UINT8 pmic_index, BOOLEAN Enable);
...
EFI_STATUS ChargerLibTarget_LoadChargerFW(void);
EFI_STATUS ChargerLibTarget_LinkChargerFW(void);
EFI_STATUS ChargerLibTarget_DisconnectChargerFW(void);
...
EFI_STATUS ChargerLibTarget_HandleRecovery(void);
...
EFI_STATUS ChargerLibTarget_EnableFgShipMode(void);

EFI_STATUS ChargerLibTarget_EnablePmicShipMode(BOOLEAN enable);

EFI_STATUS ChargerLibTarget_GetPlatformPwrOnReason(CHARGERLIB_PLATFORM_PWRON_REASON_TYPE *pPonReason);
...
EFI_STATUS ChargerLibTarget_CpdlogGetQbgDump(void);
...
EFI_STATUS ChargerLibTarget_SetReqMaxPwr(CHARGERLIB_PLATFORM_CHGR_MAX_PWR_REQ_CFG_STS_TYPE config);

EFI_STATUS ChargerLibTarget_GetMaxPwrReq(CHARGERLIB_PLATFORM_CHGR_MAX_PWR_REQ_CFG_STS_TYPE *pConfig, UINT8* pNegotiatedPwr);

EFI_STATUS ChargerLibTarget_GetBarrelChargerStatus(BOOLEAN* connected, UINT16* charger_max_voltage_mv, UINT16* charger_max_current_ma);

EFI_STATUS ChargerLibTarget_SetChargerInput(UINT8 pmic_index, UINT16 voltage_mv, UINT16 current_ma);

EFI_STATUS ChargerLibTarget_SetMaxPwrReqSkipChgReset(BOOLEAN b_skip);

EFI_STATUS ChargerLibTarget_GetBatteryType(ChgBattType *pBatteryType);

EFI_STATUS ChargerLibTarget_EnableAicl(UINT8 pmic_index, BOOLEAN enable );

EFI_STATUS ChargerLibTarget_GetActivePortPmicIndex(UINT8* pmicIndex);

EFI_STATUS ChargerLibTarget_GetBatteryEOCStatus(BOOLEAN *p_isEoC);

EFI_STATUS ChargerLibTarget_SetHardReset(void);

EFI_STATUS ChargerLibTarget_EnablePD(BOOLEAN enable);

//EFI_STATUS ChargerLibTarget_ChkChgFwLoadRequired_BattDead(CHARGERLIB_ERROR_ACTION_TYPE *pActionType, chargerlib_batt_status_info* pBattSts, BOOLEAN* b_keepChecking);

//EFI_STATUS ChargerLibTarget_ChkChgFwLoadRequired_BattMissing(CHARGERLIB_ERROR_ACTION_TYPE *pActionType);

BOOLEAN ChargerLibTarget_GetDCINSupported(void);

EFI_STATUS ChargerLibTarget_GetMaxNegotiatedPwr(CHARGERLIB_PLATFORM_CHGR_MAX_PWR_REQ_CFG_STS_TYPE *pConfig, UINT8* pNegotiatedPwr);

EFI_STATUS ChargerLibTarget_SetUsbPDFeatures(void);

EFI_STATUS ChargerLibTarget_SetPowerState(CHARGERLIB_POWER_STATE_TYPE pwr_state);

EFI_STATUS ChargerLibTarget_IsLPMOffmode(BOOLEAN* b_lpm_offmode, BOOLEAN* b_offmode);

EFI_STATUS ChargerLibTarget_GetNumSerialBattery(UINT8* num_serial_battery);

EFI_STATUS ChargerLibTarget_GetBootSOCTh(UINT32* soc_th);

EFI_STATUS ChargerLibTarget_GetTadShutdownDecision(BOOLEAN* tad_shutdown);
