...
/** ASCII Char Hex value for 2 */
#define ADSP_IMAGE_ID                            0x32
#define QCOM_CHARGER_FW_PARTITION_STRING_W       L"modem_a"
#define SUBSYS_NAME_MAX_LENGTH		         32
/*===========================================================================*/
...
EFI_STATUS ChargerLibTarget_GetChargingPath(CHARGERLIB_ATTACHED_CHGR_TYPE *ChargingPath)
{
  ...
  *ChargingPath = CHARGERLIB_ATTACHED_CHGR__NONE;
  ...
  case EfiBatteryChargerQcomPmicSchg:
    ...
    else if(typec_conn_st == EFI_PM_SCHG_TYPEC_CONNECT_MODE_UFP)
    {
      *ChargingPath = CHARGERLIB_ATTACHED_CHGR__USB;
      CHARGER_DEBUG((EFI_D_WARN, "ChargerLib:: %a Vbus Missing but UFP detected\r\n", __FUNCTION__));
    }
    else
    {
...
EFI_STATUS ChargerLibTarget_GetBatteryID(INT32 *pBatteryID)
{
  ...
  *pBatteryID = 7500;
  ...
  CHARGER_DEBUG((EFI_D_ERROR, "ChargerLib:: %a BATT_ID = %d \r\n", __FUNCTION__, *pBatteryID));
...
EFI_STATUS ChargerLibTarget_LoadChargerFW()
{
    ...
    EFIChipInfoIdType ChipId                     = EFICHIPINFO_ID_UNKNOWN;
    PIL_SUBSYS_CFG_TYPE	PilSubsysCfg             = {0};
    CHAR8 AsciiImageName[SUBSYS_NAME_MAX_LENGTH] = {0};
    CHAR16 ImageName[SUBSYS_NAME_MAX_LENGTH]     = {0};
    CHAR16 *PreStr                               = L"full_";
    CHAR16 *PreDtbStr                            = L"full_adsp";


    if (!gEnableChargerFW)
    {
         CHARGER_DEBUG(( EFI_D_ERROR, "ChargerLib:: %a Charger FW not enabled \r\n", __FUNCTION__));
         return Status;
    }
    ...
    if ((EFI_SUCCESS != Status) || (NULL == ChipInfoProtocol))
    {
        CHARGER_DEBUG(( EFI_D_ERROR, "ChargerLib:: %a Locate Chip Info Protocoal Failed %r \r\n", __FUNCTION__, Status));
        return Status;
    }

    /** Set the OS Bootloader Type */
    Status = ChargerLibTarget_SetOS(OS_BOOTLOADER);

    if (EFI_SUCCESS != Status)
    {
        CHARGER_DEBUG(( EFI_D_ERROR, "ChargerLib:: %a Set OS Type Failed %r \r\n", __FUNCTION__, Status));
        return Status;
    }

    /** Mount Partition for Charger FW  */
    Status = MountFatPartition(QCOM_CHARGER_FW_PARTITION_STRING_W);

    if (EFI_SUCCESS != Status)
    {
        CHARGER_DEBUG(( EFI_D_ERROR, "ChargerLib:: %a Mount Partition Failed %r \r\n", __FUNCTION__, Status));
        return Status;
    }

    Status = ChipInfoProtocol->GetChipId(ChipInfoProtocol, &ChipId);

    if (EFI_SUCCESS != Status)
    {
        DEBUG(( EFI_D_ERROR, "ChargerLib:: %a Failed to get chip id %r \r\n", __FUNCTION__, Status));
        return Status;
    }
    
    /** 
     * Check for SM7675 target and load the ADSP2 Image
     * Note: The same code can be used for any ADSP image by modifying the ADSP_IMAGE_ID
     * Pre Image Name: FULL_ADSP*_DTB, * denotes Image ID
     * ADSP Image path: \image\adsp* , * denotes Image ID    
     */
    if (ChipId == EFICHIPINFO_ID_SM_LAMMA)
    {
        /** Copy the Pre string of ADSP Image to the Image Name */
        UnicodeSPrint(ImageName, SUBSYS_NAME_MAX_LENGTH, L"%s", PreStr);
     
        /** Convert to Unicode string of FULL_ADSP to Image Name */
        AsciiStrToUnicodeStrS("adsp", &ImageName[StrnLenS(PreStr, SUBSYS_NAME_MAX_LENGTH)], SUBSYS_NAME_MAX_LENGTH);

        /** Reset the image path in PIL Config */ 
        ZeroMem(PilSubsysCfg.ImagePath,sizeof(PilSubsysCfg.ImagePath));

        /** Get the PIL Handler of ADSP with Unicode name */    
        Status = PILProtocol->PilInitCfgByUnicodeName(ImageName, &PilSubsysCfg);
     
        if (EFI_SUCCESS != Status)
        {
          DEBUG(( EFI_D_ERROR, "ChargerLib:: %a ADSP Failed to get pil cfg of %s \r\n", __FUNCTION__,  PilSubsysCfg.ImagePath));
          return Status;
        }
     
        /** Convert the unicode to ASCII string and store it in PIL Config (FULL_ADSP) */
        UnicodeStrToAsciiStrS(PilSubsysCfg.ImagePath, AsciiImageName, sizeof(PilSubsysCfg.ImagePath));
 
        /** Add the current ADSP Image ID at the end of the string for this variant */    
        PilSubsysCfg.ImagePath[AsciiStrLen(AsciiImageName)] = ADSP_IMAGE_ID;
     
        PilSubsysCfg.ImagePath[AsciiStrLen(AsciiImageName) + 1] = '\0';

        /** Reset the Pre Image Name, default value would be FULL_ADSP_DTB */
        ZeroMem(PilSubsysCfg.PreImage, sizeof(PilSubsysCfg.PreImage));
       
        /** Store the Pre DTB string */ 
        UnicodeSPrint(PilSubsysCfg.PreImage, SUBSYS_NAME_MAX_LENGTH, L"%s", PreDtbStr);
        
        /** Add the Image ID according to ADSP variant */
        PilSubsysCfg.PreImage[StrnLenS(PreDtbStr, SUBSYS_NAME_MAX_LENGTH)] = ADSP_IMAGE_ID;
     
        /** Store the Post DTB string at the end of Pre Image */
        UnicodeSPrint(&PilSubsysCfg.PreImage[StrnLenS(PilSubsysCfg.PreImage, SUBSYS_NAME_MAX_LENGTH)], SUBSYS_NAME_MAX_LENGTH, L"%s", L"_dtb");

        /** Invoke PIL Load with the modified PIL Config */
        Status = PILProtocol->ProcessPilImageExt(&PilSubsysCfg);
    } 
    else
    {
        /** Load the default PIL Config */
        Status = PILProtocol->ProcessPilImage(L"FULL_ADSP");
    }

    CHARGER_DEBUG(( EFI_D_WARN, "ChargerLib:: %a ProcessPilImageExt = %r \r\n", __FUNCTION__, Status));
...
/**
  ...
  EFI_UNSUPPORTED:       No Library function linked
*/
#define POWER_STATE_BATTERY_CHARGING        0x4
EFI_STATUS ChargerLibTarget_GetChargingStateForLed(BOOLEAN *pToggleLed)
{
  ...
  if ((EFI_SUCCESS == ChargerLibTarget_GetIPCStatus(&PmicGLinkStatus)) && (PmicGLinkStatus.IsPMICChannelOpen == TRUE))
  {
    ...
    {
      *pToggleLed = !!(BattStsInfo.PowerState & POWER_STATE_BATTERY_CHARGING);
