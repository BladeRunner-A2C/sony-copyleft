 ...
 *   order.
 *
 *  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/NandHalLib/QpicLib/flash_hal_nandc_predef_bam_init.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 *
...
void hal_nandc_set_memory_region(enum nandc_predef_mem_region_ids id,
  uintnt vaddr, uintnt paddr, uintnt size)
{
...
enum flash_hal_status hal_nand_bam_reg_read_buff_alloc(
  ...
  uint32 ecc_len, uintnt *mem_used)
{
  struct nandc_predef_reg_read_buff *reg_read_buff;
  uintnt temp_addr = 0;
  uintnt read_buff_start = 0, read_buff_size = 0;
  ...
  temp_addr += sizeof(uintnt) * (cw_count + 1);
  ...
  if( flash_qpic_multipage_enabled || flash_qpic_pagescope_enabled)
  {
...
enum flash_hal_status hal_nandc_predef_cmd_blk_buff_alloc(
  struct flash_hal_mem_data *buff_data, uint32 cw_count, uintnt *mem_used)
{
  ...
  if( flash_qpic_multipage_enabled || flash_qpic_pagescope_enabled)
  {
    ...
    /* Allocate memory for Pagescope/Multipage auto status enable register descriptor.
           Memory is allocated for both enable/disable of status. */
...
void hal_nandc_predef_desc_list_init(uint32 cw_count, uint32 ecc_len)
{
  ...
  uintnt *spare_fill_buff = NULL;
  uintnt ptr =  nandc_predef_bam_data.reg_read_buff.multipage_status_buffer;

  spare_fill_buff = 
    (uintnt *)(uintnt)nandc_predef_bam_data.reg_read_buff.spare_fill_buffer;
  ...
  if (IS_AARCH_64)
  {
	for(i = 0; i < ((FLASH_HAL_SPARE_UDATA_LEN_MAX >> 2)/2); i++)
  	{
    	*spare_fill_buff++ = 0xFFFFFFFFFFFFFFFF;
  	}	
  }
  else
  {
...
{
  uintnt i = 0, status_reg_buf_pa, erased_cw_status_reg_buf_pa;
  ...
  op_cmd->data = 0;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_DEV0_CFG0_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;

  op_cmd++;
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->data = 0;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_DEVn_CFG1_PADDR(0));
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  ...
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->data = 0;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_DEV0_ECC_CFG_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  ...
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_ERASED_CW_DETECT_CFG_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  ...
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_READ_LOCATION_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  ...
  { 
    ...
    op_cmd->command = FLASH_BAM_CE_WRITE_OP;
    op_cmd->data = 0x82040000;  // Default Value
    op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_READ_LOCATION_LAST_CW_PADDR);
    op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  }
  ...
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->data = 0;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_ADDR0_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  op_cmd++;
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->data = 0;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_ADDR1_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  ...
  while (i < cw_count)
  {
    ...
    op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_FLASH_STATUS_PADDR);
    ...
    op_cmd->command = FLASH_BAM_CE_READ_OP;
    op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_ERASED_CW_DETECT_STATUS_PADDR);
  ...
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->data = 0;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_FLASH_READ_STATUS_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  ...
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_FLASH_XFR_STEP1_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  op_cmd++;
  
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_FLASH_XFR_STEP2_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  op_cmd++;
  
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_FLASH_XFR_STEP3_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  op_cmd++;
  
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_FLASH_XFR_STEP4_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  op_cmd++;
  
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_FLASH_XFR_STEP5_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  op_cmd++;
  
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_FLASH_XFR_STEP6_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  op_cmd++;
  
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_FLASH_XFR_STEP7_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  op_cmd++;

  /* QPIC NAND CTRL to set boost mode */
  op_cmd->command = FLASH_BAM_CE_WRITE_OP;
  op_cmd->reg_addr = FLASH_ALIGN24(HAL_QPIC_NANDC_NAND_CTRL_PADDR);
  op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;

  /* Used for pagescope only */
  if(flash_qpic_multipage_enabled || flash_qpic_pagescope_enabled)
  {	
    ...
    op_cmd->command = FLASH_BAM_CE_WRITE_OP;
    op_cmd->data = 0x0;
    op_cmd->reg_addr = FLASH_ALIGN24(HWIO_NAND_MULTI_PAGE_CMD_ADDR);
    op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
    ...
    op_cmd->command = FLASH_BAM_CE_WRITE_OP;
    op_cmd->data = 0x90009;
    op_cmd->reg_addr = FLASH_ALIGN24(HWIO_NAND_AUTO_STATUS_EN_ADDR);
    op_cmd->mask = FLASH_BAM_CE_DEFAULT_MASK;
  ...
  struct flash_hal_mem_data *mem_data, uintnt *used_buff_size)
{
  ...
  cmd_desc_vaddr_ptr = (struct flash_hal_op_desc *)(uintnt) buff_vaddr;
  ...
  *used_buff_size = (uintnt)cmd_desc_vaddr_ptr - mem_data->buff_vaddr;
  ...
  op_list->op_list = (uintnt *)(uintnt) cmdptr_mem_data->mem_data.buff_vaddr;
  ...
  struct flash_hal_mem_data *mem_data, uintnt *used_buff_size)
{
  ...
  cmd_list = &nandc_predef_bam_data.cmd_list;
  cmdptr_mem_data = &nandc_predef_bam_data.mem_info.cmd_ptr[NANDC_90_ONFI_ID];
...
{
  uintnt buff_paddr = 0;
  uintnt cw_count = 0, tmp_cw_count = 0, buff_vaddr = 0;
  struct nandc_predef_cmd_list *cmd_list;
  struct nandc_predef_cmdptr_mem_data *cmdptr_mem_data;
  struct flash_hal_predef_op_list *op_list;
  struct flash_hal_op_desc  *cmd_desc_vaddr_ptr;
  ...
  cmd_list = &nandc_predef_bam_data.cmd_list;
  cmdptr_mem_data = &nandc_predef_bam_data.mem_info.cmd_ptr[NANDC_00_EC_READ];
...
enum flash_hal_status hal_nandc_bam_80_10_x1_init(
  ...
  struct flash_hal_mem_data *mem_data, uintnt *used_buff_size)
{
  uintnt cw_count = 0, tmp_cw_count = 0, buff_paddr = 0;
  uintnt buff_vaddr = 0;
  struct nandc_predef_cmd_list *cmd_list;
  struct nandc_predef_cmdptr_mem_data *cmdptr_mem_data;
  struct flash_hal_predef_op_list *op_list;
