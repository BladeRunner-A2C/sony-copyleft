 ...
 *
 * Copyright (c) 2008-2012, 2015, 2019-2020 Qualcomm Technologies, Inc. All Rights Reserved
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/NandHalLib/flash_hal_main.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 *
 ...
 * 02/15/19     sa      Added support for multipage read.
 * 12/03/15     sb      Fix Klocwork error
  ...
  uint32 clk_rate_mhz, uint32 wb_ns)
{
   ...
   */
  uint32 gap = 0;
  gap = (clk_rate_mhz * wb_ns) / 1000;  /* clk_rate_mhz * wb_ns * 0.001 */
  gap = ((gap * 10) + 9) / 10;          /* gap + 0.9 */
  gap = (((gap + 1) & 0xFE) - 2) >> 1;
...
void flash_hal_predef_init(uint32 dev_id, struct flash_hal_predef_init_data
  *init_data, uintnt *used_buff_size, enum flash_hal_status *status)
{
  flash_hal_dev_info  *dev_info;
 ...
 */
uintnt flash_get_ecc_bytes(uintnt *data_buffer, uint8 num_ecc_bytes)
{
  ...
  /* Checking idx to satisfy KW tool */
  ...
  struct flash_hal_predef_page_data *page_data, enum flash_hal_status *status)
{
  ...
  uintnt raw_page_buff = 0, cw_count = 0;
  uintnt value1 = 0, zero_bit_count = 0;
  uintnt *tmp_ecc_read_buff = 0;
  ...
  {
    tmp_ecc_read_buff = (uintnt *)flash_get_ecc_bytes((uintnt *)(raw_page_buff+516),ecc_bytes);
...
/* HAL API to read multiple pages at a time.
    This will be only initialised when multipage feature is enabled. */
  ...
  *status)
{
  ...
  dev_info->ctlr_ops.flash_hal_read_multipage(dev_info,
    page_data, op_list, status); 
}
...
/*
 * Optimized flash HAL Set/Get feature cmd 
 */
void flash_hal_feature_cmd(uint32 dev_id, struct flash_hal_predef_feat_data
  *feat_data, struct flash_hal_predef_op_list **op_list, enum flash_hal_status
  *status)
{
  flash_hal_dev_info  *dev_info;
  ...
  dev_info = &flash_hal_devices[dev_id];
  dev_info->ctlr_ops.flash_hal_feature_cmd(dev_info,
    feat_data, op_list, status);
}
