 ...
 *   order.
 *
 *  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/NandHalLib/QpicLib/flash_hal_nandc.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 *
...
{
  uintnt value = 0, erase_cw_count = 0;
  uint32 list_count = 0;
  ...
  *status = FLASH_HAL_SUCCESS;
         ...
         (*status != FLASH_HAL_FAIL))
  {
    if ((FLASH_HAL_DST_VERIFY == (pp_list->cmd & FLASH_HAL_DST_VERIFY)) &&
        (FLASH_HAL_ERASED_CHECK != (pp_list->cmd & FLASH_HAL_ERASED_CHECK)))
    {
      value = *(uintnt *) (uint64)pp_list->addr;
  ...
  enum flash_hal_status *status)
{
  ...
  */
  ...
  do
  {
    uintnt bytes_copied = 0, align_len = 0, temp_write_addr = 0;
    ...
    if (data->buf_len)
    {
      ...
      if (align_len < data->buf_len)
      {
        ...
        *(uintnt *) temp_write_addr = 0xFFFFFFFF;
