 ...
 *
 * Copyright (c) 2012-2015,2018-2020,2022 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/NandHalLib/QpicLib/flash_hal_nandc_predef_bam_exec.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 *
  ...
  struct flash_hal_predef_op_list **op_list, enum flash_hal_status  *status)
{
  ...
  *status = FLASH_HAL_SUCCESS;
  ...
  /* Set address buffers */
  addr0   =  probe_data->col_offset;
  addr0  |=  probe_data->page << (priv_data->col_count * 8);
  addr1   =  probe_data->page >> ((4 - priv_data->col_count) * 8);
  ...
  struct flash_hal_predef_op_list **op_list, enum flash_hal_status  *status)
{
  ...
  *status = FLASH_HAL_SUCCESS;
  ...
  /* Set address bytes */
  addr0   =  page_data->col_offset;
  addr0  |=  page_data->page << (priv_data->col_count * 8);
  addr1   =  page_data->page >> ((4 - priv_data->col_count) * 8);
  ...
  struct flash_hal_predef_op_list **op_list, enum flash_hal_status  *status)
{
  ...
  *status = FLASH_HAL_SUCCESS;
  ...
  nandc_set_block_addr(priv_data, (uintnt *)&addr0, (uintnt *)&addr1, feat_data->feat_addr, 0, 0);
