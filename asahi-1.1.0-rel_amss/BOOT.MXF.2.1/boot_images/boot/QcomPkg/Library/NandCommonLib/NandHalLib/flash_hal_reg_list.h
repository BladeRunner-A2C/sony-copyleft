 ...
 *              preparation.
 *
 * Copyright (c) 2008-2009, 2012, 2014 ,2020 Qualcomm Technologies, Inc. All Rights Reserved
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/NandHalLib/flash_hal_reg_list.h#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 *
...
/* Standard register read without any options */
#define FLASH_HAL_IN(ctlr, reg, dest)               \
    flash_hal_gen_read_list((uintnt)dest,           \
...
/* Memory Copy Macros  */
#define FLASH_HAL_COPY_TO_DEVICE(addr, src, len)                  \
   flash_hal_gen_read_list((uintnt) addr, (uintnt) src,           \
     FLASH_HAL_SRC_VIRTUAL, len)

#define FLASH_HAL_COPY_FROM_DEVICE(addr, dest, len)               \
   flash_hal_gen_read_list((uintnt)dest,                          \
     (uintnt) addr, FLASH_HAL_DEST_VIRTUAL, len)
