 ...
 * MSMs.
 *
 * Copyright (c) 2008-2012,2014-2015,2017,2019-2020,2022 Qualcomm Technologies, Inc Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/NandHalLib/flash_hal.h#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 *
 * when         who     what, where, why
 *  --------     ---     ---------------------------------------------------
 * 10/19/22      mr      Updated the size of BAM flags & MSB to support 40bit add
...
#define FLASH_ALIGN24(x)    (x & ((1 << 24) - 1))
...
enum flash_hal_status
{
  ...
  FLASH_HAL_CHECK_ECC,                 /**< Check ECC bytes to see if erased page */
  FLASH_HAL_PAGESCOPE_NOT_SUPPORTED,   /**< Pagescope not supported */
  FLASH_HAL_MULTIPAGE_NOT_SUPPORTED,   /**< MultiPage not supported */
...
enum flash_hal_ecc_length
{
  ...
  FLASH_HAL_HW_ECC_16BIT = 16,		 /**< 16 Bit correction */
...
struct flash_hal_op_desc
{
  ...
  uint32 buf_pa_msb :8;  /**< MSB of a 36/40 bit buffer physical address */
  uint32 flags : 8;      /**< Flag bitmask (see bam_iovec_options_type) */
};
...
struct flash_hal_xfer_cfg
{
  ...
  uint32 xfer_boost_mode;        /**< Enable boost mode in QPIC Nand ctrl */
  uintnt *xfer_reg_data;         /**< Transfer step register data buffer*/
};
  
/** Flash Feature Cmd types */
enum flash_hal_feat_cmd_types
{
  FLASH_HAL_FEATURE_SET_CMD,
  FLASH_HAL_FEATURE_GET_CMD,
  FLASH_HAL_FEATURE_MAX,
  FLASH_HAL_FEATURE_MAX_SIZE = 0x7FFFFFFF /* Force 32-bit enum */
};

/** Flash Set (EFh), Get(EEh) Feature operation details */
struct flash_hal_predef_feat_data
{
  enum flash_hal_feat_cmd_types cmd_id;  /**< Feature cmd */
  uintnt feat_addr;                      /**< Feature address */
  uintnt feat_buff_addr;                 /**< Feature data buffer */
};
...
void flash_hal_read_mpages(uint32 dev_id, struct flash_hal_predef_page_data
  *page_data, struct flash_hal_predef_op_list **op_list, enum flash_hal_status
  *status);
...
/**
 *
 * Invoke Set (EFh) or Get (EEh) Feature cmd on ONFI 1.0 based devices.
 * In DMA mode, this function returns list for DMA transfer. For non-DMA, this
 * this actually does the block erase operation.
 *
 * @param dev_id [IN]
 *   -- Logical device ID, as described in \ref flash_hal_dev_id
 ...
 *      FLASH_HAL_FAIL - On fail \n
 */
void flash_hal_feature_cmd(uint32 dev_id, struct flash_hal_predef_feat_data 
  *feat_data, struct flash_hal_predef_op_list **op_list, enum flash_hal_status  *status);
