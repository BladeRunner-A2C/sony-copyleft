 ...
 *
 * Copyright (c) 2011-2012,2014,2016,2020 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *   $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/flash_properties.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 06/08/20     sa      Ported for Olympic.
 * 09/07/16     eo     Default EBI2 clock rate to 100MHz
...
/* Function to set the required target specific gpio */
static flash_config_gpio_tbl flash_config_gpio =
{
  nand_bsp_gpio_control
};
...
uintnt flash_get_property(enum flash_property_id prop_id)
{
  ...
  switch (prop_id)
  {
    ...
    case FLASH_I2C_CONFIG_TABLE_ID :
      return ((uintnt) &flash_i2c_config);
    ...
    case FLASH_NAND_CONFIG_GPIO:
      return ((uintnt)&flash_config_gpio);
