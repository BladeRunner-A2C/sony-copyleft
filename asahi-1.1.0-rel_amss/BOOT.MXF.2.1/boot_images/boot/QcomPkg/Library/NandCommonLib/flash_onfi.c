 ...
 *
 * Copyright (c) 2008-2012, 2015,2018, 2020-2021, 2023 QUALCOMM Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/flash_onfi.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 01/25/23     sh      Changes for pages used for bad block marker as per ONFI compliance
 * 01/07/21     sa      Change DALSYS API to flash api.
 ...
 * 08/03/15     sb      Enhance ONFI parameter page read operation
...
/* ONFI Set and Get Feature command */
#define ONFI_FEATURE_SET_CMD                  0xEF
#define ONFI_FEATURE_GET_CMD                  0xEE
#define ONFI_FEATURE_ADDR_TIMING_MODE         0x01
...
static struct flash_spare_info flash_spare_info_onfi_2048 =
{
  { 
    ...
    {63, 2048, 4}      /* Bad Block Info - Page 63 */
#endif
  },
...
#if !defined(BUILD_RUMI_SLOW_NAND)
    {63, 4096, 4}      /* Bad Block Info - Page 63 */
#endif
  },
 ...
 */
static int nand_feature_cmd(flash_client_ctxt *client_ctxt, uint32 feat_cmd,
   uint32 feat_addr, void *data, uint32 num_bytes)
{
  uint32 hal_id = 0;
  ...
  feat_data.feat_addr = feat_addr;
  feat_data.feat_buff_addr = (uintnt)flash_vtop_get_physical((uintnt)nand_pdata->raw_page_buffer);

  flash_dcache_inval(feat_data.feat_buff_addr, num_bytes);	

  if (feat_cmd == ONFI_FEATURE_SET_CMD)
  {
    feat_data.cmd_id = FLASH_HAL_FEATURE_SET_CMD;
    flash_memscpy(nand_pdata->raw_page_buffer, num_bytes, data, num_bytes);
  }
  else if (feat_cmd == ONFI_FEATURE_GET_CMD)
  {
    feat_data.cmd_id = FLASH_HAL_FEATURE_GET_CMD;
  }
  flash_hal_feature_cmd(hal_id, &feat_data, &op_list, &hal_status);
  ...
  {
    if (feat_cmd == ONFI_FEATURE_GET_CMD)
    {
      flash_memscpy(data, num_bytes, nand_pdata->raw_page_buffer, num_bytes);
    }
  }
...
retry:
  ...
  else
  {
    ...
    flash_memcpy(buffer, nand_pdata->raw_page_buffer, num_bytes);
  }

  return status;
}

void  nand_get_onfi_timing_mode_internal(flash_client_ctxt *client_ctxt, struct flash_nand_params *param)
{
   uint16 timing_mode_msk = 1;
   uint32 dev_timing_mode = 0;

   param->timing_mode = 0;
   while ((static_onfi_parameters->timing_mode & (timing_mode_msk <<= 1)))
   {
     param->timing_mode++;
   }
   dev_timing_mode = param->timing_mode;
   nand_feature_cmd(client_ctxt, ONFI_FEATURE_SET_CMD, ONFI_FEATURE_ADDR_TIMING_MODE,
     (void *)&dev_timing_mode, 4);
   dev_timing_mode = 0;
   nand_feature_cmd(client_ctxt, ONFI_FEATURE_GET_CMD, ONFI_FEATURE_ADDR_TIMING_MODE,
     (void *)&dev_timing_mode, 4);

   if (dev_timing_mode != param->timing_mode)
   {
     FLASHLOG(3,("ONFI Set Features timing mode mismatch: Rd Pg Param: %d, Dev time mode %d\n",
       param->timing_mode, dev_timing_mode));
   }
}
...
{
  int    idx, status = NAND_DEVICE_FAIL;
  uint32 onfi_id;
  ...
  *is_onfi = FALSE;
  ...
  if ((NAND_DEVICE_DONE == status) && (0x49464E4F == onfi_id))
  {
    status = flash_malloc(sizeof(struct flash_nand_params),
      (void **)device_params);

    if (NAND_DEVICE_DONE == status)
    {
      status = flash_malloc((ONFI_PARAMETER_PAGE_SIZE * ONFI_READ_PARAMETER_PAGE_MAX),
        (void **)&onfi_parameter_page);

      if (NAND_DEVICE_DONE != status)
      {
        flash_free((void *)(*device_params));
      }
    }
      ...
      if (NAND_DEVICE_DONE == status)
      {
              ...
              (*is_onfi == FALSE))
        {
          ...
          if (computed_crc == static_onfi_parameters->integrity_crc)
          {
            ...
            /* Get the timing mode information for the onfi device */
            nand_get_onfi_timing_mode(client_ctxt, param);
