 ...
 *
 * Copyright (c) 2012-2013,2018-2020 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/flash_bam_wrapper.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 08/11/20     sa      Relocated flash_bam_init_done.
 * 06/08/20     sa      Ported for Olympic.
 * 09/20/19     sa      Clear flash_bam_init_done during bam deinit.
...
//#define QPIC_QPIC_XPU3_BASE  0x00
...
bam_handle flash_bam_status_handle;
...
struct flash_osal_meminfo desc_mem_info;
uint32 flash_bam_init_done = 0;

int flash_bam_init(uint32 qpic_version)
{
  ...
  {
    desc_fifo_size = desc_fifo_size + FLASH_BAM_STATUS_DESC_FIFO_SZ;
  }

  /* Allocate a physically contiguous buffer for Descriptor FIFO - check 
     for 8 byte alignment */
  if (flash_memregion_alloc(&desc_buf_handle, desc_fifo_size))
  {
    desc_buf_handle = NULL;
    return FLASH_BAM_FAIL;
  }
  flash_memregion_info(desc_buf_handle, &desc_mem_info);

  /* Mandatory fields to be supplied by the client */
  flash_bam_cfg.bam_pa = (uint32)NAND_BAM_BASE_PHY_ADDR; 
  flash_bam_cfg.bam_va = (uint32)NAND_BAM_BASE_ADDR;   
 ...
 /* NAND Command Pipe Initialization */ 
  flash_bam_pipe_cfg.desc_base_va = desc_mem_info.Info.VirtualAddr;
  flash_bam_pipe_cfg.desc_base_pa = desc_mem_info.Info.PhysicalAddr;
 ...
 /* NAND Write Pipe Initialization */ 
  ...
  if(flash_qpic_multipage_enabled || flash_qpic_pagescope_enabled)
  {
    /* NAND STATUS Pipe Initialization */
    flash_bam_pipe_cfg.desc_base_va += FLASH_BAM_READ_DESC_FIFO_SZ;
    flash_bam_pipe_cfg.desc_base_pa += FLASH_BAM_READ_DESC_FIFO_SZ;
    flash_bam_pipe_cfg.desc_size = FLASH_BAM_STATUS_DESC_FIFO_SZ;
    flash_bam_pipe_cfg.dir = BAM_DIR_PRODUCER;
  
    if (result == FLASH_BAM_SUCCESS)
    { 
      flash_bam_status_handle =
        flash_bam_pipe_init(NAND_BAM_STATUS_PIPE, &flash_bam_pipe_cfg);
      if(flash_bam_status_handle == NULL)
      {
        result = FLASH_BAM_FAIL;
		goto exitfunction;
      }
  ...
  struct flash_bam_xfer_desc_list *status_desc_list)
{
  int result = FLASH_BAM_SUCCESS;
    ...
    result =  flash_bam_poll_pipe(flash_bam_cmd_handle, 
     (void *)(uintnt *)cmd_desc_list->desc_count);
  ...
  if ((status_desc_list != NULL) && (status_desc_list->desc_count) &&
		(result == FLASH_BAM_SUCCESS))
  {
    result = flash_bam_poll_pipe(flash_bam_status_handle,
		  (void *)status_desc_list->desc_count);
  }
  return result;
}
  ...
  struct flash_bam_xfer_desc_list *status_desc_list)
{
  ...
  /* If the status data descriptor is present then queue it to the status pipe */
  if ((status_desc_list != NULL) && (status_desc_list->desc_count) &&
		  (result == FLASH_BAM_SUCCESS) )
  {
    result =  flash_bam_queue_desc(flash_bam_status_handle, status_desc_list);
  }
  ...
  if (result == FLASH_BAM_SUCCESS)
  {
    result = flash_bam_wait_for_xfer_completion (cmd_desc_list, read_desc_list,
      write_desc_list, status_desc_list);
  }
...
int flash_bam_deinit(void)
{
  ...
  /* Set flash_bam_init_done to 0, so that bam can be initialized again */
  flash_bam_init_done = 0;
  ...
  /* deinitialize STATUS pipe */
  if (flash_bam_pipe_deinit(flash_bam_status_handle) != FLASH_BAM_SUCCESS)
  {
    ret_status = FLASH_BAM_FAIL;
  }
  ...
  if (flash_bam_handle != NULL)
  {
    /* Set disable_bam parameter to 0 to NOT disable BAM H/W */
    if (bam_deinit(flash_bam_handle, 0) != BAM_SUCCESS)
    {
      ret_status |= FLASH_BAM_FAIL;
    }
  }
  ...
  if(desc_buf_handle)
  {
    flash_free_memregion(desc_buf_handle);
    desc_buf_handle = NULL;
  }
