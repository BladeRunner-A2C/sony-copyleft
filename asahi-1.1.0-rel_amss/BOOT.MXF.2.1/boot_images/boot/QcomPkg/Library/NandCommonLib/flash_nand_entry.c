 ...
 *
 * Copyright (c) 2008-2013,2015, 2019-2020, 2023 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/flash_nand_entry.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 02/27/23     sh      Added memory deallocation handling in probe.
 * 10/27/20     sa      Added function to set target specific gpio.
 * 08/10/20     sa      Remove malloc for block erase status.
 * 06/20/20     sa      Added variable to save physical_start_block.
 * 06/04/20     sa      Moved from flash.h to nand_api.h
 * 05/07/19     sa      Removed unused profiling API's.
 * 02/15/19     sa      Seperated R/W count to support pagescope/multipage.
...
#define INIT_BUF_DESC(desc, iovec, data, sdata)                                \
      ...
      desc[NAND_ENTRY_MAIN_BUF_DESC].VirtualAddr = (uintnt)data;               \
      ...
      desc[NAND_ENTRY_SPARE_BUF_DESC].VirtualAddr= (uintnt)sdata;
...
static int nand_entry_mem_deinit(void)
{
  ...
  {
    if (flash_free_memregion(meminfo->handle_hal_mem))
    {
  ...
  {
    if (flash_free_memregion(meminfo->handle_drv_mem))
    {
      result = NAND_DEVICE_FAIL;
    }
  }
...
int nand_entry_mem_alloc(uint32 hal_mem_size, uint32 dal_mem_size)
{
  ...
  {
    if (flash_memregion_alloc(&meminfo->handle_hal_mem, hal_mem_size))
    {
    ...
    else
    {
       ...
       * information */
      flash_memregion_info(meminfo->handle_hal_mem,
        &meminfo->hal_mem_info);
	  flash_memset((void *)meminfo->hal_mem_info.Info.VirtualAddr, 0xFF, 10); 
    }
  ...
  if ((NAND_DEVICE_DONE == result) && dal_mem_size)
  {
    ...
    if (flash_memregion_alloc(&meminfo->handle_drv_mem, dal_mem_size))
    {
    ...
    else
    {
      /* If the allocation is success, get the memory address
       * information */
      flash_memregion_info(meminfo->handle_drv_mem,
  ...
  else
  {
    ...
    flash_vtop_add_item(
      meminfo->hal_mem_info.Info.VirtualAddr,
      meminfo->hal_mem_info.Info.PhysicalAddr,
      meminfo->hal_mem_info.Info.dwLen);

    flash_vtop_add_item(
      meminfo->drv_mem_info.Info.VirtualAddr,
      meminfo->drv_mem_info.Info.PhysicalAddr,
...
static int nand_entry_init(flash_client_ctxt *client_ctxt)
{
  uint32 temp_val;
  int result;
  flash_config_gpio_tbl *gpio_tbl;
  
  /* Set the required GPIo is needed */
  if( NULL != (gpio_tbl = (flash_config_gpio_tbl *)flash_get_property(FLASH_NAND_CONFIG_GPIO)));
  {
    gpio_tbl->set_gpio_config();
  }
  ...
  {
    /* TODO write issue to the variables */    
    nand_entry_init_data.multi_read_support_count = FLASH_MULTI_R_SUPPORT_COUNT;
    nand_entry_init_data.multi_write_support_count = FLASH_MULTI_W_SUPPORT_COUNT;

    /* Set the wait function to be used by Flash HAL */
    flash_hal_set_property(FLASH_HAL_PROPERTY_WAIT_CFG,
      (void *)&flash_busy_wait);

    //TODO need to be uncached
