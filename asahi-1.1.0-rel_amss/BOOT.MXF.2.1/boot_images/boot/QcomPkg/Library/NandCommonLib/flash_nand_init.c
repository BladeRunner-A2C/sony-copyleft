 ...
 *
 * Copyright (c) 2008-2015, 2017, 2019-2020 by QUALCOMM Technologies, Inc.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/flash_nand_init.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 *
 ...
 * 07/09/20     sa      Save result for nand_retrieve_mibib.
 * 06/04/20     sa      Moved from flash.h to nand_api.h
 * 09/25/19     sa      Read mibib from SMEM/NAND based on 'mibib_from_nand'.
...
extern flash_driver_ctxt_t flash_internal_clients;
...
static int nand_mem_init(flash_client_ctxt *client_ctxt)
{
  ...
  if (TRUE == nand_entry_init_data.nand_mem_initialized)
  {
    ...
    hal_ws_buffer = (uint32) meminfo->hal_mem_info.Info.VirtualAddr;
    hal_ws_buffer_size = meminfo->hal_mem_info.Info.dwLen;
    ...
    /* Set up the driver buffers - Temperorly for probing the device.
       ...
       Size of this buffer will be equal to nand page size.*/
    nand_pdata->raw_page_buffer =
      (uint8 *) meminfo->drv_mem_info.Info.VirtualAddr;

    nand_pdata->drv_ws_buffer =
      (uint32 *) (meminfo->drv_mem_info.Info.VirtualAddr +
      dev_ctxt->dev_info.total_page_size_bytes);

    flash_memset(nand_pdata->raw_page_buffer, 0xFF,
      meminfo->drv_mem_info.Info.dwLen);
...
{
  int result = NAND_DEVICE_FAIL;
  uintnt hal_id = 0 , hal_generic_mem_size = 0;
  uintnt hal_mem_size = 0, dal_mem_size = 0;

  hal_id = client_ctxt->client_data.hal_id;

  /* Get the memory numbers from HAL */
  flash_hal_get_predef_mem_usage (hal_id,
    nand_entry_init_data.multi_write_support_count, &hal_mem_size);
...
static int nand_mem_deinit(flash_client_ctxt *client_ctxt)
{
  ...
  flash_free(nand_pdata);
  client_ctxt->flash_dev_ctxt->dev_info.dev_specific_data = 0;
...
{
  uintnt used_buff_size;
  int    status = NAND_DEVICE_FAIL;
  struct flash_osal_meminfo *hal_mem_info, *drv_mem_info;
  ...
  predef_init_data.buff_data.buff_vaddr = hal_mem_info->Info.VirtualAddr;
  predef_init_data.buff_data.buff_paddr = hal_mem_info->Info.PhysicalAddr;
  predef_init_data.buff_data.buff_size = hal_mem_info->Info.dwLen;

  predef_init_data.raw_buff_data.buff_vaddr = drv_mem_info->Info.VirtualAddr;
  predef_init_data.raw_buff_data.buff_paddr = drv_mem_info->Info.PhysicalAddr;
  predef_init_data.raw_buff_data.buff_size = drv_mem_info->Info.dwLen;
 ...
 * Close a NAND client
 */
int nand_init_close(flash_client_ctxt *client_ctxt)
{
  int     result = NAND_DEVICE_DONE;
  ...
  if (client_ctxt->client_data.client_specific_data)
  {
    ...
    flash_free(client_ctxt->client_data.client_specific_data);
  ...
  uint32 hal_id)
{
  ...
  else if ((uint32)devices->feature_flags1.ecc <= FLASH_HAL_HW_ECC_16BIT)
  {
    /* Found a match. Configure */
    nand_ecc_len = FLASH_HAL_HW_ECC_16BIT;
  }
  else
  {
    FLASHLOG(3, ("Device needs more ECC bitlength than supported\n"));
    return NAND_DEVICE_NOT_SUPPORTED;
  }
...
static int nand_init_client_info(flash_client_ctxt *client_ctxt)
{
  ...
  client_ctxt->client_data.pages_per_block =
    dev_ctxt->dev_info.pages_per_block;
  client_ctxt->client_data.page_size_bytes =
    dev_ctxt->dev_info.page_size_bytes;
  ...
  {
    case NAND_ECC_MAIN_SPARE_ENABLED:
      {
  ...
  if (client_ctxt->client_data.client_specific_data == 0)
  {
    ...
    if (NAND_DEVICE_DONE != flash_malloc(sizeof(struct nand_client_specific_data),
    ...
    flash_memset((void *) nand_client_data, 0x0,
...
int nand_deinit (flash_client_ctxt *client_ctxt)
{
  ...
  if (client_ctxt->flash_dev_ctxt->dev_info.dev_specific_data)
  {
    flash_free(client_ctxt->flash_dev_ctxt->dev_info.dev_specific_data);
    client_ctxt->flash_dev_ctxt->dev_info.dev_specific_data = NULL;
  }

  if (client_ctxt->flash_dev_ctxt->parti_ptr)
  {
    /* Free any malloced extra data for client */
    flash_free(client_ctxt->flash_dev_ctxt->parti_ptr);
    client_ctxt->flash_dev_ctxt->parti_ptr = NULL;
  }

  if (runtime_device && nand_is_onfi)
  {
    flash_free(runtime_device);
    runtime_device = NULL;
    nand_is_onfi = FALSE;
  }
  ...
  struct flash_nand_params *device)
{
  ...
  if (dev_ctxt->dev_info.dev_specific_data == NULL)
  {
    if (NAND_DEVICE_DONE != flash_malloc(sizeof(struct nand_dev_pdata),
      (void ** )&dev_ctxt->dev_info.dev_specific_data))
    {
      FLASHLOG(3, ("\nMalloc failed\n"));
  ...
  dev_ctxt->dev_info.device_name    = device->device_name;

  dev_ctxt->dev_info.maker_id    = device->dev_ids[0];
  dev_ctxt->dev_info.device_id   = device->dev_ids[1];
  dev_ctxt->dev_info.version_id  = device->dev_ids[2];
  dev_ctxt->dev_info.block_count = device->block_count;

  dev_ctxt->dev_info.pages_per_block = device->pages_per_block;

  dev_ctxt->dev_info.page_size_bytes = device->page_size;
  dev_ctxt->dev_info.total_page_size_bytes = device->page_size +
    device->spare_size;
  dev_ctxt->dev_info.flags1  = *((uint32 *)&device->feature_flags1);
  dev_ctxt->dev_info.flags2  = *((uint32 *)&device->feature_flags2);
  ...
  struct flash_nand_params *device)
{
  ...
  page_size = dev_ctxt->dev_info.page_size_bytes;
  ...
  nand_pdata->raw_page_buffer =
    (uint8 *) flash_mem_data->drv_mem_info.Info.VirtualAddr;

  nand_pdata->drv_ws_buffer =
    (uint32 *) (flash_mem_data->drv_mem_info.Info.VirtualAddr +
    dev_ctxt->dev_info.total_page_size_bytes);
  ...
  *device)
{
  ...
  if (NAND_DEVICE_DONE != nand_configure_device_ctxt(client_ctxt, device))
  {
    result = NAND_DEVICE_FAIL;
    return result;
  }

  if (NAND_DEVICE_DONE != nand_configure_pdata(client_ctxt, device))
  {
    result = NAND_DEVICE_FAIL;
    return result;
  }

  if (NAND_DEVICE_DONE != nand_configure_hal_device_info(device,
...
int nand_set_xfer_steps_internal(flash_client_ctxt *client_ctxt, struct flash_nand_cfg_data *nand_cfg, struct flash_nand_params *device)
{
  ...
  if (device != NULL)
  {
    ...
    if (device->dev_xfer_steps_cfg != NAND_MAX_XFER_CFG)
    {
      ...
      const struct nand_xfer_steps_val *xfer_steps =
        nand_cfg->ebi2_cfg->xfer_steps[device->dev_xfer_steps_cfg];
      uint16 timing_mode = 0;

      timing_mode = (device->timing_mode < FLASH_MAX_TIMING_MODES)? 
                      device->timing_mode : (FLASH_MAX_TIMING_MODES - 1);

      if (xfer_steps)
      {
	  	if (sizeof(uintnt) == sizeof(uint32))
		{
		  cfg_data.xfer_reg_count  = (uint16)xfer_steps->xfer_count;
	  	}
		else
        {
		  cfg_data.xfer_reg_count  = (uint32)xfer_steps->xfer_count;
        }
		if ( (timing_mode >= 4) && (FLASH_XFER_BOOST_MODE_DISABLED == FALSE) )
        {
          cfg_data.xfer_boost_mode = 1;
          cfg_data.xfer_reg_data = (uintnt *)&xfer_steps->xfer_data[FLASH_MAX_TIMING_MODES - 1][0];
        }
        else
        {
          cfg_data.xfer_boost_mode = 0;
          cfg_data.xfer_reg_data = (uintnt *)&xfer_steps->xfer_data[timing_mode][0];
        }
 ...
 * parameters
 */
struct flash_nand_params* nand_get_device_nand_params(
  ...
  struct flash_nand_cfg_data *nand_cfgs, int cfg_count )
{
  ...
  /* Set the xfer steps  This will modify the device parameter.*/
  nand_set_xfer_steps(client_ctxt, &nand_cfgs[cfg_count], device );
...
static int nand_intialize_primary_hal_device(flash_client_ctxt *client_ctxt,
   uint8** ids)
{
  int     result = NAND_DEVICE_DONE, cfg_count;
  ...
  {
    FLASHLOG(3, ("\nNo NAND device in Config Table\n"));
    result = NAND_DEVICE_NOT_SUPPORTED;
    return result;
  }
  ...
  if (FLASH_HAL_SUCCESS != hal_status)
  {
    FLASHLOG(3, ("\nFailed to initialize flash HAL client, HAL_ERROR = %d\n", hal_status));
    result = NAND_DEVICE_NOT_SUPPORTED;
    return result;
  }
  ...
  if (NAND_DEVICE_DONE != nand_predef_init(client_ctxt))
  {
  ...
  *ids = (uint8 *) nand_pdata->raw_page_buffer;
  ...
  buff_desc.buff_vaddr = (uintnt) nand_pdata->raw_page_buffer;
  ...
  if (NAND_DEVICE_DONE != nand_read_id(hal_id, &buff_desc))
  {
    result = NAND_DEVICE_FAIL;
    goto exitfunction;
  }
...
{
  int     result = NAND_DEVICE_NOT_SUPPORTED;
  ...
  *********************************************************************************/
  ...
  client_ctxt->flash_dev_ctxt->dev_info.dev_specific_data = nand_pdata;

  /* Initialize Memory */
  if (NAND_DEVICE_DONE != nand_mem_init(client_ctxt))
  {
    FLASHLOG(3, ("\nFlash Memory initialization failed!\n"));
    result = NAND_DEVICE_NO_MEM;
    return result;
  }
 ...
 /* The primary device hal_id is used for nand ctlr 0  using chip select 0
  ...
  if ((FLASH_DMA_ENABLED) && (NAND_DEVICE_DONE == result))
  {
  ...
  if (NAND_DEVICE_DONE == result)
  {
    ...
    if (NAND_DEVICE_DONE == result)
    {
      result = smem_retrieve_mibib(client_ctxt);

	  if(result == NAND_DEVICE_FAIL)
	  {
	    result = nand_retrieve_mibib(client_ctxt);
	  }

	  /* This is used to retrieve mibib in case of mibib update from fastboot */
      if ((NAND_DEVICE_FAIL == result) || 
	  (TRUE == flash_internal_clients.mibib_from_nand))
        {
          result = nand_retrieve_mibib(client_ctxt);
	  flash_internal_clients.mibib_from_nand = FALSE;
        }
    }
  }
