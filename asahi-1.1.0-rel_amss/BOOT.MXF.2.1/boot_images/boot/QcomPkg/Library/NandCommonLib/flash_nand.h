 ...
 *
 * Copyright (c) 2008-2012, 2015, 2020 by QUALCOMM Technologies, Inc.
 * All Rights Reserved.
 ...
 * order.
 *
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/NandCommonLib/flash_nand.h#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
 *
 *
...
#define NAND_LP_TO_PP(client_ctxt, lp)                                      \
  ((NULL == ((struct nand_dev_pdata *)                                      \
   (client_ctxt->flash_dev_ctxt->dev_info.dev_specific_data))->lp_to_pp_lookup) ?\
  ...
  ((((lp >> client_ctxt->client_data.block_size_shift) +                    \
   ...
   (client_ctxt->flash_dev_ctxt->dev_info.dev_specific_data))->lp_to_pp_lookup)[(lp &\
...
#define GET_PDATA(client_ctxt)   ((struct nand_dev_pdata *) \
  client_ctxt->flash_dev_ctxt->dev_info.dev_specific_data)
...
int nand_set_xfer_steps_internal(flash_client_ctxt *client_ctxt, struct flash_nand_cfg_data *nand_cfgs, struct flash_nand_params *device);

/* Configure timing mode parameter */
