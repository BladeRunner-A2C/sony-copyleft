...
/*============================================================================
  ...
  DEPENDENCIES: None
                ...
                Copyright (c) 2012-2015, 2018, 2020 Qualcomm Technologies, Inc.
                All Rights Reserved.
...
/*============================================================================
  ...
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/adc/devices/vadc/src/VAdcLog.c#1 $$DateTime: 2024/04/02 23:00:31 $$Author: cirrusp4svc $

  when        who  what, where, why
  ----------  ---  -----------------------------------------------------------
  2018-01-31  jjo  Log ADC result to F3 buffer as well as QXDM.
   ...
   )
{
   ...
   MSG_5(MSG_SSID_ADC,
                 MSG_LEGACY_MED,
                 "VADC conv result eStatus %i, nPhysical %i, uPercent %u, uMicrovolts %u, uCode %u",
                 pResult->eStatus,
                 pResult->nPhysical,
                 pResult->uPercent,
                 pResult->uMicrovolts,
   ...
   // This message will not get logged in the F3 buffer - strings are not supported
...
VAdc_LogDumpRegisters(
   VAdcDebugInfoType *pVAdcDebugInfo,
   struct pm_vadc_reg_dump *pVAdcRegDump
   )
{
   int32 regsize;
   uint32 uReg;

	regsize = pVAdcRegDump->reg_size;
	if (regsize <= 0) 
	{
   	MSG(MSG_SSID_ADC, MSG_LEGACY_ERROR, "VADC register dump error");
		return;
	}

   MSG(MSG_SSID_ADC, MSG_LEGACY_ERROR, "VADC register dump");

   for (uReg = 0; uReg < regsize; uReg++)
   {
      MSG_2(MSG_SSID_ADC,
            MSG_LEGACY_ERROR,
            "Offset = %x, Contents = %x",
            pVAdcRegDump->vadc_regs[uReg].offset,
            pVAdcRegDump->vadc_regs[uReg].val);
   }
