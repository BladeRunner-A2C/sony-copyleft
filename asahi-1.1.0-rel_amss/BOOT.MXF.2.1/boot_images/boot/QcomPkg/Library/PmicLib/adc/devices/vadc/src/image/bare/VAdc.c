/*============================================================================
  ...
  OVERVIEW:     Implementation of a Physical ADC device for the
                PMIC VADC peripheral.

  DEPENDENCIES: None

                Copyright (c) 2009-2016, 2018-2023 Qualcomm Technologies, Inc.
                All Rights Reserved.
...
/*============================================================================
  ...
  $Header: //components/dev/core.boot/6.1/hadigal.core.boot.6.1.kai_pmic_adc_boot_08/QcomPkg/Library/PmicLib/adc/devices/vadc/src/DalVAdc.c#1 $$DateTime: 2022/08/10 11:40:30 $$Author: hadigal $

  when        who  what, where, why
  ----------  ---  -----------------------------------------------------------
  2022-08-01  ha   Add DAL independent VADC device layer
...
/*----------------------------------------------------------------------------
 * The following functions are declared in AdcDevice Interface.
 * -------------------------------------------------------------------------*/
AdcResult
VAdc_DeviceInit(VAdcClientCtxt *pCtxt)
{
   ...
   {
      VAdc_LogError(&pDevCtxt->debug, "VADC_ERR: pmic info", TRUE);
      return DAL_ERROR;
   }
#endif
   ...
   if (pmResult != PM_ERR_FLAG_SUCCESS)
   {
      VAdc_LogError(&pDevCtxt->debug, "VADC_ERR: HAL device_init", TRUE);
      return DAL_ERROR;
   }
   ...
   for (uGpioMappingIdx = 0; uGpioMappingIdx < pDevCtxt->pBsp->uNumGpioChannelMappings; uGpioMappingIdx++)
   {
      ...
      if (pmResult != PM_ERR_FLAG_SUCCESS)
      {
         VAdc_LogError(&pDevCtxt->debug, "VADC_ERR: GPIO cfg", TRUE);
         return DAL_ERROR;
      }
   }
#endif
...
/*----------------------------------------------------------------------------
 * The following functions are extended in AdcDevice Interface.
 *
 * These functions are documented in AdcDevice.h
 * -------------------------------------------------------------------------*/
AdcResult
