...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/prm/src/pm_prm_process_rsrc.c#1 $ 
                     ...
                     uint32             *curr_msg_id)
{
  ...
  if(client == NULL || resource == NULL || curr_msg_id == NULL)
  {
    PM_LOG_MSG_ERR_FATAL("client = %d, resource = %d, msg_id = %d", 
                          client, resource, curr_msg_id);
  }
  ...
  do
  {
    ...
    /* If rsrc is xob controlled and key is not enable, do not aggregate
     * Also if address of resource is zero from command DB, do not aggregate 
     */
    if((!(rsrc_data->rpmh_data.vrm_ctrl_type == PM_PRM_VRM_XOB_CTRL &&
        rsrc_data->rsrc_key != PM_RSRC_KEY_ENABLE)) && 
        (rsrc_data->rpmh_data.address != 0))
    {
      rsrc_data->pam_req = TRUE;
      ...
      if((rsrc_data->rpmh_data.vrm_ctrl_type == PM_PRM_VRM_LOCAL_CTRL ||
         rsrc_data->ctrl_type == PM_PRM_LOCAL) && 
         (rsrc_data->rpmh_data.dirty_active == TRUE))
      {
        pm_prm_os_set_local_device(rsrc_data);
      }
      else
      {
      ...
      */
      rsrc_data->rpmh_data.dirty_active=FALSE;
      rsrc_data->rpmh_data.dirty_sleep=FALSE;
      rsrc_data->pam_req = FALSE;
    }
  ...
  pm_rpmh_post_set(&active_set, &sleep_set, &rsrc_data->rpmh_data, 
