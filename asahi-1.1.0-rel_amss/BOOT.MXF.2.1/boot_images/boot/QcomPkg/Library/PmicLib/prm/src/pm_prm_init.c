 ...
 *  
 * Copyright (c) 2016, 2022 QUALCOMM Technologies, Inc. (QTI). All Rights Reserved.  
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/prm/src/pm_prm_init.c#1 $ 
$Author: cirrusp4svc $
...
npa_resource_definition   *pm_prm_pam_rsrc = NULL;
...
pm_prm_init(void)
{
  ...
  pm_malloc(sizeof(npa_resource_definition)*(*num_of_pam_nodes),
           (void**)&pm_prm_pam_rsrc);

  if(pm_prm_pam_rsrc == NULL)
  {
  ...
  for(i = 0; i< (*num_of_pam_nodes); i++)
  {
    ...
    pm_prm_pam_rsrc[i].name             = node_rsrc->group_name;
    pm_prm_pam_rsrc[i].units            = "ModeID";
    pm_prm_pam_rsrc[i].max              = node_rsrc->max_mode;
    pm_prm_pam_rsrc[i].attributes       = node_rsrc->resource_attributes;
    pm_prm_pam_rsrc[i].data             = node_rsrc->resource_data;
    pm_prm_pam_rsrc[i].query_fcn        = pm_prm_query_fcn;
    ...
    pm_prm_os_set_pam_plugin (&pm_prm_pam_nodes[i], &pm_prm_pam_rsrc[i]);
  }

  //Register PMIC Client Layer Nodes and Resources with NPA
  for (i = 0; i < (*num_of_pam_nodes); i++)
  {
    pm_prm_pam_nodes[i].resources = &pm_prm_pam_rsrc[i];
    
    if (bPMICStub == TRUE)
    {
      npa_stub_resource(pm_prm_pam_rsrc[i].name);
    }
    else
    {
  ...
  for (i = 0; i < pm_prm_rsrc_data->num_rsrcs; i+=num_of_rsrcs, node_num++)
  {
    ...
    for(j=0; j<num_of_rsrcs; j++)
    {
      ...
      if (err_flag != PM_ERR_FLAG_SUCCESS)
      {
        ...
        PM_LOG_MSG_ERR_FATAL("Invalid cmd_db data for %a",
