 ...
 *
 * Copyright (c) 2018-2022 QUALCOMM Technologies, Inc. (QTI). All Rights Reserved.
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/prm/src/pm_prm_os.c#1 $
$Author: cirrusp4svc $
...
pm_prm_os_get_vrm_data(pm_prm_rsrc_data_type *rsrc_data)
{
  ...
  if( rsrc_data->rpmh_data.address == 0 )
  {
    ...
    if(pmic_model != PMIC_IS_UNKNOWN && pmic_model != PMIC_IS_INVALID)
    {
      ...
      PM_LOG_MSG_INFO("Error Flag: %d, RPMh Address for %a not found in cmd_db", err_flag, rsrc_data->cmd_db_name);
    }
  ...
  switch(cmd_db_version & 0xff00)
  {
    ...
    case 0x100:
      ...
      {
        err_flag |= PM_ERR_FLAG_FAILURE;
        PM_LOG_MSG_INFO("Error_flag:%d, Rpmh Aux Data Failure for %a", err_flag, rsrc_data->cmd_db_name);
      }
      break;
                          ...
                          npa_resource_definition *rsrc )
{
  ...
  for(j = 0; j < node_rsrc->dependency_count; j++)
  {
    ...
    {
      PM_LOG_MSG_ERR_FATAL("Unable to get query handle for rsrsc %a",
...
pm_prm_os_set_clk(pm_prm_rsrc_data_type *rsrc_data)
{
  ...
  {
    case PM_RSRC_KEY_ENABLE:
      err_flag |= pm_clk_sw_enable(PM_SPMI_BUSID_0, pmic_chip, clk_index,
      ...
      if ((pm_on_off_type)value == PM_ON)
      {
        do
        {
          ...
          err_flag |= pm_clk_get_status(PM_SPMI_BUSID_0, pmic_chip, clk_index, &clk_status);
...
/******************************************************************************
 ...
 Gets physical status of an LDO, SMPS upon an NPA Query and compares it
                      ...
                      pm_prm_clk_data_type *pam_data)
{
  ...
  err_flag |= pm_clk_get_status(PM_SPMI_BUSID_0, pmic_chip,
                                rsrc_data->periph_id,
                                &clk_status);
...
pm_prm_os_aux_dev_init(void)
{
  ...
  uint8 i = 0, j = 0;
  uint32 num_rsrcs  = 0, num_of_clk = 0;
  ...
  for(i = 0; i < PM_MAX_NUM_PMICS; i++)
  {
    clk_data = pm_clk_get_data(PM_SPMI_BUSID_0, i);
    if(clk_data != NULL)
    {
