...
Copyright (c) 2021-2023 QUALCOMM Technologies, Inc. (QTI). All Rights Reserved.
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/prm/src/pm_prm_driver.c#1 $
                       ...
                       npa_resource_state mode_id)
{
  ...
  } else
  {
    ...
    {
      PM_LOG_MSG_ERR_FATAL("Invalid ModeId %d resource %a by client %a",
    ...
    /*PM_LOG_MSG_INFO("Done Rsrc=%a, Client=%a, MId=%d, Type=0x%x, attr=0x%x\r\n",
                          ...
                          npa_resource_state state)
{
  ...
  {
    PM_LOG_MSG_INFO("Invalid Rsrc=%a", resource->definition->name);
    return 0;
  }
  ...
  if(rsrc_key_data->rpmh_data.nas_req_cnt > 0 &&
     is_nas_req != TRUE)
  {
    PM_LOG_MSG_INFO("Imm Req before NAS CB for %a by %a",
...
void pm_prm_nas_cb_scalar(void *npa_rsrc)
{
  ...
  PM_LOG_MSG_INFO("Done NAS_CB: rsrc=%a, AS=%d, SS=%d, msg_id=%d\r\n",
                  resource->definition->name,
