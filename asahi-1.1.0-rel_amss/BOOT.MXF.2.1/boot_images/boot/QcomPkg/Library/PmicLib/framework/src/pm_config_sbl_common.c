...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/framework/src/pm_config_sbl_common.c#1 $
...
pm_config_sbl_is_peripheral_whitelisted(uint32 bus_id, uint32 slave_id, uint16 addr, uint8 *bitmap_ptr , boolean *whitelisted)
{
  ...
  if(TRUE == pm_is_pmiclib_arch_pre7())
  {
    ...
    bitmap_value = bitmap_ptr[(bus_id * PM_MAX_NUM_SLAVE_IDS * PM_MAX_BITMAP_ENTRIES) +
                              (pmic_index * (slaves_count * PM_MAX_BITMAP_ENTRIES)) + 
                              (slave_id_index * PM_MAX_BITMAP_ENTRIES) + bitmap_index];
  }
  else
  {
    bitmap_value = bitmap_ptr[(bus_id * PM_MAX_NUM_SLAVE_IDS * PM_MAX_BITMAP_ENTRIES) +
...
pm_config_sbl_oem_psi( void )
{
  ...
  {    
    ...
    {
      clogic_index = count * PM_DT_PSI_CLOGIC_ITEM_SIZE;
  ...
  {
    settings_index = count*PM_DT_OEM_PSI_SEQ_ITEM_SIZE;
    ...
    if (settings_operation != PM_SBL_DELAY )
    {
      pm_resolve_chip_revision_ex(settings_bus_id, settings_slave_id, &valid_sid, &rev_id_read);
      if (valid_sid == FALSE)
      {
        pm_log_message("Skip opsi: bid %d sid %d addr 0x%04x data 0x%02x", settings_bus_id, settings_slave_id, settings_register_addr, settings_data);
        continue;
      }
    }
