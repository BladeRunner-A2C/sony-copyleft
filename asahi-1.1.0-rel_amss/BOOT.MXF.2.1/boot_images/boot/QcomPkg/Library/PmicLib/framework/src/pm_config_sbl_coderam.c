...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/framework/src/pm_config_sbl_coderam.c#1 $
    ...
    if( valid_config_flag == TRUE )
    {
      switch(rc->reg_operation)
      {
        ...
        case PM_SBL_PBS_RAM:
          for(i=0; i<2; i++)
          {
               ...
               ((sbl_config_info_ds->major_ver == 1) && (sbl_config_info_ds->minor_ver == 5)) ) //ver 2 - Lahaina and newer
            {
              ...
              pbs_err = pm_pbs_ram_image_load(rc->busid, rc->sid, pbs_ram_buffer, rc->address, rc->mask);
            }
            else if( (sbl_config_info_ds->major_ver == 1) && (sbl_config_info_ds->minor_ver == 3) ) //ver 2 - Added mask write support; ver 3 xml file gen, reg 0x52
            {
              ...
              pbs_err = pm_pbs_ram_image_load(rc->busid, rc->sid, pbs_ram_buffer, rc->address, DEFAULT_PBS_PERIPH_ID);
            }
        ...
        case PM_SBL_SDAM:
          ...
          err_flag |= pm_get_pmic_index_from_slave_ex(rc->busid, rc->sid, &sdam_pm_index);
