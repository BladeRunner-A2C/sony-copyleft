...
*  \n   
*  &copy; Copyright 2018-2023 QUALCOMM Technologies Incorporated, All Rights Reserved
*/
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/framework/src/pm_periph_processing.c#1 $
$Author: cirrusp4svc $
...
pm_config_lock_by_bit(void)
{
  ...
  for (i = 0; i < *num_lock_by_bit_arrays; i++)
  {
    for(j = 0; j < lock_by_bit_data_ptr[i].arr_size; j++)
    {
      ...
      bus_id = lock_by_bit_data_ptr[i].arr_ptr[j].bus_id;

      if(pm_is_slave_id_supported_ex(bus_id, slave_id) == TRUE) //write only if valid slave id 
      {
