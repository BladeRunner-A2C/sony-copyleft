...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/framework/src/pm_core_init.c#1 $
$Author: cirrusp4svc $
...
pm_err_flag_type pm_init(void)
{
  ...
  if(pm_version_is_pmic_valid(PM_PRIMARY_PMIC_INDEX) == FALSE)
  {
    PM_LOG_MSG("no valid pmic detected");
  ...
  /* Intialize PMIC XO node client */
  pm_xo_node_init();
  pm_utils_get_time("pm_xo_node_init");
