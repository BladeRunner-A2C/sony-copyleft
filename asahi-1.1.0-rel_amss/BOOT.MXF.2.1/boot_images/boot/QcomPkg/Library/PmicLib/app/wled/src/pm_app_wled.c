...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/app/wled/src/pm_app_wled.c#1 $
$Author: cirrusp4svc $
   ...
   pm_on_off_type             enable_backlight )
{
    ...
    else
    {
        ...
        err_flag = pm_wled_enable_modulator(device_index, PM_MODULATOR_TYPE_A, TRUE);   //Configure modulator of selected strings
        if(err_flag != PM_ERR_FLAG_SUCCESS){ return err_flag;}

        err_flag = pm_wled_set_led_duty_cycle(device_index, PM_MODULATOR_TYPE_A, dim_level); //configure dim level
        if(err_flag != PM_ERR_FLAG_SUCCESS){ return err_flag;}
