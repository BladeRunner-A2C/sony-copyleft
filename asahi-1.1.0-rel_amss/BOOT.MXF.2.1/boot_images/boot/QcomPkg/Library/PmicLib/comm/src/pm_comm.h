...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/comm/src/pm_comm.h#1 $
$Author: cirrusp4svc $
...
pm_comm_info_type*
pm_comm_get_comm_info(uint32 bus_id, uint32 pmic_chip_index,
...
pm_err_flag_type
pm_comm_pbs_write_byte_array(uint32 slave_id,
                         uint16 addr,
                         uint32 num_bytes,
                         uint8* data);
...
pm_err_flag_type
pm_comm_read_byte_ex(uint32 bus_id, uint32 slave_id ,
                  uint16 addr,
                  uint8* data);

pm_err_flag_type
pm_comm_read_byte_mask_ex(uint32 bus_id, uint32 slave_id ,
                       uint16 addr,
                       uint8 mask,
                       uint8* data);

pm_err_flag_type
pm_comm_write_byte_ex(uint32 bus_id, uint32 slave_id ,
                   uint16 addr,
                   uint8 data);

pm_err_flag_type
pm_comm_write_byte_mask_ex(uint32 bus_id, uint32 slave_id ,
                        uint16 addr,
                        uint8 mask,
                        uint8 value);

pm_err_flag_type
pm_comm_write_byte_array_ex(uint32 bus_id, uint32 slave_id,
                         uint16 addr,
                         uint32 num_bytes,
                         uint8* data);

pm_err_flag_type
pm_comm_read_byte_array_ex(uint32 bus_id, uint32 slave_id ,
                        uint16 addr ,
                        uint32 num_bytes,
                        uint8* data);

pm_err_flag_type
pm_comm_pbs_write_byte_array_ex(uint32 bus_id, uint32 slave_id,
                         uint16 addr,
                         uint32 num_bytes,
                         uint8* data);

pm_err_flag_type 
pm_comm_enable_debug_spmi_if(void);
