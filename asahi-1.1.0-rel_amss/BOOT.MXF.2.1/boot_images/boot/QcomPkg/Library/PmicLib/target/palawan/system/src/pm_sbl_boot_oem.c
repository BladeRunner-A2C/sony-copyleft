...
$Header: //components/dev/core.boot/6.1/syejare.core.boot.6.1.palawan_adding_3101/QcomPkg/Library/PmicLib/target/lanai/system/src/pm_sbl_boot_oem.c#1 $
$Author: syejare $
...
#define SDAM17_MEM_16				(16)
#define SDAM17_MEM_26				(26)
#define SDAM17_MEM_31				(31)
#define SDAM17_MEM_36				(36)
#define SDAM17_MEM_41				(41)
#define SDAM17_MEM_51				(51)
#define SDAM17_MEM_76				(76)
...
#define PM_WLAN_SDAM_ADDR	         0x70AC  //SDAM01_MEM_108
#define PM_WLAN_ULS_SEQ_TRIG_ADDR    0x2642  //SW_TRIG7 Trigger
#define PM_S3E_ULS_PBS_CLR_ADDR    0x2648

#define ADC_CMN2_IADC_DIG_PARAM2_ADDR 0x3049
#define ADC_CMN_BG_EXT_SNS_RESISTOR_SEL    0x305E

#define SDAM01_MEM041_ADDR 					0x7069
#define AOP_SDAM_MULTI_TRAY_CONFIG 			0x7296

#define PM_SUPPORT_HAMILTON_DT				(0x1)
#define PM_SUPPORT_MOSELLE_DT 				(0x2)
/*===========================================================================
...
static pm_err_flag_type pm_pre_psi_sdam_configs(void)
{
  ...
  err_flag |= pm_comm_write_byte(AZURA_INDEX, SDAM01_MEM041_ADDR, sw_dt_config->vdd_mdm_apq_ctrl);		//VDD_MODEM
  
  err_flag = pm_comm_write_byte(AZURA_INDEX, AOP_SDAM_MULTI_TRAY_CONFIG, sw_dt_config->multi_card_tray_config);
...
pm_sbl_boot_oem_psi_variant_config(void)
{
  ...
  err_flag |= pm_comm_write_byte(PMIC_A, PSI_VARIANT_MINOR, psi_dt_config->psi_variant_minor);//Minor SDAM is 1 byte only
...
pm_device_pre_init(void)
{
  ...
  //Kailua leverage for VDD2H --> VDD2l transition CR3295974
  err_flag |= pm_pre_psi_sdam_configs();
...
pm_set_oledb_default_config()
{
  ...
  uint8  elvdd_data  = 0;
  uint8  oledb_data  = 0;
  uint8  start_addr_sdam = SDAM17_MEM_07;
  ...
  start_addr_sdam = ((display_dt->swire_map == PM_SWIRE_MAP_A)       || (display_dt->swire_map == PM_SWIRE_MAP_B)
					 || (display_dt->swire_map == PM_SWIRE_MAP_G)    || (display_dt->swire_map == PM_SWIRE_MAP_K)) ? SDAM17_MEM_31 :
					    ((display_dt->swire_map == PM_SWIRE_MAP_C)   || (display_dt->swire_map == PM_SWIRE_MAP_H)
					 || (display_dt->swire_map == PM_SWIRE_MAP_L))   ?  SDAM17_MEM_41 :
					 	(display_dt->swire_map == PM_SWIRE_MAP_D)    ?  SDAM17_MEM_36 :
					    (display_dt->swire_map == PM_SWIRE_MAP_E)    ?  SDAM17_MEM_26 :
					    (display_dt->swire_map == PM_SWIRE_MAP_F)    ?  SDAM17_MEM_16 :
					    (display_dt->swire_map == PM_SWIRE_MAP_I)    ?  SDAM17_MEM_76 :
					    (display_dt->swire_map == PM_SWIRE_MAP_J)    ?  SDAM17_MEM_51 : SDAM17_MEM_31;


  err_flag |= pm_sdam_mem_write(AZURA_INDEX, PM_SDAM_17, start_addr_sdam, ONE_BYTE, &elvss_data);
  err_flag |= pm_sdam_mem_write(AZURA_INDEX, PM_SDAM_17, (start_addr_sdam + 1), ONE_BYTE, &elvdd_data);
  err_flag |= pm_sdam_mem_write(AZURA_INDEX, PM_SDAM_17, (start_addr_sdam + 2), ONE_BYTE, &oledb_data);
...
pm_driver_post_init(void)
{
  ...
  err_flag |= pm_comm_write_byte(PMIC_A, 0x8067, 0x01); 		//Note :: WA to be removed in next PSI release
...
static pm_err_flag_type pm_tgt_wlan_uls(void)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint8 pm_wlan_chip = 0x2; // Hamilton=1, Moselle=2
  
  pm_dt_sw_config  *sw_dt_config = NULL;
  sw_dt_config = pm_dt_get_node( PMIC_DT_NODE_PM_SW_CONFIG );
  CORE_VERIFY_PTR(sw_dt_config);
  
  err_flag |= pm_comm_write_byte(PMIC_A, PM_S3E_ULS_PBS_CLR_ADDR, 0x80);

  if(sw_dt_config->use_wlan_dt == PM_SUPPORT_HAMILTON_DT)
  {
	pm_wlan_chip = 0x1;
	pm_log_message("DT - hamilton attach");
  }
  else if(sw_dt_config->use_wlan_dt == PM_SUPPORT_MOSELLE_DT)
  {
	pm_wlan_chip = 0x2;
	pm_log_message("DT - Moselle attach");
  }
  else
  {
    ...
    {
  	  result = DAL_DeviceAttach(DALDEVICEID_PLATFORMINFO, &phPlatformInfo);
  	  if (result != DAL_SUCCESS)
  	  {
  	    pm_log_message("Failed to attach DAL Device");
  	    return PM_ERR_FLAG_FAILURE;
  	  }
    }
  
    result = DalPlatformInfo_GetPlatformInfo(phPlatformInfo, &platform_info);
    if (result != DAL_SUCCESS)
    {
	  pm_log_message("Failed to get platformInfo");
	  return PM_ERR_FLAG_FAILURE;
    }
    ...
    RCM			0x0		Moselle
    */
    if(((platform == DALPLATFORMINFO_TYPE_MTP) && (subtype == 0x01))
  	|| ((platform == DALPLATFORMINFO_TYPE_ATP) && (subtype == 0x00)))
    {
	  pm_wlan_chip = 0x1;
	  pm_log_message("HMT attach");
    }
    else if (((platform == DALPLATFORMINFO_TYPE_MTP) && ((subtype == 0x00) || (subtype == 0x02)))
  	|| ((platform == DALPLATFORMINFO_TYPE_CDP) && (subtype == 0x00))
  	|| ((platform == DALPLATFORMINFO_TYPE_QRD) && (subtype == 0x00))
  	|| ((platform == DALPLATFORMINFO_TYPE_RCM) && (subtype == 0x00)))
    {
	  pm_wlan_chip = 0x2;
	  pm_log_message("Moselle attach");	
    }
    else
    {
	  pm_wlan_chip = 0x2;
	  pm_log_message("CDT undefined, Default Moselle");
    }
  }
  
  err_flag = pm_comm_write_byte(PMIC_A, PM_WLAN_SDAM_ADDR, pm_wlan_chip);

  err_flag = pm_comm_write_byte(PMIC_A, PM_WLAN_ULS_SEQ_TRIG_ADDR, 0x80);
...
pm_err_flag_type pm_tgt_aop_pre_init(void)
{
  ...
  //No fusion planned with palawan, removing Fusion Configs
...
//Palawan specific - PBS seq for S3E ULS  
  err_flag |= pm_tgt_wlan_uls();
    ...
    if(pon_reason.rtc && (PM_ERR_FLAG_SUCCESS == err_flag))
    {
      ...
      if((err_flag == PM_ERR_FLAG_SUCCESS) && !vbus_present)
      {
        ...
        if(vbat_mv < charger_dt_config->vbat_thd_rtc_pon)
        {
		  ...
		  // Clear Pending Interrupt while disabling RTC_ALARM
          err_flag |= pm_rtc_alarm_clr(AZURA_INDEX);
