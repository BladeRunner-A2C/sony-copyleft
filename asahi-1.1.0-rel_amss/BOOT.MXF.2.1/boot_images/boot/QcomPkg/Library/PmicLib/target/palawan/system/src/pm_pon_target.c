...
$Header: //components/dev/core.boot/6.1/syejare.core.boot.6.1.palawan_adding_3101/QcomPkg/Library/PmicLib/target/lanai/system/src/pm_pon_target.c#1 $
$Author: syejare $
...
pm_err_flag_type pm_pon_get_fault_pmic_string(pm_pon_event_type event, char* fault_pmic, int size)
{
  ...
  case PM_PON_EVENT_FAULT_PMIC_B:
    strlcpy(fault_pmic, "BoneFish", size);
    break;
  ...
  case PM_PON_EVENT_FAULT_PMIC_H:
    strlcpy(fault_pmic, "Kohala/Sulpin", size);
	break;
  case PM_PON_EVENT_FAULT_PMIC_I:
    strlcpy(fault_pmic, "Seco_Jr_I", size);
	break;
  case PM_PON_EVENT_FAULT_PMIC_J:
    strlcpy(fault_pmic, "Seco_Jr_J", size);
    break;
  default: // do nothing
