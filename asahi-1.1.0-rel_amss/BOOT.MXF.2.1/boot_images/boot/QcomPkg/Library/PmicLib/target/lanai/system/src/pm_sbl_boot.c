...
*  \n
*  \n &copy; Copyright (c) 2013-2024 Qualcomm Technologies, Inc. All rights reserved.
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/target/lanai/system/src/pm_sbl_boot.c#1 $
$Author: cirrusp4svc $
...
pm_device_init ( void )
{
  ...
  {
    pm_log_message("Bootup: Device Init, No PMIC on Target");
...
pm_sbl_chg_init(void)
{
  ...
  uint32 initial_num_spmi_transn = pm_get_num_spmi_transaction(0);
  boolean primary_charger_present = FALSE;

  err_flag = pm_chg_is_primary_charger_chip_present(&primary_charger_present);
  if(primary_charger_present)
  {
    err_flag |= pm_app_bsi_strong_pullup();
