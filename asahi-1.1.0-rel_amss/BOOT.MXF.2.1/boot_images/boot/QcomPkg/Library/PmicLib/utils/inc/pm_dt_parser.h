...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/utils/inc/pm_dt_parser.h#1 $
$Author: cirrusp4svc $
...
#define PM_DT_OEM_PSI_SEQ_ITEM_SIZE       9
#define PM_DT_PSI_CLOGIC_ITEM_SIZE        7
#define PM_DT_DEV_PRE_INIT_ITEM_SIZE      5
#define PM_DT_DRV_POST_INIT_ITEM_SIZE     5
...
typedef struct
{
  uint8    primming_enable;
  uint8    vdd_mdm_apq_ctrl;
  uint8    multi_card_tray_config;
  uint8    use_wlan_dt;
  ...
  uint32   *device_pre_init;
  uint32   device_pre_init_count;
}pm_dt_sw_config;
...
typedef struct
{
  uint8   psi_variant_minor;
  uint16  psi_variant_major;  
  uint32  version;
...
typedef struct
{
  ...
  boolean enable_1s2p_id2;
  boolean enable_1s2p_therm2;
...
typedef struct
{
  uint32 battery_low_threshold_mv;
  uint32 battery_high_threshold_mv;
  uint32 no_battery_charger_threshold_mw;
  uint32 low_battery_charger_threshold_mw;
  uint32 first_msg_wait_time_us; // how long this driver waits for the first msg after ps_rdy
  uint32 done_timeout_us; // max amount of time this driver will be running
  boolean usbpd_enable;
}pm_dt_pdphy_config;
