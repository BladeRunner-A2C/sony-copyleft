...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/clk/src/pm_clk_calrc.c#1 $
$Author: cirrusp4svc $
...
pm_clk_calrc_smpl_enable(uint32 bus_id, uint32 pmic_index, pm_on_off_type enable_smpl)
{
  ...
  pm_clk_data_type          *clk_ptr = pm_clk_get_data(bus_id, pmic_index);
  ...
  else
  {
    ...
    err_flag = pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, CALRC_REG(smpl_ctl1), 0x80, data);
  }
...
pm_clk_calrc_set_smpl_delay(uint32 bus_id, uint32  pmic_index, uint32 delay_setting)
{
  ...
  {
    err_flag = pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, CALRC_REG(smpl_ctl1), 0x07, data);
  }
...
pm_clk_calrc_enable(uint32 bus_id, uint32 pmic_index, pm_on_off_type enable_calrc)
{
  ...
  {
    err_flag = pm_comm_write_byte_ex(bus_id, SLAVE_ID, CALRC_REG(calrc_enable), enable_calrc);
  }
...
pm_err_flag_type 
pm_clk_calrc_coincell_config(uint32 bus_id, uint32 pmic_index, pm_clk_calrc_coincap_type coincap_type)
{
  pm_err_flag_type          err_flag = PM_ERR_FLAG_SUCCESS;
  pm_clk_data_type          *clk_ptr = pm_clk_get_data(bus_id, pmic_index);
  pm_register_data_type     data = 0;
  ...
  else
  {
    ...
    err_flag = pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, CALRC_REG(calrc_cfg), 0x02, data);
  }
