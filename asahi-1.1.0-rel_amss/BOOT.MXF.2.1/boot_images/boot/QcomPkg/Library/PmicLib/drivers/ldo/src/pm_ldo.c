...
*  \n
*  &copy; Copyright 2012-2023 by QUALCOMM Technologies Incorporated, All Rights Reserved
*/
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/ldo/src/pm_ldo.c#1 $ 
...
#define LDO_OCP_CTL_EN_MASK               0x80
#define LDO_OCP_CTL_SELF_SHUTDOWN_EN_MASK 0x40
#define LDO_OCP_CTL_ADDR                  0x88
#define LDO_MISC1						  0x86
...
pm_err_flag_type 
pm_ldo_ocp_latched_status_clear(uint8 pmic_chip, uint8 peripheral_index, pm_vreg_ocp_info_type *ocp_info)
{
   pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
   pm_ldo_data_type *ldo_ptr = pm_ldo_get_data(pmic_chip);
   boolean is_broadcast_cfg_present = FALSE;
   peripheral_info_type  ldo_peripheral_info;
   pm_pwr_data_type *pwr_data = &(ldo_ptr->pm_pwr_data);
   memset(&ldo_peripheral_info, 0, sizeof(peripheral_info_type));
   ldo_peripheral_info.base_address = pwr_data->pwr_specific_info[peripheral_index].periph_base_address;
   pm_get_peripheral_info(pmic_chip, &ldo_peripheral_info);
    ...
    {
      if ((ldo_ptr->periph_subtype[peripheral_index] >= PM_HW_MODULE_LDO_515_LV_N300_100) || ((ldo_ptr->periph_subtype[peripheral_index] >= PM_HW_MODULE_LDO_512_MV_P50_500) && (ldo_peripheral_info.digital_major_version >= 4)))
      {
        is_broadcast_cfg_present= TRUE;
      }

      err_flag = pm_pwr_ocp_latched_status_clear_alg(&(ldo_ptr->pm_pwr_data), ldo_ptr->comm_ptr, peripheral_index, is_broadcast_cfg_present, ocp_info);
    }
...
pm_err_flag_type pm_ldo_ocp_broadcast(uint8 pmic_chip, uint8  ldo_peripheral_index, pm_on_off_type  on_off)
{
    ...
    pm_pwr_data_type *pwr_data = &(ldo_ptr->pm_pwr_data);
    memset(&ldo_peripheral_info, 0, sizeof(peripheral_info_type));
    ldo_peripheral_info.base_address = pwr_data->pwr_specific_info[ldo_peripheral_index].periph_base_address;
    pm_get_peripheral_info(pmic_chip, &ldo_peripheral_info);
    ...
    { 
      if ((ldo_ptr->periph_subtype[ldo_peripheral_index] >= PM_HW_MODULE_LDO_515_LV_N300_100) || ((ldo_ptr->periph_subtype[ldo_peripheral_index] >= PM_HW_MODULE_LDO_512_MV_P50_500) && (ldo_peripheral_info.digital_major_version >= 4)))
      {
        is_broadcast_cfg_present= TRUE;
      }

      err_flag = pm_pwr_ocp_broadcast_alg(&(ldo_ptr->pm_pwr_data), ldo_ptr->comm_ptr, ldo_peripheral_index, is_broadcast_cfg_present, on_off);
    }
...
pm_err_flag_type pm_ldo_ahc_status(uint8 pmic_chip, uint8 ldo_peripheral_index, pm_on_off_type *on_off) 
{
    pm_register_address_type reg      = 0x0;
    pm_register_data_type    data     = 0;
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    ...
    { 
      ...
      if(data & LDO_STATUS3_AHC_STATUS_MASK)
      {
          err_flag = pm_pwr_ahc_status_alg(&(ldo_ptr->pm_pwr_data), ldo_ptr->comm_ptr, ldo_peripheral_index, on_off);
      }
      else
      {
          err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
...
pm_err_flag_type pm_ldo_ocp_enable( uint8 pmic_chip, uint8  ldo_peripheral_index, pm_sw_mode_type sw_mode, pm_on_off_type  on_off)
{
    pm_err_flag_type err_flag     = PM_ERR_FLAG_SUCCESS;
    pm_ldo_data_type *ldo_ptr     = pm_ldo_get_data(pmic_chip);
    ...
    {
        err_flag = pm_pwr_ocp_en_alg (&(ldo_ptr->pm_pwr_data), ldo_ptr->comm_ptr, ldo_peripheral_index, sw_mode, on_off);
    }
    return err_flag;
}


pm_err_flag_type pm_ldo_ocp_self_shutdown_enable( uint8 pmic_chip, uint8  ldo_peripheral_index, pm_on_off_type  on_off)
{
    pm_err_flag_type err_flag     = PM_ERR_FLAG_SUCCESS;
    pm_ldo_data_type *ldo_ptr     = pm_ldo_get_data(pmic_chip);
    ...
    {
        err_flag = pm_pwr_ocp_self_shutdown_en_alg (&(ldo_ptr->pm_pwr_data), ldo_ptr->comm_ptr, ldo_peripheral_index, on_off);
    }
    return err_flag;
}

pm_err_flag_type pm_ldo_ocp_mode_status(uint8 pmic_chip, uint8 peripheral_index, boolean *ocp_mode_status)
{
   pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
   pm_ldo_data_type *ldo_ptr = pm_ldo_get_data(pmic_chip);
   peripheral_info_type  ldo_peripheral_info;
   ...
   pm_get_peripheral_info(pmic_chip, &ldo_peripheral_info);
    ...
    else
    {
      if ((ldo_ptr->periph_subtype[peripheral_index] >= PM_HW_MODULE_LDO_515_LV_N300_100) || ((ldo_ptr->periph_subtype[peripheral_index] >= PM_HW_MODULE_LDO_512_MV_P50_500) && (ldo_peripheral_info.digital_major_version >= 4)))
      {
        err_flag = pm_pwr_ocp_mode_status_alg(&(ldo_ptr->pm_pwr_data), ldo_ptr->comm_ptr, peripheral_index, ocp_mode_status);
      }
      else
      {
        err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
...
pm_err_flag_type
pm_ldo_ocp_resp_cfg(uint8 pmic_chip, uint8 peripheral_index, uint8 ldo_ocp_resp)
{
	pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_ldo_data_type *ldo_ptr = pm_ldo_get_data(pmic_chip);
    pm_register_address_type reg = 0;
    ...
    if ((ldo_ptr->periph_subtype[peripheral_index] >= PM_HW_MODULE_LDO_515_LV_N300_100) || ((ldo_ptr->periph_subtype[peripheral_index] >= PM_HW_MODULE_LDO_512_MV_P50_500) && (ldo_peripheral_info.digital_major_version >= 4)))
    {
			reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + LDO_MISC1);
			err_flag = pm_comm_write_byte_mask(ldo_ptr->comm_ptr->slave_id, reg, 0x1, ldo_ocp_resp);
		}
		else
		{
			err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
