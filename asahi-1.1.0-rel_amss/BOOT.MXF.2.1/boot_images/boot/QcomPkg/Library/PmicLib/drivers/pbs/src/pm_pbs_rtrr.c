...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/pbs/src/pm_pbs_rtrr.c#1 $ 
...
==========================================================================*/
pm_err_flag_type
pm_pbs_rtrr_ram_dump(uint32 bus_id, uint32 slave_id, pm_pbs_rtrr_dump_data_type *pm_pbs_rtrr_data)
{
  ...
  pm_pbs_data = pm_pbs_temp_data[bus_id * PM_MAX_NUM_PMICS + pmic_index];
  ...
  err_flag = pm_pbs_config_access(bus_id, slave_id, pbs_ram_rtrr_dataset_start_addr_placeholder, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM, periph_id);
  if (err_flag != PM_ERR_FLAG_SUCCESS)
  {
    return err_flag;
  }
  ...
  err_flag = pm_comm_read_byte_array_ex(bus_id, slave_id, pm_pbs_data->perph_base_addr+MEM_INTF_RD_DATA0_OFFS, 4, (uint8*)&read_data);
  if (err_flag != PM_ERR_FLAG_SUCCESS)
  {
    return err_flag;
  }
  ...
  err_flag |= pm_pbs_disable_access(bus_id, slave_id, PM_PBS_TYPE_RAM, periph_id);
  
  read_data = 0 ;
  
  err_flag = pm_pbs_config_access(bus_id, slave_id, rtrr_dataset_start_addr , PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM, periph_id);
  if (err_flag != PM_ERR_FLAG_SUCCESS)
  {
    return err_flag;
  }
