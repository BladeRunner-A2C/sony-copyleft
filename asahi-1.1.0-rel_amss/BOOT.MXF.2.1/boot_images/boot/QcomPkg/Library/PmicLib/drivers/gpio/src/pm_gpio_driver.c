...
/* =======================================================================
  ...
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/gpio/src/pm_gpio_driver.c#1 $
...
===========================================================================*/
#define PMIC_GPIO_MAX_HW_VER 0x32 
...
pm_gpio_register_info_type gpio_reg =
{
...
void pm_gpio_driver_init(pm_comm_info_type *comm_ptr, peripheral_info_type *peripheral_info, uint32 pmic_index)
{
  ...
  if (gpio_ptr == NULL)
  {
    ...
    num_of_peripherals = pm_target_information_get_periph_count_info(PM_PROP_GPIO_NUM, pmic_index);
    ...
    gpio_ptr->dig_maj_min = ((peripheral_info->digital_major_version << 0x4) | 
                             (peripheral_info->digital_minor_version));

    CORE_VERIFY(gpio_ptr->dig_maj_min <= PMIC_GPIO_MAX_HW_VER);
    ...
    gpio_ptr->base_address = peripheral_info->base_address;

    /* Num of peripherals cannot be 0 if this driver init gets called */
    gpio_ptr->num_of_peripherals = num_of_peripherals;
