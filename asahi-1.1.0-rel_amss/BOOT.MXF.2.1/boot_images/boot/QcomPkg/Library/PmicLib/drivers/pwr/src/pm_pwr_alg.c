...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/pwr/src/pm_pwr_alg.c#1 $
...
#define OCP_LATCH_EN_MASK                0x40
#define OCP_EN_MASK                      0x80
#define OCP_SELF_SHUTDOWN_EN_MASK        0x40
#define OCP_MODE_LPM                     0x00
#define OCP_MODE_NPM                     0x01
#define AHC_HR_MAX_VALUE               	 0x7F
...
pm_err_flag_type pm_pwr_get_uls_alg(pm_pwr_data_type *pwr_data, uint8 peripheral_index, pm_volt_level_type *volt_level)
{
    pm_err_flag_type                  err_flag = PM_ERR_FLAG_SUCCESS;
    ...
    else if (peripheral_index >= (pwr_data->num_of_peripherals) ||
      pwr_data->pwr_specific_info[peripheral_index].periph_type == 0)
    {
        err_flag = PM_ERR_FLAG_INVALID_PERIPHERAL;
    }
    ...
    {
        *volt_level = pwr_data->pwr_specific_info[peripheral_index].pwr_vset.RangeMax;
    }
...
pm_pwr_vreg_type(uint8 periph_type, char *vreg_type)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    ...
    else
    {
        switch(periph_type)
        {
            ...
            case PM_HW_MODULE_ULT_LDO:
                strlcpy(vreg_type, "LDO", VREG_TYPE_STRING_LEN);
                break;
            case PM_HW_MODULE_BOOST:
            case PM_HW_MODULE_BOB:
                strlcpy(vreg_type, "BOB", VREG_TYPE_STRING_LEN);
                break;
            case PM_HW_MODULE_HF_BUCK:
            case PM_HW_MODULE_HP_BUCK:
            case PM_HW_MODULE_BUCK_CMN:
            case PM_HW_MODULE_ULT_BUCK:
            case PM_HW_MODULE_FTS:
                strlcpy(vreg_type, "SMPS", VREG_TYPE_STRING_LEN);
                break;
            default:
                strlcpy(vreg_type, "UNK", VREG_TYPE_STRING_LEN);   // Unknown: doesn't fall into one of the above categories 
                break;
        }
    }
...
pm_err_flag_type pm_pwr_ocp_latched_status_clear_alg(pm_pwr_data_type *pwr_data, pm_comm_info_type *comm_ptr, uint8 peripheral_index, boolean broadcast_cfg, pm_vreg_ocp_info_type *ocp_info)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    uint8 periph_type = 0;
    pm_register_address_type ocp_status_reg  = 0x0;
    pm_register_address_type mode_at_ocp_reg = 0x0;
    ...
    else
    {
        ...
        if (pwr_data->pwr_specific_info[peripheral_index].is_periph_unified_reg == TRUE)
        {
            ...
            mode_at_ocp_reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->STATUS2);
            ocp_info->is_ocp_mode_supported = TRUE;

            if(broadcast_cfg == TRUE)
            {
                ocp_reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->BROADCAST_CFG);
                ocp_clear_mask = 0x1 << 4;
                ocp_clear_data = 0x1 << 4;
            }
            else
            {
        ...
        else if (periph_type == PM_HW_MODULE_ULT_LDO)
        {
            /*Only PMOS LDO have OCP but for other LDO either these registers are missing or the bit field is missing 
              and so status will never reported to be high. Mode at OCP reg is not available on these LDOs.*/
        ...
        err_flag |=  pm_pwr_vreg_type(periph_type, ocp_info->vreg_type);
        ...
        /*Store the OCP occured status*/
        ocp_info->ocp_occured = (status_data & status_mask)? TRUE : FALSE;

        if (ocp_info->ocp_occured)
        {
           /* Read the regulator mode at OCP, if supported */
            if(ocp_info->is_ocp_mode_supported)
            {
               err_flag |= pm_comm_read_byte(comm_ptr->slave_id, mode_at_ocp_reg, &status_data);
               ocp_info->ocp_mode_status = status_data;
            }
...
pm_err_flag_type pm_pwr_ocp_broadcast_alg(pm_pwr_data_type *pwr_data, pm_comm_info_type *comm_ptr, uint8 peripheral_index, boolean broadcast_cfg, pm_on_off_type on_off)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_register_address_type ocp_reg = 0x0;
    ...
    else
    {
        ...
        {
            ocp_reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->BROADCAST_CFG);
            pm_register_data_type ocp_broadcast_data = (on_off) ? (0xFF & 0x80) : 0x0;
            err_flag |= pm_comm_write_byte_mask(comm_ptr->slave_id, ocp_reg, 0x80, ocp_broadcast_data);
        }
        else
        {
            ocp_reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->OCP);
...
pm_err_flag_type pm_pwr_ocp_latch_en_alg(pm_pwr_data_type *pwr_data, pm_comm_info_type *comm_ptr, uint8 peripheral_index, pm_on_off_type on_off)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_register_address_type ocp_reg = 0x0;

    pm_register_data_type ocp_latch_en_data = (on_off) ? (0xFF & OCP_LATCH_EN_MASK) : 0x0;

    if ((peripheral_index >= pwr_data->num_of_peripherals) || (pwr_data->pwr_specific_info[peripheral_index].periph_type == 0) || (pwr_data->pwr_specific_info[peripheral_index].periph_type != PM_HW_MODULE_FTS))
    {
        err_flag = PM_ERR_FLAG_INVALID_PERIPHERAL;
    }
    else
    {
        ocp_reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->OCP);
        err_flag |= pm_comm_write_byte_mask(comm_ptr->slave_id, ocp_reg, OCP_LATCH_EN_MASK, ocp_latch_en_data);
    }
...
pm_err_flag_type pm_pwr_ocp_en_alg(pm_pwr_data_type *pwr_data, pm_comm_info_type *comm_ptr, uint8 peripheral_index, pm_sw_mode_type sw_mode, pm_on_off_type on_off)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_register_address_type ocp_reg = 0x0;

    pm_register_data_type ocp_en_data = (on_off) ? (0xFF & OCP_EN_MASK) : 0x0;
    ...
    {
        if(sw_mode == PM_SW_MODE_NPM)
        {
            ocp_reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->OCP);
            err_flag |= pm_comm_write_byte_mask(comm_ptr->slave_id, ocp_reg, OCP_EN_MASK, ocp_en_data);
        }
        else if(sw_mode == PM_SW_MODE_LPM)
        {
            ocp_reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->OCP_CTL2);
            err_flag |= pm_comm_write_byte_mask(comm_ptr->slave_id, ocp_reg, OCP_EN_MASK, ocp_en_data);
        }
        else
        {
...
pm_err_flag_type pm_pwr_ocp_self_shutdown_en_alg(pm_pwr_data_type *pwr_data, pm_comm_info_type *comm_ptr, uint8 peripheral_index, pm_on_off_type on_off)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_register_address_type ocp_reg = 0x0;

    pm_register_data_type ocp_self_shutdown_data = (on_off) ? (0xFF & OCP_SELF_SHUTDOWN_EN_MASK) : 0x0;

    if ((peripheral_index >= pwr_data->num_of_peripherals) || (pwr_data->pwr_specific_info[peripheral_index].periph_type == 0) || (pwr_data->pwr_specific_info[peripheral_index].periph_type != PM_HW_MODULE_LDO))
    {
        err_flag = PM_ERR_FLAG_INVALID_PERIPHERAL;
    }
    else
    {
        ocp_reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->OCP);
        err_flag |= pm_comm_write_byte_mask(comm_ptr->slave_id, ocp_reg, OCP_SELF_SHUTDOWN_EN_MASK, ocp_self_shutdown_data);
    }
...
pm_err_flag_type pm_pwr_ahc_status_alg(pm_pwr_data_type *pwr_data, pm_comm_info_type *comm_ptr, uint8 peripheral_index, pm_on_off_type *on_off)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
...
pm_err_flag_type pm_pwr_ahc_set_reserve_headroom_alg(pm_pwr_data_type *pwr_data, pm_comm_info_type *comm_ptr, uint8 peripheral_index, uint8 headroom)
{
    pm_register_address_type  reg       = 0xFF;
    pm_register_data_type     data      = 0xFF;
    pm_err_flag_type          err_flag  = PM_ERR_FLAG_SUCCESS;
    ...
    else if ( headroom > AHC_HR_MAX_VALUE)
    {
        err_flag = PM_ERR_FLAG_INVALID_PARAMETER;
    }
    else
    {
        reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->AHC_CTL1_HR);
    
        data = headroom;

        err_flag = pm_comm_write_byte_mask(comm_ptr->slave_id, reg, 0x7F, data);
    }
    return err_flag;
}

pm_err_flag_type pm_pwr_ocp_mode_status_alg(pm_pwr_data_type *pwr_data,  pm_comm_info_type *comm_ptr, uint8 peripheral_index, boolean *ocp_mode)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;

    if ((pwr_data == NULL) || (comm_ptr == NULL) || (ocp_mode == NULL))
    {
        return PM_ERR_FLAG_INVALID_PARAMETER;
    }
    ...
    else
    {
        ...
        reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->STATUS2);
        ...
        *ocp_mode = (data & 0x01) ? OCP_MODE_NPM : OCP_MODE_LPM;
    }
