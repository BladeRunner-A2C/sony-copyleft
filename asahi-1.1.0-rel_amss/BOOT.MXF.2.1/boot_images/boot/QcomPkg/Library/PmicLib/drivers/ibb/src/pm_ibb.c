...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/ibb/src/pm_ibb.c#1 $
$Author: cirrusp4svc $
...
#define MAX_IBB_OUTPUT_VOLTAGE  6600
#define MIN_IBB_OUTPUT_VOLTAGE  800
...
pm_err_flag_type pm_ibb_ibb_module_rdy(uint32 device_index, boolean ibb_rdy)
{
  ...
  {
    pm_register_address_type module_rdy = ibb_ptr->base_address + ibb_ptr->ibb_reg_table->module_rdy;
...
pm_err_flag_type pm_ibb_config_ibb_ctrl(uint32 device_index, boolean en_module, boolean swire_rdy)
{
  ...
  {
    pm_register_address_type enable_ctl = ibb_ptr->base_address + ibb_ptr->ibb_reg_table->enable_ctl;
...
pm_ibb_config_output_volt(uint32 device_index, int32 milli_volt)
{
  ...
  {
    pm_register_address_type output_voltage = ibb_ptr->base_address + ibb_ptr->ibb_reg_table->output_voltage;

    milli_volt = ABS(milli_volt);

    if(milli_volt > MAX_IBB_OUTPUT_VOLTAGE)
    {
      data = (MAX_IBB_OUTPUT_VOLTAGE - MIN_IBB_OUTPUT_VOLTAGE) / OUTPUT_VOLTAGE_STEP_SIZE;
    }
    else if(milli_volt < MIN_IBB_OUTPUT_VOLTAGE)
    {
      data = (MIN_IBB_OUTPUT_VOLTAGE - MIN_IBB_OUTPUT_VOLTAGE) / OUTPUT_VOLTAGE_STEP_SIZE;
    }
    else
    {
     data = (milli_volt - MIN_IBB_OUTPUT_VOLTAGE) / OUTPUT_VOLTAGE_STEP_SIZE;
    }
...
pm_ibb_config_default_volt(uint32 device_index, int32 milli_volt)
{
  ...
  default_voltage = ibb_ptr->base_address +
                    ibb_ptr->ibb_reg_table->default_voltage;

  milli_volt = ABS(milli_volt);

  milli_volt = milli_volt < MIN_IBB_OUTPUT_VOLTAGE ? MIN_IBB_OUTPUT_VOLTAGE : milli_volt;

  milli_volt = milli_volt > MAX_IBB_OUTPUT_VOLTAGE ? MAX_IBB_OUTPUT_VOLTAGE : milli_volt;

  //Round off to the high ceiling based on step-size
  milli_volt += (OUTPUT_VOLTAGE_STEP_SIZE - 1);

  data = (pm_register_data_type)(milli_volt - MIN_IBB_OUTPUT_VOLTAGE / OUTPUT_VOLTAGE_STEP_SIZE);

  err_flag = pm_comm_write_byte(ibb_ptr->comm_ptr->slave_id, default_voltage, data);
...
pm_err_flag_type pm_ibb_config_pwrup_pwrdn_dly(uint32 device_index, pm_ibb_dly_amnt_type pwrup_dly, pm_ibb_dly_amnt_type pwrdn_dly)
{
  ...
  else
  {
    ...
    pm_register_address_type pwrup_pwrdn_ctl_1 = ibb_ptr->base_address + ibb_ptr->ibb_reg_table->pwrup_pwrdn_ctl_1;
    pm_register_address_type pwrup_pwrdn_delay = ibb_ptr->base_address + ibb_ptr->ibb_reg_table->pwrup_pwrdn_delay;
...
pm_ibb_get_ibb_status(uint32 device_index, pm_ibb_status_type* ibb_status)
{
  ...
  else
  {
    ...
    ibb_status->output_volt = (data * OUTPUT_VOLTAGE_STEP_SIZE) + MIN_IBB_OUTPUT_VOLTAGE;

    pm_register_address_type default_voltage = ibb_ptr->base_address + ibb_ptr->ibb_reg_table->default_voltage;
    err_flag = pm_comm_read_byte(ibb_ptr->comm_ptr->slave_id, default_voltage, &data);
    ibb_status->default_volt = (data * OUTPUT_VOLTAGE_STEP_SIZE) + MIN_IBB_OUTPUT_VOLTAGE;
    ...
    pm_register_address_type status1 = ibb_ptr->base_address + ibb_ptr->ibb_reg_table->status1;
