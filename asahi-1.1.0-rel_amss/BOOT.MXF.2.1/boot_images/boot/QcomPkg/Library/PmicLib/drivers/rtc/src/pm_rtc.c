...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/rtc/src/pm_rtc.c#1 $
$Author: cirrusp4svc $
...
#define RTC_ALARM_MSEC_CFG_EN_MASK 0x01
...
/* Clear Alarm INT*/
pm_err_flag_type 
pm_rtc_alarm_clr(uint32 pmic_chip)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_rtc_data_type * rtc_ptr = pm_rtc_get_data(pmic_chip);
  ...
  err_flag = pm_comm_write_byte(SLAVE_ID, RTC_REG(PM_RTC_ALARM, alrm_clr), 0x1);
...
pm_rtc_alarm_set_time(uint32 pmic_chip, pm_rtc_time_type * set_time)
{
  ...
  if((rtc_ptr->dig_maj_min_ver >= 0x50) && (RTC_ALARM_MSEC_CFG_EN_MASK == read_data))
  {
...
pm_rtc_irq_enable(uint32 pmic_chip, pm_rtc_periph_type periph_index, boolean enable)
{
  ...
  if ((rtc_ptr == NULL) || 
      ((periph_index != PM_RTC_ALARM) && (periph_index != PM_RTC_RW)))
  {
    return PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
  }
