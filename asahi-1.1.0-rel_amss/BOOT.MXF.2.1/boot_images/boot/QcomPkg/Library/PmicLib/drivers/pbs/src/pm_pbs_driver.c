...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/pbs/src/pm_pbs_driver.c#1 $
$Author: cirrusp4svc $
...
static pm_pbs_data_type *pm_pbs_data_arr[PM_MAX_BUSES_SUPPORTED][PM_MAX_NUM_PMICS]; 
...
void
pm_pbs_driver_init(pm_comm_info_type *comm_ptr, peripheral_info_type *peripheral_info, uint32 pmic_index, uint32 bus_id)
{
  ...
  pbs_core_ptr = pm_pbs_data_arr[bus_id][pmic_index];
  ...
  if (pbs_core_ptr == NULL)
  {
    ...
    pm_pbs_data_arr[bus_id][pmic_index] = pbs_core_ptr;
  ...
  else
  {
    ...
    pm_comm_read_byte_ex(comm_ptr->bus_id, comm_ptr->slave_id, pbs_core_ptr->base_address[pbs_core_count] + NO_OF_CLIENTS_REG_OFFST , &no_of_clients);
...
pm_pbs_get_pbs_core_ver(uint8 busid, uint8 slave_id, uint16 core_base_addr, uint32 *version)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint8 maj_ver = 0, min_ver = 0;

  err_flag |= pm_comm_read_byte_ex(busid, slave_id, core_base_addr + PBS_CORE_REVISION1_ADDR,
                                &min_ver);

  err_flag |= pm_comm_read_byte_ex(busid, slave_id, core_base_addr + PBS_CORE_REVISION2_ADDR,
                                &maj_ver);
...
pm_err_flag_type
pm_pbs_get_mem_info(uint32 busid, uint32 slave_id, uint16 *pbs_start_addr,
                         uint16 *otp_size, uint16 *pbs_size)
{
  ...
  err_flag = pm_get_pmic_index_from_slave_ex(busid, slave_id, &pmic_index);
  ...
  pm_pbs_data = pm_pbs_temp_data[busid * PM_MAX_NUM_PMICS + pmic_index];
  if(NULL == pm_pbs_data)
  {
    return PM_ERR_FLAG_INVALID_PARAMETER;
  }

  err_flag |= pm_pbs_get_pbs_core_ver (busid, slave_id, pm_pbs_data->perph_base_addr, &supported_version);


  reg_offset = (supported_version > PM_PBS_VERSION_5P0) ? PBS_CORE_NVM_CFG1_V5P0_ONWA_ADDR : PBS_CORE_NVM_CFG1_UPTO_V5P0_ADDR;
  err_flag |= pm_comm_read_byte_ex(busid, slave_id, pm_pbs_data->perph_base_addr + reg_offset,
                                &otp_size_base);
  ...
  err_flag |= pm_comm_read_byte_ex(busid, slave_id, pm_pbs_data->perph_base_addr + PBS_CORE_RAM_CFG1_ADDR,
                                &pbs_size_base_lsb);
  ...
  *otp_size = otp_size_base * (1 << (otp_size_mult & otp_mask)) * 8;
  ...
  else if (supported_version >= PM_PBS_VERSION_5P0)
  {
    ...
    err_flag |= pm_comm_read_byte_ex(busid, slave_id, pm_pbs_data->perph_base_addr + reg_offset, &pbs_size_base_msb);
 ...
 */
pm_err_flag_type
pm_pbs_ram_image_verify(uint32 busid, uint32 slave_id, pm_pbs_ram_data_type *data, unsigned size, uint8 periph_id)
{
  int i;
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  ...
  {
    pm_get_pmic_index_from_slave_ex (busid, slave_id, &pmic_index);
  }
  else
  {
  ...
  err_flag = pm_pbs_get_mem_info(busid, slave_id, &pm_pbs_data->pbs_ram_start_addr,
  ...
  err_flag = pm_pbs_config_access(busid, slave_id, pm_pbs_data->pbs_ram_start_addr, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM, periph_id);
  if (err_flag != PM_ERR_FLAG_SUCCESS) return err_flag; //Stop on first error

  err_flag = pm_pbs_get_pbs_core_ver (busid, slave_id, pbs_core_base_addr, &support_version);
  ...
  {
    err_flag |= pm_comm_read_byte_array_ex(busid, slave_id, pbs_ram_read_addr, read_byte_size, (uint8*)&read_val);
    ...
    if (data_val != read_val)
    {
      ...
      {
        pm_log_message("pm_pbs_ram_image_verify: skipped for: word number = 0x%x, bus_id = 0x%x, slave_id = 0x%x", i, busid, slave_id);
        continue;
      }
      else
      {
        if (support_version >= PM_PBS_CORE_VER_SUPPORT_8BYTE)
        {
          pm_log_message("***pbs ram verify fail word number = 0x%x, bus_id = 0x%X, slave_id = 0x%x, periph_id = 0x%x, read_val = 0x%lx, dataval = 0x%lx", i, busid, slave_id, periph_id, read_val, data_val);
        }
        else
        {
          pm_log_message("***pbs ram verify fail word number = 0x%x, bus_id = 0x%X, slave_id = 0x%x, periph_id = 0x%x, read_val = 0x%x, dataval = 0x%x", i, busid, slave_id, periph_id, read_val, data_val);
        }

        pm_utils_log_err(__func__, PM_ERR_FLAG_DATA_VERIFY_ERR, __LINE__);
 ...
 */
pm_err_flag_type
pm_pbs_ram_image_load(uint32 busid, uint32 slave_id, pm_pbs_ram_data_type *data, unsigned size, uint8 periph_id)
{
  int i;
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  ...
  err_flag = pm_comm_read_byte_ex(busid, slave_id, pbs_core_base_addr + PERIPH_TYPE_OFFSET_ADDR, &read_perphtype);
  err_flag |= pm_comm_read_byte_ex(busid, slave_id, pbs_core_base_addr + PERIPH_SUBTYPE_OFFSET_ADDR, &read_perphsubtype);
  ...
  /* Configuring read access in burst mode to PBS RAM */
  err_flag = pm_pbs_config_access(busid, slave_id, pm_pbs_data->pbs_ram_start_addr, PM_PBS_ACCESS_WRITE_BURST, PM_PBS_TYPE_RAM, periph_id);
  if (err_flag != PM_ERR_FLAG_SUCCESS)
  {
    pm_utils_log_err(__func__, err_flag, __LINE__);
    return err_flag;
  ...
  if (support_version >= PM_PBS_CORE_VER_SUPPORT_8BYTE)
  {
    ...
    err_flag = pm_comm_pbs_write_byte_array_ex(busid, slave_id, pbs_ram_wr_addr, size*4, (uint8 *)&data[0]);
  }
  else
  {
    ...
    for (i = 0; i < size; i++)
    {
      err_flag = pm_comm_write_byte_array_ex(busid, slave_id, pbs_ram_wr_addr, 4, (uint8 *)&data[i]);
      if (err_flag != PM_ERR_FLAG_SUCCESS)
      {
        pm_utils_log_err(__func__, err_flag, __LINE__);
        return err_flag;
  ...
  err_flag = pm_pbs_disable_access(busid, slave_id, PM_PBS_TYPE_RAM, periph_id);
  if (err_flag != PM_ERR_FLAG_SUCCESS)
  {
    pm_utils_log_err(__func__, err_flag, __LINE__);
    return err_flag;
  ...
  err_flag = pm_pbs_ram_image_verify(busid, slave_id, data, size, periph_id);
 ...
 */
pm_err_flag_type 
pm_pbs_disable_access(uint32 busid, uint32 slave_id, pm_pbs_type pbs_type,uint8 periph_id) 
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint32 support_version = 0;
  ...
  if(PM_PBS_TYPE_ROM == pbs_type)
  {
    ...
    {
      err_flag = pm_comm_write_byte_ex(busid, slave_id, otp_cfg1_addr, 0x00);
      err_flag |= pm_comm_write_byte_ex(busid, slave_id, otp_force_on_addr, 0x00);
    }
  }

  /* Configuring to disable PBS ROM/RAM access */
  err_flag |= pm_comm_write_byte_ex(busid, slave_id, mem_intf_cfg_addr, 0x00);
  err_flag |= pm_comm_write_byte_ex(busid, slave_id, mem_intf_ctl_addr, 0x00);
  
  /* Resetting memory interface address */
  err_flag |= pm_comm_write_byte_array_ex(busid, slave_id, mem_intf_addr_lsb, 2, (uint8*)&addr);
  
  err_flag |= pm_busy_wait(10);
 ...
 */
pm_err_flag_type 
pm_pbs_config_access(uint32 busid, uint32 slave_id, uint16 addr, pm_pbs_access_type access_type, pm_pbs_type pbs_type, uint8 periph_id) 
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint8 access_val = 0;
  ...
  err_flag = pm_comm_write_byte_ex(busid, slave_id, mem_intf_cfg_addr, 0x80);
  if (err_flag != PM_ERR_FLAG_SUCCESS)
  {
    return err_flag;
  }
  ...
  err_flag |= pm_comm_write_byte_ex(busid, slave_id, mem_intf_ctl_addr, access_val);
  if (err_flag != PM_ERR_FLAG_SUCCESS)
  {
    return err_flag;
  }
  ...
  if(PM_PBS_TYPE_ROM == pbs_type)
  {
    ...
    if(support_version >= PM_PBS_CORE_VER_SUPPORT_NEW_SEQ)
    {
      err_flag = pm_comm_write_byte_ex(busid, slave_id, otp_cfg1_addr, otp_cfg_value);
      if (err_flag != PM_ERR_FLAG_SUCCESS)
      {
        return err_flag;
      }

      err_flag = pm_comm_write_byte_ex(busid, slave_id, otp_force_on_addr, otp_force_en_value);
      if (err_flag != PM_ERR_FLAG_SUCCESS)
      {
        return err_flag;
      }
      ...
      {
        err_flag = pm_comm_read_byte_mask_ex(busid, slave_id, pce_en_done_status, 0x01, &pce_en_value);
        if (err_flag != PM_ERR_FLAG_SUCCESS)
        {
          return err_flag;
        }
...
pm_pbs_data_type*
pm_pbs_get_data(uint32 bus_id, uint32 pmic_index)
{
  if((pmic_index < PM_MAX_NUM_PMICS) && (bus_id < PM_MAX_BUSES_SUPPORTED)) 
  {
    return pm_pbs_data_arr[bus_id][pmic_index];
  }
...
uint8
pm_pbs_client_get_num_peripherals(uint32 bus_id, uint32 pmic_index, pm_pbs_core_type pbs_core_index)
{
  if((pmic_index < PM_MAX_NUM_PMICS) && (bus_id < PM_MAX_BUSES_SUPPORTED) && (pm_pbs_data_arr[bus_id][pmic_index] != NULL))
  {
    return pm_pbs_data_arr[bus_id][pmic_index]->pbs_client_count[pbs_core_index];
  }
