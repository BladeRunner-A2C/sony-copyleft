...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/pbs/src/pm_pbs_client.c#1 $
$Author: cirrusp4svc $
...
#define MEM_INTF_CFG_RMSK              0x80

#define PBS_CLIENT_BASE_ADDR(pbs_core_index, pbs_client_index)  pbs_client_ptr->base_address[pbs_core_index] + (pbs_client_ptr->pbs_client_reg_table->peripheral_offset * (1 + pbs_client_index))
#define SLAVE_ID (pbs_client_ptr->comm_ptr->slave_id)
/*===========================================================================
...
===========================================================================*/
pm_err_flag_type
pm_pbs_client_enable(uint32 bus_id, uint32 pmic_device_index, pm_pbs_core_type pbs_core_index,
                    pm_pbs_client_type pbs_client_index, boolean enable)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_register_address_type reg = 0;
    pm_pbs_data_type *pbs_client_ptr = pm_pbs_get_data(bus_id, pmic_device_index);
    uint8 data = 0;
    uint8 mask = 0x80;
    ...
    if ((pbs_client_index >= pbs_client_ptr->pbs_client_count[pbs_core_index]) || (pbs_client_index >= PM_PBS_CLIENT_INVALID))
    {
      return PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
    }

    reg = (PBS_CLIENT_BASE_ADDR(pbs_core_index, pbs_client_index)) + pbs_client_ptr->pbs_client_reg_table->en_ctl1;
...
pm_err_flag_type
pm_pbs_client_enable_status(uint32 bus_id, uint32 pmic_device_index,
                            pm_pbs_core_type pbs_core_index, pm_pbs_client_type pbs_client_index,
                            boolean *status)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_register_address_type reg = 0;
    ...
    err_flag = pm_comm_read_byte_mask_ex(bus_id, SLAVE_ID, reg, mask, &data);
...
pm_err_flag_type
pm_pbs_client_sw_trigger(uint32 bus_id, uint32 pmic_chip, pm_pbs_core_type pbs_core_index,
                        pm_pbs_client_type pbs_client_index, pm_pbs_client_trigger trigger, 
                        uint8 *trigger_argument, uint8 *trigger_return_value)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_pbs_data_type *pbs_client_ptr = pm_pbs_get_data(bus_id, pmic_chip);
    uint8 data = 0x80;
    ...
    base_addr = PBS_CLIENT_BASE_ADDR(pbs_core_index, pbs_client_index);
    ...
    if (trigger_argument != NULL)
    {
        ...
        err_flag |= pm_comm_write_byte_ex(bus_id, SLAVE_ID, scratch1, *trigger_argument);
    }

    trig_ctl = base_addr + pbs_client_ptr->pbs_client_reg_table->trig_ctl;

    // Trigger the PBS sequence
    err_flag |= pm_comm_write_byte_ex(bus_id, SLAVE_ID, trig_ctl, data);
    ...
    if (trigger_return_value != NULL)
    {
        ...
        err_flag |= pm_comm_read_byte_ex(bus_id, SLAVE_ID, scratch2, trigger_return_value);
    }
...
pm_err_flag_type
pm_pbs_client_hw_trigger_enable(uint32 bus_id, uint32 pmic_chip, pm_pbs_core_type pbs_core_index,
                                pm_pbs_client_type pbs_client_index, pm_pbs_client_trigger hw_trigger,
                                pm_pbs_client_trigger_edge_type trigger_edge, boolean enable)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  ...
  //Disabling HW trigger for PBS client 0 is not allowed as it used for PON, also failing if SW trigger is given as argument
    if ( (pbs_client_index >= pbs_client_ptr->pbs_client_count[pbs_core_index]) || 
    ...
    else //Disable triggering
{
    ...
    switch (trigger_edge)
  {
    ...
    err_flag = pm_comm_write_byte_ex(bus_id, SLAVE_ID, en_ctl_addr, 0x00);
    err_flag |= pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, hw_trig_re_en_addr, hw_trig_re_fe_en_mask, hw_trigger_re_data);
    err_flag |= pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, hw_trig_fe_en_addr, hw_trig_re_fe_en_mask, hw_trigger_fe_data);

    err_flag |= pm_comm_write_byte_ex(bus_id, SLAVE_ID, trig_latch_clr_addr, 0xFF);
    err_flag |= pm_comm_write_byte_ex(bus_id, SLAVE_ID, en_ctl_addr, 0x80);

    //Enables/Disable RIF memory interface and the RIF Memory Access Mode. When enabled the PBS can run concurrently.
    err_flag |= pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, base_addr + MEM_INTF_CFG_ADDR, base_addr + MEM_INTF_CFG_RMSK, 0x80); //0x7040 = 0x80 : Enables 
    err_flag |= pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, base_addr + MEM_INTF_CFG_ADDR, base_addr + MEM_INTF_CFG_RMSK, 0x00); //0x7040 = 0x00 : Disable 
