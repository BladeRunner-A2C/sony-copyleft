...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/ldo/src/pm_ldo_driver.c#1 $
$Author: cirrusp4svc $
...
#define PM_LDO_DRIVER_HW_MAX_VER_SUPPORTED 0x42 // 1st bit are dig_major, 0 bit dig_minor = 1.0 
...
void pm_ldo_driver_init(pm_comm_info_type *comm_ptr, peripheral_info_type *peripheral_info, uint8 pmic_index)
{
    ...
    if (((peripheral_info->digital_major_version << 4) | 
          peripheral_info->digital_minor_version)  > PM_LDO_DRIVER_HW_MAX_VER_SUPPORTED )
    {
        CORE_VERIFY(0);
    }
    ...
    if (ldo_ptr == NULL)
    {
        ...
        ldo_ptr->pm_pwr_data.num_of_peripherals = num_of_peripherals;
