...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/clk/src/pm_clk.c#1 $
$Author: cirrusp4svc $
...
pm_err_flag_type 
pm_clk_drv_strength(uint32 bus_id,
                    uint32 pmic_chip, 
                    pm_clk_type periph, 
                    pm_clk_drv_strength_type drive_strength)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_clk_data_type *clk_ptr = pm_clk_get_data(bus_id, pmic_chip);
  ...
  /* Set strength */
  err_flag = pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, reg, PM_CLK_N_P_DRV_STRENGTH_MASK, (pm_register_data_type)(drive_strength));  
...
pm_err_flag_type 
pm_clk_sw_enable(uint32 bus_id,
                 uint32 pmic_chip, 
                 pm_clk_type periph, 
                 pm_on_off_type on_off)
{
  ...
  err_flag =  pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, reg , CLK_SW_EN_CTL_MSK, data); // 7

  return err_flag;
}

pm_err_flag_type 
pm_clk_pin_ctrled(uint32 bus_id,
                  uint32 pmic_chip,
                  pm_clk_type periph, 
                  pm_on_off_type on_off)
{
  ...
  err_flag = pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, reg , PC_EN_MASK, data); // 0   

  return err_flag;
}

pm_err_flag_type 
pm_clk_pull_down(uint32 bus_id,
                 uint32 pmic_chip, 
                 pm_clk_type periph, 
                 pm_on_off_type on_off)
{
  ...
  err_flag = pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, reg , 0x80, data); // 7
...
pm_err_flag_type 
pm_clk_set_out_clk_div(uint32 bus_id,
                       uint32 pmic_chip, 
                       pm_clk_type clk, 
                       uint32 divider)
{
...
pm_err_flag_type 
pm_clk_get_status(uint32 bus_id,
                  uint32 pmic_chip, 
                  pm_clk_type clk, 
                  pm_clk_status_type *clk_status)
{
  ...
  if ((clk_status == NULL) || (clk >= PM_CLK_MAX_INDEX))
  {
    return PM_ERR_FLAG_INVALID_PARAMETER; 
  }
  ...
  if(periph_index >= PM_CLK_MAX_INDEX)
  {
    return PM_ERR_FLAG_INVALID_PERIPHERAL;
  }
  ...
  err_flag |= pm_comm_read_byte_ex(bus_id, SLAVE_ID, reg, &data);
      ...
      (periph_subtype == PM_HW_MODULE_CLOCK_BB_CLK))
  {
    ...
    err_flag |= pm_comm_read_byte_mask_ex(bus_id, SLAVE_ID, reg, PC_EN_MASK, &data);
