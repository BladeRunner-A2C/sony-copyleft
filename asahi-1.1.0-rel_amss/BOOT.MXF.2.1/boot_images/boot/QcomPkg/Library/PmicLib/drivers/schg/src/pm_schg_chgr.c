...
#define PM_SCHG_FLOAT_VOLTAGE_STEP_CONSTANT         20
...
pm_err_flag_type
pm_schg_chgr_set_charge_current(uint32 bus_id, uint32 pmic_index, pm_schg_chgr_current_type charge_current_type, uint32 current)
{
  ...
  else
  {
    ...
    err_flag = pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, reg, mask, data);
  }
...
pm_err_flag_type
pm_schg_chgr_enable_charging(uint32 bus_id, uint32 pmic_index, boolean enable)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_schg_data_type *schg_ptr = pm_schg_get_data(bus_id, pmic_index);
...
pm_err_flag_type
pm_schg_chgr_enable_src(uint32 bus_id, uint32 pmic_index, boolean enable)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_schg_data_type *schg_ptr = pm_schg_get_data(bus_id, pmic_index);
  ...
  {
    pm_register_address_type reg = ADDRESS(charger_cfg2);
...
pm_err_flag_type
pm_schg_chgr_get_chgr_status(uint32 bus_id, uint32 pmic_index, pm_schg_chgr_status_type *charger_status)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;

  pm_schg_data_type *schg_ptr = pm_schg_get_data(bus_id, pmic_index);
  /*between status 2 and status 3 there is gap and hence breaking*/
  pm_register_data_type data[6] = {0};
  ...
  else
  {
    ...
    err_flag = pm_comm_read_byte_array_ex(bus_id, SLAVE_ID, status1, 2, data);
    /*status1 related*/
    ...
    err_flag |= pm_comm_read_byte_array_ex(bus_id, SLAVE_ID, status3, 6, data);
    /*status3 related*/
    ...
    charger_status->status_7.pre_rchg                     = (data[4] & 0x40) ? TRUE : FALSE;
...
/*This API configures the floating voltage. Valid range is 7200mV to 9740 mv*/
pm_err_flag_type
pm_schg_chgr_set_float_volt(uint32 bus_id, uint32 pmic_index, uint32 float_volt_mv)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  ...
  else
  {
    ...
    if (float_volt_mv < 7200)
    {
      data = 0x0;
    }
    else if (float_volt_mv >= 9740)
    {
      data = 0x7f;
    }
    else
    {
      ...
      data = (float_volt_mv - 7200) / PM_SCHG_FLOAT_VOLTAGE_STEP_CONSTANT;
    }

    err_flag = pm_comm_write_byte_ex(bus_id, SLAVE_ID, fv_cfg, data);
  }
...
pm_err_flag_type
pm_schg_chgr_set_jeita_fvcomp_cfg(uint32 bus_id, uint32 pmic_index, uint32 fv_comp_hot, uint32 fv_comp_cold)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_register_data_type data = 0;
  ...
  else
  {
    ...
    err_flag |= pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, fv_cmp_cold_cfg, mask, data);

    /* JEITA Float Voltage Compensation in hot soft limit  */
    data = fv_comp_hot / PM_SCHG_JEITA_FLOAT_VOLTAGE_STEP_CONSTANT;
    err_flag |= pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, fv_cmp_hot_cfg, mask, data);
  }
...
pm_err_flag_type
pm_schg_chgr_set_jeita_ccomp(uint32 bus_id, uint32 pmic_index, uint32 ccomp_hot, uint32 ccomp_cold)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_register_data_type data = 0;
  pm_register_data_type mask = 0x3F;
  pm_schg_data_type *schg_ptr = pm_schg_get_data(bus_id, pmic_index);
  ...
  else
  {
    ...
    err_flag |= pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, fcc_cmp_cold_cfg, mask, data);

    /* JEITA Charge Current Compensation in hot soft limit */
    data = ccomp_hot / PM_SCHG_JEITA_CHARGE_CURRENT_STEP_CONSTANT;
    err_flag |= pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, fcc_cmp_hot_cfg, mask, data);
  }
...
pm_err_flag_type
pm_schg_chgr_set_jeita_threshold_value(uint32 bus_id, uint32 pmic_index, pm_schg_chgr_jeita_threshold_type threshold_type, uint32 jeita_threshold)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_schg_data_type *schg_ptr = pm_schg_get_data(bus_id, pmic_index);
  pm_register_address_type addr_msb = 0, addr_lsb = 0;
  uint8 jeita_threshold_lsb = jeita_threshold & 0xFF;
  uint8 jeita_threshold_msb = (jeita_threshold >> 8) & 0xFF;

  if(NULL == schg_ptr)
  {
    err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
  }
  else if(threshold_type >= PM_SCHG_CHGR_JEITA_THRESHOLD_INVALID)
  {
    err_flag = PM_ERR_FLAG_INVALID_PARAMETER;
  }
  else
  {
    ...
    {
      addr_msb = ADDRESS(jeita_hot_threshold_msb);
      addr_lsb = ADDRESS(jeita_hot_threshold_lsb);
      break;
    }
    case PM_SCHG_CHGR_JEITA_THRESHOLD_COLD:
    {
      addr_msb = ADDRESS(jeita_cold_threshold_msb);
      addr_lsb = ADDRESS(jeita_cold_threshold_lsb);
      break;
    }
    case PM_SCHG_CHGR_JEITA_THRESHOLD_TOO_HOT:
    {
      addr_msb = ADDRESS(jeita_thot_threshold_msb);
      addr_lsb = ADDRESS(jeita_thot_threshold_lsb);
      break;
    }
    case PM_SCHG_CHGR_JEITA_THRESHOLD_TOO_COLD:
    {
      addr_msb = ADDRESS(jeita_tcold_threshold_msb);
      addr_lsb = ADDRESS(jeita_tcold_threshold_lsb);
      break;
    }
    case PM_SCHG_CHGR_JEITA_THRESHOLD_TOO_HOT_AFP:
    {
      addr_msb = ADDRESS(jeita_thot_afp_threshold_msb);
      addr_lsb = ADDRESS(jeita_thot_afp_threshold_lsb);
      break;
    }
    case PM_SCHG_CHGR_JEITA_THRESHOLD_TOO_COLD_AFP:
    {
      addr_msb = ADDRESS(jeita_tcold_afp_threshold_msb);
      addr_lsb = ADDRESS(jeita_tcold_afp_threshold_lsb);
      break;
    }
    default:
      return err_flag;
    }
    err_flag |= pm_comm_write_byte_ex(bus_id, SLAVE_ID, addr_msb, jeita_threshold_msb);
    err_flag |= pm_comm_write_byte_ex(bus_id, SLAVE_ID, addr_lsb, jeita_threshold_lsb);
  }
  return err_flag;
}

pm_err_flag_type
pm_schg_chgr_get_jeita_threshold_value(uint32 bus_id, uint32 pmic_index, pm_schg_chgr_jeita_threshold_type threshold_type, uint16* jeita_threshold)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_schg_data_type *schg_ptr = pm_schg_get_data(bus_id, pmic_index);
  pm_register_address_type addr_msb, addr_lsb;
  ...
  else
  {
    ...
    err_flag |= pm_comm_read_byte_ex(bus_id, SLAVE_ID, addr_msb, &jeita_threshold_msb);
    err_flag |= pm_comm_read_byte_ex(bus_id, SLAVE_ID, addr_lsb, &jeita_threshold_lsb);
...
pm_err_flag_type
pm_schg_chgr_jeita_en_cfg(uint32 bus_id, uint32 pmic_index, pm_schg_chgr_jeita_cfg_type jeita_cfg_type, boolean enable)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_schg_data_type *schg_ptr = pm_schg_get_data(bus_id, pmic_index);
  ...
  else
  {
    ...
    err_flag = pm_comm_write_byte_mask_ex(bus_id, SLAVE_ID, jeita_en_cfg, (uint8)(1 << jeita_cfg_type), (enable ? 0xFF : 0x00));
  }
...
pm_err_flag_type
pm_schg_chgr_get_schg_pmic_info(uint8 bus_id, uint32 *pmic_index, uint32 *slave_id, uint32 *charger_count, uint32 max_charger)
{
  uint32 i = 0;
  ...
  *charger_count = 0;
  pm_schg_get_pmic_info(bus_id, pmic_index, slave_id, charger_count, max_charger);
