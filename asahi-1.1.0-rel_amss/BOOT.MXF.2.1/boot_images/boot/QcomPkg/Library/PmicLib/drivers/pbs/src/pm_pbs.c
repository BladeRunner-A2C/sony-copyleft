...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/pbs/src/pm_pbs.c#1 $
$Author: cirrusp4svc $
...
pm_pbs_ram_sequences_verify(uint32 busid, uint32 slave_id, uint16 ram_addr, uint32* seq_ptr, uint8 num_seqs, uint16 periph_id) 
{
  ...
  err_flag = pm_pbs_config_access(busid, slave_id, ram_addr, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM, periph_id);
  ...
  for (seq_cnt = 0;seq_cnt < num_seqs;seq_cnt++)
  {
    ...
    err_flag = pm_comm_read_byte_array_ex(busid, slave_id, pbs_core_base_addr + MEM_INTF_RD_DATA0_ADDR, 4, (uint8*)&seq_read);
 ...
 */
static pm_err_flag_type 
pm_pbs_ram_sequences_load(uint32 busid, uint32 slave_id, uint16 ram_addr, uint32* seq_ptr, uint8 num_seqs,uint16 periph_id) 
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint8 seq_cnt = 0;
  uint16 pbs_core_base_addr = periph_id * PERIPH_OFFSET;
  ...
  err_flag = pm_pbs_config_access(busid, slave_id, ram_addr, PM_PBS_ACCESS_WRITE_BURST, PM_PBS_TYPE_RAM, periph_id);
  ...
  for (seq_cnt = 0;seq_cnt < num_seqs;seq_cnt++)
  {
    ...
    err_flag = pm_comm_write_byte_array_ex(busid, slave_id, pbs_core_base_addr + MEM_INTF_WR_DATA0_ADDR, 4, (uint8*)&seq_ptr[seq_cnt]);
  ...
  err_flag = pm_pbs_ram_sequences_verify(busid, slave_id, ram_addr, seq_ptr, num_seqs,periph_id);
