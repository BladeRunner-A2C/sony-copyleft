...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/pon/src/pm_pon.c#1 $
$Author: cirrusp4svc $
...
pm_pon_ps_hold_cfg(uint32 pmic_device_index, pm_pon_reset_cfg_type ps_hold_cfg)
{
  ...
  if((ps_hold_cfg >= PM_PON_RESET_CFG_INVALID) ||
        (pon_reset_cfg_val[ps_hold_cfg] == PON_RESET_CFG_INVALID_VAL))
  {
  ...
  else
  {
    ...
    if(err_flag == PM_ERR_FLAG_SUCCESS)
    {
      ...
      err_flag |= pm_pon_irq_enable(pmic_index, pon_ps_hold_reg.irq, FALSE);
      err_flag |= pm_pon_irq_clear(pmic_index, pon_ps_hold_reg.irq);

      //set type
      err_flag |= pm_pon_irq_set_trigger(pmic_index, pon_ps_hold_reg.irq, pon_ps_hold_reg.trigger_type);
      ...
      //enable interrupt
      err_flag |= pm_pon_irq_enable(pmic_index, pon_ps_hold_reg.irq, TRUE);
    }
  }
                        ...
                        uint32 s1_timer_ms, uint32 s2_timer_ms, pm_pon_reset_cfg_type reset_cfg)
{
  ...
  /*disable and wait for 8 sleep clock cycle*/
  err_flag = pm_comm_write_byte_mask(slave_id, reset_reg->reset_ctl2_addr, reset_reg->reset_en_mask, 0x0);

  //disable and clear irq
  err_flag |= pm_pon_irq_clear(pmic_index, reset_reg->irq);
  err_flag |= pm_pon_irq_enable(pmic_index, reset_reg->irq, FALSE);  
  ...
  //set type
  err_flag |= pm_pon_irq_set_trigger(pmic_index, reset_reg->irq, reset_reg->trigger_type);

  //re-enable s2 reset_reset_en
  ...
  err_flag |= pm_pon_irq_enable(pmic_index, reset_reg->irq, TRUE);
...
pm_pon_stage3_reset_source_cfg(uint32 pmic_device_index, pm_pon_reset_source_type reset_source, uint32 s3_timer_ms)
{
  ...
  uint8 s3_timer_id = 0;
  uint32 slave_id = 0;
  pm_register_data_type pon_pbs_revision[2] = {0};
  pm_register_data_type pon_perph_type[2] = {0};
