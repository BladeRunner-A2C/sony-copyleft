 ...
 *	\n	
 *	\n &copy; Copyright 2010-2023 QUALCOMM Technologies Incorporated, All 
	Rights Reserved
*/
/*===========================================================================
...
#define REG_GPIO(gpio, reg) 	((gpio_ptr->base_address) + \
	...
	pm_gpio_volt_src_type voltage_source)
{
  ...
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_gpio_data_type *gpio_ptr = pm_gpio_get_data(pmic_chip);
  pm_gpio_volt_src_type mask_vin;
  uint32 periph_subtype = 0;

  if ((gpio_ptr == NULL) || 
      (gpio >= gpio_ptr->num_of_peripherals) || 
  ...
  /* GPIOLV supports single VIN source [VIN0 (default)] */
  err_flag = pm_comm_read_byte (SLAVE_ID, 
                                REG_GPIO(gpio, periph_subtype), 
  ...
  if (err_flag == PM_ERR_FLAG_SUCCESS)
  {
    ...
    {
      case PM_HW_MODULE_GPIO_GPIOMV:
      case PM_HW_MODULE_GPIO_GPIOLV_VIN2:
        mask_vin = PM_GPIO_VIN1;
        break;
      case PM_HW_MODULE_GPIO_GPIOMV_VIN3:
        mask_vin = PM_GPIO_VIN2;
        break;
      default:
        return PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
    }

    if (voltage_source > mask_vin)
    {
      voltage_source = mask_vin;
      err_flag = PM_ERR_FLAG_INVALID_PARAMETER;
    }

    err_flag |= pm_comm_write_byte_mask
                   (SLAVE_ID,
                    REG_GPIO(gpio, dig_vin_ctl),
                    DIG_VIN_CTL_MASK, (pm_register_data_type) voltage_source);
  }
