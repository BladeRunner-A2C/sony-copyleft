...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/ab/src/pm_ab.c#1 $
$Author: cirrusp4svc $
...
#define ADDRESS(reg)  (ab_ptr->base_address + ab_ptr->ab_reg_table->reg)
...
#define PM_AB_OUTPUT_MVOLT_STEP 25
...
pm_ab_config_default_volt(uint32 device_index, uint32 milli_volt)
{
  ...
  err_flag |= pm_comm_write_byte(SLAVE_ID, vout_default, code);
...
pm_err_flag_type pm_ab_get_ab_status(uint32 device_index, pm_ab_status_type *ab_status)
{
  ...
  else
  {
    ...
    ab_status->output_volt = ((data) * PM_AB_OUTPUT_MVOLT_STEP) + PM_AB_OUTPUT_MVOLT_BASE;
...
pm_err_flag_type pm_ab_config_output_volt(uint32 device_index, boolean override_output_volt, uint32 volt_millivolt)
{
  ...
  else
  {
    ...
    /*limit is 4600 to 5200*/
    volt_millivolt = (volt_millivolt < PM_AB_OUTPUT_MVOLT_MIN) ? PM_AB_OUTPUT_MVOLT_MIN : volt_millivolt;

    volt_millivolt = (volt_millivolt > PM_AB_OUTPUT_MVOLT_MAX) ? PM_AB_OUTPUT_MVOLT_MAX : volt_millivolt;

    data = (pm_register_data_type)((volt_millivolt - PM_AB_OUTPUT_MVOLT_BASE)/PM_AB_OUTPUT_MVOLT_STEP);

    err_flag = pm_comm_write_byte(SLAVE_ID, output_voltage, data);
  }
