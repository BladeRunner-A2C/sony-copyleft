...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/pbs/src/pm_pbs_info.c#1 $
$Author: cirrusp4svc $
 ...
 * @dependencies None.
 */
static pm_err_flag_type pm_pbs_info_otp_read (uint32 busid, uint32 slave_id, pm_pbs_info_smem_type *pbs_info_ptr)
{
  ...
  err_flag = pm_pbs_config_access(busid, slave_id, pbs_element_info_temp_addr, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_ROM, periph_id);
  ...
  err_flag = pm_comm_read_byte_array_ex(busid, slave_id, pm_pbs_data->perph_base_addr+MEM_INTF_RD_DATA0_OFFS, 4, (uint8*)&read_info_temp);
  ...
  err_flag = pm_pbs_disable_access(busid, slave_id, PM_PBS_TYPE_ROM, periph_id);
 ...
 * @dependencies None.
 */
static pm_err_flag_type pm_pbs_info_ram_read (uint32 busid, uint32 slave_id, pm_pbs_info_smem_type *pbs_info_ptr)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint32 read_info_temp = 0;
  uint16 pbs_element_info_temp_addr = 0;  /* Address from where PBS Info to be read */
  ...
  err_flag = pm_pbs_config_access(busid, slave_id, pbs_element_info_temp_addr, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM, periph_id);
...
static pm_err_flag_type pm_pbs_info_read_from_periph (uint32 busid, uint32 slave_id, pm_pbs_info_smem_type *pbs_info_ptr)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  ...
  err_flag = pm_comm_read_byte_ex(busid, slave_id, PM_REV_ID_SLAVE_COUNT_ADDR, &pmic_sid_count);
  ...
  if (pmic_sid_count != 1)
  {
    ...
    err_flag = pm_comm_read_byte_ex(busid, slave_id, pm_pbs_data->perph_base_addr + pm_pbs_data->tp_rev, &read_info_temp8);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      return err_flag;
    }
    ...
    err_flag = pm_comm_read_byte_array_ex(busid, slave_id, pm_pbs_data->perph_base_addr + pm_pbs_data->fab_id, 4, (uint8*)&read_info_temp32);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
    return err_flag;
    }
    ...
    err_flag = pm_comm_read_byte_array_ex(busid, slave_id, lot_info_addr, PM_PBS_NUM_LOT_ID_RES, lot_arry);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      return err_flag;
    }
    ...
    err_flag = pm_comm_read_byte_ex(busid, slave_id, pm_pbs_data->perph_base_addr + pm_pbs_data->mfg_id, &read_info_temp8);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      return err_flag;
    }
    ...
    err_flag = pm_comm_read_byte_array_ex(busid, slave_id, pm_pbs_data->perph_base_addr+MEM_INTF_RD_DATA0_OFFS, 4, (uint8*)&read_info_temp32);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      return err_flag;
    }
  ...
  if (pmic_sid_count == 1)
  {
    err_flag = pm_comm_read_byte_array_ex(busid, slave_id, PM_REV_ID_TP_REV_ADDR, 4, (uint8*)&rev_id_data);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      return err_flag;
    }
    ...
    err_flag = pm_comm_read_byte_array_ex(busid, slave_id, PM_REV_ID_TP_REV_ADDR + 0x04, 4, (uint8*)&rev_id_data);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      return err_flag;
    }
    ...
    err_flag = pm_comm_read_byte_array_ex(busid, slave_id, PM_REV_ID_TP_REV_ADDR + 0x08, 4, (uint8*)&rev_id_data);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      return err_flag;
    }
    ...
    err_flag = pm_comm_read_byte_array_ex(busid, slave_id, PM_REV_ID_TP_REV_ADDR + 0x0B, 4, (uint8*)&rev_id_data);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      return err_flag;
    }
    ...
    err_flag = pm_comm_read_byte_array_ex(busid, slave_id, PM_REV_ID_OTP_ID_ADDR, 1,(uint8*)&rev_id_data);        
...
pm_err_flag_type pm_pbs_info_rom_init (void) 
{
  ...
  for (busid = 0; busid < PM_MAX_BUSES_SUPPORTED; busid++)
  {
  for (pmic_index = 0; pmic_index < PM_MAX_NUM_PMICS; pmic_index++)
  {
    /* Get PMIC device primary slave id */
      err_flag = pm_get_slave_id_ex(busid, pmic_index, 0, &slave_id);
    ...
    /* Read PBS Peripheral info and check if PBS peripheral exists */
      ...
      err_flag = pm_comm_read_byte_ex(busid, slave_id, pm_pbs_data->perph_base_addr+PERIPH_TYPE_OFFSET_ADDR, &data);
    ...
    {
        err_flag = pm_pbs_info_read_from_periph (busid, slave_id, &(pm_pbs_info_arr[pmic_index]));
    }
    else if (PM_PBS_INFO_IN_OTP == pm_pbs_data->pbs_info_place_holder) 
    {
      /* Read all the PMIC's PBS Manufacturing IDs from PBS ROM */
        err_flag = pm_pbs_info_otp_read(busid, slave_id, &(pm_pbs_info_arr[pmic_index]));
    }
    else
    {
    ...
    {
      /* Configuring to disable PBS core read access */
        pm_pbs_disable_access(busid, slave_id, PM_PBS_TYPE_ROM, periph_id);
...
pm_err_flag_type pm_pbs_info_ram_init (void) 
{
  ...
  for (pmic_index = 0; pmic_index < PM_MAX_NUM_PMICS; pmic_index++)
  {
    ...
    {
          err_flag = pm_pbs_info_ram_read(busid, slave_id, &(pm_pbs_info_arr[pmic_index]));
    }

    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      /* Configuring to disable PBS core read access */
        pm_pbs_disable_access(busid, slave_id, PM_PBS_TYPE_RAM, periph_id);
...
pm_err_flag_type
pm_pbs_pon_rsn_sdam_read(uint32 busid, uint32 slave_id, uint32 start_address, uint32* dataset_ptr, uint32 dataset_size)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint32 read_data = 0;
  uint8 i = 0; 
  ...
  {
    err_flag |= pm_comm_read_byte_array_ex(busid, slave_id, start_address, 4, (uint8 *)&read_data);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      return err_flag;
    }
...
pm_err_flag_type pm_pbs_mem_read(uint8 busid, uint8 slave_id, uint32 start_address, uint32* data_ptr, uint32 data_size)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint32 read_data = 0;
  uint8 i = 0; 
  ...
  err_flag |= pm_pbs_config_access(busid, slave_id, start_address , PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM, periph_id);
  if (err_flag != PM_ERR_FLAG_SUCCESS)
  {
    err_flag |= pm_pbs_disable_access(busid, slave_id, PM_PBS_TYPE_RAM, periph_id);
    return err_flag;
  ...
  {
    err_flag |= pm_comm_read_byte_array_ex(busid, slave_id, pm_pbs_data->perph_base_addr+MEM_INTF_RD_DATA0_OFFS, 4, (uint8 *)&read_data);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      err_flag |= pm_pbs_disable_access(busid, slave_id, PM_PBS_TYPE_RAM, periph_id);
      return err_flag;
...
pm_err_flag_type pm_pbs_mem_write(uint8 busid, uint8 slave_id, uint32 start_address, uint32* data_ptr, uint32 data_size)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint8 i = 0; 
  pm_pbs_info_data_type *pm_pbs_data = NULL;
  ...
  /* Configuring to enable PBS core access for ROM reads */   
  err_flag |= pm_pbs_config_access(busid, slave_id, start_address , PM_PBS_ACCESS_WRITE_BURST, PM_PBS_TYPE_RAM, periph_id);
  if (err_flag != PM_ERR_FLAG_SUCCESS)
  {
    err_flag |= pm_pbs_disable_access(busid, slave_id, PM_PBS_TYPE_RAM, periph_id);
    return err_flag;
  ...
  for (i =0 ; i < data_size ; i++)
  {
    err_flag |= pm_comm_write_byte_array_ex(busid, slave_id, pm_pbs_data->perph_base_addr+MEM_INTF_WR_DATA0_OFFS, 4, (uint8*)&data_ptr[i]);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
      err_flag |= pm_pbs_disable_access(busid, slave_id, PM_PBS_TYPE_RAM, periph_id);
      return err_flag;
