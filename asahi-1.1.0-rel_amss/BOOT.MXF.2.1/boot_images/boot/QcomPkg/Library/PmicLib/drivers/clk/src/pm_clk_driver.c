...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/clk/src/pm_clk_driver.c#1 $
$Author: cirrusp4svc $
...
enum {
  ...
  PMIC_CLK_CALRC3_MAX_HW_VER = 0x41,
  PMIC_CLK_XO_MAX_HW_VER     = 0x91,
  PMIC_CLK_BB_MAX_HW_VER     = 0x90, 
  PMIC_CLK_RF_MAX_HW_VER     = 0x82, 
  PMIC_CLK_SLEEP_MAX_HW_VER  = 0xB0,  
  PMIC_CLK_DIST_MAX_HW_VER   = 0xE2, 
};
...
static pm_clk_data_type *pm_clk_data_arr[PM_MAX_BUSES_SUPPORTED][PM_MAX_NUM_PMICS];
                    ...
                    uint32 pmic_index, uint32 bus_id)
{
  ...
  if (pm_clk_data_arr[bus_id][pmic_index] == NULL)
  {
  ...
  if ((peripheral_info->peripheral_subtype == PM_HW_MODULE_CLOCK_CALRC2) ||
	  ...
	  (peripheral_info->peripheral_subtype == PM_HW_MODULE_CLOCK_TCXO_CAL))
  {
    return;
  }
  else if (peripheral_info->peripheral_subtype == PM_HW_MODULE_CLOCK_CALRC3)
  {
    return pm_calrc_driver_init(comm_ptr, peripheral_info, pmic_index, bus_id);
  }

  clk_ptr = pm_clk_data_arr[bus_id][pmic_index];
  ...
  if (clk_ptr == NULL)
  {
    ...
    pm_clk_data_arr[bus_id][pmic_index] = clk_ptr;
  }
  ...
  if (clk_ptr != NULL)
  {
    ...
    pm_clk_periph_arr_init(peripheral_info, bus_id, 
...
void 
pm_calrc_driver_init(pm_comm_info_type *comm_ptr, peripheral_info_type *peripheral_info, uint32 pmic_index, uint32 bus_id)
{
  pm_clk_data_type        *clk_ptr          = pm_clk_data_arr[bus_id][pmic_index];
...
pm_clk_data_type* 
pm_clk_get_data(uint32 bus_id, uint32 pmic_index)
{
  if((pmic_index < PM_MAX_NUM_PMICS) && (bus_id < PM_MAX_BUSES_SUPPORTED))
  {
    return pm_clk_data_arr[bus_id][pmic_index];
  }
  return NULL;
}

uint8 
pm_clk_get_num_peripherals(uint32 bus_id, uint32 pmic_index)
{
  if((pmic_index < PM_MAX_NUM_PMICS) && (bus_id < PM_MAX_BUSES_SUPPORTED) &&
      (pm_clk_data_arr[bus_id][pmic_index] !=NULL))
  {
    return pm_clk_data_arr[bus_id][pmic_index]->num_of_peripherals;
  }
