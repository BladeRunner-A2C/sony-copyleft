...
*  \n  
*  &copy; Copyright 2010-2019, 2021-2023 QUALCOMM Technologies Incorporated, All Rights Reserved
*/
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/smps/src/pm_smps.c#1 $
$Author: cirrusp4svc $
...
#define SMPS_AHC_CTL3_OFFSET_HFS 0x3F
#define SMPS_AHC_CTL1_OFFSET_HFS  0x3D
#define SMPS_AHC_MAX_HR_VALUE     0x7F
#define SMPS_OCP_CTL_LATCH_EN_ADDR 0x88
#define SMPS_OCP_CTL_LATCH_EN_MASK 0x40
#define GANG_EN 0x80
...
pm_err_flag_type pm_smps_set_uls(uint8 pmic_chip, uint8 smps_peripheral_index, pm_volt_level_type volt_level)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_smps_data_type *smps_ptr = pm_smps_get_data(pmic_chip);
...
pm_err_flag_type 
pm_smps_get_uls(uint8 pmic_chip, uint8 smps_peripheral_index, pm_volt_level_type* volt_level)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_smps_data_type *smps_ptr = pm_smps_get_data(pmic_chip);
  ...
  {    
    err_flag = pm_pwr_get_uls_alg(&(smps_ptr->pm_pwr_data), smps_peripheral_index, volt_level);
  }
...
pm_err_flag_type pm_smps_switching_freq(uint8 pmic_chip, uint8 smps_peripheral_index, pm_smps_switching_freq_type freq)
{
    ...
    {
        pm_register_address_type    reg = 0;
        pm_register_address_type    gang_ctrl = 0;
        ...
        pm_pwr_data_type            *pwr_data = &(smps_ptr->pm_pwr_data);
        pm_register_data_type       gang_config[2] = {0};
        ...
        gang_ctrl = smps_freq_base_addr + pwr_data->pwr_reg_table->GANG_CTL1;       
        err_flag |= pm_comm_read_byte_array(smps_ptr->comm_ptr->slave_id, gang_ctrl, 2, gang_config);
        
        if ( (gang_config[1] & 0x80) == GANG_EN) //gang enabled
        {
            reg = (gang_config[0] * 0x100) + pwr_data->pwr_reg_table->CLK_DIV; // gang_config[0] has gang lead PID
            
            err_flag |= pm_comm_write_byte_mask(smps_ptr->comm_ptr->slave_id, reg, 0x1F, (pm_register_data_type)freq); // <0:4>          
        }
        else if (pwr_data->pwr_specific_info[smps_peripheral_index].periph_type == PM_HW_MODULE_HF_BUCK)
        {
...
pm_err_flag_type 
pm_smps_ocp_latched_status_clear(uint8 pmic_chip, uint8 peripheral_index,  pm_vreg_ocp_info_type *ocp_info)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    peripheral_info_type  smps_peripheral_info;
    boolean is_broadcast_cfg_present = FALSE;
    ...
    {
        pm_pwr_data_type *pwr_data = &(smps_ptr->pm_pwr_data);
        memset(&smps_peripheral_info, 0, sizeof(peripheral_info_type));
        smps_peripheral_info.base_address = pwr_data->pwr_specific_info[peripheral_index].periph_base_address;
        pm_get_peripheral_info(pmic_chip, &smps_peripheral_info);
        
        if(smps_peripheral_info.peripheral_subtype == PM_HW_MODULE_FTS__FTS525_CTL)
        {
            is_broadcast_cfg_present= TRUE;
        }
        
        err_flag |= pm_pwr_ocp_latched_status_clear_alg(&(smps_ptr->pm_pwr_data), smps_ptr->comm_ptr, peripheral_index, is_broadcast_cfg_present, ocp_info);
    }
...
pm_err_flag_type pm_smps_ocp_broadcast(uint8 pmic_chip, uint8  smps_peripheral_index, pm_on_off_type  on_off)
{
    ...
    else
    {
        pm_pwr_data_type *pwr_data = &(smps_ptr->pm_pwr_data);
        memset(&smps_peripheral_info, 0, sizeof(peripheral_info_type));
        smps_peripheral_info.base_address = pwr_data->pwr_specific_info[smps_peripheral_index].periph_base_address;
        pm_get_peripheral_info(pmic_chip, &smps_peripheral_info);
        ...
        err_flag |= pm_pwr_ocp_broadcast_alg(&(smps_ptr->pm_pwr_data), smps_ptr->comm_ptr, smps_peripheral_index, is_broadcast_cfg_present, on_off);
    }
...
pm_err_flag_type pm_smps_ahc_enable(uint8 pmic_chip, uint8 smps_peripheral_index, pm_on_off_type on_off)
{
    ...
    else
    {
        ...
        if((smps_peripheral_info.peripheral_type == PM_HW_MODULE_HF_BUCK) && (smps_peripheral_info.digital_major_version <= 4))
        {
            reg = (pm_register_address_type)(smps_peripheral_info.base_address + SMPS_AHC_CTL3_OFFSET_HFS);
            data = (PM_OFF == on_off) ? 0 : 0x80;
            err_flag = pm_comm_write_byte_mask(pmic_chip, reg, 0x80, data);
        }
        else
        {
...
pm_err_flag_type pm_smps_ahc_status(uint8 pmic_chip, uint8 smps_peripheral_index, pm_on_off_type *on_off)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_register_address_type  reg  = 0x00;
    ...
    else
    {
        ...
        {
            reg = (pm_register_address_type)(smps_peripheral_info.base_address + SMPS_AHC_CTL3_OFFSET_HFS);
            err_flag = pm_comm_read_byte(pmic_chip, reg, &data);
            if (err_flag != PM_ERR_FLAG_SUCCESS)
            {
                *on_off = PM_INVALID;
                return err_flag;
        ...
        {
            err_flag = pm_pwr_ahc_status_alg(&(smps_ptr->pm_pwr_data), smps_ptr->comm_ptr, smps_peripheral_index, on_off);
        }  
    }
    return err_flag;
}
pm_err_flag_type pm_smps_ocp_latch_en
(uint8 pmic_chip, uint8  smps_peripheral_index, pm_on_off_type  on_off)
{
    ...
    {
        err_flag = pm_pwr_ocp_latch_en_alg(&(smps_ptr->pm_pwr_data), smps_ptr->comm_ptr, smps_peripheral_index, on_off);
    }

    return err_flag;
}

pm_err_flag_type pm_smps_ocp_enable( uint8 pmic_chip, uint8  smps_peripheral_index, pm_sw_mode_type sw_mode, pm_on_off_type  on_off)
{
    pm_err_flag_type err_flag     = PM_ERR_FLAG_SUCCESS;
    pm_smps_data_type *smps_ptr = pm_smps_get_data(pmic_chip);
    ...
    {
        err_flag = pm_pwr_ocp_en_alg (&(smps_ptr->pm_pwr_data), smps_ptr->comm_ptr, smps_peripheral_index, sw_mode, on_off);
    }
    return err_flag;
}

pm_err_flag_type pm_smps_ahc_set_headroom(uint8 pmic_chip, uint8 smps_peripheral_index, uint8 headroom)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_register_address_type  reg  = 0xFF;
    pm_register_data_type     data = 0xFF;
    ...
    else if (headroom > SMPS_AHC_MAX_HR_VALUE)
    {
        err_flag = PM_ERR_FLAG_INVALID_PARAMETER;
    }
    else
    {
        ...
        err_flag = pm_smps_ahc_status(pmic_chip,smps_peripheral_index, &on_off);
        if (PM_ON == on_off)
        {
           err_flag |= pm_smps_ahc_enable(pmic_chip, smps_peripheral_index, PM_OFF);
        } 
        
        if((smps_peripheral_info.peripheral_type == PM_HW_MODULE_HF_BUCK) && (smps_peripheral_info.digital_major_version <= 4))
        {
            reg = (pm_register_address_type)(smps_peripheral_info.base_address + SMPS_AHC_CTL1_OFFSET_HFS);
            data = headroom;
            err_flag |= pm_comm_write_byte_mask(pmic_chip, reg, 0x7F, data);
        }
        else
        {
            err_flag |= pm_pwr_ahc_set_reserve_headroom_alg(&(smps_ptr->pm_pwr_data), smps_ptr->comm_ptr, smps_peripheral_index, headroom);
        }   
        
        if(PM_ON == on_off)
        {
           err_flag |= pm_smps_ahc_enable(pmic_chip, smps_peripheral_index, PM_ON);	
        }
    }
...
pm_err_flag_type 
pm_smps_ocp_mode_status(uint8 pmic_chip, uint8 smps_peripheral_index,  boolean *ocp_mode_status)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    peripheral_info_type  smps_peripheral_info;
    ...
    else
    {
        ...
        if(smps_peripheral_info.peripheral_subtype == PM_HW_MODULE_FTS__FTS525_CTL)
        {
            err_flag |= pm_pwr_ocp_mode_status_alg(&(smps_ptr->pm_pwr_data), smps_ptr->comm_ptr, smps_peripheral_index, ocp_mode_status);
        }
        else
        {
            err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
