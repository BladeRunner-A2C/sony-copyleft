...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/bob/src/pm_bob.c#1 $
...
pm_err_flag_type 
pm_bob_ocp_latched_status_clear(uint8 pmic_chip, uint8 peripheral_index,  pm_vreg_ocp_info_type *ocp_info)
{
   pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
   pm_bob_data_type *bob_ptr = pm_bob_get_data(pmic_chip);
   ...
   {
	    if (bob_ptr->periph_subtype[peripheral_index] >= PM_HW_MODULE_BOB_CONFIG_PERIPH_SUBTYPE)
        {
            is_broadcast_cfg_present = TRUE;
        }
       err_flag |= pm_pwr_ocp_latched_status_clear_alg(&(bob_ptr->pm_pwr_data), bob_ptr->comm_ptr, peripheral_index, is_broadcast_cfg_present, ocp_info);
   }
