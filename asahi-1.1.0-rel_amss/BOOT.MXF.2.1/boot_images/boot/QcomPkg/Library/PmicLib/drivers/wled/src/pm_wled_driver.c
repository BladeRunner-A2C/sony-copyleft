...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/PmicLib/drivers/wled/src/pm_wled_driver.c#1 $
$Author: cirrusp4svc $
...
void pm_wled_driver_init(pm_comm_info_type *comm_ptr, peripheral_info_type *peripheral_info, uint8 pmic_index)
{
    ...
    if (NULL == wled_ptr)
    {
        num_of_peripherals = pm_target_information_get_periph_count_info(PM_PROP_WLED_NUM, pmic_index);

        if (!num_of_peripherals)
        {
          return;
        }
        ...
        wled_ptr->wled_reg_table->wled_ctrl_reg->base_address = peripheral_info->base_address;
        wled_ptr->wled_reg_table->wled_sink_reg->base_address = peripheral_info->base_address + 0x100;

        //wled_ptr->num_of_peripherals = (uint32)PM_MAX_NUM_WLED;
        wled_ptr->num_of_peripherals = num_of_peripherals;
