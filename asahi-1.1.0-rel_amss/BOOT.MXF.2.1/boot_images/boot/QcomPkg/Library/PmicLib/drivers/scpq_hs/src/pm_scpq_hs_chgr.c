/*! \file
*
*  \brief  pm_scpq_hs_chgr.c driver implementation.
...
#define ADDRESS(x) (scpq_hs_ptr->base_address + \
                    scpq_hs_ptr->chgr_register->peripheral_offset + \
                    scpq_hs_ptr->chgr_register->x)

#define SLAVE_ID   (scpq_hs_ptr->comm_ptr->slave_id)

#define PM_SCPQ_HS_FLOAT_VOLTAGE_STEP_CONSTANT           10
#define PM_SCPQ_HS_JEITA_FLOAT_VOLTAGE_STEP_CONSTANT     10
#define PM_SCPQ_HS_JEITA_CHARGE_CURRENT_STEP_CONSTANT    50
#define FLOAT_VOLTAGE_MIN 3600

#define PM_SCPQ_HS_CHGR_STATUS_COUNT 10
...
pm_err_flag_type pm_scpq_hs_chgr_set_charge_current(uint32 bus_id, uint32 pmic_index, pm_scpq_hs_chgr_current_type charge_current_type, uint32 current_in_ma)
{
  ...
  else if(charge_current_type >= PM_SCPQ_HS_CHGR_CURRENT_TYPE_INVALID)
  {
  ...
  else
  {
    ...
    {
    case PM_SCPQ_HS_CHGR_PRE_CHARGE_CURRENT:
    ...
    case PM_SCPQ_HS_CHGR_FAST_CHARGE_CURRENT:
      reg = ADDRESS(ichg_cfg);
      mask = 0xFF;  // BIT<7:0>
      step_size = 50;
      max_value = 10000;  // max 10A for the case SMB is connected. TODO : 12A ?
      min_value = 0;
      break;

    case PM_SCPQ_HS_CHGR_TERMINATION_CURRENT:
    ...
    if (charge_current_type != PM_SCPQ_HS_CHGR_PRE_CHARGE_CURRENT)
    { 
...
pm_err_flag_type pm_scpq_hs_chgr_get_charge_current_cfg(uint32 bus_id, uint32 pmic_index, pm_scpq_hs_chgr_current_type charge_current_type, uint32* current_in_ma)
{
  ...
  else if((charge_current_type >= PM_SCPQ_HS_CHGR_CURRENT_TYPE_INVALID) || (NULL == current_in_ma))
  {
  ...
  else
  {
    ...
    if(PM_SCPQ_HS_CHGR_PRE_CHARGE_CURRENT == charge_current_type)
    {
...
pm_err_flag_type pm_scpq_hs_chgr_get_chgr_status(uint32 bus_id, uint32 pmic_index, pm_scpq_hs_chgr_status_type *chgr_status)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_scpq_hs_data_type *scpq_hs_ptr = pm_scpq_hs_get_data(bus_id, pmic_index);
  pm_register_data_type data[PM_SCPQ_HS_CHGR_STATUS_COUNT] = {0};
  ...
  else
  {
    ...
    err_flag = pm_comm_read_byte_array(SLAVE_ID, status_set, PM_SCPQ_HS_CHGR_STATUS_COUNT, data);

    /*charger_status related*/
    chgr_status->charger_status.battery_charger_status = (pm_scpq_hs_battery_charger_status)(data[0] & 0x7);
...
pm_err_flag_type pm_scpq_hs_chgr_set_float_volt(uint32 bus_id, uint32 pmic_index, uint32 float_volt_mv)
{
  ...
  else
  {
    ...
    data = (float_volt_mv - FLOAT_VOLTAGE_MIN + PM_SCPQ_HS_FLOAT_VOLTAGE_STEP_CONSTANT / 2)
           / PM_SCPQ_HS_FLOAT_VOLTAGE_STEP_CONSTANT;
    if(data > 0x77)
    {
      data = 0x77;
    }
...
pm_err_flag_type pm_scpq_hs_chgr_get_float_volt_cfg(uint32 bus_id, uint32 pmic_index, uint32 *float_volt_mv)
{
  ...
  else
  {
    ...
    *float_volt_mv = FLOAT_VOLTAGE_MIN + (data * PM_SCPQ_HS_FLOAT_VOLTAGE_STEP_CONSTANT);
  }
...
pm_err_flag_type pm_scpq_hs_chgr_vph_track_threshold_sel(uint32 bus_id, uint32 pmic_index, pm_scpq_hs_chgr_vph_track_sel_type track_sel)
{
  ...
  else if(track_sel >= PM_SCPQ_HS_VPH_TRACK_SEL_INVALID)
  {
    err_flag = PM_ERR_FLAG_INVALID_PARAMETER;
  }
  else
...
pm_err_flag_type pm_scpq_hs_chgr_get_scpq_hs_pmic_info(uint32 bus_id,uint32 *pmic_index, uint32 *slave_id, uint32* charger_count, uint32 max_charger)
{
  ...
  *charger_count = 0;
  pm_scpq_hs_get_pmic_info(0,pmic_index, slave_id, charger_count, max_charger);
