...
/*=============================================================================
  ...
  ----------   ---     -----------------------------------------------------------
  12/22/2022   kac     Warning raised when Invalid arg is passed
    ...
    else
    {
      ...
      *ArgvPtr = CmdStr;
      ...
      while (1)
      {
        ...
        if (((*CmdStr == ' ') && (*(CmdStr + 1) == DOUBLE_QUOTE_CHAR)) || ((*CmdStr == DOUBLE_QUOTE_CHAR) && (*(CmdStr + 1) == ' ')))
        {
          AsciiPrint("WARNING!! INVALID ARGS PASSED...\n");
        }
...
{
  EFI_STATUS                  CmdStatus = EFI_SUCCESS;
  ...
  *ArgvArrayPtr = ArgStrPtr;
  ...
  if (WideCharArg)
    CmdStatus = UnicodeStrToAsciiStrS ((CHAR16 *)CmdStr, (AllocdMem + ArgsSize), AllocSize - ArgsSize);
  else
    CmdStatus = AsciiStrnCpyS ((AllocdMem + ArgsSize), AllocSize - ArgsSize, CmdStr, AllocSize - ArgsSize);
  
  if(CmdStatus == EFI_SUCCESS)
    CmdStr = (AllocdMem + ArgsSize);
