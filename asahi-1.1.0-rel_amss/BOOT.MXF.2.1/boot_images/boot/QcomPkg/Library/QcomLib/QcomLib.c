/** @file QcomLib.c
  ...
  Copyright (c) 2010-2019,2020,2021,2022 Qualcomm Technologies, Inc. All Rights Reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 10/14/22   yps     Remove unused variables
 08/08/22   cm      Added support for Pxe
 08/16/21   kpa     Make MountFatPartitionExt robust, fix GetBlkIOHandles
 04/12/21   bpk     Added ConnectAllDrivers()
 03/05/21   visver  Added New API GetECID
 03/03/21   bpk     PartiSelectFilter update
 10/08/20   yw      remove STATIC in GetProcessorName() for thread safe
 04/16/20   cm      Changed datatype of Fmode to UINT64
 07/19/18   ps      New platformType added in PlatformTypeStrings
 03/21/18   kpa     Added new API MountFatPartitionExt
...
EFI_STATUS
FindApplicationMatchingUiSection (IN  CHAR16      *UiString OPTIONAL,
                                  OUT EFI_HANDLE  *FvHandle,
                                  OUT EFI_GUID    *NameGuid)
{
  EFI_STATUS                    Status;
  ...
  // if this is NULL then returns the first file encountered
  if (UiString != NULL)
    UiStringLen = StrLen (UiString);

  Status = gBS->LocateHandleBuffer (ByProtocol, &gEfiFirmwareVolume2ProtocolGuid,
  ...
  {
    Status = gBS->HandleProtocol (Buffer[Index], &gEfiFirmwareVolume2ProtocolGuid,
                                  (VOID **)&Fv);
    if (EFI_ERROR (Status))
      return Status;
    ...
    do {
      ...
      Status = Fv->ReadSection (Fv, NameGuid, EFI_SECTION_USER_INTERFACE, 0,
                                (VOID **)&UiSection, &Size, &Authentication);
      if (EFI_ERROR (Status))
        break;
...
EFI_STATUS
LoadPeCoffSectionFromFv (IN  EFI_HANDLE   FvHandle, IN  EFI_GUID     *NameGuid)
{
  EFI_STATUS                    Status;
  EFI_DEVICE_PATH_PROTOCOL      *DevicePath;
...
EFI_STATUS
LaunchFromFv (IN  CHAR16      *UiString)
{
  EFI_HANDLE                FvHandle;
  EFI_GUID                  NameGuid;
  EFI_STATUS                Status;
...
GetNativeKeypad (OUT  EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL    **KeypadProtocol)
{
  ...
  for (Index = 0; Index < KeypadHandleCount; Index++)
  {
    ...
    if (((RootDevicePath->Header.Length[0] |
         (RootDevicePath->Header.Length[1] << 8)) != sizeof(VENDOR_DEVICE_PATH)) ||
...
{
  /* Wrapper API maintainted for backward compatibility
    It reformats arg and calls into MountFatPartitionExt
  */
  return MountFatPartitionExt(NULL, NULL, PartitionLabel);
}
...
)
{
  ...
  /* One argument (out of PartitionType and PartitionLabel )has to be NULL
     and other non-NULL to support correct option selection
  */

  if ((PartitionType == NULL) && (PartitionLabel == NULL))
    return EFI_INVALID_PARAMETER;
  ...
  if (PartitionLabel != NULL)
    DEBUG ((EFI_D_INFO, "Mounting FAT Volume: %s\n", PartitionLabel));
  else
    DEBUG ((EFI_D_INFO, "Mounting FAT Volume: by partition GUID \n"));

  if (RootDeviceType != NULL)
    DEBUG ((EFI_D_INFO, "Mounting FAT Volume: by Root Device GUID \n"));
  ...
  if (EFI_ERROR(Status))
  {
    DEBUG ((EFI_D_INFO, "[MountFatExt] Failed to get handle\n"));
    return Status;
  }
  if (MaxHandles != 1)
  {
    DEBUG ((EFI_D_WARN, "[MountFatExt] Failed to get unique handle\n"));
    return EFI_NOT_FOUND;
  }  
  ...
  if (EFI_ERROR(Status))
  {
    DEBUG ((EFI_D_WARN, "[MountFat] Failed to connect controller\n"));
    return Status;
  }

  /*Check if FAT file System indeed got mounted */
  Status = gBS->HandleProtocol (HandleInfoList[0].Handle,
                                &gEfiSimpleFileSystemProtocolGuid, (VOID**)&Fs);
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN, "[MountFatExt] Failed to mount FAT file system \n"));
  }
  ...
  )
{
  ...
  Status = gBS->HandleProtocol(UserHandle, &gEfiPartitionRecordGuid,
                               (VOID **) &PartEntry);
  ...
  Status = gBS->HandleProtocol(UserHandle, &gEfiBlockIoProtocolGuid,
                               (VOID **) &UserBlkIo);
  ...
  Status = gBS->LocateHandleBuffer (ByProtocol,
                                    &gEfiSimpleFileSystemProtocolGuid,
                                    NULL, &FatHandleCount, &FatHandles);
  if (EFI_ERROR(Status))
    return Status;
  ...
  for (i = 0; i < FatHandleCount; i++)
  {
    ...
    Status = gBS->HandleProtocol (FatHandles[i], &gEfiBlockIoProtocolGuid,
                                  (VOID **) &BlkIo);
    /* Fv volumes will not support Blk I/O protocol */
    if (Status == EFI_UNSUPPORTED)
      continue;
                    ...
                    OUT     CHAR8** ChipVerStr       OPTIONAL)
{
  ...
  AsciiStrnCpyS (TempPtr, BufferSize + 1, PlatformTypeStrings[PlatformInfo.platform], BufferSize);
  ...
  Status = pChipInfo->GetChipIdString(pChipInfo, TempPtr, BufferSize);
                  ...
                  IN      UINTN    StrSize)
{
  ...
  AsciiStrCpyS (ProcessorNameString, MAX_PROCESSOR_STR_LEN, "Qualcomm Snapdragon ");
  ...
  pChipInfo->GetMarketingNameString (pChipInfo,
  ...
  AsciiStrCpyS (ProcNameStr, StrSize + 1, ProcessorNameString);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  Status = RootFileHandle->Open(RootFileHandle, FileHandle, Path, OpenMode, 0);
  if (EFI_ERROR(Status))
    *FileHandle = NULL;
  ...
  )
{
  ...
  if (Status == EFI_BUFFER_TOO_SMALL)
  {
    ...
    if (Info == NULL)
    {
      DEBUG ((EFI_D_WARN, "ERROR: Memory Allocation failed !\n"));
      return EFI_OUT_OF_RESOURCES;
    }
  ...
  )
{
  ...
  if ((FileBuffer == NULL) || (BytesToRead == NULL) || (*BytesToRead == 0) ||
      ((*BytesToRead + ReadOffset) > FileBufferSize))
    return EFI_INVALID_PARAMETER;

  // Open File
  Status = OpenFile(Path, RootDeviceType, PartitionType, PartitionLabel,
                    SelectNonRemovable, VolumeName, EFI_FILE_MODE_READ, &FileHandle);

  if ((EFI_ERROR(Status)) || (FileHandle == NULL))
    return Status;

  // Set read-offset
  Status = FileHandle->SetPosition (FileHandle, ReadOffset);
  ...
  // Read file content
  Status = FileHandle->Read(FileHandle, BytesToRead, FileBuffer);
...
EFI_STATUS
_StartPxeProcess(
    IN OUT  EFI_PXE_BASE_CODE_PROTOCOL            **PxePtr,
    IN OUT EFI_IP_ADDRESS                         *ServerIpAddr
    )
{
  ...
  if (EFI_ERROR(Status) || Pxe == NULL) {
    DEBUG ((DEBUG_NET, "Failed to locate Protocol gEfiPxeBaseCodeProtocolGuid; Status: %r \r\n", Status));
    return Status;
  }

  Status = Pxe->Start (Pxe, FALSE);
  if (EFI_ERROR(Status) && (Status != EFI_ALREADY_STARTED)) {
    DEBUG ((DEBUG_NET, "Failed to start PXE; Status: %r \r\n", Status));
    return Status;
  }

  Status = Pxe->Dhcp(Pxe, TRUE);
  if (EFI_ERROR(Status)  && (Status != EFI_ALREADY_STARTED)) {
    DEBUG ((DEBUG_NET, "Failed to do DHCP in PXE;Status:%r \r\n", Status));
    return Status;
  }

   CopyMem (&IpAddress, Pxe->Mode->DhcpAck.Dhcpv4.BootpSiAddr, sizeof (EFI_IPv4_ADDRESS));
  /***********************************************************************************
    Due to some wrong DHCP configuration , some time DHCP ACK do not send server IP 
	over BootpSiAddr. add this check and use DHCP option list to get server address.
	DHCP ACK Option array 5th index have server address.
  ************************************************************************************/
  if(IpAddress.v4.Addr[0] == 0) 
    CopyMem (&IpAddress.v4, Pxe->Mode->DhcpAck.Dhcpv4.DhcpOptions+DHCP_SERVER_OPTION_INDEX, sizeof (EFI_IPv4_ADDRESS));
...
/**
 Return the size of a file, through PXE that is in a network path
 
 @param  Path                File path/network path
 @param  FileSize            Size of file
 @return EFI_SUCCESS         On successfully retreiving file-size
    ...
    )
{
  ...
  CHAR8                                 *FilePath = AllocatePool((StrnLenS(Path, MAX_FILE_PATH_LENGTH_IN_NETWORK) + 1));
  ...
  Status = UnicodeStrToAsciiStrS(Path, FilePath, StrnLenS(Path, MAX_FILE_PATH_LENGTH_IN_NETWORK) + 1);
  if (EFI_ERROR(Status)) {
    DEBUG ((EFI_D_ERROR, "UnicodeStrToAsciiStrS returned Status:%r \r\n", Status));
    goto Cleanup;
  }

  Status = _StartPxeProcess(&Pxe, &ServerIp);
  if (EFI_ERROR(Status) || Pxe == NULL) {
	  DEBUG ((EFI_D_ERROR, "PXE FAILED \r\n", Status));
    goto Cleanup;
  }
  ...
  if (EFI_ERROR(Status)){
    DEBUG ((EFI_D_ERROR, "Failed to perform MTFTP in PXE;Status:%r \r\n", Status));
    goto Cleanup;
  }

  *FileSize = (UINT32)BufferSize;
...
/**
 Read a file Over Pxe network to a  buffer
      
 @param  Path                File path
 @param  FileBufferPtr       Data read from file is copied to FileBuffer
 @param  FileBufferSize      This parameter is set to the size of the buffer 
                             specified by the FileBufferPtr parameter.
                             If EFI_BUFFER_TOO_SMALL is returned,
                             *FileBufferSize returns the size of the requested file.
    ...
    )
{
  ...
  EFI_IP_ADDRESS                        ServerIp;
  CHAR8                                 *FilePath = AllocatePool((StrnLenS(Path, MAX_FILE_PATH_LENGTH_IN_NETWORK) + 1));
  UINT64                                *BufferSize = NULL;
  
  if((Path == NULL) || (FileBufferPtr == NULL))
  {
  ...
  Status = UnicodeStrToAsciiStrS(Path, FilePath, StrnLenS(Path, MAX_FILE_PATH_LENGTH_IN_NETWORK) + 1);
  if (EFI_ERROR(Status)) {
    DEBUG ((DEBUG_LOAD, "UnicodeStrToAsciiStrS returned Status:%r \r\n", Status));
    goto Cleanup;
  }
  ...
  if (EFI_ERROR(Status)){
    DEBUG ((DEBUG_LOAD, "Failed to perform MTFTP in PXE;Status:%r \r\n", Status));
    goto Cleanup;
  }

  *FileBufferSize = (UINTN)BufferSize;
  DEBUG ((DEBUG_INFO, "TFTP Succeeded\r\n"));
...
/**
 Load given file from server, through PXE that is in a network path
 
 @param  Path                File path/network path
 @param  FileSize            file size
 @param  BufferPtr           Buffer Ptr
 @return EFI_SUCCESS         On successfully download file
    ...
    )
{
  ...
  *BufferPtr = (UINT8*)AllocatePages(ElfSizeAligned/EFI_PAGE_SIZE); 
  ...
  DEBUG ((DEBUG_LOAD, "%s downloading\r\n", Path+1)); 
   Status = Pxe->Mtftp(
      Pxe,
      EFI_PXE_BASE_CODE_TFTP_READ_FILE,
      *BufferPtr,
  ...
  if (Status == EFI_SUCCESS){
	DEBUG ((DEBUG_LOAD, "File downloaded\n")); 
    Pxe->Stop(Pxe);
  }else{
  ...
  )
{
  ...
  if ((FileBuffer == NULL) || (BytesToWrite == NULL) || (*BytesToWrite == 0) ||
      ((*BytesToWrite + WriteOffset) > FileBufferSize))
    return EFI_INVALID_PARAMETER;

  FMode = EFI_FILE_MODE_READ|EFI_FILE_MODE_WRITE|EFI_FILE_MODE_CREATE;
  // Open File
  Status = OpenFile (Path, RootDeviceType, PartitionType, PartitionLabel,
                     SelectNonRemovable, VolumeName, FMode, &FileHandle);

  if ((EFI_ERROR(Status)) || (FileHandle == NULL))
    return Status;

  // Set write-offset
  Status = FileHandle->SetPosition(FileHandle, WriteOffset);

  if (!EFI_ERROR(Status))
  {
    // Write file content
    Status = FileHandle->Write(FileHandle, BytesToWrite, FileBuffer);
...
)
{
  ...
  //Open File
  Status = RootFileHandle->Open (RootFileHandle, &FileHandle, Path, (UINT64)EFI_FILE_MODE_READ, 0);
  ...
  FileInfo = NULL;
  ReadSize = 0;
  Status = FileHandle->GetInfo (FileHandle, &gEfiFileInfoGuid, &ReadSize, FileInfo);
  ...
  if (Status == EFI_SUCCESS)
  {
    *CfgBuffPtr = FileBuffer;
    if (ReadFileSize && FileInfo != NULL)
...
FlushKeys (VOID)
{
  ...
  do{
    /* Wait 5 ms before reading, helps with USB keyboards when handling
     * hotkey reading */
    gBS->Stall (5000);

    Status = ReadAnyKey (0, READ_KEY_ATTRIB_NO_BLOCKING);
  ...
  )
{
  ...
  if ((UiString == NULL) || (DataBuffer == NULL) || (DataSize == NULL))
    return EFI_INVALID_PARAMETER;
  ...
  for (Index = 0; Index < NoHandles; Index++)
  {
    ...
    do
    {
      ...
      Status = Fv->ReadSection (Fv, &NameGuid, EFI_SECTION_USER_INTERFACE,
                                0, (VOID **)&UiSection, &UiSectionSize, &Authentication);
      if (EFI_ERROR (Status))
      ...
      if (StrnCmp (UiString, UiSection, UiStringLen) == 0)
      {
        ...
        if ((DataBuffer != NULL) && (DataSize != NULL))
        {
          ...
          *DataSize       = 0;
          ...
          Status = Fv->ReadSection (Fv, &NameGuid, EFI_SECTION_RAW, 0,
                                    DataBuffer, DataSize, &Authentication);
...
EFIAPI
EFI_STATUS ConnectByProtocol (EFI_GUID *ProtocolGuid)
{
  EFI_STATUS  Status;
...
EFIAPI
EFI_STATUS
GetUUIDfromMSM (OUT VOID *Buffer,
                IN UINT32 BufferSize)
{
  EFI_STATUS Status;
  EFI_CHIPINFO_PROTOCOL *pChipInfoProtocol = NULL;
  EFIChipInfoSerialNumType chipSerialNum = 0;
  UINT8 chip_serial_num_expanded[16];
  EFI_HASH2_OUTPUT Hash2Output;
  EFI_HASH2_PROTOCOL *pEfiHash2Protocol = NULL;
  EFI_GUID *HashAlgorithm = &gEfiHashAlgorithmSha1Guid;
  UINTN DigestSize = 0;

  if ((Buffer == NULL) || (BufferSize > 16))   // Up to first 16 bytes of serial number can be returned in current implementation
  {
    DEBUG ((EFI_D_WARN,"Invalid parameters passed to GetUUID\n"));
    return EFI_INVALID_PARAMETER;
  }

  /* Get ChipInfo protocol handle */
  Status = gBS->LocateProtocol (&gEfiChipInfoProtocolGuid, NULL,
                               (VOID **) &pChipInfoProtocol);
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN, "LocateProtocol: ChipInfo unsuccessful! status: %d\n", Status));
    return Status;
  }

  // get 32-bit MSM serial number
  Status = pChipInfoProtocol->GetSerialNumber(pChipInfoProtocol, &chipSerialNum);
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN,"Failed to get MSM serial number\n"));
    return Status;
  }

  // fill in chip_serial_num_expanded
  ZeroMem(chip_serial_num_expanded, sizeof(chip_serial_num_expanded));
  CopyMem(chip_serial_num_expanded, &chipSerialNum, sizeof(EFIChipInfoSerialNumType));

  /* Get Hash2 protocol handle */
  Status = gBS->LocateProtocol(&gEfiHash2ProtocolGuid, NULL,
                               (VOID **)&pEfiHash2Protocol);
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN, "LocateProtocol: EfiHash2Protocol unsuccessful! status: %d\n", Status));
    return Status;
  }

  // hash the value of chipSerialNum using SHA1 hashing algorithm
  Status = pEfiHash2Protocol->GetHashSize(pEfiHash2Protocol, HashAlgorithm,
                                          &DigestSize);

  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN, "GetHashSize unsuccessful! status: %d\n", Status));
    return Status;
  }
  if (DigestSize != 20) // SW_SHA1_DIGEST_SIZE = 20
  {
    DEBUG ((EFI_D_WARN, "GetHashSize: DigestSize is not an expected value!\n"));
    return Status;
  }
  ...
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN, "HashInit unsuccessful! status: %d\n", Status));
    return Status;
  }

  Status = pEfiHash2Protocol->HashUpdate (pEfiHash2Protocol, (UINT8*)&chip_serial_num_expanded,
                                          sizeof(chip_serial_num_expanded));
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN, "HashUpdate unsuccessful! status: %d\n", Status));
    return Status;
  }
  ...
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN, "HashFinal unsuccessful! status: %d\n", Status));
    return Status;
  }
  ...
  IN UINT32 BufferSize)
{
  ...
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN,"Failed to get BlkIo handles\n"));
    return Status;
    return Status;
  }
  ...
  for (j = 0; j < NumberOfSdccHandles; j++)
  {
    ...
    if (Status != EFI_SUCCESS)
    {
      DEBUG ((EFI_D_WARN,"Failed to get Card Info! status: %d\n", Status));
      return Status;
    }
...
#define MAX_DEBUG_MESSAGE_LENGTH_PER_LOOP  0x100
...
{
  EFI_STATUS    Status = EFI_SUCCESS;
  UINTN            BufferSize = LOG_BUFFER_SIZE + 1; // 1 extra for NULL char
  CHAR8            *Buffer = AllocatePool(BufferSize);
  CHAR8            *OutputBuffer = AllocatePool(BufferSize);
  UINT32        Index = 0;
  EFI_QCOMDEBUG_PROTOCOL  *QcomDebugProtocol = NULL;
  UINT32        BytesPerLoop = 0;
  UINTN         Offset = 0;

  if ((Buffer == NULL)||(OutputBuffer == NULL))
  {
    DEBUG ((EFI_D_WARN, "ERROR: Could not allocate space for dump buffer\r\n"));
    return EFI_OUT_OF_RESOURCES;
  }

  SetMem(Buffer, BufferSize, 0x00);
  SetMem(OutputBuffer, BufferSize, 0x00);
  Status = gBS->LocateProtocol(&gEfiQcomDebugProtocolGuid,
                               NULL,
                               (VOID**) &QcomDebugProtocol);
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN, "WARNING: Could not find protocol to dump buffer\r\n"));
    goto Exit;
  }
  ...
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN, "ERROR: Failed to get debug log. Status = 0x%08x\r\n", Status));
    goto Exit;
  }
  ...
  DEBUG ((EFI_D_WARN, "TZApp Log Buffer: (Size = %d)\r\n",BufferSize));
  DEBUG ((EFI_D_WARN, "-- START -- \r\n"));

  for (Offset = 0; Offset < BufferSize; Offset += BytesPerLoop)
  {
    BytesPerLoop = MIN(BufferSize - Offset, MAX_DEBUG_MESSAGE_LENGTH_PER_LOOP);
    CopyMem (OutputBuffer + Offset, Buffer + Offset, BytesPerLoop);

    DEBUG ((EFI_D_WARN, "%a", (CHAR8 *)((UINTN)OutputBuffer + Offset)));
  }
...
/**
  ...
  identify the partition by label or GUID, but not both

  @param[in]  PartitionLabel    GUID of the partition from where FV gets loaded
  ...
  @param[out] ReadBuffer        Buffer cointaining loaded FV
        [out] ImageSize         Size of data read
        [in]  Attrib            Attributes for filter if any different from default like removable
    ...
    )
{
  ...
  if ((PartitionLabel == NULL && PartitionGuid == NULL) ||
     (PartitionLabel != NULL && PartitionGuid != NULL))
  {
    DEBUG ((EFI_D_WARN, "ReadFromPartitionWithAttrib requires either partition label OR Guid\n"));
    return EFI_INVALID_PARAMETER;
  }
  
  Status = GetBlkIO(PartitionGuid, PartitionLabel, &BlkIo, Attrib); 
...
/**
  Write the data in buffer to user requested partition

  @param[in]  PartitionGUID     GUID of the partition from where FV gets loaded
  @param[in]  PartitionLabel    Label of the partition from where FV gets loaded
  @param[out] Buffer            Buffer cointaining loaded FV
        [out] BufferSize        Size of data to write
        [in]  Attrib            Attributes for filter if any different from default like removable
    ...
    )
{
  ...
  UINTN                  BlockAlignedBufferSize = 0;
  UINTN                  BlockSize              = 0;
  ...
  if ((PartitionLabel == NULL && PartitionGuid == NULL) ||
     (PartitionLabel != NULL && PartitionGuid != NULL))
  {
    DEBUG ((EFI_D_WARN, "WriteToPartitionWithAttrib requires either partition label OR Guid\n"));
    return EFI_INVALID_PARAMETER;
  }
  ...
  if (BufferSize > PartitionSize){
    AsciiPrint("File is larger (%d) than partition (%d)\r\n", BufferSize, PartitionSize);
    return EFI_UNSUPPORTED;
  }

  /* Roundup user buffer to blocks*/
  if(BufferSize % BlockSize != 0)
  {
    BlockAlignedBufferSize = ((BufferSize/BlockSize)+1)*BlockSize;
    BlockAlignedBuffer =  AllocateZeroPool(BlockAlignedBufferSize);
    if(BlockAlignedBuffer == NULL)
    {
      return EFI_UNSUPPORTED;
    }

    CopyMemS(BlockAlignedBuffer, BlockAlignedBufferSize, Buffer, BufferSize);
    BufferSize = BlockAlignedBufferSize;
    Buffer = BlockAlignedBuffer;
  }
    ...
    )
{
  ...
  FilterData.PartitionType = PartitionGuid;
  FilterData.PartitionLabel = PartitionLabel;

  HandleCount = sizeof(HandleInfoList) / sizeof(*HandleInfoList);

  Status = GetBlkIOHandles(Attrib, &FilterData, HandleInfoList, &HandleCount);
  ...
  if (HandleCount > 1)
  {
    ...
    DEBUG ((EFI_D_WARN, "Warn..!! Found duplicate partitions, using the first one to continue\n"));
  }
...
EFIAPI
EFI_STATUS
GetECID (OUT UINT64 *Buffer)
{
  EFI_STATUS Status;
  EFI_CHIPINFO_PROTOCOL *pChipInfoProtocol = NULL;
  EFIChipInfoSerialNumType chipSerialNum = 0;
  EFIChipInfoQFPROMChipIdType chipQfpromId = 0;
  UINT64 chipEcid = 0;


  if (Buffer == NULL)
  {
    DEBUG ((EFI_D_WARN,"Invalid parameters passed to GetECID\n"));
    return EFI_INVALID_PARAMETER;
  }
  ...
  // get QFPROM Chip ID
  Status = pChipInfoProtocol->GetQFPROMChipId(pChipInfoProtocol, &chipQfpromId);
  if (Status != EFI_SUCCESS)
  {
    DEBUG ((EFI_D_WARN,"Failed to get QFPROM ChipID\n"));
    return Status;
  }
  ...
  chipEcid = chipQfpromId;
  chipEcid = chipEcid<<32;
  chipEcid |= chipSerialNum;
  *Buffer = chipEcid;
  ...
  )
{
  ...
  UINT64                    start_connect, end_connect;
  EFI_HANDLE                *HandleBuffer;
  EFI_STATUS                Status;
  
  start_connect = GetPerformanceCounter();
  ...
  end_connect = GetPerformanceCounter();
  
  DEBUG((EFI_D_ERROR,"Connect All Dxe in [%d]msec\n",ConvertTimerCountms(end_connect-start_connect)));
