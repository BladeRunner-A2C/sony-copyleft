 ...
 *
 * Copyright (c) 2008-2010, 2012-2014,2017, 2020-2021, 2023 Qualcomm Technologies, Inc. All rights reserved.
 ...
 *   order.
 *
 *   $Header: //components/dev/core.boot/5.1.1/glosch.core.boot.5.1.1.nand_cfg_olympic/QcomPkg/Library/NandJtagToolsLib/nand_partition.c#1 $ $DateTime: 2022/04/04 03:16:28 $ $Author: glosch $
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 09/21/23     tvra    Padded size vs image size check skipped for auto.
 * 01/07/21     sa      Changed the API to accept 1 or more page reads.
 * 06/20/20     sa      Allow nand_open for all the partitions.
...
static enum nand_block_state blk_state;
...
uint32 flash_prog_print_mibib(void)
{
  ...
  latest_mibib_block = mibib.flash_info.nand_info.mibib_info.new_mibib_block;
...
static uint32 flash_prog_print_mibib_pages(int mibib_block)
{
  ...
  page = (mibib.flash_info.nand_info.pages_per_block * mibib_block);

  result = flash_prog_read_pages(mibib.flash_handle, &mibib.flash_info,
    page, 1, NAND_ACCESS_MAIN, mibib_pg_buf);
...
uint32 flash_prog_print_layout(void)
{
  ...
  page = (mibib.flash_info.nand_info.pages_per_block * latest_mibib_block) +
  MIBIB_PAGE_PARTITION_TABLE;
  ...
  {
    FLASHLOG ( 1,("  %-20s", (char *)&parti_sys->part_entry[i].name));
    offset = parti_sys->part_entry[i].offset;
    FLASHLOG ( 1,("0x%-12x", offset));
    limit = parti_sys->part_entry[i].length;
    if (limit == -1)
    {
      limit = mibib.flash_info.nand_info.block_count;
      FLASHLOG ( 1,("0x%-10x", limit));
    }
    else
    {
      FLASHLOG ( 1,("0x%-10x", (offset + limit)));
    }

    page = mibib.flash_info.nand_info.pages_per_block * offset;

    result = flash_prog_read_pages(mibib.flash_handle, &mibib.flash_info,
      page, 1, NAND_ACCESS_MAIN, is_erased_pg_buf);
...
int flash_prog_init_for_partition_image (parti_ctl_t pctl, char *pname)
{
  ...
  /* Probe the device and get the handle*/
   if (NAND_DEVICE_DONE == nand_open(0, 
		(const unsigned char *)pname, &pctl->flash_handle))
   {
  ...
  if (FLASH_DEVICE_DONE == nand_get_info(pctl->flash_handle,
    &pctl->flash_info.nand_info))
  {
    FLASHLOG(3, ("Device Initialization: nand_get_info successful\n"));
  }
  else
  {
  ...
  FLASHLOG (3,("Pages/Block is %1d \n",
    (unsigned long int)pctl->flash_info.nand_info.pages_per_block));
  FLASHLOG (3,("Page size is 0x%x \n",
    (unsigned long int)pctl->flash_info.nand_info.page_size_bytes));
  FLASHLOG (3,("Block Count is 0x%x \n",
    (unsigned long int)pctl->flash_info.nand_info.block_count));
  FLASHLOG (3,("Attributes is 0x%x \n",
    (unsigned long int)pctl->flash_info.nand_info.attributes));

  /* Init some things that must be re-inited on every invocation. */
  pctl->start_addr_loaded = FALSE;
  pctl->partial_page_index = 0;
  pctl->page_size = pctl->flash_info.nand_info.page_size_bytes;
  pctl->write_ops = NAND_ACCESS_MAIN;
...
{
  int blockno, start_block, blk_count;
  ...
  for (blockno=start_block; blockno<blk_count; blockno++)
  {
    ...
    if (FLASH_DEVICE_DONE != nand_block_get_state(mibib.flash_handle, blockno, &blk_state))
    {
      return FLASH_PROG_FAIL;
    }
    ...
    /* erase the block */
    if (NAND_DEVICE_DONE != nand_erase(mibib.flash_handle, blockno, 1))
    {
...
int flash_prog_find_nth_good_block(nand_handle_t flash_handle, int which_block)
{
  int curblk = 0;
  int goodblks = -1;
  ...
  if(DAL_SUCCESS == nand_get_info(flash_handle,
    &flash_info.nand_info))
  {
  ...
  if (which_block > flash_info.nand_info.block_count)
  {
    return -1;
  }

  while ((curblk < flash_info.nand_info.block_count) &&
         (block_found == FALSE) && (goodblks < which_block))
  {
    if (DAL_SUCCESS != nand_block_get_state(flash_handle, curblk, &blk_state))
    {
      return -1;
    }

    if (NAND_BLOCK_BAD == blk_state)
    {
      curblk++;
    }
    else
  ...
  if (curblk >= flash_info.nand_info.block_count)
  {
    return -1;
  }
...
int flash_prog_convert_usr_parti_tbl_to_sys (nand_handle_t flash_handle,
                                             ...
                                             flash_partable_t parti2)
{
  ...
  page_length = flash_info.nand_info.page_size_bytes;
  pages_in_block = flash_info.nand_info.pages_per_block;
  total_blocks = flash_info.nand_info.block_count;
  ...
  for (i=0; i<parti1->numparts; i++)
  {
    ...
    if (mibib_indx == -1)
    {
      if ( (!strncmp(parti2->part_entry[i].name, MIBIB_PARTI_NAME, FLASH_PART_NAME_LENGTH) ||
        !strncmp(parti2->part_entry[i].name, "0:MIBIB", FLASH_PART_NAME_LENGTH)))
      {
    ...
    {
	  FLASHLOG(1,("padded vs image size check skipped for auto!\n"));
	}
	else
    {
    ...
    /* commented out to parse user partitions in units of blocks or KBs */
    ...
    FLASHLOG(4,("fncuptts: Partition length is %ld blocks\n",
    ...
    /* Runtime partition layout; conpensate for bad blocks */
    {
      ...
      {
        if (DAL_SUCCESS != nand_block_get_state(flash_handle, i, &blk_state))
        {
          return FLASH_PROG_FAIL;
        }
