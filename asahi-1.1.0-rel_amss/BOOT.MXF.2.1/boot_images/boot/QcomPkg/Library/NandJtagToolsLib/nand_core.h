 ...
 *   order.
 *
 *   $Header: //components/dev/core.boot/5.1.1/glosch.core.boot.5.1.1.nand_cfg_olympic/QcomPkg/Library/NandJtagToolsLib/nand_core.h#1 $ 
 *   $DateTime: 2022/04/04 03:16:28 $ 
 *   $Author: glosch $
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 01/04/23     sh      Updated MAX_SPARE_SIZE to support all flash layouts
 * 06/20/20     sa      Relocated physical_start_block variable
...
/* Maximim page size for available nand parts  : 0x1000 (4096B)
 * Maximum spare size for available nand parts : 0x100  (256B)
 * Maximum total size to cater all nand parts  : 0x1100 (4352B)
 */
#define MAX_SPARE_SIZE  (MAX_PAGE_SIZE >> 4)
...
#define FLASH_DEVICE_DONE               NAND_DEVICE_DONE  /**< Operation passed */
#define FLASH_DEVICE_FAIL               NAND_DEVICE_FAIL /**< Operation failed */
#define FLASH_DEVICE_NOT_SUPPORTED      NAND_DEVICE_NOT_SUPPORTED  /**< Device/operation not supported */
#define FLASH_DEVICE_INVALID_PARAMETER  NAND_DEVICE_INVALID_PARAMETER  /**< API parameters invalid */
#define FLASH_DEVICE_FAIL_PAGE_ERASED   NAND_DEVICE_FAIL_PAGE_ERASED  /**< Erased page detected */
#define FLASH_DEVICE_NO_MEM             NAND_DEVICE_NO_MEM /**< Insufficient memory */

/* Wrapper struct for nand_info. This contains come information only needed by tool */
struct flash_info
{
  enum flash_width width;
...
struct  parti_ctl
{
  ...
  enum page_access_opcode write_ops;
...
extern enum page_access_opcode write_type;
extern enum page_access_opcode read_type;
...
int flash_prog_find_nth_good_block(nand_handle_t flash_handle,
                                          int which_block);
