 ...
 *
 * Copyright (c) 2009-2014, 2020-2021,2022 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 * 09/22/29     tvra    Add jtag_id checck for auto targets.
 * 01/07/21     sa      Change flash_prog_read_page api.
 * 08/01/20     sa      Workaround for Rumi.
 * 07/01/20     sa      Close handles when done.
...
ALIGN(ALIGN_VAL) unsigned char mibib_pg_buf[TOTAL_PAGE_SIZE];
...
{
  enum nand_block_state blk_state = NAND_BLOCK_STATE_UNKNOWN;
  uint32 magic_buf[6];

  FLASHLOG( 4, ("Block number = %ld", (unsigned long int)pctl->current_block));

  if (FLASH_DEVICE_DONE != nand_block_get_state(pctl->flash_handle,
  ...
  while (blk_state == NAND_BLOCK_BAD)
  {
  ...
  if (pctl->current_block >= pctl->flash_info.nand_info.block_count)
  {
  ...
  /* erase the block */
  if (FLASH_DEVICE_DONE!= nand_erase(pctl->flash_handle, pctl->current_block, 1))
  {
  ...
  pctl->current_page =
    pctl->current_block * pctl->flash_info.nand_info.pages_per_block;
  pctl->avail_pages_in_block = pctl->flash_info.nand_info.pages_per_block;
  write_type = NAND_ACCESS_MAIN;
  read_type = NAND_ACCESS_MAIN;
  ...
  switch (current_mode)
  {
    ...
    case MODE_RAW_IMG:
    {
      ...
      write_type = NAND_ACCESS_RAW;
      read_type = NAND_ACCESS_RAW;
      pctl->page_size =
        pctl->flash_info.nand_info.total_page_size_bytes;
...
int flash_prog_image_init(void)
{
  ...
  FLASHLOG(3,("\t%s:: blk_start 0x%lx\n", parti_name,
    (unsigned long int)image.flash_info.nand_info.physical_start_block));
  FLASHLOG(3,("\t%s:: blk limit 0x%lx\n", parti_name,
    ((unsigned long int)image.flash_info.nand_info.physical_start_block +
     (unsigned long int)image.flash_info.nand_info.block_count)));
  ...
  switch (current_mode)
  {
    ...
    case MODE_CUSTOM_IMG:
    {
       ...
       * mechanism.
       */
      attribute = (uint32) ((image.flash_info.nand_info.attributes &
...
uint32 flash_prog_set_block_offsets_in_mibib(int security_mode)
{
  ...
  switch (security_mode)
  {
    ...
    case FLASH_MI_BOOT_SEC_MODE_TRUSTED:
	  ...
	  {
		boot_layout.mibib_block = FLASH_MI_BLOCK_MIBIB_AUTO;
      }
      else
      {
...
static int flash_prog_make_mibib_valid(uint32 mibib_block)
{
  ...
  if (DAL_SUCCESS != nand_block_get_state(mibib.flash_handle, mibib_block,
    &blk_state))
  {
    return FLASH_PROG_FAIL;
  }
  ...
  temp_page = mibib_block * mibib.flash_info.nand_info.pages_per_block;
  page_size_in_bits = (uint32)mibib.flash_info.nand_info.page_size_bytes * 8;
  ...
  result = flash_prog_read_pages(mibib.flash_handle, &mibib.flash_info,
    temp_page, 1, read_type, mibib_pg_buf);
  ...
  memset((void *)mibib_pg_buf, fill_char, mibib.flash_info.nand_info.page_size_bytes);
...
int flash_prog_write_mibib(void)
{
  ...
  if (newest_mibib_block == -1)
  {
    ...
    memset(mibib_magic_ptr, 0xFF,
                  mibib.flash_info.nand_info.page_size_bytes);
    ...
    /* erase the block */
    if (NAND_DEVICE_DONE != nand_erase(mibib.flash_handle, oldest_mibib_block, 1))
    {
    ...
    temp_page = (oldest_mibib_block * mibib.flash_info.nand_info.pages_per_block) +
                            ...
                            flash_partable_t sys_parti_ptr)
{
  ...
  if (prog_param.reserved != FLASH_PROG_PARTI_OVERRRIDE)
  {
    ...
    sys_parti_page = (mibib.flash_info.nand_info.pages_per_block *
      newest_mibib_block) + MIBIB_PAGE_PARTITION_TABLE;

    result = flash_prog_read_pages(mibib.flash_handle, &mibib.flash_info,
      sys_parti_page, 1, read_type, mibib_pg_buf);
  ...
  if (!sys_parti_match_flag)
  {
    /* Please note that if partition override flash is set we are guranteed to
...
uint32 flash_prog_image_finalize(void)
{
  ...
  if (image.partial_page_index > 0)
  {
      ...
      num_fill_bytes = image.flash_info.nand_info.page_size_bytes -
  ...
  /* Close the device */
  if (NAND_DEVICE_DONE !=  nand_close(image.flash_handle, 0))
  {
