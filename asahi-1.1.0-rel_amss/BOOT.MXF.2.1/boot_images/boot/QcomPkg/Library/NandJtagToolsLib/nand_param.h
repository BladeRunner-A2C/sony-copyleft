 ...
 *
 *   Copyright (c) 2008-2009,2010, 2020, 2022 Qualcomm Technologies Incorporated.
 *   All Rights Reserved.
 ...
 *   order.
 *
 *   $Header: //components/dev/core.boot/5.1.1/glosch.core.boot.5.1.1.nand_cfg_olympic/QcomPkg/Library/NandJtagToolsLib/nand_param.h#1 $ $DateTime: 2022/04/04 03:16:28 $ $Author: glosch $
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 06/30/22     gml     Updated flash_prog_param fields with uintnt
...
struct flash_prog_param {
   ...
   uintnt width;
   uintnt offset;
   uintnt addr;            // in: address of the flash to write/erase
   uintnt size;            // in: size of data to write/erase
   uintnt reserved;
   uintnt status;          // in: 1=program, 2=erase, 5=erase,
...
struct type_flash_prog_report {
   uintnt input_gap_size;
   uintnt bad_block_gap_size;
   uintnt image_size;
   uintnt reserved1;
   uintnt reserved2;
   uintnt reserved3;
   uintnt reserved4;
   uintnt status;           // out: 0=o.k., 0x100=fail
};
