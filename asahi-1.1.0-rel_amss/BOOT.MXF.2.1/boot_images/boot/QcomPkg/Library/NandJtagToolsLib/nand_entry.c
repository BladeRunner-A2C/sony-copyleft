 ...
 *
 * Copyright (c) 2008-2012, 2014, 2020, 2022-2023 QUALCOMM Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *   $Header: //components/dev/core.boot/5.1.1/glosch.core.boot.5.1.1.nand_cfg_olympic/QcomPkg/Library/NandJtagToolsLib/nand_entry.c#1 $ $DateTime: 2022/04/04 03:16:28 $ $Author: glosch $
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 01/13/23     tmt     CR-3307011 security fixes
 * 18/08/22     gml     Updated dtb buffer size to 32 KB
 * 22/06/22     gml     Updated dtb buffer size to 15 KB
 * 07/01/20     sa      Removed unused code.
...
#define FLASH_TOOLS_UNCACHED_HEAP_SIZE   (64 * 1024)
...
extern UINT8* Image$$NAND_PROGRAMMER_ZI$$Start;
extern UINT8* Image$$NAND_PROGRAMMER_ZI$$End;

uint8 flash_tools_uncached_heap[FLASH_TOOLS_UNCACHED_HEAP_SIZE];
uint8 flash_tools_heap[FLASH_TOOLS_HEAP_SIZE - FLASH_TOOLS_UNCACHED_HEAP_SIZE];

#define DEFAULT_BLOB_ID 0
#define DTB_BUFFER_SIZE  (32 * 1024)

uint8 dtb_buffer[DTB_BUFFER_SIZE] = {0x4};

//static void *fdt_handle = NULL;
static const void *fdt_load_handle = NULL;
uint32 dtb_load_done = FALSE;

void nand_init_dtb(void)
{
	int retValue = -1;
	fdt_load_handle = dtb_buffer;

	retValue = fdt_set_blob_handle(fdt_load_handle, DTB_BUFFER_SIZE, DEFAULT_BLOB_ID);
	if (retValue != 0)
	{
		return;
	}
...
{
   {
     (UINT8*)&Image$$NAND_PROGRAMMER_ZI$$Start,   
     (UINT8*)&Image$$NAND_PROGRAMMER_ZI$$End
   },
   {NULL, NULL}
};
...
#endif
{
  ...
  {
    DALSYS_HeapInit(flash_tools_uncached_heap, FLASH_TOOLS_UNCACHED_HEAP_SIZE, DALHEAP_UNCACHED,0,0);
    DALSYS_HeapInit(flash_tools_heap, FLASH_TOOLS_HEAP_SIZE - FLASH_TOOLS_UNCACHED_HEAP_SIZE, DALHEAP_DDR,0,0);
...
void main_loop_c(void)
{
   UINT32 length = (zi_data[0].len - zi_data[0].base);

   /* clear out ZI data */

   // TODO Need to check why memset is not working

   memset(zi_data[0].base, 0x0, length);
