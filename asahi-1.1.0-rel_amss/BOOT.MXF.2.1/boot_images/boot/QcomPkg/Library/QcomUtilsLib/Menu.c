/** @file
  ...
  Copyright (c) 2013-2017, 2019, 2021, 2023 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 06/21/21	cm 		Try launch Menu from fv0
 06/24/19	yg 		Flush keys to avoid endless loops
 10/12/17	mt 		Change delay 250ms to 5ms after entering Bds menu
...
AllocMem (UINTN Size)
{
  ...
  {
    if ((FreeBufferPtr = AllocateZeroPool (2 * EFI_PAGE_SIZE)) == NULL)
    {
      DEBUG((EFI_D_WARN, "MemoryAlloc failed\n"));
      return NULL;
    }
    EndPtr = FreeBufferPtr + 2 * EFI_PAGE_SIZE;
  }

  Size = (Size + 3) & (~3);
  if (FreeBufferPtr + Size > EndPtr)
  {
    DEBUG((EFI_D_WARN, "Menu size exceeds allocated memory size failed\n"));
    return NULL;
  }
...
MenuKeyValCb (UINT8* Section, UINT8* Key, UINT8* Value)
{
  ...
  if (OptionCbCnt < 5)
  {
    ...
    {
      AsciiStrCpyS (MenuCtx.MenuLabel,  sizeof(MenuCtx.MenuLabel), (CHAR8*)Section);
  ...
  AsciiStrnCpyS ((CHAR8*)StrPtr, Sz, (CHAR8*)Value, Sz);
  UniStr = AllocatePool(2*AsciiStrSize((CHAR8 *)StrPtr));
  if (UniStr == NULL)
    return;
  AsciiStrToUnicodeStrS((CHAR8 *)StrPtr, UniStr, AsciiStrSize((CHAR8 *)StrPtr));
...
LaunchMenu (CHAR8* MenuFileName OPTIONAL)
{
  ...
  AsciiStrToUnicodeStrS (MenuFileName, ConfigFileWChar, sizeof(ConfigFileWChar)/sizeof(CHAR16));
  ...
  if (Status != EFI_SUCCESS)
  {
    ...
    /* If load from everywhere fails then fall back to loading from Fv2/FV1/FV0 */
    if (Status != EFI_SUCCESS)
    {
      INT8 i = 2; // max value can be 9.
      while(i>=0)
      {
        AsciiStrCpyS (ConfigFileinFV, sizeof(ConfigFileinFV), FV_PREPEND);
        AsciiStrCatS (ConfigFileinFV, sizeof(ConfigFileinFV), MenuFileName);

        Status = LoadFromFV (ConfigFileinFV, &FileBuffer, &FileSizeFv);
        if (!EFI_ERROR(Status))
          break;
        --i;
        FV_PREPEND[2] = i+'0';  //converting integer to ascii
      }
