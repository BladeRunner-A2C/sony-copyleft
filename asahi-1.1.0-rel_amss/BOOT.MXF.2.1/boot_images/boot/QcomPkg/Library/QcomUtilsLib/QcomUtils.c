...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 09/13/18   md      Added support to load image from FV using device path name.
...
LaunchApp (IN UINTN  Argc, IN CHAR8  **Argv)
{
  ...
  if (!EFI_ERROR (Status)) {
    if (Argc >= 2) {
      ...
      AsciiStrToUnicodeStrS (Argv[1], ImageInfo->LoadOptions, ImageInfo->LoadOptionsSize/sizeof(CHAR16));
    }
               ...
               EFI_OPEN_FILE**          File)
{
  ...
  if (Name == NULL || FileBuffer == NULL || FileSize == NULL || File == NULL)
    return EFI_INVALID_PARAMETER;

  *File = EfiOpen (Name, EFI_FILE_MODE_READ, SectionType);
  if (*File == NULL)
    return EFI_LOAD_ERROR;
  
  Status = EfiReadAllocatePool (*File, FileBuffer, FileSize);
            ...
            OUT  UINTN*    FileSize)
{
  ...
  Status = LaodFromFVExt( Name, EFI_SECTION_RAW, (VOID**)FileBuffer, FileSize, &File);
...
EFI_STATUS
LoadImageFromFV (IN CHAR8 *Name,
                 IN CHAR8 *Args OPTIONAL)
{
   EFI_STATUS                 Status;
   VOID                       *Buffer;
   UINTN                      BufferSize;
   ...
   LaodFromFVExt(Name, EFI_SECTION_PE32, &Buffer, &BufferSize, &File);
  ...
  if (!EFI_ERROR (Status)) {
    if (Args) 
    {
      ArgsSize = AsciiStrSize(Args);
      ArgStr = AllocatePool(2 * ArgsSize);
      if ( ArgStr == NULL)
        return EFI_OUT_OF_RESOURCES;

      AsciiStrToUnicodeStrS (Args,  ArgStr, ArgsSize);

      Status = gBS->HandleProtocol (ImageHandle, &gEfiLoadedImageProtocolGuid, (VOID **)&ImageInfo);
      ASSERT_EFI_ERROR (Status);

      ImageInfo->LoadOptionsSize = (UINT32) StrSize (ArgStr);
      ImageInfo->LoadOptions     = AllocatePool (ImageInfo->LoadOptionsSize);
      if (ImageInfo->LoadOptions != NULL)
      {
         StrCpyS (ImageInfo->LoadOptions, ImageInfo->LoadOptionsSize/sizeof(CHAR16), ArgStr);
      }
      else
      {
        AsciiPrint ("ERROR: Memory Allocation failed !\n");
