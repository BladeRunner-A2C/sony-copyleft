...
EFI_STATUS UsbPwrCtrlLib_Post_Init(void);
void UsbPwrCtrlLibConfig_Detect_Hw(void);
...
/**
  Enable/Disable TypeC Port/PD functionality

  @param[IN]    PortIndex       Specify which port the request is for
  @param[IN]    PortEnType      Enable/Dsiable TypeC port/PD functionality only
...
*/
EFI_STATUS
UsbPwrCtrlLib_SetPortEn(
  UINT8 PortIndex,
  EFI_USB_PWR_CTRL_TYPEC_PORT_EN_TYPE PortEnType)
{
  EFI_STATUS Status = EFI_SUCCESS;
  ...
  USBPWRCTRL_LIB_DEBUG((EFI_D_INFO, "%a Port[%d]: PortEnType = %d\r\n",  __FUNCTION__, PortIndex, PortEnType));

  switch (PortEnType)
  {
  case EFI_USB_PWR_CTRL_TYPEC_PORT_ENABLE:
    pUsbPwrCtrlHwInfo->Ports[PortIndex].TypeC.bPDSupported = TRUE;
    break;
  case EFI_USB_PWR_CTRL_TYPEC_PORT_PD_DISABLE:
  case EFI_USB_PWR_CTRL_TYPEC_PORT_DISABLE:
    pUsbPwrCtrlHwInfo->Ports[PortIndex].TypeC.bPDSupported = FALSE;
    break;
  default:
    return EFI_UNSUPPORTED;
  }

  Status = UsbPwrCtrlLibPmSchg_SetPortEn(PortIndex, PortEnType);
  if(EFI_ERROR(Status))
  {
    USBPWRCTRL_LIB_DEBUG((EFI_D_ERROR, "%a Port[%d]: Failed, Error[%r] \r\n", __FUNCTION__, PortIndex, Status));
  }
...
/**
  Get Enable/Disable TypeC Port/PD functionality

  @param[IN]    PortIndex       Specify which port the request is for
  @param[OUT]   PortEnType      Enable/Dsiable TypeC port/PD functionality only
  ...
  EFI_USB_PWR_CTRL_TYPEC_PORT_EN_TYPE *PortEnType)
{
  ...
  Status = UsbPwrCtrlLibPmSchg_GetPortEn(PortIndex, PortEnType);
  if(EFI_ERROR(Status))
  {
    USBPWRCTRL_LIB_DEBUG((EFI_D_ERROR, "%a Port[%d]: Failed, Error[%r] \r\n", __FUNCTION__, PortIndex, Status));
    return Status;
    ...
    EFI_USB_PWR_CTRL_PIN_ASGMT_INFO *PinAsgmt)
{
  ...
  do
  {
    ...
    {
      case EFI_USB_PWR_CTRL_PORTHW_HW_PMUCSI:
        Status = UsbPwrCtrlLibPmUcsi_GetPinAsgmt(PortIndex, PinAsgmt);
        break;      
      case EFI_USB_PWR_CTRL_PORTHW_HW_PMSCHG:
        Status = EFI_UNSUPPORTED;
        break;
...
void
UsbPwrCtrlLibConfig_Detect_Hw(void)
{
  EFI_STATUS status = EFI_SUCCESS;
  uint8 port_index = 0;
  uint8 valid_port_index = 0;
