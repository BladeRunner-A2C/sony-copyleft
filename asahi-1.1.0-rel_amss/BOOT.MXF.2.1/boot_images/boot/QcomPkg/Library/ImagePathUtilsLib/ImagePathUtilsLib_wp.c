/**
  ...
  Copyright (c) 2019-2023 by Qualcomm Technologies, Inc.
  All Rights Reserved.
**/
...
/*=======================================================================
 ...
 --------   ----    --------------------------------------------------- 
 05/21/21   ka      remove InitFlag. Ensure variable read at Bds updates updates EnableEbsPerfFlag status 
 04/23/21   ka      Log Callback function exec time
  ...
  )
{
 ...
 /* Read the PCD value to check if EBS Perf Logging is enabled */
  if (FeaturePcdGet(PcdEBSPerfLogging))
...
/**
 ...
 @param  EventGuid                 GUID of callback event
 @param  ExecDelta                 Execution time

**/
VOID
PrintNotifyFnFileNameEx (
  IN  UINTN     NotifyFuncAddress,
  IN  EFI_GUID *EventGuid,
  IN  UINT32    ExecDelta
  )
{
  EFI_STATUS Status;
  ...
  if ((EnableEbsPerfFlag == FALSE) && (CompareGuid (EventGuid, &gEfiEventExitBootServicesGuid)))
    return;
  ...
  if ((NotifyFnImagePath != NULL) && (!RETAIL))
  {
	...
	/* Need to be fixed, temperaroy workaround */
    if(!AsciiStrnCmp(NotifyFnFileName, "UsbPwrCtrlDxe.dll", sizeof("UsbPwrCtrlDxe.dll")))
      return;

    if(ExecDelta != CALLBACK_PERF_IGNORE)
      DEBUG((EFI_D_WARN, " CallBack: "));
    else
      DEBUG((EFI_D_WARN, " - "));

     DEBUG((EFI_D_WARN, "0x%x [%5d] ", NotifyFuncAddress, GetTimerCountms ()));
    
    DEBUG((EFI_D_WARN, "%a", NotifyFnFileName));

    if(ExecDelta != CALLBACK_PERF_IGNORE)
      DEBUG((EFI_D_WARN, " WARN: ExecTime: %d ms \n", ExecDelta));
    else
      DEBUG((EFI_D_WARN, "\n"));
  }
}
