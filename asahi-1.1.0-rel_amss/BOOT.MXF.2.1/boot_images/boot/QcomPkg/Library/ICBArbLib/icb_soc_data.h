...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ICBArbLib/icb_soc_data.h#1 $

when         who     what, where, why
----------   ---     --------------------------------------------------------
2021/02/25   sds     Add support for QosDisable bit (qnoc 4.7)
...
2017/05/15   sds     Deferred QoS configuration
...
typedef struct
{
  ...
  uint64_t prev_bw;         /**< Previous total BW as computed by compute_bw() */
} icb_node_state_type;
...
typedef struct
{
  ...
  bool                          is_applied;
  bool                          qos_disable;
  uint32_t                      num_extra_regulators;
  icb_noc_extra_regulator_type *extra_regulators;
...
typedef struct icb_hw_node_type
{
  ...
  uint32_t                 num_relatives; /**< Number of related nodes */
  icb_hw_node_type **      relatives;     /**< Related BCM nodes. */
  const char *             local_clk_name;/**< Local clock name */
  uint32_t                 C1;            /**< BCM scalar for local_clk AB, fixed point use 137 for 1.37 */
  uint32_t                 C2;            /**< BCM scalar for local_clk IB, fixed point use 100 for 1.00 */
  npa_client_handle        local_clk_req_id;  /**< NPA required handle for local_clk_name */
  npa_client_handle        local_clk_supp_id; /**< NPA suppressible handle for local_clk_name */
} icb_hw_node_type;
...
typedef struct
{
  ...
  icb_node_state_type      query_state;   /**< Node state for query requests */
  bool                     is_disabled;   /**< Is this slave currently disabled? */
  uint32_t                 num_disables;  /**< Number of target disables associated with this port */
  uint32_t *               disable_ids;   /**< Array of disable ids */
} icb_slave_def_type;    
...
/* Arbiter resource node data */
typedef struct
{
  icb_topology_type *topology;
  uint32_t           num_active_required_clients; 
} icb_arbiter_node_data_type;
...
/* Chip variant data updates. */
typedef struct
{
  uint32_t *addr;
  uint32_t  val;
} icb_data_update_type;

typedef struct
{
  /* Chip variant information for this device data. */
  DalChipInfoFamilyType  family;
  bool                   match;
  DalChipInfoVersionType version;

  /* Potential register check. */
  icb_hw_version_type   reg_hw;
  uint32_t              reg_offs;
  uint32_t              reg_mask;
  uint32_t              reg_val;
  ...
  /* Data segment */
  uint32_t              num_entries;
  icb_data_update_type *data; 
} icb_variant_config_type;

typedef struct
{
  uint32_t                  num_configs;
  icb_variant_config_type **configs;
} icb_variant_list_type;
...
typedef struct
{
  ...
  icb_variant_list_type *    variants;
} icb_info_type;
