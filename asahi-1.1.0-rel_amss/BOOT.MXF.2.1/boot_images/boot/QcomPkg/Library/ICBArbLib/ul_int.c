...
DESCRIPTION: This file implements the NPA ICB Arbiter Node
    ...
    Copyright (c) 2010-2021 QUALCOMM Technologies, Incorporated.
               All Rights Reserved.
            QUALCOMM Proprietary/GTDR
-------------------------------------------------------------------------------

  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ICBArbLib/ul_int.c#1 $
...
2020/11/05   sds     Coverity warning
2020/08/26   sds     Workaround for klockwork tooling bug
2020/04/17   sds     Fix for post_init race condition
2019/02/25   sds     Add previous vote tracking
2018/09/19   sds     Standalone nodes shouldn't commit zero until post_init
2018/08/20   sds     Add CHANGED/ALL crossing types
...
#define REQUEST_LIST_SIZE    10ULL

#define ICB_QMP_BUFFER_SIZE 64
...
)
{
  ...
  /* Skip this if the user wants ALL notifications. */
  if(notify->definition.crossing_type != ICB_NOTIFY_CROSSING_ALL)
  {
    /* Check for monitored status value. */
    for( i = 0; i < states; i++ )
    {
      /* Check to see if we want UPDATE style crossing. */
      if(notify->definition.crossing_type == ICB_NOTIFY_CROSSING_CHANGED)
      {
        /* If we have a difference, the callback is required. */
        if(prev_val[i] != state_val[i])
        {
          callback_required = true;
          break;
        }
        /* Otherwise, skip the rest and see if we have other states to check. */
        else
        {
          continue;
        }
...
)
{
  ...
  total_bw = MAX(MAX(state->ib, state->supp_ib),
                 CEIL_DIV(state->ab + state->supp_ab, ports));
...
)
{
  ...
  node_state->prev_bw  = node_state->total_bw;
  state = *node_state;
...
)
{
  ...
  /* Update hardware related to the master. */
  AxiCfgInt_UpdateMasterBW( master );
...
)
{
  ...
  /* Update hardware related to the slave. */
  AxiCfgInt_UpdateSlaveBW( slave );
...
)
{
  ...
  uint64_t bcm_width = hw_node->width, node_ports, node_width;
  uint32_t req_idx;
  icb_node_state_type *state;
  ...
  if( ICB_SW_STATE_FLUSH != sw_state )
  {
    ...
    for( req_idx = 0; req_idx < hw_node->request_list.num_entries; req_idx++ )
    {
      ...
      * 2.) AB requests to be scaled to per channel */
      if(ICB_HW_NODE_KIND_BANDWIDTH == hw_node->type || ICB_HW_NODE_KIND_LOCAL_CLK == hw_node->type)
      {
    ...
    hw_node->state.prev_bw  = hw_node->state.total_bw;
...
icb_info_type *ul_int_init( void )
{
  ...
  ULOG_RT_PRINTF_0(icb_log, "  ul_int_init:");

  /* Only initialize if we haven't tried before. */
  if( ICB_SW_STATE_UNINITIALIZED != sw_state )
  {
    ULOG_RT_PRINTF_0(icb_log, "  ... FAILED: Already initialized");
    return NULL;
  }
  ...
  if( NULL == icb_info )
  {
    ULOG_RT_PRINTF_0(icb_log, "  ... FAILED: AxiCfgInt_GetProperty");
    return NULL;
  }
...
/*==============================================================================
  ...
  FUNCTION   ul_int_resource_post_init

  DESCRIPTION 
    Flush standalone hardware nodes and set to run mode.
...
==============================================================================*/
void ul_int_resource_post_init
(
  icb_standalone_node_type *node
)
{
  icb_standalone_data_type *data    = (icb_standalone_data_type *)node->node_definition.resources[0].data;
  icb_rpmh_handle_type *    rpmh    = &data->rpmh_handle;
  icb_hw_node_type *        hw_node = data->hw_node;
  npa_client                client;

  /* Update our state. */
  data->sw_state = ICB_SW_STATE_FLUSH;

  /* Flush any zero votes. */
  client.name = "ICB post init client";
  client.request_attr = NPA_REQUEST_FIRE_AND_FORGET;
  AxiCfgInt_CommitSingle(&client, rpmh, hw_node);

  /* Now we're done, so enter RUN state. */
...
)
{
  ...
  AxiCfgInt_Commit(client, topology);
}

/**
@copydoc ul_int_deinit
 */
void ul_int_deinit( void )
{
  uint32_t node_idx, topo_idx;

  /* Clean up each topology. */
  for( topo_idx = 0;
       topo_idx < icb_info->num_topologies;
       topo_idx++ )
  {
    /* Clear out any per node status or handles. */
    icb_topology_type *topology = icb_info->topologies[topo_idx];

    /* Masters */
    for( node_idx = 0;
         node_idx < topology->num_masters;
         node_idx++ )
    {
      icb_master_def_type *master = topology->masters[node_idx];

      /* Clear node state. */
      memset(&master->state, 0, sizeof(icb_node_state_type));

      /* Force clean request list. */
      master->request_list.num_entries = 0;
      memset(master->request_list.requests, 0, 
             master->request_list.list_size * sizeof(icb_bw_request_type *));
    }

    /* Slaves */
    for( node_idx = 0;
         node_idx < topology->num_slaves;
         node_idx++ )
    {
      icb_slave_def_type *slave = topology->slaves[node_idx];

      /* Clear slave state. */
      memset(&slave->state, 0, sizeof(icb_node_state_type));

      /* Force clean request list. */
      slave->request_list.num_entries = 0;
      memset(slave->request_list.requests, 0, 
             slave->request_list.list_size * sizeof(icb_bw_request_type **));
    }

    /* HW nodes */
    for( node_idx = 0;
         node_idx < topology->num_hw_nodes;
         node_idx++ )
    {
      icb_hw_node_type *hw_node = topology->hw_nodes[node_idx];

      /* Clear node state. */
      memset(&hw_node->state, 0, sizeof(icb_node_state_type));
      hw_node->output = 0;
      hw_node->active.val = 0;
      hw_node->sleep.val = 0;

      /* Force clean request list. */
      hw_node->request_list.num_entries = 0;
      memset(hw_node->request_list.requests, 0, 
             hw_node->request_list.list_size * sizeof(icb_bw_request_type **));
    }
  }
  ...
  /* Reset layer state. */
  sw_state = ICB_SW_STATE_UNINITIALIZED;
}
...
)
{
  ...
  /* Save back to our state. */
  hw_node->query_state.prev_bw  = hw_node->query_state.total_bw;
...
)
{
  ...
  uint32_t                   pend_ib       = NPA_RESOURCE_REQUIRED_REQUEST(resource);
  uint32_t                   pend_supp_ib  = NPA_RESOURCE_SUPPRESSIBLE_REQUEST(resource);
  bool                       remove_active = false;

  /* Update active indication (active required vote).
  * Apply now on the inactive->active transition.
  * Apply afterward for the active->inactive transition. */
  if( hw_node->state.ib == 0 && pend_ib > 0 )
  {
    ul_apply_active();
  }
  else if( hw_node->state.ib > 0 && pend_ib == 0 )
  {
    remove_active = true;
  }

  ULOG_RT_PRINTF_3( icb_log, "[%s]: [%s] Issue request: (%d)", resource->definition->name, client->name, pend_ib );

  /* Update hw node state. */
  hw_node->state.prev_bw  = hw_node->state.total_bw;
  hw_node->state.ib       = pend_ib;
  hw_node->state.supp_ib  = pend_supp_ib;
   ...
   *   Only use the aggregated state if non-zero, do nothing otherwise.
   */
  if( ICB_SW_STATE_RUN   == resource_data->sw_state ||
      ICB_SW_STATE_FLUSH == resource_data->sw_state ||
      (ICB_SW_STATE_HOLD == resource_data->sw_state && hw_node->state.total_bw != 0) )
  {
    AxiCfgInt_CommitSingle(client, rpmh, hw_node);
  }

  /* Remove active indication if we previously identified the need. */
  if( remove_active )
  {
    ul_remove_active();
  }
...
/*============================================================================
                   External functions - QMP resources
============================================================================*/
/** ul_issue_qmp_resource_request
 * 
 * @brief Issue a request for a QMP managed resource
 * 
 */
npa_resource_state ul_issue_qmp_resource_request
(
   npa_resource *     resource,
   npa_client   *     client,
...
)
{
...
#else
  icb_standalone_data_type * resource_data = (icb_standalone_data_type *)resource->definition->data;
  char                       request[ICB_QMP_BUFFER_SIZE];
  int64_t                    num_chars;
  glink_err_type             err;
  ...
  num_chars = snprintf(request, ICB_QMP_BUFFER_SIZE,
                       "{class: bcm, res: %s, val: %d}", resource_data->resource_name, state != 0);

  /* If we don't fit in the buffer for some reason, bail out. */
  if( num_chars < 0 || num_chars >= ICB_QMP_BUFFER_SIZE )
  {
    return resource->active_state;
  }

  /* Send the message.
   * snptrintf return value doesn't include the terminator. */
  err = send2aop(request, num_chars+1);

  /* If we failed to send, bail out. */
  if( GLINK_STATUS_SUCCESS != err )
  {
    return resource->active_state;
  }
...
/*============================================================================
                       External functions - Activity Status 
============================================================================*/
/** 
 * @copydoc ul_apply_active
 */
void ul_apply_active( void )
{
  AxiCfgOs_ApplyActive();
}

/** 
 * @copydoc ul_remove_active
 */
void ul_remove_active( void )
{
  AxiCfgOs_RemoveActive();
}
