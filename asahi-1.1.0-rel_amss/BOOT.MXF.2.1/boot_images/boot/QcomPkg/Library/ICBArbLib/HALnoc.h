...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ICBArbLib/HALnoc.h#1 $
...
typedef struct
{
  ...
  bool critical;    /**< Consider critical traffic */
  bool noncritical; /**< Consider non-critical traffic */
...
/**
@brief Set the qos disable flag for the given qos generator

@param[in] info -        A pointer to the NOC info structure
@param[in] master_port - The port number of the master to modify
@param[in] disable -     The disable flag to set

@pre HAL_noc_Init() must have been called.

@sa HAL_noc_Init()
*/
void HAL_noc_SetQosDisable( HAL_noc_InfoType *info,
                            uint32_t          master_port,
                            bool              disable );
...
@param[in] info -        A pointer to the NOC info structure
@param[in] master_port - The port number of the master to modify
@param[in] index -       The index regulator instance to modify
...
/**
@brief Set the disconnect bit for a port

Set the disconnect bit for a port

@param[in] info -    A pointer to the NOC info structure
@param[in] port -    The port to modify
@param[in] disable - Disconnect or reconnect flag

@pre HAL_noc_Init() must have been called.

@sa HAL_noc_Init()
@sa HAL_noc_IsTargetDisabled()
*/
void HAL_noc_DisableTarget( HAL_noc_InfoType *info,
                            uint32_t          port,
                            bool              disable );
...
/**
@brief Is QoS disabled on this master port?
...
@sa HAL_noc_Init()
*/
bool HAL_noc_IsQosDisabled( HAL_noc_InfoType *info,
                            uint32_t          master_port );
...
/**
@brief Check if a target has been successfully disabled

Check if a target has been successfully disabled
...
@sa HAL_noc_Init()
@sa HAL_noc_DisableTarget()
*/
bool HAL_noc_IsTargetDisabled( HAL_noc_InfoType *info,
                               uint32_t          port );
