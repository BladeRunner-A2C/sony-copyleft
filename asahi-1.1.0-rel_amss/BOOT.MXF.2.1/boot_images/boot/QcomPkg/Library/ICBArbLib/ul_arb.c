...
DESCRIPTION: This file implements the NPA ICB Arbiter Node
    ...
    Copyright (c) 2010-2022 QUALCOMM Technologies, Incorporated.
               All Rights Reserved.
            QUALCOMM Proprietary/GTDR
-------------------------------------------------------------------------------

  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ICBArbLib/ul_arb.c#1 $
...
----------   ---     -----------------------------------------------------------
2021/04/05   sds     Fix warning in _generate_query
2021/02/08   sds     Update logging to be more useful
2020/08/26   sds     Workaround for klockwork tooling bug
2020/04/17   sds     Fix for post_init race condition
2018/12/03   sds     Add slave and master info queries
2018/06/26   sds     Add query for slave scaling unit
2018/06/26   sds     Add query for scaling unit
2018/02/02   sds     Add client query
2017/06/26   sds     Add Deinit/Reinit
2017/05/10   sds     Node state reflects active REQUIRED clients
...
ULogHandle icb_log = NULL;

/* Size of Arbiter ULog */
uint32_t icb_log_size = 0x4000;
...
/*============================================================================
  NPA Definitions for QMP based resources
============================================================================*/
static npa_resource_state _ul_qmp_driver_fcn
(
   npa_resource *resource,
   npa_client   *client,
   npa_resource_state state
...
static ICBArb_ErrorType _query_master(ICBArb_QueryTypeType query_type, ICBArb_QueryUnionType *query_data);
...
static ICBArb_ErrorType _query_client_info( ICBArb_QueryClientType *query);
void ul_notify_destroy_all(void);
void ul_remove_notification(icb_notify_list_type *list, icb_notify_type *elem);
void _ul_increment_active(npa_resource *resource);
void _ul_decrement_active(npa_resource *resource);
...
/**=============================================================================
  ...
  @brief Performs post ICB standalone node creation initialization

  @param None

  @return None
==============================================================================*/
static void _ul_standalone_init_cb
(
   void         *context,
   unsigned int  event_type,
   void         *data,
...
{
   icb_standalone_data_type *rsrc_data = (icb_standalone_data_type *)context;
    ...
    * QMP resources are fully initialized at this point.
    * Others we'll set to HOLD and update again at post_init. */
   switch( rsrc_data->type )
   {
      case ICB_STANDALONE_INTERFACE_CLOCK:
      case ICB_STANDALONE_INTERFACE_FAL:
         rsrc_data->sw_state = ICB_SW_STATE_HOLD;
         break;
      case ICB_STANDALONE_INTERFACE_QMP:
         rsrc_data->sw_state = ICB_SW_STATE_RUN;
         break;
      default:
...
)
{
   ...
   bool is_complete_request = false, is_active = false, decrement_resource_active = false;
   icb_arbiter_node_data_type *resource_data =
      (icb_arbiter_node_data_type *)resource->definition->data;
   ...
   /* Log client name. */
   ULOG_RT_PRINTF_2( icb_log, "[%s]: [%s] Issue request:", resource->definition->name, client->name );
   ...
   for ( i = 0; i < request_len; i++ )
   {
      ...
      /* Update client activity flag. */
      if( 0 != bw.ib || 0 != bw.ab || 0 != bw.latency_ns )
      {
         is_active = true;
      }

      /* Log this request, but only print the latency field if the client uses it. */
      route = &master_route_vector->routes[i];
      ULOG_RT_PRINTF_7( icb_log, "  (%d)=>(%d): Ib: %lld Ab: %lld Lat: %d", 
                        route->master_id, route->slave_id,
                        ULOG64_LOWWORD(request->ib), ULOG64_HIGHWORD(request->ib),
                        ULOG64_LOWWORD(request->ab), ULOG64_HIGHWORD(request->ab),
                        request->latency_ns );
   ...
   /* Update resource active client count.
    * We only count REQUIRED vector clients and clients we aren't ignoring. */
   if( NPA_CLIENT_VECTOR == client->type &&
       (ICBARB_CLIENT_ATTR_IGNORE_ACTIVE & master_route_vector->attributes) == 0 )
   {
      bool prev_active = master_route_vector->is_active;

      /* If previously active, but now now, decrement count.
       * Make sure to apply this only *after* we've removed the associated vote. */
      if( prev_active && !is_active )
      {
         decrement_resource_active = true;
      }
      /* If not previously active, but now active, increment count.*/
      else if( !prev_active && is_active )
      {
         _ul_increment_active( client->resource );
      }
   }
   master_route_vector->is_active = is_active;

   /* "commit" all queued reqeusts */
   ul_request_commit( client, resource_data->topology );

   /* Remove active indication if necessary. */
   if( decrement_resource_active )
   {
      _ul_decrement_active( client->resource );
   }

   return resource_data->num_active_required_clients;
}
...
void icbarb_init( void )
{
   ...
   ULOG_RT_PRINTF_0(icb_log, "icbarb_init:");
      ...
      for( i = 0; i < icb_info->num_standalone_nodes; i++ )
      {
         ...
         switch( data->type )
         {
            ...
            case ICB_STANDALONE_INTERFACE_FAL:
               ...
               break;
            case ICB_STANDALONE_INTERFACE_QMP:
               node->node_definition.driver_fcn          = _ul_qmp_driver_fcn;
               node->node_definition.resources[0].plugin = &npa_binary_plugin;
               break;
            default:
               continue;
         }

         /* Update our state and define the node. */
         data->sw_state = ICB_SW_STATE_INITIALIZING;
         npa_define_node_cb( &node->node_definition, init, _ul_standalone_init_cb, data );
      }
   }
}
...
void icbarb_post_init( void )
{
   ...
   if( NULL != self )
   {
      ...
      /* Perform post init on any standalone nodes. */
      for( i = 0; i < icb_info->num_standalone_nodes; i++ )
      {
         icb_standalone_node_type *node     = icb_info->standalone_node_definitions[i];
         icb_standalone_data_type *data     = (icb_standalone_data_type *)node->node_definition.resources[0].data;
         npa_resource *            resource = node->node_definition.resources[0].handle;

         /* Only do post init for CLOCK/FAL nodes.
          * This is unnecessary for QMP. */
         switch( data->type )
         {
            case ICB_STANDALONE_INTERFACE_CLOCK:
            case ICB_STANDALONE_INTERFACE_FAL:
               npa_resource_lock(resource);
               ul_int_resource_post_init(node);
               data->sw_state = ICB_SW_STATE_RUN;
...
/*==============================================================================
  ...
  FUNCTION   icbarb_deinit

  DESCRIPTION
    This function is for deinitializing the driver for a hibernate/power down
    scenario. It will clean-up old state.
...
==============================================================================*/
void icbarb_deinit( void )
{
   /* Inform lower layers of cleanup request */
   AxiCfgInt_Deinit();

   /* Clean up notification layer */
   ul_notify_destroy_all();

   /* Clean up internal data */
   ul_int_deinit();

   /* Clean up local layer state. */
...
/*==============================================================================
  ...
  FUNCTION   icbarb_reinit

  DESCRIPTION
    This function is for reinitializing the driver after a hibernate/power down
    scenario. It will clean-up old state then issue a "cold" initialization.
   ...
   if( NULL != self )
   {
      ...
      if ( NULL != query )
      {
         ...
         switch ( query->queryType )
         {
            ...
            case ICBARB_QUERY_CLIENT_INFO:
               error = _query_client_info( &query->queryData.clientInfo );
               break;

            case ICBARB_QUERY_MASTER_INFO:
               error = _query_master(
                  query->queryType,
                  &query->queryData );
               break;
...
)
{
   ...
   switch( query_type )
   {
      case ICBARB_QUERY_SLAVE_PORT_COUNT:
         ...
         {
            uint32_t node_idx;
            icb_slave_def_type *slave = slave_list.slaves[i];

            query_data->slavePortCount.uNumPorts = slave->num_agg_ports;
            query_data->slavePortCount.uPortWidth = slave->width;

            /* Find the correct scaling unit.
             * Initialize with a non-zero scaling value just in
             * case it's not part of any domain. */
            query_data->slavePortCount.uScaleUnit = 1;
            for( node_idx = 0;
                 node_idx < slave->num_bw_nodes;
                 node_idx++ )
            {
               icb_hw_node_type *node = slave->bw_nodes[node_idx];
               if( ICB_HW_NODE_KIND_BANDWIDTH == node->type)
               {
                  query_data->slavePortCount.uScaleUnit =
                     slave->bw_nodes[node_idx]->bw_unit;
               }
            }
      ...
      case ICBARB_QUERY_SLAVE_INFO:
         ...
         if( i != slave_list.num_slaves )
         {
            ...
            query_data->slaveInfo.uNumPorts = slave->num_agg_ports;
            query_data->slaveInfo.uPortWidth = slave->width;

            /* Find the correct scaling unit.
             * Initialize with a non-zero scaling value just in
             * case it's not part of any domain. */
            query_data->slaveInfo.uScaleUnit = 1;
            for( node_idx = 0;
                 node_idx < slave->num_bw_nodes;
                 node_idx++ )
            {
               ...
               if( ICB_HW_NODE_KIND_BANDWIDTH == node->type)
               {
                  query_data->slaveInfo.uScaleUnit =
                     slave->bw_nodes[node_idx]->bw_unit;
               }
            }
            
            /* Find the estimated frequency requirement for this node.
             * We're only going to use total_bw / width for this. */
            state = ul_get_slave_state( query_data->slaveInfo.eSlaveId );
            if( NULL != state )
            {
               query_data->slaveInfo.uEstFreqKHz = (uint32_t)(state->total_bw / ((uint64_t)slave->width * 1000));
            }
            else
            {
               query_data->slaveInfo.uEstFreqKHz = 0;
            }
...
/**=============================================================================
   ...
   @brief _query_master - Queries ICB slave resource parameters
   
   This function queries various parameters from the target master.
...
==============================================================================*/
static ICBArb_ErrorType _query_master
(
  ICBArb_QueryTypeType   query_type,
  ICBArb_QueryUnionType *query_data
)
{
   ...
   switch( query_type )
   {
      case ICBARB_QUERY_MASTER_INFO:
         ...
         /* Find the master we care about. */
         for( i = 0; i < master_list.num_masters; i++ )
         {
            if( master_list.masters[i]->id ==
                query_data->masterInfo.eMasterId )
            {
               break;
            }
         }
         ...
         /* If we've found it, update query data. */
         if( i != master_list.num_masters )
         {
            uint32_t node_idx;
            icb_master_def_type *master = master_list.masters[i];
            icb_node_state_type *state = NULL;

            query_data->masterInfo.uNumPorts = master->num_agg_ports;
            query_data->masterInfo.uPortWidth = master->width;

            /* Find the correct scaling unit.
             * Initialize with a non-zero scaling value just in
             * case it's not part of any domain. */
            query_data->masterInfo.uScaleUnit = 1;
            for( node_idx = 0;
                 node_idx < master->num_bw_nodes;
                 node_idx++ )
            {
               icb_hw_node_type *node = master->bw_nodes[node_idx];
               if( ICB_HW_NODE_KIND_BANDWIDTH == node->type)
               {
                  query_data->masterInfo.uScaleUnit =
                     master->bw_nodes[node_idx]->bw_unit;
               }
            }
            
            /* Find the estimated frequency requirement for this node.
             * We're only going to use total_bw / width for this. */
            state = ul_get_master_state( query_data->masterInfo.eMasterId );
            if( NULL != state )
            {
               query_data->masterInfo.uEstFreqKHz = (uint32_t)(state->total_bw / ((uint64_t)master->width * 1000));
            }
            else
            {
               query_data->masterInfo.uEstFreqKHz = 0;
            }
...
)
{
   ...
   icb_topology_type *     topology = ((icb_arbiter_node_data_type *)query->client->resource->definition->data)->topology;
   icb_bw_request_type *   request;
...
)
{
   icb_topology_type *topology = 
      ((icb_arbiter_node_data_type *)query->client->resource->definition->data)->topology;
   icb_bw_request_type * query_requests =
      ((icb_vector_type *)(query->client->resource_data))->query_requests;
...
/** _query_client_info
 *
 *   @brief Fetch client info
 *   
 *   @param[inout] query - Pointer to query data storage
 *   
 *   @return The result of the operation (e.g. success or an error code)
 */
static ICBArb_ErrorType _query_client_info
(
   ICBArb_QueryClientType *query
)
{
   ICBArb_ErrorType error = ICBARB_ERROR_INVALID_ARG;
   ...
   {
      icb_vector_type *master_route_vector = 
         (icb_vector_type *)(query->client->resource_data);

      query->is_xo_blocking = master_route_vector->is_active &&
                              NPA_CLIENT_VECTOR == query->client->type;
      return ICBARB_ERROR_SUCCESS;
   }
...
)
{
   ...
   /* If the resource was not previously active, update this
      * change to lower layers. */
   if( resource_data->num_active_required_clients == 0 )
   {
      ul_apply_active();
   }
   resource_data->num_active_required_clients += 1;
}

void _ul_decrement_active
(
   npa_resource *resource
)
{
   ...
   /* Guard against accidental less than zero. */
   if( 0 < resource_data->num_active_required_clients )
   {
      resource_data->num_active_required_clients -= 1;

      /* If we hit zero, indicate inactive to lower layers,
         * but only after we've issued the commit. */
      if( resource_data->num_active_required_clients == 0 )
      {
         ul_remove_active();
      }
...
{
   return ul_issue_qmp_resource_request(resource, client, state);
}
...
/** ul_notify_destroy_all
 *
 * @brief Destroy all notifications as a part of deinit.
 */
void ul_notify_destroy_all( void )
{
   ul_int_master_list_type master_list;
   ...
   /* Clean up master notifications. */
   /* Reset layer state. */
   ul_target_get_master_list( &master_list );
   for( i = 0; i < master_list.num_masters; i++ )
   {
      icb_notify_list_type *notify_list = &master_list.masters[i]->notifications;
      uint32_t n, cnt = notify_list->count;

      for( n = 0; n < cnt; n++ )
      {
         icb_notify_type *notification = notify_list->head;
         ul_remove_notification( notify_list, notification );
         DALSYS_Free( notification );
      }
   }

   /* Clean up slave notifications. */
   ul_target_get_slave_list( &slave_list );
   for( i = 0; i < slave_list.num_slaves; i++ )
   {
      icb_notify_list_type *notify_list = &slave_list.slaves[i]->notifications;
      uint32_t n, cnt = notify_list->count;
...
/*============================================================================
                     Client Attribute Manipulation
============================================================================*/
/**=============================================================================
   @copydoc icbarb_set_client_attr()
==============================================================================*/
ICBArb_ErrorType icbarb_set_client_attr
(
   npa_client_handle client,
   uint32            attr
)
{
   icb_vector_type *client_data;
   ICBArb_ErrorType retval = ICBARB_ERROR_INVALID_ARG;
   
   if( NULL != client)
   {
      client_data = (icb_vector_type *)(client->resource_data);

      /* Lock to prevent client updates concurrently. */
      npa_resource_lock(client->resource);

      /* Are we setting IGNORE_ACTIVE and it isn't already set? */
      if( (ICBARB_CLIENT_ATTR_IGNORE_ACTIVE & attr) != 0 &&
         (ICBARB_CLIENT_ATTR_IGNORE_ACTIVE & client_data->attributes) == 0 )
      {
         client_data->attributes |= ICBARB_CLIENT_ATTR_IGNORE_ACTIVE;

         /* Check the current client state.
         * If the client is active, update our refcount. */
      ...
      npa_resource_unlock(client->resource);
      retval = ICBARB_ERROR_SUCCESS;
   }
...
/**=============================================================================
   @copydoc icbarb_clear_client_attr()
==============================================================================*/
ICBArb_ErrorType icbarb_clear_client_attr
(
   npa_client_handle client,
   uint32            attr
)
{
   ...
   if( NULL != client)
   {
      ...
      /* Are we clearing IGNORE_ACTIVE and is it already set? */
      if( (ICBARB_CLIENT_ATTR_IGNORE_ACTIVE & attr) != 0 &&
         (ICBARB_CLIENT_ATTR_IGNORE_ACTIVE & client_data->attributes) != 0 )
      {
         client_data->attributes &= ~ICBARB_CLIENT_ATTR_IGNORE_ACTIVE;
...
)
{
       ...
       NULL != status )
   {
       ...
       * The UL will be updating this during transactions. */
      npa_resource_lock( self );
      notify->status.high_threshold = notify->status.high_threshold && !status->high_threshold;
      notify->status.low_threshold = notify->status.low_threshold && !status->low_threshold;
      npa_resource_unlock( self );
