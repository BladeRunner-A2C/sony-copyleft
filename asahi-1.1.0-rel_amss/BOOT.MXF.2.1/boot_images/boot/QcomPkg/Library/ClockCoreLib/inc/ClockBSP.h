...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  None.
      ...
      Copyright (c) 2020, 2022, 2023 by QUALCOMM Technologies, Inc.  All Rights Reserved.
...
$Header: //components/dev/core.boot/6.1/saurku.core.boot.6.1.Hamoa_clock_lib_Enhance_Lib/QcomPkg/Drivers/ClockDxe/ClockBSP.h#1 $
...
// Unused                                       0x00020000
#define CLOCK_FLAG_LEAVE_ALWAYS_ON              0x00040000
...
#define CLOCK_FLAG_DOMAIN_FORCE_SETTINGS        0x02000000
#define CLOCK_FLAG_CESTA_CONTROLLED             0x04000000

#define CLOCK_CESTA_FLAG_DEFAULT_ENABLE         0x00000001
 ...
 *    changes.
 *  LOG_CALLBACK - Enables logging of callback invocation.
 */
...
#define CLOCK_GLOBAL_FLAG_LOG_CALLBACK                    0x00002000
#define CLOCK_GLOBAL_FLAG_CPU_TEST_IN_PROGRESS            0x00004000
...
/*
 * Macro used to indicate the maximum number of rail indices.
 */
#define CLOCK_MAX_RAIL_INDICES 0x20
 ...
 * and will not even necessarily be a pointer (i.e. could be straight uint32_t).
 */
...
typedef struct ClockCornerRefCount
{
  ...
  uint32_t           nRefCount; /**< Number of votes on eCorner. */
...
typedef struct ClockLLNodeType ClockLLNodeType;
struct ClockLLNodeType
{
  ClockLLNodeType *pNext;
};


typedef struct ClockRefCountType
{
  uint16_t nRequired;     /**< Number of active required requests */
  uint16_t nSuppressible; /**< Number of active suppressible requests */
} ClockRefCountType;


typedef struct ClockClientRefCountType ClockClientRefCountType;
struct ClockClientRefCountType
{
  /* Linked list chaining field - must be first in struct */
  ClockClientRefCountType *pNext;
  
  ClockHandle       hClient;  /**< Handle for this client */
  ClockRefCountType RefCount; /**< Reference counts for this client */
};

typedef struct
{
  uint16_t      nCESTAInst;                 /**< CESTA Instance. */
  uint16_t      nVcd;                       /**< VCD instance within CESTA. */        
}ClockCESTAInfoType;
...
struct ClockSourceNodeType
{
  ...
  uint32_t                          nRefFreqHz;                    /**< Reference frequency for the source */
  uint32_t                          nUniqueId;
  uint32_t                          nFlags;                        /**< Source flags. */
  const ClockSourceFreqConfigType   *aBSP;                          /**< Frequency configuration array. */
  uint32_t                          nBSPLen;                       /**< Length of aBSP. */
  uint32_t                         nRailMask;                      /**< Bitmask indicating the set of rails powering this domain. */
  ...
  ClockRefCountType                  RefCount;                      /**< Reference counts for all active domains */
  ClockClientRefCountType           *pClients;                      /**< Pointer to head of client list */
  ...
  uint32_t                          nCalibrationFreqConfigLen;
  ...
  uint32_t                          nCalibration;                  /**< Source calibration data */ 
  const ClockSourceRegSettingsType  *pRegSettings;                  /**< Static register settings. */
  uint32                             nRegSettingsLen;               /**< Length of pRegSettings. */
  ClockCESTAInfoType                 CESTA;                         /**< CESTA related data. */
};
...
typedef struct
{
  uint32_t            nMaxFreqHz; /**< Max frequency. */
...
{
  uint32_t             nMuxSel;      /**< Mux select value. */
  uint32_t             nFreqHz;      /**< Frequency of clock selected by nMuxSel. */
  ClockSourceNodeType *pSource;      /**< Source node associated with nMuxSel. */
  uint32_t             nSrcPostDiv;  /**< Post divider for the source */
...
struct ClockDebugMux
{
  ...
  uint32_t                   nParentSel;  /**< Value to select this mux in pParent. */
  uint32_t                   nDivider;    /**< Global divider applied to all inputs on this mux. */
...
typedef struct
{
  ...
  uint32_t                     nBSPLen;          /**< Length of aBSP. */
  uint32_t                     nRailMask;                     /**< Bitmask indicating the set of rails powering this source. */
  ClockSourceNodeType         *pSource;                       /**< Pointer to source node */
  uint32_t                     nFlags;           /**< Domain flags. */
  ...
  uint32_t                     nCallbackCount;   /**< Number of callbacks registered */
  ClockCESTAInfoType           CESTA;            /**< CESTA related data. */
} ClockDomainNodeType;
...
typedef struct
{
  ...
  uint32_t                 nUniqueId;      /**< Unique number used to identify this clock in logs. */
  uint32_t                 nCallbackCount; /**< Number of callbacks registered */
} ClockNodeType;
...
typedef struct
{
  ...
  uint32_t                     nFlags;         /**< Power domain flags. */
...
typedef enum
{
  CESTA_REG_RAW = 0,                            /**< Raw Data Type */
  CESTA_REG_CMD_DB_ADDR,                        /**< Query CmdDB */
  CESTA_REG_HLVL,                               /**< Query PwrUtils */
  CESTA_REG_MAX,  
}ClockRegType;

typedef struct
{
  const char*   szRail;                         /**< Rail string name */
  int           nVLVL;                          /**< SW VLVL for the rail */
}ClockCESTAVLVLType;

typedef struct
{
  const char*         szName;                   /**< Rail string name */
  uint32_t            nOffset;                  /**< Offset for CmdDB */           
}ClockCESTACmdDBType;

typedef union
{
  uint32_t            nVal;                     /**< Raw data value */
  ClockCESTACmdDBType CmdDB;                    /**< CmdDB Data */
  ClockCESTAVLVLType  VLVL;                     /**< Voltage level info for rail */
}ClockCESTADataType;

typedef struct 
{
  uintnt             nAddr;                     /**< Reg value address */                  
  ClockRegType       eType;                     /**< Reg type */
  ClockCESTADataType Data;                      /**< Reg Data */
}ClockCESTARegType;

typedef struct 
{
  const char*               szName;             /**< CESTA instance string name */      
  uintptr_t                 nCRMCBaseAddr;      /**< Base address for CESTA CRM */  
  uintnt                    nSequenceAddr;      /**< Sequencer start address */
  uintnt                    nBranchAddr;        /**< Branch start address */
  uintptr_t                 nRCGRLUTOffs;       /**< RCGR LUT Offset*/ 
  uintptr_t                 nLValLUTOffs;       /**< LVAL LUT Offset*/
  uintptr_t                 nVCDParam;          /**< VCD Offset*/
  uintptr_t                 nLevelParam;        /**< Level Offset*/  
  const uint32_t           *aSequence;          /**< Array of 32 bit binary sequences */
  uint32_t                  nNumSequence;       /**< Length of aSequence */     
  const uint32_t           *aBranches;          /**< Array of branch sequence */
  uint32_t                  nNumBranches;       /**< Length of aBranches */
  const ClockCESTARegType  *aRegVals;           /**< Array of reg sequence */
  uint32_t                  nNumRegVals;        /**< Length of aRegVals */
  const char               **aClockDeps;        /**< Array of Dependencies */  
  uint32_t                  nNumClockDeps;      /**< Length of aDependencies */    
  uint32_t                  nMaxPerfOL;         /**< Maximum Number of Operating levels */ 
  uint32_t                  nFlags;             /**< CESTA specific flags */ 
}ClockCESTA;
...
typedef struct ClockBSP
{
  ...
  uint32_t                    nNumDebugMuxes;   /**< Length of aDebugMuxes. */
  const ClockAliasType       *aAliases;         /**< Array of clock aliases. */
  uint32_t                    nNumAliases;      /**< Length of aAliases. */
  const ClockCESTA          **aCESTA;           /**< Array of CESTA instances */
  uint32_t                    nNumCESTA;        /**< Length of CESTA instances */
} ClockBSPType;
