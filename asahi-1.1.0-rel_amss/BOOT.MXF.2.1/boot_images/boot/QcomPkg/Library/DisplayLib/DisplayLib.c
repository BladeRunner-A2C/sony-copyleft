  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        DisplayLibBufferInfoType aDisplayBufferInfo[MDP_DISPLAY_MAX];
        ...
        MDP_OSAL_MEMZERO(&aDisplayBufferInfo[0], sizeof(aDisplayBufferInfo));
        ...
        for (eDisplayId = MDP_DISPLAY_PRIMARY; eDisplayId <= MDP_DISPLAY_SECONDARY; eDisplayId++)
        {
          ...
          {
            uWidth  = sDetectParams.aModeList[0].uWidth;
            uHeight = sDetectParams.aModeList[0].uHeight;
            ...
            aDisplayBufferInfo[eDisplayId].uWidth  = uWidth;
            aDisplayBufferInfo[eDisplayId].uHeight = uHeight;
          }
        }
        ...
        sModeParams.sSurfaceInfo.uWidth           = aDisplayBufferInfo[0].uWidth;
        sModeParams.sSurfaceInfo.uHeight          = aDisplayBufferInfo[0].uHeight;
        sModeParams.sSurfaceInfo.pPlane0Offset    = pFrameBufferBase;
        sModeParams.sSurfaceInfo.uPlane0Stride    = aDisplayBufferInfo[0].uWidth * DISPLAY_DEFAULT_BYTES_PER_PIXEL;
        ...
        {
          MDP_OSAL_MEMZERO(&sDisplayProp, sizeof(MDP_PropertiesParamType));
          sDisplayProp.sModeParams.uModeIndex = sModeParams.uModeIndex;

          if (FALSE == MDP_SUPPORTED_DISPLAY(eDisplayId))
          {
            continue;
          }
          else if (MDP_STATUS_OK != MDPSetProperty(eDisplayId, MDP_DISPLAY_PROPERTY_MODE_INFO, &sDisplayProp))
          {
            eStatus = MDP_STATUS_HW_ERROR;
          }
