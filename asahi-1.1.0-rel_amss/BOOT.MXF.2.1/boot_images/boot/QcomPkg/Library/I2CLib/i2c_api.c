...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 09/29/23   STR     Fixed logical bug in i2c parameter validation
 10/11/22   SS      Added RUMI platform check
 08/02/22   gkr     Added Clock and GPIO Disable call in de_init_hw_ctxt()
 03/25/22   bn      Addressed Unused variables
...
DalPlatformInfoPlatformType ePlatformType =  DALPLATFORMINFO_TYPE_UNKNOWN;
...
i2c_status init_hw_ctxt (hw_ctxt *h_ctxt, plat_device_config *dcfg)
{
    ...
    if (plat_map_hardware(dcfg) == FALSE)
    {
      i_status = I2C_ERROR_PLATFORM_HWIO_MAP_FAIL;
	  goto exit;
    }
...
void de_init_hw_ctxt (hw_ctxt *h_ctxt, plat_device_config *dcfg)
{
    ...
    {
        I2C_LOG(LEVEL_ERROR, "\n[I2C] Disabling Clocks Failed\n");
    }

    // Disable GPIOs
    if (plat_gpio_disable (dcfg) == FALSE)
    {
        I2C_LOG(LEVEL_ERROR, "\n[I2C] Disabling GPIOs Failed\n");
    }

    if (dcfg->core_base_addr != NULL)
    {
        if (plat_unmap_hardware(dcfg) == FALSE)
        {
          I2C_LOG(LEVEL_ERROR, "\n[I2C] Unmap Hardware Failed\n");
        }
    }
...
i2c_status i2c_open(i2c_instance instance, void **i2c_handle)
{
	...
	static boolean isRumiPlatRead = FALSE;
...
)
{
    ...
    uint32 prev_tpl_val = 0;
    
    prev_tpl_val = plat_raise_tpl();
    ...
    memset(desc,0, 2*sizeof(i2c_descriptor));
        ...
        )
{
    ...
    for (i = 0; i < num_descriptors; i++)
    {
		...
		if ((desc == NULL)          ||
            (desc->buffer != NULL && desc->length == 0)  ||
            (desc->buffer == NULL && desc->length > 0))
        {
            return FALSE;
        }
        if ((num_descriptors == 1) && (desc->buffer == NULL) && (desc->length == 0))
        {
            // address query
            break;
        }
...
i2c_status i2c_set_slave_config(void *i2c_handle, i2c_slave_config *config)
{
	if(i2c_handle == NULL || config == NULL )
	{
...
//            plat_signal_de_init (t_ctxt->signal);
		...
		//this has to be cleared as we are not updating if this is not null
		if(t_ctxt->config != config)
		{
		    plat_sys_mem_dealloc(t_ctxt -> config);
		}	
		t_ctxt -> config = NULL;
