...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 09/09/22   gkr     Redundant check removed in plat_clock_disable
 18/12/20   rk      Migration to new clock interface
 08/04/19   nm      Using HWIO APIs to map regions
 22/03/19   nm      Added changes for raising TPL
...
// For HWIO Register Mapping
#include <Protocol/EFIHWIO.h>

#define INTLOCK()

static ClockHandle clock_handle = (ClockHandle)NULL;
...
boolean plat_map_hardware (plat_device_config *config)
{
    ...
    I2C_LOG(LEVEL_INFO,"[I2C] +plat_map_hardware");
	...
	if (HWIOProtocol == NULL)
	{
	    ...
	    {
	        I2C_LOG(LEVEL_ERROR, "[I2C] plat_map_hardware ERROR: Failed to locate HWIO Protocol: 0x%08x\n", Status);
	        return FALSE;
	    }
	}
    
    Status = HWIOProtocol->MapRegionByAddress(HWIOProtocol, config->core_base_addr, &config->core_base_addr);
    if (EFI_SUCCESS != Status)
    {
        I2C_LOG(LEVEL_ERROR, "[I2C] plat_map_hardware ERROR: Failed to map core base address: 0x%08x\n", Status);
        return FALSE;
    }
    
    Status = HWIOProtocol->MapRegionByAddress(HWIOProtocol, config->common_base_addr, &config->common_base_addr);
    if (EFI_SUCCESS != Status)
    {
        I2C_LOG(LEVEL_ERROR, "[I2C] plat_map_hardware ERROR: Failed to map common base address: 0x%08x\n", Status);
        return FALSE;
    }
...
boolean plat_unmap_hardware (plat_device_config *config)
{
    ...
    I2C_LOG(LEVEL_INFO,"[I2C] +plat_unmap_hardware");
	...
	if (HWIOProtocol == NULL)
	{
	    ...
	    if ((EFI_SUCCESS != Status) || (HWIOProtocol == NULL))
	    {
	        I2C_LOG(LEVEL_ERROR, "[I2C] plat_unmap_hardware ERROR: Failed to locate HWIO Protocol: 0x%08x\n", Status);
	        return FALSE;
	    }
	}
    
    Status = HWIOProtocol->UnMapRegion(HWIOProtocol, config->core_base_addr);
    if (EFI_SUCCESS != Status)
    {
        I2C_LOG(LEVEL_ERROR, "[I2C] plat_unmap_hardware ERROR: Failed to unmap core base address: 0x%08x\n", Status);
        return FALSE;
    }
...
boolean plat_clock_enable (plat_device_config *config)
{
    ...
    /* Fails to attach to clocks, they are not available */
        res = Clock_Attach(&clock_handle, "I2C");
        //Incase of ClockTargetLibNull case, return is success and clock_handle is NULL
...
boolean plat_clock_disable (plat_device_config *config)
{
    ...
    if (clock_handle == (ClockHandle)NULL) { return FALSE; }
    ...
    {
        clock_handle = (ClockHandle)NULL; // Clear handle if detach is successful
    }
...
{
    return gBS->RaiseTPL(TPL_CALLBACK);
}

void plat_restore_tpl (uint32 prev_tpl_level)
{
    gBS->RestoreTPL(prev_tpl_level);
}
