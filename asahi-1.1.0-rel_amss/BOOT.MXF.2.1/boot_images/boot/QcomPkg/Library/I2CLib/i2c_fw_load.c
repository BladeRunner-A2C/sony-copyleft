...
--------   ---     --------------------------------------------------------
03/21/23    gkr      Minor bugs in HWIO changes are updated
02/05/23    gkr      migration to new hwio format
11/28/22    ss       Added FW load check for mini core
03/25/22    bn       Added I2C hub support
01/28/20	PR		 Added FW load check
...
{
   uint32      se_base = (uint32)(uintptr_t)dcfg->core_base_addr + dcfg->core_offset;
   uint32      qup_common_base = (uint32)(uintptr_t)dcfg->common_base_addr;
   uint32      qupv3_se_protocol = HWIO_INXF(GENI_CFG_BASE(se_base), GENI_FW_REVISION_RO, PROTOCOL);;
   uint32      qupv3_hw_ver;
   uint32      mask,i,result;
   uint32      is_SE_Minicore = HWIO_INXF(GENI_SE_DMA_BASE(se_base), SE_HW_PARAM_2, GEN_USE_MINICORES);

   elf_se_hdr* hdr = NULL;
   
   if (qupv3_se_protocol == SE_PROTOCOL_I2C)
   {
      return I2C_SUCCESS;
   }
   ...
   HWIO_OUTXF(GENI_CFG_BASE(se_base), GENI_DFS_IF_CFG, DFS_IF_EN, 0x0);
   ...
   if(!dcfg->i2c_hub)
   {
      mask = HWIO_INXF(se_base, SE_HW_PARAM_1, RX_FIFO_DEPTH);
   }
   else
   {
      mask = 16;
...
// }
   ...
   HWIO_OUTX(GENI_IMAGE_BASE(se_base), GENI_FW_REVISION,
             (HWIO_FVAL(GENI_FW_REVISION, PROTOCOL, hdr->serial_protocol) |
              HWIO_FVAL(GENI_FW_REVISION, VERSION,  hdr->fw_version)));

   HWIO_OUTX(GENI_IMAGE_BASE(se_base), GENI_S_FW_REVISION,
             (HWIO_FVAL(GENI_S_FW_REVISION, PROTOCOL, hdr->serial_protocol) |
              HWIO_FVAL(GENI_S_FW_REVISION, VERSION,  hdr->fw_version)));

   if(dcfg->i2c_mm)
   {
       HWIO_OUTXF(GENI_IMAGE_REGS_BASE(se_base), GENI_CFG_REG87,M_SW_COMP2, 0x1);
       HWIO_OUTXF(GENI_IMAGE_REGS_BASE(se_base), GENI_CFG_REG90,M_GP_CNT0, 0x98); // IDLE timeout value which will be 1.5-2.5 of SCL Period.(152)
   }

   if(!dcfg->i2c_hub)
   {
       /* Don't load the FW for mini core */
       if( !is_SE_Minicore )
      {
            memcpy((void*)(HWIO_GENI_CFG_RAMn_ADDR(GENI_IMAGE_BASE(se_base) , 0) & 0xFFFFFFFFFFFFFFFF),
                    fw_val_arr,
...
void i2c_detect_rumi_and_init(plat_device_config *dcfg)
{
   ...
   if (i2c_detect_rumi)
   {
      ...
      if (HWIO_INX(GENI_DATA_BASE(se_base) , GENI_IOS) != 0)
      {
         ...
         HWIO_OUTX(GENI_IMAGE_BASE(se_base), GENI_FW_REVISION, 0);
         HWIO_OUTX(GENI_IMAGE_BASE(se_base), GENI_S_FW_REVISION, 0);
