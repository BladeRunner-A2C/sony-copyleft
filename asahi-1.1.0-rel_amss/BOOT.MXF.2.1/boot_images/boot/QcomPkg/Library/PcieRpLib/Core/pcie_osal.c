...
typedef struct {
   ClockHandle clock_handle; /* TODO: determine if this needs to be port specific */
   char log_buff[128]; /* Temporary buffer for the log message */
} pcie_osal_ctxt_t;
...
void pcie_osal_log_msg(const char *fmt, ...)
{
   ...
   AsciiVSPrint (pcie_osal_ctxt.log_buff, sizeof(pcie_osal_ctxt.log_buff), fmt, Marker);
   DEBUG ((EFI_D_ERROR, pcie_osal_ctxt.log_buff));
...
pcie_status_t pcie_cxo_op (pcie_resources_ctxt_t* rsrc_ctxt, const pcie_pmic_config_t *pmic_config, int enable)
{
   if (enable == ENABLE)
   {
      if (rsrc_ctxt->cxo_handle == NULL)
      {
         rsrc_ctxt->cxo_handle = npa_create_sync_client(pmic_config->cxo_id, "pcie_cxo",
                                          NPA_CLIENT_REQUIRED);
         if (rsrc_ctxt->cxo_handle == NULL)
         {
            PCIE_HOST_LOG_ERROR ("Unable to get cxo handle\n");
            return PCIE_ERROR;
         }
      }
      npa_issue_required_request(rsrc_ctxt->cxo_handle, PMIC_NPA_MODE_ID_CLK_CXO_XO_ON); // Enable
      return PCIE_SUCCESS;
   }
   else
   {
      if (rsrc_ctxt->cxo_handle != NULL)
      {
         npa_issue_required_request(rsrc_ctxt->cxo_handle, PMIC_NPA_MODE_ID_CLK_CXO_XO_OFF); // off calls
         npa_destroy_client(rsrc_ctxt->cxo_handle);
         rsrc_ctxt->cxo_handle = NULL;
      }
      else
      {
            PCIE_HOST_LOG_ERROR ("Unable to get cxo handle\n");
...
pcie_status_t pcie_ldo_op (pcie_resources_ctxt_t* rsrc_ctxt, const pcie_pmic_config_t *pmic_config, int enable)
{
   if (enable == ENABLE)
   {
      if (rsrc_ctxt->pmic_handle == NULL)
      {
         rsrc_ctxt->pmic_handle = npa_create_sync_client(pmic_config->node_id, "pcie",
                                          NPA_CLIENT_REQUIRED);
         if (rsrc_ctxt->pmic_handle == NULL)
         {
      ...
      npa_issue_required_request(rsrc_ctxt->pmic_handle, PMIC_NPA_MODE_ID_GENERIC_ACTIVE); // Enable
      return PCIE_SUCCESS;
   }
   else
   {
      if (rsrc_ctxt->pmic_handle != NULL)
      {
         npa_issue_required_request(rsrc_ctxt->pmic_handle, PMIC_NPA_MODE_ID_GENERIC_OFF); // off calls
         npa_destroy_client(rsrc_ctxt->pmic_handle);
         rsrc_ctxt->pmic_handle = NULL;
      }
      else
      {
            PCIE_HOST_LOG_ERROR ("Unable to get ldo handle\n");
...
pcie_status_t pcie_cx_op (pcie_resources_ctxt_t* rsrc_ctxt, const pcie_vcs_config_t *vcs_config, int enable)
{
   if (enable == ENABLE)
   {
      if (rsrc_ctxt->cx_handle == NULL)
      {
         rsrc_ctxt->cx_handle = npa_create_sync_client(vcs_config->cx_id, "pcie_cx",
                                          NPA_CLIENT_REQUIRED);
         if (rsrc_ctxt->cx_handle == NULL)
         {
            PCIE_HOST_LOG_ERROR ("Unable to get cx handle\n");
            return PCIE_ERROR;
         }
      }
      npa_issue_required_request(rsrc_ctxt->cx_handle, vcs_config->cx_level); // Enable
      return PCIE_SUCCESS;
   }
   else
   {
      if (rsrc_ctxt->cx_handle != NULL)
      {
         npa_complete_request(rsrc_ctxt->cx_handle);
         npa_destroy_client(rsrc_ctxt->cx_handle);
         rsrc_ctxt->cx_handle = NULL;
      }
      else
      {
            PCIE_HOST_LOG_ERROR ("Unable to get cx handle\n");
...
pcie_status_t pcie_mxa_op (pcie_resources_ctxt_t* rsrc_ctxt, const pcie_vcs_config_t *vcs_config, int enable)
{
   if (enable == ENABLE)
   {
      if (rsrc_ctxt->mxa_handle == NULL)
      {
         rsrc_ctxt->mxa_handle = npa_create_sync_client(vcs_config->mxa_id, "pcie_mxa",
                                          NPA_CLIENT_REQUIRED);
         if (rsrc_ctxt->mxa_handle == NULL)
         {
            PCIE_HOST_LOG_ERROR ("Unable to get mxa handle\n");
            return PCIE_ERROR;
         }
      }
      npa_issue_required_request(rsrc_ctxt->mxa_handle, vcs_config->mxa_level); // Enable
      return PCIE_SUCCESS;
   }
   else
   {
      if (rsrc_ctxt->mxa_handle != NULL)
      {
         npa_complete_request(rsrc_ctxt->mxa_handle);
         npa_destroy_client(rsrc_ctxt->mxa_handle);
         rsrc_ctxt->mxa_handle = NULL;
      }
      else
      {
            PCIE_HOST_LOG_ERROR ("Unable to get mxa handle\n");
...
{
   pcie_status_t res;
   uint32 efi_platform_type;
   uint32 chipinfo_family;
   pcie_status_t platform_status;
   ...
   if (port_cfg->driver_config->pmic_cfg == NULL)
   {
      PCIE_HOST_LOG_ERROR("Missing pmic config");
      return PCIE_ERROR;
   }

   platform_status = pcie_get_target_platform_info(NULL, NULL, &chipinfo_family, &efi_platform_type);
   if (platform_status != PCIE_SUCCESS)
   {
   ...
   if (pcie_ldo_op (resources_ctxt, port_cfg->driver_config->pmic_cfg, ENABLE) != PCIE_SUCCESS)
   {
      ...
      //If this is error occurs on the RUMI, we can safely continue. Otherwise, fail
      if (efi_platform_type != EFI_PLATFORMINFO_TYPE_RUMI) {
         return PCIE_ERROR;
      }
   }

   if (port_cfg->driver_config->pmic_cfg->cxo_id != NULL)
   {
      if (pcie_cxo_op (resources_ctxt, port_cfg->driver_config->pmic_cfg, ENABLE) != PCIE_SUCCESS)
      {
         PCIE_HOST_LOG_ERROR ("Unable to turn ON XO_2\n");
         //If this is error occurs on the RUMI, we can safely continue. Otherwise, fail
         if (efi_platform_type != EFI_PLATFORMINFO_TYPE_RUMI) {
            return PCIE_ERROR;
         }
   ...
   if (port_cfg->driver_config->vcs_cfg != NULL)
   {
      if (pcie_cx_op (resources_ctxt, port_cfg->driver_config->vcs_cfg, ENABLE) != PCIE_SUCCESS)
      {
         PCIE_HOST_LOG_ERROR ("Unable to turn ON cx\n");
         //If this is error occurs on the RUMI, we can safely continue. Otherwise, fail
         if (efi_platform_type != EFI_PLATFORMINFO_TYPE_RUMI) {
            return PCIE_ERROR;
         }
      ...
      if (pcie_mxa_op (resources_ctxt, port_cfg->driver_config->vcs_cfg, ENABLE) != PCIE_SUCCESS)
      {
         PCIE_HOST_LOG_ERROR ("Unable to turn ON mxa\n");
         //If this is error occurs on the RUMI, we can safely continue. Otherwise, fail
         if (efi_platform_type != EFI_PLATFORMINFO_TYPE_RUMI) {
            return PCIE_ERROR;
         }
   ...
   if (chipinfo_family == EFICHIPINFO_FAMILY_MAKENA)
   {
      enable_rsc_module ();
   }
...
{
   pcie_status_t res;
   EFI_PLATFORMINFO_PLATFORM_TYPE efi_platform_type;
   pcie_status_t platform_status;
   ...
   platform_status = pcie_get_target_platform_info(NULL, NULL, NULL, &efi_platform_type);
   if (platform_status != PCIE_SUCCESS)
   {
      PCIE_HOST_LOG_ERROR ("Unable to retrieve platform info");
      return PCIE_ERROR;
   ...
   if (port_cfg->driver_config->vcs_cfg != NULL)
   {
      if (pcie_mxa_op (resources_ctxt, port_cfg->driver_config->vcs_cfg, DISABLE) != PCIE_SUCCESS)
      {
         PCIE_HOST_LOG_ERROR ("Unable to turn OFF mxa\n");
         //If this is error occurs on the RUMI, we can safely continue. Otherwise, fail
         if (efi_platform_type != EFI_PLATFORMINFO_TYPE_RUMI) {
            return PCIE_ERROR;
         }
      ...
      if (pcie_cx_op (resources_ctxt, port_cfg->driver_config->vcs_cfg, DISABLE) != PCIE_SUCCESS)
      {
         PCIE_HOST_LOG_ERROR ("Unable to turn OFF cx\n");
         //If this is error occurs on the RUMI, we can safely continue. Otherwise, fail
         if (efi_platform_type != EFI_PLATFORMINFO_TYPE_RUMI) {
            return PCIE_ERROR;
         }
   ...
   if (port_cfg->driver_config->pmic_cfg->cxo_id != NULL)
   {
      if (pcie_cxo_op (resources_ctxt, port_cfg->driver_config->pmic_cfg, DISABLE) != PCIE_SUCCESS)
      {
         PCIE_HOST_LOG_ERROR ("Unable to turn OFF XO_2\n");
         //If this is error occurs on the RUMI, we can safely continue. Otherwise, fail
         if (efi_platform_type != EFI_PLATFORMINFO_TYPE_RUMI) {
            return PCIE_ERROR;
         }
