...
/*
 ==============================================================================
  ...
  --------   ---     ---------------------------------------------------------
  01/03/24   kpm     Added support to clear sticky loopback bit
...
pcie_status_t map_mem_to_pcie_space (uint32 port_index, uint64 mem_space, uint64 size, uint64** pcie_space_ptr)
{
   ...
   pstatus = pcie_rp_get_mem_resources (port_index, PCIE_RESOURCE_NON_PREFETCH_MEM_SPACE_32, &mem_res);
   if (pstatus != PCIE_SUCCESS)
   {
...
void disable_local_analog_loopback (uint32 port_index)
{
   uint32 val;

   /* Disable LOOPBACK in the PORT_LINK_CTRL register */
   val = pcie_rp_read_config_reg (port_index, HOST_BDF_VAL, PORT_LINK_CTRL_DBI_OFFSET);
   SET_MASK_VAL (val, PORT_LINK_CTRL_OFF_LOOPBACK_ENABLE, 0);
   pcie_rp_write_config_reg (port_index, HOST_BDF_VAL, PORT_LINK_CTRL_DBI_OFFSET, val);
}
...
void disable_local_digital_loopback (uint32 port_index)
{
   uint32 val;
	
   /* Disable PIPE_LOOPBACK bit in the PIPE_LOOPBACK_CONTROL register */
   val = pcie_rp_read_config_reg (port_index, HOST_BDF_VAL, PIPE_LOOPBACK_CONTROL_DBI_OFFSET);
   SET_MASK_VAL (val, PIPE_LOOPBACK_CONTROL_OFF_PIPE_LOOPBACK, 0);
   pcie_rp_write_config_reg (port_index, HOST_BDF_VAL, PIPE_LOOPBACK_CONTROL_DBI_OFFSET, val);
...
void disable_remote_digital_loopback (uint32 port_index)
{
   uint32 val;
...
//   pcie_osal_busywait_us (5000);
   ...
   if (pstatus != PCIE_SUCCESS)
      pcie_disable_loopback (curr_root_port, curr_root_port->loopback_type);  // if Link doesn't come up this should recover
...
pcie_status_t pcie_disable_loopback (pcie_root_port_t* port_handle, pcie_loopback_type lpb_type)
{
   pcie_status_t pstatus;
   ...
   if (port_handle == NULL)
	...
	switch (lpb_type)
	{
      case PCIE_LOCAL_ANALOG_LOOPBACK:
	      disable_local_analog_loopback (port_handle->port_index);
			break;
		 
      case PCIE_LOCAL_DIGITAL_LOOPBACK:
	      disable_local_digital_loopback (port_handle->port_index);
			break;
	  
      case PCIE_REMOTE_DIGITAL_LOOPBACK:
	      disable_remote_digital_loopback (port_handle->port_index);
			break;
