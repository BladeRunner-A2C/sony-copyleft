...
/*
 ==============================================================================
  ...
  --------   ---     ---------------------------------------------------------
  12/05/23   kpm     Added support for Palawan
...
#define PCIE_PARF_SLV_ADDR_SPACE_SIZE_OFFS       (0x0358)
#define PCIE_ELBI_CS2_ENABLE_OFFS                (0x0FC4)
...
pcie_status_t pcie_assign_resources (pcie_dev_t *root_port)
{
   ...
   status = pcie_rp_get_mem_resources (port_index, PCIE_RESOURCE_NON_PREFETCH_MEM_SPACE_32, &mem_res);
...
uint32 pcie_platform_info (void)
{
   pcie_status_t pstatus;
   uint32 chipinfo_family = 0;

   pstatus = pcie_get_target_platform_info(NULL, NULL, &chipinfo_family, NULL);

   if (pstatus)
      return EFICHIPINFO_FAMILY_UNKNOWN;

   return chipinfo_family;
}
...
pcie_status_t pcie_bus_initialize (uint32 rc_index)
{
   ...
   chipinfo_family = pcie_platform_info();

   if (chipinfo_family == EFICHIPINFO_FAMILY_PALAWAN)
   {
      /* This setting is required to stop root port from exposing it's config space for bar allocation */
      pcie_cfg_write (root_h, ROOT_PORT, PCIE_ELBI_CS2_ENABLE_OFFS, 0x1);
   }
   ...
   if (chipinfo_family == EFICHIPINFO_FAMILY_PALAWAN)
   {
      /* We need to make sure to revert it back to avoid any side effects */
      pcie_cfg_write (root_h, ROOT_PORT, PCIE_ELBI_CS2_ENABLE_OFFS, 0x0);
   }
...
//   root_h->port_state = PORT_ENUMERATED; TODO needed?
   ...
   if (chipinfo_family == EFICHIPINFO_FAMILY_PALAWAN)
   {
      /* This will make sure we are picking the ADDR space size as per target configuration.
      If this is not done memio may fail on some targets */
      pstatus = pcie_rp_get_mem_resources (rc_index, PCIE_RESOURCE_DBI_BASE, &mem_resources);
      parf_reg_write (root_h, PCIE_PARF_SLV_ADDR_SPACE_SIZE_OFFS, mem_resources.sz);
   }
