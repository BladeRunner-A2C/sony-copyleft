...
// Support multiple functions : [16]
#define PORT_ATTRIB_MULTI_FUNC_EN          (1 << 16)
#define PORT_ATTRIB_IS_MULTI_FUNC_EN(pcfg) (pcfg->port_attributes & PORT_ATTRIB_MULTI_FUNC_EN)


/* Function-like macros used register modifications */
#define REG_RMW(reg_name, reg_field_mask, reg_val) \
   {PCIE_REG_OP_RMW, HWIO_OFFS(reg_name), HWIO_FMSK(reg_name, reg_field_mask), HWIO_FVAL(reg_name, reg_field_mask, reg_val)}

#define REG_RMW_FVALV(reg_name, reg_field_mask, valv) \
   {PCIE_REG_OP_RMW, HWIO_OFFS(reg_name), HWIO_FMSK(reg_name, reg_field_mask), HWIO_FVALV(reg_name, reg_field_mask, valv)}

#define REG_WRITE(reg_name, reg_val) \
   {PCIE_REG_OP_WRITE, HWIO_OFFS(reg_name), 0x0, reg_val}
...
typedef struct
{
  const uint32_t             offset; //changed from unint16_t
  const uint16_t             val;
...
typedef struct
{
   ...
   const uint8_t               hot_pluggable; // Do not attempt to link up with an endpoint, and do not power down
