...
#define PCIE_PHY_CTRL_ADDR_RUMI		0x1C00000
#define RUMI_OFFSET					0x5004
#define PCIE_PIPE_CLK_OFFSET_RUMI	0x5014
...
#define PCIE_SINGLE_FUNC_CFG_SPACE_SIZE                  (0x1000)
#define PCIE_SINGLE_DEV_CFG_SPACE_SIZE                   (PCIE_SINGLE_FUNC_CFG_SPACE_SIZE * 8)
#define PCIE_SINGLE_BUS_CFG_SPACE_SIZE                   (PCIE_SINGLE_DEV_CFG_SPACE_SIZE * 32)
...
/*PCIe register write functions */
void pcie_write_register (uintnt base, uint32 offset, uint32 val)
{
...
/**
 * Configures the ECAM blocker for 2 possible scenarios:
 * 1. If bus 1 has an endpoint, bus 1 is accessible
 * 2. If bus 1 has no endpoint, all of bus 1 configuration is blocked
 *
 * In both cases, bus 0 configuration space is blocked, except for the root port's configuration space.
 *
 * @param rp_handle pcie_rp_handle describing this segment
 * @param port_link_is_up flag specifying whether an endpoint has been detected on this segment
 *
 * @return pcie_status_t
 */
static pcie_status_t setup_ecam_blocker (pcie_rp_handle* rp_handle, uint32 port_link_is_up)
{
   ...
   /* Set the ECAM base */
   MmioWrite32((HWIO_OFFS(PCIE_PARF_ECAM_BASE) + ahb_base_pa), (UINT32)axi_base_pa);
   MmioWrite32((HWIO_OFFS(PCIE_PARF_ECAM_BASE_HI) + ahb_base_pa), (UINT32)(axi_base_pa >> 32));

   /*
    * Start the ECAM blocker base after the root port's configuration space
    *
    * If the link is up, block only to the start of ECAM for bus 1,
    * otherwise block the entire ECAM space
    */
   blocking_start = axi_base_pa + PCIE_SINGLE_FUNC_CFG_SPACE_SIZE;
   ...
   {
      blocking_end = axi_base_pa + (PCIE_SINGLE_BUS_CFG_SPACE_SIZE - 1);
   ...
   /*
    * Bus 1 blocks depends on which functions are accessible:
	* - Single function: only 1:0.0 is accessible
	* - Multi-function: 1:0.0 to 1:0.7 are accessible
	*/
   if (PORT_ATTRIB_IS_MULTI_FUNC_EN(rp_handle->port_config))
   {
      blocking_start = axi_base_pa + PCIE_SINGLE_BUS_CFG_SPACE_SIZE + PCIE_SINGLE_DEV_CFG_SPACE_SIZE;
   }
   else
   {
      blocking_start = axi_base_pa + PCIE_SINGLE_BUS_CFG_SPACE_SIZE + PCIE_SINGLE_FUNC_CFG_SPACE_SIZE;
   }
   blocking_end = axi_base_pa + PCIE_SINGLE_BUS_CFG_SPACE_SIZE + (PCIE_SINGLE_BUS_CFG_SPACE_SIZE - 1);
...
/**
 * @brief
 * Configure the iATU for a region in cfg shift mode. Config shift mode requires the ECAM
 * base to be 256MB aligned

 * @param axi_base_addr the 256MB aligned axi base that contains this iATU
 * @param atu_idx The atu region slot to configure in config shift mode
 * @param region_base_addr The base address of the start of the CFG ecam region. Must be below the region_limit_addr
 *                         Bits [27:12] form bits [31:16] that will form the BDF of the destination CFG TLP
 * @param region_limit_addr The last address of this region
 *
 * @return pcie_status_t PCIE_SUCCESS on success, PCIE_INVALID_PARAM otherwise
 */
pcie_status_t configure_iatu_cfg_shift(
   uint64 axi_base_addr,
   pcierc_atu_slot_t atu_idx,
   pcie_tlp_type_t tlp_type,
   uint64 region_base_addr,
   uint64 region_limit_addr
   )
{
   ...
   MmioWrite32((HWIO_PCIE_IATU_REGION_CTRL_2_OFF_OUTBOUND_a_OFFS(atu_idx) + axi_base_addr),
               (HWIO_PCIE_IATU_REGION_CTRL_2_OFF_OUTBOUND_a_REGION_EN_BMSK |
			    HWIO_PCIE_IATU_REGION_CTRL_2_OFF_OUTBOUND_a_CFG_SHIFT_MODE_BMSK));
...
/**
 * @brief
 * Configure the iATU for a memory region
 *
 * @param axi_base_addr The base address of the axi region that contains the iATU
 * @param atu_idx The slot for this memory region
 * @param tlp_type The outbound TLP type for this translation
 * @param region_base_addr The base of the memory region to configure
 * @param region_limit_addr The limit of the memory region to configure
 * @param target_tlp The target destination TLP address
 *
 * @return pcie_status_t
 */
                              ...
                              uint64                     target_tlp)
{
   ...
   MmioWrite32((HWIO_PCIE_IATU_UPPR_LIMIT_ADDR_OFF_OUTBOUND_a_OFFS(atu_idx) + axi_base_addr),
              (UINT32) ((region_limit_addr >> 32) & 0xFFFFFFFF));
   ...
   MmioWrite32((HWIO_PCIE_IATU_LWR_TARGET_ADDR_OFF_OUTBOUND_a_OFFS(atu_idx) + axi_base_addr),
              (UINT32) (target_tlp & 0xFFFFFFFF));
   MmioWrite32((HWIO_PCIE_IATU_UPPER_TARGET_ADDR_OFF_OUTBOUND_a_OFFS(atu_idx) + axi_base_addr),
              (UINT32) ((target_tlp >> 32) & 0xFFFFFFFF));


   /*
    * From the synopsis data book, the INCREASE_REGION_SIZE field determines whether the max region
    * size is 4GB or is set by the CX_ATU_MAX_REGION_SIZE configuration parameter.
    * In the case this region is larger than 4GB ensure this bit is set
	*/
   if ((region_limit_addr - region_base_addr + 1) > 0x100000000)
   {
...
/**
 * Configure the ATU for configuration space access for bus 1
 * This currently assumes the topology ends at bus 1 and does not support
 *
 * @param rp_handle
 *
 * @return pcie_status_t
...
{
   uint64               bus_one_ecam_base, bus_one_ecam_limit;
   ...
   bus_one_ecam_base = rp_handle->axi_base_addr + PCIE_SINGLE_BUS_CFG_SPACE_SIZE;
   bus_one_ecam_limit = bus_one_ecam_base + PCIE_SINGLE_BUS_CFG_SPACE_SIZE - 1;

   rp_handle->host_config_addr = rp_handle->axi_base_addr;
   rp_handle->ep_config_addr = bus_one_ecam_base;

   if (PORT_ATTRIB_IS_MULTI_FUNC_EN(rp_handle->port_config))
   {
      pstatus = configure_iatu_cfg_shift(rp_handle->axi_base_addr, AtuSlotCfgOut, PCIE_TLP_TYPE0_CFG_RW,
	                                     bus_one_ecam_base, bus_one_ecam_limit);
   }
   else
   {
      pstatus = configure_iatu(rp_handle->axi_base_addr, AtuSlotCfgOut, PCIE_TLP_TYPE0_CFG_RW,
	                           bus_one_ecam_base, bus_one_ecam_limit, (TLP_BUS_NUMBER_1 << 24));
   }
...
{
   pcie_status_t pstatus;
   uint32 efi_platform_type, chipinfo_family;
   ...
   pstatus = pcie_get_target_platform_info(NULL, NULL, &chipinfo_family, &efi_platform_type);
   if (pstatus != PCIE_SUCCESS)
   {
      PCIE_HOST_LOG_ERROR ("Unable to retrieve platform info");
      return PCIE_ERROR;
   }

   if (chipinfo_family == EFICHIPINFO_FAMILY_PALAWAN)
   {
      if (efi_platform_type == EFI_PLATFORMINFO_TYPE_RUMI)
      {
         pcie_write_register(PCIE_PHY_CTRL_ADDR_RUMI,RUMI_OFFSET,0xFFF);	//TO reset the phy and controller on rumi 
         pcie_write_register(PCIE_PHY_CTRL_ADDR_RUMI,RUMI_OFFSET,0x0);
         pcie_write_register(PCIE_PHY_CTRL_ADDR_RUMI,PCIE_PIPE_CLK_OFFSET_RUMI,0x05);		//To change the frequency to 62.5 MHZ 
      }
   }
...
pcie_status_t pcie_rp_enable_port_config (uint32 port_index)
{
   return pcie_rp_cfg_enable_port(port_index);
}
...
pcie_status_t pcie_rp_get_cfg_space_addr (uint32 port_index, uint16 bdf, uint32** ptr)
{
   ...
   *ptr = (uint32*)(rp_handle->axi_base_addr + (bdf * PCIE_SINGLE_FUNC_CFG_SPACE_SIZE));
...
pcie_status_t pcie_rp_setup_io_space_ATU (uint32 port_index, uint64 BaseAddr, uint64 BaseLimit, uint64 TranslationAddr)
{
   ...
   pstatus = configure_iatu (rp_handle->axi_base_addr, AtuSlotIoOut, PCIE_TLP_IORW,
...
pcie_status_t  pcie_rp_setup_mem_rw_ATU (uint32 port_index, uint64 BaseAddr, uint64 BaseLimit, uint64 TranslationAddr)
{
   ...
   pstatus = configure_iatu (rp_handle->axi_base_addr, AtuSlotNonPrefMemOut, PCIE_TLP_MEM_REQ,
                            BaseAddr, BaseLimit, TranslationAddr);
   if (pstatus != PCIE_SUCCESS)
      PCIE_HOST_LOG_ERROR ("ATU IO Setup Failed\r\n");
...
pcie_status_t pcie_rp_get_mem_resources (uint32 port_index, pcie_resource_type rsrc, pcie_mem_resource_t *res)
{
   ...
   if (rsrc == PCIE_RESOURCE_DBI_BASE)
   {
      ...
      res->sz = mem_bases[axi_reg].sz;
      res->va = OSAL_CONVERT_PA_TO_VA(res->pa);
      return PCIE_SUCCESS;
   }

   if (rsrc == PCIE_RESOURCE_NON_PREFETCH_MEM_SPACE_32)
   {
      res->pa = mem_bases[non_prefetch_32_reg].pa;
      res->sz = mem_bases[non_prefetch_32_reg].sz;
      res->va = OSAL_CONVERT_PA_TO_VA(res->pa);
      return PCIE_SUCCESS;
   }

   if (rsrc == PCIE_RESOURCE_PREFETCH_MEM_SPACE_64)
   {
      res->pa = mem_bases[prefetch_64_reg].pa;
      res->sz = mem_bases[prefetch_64_reg].sz;
      res->va = OSAL_CONVERT_PA_TO_VA(res->pa);
      return PCIE_SUCCESS;
   }
