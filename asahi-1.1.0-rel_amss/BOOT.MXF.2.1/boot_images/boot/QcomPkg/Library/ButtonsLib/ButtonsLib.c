...
#if 1
#define VOLUME_DOWN_BUTTON_IDX           0
#else
#define VOLUME_UP_BUTTON_IDX             0
#endif
/* SONY_END (Add Index for Volume Down Button) */
#define HOME_BUTTON_IDX                  1
#define CAMERA_SNAPSHOT_BUTTON_IDX       2
   ...
   )
{
   ...
   if ((PfDtKeyMaps == NULL) || (PfDtKeyMaps->PfKeyMapsCount > NUMBER_OF_KEYS) || (PfDtKeyMaps->PfKeyMapsConfig == NULL))
   {
      Status = EFI_INVALID_PARAMETER;
      DEBUG( (EFI_D_ERROR, "Key Maps dt config parsing failed, Status =  (0x%x)\r\n", Status) );
      return Status;
   }
   ...
   CopyMem( pKeyMap, &KeyMap, (PfDtKeyMaps->PfKeyMapsCount * sizeof(KEY_TYPE)) );

   *pNumberofKeys = PfDtKeyMaps->PfKeyMapsCount;
