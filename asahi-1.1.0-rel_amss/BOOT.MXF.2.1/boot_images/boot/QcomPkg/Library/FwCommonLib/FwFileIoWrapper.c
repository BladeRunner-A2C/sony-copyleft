/** @file FwFileIoWrapper.c
  ...
  Copyright (c) 2012-2015,2017-2018,2022-2023 Qualcomm Technologies, Inc.  All Rights Reserved.
...
/*=============================================================================
 ...
 --------       ---         --------------------------------------------------
 2023/03/21     manimeht    Changes to write 0xFFs to un-used space of a partition during 
                            Firmware update.
 2018/11/09     prema       Added FlushBlock handling changes wherever missed to handle
 2018/11/06     gparam      Fix to correct the validation of FwEntry size against
    ...
    )
{
  ...
  HANDLE_ERR_CONTINUE(Status,FlushBlocks);
    ...
    )
{
  ...
  UINTN                    RemainingBlocks                 = 0;
  UINTN                    RemainingBufferSizeAligned      = 0;
  UINTN                    BlocksWritten                   = 0;
  UINT32                   StartTime                       = 0;
  UINT32                   EndTime                         = 0;
  VOID                     *BufferToWrite                  = NULL;
  VOID                     *ReadBuffer                     = NULL;
  UINT32                   ReadStartTime                   = 0;
  UINT32                   ReadEndTime                     = 0;
  ...
  // Check that buffer size is less than partition size
  if (BufferSizeAligned > ((BlockIo->Media->LastBlock + 1) * BlockSize))
  {
    Status = EFI_OUT_OF_RESOURCES;
    DEBUG((EFI_D_WARN,"[%a] Requested buffer does not fit in to current partition (Partition size: %d, Aligned Buffer size: %d. "
           "Returning: %r\n", __FUNCTION__, (BlockIo->Media->LastBlock * BlockSize), BufferSizeAligned, Status));
    goto FwUpdatePartitionWriteExit;
  }

  DEBUG((EFI_D_WARN,
    "[%a] BlockSize %d Original BufferSize %d BufferSizeAligned %d LastBlock %d\n",
    __FUNCTION__, 
  ...
  BlocksWritten = BufferSizeAligned / BlockSize ;
  RemainingBlocks  = ((BlockIo->Media->LastBlock + 1) - BlocksWritten); 
  RemainingBufferSizeAligned = (RemainingBlocks * BlockSize);
  DEBUG((EFI_D_WARN,
    "[%a] BlocksWritten %d RemainingBlocks  %d RemainingBufferSizeAligned %d \n", 
    __FUNCTION__,
    BlocksWritten,
    RemainingBlocks,
    RemainingBufferSizeAligned));

  // If remaining blocks is not equal to 0
  // Check if all blocks are already filled with 0xFFs.
  // If not write 0xFF or skip
  if(RemainingBlocks != 0)
  {
    Status = AllocateAndFillOnes(
      EfiBootServicesData,
      RemainingBufferSizeAligned,
      &BufferToWrite);
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_ERROR,
        "[%a] AllocateAndFill failed. Returning: %r\n",
        __FUNCTION__,
        Status));
      goto FwUpdatePartitionWriteExit;
    }
    //// Read all remaining block into a buffer for comparison
    Status = AllocateAndZero(
      EfiBootServicesData,
      RemainingBufferSizeAligned,
      &ReadBuffer);
    if(EFI_ERROR(Status))
    {
      DEBUG((EFI_D_ERROR,
        "[%a] AllocateAndFill(%d) failed. Returning: %r\n",
        __FUNCTION__,
        RemainingBufferSizeAligned,
        Status));
      goto FwUpdatePartitionWriteExit;
    }
    ReadStartTime = GetTimerCountms();
    ...
    {
      DEBUG((EFI_D_ERROR,
        "[%a] ReadBlocks failed. Returning: %r\n",
        __FUNCTION__,
        Status));
      goto FwUpdatePartitionWriteExit;
    }  
    
    ReadEndTime = GetTimerCountms();
    DEBUG((EFI_D_INFO,
      "\n  Reading remaining blocks took  %lld Ms\n",
      (ReadEndTime - ReadStartTime)));
    StartTime = GetTimerCountms();
    if((CompareMem (
      ((char*)ReadBuffer),
      BufferToWrite,
      RemainingBufferSizeAligned) != 0))
    {
      DEBUG((EFI_D_WARN,
        "[%a] Writing 0xFF from  starting LBA %d size %d\n",
        __FUNCTION__,
        BlocksWritten,
        RemainingBufferSizeAligned)); 
      ...
      {
        DEBUG((EFI_D_WARN,
          "[%a] WriteBlocks failed. Returning: %r\n",
          __FUNCTION__,
          Status));
        goto FwUpdatePartitionWriteExit;
      }
    ...
    {
      DEBUG((EFI_D_WARN,
        "[%a] Skip writing 0xFF as remaining bytes are already 0xFFs\n",
	    __FUNCTION__)); 
    }
    EndTime = GetTimerCountms();
    DEBUG((EFI_D_INFO,
      "\n  Compare and Filling 0xFFs took  %lld Ms\n\n",
      (EndTime - StartTime)));
  }
