...
static const QCOM_FW_UPDATE_NV_IDENTIFIER NVConfigData[QCOM_FW_UPDATE_NV_MAX] = {
  ...
  {QCOM_FW_UPDATE_NV_GPT1,                    QCOM_FW_UPDATE_NV_GPT1_NAME,                    QCOM_FW_UPDATE_NV_SIZE_UNKNOWN,          (EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS)},
  {QCOM_FW_UPDATE_NV_GPT2,                    QCOM_FW_UPDATE_NV_GPT2_NAME,                    QCOM_FW_UPDATE_NV_SIZE_UNKNOWN,          (EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS)},
  {QCOM_FW_UPDATE_NV_GPT3,                    QCOM_FW_UPDATE_NV_GPT3_NAME,                    QCOM_FW_UPDATE_NV_SIZE_UNKNOWN,          (EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS)},
  ...
  {QCOM_FW_UPDATE_NV_PARTITIONS_UNDER_UPDATE, QCOM_FW_UPDATE_NV_PARTITIONS_UNDER_UPDATE_NAME, QCOM_FW_UPDATE_NV_SIZE_UNKNOWN,          (EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS)}
};
  ...
  )
{
  ...
  if(Operation == QCOM_FW_UPDATE_GET_NV_DATA)
  {
    Status = GetVariable2(
  ...
  else
  {
    ...
    if (Operation == QCOM_FW_UPDATE_SET_NV_DATA_FLUSH)
    {
...
{
  EFI_STATUS                     status                 = EFI_SUCCESS;
  UINT8                         *pGptNvDataPtr          = NULL;
  UINTN                          gptNvDataDefaultSize   = 0;
     ...
     (status == EFI_BUFFER_TOO_SMALL))
  {
    ...
    status = SetUpdateStateDonotFlush(FW_STATE_NOT_IN_PROGRESS);
    HANDLE_ERROR_LABEL(status,SetUpdateStateDonotFlush(FW_STATE_NOT_IN_PROGRESS),CheckAndCreateNVDataExit);
  }
  ...
  if(FW_STATE_NOT_IN_PROGRESS == updateState)
  {
    ...
    HANDLE_ERROR_LABEL(status, GetStorageDiskList, CheckAndCreateNVDataExit);
    ...
    for (diskIdx = 0; diskIdx < pDiskList->count; diskIdx++)
    {
      ...
      if (NULL == pDiskInfo)
      {
          ...
          DEBUG_ERR_LABEL (
              "CheckAndCreateNVData: pDiskInfo is NULL\n",
              CheckAndCreateNVDataExit);
      }

      // Some devices might choose not to have BOOT or GPP backup.
      // Ex: UFS will not take BOOT GPT backup
      gptNvDataDefaultSize = QCOM_FW_UPDATE_GPT_NV_SIZE(pDiskInfo->nvDataMaxEntries);
      if (0 != gptNvDataDefaultSize)
      {
        pGptNvDataPtr = AllocateZeroPool (gptNvDataDefaultSize);
        status = (NULL == pGptNvDataPtr) ? EFI_OUT_OF_RESOURCES : EFI_SUCCESS;
        HANDLE_ERROR_LABEL(status, AllocateZeroPool (pGptNvDataPtr), CheckAndCreateNVDataExit);

        status = SetGPTNvDataDonotFlush(pDiskInfo->nvType, pGptNvDataPtr, gptNvDataDefaultSize);
        HANDLE_ERROR_LABEL(status,SetGPTNvDataDonotFlush(pDiskInfo->nvType),CheckAndCreateNVDataExit);

        FreePool (pGptNvDataPtr);
        pGptNvDataPtr = NULL;
      }
    }
  }    
  ...
  if((status == EFI_NOT_FOUND) ||
     (status == EFI_BUFFER_TOO_SMALL))
  {
    status = SetAttemptCountDonotFlush(0);
    HANDLE_ERROR_LABEL(status,SetAttemptCountDonotFlush(0),CheckAndCreateNVDataExit);
  }
  ...
  if((status == EFI_NOT_FOUND) ||
     (status == EFI_BUFFER_TOO_SMALL))
  {
    status = SetBreakingChangeNumberDonotFlush(0);
    HANDLE_ERROR_LABEL(status,SetBreakingChangeNumberDonotFlush(0),CheckAndCreateNVDataExit);
  }
  HANDLE_ERROR_LABEL(status,GetBreakingChangeNumber,CheckAndCreateNVDataExit);

CheckAndCreateNVDataExit:

  if (NULL != pGptNvDataPtr)
  {
    FreePool (pGptNvDataPtr);
    pGptNvDataPtr = NULL;
  }
...
)
{
    return GetOrSetNVData(
      NVType,
      CapsulePendingListNvData,
      CapsulePendingListNvDataSize,
      QCOM_FW_UPDATE_GET_NV_DATA
    );
}
