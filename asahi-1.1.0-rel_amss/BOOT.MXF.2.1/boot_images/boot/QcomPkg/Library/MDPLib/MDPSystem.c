...
{
  {EFICHIPINFO_FAMILY_HAMOA,  0x00DE018},
};
...
{
  {MDP_DISPLAY_PERIPHERAL_DSI0,  0x0001},
  {MDP_DISPLAY_PERIPHERAL_DSI1,  0x0002},
  {MDP_DISPLAY_PERIPHERAL_DP0,   0x0100},
  {MDP_DISPLAY_PERIPHERAL_DP1,   0x0200},
  {MDP_DISPLAY_PERIPHERAL_DP2,   0x0400},
  {MDP_DISPLAY_PERIPHERAL_EDP0,  0x1000},
};
...
/* Query IMEM display configure address and size */
...
MDP_Status MDP_HostRouter_QMPPhy_HWIOBaseAddr(EFIChipInfoFamilyType  sEFIChipSetFamily, HAL_MDP_SWMappedHWBaseAddressesType* pMDSSAddress)
{
  ...
  gBS->LocateProtocol(&gEfiHwioProtocolGuid, NULL, (void**)&HWIOProtocol);
  
  if (NULL == pMDSSAddress)
  {
    eStatus = MDP_STATUS_BAD_PARAM;
  }
  else if (NULL == HWIOProtocol)
  {
    DEBUG ((EFI_D_ERROR, "ERROR: Failed to locate HWIO Protocol: Status=%d\n", eStatus));
  }
  else
  {
    switch (sEFIChipSetFamily)
    {
      case EFICHIPINFO_FAMILY_HAMOA:
      {
        if (EFI_SUCCESS != (eStatus = HWIOProtocol->MapRegion(HWIOProtocol, "USB4_HOST_ROUTER_0_USB4_WRAPPER", (UINT8**)&pVirtAddr)))
        {
          DEBUG((EFI_D_ERROR, "ERROR: Failed to get USB4_HOST_ROUTER_0_USB4_WRAPPER address"));
        }
        else
        {
          pMDSSAddress->uUSB4HostRouterAddress = (uintPtr)pVirtAddr;

          if (EFI_SUCCESS != (eStatus = HWIOProtocol->MapRegion(HWIOProtocol, "AHB2PHY_0_AHB2PHY_0", (UINT8**)&pVirtAddr)))
          {
            DEBUG((EFI_D_ERROR, "ERROR: Failed to get AHB2PHY_0_AHB2PHY_0 address"));
          }
          else
          {
            pMDSSAddress->uQMPPhy0BaseAddress = (uintPtr)pVirtAddr;

            if (EFI_SUCCESS != (eStatus = HWIOProtocol->MapRegion(HWIOProtocol, "AHB2PHY_1_AHB2PHY_1", (UINT8**)&pVirtAddr)))
            {
              DEBUG((EFI_D_ERROR, "ERROR: Failed to get AHB2PHY_1_AHB2PHY_1 address"));
            }
            else
            {
              pMDSSAddress->uQMPPhy1BaseAddress = (uintPtr)pVirtAddr;
            }
          }
        }
      }
...
MDP_Status MDP_SaveFirmwareEnvironmentVariable(MDP_Display_IDType eDisplayId)
{
  ...
  if (MDP_STATUS_OK == (eStatus = MDPPlatformConfigure(eDisplayId, MDPPLATFORM_CONFIG_GETPLATFORMINFO, &sPlatformParams)))
  {
    ...
    CHAR8*                      MemLabel              = "Display_Reserved";
...
MDP_Status MDP_SetDisplayBootConfig(UINT32 uFrameBuffer1Addr, UINT32 uFrameBuffers1Size)
{
  ...
  else if (sizeof(DISPLAY_BOOT_CONFIG) > uIMEMDisplayConfigSize)
  {
    eStatus = MDP_STATUS_BAD_PARAM;
...
EFI_STATUS MDP_Display_LoadFile(CHAR8 *pFileName, UINT8 **pFileBuffer, UINT32 *pBufferSize, UINT32 *pRetSize, UINT32 uFlags)
{
  ...
  CHAR16      LoadFile[LOAD_FILE_SIZE];
  ...
  else
  {
      ...
      AsciiStrToUnicodeStrS(pFileName, (CHAR16*)&LoadFile, LOAD_FILE_SIZE );
...
MDP_Status MDP_Display_LoadFileEx(CHAR16* pPatitionName, CHAR8* pFileName, UINT8** pFileBuffer, UINT32* pBufferSize, uint64 uReadOffset, UINT32* pRetSize, UINT32 uFlags)
{
  ...
  else if (RETURN_SUCCESS != AsciiStrToUnicodeStrS(pFileName, (CHAR16*)pFileNameStr, (AsciiStrLen(pFileName)+1)))
  {
...
/****************************************************************************
*
** FUNCTION: MDP_Display_GetPreferredDisplay()
*/
/*!
* \brief
*   This function will return the display to be used for fetching 
*   EDID info for EDID protocols
*
* \param none
*
* \retval MDP_Display_IDType
*
****************************************************************************/
MDP_Display_IDType MDP_Display_GetPreferredDisplay(void)
{
  const char          sEnvVar[]     = "PrebootDisplaySelection";
  uint8               uPreBootValue = 0;
  MDP_Display_IDType  eDisplayId    = MDP_DISPLAY_PRIMARY;
  
  MDP_GetEnvironmentVariable((CHAR16*)sEnvVar, (uint8*)&uPreBootValue, sizeof(uint8));

  switch (uPreBootValue)
  {
    /* Select eDP */
    case 1:
    ...
    /* Select DP1 */
    case 3:
      eDisplayId = MDP_DISPLAY_EXTERNAL3;
    break;

    /* Select DP2 */
    case 4:
      eDisplayId = MDP_DISPLAY_EXTERNAL5;
    break;    
