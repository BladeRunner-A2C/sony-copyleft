...
Local Prototypes
-------------------------------------------------------------------------- */
static MDP_Status SendPanelXMLSequence(I2C_DriverCtx *pI2CDriverCtx, void *pCommandBuffer, MDP_I2C_Configuration *pI2CConfig);
static MDP_Status SendPanelI2CSequence(I2C_DriverCtx *pI2CDriverCtx, void *pCommandBuffer, MDP_I2C_Configuration *pI2CConfig);
...
MDP_Status I2CDriver_Init(MDP_I2C_Configuration *pI2CConfig)
{
  ...
  {
    if ((0 == pI2CConfig->uI2CGSBICore) ||
        (pI2CConfig->uI2CNumOfSlaves > MDP_I2C_SLAVE_ID_NUM))
    {
    ...
    else
    {
      ...
      for (uI = 0; uI < pI2CConfig->uI2CNumOfSlaves; uI++)
      {
        
        MDP_OSAL_MEMZERO(&pI2CDriverCtx->sSlaveCtx[uI], sizeof(slave_context));
        
        // Configure QUP core number
        pI2CDriverCtx->sSlaveCtx[uI].eI2CCoreInstance = (i2c_instance)(pI2CConfig->uI2CGSBICore);
        ...
        if (I2C_SUCCESS != i2c_open(pI2CDriverCtx->sSlaveCtx[uI].eI2CCoreInstance, &pI2CDriverCtx->sSlaveCtx[uI].hI2CHandle))
        {
          ...
          DEBUG ((EFI_D_ERROR, "DisplayDxe: I2CDriver_Init() I2C Init(SlaveAddress=0x%x) failed \n", pI2CConfig->aI2CSlaveAddressList[uI]));
          break;
        }
        else
        {
          pI2CDriverCtx->sSlaveCtx[uI].sI2CCfg.slave_address              = pI2CConfig->aI2CSlaveAddressList[uI];
          pI2CDriverCtx->sSlaveCtx[uI].sI2CCfg.bus_frequency_khz          = (pI2CConfig->uI2CBusFrequency) ? pI2CConfig->uI2CBusFrequency : MDP_I2C_DEFAULT_BUS_FREQUENCY_KHZ;
                                       ...
                                       uint32                 uFlags)
{
  ...
  switch (eCommand)
  {
    ...
    {
      if ((NULL != pI2CConfig->sI2CInitSequence.pBuffer) &&
          (0 != pI2CConfig->uI2CNumOfSlaves))
      {
        eStatus = SendPanelXMLSequence(pI2CDriverCtx, pI2CConfig->sI2CInitSequence.pBuffer, pI2CConfig);
    ...
    case MDP_PANEL_COMMAND_CUSTOM:
    {
      if ((NULL != pI2CConfig->sI2CInitSequence.pBuffer) &&
          (0 != pI2CConfig->uI2CNumOfSlaves))
      {
        eStatus = SendPanelI2CSequence(pI2CDriverCtx, pI2CConfig->sI2CInitSequence.pBuffer, pI2CConfig);
...
//
//  @brief
//      Local helper function to send a I2C command sequence to the panel. This function handles 
//      I2C command packets in XML_BinaryPacketType
//
//  @params
//      [IN] pI2CDriverCtx
  ...
  else
  {
    ...
    while (0 != sXMLBinaryData.uPacketLength)
    {
      ...
      else
      {
        ...
        for (uI=0; uI< pI2CConfig->uI2CNumOfSlaves; uI++)
        {
          if ((uint8)pI2CConfig->aI2CSlaveAddressList[uI] == *pI2CCmd)
          {
            if (NULL == pI2CDriverCtx->sSlaveCtx[uI].hI2CHandle)
            {
              eStatus = MDP_STATUS_FAILED;
            ...
            {       
              ...
              uint16      uOffsetLen          = (uint16)(pI2CConfig->uI2COffsetLengthInByte);
...
//-------------------------------------------------------------------------------------------------
//
static MDP_Status SendPanelI2CSequence(I2C_DriverCtx         *pI2CDriverCtx, 
                                       void                  *pCommandBuffer, 
                                       MDP_I2C_Configuration *pI2CConfig)
{
  MDP_Status             eStatus       = MDP_STATUS_OK;
  ...
  {
    uint8 *pI2CCmd = (uint8 *)pCommandBuffer;
    uint32 uI      = 0;
