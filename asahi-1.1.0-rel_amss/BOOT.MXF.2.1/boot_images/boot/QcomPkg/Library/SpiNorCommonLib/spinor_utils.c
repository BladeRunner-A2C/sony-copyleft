 ...
 *
 * Copyright (c) 2017,2018,2020,2022,2023 Qualcomm Technologies Inc.
 * All Rights Reserved.
...
----------  ---  --------------------------------------------------
2023-04-28  sc   Modify access protection to include slot
2022-11-14  sc   Add support for multiple spinor slots
2022-09-29  ak   Use default permission to access whole nor flash
...
SPINOR_STATUS spinor_io_access_validation (spinor_handle_t handle, uint32 start_block, uint32 block_count)
{
  ...
  if (info != NULL)
  {
    ...
    for (i = 0; i < count && spinor_dev->slot_no == info[i].slot; i++)
    {
...
SPINOR_STATUS spinor_io_byte_access_validation (spinor_handle_t handle, uint32 byte_offset, uint32 byte_count)
{
  ...
  return spinor_io_access_validation (handle, start_block, block_count);
}
...
SPINOR_STATUS read_spi (spinor_handle_t handle, uint8 reg_opcode, uint8 len, uint8 *reg_value)
{
  ...
  status = spial_transfer(handle, command_mode, 0, reg_value, len, reg_opcode);
  if (status != SPINOR_DEVICE_DONE)
  {
...
SPINOR_STATUS spinor_device_lookup (spinor_handle_t handle, void *spinor_config_tbl)
{
  ...
  (void)spinor_core_read_device_id(handle, pdevice_id, DEVICE_ID_LEN_IN_BYTES);

  /* Read parameters using SFDP */
  manufacturer = spinor_dev->config_data->device_id & 0xFF;
  status = spinor_read_sfdp(handle, spinor_dev->config_data, manufacturer);
                                                  ...
                                                  uint32 start_block, uint32 block_count)
{
  ...
  spinor_drv_info *spinor_dev = (spinor_drv_info*)parti_handle;
