...
----------  ---  -------------------------------------------------------
2023-03-31  sc   Set DDR_144 as the highest priority for read mode.
2023-03-20  sc   Set default spinor frequency for 201.5MHz clock frequency.
2022-12-13  sc   Set params if they aren't already set by Device Tree.
 ...
 *
 **********************************************************/
SPINOR_STATUS spinor_read_sfdp(spinor_handle_t handle, spinor_config_data *config, uint8 manufacturer)
{
  ...
  status = spial_transfer(handle, qspi_read_sfdp_mode, 0, (uint8*)header,
  ...
  for (count = 0; count <= header->num_param_headers; count++)
  {
    ...
    status = spial_transfer(handle, qspi_read_sfdp_mode, offset, (uint8*)paramh,
  ...
  status = spial_transfer(handle, qspi_read_sfdp_mode, basic_param_offset,
  ...
  if (addr_bytes == 4)
  {
    ...
    status = spial_transfer(handle, qspi_read_sfdp_mode, inst_4byte_offset,
    ...
    if (inst4b->read_ddr_144_support == 1)
    {
    ...
    else if (inst4b->read_144_support == 1)
    {
    ...
    config->read_opcode = read_inst;
    config->read_wait_state = read_dummy_c;
    config->write_mode = prog_mode;
    config->write_opcode = prog_inst;
    if (erase_4k_index >= 4)
    {
    ...
    if (bparam->enter_4b_addr & 0b01000000) /* Always in 4 byte mode */
      spinor_dev->addr_mode_4byte = ADDR_4BYTE_MODE_7;
    else if (bparam->enter_4b_addr & 0b00000001) /* Issue B7 instruction */
      spinor_dev->addr_mode_4byte = ADDR_4BYTE_MODE_1;
    else if (bparam->enter_4b_addr & 0b00100000) /* Dedicated 4Byte ADDR Instructions. */
      spinor_dev->addr_mode_4byte = ADDR_4BYTE_MODE_6;
    else if (bparam->enter_4b_addr & 0b00000010) /* instruction 06 then B7 */
      spinor_dev->addr_mode_4byte = ADDR_4BYTE_MODE_2;
    else if (bparam->enter_4b_addr & 0b00000100) /* */
      spinor_dev->addr_mode_4byte = ADDR_4BYTE_MODE_3;
    else if (bparam->enter_4b_addr & 0b00001000)
      spinor_dev->addr_mode_4byte = ADDR_4BYTE_MODE_4;
    else if (bparam->enter_4b_addr & 0b00010000)
      spinor_dev->addr_mode_4byte = ADDR_4BYTE_MODE_5;
    else
      spinor_dev->addr_mode_4byte = ADDR_4BYTE_MODE_MAX; /* 4 byte address not supported? odd*/

  }
  else /* 3 byte address mode */
  {

    spinor_dev->addr_mode_4byte = ADDR_4BYTE_NOT_SUPPORTED;
  ...
  /* Check if the parameters are set by DT*/
  if (config->dma_enable == FALSE)
  {
    config->dma_enable = TRUE;
  }
  if (config->erase_status_polling_ms == 0)
  {
    config->erase_status_polling_ms = 5; /* not part related */
  } 
  if (config->write_status_polling_usec == 0)
  {
    config->write_status_polling_usec = 100; /* not part related */
  }
  config_data->freq_khz = 50375;
