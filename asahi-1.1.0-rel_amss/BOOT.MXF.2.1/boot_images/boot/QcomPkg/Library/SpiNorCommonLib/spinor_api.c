...
----------  ---  ---------------------------------------------------
2022-11-14  sc   Add spinor_open_slot API.
2022-06-29  sc   Add spinor background init start API
...
/* Handles for multiple spinor slots*/
#define INVALID_SLOT 0xFFFF
spinor_drv_info spinor_dev_handles[SPINOR_MAX_SLOTS];
spinor_config_data spinor_core_config_data[SPINOR_MAX_SLOTS];
uint32 last_slot_used = INVALID_SLOT;


static SPINOR_STATUS spinor_handle_validation(spinor_handle_t handle)
{
  uint32 handle_no;
  ...
  if (handle == NULL)
  {
    goto spinor_handle_validation_end;
  }

  for (handle_no = 0; handle_no < SPINOR_MAX_SLOTS; handle_no++)
  {
    if(handle == &spinor_dev_handles[handle_no])
    {
...
spinor_handle_validation_end:
  return status;
}

static SPINOR_STATUS spinor_check_slot_in_use (spinor_handle_t handle)
{
  ...
  {
    spinor_log (&spinor_dev->debug_log, 0x182);
    goto spinor_check_slot_end;
  }

  if (spinor_dev->in_use == FALSE)
  {
    status = SPINOR_DEVICE_SLOT_NOT_OPEN;
    spinor_log (&spinor_dev->debug_log, 0x180);
    goto spinor_check_slot_end;
  }
  else
  {
    ...
    {
      spinor_log (&spinor_dev->debug_log, 0x181);
      goto spinor_check_slot_end;
    }

    if(last_slot_used != spinor_dev->slot_no)
    {    
      last_slot_used = spinor_dev->slot_no;
      status = spial_init(handle, TRUE, spinor_dev->slot_no);
                              ...
                              struct spinor_info *spinor_info)
{
  ...
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x102);
  ...
  /* validate slot */
  status = spinor_check_slot_in_use (handle);
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x101);
    goto spinor_get_info_end;
  }
...
static SPINOR_STATUS spinor_power_on (spinor_handle_t handle)
{
  SPINOR_STATUS status = SPINOR_DEVICE_FAIL;
  spinor_drv_info *spinor_dev = (spinor_drv_info*)handle;
...
static SPINOR_STATUS spinor_power_off (spinor_handle_t handle)
{
  SPINOR_STATUS status = SPINOR_DEVICE_FAIL;
  spinor_drv_info *spinor_dev = (spinor_drv_info*)handle;
...
SPINOR_STATUS spinor_open (spinor_handle_t *handle)
{
  ...
  status = spinor_open_slot (handle, 0);
  if (SPINOR_DEVICE_DONE != status)
  {
...
SPINOR_STATUS spinor_close (spinor_handle_t handle)
{
  ...
  if (spinor_dev->nor_init_done == FALSE || spinor_dev->in_use == FALSE)
  {
  ...
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x123);
    goto spinor_close_end;
  }

  status = spinor_power_off(handle);
  if (SPINOR_DEVICE_DONE != status)
  {
                          ...
                          uint32 byte_count, void *buffer)
{
  ...
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x134);
  ...
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x133);
    goto spinor_read_end;
  }
  ...
  status = spinor_core_read(handle, byte_offset, byte_count, (uint8 *)buffer);
  if (status != SPINOR_DEVICE_DONE)
  {
                            ...
                            uint32 byte_count, void *buffer)
{
  ...
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x145);
  ...
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x144);
    goto spinor_write_end;
  }
  ...
  status = spinor_io_byte_access_validation (handle, byte_offset, byte_count);
  if (status != SPINOR_DEVICE_DONE)
  {
  ...
  status = spinor_core_write (handle, transfer_start_addr,
                            ...
                            uint32 start_block, uint32 block_count)
{
  ...
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x155);
  ...
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x154);
    goto spinor_erase_end;
  }
  ...
  status = spinor_io_access_validation (handle, start_block, block_count);
  if (status != SPINOR_DEVICE_DONE)
  {
  ...
  status = spinor_core_erase(handle, start_block, block_count);
  if (status != SPINOR_DEVICE_DONE)
  {
                                 ...
                                 uint32 start_block, uint32 block_count)
{
  ...
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x168);
  ...
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x167);
    goto spinor_erase_bulk_end;
  }
  ...
  if (transfer_start_block % max_transfer_blocks)
  {
    ...
    status = spinor_core_erase(handle, transfer_start_block, transfer_size);
    if (status != SPINOR_DEVICE_DONE)
    {
  ...
  if (block_count >= max_transfer_blocks)
  {
    ...
    status = spinor_core_erase_bulk(handle, transfer_start_block, transfer_size);
    if (status != SPINOR_DEVICE_DONE)
    {
  ...
  status = spinor_core_erase(handle, transfer_start_block, block_count);
  if (status != SPINOR_DEVICE_DONE)
  {
...
SPINOR_STATUS spinor_background_init_start (uint32 slot_no, uint32 init_context)
{
  bool qspi_init_status = TRUE;
SPINOR_STATUS status = SPINOR_DEVICE_FAIL;

  if (slot_no >= SPINOR_MAX_SLOTS)
  {
    status = SPINOR_DEVICE_INVALID_PARAMETER; 
    goto spinor_background_init_start_end;
  }

  spinor_handle_t handle = (spinor_handle_t)&spinor_dev_handles[slot_no];
  spinor_drv_info *spinor_dev = (spinor_drv_info*)handle;
  ...
  if ((spinor_dev->nor_init_done == FALSE))
  {
    spinor_log (&spinor_dev->debug_log, 0x107);
    goto spinor_background_init_start_end;
  }

  if ((status = spinor_get_dt_config (handle)) != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x108);
    goto spinor_background_init_start_end;
  }
  ...
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_log (&spinor_dev->debug_log, 0x109);
    goto spinor_background_init_start_end;
  }
  
   last_slot_used = spinor_dev->slot_no;
spinor_background_init_start_end:
return status;
}

/**********************************************************
 * Assigns the handle to access the spinor device
 *
 *
 * @param handle [IN/OUT]
 *   Pointer to SPINOR interface handle as an input parameter.
 ...
 * 
 * @param slot_no [IN]
 *   Slot used by the client
 *
 * @return int [OUT]
 *   Result of the operation.
 *
 **********************************************************/
SPINOR_STATUS spinor_open_slot (spinor_handle_t *handle, uint32 slot_no)
{
  SPINOR_STATUS status = SPINOR_DEVICE_FAIL;
  spinor_drv_info *spinor_dev = (spinor_drv_info*)handle;
  ...
  {
    status = SPINOR_DEVICE_INVALID_PARAMETER; 
    goto spinor_open_slot_end;
  }

  *handle = (spinor_handle_t)&spinor_dev_handles[slot_no];
  spinor_dev_handles[slot_no].slot_no = slot_no;
  spinor_dev_handles[slot_no].config_data = &spinor_core_config_data[slot_no];
  spinor_dev = &spinor_dev_handles[slot_no];
  ...
  else
  {
    if (spinor_dev->nor_init_done == FALSE)
    {
      ...
      status = spinor_init(*handle, (void *)spinor_config, &spinor_core_config_data[slot_no]);
      last_slot_used = slot_no;
    }

    if (spinor_dev->nor_init_done == FALSE)
    {
      spinor_log (&spinor_dev->debug_log, 0x173);
      goto spinor_open_slot_end;
    }

    if(last_slot_used != slot_no && last_slot_used != INVALID_SLOT)
    {
      last_slot_used = slot_no;
      status = spial_init(*handle, TRUE, slot_no);

      if (SPINOR_DEVICE_DONE != status)
      {
        spinor_log (&spinor_dev->debug_log, 0x174);
        goto spinor_open_slot_end;
      }
    }
  }

  status = spinor_power_on(*handle);
  if (SPINOR_DEVICE_DONE != status)
  {
    spinor_log (&spinor_dev->debug_log, 0x175);
    goto spinor_open_slot_end;
  ...
  spinor_dev->in_use = TRUE;
  last_slot_used = slot_no;
  status = SPINOR_DEVICE_DONE;

spinor_open_slot_end:
  return status;
}
