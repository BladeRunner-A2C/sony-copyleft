...
----------  ---  ---------------------------------------------------
2023-03-20  sc   Use DT values for spinor frequency.
2022-11-18  sc   Update driver to utilize spinor handle.
2022-06-29  sc   Add spinor device tree related APIs
2022-02-01  sa   Fixed compilation errors.
2020-07-23  wek  Use the same error codes on all SPI-NOR dirver files.
2020-04-08  wek  Add SFDP support, Add one option for 4-byte address.
...
static SPINOR_STATUS enable_quad_mode_3 (spinor_handle_t handle);
static SPINOR_STATUS enable_quad_mode_1_4_5 (spinor_handle_t handle);
static SPINOR_STATUS enable_quad_mode_2 (spinor_handle_t handle);
static SPINOR_STATUS vendor_error_check (spinor_handle_t handle, uint8 operation_type);
...
SPINOR_STATUS spinor_core_read_device_id (spinor_handle_t handle, void *device_id, uint8 device_id_len)
{
  ...
  status = read_spi(handle, READ_ID_CMD, device_id_len, (uint8 *)device_id);
  return status;
}
...
SPINOR_STATUS spinor_core_erase (spinor_handle_t handle, uint32 start_block, uint32 block_count)
{
  ...
  while (block_count)
  {
    status = write_enable (handle, &qspi_status);
    ...
    if (SPINOR_DEVICE_DONE != spial_transfer(handle, command_mode, address, NULL, 0,0))
    {
    ...
    while (timeout > 0)
    {
      ...
      (void)read_spi(handle, READ_STATUS_CMD, 1, &qspi_status);
      if ((qspi_status & PROG_ERASE_WRITE_BUSY_BMSK) == 0)
      {
        status = vendor_error_check(handle, ERASE_OPERATION);
        if (status == SPINOR_DEVICE_DONE)
        {
...
static SPINOR_STATUS vendor_error_check (spinor_handle_t handle, uint8 operation_type)
{
  ...
  status = read_spi(handle, err_status_register, 1, &qspi_status);

  /* Check device status */
  if ((status == SPINOR_DEVICE_DONE) && (qspi_status & status_mask))
  {
    status = SPINOR_DEVICE_FAIL;
 ...
 *
 **********************************************************/
SPINOR_STATUS spinor_core_write (spinor_handle_t handle, uint32 address,
                                      uint32 byte_count, uint8 *buffer)
{
  ...
  if (((uintnt)buffer & 0x1F) || (byte_count & 0x3))
  {
  ...
  while (byte_count)
  {
    ...
    /* Command | data addr | data buffer | length | opcode */
      ...
      if (SPINOR_DEVICE_DONE != spial_transfer(handle, command_mode, address, buffer, transfer_size,0))
    {
      status = SPINOR_DEVICE_FAIL;
    ...
    while (timeout > 0)
    {
      read_spi(handle, READ_STATUS_CMD, 1, &qspi_status);
      if ((qspi_status & PROG_ERASE_WRITE_BUSY_BMSK) == 0)
      {
        status = vendor_error_check(handle, WRITE_OPERATION);
        if (status == SPINOR_DEVICE_DONE)
        {
          /* No error. Break out of "while" loop */
          break;
 ...
 *
 **********************************************************/
SPINOR_STATUS spinor_core_read (spinor_handle_t handle, uint32 address,
                                     uint32 byte_count, uint8 *buffer)
{
  SPINOR_STATUS status = SPINOR_DEVICE_DONE;
  ...
  while (byte_count > 0)
  {
    ...
    /* Command | data addr | data buffer | length | opcode */
      ...
      if (SPINOR_DEVICE_DONE != spial_transfer(handle, command_mode, address, temp_buffer,transfer_size, 0))
    {
      status = SPINOR_DEVICE_FAIL;
...
SPINOR_STATUS write_enable (spinor_handle_t handle, uint8 *result)
{
  ...
  spinor_drv_info *spinor_dev = (spinor_drv_info*)handle;
    ...
    if (SPINOR_DEVICE_DONE != spial_transfer(handle, command_mode, 0, NULL, 0,0))
  {
    status = SPINOR_DEVICE_FAIL;
  ...
  while (timeout > 0)
  {
    ...
    status = read_spi(handle, READ_STATUS_CMD, 1, result);
    if ((status == SPINOR_DEVICE_DONE) && (*result & STATUS_WR_EN_MASK))
    {
      break;
    }
...
SPINOR_STATUS enable_quad_mode (spinor_handle_t handle, uint8 qer_mode)
{
  ...
  case ENABLE_QUAD_MODE_5:
    ...
    status = enable_quad_mode_1_4_5(handle);
    break;

  case ENABLE_QUAD_MODE_2:
    status = enable_quad_mode_2(handle);
    break;

  case ENABLE_QUAD_MODE_3:
    status = enable_quad_mode_3(handle);
    break;
...
static SPINOR_STATUS enable_quad_mode_1_4_5 (spinor_handle_t handle)
{
  ...
  (void)read_spi(handle, READ_STATUS_CMD, 1, &status_reg1);
  (void)read_spi(handle, READ_CFG1_CMD, 1, &status_reg2);
  ...
  status = write_spi(handle, WRITE_STATUS_CMD, 2, &temp[0]);
  if (status != SPINOR_DEVICE_DONE)
  {
  ...
  status = check_operation_status(handle, &temp[0]);
  if (status != SPINOR_DEVICE_DONE)
  {
...
static SPINOR_STATUS enable_quad_mode_2 (spinor_handle_t handle)
{
  ...
  (void)read_spi(handle, READ_STATUS_CMD, 1, &status_reg);
  ...
  status = write_spi(handle, WRITE_STATUS_CMD, 1, &status_reg);
  if (status != SPINOR_DEVICE_DONE)
  {
  ...
  status = check_operation_status(handle, &temp);
  if (status != SPINOR_DEVICE_DONE)
  {
...
static SPINOR_STATUS enable_quad_mode_3 (spinor_handle_t handle)
{
  ...
  (void)read_spi(handle, READ_STATUS_2_CMD, 1, &status2_reg);
  ...
  status = write_enable(handle, &temp);
  if (status != SPINOR_DEVICE_DONE)
  {
    spinor_dev->debug_log.spinor_status = temp;
  ...
  status = write_spi(handle, WRITE_STATUS_2_CMD, 1, &status2_reg);
  if (status != SPINOR_DEVICE_DONE)
  {
...
SPINOR_STATUS set_4byte_address_mode(spinor_handle_t handle)
{
  ...
  switch (spinor_dev->addr_mode_4byte)
  {
  ...
  case ADDR_4BYTE_MODE_1:
    ...
    status = write_spi(handle, ENTER_4B_ADDR_CMD, 0, &temp);
    if (status != SPINOR_DEVICE_DONE)
    {
...
SPINOR_STATUS check_operation_status (spinor_handle_t handle, uint8 *result)
{
  ...
  while (timeout > 0)
  {
    ...
    (void)read_spi(handle, READ_STATUS_CMD, 1, result);
...
SPINOR_STATUS spinor_init(spinor_handle_t handle, void *spinor_config_table, spinor_config_data *spinor_core_config_data)
{
  ...
  spinor_drv_info *spinor_dev = (spinor_drv_info*)handle;
  uint32 slot_no = spinor_dev->slot_no;
  ...
  /* Initialize all the spinor_dev structure, except client's handles info and slot_no */
  spinor_drv_info_init(handle, spinor_core_config_data);
  spinor_dev->slot_no = slot_no;
  ...
  nor_status = spinor_controller_init(handle);
  ...
  status = spinor_device_lookup(handle, spinor_config_table);
  if (status != SPINOR_DEVICE_DONE)
  {
  ...
  nor_status = spial_init (handle, qspi_init_status, spinor_dev->slot_no);
  if (status != SPINOR_DEVICE_DONE)
  {
  ...
  {
    nor_status = enable_quad_mode(handle, spinor_dev->config_data->quad_enable_mode);
    if (nor_status != SPINOR_DEVICE_DONE)
    {
  ...
  {
    nor_status = set_4byte_address_mode(handle);
    if (nor_status != SPINOR_DEVICE_DONE)
    {
...
SPINOR_STATUS spinor_controller_init (spinor_handle_t handle)
{
  ...
  status = spial_init(handle, qspi_init_status, spinor_dev->slot_no);
  if (status != SPINOR_DEVICE_DONE)
  {
...
SPINOR_STATUS spinor_deinit (spinor_handle_t handle)
{
  ...
  (void)spinor_core_deinit(handle);
...
{
  return spial_enter_working_state();
}

SPINOR_STATUS spinor_core_power_saving_mode(void)
{
  return spial_enter_low_power_state();
}
...
SPINOR_STATUS spinor_get_dt_default_config (spinor_handle_t handle)
{
  ...
  if (spinor_dev->config_data != NULL)
  {
    spinor_dev->config_data->erase_status_polling_ms = ERASE_POLL_MS; 
    spinor_dev->config_data->write_status_polling_usec = WRITE_POLL_US; 
...
SPINOR_STATUS spinor_get_dt_config(spinor_handle_t handle)
{
  ...
  uint8 enable_logging = 0;
  SPINOR_STATUS status = SPINOR_DEVICE_FAIL;
  spinor_drv_info *spinor_dev = (spinor_drv_info*)handle;

  snprintf (fdt_node_str, DT_PATH_LEN, "/soc/qspi/spinor");

  if (fdt_get_node_handle (&node, NULL, fdt_node_str) == 0) 
  {
    if (fdt_get_uint8_prop (&node, "enable-logging", (uint8 *)&enable_logging) != 0) 
    {
      goto spinor_get_dt_config_end; 
    }
    if (fdt_get_uint8_prop(&node, "dma-enable", (uint8 *)&spinor_dev->config_data->dma_enable) != 0) 
    {
      goto spinor_get_dt_config_end; 
    }
  }
  ...
  if(spinor_dev->slot_no == SPINOR_SLOT_0)
  {
    snprintf (fdt_node_str, DT_PATH_LEN, "/soc/qspi/spinor/device-params/slot-0"); 
  }
  else if(spinor_dev->slot_no == SPINOR_SLOT_1)
  {
    snprintf (fdt_node_str, DT_PATH_LEN, "/soc/qspi/spinor/device-params/slot-1"); 
  }
  else 
  {
    goto spinor_get_dt_config_end; 
  ...
  if (fdt_get_node_handle (&node, NULL, fdt_node_str) == 0) 
  {
    if (fdt_get_uint32_prop(&node, "max-freq-khz", (uint32 *)&spinor_dev->config_data->freq_khz) != 0) 
    {
        goto spinor_get_dt_config_end; 
    }
  }

  snprintf (fdt_node_str, DT_PATH_LEN, "/soc/qspi/spinor/polling-params"); 
  if (fdt_get_node_handle (&node, NULL, fdt_node_str) == 0) 
  {
    if (fdt_get_uint8_prop (&node, "erase-status-polling-msec", (uint8 *)&spinor_dev->config_data->erase_status_polling_ms) != 0)
    {
      goto spinor_get_dt_config_end;
    }
    if ( fdt_get_uint16_prop (&node, "write-status-polling-usec", (uint16 *)&spinor_dev->config_data->write_status_polling_usec) != 0)
    {
      goto spinor_get_dt_config_end;
    }        
  }
  ...
  if (enable_logging) 
  {
    DEBUG ((EFI_D_INFO, "SPINOR Device Tree Settings"));
    DEBUG ((EFI_D_INFO, "SPINOR: max-freq-khz %d kHz", spinor_dev->config_data->freq_khz)); 
    DEBUG ((EFI_D_INFO, "SPINOR: dma-enable %d", spinor_dev->config_data->dma_enable)); 
    DEBUG ((EFI_D_INFO, "SPINOR Polling parameters: erase-status-polling-msec %d ms", spinor_dev->config_data->erase_status_polling_ms)); 
    DEBUG ((EFI_D_INFO, "SPINOR Polling parameters: write-status-polling-usec %d us", spinor_dev->config_data->write_status_polling_usec)); 
  }

  status = SPINOR_DEVICE_DONE;
spinor_get_dt_config_end:
  if (status != SPINOR_DEVICE_DONE)
  {
    status = spinor_get_dt_default_config(handle);
  }
  return status;
}
