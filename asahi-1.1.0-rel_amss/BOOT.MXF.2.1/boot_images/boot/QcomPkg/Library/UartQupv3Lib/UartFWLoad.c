...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/UartQupv3Lib/UartFWLoad.c#1 $

when       who     what, where, why
--------   ---     --------------------------------------------------------
11/01/23   gst      CGS WA for RUMI Console logs
02/05/23   gkr      migration to new hwio format
08/23/22   ss       Added check for FW loading on mini cores
...
{
   uint32      soc_emulation_type;
   uint32      se_base = uart_ctxt->properties->uart_base;
   uint32      qup_common_base = uart_ctxt->properties->qup_common_base;
   uint32      qupv3_hw_ver = HWIO_INX(qup_common_base, QUPV3_HW_VERSION);
   
   uint32      qupv3_hw_param = HWIO_INX(GENI_SE_DMA_BASE(se_base), SE_HW_PARAM_2);
   elf_se_hdr* hdr;
   ...
   soc_emulation_type = uart_get_platform_type();
   if(soc_emulation_type == SOC_EMULATION_TYPE_RUMI)
   {
        HWIO_OUTX (GENI_CFG_BASE(se_base), GENI_CGC_CTRL, 0x78);   //WA for Pakala to be removed when rtl issue will be fixed
   }
   /* HPG section 3.1.7.4 */
...
// }
   ...
   HWIO_OUTX(GENI_IMAGE_BASE(se_base), GENI_S_FW_REVISION,
	...
	/* Load the FW image for non mini core qup */
   if( !(qupv3_hw_param & HWIO_SE_HW_PARAM_2_GEN_USE_MINICORES_BMSK) )
   {
     memscpy((void*)(HWIO_GENI_CFG_RAMn_ADDR(GENI_IMAGE_BASE(se_base) , 0) & 0xFFFFFFFFFFFFFFFF),
   ...
   if(soc_emulation_type == SOC_EMULATION_TYPE_RUMI)
   {
        HWIO_OUTX (GENI_CFG_BASE(se_base), GENI_CGC_CTRL, 0x78);
   }
