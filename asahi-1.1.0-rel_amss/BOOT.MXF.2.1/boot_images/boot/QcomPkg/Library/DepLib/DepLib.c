/**@file DepLib.c
  ...
  Copyright (c) 2015 - 2016, 2021, 2023 Qualcomm Technologies, Inc. All Rights Reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 11/15/23   ps      module name input string parsing logic updated 
 03/16/21   cm      Changes in SetNoExecSetReadWrite
...
PrintModuleName (IN CHAR8* PdbPointer)
{
  ...
  if (PdbPointer)
  {
    ...
    {
      ModuleNamePtr = PdbPointer;
      while(AsciiStrStr(ModuleNamePtr, "/") != NULL)
      {
        ModuleNamePtr = AsciiStrStr(ModuleNamePtr, "/") + AsciiStrLen ("/");
      }
    }
...
SetNoExecSetReadWrite (IN EFI_PHYSICAL_ADDRESS StartAddr, IN UINT64 Length)
{
  ...
  //On Armv7 the clear memory regions clears RWX attributes and not Readonly.
  //So clearing first and then setting NoExec later
