...
#define IORTNODE_TYPE_INVALID         0
...
IORTData* IORT_GetData()
{
    IORTData* pIORT = NULL;
    /* TODO: Need to add a Mutex. */
    if (!gIORTData.bValid) 
    {
      gIORTData.pIORT = &g_iort_info;
...
static IORTNamedComponentNode* IORT_FindNamedNode(char* DevObjName)
{
    ...
    if(IORTData) {
        ...
        while(NumIORTNodes) {
            ...
            if(IORTNODE_TYPE_INVALID != pIORTNode->Type)
            {
                NumIORTNodes--;
            }
        }
...
static IORTSMMUv1v2Node* IORT_GetSMMUv1v2Node(uint64 BaseAddr)
{
