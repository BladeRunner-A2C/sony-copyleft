 ...
 * @note        
 *
 *              Copyright 2011,2023 Qualcomm Technologies, Incorporated.
 *              All Rights Reserved.
...
void qusb_al_bulk_init(qusb_max_speed_required_t speed_required)
{
  ...
  {
    // reach here due to Vbus low timeout, this is Work around for Pmic reading success but Vbus report wrong value.
    // We dont start the enumeration timeout timer for real USB disconnect state. (Match Vbus looping w/o workaround)
	// Side affect, enter timeout will assume Vbus connect and typeC CC_Lane_A.
	// So if USB later connect LaneB, USB will only enumerate in HighSpeed
	// To recover, USB need POPI within 30 Sec to go back to Superspeed.
    qusb_uart_log("Pmic_error_or_VBUS_LOW", qusb_vbus_detect());
	qusb_timer_stop();
  }
