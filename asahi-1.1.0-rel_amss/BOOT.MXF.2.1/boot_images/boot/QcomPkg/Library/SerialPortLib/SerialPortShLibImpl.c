...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 03/22/23   rk      Reading Pcd value to update static SyncIOEnable.
 07/28/21   cm      Minor change in sync IO print statement
 07/22/20   pr      Add IOCTL to support setting UART Baud Rate
 06/23/20   cm      Support to log UEFI independent of SBL passing params
 12/03/19   md      Added SerialPortDrainToDest and ReplayLogs API.
...
#ifdef FEATURE_XBOOT
//#include <Library/XLogHookLib.h>
#endif /* FEATURE_XBOOT */

/* Version 1.2 */
#define CURRENT_SIO_LIB_VERSION  0x00010002
...
STATIC BOOLEAN SyncIOEnable = FixedPcdGetBool(PcdSyncIOEnable);
STATIC BOOLEAN UartPowered = TRUE;
STATIC BOOLEAN ReInitDone = FALSE;
#ifdef FEATURE_XBOOT
STATIC struct xlog_meta_info
{
	unsigned long log_buf_start; /* 64bit */
	unsigned long log_buf_ptr;   /* 64bit */
	unsigned int  log_buf_size;  /* 32bit */
	unsigned int  log_buf_wrap;  /* 32bit */
} *MetaInfo;
#endif /*FEATURE_XBOOT*/
...
UINTN DequeueSendBufferedData (UINT8* , UINTN);
...
UINTN 
static DequeueToDest (UINT8* DestBuffer, UINTN BufferSize)
{
  UINT8* BufferEndPtr;
  UINTN BytesToSend, ContiguousSize;
  UINTN BytesSent = 0;
...
/*    Destbuff  NoPort 
 *    0         0         If Dest Buff is NULL and NOPORT == FALSE then write only to UART
 *    0         1         If Dest Buff is NULL and NOPORT == TRUE then no write. Make active data size 0.
 *    1         0         If Dest Buff is not NULL and NOPORT == FALSE then write to buffer and UART. 
 *    1         1         If Dest Buff is not NULL and NOTPORT == TRUE then write only to buffer. 
 * */
    if (NoPortOut == TRUE && DestBuffer == NULL)
    {
      BytesSent = ActiveDataSize;
      //BytesToSend = ActiveDataSize = 0;
      return BytesToSend;
    }

    if (DestBuffer)
        BytesSent = CopyMemS (DestBuffer, BufferSize, DeQPtr, BytesToSend);
...
UINTN
DequeueSendBufferedData (UINT8* DestBuffer OPTIONAL,
                         UINTN BufferSize OPTIONAL)
{
  UINTN BytesToSend;
  ...
  BytesToSend = DequeueToDest (DestBuffer, BufferSize);
...
EnqueueData (UINT8* SrcDataBuffer, UINTN ReqBytes)
{
  ...
  while (BytesRemaining)
  {
    ...
    if (EnQPtr >= BufferEndPtr) {
    	...
    	MetaInfo->log_buf_wrap = 1;
    }
    MetaInfo->log_buf_ptr = (unsigned long)EnQPtr;
#else /*!FEATURE_XBOOT*/
...
/* Copies log from source buffer to destination buffer */
EFI_STATUS 
static CopyLogBuffer (
   UINT8 *DestBuffer,
   UINTN  DestBuffSize,
   UINTN  StartOffset OPTIONAL
   )
{
  UINT8 *SourceStartPtr = DeQPtr;
  UINT8 *BufferEndPtr;

  if ((DestBuffer == NULL) || (DestBuffSize == 0) || 
      (((UINTN)DestBuffer + DestBuffSize) <= (UINTN)DestBuffer))
  {
  ...
  if (DeQPtr + StartOffset > BufferEndPtr)
    return EFI_INVALID_PARAMETER;

  if (StartOffset)
    SourceStartPtr = (UINT8*)(DeQPtr + StartOffset);
  ...
  * */
  ...
  if (EnQPtr < DeQPtr)
  {        
    CopyMemS (DestBuffer, DestBuffSize, SourceStartPtr, BufferEndPtr - DeQPtr);
    DestBuffer += BufferEndPtr - DeQPtr;

    CopyMemS (DestBuffer, DestBuffSize, SerialPortBuffer, DeQPtr - SerialPortBuffer);
  }
  else /* Scenario 1 */
    CopyMemS (DestBuffer, DestBuffSize, SerialPortBuffer, BufferEndPtr - SerialPortBuffer);  

  return EFI_SUCCESS;
}


/* Adjust DeQPtr and EnQPtr to point to new buffer positions */
EFI_STATUS 
static ReInitPosPtrs (UINT8 *SerialBuffer, UINTN BufferSize)
{
  ...
  {  
    ...
    DeQPtr = SerialBuffer ;
    EnQPtr = SerialBuffer + ActiveDataSize;
  }
  else /* Scenario 1 */
  {
    /* Adjust EnQPtr and DeqPtr to point to new Buffer location */
    DeQPtr = ((DeQPtr - SerialPortBuffer) + SerialBuffer) ;
    EnQPtr = ((EnQPtr - SerialPortBuffer) + SerialBuffer);
  }
...
#ifdef FEATURE_XBOOT
  MetaInfo->log_buf_start = (unsigned long)SerialPortBuffer;
  MetaInfo->log_buf_ptr = (unsigned long)EnQPtr;
  MetaInfo->log_buf_size = PortBufferSize;
  MetaInfo->log_buf_wrap = 0;
#endif /* FEATURE_XBOOT */
...
#ifdef FEATURE_XBOOT
//  XLogSerialPortWriteHook((char *)Buffer, (long)WriteBytes);
#endif /* FEATURE_XBOOT */
  ...
  while (Remaining != 0) {
    ...
    DequeueSendBufferedData (NULL, 0);
  }
                  ...
                  IN  UINT32   BufferSize    OPTIONAL)
{
...
#ifdef FEATURE_XBOOT
  MetaInfo = (struct xlog_meta_info *)SerialBuffer;
  SerialPortBuffer = SerialBuffer + sizeof(struct xlog_meta_info);
  PortBufferSize = BufferSize - sizeof(struct xlog_meta_info);
  MetaInfo->log_buf_start = (unsigned long)SerialPortBuffer;
  MetaInfo->log_buf_ptr = (unsigned long)SerialPortBuffer;
  MetaInfo->log_buf_size = PortBufferSize;
  MetaInfo->log_buf_wrap = 0;
#else /*!FEATURE_XBOOT*/
  SerialPortBuffer = SerialBuffer;
  ...
  if(boot_shared_imem_cookie_ptr != NULL && boot_shared_imem_cookie_ptr->boot_log_size <= SIZE_32KB)
  {
    ...
    while ((LogBuffPtr != NULL) && (*(LogBuffPtr) != '\0')) ++LogBuffPtr;
...
SerialPortDrain (VOID)
{
  ...
  if (NoPortOut == FALSE)
    Bytes = DequeueSendBufferedData (NULL, 0);
...
SerialPortFlush (VOID)
{
  ...
  {
    do {
      BytesSent = DequeueSendBufferedData (NULL, 0);
                    ...
                    IN  UINT32   BufferSize    OPTIONAL)
{
  ...
  /*NoPortOut = TRUE;  
  DequeueToDest(SerialBuffer, BufferSize);*/

  Status = CopyLogBuffer (SerialBuffer, BufferSize, 0) ;   
  if (Status != EFI_SUCCESS)
    DEBUG ((EFI_D_ERROR, "Copy of logs to buffer failed \n"));

  Status = ReInitPosPtrs (SerialBuffer, BufferSize);
  if (Status != EFI_SUCCESS)
    DEBUG ((EFI_D_ERROR, "ReInit of buffer's position pointers failed.  \n"));
...
UINTN
SerialPortDrainToDest (UINT8* DestBuffer,
                       UINTN BufferSize)
{
  UINTN Bytes = 0;
  UINTN  State = 0;

  if (DestBuffer == NULL || BufferSize == 0)
  {
    Bytes = ActiveDataSize = 0;
    return Bytes;
  }

  State = SerialPortLock (); 
  Bytes = DequeueSendBufferedData (DestBuffer, BufferSize);
  SerialPortUnlock (State);
   ...
   )
{
  ...
  if (MaxLogSize != NULL)
  {
    *MaxLogSize = PortBufferSize;
    return EFI_SUCCESS;
  }
  if (DestBuffer == NULL || DestBuffSize == 0)
  return EFI_INVALID_PARAMETER;

  Status = CopyLogBuffer (DestBuffer, DestBuffSize, StartOffset);
  if (Status != EFI_SUCCESS) 
      DEBUG ((EFI_D_ERROR, "Replay of logs to buffer failed \n"));
