/** @file FwProvisionFuseLib.c
  ...
  Copyright (c) 2013-2015,2017-2018,2023 Qualcomm Technologies, Inc.  All Rights Reserved.
...
--------    ---        ----------------------------------------------------------- 
2023/07/24  kabhi      Moving screen-visible debug messages to the debug stream
2018/08/02  pj         Updated call to Rng protocol to directly use raw Rng algo
2017/07/16  gpavithr   Replaced error logs with relevant information
...
)
{
  ...
  if(EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR,"LocateProtocol(DPP) returned Status:%r\n", Status));
    goto DppItemDeleteExit;
  }
...
QrdProvStage1(VOID) 
{
  ...
  DEBUG((DEBUG_WARN, "  GENMAC.DAT found, deleting it..\n"));
  /* Delete QCOM/GENMAC.DAT exist */
  Status = DppItemDelete(L"QCOM",GEN_MAC_FILENAME);
  HANDLE_ERROR_LABEL(Status, DppItemDelete, QrdProvStage1Exit);

  /* locate rng protocol */
  Status = gBS->LocateProtocol(&gQcomRngProtocolGuid, NULL, (VOID **) &pEfiQcomEngProtocol);
  if(EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR,"LocateProtocol(QCOM_RNG_PROTOCOL) returned Status:%r\n", Status));
    goto QrdProvStage1Exit;
  }
  ...
  /* Passing NULL in the Algo ID field to call raw Rng algorithm */
  Status = pEfiQcomEngProtocol->GetRNG(pEfiQcomEngProtocol, NULL, RngBufSize, pRngBuffer);
  ...
  DEBUG((DEBUG_WARN, "Provisioning BT MAC address completed.\n"));
  ...
  */
  ...
  HANDLE_ERROR_LABEL(Status, DppItemWrite, QrdProvStage1Exit);
  DEBUG((DEBUG_WARN, "Provisioning WLAN MAC address completed.\n"));
QrdProvStage1Exit:
