                                ...
                                uint32                        uAUXWriteFifoFlags)
{
  ...
  {
    uAUXDataReg = HWIO_OUT_FLD(0, DPTX_0_AUX_DATA, DATA, puByte[i]);
    if (0 == i)
    {
      /* For the first write, enable INDEX_WRITE with INDEX = 0 
       * Address will auto-increment for the rest of the writes
       */
      uAUXDataReg |= HWIO_DPTX_0_AUX_DATA_INDEX_WRITE_BMSK;
    }
    out_dword(uOffset + HWIO_DPTX_0_AUX_DATA_ADDR, uAUXDataReg);
  }

  /* Set Transaction details */
  uAUXTransCtrlReg = HWIO_OUT_FLD(0, DPTX_0_AUX_TRANS_CTRL, NUM_TR, (puWriteData->uNumofTransactions - 1));

  if(HAL_DP_AUX_TRANSACTION_MODE_I2C == puWriteData->eTransactionType )
  {
    uAUXTransCtrlReg = HWIO_OUT_FLD(uAUXTransCtrlReg, DPTX_0_AUX_TRANS_CTRL, TR_TYPE, 1);
  }

  if (HAL_DP_AUXFLAGS_READ & uAUXWriteFifoFlags )
  {
    uAUXTransCtrlReg = HWIO_OUT_FLD(uAUXTransCtrlReg, DPTX_0_AUX_TRANS_CTRL, NO_SEND_STOP, 0);
    uAUXTransCtrlReg = HWIO_OUT_FLD(uAUXTransCtrlReg, DPTX_0_AUX_TRANS_CTRL, NO_SEND_ADDR, 1);
  }
  else
  {
    uAUXTransCtrlReg = HWIO_OUT_FLD(uAUXTransCtrlReg, DPTX_0_AUX_TRANS_CTRL, NO_SEND_STOP, 1);
    uAUXTransCtrlReg = HWIO_OUT_FLD(uAUXTransCtrlReg, DPTX_0_AUX_TRANS_CTRL, NO_SEND_ADDR, 1);
  }
  ...
  uAUXTransCtrlReg = HWIO_OUT_FLD(uAUXTransCtrlReg, DPTX_0_AUX_TRANS_CTRL, GO, 1);

  out_dword(uOffset + HWIO_DPTX_0_AUX_TRANS_CTRL_ADDR, uAUXTransCtrlReg);
...
{
  uint32  i           = 0;
  uint32  uAUXDataReg = (HWIO_DPTX_0_AUX_DATA_DATA_RW_BMSK | HWIO_DPTX_0_AUX_DATA_INDEX_WRITE_BMSK);
  uintPtr uOffset     = HAL_DP_GetRegBaseOffset(eControllerId);
  ...
  uAUXDataReg = in_dword(uOffset + HWIO_DPTX_0_AUX_DATA_ADDR);

  /* Read the data from the DATA FIFO */
  for (i = 0; i < psReadPacket->uReadSize; i++)
  {
    uAUXDataReg                       = in_dword(uOffset + HWIO_DPTX_0_AUX_DATA_ADDR);
    psReadPacket->puReadDataBuffer[i] = HWIO_GETVAL(DPTX_0_AUX_DATA, uAUXDataReg, DATA);
  }
...
{
  uintPtr uOffset = HAL_DP_GetRegBaseOffset(eControllerId);
  uint32  uRegVal = in_dword(uOffset + HWIO_DPTX_0_AUX_CTRL_ADDR);

  if (TRUE == bEnable)
  {
    uRegVal = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_CTRL, AUX_ENABLE, 1);

    // Do not stop controller on AUX error
    uRegVal = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_CTRL, AUX_DPPHY_ERROR_STOP_TX, 1);
  }
  else
  {
    uRegVal = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_CTRL, AUX_ENABLE, 0);
  }
...
{
  uintPtr uOffset    = HAL_DP_GetRegBaseOffset(eControllerId);
  uint32  uAuxStatus = in_dword(uOffset + HWIO_DPTX_0_AUX_STATUS_ADDR);
...
void HAL_DP_AUX_ClearTransaction(DP_ControllerIDType eControllerId)
{
  ...
  out_dword(uOffset + HWIO_DPTX_0_AUX_TRANS_CTRL_ADDR, 0);
}
                                    ...
                                    uint32                     uAUXConfigFlags)
{
  ...
  if (NULL != psAUXCtrlConfig->psAUXAttrib)
  {
    ...
    {
      uRegVal = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_CTRL, AUX_TX_ENDIANNESS, *(psAUXCtrlConfig->psAUXAttrib->pbTXLittleEndian));
    }
    if (NULL != psAUXCtrlConfig->psAUXAttrib->pbRXLittleEndian)
    {
      uRegVal = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_CTRL, AUX_RX_ENDIANNESS, *(psAUXCtrlConfig->psAUXAttrib->pbRXLittleEndian));
    }
    if (NULL != psAUXCtrlConfig->psAUXAttrib->pbStopTXOnPhyErr)
    {
      uRegVal = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_CTRL, AUX_DPPHY_ERROR_STOP_TX, *(psAUXCtrlConfig->psAUXAttrib->pbStopTXOnPhyErr));
    }
    ...
    {
      uRegVal = HWIO_OUT_FLD(uRegVal, DPTX_0_TIMEOUT_COUNT, TIMEOUT_COUNT, *(psAUXCtrlConfig->psAUXAttrib->puTimeoutCount));
      out_dword(uOffset +  HWIO_DPTX_0_TIMEOUT_COUNT_ADDR, uRegVal);
    }

    /* AUX limits for an interrupt  */
    uRegVal  = in_dword(uOffset + HWIO_DPTX_0_AUX_LIMITS_ADDR);
    if (NULL != psAUXCtrlConfig->psAUXAttrib->puTimeoutLimit)
    {
      uLimit = *(psAUXCtrlConfig->psAUXAttrib->puTimeoutLimit);
      
      uRegVal  = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_LIMITS, TIMEOUTS_LIMIT,      (uLimit & 0xF));
      uRegVal  = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_LIMITS, TIMEOUTS_LIMIT_MSB,  ((uLimit >> 4) & 0xF));
    }
    if (NULL != psAUXCtrlConfig->psAUXAttrib->puReadZeroLimit)
    {
      uRegVal  = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_LIMITS, AUX_READ_ZERO_LIMIT, *(psAUXCtrlConfig->psAUXAttrib->puReadZeroLimit));
    }
    if (NULL != psAUXCtrlConfig->psAUXAttrib->puNackDeferLimit)
    {
      uRegVal  = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_LIMITS, AUX_NACK_DEFER_LIMIT, *(psAUXCtrlConfig->psAUXAttrib->puNackDeferLimit));
    }
    if (NULL != psAUXCtrlConfig->psAUXAttrib->puI2CDeferLimit)
    {
      uLimit = *(psAUXCtrlConfig->psAUXAttrib->puI2CDeferLimit);
      
      uRegVal  = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_LIMITS, I2C_DEFER_LIMIT,      (uLimit & 0xF));
      uRegVal  = HWIO_OUT_FLD(uRegVal, DPTX_0_AUX_LIMITS, I2C_DEFER_LIMIT_MSB,  ((uLimit >> 4) & 0xF));
    }

    out_dword(uOffset + HWIO_DPTX_0_AUX_LIMITS_ADDR, uRegVal);
  }
