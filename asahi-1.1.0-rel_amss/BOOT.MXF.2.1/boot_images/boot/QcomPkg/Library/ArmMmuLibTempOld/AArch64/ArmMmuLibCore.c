...
STATIC BOOLEAN IsBBMSafe (UINT64, UINT64*);
UINT32 gBBM = 0;

#define CHECK_OVERLAP(RgnPtr, ChkAddrPtr) ((RgnPtr & ~(SIZE_2MB - 1)) == (ChkAddrPtr & ~(SIZE_2MB - 1)) ? 1 : 0)

UINT64 *gOldTranslationTable;
#endif
  ...
  )
{
  ...
  case ARM_MEMORY_REGION_ATTRIBUTE_WRITE_BACK_XN:
    if (ArmReadCurrentEL () == AARCH64_EL3 || ArmReadCurrentEL () == AARCH64_EL2)
      return TT_ATTR_INDX_MEMORY_WRITE_BACK|TT_XN_MASK|TT_SH_INNER_SHAREABLE;
    else
  ...
  case ARM_MEMORY_REGION_ATTRIBUTE_WRITE_BACK_RO:
    return TT_ATTR_INDX_MEMORY_WRITE_BACK|TT_SH_INNER_SHAREABLE|TT_AP_RO_RO;

  case ARM_MEMORY_REGION_ATTRIBUTE_WRITE_THROUGH_XN:
    if (ArmReadCurrentEL () == AARCH64_EL3 || ArmReadCurrentEL () == AARCH64_EL2)
      return TT_ATTR_INDX_MEMORY_WRITE_THROUGH|TT_XN_MASK|TT_SH_INNER_SHAREABLE;
    else
      return TT_ATTR_INDX_MEMORY_WRITE_THROUGH|TT_UXN_MASK|TT_PXN_MASK|TT_SH_INNER_SHAREABLE;
  
  case ARM_MEMORY_REGION_ATTRIBUTE_WRITE_BACK_XN_RO:
    if (ArmReadCurrentEL () == AARCH64_EL3 || ArmReadCurrentEL () == AARCH64_EL2)
      return TT_ATTR_INDX_MEMORY_WRITE_BACK|TT_XN_MASK|TT_SH_INNER_SHAREABLE|TT_AP_RO_RO;
    else
      return TT_ATTR_INDX_MEMORY_WRITE_BACK|TT_UXN_MASK|TT_PXN_MASK|TT_SH_INNER_SHAREABLE|TT_AP_RO_RO;
#endif
  ...
  case ARM_MEMORY_REGION_ATTRIBUTE_UNCACHED_UNBUFFERED_XN:
    if (ArmReadCurrentEL () == AARCH64_EL3 || ArmReadCurrentEL () == AARCH64_EL2)
      return TT_ATTR_INDX_MEMORY_NON_CACHEABLE| TT_XN_MASK;
    else
      return TT_ATTR_INDX_MEMORY_NON_CACHEABLE| TT_UXN_MASK | TT_PXN_MASK;
  ...
  case ARM_MEMORY_REGION_ATTRIBUTE_DEVICE_RO:
    if (ArmReadCurrentEL () == AARCH64_EL3 || ArmReadCurrentEL () == AARCH64_EL2)
      return TT_ATTR_INDX_DEVICE_MEMORY|TT_XN_MASK|TT_AP_RO_RO;
    else
      return TT_ATTR_INDX_DEVICE_MEMORY|TT_UXN_MASK|TT_PXN_MASK|TT_AP_RO_RO;
#endif
...
#if defined (QCOM_EDK2_PATCH)
        if (IsBBMSafe(RegionStart, BlockEntry))
        {
...
{
  INT64 FnAddr = (INT64)GetBlockEntryListFromAddress, Addr =0;
  INT64 FnLowerBound =0, FnUpperBound =0;
  
  if (!ArmMmuEnabled())
    return TRUE;

  // Make sure RegionStart does not reside in same
  // 2MB block as the code being executed by comparing with bounds
  // of its block region

  // Note: Updates done to narrow down the range of memory that is truly
  // Unsafe. This reduction is necessary to reduce the amount of time
  // MMU is toggled (turned off-on) since MMU being off has side effects
  // for page faults on another core in multicore operation

  // Stack
  Addr = (INT64)&FnAddr;
  if (CHECK_OVERLAP (RegionStart, Addr ))
    return FALSE;

  // Page Tables
  if (CHECK_OVERLAP (RegionStart, (INT64)BlockEntry )) 
    return FALSE;

  // Set bounds region to 3 contiguous pages containing fn pointer
  Addr = FnAddr & ~(SIZE_4KB - 1);
  FnLowerBound = Addr - SIZE_4KB;
  FnUpperBound = Addr + SIZE_8KB;
  
  // Check Text section
  if (CHECK_OVERLAP (RegionStart, FnLowerBound )) 
    return FALSE;

  if (CHECK_OVERLAP (RegionStart, FnUpperBound )) 
    return FALSE;
  
  // if we reach here there is no overlap
  return TRUE; 
}
#endif
...
#if defined(QCOM_EDK2_PATCH)
      ArmDataSynchronizationBarrier();
      WriteBackDataCacheRange (BlockEntryStart, ((UINT64)BlockEntry - (UINT64)BlockEntryStart));
      ArmInvalidateTlb();
#endif    
  ...
  )
{
  ...
  // Flush D-Cache so that any dirty cache lines would make it to the target
  ...
  )
{
  ...
  // Disable MMU and caches. ArmDisableMmu() also invalidates the TLBs
    ArmDisableMmu ();
    ArmDisableDataCache ();
    //ArmDisableInstructionCache (); Commented out to improve boot kpi
  }
#endif
...
#if defined(QCOM_EDK2_PATCH)
   /* Restoring old TTBR0 address before we modify it. */
  gOldTranslationTable = ArmGetTTBR0BaseAddress();
