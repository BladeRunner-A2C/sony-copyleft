/** @file
  ...
  Copyright (c) 2010-2015,2019,2022-23 Qualcomm Technologies Inc. All rights
  ...
  )
{
  ...
  if ((PmicRtc != NULL) && !EfiAtRuntime()) {
    ...
    /* If RTC is not ticking, return the error and let UEFI framework handle the error */
  ...
  /* TimeZone difference should be 24Hours max, i.e., (-24*60) to (+24*60) */
  ...
  if(Time->TimeZone != EFI_UNSPECIFIED_TIMEZONE) {
    if(Time->TimeZone >= 0) {
      TimeInSeconds += (UINT32)(Time->TimeZone)*60;
    }
    else
    {
      TimeInSeconds -= (UINT32)(-Time->TimeZone)*60;
  ...
  )
{
  ...
  /* Adjust for UTC Time */
  if(TimeZone >= 0) {
    TimeInSeconds -= (UINT32)(TimeZone)*60;
  }
  else
  {
    TimeInSeconds += (UINT32)(-TimeZone)*60;
