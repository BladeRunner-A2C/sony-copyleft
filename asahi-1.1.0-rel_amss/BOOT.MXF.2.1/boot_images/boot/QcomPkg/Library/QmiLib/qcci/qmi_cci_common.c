...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/QmiLib/qcci/qmi_cci_common.c#1 $
...
LIST(qmi_cci_client_type, qcci_client_tbl)[CLIENT_TBL_COUNT];
LIST(qmi_cci_client_type, qcci_client_release_tbl);
...
void qmi_cci_common_init(void)
{
  if(qcci_fw_inited == 0)
  {
    qmi_cci_os_init();            //Define empty function in qmi_cci_target.h 
                                  //if this is not present in any OS target files
 ...
 )
{
  ...
  *client = NULL;
  ...
  clnt = (qmi_cci_client_type *)CALLOC_QCCI(1, sizeof(*clnt));
  if(!clnt)
...
static void qmi_cci_client_unlink(qmi_cci_client_type *clnt)
{
  ...
  if(i)
  {
	...
	/*if some error happens after removing clnt  from the qcci_client_tbl and before it is getting freed,then we don't have any reference to look at the clnt structure. 
	 In order to preserve the reference, we are using this qcci_client_release_tbl.*/
    //adding the clnt in to the qcci_client_release_tbl after it is removed from qcci_client_tbl
    LINK_INIT(clnt->priv.link);
    LIST_ADD(qcci_client_release_tbl,clnt,priv.link);
  }
}
...
static void qmi_cci_client_free(qmi_cci_client_type *clnt)
{
  ...
  //removing the clnt from qcci_client_release_tbl before freeing it.
   LOCK(qcci_tbl_lock);
   LIST_FIND(qcci_client_release_tbl, i, priv.link, i->priv.clid == clnt->priv.clid);
   if(i)
   {
      LIST_REMOVE(qcci_client_release_tbl, clnt,priv.link);
   }
  UNLOCK(qcci_tbl_lock);   
  FREE_QCCI(clnt);
}
 ...
 )
{
  ...
  txn = (qmi_cci_txn_type *)CALLOC_QCCI(1, sizeof(qmi_cci_txn_type));
  if(!txn)
    return QMI_CLIENT_ALLOC_FAILURE;
  ...
  txn->ref_count = 2; /* The txn_list takes a reference.
                         ...
                         from the list.
                         The caller reference will be released
                         When they are done. */
...
)
{
  ...
  if(c_struct && c_struct_len)
  {
    ...
    msg = (unsigned char *)MALLOC_QCCI(max_msg_len + QMI_HEADER_SIZE);
    if(!msg)
      return QMI_CLIENT_ALLOC_FAILURE;
  ...
  else
  {
    /* Empty message */
    out_len = 0;
    msg = (unsigned char *)MALLOC_QCCI(QMI_HEADER_SIZE);
    if(!msg)
      return QMI_CLIENT_ALLOC_FAILURE;
  }
 ...
 )
{
  ...
  *user_handle = INVALID_HANDLE;
  ...
  for(i = 0; i < num_xports; i++)
  {
    ...
    xport = (struct qmi_cci_xport_s *)CALLOC_QCCI(1, sizeof(*xport));
    if(!xport)
    {
      continue;
    }
 ...
 )
{
  ...
  *user_handle = INVALID_HANDLE;
  ...
  while(1)
  {
    ...
    service_array = (qmi_service_info *)MALLOC_QCCI(sizeof(qmi_service_info) * num_services);
    if(!service_array)
    {
      rc = QMI_CLIENT_ALLOC_FAILURE;
 ...
 )
{
  ...
  buf = (unsigned char *)MALLOC_QCCI(req_buf_len + QMI_HEADER_SIZE);
  if(!buf)
  {
    rc = QMI_CLIENT_ALLOC_FAILURE;
    goto bail;
