/** @file RpmbLib.c
  ...
  Copyright (c) 2011-2015,2018,2023 Qualcomm Technologies, Inc. 
  All Rights Reserved. 
...
/*=============================================================================
 ...
 ----------   ---     -----------------------------------------------------------
 2023-04-06   pz      Add support for reading ufs rpmb in RPMBReadData
 2018-09-11   gp      check buffer limit before copying to AppName
 2018-04-24   jt      Fix device ID check for GPT 
 2018-03-21   ai      GPT based VarStore support
...
ProvisionRpmbTestKey (UINT32 *Result)
{
   ...
   if (NULL == Result)
   {
      DEBUG((EFI_D_ERROR, "ProvisionRpmbTestKey: Invalid param!\r\n")); 
   ...
   DEBUG((RPMB_DBG_LVL, "ProvisionRpmbTestKey: Calling TZ\n"));
...
ConfigKeyValCb (UINT8* Section, UINT8* Key, UINT8* Value)
{
   ...
   else if (AsciiStriCmp ((CHAR8*)Key, "PartitionGuid") == 0)
   {
      if (GuidFound == TRUE)
      {
         /* The CurrentPartitionGuid has already been found and processed.
            Finding a new PartitoinGuid implies the start of the next logical partition section.
            Prevent the soft partitions from these logical partitions from being incorrectly added. */
         GuidFound = FALSE;
      }
      if (HexStrToBin (Value, AsciiStrLen((CHAR8*)Value), (UINT32 *) &PartitionGuid, 
                       sizeof(PartitionGuid)))
      {
         /* Convert byte stream to GUID */
   ...
   {
      DEBUG((RPMB_DBG_LVL, "ConfigKeyValCb: GuidNotFound! CurrDevId=%d\n", CurrentDevId));
      return; 
   }
...
ConfigRpmbStorPartitions (UINT32 *NumPartitions, UINT8 *BufferPtr)
{
   ...
   DEBUG((RPMB_DBG_LVL, "ConfigRpmbStorPartitions: RPMBPartitionSize:%u\n", RPMBProtocol->RPMBPartitionSizeInBytes));
   ...
   DEBUG((RPMB_DBG_LVL, "ConfigRpmbStorPartitions: SectionIndex:%d\n", SectionIndex));
   ...
   if (SectionIndex == -1)
   {
      DEBUG((EFI_D_ERROR, "ConfigRpmbStorPartitions: SectionIndex=-1\n"));
   ...
   {
      DEBUG ((EFI_D_ERROR, "ConfigRpmbStorPartitions: DevId invalid:%d\n", CurrentDevId));
      return EFI_INVALID_PARAMETER; 
   }
   ...
   *NumPartitions = PartitionsToAdd; 
   ...
   DEBUG((RPMB_DBG_LVL, "ConfigRpmbStorPartitions: RPMB Soft partitions:\n"));
   for(i=0; i<PartitionsToAdd; i++)
   {
     DEBUG((RPMB_DBG_LVL, "  %a, %d, %d\n",
        PartiAdditionData[i].AppName,
        PartiAdditionData[i].PartitionId,
        PartiAdditionData[i].NumSectors
        ));
   }
...
{ 
   ...
   DEBUG((RPMB_DBG_LVL, "ConfigGptStorPartitions called!\n"));
   
   if ((NULL == PartitionGuid) || (NULL == NumPartitions) || 
       (NULL == BufferPtr) || (NULL == BlkIoMedia))
   {
      DEBUG((EFI_D_ERROR, "GptGptStorPartitions: Invalid params!\n"));
      return EFI_INVALID_PARAMETER;
   } 
   ...
   /* Check the DevId */
   if ((SD_MGR_DEVICE_EMMC_RPMB == CurrentDevId) ||
       (SD_MGR_DEVICE_ID_MAX < CurrentDevId))
   {
   ...
   *NumPartitions = PartitionsToAdd; 
   ...
   DEBUG((RPMB_DBG_LVL, "ConfigGptStorPartitions: GPT Soft partitions:\n"));
   for(i=0; i<PartitionsToAdd; i++)
   {
     DEBUG((RPMB_DBG_LVL, "  %a, %d, %d\n",
        PartiAdditionData[i].AppName,
...
ConfigGptExit: 
   ...
   DEBUG ((RPMB_DBG_LVL, "ConfigGptStorPartitions: Return=%r\n", Status));
   return Status; 
}
                      ...
                      VOID *BlkIoMedia)
{
   ...
   {
      DEBUG ((EFI_D_ERROR, "ConfigStorPartitions: Invalid params!\n"));
      return EFI_INVALID_PARAMETER; 
   }
   ...
   if (FALSE == CallbackEntered)
   {
      DEBUG ((EFI_D_ERROR, "ConfigStorPartitions: Did not enter callback!\n"));
      Status = EFI_UNSUPPORTED; 
      goto ConfigExit; 
   }
...
ConfigExit: 
   CloseParser(Pd); 
   DEBUG ((EFI_D_ERROR, "ConfigStorPartitions: Status:%r\n", Status));
   return Status; 
}
...
InitPartitionConfig(VOID)
{
   ...
   /* Load the partiapp.cfg file */
   Status = ReadFromFV(PARTITION_CONFIG_FILE, (void **) &CfgBuffer, &FileSize);
   if (EFI_SUCCESS != Status)
   {
      DEBUG((EFI_D_ERROR, "Failed to load SecParti.cfg!"
                          "Status = (0x%x)\r\n", Status));
      return Status; 
   }
...
EFI_STATUS
EFIAPI
RPMBCreateReadPacket (UINT32 StartSector, UINT8* PktBuffer, UINT32 HalfBlockCount)
{
   RPMB_DATA_FRAME *RpmbDataFrame = (RPMB_DATA_FRAME *)PktBuffer;
   UINT16 RequestResponse = 0, TempStartSector = 0;
   UINT16 TransferCount = 0;
   ...
   if (boot_from_ufs())
   {  
      if (HalfBlockCount > UINT16_MAX)
      {
         return EFI_INVALID_PARAMETER;
      }

      TransferCount = (UINT16)HalfBlockCount;

      /* Format blockcount of the RPMB Data Frame */
      TransferCount = IS_BIG_ENDIAN() ? TransferCount : REVERSE_WORD(TransferCount);
      CopyMem(RpmbDataFrame->BlockCount, &TransferCount, 
              sizeof(RpmbDataFrame->BlockCount));
   }
   ...
   )
{
   ...
   UINT32 HalfBlockCount =  2 * BlockCount;
   UINT32 i = 0; 
   ...
   if (boot_from_ufs())
   {
      if(HalfBlockCount > RpmbProtocol->ReliableWriteCount)
      {
         return EFI_INVALID_PARAMETER;
      }
   }

   Status = RPMBCreateReadPacket (Lba, ReadPktBuffer, HalfBlockCount);
   if (EFI_SUCCESS != Status)
   {
