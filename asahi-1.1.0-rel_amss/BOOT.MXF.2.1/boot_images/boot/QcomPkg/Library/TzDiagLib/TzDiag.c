...
static EFI_STATUS tz_diag_get_debug_policy(sec_dbg *pDebugPolicy, UINT32 Size)
{
    ...
    do
    {
        if ((pDebugPolicy == NULL) || (Size != sizeof(sec_dbg)) || (p_debug_policy == NULL))
        {
            status = EFI_INVALID_PARAMETER;
            break;
        }
...
static void tz_diag_read_display_encrpted_log(UINT32 LogId)
{
    ...
    do
    {
        ...
        param[0] = (UINT64) (UINT64 *) log_buf;
        param[1] = (UINT32) log_buf_size;
        param[2] = (UINT32) LogId;
        param[3] = (UINT32) 1; //passing dummy 4th parameter since, 4th is parameter is expected by TZ SMC call.
...
#if 0 // Feature Needs to be reviewed for non secure device CR : https://orbit/cr/3012548
        else
        {
            /*
