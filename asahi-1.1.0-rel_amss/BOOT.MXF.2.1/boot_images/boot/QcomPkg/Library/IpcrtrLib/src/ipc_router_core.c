...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/IpcrtrLib/src/ipc_router_core.c#1 $
...
/*===========================================================================
FUNCTION      ipc_router_processor_down
...
SIDE EFFECTS  None
===========================================================================*/
static void ipc_router_processor_down(uint32 processor_id)
{
  ipc_router_search_criteria_type cr;
  ipc_router_search_results_type results;
  ipc_router_client_list_type *client_list;

  IPC_ROUTER_OS_MESSAGE(IPC_ROUTER_MSG_NOTICE,"Processor down, proc:%d\n", processor_id);
  /* remove all clients and servers known over this transport */
  cr.crit = IPC_ROUTER_CRITERIA_PROCESSOR;
  cr.arguments.proc_arg = processor_id;
  ...
  /* remove processor */
  ipc_router_unregister_proc(processor_id);
}
...
)
{
  ...
  switch(cntl_msg->command) {
    ...
    case IPC_ROUTER_CONTROL_PROC_DOWN:
      if( IPC_ROUTER_PROC_DOWN_MAGIC == ipc_router_compute_checksum( cntl_msg, sizeof(ipc_router_control_msg_body) ) )
      {
        ipc_router_processor_down(cntl_msg->arguments.proc_down_arg.processor_id);
      }
      else
      {
        IPC_ROUTER_OS_MESSAGE(IPC_ROUTER_MSG_ERR,"Invalid checksum=0x%08X for PROC_DOWN command\n",
                              cntl_msg->arguments.proc_down_arg.checksum);
      }
      break;
