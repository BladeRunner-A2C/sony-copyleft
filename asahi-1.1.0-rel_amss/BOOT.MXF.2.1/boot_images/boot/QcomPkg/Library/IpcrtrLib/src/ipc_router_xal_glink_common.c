...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/IpcrtrLib/src/ipc_router_xal_glink_common.c#1 $
$DateTime: 2024/04/02 23:00:31 $
$Author: cirrusp4svc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/05/18   mad     Free pending tx packets on remote disconnect
...
EFI_GLINK_PROTOCOL * ipcr_glink_protocol = NULL;
...
)
{
     ...
     NULL == (intent->pkt = ipc_router_packet_new(0,0)))
  {
	...
	if (ipcr_glink_protocol == NULL)
    {
      EFI_STATUS status;
      status = gBS->LocateProtocol(&gEfiGLINKProtocolGuid, NULL, (void**)&ipcr_glink_protocol);
   ...
   ipcr_glink_protocol->GlinkRxDone(port->handle, ptr, FALSE, &ret);
  ...
  if (ipcr_glink_protocol == NULL)
   ...
   ipcr_glink_protocol->GlinkRxDone(port->handle, ptr, TRUE, &ret);
...
)
{
  ...
  ipcr_glink_protocol->GlinkQueueRxIntent(port->handle, intent, size, &ret);

  if(GLINK_STATUS_SUCCESS != ret)
  {
    ipc_router_packet_free(&intent->pkt);
    ipc_router_os_free(intent);
    return IPC_ROUTER_STATUS_FAILURE;
...
)
{
  ...
  ipcr_glink_protocol->GlinkOpen(&config, &port->handle, &err);
...
)
{
  ...
  {
    /* Just do packet free, it internally calls the ipc_free_cb() function 
    ipc_free_cb() function frees the data buffer and intent structure */
    ipc_router_packet_free(&pkt);
  }
  while(NULL != (pkt = ipc_router_packet_queue_get(&port->tx_queue)))
  {
    ipc_router_packet_free(&pkt);
  }
  if(port->tx_leftover)
...
)
{
  ...
  port->ch_state = GLINK_REMOTE_DISCONNECTED;
  ipc_router_os_sem_unlock(&port->lock);
  /* Unlocking before calling into ipc_router_packet_free, as there
   is a possibility of recursive acquire of spinlock in ipc_free_cb
  ...
  glink_err_type err;
  ipcr_glink_protocol->GlinkClose(handle, &err);
  //glink_close(handle);
}
...
)
{
  ...
  do{
      ...
      if (ipcr_glink_protocol == NULL)
		...
		ipcr_glink_protocol->GlinkTxV(port->handle, packet, packet, ipc_router_packet_length(packet), ipc_packet_provider, NULL, GLINK_TX_REQ_INTENT, &err);
...
)
{
  ...
  *out_handle = (void *)port;
  ...
  ipcr_glink_protocol->GlinkRegisterLinkStateCB(&link_id, port,&err);
...
)
{
  ...
  ipcr_glink_protocol->GlinkDeregisterLinkStateCB(port->link_state_handle, &err);
