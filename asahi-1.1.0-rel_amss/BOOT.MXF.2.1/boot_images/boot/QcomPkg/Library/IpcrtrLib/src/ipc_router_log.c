/*===========================================================================
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2012-2014, 2022 Qualcomm Technologies, Inc.  All Rights Reserved.
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/IpcrtrLib/src/ipc_router_log.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
...
/*    do { \
      if(ipcr_ulog_handle) {\
         ULogFront_RealTimePrintf(ipcr_ulog_handle, count, format,  ##args); \
      } \
	} while(0)*/
 ...
 )
{
  ...
  uint32 addr __attribute__((unused)); /* to avoid compiler warning -Wunused-variable 
                                          for some targets */
  ...
  switch(type)
  {
    ...
    case IPC_ROUTER_CONTROL_RESUME_TX:
     ...
     case IPC_ROUTER_CONTROL_PROC_DOWN:
      LOG_EVENT3(dir | (type << 8),
         cntl_msg->arguments.proc_down_arg.processor_id, 
         cntl_msg->arguments.proc_down_arg.checksum, type);
      break;
    default:
      break;
  }
...
)
{
  ...
  /* to avoid compiler warning -Wunused-variable for some targets */
  uint32 src __attribute__((unused)), dst __attribute__((unused));
...
)
{
  ...
  IPCR_ERR_ULOG_MSG(1, format, arg);
}
