...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/IpcrtrLib/src/ipc_router_xal_glink_os.c#1 $
...
===========================================================================*/
static int ipc_router_xal_glink_worker_thread(void *priv)
{
...
)
{
  ...
  if(port->params.stack_size > 0)
  {
	...
	*/
	...
	os_priv->signal = EventInit (0, 0,1);
    ...
    //                   (void *)port);
	os_priv->tid = ThreadCreate (os_priv->name, &ipc_router_xal_glink_worker_thread, (VOID *)port, port->params.priority, port->params.stack_size);
	ThreadResume (os_priv->tid);
  }
  else
  {
    port->os_priv->worker_started = TRUE;
...
)
{
  ...
  if(port->params.stack_size > 0)
  {
	...
	ipc_router_os_sem_lock(&port->lock);
	port->os_priv->eventmask |= EVENT_MASK(event);
	ipc_router_os_sem_unlock(&port->lock);
	EventSignal(port->os_priv->signal,0);
  }
  else
  {
