/*=============================================================================
 ...
 Copyright (c)  2016,2018,2022 Qualcomm Technologies, Inc. All Rights Reserved.
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/SmemLib/src/smem_target.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/08/22   dg      Avoid the TCSR Mutex Register access of unused spinlocks (0, 1, 4, 5, 6, & 7)
...
{
  if (smem_targ_mutex_reg_addrs[lock] == NULL)
  {
    SMEM_OS_LOG_ERR(1, "Trying to acquire invalid spinlock: %d", lock);
    return;
  }
...
void smem_targ_spin_unlock(uint8 lock)
{
  ...
  if (smem_targ_mutex_reg_addrs[lock] == NULL)
  {
    SMEM_OS_LOG_ERR(1, "Trying to unlock invalid spinlock: %d", lock);
    return;
  }
...
void smem_targ_init(void)
{
  ...
  {
	/* Assume register offset value zero as unused spinlock */
	if (prop.Val.pdwVal[idx] != 0)
	{
	...
	{
		smem_targ_mutex_reg_addrs[idx] = NULL;
	}
  }
