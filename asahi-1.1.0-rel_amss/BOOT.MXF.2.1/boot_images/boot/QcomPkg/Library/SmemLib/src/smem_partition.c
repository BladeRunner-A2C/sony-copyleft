...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/SmemLib/src/smem_partition.c#1 $
...
)
{
  ...
  if (canary != SMEM_ALLOC_HDR_CANARY)
  {
    SMEM_OS_LOG_ERR(2, "Invalid SMEM canary 0x%X at location 0x%X",
        canary, (uint32)(uintptr_t)(*item_hdr));
    return FALSE;
  }
...
)
{
  ...
  /* Point to first item */
  item_hdr = (smem_partition_allocation_header_type *)
                ((uint32)(uintptr_t)info->header + sizeof(smem_partition_header_type));

  /* Set limit of scan */
  limit_addr = (uint32)(uintptr_t)info->header + info->offset_free_uncached;

  /* Scan from uncached side of heap */
  while ((uint32)(uintptr_t)item_hdr < limit_addr)
  {
   ...
   * Point to first item */
  item_hdr = (smem_partition_allocation_header_type *)
                ((uint32)(uintptr_t)info->header + info->size - padding_header -
                sizeof(smem_partition_allocation_header_type));

  /* Set limit of scan */
  limit_addr = (uint32)(uintptr_t)info->header + info->offset_free_cached;
...
void smem_part_init(void)
{
  ...
  for (i = 0; i < toc->num_entries; i++)
  {
    ...
    if (size != 0 && remote_host != SMEM_INVALID_HOST)
    {
      ...
      SMEM_OS_LOG_ASSERT(((unsigned int)(uintptr_t)hdr & SMEM_PAGE_ALIGN_MASK) == 0);
