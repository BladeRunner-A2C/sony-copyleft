/*=============================================================================
 ...
 Copyright (c)  2016 , 2021-2023 Qualcomm Technologies, Inc. All Rights Reserved.
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/SmemLib/src/smem_xbl_loader.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/03/23   ck      Moved DALSys.h to top of include list
14/11/22   sym     remove void* smem_dt_blob variable
...
void smem_boot_targ_info_set(void)
{
   ...
   * total size. */
  uintptr_t smem_targ_info_addr_readback = 0;
  ...
  *smem_targ_wonce_reg_addrs[1] = (uint64)smem_targ_info_addr >> 32;
  ...
  /* Readback and verify the value written to WONCE register */
  smem_targ_info_addr_readback = (((uint64)*smem_targ_wonce_reg_addrs[1]) << 32) | (((uint64)*smem_targ_wonce_reg_addrs[0]) & 0xFFFFFFFF);
  if(smem_targ_info_addr_readback != smem_targ_info_addr)
  {
    SMEM_OS_LOG_FATAL(2, "Unable to write to TCSR Wonce register, value written=0x%X, value read=0x%X.", 
	   smem_targ_info_addr, smem_targ_info_addr_readback);
  }
}

/*===========================================================================
  FUNCTION  smem_quickboot_targ_info_set
===========================================================================*/
/**
  Saves the SMEM base physical address and size in a structure at a set 
  address.  This must be called from the beginning of smem_boot_init().
...
*/
/*=========================================================================*/
void smem_quickboot_targ_info_set(void)
{
  uintptr_t smem_targ_info_addr_readback = 0;
  uint8 *smem_toc_addr =  smem_boot_targ_smem_base_addr + 
                                smem_boot_targ_smem_size - SMEM_TOC_SIZE;
  smem_toc_type *toc = (smem_toc_type *)smem_os_map_base_va((void*)(uintptr_t)smem_toc_addr, SMEM_TOC_SIZE);
  uint32 toc_size = sizeof(smem_toc_type) + 
      (sizeof(smem_toc_entry_type) * toc->num_entries);
  /* This should always be true, but ensure that the address for the smem
   * target info is 64-bit aligned, so any architecture will read it the same
   * without needing any assumptions about alignment/packing. */
  uint32 toc_padded_size = (toc_size + 0x7) & (~(0x7));
  smem_targ_info_addr = (uintptr_t)smem_toc_addr + toc_padded_size;
  smem_targ_info_type *smem_targ_info_ptr = (smem_targ_info_type *)((uint8*)toc + toc_padded_size);
  ...
  /* Fill out the SMEM target info structure. */
  if (smem_targ_info_ptr->identifier != SMEM_TARG_INFO_IDENTIFIER)
  {
    SMEM_OS_LOG_FATAL(1, "Invalid SMEM targ info at 0x%X.", smem_targ_info_addr);
  }
...
void smem_dt_parse(void)
{
   ...
   int err = FDT_ERR_QC_NOERROR;
   uint32_t val,idx;

   err = fdt_get_node_handle(&smem_node_hdl, NULL, "/sw/smem");
   if(FDT_ERR_QC_NOERROR != err )
   {
    return;
   }
   ...
   if (FDT_ERR_QC_NOERROR != err || !smem_boot_targ_cfg_smem_version || !smem_boot_targ_cfg_toc_version || !val)
   {
    return;
   }
  ...
  err = fdt_get_node_handle(&smem_part_main_node_hdl, NULL, "/sw/smem/smem-partitions");
  if(FDT_ERR_QC_NOERROR != err)
  {
    return;
  }
  err = fdt_get_count_of_subnodes(&smem_part_main_node_hdl, &val);
  if(FDT_ERR_QC_NOERROR != err || val == 0 || val >= 100)
  {
	return;
  }
  ...
  */
  ...
  {
	fdt_get_prop_values_of_node(&cache[idx], "wwwwwi", (void*)(&smem_boot_targ_cfg_toc_tbl[idx]), sizeof(smem_toc_config_type));
	if (FDT_ERR_QC_NOERROR != err) {
		return ;
	}
