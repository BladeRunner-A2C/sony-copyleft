/*=============================================================================
 ...
 Copyright (c)  2011-2013, 2016-2017, 2022-2023 Qualcomm Technologies, Inc. All Rights Reserved.
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/SmemLib/src/smem_boot.c#1 $
...
=============================================================================*/
#ifdef SMEM_FEATURE_ADD_AC_RULES
void smem_init_partitions(void);
#endif

uint32 smem_debug_ramdump_mode_sdi_pass_2 = 0;
/*=============================================================================
...
void smem_boot_init(void)
{
  ...
  if (smem_debug_ramdump_mode_sdi_pass_2 != 1)
  {
  #ifdef SMEM_FEATURE_ADD_AC_RULES
  smem_init_partitions();
  #endif
  }
  else 
  {
    smem_debug_ramdump_mode_sdi_pass_2 = 0;
  }
...
void smem_quickboot_init(void)
{
  ...
  /* Save the SMEM target information.*/
  smem_quickboot_targ_info_set();
...
{
  /* This function is called if SDI Pass 1 fails and full DDR init needs to be done in SDI Pass 2.
     In this case the AC rules are not applied since ramdump is collected after full DDR init */
  smem_debug_ramdump_mode_sdi_pass_2 = 1;
