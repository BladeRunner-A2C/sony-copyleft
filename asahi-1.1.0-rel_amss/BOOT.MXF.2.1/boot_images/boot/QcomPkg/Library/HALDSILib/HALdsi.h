...
/*=============================================================================
  ...
  Copyright (c) 2010-2022, 2024 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
/* Flag to enable DSI command mode NULL packet insertion */
#define HAL_DSI_FLAG_ENABLE_NULL_PKT_INSERTION          0x02

/* Flag to enable DSI non embedded mode */
#define HAL_DSI_FLAG_ENABLE_NONEMBEDDED                 0x04

/* Maximum bytes of word count for automatic NULL packet insertion
*/
#define HAL_DSI_MAX_NULL_PKT_INSERTION_WC_IN_BYTE       64

/* Default byte of word count for automatic NULL packet insertion, 
   1 means no payload in NULL packet to allow faster switch to video transfrer
*/
#define HAL_DSI_DEFAULT_NULL_PKT_INSERTION_WC_IN_BYTE   1

/* Define the maximum number of the supported dynamic refresh rate
*/
#define MAX_NUM_DYNAMIC_REFRESH                         16

/* DSI data long packet min size */
#define HAL_DSI_LONG_PKT_SIZE                           6

/* DSI CHECK SUM SIZE */
#define HAL_DSI_CHECKSUM_SIZE                           2

/* DSI Read buffer use RDFIFO from DSI6Gv2.7.0, otherwise use RDBK buffer */
#ifdef HWIO_DSI_0_RDFIFO_DATA0_ADDR
#define DSI_RDFIFO_BASE_OFFSET  HWIO_DSI_0_RDFIFO_DATA0_ADDR
#define HAL_DSI_ONE_RDFIFO_BYTE (HWIO_DSI_0_RDFIFO_DATA1_ADDR - HWIO_DSI_0_RDFIFO_DATA0_ADDR)
#define HAL_DSI_MAX_RDFIFO_BYTE (HWIO_DSI_0_RDFIFO_DATA15_ADDR - HWIO_DSI_0_RDFIFO_DATA0_ADDR + HAL_DSI_ONE_RDFIFO_BYTE)
#else
#define DSI_RDFIFO_BASE_OFFSET  HWIO_DSI_0_RDBK_DATA0_ADDR
#define HAL_DSI_ONE_RDFIFO_BYTE (HWIO_DSI_0_RDBK_DATA1_ADDR - HWIO_DSI_0_RDBK_DATA0_ADDR)
#define HAL_DSI_MAX_RDFIFO_BYTE (HWIO_DSI_0_RDBK_DATA3_ADDR - HWIO_DSI_0_RDBK_DATA0_ADDR + HAL_DSI_ONE_RDFIFO_BYTE)
#endif

/* DSI support max packet size */
#if (HAL_DSI_MAX_RDFIFO_BYTE > (HAL_DSI_EMBEDDED_HEADER_SIZE + HAL_DSI_CHECKSUM_SIZE))
#define HAL_DSI_MAX_PACKETSIZE (HAL_DSI_MAX_RDFIFO_BYTE - (HAL_DSI_EMBEDDED_HEADER_SIZE + HAL_DSI_CHECKSUM_SIZE))
#endif
...
typedef enum
{
  DSI_LaneID_0           = 0,               /**< Data Lane 0               */
  DSI_LaneID_1,                             /**< Data Lane 1               */
  DSI_LaneID_2,                             /**< Data Lane 2               */
  DSI_LaneID_3,                             /**< Data Lane 3               */
  DSI_LaneID_CLK,                           /**< Clock Lane                */
  DSI_LaneID_CLKSL,                         /**< Clock Lane for split link */
  DSI_LaneID_Max,
...
typedef enum
{
  ...
  DSI_COLOR_RGB_101010_30BPP,
  DSI_COLOR_RBG_101010_30BPP,
  DSI_COLOR_BGR_101010_30BPP,
  DSI_COLOR_BRG_101010_30BPP,
  DSI_COLOR_GRB_101010_30BPP,
  DSI_COLOR_GBR_101010_30BPP,
 ...
 * \b DSI_SSCConfigType
 *
 * DSI SSC Control configuration
 */
typedef enum
{
  DSI_SSC_DISABLED              = 0x00000000,        /**< SSC disabled */
  DSI_SSC_ENABLED_DOWNSPREAD,                        /**< SSC enabled with downspread */
  DSI_SSC_ENABLED_CENTERSPREAD,                      /**< SSC enabled with centerspread */
  DSI_SSC_CONFIG_MAX
} DSI_SSCConfigType;

/*!
 * \b HAL_DSI_PhyTimingMargin
 *
 * Defines the DSI PHY timing margin.
 */
typedef enum
{
  HAL_DSI_PHY_TIMING_REDUCED_MARGIN    = 0x00,       /**< Reduced Margin */
  HAL_DSI_PHY_TIMING_REGULAR_MARGIN,                 /**< Regular Margin */
  HAL_DSI_PHY_TIMING_MAX,
  HAL_DSI_PHY_TIMING_FORCE_32BIT      = 0x7FFFFFFF
} HAL_DSI_PhyTimingMargin;

/* DSI_SplitLinkConfigType
 *
 * Enumeration of split link configuration.
 */
typedef enum
{
  DSI_SPLIT_LINK_CONFIG_DISABLED   = 0,               /* Split link is disabled                                    */
  DSI_SPLIT_LINK_CONFIG_ENABLED_LR = HAL_DSI_BIT(0),  /* Split link is enabled with left right sublink not swapped */
  DSI_SPLIT_LINK_CONFIG_ENABLED_RL = HAL_DSI_BIT(1),  /* Split link is enabled with left right sublink swapped     */
  DSI_SPLIT_LINK_CONFIG_MAX
} DSI_SplitLinkConfigType;

/*!
* \b DSI_SplitLinkSelectType
*
* Defines the selection of sublink in split link.
*/
typedef enum
{
  DSI_SPLIT_LINK_SEL_NONE          = 0x00,                                                        /**< Select none of sublink */
  DSI_SPLIT_LINK_SEL_SUBLINK_0     = HAL_DSI_BIT(0),                                              /**< Select sublink 0 only  */
  DSI_SPLIT_LINK_SEL_SUBLINK_1     = HAL_DSI_BIT(1),                                              /**< Select sublink 1 only  */
  DSI_SPLIT_LINK_SEL_SUBLINK_ALL   = DSI_SPLIT_LINK_SEL_SUBLINK_0| DSI_SPLIT_LINK_SEL_SUBLINK_1,  /**< Select all sublinks    */
  DSI_SPLIT_LINK_SEL_FORCE_32BIT   = 0x7FFFFFFF
} DSI_SplitLinkSelectType;
...
typedef struct
{
  uint32                        uDscStreamId;         /**< Stream Id */
  uint32                        uDscBytesInSlice;     /**< Bytes in slice */
  uint32                        uDscPktPerLine;       /**< Packets per line */
  uint32                        uDscEolByteNum;       /**< EOL byte num */
  bool32                        uDscEnable;           /**< Enable DSC */
}DSI_DscCmdModeConfigType;

/*!
 * \b DSI_VdcmCmdModeConfigType
 *
 *  Controller config for VDCM CMD Mode.
 */
typedef struct
{
  uint32                        uVdcmStreamId;         /**< Stream Id */
  uint32                        uVdcmBytesInSlice;     /**< Bytes in slice */
  uint32                        uVdcmPktPerLine;       /**< Packets per line */
  uint32                        uVdcmEolByteNum;       /**< EOL byte num */
  bool32                        uVdcmEnable;           /**< Enable VDCM */
}DSI_VdcmCmdModeConfigType;
...
typedef struct
{
  ...
  uint32                        uDscPktPerLine;       /**< Packets per line */
  uint32                        uDscPclkPerLine;      /**< Compressed width */
  uint32                        uDscEolByteNum;       /**< EOL byte num */
  bool32                        uDscEnable;           /**< Enable DSC */
}DSI_DscVideoModeConfigType;

/*!
 * \b DSI_VdcmVideoModeConfigType
 *
 *  Controller config for VDCM Video Mode.
 */
typedef struct
{
  uint32                        uVdcmStreamId;         /**< Stream Id */
  uint32                        uVdcmBytesPerPkt;      /**< Bytes per packet */
  uint32                        uVdcmPktPerLine;       /**< Packets per line */
  uint32                        uVdcmEolByteNum;       /**< EOL byte num */
  bool32                        uVdcmEnable;           /**< Enable VDCM */
}DSI_VdcmVideoModeConfigType;
...
/*!
 * \b DSI_VdcmCompressionInfo
 *
 *  Controller config for VDCM.
 */
typedef struct
{
    DSI_VdcmVideoModeConfigType  sVdcmVidCntrlrConfig; /**< Video mode vdcm info */
    DSI_VdcmCmdModeConfigType    sVdcmCmdCntrlrConfig; /**< CMD mode vdcm info   */
}DSI_VdcmCompressionInfo;


/* !
 * use pragma pack(1) to make the structure byte aligned,
 * this structure is interpreted by the kernel mode driver.
 */
#pragma pack(push, 1)
 ...
 *
 * structure for storing dynamic refresh calibration result
 * This structure will be put to the last 4K of framebuffer,
 * which will be read by kernel from that position.
 *
 */
typedef struct
{
...
typedef struct
{
  ...
  DSI_SSCConfigType             eSSCConfig;           /**< DSI SSC configuration, including enable/disable ssc, and ssc spread mode selection */
  uint32                        uHeightTotal;         /**< MDP total height */
  uint32                        uWidthTotal;          /**< MDP total width */
  uint32                        uWordCount;           /**< MDP word count */
  bool32                        bTwoPixelsPerPclk;    /**< Two pixels per pclk from mdp to dsi */
  HAL_DSI_PhyTimingMargin       ePHYTimingMargin;     /**< DSI PHY Timing Margin selection map to HAL_DSI_PhyTimingMargin */
}DSI_DisplayInfoType;
...
typedef struct
{
  ...
  DSI_VdcmVideoModeConfigType   sVdcmVideoConfig; /**< VDCM related configurations */
...
typedef struct
{
  ...
  DSI_VdcmCmdModeConfigType     sVdcmCmdConfig;             /**< VDCM related configurations */
...
typedef struct
{
  ...
  DSI_Datatype      uCmdDataType;     /**< Packet Data Type                           */
  uint32            uDmaScheduleLine; /**< DMA Scheduling Line Number                 */
...
/*!
* \Struct HAL_DSI_SplitLinkCfgType
*
* Split link configuration 
*/
typedef struct _HAL_DSI_SplitLinkCfgType
{
  DSI_SplitLinkConfigType       eSplitLinkConfig;       /**< Split link configuration                                     */
  DSI_Channel_IDType            eChannelId;             /* Video or command mode                                          */
  DSI_SplitLinkSelectType       eDMADataSel;            /* Select which sublink(s) the DMA command should be sent to      */
  DSI_SplitLinkSelectType       eBTASel;                /* Select which sublink(s) the BTA/Read command should be sent to */
} HAL_DSI_SplitLinkCfgType;
...
/****************************************************************************
*
** FUNCTION: HAL_DSI_Ctrl_Phy_Reset()
*/
/*!
* \brief
*   Use the DSI hardware reset mechanism to reset all DSI hardware blocks
*   with any available hardware reset mechanism present. It will reset
*   PHY hardware for all PHY PLL, LANES and REGULATOR (for DSI CORE 1 only).
*   A reset pulse is issued by writing 1 followed by a 0 to this bit.
*
* \param [in]  eDeviceID  - DSI core device ID.
*
* \retval void
****************************************************************************/
void HAL_DSI_Ctrl_Phy_Reset(DSI_Device_IDType eDeviceId);
...
/****************************************************************************
*
** FUNCTION: HAL_DSI_IsCPhyModeEnabled()
*/
/*!
* \brief
...
/****************************************************************************
*
** FUNCTION: HAL_DSI_VideoVdcmConfig()
*/
/*!
* \brief
*   This API configures the DSI command mode engine.
*
* \param [in] eDeviceID         - DSI Core Device ID 
* \param [in] psVideoDscConfig  - pointer to the vdcm video mode configuration.
*
* \retval NONE
*
****************************************************************************/
void HAL_DSI_VideoVdcmConfig(
    DSI_Device_IDType                      eDeviceID,
    DSI_VdcmVideoModeConfigType*           psVideoVdcmConfig
    );
...
*
* \param [in] eDeviceID       - DSI Core Device ID 
* \param [in] psCMDDscConfig  - pointer to the vdcm cmd mode configuration.
*
* \retval NONE
*
****************************************************************************/
void HAL_DSI_CMDVdcmConfig(
    DSI_Device_IDType                      eDeviceID,
...
Description:
  Clear read back data and counter by writing a 1 followed by a 0 to this bit.

****************************************************************************/
void HAL_DSI_Clear_RDBK(DSI_Device_IDType       eDeviceID);
