...
/* Sets DSI priority level.
 */
static void HAL_DSI_SetDsiPriorityLevel(DSI_Device_IDType eDeviceID,
                                        uint32            uLevel);
...
void HAL_DSI_Init(DSI_Device_IDType  eDeviceID)
{
   ...
   */
  HAL_DSI_Ctrl_Phy_Reset(eDeviceID);
  ...
  /* Set Dsi Priority level to keep QoS settings consistent and ensure LUTDMA/DSI performance robustness, Internal: PL7 */
  HAL_DSI_SetDsiPriorityLevel(eDeviceID, 7);
}
...
void HAL_DSI_Ctrl_Phy_Reset(DSI_Device_IDType eDeviceId)
{
  ...
  out_dword(uOffset + HWIO_DSI_0_PHY_SW_RESET_ADDR, 0x01);
  DSI_OSAL_SleepUs(10);
  out_dword(uOffset + HWIO_DSI_0_PHY_SW_RESET_ADDR, 0x00);
...
void HAL_DSI_Reset(DSI_Device_IDType        eDeviceID)
{
  ...
  uint32    uRegVal          = in_dword(uOffset + HWIO_DSI_0_SOFT_RESET_ADDR);

  /*Reset DSI HW */
  uRegVal = HWIO_OUT_FLD(uRegVal, DSI_0_SOFT_RESET, DSI_SOFT_RESET, 1);
  out_dword(uOffset + HWIO_DSI_0_SOFT_RESET_ADDR, uRegVal);

  /* Clear Reset Bit */
  uRegVal = HWIO_OUT_FLD(uRegVal, DSI_0_SOFT_RESET, DSI_SOFT_RESET, 0);
  out_dword(uOffset + HWIO_DSI_0_SOFT_RESET_ADDR, uRegVal);
                        ...
                        HAL_DSI_WritePacketType  *psWritePacket)
{
  ...
  else
  {
    ...
    uint32  uCmdDmaFifoCtrl = 0;
    uintPtr uOffset         = HAL_DSI_GetRegBaseOffset(eDeviceID);

    if (HAL_DSI_FLAG_ENABLE_NONEMBEDDED & psWritePacket->uFlags)
    {
      // Non embedded mode
      uCmdDmaCtrlReg  = HWIO_OUT_FLD(uCmdDmaCtrlReg, DSI_0_COMMAND_MODE_DMA_CTRL, EMBEDDED_MODE,    HAL_DSI_CMDMODE_DMA_NON_EMBEDED);
      uCmdDmaCtrlReg  = HWIO_OUT_FLD(uCmdDmaCtrlReg, DSI_0_COMMAND_MODE_DMA_CTRL, BROADCAST_EN,     0);
      uCmdDmaCtrlReg  = HWIO_OUT_FLD(uCmdDmaCtrlReg, DSI_0_COMMAND_MODE_DMA_CTRL, BROADCAST_MASTER, 0);
      uCmdDmaCtrlReg  = HWIO_OUT_FLD(uCmdDmaCtrlReg, DSI_0_COMMAND_MODE_DMA_CTRL, POWER_MODE,       (TRUE == psWritePacket->bHighSpeedMode? 0:1));
      uCmdDmaCtrlReg  = HWIO_OUT_FLD(uCmdDmaCtrlReg, DSI_0_COMMAND_MODE_DMA_CTRL, PACKET_TYPE,      1);
      uCmdDmaCtrlReg  = HWIO_OUT_FLD(uCmdDmaCtrlReg, DSI_0_COMMAND_MODE_DMA_CTRL, WC_SEL,           1);
      uCmdDmaCtrlReg  = HWIO_OUT_FLD(uCmdDmaCtrlReg, DSI_0_COMMAND_MODE_DMA_CTRL, DT,                psWritePacket->uCmdDataType);
      out_dword(uOffset + HWIO_DSI_0_COMMAND_MODE_DMA_CTRL_ADDR, uCmdDmaCtrlReg);

      uCmdDmaFifoCtrl = HWIO_OUT_FLD(uCmdDmaFifoCtrl, DSI_0_DMA_FIFO_CTRL, WRITE_WATERMARK_DISABLE, 1);
      uCmdDmaFifoCtrl = HWIO_OUT_FLD(uCmdDmaFifoCtrl, DSI_0_DMA_FIFO_CTRL, READ_WATERMARK_DISABLE,  1);
      uCmdDmaFifoCtrl = HWIO_OUT_FLD(uCmdDmaFifoCtrl, DSI_0_DMA_FIFO_CTRL, READ_WATERMARK,          3);
      uCmdDmaFifoCtrl = HWIO_OUT_FLD(uCmdDmaFifoCtrl, DSI_0_DMA_FIFO_CTRL, WRITE_WATERMARK,         3);
      out_dword(uOffset + HWIO_DSI_0_DMA_FIFO_CTRL_ADDR, uCmdDmaFifoCtrl);
    }
    else
    {
      // Embedded mode
    ...
    if (0 == (psWritePacket->uFlags & HAL_DSI_FLAG_DATA_FETCH_VIA_FIFO))
    {
      ...
      uRegVal = in_dword(uOffset + HWIO_DSI_0_DMA_SCHEDULE_CTRL_ADDR);

      if (0 != psWritePacket->uDmaScheduleLine)
      {
        uRegVal = HWIO_OUT_FLD(uRegVal, DSI_0_DMA_SCHEDULE_CTRL, DMA_SCHEDULE_EN, 1);
        uRegVal = HWIO_OUT_FLD(uRegVal, DSI_0_DMA_SCHEDULE_CTRL, DMA_SCHEDULE_LINE, psWritePacket->uDmaScheduleLine);
      }
      else
      {
        uRegVal = HWIO_OUT_FLD(uRegVal, DSI_0_DMA_SCHEDULE_CTRL, DMA_SCHEDULE_EN, 0);
      }
      out_dword(uOffset + HWIO_DSI_0_DMA_SCHEDULE_CTRL_ADDR, uRegVal);
...
{
  uint8   uReadDataReg[HAL_DSI_MAX_RDFIFO_BYTE];
  uint32  uErrorStatus;
  uint32  uStartPos                     = 1; 
  uint32  uEndPos                       = 0;
  uint32  uWC                           = 0;
  ...
  uint32  uRdbkCount                    = 0;
  uint32  uMaxReturnBytes               = HAL_DSI_MAX_RDFIFO_BYTE;

  if (NULL == psReadPacket->puReadPacket)
  {
    eRetStatus = HAL_DSI_FAILED_EXPECTED_NON_NULL_PTR;
  }
  else if (0 == psReadPacket->uReadPacketBufferSize)
  {
  ...
  else
  {
    ...
    {
      uint32 uIndex;
      uint32 uMaxIndex = (uMaxReturnBytes / sizeof(uint32));

      /*
       * The read fifo register aperture is different between generations.
       * This loops through the available register aperture size.
       */
      for (uIndex = 0; uIndex < uMaxIndex; uIndex++)
      {
        /* Read the fifo from last to first, Index is uMaxIndex -1 -> ... -> 1 -> 0 */
        uint32 uRegVal = in_dword(uOffset + DSI_RDFIFO_BASE_OFFSET + ((uMaxIndex - 1) - uIndex) * sizeof(uint32));
        SwapEndian(uRegVal, *((uint32 *)&uReadDataReg[uIndex * sizeof(uint32)]));
      }

      /* The position index start from zero, need to subtract one */
      uEndPos = uMaxReturnBytes - 1;

      uRdbkCount = in_dword(uOffset + HWIO_DSI_0_RDBK_DATA_CTRL_ADDR);
      uRdbkCount = ((uRdbkCount >> HWIO_DSI_0_RDBK_DATA_CTRL_RDBK_DATA_COUNT_SHFT) & 0xff);

      /* initial packet start position */
      uStartPos = uEndPos - HAL_DSI_EMBEDDED_HEADER_SIZE + 1;

      /* confirm a valid short packet */
      if ((uRdbkCount > HAL_DSI_LONG_PKT_SIZE) ||
          (FALSE == HAL_DSI_IsShortPacket(uReadDataReg, &uStartPos, &uEndPos)))
      {
          ...
          (uEndPos >= uStartPos))
      {
        ...
        for (i=0; i < psReadPacket->uRetPacketByte; i++)
        {
          ...
          if ((i < psReadPacket->uReadPacketBufferSize) &&
              (j < uMaxReturnBytes))
          {
...
void HAL_DSI_Clear_RDBK(DSI_Device_IDType       eDeviceID)
{
  ...
  out_dword(uOffset + HWIO_DSI_0_RDBK_DATA_CTRL_ADDR , 0x1);
  out_dword(uOffset + HWIO_DSI_0_RDBK_DATA_CTRL_ADDR , 0x0);
}
...
/****************************************************************************
*
** FUNCTION: HAL_DSI_SetDsiPriorityLevel()
*/
/*!
* \brief
*    Returns the DSI controller version information.
*
* \param [in] eDeviceID      - DSI Core HW Id.
* \param [in] uLevel         - Priority Level.
*
* \retval void
*
****************************************************************************/
...
{
  uintPtr uOffset         = HAL_DSI_GetRegBaseOffset(eDeviceID);
  uint32  uVbifRegVal     = 0;

  uVbifRegVal = in_dword(uOffset + HWIO_DSI_0_VBIF_CTRL_ADDR);
  uVbifRegVal = HWIO_OUT_FLD(uVbifRegVal, DSI_0_VBIF_CTRL, PRIORITY, uLevel);
  out_dword(uOffset + HWIO_DSI_0_VBIF_CTRL_ADDR, uVbifRegVal);
}
