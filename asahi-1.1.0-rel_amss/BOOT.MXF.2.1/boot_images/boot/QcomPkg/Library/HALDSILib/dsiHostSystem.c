                                         ...
                                         DSI_DisplayConfigType  *psDisplayConfig)
{
    ...
    sDsiPhyConfig.uConfigFlags      |= (DSI_SPLIT_LINK_CONFIG_DISABLED != psSharedData->sDevConfig.eSplitLinkConfig) ? HAL_DSI_PHY_CONFIGFLAG_SPLITLINK : 0;
...
//-------------------------------------------------------------------------------------------------
//  dsiSystem_GetProperty
//
//  @brief
//      Get DSI system property
//
//  @params
//      [IN] hDevice
//          DSI device handle
//      [IN] pDSIVersionInfo
//          DSI controller version
...
//-------------------------------------------------------------------------------------------------
//
DSI_StatusType dsiSystem_GetDSIVersion(DSI_HandleType               hDevice,
                                       HAL_DSI_VersionType         *pDSIVersionInfo)
{
  DSI_StatusType     eStatus            = DSI_STATUS_SUCCESS;
  ...
  {
    DSI_OSAL_MemZero(pDSIVersionInfo, sizeof(HAL_DSI_VersionType));

    if (EFI_PLATFORMINFO_TYPE_RUMI != psMDPHwPrivateInfo->eEFIPlatformInfo)
    {
      HAL_DSI_ReadCoreVersionInfo(pDSIVersionInfo);
    }
  }
