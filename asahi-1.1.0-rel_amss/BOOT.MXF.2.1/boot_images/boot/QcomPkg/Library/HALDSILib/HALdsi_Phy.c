...
/* Dsi Phy FunctionTable Definition */
extern HAL_DSI_PhyFunctionTable HAL_DSI_FunctionTable(2, 6, 0);
extern HAL_DSI_PhyFunctionTable HAL_DSI_FunctionTable(2, 7, 0);
extern HAL_DSI_PhyFunctionTable HAL_DSI_FunctionTable(2, 8, 0);
 ...
 *
 * \param [in]   sDSISystemInfo   - DSI system info
 *
 * \retval None
 *
 ****************************************************************************/
...
{
  if((NULL == pDSIVersionInfo) || 
     (0x00 == pDSIVersionInfo->uMajorVersion))
  {
    /* No DSI PHY present */
    DSI_OSAL_MemZero(&gDsiPhyFxnTbl, sizeof(gDsiPhyFxnTbl));
  }
  else if ((0x02 == pDSIVersionInfo->uMajorVersion) &&
           (0x07 == pDSIVersionInfo->uMinorVersion))
  {
    /* Hamoa 2.7.0 */
    DSI_OSAL_MemCpy(&gDsiPhyFxnTbl, &HAL_DSI_FunctionTable(2, 7, 0), sizeof(HAL_DSI_PhyFunctionTable));
  }
  else if ((0x02 == pDSIVersionInfo->uMajorVersion) &&
           (0x08 == pDSIVersionInfo->uMinorVersion))
  {
    /* Lanai 2.8.0 */
    DSI_OSAL_MemCpy(&gDsiPhyFxnTbl, &HAL_DSI_FunctionTable(2, 8, 0), sizeof(HAL_DSI_PhyFunctionTable));
  }
  else
  {
    // Clear the table for unsupported targets
    DSI_OSAL_MemZero(&gDsiPhyFxnTbl, sizeof(gDsiPhyFxnTbl));
    DSI_Log_Message(DSI_LOGLEVEL_ERROR, "HAL_DSI_PhyFxnsInit, Wrong version, maj=%d minor=%d\n", pDSIVersionInfo->uMajorVersion, pDSIVersionInfo->uMinorVersion);
  }
}
