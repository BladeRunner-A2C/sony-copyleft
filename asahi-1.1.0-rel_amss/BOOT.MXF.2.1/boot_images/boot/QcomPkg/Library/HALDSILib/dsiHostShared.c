/*=============================================================================
  ...
  Copyright (c) 2010-2024 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
#define DSI_MAX_REFRESH_RATE       0x900000       //144fps
...
DSI_StatusType dsiShared_InitDsiDriver(DSI_DeviceConfigType*   psDeviceConfig)
{
  ...
  else
  {
    ...
    if (TRUE == bOnetimeConfig)
    {
      ...
      DSI_OSAL_MemZero(&sDSIVersionInfo, sizeof(HAL_DSI_VersionType));

      if (DSI_STATUS_SUCCESS == dsiSystem_GetDSIVersion(NULL, &sDSIVersionInfo))
      {
        /* Init DSI PHY function table */
        HAL_DSI_PhyFxnsInit(&sDSIVersionInfo);
    ...
    sPhyConfig.bEnableLP11AtInit = psDeviceConfig->bEnableLP11AtInit;
                                          ...
                                          DSI_DisplayConfigType  *psDisplayConfig)
{
  ...
  if (TRUE == psDisplayConfig->sVideoModeConfg.bEnable)
  {
    ...
    /* VDCM Configuration for the DSI controller */
    DSI_OSAL_MemCpy(&psDisplayCtx->sVdcmCompressionInfo.sVdcmVidCntrlrConfig, &psDisplayConfig->sVideoModeConfg.sVDCMVideoConfig,
                sizeof(DSI_VdcmVideoModeConfigType));
  }
  ...
  if (TRUE == psDisplayConfig->sCmdModeConfg.bEnable)
  {
    ...
    /* VDCM Configuration for the DSI controller */
    DSI_OSAL_MemCpy(&psDisplayCtx->sVdcmCompressionInfo.sVdcmCmdCntrlrConfig, &psDisplayConfig->sCmdModeConfg.sVDCMCmdConfig,
                    sizeof(DSI_VdcmCmdModeConfigType));
  }
                                        ...
                                        DSI_ReadPacketType    *psReadPacket)
{
  ...
  else
  {
    ...
    bool32                  bNonEmbeddedMode  = FALSE;
    uint32                  uFlags            = 0;

    DSI_OSAL_MemZero(&sHALPackData,    sizeof(HAL_DSI_PackDataType));
    DSI_OSAL_MemZero(&sHALWritePacket, sizeof(HAL_DSI_WritePacketType));
    ...
    if (NULL != psWritePacket)
    {
      ...
      uPayloadLen     = psWritePacket->uPayloadLen;
      ...
      if (TRUE == DSI_CHECK_FLAG_SET(psWritePacket->uFlags, DSI_DISPLAY_CMDFLAG_NONEMBEDDED))
      {
        if (DSI_DATATYPE_LONGPACKET != (uDataType & 0x0F))
        {
          // Not support non embedded mode if cmd data type is not long packet
          psWritePacket->uFlags &= ~DSI_DISPLAY_CMDFLAG_NONEMBEDDED;
          uFlags                 = psWritePacket->uFlags;
        }
        else
        {
          bNonEmbeddedMode = TRUE;
        }
      }
    }
    else
    ...
    if (DSI_CHECK_FLAG_SET(uFlags, DSI_DISPLAY_CMDFLAG_DMA_INTERLEAVED))
    {
    ...
    else
    {
      ...
      else
      {
        ...
        sHALWritePacket.uFlags = HAL_DSI_FLAG_DATA_FETCH_VIA_FIFO;
      }
    }
    ...
    if (DSI_STATUS_SUCCESS == eStatus)
    {
      ...
      if (TRUE == bNonEmbeddedMode)
      {
        sHALWritePacket.bHighSpeedMode    = bHighSpeedMode;
        sHALWritePacket.uPhyAddress       = psSharedData->iCmdPhyAddr;
        sHALWritePacket.uPayloadLen       = uPayloadLen;
        sHALWritePacket.pPackedFifoBuffer = psSharedData->uCmdFIFOBuffer;
        sHALWritePacket.uCmdDataType      = uDataType;;
        sHALWritePacket.uFlags           |= HAL_DSI_FLAG_ENABLE_NONEMBEDDED;

        DSI_OSAL_MemCpy(pDmaDataBuffer, pInputData, sHALWritePacket.uPayloadLen);
      ...
      /* Pack data into memory */
        ...
        if (TRUE == sHALPackData.bLastPacket )
        {
          ...
          sHALWritePacket.pPackedFifoBuffer = psSharedData->uCmdFIFOBuffer;
          sHALWritePacket.uDmaScheduleLine  = (psWritePacket) ? psWritePacket->uDmaScheduleLine : psReadPacket->uDmaScheduleLine;
                                     ...
                                     DSI_WritePacketType*    psWritePacket)
{
  ...
  if (DSI_STATUS_SUCCESS == eStatus)
  {
    ...
    DSI_OSAL_MB_Ex(psSharedData->pCmdVirtualAddr, psSharedData->pCurrentBufferSize);
    ...
    if(0 == (psWritePacket->uFlags & DSI_DISPLAY_CMDFLAG_ASYNC))
    {
    ...
    )
{
  ...
  /* Clear RDBK before pack data into the hardware */
  HAL_DSI_Clear_RDBK(eDeviceId);
  ...
  if (DSI_STATUS_SUCCESS == eStatus)
  {
    ...
    if(0 == (psReadPacket->uFlags & DSI_DISPLAY_CMDFLAG_ASYNC))
    {
...
static void dsiShared_CalculateDynamicRefresh(DSI_DisplayConfigType                  *psDisplayConfig)
{
    ...
    while((i < MAX_NUM_DYNAMIC_REFRESH) &&
          (psDisplayConfig->sDisplayInfo.sDynamicRefreshInfo.uDynamicRefreshRates[i] > 0))
    {
      i++;
    }
...
/****************************************************************************
*
** FUNCTION: dsiShared_PhyPllSetup()
*/
/*!
* \brief
*       Setup DSI PLL for given bit clock, return the VCO output freq 
*
* \param [in] psDsiPhyConfig      - DSI PHY config info,
* \param [in] psDisplayConfig     - Pass in parameter for setup DSI Phy timing
* \param [in] uDesiredBitClkFreq  - Desired DSI bit clock 
* \param [in] uRefreshRate        - Panel refresh rate.
* \param [in] puVCOOutputFreq     - VCO output frequency.
*
* \retval - none
*
****************************************************************************/
static DSI_StatusType dsiShared_PhyPllSetup(HAL_DSI_PhyConfigType *psDsiPhyConfig,
                                            DSI_DisplayConfigType *psDisplayConfig,
                                            uint32                 uDesiredBitClkFreq,
                                            uint32                 uRefreshRate,
                                            uint32                *puVCOOutputFreq)
{
  DSI_StatusType               eStatus            = DSI_STATUS_SUCCESS;
  DSI_Device_IDType            eDeviceId          = psDsiPhyConfig->eDeviceId;
  DSI_UserCtxType             *psSharedData       = DSIHOST_GET_USERCTX(eDeviceId);
  DSI_DeviceConfigType        *psDeviceConfig     = &psSharedData->sDevConfig;
  ...
  DSI_OSAL_MemZero((void*)&sDsiPhyConfigInfo, sizeof(sDsiPhyConfigInfo));

  /* Initialize  sDsiTimingParams*/
  DSI_OSAL_MemZero((void*)&sDsiTimingParams, sizeof(sDsiTimingParams));

  psDsiPhyConfig->uDesiredBitClkFreq  = uDesiredBitClkFreq;

  for(uint32 uI = 0; uI < (DSI_LaneMap_MAX-1); uI++)
  {
    if( DSI_LaneMap_NONE !=  psDeviceConfig->eDisplayLane[uI])
    {
      psDsiPhyConfig->uLaneMask |= (1 << psDeviceConfig->eDisplayLane[uI]); 
    }
  }

  /* Setup DSI Phy timing */
  sDsiTimingParams.eDeviceId          = eDeviceId;
  sDsiTimingParams.uBitclock          = uDesiredBitClkFreq;
  ...
  sDsiTimingParams.uRefreshRate       = uRefreshRate;
  ...
  *puVCOOutputFreq = sDsiPhyConfigInfo.uPllVcoOutputFreq;
                              ...
                              DSI_DisplayConfigType  *psDisplayConfig)
{
  ...
  if (psDisplayConfig->sDisplayInfo.sDynamicRefreshInfo.uNumOfDynamicFreq > 0)
  {
    int                          i                  = 0;
    DSI_Device_IDType            eDeviceId          = psDsiPhyConfig->eDeviceId;
    uint32                       uVCOOutputFreq;

    /* Make the default value of PllCodes to 0 */
    DSI_OSAL_MemZero((void*)&psDisplayConfig->sDisplayInfo.sPllCodesInfo, sizeof(psDisplayConfig->sDisplayInfo.sPllCodesInfo));
    ...
    for (i = 0; i < psDisplayConfig->sDisplayInfo.sDynamicRefreshInfo.uNumOfDynamicFreq; i++)
    {
      /* Setup PLL for each bit clock  */
      dsiShared_PhyPllSetup(psDsiPhyConfig, psDisplayConfig, psDisplayConfig->sDisplayInfo.sDynamicRefreshInfo.uDynamicBitClkFreq[i], psDisplayConfig->sDisplayInfo.sDynamicRefreshInfo.uDynamicRefreshRates[i], &uVCOOutputFreq);
      /* get Pll Codes for each refresh rate */
      psDisplayConfig->sDisplayInfo.sPllCodesInfo.sPllCodes[i].uValid   = 1;
      psDisplayConfig->sDisplayInfo.sPllCodesInfo.sPllCodes[i].uVcoRate = uVCOOutputFreq;
      HAL_DSI_PhyGetPllCodes(eDeviceId, &psDisplayConfig->sDisplayInfo.sPllCodesInfo.sPllCodes[i]);
    }

    /*restore the default panel bit clock */
    dsiShared_PhyPllSetup(psDsiPhyConfig, psDisplayConfig, psDisplayConfig->sDisplayInfo.uBitClockFreq, psDisplayConfig->sDisplayInfo.uRefreshRate, &uVCOOutputFreq);
...
{ 
  ...
  DSI_OSAL_MemCpy(&sHALVideoConfig.sVdcmVideoConfig, &psDisplayCtx->sVdcmCompressionInfo.sVdcmVidCntrlrConfig, sizeof(DSI_VdcmVideoModeConfigType));
  ...
  else if (sHALVideoConfig.sVdcmVideoConfig.uVdcmEnable)
  {
    /* Enable video mode controller VDCM related settings */
    HAL_DSI_VideoVdcmConfig(eDeviceID, &sHALVideoConfig.sVdcmVideoConfig);
  }
...
{ 
  ...
  sHALCmdConfig.uNullPacketInsertionLength = psSharedData->sDevConfig.uNullPacketInsertionLength;
  ...
  DSI_OSAL_MemCpy(&sHALCmdConfig.sVdcmCmdConfig, &psDisplayCtx->sVdcmCompressionInfo.sVdcmCmdCntrlrConfig,sizeof(DSI_VdcmCmdModeConfigType)); 
  ...
  else if (sHALCmdConfig.sVdcmCmdConfig.uVdcmEnable)
  {
    /* Enable the VDCM compression settings */
    HAL_DSI_CMDVdcmConfig(eDeviceID, &(sHALCmdConfig.sVdcmCmdConfig));
  } 
  /*Clear the dirty bits */
  psDisplayCtx->iDispDirtyBits   &= ~DSI_DIRTYBIT_CMDMODE_MISC; 
...
{ 
  ...
  HAL_DSI_SplitLinkCfgType        sSplitLinkCfg;
  ...
  DSI_OSAL_MemZero(&sSplitLinkCfg,  sizeof(HAL_DSI_SplitLinkCfgType));
  ...
  sSplitLinkCfg.eChannelId        = (psSharedData->iVideoModeRef) ? DSI_ChannelID_Video0 : DSI_ChannelID_CMD0;
  sSplitLinkCfg.eSplitLinkConfig  = psSharedData->sDevConfig.eSplitLinkConfig;
  sSplitLinkCfg.eDMADataSel       = DSI_SPLIT_LINK_SEL_SUBLINK_0;
  sSplitLinkCfg.eBTASel           = DSI_SPLIT_LINK_SEL_SUBLINK_ALL;
  ...
  HAL_DSI_CPhyModeEnable(eDeviceID, psSharedData->bCPhyMode);

  /* Config DSI split link */
  HAL_DSI_SplitLinkConfig(eDeviceID, &sSplitLinkCfg);
                                ...
                                HAL_DSI_PhyConfigType *psPhyConfig)
{
      ...
      (NULL != psPhyConfig))
  {
    ...
    psPhyConfig->uConfigFlags       |= (DSI_SPLIT_LINK_CONFIG_DISABLED != psDeviceConfig->eSplitLinkConfig ) ? HAL_DSI_PHY_CONFIGFLAG_SPLITLINK : 0;
    
    for(uint32 uI = 0; uI < (DSI_LaneMap_MAX-1); uI++)
    {
      if( DSI_LaneMap_NONE !=  psDeviceConfig->eDisplayLane[uI])
      {
        psPhyConfig->uLaneMask |= (1 << psDeviceConfig->eDisplayLane[uI]);
      }
    }
