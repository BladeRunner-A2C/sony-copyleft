/*=============================================================================
  ...
  Copyright (c) 2010-2021, 2024 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
/****************************************************************************
HAL_DSI_IsCPhyModeEnabled()

Description:
    This API can get DSI Cphy enable status

****************************************************************************/
bool32 HAL_DSI_IsCPhyModeEnabled(DSI_Device_IDType eDeviceID)
{
  uint32    uCPhyModeCtrl     = 0;
  uint32    uCPhyModeEn       = 0;
  uintPtr   uOffset           = HAL_DSI_GetRegBaseOffset(eDeviceID);

  uCPhyModeCtrl = in_dword(uOffset + HWIO_DSI_0_CPHY_MODE_CTRL_ADDR);
  uCPhyModeEn   = HWIO_GETVAL(DSI_0_CPHY_MODE_CTRL, uCPhyModeCtrl, EN);

  return ((1 == uCPhyModeEn) ? TRUE : FALSE);
}
                        ...
                        HAL_DSI_LanePropertyType   *psLaneProperty)
{
  ...
  else
  {
    ...
    case DSI_LANE_OP_CLOCK_HS_FORCE_REQUEST:
      {
        /*
         * configure CLKLN_HS_REQUEST_SEL through Phy to match with
         * DSI_0_PHY_DSIPHY_CMN_CTRL_3.DSIPHY_RQST_SEL
         */
        uDsiLaneCntl = HWIO_OUT_FLD(uDsiLaneCntl, DSI_0_LANE_CTRL, CLKLN_HS_REQUEST_SEL,  0);
    ...
    case DSI_LANE_OP_CLOCK_HS_FORCE_REQUEST_CLEAR:
      {
        /* Restore DSI_0_LANE_CTRL.CLKLN_HS_REQUEST_SEL to POR value */
        uDsiLaneCntl = HWIO_OUT_FLD(uDsiLaneCntl, DSI_0_LANE_CTRL, CLKLN_HS_REQUEST_SEL,  1);
...
/****************************************************************************
HAL_DSI_VideoVdcmConfig()

Description:
    This API configures the DSI controller related registers for VDCM for
    the video mode engine.
****************************************************************************/
void HAL_DSI_VideoVdcmConfig(DSI_Device_IDType                 eDeviceID,
                             DSI_VdcmVideoModeConfigType*      psVideoConfig)
{
  uint32  uCompressionModeCtrl      = 0;
  uintPtr uOffset                   = HAL_DSI_GetRegBaseOffset(eDeviceID);
  ...
  {
    uCompressionModeCtrl  = HWIO_OUT_FLD(0,                    DSI_0_VIDEO_COMPRESSION_MODE_CTRL, WC, (psVideoConfig->uVdcmBytesPerPkt));
    uCompressionModeCtrl  = HWIO_OUT_FLD(uCompressionModeCtrl, DSI_0_VIDEO_COMPRESSION_MODE_CTRL, DATATYPE, 0x0B); //Compressed Image
    uCompressionModeCtrl  = HWIO_OUT_FLD(uCompressionModeCtrl, DSI_0_VIDEO_COMPRESSION_MODE_CTRL, PKT_PER_LINE, (psVideoConfig->uVdcmPktPerLine));
    uCompressionModeCtrl  = HWIO_OUT_FLD(uCompressionModeCtrl, DSI_0_VIDEO_COMPRESSION_MODE_CTRL, EOL_BYTE_NUM, (psVideoConfig->uVdcmEolByteNum));
    uCompressionModeCtrl  = HWIO_OUT_FLD(uCompressionModeCtrl, DSI_0_VIDEO_COMPRESSION_MODE_CTRL, EN, (psVideoConfig->uVdcmEnable));
                              ...
                              HAL_DSI_VideoModeConfigType*    psVideoConfig)
{
  ...
  {
    /* if DSC is enabled we have to configure width as compressed width i.e (panel width / compression ratio) */
    uVisWidthPx = psVideoConfig->sDscVideoConfig.uDscEnable ? psVideoConfig->sDscVideoConfig.uDscPclkPerLine : psVideoConfig->sDisplayInfo.iVisWidthPx;
    
    HAL_DSI_VideoTimingConfig(eDeviceID, uVisWidthPx, psVideoConfig->sDisplayInfo.iVisHeightPx,&psVideoConfig->sVideoTiming);
    ...
    if (psVideoConfig->sDisplayInfo.bTwoPixelsPerPclk)
    {
       iVideoModeReg = HWIO_OUT_FLD(iVideoModeReg, DSI_0_VIDEO_MODE_CTRL, DATABUS_WIDEN,        1); 
    }
    else
    {
       iVideoModeReg = HWIO_OUT_FLD(iVideoModeReg, DSI_0_VIDEO_MODE_CTRL, DATABUS_WIDEN,        0); 
    }
                                ...
                                DSI_DscCmdModeConfigType*    psCMDDscConfig)
{
  ...
  else
  {
    ...
    uCompressionModeCtrl2  = HWIO_OUT_FLD(0,                    DSI_0_COMMAND_COMPRESSION_MODE_CTRL2, STREAM0_SLICE_WIDTH,  (psCMDDscConfig->uDscBytesInSlice));
...
/****************************************************************************
HAL_DSI_CMDVdcmConfig()

Description:
    This API configures the DSI controller related registers for VDCM for
    the command mode engine.
****************************************************************************/
void HAL_DSI_CMDVdcmConfig( DSI_Device_IDType                 eDeviceID,
                                DSI_VdcmCmdModeConfigType*    psCMDVdcmConfig)
{
  uint32  uCompressionModeCtrl      = 0;
  uint32  uCompressionModeCtrl2     = 0;
  uintPtr uOffset                   = HAL_DSI_GetRegBaseOffset(eDeviceID);

  if (NULL == psCMDVdcmConfig)
  {
    geDSIErrorCode = HAL_DSI_FAILED_EXPECTED_NON_NULL_PTR;
  }
  else
  {
    uCompressionModeCtrl   = HWIO_OUT_FLD(0,                    DSI_0_COMMAND_COMPRESSION_MODE_CTRL,  STREAM0_DATATYPE,     0x39); //Long Packet
    uCompressionModeCtrl   = HWIO_OUT_FLD(uCompressionModeCtrl, DSI_0_COMMAND_COMPRESSION_MODE_CTRL,  STREAM0_PKT_PER_LINE, (psCMDVdcmConfig->uVdcmPktPerLine));
    uCompressionModeCtrl   = HWIO_OUT_FLD(uCompressionModeCtrl, DSI_0_COMMAND_COMPRESSION_MODE_CTRL,  STREAM0_EOL_BYTE_NUM, (psCMDVdcmConfig->uVdcmEolByteNum));
    uCompressionModeCtrl   = HWIO_OUT_FLD(uCompressionModeCtrl, DSI_0_COMMAND_COMPRESSION_MODE_CTRL,  STREAM0_EN,           (psCMDVdcmConfig->uVdcmEnable));

    uCompressionModeCtrl2  = HWIO_OUT_FLD(0,                    DSI_0_COMMAND_COMPRESSION_MODE_CTRL2, STREAM0_SLICE_WIDTH,  (psCMDVdcmConfig->uVdcmBytesInSlice));
                            ...
                            HAL_DSI_CmdModeConfigType*        psCMDConfig)
{
  ...
  else
  {
    ...
    uCommandModeMDPCtrlReg = in_dword(uOffset + HWIO_DSI_0_COMMAND_MODE_MDP_CTRL_ADDR);
    ...
    if (DSI_ChannelID_CMD0 == psCMDConfig->sDisplayInfo.eDisplayId)
    {
      ...
      uCommandMdpTotalReg = HWIO_OUT_FLD(uCommandMdpTotalReg, DSI_0_COMMAND_MODE_MDP_STREAM0_TOTAL, MDP0_H_TOTAL, psCMDConfig->sDisplayInfo.uWidthTotal);
      uCommandMdpTotalReg = HWIO_OUT_FLD(uCommandMdpTotalReg, DSI_0_COMMAND_MODE_MDP_STREAM0_TOTAL, MDP0_V_TOTAL, psCMDConfig->sDisplayInfo.uHeightTotal);

      /* Register value for DSI_COMMAND_MODE_MDP_STREAM0_CTRL */
      uCommandMdpStreamReg = HWIO_OUT_FLD(uCommandMdpStreamReg, DSI_0_COMMAND_MODE_MDP_STREAM0_CTRL, MDP0_WC, psCMDConfig->sDisplayInfo.uWordCount);
    ...
    {
      //STREAM1 is deprecated
      //#TODO# Remove support for DSI_ChannelID_CMD1
    }
    ...
    if (psCMDConfig->sDisplayInfo.bTwoPixelsPerPclk)
    {
        uCommandModeMDPCtrlReg = HWIO_OUT_FLD(uCommandModeMDPCtrlReg, DSI_0_COMMAND_MODE_MDP_CTRL2, DATABUS_WIDEN, 1);
    }
    else
    {
        uCommandModeMDPCtrlReg = HWIO_OUT_FLD(uCommandModeMDPCtrlReg, DSI_0_COMMAND_MODE_MDP_CTRL2, DATABUS_WIDEN, 0);
    }
    ...
    */
    ...
    /* Enable NULL packet automatic insertion to avoid DSI lanes to go into LP11 mode during H blank period for faster frame transmission */
    /* uNullPacketInsertionLength is 0 based, 1 means WC is 0 which means empty NULL packet */
    if (0 != psCMDConfig->uNullPacketInsertionLength)
    {
      /* Check max WC for automatic generated NULL parket  */
      if (psCMDConfig->uNullPacketInsertionLength > HAL_DSI_MAX_NULL_PKT_INSERTION_WC_IN_BYTE)
      {
        psCMDConfig->uNullPacketInsertionLength = HAL_DSI_DEFAULT_NULL_PKT_INSERTION_WC_IN_BYTE;
      }
      uNullPktInsertionReg = HWIO_OUT_FLD(uNullPktInsertionReg, DSI_0_COMMAND_MODE_NULL_INSERTION_CTRL, EN, 1);
      uNullPktInsertionReg = HWIO_OUT_FLD(uNullPktInsertionReg, DSI_0_COMMAND_MODE_NULL_INSERTION_CTRL, VC, psCMDConfig->sDisplayInfo.eDisplayVC);
      uNullPktInsertionReg = HWIO_OUT_FLD(uNullPktInsertionReg, DSI_0_COMMAND_MODE_NULL_INSERTION_CTRL, WC, psCMDConfig->uNullPacketInsertionLength - 1);
    }
    out_dword(uOffset + HWIO_DSI_0_COMMAND_MODE_NULL_INSERTION_CTRL_ADDR, uNullPktInsertionReg);
  }
}
                            ...
                            HAL_DSI_TriggerModeType         *psTriggerMode)
{
  ...
  else
  {
    ...
    uTriggerCtrlReg = HWIO_OUT_FLD(uTriggerCtrlReg, DSI_0_TRIG_CTRL, COMMAND_MODE_DMA_STREAM_SEL, 0);
                              ...
                              HAL_DSI_ColorInfoType  *psColorInfo)
{
  ...
  case DSI_COLOR_GBR_888_24BPP:
    ...
    /* */  
  case DSI_COLOR_RGB_101010_30BPP:
    psColorInfo->iColorDepth      = HAL_DSI_VIDEO_DST_FOMRAT_RGB101010_30BIT;
    psColorInfo->iColorSwap       = HAL_DSI_DST_FOMRAT_SWAP_RGB;
    psColorInfo->iColorRSel       = HAL_DSI_R_SEL_NONE;
    psColorInfo->iColorGSel       = HAL_DSI_G_SEL_NONE;
    psColorInfo->iColorBSel       = HAL_DSI_B_SEL_NONE;
    psColorInfo->iBpp             = 30;
    break;
  case DSI_COLOR_RBG_101010_30BPP:
    psColorInfo->iColorDepth      = HAL_DSI_VIDEO_DST_FOMRAT_RGB101010_30BIT;
    psColorInfo->iColorSwap       = HAL_DSI_DST_FOMRAT_SWAP_RBG;
    psColorInfo->iColorRSel       = HAL_DSI_R_SEL_NONE;
    psColorInfo->iColorGSel       = HAL_DSI_G_SEL_NONE;
    ...
    psColorInfo->iBpp             = 30;
    break;
  case DSI_COLOR_BGR_101010_30BPP:
    psColorInfo->iColorDepth      = HAL_DSI_VIDEO_DST_FOMRAT_RGB101010_30BIT;
    psColorInfo->iColorSwap       = HAL_DSI_DST_FOMRAT_SWAP_BGR;
    psColorInfo->iColorRSel       = HAL_DSI_R_SEL_NONE;
    psColorInfo->iColorGSel       = HAL_DSI_G_SEL_NONE;
    ...
    psColorInfo->iBpp             = 30;
    break;
  case DSI_COLOR_BRG_101010_30BPP:
    psColorInfo->iColorDepth      = HAL_DSI_VIDEO_DST_FOMRAT_RGB101010_30BIT;
    psColorInfo->iColorSwap       = HAL_DSI_DST_FOMRAT_SWAP_BRG;
    psColorInfo->iColorRSel       = HAL_DSI_R_SEL_NONE;
    psColorInfo->iColorGSel       = HAL_DSI_G_SEL_NONE;
    ...
    psColorInfo->iBpp             = 30;
    break;
  case DSI_COLOR_GRB_101010_30BPP:
    psColorInfo->iColorDepth      = HAL_DSI_VIDEO_DST_FOMRAT_RGB101010_30BIT;
    psColorInfo->iColorSwap       = HAL_DSI_DST_FOMRAT_SWAP_GRB;
    psColorInfo->iColorRSel       = HAL_DSI_R_SEL_NONE;
    psColorInfo->iColorGSel       = HAL_DSI_G_SEL_NONE;
    ...
    psColorInfo->iBpp             = 30;
    break;
  case DSI_COLOR_GBR_101010_30BPP:
    psColorInfo->iColorDepth      = HAL_DSI_VIDEO_DST_FOMRAT_RGB101010_30BIT;
    psColorInfo->iColorSwap       = HAL_DSI_DST_FOMRAT_SWAP_GBR;
    psColorInfo->iColorRSel       = HAL_DSI_R_SEL_NONE;
    psColorInfo->iColorGSel       = HAL_DSI_G_SEL_NONE;
                            ...
                            HAL_DSI_ColorInfoType  *psColorInfo)
{
  ...
  case DSI_COLOR_GBR_888_24BPP:
    ...
    /* */
  case DSI_COLOR_RGB_101010_30BPP:
    psColorInfo->iColorDepth      = HAL_DSI_CMDMODE_DST_FOMRAT_RGB101010_30BIT;
    psColorInfo->iColorSwap       = HAL_DSI_DST_FOMRAT_SWAP_RGB;
    psColorInfo->iColorRSel       = HAL_DSI_R_SEL_NONE;
    psColorInfo->iColorGSel       = HAL_DSI_G_SEL_NONE;
    psColorInfo->iColorBSel       = HAL_DSI_B_SEL_NONE;
...
/****************************************************************************
*
** FUNCTION: HAL_DSI_SplitLinkConfig()
*/
/*!
* \brief
*    Configure split link 
*
* \param [in] eDeviceID       - DSI device ID.
* \param [in] psSplitLinkCfg  - DSI split link configuration.
*
* \retval eStatus
*
****************************************************************************/
HAL_dsi_StatusType HAL_DSI_SplitLinkConfig(DSI_Device_IDType            eDeviceID,
                                           HAL_DSI_SplitLinkCfgType    *psSplitLinkCfg)
{
  HAL_dsi_StatusType   eStatus = HAL_DSI_SUCCESS;

  if (NULL == psSplitLinkCfg)
  {
    eStatus = HAL_DSI_FAILED_EXPECTED_NON_NULL_PTR;
  }
  else
  {
    uintPtr uOffset           = HAL_DSI_GetRegBaseOffset(eDeviceID);
    uint32  uSplitLinkCtrlReg = 0;
    uint32  uSplitLinkEn      = (DSI_SPLIT_LINK_CONFIG_DISABLED   == psSplitLinkCfg->eSplitLinkConfig) ? 0 : 1;
    uint32  uSwapSublink      = (DSI_SPLIT_LINK_CONFIG_ENABLED_RL == psSplitLinkCfg->eSplitLinkConfig) ? 1 : 0;

    uSplitLinkCtrlReg = HWIO_OUT_FLD(uSplitLinkCtrlReg, DSI_0_SPLIT_LINK_CTRL, EN,                    uSplitLinkEn);
    uSplitLinkCtrlReg = HWIO_OUT_FLD(uSplitLinkCtrlReg, DSI_0_SPLIT_LINK_CTRL, DMA_LINK_SEL,          psSplitLinkCfg->eDMADataSel);
    uSplitLinkCtrlReg = HWIO_OUT_FLD(uSplitLinkCtrlReg, DSI_0_SPLIT_LINK_CTRL, BTA_LINK_SEL,          psSplitLinkCfg->eBTASel);
    uSplitLinkCtrlReg = HWIO_OUT_FLD(uSplitLinkCtrlReg, DSI_0_SPLIT_LINK_CTRL, MDP0_LINK_SEL,         HAL_DSI_SPLIT_LINK_MDP_LINK_SEL_ALL);
    uSplitLinkCtrlReg = HWIO_OUT_FLD(uSplitLinkCtrlReg, DSI_0_SPLIT_LINK_CTRL, TX_TRIGGER_LINK_SEL,   DSI_SPLIT_LINK_SEL_SUBLINK_ALL);
    uSplitLinkCtrlReg = HWIO_OUT_FLD(uSplitLinkCtrlReg, DSI_0_SPLIT_LINK_CTRL, HDCP_LINK_SEL,         DSI_SPLIT_LINK_SEL_SUBLINK_ALL);
    uSplitLinkCtrlReg = HWIO_OUT_FLD(uSplitLinkCtrlReg, DSI_0_SPLIT_LINK_CTRL, VIDEO_INPUT_SWAP,      (DSI_ChannelID_Video0 == psSplitLinkCfg->eChannelId)? uSwapSublink : 0);
    uSplitLinkCtrlReg = HWIO_OUT_FLD(uSplitLinkCtrlReg, DSI_0_SPLIT_LINK_CTRL, COMMAND_INPUT_SWAP,    (DSI_ChannelID_CMD0   == psSplitLinkCfg->eChannelId)? uSwapSublink : 0);

    out_dword(uOffset + HWIO_DSI_0_SPLIT_LINK_CTRL_ADDR, uSplitLinkCtrlReg);
  }
