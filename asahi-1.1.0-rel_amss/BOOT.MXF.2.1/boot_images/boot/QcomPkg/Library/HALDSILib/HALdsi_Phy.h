...
#define  DSIV2_MINOR_VERSION  0x03

/* Macro to define different dsi phy function table name based on phy version */
#define HAL_DSI_FunctionTable(_major_, _minor_, _rev_)\
  gDsiPhyFxnTbl_##_major_##_##_minor_##_##_rev_
...
typedef enum
{
  ...
  HAL_DSI_PHY_PROPERTYID_POST_CLK_CONFIG,               /**< Any PHY config needed after clock source is configured */
  HAL_DSI_PHY_PROPERTYID_MAX,
  HAL_DSI_PHY_PROPERTYID_FORCE_32BIT = 0x7FFFFFFF
}HAL_DSI_PhyPropertyType;

/*!
 * \b HAL_DSI_PhyConfigFlagType
 *
 * Defines list of DSI Phy config flag.
 */
typedef enum
{
  HAL_DSI_PHY_CONFIGFLAG_NONE         = 0,
  HAL_DSI_PHY_CONFIGFLAG_SPLITLINK    = HAL_DSI_BIT(0), /* Split Link is enabled */
  HAL_DSI_PHY_CONFIGFLAG_MAX,
  HAL_DSI_PHY_CONFIGFLAG_FORCE_32BIT = 0x7FFFFFFF
}HAL_DSI_PhyConfigFlagType;

/*!
 * \b HAL_DSI_PhyClkSelType
 *
 * Defines pixel clk source select .
 */
typedef enum
{
  HAL_DSI_PHY_CLK_SEL_0 = 0x0,      /* Source Freq =  vco * phy post div * pllpost div */
  HAL_DSI_PHY_CLK_SEL_1 = 0x1,      /* Source Freq =  vco * phy post div * pllpost div / 2 */
  HAL_DSI_PHY_CLK_SEL_2 = 0x2,      /* Source Freq =  vco *  pllpost div */
  HAL_DSI_PHY_CLK_SEL_3 = 0x3,      /* For Cphy :- Source Freq =  vco *  pllpost div /3.5 
                                       For Dphy :- Source Freq = vco *  pllpost div / 4 */
  HAL_DSI_PHY_CLK_SEL_MAX,
}HAL_DSI_PhyClkSelType;
...
typedef struct _HAL_DSI_PhyConfigType
{
   ...
   HAL_DSI_PhyClkSelType        eDsiClkSel;             /* pixel clk source select */
   uint32                       uPhyPostDiv;
   uint32                       uPllPostDiv;
   DSI_SSCConfigType            eSSCConfig;             /* DSI SSC configuration, including enable/disable ssc, and ssc spread mode selection */
   bool32                       bCalibrationMode;       /* Enable phy in PLL calibration mode */
   bool32                       bEnableLP11AtInit;      /* Put the line state in to LP11 after init */
   uint32                       uLaneMask;              /* Lane Mask to know which lanes are programmed */               
...
typedef struct _HAL_DSI_TimingSettingType
{
  ...
  HAL_DSI_PhyTimingMargin   eTimingMargin;         /* PHY timing margin */
  uint32                    uBitclock;
  uint32                    uEscapeFreq;
...
*
* \param [in]   pDSIVersionInfo   - DSI CTL version info
*
* \retval None
*
****************************************************************************/
void HAL_DSI_PhyFxnsInit(HAL_DSI_VersionType* pDSIVersionInfo);
...
*
****************************************************************************/
int32 HAL_DSI_Phy_Roundup(float fFloatVal);
