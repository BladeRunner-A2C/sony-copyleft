    ...
    )
{
  ...
  else
  {
    ...
    {
      /* Do dynamic refresh calibration here, if uNumOfDynamicFreq is 0, this function actually do nothing*/
      if (DSI_STATUS_SUCCESS == (eStatus = dsiSystem_FrequencyCalibration(hDevice, psDisplayConfig)))
      {
        ...
        DSI_Channel_IDType  eDisplayID   = psDisplayConfig->sDisplayInfo.eDisplayId;
        DSI_DisplayCtxType* psDisplayCtx = DSIHOST_GET_DISPLAYCTX(eDeviceID, eDisplayID);

        /* Save the Pll codes related information */
        psDisplayCtx->sDisplayInfo.sDynamicRefreshInfo.uNumOfDynamicFreq = psDisplayConfig->sDisplayInfo.sPllCodesInfo.uRefreshRateCount;
        DSI_OSAL_MemCpy(&psDisplayCtx->sDisplayInfo.sPllCodesInfo, &psDisplayConfig->sDisplayInfo.sPllCodesInfo,
                   sizeof(psDisplayCtx->sDisplayInfo.sPllCodesInfo));
      }
    ...
    )
{
  ...
  uRetryCount = (DSI_DISPLAY_CMDFLAG_NORETRY & psWritePacket->uFlags)?1:DSI_WRITE_RETRY_COUNT;
    ...
    DSI_PropertyParamsType   *pPropertyData)
{
  ...
  switch(eProperty)
  {
    case DSI_DISPLAY_PROPERTYID_DISPINFO:
    {
      ...
      // DSI_ChannelID_CMD1 is deprecated
      break;
    }
    ...
    {        
        ...
        for(uint32 uI = 0; uI < (DSI_LaneMap_MAX-1); uI++)
        {
          ...
          {
            sDsiPhyConfig.uLaneMask |= (1 << psDeviceConfig->eDisplayLane[uI]); 
          }
        }
    ...
    case DSI_DISPLAY_PROPERTYID_LANESTATE:
      {
        ...
        else if (DSI_LANE_STATE_CLOCK_HS == pPropertyData->eLaneState)
        {
          ...
          sPhyPropertyType.sLaneConfig.eClockLaneOp = DSI_LANE_OP_CLOCK_HS_FORCE_REQUEST;
          HAL_DSI_PhySetProperty(eDeviceId, HAL_DSI_PHY_PROPERTYID_LANECONFIG, &sPhyPropertyType);
        }
        else if (DSI_LANE_STATE_CLOCK_LP11 == pPropertyData->eLaneState)
        {
          ...
          sPhyPropertyType.sLaneConfig.eClockLaneOp = DSI_LANE_OP_CLOCK_HS_FORCE_REQUEST_CLEAR;
          HAL_DSI_PhySetProperty(eDeviceId, HAL_DSI_PHY_PROPERTYID_LANECONFIG, &sPhyPropertyType);
        }
      }
      break;      
    ...
    case DSI_DISPLAY_PROPERTYID_LANEMAP:
      ...
      break;
    case DSI_DISPLAY_PROPERTYID_SPLITLINK_BROADCAST:
      {
        HAL_DSI_SplitLinkCfgType sSplitLinkCfg;
        DSI_SplitLinkSelectType  eSplitLinkSelectLink = (pPropertyData->bSplitLinkCmdBroadcast) ? DSI_SPLIT_LINK_SEL_SUBLINK_ALL : DSI_SPLIT_LINK_SEL_SUBLINK_0;
        ...
        sSplitLinkCfg.eDMADataSel      = eSplitLinkSelectLink;
        sSplitLinkCfg.eBTASel          = eSplitLinkSelectLink;
  ...
  case DSI_DISPLAY_PROPERTYID_GETCALCODES:
      {
        ...
        if ((psDisplayCtx->sDisplayInfo.sDynamicRefreshInfo.uNumOfDynamicFreq > 0) &&
            (psDisplayCtx->sDisplayInfo.sDynamicRefreshInfo.uNumOfDynamicFreq <= MAX_NUM_DYNAMIC_REFRESH))
        {
          ...
          DSI_OSAL_MemCpy((void *) &pPropertyData->sDisplayInfo.sPllCodesInfo, 
                          ...
                          sizeof(pPropertyData->sDisplayInfo.sPllCodesInfo));
        }
        else
        {
