...
Copyright (c) 2015-2018,2020,2022 Qualcomm Technologies, Inc.  All Rights Reserved.
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ServicesLib/memheap_lite.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
02/05/22    ds     change to increase number of heaps allowed
...
#define MAX_HEAP_INIT 9
uint32 magic_num[MAX_HEAP_INIT] = {(uint32)-1,(uint32)-1,(uint32)-1,(uint32)-1,(uint32)-1,(uint32)-1,(uint32)-1,(uint32)-1,(uint32)-1};
uint16 magic_num_index_array[MAX_HEAP_INIT] = {0,1,2,3,4,5,6,7,8};

uint32 magic_num_index = 0;
...
static void mem_heap_get_random_num(void*  random_ptr, int random_len)
{
  ...
  if(random_len != (int)uclib_get_prng_data(ucenv_get_env(), ((uint8*)random_ptr), (size_t)random_len))
  {
    MEMHEAP_ASSERT(0);
  }
#else
...
{
   return mem_init_heap_with_resrv_alignd_mem(heap_ptr, heap_mem_ptr, heap_mem_size, 0, 0);
} /* END mem_init_heap */

void mem_init_heap_with_resrv_alignd_mem(
	mem_heap_type                 *heap_ptr,
	void                          *heap_mem_ptr,
	unsigned int                  heap_mem_size,
	unsigned int alignment,    /* Number of bytes to be aligned with and a chunk size */
	unsigned int number_of_chunks /* Number of chunks with the alignment */
	)
{
	char *memory_end_ptr;
	...
	*/
	...
	if (alignment && number_of_chunks){
		int i = 0;
		unsigned int size = number_of_chunks*alignment;

		MEMHEAP_ASSERT(number_of_chunks <= MEM_MAX_ALIGNED_CHUNKS);
		heap_ptr->aligned_mem_buffer = mem_memalign(heap_ptr, alignment, size);
		MEMHEAP_ASSERT(heap_ptr->aligned_mem_buffer != NULL);
		heap_ptr->alignment = alignment;
		while (number_of_chunks > 0){
			heap_ptr->aligned_mem_bits_array = heap_ptr->aligned_mem_bits_array | (1 << i);
			i++;
			number_of_chunks--;
		}
		heap_ptr->aligned_mem_buffer_end = (void*)((char*)(heap_ptr->aligned_mem_buffer) + size);
	}
...
DESCRIPTION
  Allocates an aligned block of size bytes from the heap pHeap. The
  allocated block will be aligned along alignment number of bytes. If pHeap
  is NULL, size is 0, or alignment is not a power-of-2 greater than 8, this
  function will return NULL.

  Returns a pointer to the newly allocated block of aligned memory, or NULL
...
(
  ...
  /* Number of bytes to be aligned with. */
...
{
  mem_block_header_type* pPrevBlk = NULL;
  mem_block_header_type* pTrailingBlk = NULL;
  mem_block_header_type* pNewBlk = NULL;
  ...
  unsigned long unalignedSize = 0;
  unsigned long actualSize = 0;
  unsigned long used_bytes = 0;
  unsigned int alignMask = alignment - 1;
  unsigned int prevBlkSize = 0;
  unsigned int trailingBlkSize = 0;

  /* Parameter validation. */
  MEMHEAP_ASSERT(heap_ptr);
  if ( (!size) ||
       (size > heap_ptr->total_bytes) ||
       (kMinChunkSize > alignment) ||
       (alignment & alignMask) )
  {
    return NULL;
  }
  /*first check if heap_ptr->aligned_mem_buffer was allocated 
  and this alignment request falls in that category */
  BEGIN_CRITICAL_SECTION(heap_ptr);
  if (heap_ptr->alignment == alignment 
	  && (heap_ptr->aligned_mem_bits_array!=0)
	  && (size <= alignment))
  {
	  /*find first set bit in the heap_ptr->aligned_mem_bits_array and allocate 
	  the corrsponding position buffer from heap_ptr->aligned_mem_buffer and set 
	  the bit position to zero to idicate that the corrsponding chunk is 
	  allocated from the heap_ptr->aligned_mem_buffer*/
	  int i;
	  for (i = 0; i < MEM_MAX_ALIGNED_CHUNKS; i++)
	  {
		  if (heap_ptr->aligned_mem_bits_array & (1 << i))
		  {
			  void *aligned_ptr = NULL;
			  /*ith bit is set, so its a free chunk that can be allocated*/
			  aligned_ptr = (char*)(heap_ptr->aligned_mem_buffer) + (alignment*i);
			  heap_ptr->aligned_mem_bits_array = (heap_ptr->aligned_mem_bits_array) & (~(1 << i));
			  END_CRITICAL_SECTION(heap_ptr);
			  return aligned_ptr;
		  }
  ...
  /* mem_malloc is essentially aligned allocation on kMinChunkSize. */
  /* Short-circuit to mem_malloc call. */
  if (kMinChunkSize == alignment)
  {
    return mem_malloc(heap_ptr, size);
  }

  /* # of chunks needed for requested size (unaligned). */
  chunks = (size + sizeof(mem_block_header_type) + kMinChunkSize - 1) /
    kMinChunkSize;

  /* # of bytes needed for requested size (unaligned). */
  unalignedSize = chunks * kMinChunkSize;

  /* # of bytes needed (to accomodate alignment). */
  actualSize = (2 * alignment) + unalignedSize +
  ...
  /* Allocate 1 large block. */
  pAlloc = mem_malloc(heap_ptr, actualSize);

  if ( (!pAlloc) || // Fail.
       (!((unsigned int)(uintptr_t)pAlloc & alignMask)) ) // Success & aligned.
  {
    END_CRITICAL_SECTION(heap_ptr);
    return pAlloc;
  }
  /* If we are here, allocation succeeded but block is not aligned. */

  /* Get first address in block which is aligned. */
  pAlign = (void*)(((unsigned long)pAlloc + alignMask) & ~alignMask);

  /* There must be enough room to accomodate the block header. */
  if (((char*)pAlign - (char*)pAlloc) < (sizeof(mem_block_header_type) + 1))
  {
    /* Get second address in block which is aligned. */
    pAlign = (void*)(((unsigned long)pAlign + 1 + alignMask) & ~alignMask);
  }
  /* We now have a block we will use (pNewBlk) from the large allocation. */
  /* However, the block we want now has excess space above (and possibly below). */
  /* We must set these up as separate blocks and then free them. */
  
  pPrevBlk =
    (mem_block_header_type*)((char*)pAlloc - sizeof(mem_block_header_type));
  prevBlkSize = (char*)pAlign - (char*)pAlloc;
  trailingBlkSize = pPrevBlk->forw_offset - prevBlkSize - unalignedSize;
  pPrevBlk->forw_offset = prevBlkSize;

  /* Increment block count (free will soon decrement). */
  heap_ptr->total_blocks++;

  pNewBlk =
    (mem_block_header_type*)((char*)pAlign - sizeof(mem_block_header_type));

  /* Initialize a new block header for this new block. */
  mem_init_block_header(pNewBlk, unalignedSize, heap_ptr);

  pNewBlk->last_flag = pPrevBlk->last_flag;
  pNewBlk->free_flag = kBlockUsed;

  pNewBlk->extra =
    (unsigned char)(unalignedSize - size - sizeof(mem_block_header_type));

  /* Set-up guard pages for new block and soon-to-be-freed prev block. */
  pBlk = (uint16*)pNewBlk;
  ADD_GUARD_BYTES_TO_USED_HEADER(heap_ptr->magic_num_used, pBlk);
  pBlk = (uint16*)pPrevBlk;
  ADD_GUARD_BYTES_TO_USED_HEADER(heap_ptr->magic_num_used, pBlk);

  mem_free(heap_ptr, pAlloc);

  /* If excess space below is larger than header, make and free block here also. */
  if (trailingBlkSize > sizeof(mem_block_header_type))
  {
    pTrailingBlk =
      (mem_block_header_type*)((char*)pAlign + unalignedSize -
        sizeof(mem_block_header_type));

    /* Initialize a new block header for this new block. */
    mem_init_block_header(pTrailingBlk, trailingBlkSize, heap_ptr);

    pTrailingBlk->last_flag = pNewBlk->last_flag;
    pTrailingBlk->free_flag = kBlockUsed;

    /* Increment block count (free will soon decrement). */
    heap_ptr->total_blocks++;

    pBlk = (uint16*)pTrailingBlk;
    ADD_GUARD_BYTES_TO_USED_HEADER(heap_ptr->magic_num_used, pBlk);
    
    mem_free(heap_ptr, (((char*)pTrailingBlk) + sizeof(mem_block_header_type)));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*check if the ptr being freed belongs to aligned_mem_buffer */ 
  if (ptr < heap_ptr->aligned_mem_buffer_end && ptr >= heap_ptr->aligned_mem_buffer)
  {
	  int i = ((uint64)ptr - (uint64)(heap_ptr->aligned_mem_buffer))/(heap_ptr->alignment); /*calculate the bit position*/

      MEMHEAP_ASSERT((((uint32)(uintptr_t)ptr)%(heap_ptr->alignment)) == 0); /*ASSERT if the ptr is not aligned to reserved alignment size */
	  MEMHEAP_ASSERT(i < MEM_MAX_ALIGNED_CHUNKS); /*ASSERT if i is greater than MEM_MAX_ALIGNED_CHUNKS */
	  
	  /*check if the ith bit is already set , just to make sure we are freeing the right ptr and double free case*/
	  MEMHEAP_ASSERT(!(heap_ptr->aligned_mem_bits_array & (1 << i)));

	  /*now set the ith bit to indicate its now free*/
	  heap_ptr->aligned_mem_bits_array = heap_ptr->aligned_mem_bits_array | (1 << i);
	  END_CRITICAL_SECTION(heap_ptr);
	  return;
  }
  ...
  MEMHEAP_ASSERT((((uint32)(uintptr_t)theBlock)%kMinChunkSize) == 0);
	...
	if(theBlock != heap_ptr->first_block)
    {
      ...
      if(!((temp->freeBlock_frdOff)%kMinChunkSize)){
        ...
        if(validBlock){
           ...
           if(BOUNDARY_CHECK(preFreeBlock, heap_ptr)){
