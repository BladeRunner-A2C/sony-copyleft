...
boot_recovery_status boot_recovery_add_recovery_info(uint8 *partition_id, uint8 *recovery_partition_id, uint64 last_reported_errorcode, uint32 flags)
{
  ...
  uint32 copy_size = 0;
  boot_boolean corruption_reports_flag = FALSE;
  ...
  do
  {
    ...
    /* If same image is reported as corrupted again, increment number_of_corruption_reports and break without adding new entry in smem*/
    for(uint8 i = 0; i < recovery_partition_base->num_of_entries; i++)
    {
      if( memcmp(partition_id, recovery_partition_base->partition_info[i].corrupted_partition_id, PARTITION_ID_MAX_SIZE) == 0)
      {
        ...
        recovery_partition_base->partition_info[i].last_errorcode = last_reported_errorcode;
        recovery_partition_base->partition_info[i].flags |= flags;
        corruption_reports_flag = TRUE;
        break;
      }
    }

    if(corruption_reports_flag)
      break;
       ...
       (recovery_partition_base->num_of_entries < recovery_partition_base->max_entries))
    {
      ...
      recovery_partition_base->partition_info[recovery_partition_base->num_of_entries].last_errorcode = last_reported_errorcode;
      recovery_partition_base->partition_info[recovery_partition_base->num_of_entries].flags = flags;

      recovery_partition_base->partition_info[recovery_partition_base->num_of_entries].number_of_corruption_reports = 1;

      if(recovery_partition_base->num_of_entries < recovery_partition_base->max_entries)
      {
