...
/*======================================================================
  ...
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/UfsCommonLib/ufs_core.h#1 $
  $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $

when         who     what, where, why
----------   ---     --------------------------------------------------- 
2023-07-24   sa      Update power mode sequence to set value of de-emphasis
2023-07-21   jt      Add field for EOM enablement 
2023-05-10   pz      Add lba in ufs_cmd_req_t 
2023-04-21   jt      Eye Monitor updates 
2022-05-14   sa      Added support for BlockIo2 with multiple doorbells.
2022-05-10   sa      Added SharedIce related structs.
2022-04-06   pz      Report RPMB partition region 0 size
2021-10-25   jt      Add write booster support 
...
2020-10-05   jt      Add PA_Granularity  
2020-03-31   jt      Add power management information
...
2018-08-16   jt      Add field to keep track of link state 
...
#define RX_EYEMON_Capabililty                   0xF1
#define RX_EYEMON_Timing_MAX_Steps_Capability   0xF2
#define RX_EYEMON_Timing_MAX_Offset_Capability  0xF3
#define RX_EYEMON_Voltage_MAX_Steps_Capability  0xF4
#define RX_EYEMON_Voltage_MAX_Offset_Capability 0xF5
#define RX_EYEMON_Enable                        0xF6
#define RX_EYEMON_Timing_Steps                  0xF7
#define RX_EYEMON_Voltage_Steps                 0xF8
#define RX_EYEMON_Target_Test_Count             0xF9
#define RX_EYEMON_Tested_Count                  0xFA
#define RX_EYEMON_Error_Count                   0xFB
#define RX_EYEMON_Start                         0xFC
...
#define PA_Granularity_MinVal                   0x1
#define PA_Granularity_MaxVal                   0x6
...
#define DME_VS_UNIPRO_STATE                     0xD000
#define DME_VS_CORE_CLK_CTRL                    0xD002

#define UIC_ARG0_MIBattr(Id, Gs)                ((((uint32_t)Id << 16) & 0xffff0000) | (Gs & 0xffff))
#define GenSelectRx(Ln)                         (Ln + 4)   
#define GenSelectTx(Ln)                         Ln
...
typedef struct {
   ...
   uint16_t       wLUMaxActiveHPBRegions;
   uint16_t       wHPBPinnedRegionStartIdx;
   uint16_t       wNumHPBPinnedRegions;
...
typedef struct {
   ...
   uint8_t        bHPBRegionSize;
   uint8_t        bHPBNumberLU;
   uint8_t        bHPBSubRegionSize;
   uint16_t       wDeviceMaxActiveHPBRegions; 
...
typedef struct {
   uint8_t  ce_enabled;		    /* Is crypto engine enabled? */
   uint8_t  shared_ice_enabled;  /* Is SharedIce enabled? */
   uint8_t  shared_ice_algo   ;  /* Shared Ice Algorithm enabled. */
}ufs_ice_params;

typedef struct ufs_host_info {
   uint8_t                   ufs_core_id;         /* UFS Core */
   uint8_t                   init_stage;          /* 0 = uninitialized, 1 = partial, 2 = full */
   uint16_t                  ufs_hw_version;      /* UFS controller HW Version */
   ...
   uint8_t                   fDeviceInit_async;   /* Asynchronous fDeviceInit in progress */
   ...
   ufs_bsp_platform_config   plat_cfg;            /* Platform configuration */
   ...
   uint8_t                   link_state;          /* Link in hibernate or not? */ 
   uint8_t                   curr_volt_corner;    /* Current voltage corner of UFS controller */
   uint8_t                   auto_h8_enabled;     /* Is auto-hibernate enabled? */
   ufs_ice_params            ice_params;          /* Parameters for Crypto and Shared Oce */
   ufs_bsp_speed_config      curr_speed_config;   /* Current speed configuration */
   uint8_t                   has_dt_cfg;          /* Got the config attributes from Device Tree? */
   ufs_bsp_config            cfg_entries;         /* Configuration entries from Device Tree */

   uint8_t                   wb_enabled;          /* Is write booster enabled? */
   uint8_t                   eom_enabled;         /* Is EOM enabled? */
} ufs_host_t;
...
typedef struct ufs_command_request_info {
   ...
   uint8_t        async;            // Asynchronous or not
   ...
   uint32_t       utp_doorbell_bit; // Cmd slot in use
   uint64_t       utp_cmd_desc_phy; // Buffer for command descriptor
   uint32_t       resp_upiu_offset; // Offset for the resp UPIU
   uint32_t       prdt_offset;      // Offset for the PRDT

   uint8_t        crypto;           // Enable crypto operations or not
   uint8_t        crypto_cfg_index; // CCI used for this request
   uint64_t       data_unit_number; // DUN cryptographic parameter
} ufs_cmd_req_t;
...
/* Shared ICE Information. This should match with Protocol file. */
...
typedef enum
{
  ...
  SHARED_ICE_ALGO_NONE = 0xFF
}ufs_shared_ice_algorithms;

/* Buffer to share Shared ICE info between driver and tools. */
typedef struct
{
    uint8_t shared_ice;
  ufs_shared_ice_algorithms shared_ice_algo;
    uint8_t *algo_value_buffer;
}ufs_shared_ice_info;

typedef struct 
{
  uint8_t algo_valid;
  uint8_t no_of_tx_core;
  uint8_t no_of_rx_core;
  uint8_t no_of_write_task_max;
  uint8_t no_of_write_task_min;
  uint8_t no_of_read_task_max;
  uint8_t no_of_read_task_min;
}ufs_shared_ice_floor_val;

typedef struct
{
  uint8_t no_of_tx_core;
  uint8_t no_of_rx_core;
}ufs_shared_ice_static_val;

typedef struct
{
  uint8_t no_of_tx_r1w1_core;
  uint8_t no_of_rx_r1w1_core;
  uint8_t no_of_tx_r0w1_core;
  uint8_t no_of_rx_r1w0_core;
}ufs_shared_ice_instant_val;
