...
----------   ---     ----------------------------------------------------- 
2023-07-21   jt      Add ufs_finish_reinit
2023-06-02   jt      Set adapt in current speed config 
2023-05-10   pz      Print out abnormal LBA while LBA out of range 
2023-04-21   jt      Eye Monitor updates 
2023-03-17   sa      Add API to dump UFS registers when requested.
2023-02-06   qw      Add write booster buffer size attributes
...
int32_t ufs_scsi_process_sense (ufs_host_t *hba, ufs_cmd_req_t *req)
{
   ...
   if ((req->req_sense[4] & 0x0f) == SENSE_KEY_UNIT_ATTENTION) {
      ...
      // While LBA out of range, add related log to locate abnormal LBA
      if (((req->req_sense[4] & 0x0f) == SENSE_KEY_ILLEGAL_REQUEST) && (req->req_sense[14] == 0x21) && (req->req_sense[15] == 0)) {
         osal_memset (log_buf, 0, sizeof(log_buf));
         snprintf(log_buf, 64, "LBA out of Range, LUN:0x%x, LBA:0x%x, BLOCKS:0x%x", 
                  req->lun, req->lba, req->expected_len / DEFAULT_SECTOR_SIZE); 
         ufs_bsp_log(log_buf, UFS_LOG_ERROR); 
      }
   }
...
// Steps to finish a reinit (NOP, fDeviceInit, request sense, gear switch)
int32_t ufs_finish_reinit (struct ufs_handle *hufs)
{
  ufs_host_t *hba;
  int i = 0;
  int32_t rc = EOK; 
...
int32_t ufs_reset_host_controller (struct ufs_handle *hufs)
{
  ...
  rc = ufs_finish_reinit (hufs); 
  if (rc != EOK) {
    ufs_error_log (hba, 0x02cf); 
    return rc; 
  }
...
int32_t ufs_set_gear (struct ufs_handle *hufs)
{
   ...
   if (hba->plat_cfg.core_skip && (hba->ufs_core_id == 0)) {
      ...
      // Set adapt = 1 for mission mode
      hba->curr_speed_config.adapt = 1; 
...
// Get the bAvailableWriteBoosterBufferSize field in the attributes
int32_t ufs_get_bAvailableWriteBoosterBufferSize (struct ufs_handle *hufs, UINT32 *AvailBufferSize)
{
   char log_buf[32] = {0}; 
   ufs_attr_req_t req;
   int32_t rc = EOK;
   ...
   osal_memset (&req, 0, sizeof (ufs_attr_req_t));
   req.phc = hufs->phc;
   req.idn = UFS_IDN_bAvailableWriteBoosterBufferSize;
   req.idx = 0;
   req.val = 0;
   req.sel = 0; 
   ...
   *AvailBufferSize = req.val;

   if (rc == EOK) {
      snprintf(log_buf, 32, "UFS WB Available Buffer Size: %d", req.val);
      ufs_bsp_log(log_buf, UFS_LOG_INFO);
   }
   return rc;
}

// Get the dCurrentWriteBoosterBufferSize field in the attributes
int32_t ufs_get_dCurrentWriteBoosterBufferSize (struct ufs_handle *hufs, UINT32 *CurBufferSize)
{
   char log_buf[32] = {0}; 
   ufs_attr_req_t req;
   int32_t rc = EOK;
   ...
   osal_memset (&req, 0, sizeof (ufs_attr_req_t));
   req.phc = hufs->phc;
   req.idn = UFS_IDN_dCurrentWriteBoosterBufferSize;
   req.idx = 0;
   req.val = 0;
   req.sel = 0; 
   ...
   *CurBufferSize = req.val;

   if (rc == EOK) {
      snprintf(log_buf, 32, "UFS WB Current Buffer Size: %d", req.val);
      ufs_bsp_log(log_buf, UFS_LOG_INFO);
   }
   return rc;
}
...
/* Dump UFS regisers based on type of registers to dump*/
int32_t ufs_do_dump_registers(struct ufs_handle *hufs,ufs_register_masks reg_mask)
{
  int32_t rc = EOK;
  ufs_host_t *hba = hufs->phc; 
  
  rc = ufs_utp_dump_registers(hba, reg_mask);
