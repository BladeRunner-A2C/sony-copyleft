...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/UfsCommonLib/ufs_api.c#1 $
...
2021-10-25   jt      Enable write booster and add sync cache support 
...
int32 ufs_background_init_start (uint32 core_id, uint32 init_ctxt)
{
   ...
   rc = ufs_set_fDeviceInit (hufs, TRUE);
   if (rc == -EBUSY) {
      rc = EOK; 
   }
...
{
   struct ufs_handle *hdl, *boot_hdl;
   (void) init_ctxt; 
   ...
   boot_hdl = ufs_open(core_id, UFS_WLUN_BOOT);
   if (boot_hdl == NULL) {
     return UFS_EINVAL;
   }
...
struct ufs_handle *ufs_open (uint32 core_id, uint32 lun)
{
   ...
   rc = ufs_scsi_req_sense (hufs, lun);
   if (rc != EOK) {
      ufs_free_handle (hufs); 
      return NULL;
   }
...
int32 ufs_set_config (struct ufs_handle *handle, ufs_config_mode mode)
{
   ...
   rc = ufs_do_finish_config (handle, mode); 
   return rc; 
}
                            ...
                            struct ufs_lun_config_descr *cfg )
{
   ...
   uint8 i, lun_to_grow = 0, num_luns = 0, num_hpb_luns = 0;
   uint16 active_hpb_regions = 0; 
   ...
   rc = ufs_set_bRefClkFreq (handle, handle->phc->plat_cfg.refclk_freq);
   if (rc != EOK) {
      return rc;
   }
   ...
   for (i = 0; ((i < num_luns) && (alloc_units_to_create <= alloc_units_total) && 
                ...
                (active_hpb_regions <= dev_info.wDeviceMaxActiveHPBRegions)); i++) {

      if (cfg->unit[i].bLUEnable == 2) {
         if (num_hpb_luns == dev_info.bHPBNumberLU) {
            snprintf(log_buffer, 64, "Maximum number of HPB LUNs (%d) reached", 
                     dev_info.bHPBNumberLU);
            rc = -EINVAL; 
            goto ConfigLog;
         }

         // wLUMaxActiveHPBRegions must be greater than or equal to wNumHPBPinnedRegions
         if (cfg->unit[i].wLUMaxActiveHPBRegions < cfg->unit[i].wNumHPBPinnedRegions) {
            snprintf(log_buffer, 64, "wNumHPBPinnedRegions (%d) is larger than wLUMaxActiveHPBRegions (%d)", 
                     cfg->unit[i].wNumHPBPinnedRegions, cfg->unit[i].wLUMaxActiveHPBRegions); 
            rc = -EINVAL; 
            goto ConfigLog;
         }

         active_hpb_regions += cfg->unit[i].wLUMaxActiveHPBRegions; 
         lun_cfg.bHPBControl = cfg->bHPBControl; 

         lun_cfg.unit[i].wLUMaxActiveHPBRegions = cfg->unit[i].wLUMaxActiveHPBRegions; 
         lun_cfg.unit[i].wHPBPinnedRegionStartIdx = cfg->unit[i].wHPBPinnedRegionStartIdx;
         lun_cfg.unit[i].wNumHPBPinnedRegions = cfg->unit[i].wNumHPBPinnedRegions; 
      }
   ...
   // Check if we are over allocating the maximum number of HPB regions
   if (active_hpb_regions > dev_info.wDeviceMaxActiveHPBRegions) {
      snprintf(log_buffer, 64, 
               "Specified active HPB regions %d exceeds maximum HPB regions %d",
               active_hpb_regions, dev_info.wDeviceMaxActiveHPBRegions); 
      rc = -EINVAL; 
      goto ConfigLog; 
   }
...
int32 ufs_purge_and_wait( struct ufs_handle *handle)
{
   ...
   rc = ufs_set_n_wait_fPurgeEnable (handle, FALSE);
   return rc;
}
...
int32 ufs_purge_nonblock_setup (struct ufs_handle *handle)
{
   ...
   rc = ufs_set_n_wait_fPurgeEnable (handle, TRUE);
   return rc;
}
                ...
                uint32           blocks)
{
   ...
   rc = ufs_scsi_read (handle, buff, lba, blocks, NULL, NULL);
                 ...
                 uint32             blocks)
{
   ...
   // Call the SCSI command building function
   rc = ufs_scsi_write (handle, buff, lba, blocks, NULL, NULL);
                   ...
                   struct ufs_crypto_context *crypt_context)
{
   ...
   // Call the SCSI command building function
   rc = ufs_scsi_read (handle, buff, lba, blocks, xfr_context, crypt_context); 
   return rc;
}
                    ...
                    struct ufs_crypto_context *crypt_context )
{
   ...
   // Call the SCSI command building function
   rc = ufs_scsi_write (handle, buff, lba, blocks, xfr_context, crypt_context);
   return rc;
}
...
/* Public API */
int32 ufs_sync_cache (struct ufs_handle *handle, uint32_t lba, uint32_t blocks)
{
   int32 rc = EOK;
   ...
   // Call the SCSI command building function
   rc = ufs_scsi_sync_cache (handle, lba, blocks);

   STOR_PROFILE_END(UFS_PROFILE_FLUSH_BIT_MASK, "UFS_FLUSH", lba, blocks);
...
int32 ufs_poll_xfer (struct ufs_handle *handle, struct ufs_xfr_context *xfr_context)
{
   int32 rc = EOK;
   // Check input parameter
   if ((handle == NULL) || (xfr_context == NULL)) {
      return -EINVAL;
   }
   ...
   rc = ufs_finish_pending_xfer (handle, xfr_context);
   return rc;
}
...
int32 ufs_get_error_statistics (struct ufs_handle *handle, struct ufs_error_stats *err_info) 
{
   ...
   ufs_fill_in_error_stats (handle, err_info); 

   return EOK; 
}

// Get operating mode capabilities 
int32 ufs_get_operational_mode_capabilities (struct ufs_handle *handle, void *caps)
{
   ...
   ufs_fill_in_operation_mode_caps (handle, caps); 

   return EOK;  
}

// Get current UFS operational mode
                                             ...
                                             struct ufs_current_operational_mode_info *curr_op_mode)
{
   ...
   ufs_fill_in_current_operating_mode (handle, curr_op_mode);  
   
   return EOK; 
}

// Get information on whether switch to turbo mode frequency is enabled or not. 
int32 ufs_get_turbofreq_diff_info (struct ufs_handle *handle, uint8_t *diff_turbo_support)
{
   ...
   *diff_turbo_support = hba->plat_cfg.core_turbo_switch;

   return EOK; 
}
// Configure UFS gear and voltage corner
int32 ufs_set_operational_mode (struct ufs_handle *handle, uint8_t volt_corner, uint8_t gear)
{
   ...
   // If both the volt_corner and gear match the current configuration, do nothing
   if ((handle->phc->curr_volt_corner == volt_corner) && (handle->phc->curr_speed_config.gear == gear)) {  
      return EOK; 
   }

   // Check to see if the volt_corner and gear combination is valid and adjust 
   // accordingly if valid
   rc = ufs_do_operational_mode (handle, volt_corner, gear);
   
   return rc;
}

// Set UFS speed to the gear requested 
int32 ufs_set_speed (struct ufs_handle *handle, uint8_t gear)
{
   ...
   return ufs_set_operational_mode (handle, handle->phc->curr_volt_corner, gear); 
}

// Configure UFS to the indicated voltage corner
int32 ufs_configure_volt_corner (struct ufs_handle *handle, uint8_t volt_corner)
{
   ...
   return ufs_set_operational_mode (handle, volt_corner, handle->phc->curr_speed_config.gear);
}

// Put UFS link in hibernate (manual)
int32 ufs_h8 (struct ufs_handle *handle, uint8_t enter)
{
   ...
   rc = ufs_link_h8 (handle, enter);
   return rc; 
}

// Configures UFS link to use auto-hibernation
int32 ufs_autoH8 (struct ufs_handle *handle, uint16_t idle_time_ms)
{
   ...
   ufs_link_autoH8 (handle, idle_time_ms);

   return EOK;
}

// Configures UFS device mode
int32 ufs_set_device_mode (struct ufs_handle *handle, ufs_device_mode Device_Mode)
{
   ...
   rc = ufs_scsi_start_stop_unit (handle, Device_Mode);
...
int32 ufs_suspend (struct ufs_handle *handle)
{
   ...
   rc = ufs_scsi_start_stop_unit (handle, UFS_SLEEP);
   if (rc != EOK) {
      return rc;
   }

   rc = ufs_clock_gate (handle);
   if (rc != EOK) {
      return rc;
   }

   rc = ufs_bsp_volt_set(handle->ufs_core_id, UFS_BSP_REGULATORS_SLEEP);
   if (rc != EOK) {
      return rc;
   }
...
int32 ufs_resume (struct ufs_handle *handle)
{
   ...
   rc = ufs_bsp_volt_set(handle->ufs_core_id, UFS_BSP_REGULATORS_ON);
   if (rc != EOK) {
      return rc;
   }

   rc = ufs_clock_ungate (handle);
   if (rc != EOK) {
      return rc;
   }

   rc = ufs_scsi_start_stop_unit (handle, UFS_ACTIVE);
   if (rc != EOK) {
      return rc;
   }
...
/* Set the Shared Ice algorithm */
int32 ufs_set_shared_ice_algo ( struct ufs_handle *handle, void *algo_info)
{
   ...
   rc = ufs_do_set_shared_ice_algo(handle, algo_info);
...
/* Get the SharedIce algorithm and values programmed. */
int32 ufs_get_shared_ice_algo (struct ufs_handle *handle, void *algo_info)
{
   #if defined (FEATURE_NO_SHARED_ICE)
   return -EPERM;
   #else
   ...
   rc = ufs_do_get_shared_ice_algo(handle, algo_info);
...
int32 ufs_dump_registers(struct ufs_handle *handle, ufs_register_masks reg_mask)
{
   int32 rc = EOK;
   ...
   rc = ufs_do_dump_registers(handle, reg_mask);
   
   return rc; 
}

// Get WriteBooster available buffer size
int32 ufs_get_wb_avail_buffer (struct ufs_handle *handle, UINT32 *AvailBufferSize)
{     
   int32 rc = EOK;
   
   rc = ufs_get_bAvailableWriteBoosterBufferSize(handle, AvailBufferSize);
   
   return rc; 
}

// Get WriteBooster current buffer size
int32 ufs_get_wb_cur_buffer (struct ufs_handle *handle, UINT32 *CurBufferSize)
{     
   int32 rc = EOK;
   
   rc = ufs_get_dCurrentWriteBoosterBufferSize(handle, CurBufferSize);
...
void storage_ufs_test(void)
{
  ...
  for (j=0; j<=13;j++) // For different chunk sizes
  {
    ...
    STOR_PROFILE_END(STOR_PROFILE_BENCHMARK_BIT_MASK, "UFS_WRITE", chunk_in_sectors[j], try_count);
...
{
   return ufs_get_descriptor_device_raw(hufs, buf, size);
}
...
{
   return ufs_get_descriptor_unit_raw(hufs, lun, buf, size);
}
#endif /* FEATURE_XBOOT */
