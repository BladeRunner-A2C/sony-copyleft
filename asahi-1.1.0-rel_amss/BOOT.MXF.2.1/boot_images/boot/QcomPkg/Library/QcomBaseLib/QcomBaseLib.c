 ...
 *
 * Copyright (c) 2011, 2013-2017,2021 by Qualcomm Technologies, Inc.
 * All Rights Reserved.
 *
 *********************************************************************/
/*=======================================================================
...
CHAR8* ExtractModuleName (IN CHAR8* ImgPathPtr, IN CHAR8* ModuleNamePtr)
{
  ...
  AsciiStrCpyS (ModuleNamePtr, Len + 1, ModulePtr);
...
VOID SetIframePtrToInfoBlkPtr ( UINT32 CpuNum, VOID *IframePtr )
{
  UefiInfoBlkType  *UefiInfoBlkPtr;

  if (IframePtr == NULL || CpuNum >= SMP_MAX_CPUS)
    return;

  UefiInfoBlkPtr = (UefiInfoBlkType*)GetInfoBlkPtr();
  if (UefiInfoBlkPtr == NULL)
    return;

  UefiInfoBlkPtr->IframePtr[CpuNum] = (UINT64)IframePtr;
}
