/** @file FwPayloadEntry.h
  ...
  Copyright (c) 2012-2013,2015-2019,2021,2023 Qualcomm Technologies, Inc.  All Rights
...
/*=============================================================================
 ...
 2021/09/24     dmohan      Fixed FWUpdate logging issue for BACKUP_XBL/XBL_CONFIG
 2021/02/13     dmohan      Changes to support A/B Recovery feature
 2019/03/13     vkasilya    Debug messages changed appropriately.
 ...
 2018/09/26     prema       Changes to skip any SysFwVersion FwEntry present
                            in payload during FwEntry validation process
...
/*FwEntry LinkedList */
FW_LINKED_LIST_TYPE         FwEntryList         = {0, 0, 0, 0, NULL,FwEntryDestructor};
    ...
    )
{
    ...
    if(pFwEntry->Metadata.UpdatePath.DiskPartitionType < FWENTRY_DISK_PARTITION_MAX)
    {
            ...
            (pFwEntry->Metadata.UpdatePath.DiskPartitionType == FWENTRY_UFS_LUN2)))
        {
            ...
            else
            {
                DEBUG((
                    DebugLevel,
                    "        DiskPartitionType  = 0x%x,{Unknown}\n",
    ...
    {
        DEBUG((DebugLevel,"        FwEntry Encrypted = True\n"));
    }
    else
    {
        DEBUG((DebugLevel,"        FwEntry Encrypted = False\n"));
    }

    if (DebugLevel == EFI_D_ERROR)
    {
        DEBUG((DebugLevel,"        Buffer address    = 0x%x\n", pFwEntry->FwEntryBuffer));
        DEBUG((DebugLevel,"        Buffer size       = 0x%x\n", pFwEntry->FwEntrySize));
        DEBUG((DebugLevel,"\n      Handle address\n"));
    ...
    )
{
    ...
    if(pFwEntry->FwEntryBuffer != NULL)
    {
        /*The decrypted buffer is always uncached*/ // Giri: Remove this comment.
        gBS->FreePool(pFwEntry->FwEntryBuffer);
        pFwEntry->FwEntryBuffer = NULL;
    }
    ...
    )
{
    ...
    case FWENTRY_UPDATE_TYPE_PARTITION:
        /* Partitions are kept in sync prior to FW update proccess.
           So, skipping backup for all update types, except custom firmware.
        */
        FwEntry->BackupAlreadyDone = TRUE;
    ...
    case FWENTRY_UPDATE_TYPE_OPM_PRIV_KEY:
        ...
        // Configure storage disk contains partitions marked for FwUpdate.
        Status = SetStorageDiskNeedsSwitchGpt(hTemp);
        HANDLE_ERROR_LABEL (Status, SetStorageDiskNeedsSwitchGpt, FwEntrySetPartitionHandlesExit);

        break;

    case FWENTRY_UPDATE_TYPE_CUSTOM_FIRMWARE1:
        /* Destination partition location is not applicable to this update type 
           as it would be on external HW. So, marking hTemp handle to NULL.
        */
        hTemp = NULL;
        break;
    ...
    case FWENTRY_BACKUP_TYPE_FAT_FILE:
        ...
        goto FwEntrySetPartitionHandlesExit;
        break;
    case FWENTRY_BACKUP_TYPE_CUSTOM_FIRMWARE1:
        /* Backup location of this backup type is not partition/FAT file system. 
           So, marking hTemp handle to NULL.
        */
        hTemp = NULL;
        break;
     ...
     * bootable partition handler between LUN1 and LUN2.
     */
    if((boot_from_ufs()) &&
       ((FwEntry->Metadata.UpdatePath.DiskPartitionType == FWENTRY_UFS_LUN1) ||
        (FwEntry->Metadata.UpdatePath.DiskPartitionType == FWENTRY_UFS_LUN2)))
    {
        ...
        /* Dest and backup partition handles need to swapped only when destination
         * partition GUID is not same as boot partition GUID.
         */
    ...
    /* If both update and back type are partition and EMMC/LUN type of update and
       ...
       to FW_STATE_SYNC_START or onward, and the GPT table has been
       switched, then exchange the destination handle and backup handle.
    */
    ...
    if((FwEntry->Metadata.UpdatePath.DiskPartitionType == FwEntry->Metadata.BackupPath.DiskPartitionType) &&
       ...
       (State >= FW_STATE_SYNC_START))
    {
        Status = GetStorageDiskInfoFromPartHandle(
            FwEntry->Handle[FWENTRY_OPERATION_PATH_TYPE_DEST],
            &pDiskInfo);
        HANDLE_ERROR_LABEL(Status,GetStorageDiskInfoFromPartHandle,FwEntrySetPartitionHandlesExit);

        DEBUG((EFI_D_INFO, "Sync started previously. Call CmpGPTTable\n"));
        Status = CmpGPTTable(
            BACKUP_GPT_TABLE,
            pDiskInfo->guid,
            pDiskInfo->nvType,
        ...
        DEBUG((EFI_D_INFO, "GptState: %d\n", pDiskInfo->gptState));

        if(pDiskInfo->gptState == SWITCHED_TABLE)
        {
            hTemp = FwEntry->Handle[FWENTRY_OPERATION_PATH_TYPE_BACKUP];
    ...
    )
{
    ...
    *QcResultFlag = QCFW_UPDATE_INIT;
    ...
    case FWENTRY_UPDATE_TYPE_OPM_PRIV_KEY:
        ...
        Status = FWCryptoOpmPrivKey(FwEntry->FwEntryBuffer,FwEntry->FwEntrySize,&DecryptedBuf,&DecryptedBufSize);
        if (EFI_ERROR(Status))
        {
    ...
    {
        FwEntry->FwEntryBuffer = DecryptedBuf;
        FwEntry->FwEntrySize = DecryptedBufSize;
    }
    ...
    )
{
  ...
  *bEntryGood = FALSE;
  ...
  if(mData->Operation == FWENTRY_OPERATION_TYPE_UPDATE)
  {
    ...
    else if(mData->UpdateType == FWENTRY_UPDATE_TYPE_CUSTOM_FIRMWARE1)
    {
      mData->UpdatePath.DiskPartitionType = FWENTRY_DISK_PARTITION_DONT_CARE;
      ZeroMem(
        mData->UpdatePath.PartitionName,
        (PARTITION_NAME_MAX_CHAR16_SIZE * sizeof(CHAR16))
        );
      ZeroMem(
        &mData->UpdatePath.PartitionTypeGUID,
        sizeof(EFI_GUID)
        );
    }
    else
    ...
    else if (mData->BackupType == FWENTRY_BACKUP_TYPE_CUSTOM_FIRMWARE1)
    {
      mData->BackupPath.DiskPartitionType = FWENTRY_DISK_PARTITION_DONT_CARE;
      ZeroMem(
        mData->BackupPath.PartitionName,
        (PARTITION_NAME_MAX_CHAR16_SIZE * sizeof(CHAR16))
        );
      ZeroMem(
        &mData->BackupPath.PartitionTypeGUID,
        sizeof(EFI_GUID)
        );
    }
    else
    ...
    )
{
  ...
  *bEntryGood = FALSE;
  ...
  if((CompareGuid((GUID*)&mData->UpdatePath.PartitionTypeGUID, &gSystemFwVersionPartitionGuid)) ||
     (StrnCmp(mData->UpdatePath.PartitionName, SYSFW_VERSION_PARTITION_NAME, StrLen(SYSFW_VERSION_PARTITION_NAME)) == 0))
  {
    Status = EFI_UNSUPPORTED;  
    DEBUG_ERR_LABEL("\nUnsupported PartitionName/PartitionGuid detected\n",FwEntryValueCheckingExit);
  }
               ...
               (pFwEntry->Metadata.Operation < FWENTRY_OPERATION_TYPE_MAX))
            {
                ...
                if (EFI_ERROR(Status))
                {
                    *QcResultFlag = QCFW_UPDATE_ERR_FWENTRY_READ_ENTRY_FROM_FV;
                    HANDLE_ERROR_LABEL(Status,FwUpdateReadEntryFromFV,FwPayloadV3ParserExit);
                }
