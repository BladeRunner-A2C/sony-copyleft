/** @file PowerCheck.c
  ...
  Copyright (c) 2012, 2013, 2017, 2022-2023 Qualcomm Technologies, Inc.  All Rights Reserved.
...
/*=============================================================================
 ...
 --------       ---         --------------------------------------------------
 2023/02/22     dmohan      Enabled powercheck for fwupdate
 2023/01/02     dmohan      Added BU hack to disable powercheck for all device types
...
// Added flag to enable workarounds only needed for BU.
// 1 - Enable Hack; 0 - Disable Hack
#define ENABLE_BU_HACK              0
...
)
{
  ...
  if((PlatformType == EFI_PLATFORMINFO_TYPE_MTP) ||
     ...
     (PlatformType == EFI_PLATFORMINFO_TYPE_QRD) ||
     (PlatformType == EFI_PLATFORMINFO_TYPE_CRD))
  {
    *bPowerGood = TRUE;
    goto power_exit;    
  }
#endif //#if ENABLE_BU_HACK
