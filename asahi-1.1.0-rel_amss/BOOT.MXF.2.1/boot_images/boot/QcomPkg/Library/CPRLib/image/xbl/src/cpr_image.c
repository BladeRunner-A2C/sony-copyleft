 ...
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/CPRLib/image/xbl/src/cpr_image.c#1 $
 * $Change: 53083464 $
 */
#include <stdlib.h>
#include "cpr_image.h"
...
void cpr_image_open_remote_cfg(void** cfg, uint32* size)
{
    *cfg = smem_get_addr( SMEM_CPR_CONFIG, size );
    ...
    if(!(*cfg))
    {
        *size = 0x4000;     //Reserve 16K for now
        *cfg = smem_alloc(SMEM_CPR_CONFIG, *size);
        ...
        msg_ram_set_smem_address((uint32)SMEM_CPR_CONFIG, (uint32)(uintptr_t)*cfg);
    }
...
{
    cpr_mode_settings *modeSettings = NULL;
    cpr_submode_settings *subModeSettings = NULL;

    cpr_domain_id domain = cpr_image_target_railway_rail_to_cpr_domain( railwayRailId );
    cpr_voltage_mode mode = cpr_image_target_railway_corner_to_cpr_mode(railwayCorner);
    cpr_domain_info info = {CPR_DOMAIN_TYPE_MODE_BASED, {mode}};

    //Railway driver calls this wrapper as part of the voltage transition.
    //Update the active mode state in CPR driver to match what Railway driver is doing.
    //This updated data is utilized when sending the data via SMEM to other images.
    cpr_utils_get_mode_settings(domain, &info, &modeSettings, &subModeSettings);
    cpr_utils_set_active_mode_setting(domain, modeSettings, subModeSettings);
...
{
    cpr_image_target_extra_init();
}

void cpr_image_init_rail_config(cpr_rail *rail)
{
    cpr_image_target_init_rail_config(rail);
}
