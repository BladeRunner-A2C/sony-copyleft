 ...
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/CPRLib/common/src/cpr.c#1 $
 * $Change: 53083464 $
 */
#include "cpr.h"
#include "cpr_logs.h"
...
static void init_cpr_config(void)
{
    ...
    fused_vpId           = cpr_image_decode_vp_id();
    cpr_image_target_parse_bin();
...
static void resume_control(cpr_domain_id railId, cpr_domain_info* info, uint32 voltage)
{
    ...
    cpr_submode_settings* newSubmodeState = NULL;
    cpr_rail* rail = cpr_utils_get_rail( railId );
...
void cpr_init()
{
    if (cpr_info.fullInit == true)
    {
      CPR_LOG_INFO( "Skipping CPR init as the init is already done" );
      return;
    }
...
{
    cpr_submode_settings* modeState = NULL;
