 ...
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/CPRLib/common/src/cpr_measurements.c#1 $
 * $Change: 53083464 $
 */
...
void cpr_measurements_aging(cpr_rail* rail)
{
    ...
    /*
     * Verify mode to run aging. If aging mode is greater than the highest
     * mode then change the aging mode to the highest supported mode.
     */
    agingIdx = cpr_utils_get_mode_idx(rail, agingCfg->modeToRun);
    agingMode = rail->vp->supportedModes[agingIdx];
    if (agingMode != agingCfg->modeToRun)
        CPR_LOG_WARNING("Aging mode changed from %d to %d",
                agingCfg->modeToRun, agingMode);
     ...
     * and not open loop.
     */
    cpr_domain_info info = {CPR_DOMAIN_TYPE_MODE_BASED, {agingMode}};

    CPR_LOG_INFO("Aging init: set %s rail mode to %u", rail->name, agingMode);

    #if defined(LASSEN)
    // Need to vote MX rail to Turbo before voting CX to Turbo to meet the CX-MX dependency of voltages
        if (rail->id == CPR_RAIL_CX)
        {
            cpr_domain_info info_mx = {CPR_DOMAIN_TYPE_MODE_BASED, {CPR_VOLTAGE_MODE_TURBO}};
            if(!cpr_image_set_rail_mode( CPR_RAIL_MX, &info_mx ))
            {
                CPR_LOG_FATAL(" Failed to set %s rail mode to %u", CPR_RAIL_MX, CPR_VOLTAGE_MODE_TURBO);
                return;
            }
        }
    #endif
    ...
    if(!cpr_image_set_rail_mode( rail->id, &info ))
    {
        CPR_LOG_FATAL(" Failed to set %s rail mode to %u", rail->name, agingMode);
        return;
    }
    ...
    #if defined(LASSEN)
    // Need to remove MX rail vote after removing CX vote to meet the CX-MX dependency of voltages
        if (rail->id == CPR_RAIL_CX)
        {
            cpr_domain_info info_mx = {CPR_DOMAIN_TYPE_MODE_BASED, {CPR_VOLTAGE_MODE_OFF}};
            cpr_image_set_rail_mode( CPR_RAIL_MX, &info_mx );
        }
    #endif
