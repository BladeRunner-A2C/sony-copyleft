 ...
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/CPRLib/common/hal/v3/src/cpr_hal.c#1 $
 * $Change: 53083464 $
 *
 */
#include "cpr_hal.h"
...
#define CPRF_DN_TIMER_INTERVAL 0x2590 //500us
...
/*
 * CR 1097288:
 * SENSOR_MASK_WRITE and SENSOR_BYPASS_WRITE registers must be written in
 * one shot without relying on the previously written value.
 *
 * CPR Controller won't update SENSOR_MASK_READ/SENSOR_BYPASS_READ registers
 * until LOOP_EN is set to 1.
 * If we do read-modify-write multiple times when LOOP_EN is 0, MASK/BYPASS
 * setting became unexpected value, causing closed loop voltage to stuck at
 * ceiling voltage and also aging measurement failure
 */
__attribute__((noinline))
static void writeSensorRegisters(uint32 writeAddr, const uint8* bitList,
        uint32 bitListCount, uint32 totalBits, boolean set)
{
...
static void cpr_hal_enable_ro(cpr_hal_handle *hdl, uint32 ro, boolean enable)
{
    ...
    {
        #if defined(LASSEN)
            // Lassen - MX rail clock is 9.6Mhz, hence GCNT needs to be programmed as 9
            if (controller->base == 0x62C000){
                CPR_HWIO_OUT_FIELD( HWIO_CPR_GCNT0_ADDR( controller->base ) + gcntOffset, HWIO_CPR_GCNT0_GCNT0, 9 );
            }
            else{
...
static void cpr_hal_set_ro_mask(cpr_hal_handle *hdl, uint16 roMask)
{
    ...
    if (thread->id <= HWIO_CPR_MASK_THREAD_t_MAXt)
    {
        // Update RO mask field for the thread
        CPR_HWIO_OUT_FIELD(
                HWIO_CPR_MASK_THREAD_t_ADDR(hdl->controller->base,
                    thread->id),
                HWIO_CPR_MASK_THREAD_t_RO_MASK4THREAD, roMask );
    }

    thread->roMask4Thread = roMask;
}

void cpr_hal_set_target_quotient(cpr_hal_handle *hdl, uint32 mode, uint32 ro,
        uint32 quotient)
{
    uint32 targetOffset = (HWIO_CPR_TARGET1_0_m_ADDR( 0, 0 ) -
            HWIO_CPR_TARGET0_0_m_ADDR( 0, 0 )) * ro;

    CPR_LOG_TRACE( " Setting RO %u to %u", ro, quotient );

    CPR_HWIO_OUT_FIELD(
            (HWIO_CPR_TARGET0_0_m_ADDR( hdl->controller->base, mode ) +
                targetOffset),
            HWIO_CPR_TARGET0_0_m_TARGET0, quotient );

    /*
     * NOTE: Setting mode_gcnt does not take effect until CPR_GCNT_PER_MODE_EN
     * is set, which is set along with other HW Closed Loop enablement regs
     * when HW Closed Loop is enabled.
     */
    #if defined(LASSEN)
        // Lassen - MX rail clock is 9.6Mhz, hence GCNT needs to be programmed as 9
        if (hdl->controller->base == 0x62C000){
            CPR_HWIO_OUT_FIELD((HWIO_CPR_TARGET0_0_m_ADDR( hdl->controller->base, mode ) + targetOffset), HWIO_CPR_TARGET0_0_m_MODE_GCNT0, 9 );
        }
        else{
            CPR_HWIO_OUT_FIELD((HWIO_CPR_TARGET0_0_m_ADDR( hdl->controller->base, mode ) + targetOffset), HWIO_CPR_TARGET0_0_m_MODE_GCNT0, CPR_DEFAULT_GCNT );
        }
    #else
        CPR_HWIO_OUT_FIELD((HWIO_CPR_TARGET0_0_m_ADDR( hdl->controller->base, mode ) + targetOffset), HWIO_CPR_TARGET0_0_m_MODE_GCNT0, CPR_DEFAULT_GCNT );
    #endif
...
{
    CPR_HWIO_OUT( HWIO_CPR_IRQ_EN_t_ADDR( hdl->controller->base,
                hdl->thread->id ), 0 );
}

static void cpr_hal_init_thread(cpr_hal_handle* hdl, cpr_hal_thread *thread,
        boolean configureHW)
{
    const uintptr_t base = hdl->controller->base;
    ...
    if(configureHW)
    {
        ...
        if (thread->id <= HWIO_CPR_THRESHOLD_t_MAXt)
        {
...
void cpr_hal_init_controller(cpr_hal_handle* hdl, boolean configureHW)
{
    ...
    if(configureHW)
    {
        ...
        CPR_HWIO_OUT_FIELD(HWIO_CPR_CPRF_DN_TIMER_ADDR(base),
                HWIO_CPR_CPRF_DN_TIMER_DN_TIMER_INTERVAL,
                CPRF_DN_TIMER_INTERVAL);
    }
 ...
 */
void cpr_hal_write_cprf_voltages(uint8 cprf_id, uint32 mode, uint32 vmax,
        uint32 vmin)
{
    uint32 val = 0;

    CPR_ASSERT(cprf_id <= HWIO_RPMH_CPRF_CPRFm_VOLTAGE_LIMITS_n_MAXm);

    val = vmax<<HWIO_RPMH_CPRF_CPRFm_VOLTAGE_LIMITS_n_VMAX_SHFT;
    val=val|(vmin<<HWIO_RPMH_CPRF_CPRFm_VOLTAGE_LIMITS_n_VMIN_SHFT);
    HWIO_RPMH_CPRF_CPRFm_VOLTAGE_LIMITS_n_OUTI2(cprf_id, mode, val);
    HWIO_RPMH_CPRF_CPRFm_SAFE_VOLTAGE_n_OUTI2(cprf_id, mode, vmax);
}

void cpr_hal_enable_cpr_ctrl_interaction( cpr_hal_handle* hdl, uint8 cprf_id, boolean enable )
{
    uint8 val = 0;
    CPR_ASSERT(cprf_id <= HWIO_RPMH_CPRF_CPRFm_CTRL_MAXm);

    HWIO_RPMH_CPRF_CPRFm_CTRL_OUTMI(cprf_id,
            HWIO_RPMH_CPRF_CPRFm_CTRL_CTRL_INTERACTION_EN_BMSK,
            enable<<HWIO_RPMH_CPRF_CPRFm_CTRL_CTRL_INTERACTION_EN_SHFT);

    val = (HWIO_RPMH_CPRF_CPRFm_CTRL_INMI(cprf_id,
            HWIO_RPMH_CPRF_CPRFm_CTRL_CTRL_INTERACTION_EN_BMSK) >>
            HWIO_RPMH_CPRF_CPRFm_CTRL_CTRL_INTERACTION_EN_SHFT);

    if (enable != val)
    {
        CPR_LOG_ERROR("cpr_hal_enable_cpr_ctrl_interaction for CPRF:%d (%d) != %d",
                cprf_id, enable, val);
    }
}

void cpr_hal_enable_gcnt_per_mode(cpr_hal_handle* hdl, boolean enable)
{
    CPR_HWIO_OUT_FIELD((HWIO_CPR_MISC_REGISTER_ADDR(hdl->controller->base)),
            HWIO_CPR_MISC_REGISTER_CPR_GCNT_PER_MODE_EN, enable);
}
...
void cpr_hal_enable_cprf_interface( cpr_hal_handle* hdl, boolean enable )
{
    ...
    if (enable != val)
    {
        CPR_LOG_ERROR("cpr_hal_enable_cprf_interface(%d) != %d",
                enable, val);
    }
}

void cpr_hal_enable_cprf_settle_timer_down(cpr_hal_handle* hdl, boolean enable)
{
    CPR_HWIO_OUT_FIELD((HWIO_CPR_MISC_REGISTER_ADDR(hdl->controller->base)),
            HWIO_CPR_MISC_REGISTER_CPRF_SETTLE_TIMER_DOWN_EN, enable);
}

void cpr_hal_enable_hw_closed_loop( cpr_hal_handle* hdl, uint8 cprf_id)
{
    ...
    cpr_hal_enable_cpr_ctrl_interaction(hdl, cprf_id, true);
    cpr_hal_enable_reset_step_quot_loop_en(hdl, true);
    cpr_hal_enable_gcnt_per_mode(hdl, true);
    cpr_hal_enable_cprf_interface(hdl, true);
    cpr_hal_enable_cprf_settle_timer_down(hdl, true);
...
void cpr_hal_set_vrm_settings( const char* name, uint8 cprf_id)
{
    ...
    CPR_ASSERT(cprf_id <= HWIO_RPMH_CPRF_CPRFm_VRM_SETTINGS_MAXm);

    vrm = cmd_db_query_addr(cpr_image_get_vrm_map(name));
...
void cpr_hal_set_cprf_defaults ( const char* name, uint8 cprf_id )
{
    ...
    //ToDo: read the step settings from VP.
    uint32 CPRFm_STEP_SETTINGS = 0x24;
    uint32 CPRFm_TIMEOUT = 0x4B001; //8msec
    uint32 CPRFm_FORCE_SETTINGS = 0x1;
    uint32 CPRFm_CTRL = 0x1eb9;
    uint32 CPRFm_ON_SETTINGS = 0x21;
...
void cpr_hal_enable_aging(cpr_hal_handle* hdl, const cpr_aging_cfg* aging_cfg)
{
    ...
    /*
     * Mask all the sensors except the sensor which has selected to be aging sensor only
     * if there are no sensors to be bypassed
     */
    if (0 == aging_cfg->bypassSensorIDsCount)
    {
