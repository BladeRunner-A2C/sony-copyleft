 ...
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/CPRLib/common/src/cpr_smem_vp.c#1 $
 * $Change: 53083464 $
 */
#include "cpr_smem.h"
#include "cpr_image.h"
        ...
        cpr_voltage_mode *list, int count)
{
    ...
    uint8* s_modes = (uint8 *) GET_BUFFER();
    INCREMENT_OFFSET((sizeof(uint8) * count));

    for (i = 0; i < count; i++)
        s_modes[i] = (uint8)list[i];
...
static boolean cpr_smem_write_margin_data(cpr_rail_state* state,
        cpr_margin_cfg *margins,int modeIdx, cpr_voltage_mode mode)
{
    int i = 0;
	...
	{
		if( cpr_utils_vp_id_cpr_rev_match_found(fused_vpId, margins->data[i].cprRevMin, margins ))
			vpId = fused_vpId;
		else
			vpId = cpr_utils_get_cpr_rev_max_vp_id(margins->data[i].cprRevMin, margins);
			
	if( vpId >= margins->data[i].vpIdMin && vpId <= margins->data[i].vpIdMax )	
	{		
        ...
        // VBump margins
        #if defined(CPR_FEATURE_VD)
            int32 vBump = cpr_image_get_vbump_margin(rail->id, mode);
            s_data->openLoop += vBump;
            s_data->closedLoop += vBump;
        #endif
...
static boolean cpr_smem_write_margin_cfg(cpr_rail_state* state,
        struct cpr_voltage_data* modes, int modeIdx, cpr_voltage_mode mode)
{
    cpr_rail *rail = cpr_utils_get_rail(state->id);
    CPR_ASSERT(rail);
    ...
    for (int a = 0; a < margins->count; a++) 
	{
		uint8 vpId = 0;
	
		if( cpr_utils_vp_id_cpr_rev_match_found(fused_vpId, margins->data[a].cprRevMin, margins ))
			vpId = fused_vpId;
		else
			vpId = cpr_utils_get_cpr_rev_max_vp_id(margins->data[a].cprRevMin, margins);
 
		if( vpId >= margins->data[a].vpIdMin && vpId <= margins->data[a].vpIdMax ) 
		{
			s_margins->count++;
		}
	}

    s_margins->data = cpr_smem_write_margin_data(state, margins,modeIdx, mode); 
        ...
        cpr_voltage_plan *vp)
{
    ...
    for (i = 0; i < vp->modesCount; i++) {
        ...
        s_modes->margins = cpr_smem_write_margin_cfg(state, modes, i, vp->supportedModes[i]);
...
static void cpr_smem_read_vvps(cpr_settings *settings, uint8 idx)
{
    CPR_ASSERT(settings->railsCount);
    CPR_ASSERT(settings->railVVPs);
    CPR_ASSERT(idx < settings->railsCount);

    cpr_versioned_voltage_plan* vvp = settings->railVVPs[idx];
    CPR_ASSERT(vvp);
...
static void cpr_smem_read_controllers(cpr_settings *settings, uint8 idx)
{
    CPR_ASSERT(settings->controllersCount);
    CPR_ASSERT(settings->controllers);
    CPR_ASSERT(idx < settings->controllersCount);

    cpr_hal_controller *controllers = settings->controllers[idx];
    CPR_ASSERT(controllers);
        ...
        uint8 enablementsCount)
{
    ...
    for (i = 0; i < enablementsCount; i++) {
        ...
        s_enablement->enable = 1;
    }
    return true;
}

static void cpr_smem_read_enablements(cpr_settings *settings, uint8 idx)
{
    int i = 0;

    CPR_ASSERT(settings->enablementsCount);
    CPR_ASSERT(settings->enablements);
    CPR_ASSERT(idx < settings->enablementsCount);

    cpr_enablement* enablement = settings->enablements[idx]; 
    CPR_ASSERT(enablement);
...
static void cpr_smem_read_rails(cpr_settings *settings, uint8 idx)
{
    CPR_ASSERT(settings->railsCount);
    CPR_ASSERT(settings->rails);
    CPR_ASSERT(idx < settings->railsCount);

    cpr_rail *rail = settings->rails[idx];
    CPR_ASSERT(rail);
    ...
    rail->hal.controller = settings->controllers[idx];
    rail->hal.thread = 0x0;
    if (rail->hal.controller)
        rail->hal.thread = settings->controllers[idx]->threads[0];
        ...
        uint8 idx)
{
    ...
    if (s_settings->railVVPs)
        cpr_smem_read_vvps(settings, idx);
    if (s_settings->controllers)
        cpr_smem_read_controllers(settings, idx);
    if (s_settings->rails)
        cpr_smem_read_rails(settings, idx);
    if (s_settings->enablements)
        cpr_smem_read_enablements(settings, idx);
...
{
    CPR_LOG_INFO(" Exporting cpr misc data @ (offset: 0x%x)", *currOffset);

    cpr_smem_misc_data *s_misc_data = (cpr_smem_misc_data *) GET_BUFFER();
    INCREMENT_OFFSET(sizeof(cpr_smem_misc_data));

    s_misc_data->vpid           = fused_vpId;
    s_misc_data->railsCount     = cpr_utils_get_num_rails();
    s_misc_data->agingData      = true;
    
    for (int i=0;i<s_misc_data->railsCount;i++)
    {
        cpr_smem_aging_data *s_aging_data = (cpr_smem_aging_data *) GET_BUFFER();
        INCREMENT_OFFSET(sizeof(cpr_smem_aging_data));
        
        s_aging_data->id = cpr_info.settings->rails[i]->id;
        cpr_rail_state* state = cpr_utils_get_rail_state(s_aging_data->id);
        
        int index = CPR_MARGIN_ADJUSTMENT_AGING;
        if (s_aging_data->id == CPR_RAIL_MX)
        {
            index = CPR_MARGIN_ADJUSTMENT_TEST;
        }
        
        s_aging_data->marginAdjustments = state->marginAdjustments[index];
    }
...
void cpr_smem_serialize_misc_data()
{
    uint32 currOffset = 0;
    uint32 size = 0;
    void* buf;
    ...
    cpr_smem_write_misc_data(hdr, &currOffset);
    
    cpr_image_close_remote_cfg();
}

void cpr_smem_update_misc_data(cpr_domain_id id)
{
    uint32 temp = 0;
    uint32 *currOffset = &temp;
    uint32 size = 0;
    void* buf;
    ...
    for(int i = 0; i < hdr->railCount; i++)
    {
        ...
        INCREMENT_OFFSET(railHdr->size);
    }
    
    cpr_smem_misc_data* misc_data = (cpr_smem_misc_data*) GET_BUFFER();
    INCREMENT_OFFSET(sizeof(cpr_smem_misc_data));
    
    for(int i = 0; i < misc_data->railsCount; i++)
    {
        cpr_smem_aging_data* aging_data = (cpr_smem_aging_data*) GET_BUFFER();
        if(aging_data->id == id)
        {
            cpr_rail_state* state = cpr_utils_get_rail_state(id);
            aging_data->marginAdjustments = state->marginAdjustments[CPR_MARGIN_ADJUSTMENT_AGING];
            
            CPR_LOG_INFO("Found cpr misc data for %d @ (offset: 0x%x), updated to %d", id, *currOffset, aging_data->marginAdjustments);
