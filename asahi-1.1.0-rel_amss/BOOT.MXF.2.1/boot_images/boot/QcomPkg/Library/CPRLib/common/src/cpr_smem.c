 ...
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/CPRLib/common/src/cpr_smem.c#1 $
 * $Change: 53083464 $
 */
#include "cpr_smem.h"
#include "cpr_image.h"
...
void deserialize_settings(cpr_rail* rail, cpr_settings* setting,
        void* buf, uint32 size, uint8 idx)
{
    uint32 currOffset = 0;
    cpr_smem_hdr* hdr = cpr_smem_read_header(buf, &currOffset);
    boolean foundRail = false;
    ...
    for(int i = 0; i < hdr->railCount; i++)
    {
        ...
        if(railHdr)
        {
            ...
            cpr_smem_read_rail_settings(hdr, railHdr, &currOffset, setting, idx);
        ...
        uint8 idx)
{
    ...
    switch(hdr->version)
    {
        case CURRENT_SMEM_CFG_VERSION:
            deserialize_settings(rail, setting, hdr, size, idx);
            break;
        default:
            CPR_LOG_FATAL( "Unsupported serialization version: %u",
                    hdr->version );
