 ...
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/CPRLib/common/src/cpr_rail.c#1 $
 * $Change: 53083464 $
 */
#include <stdbool.h>
#include <string.h>
...
static void cpr_rail_init_rail_config(cpr_rail* rail)
{
    ...
    {
        if(matches_device_version(
                    (const cpr_version**)&vvp->list[i].version, 1))
        {
            rail->vp = vvp->list[i].cfg;
            CPR_LOG_INFO( "Found voltage plan for rail %s: (idx=%d)",
                    rail->name, i );
            break;
        }
    }
    ...
    cpr_image_init_rail_config(rail);

    for(int i = 0; (i < cpr_info.settings->enablementsCount &&
            rail->enablement == NULL); i++)
    {
        ...
        if(e->id == rail->id)
        {
            ...
            {
                rail->enablement = e;
                CPR_LOG_INFO("Found enablement config for rail %s: (idx=%d)",
                        rail->name, i );
                break;
            }
        }
...
static void cpr_rail_init_rail_data(cpr_rail* rail)
{
    ...
    CPR_LOG_INFO("Init data for %s with CPRRev: %d", rail->name,
            railState->railCPRRev);

    if (!rail->enablement->enable)
        return;
        ...
        if (fusedMode)
        {
            ...
            if(!useGlobalFloor)
            {
                ...
                else
                {
                   ...
                   //Updating Floor value same as ceil to avoid huge delta between Vmax and Vmin on Rev0 parts due to ceil uplift change.
					fusedFloor = fusedCeiling;
                }
            }
        }
...
static void cpr_rail_set_offset_fused_voltages(cpr_rail* rail, boolean useGlobalCeiling, boolean useGlobalFloor)
{
    const cpr_voltage_plan *vp = rail->vp;
    const cpr_enablement   *e  = rail->enablement;
    cpr_rail_state *railState = cpr_utils_get_rail_state( rail->id );
    ...
    for(int modeIdx = vp->modesCount-1; modeIdx >= 0; modeIdx--)
    {
        ...
        {
            decodedFuseSteps = cpr_utils_decode_fuse_value(
                    vpMode->fuses->voltOffset, 1, true );
            CPR_LOG_INFO("Decoded TARG_VOLT_OFFSET fuse = %d for mode %d",
                    decodedFuseSteps, vp->supportedModes[modeIdx]);
        }
        else
        {
        ...
        {
            /*
             * Update the mode's decodedFuseSteps with the TARG_VOLT_OFFSET's
             * decodedFuseSteps.
             */
            modeSettings->decodedFuseSteps += decodedFuseSteps;

            /*
             * Calculate the fuseAdjustment from the decodedFuseSteps.
             */
            fuseAdjustment = e->fuseMultiplier * decodedFuseSteps;

            for(int submodeIdx = 0; submodeIdx < modeSettings->subModesCount;
                    submodeIdx++)
            {
                cpr_submode_settings* submode  =
                    &modeSettings->subModes[submodeIdx];

                if(!useGlobalCeiling)
                    submode->ceiling += fuseAdjustment;
                ...
                if(!useGlobalFloor)
                {
                    ...
                    {
                        submode->floor =
                            submode->ceiling - margin->maxFloorToCeil;
                    }
                }

                CPR_LOG_INFO("Offset-Fused open loop voltages for mode %d:\
                        (floor:%u) (ceiling:%u)", vp->supportedModes[modeIdx],
...
static void cpr_rail_interpolate_fused_voltages(cpr_rail* rail)
{
    ...
    for (modeIdx = 0; modeIdx < railState->modeSettingsCount; modeIdx++)
    {
                ...
                subModeIdx++)
        {
            ...
            cpr_rail_find_adj_hi_fused_mode(rail, modeIdx,
                    &modeSettingsHi);
            subModeHi = &modeSettingsHi->subModes[subModeIdx];

            cpr_rail_find_adj_lo_fused_mode(rail, modeIdx,
                    &modeSettingsLo);
...
static void cpr_rail_adjust_fused_voltages(cpr_rail* rail)
{
	...
	#ifdef KUNO
	uint32 mx_lowsvsd1_ceil = 0;
	uint32 mx_lowsvsd1_floor =0;
	#endif
    ...
    for(int modeIdx = 0; modeIdx < vp->modesCount; modeIdx++)
    {
        ...
        for(int submodeIdx = 0; submodeIdx < modeSettings->subModesCount; submodeIdx++)
        {
			...
			{
					if(modeSettings->mode > CPR_VOLTAGE_MODE_LOW_SVS_D1 && submode->ceiling < mx_lowsvsd1_ceil)
					{
						submode->ceiling = mx_lowsvsd1_ceil;
						submode->floor = mx_lowsvsd1_floor;
					}
			}
			#endif
            ...
            // VBump margins
                ...
                int32 vBump = cpr_image_get_vbump_margin(rail->id, modeSettings->mode);
                submode->ceiling += vBump;
                submode->floor   += vBump;
            #endif
            
            /*
             * Add the calculate Aging margin to the OL Ceiling and Floor,
             * to compensate for the reduction in Margins when Aging is
             * enabled.
             * For Cl rails with Aging disabled, Aging margin is 0.
             * For OL rail, Aging margin is 0.
             */
            if(railState)
            {
                submode->ceiling +=
			...
			#ifdef KUNO
			if(rail->id == CPR_RAIL_MX && modeSettings->mode == CPR_VOLTAGE_MODE_LOW_SVS_D1)
				{
					mx_lowsvsd1_ceil = submode->ceiling;
					mx_lowsvsd1_floor = submode->floor;
				}
			#endif
...
void cpr_rail_set_initial_voltages(cpr_rail* rail, boolean useGlobalCeiling, boolean useGlobalFloor)
{
    ...
    /*
     * Calcualte fused voltage for the intermediate modes
     * (i.e: Modes without TARG_VOLT fuse) using Open Loop Interpolation.
     */
    cpr_rail_interpolate_fused_voltages(rail);

    /*
     * Decode TARG_VOLT_OFFSET, Calculate its adjustment and Add the adjustmet
     * to the respective mode's OL ceiling and recalc the floor.
     */
    cpr_rail_set_offset_fused_voltages(rail, useGlobalCeiling, useGlobalFloor);
...
void cpr_rail_calculate_target_quotients(cpr_rail* rail, cpr_rail_state* state)
{
    ...
    for(int i = 0; i < vp->modesCount; i++)
    {
        ...
        // VBump margins
        #if defined(CPR_FEATURE_VD)
            int32 vBump = cpr_image_get_vbump_margin(rail->id, vp->supportedModes[i]);
            margin_uv += vBump;
        #endif
...
void cpr_rail_write_all_voltages(cpr_rail* rail)
{
    ...
    cpr_rail_state* railState = &cpr_info.railStates[rail->railIdx];
    uint8 cprf_id = cpr_image_get_cprf_id(rail);

    /* ToDo: Need a better way to factor in the right mode with the right hlvl.
     * Set the voltages for modes starting @ index 1, To account for OFF.
    ...
    for(i = 0; i < railState->modeSettingsCount; i++) {
        ...
        CPR_LOG_TRACE( "For %s(%d) mode=%d: Set vmax=%d vmin=%d", rail->name, cprf_id,
                i+1, submodeState->ceiling, submodeState->floor );
        cpr_hal_write_cprf_voltages(cprf_id, i+1, submodeState->ceiling/1000,
     ...
     * increment it by 1 to account for OFF.
     */
    for(i+=1; i<16; i++) {
        CPR_LOG_TRACE( "For %s(%d) mode=%d: Set vmax=%d vmin=%d", rail->name, cprf_id,
                i, submodeState->ceiling, submodeState->floor );
        cpr_hal_write_cprf_voltages(cprf_id, i, submodeState->ceiling/1000,
                submodeState->floor/1000);
    }
}
...
void cpr_rail_write_all_target_quotients(cpr_rail* rail)
{
    ...
    /*
     * HW Workaround: Set quotients for all RO's of Mode 0 to 0xfff
     */
    for (j = 0; j < 16; j++) {
     ...
     * hlvl. Set the voltages for modes starting @ index 1, To account for
     * OFF. Assuming here that we will always have modes and there
    ...
    for(i = 0; i < railState->modeSettingsCount; i++) {
        ...
        for(j = 0; j < modeState->targetsCount; j++) {
            ...
            CPR_LOG_TRACE( "For %s mode=%d: Set ro=%d quot=%d", rail->name,
                    i+1, target->ro, target->quotient );
            cpr_hal_set_target_quotient(&rail->hal, i+1,
                    target->ro, target->quotient);
        }
    }
}

void cpr_rail_enable_hw_closed_loop( cpr_rail *rail, uint8 cprf_id )
{
    CPR_LOG_TRACE( "Prepare and Enable HW Closed Loop for %s", rail->name);
    cpr_hal_enable_hw_closed_loop(&rail->hal, cprf_id);
}
