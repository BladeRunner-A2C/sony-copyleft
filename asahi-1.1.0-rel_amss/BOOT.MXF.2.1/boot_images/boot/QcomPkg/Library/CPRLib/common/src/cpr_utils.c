 ...
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/CPRLib/common/src/cpr_utils.c#1 $
 * $Change: 53083464 $
 */
...
const boolean cpr_utils_vp_id_cpr_rev_match_found(const uint8 fused_vpId, const uint8 railCPRRev, const cpr_margin_cfg* marginList)
{
	if(marginList != NULL)
	{
		for(int i = 0; i < marginList->count; i++)
		{
			if ( (fused_vpId >= marginList->data[i].vpIdMin && fused_vpId <= marginList->data[i].vpIdMax) &&
				 (railCPRRev >= marginList->data[i].cprRevMin && railCPRRev <= marginList->data[i].cprRevMax) )
			{
				return true;
			}
		}
...
const uint8 cpr_utils_get_cpr_rev_max_vp_id(const uint8 cprRev, const cpr_margin_cfg* marginList)
{
	uint8 vpId = 0;
	if(marginList != NULL)
	{
		for(int k = marginList->count - 1; k >= 0; k--)
		{
			if(cprRev >= marginList->data[k].cprRevMin && cprRev <= marginList->data[k].cprRevMax)
			{
				vpId = marginList->data[k].vpIdMax;
				break;
			}	
		}
	}
...
cpr_margin_data* cpr_utils_get_margins(const uint8 railCPRRev, const cpr_margin_cfg* marginList)
{
    ...
    if(marginList != NULL)
    {
		if(cpr_utils_vp_id_cpr_rev_match_found(fused_vpId, railCPRRev, marginList))
			vpId = fused_vpId;
		else
			vpId = cpr_utils_get_cpr_rev_max_vp_id(railCPRRev, marginList);			
		
        for(int i = 0; i < marginList->count; i++)
        {
            if ( (vpId >= marginList->data[i].vpIdMin && vpId <= marginList->data[i].vpIdMax) &&
			     (railCPRRev >= marginList->data[i].cprRevMin && railCPRRev <= marginList->data[i].cprRevMax) )
			{
