                                            ...
                                            uint32                      uFlags)
{
 ...
 /****************************************************************************
*
** FUNCTION: HAL_MDP_Validate_CWBPingPongId()
*/
/*!
* \brief
*     Validates that CWB Ping Pong Id is within allowed range
*
* \param [in] eCWBPingPongId    - CWB pingpong block ID (none, 0, 1, 2)
*
* \retval HAL_MDSS_ErrorType
*
****************************************************************************/
static HAL_MDSS_ErrorType HAL_MDP_Validate_CWBPingPongId(HAL_MDP_CWB_PingPongId  eCWBPingPongId)
{
  HAL_MDSS_ErrorType   eStatus = HAL_MDSS_STATUS_SUCCESS;

  if (HAL_MDP_CWB_PINGPONG_MAX <= eCWBPingPongId)
  {
...
/****************************************************************************
*
** FUNCTION: HAL_MDP_Get_CWB_PingPong_BaseOffset()
*/
/*!
* \brief
*     Returns offset of the cwb pingpong block
*
* \param [in] eCWBPingPongId    - cwb pingpong block ID (none, 0, 1, 2)
*
* \retval uintPtr
*
****************************************************************************/
static uintPtr HAL_MDP_Get_CWBPingPong_BaseOffset(HAL_MDP_CWB_PingPongId  eCWBPingPongId)
{
  uintPtr   uRegOffset = 0x0;

  if (HAL_MDSS_STATUS_SUCCESS == HAL_MDP_Validate_CWBPingPongId(eCWBPingPongId))
  {
    uRegOffset = uMDPCWBPingPongRegBaseOffset[eCWBPingPongId];
  }
...
/****************************************************************************
*
** FUNCTION: HAL_MDP_WB_ROIConfig()
*/
/*!
* \brief
*     WB ROI configuration
*
* \param [in] sROIConfig - ROI config info
*
* \retval NONE
*
****************************************************************************/
static void  HAL_MDP_WB_ROIConfig(HAL_MDP_DataPath_ROIConfigType* sROIConfig)
{
  uint32 uCropOffset = 0;
  uint32 uCropEnable = (TRUE == sROIConfig->bCropEnabled) ? 1 : 0;

  // crop ctrl enable
  out_dword(HWIO_MDP_WB_2_CROP_CTRL_ADDR, HWIO_OUT_FLD(0, MDP_WB_2_CROP_CTRL, ENABLE, uCropEnable));

  if (TRUE == sROIConfig->bCropEnabled)
  {
    uCropOffset = HWIO_OUT_FLD(uCropOffset, MDP_WB_2_CROP_OFFSET, START_X, sROIConfig->uPosX);
    uCropOffset = HWIO_OUT_FLD(uCropOffset, MDP_WB_2_CROP_OFFSET, START_Y, sROIConfig->uPosY);

    //set up crop offset
    out_dword(HWIO_MDP_WB_2_CROP_OFFSET_ADDR, uCropOffset);
  }
}
...
/****************************************************************************
*
** FUNCTION: HAL_MDP_CWB_PingPong_DitherConfig()
*/
/*!
* \brief
...
*
****************************************************************************/
static HAL_MDSS_ErrorType HAL_MDP_CWB_PingPong_DitherConfig(HAL_MDP_CWB_DitherConfigType* psCWBDitherConfig)
{
  HAL_MDSS_ErrorType eStatus         = HAL_MDSS_STATUS_SUCCESS;
  ...
  if (HAL_MDSS_STATUS_SUCCESS == (eStatus = HAL_MDP_Validate_CWBPingPongId(psCWBDitherConfig->eCWBPingPongId)))
  {
    // Extract the base address to program the HW registers
    uRegisterOffset = HAL_MDP_Get_CWBPingPong_BaseOffset(psCWBDitherConfig->eCWBPingPongId);

    uDsppOpModeInfo = in_dword(HWIO_MDP_PPB_CWB_0_DITHER_OP_MODE_ADDR + uRegisterOffset);

    // Dither is only for RGB destination with lower color depth than RGB888
    if (TRUE == psCWBDitherConfig->psPingPongDitherConfig->bEnable)
    {

      uDitherBitDepth = HWIO_OUT_FLD(uDitherBitDepth,
                                     MDP_PPB_CWB_0_DITHER_BITDEPTH,
                                     C0_BITS,
                                     psCWBDitherConfig->psPingPongDitherConfig->sDitherBitDepth.uColor0);

      uDitherBitDepth = HWIO_OUT_FLD(uDitherBitDepth,
                                     MDP_PPB_CWB_0_DITHER_BITDEPTH,
                                     C1_BITS,
                                     psCWBDitherConfig->psPingPongDitherConfig->sDitherBitDepth.uColor1);

      uDitherBitDepth = HWIO_OUT_FLD(uDitherBitDepth,
                                     MDP_PPB_CWB_0_DITHER_BITDEPTH,
                                     C2_BITS,
                                     psCWBDitherConfig->psPingPongDitherConfig->sDitherBitDepth.uColor2);

      if (psCWBDitherConfig->psPingPongDitherConfig->eDitherType == HAL_MDP_DITHER_TYPE_TEMPORAL)
      {
        uDitherBitDepth = HWIO_OUT_FLD(uDitherBitDepth,
                                       MDP_PPB_CWB_0_DITHER_BITDEPTH,
                                       TEMPORAL_DITHER_EN,
      ...
      out_dword(HWIO_MDP_PPB_CWB_0_DITHER_BITDEPTH_ADDR + uRegisterOffset, uDitherBitDepth);

      // Dither matrix
      if (NULL != psCWBDitherConfig->psPingPongDitherConfig->pDitherMatrix)
      {
        out_dword(HWIO_MDP_PPB_CWB_0_DITHER_MATRIX_ROW0_ADDR + uRegisterOffset, psCWBDitherConfig->psPingPongDitherConfig->pDitherMatrix->uRow0Value);    // ROW#0
        out_dword(HWIO_MDP_PPB_CWB_0_DITHER_MATRIX_ROW1_ADDR + uRegisterOffset, psCWBDitherConfig->psPingPongDitherConfig->pDitherMatrix->uRow1Value);    // ROW#1
        out_dword(HWIO_MDP_PPB_CWB_0_DITHER_MATRIX_ROW2_ADDR + uRegisterOffset, psCWBDitherConfig->psPingPongDitherConfig->pDitherMatrix->uRow2Value);    // ROW#2
        out_dword(HWIO_MDP_PPB_CWB_0_DITHER_MATRIX_ROW3_ADDR + uRegisterOffset, psCWBDitherConfig->psPingPongDitherConfig->pDitherMatrix->uRow3Value);    // ROW#3
      }

      uDsppOpModeInfo = HWIO_OUT_FLD(uDsppOpModeInfo,
                                     MDP_PPB_CWB_0_DITHER_OP_MODE,
                                     DITHER_EN,
                                     1);
    }
    else
    ...
    out_dword(HWIO_MDP_PPB_CWB_0_DITHER_OP_MODE_ADDR + uRegisterOffset, uDsppOpModeInfo);
  }
  return eStatus;
}
...
/****************************************************************************
*
** FUNCTION: HAL_MDP_ControlPath_CWBConfig()
*/
/*!
* \brief
*     Control path configuration
*
* \param [in] HAL_MDP_CWBConfigType       - CWB configs
*
* \retval HAL_MDSS_ErrorType
*
****************************************************************************/
HAL_MDSS_ErrorType  HAL_MDP_ControlPath_CWBConfig(HAL_MDP_CWBConfigType* pCWBConfig)
{
  HAL_MDSS_ErrorType eStatus = HAL_MDSS_STATUS_SUCCESS;

  if ((HAL_MDSS_STATUS_SUCCESS != (eStatus = HAL_MDP_Validate_ControlPathId(pCWBConfig->eControlPathId))))
  {
    eStatus = HAL_MDSS_MDP_STATUS_FAILED;
  }
  else if (HAL_MDP_CWB_TAPPOINT_DISCONNECT == pCWBConfig->eTapPoint)
  {
    uintPtr uRegOffset  = HAL_MDP_Get_ControlPath_BaseOffset(pCWBConfig->eControlPathId);
    uint32  uRegVal     = in_dword(HWIO_MDP_CTL_0_WB_ACTIVE_ADDR);

    out_dword(HWIO_MDP_CTL_0_WB_ACTIVE_ADDR  + uRegOffset, HWIO_OUT_FLD(uRegVal, MDP_CTL_0_WB_ACTIVE, WB_2, 0));
    out_dword(HWIO_MDP_CTL_0_CWB_ACTIVE_ADDR + uRegOffset, 0);
  }
  else if (pCWBConfig->eLayerMasterMixerId >= HAL_MDP_LAYER_MIXER_0)
  {
    uintPtr uRegOffset = HAL_MDP_Get_ControlPath_BaseOffset(pCWBConfig->eControlPathId);
    uint32  uMuxSel    = pCWBConfig->eLayerMasterMixerId - HAL_MDP_LAYER_MIXER_0;

    //Setup CDP CTRL reg
    out_dword(HWIO_MDP_WB_2_CDP_CNTL_ADDR, 0xB);

    // Set up WB active reg to trigger writeback path
    out_dword(HWIO_MDP_CTL_0_WB_ACTIVE_ADDR + uRegOffset, HWIO_OUT_FLD(0, MDP_CTL_0_WB_ACTIVE, WB_2, 0x1));

    // Setting for triggering concurrent writeback using CWB_PP 01
    out_dword(HWIO_MDP_WB_2_MUX_ADDR, HWIO_OUT_FLD(0, MDP_WB_2_MUX, SEL, 0xd));

    // Configure CWB active, mux and mode for the CWB data path
    if (1 == pCWBConfig->uNumMixers)
    {
      out_dword(HWIO_MDP_MERGE_3D_CWB_0_MODE_ADDR, HWIO_OUT_FLD(0, MDP_MERGE_3D_CWB_0_MODE, ENABLE, 0x0));

      if (0 == (uMuxSel & 0x1))
      {
        out_dword(HWIO_MDP_CTL_0_CWB_ACTIVE_ADDR + uRegOffset, HWIO_OUT_FLD(0, MDP_CTL_0_CWB_ACTIVE, CWB_0, 0x1));
        out_dword(HWIO_MDP_CWB_0_MUX_ADDR, HWIO_OUT_FLD(0, MDP_CWB_0_MUX, SEL, uMuxSel));
        out_dword(HWIO_MDP_CWB_0_MODE_ADDR, HWIO_OUT_FLD(0, MDP_CWB_0_MODE, TAP, pCWBConfig->eTapPoint));
      }
      else
      {
        out_dword(HWIO_MDP_CTL_0_CWB_ACTIVE_ADDR + uRegOffset, HWIO_OUT_FLD(0, MDP_CTL_0_CWB_ACTIVE, CWB_1, 0x1));
        out_dword(HWIO_MDP_CWB_1_MUX_ADDR, HWIO_OUT_FLD(0, MDP_CWB_1_MUX, SEL, uMuxSel));
        out_dword(HWIO_MDP_CWB_1_MODE_ADDR, HWIO_OUT_FLD(0, MDP_CWB_1_MODE, TAP, pCWBConfig->eTapPoint));
      }
    }
    else if (2 == pCWBConfig->uNumMixers)
    {
      uint32 uActiveCWBBits = 0;
      uint32 uCWBMode       = 0;

      // Packing is horizontal row interleave
      uCWBMode = HWIO_OUT_FLD(uCWBMode, MDP_MERGE_3D_CWB_0_MODE, ENABLE, 0x1);
      uCWBMode = HWIO_OUT_FLD(uCWBMode, MDP_MERGE_3D_CWB_0_MODE, PACK, 0x1);

      out_dword(HWIO_MDP_MERGE_3D_CWB_0_MODE_ADDR, uCWBMode);

      // Configure both CWBs
      uActiveCWBBits = HWIO_OUT_FLD(uActiveCWBBits, MDP_CTL_0_CWB_ACTIVE, CWB_0, 0x1);
      uActiveCWBBits = HWIO_OUT_FLD(uActiveCWBBits, MDP_CTL_0_CWB_ACTIVE, CWB_1, 0x1);

      out_dword(HWIO_MDP_CTL_0_CWB_ACTIVE_ADDR + uRegOffset, uActiveCWBBits);

      // Configure both MUXs
      out_dword(HWIO_MDP_CWB_0_MUX_ADDR, HWIO_OUT_FLD(0, MDP_CWB_0_MUX, SEL, uMuxSel));
      out_dword(HWIO_MDP_CWB_1_MUX_ADDR, HWIO_OUT_FLD(0, MDP_CWB_1_MUX, SEL, uMuxSel + 1));

      // Configure tap points foe both PPBs
      out_dword(HWIO_MDP_CWB_0_MODE_ADDR, HWIO_OUT_FLD(0, MDP_CWB_0_MODE, TAP, pCWBConfig->eTapPoint));
      out_dword(HWIO_MDP_CWB_1_MODE_ADDR, HWIO_OUT_FLD(0, MDP_CWB_1_MODE, TAP, pCWBConfig->eTapPoint));
    }
  }
                                          ...
                                          uint32                       uFlags)
{
   ...
   if (NULL != psDataPathConfig->psCWBConfig) 
   {
     eStatus = HAL_MDP_ControlPath_CWBConfig(psDataPathConfig->psCWBConfig);
   }
   ...
   /* ROI configuration */
   if (NULL != psDataPathConfig->psROIConfig)
   {
     HAL_MDP_WB_ROIConfig(psDataPathConfig->psROIConfig);
   }

   if (NULL != psDataPathConfig->psCWBDitherConfig)
   {
     HAL_MDP_CWB_PingPong_DitherConfig(psDataPathConfig->psCWBDitherConfig);
   }
...
void HAL_MDP_DataPath_Init(HAL_MDP_DataPathId      eDataPathId,
                           ...
                           uint32                  uFlags)
{
  out_dword(HWIO_MDP_WB_2_SAFE_LUT_ADDR,   0x00000001);
  out_dword(HWIO_MDP_WB_2_DANGER_LUT_ADDR, 0x00000000);
  out_dword(HWIO_MDP_WB_2_CREQ_LUT_0_ADDR, 0x66666540);
  out_dword(HWIO_MDP_WB_2_CREQ_LUT_1_ADDR, 0x77776666);
  out_dword(HWIO_MDP_WB_2_CDP_CNTL_ADDR,   0xB);

  // Disable Danger Safe Signaling for NRT client
  out_dword(HWIO_MDP_WB_2_QOS_CTRL_ADDR, 0);
