                                             ...
                                             uint32                                uFlags)
{
   ...
   uintPtr                 uLayerMixerExt4RegAddr  = 0x00;
   ...
   uLayerMixerExt4RegAddr = HWIO_MDP_CTL_0_LAYER_0_EXT4_ADDR + uRegOffset + uLayerMixerExtRegOffsets[uMixerId];
   ...
   uRegValueEx4   = in_dword(uLayerMixerExt4RegAddr);
           ...
           uBlendOrderIndex++)
       {
           ...
           for (uI = 0; uI < HAL_MDP_MAX_SSPP_IN_DUALPIPE_MODE; uI++)
           {
               ...
               case HAL_MDP_SOURCE_PIPE_DMA_4:
                   uRegValueEx4 = HWIO_OUT_FLD(uRegValueEx4,   MDP_CTL_0_LAYER_0_EXT4, DMA_4_OUT, uBlendOrderIndex);
                   break;

               case HAL_MDP_SOURCE_PIPE_DMA_5:
                   uRegValueEx4 = HWIO_OUT_FLD(uRegValueEx4,   MDP_CTL_0_LAYER_0_EXT4, DMA_5_OUT, uBlendOrderIndex);
                   break;
               ...
               case HAL_MDP_SOURCE_PIPE_DMA_4_REC1:
                   uRegValueEx4 = HWIO_OUT_FLD(uRegValueEx4, MDP_CTL_0_LAYER_0_EXT4, DMA_4_REC1_OUT, uBlendOrderIndex);
                   break;
               
               case HAL_MDP_SOURCE_PIPE_DMA_5_REC1:
                   uRegValueEx4 = HWIO_OUT_FLD(uRegValueEx4, MDP_CTL_0_LAYER_0_EXT4, DMA_5_REC1_OUT, uBlendOrderIndex);
                   break;
  ...
  out_dword(uLayerMixerExt4RegAddr, uRegValueEx4);
}
                                                 ...
                                                 uint32                                    uFlags)
{
  ...
  if(HAL_MDP_INTERFACE_WRITEBACK_TYPE_BASE > pInterfaceConfig->eInterfaceId) // Physical interface connection
  {
    ...
    uRegValue = HWIO_OUT_FLD(uRegValue, MDP_CTL_0_TOP, GROUP_ID, 0xF);  // Disable group id for all use cases.
                                                     ...
                                                     uint32                           uFlags)
{
  ...
  else if (HAL_MDP_VDCM_NONE != pActiveModulesConfig->uVDCMId)
  {
    // VDCM uses MDP_CTL_0_DSC_ACTIVE to activate hw module, BIT16:VDCM0
    uRegValue = (1 << (pActiveModulesConfig->uVDCMId - HAL_MDP_VDCM_0 + HWIO_MDP_CTL_0_DSC_ACTIVE_VDC_0_SHFT));
...
/****************************************************************************
*
** FUNCTION: HAL_MDP_ControlPath_FetchPipeActiveConfig()
*/
/*!
* \brief
*     Control path Active fetch pipe configuration
*
* \param [in] eControlPath                - Control Path ID (0, 1, 2, 3, 4)
* \param [in] psFetchPipeAcitve           - structure to configure active fetch pipe
*
* \retval NONE
*
****************************************************************************/
static void HAL_MDP_ControlPath_FetchPipeActiveConfig(HAL_MDP_ControlPathId                    eControlPath,
                                                      HAL_MDP_ControlPath_FetchPipeActiveType *psFetchPipeAcitve)
{
  uintPtr     uRegOffset = HAL_MDP_Get_ControlPath_BaseOffset(eControlPath);

  if (NULL != psFetchPipeAcitve)
  {
    uint32      uFetchPipeActiveRegVal  = in_dword(HWIO_MDP_CTL_0_FETCH_PIPE_ACTIVE_ADDR + uRegOffset);
    uint32      uPipeActive             = (psFetchPipeAcitve->bPipeActive) ? 1 : 0;

    switch (psFetchPipeAcitve->eSourcePipeId)
    {
    case HAL_MDP_SOURCE_PIPE_VIG_0:
      uFetchPipeActiveRegVal = HWIO_OUT_FLD(uFetchPipeActiveRegVal, MDP_CTL_0_FETCH_PIPE_ACTIVE, VIG_0, uPipeActive);
      break;

    case HAL_MDP_SOURCE_PIPE_VIG_1:
      uFetchPipeActiveRegVal = HWIO_OUT_FLD(uFetchPipeActiveRegVal, MDP_CTL_0_FETCH_PIPE_ACTIVE, VIG_1, uPipeActive);
      break;

    case HAL_MDP_SOURCE_PIPE_VIG_2:
      uFetchPipeActiveRegVal = HWIO_OUT_FLD(uFetchPipeActiveRegVal, MDP_CTL_0_FETCH_PIPE_ACTIVE, VIG_2, uPipeActive);
      break;

    case HAL_MDP_SOURCE_PIPE_VIG_3:
      uFetchPipeActiveRegVal = HWIO_OUT_FLD(uFetchPipeActiveRegVal, MDP_CTL_0_FETCH_PIPE_ACTIVE, VIG_3, uPipeActive);
      break;

    case HAL_MDP_SOURCE_PIPE_DMA_0:
      uFetchPipeActiveRegVal = HWIO_OUT_FLD(uFetchPipeActiveRegVal, MDP_CTL_0_FETCH_PIPE_ACTIVE, DMA_0, uPipeActive);
      break;

    case HAL_MDP_SOURCE_PIPE_DMA_1:
      uFetchPipeActiveRegVal = HWIO_OUT_FLD(uFetchPipeActiveRegVal, MDP_CTL_0_FETCH_PIPE_ACTIVE, DMA_1, uPipeActive);
      break;

    case HAL_MDP_SOURCE_PIPE_DMA_2:
      uFetchPipeActiveRegVal = HWIO_OUT_FLD(uFetchPipeActiveRegVal, MDP_CTL_0_FETCH_PIPE_ACTIVE, DMA_2, uPipeActive);
      break;

    case HAL_MDP_SOURCE_PIPE_DMA_3:
      uFetchPipeActiveRegVal = HWIO_OUT_FLD(uFetchPipeActiveRegVal, MDP_CTL_0_FETCH_PIPE_ACTIVE, DMA_3, uPipeActive);
      break;

    case HAL_MDP_SOURCE_PIPE_DMA_4:
      uFetchPipeActiveRegVal = HWIO_OUT_FLD(uFetchPipeActiveRegVal, MDP_CTL_0_FETCH_PIPE_ACTIVE, DMA_4, uPipeActive);
      break;

    case HAL_MDP_SOURCE_PIPE_DMA_5:
      uFetchPipeActiveRegVal = HWIO_OUT_FLD(uFetchPipeActiveRegVal, MDP_CTL_0_FETCH_PIPE_ACTIVE, DMA_5, uPipeActive);
      break;
    ...
    out_dword(HWIO_MDP_CTL_0_FETCH_PIPE_ACTIVE_ADDR + uRegOffset, uFetchPipeActiveRegVal);
  }
}

/****************************************************************************
*
** FUNCTION: HAL_MDP_ControlPath_DSPPFlushConfig()
*/
/*!
* \brief
*     Control path DSPP flush configuration
*
* \param [in] eControlPath                - Control Path ID (0, 1, 2, 3, 4)
* \param [in] psDsppFlushConfig           - structure to configure DSPP module flush 
*
* \retval NONE
*
****************************************************************************/
static void HAL_MDP_ControlPath_DSPPFlushConfig(HAL_MDP_ControlPathId                    eControlPath,
                                                HAL_MDP_ControlPath_DSPPFlushType       *psDsppFlushConfig)
{
  uintPtr uDSPPFlushBaseRegOffset[HAL_MDP_DESTINATION_PIPE_MAX] =
  {
    0x0,                                                                  //HAL_MDP_DESTINATION_PIPE_NONE;
    0x0,                                                                  //HAL_MDP_DESTINATION_PIPE_0;
    HWIO_MDP_CTL_0_DSPP_1_FLUSH_OFFS - HWIO_MDP_CTL_0_DSPP_0_FLUSH_OFFS,  //HAL_MDP_DESTINATION_PIPE_1;
    HWIO_MDP_CTL_0_DSPP_2_FLUSH_OFFS - HWIO_MDP_CTL_0_DSPP_0_FLUSH_OFFS,  //HAL_MDP_DESTINATION_PIPE_2;
    HWIO_MDP_CTL_0_DSPP_3_FLUSH_OFFS - HWIO_MDP_CTL_0_DSPP_0_FLUSH_OFFS,  //HAL_MDP_DESTINATION_PIPE_3;
  };

  if (psDsppFlushConfig->eDestinationPipeId < HAL_MDP_DESTINATION_PIPE_MAX)
  {
    uintPtr uRegOffset          = HAL_MDP_Get_ControlPath_BaseOffset(eControlPath);
    uintPtr uDSPPFlushRegOffset = uDSPPFlushBaseRegOffset[psDsppFlushConfig->eDestinationPipeId];
    uint32  uDsppFlushRegVal    = in_dword(HWIO_MDP_CTL_0_DSPP_0_FLUSH_ADDR + uRegOffset + uDSPPFlushRegOffset);

    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_SB)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, DSPP_SB, 1);
    }
    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_RC)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, RC, 1);
    }
    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_DEMURA)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, DEMURA, 1);
    }
    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_SPR)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, SPR, 1);
    }
    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_LTM)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, LTM, 1);
    }
    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_ROI_BYPASS)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, DSPP_ROI_BYPASS, 1);
    }
    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_PGC)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, DSPP_PGC, 1);
    }
    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_PCC)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, DSPP_PCC, 1);
    }
    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_GAMUT)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, DSPP_GAMUT, 1);
    }
    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_IGC)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, DSPP_IGC, 1);
    }
    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_PA_LUTV)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, DSPP_PA_LUTV, 1);
    }
    if (psDsppFlushConfig->eDsppMoudle & HAL_MDP_DSPPMODULE_PA)
    {
      uDsppFlushRegVal = HWIO_OUT_FLD(uDsppFlushRegVal, MDP_CTL_0_DSPP_0_FLUSH, DSPP_PA, 1);
    }

    out_dword(HWIO_MDP_CTL_0_DSPP_0_FLUSH_ADDR + uRegOffset + uDSPPFlushRegOffset, uDsppFlushRegVal);
  }
}

/****************************************************************************
*
** FUNCTION: HAL_MDP_ControlPath_MicroIdleConfig()
*/
/*!
* \brief
*     Control path micro idle configuration
*
* \param [in] eControlPath         - Control Path ID (0, 1, 2, 3, 4)
* \param [in] psMicroIdleConfig    - structure to configure micro idle
*
* \retval void
*
****************************************************************************/
static void  HAL_MDP_ControlPath_MicroIdleConfig(HAL_MDP_ControlPathId                    eControlPath,
                                                 HAL_MDP_ControlPath_MicroIdleConfigType *psMicroIdleConfig)
{
  uintPtr uCtrlPathOffset = HAL_MDP_Get_ControlPath_BaseOffset(eControlPath);
  uint32  uRegValue       = 0;

  // Enable/Disable micro idle HW for this control path depending on global micro idle controller state
  uRegValue = HWIO_OUT_FLD(uRegValue, MDP_CTL_0_UIDLE_ACTIVE, UIDLE, (psMicroIdleConfig->bEnable) ? 1 : 0);

  out_dword(HWIO_MDP_CTL_0_UIDLE_ACTIVE_ADDR + uCtrlPathOffset, uRegValue);
}
                                             ...
                                             uint32                          uFlags)
{
   ...
   // Fetch pipe active configuration
   if (NULL != pControlPathConfig->psFetchPipeActiveConfig)
   {
     HAL_MDP_ControlPath_FetchPipeActiveConfig(eControlPath, pControlPathConfig->psFetchPipeActiveConfig);
   }

   // DSPP flush configuration
   if (NULL != pControlPathConfig->psDsppFlushConfig)
   {
     HAL_MDP_ControlPath_DSPPFlushConfig(eControlPath, pControlPathConfig->psDsppFlushConfig);
   }

   // Micro idle configuration
   if (NULL != pControlPathConfig->psMicroIdleConfig)
   {
     HAL_MDP_ControlPath_MicroIdleConfig(eControlPath, pControlPathConfig->psMicroIdleConfig);
   }
                                              ...
                                              uint32                        uFlags)
{
   ...
   * So DSC/VDCM, 3d merge & PERIPH modules will be flushed on INTF change
   */
  if (uRegValue & (HAL_CONTROL_PATH_FLUSH_INTF | HAL_CONTROL_PATH_FLUSH_WB))
  {
    ...
    if (0 < uActive)
    {
      ...
      uRegValue |= HAL_CONTROL_PATH_FLUSH_WB;
    }

    // Concurrent Writeback
    uActive = in_dword(HWIO_MDP_CTL_0_CWB_ACTIVE_ADDR + uRegOffset);
    if (0 < uActive)
    {
      out_dword(HWIO_MDP_CTL_0_CWB_FLUSH_ADDR + uRegOffset, uActive);
      uRegValue |= HAL_CONTROL_PATH_FLUSH_CWB;
    }
  }
