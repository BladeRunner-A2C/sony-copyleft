...
#define HAL_MDP_FRAME_RATE_SW_FUSE_SHIFT          4
/****************************************************************************
*
                                                 ...
                                                 uint32                                      uSetupFlags)
{
   ...
   if (pAutoRefreshEnable->bEnable)
   {
     ...
     uRegVal = HWIO_OUT_FLD(uRegVal, MDP_INTF_1_TEAR_AUTOREFRESH_CONFIG, FRAME_COUNT, pAutoRefreshEnable->uAutoRefreshFrameNumDiv);
                                        ...
                                        uint32                               uSetupFlags)
{
  ...
  uRegVal = HWIO_OUT_FLD(uRegVal, MDP_INTF_1_TEAR_TEAR_CHECK_EN, EN,                  ((TRUE == pTEEnable->bEnable)        ? 1 : 0));
  uRegVal = HWIO_OUT_FLD(uRegVal, MDP_INTF_1_TEAR_TEAR_CHECK_EN, GUARD_WINDOW_EN,     ((TRUE == pTEEnable->bGuardWindowEn) ? 1 : 0));
  uRegVal = HWIO_OUT_FLD(uRegVal, MDP_INTF_1_TEAR_TEAR_CHECK_EN, DSI_DMA_SCHEDULE_EN, ((TRUE == pTEEnable->bDMAScheduleEn) ? 1 : 0));
  uRegVal = HWIO_OUT_FLD(uRegVal, MDP_INTF_1_TEAR_TEAR_CHECK_EN, SINGLE_UPDATE_EN,    ((TRUE == pTEEnable->bSingleUpdate)  ? 1 : 0));

  out_dword(HWIO_MDP_INTF_1_TEAR_TEAR_CHECK_EN_ADDR + uRegOffset, uRegVal);
}
                                           ...
                                           uint32                               uSetupFlags)
{
  ...
  *========================================================================*/
  ...
  /*========================================================================
  * (2) program read-pointer IRQ line
  *========================================================================*/
  ...
  /*========================================================================
  * (3) program write-pointer IRQ line
  *========================================================================*/
                                        ...
                                        uint32                               uSetupFlags)
{
  ...
  uRegVal = pTEConfig->uStartPosition + pTEConfig->uStartThreshold + 1;
  out_dword(HWIO_MDP_INTF_1_TEAR_SYNC_WRCOUNT_ADDR + uRegOffset, uRegVal);
                                               ...
                                               uint32                                uSetupFlags)
{
   ...
   if(HAL_MDP_INTERFACE_MODE_VIDEO == pInterfaceConfig->eInterfaceMode)
   {
      ...
      /*
       * DP timing requires the active region to be defined in the bottom-right corner of the frame dimensions
       * It is required to work with correct packet/audio transfers, dynamic refresh, AVR, etc
       */
      if((HAL_MDP_INTERFACE_STANDARD_DP  == pInterfaceCfg->eInterfaceStandard) ||
         (HAL_MDP_INTERFACE_STANDARD_eDP == pInterfaceCfg->eInterfaceStandard))
      {
         pInterfaceCfg->uHsyncBackPorchInPixels += pInterfaceCfg->uHsyncFrontPorchInPixels;
         pInterfaceCfg->uHsyncFrontPorchInPixels = 0;

         pInterfaceCfg->uVsyncBackPorchInLines += pInterfaceCfg->uVsyncFrontPorchInLines;
         pInterfaceCfg->uVsyncFrontPorchInLines = 0;
      }
     ...
     // DISPLAY_DATA_HCTL
     /* 2 pixels per pclk from mdp to dsi enabled */
     if (pInterfaceCfg->bTwoPixelsPerPclk)
     {
          uHsyncStartX         = (pInterfaceCfg->uHsyncBackPorchInPixels +
                                  pInterfaceCfg->uHsyncPulseInPixels);
          ...
          uTimingRegValue      = 0x00;
          uTimingRegValue      = HWIO_OUT_FLD(uTimingRegValue, 
                                          MDP_INTF_0_DISPLAY_DATA_HCTL,
                                          DISPLAY_DATA_START_X,
                                          uHsyncStartX);

          uTimingRegValue      = HWIO_OUT_FLD(uTimingRegValue, 
                                          MDP_INTF_0_DISPLAY_DATA_HCTL,
                                          DISPLAY_DATA_END_X,
                                          uHsyncStartX + pInterfaceCfg->uPclkPerLine - 1);

          out_dword(HWIO_MDP_INTF_0_DISPLAY_DATA_HCTL_ADDR + uRegOffset, uTimingRegValue);       // DISPLAY_DATA_HCTL
     }
     else if ((HAL_MDP_INTERFACE_STANDARD_DP  == pInterfaceCfg->eInterfaceStandard) ||
              (HAL_MDP_INTERFACE_STANDARD_eDP == pInterfaceCfg->eInterfaceStandard))
     {
          /* For the dp case, since currently we don't use compression, it is same as the DISPLAY_HCTL calculation */
          out_dword(HWIO_MDP_INTF_0_DISPLAY_DATA_HCTL_ADDR + uRegOffset, uTimingRegValue);       // DISPLAY_DATA_HCTL
     }
     else
     {
          out_dword(HWIO_MDP_INTF_0_DISPLAY_DATA_HCTL_ADDR + uRegOffset, 0); 
     } 
      ...
      else if(HAL_MDP_CHECK_PLANAR_PIXEL_FORMAT(pInterfaceCfg->eInterfacePixelFormat))
      {
        uPanelFormat        |= guPanelFormatInfo[HAL_MDP_PIXEL_FORMAT_RGB_PLANAR_888_8BPP];
      }
      else
      {
     ...
     // Dual pixel per pclk mode
     uRegValue = in_dword(HWIO_MDP_INTF_0_INTF_CONFIG2_ADDR + uRegOffset);

     if ((pInterfaceCfg->bTwoPixelsPerPclk) || 
         (bTwoPixelsMode))
     {
        uRegValue = HWIO_OUT_FLD(uRegValue, MDP_INTF_0_INTF_CONFIG2, DATABUS_WIDEN, 1);
        uRegValue = HWIO_OUT_FLD(uRegValue, MDP_INTF_0_INTF_CONFIG2, DATA_HCTL_EN, 1);
        
        if ((HAL_MDP_INTERFACE_STANDARD_DP  == pInterfaceCfg->eInterfaceStandard) ||
            (HAL_MDP_INTERFACE_STANDARD_eDP == pInterfaceCfg->eInterfaceStandard))
        {
          uRegValue = HWIO_OUT_FLD(uRegValue, MDP_INTF_0_INTF_CONFIG2, DCE_DATA_COMPRESS, 0);
        }
        else
        {
          uRegValue = HWIO_OUT_FLD(uRegValue, MDP_INTF_0_INTF_CONFIG2, DCE_DATA_COMPRESS, 1);
        }
      }
      else
      {
        uRegValue = HWIO_OUT_FLD(uRegValue, MDP_INTF_0_INTF_CONFIG2, DATABUS_WIDEN, 0);
        uRegValue = HWIO_OUT_FLD(uRegValue, MDP_INTF_0_INTF_CONFIG2, DATA_HCTL_EN, 0);
        uRegValue = HWIO_OUT_FLD(uRegValue, MDP_INTF_0_INTF_CONFIG2, DCE_DATA_COMPRESS, 0);
      }
      out_dword(HWIO_MDP_INTF_0_INTF_CONFIG2_ADDR + uRegOffset, uRegValue);
	  
      if (HAL_MDP_FLAG_TIMESTAMP_FEATURE & pgsMdpHwInfo->sHwInfo.uFeatureFlags)
      {
        // Enable timestamp logging.
        out_dword(HWIO_MDP_INTF_0_VSYNC_TIMESTAMP_CTRL_ADDR + uRegOffset, VIDEO_MODE_TIMESTAMP_CTRL_CFG);
      }
   ...
   else
   {
      ...
      {
        // Enable timestamp logging.
        out_dword(HWIO_MDP_INTF_0_VSYNC_TIMESTAMP_CTRL_ADDR + uRegOffset, COMMAND_MODE_TIMESTAMP_CTRL_CFG);
      }
   }
...
/****************************************************************************
*
** FUNCTION: HAL_MDP_Interface_GetFPSFuse()
*/
/*!
* \brief
*     Get panel refresh rate fuse set by SW fuse registers.
*
* \param [in]     eInterfaceId     - Requested interface
* \param [out]    pfFPSFuse        - Max fps SW fuse allowed 
*
* \retval HAL_MDSS_ErrorType
*
****************************************************************************/
static HAL_MDSS_ErrorType HAL_MDP_Interface_GetFPSFuse(HAL_MDP_InterfaceId   eInterfaceId, 
                                                       float                *pfFPSFuse)
{
  HAL_MDSS_ErrorType   eStatus = HAL_MDSS_STATUS_SUCCESS;
  ...
  else if ((HAL_MDP_GET_PHYSICAL_INTERFACE_COUNT() < eInterfaceId) ||
           (HAL_MDP_INTERFACE_NONE                == eInterfaceId))
  {
    eStatus = HAL_MDSS_STATUS_FAILED_NO_HW_SUPPORT;
  }
  else
  {
    uint32 uInterfaceIndex = eInterfaceId - 1;
    uint32 uSWFpsFuseEnReg = in_dword(HWIO_SW_FUSE_SW_FUSE_FPS_EN_ADDR);

    if (uSWFpsFuseEnReg & HAL_MDSS_BIT(uInterfaceIndex))
    {
      uint32 uFpsFuseValReg = in_dword(HWIO_SW_FUSE_SW_FUSE_FPS_INTFi_ADDR(uInterfaceIndex));
      ...
      {
        // uFpsFuseValReg  store timestamp value bit[19-4], timestamp is counting on CXO 19.2Mhz between Vsync.
        *pfFPSFuse = (float)HAL_MDP_DEFAULT_CXO_CLK / (uFpsFuseValReg << HAL_MDP_FRAME_RATE_SW_FUSE_SHIFT);
  
      }
    }
    else
    {
      // SW FUSE is not enabled.
      eStatus = HAL_MDSS_STATUS_FAILED_NO_HW_SUPPORT;
    }
  }
                                                          ...
                                                          uint32                                     uSetupFlags)
{
  ...
  if(pDualInterfaceConfig->bEnableDualInterface)
  {
    ...
    {
      // To ensure that command mode stream0 output to DSI0 and DSI1 are aligned on frame, 
      // set SMART_PANEL_ALIGN_MODE to 1 to avoid corruption on some panels.
      uAlignMode = 0x1;
  ...
  uSplitCTRLVal = 0;
  uSplitCTRLVal = HWIO_OUT_FLD(uSplitCTRLVal, MDP_SPLIT_DISPLAY_LOWER_PIPE_CTRL, SMART_PANEL_ALIGN_MODE, uAlignMode);
                                           ...
                                           uint32                        uSetupFlags)
{
  ...
  if(NULL != pInterfaceConfig->pIntrTimingGenConfig)
  {
    if(HAL_MDP_INTERFACE_WRITEBACK_TYPE_BASE > eInterfaceId) // Physical interface
    {
      ...
      if ((pInterfaceConfig->ePingPongId[0] < HAL_MDP_PINGPONG_MAX) &&
          (pInterfaceConfig->ePingPongId[0] > HAL_MDP_PINGPONG_NONE))
      {
        // Configure Ping Pong Mux in the interface, if split link is enabled, this will be Ping Pong Id of sublink0
        uRegVal = HWIO_OUT_FLD(uRegVal, MDP_INTF_0_MUX, SEL, pInterfaceConfig->ePingPongId[0] - HAL_MDP_PINGPONG_0);
      
        // Configure split link Pingpong Mux for sublink1
        if ((pInterfaceConfig->ePingPongId[1] < HAL_MDP_PINGPONG_MAX) &&
            (pInterfaceConfig->ePingPongId[1] > HAL_MDP_PINGPONG_NONE))
        {
          uRegVal = HWIO_OUT_FLD(uRegVal, MDP_INTF_0_MUX, SPLIT_SEL, pInterfaceConfig->ePingPongId[1] - HAL_MDP_PINGPONG_0);
        }
        else
        {
          // Split Link disabled.
          uRegVal = HWIO_OUT_FLD(uRegVal, MDP_INTF_0_MUX, SPLIT_SEL, 0xF);
        }
        out_dword(HWIO_MDP_INTF_0_MUX_ADDR + uRegOffset, uRegVal);
      }
    ...
    else
    {
      ...
      out_dword(HWIO_MDP_WB_2_MUX_ADDR, pInterfaceConfig->ePingPongId[0] - HAL_MDP_PINGPONG_0);
                                                 ...
                                                 uint32                           uSetPropertyFlags)
{
  ...
  // Interface misc control 
  if (NULL != pInterfaceConfig->pMiscControlFlag)
  {
    uintPtr uRegOffset = HAL_MDP_Get_PhyInterface_BaseOffset((HAL_MDP_PhyInterfaceId)eInterfaceId);
    uint32  uRegVal    = in_dword(HWIO_MDP_INTF_0_TEST_CTL_ADDR + uRegOffset);

    uRegVal            = HWIO_OUT_FLD(uRegVal, MDP_INTF_0_TEST_CTL, PIXEL_DRAIN_EN, ((pInterfaceConfig->pMiscControlFlag && HAL_MDP_INTF_MISCCTL_FLAG_PIXEL_DRAIN) ? 1 :0));

    out_dword(HWIO_MDP_INTF_0_TEST_CTL_ADDR + uRegOffset, uRegVal);
  }
                                                 ...
                                                 uint32                          uGetPropertyFlags)
{
  ...
  // FPS Fuse in SW fuse setting
  if (NULL != pInterfaceInfoData->pfFPSFuse)
  {
    eStatus = HAL_MDP_Interface_GetFPSFuse( eInterfaceId,
                                            pInterfaceInfoData->pfFPSFuse);
  }
