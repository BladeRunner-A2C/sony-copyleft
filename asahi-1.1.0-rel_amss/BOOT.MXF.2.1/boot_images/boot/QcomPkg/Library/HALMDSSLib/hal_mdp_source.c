...
/****************************************************************************
*
** FUNCTION: HAL_MDP_Validate_SSPPId()
*/
/*!
* \brief
*     Validates that SSPP Id is within allowed range
*
* \param [in]   eSourcePipe         - Source pipe id
*
* \retval HAL_MDSS_ErrorType
*
****************************************************************************/
HAL_MDSS_ErrorType HAL_MDP_Validate_SSPPId(HAL_MDP_SourcePipeId  eSourcePipe)
{
  HAL_MDSS_ErrorType   eStatus = HAL_MDSS_STATUS_SUCCESS;
                                         ...
                                         HAL_MDP_PixelExtLayerConfigType *psPixelExtLayerConfig)
{
   ...
   if (HAL_MDP_LAYER_TYPE_VIG == eLayerType)
   {
      // Only VIG layers support scaling   
...
static void HAL_MDP_SSPP_SharpenConfig(HAL_MDP_SourcePipeId         eSourcePipe,
                                       ...
                                       uint32                       uFlags)
{
#if 0 //Poipu #TODO#IMPLEMENT Use QSEED3
...
static void HAL_MDP_SSPP_PictureAdjustmentConfig(HAL_MDP_SourcePipeId             eSourcePipe,
                                                 ...
                                                 uint32                           uFlags)
{
  //#TODO#VERIFY - Picture Adjustment removed from SSPP
}
                                         ...
                                         uint32                  uFlags)
{
   if (NULL!= pgsMdpHwInfo->psSSPPQoSInfo)
   {
      ...
      uRegisterOffset = HAL_MDP_Get_SSPP_BaseOffset(eSourcePipe);
      if (pgsMdpHwInfo->psSSPPQoSInfo->pLinearQoSInfo)
      {
         out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_DANGER_LUT_ADDR + uRegisterOffset, pgsMdpHwInfo->psSSPPQoSInfo->pLinearQoSInfo->uDangerLutValue);
         out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_SAFE_LUT_ADDR   + uRegisterOffset, pgsMdpHwInfo->psSSPPQoSInfo->pLinearQoSInfo->uSafeLutValue[0]);
         out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_CREQ_LUT_0_ADDR + uRegisterOffset, pgsMdpHwInfo->psSSPPQoSInfo->pLinearQoSInfo->uCreqLutData[0][0]);
         out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_CREQ_LUT_1_ADDR + uRegisterOffset, pgsMdpHwInfo->psSSPPQoSInfo->pLinearQoSInfo->uCreqLutData[0][1]);
      }
   }
}
...
/****************************************************************************
*
** FUNCTION: HAL_MDP_SSPP_MicroIdleConfig()
*/
/*!
* \brief
*     Set up configurations for a particular source pipe
...
* \param [in]   eSourcePipe         - Source pipe id
* \param [in]   eLayerType          - Layer Type (RGB, VIG, DMA, CURSOR etc)
* \param [in]   sMicroIdleConfig    - Micro idle configuration information
* \param [in]   uFlags              - Reserved.
*
* \retval NONE
*
****************************************************************************/
static void HAL_MDP_SSPP_MicroIdleConfig(HAL_MDP_SourcePipeId              eSourcePipe,
                                         HAL_MDP_LayerType                 eLayerType,
                                         HAL_MDP_SSPP_MicroIdleConfigType *pMicroIdleConfig,
                                         uint32                            uFlags)
{
  if ((HAL_MDSS_STATUS_SUCCESS == HAL_MDP_ValidateSourcePipe(eSourcePipe, eLayerType)) &&
      ...
      (NULL != pgsMdpHwInfo->psMicroIdleSettingsInfoType->psSSPPMicroIdleSettingsInfo) &&
      (0 != pMicroIdleConfig->uLineTime))
  {    
    uintPtr uRegisterOffset = 0;
    uint32  uRegValue       = 0;
    ...
    // Diable/Enable micro idle feature
    uRegValue = HWIO_OUT_FLD(uRegValue,
                             MDP_VP_0_VIG_0_SSPP_UIDLE_CTRL_VALUE,
                             DISABLE,
                             (pMicroIdleConfig->bEnable) ? 0 : 1);

    // Configure other micro idle params if being enabled
    if (TRUE == pMicroIdleConfig->bEnable)
    {
      uint32                              uLineTime = pMicroIdleConfig->uLineTime;
      HAL_MDP_SSSP_MicroIdleSettingsInfo* psMicroIdleSettings;
      uint32                              uFALAllowedThreshold;
      uint32                              uFAL10ExitThreshold;
      uint32                              uFAL10Threshold;
      uint32                              uFAL1Threshold;

      psMicroIdleSettings     = pgsMdpHwInfo->psMicroIdleSettingsInfoType->psSSPPMicroIdleSettingsInfo;
      uFALAllowedThreshold    = psMicroIdleSettings->uFAL10Threshold +
                                ((psMicroIdleSettings->uFAL10TargetIdleTime * 1000 / (uLineTime * 2)) / 1000);
      uFAL10ExitThreshold     = psMicroIdleSettings->uFAL10Threshold + 2;
      uFAL10Threshold         = psMicroIdleSettings->uFAL10Threshold;
      uFAL1Threshold          = 1 + ((psMicroIdleSettings->uFAL1TargetIdleTime * 1000 / (uLineTime * 2)) / 1000);

      // FAL allowed threshold
      uRegValue = HWIO_OUT_FLD(uRegValue,
                               MDP_VP_0_VIG_0_SSPP_UIDLE_CTRL_VALUE,
                               FAL_ALLOWED_THRESHOLD,
                               uFALAllowedThreshold);

      // FAL10 exit threshold
      uRegValue = HWIO_OUT_FLD(uRegValue,
                               MDP_VP_0_VIG_0_SSPP_UIDLE_CTRL_VALUE,
                               FAL10_EXIT_THRESHOLD,
                               uFAL10ExitThreshold);
    ...
    // Configure micro idle SSPP register 
    // The offsets for REC0 and REC1 registers are different
    // Therefore the Smart DMA base is treated differently from physcial DMA
    if (HAL_MDP_SMART_DMA_REC1_ID == HAL_MDP_Get_SmartDMARecID(eSourcePipe)) // SMART DMA REC1
    {
      out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_UIDLE_CTRL_VALUE_REC1_ADDR + uRegisterOffset, uRegValue);
    }
    else 
    {
      // Micro idle SSPP configuration only applicable on VIG and DMA layer
      out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_UIDLE_CTRL_VALUE_ADDR + uRegisterOffset, uRegValue); 
    }
  }
}
                                            ...
                                            uint32                          uSetupFlags)
{
   ...
   if(HAL_MDSS_STATUS_SUCCESS == (eStatus = HAL_MDP_ValidateSourcePipe(eSourcePipe, eLayerType)))
   {
      ...
      /** GPU highest memory bank bit configuration - Moved from SSPP to top level static configuration */
      ...
      if (NULL != psSourcePipeConfig->psMicroIdleConfig)
      {
        HAL_MDP_SSPP_MicroIdleConfig(eSourcePipe, eLayerType, psSourcePipeConfig->psMicroIdleConfig, 0x00);
      }
   }
...
{
  if ((NULL                    != psQoSInfo)                          &&
      (HAL_MDSS_STATUS_SUCCESS == HAL_MDP_Validate_SSPPId(eSourcePipe)))
  {
    uintPtr uRegisterOffset   = 0;
    uint32  uRegValue         = 0;
    ...
    if (HAL_MDP_TRFCTRL_LATENCY_REALTIME == eLatency)
    {
      uint32                       uSafeIndex   = 0;
      uint32                       uCReqIndex   = 0;
      HAL_MDP_QoSLutInfo          *pQoSLutInfo  = pgsMdpHwInfo->psSSPPQoSInfo->pLinearQoSInfo;;

      #define BASE_QOS_LINES  4.5                                                       // 9 fill levels @ WQHD120
      double fTargetTime      = BASE_QOS_LINES * 1.0 / 120.0 / 3360.0;                  // 11.2 us latency tolerance
      double fPanelLineTime   = 1.0 / (psQoSInfo->uHeightTotalPx * psQoSInfo->uFps);    // Line time = 1/(total ht * refreshrate). Converting it to microseconds
      double fFLRequire0      = (double)fTargetTime / fPanelLineTime * 2;               // 2FL per line
      uint32 uFLTarget        = 8;                                                      // 8 fill levels for target_time
      double fFLRequire       = (uFLTarget > fFLRequire0) ? uFLTarget: fFLRequire0;
      double fFLScale         = fFLRequire / uFLTarget;
      uint32 uFLScaleReg      = (fFLScale <= 1) ? 0 : 32 / fFLScale;

      // If QSeed is enabled, use LUT other than default as per system's team recommendation. 
      if (HAL_MDP_QOS_FLAG_QSEED_ENABLED & psQoSInfo->uFlags)
      {
        uCReqIndex = 1;
      }

      // If panel is landscape or fill level is less than threshold, use SAFE_LUT other than default as per QOS HSR. 
      if ((HAL_MDP_QOS_FLAG_LANDSCAPE_PANEL & psQoSInfo->uFlags) &&
          (fFLRequire0 < BASE_QOS_LINES))
      {
        uSafeIndex = 1;
      }

      uRegValue = HWIO_OUT_FLD(0,         MDP_VP_0_VIG_0_SSPP_FILL_LEVEL_SCALE, FILL_LEVEL_V1_SCALE,          uFLScaleReg);
      uRegValue = HWIO_OUT_FLD(uRegValue, MDP_VP_0_VIG_0_SSPP_FILL_LEVEL_SCALE, FILL_LEVEL_V2_SCALE,          uFLScaleReg);
      uRegValue = HWIO_OUT_FLD(uRegValue, MDP_VP_0_VIG_0_SSPP_FILL_LEVEL_SCALE, FILL_LEVEL_SCALE_FAL10_EXIT,  uFLScaleReg);
      out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_FILL_LEVEL_SCALE_ADDR + uRegisterOffset, uFLScaleReg);

      // Write Danger, Safe and CReq LUT values
      out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_DANGER_LUT_ADDR + uRegisterOffset, pQoSLutInfo->uDangerLutValue);
      out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_SAFE_LUT_ADDR   + uRegisterOffset, pQoSLutInfo->uSafeLutValue[uSafeIndex]);
      out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_CREQ_LUT_0_ADDR + uRegisterOffset, pQoSLutInfo->uCreqLutData[uCReqIndex][0]);
      out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_CREQ_LUT_1_ADDR + uRegisterOffset, pQoSLutInfo->uCreqLutData[uCReqIndex][1]);

      // Enable Danger Safe Signaling
      uRegValue = HWIO_OUT_FLD(0, MDP_VP_0_VIG_0_SSPP_QOS_CTRL, DANGER_SAFE_EN, 0x1);
      out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_QOS_CTRL_ADDR + uRegisterOffset, uRegValue);
    ...
    {
      uint32 uCReqLutValue = 0; // Recommended value for NRT client 

      // Write CReq LUT values (Danger/Safe values are not required to be written as it is disabled)
      out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_CREQ_LUT_0_ADDR + uRegisterOffset, uCReqLutValue);
      out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_CREQ_LUT_1_ADDR + uRegisterOffset, uCReqLutValue);

      // Disable Danger Safe Signaling for NRT client
      uRegValue = HWIO_OUT_FLD(0, MDP_VP_0_VIG_0_SSPP_QOS_CTRL, DANGER_SAFE_EN, 0x0);
      out_dword(HWIO_MDP_VP_0_VIG_0_SSPP_QOS_CTRL_ADDR + uRegisterOffset, uRegValue);
    }
  }
}
