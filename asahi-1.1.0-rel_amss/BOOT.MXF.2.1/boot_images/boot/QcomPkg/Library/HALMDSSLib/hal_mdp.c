...
const HAL_MDP_SourcePipeId gMultiRectMappingTable[HAL_MDP_SOURCE_PIPE_MAX] = 
{
  ...
  HAL_MDP_SOURCE_PIPE_DMA_4_REC1, // HAL_MDP_SOURCE_PIPE_DMA_4,
  HAL_MDP_SOURCE_PIPE_DMA_5_REC1, // HAL_MDP_SOURCE_PIPE_DMA_5,
  ...
  HAL_MDP_SOURCE_PIPE_DMA_4,      // HAL_MDP_SOURCE_PIPE_DMA_4_REC1,
  HAL_MDP_SOURCE_PIPE_DMA_5,      // HAL_MDP_SOURCE_PIPE_DMA_5_REC1,
};
...
uintPtr  uMDPCWBPingPongRegBaseOffset[HAL_MDP_CWB_PINGPONG_MAX];                   // CWB PingPong register offsets
...
uintPtr  uMDPDSCCoreBaseOffset[HAL_MDP_DSC_CORE_MAX];                              // DSC Core register offset
uintPtr  uMDPDSCEncBaseOffset[HAL_MDP_DSC_MAX];                                    // DSC Enc register offset
uintPtr  uMDPDSCCtlBaseOffset[HAL_MDP_DSC_MAX];                                    // DSC Ctl register offset
uintPtr  uMDPVDCMCoreBaseOffset[HAL_MDP_VDCM_CORE_MAX];                            // VDCM Core register offset
uintPtr  uMDPVDCMEncBaseOffset[HAL_MDP_VDCM_MAX];                                  // VDCM Enc register offset
uintPtr  uMDPVDCMCtlBaseOffset[HAL_MDP_VDCM_MAX];                                  // VDCM Ctl register offset
...
uintPtr  uMDPIntfTearRegBaseOffset[HAL_MDP_PHYSICAL_INTERFACE_MAX];                // Physical Interface TEAR check register offset
uintPtr  uMDP3DMuxRegBaseOffset[HAL_MDP_3DMUX_MAX];                                // 3D Mux register offset
uintPtr  uMDPSprRegBaseOffset[HAL_MDP_SPR_MAX];                                    // SPR register offset
uintPtr  uMDPDemuraRegBaseOffset[HAL_MDP_DEMURA_MAX];                              // Demura register offset

/**
 * Configure the UBWC static control register 
 **/
static void HAL_MDP_ConfigUBWCSettings(void);

/**
 * Disable micro idle feature
 **/
static void HAL_MDP_MicroIdleConfig(bool32 bEnable);
...
{
   if (9 == psVersionInfo->uMajorVersion)
   {
      if(2 == psVersionInfo->uMinorVersion)
      {
         // Query MDSS 9.2.0 hardware capabilities
         HAL_MDP_ReadHardwareInfo_9_2_0(psHwInfo, psMdpHwInfo);
      }
   }
   else if (10 == psVersionInfo->uMajorVersion)
   {
      if(1 == psVersionInfo->uMinorVersion)
      {
         // Query MDSS 10.1.0 hardware capabilities
         HAL_MDP_ReadHardwareInfo_10_1_0(psHwInfo, psMdpHwInfo);
      }
      else
      {
         // Query MDSS 10.0.0 hardware capabilities
         HAL_MDP_ReadHardwareInfo_10_0_0(psHwInfo, psMdpHwInfo);
      }
   }
}
                                                           ...
                                                           uint32                       uRotatorIntrStatus)
{
   ...
   case HAL_MDP_WRITEBACK_INTERFACE_0:
      ...
      // DATA TRANSFER DONE
      if (uRotatorIntrStatus & HWIO_ROT_ROTTOP_INTR_STATUS_ROT_DONE_BMSK)
      {
...
HAL_MDSS_ErrorType HAL_MDP_Init(HAL_MDP_InitConfigType *pInitConfig, uint32 uInitFlags)
{
   ...
   if(HAL_MDSS_STATUS_SUCCESS == eStatus)
   {
      ...
      if (NULL != pInitConfig)
      {
        HAL_MDP_GpuMemoryBankConfigType sMemoryBankConfig;

        sMemoryBankConfig.uGpuHighestMemoryBankBit  = pInitConfig->uGpuHighestMemoryBankBit;
        sMemoryBankConfig.uGpuMacroTileMode         = pInitConfig->uGpuMacroTileMode;

        /* Update Highest bank bit */
        pgsMdpHwInfo->sUBWCConfig.uHighestBankBit   = HAL_MDP_Get_GpuMemoryBankBitConfig(&sMemoryBankConfig, 0);

        /* Update Max DDR channels */
        if (0 < pInitConfig->uMaxDDRChannels)
        {
          pgsMdpHwInfo->sUBWCConfig.uMaxDDRChannels   = pInitConfig->uMaxDDRChannels;
        }
      }

      /* Configure the UBWC static control register setting */
      HAL_MDP_ConfigUBWCSettings();

      /* Disable micro idle globally, on control paths and on source pipes */
      HAL_MDP_MicroIdleConfig(FALSE);
   }
   ...
   if(HAL_MDSS_STATUS_SUCCESS == eStatus) // Only initializes the HW tables
   {
      ...
      // Chroma-down block initialization
      uHWBlockIndex  = HAL_MDP_CDM_0;
      for(uIndex = 0; uIndex < HAL_MDP_GET_CHROMADOWN_BLOCK_COUNT(); uIndex++, uHWBlockIndex++)
      {
         HAL_MDP_CDM_Init((HAL_MDP_ChromaDownModuleId)uHWBlockIndex, HAL_MDP_CHROMABLOCK_DEFAULT_FEATURE_INIT, 0x00);
      }
   }
   return eStatus;
}
...
/****************************************************************************
*
** FUNCTION: HAL_MDP_UIdle_AXIClockForceOn()
*/
/*!
* \brief
*     Force gcc_disp_hf_axi_clk/gcc_disp_sf_axi_clk on
*
* \param [in] eVbifId    - VBIF block ID
* \param [in] bForceOn   - flag to force AXI clock on
* \param [in] uFlags     - reserved flags
*
* \retval HAL_MDSS_ErrorType
*
****************************************************************************/
HAL_MDSS_ErrorType HAL_MDP_UIdle_AXIClockForceOn(HAL_MDP_VbifId eVbifId, bool32 bForceOn, uint32 uFlags)
{
  HAL_MDSS_ErrorType eStatus   = HAL_MDSS_STATUS_SUCCESS;
  uint32             uRegValue = 0;
  ...
  switch (eVbifId)
  {
    case HAL_MDP_VBIF_0:
    {
      // VBIF_0 is the realtime VBIF client used by MDP 
      uRegValue = in_dword(HWIO_MDP_UIDLE_QACTIVE_HF_OVERRIDE_ADDR);

      if (bForceOn)
      {
        uRegValue = HWIO_OUT_FLD(uRegValue, MDP_UIDLE_QACTIVE_HF_OVERRIDE, ENABLE, 1);
        uRegValue = HWIO_OUT_FLD(uRegValue, MDP_UIDLE_QACTIVE_HF_OVERRIDE, VALUE,  1);
      }
      else
      {
        uRegValue = HWIO_OUT_FLD(uRegValue, MDP_UIDLE_QACTIVE_HF_OVERRIDE, ENABLE, 0);
        uRegValue = HWIO_OUT_FLD(uRegValue, MDP_UIDLE_QACTIVE_HF_OVERRIDE, VALUE,  0);
      }
      out_dword(HWIO_MDP_UIDLE_QACTIVE_HF_OVERRIDE_ADDR, uRegValue);

      break;
    }
    case HAL_MDP_VBIF_1:
    {
      // VBIF_1 is the non-realtime VBIF client used by Rotator 
      uRegValue = in_dword(HWIO_ROT_ROTTOP_QACTIVE_SF_OVERRIDE_ADDR);
      
      if (bForceOn)
      {
        uRegValue = HWIO_OUT_FLD(uRegValue, ROT_ROTTOP_QACTIVE_SF_OVERRIDE, ENABLE, 1);
        uRegValue = HWIO_OUT_FLD(uRegValue, ROT_ROTTOP_QACTIVE_SF_OVERRIDE, VALUE,  1);
      }
      else
      {
        uRegValue = HWIO_OUT_FLD(uRegValue, ROT_ROTTOP_QACTIVE_SF_OVERRIDE, ENABLE, 0);
        uRegValue = HWIO_OUT_FLD(uRegValue, ROT_ROTTOP_QACTIVE_SF_OVERRIDE, VALUE,  0);
      }
      out_dword(HWIO_ROT_ROTTOP_QACTIVE_SF_OVERRIDE_ADDR, uRegValue);
...
HAL_MDSS_ErrorType HAL_MDP_Term(uint32 uFlags)
{
  ...
  for (uVBIFId = 0; uVBIFId < HAL_MDP_GET_VBIF_COUNT(); uVBIFId++)
  {
    // Force gcc_disp_hf_axi_clk/gcc_disp_sf_axi_clk on, so that VBIF AXI can be halt.
    HAL_MDP_UIdle_AXIClockForceOn(uVBIFId, TRUE, uFlags);

    // Halt all Vbif AXI
    eStatus |= HAL_MDP_Vbif_Deinit((HAL_MDP_VbifId) (HAL_MDP_VBIF_0 + uVBIFId), 0x0);
 
    // restore the override setting.
    HAL_MDP_UIdle_AXIClockForceOn(uVBIFId, FALSE, uFlags);
  }
                                          ...
                                          uint32                          uFlags)
{
  ...
  switch(psGpuMemoryBankConfig->uGpuHighestMemoryBankBit)
  {
    ...
    default:
      uGpuHighestMemoryBankBit = pgsMdpHwInfo->sUBWCConfig.uHighestBankBit;
  }
...
/****************************************************************************
*
** FUNCTION: HAL_MDP_ConfigUBWCSettings()
*/
/*!
* \brief
*     Configure the UBWC settings
*
* \param  None
*
* \retval None
...
static void HAL_MDP_ConfigUBWCSettings(void)
{
  uint32               uRegValue      = 0x0;
  uint32               uDdrChannel    = 0x0;
  uint32               uUBWCVer       = 0x0;

  if (8 == pgsMdpHwInfo->sUBWCConfig.uMaxDDRChannels)
  {
    uDdrChannel = 0x1;
  }

  if (3 == pgsMdpHwInfo->sUBWCConfig.uMajorRevision)
  {
    uUBWCVer = 0x1;
  }
  else if (4 == pgsMdpHwInfo->sUBWCConfig.uMajorRevision)
  {
    if (3 == pgsMdpHwInfo->sUBWCConfig.uMinorRevision)
    {
      uUBWCVer = 0x3;
    }
    else
  ...
  // Get the current UBWC static control register setting
  uRegValue = HWIO_MDSS_HW_UBWC_STATIC_CTRL_IN;

  uRegValue = HWIO_OUT_FLD(uRegValue,
                           MDSS_HW_UBWC_STATIC_CTRL,
                           UBWC_MALSIZE,
                           pgsMdpHwInfo->sUBWCConfig.uMALSize);

  // UBWC Level 1 Bank Swizzling Setting
  uRegValue = HWIO_OUT_FLD(uRegValue,
                           MDSS_HW_UBWC_STATIC_CTRL,
                           UBWC_LVL1_BNK_SWIZ_EN,
                           pgsMdpHwInfo->sUBWCConfig.uLvl1BankSwizzling);
                           
  // UBWC Level 2 Bank Swizzling Setting
  uRegValue = HWIO_OUT_FLD(uRegValue,
                           MDSS_HW_UBWC_STATIC_CTRL,
                           UBWC_LVL2_BNK_SWIZ_EN,
                           pgsMdpHwInfo->sUBWCConfig.uLvl2BankSwizzling);
						   
  // UBWC Level 3 Bank Swizzling Setting
  uRegValue = HWIO_OUT_FLD(uRegValue,
                           MDSS_HW_UBWC_STATIC_CTRL,
                           UBWC_LVL3_BNK_SWIZ_EN,
                           pgsMdpHwInfo->sUBWCConfig.uLvl3BankSwizzling);

  // UBWC Bank Swizzling Setting
  uRegValue = HWIO_OUT_FLD(uRegValue,
                           MDSS_HW_UBWC_STATIC_CTRL,
                           UBWC_MAX_BANK_BIT,
                           pgsMdpHwInfo->sUBWCConfig.uHighestBankBit);
  // UBWC Bank Spread
  uRegValue = HWIO_OUT_FLD(uRegValue,
                           MDSS_HW_UBWC_STATIC_CTRL,
                           UBWC_BANK_SPREAD,
                           pgsMdpHwInfo->sUBWCConfig.uBankSpread);
                         
  // UBWC DDR channel Setting
  uRegValue = HWIO_OUT_FLD(uRegValue,
                           MDSS_HW_UBWC_STATIC_CTRL,
                           UBWC_MACRO_TILE_MODE,
                           uDdrChannel);

  // Update the UBWC static control register setting with new value
  HWIO_MDSS_HW_UBWC_STATIC_CTRL_OUT(uRegValue);

  // UBWC Version Setting - HW uses this to decide whether to use UBWC ver 3.0 (UBWC_VER = 1) or 4.0 (UBWC_VER = 2) or 4.3 (UBWC_VER = 3)
  uRegValue = 0;
  uRegValue = HWIO_OUT_FLD(uRegValue,
                           MDSS_HW_UBWC_CTRL_2,
                           UBWC_VER,
  ...
  // Update the UBWC Ctrl_2 register setting with new value
  HWIO_MDSS_HW_UBWC_CTRL_2_OUT(uRegValue);

  // UBWC Predicition mode setting
  uRegValue = 0;
  uRegValue = HWIO_OUT_FLD(uRegValue,
                           MDSS_HW_UBWC_PREDICTION_MODE,
                           PREDICTION_MODE,
                           pgsMdpHwInfo->sUBWCConfig.uPredictionMode);

  // Update the UBWC Predicition mode register setting with new value
  HWIO_MDSS_HW_UBWC_PREDICTION_MODE_OUT(uRegValue);
...
/****************************************************************************
*
** FUNCTION: HAL_MDP_MicroIdleConfig()
*/
/*!
* \brief
*     Configure micro idle globally, on control paths and on source pipes
*
* \param [in]     bEnable  - Enable/Disable Micro idle HW
*
* \retval None
*
****************************************************************************/
static void HAL_MDP_MicroIdleConfig(bool32 bEnable)
{
  HAL_MDP_GlobalMicroIdleConfigType       sGlobalUidleCfg;
  HAL_MDP_ControlPathId                   eCtrlPathId;
  HAL_MDP_ControlPath_MicroIdleConfigType sCtrlPathUidleCfg;
  HAL_MDP_ControlPathConfigType           sCtrlPathCfg;
  HAL_MDP_SourcePipeId                    eSourcePipeId;
  HAL_MDP_LayerType                       eLayerType;
  HAL_MDP_SSPP_MicroIdleConfigType        sSSPPUidleConfig;
  HAL_MDP_SourcePipeConfigType            sSourcePipeConfig;

  // Setup global micro idle config structure
  MDSS_OSAL_MEMZERO(&sGlobalUidleCfg, sizeof(HAL_MDP_GlobalMicroIdleConfigType));

  sGlobalUidleCfg.bControllerEnable = bEnable;
  sGlobalUidleCfg.bWDTimerEnable    = FALSE;   // By default, disable micro idle watchdog timer
  sGlobalUidleCfg.bCounterEnable    = bEnable; 

  // Configure micro idle global registers
  HAL_MDP_GlobalMicroIdleConfig(&sGlobalUidleCfg);

  // Setup control path micro idle configuration structures
  MDSS_OSAL_MEMZERO(&sCtrlPathUidleCfg, sizeof(HAL_MDP_ControlPath_MicroIdleConfigType));
  MDSS_OSAL_MEMZERO(&sCtrlPathCfg, sizeof(HAL_MDP_ControlPathConfigType));

  sCtrlPathUidleCfg.bEnable      = bEnable;
  sCtrlPathCfg.psMicroIdleConfig = &sCtrlPathUidleCfg;

  for (eCtrlPathId = HAL_MDP_CONTROL_PATH_0; eCtrlPathId <= HAL_MDP_GET_CONTROL_PATHS_COUNT(); eCtrlPathId++)
  {
    if (0 != uMDPControlPathRegBaseOffset[eCtrlPathId])
    {
      // Configure micro idle control path register
      HAL_MDP_ControlPath_Setup(eCtrlPathId, &sCtrlPathCfg, 0);
    }
  }

  // Setup source pipe micro idle configuration structures
  MDSS_OSAL_MEMZERO(&sSSPPUidleConfig, sizeof(HAL_MDP_SSPP_MicroIdleConfigType));
  MDSS_OSAL_MEMZERO(&sSourcePipeConfig, sizeof(HAL_MDP_SourcePipeConfigType));

  sSSPPUidleConfig.bEnable            = bEnable;
  sSourcePipeConfig.psMicroIdleConfig = &sSSPPUidleConfig;

  for (eSourcePipeId = HAL_MDP_SOURCE_PIPE_NONE; eSourcePipeId < HAL_MDP_SOURCE_PIPE_MAX; eSourcePipeId++)
  {
    if (0 != uMDPSSPPRegBaseOffset[eSourcePipeId])
    {
      eLayerType = HAL_MDP_LAYER_TYPE_NONE;

      if ((HAL_MDP_SOURCE_PIPE_VIG_BASE <= eSourcePipeId) && (HAL_MDP_SOURCE_PIPE_VIG_END >= eSourcePipeId))
      {
        eLayerType = HAL_MDP_LAYER_TYPE_VIG;
      }
      else if (((HAL_MDP_SOURCE_PIPE_DMA_BASE <= eSourcePipeId) && (HAL_MDP_SOURCE_PIPE_DMA_END >= eSourcePipeId)) ||
               ((HAL_MDP_SOURCE_PIPE_DMA_BASE <= eSourcePipeId) && (HAL_MDP_SOURCE_PIPE_DMA_END >= eSourcePipeId)))
      {
        eLayerType = HAL_MDP_LAYER_TYPE_DMA;
      }

      // Configure micro idle source pipe register
      HAL_MDP_SourcePipe_Setup(eSourcePipeId, eLayerType, &sSourcePipeConfig, 0x0);
    }
  }
}
...
/*!
* \brief
*     Configure global micro idle registers
*
...
{
  if ((NULL != psGlobalUidleCfg) &&
      (NULL != pgsMdpHwInfo->psMicroIdleSettingsInfoType) &&
      (NULL != pgsMdpHwInfo->psMicroIdleSettingsInfoType->psGlobalMicroIdleSettingsInfo))
  {
    uint32 uControllerVal  = 0;
    uint32 uWDTimerVal     = 0;
    uint32 uWDTimer2Val    = 0;
    uint32 uWDTimerLoadVal = 0;
    uint32 uCounterVal     = 0;

    if (FALSE == psGlobalUidleCfg->bControllerEnable)
    {
      // Disable uidle controller 
      uControllerVal = HWIO_OUT_FLD(uControllerVal, MDP_UIDLE_CTL, ENABLE, (psGlobalUidleCfg->bControllerEnable) ? 1 : 0);
      HWIO_MDP_UIDLE_CTL_OUT(uControllerVal);

      // Clear watchdog timer settings
      uWDTimerVal = HWIO_OUT_FLD(uWDTimerVal, MDP_UIDLE_WD_TIMER_CTL, CLEAR, 1);
      HWIO_MDP_UIDLE_WD_TIMER_CTL_OUT(uWDTimerVal);

      // Clear counter settings
      uCounterVal = HWIO_OUT_FLD(uCounterVal, MDP_UIDLE_GATE_CNTR_CTL, CNTR_CLR, 1);
      HWIO_MDP_UIDLE_GATE_CNTR_CTL_OUT(uCounterVal);
    }
    else
    {
      HAL_MDP_GlobalMicroIdleSettingsInfo* psUIdleSettings = pgsMdpHwInfo->psMicroIdleSettingsInfoType->psGlobalMicroIdleSettingsInfo;

      // Enable micro idle controller settings
      uControllerVal = HWIO_MDP_UIDLE_CTL_IN;

      uControllerVal = HWIO_OUT_FLD(uControllerVal, MDP_UIDLE_CTL, ENABLE, (psGlobalUidleCfg->bControllerEnable) ? 1 : 0);
      uControllerVal = HWIO_OUT_FLD(uControllerVal, MDP_UIDLE_CTL, FAL10_EXIT_CNT, psUIdleSettings->uFal10ExitCnt);
      uControllerVal = HWIO_OUT_FLD(uControllerVal, MDP_UIDLE_CTL, FAL10_EXIT_DANGER, psUIdleSettings->uFal10ExitDanger);
      uControllerVal = HWIO_OUT_FLD(uControllerVal, MDP_UIDLE_CTL, FAL10_DANGER, psUIdleSettings->uFal10Danger);

      HWIO_MDP_UIDLE_CTL_OUT(uControllerVal);

      // Watchdog timer settings
      uWDTimerVal = HWIO_OUT_FLD(uWDTimerVal, MDP_UIDLE_WD_TIMER_CTL, CLEAR, 0); // Explicit clear not required

      HWIO_MDP_UIDLE_WD_TIMER_CTL_OUT(uWDTimerVal);

      uWDTimer2Val = HWIO_MDP_UIDLE_WD_TIMER_CTL2_IN;

      uWDTimer2Val = HWIO_OUT_FLD(uWDTimer2Val, MDP_UIDLE_WD_TIMER_CTL2, HEARTBEAT_ENABLE, 0); // Heartbeat not required
      uWDTimer2Val = HWIO_OUT_FLD(uWDTimer2Val, MDP_UIDLE_WD_TIMER_CTL2, GRANULARITY, psUIdleSettings->uWDTimerGranularity);
      uWDTimer2Val = HWIO_OUT_FLD(uWDTimer2Val, MDP_UIDLE_WD_TIMER_CTL2, ENABLE, (psGlobalUidleCfg->bWDTimerEnable) ? 1 : 0);

      HWIO_MDP_UIDLE_WD_TIMER_CTL2_OUT(uWDTimer2Val);

      uWDTimerLoadVal = HWIO_MDP_UIDLE_WD_TIMER_LOAD_VALUE_IN;

      uWDTimerLoadVal = HWIO_OUT_FLD(uWDTimerLoadVal, MDP_UIDLE_WD_TIMER_LOAD_VALUE, VALUE, psUIdleSettings->uWDTimerLoadValue);

      HWIO_MDP_UIDLE_WD_TIMER_LOAD_VALUE_OUT(uWDTimerLoadVal);

      // Micro idle counter settings
      uCounterVal = HWIO_MDP_UIDLE_GATE_CNTR_CTL_IN;

      uCounterVal = HWIO_OUT_FLD(uCounterVal, MDP_UIDLE_GATE_CNTR_CTL, CNTR_CLR, 0); // Explicit clear not required
      uCounterVal = HWIO_OUT_FLD(uCounterVal, MDP_UIDLE_GATE_CNTR_CTL, CNTR_EN, (psGlobalUidleCfg->bCounterEnable) ? 1 : 0);
