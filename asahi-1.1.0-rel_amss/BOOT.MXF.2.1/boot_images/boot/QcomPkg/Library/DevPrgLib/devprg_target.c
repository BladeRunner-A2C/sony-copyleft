...
/* Get thetransport type from boot */
enum devprg_transport_type devprg_get_transport_type(void)
{
  ...
  if (boot_shared_imem_cookie_ptr != NULL)
  {
    ...
    boot_device = (boot_media_type)boot_shared_imem_cookie_ptr->boot_device_type;
  }
...
/* Return the pointer to the transport functions */
struct devprg_transport *devprg_transport_init_target(enum devprg_transport_type transport_type)
{
  if (transport_type == DEVPRG_TRANSPORT_PCIE) 
  {
    return devprg_transport_pcie_init();
  }
  else /*Default to USB transport*/
  {
    return devprg_transport_usb_init();
  }
