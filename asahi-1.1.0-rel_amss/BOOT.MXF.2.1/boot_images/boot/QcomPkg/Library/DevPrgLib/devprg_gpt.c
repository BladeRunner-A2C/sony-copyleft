 ...
 *
 * Copyright (c) 2016-2018,2021,2023 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
----------   ---   ---------------------------------------------------------
2023-06-26   jt    Update partition size of the protective MBR entry to the disk size 
2021-06-24   sa    Copy last LBA for the backup GPT during fixgpt.
2021-02-02   wek   Expose the incremental CRC calculation to other files.
2017-09-18   wek   Use byte array instead int array to remove compiler warn.
2017-07-21   wek   Free the buffer after use.
...
PACKED struct devprg_mbr_partition_record {
  uint8 bootable;
...
PACKED struct devprg_mbr_layout {
  uint8 boot_code[440];
  uint8 disk_signature[4];
  uint8 pad[2];       /* There is no specified use for this field. */
  struct devprg_mbr_partition_record records[4];
...
uint32
devprg_calc_crc32(const uint8 *data, uint32 nbytes, uint32 seed)
{
  return gpt_calc_crc32(data, nbytes, seed);
}
...
gpt_is_entry_valid (struct devprg_gpt_entry *gpt_entry)
{
  ...
  /* Check if Type GUID is zeros, use uint8 instead of uint32 because some
   * compilers or target don't allow unaligned access of uint32. */
  buffer = (uint8 *)&gpt_entry->partition_type_guid.data1;
              ...
              int grow_last)
{
  ...
  struct devprg_mbr_layout *mbr_layout = NULL; 
  struct devprg_mbr_partition_record *precord = NULL; 
  uint8 mbr_index = 0; 
  uint8 *buffer;
  ...
  dp_res_t error = DEVPRG_SUCCESS;
  uint64 mbr_lba = 0, gpt_lba = 1; /* Primary should be at 1. */
  uint64 lba;
  ...
  /* Read MBR and fix the partition size of the protective MBR entry to the disk size */
  buffer = pparser_read_lba (mbr_lba, 1, cache);
  if (buffer == NULL)
  {
    DP_LOGE("Can't read header at %d for fixing MBR", mbr_lba);
    return DEVPRG_ERROR_STORAGE;
  }

  mbr_layout = (struct devprg_mbr_layout *)buffer; 

  for (mbr_index = 0; mbr_index < 4; mbr_index++)
  {
    precord = &mbr_layout->records[mbr_index];

    /* Look for the protective MBR entry and fix the partition size to the size of the disk */
    if (precord->partition_type == PROTECTIVE_MBR_TYPE)
    {
      if (dev_size_blocks > UINT32_MAX) 
      {
        dev_size_blocks = UINT32_MAX; 
      }

      if (precord->size_in_blocks != dev_size_blocks)
      {
        precord->size_in_blocks = dev_size_blocks;

        error = devprg_storage_write (handle, mbr_layout, mbr_lba, 1);
        if (error != DEVPRG_SUCCESS)
        {
          DP_LOGE("GPT failed to write protective MBR error %d", error);
          return DEVPRG_ERROR_STORAGE;
        }
      }
