...
{
  DEVPRG_USB_ZLP_KEY,
  DEVPRG_TRANSPORT_SHUTDOWN_KEY, /* Prepare to reboot the device. */
...
int devprg_transport_init(enum devprg_transport_type t_type);

/* Returns the pointer to the transport layer functions */
struct devprg_transport *devprg_transport_init_target(enum devprg_transport_type t_type);


/* Gets the transport type to be initialised from boot context handle */
enum devprg_transport_type devprg_get_transport_type(void);
