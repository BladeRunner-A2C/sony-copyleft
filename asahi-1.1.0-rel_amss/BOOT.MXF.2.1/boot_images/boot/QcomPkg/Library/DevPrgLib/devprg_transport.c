...
/*Initialise the supported transport layer */
int devprg_transport_init(enum devprg_transport_type transport_device)
{
  transport_reset_data.reboot_type = DEVPRG_REBOOT_NONE;
  transport_reset_data.timeout_ms = 0;


  if(transport_device < DEVPRG_TRANSPORT_MAX)
  {
     transport = devprg_transport_init_target(transport_device);
     return DEVPRG_SUCCESS;
  }
