...
----------   ---   ---------------------------------------------------------
2022-08-24   sk    Rename BAD_BLOCK_FILL to BAD_BLOCK_NO_SKIP in skip_bb
2021-04-23   wek   Add flush operation to storage.
2020-12-08   jtu   Add HPB provisioning changes
2019-10-10   jtu   Change Turbo Write to Write Booster
2019-08-14   jtu   Add fields for JEDEC-compliant turbo write
2019-04-05   jtu   Add fields for turbo write
2019-03-04   wek   Add NVMe support.
...
/* The partition/LUN used for boot partition changes depending on storage
 * technology. UFS uses LUN1 and LUN2, NVMe has dedicated commands for
 * boot partition. To make the interface consistent between UFS and NVMe
 * we define two special values that would map to Boot partition 1 and 2 */
#define DEVPRG_BOOT_PARTITION_1 0xB007
#define DEVPRG_BOOT_PARTITION_2 0xB008
...
enum devprg_skip_bb
{
  ...
  BAD_BLOCK_NO_SKIP, /* Fill the data buffer while reading with a pattern */
};
...
/*
 * Flush data to backing store (NAND/NOR)
 *
 * @param device   Handle returned from devprg_storage_open()
 *
 * @return Error code as given by dp_res_t.
 **/
dp_res_t devprg_storage_flush(struct devprg_storage *device);
...
struct devprg_ufs_unit_descr {
  ...
  uint16    wLUMaxActiveHPBRegions;      /**< Maximum number of HPB Regions for this LU */
  uint16    wHPBPinnedRegionStartIdx;    /**< HPB Pinned Region Start Offset */
  uint16    wNumHPBPinnedRegions;        /**< Number of HPB pinned Regions for this LU */
  uint64    qWriteBoosterBufferSizeInKb; /**< Write booster buffer size in kilobytes for this LU */
};
...
struct devprg_ufs_config_descr {
  ...
  uint8     bWriteBoosterBufferPreserveUserSpaceEn; /**< 1 to reduce user space */
  uint8     bWriteBoosterBufferType;                /**< 1 for shared write booster buffer */
  uint64    qSharedWriteBoosterBufferSizeInKb;      /**< Shared write booster buffer size in kilobytes */
