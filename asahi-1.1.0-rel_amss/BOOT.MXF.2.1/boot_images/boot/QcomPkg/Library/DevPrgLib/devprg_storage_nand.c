...
----------   ---   ---------------------------------------------------------
2023-02-03   eo    Fix KW warnings
2022-12-14   sk    Fix bad block pattern fill issue while reading
2022-10-27   gk    Added Jtag id check to defferentiate Pinnacles and Kuno
2022-10-22   gml   Fix for last pages write issue
2022-09-29   tvra  Add jtag_id check for auto targets.
2022-09-27   gk    Updated APIs to make devprg work on NAND.
...
int is_block_read_only(uint32 block)
{
  ...
  {
	 return (block>MIBIB_BLOCK_SEARCH_MAX_AUTO)? 0 : 1;
  }
  else
  {
	 return (block>MIBIB_BLOCK_SEARCH_MAX)? 0 : 1;
  }
} 
#else
int is_block_read_only(uint32 block)
...
{
  int fresult = 0;
  enum nand_block_state block_state = 0;
                                   ...
                                   struct devprg_io_options *io_options)
{
  ...
  if(skip_bb != BAD_BLOCK_SKIP && skip_bb != BAD_BLOCK_NO_SKIP)
  {
    DDP_LOGE("[%d] invalid params passed \n", __LINE__);
    return DEVPRG_ERROR_INVAL_PARAM;
  }
                                  ...
                                  uint64 num_of_pages)
{
  ...
  if(skip_bb == BAD_BLOCK_NO_SKIP)
  {
    pba = start_block_num;
    if(FALSE == DEVPRG_STORAGE_NAND_BLOCK_IS_BAD(dev->bbt, start_block_num))
      error = DEVPRG_SUCCESS;
    else
      error = DEVPRG_ERROR_NOT_SUPPORTED;
  }
  ...
  while( total_pages_to_read )
  {
    ...
    { /* reading from next block */
      ...
      if(error != DEVPRG_SUCCESS)
      {
        ...
        {
          /*  in case where BAD_BLOCK_NO_SKIP option is set
      ...
      if((BAD_BLOCK_NO_SKIP == skip_bb) && (pba - start_block_num))
      {
                                    ...
                                    uint64 start_page, uint64 num_of_pages)
{
  ...
  while( total_pages_to_write )
  {
    ...
    {
      /* page_count write succeeded, update data buffer */
      data += (page_count * nand_page_size);
