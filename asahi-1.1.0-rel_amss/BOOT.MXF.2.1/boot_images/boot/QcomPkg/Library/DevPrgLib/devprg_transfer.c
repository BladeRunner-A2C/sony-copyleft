...
2021-06-08   wek   Change header size when testing VIP with no signature.
2020-06-16   wek   Add USB shutdown API for reboot/powerdown.
2019-05-17   wek   Allow multiple runs of VIP when there is single command
2018-18-09   wek   Fix temporary variables used for VIP debug.
2017-11-28   yps   Replaced banned functions
2017-11-01   wek   Support signed tables larger than 8Kb
...
void devprg_log_hash(uint8 *data, size_t size)
{
  ...
  for(i = 0; i < size; i++)
  {
    ...
    if (result > 0 && result <= 3) /* Compare with 3 to get around lint/SABOP warning. */
    {
      offset += result;
      remain -= result;
    }
...
int devprg_vip_send_xml_response(int ack)
{
  ...
  rstatus = devprg_transport_tx_blocking(xml_buf, res);
...
dp_res_t devprg_rx_queue(void *data, size_t size)
{
  ...
  {
    vip->payload = data;
    return devprg_transport_rx_queue(data, size);
  }
...
/* We can test VIP on a device with secure boot disabled by enabling the line below
   and forcing the VIP state to VIP_INIT on init function.
...
*/
    ...
    {
      /* Note: We may have VIP enabled and secboot == 0 for testing VIP.
       * For this cases it is expected the host to use --createvipdigests=none */
      table = vip->table;
      table_size = rsize;
    }
    ...
    /* Some versions of fh_loader the first table of hashes has space for
       54 hashes, but it is possible only some of them are populated,
       leaving the last set of hashes as zero.
       The code needs to know how many valid hashes are in the table in
       order to determine when to go back to VIP_INIT state.
       This enables running single command using fh_loader back to back
       with no error. (e.g. fh_loader.exe --nop then another call
       fh_loader --getstorageinfo=0)) With out this code the second call
       would return error. */
    for (i = vip->max_index; i >= 0;)
    {
      uint8 *last_hash;
      size_t offset;
      int is_zero = 1;
      i--;
      last_hash = vip->curr_hash + (DEVPRG_HASH_SIZE * i);
      for (offset=0; offset < DEVPRG_HASH_SIZE; offset++)
      {
        if (*(last_hash + offset) !=0)
        {
          is_zero = 0;
          break;
        }
  ...
  if (vip->state == VIP_GET_NEXT_TABLE)
  {
    ...
    memscpy (expected_hash, DEVPRG_HASH_SIZE, vip->curr_hash, sizeof(expected_hash));

    rstatus = devprg_transport_rx_blocking(vip->table, vip->table_size, &rsize);
    if (rstatus != DEVPRG_SUCCESS)
    {
  ...
  {
    vip->payload = data;
    rstatus = devprg_transport_rx_queue(data, size);
    return rstatus;
  }
...
dp_res_t devprg_rx_queue_wait(size_t *rx_bytes)
{
  ...
  if (vip->state == VIP_DISABLED)
  {
    rstatus = devprg_transport_rx_queue_wait(rx_bytes);
...
int64 devprg_rx_blocking(void *data, size_t size)
{
  ...
  if (vip->state == VIP_DISABLED)
  {
    res = devprg_transport_rx_blocking(data, size, &ret_size);
    read_size = ret_size;
    return read_size;
  }
...
{
  return devprg_transport_tx_blocking(data, size);
}

dp_res_t devprg_transfer_set_zlp_mode(unsigned int val)
{
  return devprg_transport_set_mode(DEVPRG_USB_ZLP_KEY, val);
}

int devprg_transfer_init(enum devprg_transport_type t_type)
{
#if defined(FEATURE_DEVICEPROGRAMMER_SOMC) && defined(FEATURE_SOMC_SKIP_SECBOOT_CHECK)
  int result;
#else  /* !(FEATURE_DEVICEPROGRAMMER_SOMC && FEATURE_SOMC_SKIP_SECBOOT_CHECK) */
  int secboot, result;
#endif /* !(FEATURE_DEVICEPROGRAMMER_SOMC && FEATURE_SOMC_SKIP_SECBOOT_CHECK) */
  struct vip_data *vip = &vip_data;
  ...
  result = devprg_transport_init(t_type);
...
dp_res_t devprg_transfer_shutdown(void)
{
  return devprg_transport_set_mode(DEVPRG_TRANSPORT_SHUTDOWN_KEY, 1);
}
