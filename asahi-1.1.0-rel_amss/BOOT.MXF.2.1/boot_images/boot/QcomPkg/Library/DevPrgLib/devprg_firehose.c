...
----------   ---   ---------------------------------------------------------
2024-01-29   wek   Verify the patch offset is within bounds.
2023-01-09   gml   Added Pinnacles DevPrg PCIe support
2022-12-14   sk    Rename BAD_BLOCK_FILL to BAD_BLOCK_NO_SKIP
2022-02-02   wek   Remove poke command.
2022-01-27   wek   Display the Chip ID in the logs.
2021-04-23   wek   Make program call flush after write.
2021-02-02   wek   Add support for patching with small buffers.
2020-12-17   wek   Make the entry function for device programmer void.
2020-12-08   jt    Add HPB provisioning changes
2020-10-28   wek   Do wrap around when reaching end of storage in benchmarks
2020-07-23   rgy   Obfuscate pbl_shared to void*
2020-06-20   sb    Update slot number to eMMC slot by default.
2020-06-19   wek   Add USB shutdown API for reboot/powerdown.
2020-05-29   wek   Output error when invalid XML command is detected.
2019-10-10   jt    Update Turbo Write to Write Booster
2019-08-14   jt    Define JEDEC-compliant turbo write parameters
2019-04-05   jt    Add support to get turbo write parameters
2018-10-04   wek   Power down modes moved to header file.
2018-09-21   wek   Add key/value to return the sha256 in response xml.
2018-02-14   wek   Transfer the same log lines for peek/poke as before.
2018-01-23   wek   Return error for peek/poke if secure boot is enabled
2018-01-15   wek   Implement command to get sha256 from region in storage
...
/* Added by SONY Start */
/* SOMC addition, needed also in QCT built version of loader */
...
/* Converts an byte array into an ascii string representing the array in HEX.
 *
 * param  in            Input array
 * param  in_size       size of input array.
 * param  out[out]      Output array,
 * param  out_size[out] size of output array.
 *
 * returns  0 on success, -1 on error
 **/
static int convert_array_to_ascii(const uint8 *in, size_t in_size,
                                  char *out, size_t out_size)
{
  ...
  /* Check for enough space in the buffers. Each byte of the input will take
   * two characters in the output plus the null character. */
  ...
  for (i = 0; i < in_size; i++)
  {
    int remain = out_size - offset;
    len = snprintf(&out[offset], remain, "%02X", in[i] & 0xFF);
    if (len < 0 || remain < len) /* Add len>remain because of static analysis.*/
      goto error;
...
error:
  DP_LOGE("Error while converting array to ascii %d", len);
  memset(out, 0, out_size);
  return -1;

}

static enum devprg_storage_type get_storage_type (const char *storage_type)
{
  ...
  if(strncasecmp("NVMe", storage_type, 4) == 0)
    return DEVPRG_STORAGE_NVME;

  return DEVPRG_STORAGE_NULL;
}

static const char *get_storage_type_string (enum devprg_storage_type type)
{
...
static int get_uint16_attr(struct dp_xml_handle *xml_handle, const char *attr,
                           uint16 *value)
{  
  int res;
  uint64 val64;
  ...
  if (val64 > UINT16_MAX)
  {
    DP_LOGE("Integer value for %s is too high %d %d", attr, val64, UINT16_MAX);
    return -1;
  }
  *value = (uint16) val64;
...
/* Check if the sector size in the XML matches the one in the device.
 * This size may be different than the one reported by get storage info,
 * for example in SPI-NOR the sector size configured is 4K but the real
...
static int send_response_buffer(void *buffer, size_t buf_size)
{
  int result = 0;
  dp_res_t error;
...
static int send_response_xml(void *buffer, size_t buf_size,
                            int ack, int rawmode)
{
  int result = -1;
  result = devprg_xml_build_response(buffer, buf_size, ack, rawmode);
  if (result > 0)
    result = send_response_buffer(buffer, result);
                          ...
                          uint64 start_sector)
{
  ...
  if(result == 0 && bad_block != 0)
    io_options.skip_bb = BAD_BLOCK_NO_SKIP;
  else
...
static int handle_write(struct dp_xml_handle *xml_handle)
{
  ...
  ack = 1; rawmode = 1;
  result = send_response_xml(data[0], buf_size, ack, rawmode);
  if (result != 0)
    goto cleanup;
  ...
  if (error == DEVPRG_SUCCESS)
    error = devprg_storage_flush(store_handle);
...
static int handle_read(struct dp_xml_handle *xml_handle)
{
  ...
  ack = 1; rawmode = 1;
  result = send_response_xml(data, buf_size, ack, rawmode);
  if (result != 0)
    goto cleanup;
...
#ifndef FEATURE_DEVICEPROGRAMMER_SOMC
                              ...
                              uint32 sec_size, uint32 *crc_out)
{
  ...
  do
  {
    read_size = (num_bytes < buf_size) ? num_bytes : buf_size;
    /* Round up to the sector size. */
    num_sectors = ((read_size - 1) / sec_size) + 1;

    error = devprg_storage_read(store_handle, buffer, start_sector, num_sectors);
    if (error != DEVPRG_SUCCESS)
      return -1;

    crc = devprg_calc_crc32(buffer, read_size, crc);
    num_bytes -= read_size;
...
static int handle_patch(struct dp_xml_handle *xml_handle)
{
  ...
  uint64 patch_value, overflow_patch_value = UINT64_MAX, start_sector;
  uint32 sector_size;
  uint32 byte_offset;
  ...
  {
    DP_LOGE("Patch byte_offset too large %d>=%d", byte_offset, sector_size);
    result = -1;
    goto cleanup;
  }
...
//    DP_LOGW("PATCHING USING CRC IS DEPRECATED and scheduled to be removed%c",'.');
  ...
  /* Based on patch_size_in_bytes, make sure patch value doesn't overflow. If the
     patch is for the partition size of the protective MBR entry, set the value
     to 0xFFFFFFFF if overflow is detected. Otherwise, return error. */
  if (patch_size_in_bytes < 8)
  {
    overflow_patch_value = (1ULL << (8 * patch_size_in_bytes)) - 1;
  }
  else
  {
    overflow_patch_value = UINT64_MAX;
  }

  if (patch_value > overflow_patch_value) 
  {
    if ((start_sector == 0) && (byte_offset == PROTECTIVE_MBR_PARTITION_ENTRY_BYTE_OFFSET) && 
        (patch_size_in_bytes == PROTECTIVE_MBR_PARTITION_SIZE_IN_BYTES))
    {
      patch_value = 0xFFFFFFFF; 
    }
    else
    {
      DP_LOGE("Patch value %d larger than max value allowed %d",
              patch_value, overflow_patch_value); 
      error = DEVPRG_ERROR_INVAL_PARAM; 
      goto cleanup; 
    }
  }

  /* The code above makes sure byte_offset < sector_size, if its not it errors
     out. The second check here is will prevent out of bounds memory access,
     in case the code above is modified incorrectly. */
  if (buf_size > byte_offset)
...
#ifdef FEATURE_DEVICEPROGRAMMER_SOMC
#define SOMCLIB_CMD_HEAP_SIZE (8*1024)
byte somclib_cmd_heap[SOMCLIB_CMD_HEAP_SIZE]  __attribute__ ((section (".bss.somclib_heap"))) = {0};

static int handle_auth(struct dp_xml_handle *xml_handle)
{
  int result = -1;
  uint16_t sake_method = 1;
  uint16_t sake_state = 0;
  uint8_t sake_level = 1;
  size_t bytes_read_in_layer;
  size_t buf_len = 0;
  size_t buf_size, resp_buf_size;
  uint64 size_in_bytes;
  void *data = NULL;
  int ack, rawmode;

  char *resp_buffer = NULL; /* For response message */
  struct devprg_xml_write *xmlw;
  const char trb_msg[]="Couldn't get trans buf";
  const char szb_msg[]="Attr 'size_in_bytes'=";
  const char dl_msg[]="Unexpected data length : Expected:";

  byte* raw_data_buffer;
  int sake_res = FCSERR_OK;
  ...
  resp_buffer = devprg_get_transfer_buffer(&resp_buf_size);
  if (resp_buffer == NULL)
  {
    DP_LOGE("%s", trb_msg);
    result = -1;
  ...
  result = get_uint64_attr(xml_handle, "size_in_bytes", &size_in_bytes);
  if (result != 0) {
    DP_LOGE("Failed to get attr 'size_in_bytes', res=%d", result);
    result = -1;
    goto cleanup;
  }
  ...
  if (size_in_bytes < 5)
  {
    DP_LOGE("%s" ,"%lld too small", szb_msg, size_in_bytes);
    result = -1;
    goto cleanup;
  }

  if (size_in_bytes > protoc_cfg.max_transfer_size)
  {
    DP_LOGE("%s","%lld too big", szb_msg, size_in_bytes);
    result = -1;
    goto cleanup;
  }
  ...
  bytes_read_in_layer = devprg_rx_blocking(data, size_in_bytes);
  if (bytes_read_in_layer == 0)
  {
    DP_LOGE("%s", "USB read failed. Returned 0 bytes.");
//    DP_LOGE("Did you set ZlpAwareHost='%d' in your configure tag", 1);
    result = -1;
    goto cleanup;
  }

  if (bytes_read_in_layer != size_in_bytes)
  {
    DP_LOGE("%s %d, Was: %d", dl_msg, size_in_bytes, bytes_read_in_layer);
    result = -1;
    goto cleanup;
  }

  /* Method 0x0001 Sake authentication */
  /* Method 0x0000 Part96 (Deprecated) */
  if (raw_data_buffer[0] != 0 || raw_data_buffer[1] != 1)
  {
    DP_LOGE("%s", "Unexp method");
    result = -1;
    goto cleanup;
  }

  /* data[2] and data[3] = big endian representation of SAKE state */
  /* Only valid states are 0 + 1 */
  if (!(raw_data_buffer[2] == 0 && raw_data_buffer[3] == 0) &&
      !(raw_data_buffer[2] == 0 && raw_data_buffer[3] == 1))
  {
    DP_LOGE("%s", "Unexpected state");
    result = -1;
    goto cleanup;
  }

  sake_state = raw_data_buffer[2] * 256 + raw_data_buffer[3];
  /* SAKE state 0 = Authentication request */
  if (sake_state == 0)
  {
    // Sake hello
    if (bytes_read_in_layer != 5)
    {
      DP_LOGE("%s %d, Was: %d", dl_msg, 5, bytes_read_in_layer);
      result = -1;
      goto cleanup;
    }
    /* Authentication level 1 = Production */
    if (raw_data_buffer[4] != 1)
    {
      DP_LOGE("%s", "Unexpected level");
      result = -1;
      goto cleanup;
    }
  }
  else
  {
    ...
    if (bytes_read_in_layer < 5)
    {
      DP_LOGE("Unexpected data length: Expected: >=5, Was: %d", bytes_read_in_layer);
      result = -1;
      goto cleanup;
    }
    // Verify response
    // If Ok Set sake_state = 1
    // (Not used: If Ok but more to autenticate Set sake_state = 0)
    // If not Ok Set sake_state = 3
    buf_len = bytes_read_in_layer - 4;
    // Use global buffer for temporary heap
    somclib_setup_allocation(somclib_cmd_heap, SOMCLIB_CMD_HEAP_SIZE);
    sake_res = somclib_cmd_authenticate(&raw_data_buffer[4],
                                        &sake_state,
                                        &buf_len,
                                        &sake_method,
                                        &sake_level);
    somclib_teardown_allocation();
    if (sake_res != FCSERR_OK)
    {
      DP_LOGE("Sake verify failed, error code: %x state: %x", sake_res, sake_state);
      result = -1;
      goto cleanup;
    }
  }

  // Handle firehose response
  if (sake_state == 0)
  {
    // Generate Sake Hello and set up firehose raw response
    buf_len = 512;
    ...
    if (sake_res != FCSERR_OK)
    {
      DP_LOGE("Sake hello failed, error code: %x state: %x", sake_res, sake_state);
      result = -1;
      goto cleanup;
    }

    ack = 1; rawmode = 1;
    xmlw = devprg_xml_resp_open(resp_buffer, buf_size);
    devprg_xml_resp_tag(xmlw, "response");
    devprg_xml_resp_ack(xmlw, ack);
    devprg_xml_resp_rawmode(xmlw, rawmode);
    devprg_xml_resp_attr_i(xmlw, "size_in_bytes", buf_len+4);
    devprg_xml_resp_close(xmlw);
    result = send_response_buffer(resp_buffer, strlen(resp_buffer));
    if (result != 0)
    {
      DP_LOGE("Send response failed, error code: %x", result);
      result = -1;
      goto cleanup;
    }

    raw_data_buffer[0] = (sake_method & 0xFF00) >> 8;
    raw_data_buffer[1] = sake_method & 0xFF;
    raw_data_buffer[2] = (sake_state & 0xFF00) >> 8;
    raw_data_buffer[3] = sake_state & 0xFF;

    devprg_tx_blocking(raw_data_buffer, buf_len+4);

    result = 0;
    goto cleanup;
  }
  else if (sake_state == 1)
  {
    // Authentication Ok firehose response
    ack = 1; rawmode = 1;
    xmlw = devprg_xml_resp_open(resp_buffer, buf_size);
    devprg_xml_resp_tag(xmlw, "response");
    devprg_xml_resp_ack(xmlw, ack);
    devprg_xml_resp_rawmode(xmlw, rawmode);
    devprg_xml_resp_attr_i(xmlw, "size_in_bytes", 4);
    devprg_xml_resp_close(xmlw);
    result = send_response_buffer(resp_buffer, strlen(resp_buffer));
    if (result != 0)
    {
    ...
    devprg_tx_blocking(raw_data_buffer, 4);
  ...
  {
    DP_LOGE("%s", "Failed to verify sake response");
    result = -1;
    goto cleanup;
  }
...
cleanup:
  ...
  if (result != 0)
    DP_LOGE("Send response failed, error code: %x", result);
  devprg_free_transfer_buffer(data);
  devprg_free_transfer_buffer(resp_buffer);
...
static int handle_power(struct dp_xml_handle *xml_handle)
{
  ...
  size_t buf_size;
  const char *string;
  enum devprg_reboot_type power_state = DEVPRG_REBOOT_RESET;
  uint64 delay_sec;
  uint64 delay_usec;
  ...
  if (strncasecmp(string, "reset_to_edl", 13) == 0)
    power_state = DEVPRG_REBOOT_EDL;
  else if (strncasecmp(string, "reset", 5) == 0)
    power_state = DEVPRG_REBOOT_RESET;
  else if (strncasecmp(string, "off", 3) == 0)
    power_state = DEVPRG_REBOOT_POWER_OFF;
  else
    DP_LOGE("power/reset option not specified, default is do reset %d", DEVPRG_REBOOT_RESET);

  result = get_uint64_attr(xml_handle, "DelayInSeconds", &delay_sec);
  if(result == 0)
    delay_usec = delay_sec * 1000000;
  else
    delay_usec = 300000; /* 300 ms. */
  ...
  case DEVPRG_REBOOT_EDL:
    DP_LOGI("bsp_target_reset_edl() %d", power_state);
    break;
  case DEVPRG_REBOOT_POWER_OFF:
    DP_LOGI("bsp_target_poweroff() %d", power_state);
    break;
  case DEVPRG_REBOOT_RESET:
  default:
...
int handle_peek(struct dp_xml_handle *xml_handle)
{
  ...
  DP_LOGA("Using address %p", addr);
  offset = 0;
  memset(output, 0, buf_size);
  for (i = 0; i < size_in_bytes; i++)
  {
    len = snprintf(&output[offset], buf_size - offset, "0x%02X ", addr[i] & 0xFF);
    if (len < 0 || len > 5) /* Compare with 5 to get around lint/SABOP warning. */
    {
...
/* SpiNor protects the first sectors, skip them for the write benchmark */
#define DEVPRG_BENCHMARK_WRITE_START_SECTOR 0x12D;

#define BENCHMARK_MAX_INTERVAL_US   (4 * 1000 * 1000) /* 4 seconds */
...
{
  uint32 sector_size;
  uint64 start_sector, sector_num, last_sector;
  ...
  start_sector = DEVPRG_BENCHMARK_WRITE_START_SECTOR;
  last_sector = dev_info->total_blocks * (dev_info->block_size / sector_size);
  ...
  for (size = 1; size <= sectors_in_buffer; size = size*2)
  {
    ...
    do
    {
      if (sector_num + size > last_sector)
        sector_num = start_sector;
      ...
      {
        result = devprg_storage_flush(store_handle);
      }
      total_io += size;
      sector_num += size;
      count++;
                           ...
                           void *buffer, size_t buf_size, uint32 trials)
{
  ...
  for (size = 1; size <= sectors_in_buffer; size = size*2)
  {
    ...
    if (total_time != 0)
    {
      uint64 throughput;
      throughput = (((uint64)total_io) * 1000 * 1000)/((uint64)total_time);
      DP_LOGI("Read throughput %uBps, Chunk size %u, read %u in %uus",
              throughput, size * sector_size, total_io, total_time);
    }
    else
    {
...
static int benchmark_sha256(void *buffer, size_t buf_size, uint32 trials)
{
  ...
  for (size = 4096; size <= buf_size; size = size*2)
  {
    ...
    if (total_time != 0)
    {
      uint64 throughput;
      throughput = (((uint64)total_io) * 1000 * 1000)/((uint64)total_time);
      DP_LOGI("SHA256 throughput %uBps, Chunk size %u, computed %u in %uus",
                                ...
                                struct devprg_storage_cfg_data *c)
{
  ...
  const char *tag;
  static char has_wb_cfg_been_init = 0;
  ...
  if(result == 0 && lun < sizeof(ufs->unit)/sizeof(ufs->unit[0])/*=8*/)
  {
    ...
    get_uint64_attr(xml, "wb_buffer_size_in_kb", &ufs->unit[lun].qWriteBoosterBufferSizeInKb); 
    get_uint16_attr(xml, "wLUMaxActiveHPBRegions", &ufs->unit[lun].wLUMaxActiveHPBRegions); 
    get_uint16_attr(xml, "wHPBPinnedRegionStartIdx", &ufs->unit[lun].wHPBPinnedRegionStartIdx); 
    get_uint16_attr(xml, "wNumHPBPinnedRegions", &ufs->unit[lun].wNumHPBPinnedRegions); 
    return 0;
  }
  ...
  if (0 == get_uint64_attr(xml, "wPeriodicRTCUpdate", &value))
  {
    ufs->wPeriodicRTCUpdate = value & 0xFFFF;
  }
 
  get_uint8_attr(xml, "bHPBControl", &ufs->bHPBControl); 

  if(has_wb_cfg_been_init == 0) 
  {
    has_wb_cfg_been_init = 1; 
    ufs->bWriteBoosterBufferPreserveUserSpaceEn = 0xFF; 
    ufs->bWriteBoosterBufferType = 0xFF; 
    ufs->qSharedWriteBoosterBufferSizeInKb = 0; 
  }
  get_uint8_attr(xml, "bWriteBoosterBufferPreserveUserSpaceEn", &ufs->bWriteBoosterBufferPreserveUserSpaceEn); 
  get_uint8_attr(xml, "bWriteBoosterBufferType", &ufs->bWriteBoosterBufferType);
  get_uint64_attr(xml, "shared_wb_buffer_size_in_kb", &ufs->qSharedWriteBoosterBufferSizeInKb); 
...
int handle_get_sha256(struct dp_xml_handle *xml_handle)
{
  struct dp_basic_io_data io_data;
  struct dp_basic_dev_data dev_data;
  int result = 0, ack;
  ...
  size_t buf_size, read_size;
  uint64 sectors_remaining, sector_count, sector_size, start_sector;
  struct devprg_sha256_data *sha_handle;
  uint8 sha256out[32];
  char ascii_sha256out[32 * 2 + 1];
  int errors = 0;
  struct devprg_xml_write *xmlw;
  ...
  memset(ascii_sha256out, 0, sizeof(ascii_sha256out));
  ...
  sha_handle = devprg_sha256_init();
  if (sha_handle == NULL)
  {
    DP_LOGE("Can't open sha256 handle %d", 0);
    goto cleanup;
  }
  ...
  {
    sector_count = read_size / sector_size;
    error = devprg_storage_read(store_handle, buffer, start_sector, sector_count);
    if (error != DEVPRG_SUCCESS)
    {
      DP_LOGE("Failed to read data for hashing, sector %ld count%ld, error %d",
              start_sector, sector_count, error);
      errors = 1;
      break;
    }

    result = devprg_sha256_update(sha_handle, buffer, read_size);
    if (result != 0)
    {
      DP_LOGE("Failed to update the SH256 %d", result);
      errors = 1;
      break;
    }
  ...
  result = devprg_sha256_final(sha_handle, sha256out, sizeof(sha256out));
  if (result != 0)
  {
    DP_LOGE("Failed to finalize the SH256 %d", result);
    errors = 1;
  }
  ...
  result = convert_array_to_ascii(sha256out, sizeof(sha256out),
                                  ascii_sha256out, sizeof(ascii_sha256out));

  if (result == 0)
    DP_LOGI("Digest %s", ascii_sha256out);

cleanup:
  if (close_dev(store_handle) != 0)
  {
    DP_LOGE("D Failed to close device%c", '.');
    result = -1;
  }
  ...
  devprg_xml_resp_tag(xmlw, "response");
  devprg_xml_resp_ack(xmlw, ack);
  devprg_xml_resp_attr_s(xmlw, "rawmode", "false");
  devprg_xml_resp_attr_s(xmlw, "sha256", ascii_sha256out);
  devprg_xml_resp_close(xmlw);
  result = send_response_buffer(buffer, strlen(buffer));
done:
...
#ifdef FEATURE_DEVICEPROGRAMMER_SOMC
// Boolean in column three is only used with FEATURE_DEVICEPROGRAMMER_SOMC
struct devprg_firehose_cmd_list cmd_handlers[] =
{
  {"program", handle_write, TRUE},
  {"read", handle_read, TRUE},
  // {"nop", handle_nop, FALSE},
  {"somcauth", handle_auth, FALSE},
  // {"patch", handle_patch, TRUE},
  {"configure", handle_protocol_configure, FALSE},
  {"setbootablestoragedrive", handle_set_bootable_drive, TRUE},
  {"erase", handle_erase, TRUE},
  {"power", handle_power, FALSE},
  // {"firmwarewrite", handle_firmware_update, TRUE},
  {"getstorageinfo", handle_storage_info, FALSE},
  // {"benchmark", handle_benchmark, TRUE},
  // {"peek", handle_peek, TRUE},
  // {"poke", handle_poke, TRUE},
  // {"emmc", handle_storage_configure, TRUE},
  {"ufs", handle_storage_configure, TRUE},
  {"fixgpt", handle_fix_gpt, TRUE},
};
#else /* !FEATURE_DEVICEPROGRAMMER_SOMC */
struct devprg_firehose_cmd_list cmd_handlers[] =
{
...
#ifdef FEATURE_DEVPRG_PEEK
  {"peek", handle_peek},
#endif
  {"emmc", handle_storage_configure},
  {"ufs", handle_storage_configure},
  {"fixgpt", handle_fix_gpt},
  {"getsha256digest", handle_get_sha256},
};
#endif /* !FEATURE_DEVICEPROGRAMMER_SOMC */

static void log_basic_dev_characteristics(void)
{
  uint32 serial_num = 0, chip_id = 0;
  int res, res2;
  size_t i;
  size_t num_handlers = sizeof(cmd_handlers) /
    sizeof(struct devprg_firehose_cmd_list);
#ifdef FEATURE_DEVICEPROGRAMMER_SOMC
#define SAKE_ROOT_PUB_HASH_ID_LEN (4 + 1)
  char sake_root_id[SAKE_ROOT_PUB_HASH_ID_LEN];
#endif /* FEATURE_DEVICEPROGRAMMER_SOMC */
...
/* Added by SONY Start */
#ifdef FEATURE_DEVICEPROGRAMMER_SOMC
  DP_LOGI("Loader version: %s-%s", FIREHOSE_REV_STR, "SOMC");
  somclib_get_sake_root_id(sake_root_id, SAKE_ROOT_PUB_HASH_ID_LEN);
  DP_LOGI("Sake public key hash identifier: %s", sake_root_id);
#else /* !FEATURE_DEVICEPROGRAMMER_SOMC */
  /* needed also in QCT built version of loader */
  DP_LOGI("Loader version: %s", FIREHOSE_REV_STR);
#endif /* !FEATURE_DEVICEPROGRAMMER_SOMC */
  DP_LOGI("Loader version: %s", QC_IMAGE_VERSION_STRING_AUTO_UPDATED);
/* Added by SONY End */

  res = devprg_get_soc_serial_number(&serial_num);
  res2 = devprg_get_soc_chip_id(&chip_id);
  if (res == 0 || res2 == 0)
    DP_LOGI("Chip serial num (0x%x) Chip ID (0x%x)", serial_num, chip_id);
...
int handle_xml(char *xml_buffer, size_t size)
{
  ...
  for (i = 0; i < num_handlers; i++)
  {
    ...
    {
#ifdef FEATURE_DEVICEPROGRAMMER_SOMC
      if (!(cmd_handlers[i].need_authentication) || somclib_is_authenticated())
      {
#endif /* FEATURE_DEVICEPROGRAMMER_SOMC */
      ...
      break;
          ...
          result = -1;
          DP_LOGW("WARNING Tag %s Needs authentication", cmd_handlers[i].tag);
          break;
      }
#endif /* FEATURE_DEVICEPROGRAMMER_SOMC */
    }
...
static void devprg_hw_init(enum devprg_transport_type t_type)
{
  int result;
  ...
  if(result == DEVPRG_SUCCESS)
  {
    result = devprg_transfer_init(t_type);
  }
...
void main_loop(void)
{
  ...
  char *xml_buffer;
  size_t xml_buf_size;
  enum devprg_transport_type transport_type;

  devprg_buffers_init();
  transport_type = devprg_get_transport_type();
  devprg_hw_init(transport_type);
  devprg_xml_init();
  devprg_storage_init();
#ifdef FEATURE_DEVICEPROGRAMMER_SOMC
  somclib_setup();
