/** @file ResetSystemLib.c
  ...
  Copyright (c) 2011-2016,2018, 2020, 2022 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 11/24/20   am      Move ShutdownEfi calls before PSCI call
...
#define CLEAR_DLOAD_COOKIE          0x001
#define SDI_DISABLE                 0x002
#define SDI_DISABLE_SECURE_RESET    0x004
#define SET_EDL_COOKIE              0x008


STATIC
EFI_STATUS
PerformSelectiveReset (UINT32 OperationFlags)
{
  EFI_STATUS Status = EFI_SUCCESS;
  /* These routines that are called here  */

  /* Do not go into crashdump mode after reset from EDL mode */
  if (OperationFlags & CLEAR_DLOAD_COOKIE)
    ClearDLOADCookie();

  /* Avoid going into SDI path after warm reset */
  if (OperationFlags & SDI_DISABLE)
    SDIDisable();

  /* Here SDIDisableAndReset triggers a Secure WDog reset on secure side
   * that results into warm reset, which is not desirable sometimes.
   * Also WDog reset behavior is different in full secure device compared
   * to non secure device before fuses are blown or debug enabled */
  //if (OperationFlags & SDI_DISABLE_SECURE_RESET)
  //SDIDisableAndReset();

  /* Go into EDL mode in PBL */
  if (OperationFlags & SET_EDL_COOKIE)
    SetEDLCookie();

  /* Call exit boot services */
  ShutdownEfi ();

  /* Do WARM reset to retain the cookies */
...
PlatformSpecificReset(UINTN DataSize, CHAR16 *ResetData)
{
  ...
  {
    Status = PerformSelectiveReset (CLEAR_DLOAD_COOKIE | SDI_DISABLE | SET_EDL_COOKIE);
    if (Status != EFI_SUCCESS)
       DEBUG ((EFI_D_INFO, "ERROR: PSCI call failed: 0x%08x\n", Status));
    return Status;
  }

  if (StrCmp(STR_RESET_PLAT_SPECIFIC_TESTONLY_WARM_RESET, ResetData) == 0)
  {
    Status = PerformSelectiveReset (CLEAR_DLOAD_COOKIE | SDI_DISABLE);
    if (Status != EFI_SUCCESS)
       DEBUG ((EFI_D_INFO, "ERROR: PSCI call failed: 0x%08x\n", Status));
    return Status;
  }
...
{
  EFI_STATUS Status = EFI_SUCCESS;
  BOOLEAN IsPSHoldAddressValid = TRUE;
  ...
  // Check if gPSHoldAddress is 32-bit aligned
  if (gPSHoldAddress == 0 || (gPSHoldAddress & 3) != 0)
  {
    IsPSHoldAddressValid = FALSE;
    DEBUG((EFI_D_WARN, "WARN: PSHoldAddress is 0 or not 32-bit aligned !\n"));
  }

  /* Make PSCI call based on type of reset */
  switch (ResetType)
  {
  ...
  case EfiResetPlatformSpecific:
     {
        if (ResetData !=NULL)
           ...
           Status = PlatformSpecificReset(DataSize, NULL);
        break;
     }
    case EfiResetWarm:
    case EfiResetCold:
  ...
  // De-assert PSHold only if address is valid
  if (IsPSHoldAddressValid)
  {
    /* This register controls the PSHOLD value.
       1: Asserted
       0: Deasserted */
    *(volatile UINT32*)gPSHoldAddress = 0;
  }
...
ConvertToResetReasonStr (UINT32 *ResetReason, CHAR16 *ResetReasonStr, UINT32 *ResetReasonStrLen)
{
  ...
  switch (*ResetReason)
  {
    case 0:
      ...
      StrCpyS (CurrentResetStr, MAX_RESET_STR_LEN, STR_RESET_UNKNOWN);
      break;

    default:
      CurrentResetStrLen = StrLen (STR_RESET_PARAM);
      StrCpyS (CurrentResetStr, MAX_RESET_STR_LEN, STR_RESET_PARAM);
      break;
  }

  if (CurrentResetStrLen <= *ResetReasonStrLen)
  {
    StrCpyS (ResetReasonStr, MAX_RESET_STR_LEN, CurrentResetStr);
    Status = EFI_SUCCESS;
  }
  else
  {
  ...
  )
{
  ...
  Status = HWIOProtocol->MapRegion(HWIOProtocol, "+MPM_POWER_STATE_HOLD", (UINT8**) &gPSHoldAddress);
  if (EFI_SUCCESS != Status)
  {
