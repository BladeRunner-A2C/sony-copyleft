...
JtagProgrammer_spi.c
 ...
 Copyright (c) 2017,2020,2022,2023 Qualcomm Technologies, Incorporated.
 All Rights Reserved.
...
----------   ---     ----------------------------------------------------------
2022-11-14   sc      Replace spinor_open with spinor_open_slot
2020-10-30   wek     Fix round up error of erasing the next block during writes
...
#define JTAG_MAX_SPINOR_SLOTS   2

UINT32 dev_id= 0;
/* NOR device structure */
spinor_handle_t handle_spinor[JTAG_MAX_SPINOR_SLOTS];
spinor_handle_t unit_test_handle[JTAG_MAX_SPINOR_SLOTS];

struct spinor_info dev_info[JTAG_MAX_SPINOR_SLOTS];
...
char device_name[32];
bool dev_is_initialized[JTAG_MAX_SPINOR_SLOTS];
...
static NOR_DEVICE_INFO nor_device_info[JTAG_MAX_SPINOR_SLOTS];

#define SIZE_IN_PAGES(x) ( ((x % nor_device_info[card_slot_in_use].page_size_bytes) == 0) ? x/nor_device_info[card_slot_in_use].page_size_bytes : x/nor_device_info[card_slot_in_use].page_size_bytes+1 )
...
{
  if (dev_is_initialized[card_slot_in_use])
  {
    spinor_close(handle_spinor[card_slot_in_use]);
    dev_is_initialized[card_slot_in_use] = FALSE;
  }
}
...
UINT32 spinor_utils_init(void)
{
  ...
  {
    for(slot = 0; slot < JTAG_MAX_SPINOR_SLOTS; slot++)
    {
        dev_is_initialized[slot] = FALSE;
    }
    first_access = FALSE;
  }
  ...
  if(SPINOR_DEVICE_DONE != spinor_open_slot(&handle_spinor[card_slot_in_use], card_slot_in_use))
  {
    outputMessage("Error Opening Spinor Device\n");
    return SPINOR_DEVICE_FAIL;
  }

  result = spinor_get_info(handle_spinor[card_slot_in_use], &dev_info[card_slot_in_use]);
  ...
  total_block_count = dev_info[card_slot_in_use].block_count;
  page_size_bytes = dev_info[card_slot_in_use].page_size_bytes;

  nor_utils_strcpy(device_name, dev_info[card_slot_in_use].device_name,
                   sizeof(device_name));

  nor_utils_strcpy(nor_device_info[card_slot_in_use].device_name, dev_info[card_slot_in_use].device_name,
                   sizeof(device_name));

  total_pages = dev_info[card_slot_in_use].pages_per_block * total_block_count;
  block_size_bytes = dev_info[card_slot_in_use].pages_per_block * page_size_bytes;

  BLOCK_SIZE = block_size_bytes;
  nor_device_info[card_slot_in_use].block_size_bytes = block_size_bytes;
  nor_device_info[card_slot_in_use].pages_per_block= dev_info[card_slot_in_use].pages_per_block;
  nor_device_info[card_slot_in_use].total_block_count = total_block_count;
  nor_device_info[card_slot_in_use].page_size_bytes = page_size_bytes;
  nor_device_info[card_slot_in_use].total_pages = total_pages;

  dev_is_initialized[card_slot_in_use] = TRUE;
...
UINT32 jsdcc_mem_read_spi(void)
{
  ...
  result = spinor_read(handle_spinor[card_slot_in_use],byte_offset, byte_count,pdata);
...
UINT32 jsdcc_mem_write_spi(void)
{
  ...
  result = spinor_erase (handle_spinor[card_slot_in_use], SIZE_IN_BLOCKS (byte_offset), SIZE_IN_BLOCKS (byte_count));
  if (SPINOR_DEVICE_DONE != result)
  {
  ...
  pdata = jtag_prog_param.data;
  result = spinor_write(handle_spinor[card_slot_in_use], byte_offset, byte_count,pdata);
...
UINT32 jsdcc_mem_erase_partition_spi()
{
  ...
  for(index = 0; index < jtag_prog_param.data_length; index++)
  {
    ...
    if (SPINOR_DEVICE_DONE != spinor_erase(handle_spinor[card_slot_in_use], start_block, 1))
    {
...
UINT32 jsdcc_mem_program_spi(void)
{
  byte *pdata;
  INT32 result = SPINOR_DEVICE_DONE;
  UINT32 byte_offset, data_length, byte_end_offset, block_num, block_offset;
  ...
  /* Only erase when the data to be written includes the start of a block.
     T32 loads data in 1k chunk multiples, therefore we don't always get a
     4K chunk to write. The code writes sequentially, therefore we only
     need to erase when we access the first sector in that block. */
  block_num = SIZE_IN_BLOCKS (byte_offset); /* Rounds up */
  block_offset = block_num * BLOCK_SIZE;
  byte_end_offset = byte_offset + data_length;
  if (byte_offset <= block_offset && block_offset < byte_end_offset)
    result = spinor_erase (handle_spinor[card_slot_in_use], block_num, SIZE_IN_BLOCKS (data_length));
  ...
  result = spinor_write(handle_spinor[card_slot_in_use],
...
UINT32 jsdcc_mem_print_info_spi(void)
{
  ...
  outputMessage("Device Name = %s\n", nor_device_info[card_slot_in_use].device_name);
  outputMessage("Block size = %d \n", nor_device_info[card_slot_in_use].block_size_bytes);
  outputMessage("Pages per block = %d \n", nor_device_info[card_slot_in_use].pages_per_block);
  outputMessage("Total Block count is %d \n", nor_device_info[card_slot_in_use].total_block_count);
  outputMessage("Page size  is %d \n", nor_device_info[card_slot_in_use].page_size_bytes);
  outputMessage("Total Pages Avail is %d \n", nor_device_info[card_slot_in_use].total_pages);
...
UINT32 jsdcc_mem_erase_bulk_spi()
{
  ...
  if (SPINOR_DEVICE_DONE != spinor_erase_bulk(handle_spinor[card_slot_in_use], start_block, data_length))
  {
    outputMessage("Failed\n");
    spinor_utils_deinit();
    return STATUS_FAIL;
...
UINT32 jsdcc_mem_unit_test_spi()
{
  ...
  result = spinor_open_slot(&handle_spinor[card_slot_in_use], card_slot_in_use);
  if(SPINOR_DEVICE_DONE != result)
  {
  ...
  result = spinor_open_slot(&unit_test_handle[card_slot_in_use], card_slot_in_use);
  if(SPINOR_DEVICE_DONE != result)
  {
    outputMessage("Error Opening The Device For TestClient-1\n");
    flag = 3;
  ...
  {
    if (SPINOR_DEVICE_DONE != spinor_erase(handle_spinor[card_slot_in_use], erase_start_block, Block_count))
    {
  ...
  {
    if (SPINOR_DEVICE_DONE != spinor_erase_bulk(handle_spinor[card_slot_in_use], erase_start_block ,  Block_count))
    {
  ...
  {
    result = spinor_write(handle_spinor[card_slot_in_use],rw_page, (16 * SIZE_Page), &Test_Write_Buffer);
    if (SPINOR_DEVICE_DONE != result)
    {
    ...
    result = spinor_read(handle_spinor[card_slot_in_use], rw_page, (16 * SIZE_Page), &Test_Read_Buffer);
    if (SPINOR_DEVICE_DONE != result)
    {
  ...
  result = spinor_close(handle_spinor[card_slot_in_use]);
  if(SPINOR_DEVICE_DONE != result)
  {
  ...
  result = spinor_close(unit_test_handle[card_slot_in_use]);
  if(SPINOR_DEVICE_DONE != result)
  {
    outputMessage("Error Closing The Device For TestClient-1\n");
    flag = 15;
...
UINT32 jsdcc_mem_chip_erase_spi(void)
{
  ...
  block_count = nor_device_info[card_slot_in_use].total_block_count/(SIZE_64K/BLOCK_SIZE);
    /* Erase every block in the spinor, one at a time */
  start_block = jtag_prog_param.addr;
  result = spinor_erase_bulk(handle_spinor[card_slot_in_use], start_block, block_count);
