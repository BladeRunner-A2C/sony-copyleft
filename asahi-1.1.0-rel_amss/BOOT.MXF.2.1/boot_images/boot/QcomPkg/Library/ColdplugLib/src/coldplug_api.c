...
/*===========================================================================
  ...
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ColdplugLib/src/coldplug_api.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $

  when         who   what, where, why
  ----------   ---   ---------------------------------------------------------
  2023-04-04   sc    Add coldplug_get_total_lba support
  2022-02-18   ss    Added Check for Unsupported devices
  ...
  2021-01-11   rp    Add coldplug_mount API to mount FAT partitions.
  2020-11-16   rp    Allocate coldplug_partition to grow at runtime.
  2019-01-09   rp    Add flush support.
  2018-08-08   eo    In open device(), check up to max lun number to open a valid lun
	...
	int is_embedded_device)
{
	...
	for (slot_no = 0; slot_no < cdev->max_slots; ++slot_no) {
		...
		if (result != 0) {
			...
			if (result == COLDPLUG_RESULT_NOT_SUPPORTED)
				continue;
		...
		if (cdev->boot_lun_no == COLDPLUG_SD_ALL_LUNS) {
			memset(dev_info, 0, sizeof(*dev_info));
			...
			clun->lba_size    =  dev_info->bytes_per_lba;
			clun->total_lba   =  dev_info->total_lba_count;
			clun->is_boot_lun =  FALSE;
		}

        result = cdev->bdev_ops->get_total_lba(bdev_hdl, clun, dev_info);
        if(result != COLDPLUG_RESULT_SUCCESS && 
            result != COLDPLUG_RESULT_NOT_SUPPORTED)
        {
			result = COLDPLUG_RESULT_DEVICE_ERROR;
			goto Error;
...
struct coldplug_handle* coldplug_open_partition(const char *parti_name)
{
	...
	for (cparti = coldplug_partitions.next;
		cparti != NULL; cparti = cparti->next) {
...
int coldplug_close_partition(struct coldplug_handle *chdl)
{
	...
	result = cdev->bdev_ops->flush(clun->bdev_hdl);
	if (result == COLDPLUG_RESULT_SUCCESS)
...
int coldplug_set_bootable_partition(enum coldplug_device_type dev_type,
                int is_embedded_device, uint32 lun_no)
{
	struct coldplug_device *cdev;
	struct coldplug_bdev_ops *bdev_ops;
	struct coldplug_slot *cslot;
	...
	for (slot_no = 0; slot_no < cdev->max_slots; ++slot_no) {
		...
		result = bdev_ops->set_bootable_partition(slot_no, lun_no);
		if (result != 0)
			goto End;
	}
End:
...
int coldplug_get_bootable_partition(enum coldplug_device_type dev_type,
                int is_embedded_device, uint32 *out_lun_no)
{
	struct coldplug_device *cdev;
	struct coldplug_bdev_ops *bdev_ops;
	struct coldplug_slot *cslot;
	...
	for (slot_no = 0; slot_no < cdev->max_slots; ++slot_no) {
		...
		result = bdev_ops->get_bootable_partition(slot_no, out_lun_no);
		if (result == 0)
			goto End;
	}
End:
	...
	int is_embedded_device, const struct coldplug_guid *guid)
{
	...
	for (cparti = coldplug_partitions.next; cparti != NULL;
		cparti = cparti->next) {
...
int coldplug_set_config(enum coldplug_device_type dev_type,
	int is_embedded_device, enum coldplug_set_cfg_type cfg)
{
	struct coldplug_device *cdev;
	struct coldplug_bdev_ops *bdev_ops;
	struct coldplug_slot *cslot;
	...
	for (slot_no = 0; slot_no < cdev->max_slots; ++slot_no) {
		...
		result = bdev_ops->set_config(slot_no, cfg);
		if (result != 0)
			goto End;
	}
End:
