...
/*===========================================================================
  ...
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ColdplugLib/src/coldplug_iter.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
...
struct coldplug_iter_record* coldplug_iter_next(struct coldplug_iter *citer)
{
	...
	if (citer->cur_parti_idx >= coldplug_partitions_count)
		goto End;

	cparti = coldplug_partitions.next;
	for (i = 0; i < citer->cur_parti_idx; ++i) {
		COLDPLUG_ASSERT(cparti != NULL);
		cparti = cparti->next;
	}
	for (; cparti != NULL; cparti = cparti->next) {
		++citer->cur_parti_idx;
		clun = cparti->clun;
		cslot = clun->cslot;
		cdev = cslot->cdev;
