...
/*===========================================================================
  ...
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ColdplugLib/src/coldplug_pcache.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
...
static uint8 _coldplug_pcache_data_buffer[COLDPLUG_PCACHE_SIZE_IN_BYTES]  __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));

/* Use variables to reference the buffer, it allows off target tests to
 * change the cache size. */
static uint8 *coldplug_pcache_data_buffer __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES))) = _coldplug_pcache_data_buffer;
