...
/*===========================================================================
  ...
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ColdplugLib/src/coldplug_device.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
...
struct coldplug_partition coldplug_partitions;
struct coldplug_handle coldplug_handles[COLDPLUG_MAX_HANDLES];
uint32 coldplug_partitions_count = 0;
uint32 coldplug_partitions_count_total = 0;
...
void coldplug_device_init(void)
{
	...
	coldplug_partitions_count = 0;
	coldplug_partitions_count_total = 0;
}
...
{
	struct coldplug_partition *cparti, *cparti_prev, *cparti_tmp;

	coldplug_lun_validate(clun);

	cparti_prev = &coldplug_partitions;
	cparti = coldplug_partitions.next;
	while (cparti != NULL) {
		COLDPLUG_ASSERT(cparti_prev != NULL);
		if (cparti->clun == clun) {
			coldplug_invalidate_all_handles(cparti);
			cparti_prev->next = cparti->next;
			cparti_tmp = cparti;
			cparti = cparti->next;
			memset(cparti_tmp, 0, sizeof(*cparti_tmp));
			coldplug_free(cparti_tmp);
            --coldplug_partitions_count;		
		} else {
			cparti_prev = cparti;
			cparti = cparti->next;
		}
...
struct coldplug_partition* coldplug_partition_alloc(
			struct coldplug_lun *clun)
{
	struct coldplug_partition *cparti = NULL, *cparti_tmp = NULL;
	uint32 size;

	coldplug_lun_validate(clun);

	if (coldplug_partitions_count >= COLDPLUG_MAX_PARTITIONS)
 		return NULL;

	size = sizeof(struct coldplug_partition);
	cparti = coldplug_malloc(size);
	if (cparti == NULL)
		return NULL;

	memset(cparti, 0, size);
	cparti->magic = COLDPLUG_PARTI_MAGIC;
	cparti->clun = clun;
	snprintf(cparti->name, sizeof(cparti->name), "p-%d",
		coldplug_partitions_count_total);
	++coldplug_partitions_count;
	++coldplug_partitions_count_total;

	cparti_tmp = &coldplug_partitions;
	while (cparti_tmp->next != NULL)
		cparti_tmp = cparti_tmp->next;
	cparti_tmp->next = cparti;

	return cparti;
}

void coldplug_invalidate_all_handles(struct coldplug_partition *cparti)
{
