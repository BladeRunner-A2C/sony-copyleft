 ...
 *
 * Coldplug
 * Copyright (C) 2015,2018-2019,2021-2023 QUALCOMM Technologies, Inc.
 *
...
/*===========================================================================
  ...
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ColdplugLib/src/coldplug_bdev_spi_nor.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $

  when         who   what, where, why
  ----------   ---   ---------------------------------------------------------
  2023-04-04   sc    Add coldplug_get_total_lba support
  2022-11-14   sc    Replace spinor_open with spinor_open_slot API
  2022-06-29   sc    Add spinor background init start API usage
...
#define COLDPLUG_BDEV_NOR_MAX_HANDLES  (3)

struct coldplug_bdev_nor_handle {
	int is_in_use;
	spinor_handle_t flash_hdl;
...
static inline void coldplug_bdev_spi_nor_debug_add(
	...
	SPINOR_STATUS nor_res, uint32 val1, uint32 val2)
{
		...
		uint32 *max_luns, uint32 *boot_lun_no)
{
	...
	*boot_lun_no = COLDPLUG_SPI_NOR_BOOT_LUN;
		...
		uint32 slot_no, uint32 lun_no)
{
	...
	struct spinor_info nor_info;
	SPINOR_STATUS nor_res;
	uint32 i;
	...
	nor_res = spinor_open_slot((spinor_handle_t *)&nor_hdl->flash_hdl, slot_no);
	if (nor_res != SPINOR_DEVICE_DONE) {
		coldplug_bdev_spi_nor_debug_add(COLDPLUG_DEBUG_ID_OPEN,
	...
	nor_res = spinor_get_info(nor_hdl->flash_hdl, &nor_info);
	if (nor_res != SPINOR_DEVICE_DONE) {
		coldplug_bdev_spi_nor_debug_add(COLDPLUG_DEBUG_ID_OPEN,
			nor_res, slot_no, lun_no);
		goto Error;
...
Error:
	if (nor_hdl != NULL) {
		if (nor_hdl->flash_hdl != NULL)	{
			(void) spinor_close(nor_hdl->flash_hdl);
		}
		memset(nor_hdl, 0, sizeof(*nor_hdl));
	}
...
static int coldplug_bdev_nor_ops_close(void *bdev_hdl)
{
	...
	nor_res = spinor_close(nor_hdl->flash_hdl);
	if (nor_res != SPINOR_DEVICE_DONE) {
...
{
	struct coldplug_bdev_nor_handle *nor_hdl = NULL;
	uint32 lba, count, start_page, page_count, byte_offset, byte_count;
	SPINOR_STATUS nor_res;
	int result = COLDPLUG_RESULT_ENODEV;
	...
	byte_offset = start_page * nor_hdl->page_size;
	byte_count = page_count * nor_hdl->page_size;

	nor_res = spinor_read(nor_hdl->flash_hdl,
				byte_offset, byte_count, buff);
	if (nor_res != SPINOR_DEVICE_DONE) {
...
{
	struct coldplug_bdev_nor_handle *nor_hdl = NULL;
	uint32 start_page, page_count, byte_offset,byte_count, start_lba, count;
	SPINOR_STATUS nor_res;
	int result = COLDPLUG_RESULT_ENODEV;
	...
	nor_res = spinor_erase(nor_hdl->flash_hdl, start_lba, count);
	if (nor_res != SPINOR_DEVICE_DONE) {
	...
	page_count = count * nor_hdl->block_size;
	 ...
	 /* convert the page no and page count into byte offset and byte count */
	byte_offset = start_page * nor_hdl->page_size;
	byte_count = page_count * nor_hdl->page_size;

	nor_res = spinor_write(nor_hdl->flash_hdl, byte_offset, byte_count, buff);
	if (nor_res != SPINOR_DEVICE_DONE) {
		coldplug_bdev_spi_nor_debug_add(COLDPLUG_DEBUG_ID_WRITE,
				nor_res, start_lba, count);
		goto End;
...
static int coldplug_bdev_nor_ops_ops_flush(void *bdev_hdl)
{
	(void) bdev_hdl;
...
static int coldplug_bdev_nor_ops_set_bootable_partition(uint32 slot_no, uint32 lun_no)
{
	(void) slot_no; (void) lun_no;
	return COLDPLUG_RESULT_ENODEV;
}

static int coldplug_bdev_nor_ops_get_bootable_partition(uint32 slot_no, uint32 *out_lun_no)
{
	(void) slot_no; (void) out_lun_no;
	return COLDPLUG_RESULT_NOT_SUPPORTED;
}

static int coldplug_bdev_nor_ops_background_init_start(uint32 slot_no, uint32 init_context)
{
	SPINOR_STATUS nor_res;
	int result = COLDPLUG_RESULT_ENODEV;

	nor_res = spinor_background_init_start(slot_no, init_context);
	if (nor_res != SPINOR_DEVICE_DONE)
	{
		coldplug_bdev_spi_nor_debug_add(COLDPLUG_DEBUG_ID_BACKGROUND_START,
			nor_res, slot_no, 0);		
		goto End;
	}
...
static int coldplug_bdev_nor_ops_set_config(uint32 slot_no,
...
static int coldplug_bdev_nor_ops_get_total_lba(void *bdev_hdl, void *clun,
        struct coldplug_device_info *device_info)
{
	struct coldplug_bdev_nor_handle *nor_hdl = NULL;
    struct coldplug_device_info *nor_info = NULL;
    struct coldplug_lun *nor_clun = NULL;
	int result = COLDPLUG_RESULT_ENODEV;

	COLDPLUG_ASSERT(bdev_hdl != NULL);
	COLDPLUG_ASSERT(device_info != NULL);
    COLDPLUG_ASSERT(clun != NULL);

	nor_hdl = (struct coldplug_bdev_nor_handle *) bdev_hdl;
    nor_info = (struct coldplug_device_info *) device_info;
    nor_clun = (struct coldplug_lun *) clun;

    memset(nor_info, 0, sizeof(*nor_info));

    result = coldplug_bdev_nor_ops_get_info(nor_hdl, nor_info);
    if (result != 0) {
        coldplug_bdev_nor_ops_close(nor_hdl);
        goto End;
    }

    nor_clun->lba_size    =  nor_info->bytes_per_lba;
    nor_clun->total_lba   =  nor_info->total_lba_count;
    nor_clun->is_boot_lun =  FALSE;
...
static struct coldplug_bdev_ops coldplug_bdev_nor_ops = {
	...
	coldplug_bdev_nor_ops_ops_flush,
	...
	coldplug_bdev_nor_ops_get_bootable_partition,
	coldplug_bdev_nor_ops_background_init_start,
	coldplug_bdev_nor_ops_background_init_finish,
	coldplug_bdev_nor_ops_set_config,
    coldplug_bdev_nor_ops_get_total_lba
};
