 ...
 *
 * Coldplug
 * Copyright (C) 2015,2019-2023 QUALCOMM Technologies, Inc.
 *
...
/*===========================================================================
  ...
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/ColdplugLib/src/coldplug_bdev_sd.c#1 $ $DateTime: 2024/04/02 23:00:31 $ $Author: cirrusp4svc $
		...
		uint32 *max_luns, uint32 *boot_lun_no)
{
	...
	*boot_lun_no = COLDPLUG_SD_ALL_LUNS;
		...
		uint32 slot_no, uint32 lun_no)
{
	...
	struct sdcc_device *sd_hdl = NULL, *first_sd_hdl = NULL;
	boolean is_valid_slot = FALSE;
	uint32 num_slots=0,lun=0, max_luns=0, start_lun=0;
	...
	if (slot_no != 0 && lun_no == COLDPLUG_SD_ALL_LUNS)
	 ...
	 */
	if (lun_no == COLDPLUG_SD_ALL_LUNS)
	{
		start_lun = COLDPLUG_SD_START_LUN;
		max_luns  = COLDPLUG_MAX_LUNS_IN_ONE_SLOT;
		for (lun = start_lun;max_luns > 0;--max_luns) {
	...
	{
		first_sd_hdl = sd_hdl = sdcc_handle_open(slot_no, lun_no);
		if (sd_hdl == NULL) {
			coldplug_bdev_sd_debug_add(COLDPLUG_DEBUG_ID_OPEN,
...
static int coldplug_bdev_sd_ops_get_total_lba(void *bdev_hdl, void *clun,
...
static struct coldplug_bdev_ops coldplug_bdev_sd_ops =
{
    ...
    coldplug_bdev_sd_ops_get_total_lba
};
