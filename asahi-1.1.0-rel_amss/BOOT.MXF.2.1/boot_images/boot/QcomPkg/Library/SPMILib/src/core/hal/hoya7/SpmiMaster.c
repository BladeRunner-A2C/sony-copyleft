 ...
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/SPMILib/src/core/hal/hoya7/SpmiMaster.c#1 $
 * $Change: 53083464 $ 
 * 
 * when       who     what, where, why
 * -------- ------- -------------------------------- 
 * 10/1/13  al       Initial Version 
 */
...
static void clearSpmiMemory(SpmiDevInfo *pDev)
{
    ...
    for(i = 0; i < SPMI_NUM_CHANNELS_SUPPORTED(pDev); i++) {
        SPMI_HWIO_OUT( HWIO_SPMI_PIC_IRQ_CLEARn_ADDR( base_addr, (i + pDev->busChanStart)), HWIO_SPMI_PIC_IRQ_CLEARn_INT_CLEAR_BMSK );
        SPMI_HWIO_OUT( HWIO_SPMI_PIC_ACC_ENABLEn_ADDR( base_addr, (i + pDev->busChanStart) ), 0 );
        SPMI_HWIO_OUT( HWIO_SPMI_PERIPHm_2OWNER_TABLE_REG_ADDR( base_addr, pDev->uDevIdx, (i + pDev->busChanStart) ), 0 );
        SPMI_HWIO_OUT( HWIO_PMIC_ARB_REG_ADDRp_ADDR( base_addr, (i + pDev->busChanStart) ), 0 );
    }
    ...
    for(i = 0; i < SPMI_NUM_MASTERS(pDev); i++)
    {
        ...
        {
            uint32 val = SPMI_HWIO_IN( HWIO_SPMI_PIC_OWNERm_ACC_STATUSn_ADDR( base_addr, pDev->uDevIdx, i, k ) );
            if(val != 0) {
...
static Spmi_Result loadMin(SpmiDevInfo *pDev, uint8 masterId)
{
    ...
    SpmiTarget_PreInit(pDev);

    // 0. Don't make assumptions - set ser_clk_sel to cc_spmi_ahb_clk for RAM programming
    SPMI_HWIO_OUT_CLEAR( HWIO_SPMI_GENI_CLK_CTRL_ADDR( base_addr, pDev->uDevIdx), HWIO_SPMI_GENI_CLK_CTRL_SER_CLK_SEL_BMSK );
    
    // 1. FW Image download
    SpmiHal_LoadFirmware( base_addr, SPMI_NUM_PROG_RAM_REGS(pDev) );
    
    // 2. Initiate FORCE-DEFAULT command
    SPMI_HWIO_OUT_SET( HWIO_SPMI_GENI_FORCE_DEFAULT_REG_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_GENI_FORCE_DEFAULT_REG_FORCE_DEFAULT_BMSK );
    
    // 3. Switch PROGRAM-RAM clock muxing to serial clock
    SPMI_HWIO_OUT_SET( HWIO_SPMI_GENI_CLK_CTRL_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_GENI_CLK_CTRL_SER_CLK_SEL_BMSK );
    ...
    SPMI_HWIO_OUT_SET( HWIO_SPMI_GENI_OUTPUT_CTRL_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_GENI_OUTPUT_CTRL_SOE0_EN_BMSK |
                                                                     HWIO_SPMI_GENI_OUTPUT_CTRL_SOE1_EN_BMSK );
    
    // 6. Configure SPMI_MID_REG register
    SPMI_HWIO_OUT_FIELD( HWIO_SPMI_MID_REG_ADDR(base_addr, pDev->uDevIdx), HWIO_SPMI_MID_REG_MID, masterId );
    
    // Multi-master settings
    SPMI_HWIO_OUT_SET( HWIO_SPMI_CFG_REG_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_CFG_REG_ASYNC_SSC_DET_INT_DIS_BMSK );
    SPMI_HWIO_OUT_SET( HWIO_SPMI_CFG_REG_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_CFG_REG_ARB_UNKNOWN_EN_BMSK );
    SPMI_HWIO_OUT_SET( HWIO_SPMI_CFG_REG_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_CFG_REG_SSC_WINDOW_EN_BMSK ); // should be after??
    SPMI_HWIO_OUT_SET( HWIO_SPMI_CFG_REG_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_CFG_REG_SSC_DET_INT_DIS_BMSK );
    SPMI_HWIO_OUT_SET( HWIO_SPMI_CFG_REG_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_CFG_REG_FORCE_ARB_AFTER_MASTER_TO_BMSK );//not initially tested

    
    // 7. Set fields FORCE_MASTER_WRITE_ON_ERROR and ARBITER_ENABLE in SPMI_CFG_REG
    SPMI_HWIO_OUT_SET( HWIO_SPMI_CFG_REG_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_CFG_REG_FORCE_MASTER_WRITE_ON_ERROR_BMSK );


//<Rajiv> To be removed after fixing the flow of enabling SPMI Master2 in Tokei
    if (pDev->uDevIdx == 0)
    ...
    SPMI_HWIO_OUT_CLEAR( HWIO_SPMI_CFG_REG_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_CFG_REG_ARBITER_BYPASS_BMSK );
...
static Spmi_Result loadFull(SpmiDevInfo *pDev, uint8 masterId)
{
    ...
    // Insure bus is idle
    if(SPMI_HWIO_IN_FIELD( HWIO_SPMI_GENI_STATUS_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_GENI_STATUS_M_GENI_CMD_ACTIVE )) {
    ...
    SPMI_HWIO_OUT_CLEAR( HWIO_SPMI_GENI_PROG_ROM_CTRL_REG_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_GENI_PROG_ROM_CTRL_REG_PROG_ROM_EN_BMSK );

    // 15. Enabling HW control CGC
    SPMI_HWIO_OUT_CLEAR( HWIO_SPMI_GENI_CGC_CTRL_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_GENI_CGC_CTRL_PROG_RAM_SCLK_OFF_BMSK );
    SPMI_HWIO_OUT_SET( HWIO_SPMI_GENI_CGC_CTRL_ADDR( base_addr, pDev->uDevIdx ), 
