 ...
 * @file:  PmicArb.c
 * 
 * Copyright (c) 2013-2017,2022 by Qualcomm Technologies Incorporated. All Rights Reserved.
 * 
 * $DateTime: 2024/04/02 23:00:31 $
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/SPMILib/src/core/hal/hoya7/PmicArb.c#1 $
 * $Change: 53083464 $ 
 * 
 *                              Edit History
 * Date     Description
...
//******************************************************************************
boolean PmicArb_IsBusEnabled(SpmiDevInfo *pDev)
{
  ...
  return SPMI_HWIO_IN_MASK(HWIO_SPMI_CFG_REG_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_CFG_REG_ARBITER_ENABLE_BMSK) ? TRUE : FALSE;
}
...
Spmi_Result PmicArb_UpdateCache(SpmiInfo *pSpmiInfo)
{
    ...
    for(bid = 0; bid < pSpmiInfo->uNumDevices; bid++) 
    {
        ...
        {
            reg = SPMI_HWIO_IN( HWIO_PMIC_ARB_REG_ADDRp_ADDR( base_addr, (chan + pDev->busChanStart) ) );
    ...
    if(!newRoot)
    {
        ...
        do
        {
            ...
            if(chanIdx == chan) {
                ...
                reg = SPMI_HWIO_IN( HWIO_SPMI_PERIPHm_2OWNER_TABLE_REG_ADDR( base_addr, pDev->uDevIdx, chan ) );
...
Spmi_Result PmicArb_ClearChannelIrqCache(SpmiDevInfo *pDev, uint16 channel)
{
...
Spmi_Result PmicArb_ClearChannelOwnerCache(SpmiDevInfo *pDev, uint16 channel)
{
    Spmi_Result rslt = SPMI_SUCCESS;
    ChannelMapEntry* entry = &pDev->channelMap[channel];
    entry->owner = INVALID_OWNER;
...
Spmi_Result PmicArb_FindIrqChannel(SpmiDevInfo *pDev, uint8 slaveId, uint8 periph, uint16* channel)
{
                                   ...
                                   uint32* bytesTransacted)
{
    ...
    *bytesTransacted = 0;
    ...
    switch(cmd)
    {
        ...
        case PMIC_ARB_CMD_EXTENDED_PBS_REG_WRITE_LONG:
            doWrite = TRUE;
            irqStatus = SPMI_HWIO_IN( HWIO_SPMI_PROTOCOL_IRQ_STATUS_ADDR( base_addr, pDev->uDevIdx ) );
            // To avoid code check errors
            if (irqStatus){}
            break;
    ...
    while(dataLen > 0)
    {
        ...
        if(cmd == PMIC_ARB_CMD_EXTENDED_PBS_REG_WRITE_LONG)
        {
          writeCommand( base_addr, owner, doRead, PMIC_ARB_CMD_EXTENDED_REG_WRITE_LONG, channel, priority, (uint8)(offset + 0), len - 1 );
        }
        else
        {
                   ...
                   Spmi_AccessPriority priority, uint16 address)
{
    ...
    {
        SPMI_HWIO_OUT( HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_WDATAg_ADDR(base_addr,i), (uint32)(data[i]) );
    }
    return;    
}
                   ...
                   uint8 regOffset, uint32 len)
{
    ...
    SPMI_HWIO_OUT( HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_CMD1_ADDR( base_addr ),
                  (len       << HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_CMD1_BYTE_CNT_SHFT) |
                  (slaveId   << HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_CMD1_SID_SHFT) );
                  
    SPMI_HWIO_OUT( HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_CMD2_ADDR( base_addr ),
                  (ppId      << HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_CMD2_PPID_SHFT) );
                  
    SPMI_HWIO_OUT( HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_CMD3_ADDR( base_addr ),
                  (regOffset << HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_CMD3_ADDRESS_SHFT) );
                  
    SPMI_HWIO_OUT( HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_CMD0_ADDR( base_addr ),
                  (cmd       << HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_CMD0_OPCODE_SHFT) |
                  (priority  << HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_CMD0_PRIORITY_SHFT) );
...
static void readSpmiDataDebug(uint8 *base_addr, uint8* data, uint32 bytesToRead)
{
    ...
    {
        data[i] = (uint8)( SPMI_HWIO_IN( HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_RDATAh_ADDR(base_addr, i) ) );
    }
    return;
}
...
static Spmi_Result waitForStatusDoneDebug(uint8 *base_addr)
{
    ...
    {
        SpmiOs_Wait( 1 );
        status = SPMI_HWIO_IN( HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_ADDR( base_addr ) ); 
    }
    
    if(status & HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_DROPPED_BMSK) 
    {
        result = SPMI_FAILURE_TRANSACTION_DROPPED;
    }
    else if(status & HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_DONE_BMSK)
    {
        if(status & HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_FAILURE_BMSK) {
            result = SPMI_FAILURE_TRANSACTION_FAILED;
        }
        else if(status & HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_DENIED_BMSK) {
            result = SPMI_FAILURE_TRANSACTION_DENIED;
        }
    }
    else {
                              ...
                              uint32*             bytesTransacted )
{
    ...
    *bytesTransacted = 0;    
    ...
    while(dataLen > 0)
    {
        ...
        status = SPMI_HWIO_IN( HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_ADDR( dbgBaseAddrs ) );
        
        if( !(status & HWIO_QDSS_AOSS_APB_AOSS_PMIC_ARB_DEBUG_CSR_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_DONE_BMSK) )
        {
            return SPMI_FAILURE_BUS_BUSY;
        }
