 ...
 * $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Library/SPMILib/src/core/hal/hoya7/SpmiBusCfg.c#1 $
 * $Change: 53083464 $
 *
 *                              Edit History
 * Date     Description
...
//<Rajiv> chanCfg->channel - pDev->busChanStart
        if(chanCfg->channel /*- pDev->busChanStart*/ > (SPMI_NUM_CHANNELS_SUPPORTED(pDev) - 1))
        {
...
static Spmi_Result configChannels(SpmiBusCfg_ChannelCfg* entries, uint32 numEntries,
                                  boolean autoAssignChan)
{
    ...
    SpmiDevInfo *pDev = NULL;
    uint8 *base_addr = NULL;      // pDev->baseAddrs;
    uint16 periphOwnerMask = 0;
    uint16 irqOwnerMask = 0;
    uint8 ownerIndex = 0;
    uint8 nEntriesPerBus[pSpmiInfo->uNumDevices];
    ...
    if(pSpmiInfo->uNumDevices == 0){
        return SPMI_FAILURE_INVALID_PARAMETER;
    }

    for (uint8 busIdx = 0; busIdx < pSpmiInfo->uNumDevices; busIdx++)
    {
      pDev = SPMI_PDEV(pSpmiInfo, busIdx);
      ...
      for (i = 0; i < SPMI_NUM_CHANNELS_SUPPORTED(pDev); i++)
      {
          ...
          SPMI_HWIO_OUT( HWIO_SPMI_PERIPHm_2OWNER_TABLE_REG_ADDR( base_addr, pDev->uDevIdx, i ), 0 );
          SPMI_HWIO_OUT( HWIO_PMIC_ARB_REG_ADDRp_ADDR( base_addr, (pDev->busChanStart + i) ), 0 );
      }
    ...
    for (i = 0; i < numEntries; i++)
    {
        ...
        entry = &entries[i];
        pDev = SPMI_PDEV(pSpmiInfo, entry->busId);
        base_addr = pDev->baseAddrs;
        ...
        if (nEntriesPerBus[pDev->uDevIdx] >= SPMI_NUM_CHANNELS_SUPPORTED(pDev))
        {
          return SPMI_FAILURE_TOO_MANY_ENTRIES;
        }

        if(entry->periphOwnerMask == SPMI_OWNER_NONE)
          continue;

        if((entry->periphOwnerMask >= SPMI_OWNER_INVALID)) {
      ...
      base_addr = pDev->baseAddrs;
        if(!pDev->dynamicChannelMode) {
            ...
            //SPMI_HWIO_OUT_SET( HWIO_SPMI_CMPR_EN_REG_ADDR(base_addr), HWIO_SPMI_CMPR_EN_REG_CMPR_ENABLE_BMSK );
              ...
              SPMI_HWIO_OUT_CLEAR( HWIO_SPMI_SEC_DISABLE_REG_ADDR(base_addr, pDev->uDevIdx), HWIO_SPMI_SEC_DISABLE_REG_DISABLE_SECURITY_BMSK );

              // Enable interrupt processing
              SPMI_HWIO_OUT_SET( HWIO_SPMI_MWB_ENABLE_REG_ADDR(base_addr, pDev->uDevIdx), HWIO_SPMI_MWB_ENABLE_REG_MWB_ENABLE_BMSK );
        }
    }
...
Spmi_Result SpmiBusCfg_ConfigureOemChannels(SpmiBusCfg_OemChannelCfg* entries, uint32 numEntries)
{
    return SPMI_SUCCESS;
}
...
Spmi_Result SpmiBusCfg_Init(boolean initHw)
{
    ...
    for(i = 0; i < pSpmiInfo->uNumDevices; i++)
    {
        if(initHw)
        {
            ...
            {
                SPMI_HWIO_OUT( HWIO_PMIC_ARB_PRIORITIESn_ADDR( base_addr,pDev->uDevIdx, k ), k );
            }
        }
    }
...
Spmi_Result SpmiBusCfg_GetChannelInfo(uint8 busId, uint16 channel, SpmiBusCfg_ChannelCfg* chanCfg, SpmiBusCfg_RGConfig* rgCfg)
{
    ...
    if(rgCfg != NULL) {
        ...
        rgCfg->startAddr = (uint32)(uintptr_t)HWIO_PMIC_ARB_CHNLn_CMD_ADDR( base_addr, channel );
        rgCfg->size = (uint32)(uintptr_t)HWIO_PMIC_ARB_CHNLn_CMD_ADDR( base_addr, channel+1 ) - rgCfg->startAddr;
...
//Can remove bus id from the argument field and make this generic
Spmi_Result SpmiBusCfg_SetDynamicChannelMode(uint8 busId, boolean enabled)
{
    SpmiDevInfo *pDev;
    uint8 * base_addr;

    for (busId=0; busId < SPMI_NUM_BUSES_SUPPORTED(pSpmiInfo); busId++)
    {
        pDev = SPMI_PDEV(pSpmiInfo, busId);
        base_addr = pDev->baseAddrs;
        ...
        if(enabled)
        {
            ...
            SPMI_HWIO_OUT_SET( HWIO_SPMI_SEC_DISABLE_REG_ADDR( base_addr, pDev->uDevIdx ), HWIO_SPMI_SEC_DISABLE_REG_DISABLE_SECURITY_BMSK );
            SPMI_HWIO_OUT_CLEAR( HWIO_SPMI_MWB_ENABLE_REG_ADDR( base_addr,  pDev->uDevIdx ), HWIO_SPMI_MWB_ENABLE_REG_MWB_ENABLE_BMSK );
        }
...
//<Rajiv> need a spmi0 and spmi1 distinction here
    reg = SPMI_HWIO_SET_FIELD_VALUE( chanCfg->periphOwner,
                                   HWIO_SPMI_PERIPHm_2OWNER_TABLE_REG_PERIPH2OWNER );

    SPMI_HWIO_OUT( HWIO_SPMI_PERIPHm_2OWNER_TABLE_REG_ADDR( base_addr, pDev->uDevIdx, chanCfg->channel ), reg );
...
//<Rajiv> must add the start index of bus channels here for pmic arb conversion channel
    SPMI_HWIO_OUT( HWIO_PMIC_ARB_REG_ADDRp_ADDR( base_addr, (chanCfg->channel + pDev->busChanStart) ), reg );
...
{
    return configChannels(entries, numEntries, TRUE );
}
    ...
    for(channel = 0; channel < SPMI_NUM_CHANNELS_SUPPORTED(pDev); channel++) {
        ...
        if(periphOwnerMask == 0) {
            ...
            if(rgStartFlag == TRUE) {
                ...
                channelRG.size = (uint32)(uintptr_t)HWIO_PMIC_ARB_CHNLn_CMD_ADDR( base_addr, channel ) - channelRG.startAddr;
        ...
        if((periphOwnerMask != channelRG.ownerMask) || (rgStartFlag == FALSE)) {
            ...
            channelRG.startAddr = (uint32)(uintptr_t)HWIO_PMIC_ARB_CHNLn_CMD_ADDR( base_addr, channel );
            rgStartFlag = TRUE;
        }
    }
...
Spmi_Result SpmiBusCfg_ConfigurePvcPort(uint8 busId, const SpmiBusCfg_PvcPortCfg* portCfg)
{
    ...
    SPMI_HWIO_OUT_FIELD( HWIO_PMIC_ARB0_PVC_PORTn_CTL_ADDR( base_addr, portCfg->pvcPortId ),
                         HWIO_PMIC_ARB0_PVC_PORTn_CTL_SPMI_PRIORITY,
                         portCfg->priority );
    ...
    for(i = 0; i < portCfg->numPpids; i++)
    {
        ...
        SPMI_HWIO_OUT_FIELD( HWIO_PMIC_ARB0_PVCn_ADDRm_ADDR( base_addr, portCfg->pvcPortId, i ),
                             HWIO_PMIC_ARB0_PVCn_ADDRm_SID,
                             portCfg->ppids[i].slaveId );

        SPMI_HWIO_OUT_FIELD( HWIO_PMIC_ARB0_PVCn_ADDRm_ADDR( base_addr, portCfg->pvcPortId, i ),
                             HWIO_PMIC_ARB0_PVCn_ADDRm_ADDRESS,
                             portCfg->ppids[i].address );
    }

    // Enable the individual port
    SPMI_HWIO_OUT_SET( HWIO_PMIC_ARB0_PVC_PORTn_CTL_ADDR( base_addr , portCfg->pvcPortId ),
                       HWIO_PMIC_ARB0_PVC_PORTn_CTL_PVC_PORT_EN_BMSK );
...
Spmi_Result SpmiBusCfg_SetPVCPortsEnabled(uint8 busId, boolean enable)
{
    ...
    SPMI_HWIO_OUT_FIELD( HWIO_PMIC_ARBm_PVC_INTF_CTL_ADDR( base_addr, busId ),
                         HWIO_PMIC_ARBm_PVC_INTF_CTL_PVC_INTF_EN,
                         enable );
    return SPMI_SUCCESS;
}

Spmi_Result SpmiBusCfg_ConfigureVrmPort(const SpmiBusCfg_VrmPortCfg* portCfg)
{
    uint32 i;
    uint8 * base_addr;
    SpmiDevInfo *pDev;
    ...
    INIT_CHECK();
    //Get bus id 0 base addr as vrm config is common for bus 0 and 1
    pDev = SPMI_PDEV(pSpmiInfo, 0);
    base_addr = pDev->baseAddrs;
    ...
    for(i = 0; i < pSpmiInfo->uNumDevices; i++)
    {
        SPMI_HWIO_OUT_FIELD( HWIO_PMIC_ARBm_VRM_S_PORT_CTL_ADDR( base_addr, i),
                             HWIO_PMIC_ARBm_VRM_S_PORT_CTL_SPMI_PRIORITY,
                             portCfg->priority );
    }
    ...
    for(i = 0; i < portCfg->numPpids; i++)
    {
        if( (portCfg->ppids[i].slaveId > SPMI_MAX_SLAVE_ID) || (portCfg->ppids[i].busId >= pSpmiInfo->uNumDevices) ) {
            return SPMI_FAILURE_INVALID_SLAVE_ID;
        }
        ...
        SPMI_HWIO_OUT_FIELD( HWIO_PMIC_ARB_VRM_S_ADDRw_ADDR( base_addr, i ),
                             HWIO_PMIC_ARB_VRM_S_ADDRw_BUS_INST,
                             portCfg->ppids[i].busId );
    ...
    for(i = 0; i < pSpmiInfo->uNumDevices; i++)
    {
       SPMI_HWIO_OUT_SET( HWIO_PMIC_ARBm_VRM_S_PORT_CTL_ADDR( base_addr, i ),
                          HWIO_PMIC_ARBm_VRM_S_PORT_CTL_VRM_S_PORT_EN_BMSK );
    }
    return SPMI_SUCCESS;
}

Spmi_Result SpmiBusCfg_SetVrmPortsEnabled(boolean enable)
{
    uint8 * base_addr;

    INIT_CHECK();
     //Get bus id 0 base addr as vrm config is common for bus 0 and 1
    base_addr = SPMI_BID2ADDR(pSpmiInfo, 0);
...
Spmi_Result SpmiBusCfg_ConfigureMgpiPort(uint8 busId, const SpmiBusCfg_MgpiPortCfg* portCfg)
{
    ...
    if(portCfg->muxSelEn)
    {
        SPMI_HWIO_OUT_FIELD(  HWIO_PMIC_MGPI_MUX_SELn_ADDR(base_addr,portCfg->mgpiPortId),
                               HWIO_PMIC_MGPI_MUX_SELn_MUX_SEL, portCfg->muxSelVal );
    }
...
Spmi_Result SpmiBusCfg_ConfigureVioctl(const SpmiBusCfg_VioctlCfg* vioctlCfg)
{
    uint8 * base_addr;
    ...
    base_addr = SPMI_BID2ADDR(pSpmiInfo, vioctlCfg->busId);

    SPMI_HWIO_OUT(HWIO_SPMI_PIC_RCS_VIOCTL_SELF_CLEAR_CFG_n_ADDR(base_addr, vioctlCfg->busId,vioctlCfg->vioCtlId), vioctlCfg->selfClearEn);

    if(vioctlCfg->posEdgeEn)
    {
        if(vioctlCfg->posEdgeSlaveId > SPMI_MAX_SLAVE_ID) {
            return SPMI_FAILURE_INVALID_SLAVE_ID;
        }

        SPMI_HWIO_OUT_FIELD( HWIO_SPMI_PIC_RCS_VIOCTL_n_ADDR(base_addr, vioctlCfg->busId, vioctlCfg->vioCtlId),
                             HWIO_SPMI_PIC_RCS_VIOCTL_n_SID,
                             vioctlCfg->posEdgeSlaveId);

        SPMI_HWIO_OUT_FIELD( HWIO_SPMI_PIC_RCS_VIOCTL_n_ADDR(base_addr, vioctlCfg->busId, vioctlCfg->vioCtlId),
                             HWIO_SPMI_PIC_RCS_VIOCTL_n_PID,
                             vioctlCfg->posEdgePeriphId);
    ...
    {
        if(vioctlCfg->negEdgeSlaveId > SPMI_MAX_SLAVE_ID) {
            return SPMI_FAILURE_INVALID_SLAVE_ID;
        }

        SPMI_HWIO_OUT_FIELD( HWIO_SPMI_PIC_RCS_VIOCTL_N_n_ADDR( base_addr, vioctlCfg->busId, vioctlCfg->vioCtlId),
                             HWIO_SPMI_PIC_RCS_VIOCTL_N_n_SID,
                             vioctlCfg->negEdgeSlaveId);

        SPMI_HWIO_OUT_FIELD( HWIO_SPMI_PIC_RCS_VIOCTL_N_n_ADDR( base_addr, vioctlCfg->busId, vioctlCfg->vioCtlId),
                             HWIO_SPMI_PIC_RCS_VIOCTL_N_n_PID,
                             vioctlCfg->negEdgePeriphId);

        SPMI_HWIO_OUT_FIELD( HWIO_SPMI_PIC_RCS_VIOCTL_N_n_ADDR( base_addr, vioctlCfg->busId, vioctlCfg->vioCtlId),
                             HWIO_SPMI_PIC_RCS_VIOCTL_N_n_MSG_MASK,
                             vioctlCfg->negEdgeIrqMask );

        SPMI_HWIO_OUT_FIELD( HWIO_SPMI_PIC_RCS_VIOCTL_N_n_ADDR( base_addr, vioctlCfg->busId, vioctlCfg->vioCtlId),
                             HWIO_SPMI_PIC_RCS_VIOCTL_N_n_RCS_MSG,
                             vioctlCfg->negEdgeIrqMask );

        SPMI_HWIO_OUT_FIELD( HWIO_SPMI_PIC_RCS_VIOCTL_N_n_ADDR( base_addr, vioctlCfg->busId, vioctlCfg->vioCtlId),
                             HWIO_SPMI_PIC_RCS_VIOCTL_N_n_VIOCTL_EN,
                             vioctlCfg->negEdgeEn );
    }
