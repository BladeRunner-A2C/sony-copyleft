...
pm_err_flag_type boot_pm_flash_led_module_control(uint32 device_index, uint8 enable)
{
  return pm_flash_led_module_control(device_index, enable);
}
...
boot_pmic_if_type pmic_if =
{
  ...
  pm_pon_get_poff_reason,         // pm_pon_get_poff_reason
#if defined(FEATURE_XBOOT) && defined(FEATURE_SONY_RAMDUMP)
  boot_pm_flash_led_module_control // pm_flash_led_module_control
#endif /* defined(FEATURE_XBOOT) && defined(FEATURE_SONY_RAMDUMP) */
};
