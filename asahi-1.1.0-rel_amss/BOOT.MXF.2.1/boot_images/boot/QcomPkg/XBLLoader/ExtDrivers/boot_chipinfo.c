...
--------   ---     ----------------------------------------------------------
28/04/23   nnunna  ChipInfo API udpates
...
**  FUNCTION  boot_get_disabled_cpus

===========================================================================*/
/**
  Return the disabled cpu part of the chip

  This function is a wrapper around ChipInfo_GetDisabledCPUs(). It provides
  disabled part retrieval.

  @return
  ChipInfoPartType *                  -- pointer to store disabled part
...
*/
static bl_error_boot_type boot_get_disabled_cpus(uint32 cpu_cluster, uint32 * mask)
{
  DALResult dal_status = DAL_SUCCESS;
  bl_error_boot_type status = BL_ERR_NONE;
  ...
  {
    dal_status = ChipInfo_GetDisabledCPUs(cpu_cluster, mask);
    if(dal_status != DAL_SUCCESS)
    {
      status = GET_ERROR_CODE(BL_ERROR_GROUP_CHIPINFO_IF, BL_ERR_CHIPINFO_INTERFACE_FAILURE);
    }
...
/**
  ...
  This function is a wrapper around ChipInfo APIs. It provides
  CHIP-ID, CHIP-VERSION, CHIP-FAMILY retrieval.
...
===========================================================================*/
/**
  Return the disabled part of the chip
...
**  FUNCTION  boot_get_disabled_features

===========================================================================*/
/**
  Return the Part(IP) Specific details from the fuse.

  This function is a wrapper around ChipInfo_GetDisabledFeatures(). It provides
  disabled part retrieval.

  @return
  ChipInfoPartType *                  -- pointer to store parts details
  uint32                              -- size of buffer
  BL_ERR_CHIPINFO_INTERFACE_FAILURE   -- failure in interface
...
*/
static bl_error_boot_type boot_get_disabled_features(ChipInfoPartType chipinfo_part, uint32 nIdx, uint32 * mask)
{
  ChipInfoResult eResult = CHIPINFO_SUCCESS;
  bl_error_boot_type status = BL_ERR_NONE;
  ...
  {
    eResult = ChipInfo_GetDisabledFeatures(chipinfo_part, nIdx, mask);
...
**  FUNCTION  boot_is_part_disabled

===========================================================================*/
/**
  Return if the specified Part is enabled or Disabled

  This function is a wrapper around ChipInfo_IsPartDisabled(). It provides
  disabled part retrieval.
...
*/
static bl_error_boot_type boot_is_part_disabled(ChipInfoPartType chipinfo_part, uint32 nIdx, boolean * mask)
{
  ChipInfoResult eResult = CHIPINFO_SUCCESS;
  bl_error_boot_type status = BL_ERR_NONE;
  ...
  {
    eResult = ChipInfo_IsPartDisabled(chipinfo_part, nIdx, mask);
    if(eResult != CHIPINFO_SUCCESS)
    {
      status = GET_ERROR_CODE(BL_ERROR_GROUP_CHIPINFO_IF, BL_ERR_CHIPINFO_INTERFACE_FAILURE);
    }
...
boot_chipinfo_if_type chipinfo_if = {
  ...
  boot_get_disabled_cpus,
  boot_get_chipinfo_data,
  boot_chipinfo_init,
  boot_get_disabled_features,
  boot_is_part_disabled
};