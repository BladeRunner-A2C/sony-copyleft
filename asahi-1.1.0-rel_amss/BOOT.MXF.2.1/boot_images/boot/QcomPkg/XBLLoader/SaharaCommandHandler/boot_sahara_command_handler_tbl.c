...
--------    ---      ----------------------------------------------------------
03/06/23    batta    Updated boot_get_ddr_training API and removed support for MBN header.
03/08/23    vk       Remove conflicting header, cleanup
02/24/23    batta    Updated boot_get_training_data API to append training data to MBN header
...
)
{
  ...
  do
  {
    ...
    *ddr_training_data_ptr = (uint8 *)ddr_if->params_get_training_data(tr_data_size);
    ...
    cache_if->WriteBackInvalidateDataCacheRange((void *)(*ddr_training_data_ptr), (*tr_data_size));
