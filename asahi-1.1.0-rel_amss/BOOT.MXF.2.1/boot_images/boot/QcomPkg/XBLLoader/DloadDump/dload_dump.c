...
--------   ---     ----------------------------------------------------------
11/30/22   krd     changed boot boolean from int8 as it was being typecasted
...
bl_error_boot_type boot_dload_is_security_region_dump_allowed(boot_handle config_context_handle, boot_boolean *mem_dump_supported) 
{
 ...
 do
 {
  ...
  return_status = dbginfo_if->is_ramdump_allowed(mem_dump_supported);
  if(return_status != BL_ERR_NONE)
  {
    break;
  }
...
{
  bl_error_boot_type return_status = BL_ERR_NONE;
  boot_boolean mem_dump_supported = FALSE;
