...
Copyright 2020,2022,2023 by Qualcomm Technologies, Inc.  All Rights Reserved.
=============================================================================*/
...
--------   ---          -------------------------------------------------------
04/15/23   digants      Added check region without updating
09/29/22   Rhythm       Added a condition for overlapping memory
...
#define UPDATE_BLACKLIST TRUE
#define DONT_UPDATE_BLACKLIST FALSE
...
**  Function :  _check_region_and_opt_update
...
*
* @brief
* Function to check address range against the blacklist and conditionally update the blacklist
* with the new region based in input flag.
*
* @param[in] start address of the range
* @param[in] size of the address range
* @param[in] is_update_needed boolean to tell if blacklist needs to be updated
*
*/
static bl_error_boot_type _check_region_and_opt_update(uintnt start_addr, uintnt size, boolean is_update_needed)
{
  ...
  if (bl_info.is_disabled == 0x1 || is_update_needed == FALSE)
  {
    return error;
  }
...
**  Function :  boot_blacklist_check_region
...
*
* @brief
* Function to check address range against the blacklist 
*
* @param[in] start address of the range
* @param[in] size of the address range
*
*/
static bl_error_boot_type boot_blacklist_check_region(uintnt start_addr, uintnt size)
{
  return _check_region_and_opt_update(start_addr,size,DONT_UPDATE_BLACKLIST);
}
...
{
  return _check_region_and_opt_update(start_addr,size,UPDATE_BLACKLIST);
}
...
bl_error_boot_type boot_blacklist_get_interface(void *blacklist_if)
{
  ...
  ((boot_blacklist_if_type *)blacklist_if)->check_region = boot_blacklist_check_region;
