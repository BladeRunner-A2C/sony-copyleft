...
--------   ---          -------------------------------------------------------
08/15/22   rhy          Check for Image ID if mode is Not Debug mode
...
static bl_error_boot_type boot_flashless_sahara_open(const boot_handle config_context_handle, boot_handle *handle_ptr)
{
  ...
  /* Get image ID from config table */
  ...
  if(image_id == NULL)
  {
    /* If in memory mode (RAMDump/MiniDump), Image ID can be NULL */
    if ((boot_sahara_info_ptr->sahara_data).sahara_mode != SAHARA_MODE_MEMORY_DEBUG)
    {
      return GET_ERROR_CODE(BL_ERROR_GROUP_BOOT_SAHARA, BL_ERR_INVALID_PARAMETER);
    }
  }
