...
typedef struct boot_platforminfo_if_type
{
  ...
  bl_error_boot_type (*PlatformOEMVariant) (uint32 * platform_oemvariant);
  bl_error_boot_type (*ChipId)(void * pChipSetId);
  bl_error_boot_type (*ChipFamily)(void * pChipsetfamily);
  bl_error_boot_type (*get_platforminfo_data)(DalPlatformInfoPlatformType *pType, uint32 *pSubtype, uint32 *pVersion, uint32 *pOEMVariant);
