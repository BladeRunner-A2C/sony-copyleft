...
12/21/22   ck           Added reconfig init logic
...
static boot_boolean boot_page_dev_bam_initialized = FALSE;
static boot_boolean boot_page_dev_reconfig_init = FALSE;
                                             ...
                                             uint32 bytes_to_read)
{
  ...
  cache_if->InvalidateDataCacheRange((void *)temp_page_buffer,
                                     page_dev_handle->nand_info.page_size_bytes);
...
static bl_error_boot_type boot_page_dev_hw_init(const boot_handle config_context_handle)
{
  bl_error_boot_type error = BL_ERR_NONE;
  NAND_STATUS nand_status = NAND_DEVICE_DONE;
  boot_nand_if_type * nand_if = NULL;
  bam_status_type bam_error = BAM_SUCCESS;
  boot_bam_if_type * bam_if = NULL;


  /* Initialize BAM if applicable */
  if(!boot_page_dev_bam_initialized)
  {
    /* Validate pointers */
    if (config_context_handle == NULL)
    {
    ...
    boot_page_dev_bam_initialized = TRUE;
  }


  /* Call reconfig function in NAND driver if applicable */
  if (boot_page_dev_reconfig_init == TRUE)
  {
    /* Get nand driver interface */
    error = boot_config_context_get_ptr(config_context_handle,
                                        CONFIG_CONTEXT_NAND_INTERFACE,
    ...
    /* Call nand reconfig */
    nand_status = nand_if->nand_reconfig();
    if (nand_status != NAND_DEVICE_DONE)
    {
      error = GET_ERROR_CODE(BL_ERROR_GROUP_PAGEDEV, BL_ERR_INTERFACE_FAILURE);
      goto exit;
    ...
    boot_page_dev_reconfig_init = FALSE;
  }
...
static bl_error_boot_type boot_page_dev_hw_deinit(const boot_handle config_context_handle)
{
  ...
  if(boot_page_dev_bam_initialized)
  {
    ...
    boot_page_dev_bam_initialized = FALSE;
  }
...
{
  if (cmd == PDEV_SET_RECONFIG_INIT)
  {
    boot_page_dev_reconfig_init = TRUE;
  }
