...
----------      ---   ----------------------------------------------------------
2023-08-14      cm    Correcting perf Info
...
#define STOR_SPINOR_MAX_SLOTS 2
UINT64 gTotalTime = 0, gWriteTime = 0, gReadTime = 0, gEraseTime = 0, gOtherTime = 0;
...
static QCSPINOR_DEVICE_PATH gSpiNorDevicepath[STOR_SPINOR_MAX_SLOTS];
...
static EFI_GUID SpiNorDevicePathGuid[STOR_SPINOR_MAX_SLOTS] = {EFI_SPINOR0_GUID, EFI_SPINOR1_GUID};
static SPINOR_DEV gSpiNorDevice[STOR_SPINOR_MAX_SLOTS] = {
    { SPINOR_DEV_SIGNATURE, 0, NULL, 0, 0, 0, 0},
    { SPINOR_DEV_SIGNATURE, 1, NULL, 0, 0, 0, 0}
};

static EFI_BLOCK_IO_MEDIA gMediaSpiNor[STOR_SPINOR_MAX_SLOTS] = {
 {  
...
/* get device Info */
EFI_STATUS SpiNorSlotsGetInfo (VOID)
{
  ...
  for(i=0; i<STOR_SPINOR_MAX_SLOTS; i++)
  {
    /* Send SCM syscall to do the actual get info */
    Status = SpiNorScmSyscall(STOR_GET_INFO, 0, BufferSize, unbuf, i);
    if (EFI_SUCCESS != Status)
    {
      ...
      {
        DEBUG ((EFI_D_WARN, "SPINOR Get Info with return value of %x for slot %d\n", Status, i));
        gSpiNorDevice[i].IsValid = FALSE;
        Status = EFI_SUCCESS;
      }
    }
    else
    {
        ...
        gMediaSpiNor[i].MediaPresent  = TRUE;
        gMediaSpiNor[i].BlockSize     = dev_info->block_size;
        gMediaSpiNor[i].LastBlock     = dev_info->total_blocks -1;
        gSpiNorDevicepath[i].SpiNor  = SpiNorPath;
        gSpiNorDevicepath[i].End     = SpiNorEndPath;
        gSpiNorDevice[i].Signature   = SPINOR_DEV_SIGNATURE;
        gSpiNorDevice[i].BlkIo       = gBlockIoSpiNor;
        gSpiNorDevice[i].EraseBlk    = gSpiNorEraseBlkProtocol;
        gSpiNorDevice[i].WriteProtect = gSpiNorWriteProtectProtocol;
        gSpiNorDevice[i].BlkIo.Media = &gMediaSpiNor[i];
        gSpiNorDevice[i].SlotNum     = i;
        gSpiNorDevice[i].IsValid     = TRUE;
        gSpiNorDevicepath[i].SpiNor.Guid = SpiNorDevicePathGuid[i];
    }
  }

  UncachedFreePool(unbuf);
  endTime = GetTimerCountus();
  gOtherTime = endTime - startTime + gOtherTime;
  return Status;
}
  ...
  OUT VOID                   *Buffer)
{
  ...
  SpiNorDevice = SPINOR_DEV_FROM_BLOCKIO (This);
  if (SpiNorDevice->IsValid == FALSE)
  {
    return EFI_INVALID_PARAMETER;
  }
  slot_num = SpiNorDevice->SlotNum;

  /* If BufferSize is zero, the host is pinging the card for its presence */
  if (0 == BufferSize)
  {
    if (gSpiNorDevice[slot_num].BlkIo.Media->MediaPresent == TRUE)
    {
      return EFI_SUCCESS;
    }
    else
  ...
  Status = SpiNorScmSyscall(STOR_READ, Lba , BufferSize, unbuf, slot_num);
  if (EFI_SUCCESS != Status)
  {
  ...
  IN VOID                    *Buffer)
{
  ...
  Status = SpiNorScmSyscall(STOR_WRITE, Lba , BufferSize, unbuf, slot_num);
  if (EFI_SUCCESS != Status)
  {
  ...
  )
{
  ...
  Status = SpiNorScmSyscall(STOR_ERASE,LBA, Size, NULL, slot_num);
  if (EFI_SUCCESS != Status)
  {
 ...
 )
{
  ...
  SpiNorDevice = SPINOR_DEV_FROM_WRITE_PROTECT (This);
  if (SpiNorDevice->IsValid == FALSE)
  {
    return EFI_INVALID_PARAMETER;
  }
  ...
  while(Wpcount < WpConfig->NumEntries)
  {
	...
	Status = SpiNorScmSyscall(STOR_WP_EN , WpEntry->Lba, WpEntry->NumBlocks * gMediaSpiNor[slot_num].BlockSize, NULL, slot_num);
 ...
 )
{
  ...
  /* Send SCM syscall to do the actual get info */
  Status = SpiNorScmSyscall(STOR_WP_GET_INFO, 0, BufferSize, unbuf, slot_num);
  if (EFI_SUCCESS != Status)
  {
...
{
  gTotalTime = gReadTime + gWriteTime + gEraseTime + gOtherTime; 
  DEBUG((EFI_D_ERROR, "spinor t=%ldms r=%ldms w=%ldms e=%ldms o=%ldms\n", GET_MILLI_TO_MIRCO_SEC(gTotalTime),
	GET_MILLI_TO_MIRCO_SEC(gReadTime),
	GET_MILLI_TO_MIRCO_SEC(gWriteTime), 
	GET_MILLI_TO_MIRCO_SEC(gEraseTime),
    GET_MILLI_TO_MIRCO_SEC(gOtherTime)));
}
  ...
  IN EFI_SYSTEM_TABLE   *SystemTable)
{
  ...
  for(i=0; i<STOR_SPINOR_MAX_SLOTS; i++)
  {
    if( gSpiNorDevice[i].IsValid == TRUE)
    {    
      Status = gBS->InstallMultipleProtocolInterfaces (
      &gSpiNorDevice[i].ClientHandle,
      &gEfiBlockIoProtocolGuid, &gSpiNorDevice[i].BlkIo,
      &gEfiDevicePathProtocolGuid, &gSpiNorDevicepath[i],
      &gEfiEraseBlockProtocolGuid, &gSpiNorDevice[i].EraseBlk,
      &gEfiStorageWpProtocolGuid, &gSpiNorDevice[i].WriteProtect,
      NULL
      );  
      if (EFI_SUCCESS != Status)
      {
  ...
  else
  {
    /* Register SPI-NOR GPT listener */
     RegisterListener(ScmInitialized, &gSpiNorDevice[0].BlkIo);
  }
