...
)
{
  ...
  else 
  {
    ...
    BOOLEAN                  bPanelPwrChanged = FALSE;
    ...
    for (eDisplayId = MDP_DISPLAY_PRIMARY; eDisplayId <= MDP_DISPLAY_EXTERNAL; eDisplayId++)
    {
      ...
      if ((TRUE          == MDP_SUPPORTED_DISPLAY(eDisplayId))                                              &&
          (MDP_STATUS_OK == MDPGetProperty(eDisplayId, MDP_DISPLAY_PROPERTY_DETECTION_INFO, &sDisplayProp)) &&
          (TRUE          == sDisplayProp.bDisplayDetected))
      {
        ...
        if (bDisplayState != sDisplayProp.bDisplayPwrState)
        {
          bPanelPwrChanged = TRUE;
          break;
        }
      }
    }
    ...
    /* Make sure that all the displays are powered down in order to de-initialize the MDP core and free all the resources */
...
)
{
  ...
  else /* Display power state is set to OFF */
  {
    for (eDisplayId = MDP_DISPLAY_PRIMARY; eDisplayId <= MDP_DISPLAY_EXTERNAL; eDisplayId++)
    {
      MDP_OSAL_MEMZERO(&sDisplayProp, sizeof(MDP_PropertiesParamType));
      ...
      else if (MDP_STATUS_OK != MDPGetProperty(eDisplayId, MDP_DISPLAY_PROPERTY_POWER_STATE, &sDisplayProp))
      {
        continue;
      }
      else if (FALSE == sDisplayProp.bDisplayPwrState)
      {
        /* Panel is not enabled, no need to power off */
        continue;
      }
      else if(MDP_STATUS_OK != MDPDeInit(eDisplayId, 0x0))                         /* De-initialize the interfaces */
      {
        continue;
      }   
      else if (MDP_STATUS_OK != MDPPower(eDisplayId, &sPowerParams, 0x0))          /* Request the MDP hardware to power off the display */
      {
