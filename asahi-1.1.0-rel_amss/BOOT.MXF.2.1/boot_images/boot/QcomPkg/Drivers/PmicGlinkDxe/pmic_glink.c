...
#define PMIC_GLINK_MAX_WAIT_COUNT    1000//1000
...
void pmic_glink_deinit()
{
  ...
  if (pmic_glink_context.IsPMICChannelOpen)
  {
    ...
    if (pmic_glink_context.IsPMICChannelOpen)
    {
      DEBUG((EFI_D_WARN, "pmic_glink_deinit:: GlinkClose failed. Polling timeout.\r\n"));
      return;
    }
  }
...
EFI_STATUS pmic_glink_open(void *context)
{
  ...
  while (count < PMIC_GLINK_MAX_WAIT_COUNT)
  {
    ...
    if (pmic_glink_context.IsPMICChannelOpen && GLINK_STATUS_SUCCESS == ret && EFI_SUCCESS == status)
    {
      DEBUG((EFI_D_WARN, "pmic_glink_open:: success, retry count = %d.  \r\n", count));
      break;
    }
    count++;
  }
...
EFI_STATUS pmic_glink_Init()
{
  ...
  while (count < PMIC_GLINK_MAX_WAIT_COUNT)
  {
    ...
    {
      DEBUG((EFI_D_WARN, "pmic_glink_Init:: glink is up, retry count = %d.  \r\n", count));
      break;
    }
    count++;
  }
  ...
  {
    DEBUG((EFI_D_INFO, "pmic_glink_Init:: open glink channel success \r\n"));
  }
  ...
  const void *data, const size_t size)
{
  ...
  for(i = 0; i < PMIC_GLINK_MAX_WAIT_COUNT; i++)
  {
    ...
    if ((EFI_ERROR(status)) || (GLINK_STATUS_SUCCESS != ret))
    {
      ...
      //chargerPD todo error msg
      DEBUG((EFI_D_WARN, "pmic_glink_tx:: glink tx failed status = %r.ret = %d \r\n", status, ret));
    }
    else
    {
      break;
...
EFI_STATUS pmic_glink_send_shutdown_notification()
{
  EFI_STATUS status = EFI_SUCCESS;
  batt_mngr_generic_req_msg reqMsg = { { 0 } };

  reqMsg.header.msg_owner = PMIC_GLINK_MSG_OWNER_CHARGER;
  reqMsg.header.msg_type = PMIC_GLINK_MSG_TYPE_REQ_RESP;
  reqMsg.header.opcode = BATT_MNGR_SET_SHDN_REQ;
  status = pmic_glink_sync_send_receive(BATT_MNGR_SET_SHDN_REQ, &reqMsg, sizeof(batt_mngr_generic_req_msg));
  return status;
}
