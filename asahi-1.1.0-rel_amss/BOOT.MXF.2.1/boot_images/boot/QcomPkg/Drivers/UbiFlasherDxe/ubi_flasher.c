...
----------   ---   ---------------------------------------------------------
2023-02-22   az    Fix for fastboot ubi volume flashing support.
2022-11-01   az    Support for detecting system partition name at runtime.
2022-11-01   az    Fix for individual UBI volume flashing support.
...
#define UBI_ARRAY_SIZE(array) (sizeof(array) / sizeof(array[0]))

  static const char *system_pname[] = {
  UBI_FLASHER_SYSTEM_PARTITION_NAME,
  "nad_ubi"
};
	...
	const char *volume_name)
{
	...
	for (i = 0; i < (UBI_ARRAY_SIZE(system_pname)); i++)
		{
		result = ubi_open_partition(hdl, (const char *)system_pname[i]);
		if (result == 0)
		{
			DEBUG((EFI_D_ERROR, "Found - system partition name: %a\n",
					(const char *)system_pname[i]));
			break;
		}
	}
	if (result != 0)
	...
	for (i = 0; i < hdl->vtbl_count; ++i) {
		...
		for (i = 0; i < vtbl_records; ++i) {
			...
			if (result == 1) {
				...
				DEBUG((EFI_D_ERROR, "find volume vtbl-name: %a\n", vtbl_rec->name));
	...
	uint32 *page_size_arg, uint32 *block_size_arg)
{
	...
	*hdl_arg = NULL;
	...
	if (result != 0) {
		...
		if (result != 0) {
			DEBUG((EFI_D_ERROR, "[%a] Failed to open volume\n",hdl->partition_name));
...
static int ubi_flasher_vol_write(struct ubi_flasher_handle_type *hdl)
{
	...
	for (;	hdl->cur_write_block_no < hdl->block_count;
		...
		++hdl->leb_no;
		buf_size -= (data_len_size - hdl->data_offset);
		offset += (data_len_size - hdl->data_offset);
	}
