...
QCOM_USB_CONFIG_PROTOCOL              gUsbConfigTemplate = {
    EFI_QCOM_USB_CONFIG_PROTOCOL_REVISION_2,
    ...
    UsbConfigEnableVbus,
    UsbConfigPollSSPhyTraining,
    UsbConfigAdvanceSSCmplPattern,
    UsbConfigGetWoLState,  /* Added in EFI_QCOM_USB_CONFIG_PROTOCOL_REVISION_2 */
    UsbConfigSetWoLState,  /* Added in EFI_QCOM_USB_CONFIG_PROTOCOL_REVISION_2 */
};
...
static EFI_EVENT UsbConfigStopUsbEvt           = NULL;
static EFI_EVENT UsbConfigUsbLoopbackEvt       = NULL;
static EFI_EVENT UsbConfigDisableUsbEvt        = NULL;
static EFI_EVENT UsbConfigEnableUsbEvt         = NULL;
   ...
   )
{
  ...
  Status = GetSSUsbFnConfig (This->coreNum, CoreType);
   ...
   )
{
  ...
  Status = PollSSPhyTraining(CoreType);
...
*/
EFI_STATUS
EFIAPI UsbConfigAdvanceSSCmplPattern (
  IN  QCOM_USB_CONFIG_PROTOCOL  *This,
  IN  QCOM_USB_CORE_NUM  CoreNum
  )
{
  EFI_STATUS Status = EFI_UNSUPPORTED;
  FNC_ENTER_MSG();
  Status = AdvanceSSCmplPattern(CoreNum);
  FNC_LEAVE_MSG();
  return Status;   
}
...
*/
EFI_STATUS
EFIAPI UsbConfigSetWoLState(
  IN  QCOM_USB_CONFIG_PROTOCOL        *This,
  IN  QCOM_USB_CONFIG_WOL_STATE_TYPE   WoLSatate
  )
{
  EFI_STATUS Status = EFI_UNSUPPORTED;
  FNC_ENTER_MSG();

  Status = SetWoLState(WoLSatate);
...
{  
  ...
  // Toggle the USB mode on primary port - Hardcoded
  Status = UsbConfigToggleMode(&gUsbConfigTemplate, USB_CORE_0);  
#if USB_SEC_FLAG
  Status = UsbConfigToggleMode(&gUsbConfigTemplate, USB_CORE_1);  
#endif
#if USB_TER_FLAG
    Status = UsbConfigToggleMode(&gUsbConfigTemplate, USB_CORE_2);  
#endif
  ...
  )
{
  ...
  //Start USB Controller for device mode (fastboot/massstorage)
  Status = UsbStartDevice();

  ERR_CHK("UsbStartDevice failed %r", Status);
  
ON_EXIT:
  FNC_LEAVE_MSG ();
  return;
}
QCOM_USB_CORE_NUM gLoopbackHostCoreNumber = USB_CORE_MAX_NUM;  
  ...
  )
{
  ...
  FNC_ENTER_MSG ();
  QCOM_USB_CORE_NUM device_core = USB_CORE_0;
  EFI_STATUS Status = EFI_SUCCESS;

  gLoopbackHostCoreNumber = UsbStartLoopbackInit();
  DBG (EFI_D_INFO, "init host %d", gLoopbackHostCoreNumber);  
  if (gLoopbackHostCoreNumber == USB_CORE_0)
  {
    device_core = USB_CORE_1;
  }
  else
  {
    device_core = USB_CORE_0;
  }
  DBG (EFI_D_INFO, "Init Device at core %d", device_core);
...
UINT32 gUsbBeforeDisableMode[3] ={3,3,1};
VOID
EFIAPI
UsbConfigEnableUsbControllerCb (
  IN EFI_EVENT  Event,
  IN VOID       *Context
  )
{
  ...
  EFI_STATUS Status = EFI_SUCCESS; 
  UINT8 i;
  UINT32* usb_mode = (UINT32*)Context;
  if (NULL == Context)
  {
    DBG (EFI_D_WARN, "No event_ctx 0x%x", usb_mode);
  }
  else
  {
    DBG (EFI_D_WARN, "Pri %d, sec %d, mp %d", usb_mode[0], usb_mode[1], usb_mode[2]);
    for (i=0; i<3;i++)
    {
      if (usb_mode[i] == 1)
      {
        Status = UsbStartController(
            i,
            USB_HOST_MODE_XHCI);
        WRN_CHK("UsbStopController core %d, Host_mode, failed %r", i, Status);
  }
      else if (usb_mode[i]==2)
      {
  Status = UsbStartController(
            i,
            USB_DEVICE_MODE_SS);
        WRN_CHK("UsbStopController core %d, Device_mode, failed %r", i, Status);
      }
      else
      {
        DBG (EFI_D_WARN, "Dont enable core %d mode %d", i, usb_mode[i]);
      }
    }
  }
#endif
...
VOID
EFIAPI
UsbConfigDisableUsbControllerCb (
  IN EFI_EVENT  Event,
  IN VOID       *Context
  )
{
  ...
  else
  {
    ...
    for (i=0; i<3;i++)
    {
      ...
      {
        DBG (EFI_D_WARN, "Dont disable core %d mode %d", i, usb_mode[i]);
      }
	  if (Status)
	  {
	    DBG (EFI_D_WARN, "Stop controller core %d  failed status %d", i, Status);
	  }
    }
  }
  #endif
  ...
  )
{
  ...
  if (gLoopbackHostCoreNumber != USB_CORE_MAX_NUM )
  {
    if ((USB_CORE_0 == gLoopbackHostCoreNumber))
    {
      DBG (EFI_D_WARN, "Stop Host for core %d, Dev for core %d", gLoopbackHostCoreNumber, USB_CORE_1);
      Status = UsbStopController(
        USB_CORE_1,
        USB_DEVICE_MODE_SS);
      WRN_CHK("UsbStopController Device %d, failed %r", USB_CORE_1, Status);
    
    Status = UsbStopController(
        USB_CORE_0,
        USB_HOST_MODE_XHCI);
      WRN_CHK("UsbStopController Host %d, failed %r", USB_CORE_0, Status);
    }
    else
    {
      DBG (EFI_D_WARN, "Stop Host for core %d, Dev for core %d", gLoopbackHostCoreNumber, USB_CORE_0);
      //core 1 host case
      Status = UsbStopController(
        USB_CORE_0,
        USB_DEVICE_MODE_SS);
      WRN_CHK("UsbStopController Device %d, failed %r", USB_CORE_0, Status);

      Status = UsbStopController(
        USB_CORE_1,
        USB_HOST_MODE_XHCI);
      WRN_CHK("UsbStopController Host %d, failed %r", USB_CORE_1, Status);
    }
  }
  else
  {
    //Default case or core 1 host case
    DBG (EFI_D_WARN, "Stop Dev for core %d", USB_CORE_0);
    Status = UsbStopController(
      USB_CORE_0,
      USB_DEVICE_MODE_SS);
    WRN_CHK("UsbStopController Device %d, failed %r", USB_CORE_0, Status);
  ...
  )
{
  ...
  Status = gBS->CreateEventEx(
    ...
    UsbConfigStartUsbLoopbackCb,
    NULL,
    &gInitUsbLoopbackControllerGuid,
    &UsbConfigUsbLoopbackEvt);
  ERR_CHK("Failed to Create ConfigUsbEvent. %r", Status);
  ...
  Status = gBS->CreateEventEx(
    ...
    TPL_CALLBACK,
    UsbConfigDisableUsbControllerCb,
    &gUsbBeforeDisableMode,
    &gInitUsbDisableUsbControllerGuid,
    &UsbConfigDisableUsbEvt);
  ERR_CHK("Failed to Create ConfigUsbEvent. %r", Status);
  ...
  Status = gBS->CreateEventEx(
    ...
    UsbConfigEnableUsbControllerCb,
    &gUsbBeforeDisableMode,
    &gInitUsbEnableUsbControllerGuid,
    &UsbConfigEnableUsbEvt);
