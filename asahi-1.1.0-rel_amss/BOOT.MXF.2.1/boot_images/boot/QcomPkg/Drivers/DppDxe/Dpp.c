/** @file Dpp.c
  ...
  Copyright (c) 2012 - 2020,2023 Qualcomm Technologies, Inc. All rights 
  reserved. 
...
--------   ---     -----------------------------------------------------------
01/31/23   gp      Creating an event for firmware privisioning(BT & WLAN) 
02/13/20   dm      Added Simple file system support for block-based DPP access
...
static EFI_HANDLE                    gDPPHandle  = NULL;
    ...
    )
{
  ...
  {   
    ...
    {    
      ...
      {
        if((PartitionName==NULL) || (PartitionName!=NULL && CompareMem(TempPartitionName, PartitionName, StrSize(TempPartitionName)) == 0))
        {
  ...
  )
{
  ...
  // construct file name
  UINTN pFilePathLen = nTypeLen + nNameLen + StrSize(L"\\");
  pFilePath = AllocateZeroPool(sizeof(CHAR16) * (pFilePathLen));
  if (pFilePath != NULL)
  {
    StrCatS(pFilePath, pFilePathLen, Type);
    StrCatS(pFilePath, pFilePathLen, L"\\");
    StrCatS(pFilePath, pFilePathLen, Name);
  }
  else
  {
  ...
  Status = gBS->HandleProtocol(gDPPHandle,
                              &gEfiSimpleFileSystemProtocolGuid,
                              (VOID **)&Fs);
  HANDLE_ERROR_LABEL(Status, gBS->HandleProtocol, BlockSetDPPExit);

  Status = Fs->OpenVolume(Fs,&FsRoot);
  HANDLE_ERROR_LABEL(Status, Fs->OpenVolume, BlockSetDPPExit); 
  ...
  if (FsRoot != NULL)
  {
    Status = FsRoot->Open(FsRoot,
                          ...
                          Attributes);
    HANDLE_ERROR_LABEL(Status, FsRoot->Open, BlockSetDPPExit); 
  }
  else
  {
    Status = EFI_NOT_FOUND;
    HANDLE_ERROR_LABEL(Status, FsRoot->OpenVolume, BlockSetDPPExit); 
  }
  ...
  )
{
  ...
  Status = gBS->HandleProtocol(gDPPHandle,
                              &gEfiSimpleFileSystemProtocolGuid,
                              (VOID **)&Fs);
  HANDLE_ERROR_LABEL(Status, gBS->HandleProtocol, BlockGetDPPExit);

  Status = Fs->OpenVolume(Fs,&FsRoot);
  HANDLE_ERROR_LABEL(Status, Fs->OpenVolume, BlockGetDPPExit); 
  ...
  if (FsRoot != NULL)
  {
    Status = FsRoot->Open(FsRoot,
                          ...
                          EFI_FILE_MODE_READ,
                          Attributes);
    HANDLE_ERROR_LABEL(Status, FsRoot->Open, BlockGetDPPExit); 
  }
  else
  {
    Status = EFI_NOT_FOUND;
    HANDLE_ERROR_LABEL(Status, FsRoot->OpenVolume, BlockGetDPPExit); 
  }

  Status = File->Read(File, DataSize, Data);
  if ((EFI_BUFFER_TOO_SMALL == Status) && (NULL == Data))
  {
    DEBUG((EFI_D_INFO, "[DppDxe] Retrieved datasize to allocate; will return success!\n"));
    Status = EFI_SUCCESS;
  }
  ...
  )
{
  ...
  Status = gBS->HandleProtocol(gDPPHandle,
                              &gEfiSimpleFileSystemProtocolGuid,
                              (VOID **)&Fs);
  HANDLE_ERROR_LABEL(Status, gBS->HandleProtocol, BlockDeleteDPPExit);

  Status = Fs->OpenVolume(Fs,&FsRoot);
  HANDLE_ERROR_LABEL(Status, Fs->OpenVolume, BlockDeleteDPPExit); 
  ...
  if (FsRoot != NULL)
  {
    Status = FsRoot->Open(FsRoot,
                          ...
                          Attributes);
    HANDLE_ERROR_LABEL(Status, FsRoot->Open, BlockDeleteDPPExit); 
  }
  else
  {
    Status = EFI_NOT_FOUND;
    HANDLE_ERROR_LABEL(Status, FsRoot->OpenVolume, BlockDeleteDPPExit); 
  }
  ...
  )
{
  ...
  UINTN pFilePathLen = nTypeLen + nNameLen + 2 * StrSize (L"\\");
  pFilePath = AllocateZeroPool ( 2*pFilePathLen);
  if (pFilePath != NULL)
  {
    StrCatS (pFilePath, pFilePathLen, L"\\");
    StrCatS (pFilePath, pFilePathLen, Type);
  ...
  )
{
  ...
  if (gDppPartType == DPP_PARTITION_TYPE_BLOCK)
  {
    ...
    Status = gBS->LocateProtocol(
                    ...
                    NULL,
                    (VOID **)&RawFSProtocol);
    HANDLE_ERROR_LABEL(Status, EraseDPP, EraseDPPExit);

    if (boot_from_ufs())
    {
      DiskTypeGuid = &gEfiUfsLU4Guid;
    }
    else if (boot_from_nvme())
    {
      DiskTypeGuid = &gEfiSpiNor0Guid;
    }
    else if (boot_from_emmc())
    {
      DiskTypeGuid = &gEfiEmmcGppPartition1Guid;
    }

    Status = RawFSProtocol->FormatFileSystem(RawFSProtocol, DEFAULT_FS_BLOCKSIZE, 1, NULL, &gEfiDppBlockPartitionGuid, DiskTypeGuid);
...
/**
  ...
  This is used for BT and WLAN provisioning.
   ...
   )
{
  ...
  HANDLE_ERROR_LABEL(Status, RunFwProvisionMain, FwProvisionEventHandlerExit);

FwProvisionEventHandlerExit:
  ...
  )
{
  ...
  HANDLE_ERROR_LABEL(Status , gBS->CreateEventEx(ReadyToBootEvent), InitializeDppProtocolExit);
