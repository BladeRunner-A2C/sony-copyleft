/** @file ScmCmdVar.c
  ...
  Copyright (c) 2011-2017,2019,2023, Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 02/09/23   sree    Align to EDK2 variable policy
...
#define SERVICE_UEFI_VAR_POL_REGISTER_VARIABLE_POLICY       \
        SERVICE_UEFI_VAR_CREATE_CMD(20)     /* Function 20 */

#define SERVICE_UEFI_VAR_POL_DUMP_VARIABLE_POLICY           \
        SERVICE_UEFI_VAR_CREATE_CMD(21)     /* Function 21 */

#define SERVICE_UEFI_VAR_POL_IS_VARIABLE_POLICY_ENABLED     \
        SERVICE_UEFI_VAR_CREATE_CMD(22)     /* Function 22 */

#define SERVICE_UEFI_VAR_POL_LOCK_VARIABLE_POLICY           \
        SERVICE_UEFI_VAR_CREATE_CMD(23)     /* Function 23 */

#define SERVICE_UEFI_VAR_POL_DISABLE_VARIABLE_POLICY        \
        SERVICE_UEFI_VAR_CREATE_CMD(24)     /* Function 24 */
...
typedef struct
{
  ...
  /* Parameters required for the service */
  /** Byte offset from the start of this structure to the policy entry */
  UINT32             PolicyEntryOffset;

  /** Size in bytes of the policy entry */
  UINT32             PolicyEntrySize;

} TZ_SVC_REQ_VAR_POL_REGISTER_VARIABLE_POLICY;
...
} TZ_SVC_RSP_VAR_POL_REGISTER_VARIABLE_POLICY;
...
typedef struct
{
  ...
  /* Parameters required for the service */
  /** Byte offset from the start of this structure to the buffer where
   * policies will be copied */
  UINT32             PolicyBufferOffset;

  /** Size in bytes of the policy buffer */
  UINT32             PolicyBufferSize;

  /** Byte offset from the start of this structure to size parmater
   * (remoted pass-by-pointer semantics) */
  UINT32             SizeBufferOffset;

  /** Size in bytes of the size buffer */
  UINT32             SizeBufferSize;

} TZ_SVC_REQ_VAR_POL_DUMP_VARIABLE_POLICY;
...
} TZ_SVC_RSP_VAR_POL_DUMP_VARIABLE_POLICY;
...
typedef struct
{
  ...
  /* Parameters required for the service */
  /** Byte offset from the start of this structure to the state */
  UINT32             StateOffset;

  /** Size in bytes of the state variable */
  UINT32             StateSize;

} TZ_SVC_REQ_VAR_POL_IS_VARIABLE_POLICY_ENABLED;
...
} TZ_SVC_RSP_VAR_POL_IS_VARIABLE_POLICY_ENABLED;
...
} TZ_SVC_REQ_VAR_POL_LOCK_VARIABLE_POLICY;
...
} TZ_SVC_RSP_VAR_POL_LOCK_VARIABLE_POLICY;
...
} TZ_SVC_REQ_VAR_POL_DISABLE_VARIABLE_POLICY;
...
} TZ_SVC_RSP_VAR_POL_DISABLE_VARIABLE_POLICY;
  ...
  )
{
  ...
  do
  {
    ...
    Request->DataSize = (UINT32)*DataSize;  //size wont be 64bit value so should be ok.
    ...
    /* Validate the response length to prevent overflow*/
    if ((MAX_UINT32 - sizeof(TZ_SVC_RSP_VAR_GET_VARIABLE)) < ((UINT32)*DataSize))
    {  
      Status = EFI_INVALID_PARAMETER;
      break;
    }
  ...
  )
{
  ...
  do
  {
    ...
    /* Validate the request length to prevent overflow*/
    if ((MAX_UINT32 - Request->DataOffset) < (Request->DataSize))
    {
      Status = EFI_INVALID_PARAMETER;
      break;
    }
  ...
  )
{
  ...
  do
  {
    ...
    Request->VariableNameSize = (UINT32)*VariableNameSize;  //this typecasting is ok since it is related to size
  ...
  )
{
  EFI_STATUS  Status          = EFI_SUCCESS;
  UINT8*      ParamLocation   = NULL;
  UINT32      RequestLength   = 0;
  UINT32      ResponseOffset  = 0;
  UINT32      ResponseLength  = 0;
  EFI_TPL     PrevTPL         = (EFI_TPL)TPL_APPLICATION;

  TZ_SVC_REQ_VAR_POL_REGISTER_VARIABLE_POLICY *Request =
    (TZ_SVC_REQ_VAR_POL_REGISTER_VARIABLE_POLICY *) ScmCmdParams.ScmCmdBuffer;
  TZ_SVC_RSP_VAR_POL_REGISTER_VARIABLE_POLICY *Response = NULL;
  ...
  do
  {
    ...
    Request->CommandId = SERVICE_UEFI_VAR_POL_REGISTER_VARIABLE_POLICY;

    /* VARIABLE_POLICY_ENTRY *PolicyEntry */
    Request->PolicyEntryOffset = ALIGN_VALUE(sizeof(TZ_SVC_REQ_VAR_POL_REGISTER_VARIABLE_POLICY), sizeof(UINT32));
    Request->PolicyEntrySize = PolicyEntry->Size;
    Request->Length = Request->PolicyEntryOffset + Request->PolicyEntrySize;
    RequestLength = Request->Length;

    /* Align response to next cache line */
    ResponseOffset = ALIGN_VALUE(Request->Length, ArmDataCacheLineLength());
    RequestLength = ResponseOffset;
    Response = (TZ_SVC_RSP_VAR_POL_REGISTER_VARIABLE_POLICY *)((UINT8*)Request + ResponseOffset);
    ResponseLength = sizeof(TZ_SVC_RSP_VAR_POL_REGISTER_VARIABLE_POLICY);
    ...
    ParamLocation = (UINT8*)((UINT8*)Request +
                             Request->PolicyEntryOffset);
    CopyMem(ParamLocation, PolicyEntry, Request->PolicyEntrySize);
  ...
  )
{
  ...
  TZ_SVC_REQ_VAR_POL_DUMP_VARIABLE_POLICY *Request  =
    (TZ_SVC_REQ_VAR_POL_DUMP_VARIABLE_POLICY *) ScmCmdParams.ScmCmdBuffer;
  TZ_SVC_RSP_VAR_POL_DUMP_VARIABLE_POLICY *Response = NULL;
  ...
  do
  {
    ...
    Request->CommandId = SERVICE_UEFI_VAR_POL_DUMP_VARIABLE_POLICY;

    /* UINT8* Policy */
    Request->PolicyBufferOffset = ALIGN_VALUE(sizeof(TZ_SVC_REQ_VAR_POL_DUMP_VARIABLE_POLICY), sizeof(UINT8));
    Request->PolicyBufferSize = *Size;
    Request->Length = Request->PolicyBufferOffset + Request->PolicyBufferSize;

    /* UINT32* Size */
    Request->SizeBufferOffset = ALIGN_VALUE(Request->Length, sizeof(UINT32));
    Request->SizeBufferSize = sizeof(UINT32);
    Request->Length = Request->SizeBufferOffset + Request->SizeBufferSize;

    RequestLength = Request->Length;

    /* Align response to the next cache line */
    ResponseOffset = ALIGN_VALUE(Request->Length, ArmDataCacheLineLength());
    RequestLength = ResponseOffset;
    Response = (TZ_SVC_RSP_VAR_POL_DUMP_VARIABLE_POLICY *)((UINT8*)Request + ResponseOffset);
    ResponseLength = sizeof(TZ_SVC_RSP_VAR_POL_DUMP_VARIABLE_POLICY);
    ...
    /* Copy input param - Size */
    ParamLocation = (UINT8*)((UINT8*)Request +
                             Request->SizeBufferOffset);
    CopyMem(ParamLocation, Size, Request->SizeBufferSize);
    ...
    if ((Status == EFI_SUCCESS) || (Status == EFI_BUFFER_TOO_SMALL))
    {
      ParamLocation = (UINT8*)((UINT8*)Request +
                             Request->SizeBufferOffset);
      CopyMem(Size, ParamLocation, Request->SizeBufferSize);
    }

    /* Copy OUT param - Policy */
    if (Status == EFI_SUCCESS && Policy != NULL)
    {
      ParamLocation = (UINT8*)((UINT8*)Request +
                             Request->PolicyBufferOffset);
      CopyMem(Policy, ParamLocation, *Size); /* Copy Size worth of bytes */
    }
  ...
  )
{
  ...
  TZ_SVC_REQ_VAR_POL_IS_VARIABLE_POLICY_ENABLED *Request =
    (TZ_SVC_REQ_VAR_POL_IS_VARIABLE_POLICY_ENABLED *) ScmCmdParams.ScmCmdBuffer;
  TZ_SVC_RSP_VAR_POL_IS_VARIABLE_POLICY_ENABLED *Response = NULL;
  ...
  do
  {
    ...
    Request->CommandId = SERVICE_UEFI_VAR_POL_IS_VARIABLE_POLICY_ENABLED;

    /* UINT8 *State */
    Request->StateOffset = ALIGN_VALUE(sizeof(TZ_SVC_REQ_VAR_POL_IS_VARIABLE_POLICY_ENABLED), sizeof(UINT8));
    Request->StateSize = sizeof(UINT8);
    Request->Length = Request->StateOffset + Request->StateSize;
    RequestLength = Request->Length;

    /* Align response to next cache line */
    ResponseOffset = ALIGN_VALUE(Request->Length, ArmDataCacheLineLength());
    RequestLength = ResponseOffset;
    Response = (TZ_SVC_RSP_VAR_POL_IS_VARIABLE_POLICY_ENABLED *)((UINT8*)Request + ResponseOffset);
    ResponseLength = sizeof(TZ_SVC_RSP_VAR_POL_IS_VARIABLE_POLICY_ENABLED);
    ...
    /* Copy OUT params - State */
    ParamLocation = (UINT8*)((UINT8*)Request +
                             Request->StateOffset);
    CopyMem(State, ParamLocation, Request->StateSize);
  ...
  )
{
  ...
  TZ_SVC_REQ_VAR_POL_LOCK_VARIABLE_POLICY *Request =
    (TZ_SVC_REQ_VAR_POL_LOCK_VARIABLE_POLICY *) ScmCmdParams.ScmCmdBuffer;
  TZ_SVC_RSP_VAR_POL_LOCK_VARIABLE_POLICY *Response = NULL;
  ...
  do
  {
    ...
    Request->CommandId = SERVICE_UEFI_VAR_POL_LOCK_VARIABLE_POLICY;

    /* Request */
    Request->Length = sizeof(TZ_SVC_REQ_VAR_POL_LOCK_VARIABLE_POLICY);
    RequestLength = Request->Length;

    /* Align response to next cache line */
    ResponseOffset = ALIGN_VALUE(Request->Length, ArmDataCacheLineLength());
    RequestLength = ResponseOffset;
    Response = (TZ_SVC_RSP_VAR_POL_LOCK_VARIABLE_POLICY *)((UINT8*)Request + ResponseOffset);
    ResponseLength = sizeof(TZ_SVC_RSP_VAR_POL_LOCK_VARIABLE_POLICY);
  ...
  )
{
  ...
  TZ_SVC_REQ_VAR_POL_DISABLE_VARIABLE_POLICY *Request =
    (TZ_SVC_REQ_VAR_POL_DISABLE_VARIABLE_POLICY *) ScmCmdParams.ScmCmdBuffer;
  TZ_SVC_RSP_VAR_POL_DISABLE_VARIABLE_POLICY *Response = NULL;
  ...
  do
  {
    ...
    Request->CommandId = SERVICE_UEFI_VAR_POL_DISABLE_VARIABLE_POLICY;

    /* Request */
    Request->Length = sizeof(TZ_SVC_REQ_VAR_POL_DISABLE_VARIABLE_POLICY);
    RequestLength = Request->Length;

    /* Align response to next cache line */
    ResponseOffset = ALIGN_VALUE(Request->Length, ArmDataCacheLineLength());
    RequestLength = ResponseOffset;
    Response = (TZ_SVC_RSP_VAR_POL_DISABLE_VARIABLE_POLICY *)((UINT8*)Request + ResponseOffset);
    ResponseLength = sizeof(TZ_SVC_RSP_VAR_POL_DISABLE_VARIABLE_POLICY);
