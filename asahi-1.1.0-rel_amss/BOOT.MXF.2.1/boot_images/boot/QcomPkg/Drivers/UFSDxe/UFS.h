...
and interfaces for the UFS driver in UEFI.
 ...
 Copyright (c) 2013 - 2018, 2021-2023 Qualcomm Technologies, Inc. All rights reserved.
**/
...
typedef struct {
  ...
  EFI_STORAGE_EYEMON_PROTOCOL   Eyemon;        /**< -- Eye Monitor */
} UFS_DEV;
...
/**
  Get Write Booster available buffer size 

  @param[in]      This       Indicates a pointer to the calling context.
  @param[in]  CurBufferSize   Pointer to write booster available buffer size
...
/**
  Get Write Booster current buffer size 

  @param[in]      This       Indicates a pointer to the calling context.
  @param[in]  CurBufferSize   Pointer to write booster current buffer size
...
/**
  Dumps the requested UFS registers

  @param[in]  This                Pointer to the EFI_STORAGE_DEV_MGMT_PROTOCOL instance
  @param[out] RegMask             Register bitmask to dump registers. 
