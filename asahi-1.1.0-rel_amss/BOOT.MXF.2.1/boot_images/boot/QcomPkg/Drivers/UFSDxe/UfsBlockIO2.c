/** @file UfsBlockIO2.c
   ...
   Copyright (c) 2014-2017, 2020-2023 Qualcomm Technologies, Inc. 
   All Rights Reserved. 
...
----------   ----    ---------------------------------------------------------- 
2023-04-25   pz      Get UfsDevice from BlkIO2 in BlkIoSlotFullHandler
...
2021-05-26   vm      Invalidate the read buffer rather than flush
      ...
      if (pReq->IsStart) {
         ...
         if (pReq->EventPending == TRUE) {
            ...
            if ((NULL != CryptoIndex) && (NULL != CryptoIvInput)) {
               ...
               if ((rc == UFS_EOK) && (pReq->Type == BLOCKIO2_READ)) 
               {
                  CurBuffer = (UINT8 *) ((UINT8 *)pReq->Buffer - pReq->XferSizeInBytes);
                  InvalidateDataCacheRange((VOID *)CurBuffer, pReq->XferSizeInBytes);
               }
            }
