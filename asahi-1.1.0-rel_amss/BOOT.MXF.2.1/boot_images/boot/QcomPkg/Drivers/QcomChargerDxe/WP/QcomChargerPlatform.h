...
typedef struct
{
    UINT32  DCInBootThreshold;
    /**< boot up threshold for DCIn charging */

    BOOLEAN DCINDisableOnExit;
    /**< suspend dcin or not */

}QCOM_CHARGER_PLATFORM_DCIN_CFGDATA_TYPE;


typedef struct
{
  UINT64 LogCategories;
  /**<64 bit value, each bit is one category*/
  
  UINT32 LogLevel;
  /**<  0: verbose, 1: info, 2: warn, 4, error, 8*/

	UINT32 MaxLogSize;
  /**<  max size of logs can be sent over to uefi */ 

  BOOLEAN EnableLogging;
	/**< enable and configure ChargerPD Logging*/

  UINT32 LoggingIntervalInSec;
  /**< Interval in sec after which ChargerPD logs are read*/
  
}QCOM_CHARGERPD_LOGGING_CFGDATA_TYPE;
...
typedef struct {
  ...
  QCOM_CHARGER_PLATFORM_DCIN_CFGDATA_TYPE     DCInCfgData;

  chargerlib_cfgdata_type    ChargerLibCfgData;

  QCOM_CHARGERPD_LOGGING_CFGDATA_TYPE    ChargerPDLoggingCfgData;

  UINT32   SWFlashMinBattSOC;
  /**< Min battery SOC for SW image flash required */
