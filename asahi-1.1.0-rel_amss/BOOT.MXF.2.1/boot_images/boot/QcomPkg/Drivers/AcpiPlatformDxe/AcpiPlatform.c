/** @file
  ...
  Copyright (c) 2011-2020, 2022,2023 Qualcomm Technologies Inc. All rights
  reserved. 
...
/*=============================================================================
 ...
 09/16/22	skanni	Added additional details to debug message for SetAmlVariableCallback
...
const AMLRegisterStruct AMLRegisterInfo[]=
{
	...
	{{'F','P','I','D'},GETMODE_PREFIX,NULL,AmlUpdateFPID},
	{{'D','P','B','M'},GETMODE_PREFIX,NULL,AmlUpdateDpbm},
};
    ...
    )
{
    ...
    {
        Status = EFI_ERROR(Status) ? Status : EFI_INVALID_PARAMETER;
        DEBUG((EFI_D_ERROR, "ACPIPlatformDXE : UpdateSTA: No child STA value handle found\n"));
        goto Error;
    }
    ...
    )
{
    ...
    for ( i = 0; i < pDsdtStaDeviceList->NumDsdtStaDevices; i++)
    {
        ...
        if (EFI_ERROR(Status))
        {
            ...
            Status = EFI_SUCCESS; // ACPI patching was not possible, ignore this device and continue with next.
        }
  ...
  for (CallbackIndex = 0; CallbackIndex < NumCallbacks; CallbackIndex++)
  {
    if (!CallbackCalled[CallbackIndex])
    {
      if(pRegisteredAmlVariableCallbacks[CallbackIndex].SetAmlVariableCallbackEx != NULL)
      {
        ...
        if ( EFI_ERROR (Status)) 
        {
          DEBUG(( EFI_D_WARN, " ACPIPlatformDXE failed to call SetAmlVariableCallbackEx index: %i function:%r\r\n", CallbackIndex, Status));
          return Status;
        }
      }
      else if(pRegisteredAmlVariableCallbacks[CallbackIndex].SetAmlVariableCallback != NULL)
      {
        ...
        if ( EFI_ERROR (Status)) 
        {
          DEBUG(( EFI_D_WARN, " ACPIPlatformDXE failed to call SetAmlVariableCallback index: %i function:%r\r\n", CallbackIndex, Status));
          return Status;
        }
      }
    }
