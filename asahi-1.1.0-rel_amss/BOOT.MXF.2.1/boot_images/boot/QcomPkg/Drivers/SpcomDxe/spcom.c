...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 03/03/23	by      Region address changes for better NVM features in Hamoa
 12/31/22   vp      Porting stability fixes and health api support from Makena to Hamoa
 12/08/22   gn      Updating the address for Hamoa & increase SPSS link up timeout
...
// Hamoa SPU secure shared memory (512 KB) (0x85300000 to 0x85380000)
const uint64_t SHARED_BUFFER_PHYSICAL_ADDRESS = 0x85300000;
const size_t SHARED_BUFFER_PHYSICAL_ADDRESS_SIZE = 512 * 1024;
...
static sp_app_info_t sp_apps_info[] = {
       ...
       */
       ...
       /* This app is used for test purposes */
       {
           .ch_name = "echo",
           .id = 0x174A9EF9,
           .app_load_ddr_offset = 0x8531D000,
...
/**
 * struct _sp_health_request_t - health API request structure.
 */
...
/**
 * struct _sp_health_response_t - health API response structure.
 */
typedef struct _sp_health_response_t {
    uint32_t error_code;
    struct sp_health_status_data checks_values;
...
spcom_register_link_up_notification (void)
{
   ...
   //Increasing the iteration to 60 for Hamoa bring up. Will revert once the build stabilizes
   while (i++ < 60)
   {
      sp2soc = spcom_read_sp_registers(SP_CNOC_SP_SCSR_RMB_SP2SOC_IRQ_STATUS);
      if(sp2soc == SP_SCSR_REGISTER_ACCESS_ERROR){
      ...
      if (sp2soc == 0x0300000F){
         ...
         if (glink_link_state == GLINK_LINK_STATE_UP){
        	 if (i > 9){
        		LOGE("spcom_glink_up:: loop_count = %d. \r\n", i);
        	 }
             break;
         }
      }
...
poll_on_nvm_ch (void)
{
   ...
   for (int i = (SPCOM_MAX_NVM_SERVICES-1); i >= 0; --i)
   {
      ...
      resp_ptr = (void*)spcom_os_malloc(SPCOM_MAX_RESPONSE_SIZE);
...
/*
**************************************************** 
         SPSS_REGION: 0x86700000 to 0x86995000
****************************************************
Usecase                             Size  Start Addr
****************************************************
SPU Firmware plus boot apps          896  0x86700000
sTPM-Lite Image                      512  0x867E0000
sTPM-Full Image                     1024  0x86860000
sTPM-Lite NVM Data                    36  0x86960000
SegDB                                 36  0x86969000
UEFI-SPU SPCOM  - sTPM_lite channel    8  0x86972000
Echoapp                               48  0x86989000
Reserved                            1280  0x86995000
****************************************************
...
/**
 * spcom_check_sp_health() - retrieve SPU health
 *
 * @health_status_param:     SPU health API request parameter
 * @status_data_ptr:         a pointer to the response buffer containing SPU health
 * 
 * return: Operation success or failure
 */
EFI_STATUS spcom_check_sp_health (
      sp_health_status_t health_status_param, struct sp_health_status_data* status_data_ptr)
{
   /*
   ...
   req.type_of_check = health_status_param;
   ...
   {  
      if (rsp.error_code == 0)
      {
         status_data_ptr->ari_lifecycle_state                   = rsp.checks_values.ari_lifecycle_state;
         status_data_ptr->fips_enabled                          = rsp.checks_values.fips_enabled;
         status_data_ptr->fips_self_test_passed                 = rsp.checks_values.fips_self_test_passed;
         status_data_ptr->sensors_calibrated                    = rsp.checks_values.sensors_calibrated;
         status_data_ptr->por_indicator                         = rsp.checks_values.por_indicator;
         status_data_ptr->por_data.por_counter                  = rsp.checks_values.por_data.por_counter;
         status_data_ptr->por_data.normal_boot_count_since_por  = rsp.checks_values.por_data.normal_boot_count_since_por;
         status_data_ptr->por_data.commit_count_since_por       = rsp.checks_values.por_data.commit_count_since_por;
         status_data_ptr->aux_data0               				= rsp.checks_values.aux_data0;
         status_data_ptr->aux_data1              				= rsp.checks_values.aux_data1;
         status_data_ptr->aux_data2               				= rsp.checks_values.aux_data2;
	     status_data_ptr->aux_data3               			    = rsp.checks_values.aux_data3;
         status_data_ptr->ari_fuse_gauge          				= rsp.checks_values.ari_fuse_gauge;
      }
      else
      {
...
EFI_STATUS spcom_init (void)
{
   ...
   static int ret_hyp_assign = 0;
	...
	ret_hyp_assign = hyp_assign(SHARED_BUFFER_PHYSICAL_ADDRESS, SHARED_BUFFER_PHYSICAL_ADDRESS_SIZE, AC_VM_CP_SPSS_HLOS_SHARED);

	if (ret_hyp_assign < 0){
		LOGE("hyp_assign() failed, status [0x%x]", ret_hyp_assign);
