...
/*  
  ...
  ==================================================================== 
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Drivers/VcsDxe/VCSRails.c#1 $
...
)
{
  ...
  *pRail = (VCSRailNodeType *)NULL;
  ...
  {
    if (!VCS_IsBSPSupported(VCS_HW_VERSION(&VCSBSP.pRails[i])))
    {
      continue;
    }

    if (!strcmp(szName, VCS_RAIL_NAME(&VCSBSP.pRails[i])))
    {
    ...
    for (j = 0; j < VCSBSP.pRails[i].nNumAliases; j++)
    {
      if (!strcmp(szName, VCSBSP.pRails[i].szNameAliases[j]))
      {
        *pRail = (VCSRailNodeType *)&VCSBSP.pRails[i];
