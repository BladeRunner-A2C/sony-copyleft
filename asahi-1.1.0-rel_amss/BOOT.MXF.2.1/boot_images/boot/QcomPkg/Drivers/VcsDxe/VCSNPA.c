...
/*
  ...
  Copyright (c) 2017- 2019,2021 QUALCOMM Technologies Incorporated.  All Rights Reserved.
  QUALCOMM Proprietary and Confidential.

  ====================================================================
  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Drivers/VcsDxe/VCSNPA.c#1 $
...
)
{
  ...
  else if (nPendingMax > RAIL_VOLTAGE_LEVEL_RET)
  {
...
)
{
  uint32               i, nIndex;
  rail_voltage_level   eCorner;
  npa_client_handle    hDependency = NULL;
  ...
  CORE_VERIFY_PTR(pNode->dependencies);
  ...
  for (i = 0; i < pRail->nNumRailDependencies * VCS_NPA_RAIL_NUM_RES_PER_DEP; i++)
  {
    pDependency = &pNode->dependencies[i];
      if (pDependency != NULL &&
        ...
        hClient->type == VCS_NPA_CLIENT_ACTIVE_FLOOR)
    {
      ...
      eCorner = (rail_voltage_level)hClient->resource->internal_state[nIndex];
      if (pComparator((rail_voltage_level)NPA_ACTIVE_REQUEST(hDependency).state, eCorner))
      {
...
)
{
  DALResult                eResult;
  VCSRailNodeType         *pRail;
  rail_voltage_level       eCorner, eCornerPreChange;
  ...
  eCornerPreChange = pRail->eCorner;

  if (eCorner != eCornerPreChange)
  {
    if (pRail->pControl && pRail->pControl->fpPreChange)
    {
      eResult = pRail->pControl->fpPreChange(hClient, eCorner);
      CORE_VERIFY(DAL_SUCCESS == eResult);
    }
    else
    {
      RailEventData.PreChange.eCorner = eCornerPreChange;
      ...
      //npa_dispatch_custom_events(pResource, (npa_event_type)VCS_NPA_RAIL_EVENT_PRE_CHANGE, &RailEventData);
    }
  }
  ...
  if (eCorner != eCornerPreChange)
  {
    if (pRail->pControl && pRail->pControl->fpPostChange)
    {
      eResult = pRail->pControl->fpPostChange(hClient, eCorner);
      CORE_VERIFY(DAL_SUCCESS == eResult);
    }
    else
    {
      ...
      // TODO: NpaApiLib does not support npa_dispatch_custom_events and NPA itself is lacking support for custom events
      //npa_dispatch_custom_events(pResource, (npa_event_type)VCS_NPA_RAIL_EVENT_POST_CHANGE, &RailEventData);
    }
  }
...
)
{
  ...
  npa_node_dependency     *pDependency;
  uint8_t                  nIndex, nImplIndex, nCount;
  ...
  /*-----------------------------------------------------------------------*/
  /* Create aliases if defined.                                            */
  /*-----------------------------------------------------------------------*/

  for (nIndex = 0; nIndex < pRail->nNumAliases; nIndex++)
  {
    npa_alias_resource_cb(pRail->mNode.name,
      pRail->szNameAliases[nIndex], NULL, NULL);
  }
  ...
  eCornerInit = VCS_RAIL_INIT(pRail);
  anInitialState[VCS_NPA_RAIL_RES_IDX_RAIL] = eCornerInit;

  /*
   * Some rails are on via PMIC PON sequence. Bypass NPA short-circuiting
   * for the first request to ensure this image vote is honored.
   */
  if (RAIL_VOLTAGE_LEVEL_OFF != eCornerInit)
  {
    pResource->attributes |= NPA_RESOURCE_DRIVER_UNCONDITIONAL_FIRST;
  }

  /*-----------------------------------------------------------------------*/
  /* Allocate space for impl dependency map.                               */
  /*-----------------------------------------------------------------------*/

  if (pRail->nNumImplDependencies)
  {
    CORE_VERIFY_PTR(pRail->szNameImplDependencies);
    DALSYS_Malloc((pRail->nNumImplDependencies * sizeof(*pRail->aImplDependencyMap)),(void **)(&pRail->aImplDependencyMap));
    CORE_VERIFY_PTR(pRail->aImplDependencyMap);
  }
  ...
  nCount = pRail->nNumRailDependencies;
  for (nIndex = 0; nIndex < pRail->nNumImplDependencies; nIndex++)
  {
    // Update impl map for this dependency.
    if (pRail->szNameImplDependencies[nIndex])
    {
      pRail->aImplDependencyMap[nIndex] = nCount++;
    }
    // Mark this entry invalid.
    else
    {
      pRail->aImplDependencyMap[nIndex] = VCS_DEPENDENCY_EMPTY;
    }
  }

  /*-----------------------------------------------------------------------*/
  /* Allocate space for NPA dependency definitions.                        */
  /*-----------------------------------------------------------------------*/

  if (nCount)
  {
    pRail->mNode.dependency_count = nCount * VCS_NPA_RAIL_NUM_RES_PER_DEP;

    DALSYS_Malloc((pRail->mNode.dependency_count* sizeof(*pRail->aDependencies)),(void **)(&pRail->aDependencies));
    CORE_VERIFY_PTR(pRail->aDependencies);
  }

  /*-----------------------------------------------------------------------*/
  /* The NPA framework requires a continguous array when defining the set  */
  /* of node dependencies.  We combine the set of rail and impl            */
  /* dependencies into two separate regions of this array as follows:      */
  /*-----------------------------------------------------------------------*/
  /* [0] Rail Dependency #0 - client type = REQUIRED                       */
  /* [1] Rail Dependency #0 - client type = SUPPRESSIBLE                   */
  /*  -------------------------------------------------------------------- */
  /* [2] Rail Dependency #1 - client type = REQUIRED                       */
  /* [3] Rail Dependency #1 - client type = SUPPRESSIBLE                   */
  /*  -------------------------------------------------------------------- */
  /* [X*2]     Rail Dependency #X - client type = REQUIRED                 */
  /* [X*2 + 1] Rail Dependency #X - client type = SUPPRESSIBLE             */
  /*-----------------------------------------------------------------------*/
  /* [(X+1)*2]     Impl Dependency #0 - client type = REQUIRED             */
  /* [(X+1)*2 + 1] Impl Dependency #0 - client type = SUPPRESSIBLE         */
  /*-----------------------------------------------------------------------*/
  /* [(X+2)*2]     Impl Dependency #1 - client type = REQUIRED             */
  /* [(X+2)*2 + 1] Impl Dependency #1 - client type = SUPPRESSIBLE         */
  /*-----------------------------------------------------------------------*/
  /* [(X+Y+1)*2]     Impl Dependency #Y - client type = REQUIRED           */
  /* [(X+Y+1)*2 + 1] Impl Dependency #Y - client type = SUPPRESSIBLE       */
  /*-----------------------------------------------------------------------*/

  /*-----------------------------------------------------------------------*/
  /* Initialize NPA dependency definitions.                                */
  /*-----------------------------------------------------------------------*/

  // Start with the set of rail dependencies.
  for (nIndex = 0; nIndex < pRail->nNumRailDependencies; nIndex++)
  {
    pDependency = &pRail->aDependencies[nIndex * VCS_NPA_RAIL_NUM_RES_PER_DEP + VCS_NPA_RAIL_DEP_IDX_REQ];
    pDependency->name = pRail->szNameRailDependencies[nIndex];
    pDependency->client_type = NPA_CLIENT_REQUIRED;

    pDependency = &pRail->aDependencies[nIndex * VCS_NPA_RAIL_NUM_RES_PER_DEP + VCS_NPA_RAIL_DEP_IDX_SUP];
    pDependency->name = pRail->szNameRailDependencies[nIndex];
    pDependency->client_type = NPA_CLIENT_SUPPRESSIBLE;
  }

  // Followed by the set of impl dependencies.
  for (nImplIndex = 0; nImplIndex < pRail->nNumImplDependencies; nImplIndex++)
  {
    // Skip empty entries.
    if (!pRail->szNameImplDependencies[nImplIndex])
    {
      continue;
    }

    pDependency = &pRail->aDependencies[nIndex * VCS_NPA_RAIL_NUM_RES_PER_DEP + VCS_NPA_RAIL_DEP_IDX_REQ];
    pDependency->name = pRail->szNameImplDependencies[nImplIndex];
    pDependency->client_type = NPA_CLIENT_REQUIRED;
    ...
    // Increment NPA dependency index.
    nIndex++;
  }

  // Verify all rail and impl dependencies were accounted for.
  CORE_VERIFY(nIndex == nCount);
  ...
  {
    // TODO: NpaApiLib does not support npa_remote_publish_resource
