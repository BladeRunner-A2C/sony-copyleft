...
/*
  ...
  Copyright (c) 2015-2019,2021 Qualcomm Technologies Incorporated.  All Rights Reserved.
  QUALCOMM Proprietary and Confidential.

  ====================================================================

  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Drivers/VcsDxe/VCSBSP.h#1 $
...
#define VCS_NPA_RAIL_DEP_MAX_COUNT 2

/*
 * Macro to define the number of resources per depedency.
 */
#define VCS_NPA_RAIL_NUM_RES_PER_DEP 2
...
/*
 * Helper macros to define multiple dependencies
 */
#define VCS_DEF_RAIL_DEP_LIST(rail) \
  .nNumRailDependencies = ARR_SIZE(rail), \
  .szNameRailDependencies = rail

#define VCS_DEF_IMPL_DEP_LIST(impl) \
  .nNumImplDependencies = ARR_SIZE(impl), \
  .szNameImplDependencies = impl
...
/*
 * Helper macro to define HW version for a given rail.
 */
#define VCS_DEF_VERSION(min_minor, min_major, max_minor, max_major, chip_family, chip_id) \
  .HWVersion = { {min_minor, min_major}, {max_minor, max_major}, chip_family, chip_id }

/*
 * Help macro to define the string name of the resource being aliased.
 */
#define VCS_DEF_ALIASES(x) \
  .szNameAliases = x, \
  .nNumAliases = ARR_SIZE(x)
...
typedef struct VCSRailControlType
{
  ...
  DALResult (*fpPreChange)(npa_client_handle hClient, rail_voltage_level eCorner);
  DALResult (*fpPostChange)(npa_client_handle hClient, rail_voltage_level eCorner);
} VCSRailControlType;
 ...
 * VCS rail node type.
 *
 *  HWVersion          - Specifies which HW versions support this node.
 ...
 *  aDependencies      - NPA dependencies for this node.
 ...
 *  pRailDependencies  - Pointer to array of rail dependencies.
 *  szNameDependencies - Array of strings that identify each dependency.
 *  szNameAliases      - Array of strings that identify each alias to this rail.
...
typedef struct VCSRailNodeType
{
  ...
  npa_node_dependency     *aDependencies;
  ...
  const char             **szNameAliases;
  uint8_t                  nNumAliases;

  const char             **szNameRailDependencies;
  uint8_t                  nNumRailDependencies;

  const char             **szNameImplDependencies;
  uint8_t                 *aImplDependencyMap;
  uint8_t                  nNumImplDependencies;
} VCSRailNodeType;
