...
/*=============================================================================
 ...
 08/08/22   cm      Removed boot_from_flashless check at BDS Entry
 10/04/21   cm      Added DisplayPerfInfo
 07/16/21   cm      Mounting Aux FV is now made generic
 02/04/21   cm      Added PlatformBdsProtocol
 12/16/20   cm      BDS entry Time stats fix
 09/14/20   cm      Add support for flashless usecase
 08/22/19   kpa     Add support for optimizing decompression in SEC phase
 07/02/18   kpa     Log Bds Entry time
 07/02/18   kpa     Print once debug messages in Enumerate* apis
...
#include <Protocol/EFIPlatformBds.h>
...
extern VOID DisplayPerfInfo( VOID) ;
extern EFI_STATUS ExecImageFromNetwork(VOID);
...
{
  DEBUG((EFI_D_INFO, "[QcomBds]: PreConsoleInit Invoked\n"));
  return PlatformBdsPreConsoleInit();
}
...
{  
  ...
  DEBUG((EFI_D_INFO, "[QcomBds]: PostConsoleInit Invoked\n"));
  PlatformBdsPostConsoleInit();  
  return Status;
}

EFI_STATUS
EFI_SetPlatformSecurity (
  IN EFI_PLATFORMBDS_PROTOCOL    *This  
  )
{
  EFI_STATUS                     Status; 
  DEBUG((EFI_D_INFO, "[QcomBds]: SetPlatformSecurity Invoked\n"));
  Status = PlatformSecurityInit();
  return Status;
}
...
{
  EFI_STATUS                     Status;    
  DEBUG((EFI_D_INFO, "[QcomBds]: FwProvision Invoked\n"));
  Status = PlatBdsSocPostProc();
  return Status;
}
...
{
  EFI_STATUS                     Status; 
  DEBUG((EFI_D_INFO, "[QcomBds]: Platform Bds ProcessHotkeys Invoked\n"));
  Status = PlatformBdsDetectHotKey();
  return Status;
}
...
{
  EFI_STATUS                     Status; 
  DEBUG((EFI_D_INFO, "[QcomBds]: Platform Bds PostFwConfigSec Invoked\n"));
  Status = PlatBdsPostFwConfigSec();
  return Status;
}

EFI_STATUS
EFI_ProcessFwCapsules (
  IN EFI_PLATFORMBDS_PROTOCOL    *This  
  )
{
  EFI_STATUS                     Status;
  DEBUG((EFI_D_INFO, "[QcomBds]: Capsule Update Invoked\n"));
  Status = PlatBdsProcessFwCapsules();
  return Status;
}

STATIC EFI_PLATFORMBDS_PROTOCOL PlatformBdsProtocol = 
{
  EFI_PLATFORMBDS_PROTOCOL_REVISION,
  EFI_PreConsoleInit,
  EFI_PostConsoleInit,
  EFI_SetPlatformSecurity,
  EFI_FwProvision,
  ...
  )
{
  ...
  //Check if PXE boot is enabled, if so do PXE Boot
  //For DEBUG build, NV variable is checked which can be set from BDS Menu
  //For RELEASE build, uefiplat config value is checked
  if(!RETAIL)
  {
    VarSize = sizeof(Flag);
    Status = gRT->GetVariable (L"EnablePxeBoot", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
     if((Status == EFI_SUCCESS) && (Flag == 1))
      EnablePXE = 1;
  }
  else
  {
    Status = GetConfigValue("EnablePXE", &EnablePXE);
    if(Status != EFI_SUCCESS)
      EnablePXE = 0;
  }
  if(EnablePXE)
  {
    ExecImageFromNetwork();
  }
...
/**
  ...
  the system halts and launches shell if RETAIL is set,
...
/* Platform specific config init 
   
   @param     InitOption
   @return    EFI_SUCCESS    No error in config init

 **/
EFI_STATUS
BdsPlatformInit( BDS_INIT_OPTION *InitOption )
{
  EFI_STATUS        Status;
  EFI_STATUS        HotKeyStatus;
  EFI_PLATFORMBDS_PROTOCOL  *PlatBdsProtocol = NULL;  

  Status = gBS->LocateProtocol(&gEfiPlatformBdsProtocolGuid, NULL, (void**)&PlatBdsProtocol);
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] PlatformBdsProtocol not found \n"));
    return Status;
  }

  Status = PlatBdsProtocol->PreConsoleInit(PlatBdsProtocol);
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] PreConsoleInit failed \n"));
    return Status;
  }

  /* Connect Consoles */
  Status = PlatBdsConnectCon();
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] PlatBdsConnectCon failed \n"));
    return Status;
  }
  
  Status = PlatBdsProtocol->PostConsoleInit(PlatBdsProtocol);
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] PostConsoleInit failed \n"));
    return Status;
  }
  
  /* Launch default OEM Setup App */
  Status = PlatBdsProcessOsIndicationsForOemSetupApp();
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] PlatBdsProcessOsIndications failed \n"));
    return Status;      
  }  
  
  /* Handle Hotkeys */
  Status = PlatBdsProtocol->ProcessHotkeys(PlatBdsProtocol);
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] ProcessHotkeys failed \n"));
    return Status;      
  }    

  /* Handle DT config */
  Status = PlatBdsProcessDTconfig();   //Dont see a reason to make it a protocol.Check with Ajay??
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] ProcessDTconfig failed \n"));
    return Status;      
  }  

  /* Load default debug application specified in config file 
     if applicable. To be called before setting up security.
  */
  Status = PlatBdsLaunchDefaultApps();
  if (Status != EFI_SUCCESS && Status != EFI_NOT_FOUND)
  {
    /*DefaultApps optionally loaded. Ignore if not found.*/
    return Status;
  }

  /*  SetPlatformSecurity is the place where UEFI Security is enabled,
   *  including UEFI Image Authentication.
  ...
  /* Setup Security */
  Status = PlatBdsProtocol->SetPlatformSecurity(PlatBdsProtocol);
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] SetPlatformSecurity failed \n"));
    return Status;      
  }

  // Signal Acpi load event before Recovery event trigger and capsule update. 
  // This is done as Acpi need to be restored from backup partition, 
  // if there is any corruption identified in the primary partition.
  Status = EfiEventGroupSignal(&gEfiACPITableLoadGuid);
  if (EFI_ERROR (Status))
  {
    DEBUG((EFI_D_ERROR, "[QcomBds] Acpi table load event signalling failed. Status: %r\n", Status));
    return Status;
  }

  // Signal Recovery event before capsule update. This is done to restore the 
  // corrupted partitions and keep them in sync before updating the new capsule contents.
  Status = EfiEventGroupSignal(&gEfiRecoveryEventGuid);
  if (EFI_ERROR (Status))
  {
    DEBUG((EFI_D_ERROR, "[QcomBds] Recovery event signalling failed. Status: %r\n", Status));
    return Status;
  }

  /* Capsule Update */
  Status = PlatBdsProtocol->ProcessFwCapsules(PlatBdsProtocol);
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] Capsule Update failed \n"));
  }

  /* Provision Firmware */
  Status = PlatBdsProtocol->FwProvision(PlatBdsProtocol);
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] FwProvision failed \n"));
    return Status;
  }

  /* Post Provision Firmware Security */
  Status = PlatBdsProtocol->PostFwConfigSec(PlatBdsProtocol);
  if (Status != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] PostFwConfigSec failed \n"));
    return Status;
  }    
  
  HotKeyStatus = QcomBdsDetectBootHotKey(InitOption);
  if (HotKeyStatus != EFI_SUCCESS)
  {
    DEBUG((EFI_D_WARN, "[QcomBds] QcomBdsDetectBootHotKey failed \n"));
  }
  ...
  )
{
  ...
  DEBUG ((EFI_D_WARN, "BDS Entry      [%5d] \r\n", GetTimerCountms()));

  Status = UpdateFWVendorInST();
  ASSERT_EFI_ERROR(Status);

  Status = BdsPlatformInit(&InitOption);
  ASSERT_EFI_ERROR(Status);
  ...
  )
{
  ...
  /* Register ExitBootServices handler */
  RegisterEBSCallback(ImageHandle ,SystemTable);
  
  Status = MountDecompressedFvByStage (STAGE1);
  
  if ((Status != EFI_SUCCESS) && (Status != EFI_NOT_FOUND))
    DEBUG ((EFI_D_ERROR, "MountDecompressedFvByStage failed, Status: %d\r\n", Status));
  ...
  Status = gBS->InstallMultipleProtocolInterfaces (
                  &ImageHandle, 
                  &gEfiPlatformBdsProtocolGuid, &PlatformBdsProtocol, 
                  NULL);
  ASSERT_EFI_ERROR (Status);
...
// -------- NOTE for OEM With a need to Replace QcomBds implementation --------
// Example Code to uninstall Qualcomm Bds Arch protocol. Do NOT uncomment below.
// Rather use it in similar BDS implementation that would get loaded after QcomBds
// Idea here is to retain QcomBds implementation for PlatformBdsProtocol and a 
// second OEM BDS implementation shall replace the BdsArchProtocol installed by
// Qcom if needed.
/*
  // Get Bds Arch protocol Handle
  ...
  ASSERT_EFI_ERROR ((HandleCount ==1));

  // Get Bds Arch protocol Interface  
  Status = gBS->HandleProtocol (
                  HandleBuffer[0],
                  &gEfiBdsArchProtocolGuid,
                  (VOID **)&gBdsProtocol2
  ...
  //  Replace Bds Arch protocol
  Status = gBS->UninstallMultipleProtocolInterfaces (
                    HandleBuffer[0],
                    &gEfiBdsArchProtocolGuid, gBdsProtocol2,
                    NULL
                    );                    

  if (EFI_ERROR(Status))
    DEBUG((EFI_D_WARN, "Could not uninstall gEfiBdsArchProtocolGuid !!, status: %r\n", Status));  
  else
    DEBUG((EFI_D_WARN, "Successfully Uninstalled gEfiBdsArchProtocolGuid \n"));  
     
  
  Status = gBS->InstallMultipleProtocolInterfaces (
                  &ImageHandle,
                  &<CUSTOM gEfiBdsArchProtocolGuid>, &<CUSTOM gBdsProtocol>,
                  NULL
                  );
  ASSERT_EFI_ERROR (Status);
  
  if (EFI_ERROR(Status))
    DEBUG((EFI_D_WARN, "Could not Reinstall gEfiBdsArchProtocolGuid !, status: %r\n", Status));  
  else
    DEBUG((EFI_D_WARN, "Successfully Reinstalled gEfiBdsArchProtocolGuid \n"));
*/
//---------------------------------------------------------------------------
