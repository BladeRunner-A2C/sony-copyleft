...
Copyright (c) 2012 - 2015, 2017-2019 Qualcomm Technologies, Inc.	All Rights Reserved.
...
/*=============================================================================
 ...
 --------   ---      ---------------------------------------------------------
 11/19/19   rg       Updated XhcExecTransfer(XHC_1_MILLISECOND/40) for boot lockup issue
 10/15/19   rg       Updated XhcExecTransfer(XHC_1_MILLISECOND/100) for boot lockup issue
 10/11/18   kameya   Access UEFI variable one time for SLT loopback.
 10/11/18   kameya   Support USB SLT loopback feature
 09/21/18   gk	     Fixed KW errors on boot.xf.3.0 
 02/12/17   gk	     Lower the Print of Short Packet Message
...
STATIC EFI_USB_MSD_PERIPHERAL_PROTOCOL *UsbMsdProtocol = NULL;

STATIC
VOID
EFIAPI
UsbfnMsdEventHandler(
  VOID 
  )
{
  EFI_STATUS      Status;
  if (UsbMsdProtocol == NULL)
  {
    ...
    if (EFI_ERROR (Status))
    {
      DEBUG ((EFI_D_ERROR, "UsbfnMsdEventHandler: Error Locating EfiUsbMsdPeripheralProtocol\n"));
      return;
    }
  }
  UsbMsdProtocol->EventHandler(UsbMsdProtocol);
}

BOOLEAN SltLbIsEnabled(void) {
  EFI_STATUS              Status;
  UINTN                   UefiVarSize;
  static UINT8            SltLbEnabled = 0;
  static BOOLEAN          SltLbVarRead = FALSE;
    ...
    switch (EvtTrb->Completecode) {
      ...
      case TRB_COMPLETION_SUCCESS:
        if (EvtTrb->Completecode == TRB_COMPLETION_SHORT_PACKET) {
          DEBUG ((EFI_D_INFO, "XhcCheckUrbResult: short packet happens!\n"));
        }
  ...
  )
{
  ...
  PollUsbMsdEvt = SltLbIsEnabled();  

  Status = EFI_SUCCESS;

#if 0
  //To reduce the booting lockup time, dividing XHC_1_MILLISECOND by 40
  /*
  Delta of time:
  No change   ---> 39128 ms 
  by40 change ---> 984 ms
  */
  Loop   = Timeout * XHC_1_MILLISECOND/40;
  if (Timeout == 0) {
    Loop = 0xFFFFFFFF;
  }

#else // Enable fix to reduce USB causing big timeout, but also avoiding very short time-out

#define XHC_EXEC_XFER_TIMEOUT_MS_MIN (20)
#define XHC_EXEC_XFER_TIMEOUT_MS_MAX (200)


  Loop = Timeout * XHC_1_MILLISECOND;

  // Handle exception case where timeout is too small or too big
  if (Timeout == 0) {
    Loop = 0xFFFFFFFF;
    DEBUG ((EFI_D_ERROR, "XhcExecTransfer: Timeout 0 (infinite) received, Core = %d, Dir = %d, Length = %d \n", 
      Xhc->CoreNum, Urb->Ep.Direction, Urb->Completed));
  }
  else if (Timeout < XHC_EXEC_XFER_TIMEOUT_MS_MIN)
  {
    // Set min timeout for synchronous us-loop
    Loop = XHC_EXEC_XFER_TIMEOUT_MS_MIN  * XHC_1_MILLISECOND;
  }
  else if (Timeout > XHC_EXEC_XFER_TIMEOUT_MS_MAX)
  {
    // Set max timeout for synchronous us-loop
    Loop = XHC_EXEC_XFER_TIMEOUT_MS_MAX * XHC_1_MILLISECOND;
  }
#endif
  ...
  for (Index = 0; Index < Loop; Index++) {
    ...
    if (Finished) {
      DEBUG ((EFI_D_INFO, "XhcExecTransfer: Transfer Complete, Core = %d, Dir = %d, Length = %d \n", 
        Xhc->CoreNum, Urb->Ep.Direction, Urb->Completed));
      if (PollUsbMsdEvt) {      
        for (Cnt = 0; Cnt < USB_MSD_EVT_CNT_MAX; Cnt++) {
          UsbfnMsdEventHandler();        
        }
      }
      break;
  ...
  if (Urb->Result & EFI_USB_ERR_TIMEOUT)
  {
    DEBUG ((EFI_D_ERROR, "XhcExecTransfer: Transfer timeout, Core = %d, Loop = %d / %d, Dir = %d, Length = %d \n", 
      Xhc->CoreNum, Index, Loop, Urb->Ep.Direction, Urb->Completed));
  }
  ...
  )
{
  ...
  USB_ASSERT_RETURN ((EvtTrb->SlotId <= Xhc->MaxSlotsEn), EFI_INVALID_PARAMETER);
  DEBUG ((EFI_D_INFO, "Enable Slot Successfully, The Slot ID = 0x%x, dev_spd %d\n", EvtTrb->SlotId, DeviceSpeed));
  SlotId = (UINT8)EvtTrb->SlotId;
  USB_ASSERT_RETURN ((SlotId != 0), EFI_INVALID_PARAMETER);
