...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 03/14/23   tarub    Fix SS portsc offset for Multiport controller
 11/28/19   rg       Change for electrical compliance failure
 07/26/19   akr      Multiport changes to enable test patterns from third controller of USB
...
#define XHC_PORTINDEX_OFFSET 0x10
...
QCOM_USB_PORT_TEST_PROTOCOL gXhciUsbPortTestTemplate = {
  EFI_QCOM_USB_PORT_TEST_PROTOCOL_REVISION_1,
  XhcRunUsbPortTest,
  XhcPortTestGetParam,
  XhcPortTestSetParam,
  XhcPortTestIdentify,
  ...
  )
{
  ...
  switch (TestMode) {
    ...
    case USB_PORT_TEST_DISABLE:
      ...
      for (Index = 0; Index < (Xhc->HcSParams1.Data.MaxPorts/2); Index++) {
        RegVal = XhcReadOpReg(Xhc, XHC_PORTPMSC_OFFSET + (XHC_PORTINDEX_OFFSET * Index));
        RegVal &= ~XHC_PORTPMSC2_PTC;
        XhcWriteOpReg(Xhc, XHC_PORTPMSC_OFFSET + (XHC_PORTINDEX_OFFSET * Index), RegVal);
      }
      // Halt (if needed) and reset the controller to end test mode
      Status = XhcResetHC(Xhc, XHC_GENERIC_TIMEOUT);
      if (EFI_ERROR(Status)) {
        DBG(EFI_D_ERROR, "Failed to reset host controller");
      }
      break;
    ...
    case USB_PORT_TEST_FORCE_ENABLE:
      ...
      DBG(EFI_D_ERROR, "Test enable for core with %d ports", Xhc->HcSParams1.Data.MaxPorts);
    ...
    case USB_PORT_TEST_ENABLE_SS_COMPLIANCE:
      // Set SS compliance mode
      // For Single port controller --> Index 0 = HS USB2 and 1 = SS USB3
      // For Multiport controller   --> Index 0, 1 = HS USB2 and 2, 3 = SS USB3
      for (Index = Xhc->HcSParams1.Data.MaxPorts/2; Index < Xhc->HcSParams1.Data.MaxPorts; Index++)
      {
        XhcWriteOpReg(Xhc, XHC_PORTSC_OFFSET + (0x10 * Index), XHC_PORTSC_LWS|XHC_PORTSC_PP|XHC_PORTSC_PLS_COMPLIANCE_MASK);
      }
    break;
	
    case USB_PORT_TEST_ADVANCE_COMP_PATTERN:
      Xhc->UsbConfigProtocol->AdvanceSSCmplPattern(Xhc->UsbConfigProtocol, Xhc->CoreNum);
    break;
