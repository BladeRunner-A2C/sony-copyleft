...
Copyright (c) 2014, 2017, 2021-2022 Qualcomm Technologies, Inc.	All Rights Reserved.
  ...
  )
{
  ...
  BufHost = UncachedAllocateAlignedPool(Bytes, USBFN_DEFAULT_BUFFER_ALIGN);
...
#else
  UncachedFreePool((void*)Block->BufHost);
#endif
  gBS->FreePool (Block->Bits);
  gBS->FreePool (Block);
}
  ...
  )
{
  UINTN                 Bytes =0;
  Bytes = EFI_PAGES_TO_SIZE (Pages);
  *HostAddress = UncachedAllocateAlignedPool(Bytes, Alignment);
  if (*HostAddress == NULL)
  {
...
{
  USB_ASSERT (Pages != 0);
  UncachedFreePool((void*)HostAddress);
}
