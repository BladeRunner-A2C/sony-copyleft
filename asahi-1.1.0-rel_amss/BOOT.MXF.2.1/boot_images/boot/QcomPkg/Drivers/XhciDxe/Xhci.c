...
// Change is needed in XHC prevent errors when CDC device (USB ETH)
// tries to send BULK transfer while in BUS SUSPEND state
// This feature needs to be disabled in this file to allow multiple device to enumerate
// or multiple plug out and plug in to be supported.
#undef FEATURE_QC_USB_WOL_EN
#define FEATURE_QC_USB_WOL_EN 0
//
...
EFI_USB2_HC_PROTOCOL gXhciUsb2HcTemplate = {
  .GetCapability = XhcGetCapability,
  .Reset = XhcReset,
  .GetState = XhcGetState,
  .SetState = XhcSetState,
  .ControlTransfer = XhcControlTransfer,
  .BulkTransfer = XhcBulkTransfer,
  .AsyncInterruptTransfer = XhcAsyncInterruptTransfer,
  .SyncInterruptTransfer = XhcSyncInterruptTransfer,
  .IsochronousTransfer = XhcIsochronousTransfer,
  .AsyncIsochronousTransfer = XhcAsyncIsochronousTransfer,
  .GetRootHubPortStatus = XhcGetRootHubPortStatus,
  .SetRootHubPortFeature = XhcSetRootHubPortFeature,
  .ClearRootHubPortFeature = XhcClearRootHubPortFeature,
  .MajorRevision = 3,
  .MinorRevision = 1,
  .Revision = EFI_USB2_HC_PROTOCOL_REVISION_3,
  .AsyncBulkTransfer = XhcAsyncBulkTransfer,
  .AsyncControlTransfer = XhcAsyncControlTransfer
};


#ifdef FEATURE_QC_USB_WOL_EN
/**
  Gets Wake on Lan state

  @param  Xhc                             Xhc instance.
  @retval QCOM_USB_CONFIG_WOL_STATE_TYPE  Wake on Lan State
...
{
  EFI_STATUS                     Status;
  QCOM_USB_CONFIG_WOL_STATE_TYPE WoLState = USB_CONFIG_WOL_DISABLED;
  
  if (Xhc && Xhc->UsbConfigProtocol && Xhc->UsbConfigProtocol->GetWoLState)
  {
    Status = Xhc->UsbConfigProtocol->GetWoLState(Xhc->UsbConfigProtocol, &WoLState);
    if (EFI_ERROR (Status))
    {
      DEBUG (( EFI_D_ERROR, "ERROR: XchGetWoLState: Failed - Error Code = 0x%08X\n\r", Status));
    }
  }
  return WoLState;
}
  ...
  )
{
  ...
  *TransferResult = EFI_USB_ERR_SYSTEM;
  ...
  if (XchGetWoLState(Xhc) >= USB_CONFIG_WOL_EN_ETH_ARMED)
  {
    //DEBUG ((EFI_D_ERROR, "XhcControlTransfer: Skip access when Bus Suspended on port 0\n"));    
    Status = EFI_SUCCESS;
    goto ON_EXIT;    
  }
#endif
  ...
  )
{
  ...
  //
  // Runstop bit will be trigger, when loading the very next driver (UsbBusControllerDriverStart->UsbBusBuildProtocl)
  // if we enable runstop now, bus driver will take extra time to halt the traffic and reset xhc
  // comment it out will saved 20ms per connect devices.
  #ifdef USB_REDUCE_BOOT_TIME
