    ...
    )
{
    ...
    DispPwrInit = TRUE;
    DEBUG ((EFI_D_INFO, "INFO: CPRDxe: Received Display Notification\n"));
...
if(CHIPINFO_FAMILY_MILOS != DalChipInfo_ChipFamily()){
    BaseAddr = 0x00630000;
    ...
    // Map MXG Rail Controller Address for Hamoa Target
    if(CHIPINFO_FAMILY_HAMOA == DalChipInfo_ChipFamily()){
        BaseAddr = 0x00634000;
        Status = HWIOProtocol->MapRegion(HWIOProtocol, "GPU_MXC_CPR3",
                (UINT8**)&(BaseAddr));
    ...
    BaseAddr = 0x221C0000;
    Status = HWIOProtocol->MapRegion(HWIOProtocol, "TMESS",
            (UINT8**)&(BaseAddr));
    if ((EFI_SUCCESS != Status))
    {
        DEBUG((EFI_D_ERROR, "ERROR: Failed to MapRegion: 0x%08x\n",
    ...
    )
{
    ...
    {
        DEBUG ((EFI_D_INFO, "INFO: CPRDxe: Successfully located Display Protocol\n"));
        DispPwrInit = TRUE;
    }
