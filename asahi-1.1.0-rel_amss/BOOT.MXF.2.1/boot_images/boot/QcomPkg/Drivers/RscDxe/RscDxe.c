...
STATIC EFI_STATUS EFI_rsc_instance_init(const char *name, void **pHandle)
{
  ...
  if (rsc_is_pcie(*rsc))
  {
    if(CLOCK_SUCCESS != Clock_Attach(&pClockHandle, "rsc"))
    {
      return EFI_LOAD_ERROR;
    }
    if(CLOCK_SUCCESS == Clock_GetId(pClockHandle, "pcie_cesta", &clock_id))
    {
      if(CLOCK_SUCCESS != Clock_Enable(pClockHandle, clock_id))
      {
        return EFI_LOAD_ERROR;
      }
    }    
