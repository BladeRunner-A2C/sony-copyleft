...
{
  USB_CONFIG_DESC         *Config = NULL;
  USB_INTERFACE_SETTING   *Setting = NULL;
  USB_INTERFACE_DESC      *Interface = NULL;
  UINTN                   Index;
  UINTN                   NumIf;
  UINTN                   Consumed;
  ...
  )
{
  ...
  ZeroMem (&Desc, sizeof (EFI_USB_STRING_DESCRIPTOR));
  ...
  )
{
  ...
  ZeroMem (&Desc, sizeof (EFI_USB_CONFIG_DESCRIPTOR));
...
/**
  Set the device's feature. This function changes
  the device's internal state.

  @param  UsbDev                The USB device to set configure to.
  @param  UsbSetFeature         Feature to set
  @param  AsyncFlag             Flag to define async control transfer
  @param  AsyncConext           Async control transfer context
