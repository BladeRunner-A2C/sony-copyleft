...
static QCOM_USB_CONFIG_PROTOCOL      *gUsbConfigProtocol;
#endif

EFI_USB_IO_PROTOCOL mUsbIoProtocol = {
  .UsbControlTransfer          = UsbIoControlTransfer,
  .UsbBulkTransfer             = UsbIoBulkTransfer,
  .UsbAsyncInterruptTransfer   = UsbIoAsyncInterruptTransfer,
  .UsbSyncInterruptTransfer    = UsbIoSyncInterruptTransfer,
  .UsbIsochronousTransfer      = UsbIoIsochronousTransfer,
  .UsbAsyncIsochronousTransfer = UsbIoAsyncIsochronousTransfer,
  .UsbGetDeviceDescriptor      = UsbIoGetDeviceDescriptor,
  .UsbGetConfigDescriptor      = UsbIoGetActiveConfigDescriptor,
  .UsbGetInterfaceDescriptor   = UsbIoGetInterfaceDescriptor,
  .UsbGetEndpointDescriptor    = UsbIoGetEndpointDescriptor,
  .UsbGetStringDescriptor      = UsbIoGetStringDescriptor,
  .UsbGetSupportedLanguages    = UsbIoGetSupportedLanguages,
  .UsbPortReset                = UsbIoPortReset,
  .Revision                    = EFI_USB_IO_PROTOCOL_REVISION_1,
  .UsbAsyncBulkTransfer        = UsbIoAsyncBulkTransfer,
  .UsbAsyncControlTransfer     = NULL
};
...
VOID
EFIAPI
UsbBusInitWoLUsbConfigProtocol(
   VOID
   )
{
  EFI_STATUS  Status;

  if (NULL == gUsbConfigProtocol)
  {
    // Check if UsbConfig protocol is installed
    Status = gBS->LocateProtocol(&gQcomUsbConfigProtocolGuid, NULL, (VOID **)&gUsbConfigProtocol);
      
    if ((Status != EFI_SUCCESS) || (NULL == gUsbConfigProtocol)) 
    {
       DEBUG((EFI_D_ERROR, "ERROR: UsbBusSetWoLState - Failed to locate gUsbConfigProtocol! Error Code = 0x%08X\r\n", Status));
    }
  }
}
...
EFIAPI
UsbBusSetWoLState(
  QCOM_USB_CONFIG_WOL_STATE_TYPE WoLState
  )
{
  EFI_STATUS  Status;

  if (gUsbConfigProtocol && gUsbConfigProtocol->SetWoLState)
  {
    Status = gUsbConfigProtocol->SetWoLState(gUsbConfigProtocol, WoLState);
    if (EFI_ERROR (Status))
    {
      DEBUG (( EFI_D_ERROR, "ERROR: UsbBusSetWoLState(%d): Failed\n\r", WoLState));
    }
  }
}
  ...
  )
{
  ...
  if (gUsbConfigProtocol && gUsbConfigProtocol->GetWoLState)
  {
    Status = gUsbConfigProtocol->GetWoLState(gUsbConfigProtocol, &WoLState);
    if (EFI_ERROR (Status))
    {
      DEBUG (( EFI_D_ERROR, "ERROR: UsbBusGetWoLState: Failed - Error Code = 0x%08X\n\r", Status));
    }
  }
  return WoLState;
}
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (UsbBus->Usb2Hc->Revision >= 0x30000) {
