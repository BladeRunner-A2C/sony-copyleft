...
Copyright (c) 2010-2021,2023 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 05/14/21   jaychoi  Fixed Wake On Lan not working when connected to Hub with multiple devices 
...
// QC addition to support USB Ethernet Wake On Lan during Offmode Charging
...
In uefiplat.cfg, static memory is defined for Windows OS static memory usage: WARNING this memory map is no longer valid
...
#if FEATURE_QC_USB_WOL_EN
    if (UsbBusGetWoLState() != USB_CONFIG_WOL_DISABLED)
    {
      DEBUG((EFI_D_ERROR, "UsbConnectDriver: Skipped on WoL - UsbIf 0x%x\n\r", UsbIf));
      goto ON_EXIT;
    }
#endif
  ...
  )
{
  ...
  for (Index = 0; Index < Device->NumOfInterface; Index++) {
    ...
    if (!EFI_ERROR (Status)) {
	  ...
	  if (EFI_ERROR (Status)) {
	  	UsbConnectDriver (UsbIf, FALSE, NULL);
	  	}
    }
	if (!EFI_ERROR (Status)) {
...
/**
  Callback function for timer expiry to poll USB Root Hub for Remote Wake-up
  
  @param  Event           Unused
  @param  Context         USB_INTERFACE_CONTEXT pointer
**/

STATIC
VOID
EFIAPI
UsbWoLRootHubBusChangeCb(
  IN EFI_EVENT       Event,
  IN VOID           *Context
  )
{
  ...
  USB_INTERFACE             *HubIf;
  USB_CONTEXT               *HubCont;
  EFI_USB_PORT_STATUS       PortState = {0};
  UINT8                     Port;
  EFI_STATUS                Status = EFI_SUCCESS;
  USB_DEVICE                *Parent;
  USB_BUS                   *Bus;
  ...
  {
    DEBUG ((EFI_D_ERROR, "UsbWoLRootHubBusChangeCb: Invalid Signature for RootHub = 0x%x \n", HubIf->Signature));

    Status = gBS->SetTimer(
      HubIfCont->usb_contxt->UsbBusChangeWoLEvt,
      TimerCancel,
      0);
    if (EFI_ERROR (Status)) {
      DEBUG (( EFI_D_ERROR, "ERROR: UsbWoLRootHubBusChangeCb: TimerCancel failed\n\r" ));
    }
  ...
  // WoL supported on Port0 of RootHub
  Port  = 0;

  Status = UsbHcGetRootHubPortStatus (Bus, Port, &PortState);
  if (EFI_ERROR (Status)) {
    DEBUG (( EFI_D_ERROR, "ERROR: UsbWoLRootHubBusChangeCb: GetRootHubPortStatus failed on port %d\n\r", Port));
  ...
  else
  {
    ...
    {
      DEBUG (( EFI_D_ERROR, "USB Port(%u): Status(0x%X), Change(0x%X)\n\r", 
        Port, 
  ...
  if (!USB_BIT_IS_SET (PortState.PortStatus, USB_PORT_STAT_CONNECTION) )
  {
    if (TRUE == HubCont->WoLSuspendArmed)
    {
      Status = gBS->SetTimer(
        HubIfCont->usb_contxt->UsbBusChangeWoLEvt,
        TimerCancel,
      ...
      DEBUG((EFI_D_ERROR, "UsbWoLRootHubBusChangeCb - Wake On LAN - Root Hub Disconnected on port%d\n\r", Port));

      UsbBusSetWoLState(USB_CONFIG_WOL_EN_ETH_INIT);
    }
  }
  else if (!USB_BIT_IS_SET (PortState.PortStatus, USB_PORT_STAT_SUSPEND))
  {
    if (TRUE == HubCont->WoLSuspendArmed)
    {
      Status = gBS->SetTimer(
      ...
      DEBUG((EFI_D_ERROR, "UsbWoLRootHubBusChangeCb - Wake On LAN - Perform SYSTEM RESET on port%d\n\r", Port));

      UsbBusSetWoLState(USB_CONFIG_WOL_EN_ETH_TRIGGERED);
    }
    else
    {
      // Set Remote Wakeup Feature to DEVICE
      #define USB_FSEL_DEV_REMOTE_WAKEUP (0x1)

      if (HubCont->Child != HubCont->WoLDevice)
      {
        DEBUG (( EFI_D_ERROR, "UsbWoLRootHubBusChangeCb: Child (%p) != WoLDevice (%p)\n\r", 
          HubCont->Child, HubCont->WoLDevice));        
      }
      
      Status = UsbSetFeature(HubCont->WoLDevice, USB_FSEL_DEV_REMOTE_WAKEUP, FALSE, 0);
      if (EFI_ERROR (Status)) {
        DEBUG (( EFI_D_ERROR, "ERROR: UsbWoLRootHubBusChangeCb: failed to SET_FEATURE(REMOTE_WAKEUP) on port %d\n\r", Port));
      }
      else
      {
        DEBUG (( EFI_D_ERROR, "UsbWoLRootHubBusChangeCb: SET_FEATURE(REMOTE_WAKEUP) on port %d\n\r", Port));
      }
      
      Status  = UsbHcSetRootHubPortFeature (Bus, Port, EfiUsbPortSuspend);
      if (EFI_ERROR (Status)) {
        DEBUG (( EFI_D_ERROR, "UsbWoLRootHubBusChangeCb: failed to Suspend on port %d\n\r", Port));
      }
      else
      {
        DEBUG (( EFI_D_ERROR, "UsbWoLRootHubBusChangeCb: Suspend Successful: port %d\n\r", Port));
      }

      HubCont->WoLSuspendArmed = TRUE;
      
      UsbBusSetWoLState(USB_CONFIG_WOL_EN_ETH_ARMED);

      DEBUG (( EFI_D_ERROR, "UsbWoLDetectEth: HubIfCont.usbif(0x%x), ctx(0x%x), idx(0x%x)\n\r", 
        HubIfCont->usbif, HubIfCont->usb_contxt, HubIfCont->index));

#define USB_CDC_POLL_INTERVAL (100 * 1000U)
            
      Status = gBS->SetTimer(
        HubIfCont->usb_contxt->UsbBusChangeWoLEvt,
        TimerPeriodic,
        USB_CDC_POLL_INTERVAL);

      if (EFI_ERROR (Status)) {
        DEBUG (( EFI_D_ERROR, "ERROR: UsbWoLRootHubBusChangeCb: failed to set timer UsbBusChangeEvt on port %d\n\r", Port));
      }
    }
  }
}
...
/**
  ...
  @param  Child           A pointer to a buffer that is filled with key data which is
  @param  Port            Port numbe within the Hub
  @param  Context         USB_INTERFACE_CONTEXT pointer
  ...
  )
{
  ...
  USB_INTERFACE_CONTEXT       *HubIfCont    = NULL;

  HubIfCont = (USB_INTERFACE_CONTEXT *)Context;

  // Skip WoL logic if WoL is disabled
  if (UsbBusGetWoLState() == USB_CONFIG_WOL_DISABLED)
  {
    return;
  }

  DEBUG (( EFI_D_ERROR, "UsbWoLDetectEth: HubIfCont.usbif(0x%x), ctx(0x%x), idx(0x%x), Child(0x%x), Port(0x%x)\n\r", 
    HubIfCont->usbif, HubIfCont->usb_contxt, HubIfCont->index, Child, Port));

  DEBUG (( EFI_D_ERROR, "UsbWoLDetectEth: ============ WOL START ==============: port %d\n\r", Port));
  ...
  for (StrIndex = 1; StrIndex < 4; StrIndex++)
  {
    ...
    if (StrDesc)
    {
      DEBUG (( EFI_D_ERROR, "UsbWoLDetectEth: StrDesc[%u] =  0x%X, size (%u), port %d: ", 
        StrIndex, StrDesc, (UINT8)*((UINT8*)StrDesc), Port));
      
      for (StrIterIndex = 0; StrIterIndex < (UINT8)*((UINT8*)StrDesc); StrIterIndex+=2)
      {
        DEBUG ((EFI_D_ERROR, "%c", (UINT8)*((UINT8*)StrDesc + StrIterIndex)));
      }
      DEBUG ((EFI_D_ERROR, "\n"));
      FreePool (StrDesc);
    }
...
#if 0 // Will be done just before SUSPEND is driven on the root port
  // Set Remote Wakeup Feature to DEVICE
  #define USB_FSEL_DEV_REMOTE_WAKEUP (0x1)

  Status = UsbSetFeature(Child, USB_FSEL_DEV_REMOTE_WAKEUP, FALSE, 0);
  if (EFI_ERROR (Status)) {
    DEBUG (( EFI_D_ERROR, "ERROR: UsbWoLDetectEth: failed to SET_FEATURE(REMOTE_WAKEUP) on port %d\n\r", Port));
  }
  else
  {
    DEBUG (( EFI_D_ERROR, "UsbWoLDetectEth: SET_FEATURE(REMOTE_WAKEUP) on port %d\n\r", Port));
  }
#endif 

  if (HubIfCont->usb_contxt->WoLSuspendArmed == FALSE)
  {
    UsbBusSetWoLState(USB_CONFIG_WOL_EN_ETH_ENUM);

    // Save WoLDevice with current Child.  Note that Child may change if there are more devices.
    HubIfCont->usb_contxt->WoLDevice = Child;
    
    // Create a timer to put in SUSPEND and POLL for RESUME
    ...
    if (EFI_ERROR (Status)) {
      DEBUG (( EFI_D_ERROR, "ERROR: UsbWoLDetectEth: failed create timer UsbBusChangeEvt on port %d\n\r", Port));
    }
    else
    {
      #define USB_CDC_TTMER_REL (200 * 1000U)
      ...
      if (EFI_ERROR (Status)) {
        DEBUG (( EFI_D_ERROR, "ERROR: UsbWoLDetectEth: failed to set timer UsbBusChangeEvt on port %d\n\r", Port));
      }
    }
  }
}
  ...
  switch (DevState)
  {
    ...
    { // Perform rest of enumeration till the completion
      ...
      if (USB_BIT_IS_SET (PortState->PortStatus, USB_PORT_STAT_SUPER_SPEED_PLUS)) {
        // EFI_USB_SPEED_SUPER_PLUS should not be used to avoid compatibility issue with EDK2 code,
        // where it only checks for EFI_USB_SPEED_SUPER
          ...
          for (Index_i = 0; Index_i < Child->DevDesc->Configs[Index_c]->Desc.NumInterfaces; Index_i++) {
            if (Child->DevDesc->Configs[Index_c]->Interfaces[Index_i] && Child->DevDesc->Configs[Index_c]->Interfaces[Index_i]->Settings[0]) {
              ...
              switch (InterfaceClass)
              {
                ...
                {
                  DEBUG ((EFI_D_ERROR, "UsbEnumNewDevRootHub: dropping audio device on port %d, if %d\n", Port, Index_i));
                IsAudioClass = TRUE;
                goto ON_ERROR;
              }
                break;
#if FEATURE_QC_USB_WOL_EN
                case USB_CDC_CONTROL_CLASS:
                {
                  DEBUG ((EFI_D_ERROR, "UsbEnumNewDevRootHub: CDC Control device on port %d, if %d\n", Port, Index_i));
                  IsCDCClass = TRUE;
                }
                break;
                case USB_CDC_DATA_CLASS:
                {
                  DEBUG ((EFI_D_ERROR, "UsbEnumNewDevRootHub: CDC Data device on port %d, if %d\n", Port, Index_i));
                  IsCDCClass = TRUE;
                }
                break;
#endif
...
#ifdef USB_CAMERA_FLAG
      #error "USB_CAMERA_FLAG should not be enabled for this target"
      //
      ...
      if (IsCDCClass)
      {
        UsbWoLDetectEth(Child, Port, Context);
      }
#endif
          ...
          for (Index_i = 0; Index_i < Child->DevDesc->Configs[Index_c]->Desc.NumInterfaces; Index_i++) {
            if (Child->DevDesc->Configs[Index_c]->Interfaces[Index_i] && Child->DevDesc->Configs[Index_c]->Interfaces[Index_i]->Settings[0]) {
               ...
               switch (InterfaceClass)
               {
                 case USB_AUDIO_CLASS:
                 {
                   DEBUG ((EFI_D_ERROR, "UsbEnumNewDevHub: dropping audio device on port %d, if %d\n", Port, Index_i));
                IsAudioClass = TRUE;
                goto ON_ERROR;
              }
                 break;
                 ...
                 case USB_CDC_CONTROL_CLASS:
                 {
                   DEBUG ((EFI_D_ERROR, "UsbEnumNewDevHub: CDC Control device on port %d, if %d\n", Port, Index_i));
                   IsCDCClass = TRUE;
                 }
                 break;
                 case USB_CDC_DATA_CLASS:
                 {
                   DEBUG ((EFI_D_ERROR, "UsbEnumNewDevHub: CDC Data device on port %d, if %d\n", Port, Index_i));
                   IsCDCClass = TRUE;
                 }
                 break;
#endif
