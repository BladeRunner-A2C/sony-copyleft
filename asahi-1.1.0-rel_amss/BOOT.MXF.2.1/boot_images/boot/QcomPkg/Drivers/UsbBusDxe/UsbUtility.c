  ...
  while (ListIndex != UsbIoDPList){
    ...
    if (UsbDpDevicePathSize == GetDevicePathSize (ListItem->DevicePath)) {
      if ((CompareMem (UsbDP, ListItem->DevicePath, UsbDpDevicePathSize)) == 0) {
        ...
        {
          DEBUG ((EFI_D_INFO, "SearchUsbDPInList: DP 0x%x, UsbDP 0x%x, size(%d)\n", 
            ListItem->DevicePath,  UsbDP, UsbDpDevicePathSize));

          for (Index = 0; Index < UsbDpDevicePathSize; Index++)
          {
            DEBUG ((EFI_D_INFO, "0x%02X ", (UINT8)*((UINT8*)UsbDP + Index)));
          }
          DEBUG ((EFI_D_ERROR, "\n"));
        }
#endif
  ...
  while (WantedListIndex != WantedUsbIoDPListPtr){
    ...
    case MSG_USB_DP:
      ...
      if (FirstDevicePathSize == GetDevicePathSize (DevicePathPtr)) {
        ...
        if (UsbBusGetWoLState() != USB_CONFIG_WOL_DISABLED)
        {
          DEBUG ((EFI_D_INFO, "UsbBusIsWantedUsbIO: MSG_USB_DP 0x%x, 0x%x\n", 
            WantedListItem->DevicePath,  DevicePathPtr));
        }
#endif
          DoConvert = TRUE;
        }
    ...
    case MSG_USB_CLASS_DP:
      if (MatchUsbClass((USB_CLASS_DEVICE_PATH *)WantedListItem->DevicePath, UsbIf)) {
        ...
        if (UsbBusGetWoLState() != USB_CONFIG_WOL_DISABLED)
        {
          DEBUG ((EFI_D_INFO, "UsbBusIsWantedUsbIO: MSG_USB_CLASS_DP 0x%x, 0x%x\n", 
            WantedListItem->DevicePath,  UsbIf));
        }
#endif
        DoConvert = TRUE;
      }
   ...
   case MSG_USB_WWID_DP:
      if (MatchUsbWwid((USB_WWID_DEVICE_PATH *)WantedListItem->DevicePath, UsbIf)) {
        ...
        if (UsbBusGetWoLState() != USB_CONFIG_WOL_DISABLED)
        {
          DEBUG ((EFI_D_INFO, "UsbBusIsWantedUsbIO: MSG_USB_WWID_DP 0x%x, 0x%x\n", 
            WantedListItem->DevicePath,  UsbIf));
        }
#endif
        DoConvert = TRUE;
