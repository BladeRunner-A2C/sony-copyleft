...
//============================================================================
/**
  Copyright (c) 2016-2018,2021-20 Qualcomm Technologies, Inc. All rights reserved.
...
)
{
  ...
  for(Index = 0; Index < NumEp; Index++) {
    if (USB_BCD_MAJOR_VER (UsbBcd) == 2) {
	  ...
	  if(usb_shared_is_eud_bulk_out_ep_limit_required(CEp->EndpointAddress, CEp->Attributes, CEp->MaxPacketSize) == TRUE) {
	    CEp->MaxPacketSize = 128;
	  }
	  #endif
...
)
{
  ...
  //Change bcdUSB to 0x0200 when EUD is active for EP Desc with MaxPacketSize 128 to be valid 
  if(is_eud_active) {
