...
--------   ---      ----------------------------------------------------------
12/29/22   kanandac Locate smp2pGuid Protocol before init
...
)
{
  ...
  EFI_STATUS status;
  EFI_SMP2P_PROTOCOL* Smp2pInterface;

  status = gBS->LocateProtocol(&gEfiSMP2PProtocolGuid,
                               NULL,
                               (VOID**)&Smp2pInterface);

  if(status != EFI_NOT_FOUND){
    DEBUG ((EFI_D_ERROR,"Smp2pdxe is already initialized.\n"));
    return status;
  }
