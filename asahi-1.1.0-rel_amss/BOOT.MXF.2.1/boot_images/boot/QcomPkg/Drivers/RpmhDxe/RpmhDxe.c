/** @file RpmhDxe.c
  ...
  Copyright (c) 2016-2017, 2020 Qualcomm Technologies, Inc. All rights 
  reserved. 
 ...
 * <!-- rpmh_sleep -->
 *
  @brief rpmh_sleep - Registration function for LPM
* @return       EFI_SUCCESS
 */
STATIC void EFI_rpmh_sleep(enum PwrTxnType mode)
{
  switch(mode)
  {
    case DevicePwrOFF :
      rpmh_enter_sleep(RSC_DRV_HLOS);
      break;
    case DevicePwrON :
      rpmh_exit_sleep(RSC_DRV_HLOS);
      break;
    default :
      break;
  }  
...
/**
 * <!-- Reset Notification CB -->
 *
 * @brief Call RPMH de-initialization
 *
 * @return       VOID
 */
VOID
RpmhResetSystemCb    (IN EFI_RESET_TYPE  ResetType,
  IN EFI_STATUS      ResetStatus,
  IN UINTN           DataSize,
  IN VOID            *ResetData OPTIONAL
  )
{
  DEBUG ((EFI_D_WARN, "Reset detected in Rpmhdxe\n"));
  EFI_rpmh_deinit();
}
...
VOID
EFIAPI
ResetNotificationProtocolNotifier (
  IN EFI_EVENT Event,
  IN VOID *Context
)
{
  ...
  if(EFI_ERROR(Status)) {
    DEBUG ((EFI_D_WARN, "Reset notification not located in RpmhDxe, Status:%r\n", Status));
    return;
  }

  Status = ResetNotifyProtocol->RegisterResetNotify(ResetNotifyProtocol, RpmhResetSystemCb);
...
/**
  Register for Reset Protocol Installation event
**/
STATIC EFI_STATUS
...
)
{
  ...
  Status = gBS->CreateEvent (EVT_NOTIFY_SIGNAL, TPL_CALLBACK, ResetNotificationProtocolNotifier, NULL, &ResetNotificationProtocolInstalledEvent);
  if(EFI_ERROR(Status)) {
    DEBUG((EFI_D_WARN, "Failed to create event for Reset Notification call back in RpmhDxe!\n"));
    return Status;
  }
  ...
  if(EFI_ERROR(Status)) {
    DEBUG((EFI_D_WARN, "Failed to Register Protocol notification for ResetNotificationProtocol in RpmhDxe!\n"));
    return Status;
  }
...
EFI_STATUS EFIAPI RpmhDxeEntryPoint(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
  ...
  //Register for Reset Notification Callback
  RegisterForResetNotificationProtocol();

  // Register sleep callback with SystemSleepLib
  status = RegisterDevicePowerTransitionCallback ((PwrTxnNotifyFn)EFI_rpmh_sleep, NULL);
  if (EFI_ERROR (status))
  {
    DEBUG ((EFI_D_ERROR, "Rpmh Sleep callback registration failed, Status = 0x%lx\n", status));
  }
