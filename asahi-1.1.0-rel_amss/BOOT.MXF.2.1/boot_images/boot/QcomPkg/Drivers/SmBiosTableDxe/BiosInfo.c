/** @file
  ...
  Copyright (c) 2010-2013, 2015, 2017, 2019-2020, 2022-2023 Qualcomm Technologies Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---      -----------------------------------------------------------
 03/22/23   bkulkarn Changes to update EC Firmware version in SMBIOS Type-0 table
...
EFI_STATUS UpdateBiosInfoTable( EFI_SMBIOS_PROTOCOL     *Smbios)
{
  ...
  CHAR8                   AsciiString[MAX_VERSION_STR_LENGTH] = {0};
  UINTN                   i = 0;
  CHAR16                 *UnicodeStringPCD = NULL;
  CHAR8                  *RelDateString, *FwVerStr = NULL;  /* format: mm/dd/yyyy */
  UINTN                   Length = 0;
  VERSION_INFO            EcFwVersion = {0};

  Status = GetEcFwVersion(&EcFwVersion);
  if (EFI_SUCCESS == Status)
  {
    SmBiosBootInfo.EmbeddedControllerMajorRevision = EcFwVersion.main_ver;
    SmBiosBootInfo.EmbeddedControllerMinorRevision = EcFwVersion.sub_ver;
  }
  else
  {
    DEBUG((EFI_D_INFO, "GetEcFwVersion() failed with %r, ignore and proceed\n", Status));
    Status = EFI_SUCCESS;
  }
