/** @file
  ...
  Copyright (c) 2010-2014,2017,2022 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 09/08/21   dmohan  Added SMBIOS_HANDLE_PI_RESERVED as default handle
 05/06/20   vb      Port changes from boot 3.0.1
 02/24/20   kbarad  Giving PCD configurable access to SMBIOS table
  ...
  )
{
  ...
  newNode->TableHandle = SMBIOS_HANDLE_PI_RESERVED;
  newNode->next = NULL;
...
/**
  CheckTableListBasedonPCD() will check PCDs value for SMBIOS table to make decision on filling it.

  @param  SmbiosConfigInitFuncType      Pointer to smbios table function linked list

  @retval BOOLEAN    TRUE if its disable
                     FALSE if it enable

**/
BOOLEAN IsSmbiosConfigInitForCurrentTypeDisabled(SmbiosConfigInitFuncType *CurrTableProc)
{
  if(FALSE == PcdGetBool(PcdUpdateBiosInfoTable) && *CurrTableProc == BiosInfoTableInit)
  {
    DEBUG ((DEBUG_INFO, "\n Disable so skip filling of BiosInfoTableInit\n"));
    return TRUE;
  }

  if((FALSE == PcdGetBool(PcdUpdateSmbiosTableType7)) &&
     ((*CurrTableProc == L1InstCacheInfoTableInit) ||
      (*CurrTableProc == L1DataCacheInfoTableInit) ||
      (*CurrTableProc == L2CacheInfoTableInit) ||
      (*CurrTableProc == L3CacheInfoTableInit)))
  {
    DEBUG ((DEBUG_INFO, "\n Disable so skip filling of L1InstCacheInfoTableInit|L1DataCacheInfoTableInit|L2CacheInfoTableInit|L3CacheInfoTableInit\n"));
    return TRUE;
  }

  if(FALSE == PcdGetBool(PcdUpdateProcessorInfoTable) && *CurrTableProc == ProcessorInfoTableInit)
  {
    DEBUG ((DEBUG_INFO, "\n Disable so continueProcessorInfoTableInit\n"));
    return TRUE;
  }

  if(FALSE == PcdGetBool(PcdUpdateSystemInfoTable) && *CurrTableProc == SystemInfoTableInit)
  {
    DEBUG ((DEBUG_INFO, "\n Disable so skip filling of SystemInfoTableInit\n"));
    return TRUE;
  }

  if((FALSE == PcdGetBool(PcdUpdateSmbioTableType16_Type17_Type19)) &&
     ((*CurrTableProc == MemoryArrayTableInit) ||
      (*CurrTableProc == MemoryDeviceInfoInit) ||
      (*CurrTableProc == MemoryArrayMappedAddressInfoInit)))
  {
    DEBUG ((DEBUG_INFO, "\n Disable so skip filling of table MemoryArrayMappedAddressInfoInit|MemoryDeviceInfoInit|MemoryArrayTableInit\n"));
    return TRUE;
  }
  return FALSE;
}
  ...
  )
{
  ...
  {
    DEBUG((EFI_D_ERROR,"gEfiChipInfoProtocolGuid protocol failed\r\n" ));
    return Status;
  }
  ...
  {
    if(IsSmbiosConfigInitForCurrentTypeDisabled(CurrTableProc))//Check if table filled by OEM or not. if yes ignore filling table here.
    {
      DEBUG ((DEBUG_INFO, "\nTable disable by PCD so skip filling\n"));
      continue;
    }
	if ((*CurrTableProc == L3CacheInfoTableInit) && (chipID == EFICHIPINFO_ID_SCP_HAMOA))
    {
		continue;
	}
    //Allocate new node
