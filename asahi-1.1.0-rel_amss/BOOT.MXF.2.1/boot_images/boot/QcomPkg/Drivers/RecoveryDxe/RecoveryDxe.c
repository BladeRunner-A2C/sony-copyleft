...
2023/03/15     gpavithr    Move CheckIfCapsulePending() to FwUpdateInfoLib
2022/12/26     dmohan      Added changes needed for 8380
2022/12/05     dmohan      Removed unnecessary log messages
...
#define     BACKUP_PARTITION_SUFFIX         L"_BACKUP"
  ...
  )
{
  ...
  StrnCatS (
    tempStr,
    (sizeof(secondaryEntry->PartitionName) / sizeof (CHAR16)),
    BACKUP_PARTITION_SUFFIX,
    (StrLen (BACKUP_PARTITION_SUFFIX) + 1)
    );
  ...
  )
{
  ...
  // Backup partition name should always contain XXX_BACKUP from Hamoa onwards,
  ...
  )
{
  ...
  for (idx = 0; idx < pPrivate->RecoveryItemCount; idx++)
  {
    status = GetSmemRecoveryPartitionInfoFlags(idx, &flags);
    if (EFI_ERROR(status))
    {
      DEBUG((
        EFI_D_WARN,
        "RestoreCorruptedPartitions: GetSmemRecoveryPartitionInfoFlags failed. PrimaryGuid[%d]: %g, Status: %r. Ignore and Proceed\n",
    ...
    // Skip partitions pending for capsule update. And partitions whose RECOVERED_FLAG is set.
    // As per Boot team inputs, SMEM data can hold critical data partition GUIDs as well.
    // And its RECOVERED status will be set by StorageResiliencyLib once it restores primary guid
    // from its backup when corruption (hmac mismatch) is detected on partition read.
    if ((pPrivate->RecoveryEntries[idx].RecoveryStatus == RECOVERY_NOT_ATTEMPTED)||
        (flags & RECOVERED_FLAG))
    {
      continue;
    }
    ...
    if (EFI_ERROR(status))
    {
      DEBUG((
        EFI_D_WARN,
        "RestoreCorruptedPartitions: ValidatePartitionGuidPairs failed. Status: %r. Ignore and Proceed\n",
        status
        ));
      status = EFI_SUCCESS;
      continue;
    ...
    if (EFI_ERROR(status))
    {
      DEBUG((
        EFI_D_WARN,
        "RestoreCorruptedPartitions: GetRecoveryPartitionHandle failed. Status: %r. Ignore and Proceed\n",
        status
        ));
      status = EFI_SUCCESS;
      continue;
    ...
    for (loopCount = 0; loopCount < MAX_RECOVERY_ATTEMPTS; loopCount++)
    {
      ...
      if (EFI_ERROR(status))
      {
        DEBUG((
          EFI_D_WARN,
          "RestoreCorruptedPartitions: FwUpdatePartitionRead failed. LoopCount: %d, Status: %r. Ignore and Proceed\n",
          loopCount,
          status
          ));
        status = EFI_SUCCESS;
      ...
      if (EFI_ERROR(status))
      {
        DEBUG((
          EFI_D_WARN,
          "RestoreCorruptedPartitions: FwUpdatePartitionWrite failed. LoopCount: %d, Status: %r. Ignore and Proceed\n",
          loopCount,
          status
          ));
        status = EFI_SUCCESS;
      ...
      if (EFI_ERROR(status))
      {
        DEBUG((
          EFI_D_WARN,
          "RestoreCorruptedPartitions: FwUpdatePartitionRead back failed. LoopCount: %d, Status: %r. Ignore and Proceed\n",
          loopCount,
          status
          ));
        status = EFI_SUCCESS;
      ...
      else if (CompareMem (readBuffer,buffer,bufferSize) != 0)
      {
        DEBUG((
          EFI_D_WARN,
          "RestoreCorruptedPartitions: Mismatch in buffer written and buffer read back. LoopCount: %d. Ignore and Proceed\n",
          loopCount
          ));
        status = EFI_SUCCESS;
        continue;
    ...
    status = SetSmemPartitionRecoveredFlag(idx);
    if (EFI_ERROR(status))
    {
      DEBUG((
        EFI_D_WARN,
        "RestoreCorruptedPartitions: SetSmemPartitionRecoveredFlag failed. LoopCount: %d, Status: %r. Ignore and Proceed\n",
          loopCount,
          status
        ));
      status = EFI_SUCCESS;
