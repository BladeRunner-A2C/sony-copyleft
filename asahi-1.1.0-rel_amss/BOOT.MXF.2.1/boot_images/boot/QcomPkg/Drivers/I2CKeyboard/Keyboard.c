...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 10/13/22   ananns  Added Shift key and End Key to struct and enabled CAP's Lock LED functionality
  ...
  )
{
  ...
  UINT8                          IsShiftKeycodeMatch = FALSE;
  ...
  //
  if (KeyData->Key.ScanCode == SCAN_NULL && KeyData->Key.UnicodeChar == CHAR_NULL && IsShiftKeycodeMatch == FALSE) {
    if (!I2CKeyboardDevice->IsSupportPartialKey) {
	  DEBUG((EFI_D_WARN, "[I2CKeyboard] keycode can not be converted\n"));
  ...
  if (KeyData->Key.ScanCode == SCANCODE_CAPS_LOCK){
    ...
    //
    // Enable the KB CAP's Lock LED
    //
    Status = SetKeyboardLED (I2CKeyboardDevice);
    if ( Status != EFI_SUCCESS){
      DEBUG((EFI_D_WARN, "[I2CKeyboard] SetKeyboardLED Failed : %r\n", Status));
    }
  }
