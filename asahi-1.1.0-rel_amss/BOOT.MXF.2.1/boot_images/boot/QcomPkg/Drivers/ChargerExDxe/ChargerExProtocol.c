                                ...
                                IN EFI_SYSTEM_TABLE   *SystemTable )
{
  ...
  {
    DEBUG(( EFI_D_ERROR, "ChargerExProtocol:: %a Failed to install Protocol Interfaces: %r \r\n", __FUNCTION__, Status));
    return Status;
  }
  ...
  {
    DEBUG(( EFI_D_WARN, "ChargerExProtocol:: %a Charging is not supported on this platform. \r\n", __FUNCTION__));
    Status = EFI_SUCCESS;
  }
  else 
  {
...
)
{
  ...
  if((EFI_PLATFORMINFO_TYPE_CLS == PlatformType) || (EFI_PLATFORMINFO_TYPE_CDP == PlatformType) || (EFI_PLATFORMINFO_TYPE_ATP == PlatformType))
  {
    DEBUG(( EFI_D_WARN, "ChargerExProtocol:: %a CDP/CLS/ATP (%d) Platform detected. No Battery information available. \r\n", __FUNCTION__, PlatformType));
    return EFI_UNSUPPORTED;
  }
...
)
{
  ...
  *bOffModeCharging = FALSE;
  ...
  if((EFI_PLATFORMINFO_TYPE_CDP == PlatformType) || (EFI_PLATFORMINFO_TYPE_ATP == PlatformType))
  {
    DEBUG(( EFI_D_WARN, "ChargerExProtocol:: %a CDP/ATP (%d) Platform detected. No Battery information available. \r\n", __FUNCTION__, PlatformType));
    return EFI_UNSUPPORTED;
  }
  ...
  else
  {
    ...
    if (ChargerPresent == FALSE) {
      if (!PmicUsbProtocol) {
        Status |= gBS->LocateProtocol (&gQcomPmicUsbProtocolGuid, NULL,
                                     (VOID **)&PmicUsbProtocol);
        if((EFI_SUCCESS != Status) || (NULL == PmicUsbProtocol)) {
          DEBUG ((EFI_D_ERROR, "Error locating pmic USB protocol: %r\n",
                Status));
          return EFI_DEVICE_ERROR;
        }
      }
      Status |= PmicUsbProtocol->UsbinValid(&ChargerPresent);
      if (EFI_SUCCESS != Status) {
        DEBUG ((EFI_D_ERROR, "Error getting UsbinValid: %r\n", Status));
        return Status;
      }
      if (ChargerPresent != FALSE)
        DEBUG ((EFI_D_ERROR,
            "[WA] ChargerPresent is FALSE then UsbinValid instead\n"));
    }
#endif  /* FEATURE_XBOOT */
    ...
    if(Status == EFI_SUCCESS)
    {
      if((IsColdBoot) 
        ...
        && (PonReasonType.PON1 || PonReasonType.USB_CHG || PonReasonType.CBLPWR || PonReasonType.DC_CHG))       /* if PON1 or USB triggered PON or cable PON
...
)
{
  ...
  if((EFI_PLATFORMINFO_TYPE_CDP == PlatformType) || (EFI_PLATFORMINFO_TYPE_CLS == PlatformType) || (EFI_PLATFORMINFO_TYPE_ATP == PlatformType))
  {
      DEBUG(( EFI_D_WARN, "ChargerExProtocol:: %a CDP/CLS/ATP (%d) Platform detected. No Battery information available. \r\n", __FUNCTION__, PlatformType));
  ...
  switch(PowerType)
  {
    case EFI_CHARGER_EX_POWER_FLASH_BATTERY_VOLTAGE_TYPE:
      ...
      {
          DEBUG(( EFI_D_WARN, "ChargerExProtocol:: %a No Battery information available. \r\n", __FUNCTION__));
          pFlashInfo->bCanFlash = TRUE;
          return Status;
      }
      Status |= EFI_ChargerExGetChargerPresence(&ChargerPresent);
      ...
      if(Status == EFI_SUCCESS)
      {
        ...
        {
          DEBUG(( EFI_D_INFO, "ChargerExProtocol:: %a ok to flash SwFlashBattMinVoltage = %d mV\r\n", __FUNCTION__,SwFlashBattMinVoltage));
          pFlashInfo->bCanFlash = TRUE;
        }
        else
        {
