...
extern EDKII_IOMMU_PROTOCOL *gHalIoMmuProtocol;
...
STATIC CONST EFI_PCI_ROOT_BRIDGE_DEVICE_PATH  gDevicePathTemplate = {
    {
      ...
      EISA_PNP_ID (0x0A08),  // TODO: Is this for a generic PCI bridge?? unable find the assignment
      0
    },
    {
...
#define PCIE_MEM_SPACE_ATTRIBUTES        (EFI_PCI_ATTRIBUTE_IDE_PRIMARY_IO |\
  ...
  )
{
  ...
  if ((This == NULL) || (Result == NULL) || (Width > EfiPciWidthUint64))
  ...
  )
{
  ...
  if ((This == NULL) || (Buffer == NULL) || ((UINT32)Width >= EfiPciWidthMaximum))
    return EFI_INVALID_PARAMETER;
  ...
  {
    Base = RESOURCE_BASE(RootBridgeInstance->ResAperture[PciRbResTypeMem32]);
    Limit = RESOURCE_LIMIT(RootBridgeInstance->ResAperture[PciRbResTypeMem32]);
    /* Base <= Address <= Limit */
    if ((Address < Base) || (Address > Limit))  // TODO: Improve it by requesting the actual type from top level
	{
      Base = RESOURCE_BASE(RootBridgeInstance->ResAperture[PciRbResTypePMem64]);
      Limit = RESOURCE_LIMIT(RootBridgeInstance->ResAperture[PciRbResTypePMem64]);
	}
  }
  else
  {
  ...
  )
{
  ...
  if ((Address >= Base) && (Address <= Limit))
    Address = PCI_ADDR_TO_SYS_ADDR (Address, RootBridgeInstance->ResAperture[PciRbResTypeMem32]);
  else
  {
    Base = RESOURCE_BASE(RootBridgeInstance->ResAperture[PciRbResTypePMem64]);
    Limit = RESOURCE_LIMIT(RootBridgeInstance->ResAperture[PciRbResTypePMem64]);
    if ((Address >= Base) && (Address <= Limit))
      Address = PCI_ADDR_TO_SYS_ADDR (Address, RootBridgeInstance->ResAperture[PciRbResTypePMem64]);
  }
  ...
  )
{
  ...
  if ((This == NULL) || (Width > EfiPciWidthUint64))
    return EFI_INVALID_PARAMETER;
  ...
  {
    Status = PciRbMemRead (This, Width, SrcAddress, 1, &Result);
    if (EFI_ERROR (Status))
      return Status;

    Status = PciRbMemWrite (This, Width, DestAddress, 1, &Result);
    if (EFI_ERROR (Status))
      return Status;
  ...
  )
{
  ...
  *Mapping = NULL;
  ...
  if (gHalIoMmuProtocol != NULL)
  {
    Status = gHalIoMmuProtocol->Map (gHalIoMmuProtocol,
  ...
  if ((PhysicalAddress + *NumberOfBytes) > 0x100000000ULL)
  {
    ...
    *Mapping = MapInfo;
    ...
    {
      CopyMem ((VOID *)(UINTN)MapInfo->MappedHostAddress,
  ...
  )
{
  ...
  if (gHalIoMmuProtocol != NULL)
  {
    Status = gHalIoMmuProtocol->Unmap (gHalIoMmuProtocol, Mapping);
    return Status;
  }
  ...
  if (This != NULL)
  {
    ...
    if (Mapping != NULL)
    {
      ...
      {
        CopyMem ((VOID *)(UINTN)MapInfo->HostAddress,
  ...
  )
{
  ...
  if ((This == NULL) || ((Attributes == NULL) && (Supported == NULL)))
    return EFI_INVALID_PARAMETER;
  ...
  )
{
  ...
  *Resources = &Configuration;    // YUG:  A private defined data type and global is being returned..!!
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  if (Metronome == NULL)
  {
    ...
    if (EFI_ERROR (Status))
    {
      DEBUG((EFI_D_ERROR, "ERROR: Metronome protocol not found\r\n"));
      return Status;
    }
  }

  Status = PcieConfigLibGetSegmentNumber (HostBridge->HostBridgeIndex, &SegmentNum);
  if (EFI_ERROR (Status))
  ...
  RootBridge->Io.SegmentNumber = SegmentNum;
  ...
  Status = PcieConfigLibGetResourceAperture (HostBridge->HostBridgeIndex, RootBridge->ResAperture);
  ...
  // Is IO space mapping/support really needed?
#ifdef PCIE_IO_SPACE_SUPPORTED
  PcieConfigLibSetupIoSpace (HostBridge->HostBridgeIndex,
                             RootBridge->ResAperture[PciRbResTypeIo].Start,
                             RootBridge->ResAperture[PciRbResTypeIo].End,
                             RootBridge->ResAperture[PciRbResTypeIo].Base);
#endif
  ...
  // PCI Attributes
  RootBridge->Supports = PCIE_MEM_SPACE_ATTRIBUTES;
  RootBridge->Attributes = RootBridge->Supports;
  // Do not combine 32/64 bit resource space requests
  RootBridge->MemAllocAttributes = EFI_PCI_HOST_BRIDGE_MEM64_DECODE; // EFI_PCI_HOST_BRIDGE_COMBINE_MEM_PMEM;

  // Install Protocol Instances. It will also generate a device handle for the PCI Root Bridge
  Status = gBS->InstallMultipleProtocolInterfaces (&RootBridge->Handle,
                      &gEfiDevicePathProtocolGuid, &RootBridge->DevicePath,
                      &gEfiPciRootBridgeIoProtocolGuid, &RootBridge->Io, NULL);
...
**/
EFI_STATUS
PciRbDestructor (IN  PCI_ROOT_BRIDGE_INSTANCE* RootBridge)
{
  EFI_STATUS Status;
  ...
  Status = gBS->UninstallMultipleProtocolInterfaces (RootBridge->Handle,
                        &gEfiDevicePathProtocolGuid, &RootBridge->DevicePath,
                        &gEfiPciRootBridgeIoProtocolGuid, &RootBridge->Io, NULL);
