...
2022/02/25   jt      Add Level Shifter updates
2021/04/27   jt      Register for sleep callback 
2020/06/04   jt      Add DeviceMgmt protocol 
2020/05/22   vm      Fill serial_num_len in Card_info
2019/07/22   jt      Add SMMU configuration
2018/04/24   jt      Changes to RegisterListener
2018/04/12   kpa     Added gEfiSDBootOptionGuid
2018/02/21   sb      Populate card_type in GetCardInfo 
...
/* eMMC/SD device management protocol */
static const EFI_STORAGE_DEV_MGMT_PROTOCOL gSdDeviceMgmtProtocol = {
   EFI_STORAGE_DEV_MGMT_PROTOCOL_REVISION2,  /* Revision */
   SdccFillDeviceInfo,                       /* GetDeviceInfo */
   SdccGetErrorStats,                        /* GetErrorStats */
   {                                         /* PowerMgmt */
      SdccGetOpModeCaps,                     /* GetOpModeCaps */
      SdccGetCurrentOpMode,                  /* GetCurrentOpMode */
      SdccSetSpeed,                          /* SetSpeedLevel */
      SdccSetVoltageCorner,                  /* SetVoltageCorner */
      SdccSetOperationalMode,                /* SetOperationalMode */
      SdccConfigLinkParams,                  /* ConfigLink */
      SdccSetLinkState,                      /* SetLinkState */
      SdccSetDeviceMode                      /* SetDeviceMode */
   },
   {                                         /* HwConfig */
      SdccGetLevelShifterInfo                /* GetLevelShifterInfo */
   }
};
   ...
   )
{
   ...
   card_info->serial_num_len = sizeof(UINT32);
   CopyMem(card_info->product_serial_num, &gCardInfo.prod_serial_num, card_info->serial_num_len);  
...
EFI_STATUS HexToStringConverter(UINT32 HexValue, UINT32 Size, CHAR8 **String)
{
   INT8 i = 0; 
   UINT8 HexNibble = 0; 
   UINT32 StringLen = 2 * Size;
   CHAR8 *TextString = NULL; 
   ...
   TextString = AllocateZeroPool (StringLen + 1); 
   if (NULL == TextString) 
   {
   ...
   {
      HexNibble = (HexValue % 16); 
      if (HexNibble < 0xa)
      {
         TextString[i] = HexNibble + '0'; 
      }
      else
      {
         TextString[i] = (HexNibble - 0xa) + 'a'; 
      }
...
/**
  Function: SdccFillDeviceInfo

  Gets the storage device information 

  @param  This                Pointer to the EFI_STORAGE_DEV_MGMT_PROTOCOL instance
  @param  DevInfo             Storage device information

  @return EFI_SUCCESS             Function completed successfully.
  @return EFI_INVALID_PARAMETER   Parameter is invalid.
  @return EFI_OUT_OF_RESOURCES    Buffer could not be allocated.
  @return EFI_DEVICE_ERROR        Device encountered an error getting the device information.
**/
EFI_STATUS EFIAPI SdccFillDeviceInfo (
    ...
    OUT EFI_STORAGE_DEV_INFO *DevInfo)
{
   ...
   SDCC_STATUS rc = SDCC_NO_ERROR;
   struct sdcc_device *sdcc_handle = NULL;
   static sdcc_mem_info_type CardInfo; 

   if ((NULL == This) || (NULL == DevInfo)) {
      return EFI_INVALID_PARAMETER;
   }

   MmchsDevice = MMCHS_DEV_FROM_DEV_MGMT (This);
   sdcc_handle = MmchsDevice->DeviceHandle;
   ...
   /* Get SD/eMMC Card Info */
   rc = sdcc_handle_mem_get_device_info (sdcc_handle, &CardInfo);
   if (SDCC_NO_ERROR != rc)
   {
      DEBUG ((EFI_D_ERROR, "SdccFillDeviceInfo: Get Card Info FAILED. %x\n", rc));
      return EFI_DEVICE_ERROR;
   }

   /* Get Spec Version - spec_version of CardInfo */
   DevInfo->st_info.SpecVersion = CardInfo.spec_version;

   /* Get Vendor Id- OEM ID for SD card (2 bytes) */
   DevInfo->st_info.VendorId = AllocateZeroPool (4); 
   if (NULL == DevInfo->st_info.VendorId) 
   {
      return EFI_OUT_OF_RESOURCES; 
   }
   DevInfo->st_info.VendorId[0] = (CardInfo.oem_id >> 8) & 0xff;
   DevInfo->st_info.VendorId[1] = CardInfo.oem_id & 0xff; 

   /* Get Product Id- 5 characters */
   DevInfo->st_info.ProductId = AllocateZeroPool (sizeof(CardInfo.prod_name)); 
   if (NULL == DevInfo->st_info.ProductId) 
   {
      return EFI_OUT_OF_RESOURCES; 
   }
   CopyMem(DevInfo->st_info.ProductId, CardInfo.prod_name, sizeof(CardInfo.prod_name)); 

   /* Serial number is a UINT32 and 2 characters represent 1 numerical byte + NULL */
   EfiStatus = HexToStringConverter(CardInfo.prod_serial_num, sizeof(CardInfo.prod_serial_num), &DevInfo->st_info.SerialNumber);
   if (EFI_SUCCESS != EfiStatus)
   {
      return EfiStatus;
   }

   /* Firmware revision- 2 binary digits of 4 bits each */
   EfiStatus = HexToStringConverter(CardInfo.prod_rev, sizeof(CardInfo.prod_rev), &DevInfo->st_info.FirmwareRevision);
   if (EFI_SUCCESS != EfiStatus)
   {
      return EfiStatus;
   }
...
/**
  Function: SdccGetErrorStats

  Gets the storage error statistics 

  @param This                Pointer to the EFI_STORAGE_DEV_MGMT_PROTOCOL instance
  @param ErrInfo             Information about the error statistics

  @return EFI_SUCCESS             Function completed successfully.
  @return EFI_INVALID_PARAMETER   Parameter is invalid.
  @return EFI_UNSUPPORTED         Function doesn't have error statistics to report.
**/
EFI_STATUS EFIAPI SdccGetErrorStats (
    IN EFI_STORAGE_DEV_MGMT_PROTOCOL *This, 
...
/**
  Function: SdccGetOpModeCaps

  Gets the storage operational mode capabilities 

  @param  This                Pointer to the EFI_STORAGE_DEV_MGMT_PROTOCOL instance
  @param  OpModeCaps          Information about the operational mode capabilities

  @return EFI_SUCCESS             Function completed successfully.
  @return EFI_INVALID_PARAMETER   Parameter is invalid.
  @return EFI_DEVICE_ERROR        Failure to get the capabilities.
**/
EFI_STATUS EFIAPI SdccGetOpModeCaps (
    IN EFI_STORAGE_DEV_MGMT_PROTOCOL *This,
    OUT EFI_STORAGE_OP_MODE_CAPABILITIES *OpModeCaps)
{
   ...
   rc = sdcc_handle_get_operational_mode_capabilities (sdcc_handle, (void *)OpModeCaps);
   if (SDCC_NO_ERROR != rc)
   {
      DEBUG ((EFI_D_ERROR, "sdcc_handle_get_operational_mode_capabilities failed with return value %d\n", rc)); 
      return EFI_DEVICE_ERROR;
   }
...
/**
  Function: SdccGetCurrentOpMode 

  Gets the current storage operational mode 

  @param  This                Pointer to the EFI_STORAGE_DEV_MGMT_PROTOCOL instance
  @param  OpModeInfo          Information about the current operational mode

  @return EFI_SUCCESS             Function completed successfully.
  @return EFI_INVALID_PARAMETER   Parameter is invalid.
  @return EFI_DEVICE_ERROR        Failure to get the current operational mode.
**/
EFI_STATUS EFIAPI SdccGetCurrentOpMode (
    IN EFI_STORAGE_DEV_MGMT_PROTOCOL *This, 
    OUT EFI_STORAGE_OP_MODE_INFO *OpModeInfo)
{
   ...
   rc = sdcc_handle_get_operational_mode_info (sdcc_handle, &curr_info);
   if (SDCC_NO_ERROR != rc)
   {
      DEBUG ((EFI_D_ERROR, "sdcc_handle_get_operational_mode_info failed with return value %d\n", rc));
      return EFI_DEVICE_ERROR;
   }

   OpModeInfo->CurrentSpeedLevel = curr_info.speed_mode;
   OpModeInfo->CurrentVoltCorner = curr_info.volt_corner; 
   OpModeInfo->LinkState = 1; 

   return EFI_SUCCESS;
}

/**
  Function: SdccSetSpeed 

  Sets the storage speed mode

  @param  This                Pointer to the EFI_STORAGE_DEV_MGMT_PROTOCOL instance
  @param  SpeedLevel          Speed level to set

  @return EFI_SUCCESS             Speed level was set successfully.
  @return EFI_INVALID_PARAMETER   Parameter is invalid. 
  @return EFI_UNSUPPORTED         The speed level passed is unsupported with the current voltage corner.
  @return EFI_DEVICE_ERROR        The speed level was not set successfully.
**/
EFI_STATUS EFIAPI SdccSetSpeed (
    IN EFI_STORAGE_DEV_MGMT_PROTOCOL *This,
    IN UINT8 SpeedLevel)
{
   MMCHS_DEV *MmchsDevice;
   ...
   if (NULL == sdcc_handle) 
   {
    ...
    /* Set the speed to the requested one */
   rc = sdcc_handle_set_speed (sdcc_handle, SpeedLevel);
   if (SDCC_NO_ERROR != rc) 
   {
      DEBUG ((EFI_D_ERROR, "sdcc_handle_set_speed failed with return value %d\n", rc)); 
      return EFI_DEVICE_ERROR;
   }
...
/**
  Function: SdccSetVoltageCorner

  Sets the voltage corner that storage should enter

  @param  This                Pointer to the EFI_STORAGE_DEV_MGMT_PROTOCOL instance
  @param  VoltCorner          Voltage corner to enter

  @return EFI_SUCCESS             No issues changing storage to the voltage corner.
  @return EFI_INVALID_PARAMETER   Parameter is invalid. 
  @return EFI_UNSUPPORTED         The voltage corner is unsupported with the current speed level.
  @return EFI_DEVICE_ERROR        Issues were encountered while changing storage to the voltage corner.
**/
EFI_STATUS EFIAPI SdccSetVoltageCorner (
    IN EFI_STORAGE_DEV_MGMT_PROTOCOL *This,
    IN EFI_STORAGE_VOLT_CORNER VoltCorner)
{
   ...
   rc = sdcc_handle_configure_volt_corner (sdcc_handle, VoltCorner);
   if (SDCC_NO_ERROR != rc) 
   {
      DEBUG ((EFI_D_ERROR, "sdcc_handle_configure_volt_corner failed with return value %d\n", rc)); 
      return EFI_DEVICE_ERROR;
   }
...
/**
  Function: SdccSetOperationalMode

  Sets the storage operating mode (speed level + voltage corner)

  @param  This                Pointer to the EFI_STORAGE_DEV_MGMT_PROTOCOL instance
  @param  VoltCorner          Voltage corner to enter
  @param  SpeedLevel          Speed level to be set

  @return EFI_SUCCESS             Storage switched to the indicated speed level and voltage corner.
  @return EFI_INVALID_PARAMETER   Parameter is invalid. 
  @return EFI_UNSUPPORTED         The voltage corner and speed level combination is unsupported. 
  @return EFI_DEVICE_ERROR        Issues were encountered while changing storage to the speed level and/or voltage corner. 
**/
EFI_STATUS EFIAPI SdccSetOperationalMode (
    IN EFI_STORAGE_DEV_MGMT_PROTOCOL *This,
    IN EFI_STORAGE_VOLT_CORNER VoltCorner,
    IN UINT8 SpeedLevel)
{
   ...
   rc = sdcc_handle_set_operational_mode (sdcc_handle, VoltCorner, SpeedLevel);
   if (SDCC_NO_ERROR != rc) 
   {
      DEBUG ((EFI_D_ERROR, "sdcc_handle_set_operational_mode failed with return value %d\n", rc)); 
      return EFI_DEVICE_ERROR;
   }
...
/**
  Function: SdccConfigLinkParams

  Configures any link parameters 

  @param  This                Pointer to the EFI_STORAGE_DEV_MGMT_PROTOCOL instance
  @param  Params              Link parameters to configure

  @return EFI_SUCCESS             Function completed successfully.
  @return EFI_INVALID_PARAMETER   Parameter is invalid.
  @return EFI_DEVICE_ERROR        Issues were encountered when configuring the parameters given 
  @return EFI_UNSUPPORTED         Function not supported for this storage type
**/
EFI_STATUS EFIAPI SdccConfigLinkParams (
    IN EFI_STORAGE_DEV_MGMT_PROTOCOL *This,
...
/**
  Function: SdccSetLinkState

  Changes the storage link state 

  @param  This                Pointer to the EFI_STORAGE_DEV_MGMT_PROTOCOL instance
  @param  LinkState           The state to change the link to 

  @return EFI_SUCCESS             Function completed successfully.
  @return EFI_INVALID_PARAMETER   Parameter is invalid.
  @return EFI_DEVICE_ERROR        Failure to change link state. 
  @return EFI_UNSUPPORTED         Function not supported for this storage type.
**/
EFI_STATUS EFIAPI SdccSetLinkState (
    IN EFI_STORAGE_DEV_MGMT_PROTOCOL *This,
...
/** 
  Function: SdccSetDeviceMode 

  Sets the storage device mode
  ...
  @return EFI_INVALID_PARAMETER   Parameter is invalid. 
  @return EFI_UNSUPPORTED         Device mode is not supported.
  @return EFI_DEVICE_ERROR        Device reported an error.  
**/
EFI_STATUS EFIAPI SdccSetDeviceMode (
    IN EFI_STORAGE_DEV_MGMT_PROTOCOL *This,
...
/** 
  Function: SdccGetLevelShifterInfo 
  ...
  @return EFI_DEVICE_ERROR        Device reported an error.  
**/
EFI_STATUS EFIAPI SdccGetLevelShifterInfo (
    IN EFI_STORAGE_DEV_MGMT_PROTOCOL *This, 
    OUT EFI_STORAGE_LS_INFO *LsInfo)
{
   struct sdcc_device *sdcc_handle = (struct sdcc_device *)MmchsDeviceSd.DeviceHandle; 

   if ((This == NULL) || (LsInfo == NULL)) 
   {
      return EFI_INVALID_PARAMETER; 
   }

   if (This->Revision < EFI_STORAGE_DEV_MGMT_PROTOCOL_REVISION2) 
   {
      return EFI_INCOMPATIBLE_VERSION; 
   }

   if ((sdcc_handle == NULL) || (sdcc_handle->hdev == NULL) || (sdcc_handle->hdev->pslot == NULL))
   {
      return EFI_DEVICE_ERROR; 
   }

   LsInfo->LevelShifterPresent = (sdcc_handle->hdev->pslot->capabilities.HAL_sdhci_Reg_44_46 & HAL_SDHCI_CAPS_USE_TUNING_FOR_SDR50_BMSK) >> HAL_SDHCI_CAPS_USE_TUNING_FOR_SDR50_SHFT;
   return EFI_SUCCESS; 
}
  ...
  )
{
   ...
   status = gBS->InstallMultipleProtocolInterfaces (
                        ...
                        &gEfiStorageDevMgmtProtocolGuid, &MmchsDeviceSd.DeviceMgmt,
                        NULL
                        );
  ...
  )
{
   ...
   Status = RPMBListenerInit(BlkIo->Media->MediaId); 
   if (EFI_SUCCESS != Status)
   {
      DEBUG ((EFI_D_ERROR, "Failed to initialize RPMB Listener, Status 0x%08x\n", Status));
   }

   Status = GPTListenerInit(BlkIo);
   if (EFI_SUCCESS != Status)
   {
...
static void SdccSleepCb (enum PwrTxnType SleepMode, UINT32 *SlotNumber)
{
   INT32 rc = 0; 
   ...
   {
      case DevicePwrOFF:
         if (*SlotNumber == PcdGet32(SDCCExtSlotNumber)) 
         {
             rc = sdcc_handle_enable_retuning(MmchsDeviceSd.DeviceHandle);
             if (SDCC_NO_ERROR != rc) 
             {
                 DEBUG ((EFI_D_ERROR, "sdcc_handle_enable_retuning with return value of %d\n", rc));
             }
         }
         break;
      default:
   ...
   )
{
   ...
   /* Configure SMMU for SDCC */
   if (SDCC_BSP_NO_ERROR != sdcc_bsp_configure_smmu())
   {
      return EFI_DEVICE_ERROR; 
   }
   ...
   if (Status == EFI_SUCCESS)
   {
      RegisterListener(ScmInitialized, (VOID *)&MmchsDeviceEmmc[0].BlkIo); 
   }
   else
   {
...
Install_SD_Handle:
   ...
   MmchsDeviceSd.DeviceMgmt     = gSdDeviceMgmtProtocol;
   ...
   /* Register for Sleep callback with SystemSleepLib to handle CX power collapse */
   Status = RegisterDevicePowerTransitionCallback ((PwrTxnNotifyFn)SdccSleepCb, &MmchsDeviceSd.SlotNumber); 
   if (EFI_SUCCESS != Status)
   {
       DEBUG ((EFI_D_ERROR, "Sdcc Sleep callback registration failed, Status = 0x%lx\n", Status));
       ASSERT_EFI_ERROR (Status); 
   }
