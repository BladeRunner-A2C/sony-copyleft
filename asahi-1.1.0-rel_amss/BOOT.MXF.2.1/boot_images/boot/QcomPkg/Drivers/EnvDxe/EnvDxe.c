...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 04/10/23    rk       Added support for Remap Event callback.
 03/02/23    cm       Added support to Register for Reset Arch and Notification
 02/15/23    rk       Added Pasr protocol registration related code.
...
extern LockHandle  *UefiCfgLockPtr;
...
EFI_EVENT  VirtualAddressChangeEvnt = NULL;

VOID
EFIAPI
VirtualAddressChangeCB (
  IN EFI_EVENT  Event,
  IN VOID       *Context
  )
{
  ...
  )
{
  ...
  Status = GetMemRegionInfoByName ("CPU_Vectors", &MemRegion);
  if (Status != EFI_SUCCESS) {
    DEBUG ((EFI_D_ERROR, "ERROR: CPU_Vectors region not present in cfg file !\n"));
    return;
  }
...
EFI_GetPreLoadedRamPartitions (
  IN EFI_RAMPARTITION_PROTOCOL  *This,
  OUT PreLoadedRamPartition     *PreLoadedRamPartitionTable,
  OPTIONAL
  ...
  )
{
  ...
  UINT64      PasrSupportMemoryStart = 0;
  ...
  {
    Status = RegisterPasrProtocol();
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_WARN, "Error: Unable to register Pasr protocol notification callback\n"));
    }

    /*
     * Register for remap event. This event will be sent by chargerDxe to indicate that
     * current charging mode does nto require pasr feature activation i.e. offModeCharging 
     * is FALSE. As callback uefi firmware should remap the regions above
     * PasrSupportMemoryStart boundary.
     */ 
    Status = RegisterRemapEvent();
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_ERROR, "Error: Unable to register remap event callback\n"));
    }
  }
  
  if(PcdGetBool(PcdResetNotifyEnable))
  {
    RegisterForResetArchProtocol();
	RegisterForResetNotificationProtocol();
  }
