/*===============================================================================
  ...
  --------   ---         --------------------------------------------------------
  03/30/23   cm         Added code to update NonFatal field.
  01/26/23   rk         Added code to update isFileCompressed field.
...
STATIC VOID ConfigKeyValCb (UINT8* Section, UINT8* Key, UINT8* Value)
{
    ...
    {
        AsciiStrToUnicodeStrS((CHAR8*)Value, TempCfg.BackupPartiLabel, NAME_MAX_LENGTH);
    }
    ...
    else if (AsciiStriCmp ((CHAR8*)Key, "IsFileCompressed") == 0)
    {
        if (AsciiStriCmp((CHAR8*)Value, "Yes") == 0)
            TempCfg.IsFileCompressed = TRUE;
        else if (AsciiStriCmp((CHAR8*)Value, "No") == 0)
            TempCfg.IsFileCompressed = FALSE;
    }
    
    else if (AsciiStriCmp ((CHAR8*)Key, "NonFatal") == 0)
    {
		if (AsciiStriCmp((CHAR8*)Value, "Yes") == 0)
            TempCfg.NonFatal = TRUE;
        else if (AsciiStriCmp((CHAR8*)Value, "No") == 0)
            TempCfg.NonFatal = FALSE;
    }
