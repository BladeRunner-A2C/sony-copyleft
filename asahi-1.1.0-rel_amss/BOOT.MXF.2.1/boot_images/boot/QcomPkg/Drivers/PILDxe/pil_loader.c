/*===============================================================================
  ...
  01/10/23   rk         Added image decompression support.
  08/17/22   var        Added error logging support for PIL authentication
  08/08/22   cm         Added support for read files over Pxe(network)
  05/03/22   cm         Retry shutting down subsystem
  07/01/21   ka         Flush cache for buffer being passed to TZ
  03/25/21   cm         Remove Assignment in ASSERT macros
  01/20/21   yw         make two instance of metadata buffer for PIL and tz seperately 
                        since tz will lock the buffer until bring modem out of reset
  09/10/20   yw         add PilShutdown()
  07/07/20   kpa        Support loading images from raw partition when imagePath is Null
...
extern UINT64 StorageSpentCnt, FvProcSpentCnt, DecompressProcSpentCnt;
extern UINT64 DecompressRatio;
...
PIL_STATUS PilValidateHdr(PIL_SUBSYS_CTX_TYPE* Ctx, UINT8* fwbuf) 
{
    ...
    ASSERT(Phdr < (Elf32_Phdr*)((uint8*)fwbuf + Priv->MdtBufSize));
    ASSERT_FAIL_PIL_RETURN((Elf32_Phdr*)((uint8*)fwbuf + Priv->MdtBufSize), PIL_INVALID_PARAM, PIL_INVALID_PARAM);
...
PIL_STATUS PilValidateMetadata(PIL_SUBSYS_CTX_TYPE* Ctx)
{
    ...
    */
    ...
    Status = PilScmStdCall(Ctx, TZ_PIL_INIT_ID, TZ_PIL_INIT_ID_PARAM_ID, Cfg->SubsysID, ConvertToPhysicalAddress(Priv->UncachedMdtBuf));
    if (EFI_ERROR(Status)) {
...
/**
  This function decompresses the provided compressed file. Currently only lzma
  decompression alogorithm is supported. If decompression is successful then
  ElfBuffer contains the decompressed image and ElfSize contains the size 
  of decompressed image buffer.

  @param[in] ElfBuffer    ptr to buffer containing compressed file.
  @param[in] ElfSize      Size of buffer containing compressed file.
  @retval PIL_SUCCESS       success
  @retval other             failure

**/
PIL_STATUS PilDecompressElfFile(UINT8** ElfBuffer, UINTN* ElfSize)
{
    /*Params required for decompressing the read file*/
    UINT32          DestinationSize            = 0;
    UINT32          ScratchSize                = 0;
    VOID            *pScratchBuffer            = NULL;
    RETURN_STATUS   DecompressReturnStatus     = RETURN_SUCCESS;
    UINTN           ElfDecompressedSize        = 0;
    UINT8*          ElfDecompressedBuffer      = NULL;
    UINT32          ElfDecompressedSizeAligned = 0;
    UINT32          ElfSizeAligned             = ALIGN_UP_SZ(*ElfSize, EFI_PAGE_SIZE);
    UINT64          DecompressStartTime        = 0;
    UINT64          DecompressEndTime          = 0;
    PIL_INFO("PilDecompressElfFile:Incoming buffer *ElfBuffer = 0x%x *ElfSize =0x%x\n",*ElfBuffer,*ElfSize);
    DecompressStartTime = GetPerformanceCounter();
    /*
     * Call getDecompressInfo to get decompression params.
     */
    DecompressReturnStatus = LzmaUefiDecompressGetInfo(*ElfBuffer, *ElfSize, &DestinationSize, &ScratchSize );
    if(DecompressReturnStatus != RETURN_SUCCESS)
    {
        PIL_ERROR("Decompressed file size is too large.\n");
        return PIL_DECOMPRESS_ERR;
    }

    /*
     * Since we now have the expected size of decompressed image,
     * allocate a new (bigger) buffer which can hold decompressed
     * image.
     * Make sure to free the buffer containing incoming compressed
     * images.
     * If the required size is more than the size of given buffer then
     * bail out. Compressed file header and the file data read is not
     * matching.
     */

    /*Allocate scratch buffer which is needed to perform decompression*/
    UINT32 ScratchSizeAligned = ALIGN_UP_SZ(ScratchSize, EFI_PAGE_SIZE);
    pScratchBuffer = (UINT8*)AllocatePages(ScratchSizeAligned/EFI_PAGE_SIZE);

    /*
     * If the requested scratch buffer was non-zero but buffer provided is NULL,
     * then log an error and bail out.
     */
    if((ScratchSizeAligned != 0) && (pScratchBuffer == NULL))
    {
        PIL_ERROR("Decompression scratchBuffer allocation failed.\n");
        return PIL_OUT_OF_MEMORY;
    }
    /*
     * Get aligned size for the destination(decompressed folder).
     * This size is provided by LzmaUefiDecompressGetInfo().
     */
    ElfDecompressedSizeAligned = ALIGN_UP_SZ(DestinationSize, EFI_PAGE_SIZE);
    /*
     * Allocate buffers to store decompressed images. These would be
     * bigger than the buffer presetly defined in ElfBuffer.
     * Currently ElfBuffer contains the compressed file.
     */
    ElfDecompressedBuffer = (UINT8*)AllocatePages(ElfDecompressedSizeAligned/EFI_PAGE_SIZE);
    ElfDecompressedSize = DestinationSize;

    /*
     * If the requested ElfDecompressedBuffer was non-zero but buffer 
     * provided is NULL,then log an error and bail out.
     */
    if((ElfDecompressedSizeAligned != 0) && (ElfDecompressedBuffer == NULL))
    {
        PIL_ERROR("Decompression ElfBuffer allocation failed.\n");
        /*
         * Free up scratch buffer as that was allocated successfully earlier.
         */
        FreePages(pScratchBuffer, (ScratchSizeAligned/EFI_PAGE_SIZE));
        return PIL_OUT_OF_MEMORY;
    }

    /*
     * Perform decompression using the scratch and orginal buffers.
     */
    DecompressReturnStatus = LzmaUefiDecompress(*ElfBuffer, *ElfSize, ElfDecompressedBuffer, pScratchBuffer);
    if(DecompressReturnStatus != RETURN_SUCCESS)
    {
        PIL_ERROR("LzmaUefiDecompress failed with error 0x%x\n",DecompressReturnStatus);
        /*
         * Free buffers which were successfuly allocated.
         */
        FreePages(pScratchBuffer, (ScratchSizeAligned/EFI_PAGE_SIZE));
        FreePages(ElfDecompressedBuffer, (ElfDecompressedSizeAligned/EFI_PAGE_SIZE));
        return PIL_DECOMPRESS_ERR;
    }

    /*
     * Decompression is successful.
     * Free buffers used for decompression and the
     * the original buffer containing incoming
     * compressed image via FreePages().
     */
    FreePages(pScratchBuffer, (ScratchSizeAligned/EFI_PAGE_SIZE));
    FreePages(*ElfBuffer, (ElfSizeAligned/EFI_PAGE_SIZE));
    DecompressRatio = DestinationSize/(*ElfSize);
    /*
     * Store the decompressed buffer address and size in the incoming
     * parameters from caller i.e. update ElfBuffer with decompressed
     * buffer address.
     */
    *ElfBuffer = ElfDecompressedBuffer;
    *ElfSize  = ElfDecompressedSize;
    PIL_INFO("PilDecompressElfFile was successful. ElfBuffer=0x%x  ElfSize=0x%x DecompressRatio =%d\%\n",
              *ElfBuffer,*ElfSize, DecompressRatio);
    DecompressEndTime = GetPerformanceCounter();
    DecompressProcSpentCnt = DecompressEndTime - DecompressStartTime;
    return PIL_SUCCESS;
}
...
PIL_STATUS PilLoadElfFile(PIL_SUBSYS_CTX_TYPE* Ctx)
{
    ...
    UINT32      ElfSizeAligned = 0;
    EFI_GUID*   RootGuid;
    EFI_GUID*   PartiGuid;
    CHAR16*     PartiLabel, *LabelStr;
    UINT64      StorageStart, StorageEnd;
    PIL_STATUS  DecompressionStatus = PIL_GENERIC_ERR;

    ASSERT(Ctx != NULL);
    ASSERT_FAIL_PIL_RETURN(Ctx != NULL, PIL_INVALID_PARAM, PIL_INVALID_PARAM);
    ASSERT(NULL != Ctx->Priv);
    Priv = Ctx->Priv;
    ASSERT_FAIL_PIL_RETURN(Priv != NULL, PIL_INVALID_PARAM, PIL_INVALID_PARAM);
    ASSERT(NULL != Ctx->Cfg);
    Cfg = Ctx->Cfg;
    ...
    //If ImagePath is null for ELF_SINGLE treat it as raw file and load from partition
    if ( (Cfg->Type == ELF_FV) ||
         ((Cfg->Type == ELF_SINGLE) && (Cfg->ImagePath[0] == 0))
      )
    {
        ...
        if((Cfg->Version >= PIL_SUBSYS_CFG_VERSION_V3) && 
            !StrnCmp(Cfg->MediaType, MEDIA_TYPE_NETWORK, StrLen(MEDIA_TYPE_NETWORK)))
        {
          if(Cfg->ImagePath[0] == 0)
          {
            PIL_ERROR_CFG(Cfg, "ImagePath is required as MediaType is Network");
            return PIL_LOAD_ERR;
          }
          Status = LoadFileOverPxe(Cfg->ImagePath, &ElfSize ,(void**)&ElfBuffer);
        }
        else
        {
        ...
        if (EFI_ERROR(Status)){
            PIL_ERROR_CFG(Cfg, "Failed to read from Path\n");
            return PIL_LOAD_ERR;
        }
    }
    else if (Cfg->Type == ELF_SINGLE && (Cfg->Version >= PIL_SUBSYS_CFG_VERSION_V3) &&
              !StrnCmp(Cfg->MediaType, MEDIA_TYPE_NETWORK, StrLen(MEDIA_TYPE_NETWORK))){
    ...
    else if (Cfg->Type == ELF_SINGLE)
    {
        do {
          ...
          UnicodeSPrint(ElfFilePath, sizeof(ElfFilePath), L"%s", Cfg->ImagePath);
    ...
    /*
    * It is possible that the currently read file is in compressed
    * format. Hence Cfg->isFileCompressed flag should be checked.
    * If the file is compressed then it should be decompressed
    * before being processed further.
    */
    if((Cfg->Version >= PIL_SUBSYS_CFG_VERSION_V4) &&
       (Cfg->IsFileCompressed == TRUE))
    {
        DecompressionStatus = PilDecompressElfFile(&ElfBuffer, &ElfSize);
        if( DecompressionStatus != PIL_SUCCESS)
        {
            PIL_ERROR("Lzma decompression failed. Freeing ElfBuffer=0x%x  ElfSize=0x%x\n",ElfBuffer,ElfSize);
            /* Free Elfbuffer which containes corrupted compressed image*/
            FreePages(ElfBuffer, (ElfSizeAligned/EFI_PAGE_SIZE));
            return DecompressionStatus;
        }
    }
...
PIL_STATUS CopyMetaDataFromLoadedElf(PIL_SUBSYS_CTX_TYPE* Ctx)
{
    ...
    UINT8* MetaDataBuffer,*UncachedMetaDataBuffer;
    VOID*  ElfBuffer;
    PIL_SUBSYS_PRIV_TYPE* Priv;
    PIL_SUBSYS_CFG_TYPE* Cfg;
    PIL_STATUS PilRet;
    ...
    MetaDataBuffer = (UINT8*)AllocatePool(MetaDataSize);
    UncachedMetaDataBuffer = (UINT8*)UncachedAllocateAlignedZeroPool(MetaDataSizeAligned, SZ_4K);
    if (UncachedMetaDataBuffer==NULL || MetaDataBuffer==NULL) {
        PIL_ERROR_CFG(Cfg, "Failed to alloc metadata buffer\n");
        if (MetaDataBuffer) FreePool(MetaDataBuffer);
        if (UncachedMetaDataBuffer) UncachedFreePool(UncachedMetaDataBuffer);
        return PIL_OUT_OF_MEMORY;
    }
    ...
    Priv->MdtBufSize = MetaDataSize;
    
    /* make another instance to uncached buffer */
    CopyMem(UncachedMetaDataBuffer, MetaDataBuffer, MetaDataSize);
    Priv->UncachedMdtBuf = UncachedMetaDataBuffer;
...
PIL_STATUS PilLoadMdtFile(PIL_SUBSYS_CTX_TYPE* Ctx)
{
    ...
    UINT8*       MetaDataBuffer = NULL, *UncachedMetaDataBuffer = NULL;
    EFI_GUID*   RootGuid;
    EFI_GUID*   PartiGuid;
    CHAR16*     PartiLabel;
    UINT64      StorageStart, StorageEnd;
    ...
    if((Cfg->Version >= PIL_SUBSYS_CFG_VERSION_V3) &&
		...
		Status = LoadFileOverPxe(Cfg->ImagePath, (UINTN*)&MetaDataSize ,(void**)&MetaDataBuffer);
        ...
        MetaDataSizeAligned = ALIGN_UP_SZ(MetaDataSize, SZ_4K);
        UncachedMetaDataBuffer = (UINT8*)UncachedAllocateAlignedZeroPool(MetaDataSizeAligned, SZ_4K);
        if(UncachedMetaDataBuffer == NULL)
            return PIL_OUT_OF_MEMORY;
        ...
        MetaDataBuffer = (UINT8*)AllocatePool(MetaDataSize);
        UncachedMetaDataBuffer = (UINT8*)UncachedAllocateAlignedZeroPool(MetaDataSizeAligned, SZ_4K);
        if (NULL == MetaDataBuffer || NULL == UncachedMetaDataBuffer) {
            PIL_ERROR_CFG(Cfg, "Failed to alloc pool for MDT buffer\n");
            if (MetaDataBuffer) FreePool(MetaDataBuffer);
            if (UncachedMetaDataBuffer) UncachedFreePool(UncachedMetaDataBuffer);
            return PIL_OUT_OF_MEMORY;
        }
        ...
        if(EFI_ERROR(Status) || MetaDataSizeRead!=MetaDataSize) {
            PIL_ERROR_CFG(Cfg, "Failed to read mdt file\n");
            FreePool(MetaDataBuffer);
            UncachedFreePool(UncachedMetaDataBuffer);
            return PIL_LOAD_ERR;
        }		
    ...
    /* copy another instance of metadata to uncached buffer for tz use */
    CopyMem(UncachedMetaDataBuffer, MetaDataBuffer, MetaDataSize);
    Priv->UncachedMdtBuf = UncachedMetaDataBuffer;
...
PIL_STATUS PilLoadSegments(PIL_SUBSYS_CTX_TYPE* Ctx)
{
    ...
    ASSERT(Ehdr != NULL);
    ASSERT_FAIL_PIL_RETURN(Ehdr != NULL, PIL_INVALID_PARAM, PIL_INVALID_PARAM);
    Status = UpdateImageEntryPoint(Ehdr->e_entry);
    if(EFI_ERROR(Status))
    {
	  PIL_ERROR_CFG(Cfg, "UpdateImageEntry failed , reason: %r\n", Status);
      return PIL_GENERIC_ERR;
    }
    Phdr = (Elf32_Phdr*)((uint8*)Ehdr + Ehdr->e_phoff);
    PhdrCnt = Ehdr->e_phnum;
    Status = InitSegments(PhdrCnt);
	if(EFI_ERROR(Status))
	{
      PIL_ERROR_CFG(Cfg, "InitSegments failed , reason: %r\n", Status);
      return PIL_GENERIC_ERR;
    }
    for (i=0; i<PhdrCnt; i++)
    {
        ...
        if (Cfg->Type == ELF_SPLIT)
        {
            ...
            if((Cfg->Version >= PIL_SUBSYS_CFG_VERSION_V3) &&
                !StrnCmp(Cfg->MediaType, MEDIA_TYPE_NETWORK, StrLen(MEDIA_TYPE_NETWORK)))
            {
              Status = ReadFileOverPxe(SegName, (VOID*)SegStartPaddr, &SegSize);
            }
            else
            {            
              /* use parti guid if have one, otherwise use label */
		...
		Status = AddSegment(SegStartPaddr, p_seg->p_filesz);
		if(EFI_ERROR(Status))
		{
		  PIL_ERROR_CFG(Cfg, "AddSegment failed , reason: %r\n", Status);
		  return PIL_GENERIC_ERR;
		}
    }
...
PIL_STATUS PilAuthAndReset(PIL_SUBSYS_CTX_TYPE* Ctx)
{
    ...
    /*Ensure cache contents are flushed for the buffer being passed over to TZ*/
    WriteBackDataCacheRange((void *)(param.RegionStart), param.RegionSize);
    
    PilStatus = PilSendEvent(Ctx, PIL_EVENT_PRE_AUTH_AND_RESET, &param);
    ...
    PilStatus = PilSendEvent(Ctx, PIL_EVENT_POST_AUTH_AND_RESET, NULL);
    if (PilStatus != PIL_SUCCESS) {
...
STATIC PIL_STATUS CopyImageName(PIL_SUBSYS_CTX_TYPE* Ctx, CHAR8* Buffer)
{
    ...
    UnicodeStrToAsciiStrS(Start, Buffer, FWNAME_SHORT_LEN + 1);
    return PIL_SUCCESS;
}
...
PIL_STATUS PilPostLoad(PIL_SUBSYS_CTX_TYPE* Ctx)
{
    ...
    if (Cfg->Type == ELF_FV) {
        ...
        WriteBackInvalidateDataCacheRange ((VOID*)(Priv->RegionStart), Priv->RegionSize);
    ...
    Status = AddElfInfo(Priv->RegionStart, Priv->RegionSize);
    if(EFI_ERROR(Status))
    {
      PIL_ERROR_CFG(Cfg, "AddElfInfo failed , reason: %r\n", Status);
      return PIL_GENERIC_ERR;
    }	
    
    Status = AddImageEntry();
    if(EFI_ERROR(Status))
    {
      PIL_ERROR_CFG(Cfg, "AddImageEntry failed , reason: %r\n", Status);
      return PIL_GENERIC_ERR;
    }
...
PIL_STATUS PilCleanUp(PIL_SUBSYS_CTX_TYPE* Ctx)
{
    ...
    if (Priv->MdtBuf != NULL) {
        FreePool(Priv->MdtBuf);
        Priv->MdtBuf = NULL;
        Priv->MdtBufSize = 0;
    }
    if (NULL!=Priv->UncachedMdtBuf) {
        UncachedFreePool(Priv->UncachedMdtBuf);
        Priv->UncachedMdtBuf = NULL;
    }
...
PIL_STATUS PilShutdown(PIL_SUBSYS_CTX_TYPE* Ctx)
{
...
#define RETRY_DELAY_US     5000
    ...
    EFI_STATUS Status;
    PIL_SUBSYS_CFG_TYPE* Cfg;
    INTN RemainingRetries = MAX_RETRY_ATTEMPTS;

    ASSERT( Ctx->Cfg != NULL);
    Cfg = Ctx->Cfg;
    
    /* fv image which need to unlock post bring-out-of-reset, cannot be shutdown */
    if (Cfg->Unlock)
        return PIL_SUBSYS_NOT_SUPPORTED;
    
    do {
        /* proxy vote */
        Status = PilSendEvent(Ctx, PIL_EVENT_PRE_SHUTDOWN, NULL);
        if (EFI_ERROR(Status)) {
            PIL_ERROR_CFG(Cfg, "SendEvent PIL_EVENT_PRE_SHUTDOWN Failed: %r\n", Status);
            break;
        }
        PIL_INFO_CFG(Cfg, "Sending event before shutdown\n");
        ...
        do {
          ...
          if (EFI_ERROR(Status)) {
            ...
            PIL_INFO_CFG(Cfg, "shutdown subsys status after attempt %d: %r\n", MAX_RETRY_ATTEMPTS - RemainingRetries + 1, Status);
          }
        } while(EFI_ERROR(Status) && --RemainingRetries > 0);

        if (EFI_ERROR(Status)) {
            PIL_ERROR_CFG(Cfg, "Failed to shutdown subsys %r\n", Status);
            break;
        }
        PIL_INFO_CFG(Cfg, "Successfully unlock subsys\n");
        
        /* proxy unvote */
        Status = PilSendEvent(Ctx, PIL_EVENT_POST_SHUTDOWN, NULL);
        if (EFI_ERROR(Status)) {
            PIL_ERROR_CFG(Cfg, "SendEvent PIL_EVENT_POST_SHUTDOWN Failed: %r\n", Status);
            break;
        }
        PIL_INFO_CFG(Cfg, "Sending event after shutdown\n");
        
    } while(0);
    
    return (EFI_ERROR(Status))?PIL_SUBSYS_NOT_SUPPORTED:PIL_SUCCESS;
}
