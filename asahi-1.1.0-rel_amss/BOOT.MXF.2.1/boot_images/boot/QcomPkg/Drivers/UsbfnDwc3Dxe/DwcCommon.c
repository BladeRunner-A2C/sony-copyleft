...
/*==============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 04/22/21   pw      check Dev->CoreBaseAddr is NULL
  ...
  )
{
  ...
  if ((NULL == Dev) || (NULL == RegsToDump) || (0 == RegCount) || (0 == Dev->CoreBaseAddr)) {
  ...
  )
{
  ...
  // validate parameters
  if ((NULL == Dev) || (Ep > DWC_USB_MAX_ENDPOINT_NUMBER) || (EpDir >= DWC_EP_DIR_MAX) || (0 == Dev->CoreBaseAddr)) {
    TraceErr(CommonCommand, "invalid parameter, Dev 0x%x, Ep %d, EpDir %d", Dev, Ep, EpDir);
    return DWC_INVALID_PARAMETER;
  }
  ...
  )
{
  ...
  if ((NULL == Dev) || (NULL == Urb) || (Urb->NumTRBs == 0) ||
      (NULL == Urb->XferDesc) || (0 == Dev->CoreBaseAddr)) {
    TraceErr(CommonCommand, "invalid parameter");
    return DWC_INVALID_PARAMETER;
  }
  ...
  if ((Dev->Ep[EpIndex].State == DWC_EP_STATE_INACTIVE) ||
    ...
    if ((DWC_SUPER_SPEED != DWC_HWIO_INF(Dev->CoreBaseAddr, DSTS, CONNECTSPD)) &&
        (DWC_SUPER_SPEED_PLUS != DWC_HWIO_INF(Dev->CoreBaseAddr, DSTS, CONNECTSPD)) &&
  ...
  )
{
  ...
  // validate parameters
  if ((NULL == Dev) || (Ep > DWC_USB_MAX_ENDPOINT_NUMBER) ||
      (EpDir >= DWC_EP_DIR_MAX) || (0 == Dev->CoreBaseAddr)) {
    TraceErr(CommonCommand, "invalid parameter");
    return DWC_INVALID_PARAMETER;
  }
...
{
  // validate parameters
  if ((NULL == Dev) || (NULL == Cmd) || (0 == Dev->CoreBaseAddr)) {
    TraceErr(CommonCommand, "invalid parameter");
    return DWC_INVALID_PARAMETER;
  }
  ...
  *((UINT32 *)Evt) = *EvtBuf->NextEvt;
  ...
  // process endpoint events before returning them to upper layer
    ...
    if (((Evt->Endpoint.EventType == DWC_EVT_EP_XFER_COMPLETE) ||
      ...
      // Only check for SETUP PENDING bit for EP0 IN/OUT
      if (Urb->Ep == 0)
      {
        SetupPndg = (Trb->TRBSTS & DWC_TRB_STS_SETUP_PENDING);
        if (SetupPndg) {
          TraceErr(CommonEvent, "DWC_TRB_STS_SETUP_PENDING Pending Setup received: possible fast-forward condition");
        }
      }
