 ...
 *
 *
 * @copyright Copyright (C) 2013-2018 by QUALCOMM Technologies, Inc.  All Rights Reserved.
 */
/*==============================================================================
 EDIT HISTORY
 ...
 ******************************************************************************/
static UINT8 UsbFnIsComplainceMode (void)
{
  static UINT8 usb_compliance_mode = 0xFF;
  UINT8            UefiVar = 0;
  UINTN            UefiVarSize = 0;
  EFI_STATUS       status = EFI_SUCCESS;
    
  if (usb_compliance_mode == 0xFF)
  {
    UefiVarSize = sizeof(UefiVar);
    status = gRT->GetVariable(L"ForceUsbDeviceModeCompliance", &gQcomTokenSpaceGuid, NULL, &UefiVarSize, &UefiVar);
    if (!EFI_ERROR(status))
    {
      usb_compliance_mode = UefiVar;
      DBG(EFI_D_INFO, "read force Complaine mode %d",UefiVar);
    }
    else
    {
        DBG(EFI_D_INFO, "fail read return %d",status);
        usb_compliance_mode = 0;
    }
  }
  return usb_compliance_mode;
}
  ...
  )
{
  ...
  UINT32                                  DwcNumINEps;
  UINT32                                  DwcMaxINEpNumber; // DwcNumINEps-1
  ...
  if (Usbfn->SSDeviceInfo) {
    ...
    NextTxFifoAddr = (RegVal >> 16) + (RegVal & 0xFFFF);

    DwcNumINEps = (DWC_HWIO_IN(DwcBase, GHWPARAMS3) & HWIO_GHWPARAMS3_GHWPARAMS3_22_18_BMSK) >> HWIO_GHWPARAMS3_GHWPARAMS3_22_18_SHFT;
    DwcMaxINEpNumber = DwcNumINEps - 1;

    // Update all TX FIFOs because they are dependent on each other
    for (TxFifoIdx = 1; TxFifoIdx <= DwcMaxINEpNumber; TxFifoIdx++) {

      // use default GTXFIFOSIZ Start Address and depth
      RegVal = DWC_HWIO_INI(DwcBase,GTXFIFOSIZ,TxFifoIdx);
      NextTxFifoSize = RegVal & 0xFFFF;
  ...
  )
{
  ...
  // set U2RSTECN
  DWC_HWIO_OUTF(DwcUsb3RegBase,GCTL,U2RSTECN,0);
  ...
  Usbfn->MaxSpeed = (EFI_ERROR(Status)) ? DWC_DEV_SPEED_DEFAULT : UefiVar;

  // Default to high speed if SuperSpeed descriptors are absent
  if (((Usbfn->MaxSpeed == DWC_DEV_SPEED_SS) || (Usbfn->MaxSpeed == DWC_DEV_SPEED_SS_PLUS)) && !Usbfn->SSDeviceInfo) {
    Usbfn->MaxSpeed = DWC_DEV_SPEED_HS;
  }

  DBG (EFI_D_INFO, "Max UsbFN speed: %d", UefiVar);

  DWC_HWIO_OUTF(DwcUsb3RegBase, DCFG, DEVSPD, Usbfn->MaxSpeed);
  ...
  InterrupEnableMask |=
      (
          DWC_BMSK(DEVTEN, VENDEVTSTRCVDEN)     |
  ...
  )
{
  ...
  //
  // Don't allow device to enter U1 when operating at SuperSpeedPlus
  compliance_mode = UsbFnIsComplainceMode();
  if (FeaturePcdGet(USBU1U2Supported) && ((0 != compliance_mode) || (Usbfn->Speed != UsbBusSpeedSuperPlus))) {
    DBG(EFI_D_INFO, "U1 Enabled");
    DWC_HWIO_OUTF(Usbfn->DwcUsb3RegBase, DCTL, ACCEPTU1ENA, 1);
  }
  else {
    DBG(EFI_D_WARN, "U1 Disabled");
  }
  ...
  )
{
  ...
  if(DWC_SUPER_SPEED_PLUS == Spd){
    Usbfn->Speed = UsbBusSpeedSuperPlus;
  } else if(DWC_SUPER_SPEED == Spd){
  ...
  )
{
  ...
  CurrentState = UsmGetCurrentState();
  ...
  )
{
  ...
  case DWC_EVT_DEV_U3L2L1_SUSPEND:
    {
      // SUSPEND event in High Speed could come for L1 (SLEEP) and EARLY SUSPEND.
      // And let HW handle it automatically.
      DWC_USB_LINK_STATE LinkState = (DWC_USB_LINK_STATE)(Evt->Device.EventInfo & 0xF);
      if (LinkState == DWC_LS_SUSPEND)
      {
