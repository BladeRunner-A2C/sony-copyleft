...
/*==============================================================================
 ...
 06/20/18	gk		Added Fix for UsbFnTestApp asserting
 04/11/18   amitg   Enable SS USB by default for device mode
...
// unknown, low,    full,   high,   super,  super-plus
  ...
  )
{
  ...
  Usbfn->UsbfnIo.UpdateSSDescriptor         = UsbUpdateSSDescriptor;
  ...
  for (i = 0; i < USBFN_MESSAGE_NODE_COUNT; ++i) {
	...
	if(NewMsg){
	  ...
	  DBG (EFI_D_ERROR, "Failed to allocate memory for EFI_USBFN_MESSAGE_NODE");
	}
  }
  ...
  Status = gRT->GetVariable(L"HighSpeedOnlyForPrimary", &gQcomTokenSpaceGuid, NULL, &UefiVarSize, &UefiVar);
  if (EFI_ERROR(Status)) {
    UefiVar = 0;
  }
  ...
  )
{
  ...
  Usbfn->CoreNum = Usbfn->UsbConfigProtocol->coreNum;
  ...
  )
{
  ...
  if ( Usbfn->HsTestMode != USB_TEST_DISABLE ||
       ( Usbfn->HijackData.RequestHijackingEnabled &&
         ((Usbfn->Speed == UsbBusSpeedSuper) || (Usbfn->Speed == UsbBusSpeedSuperPlus))) ) {
    ...
    if (Req->Request == USB_REQ_GET_DESCRIPTOR) {
      ...
      if (DescType == USB_DESC_TYPE_BOS) {
        DBG(EFI_D_INFO, "Hijacking BOS Descriptor");
    ...
    if (Req->Request == USB_REQ_SET_ISOCH_DELAY) {
      DBG(EFI_D_INFO, "Hijacking SET_ISOCH_DELAY request");
    ...
    if (Req->Request == USB_REQ_SET_SEL) {
      DBG(EFI_D_INFO, "Hijacking SET_SEL request");
    ...
    if (Req->Request == USB_REQ_SET_FEATURE) {
      switch (Req->Value) {
        ...
        case USB_FEATURE_HS_TEST_MODE:
          ...
          DBG(EFI_D_INFO, "Hijacking SET_FEATURE 0x%x request", Req->Value);
  ...
  )
{
  ...
  // Disconnect first if we are already attached
  UsbfnDwcCoreShutdown(Usbfn);
  //Disabling the HijackData variable for LA Fastboot. 
  //LA fastboot already enumerates in SUperspeed. We don't need HijackData variable for that.
  if (Usbfn->HijackData.RequestHijackingEnabled)
    Usbfn->HijackData.RequestHijackingEnabled = FALSE;
  ...
  )
{
  ...
  if (NULL == This || NULL == MaxPacketSize ||
      (EndpointType > UsbEndpointInterrupt) || (BusSpeed > UsbBusSpeedMaximum)) {
    Status = EFI_INVALID_PARAMETER;
    DBG(EFI_D_ERROR, "invalid parameter");
    goto ON_EXIT;
  }
  ...
  )
{
  ...
  if (boot_from_ufs()) {
    MediaSignature = SIGNATURE_32('u','f','s',' ');
  }
  else if (boot_from_nvme()) {
    MediaSignature = SIGNATURE_32('n','v','m','e');
  }
  else if (boot_from_emmc()) {
    MediaSignature = SIGNATURE_32('e','m','m','c');
  }
  else if (boot_from_spi_nor()) {
    MediaSignature = SIGNATURE_32('s','n','o','r');
  }
  ...
  if (!CardInfo) {
    ...
    // Update buffer size to needed buffer size
    *BufferSize = NeededBuffSize;
  ...
  OUT    VOID                   *Buffer)
{
  ...
  // Copy the string.
  //CopyMem(Buffer, Str, Size);
  Size = usb_shared_get_product_str_desc_with_chip_serial(
  ...
  OUT   VOID                                ***LConfigurations)
{
  ...
  for (Index = 0; Index < ConfigurationCount; Index++) {
    TotalLen = ((EFI_USB_CONFIG_DESCRIPTOR *) CConfigurations[Index]->ConfigDescriptor)->TotalLength;
    ...
    gBS->CopyMem(Configurations[Index], CConfigurations[Index]->ConfigDescriptor, TotalLen);
  }
...
ON_EXIT:
  ...
  if (!EFI_ERROR(Status)) {
    *LConfigurations = Configurations;
    ...
    if (NULL != Configurations) {
      UsbFreeDescriptorArray(ConfigurationCount, Configurations);
...
)
{
  ...
  Usbfn = USBFN_FROM_THIS (This);
  // override client's SuperSpeed Descriptor if hijacking enabled
  if (Usbfn->HijackData.RequestHijackingEnabled) {
      // copy device descriptor
      gBS->CopyMem(&DescriptorSet->SSDeviceDescriptor, Usbfn->SSDeviceInfo->DeviceDescriptor , sizeof(EFI_USB_DEVICE_DESCRIPTOR));
      // copy usb configurations
    if (DescriptorSet->SSDeviceDescriptor.NumConfigurations > 0) {
      Status = UsbUpdateConfigurations(
        DescriptorSet->SSDeviceDescriptor.NumConfigurations,
        Usbfn->SSDeviceInfo->ConfigInfoTable,
