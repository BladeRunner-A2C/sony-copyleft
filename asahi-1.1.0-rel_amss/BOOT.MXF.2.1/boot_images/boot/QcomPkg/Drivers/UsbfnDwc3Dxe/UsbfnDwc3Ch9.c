...
/*==============================================================================
  ...
  08/22/17   gk      Fix USB3.1 device link layer test failure
  ...
  )
{
  ...
  if ((UsbBusSpeedSuper == Usbfn->Speed) || (UsbBusSpeedSuperPlus == Usbfn->Speed)) {
    ...
    for (i = 0; i < ifcCount; i++) {
      ...
      if (ifcInfo->InterfaceDescriptor->InterfaceNumber == IfcNum &&
        ...
        for (j = 0; j < epCount; j++) {
		  ...
		  UINT16 epMaxPktSize = UsbEpMaxPacketSize[epType][Usbfn->Speed];
		  
		  #ifndef REMOVE_EUD_BULK_OUT_EP_LIMIT
		  if(usb_shared_is_eud_bulk_out_ep_limit_required(epDesc->EndpointAddress, epDesc->Attributes, epMaxPktSize) == TRUE) {
		    epMaxPktSize = 128;
		  }
          #endif
          UsbfnDwcConfigEndpoints(
  ...
  )
{
  ...
  if (NewConfig != Usbfn->ConfigValue) {
    ...
    if (NewConfig != 0) { // configured
      ...
      //
      if ((Usbfn->Speed == UsbBusSpeedSuper) || (Usbfn->Speed == UsbBusSpeedSuperPlus)) {
  ...
  )
{
  ...
  if (Usbfn->UsbConfigProtocol->AlwaysConnected)
  {
  ...
  if(Usbfn->IsAttached) {
    ...
    if(!IsConnectionEstablished) {      
      ...
      if ( (Usbfn->MaxSpeed == DWC_DEV_SPEED_SS_PLUS) && (Usbfn->Speed == UsbBusSpeedUnknown) ) {
        // Handle target specific polling to allow Super Speed Plus on
        // some of Host Controllers
        Status = Usbfn->UsbConfigProtocol->PollSSPhyTraining(Usbfn->UsbConfigProtocol, Usbfn->CoreType);
        if(EFI_ERROR(Status)) {
          DBG(EFI_D_INFO,"Usbfn->UsbConfigProtocol returned PollSSPhyTraining with error: %r\r\n", Status);
        }      
      }
  ...
  )
{
  ...
  if(TRUE == IsAttached) {
    ...
    else
    {
        ...
        do {
          ...
          Status = UsbfnDwc3GetChargerPortType(Usbfn->CoreNum, &Usbfn->ChgPortType);
          if(EFI_ERROR(Status)) {
          ...
          Cnt++;
          if (Cnt >MAX_CHARGER_TYPE_READ_ATTEMPTS)
          {
            DBG(EFI_D_ERROR,"PmicSmbchgProtocol->ChargerPort() timeout: %d. Assume SDP\r", Cnt);
            //Assume always connect due to PMIC issue
            Usbfn->ChgPortType = DCI_DWC_CHG_PORT_SDP;
          }
        } while((DCI_DWC_CHG_PORT_UNKNOWN == Usbfn->ChgPortType));
    }
    ...
    switch (Usbfn->ChgPortType) {
      ...
      case DCI_DWC_CHG_PORT_CDP:
        ...
        dci_connect(Usbfn->CoreNum, Usbfn->Usb30Qscratch);
  ...
  {
    DBG(EFI_D_WARN, "Cable Detached");

    dci_disconnect (Usbfn->CoreNum, Usbfn->Usb30Qscratch);
