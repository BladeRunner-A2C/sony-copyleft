...
--------     ---     -----------------------------------------------------------
2021/06/21   sood    Fixed MISRA rules: Updated InstallSimpleFSonPartHandle() 
                     definition with void parameter
2021/03/30   kbarad  Fixed Klocwork issue
...
)
{
    ...
    if (!pInputFilePath)
    {
        DEBUG((EFI_D_ERROR, "[RawFsSimpleFsDxe] pInputFilePath is NULL.\n"));
    ...
    if (StrCmp(pInputFilePath, L"") == 0 || 
        StrCmp(pInputFilePath, L"\\") == 0 )
    {
...
static EFI_STATUS
InstallSimpleFSonPartHandle(VOID)
{
    EFI_STATUS                    status = EFI_SUCCESS;
