   ...
   )
{
    ...
    errFlag = pm_clk_sw_enable(SPMI_BUSID_0, PmicDeviceIndex, (pm_clk_type)ClkBuffType, (pm_on_off_type)Enable);
   ...
   )
{
    ...
    errFlag = pm_clk_drv_strength(SPMI_BUSID_0, PmicDeviceIndex, (pm_clk_type)ClkBuffType, (pm_clk_drv_strength_type)OutDrvStrength);
    if (PM_ERR_FLAG_SUCCESS != errFlag)
    {
        return EFI_DEVICE_ERROR;
    }
   ...
   )
{
    ...
    errFlag = pm_clk_set_out_clk_div(SPMI_BUSID_0, PmicDeviceIndex, (pm_clk_type)ClkBuffType, (uint32)Divider);
    if (PM_ERR_FLAG_SUCCESS != errFlag)
    {
        return EFI_DEVICE_ERROR;
    }
...
{
    pm_err_flag_type  errFlag  = PM_ERR_FLAG_SUCCESS;
    pm_clk_status_type clk_status = {0};
    ...
    errFlag =  pm_clk_get_status(SPMI_BUSID_0, (uint32)PmicDeviceIndex, (pm_clk_type)ClkBuffType, &clk_status);
    ...
    else
    {
        ...
        ClkStatus->ClkOk       = (BOOLEAN)(clk_status.clk_ok);
    }
