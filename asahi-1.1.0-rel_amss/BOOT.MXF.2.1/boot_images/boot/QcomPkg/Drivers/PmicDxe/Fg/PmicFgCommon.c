...
)
{
  ...
  errFlag = pm_bmd_get_battery_missing_rt_status(SPMI_BUSID_0, PmicDeviceIndex, PM_BMD_BAT_MISS_STS_RAW, &batt_miss); 
...
)
{
  ...
  *battery_type = BATTERY_1S;
  ...
  errFlag = pm_chg_get_primary_charger_pmic_index(&PmicDeviceIndex);
  if(PM_ERR_FLAG_SUCCESS == errFlag)
  {
    errFlag = pm_chg_is_2s_batt(&is_2s_battery);
    if((errFlag == PM_ERR_FLAG_SUCCESS) && is_2s_battery)
    {
