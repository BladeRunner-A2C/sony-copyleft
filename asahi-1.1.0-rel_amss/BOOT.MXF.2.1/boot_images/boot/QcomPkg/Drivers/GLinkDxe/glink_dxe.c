...
#include <Library/glink_restricted.h>
...
extern void glink_ssr_tear_down( const char *remote_name, const char *transport_name );
extern void xport_smem_init(void);
extern void xport_qmp_init(void);
extern void glink_ssr(const char*);
 ...
 * @sideeffects  None
 */
static EFI_STATUS EFI_glink_link_state_poll
(
  glink_link_handle_type handle,
...
{
  *ret = glink_link_state_poll(handle, state);
 ...
 * @sideeffects  None
 */
static EFI_STATUS EFI_glink_rx_poll
(
  glink_handle_type handle,
  glink_err_type    *ret  
)
{
  *ret = glink_rx_poll(handle);
...
*/
/*=========================================================================*/
static EFI_STATUS EFI_glink_ssr
(
...
{
  glink_ssr_tear_down("spss" , "SMEM");
  glink_ssr_tear_down("lpass" , "SMEM");
  glink_ssr_tear_down("mpss" , "SMEM");
}
...
static VOID GlinkRegisterEBSCallback(VOID)
{
  ...
  FdtNodeHandle uefi_feature_dt_node_hdl;
  UINT32  exitcb_defined=0;
  ...
  status = dtb_protocol->FdtGetNodeHandle(&uefi_feature_dt_node_hdl, "/sw/glink/uefi-feature-config");
  if (EFI_ERROR(status)) 
  {
    return;
  }
  
  status = dtb_protocol->FdtGetUint32Prop(&uefi_feature_dt_node_hdl, "exitcb-defined", &exitcb_defined);
  if (EFI_ERROR(status))
  {
    DEBUG((EFI_D_ERROR, "%s: FdtGetUint32Prop for exitcb-defined failed with error %r \n", __FUNCTION__, status));
    return;
  }
