...
Copyright (c) 2018 - 2023 Qualcomm Technologies, Inc. All rights
reserved.
...
--------   ---     -----------------------------------------------------------
12/08/22   dnkg    Fixing KW issues
11/18/22   dnkg    Removing unused variables and fixing log format
12/10/21   dnkg    Bug Fixing in PrepareAddFileWriteBlocks function
...
)
{
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] One or more arguments is invalid.\n", __FUNCTION__));
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] pReadFileName is empty!\n", __FUNCTION__));
        goto Exit;
    }

    DEBUG((EFI_D_INFO, "[%a] folderName: %s\n", __FUNCTION__, folderName));
    DEBUG((EFI_D_INFO, "[%a] fileName: %s\n", __FUNCTION__, fileName));
    DEBUG((EFI_D_INFO, "[%a] pPartitionGuid: 0x%x\n", __FUNCTION__, pPartitionGuid[0]));
    ...
    if (EFI_ERROR(status))
    {
        ...
        {
            DEBUG((EFI_D_ERROR, "[%a] Partition is corrupted. Returning: %r\n", __FUNCTION__, status));
        }
        else if (status == EFI_NOT_FOUND)
        {
            DEBUG((EFI_D_ERROR, "[%a] Partition not formated. Returning: %r\n", __FUNCTION__, status));
        }
        else
        {
            DEBUG((EFI_D_ERROR, "[%a] Retrieve filesystem header failed. Returning: %r\n", __FUNCTION__, status));
        }
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] RetrieveFileSystemHeaders failed.\n", __FUNCTION__));
        goto Exit;
    }
    ...
    if (foundFile == FALSE)
    {
        DEBUG((EFI_D_WARN, "[%a] FileRead File with name %s not found!\n", __FUNCTION__, fileName));
    ...
    else
    {
        ...
        {
            DEBUG((EFI_D_WARN, "[%a] File does not fit in the given buffer!\n", __FUNCTION__));
        ...
        if (EFI_ERROR(status))
        {
            DEBUG((EFI_D_ERROR, "[%a] FileRead failed. Returning: %r\n", __FUNCTION__, status));
            goto Exit;
        }
...
Exit:
    DEBUG((EFI_D_INFO, "[%a] end. Returning: %r", __FUNCTION__, status));
    return status;
}
...
)
{
    ...
    if (EFI_ERROR(status))
    {
        ...
        {
            DEBUG((EFI_D_ERROR, "[%a] Retrieve filesystem headers failed. Returning: %r\n", __FUNCTION__, status));
        }
...
)
{
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] One or more arguments are invalid.\n", __FUNCTION__));
        status = EFI_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] AllocateZeroPool error\n", __FUNCTION__));
    ...
    { 
        ...
        DEBUG((EFI_D_ERROR, "[%a] Not enough space in the partition!\r\n", __FUNCTION__));
        goto Exit;
    }
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] Write block data failed.\n", __FUNCTION__));
        goto Exit;
    }
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] Flush blocks failed.\n", __FUNCTION__));
        goto Exit;
    }

    if (VerifyRawFileSystemSignature(RawFSBlockIo) == FALSE)
    {
        DEBUG((EFI_D_ERROR, "[%a] VerifyRawFileSystemSignature failed.\n", __FUNCTION__));
...
)
{
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] File name was not retrieved correctly!\n", __FUNCTION__));
        status = EFI_NOT_FOUND;
        goto Exit;
    }
    ...
    {
        DEBUG((EFI_D_WARN, "[%a] Using existing empty entry for writing current file\r\n", __FUNCTION__));
        status = EFI_SUCCESS;
    }
    else if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] GetNewFilePlacementInformation failed. Returning: %r\n", __FUNCTION__, status));
        goto Exit;
    }
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] PrepareFileSystemBlocks failed. Returning: %r\n", __FUNCTION__, status));
        goto Exit;
    }
Exit:
    return status;
...
)
{
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] File System Pointer is NULL!\n", __FUNCTION__));
        goto Exit;
    }
    ...
    if (!pFSHeader || !pDirHeader)
    {
        if (!pCombinedFSHeaders)
        {
            DEBUG((EFI_D_ERROR, "[%a] FS/DIR Header is NULL!\n", __FUNCTION__));
        }
        goto Exit;
    }
    ...
    if (!pFileEntries)
    {
        if (!pCombinedFSHeaders)
        {
            DEBUG((EFI_D_ERROR, "[%a] File Entry Header is NULL!\n", __FUNCTION__));
        }
        goto Exit;
    }
    ...
    DEBUG((EFI_D_INFO, "[%a] numFiles = %d, allocatedSpaceInUse = %d\n", __FUNCTION__, numFiles, allocatedSpaceInUse));
...
)
{
    ...
    {
        DEBUG((EFI_D_INFO, "[%a] gCachedFSHeader.partitionGuid: 0x%x\n", __FUNCTION__, gCachedFSHeader.partitionGuid));
        DEBUG((EFI_D_INFO, "[%a] gCachedFSHeader.fsHeader: 0x%x\n", __FUNCTION__, gCachedFSHeader.fsHeader));
    }
        ...
        (CompareGuid(pPartitionGuid, &gCachedFSHeader.partitionGuid) != 1))
    {
        ...
        *ppFSHeader = NULL;
        ...
        if (EFI_ERROR(status))
        {
            DEBUG((EFI_D_ERROR, "[%a] ReadBlocksFromFileSystem failed. Returning: %r\n", __FUNCTION__, status));
            goto Exit;
        }
        ...
        *ppFSHeader = (PFS_HEADER)combinedHeaderBlocks.pBlockData;
        ...
        if (*ppFSHeader == NULL)
        {
            DEBUG((EFI_D_ERROR, "[%a] FS_HEADER is null.\n", __FUNCTION__));
            status = EFI_NOT_FOUND;
            goto Exit;
        }
        ...
        if ((*ppFSHeader)->Signature != (UINT32)RAWFS_SIGNATURE)
        {
            ...
            DEBUG((EFI_D_ERROR, "[%a] Signature is corrupted! Returning: %r\n", __FUNCTION__, status));
            goto Exit;
        }
        ...
        {
            status = EFI_VOLUME_CORRUPTED;
            DEBUG((EFI_D_ERROR, "[%a] RawFS blocksize is corrupted! Returning: %r\n", __FUNCTION__, status));
            goto Exit;
        }
    ...
    *ppFileEntries = (PFILE_ENTRY)(*ppCombinedHeaders + (*ppDirHeader)->FileEntriesStartOffset);
    ...
    DEBUG((EFI_D_INFO, "[%a] *ppFSHeader = 0x%x\n", __FUNCTION__, *ppFSHeader));
    DEBUG((EFI_D_INFO, "[%a] *ppDirHeader = 0x%x\n", __FUNCTION__, *ppDirHeader));
    DEBUG((EFI_D_INFO, "[%a] *ppFileEntries = 0x%x\n", __FUNCTION__, *ppFileEntries));
...
)
{
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] One or more agruments is invalid.\n", __FUNCTION__));
        status = EFI_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    if (EFI_ERROR(status))
    {
        if (status == EFI_VOLUME_CORRUPTED)
        {
            DEBUG((EFI_D_ERROR, "[%a] Partition is corrupted. Returning: %r", __FUNCTION__, status));
        }
        else
        {
            DEBUG((EFI_D_ERROR, "[%a] Retrieve file system headers failed. Returning: %r", __FUNCTION__, status));
        }
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] Retrieved File System Header is NULL!\n", __FUNCTION__));
        status = EFI_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] No more files can be added to the filesystem.\n", __FUNCTION__));
        status = EFI_UNSUPPORTED;
        goto Exit;
    }
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] Retrieved input file buffer is NULL!\n", __FUNCTION__));
        status = EFI_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] PrepareAddFileWriteBlocks failed. Returning: %r\n", __FUNCTION__, status));
        goto Exit;
    }
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] FlushFSBuffer failed. Returning: %r\n", __FUNCTION__, status));
        goto Exit;
    }
...
)
{
    ...
    */
    ...
    if (fileSystemSize == 0)
    {
        DEBUG((EFI_D_ERROR, "[%a] Filesystem size is 0!\n", __FUNCTION__));
        goto Exit;
    }
    ...
    DEBUG((EFI_D_INFO, "[%a] Blocksize to read: %ld bytes.\n", __FUNCTION__, alignedSz));
    ...
    DEBUG((EFI_D_INFO, "[%a] Start time: %ld us.\n", __FUNCTION__, startTime));
    DEBUG((EFI_D_INFO, "[%a] End time: %ld us.\n", __FUNCTION__, endTime));
    DEBUG((EFI_D_INFO, "[%a] Time diff: %ld us.\n", __FUNCTION__, endTime-startTime));

    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] Failed to read block data from partition.\n", __FUNCTION__));
        goto Exit;
    }
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] Failed to flush blocks.\n", __FUNCTION__));
        goto Exit;
    }
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] Failed to write block data to partition.\n", __FUNCTION__));
        goto Exit;
    }
...
)
{
    ...
    if (0 == pFSBlocks->StartBlock)
    {
        ...
        if (fileSystemSize == 0)
        {
            DEBUG((EFI_D_WARN, "[%a] Filesystem size is 0!\n", __FUNCTION__));
            goto Exit;
        }
    }
    ...
    IN OUT UINT32 *pDestFileNameLen)
{
    ...
    *pDestFileNameLen = 0;
    destPathLen = (UINT32)StrLen(destPath);
    ...
    for (UINT16 charIdx = 0; charIdx < destPathLen; charIdx++)
    {
        if (destPath[charIdx] == L'\\')
        {
            if (slashIdx != 0)
            {
                ...
                DEBUG((EFI_D_ERROR, "[%a] File path has more than one set of backslash! Will bail out. Returning: %r\n", __FUNCTION__, status));
                goto Exit;
            }
    ...
    if ((destPathLen - slashIdx) >= MAX_FILE_NAME_LENGTH)
    {
        ...
        DEBUG((EFI_D_ERROR, "[%a] File name is too long! Length: %u\n", __FUNCTION__, destPathLen - slashIdx));
        goto Exit;
    }
    ...
    {
        // Fail due to length overflow
        DEBUG((EFI_D_ERROR, "[%a] Folder name is too long! Length: %u\n", __FUNCTION__, slashIdx + 1));
        goto Exit;
    }
    ...
    UINT32 *pFileIdx)
{
    ...
    if (fileNameLen == 0 && folderNameLen == 0)
    {
        DEBUG((EFI_D_ERROR, "[%a] SplitFilePath pDestFileName is empty!\n", __FUNCTION__));
        goto Exit;
    }
    ...
    if (!pFSHeader || !pDirHeader)
    {
        DEBUG((EFI_D_ERROR, "[%a] File System offsets don't exist!\n", __FUNCTION__));
        goto Exit;
    }
...
)
{
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] One or more input parameters is null!\n", __FUNCTION__));
        status = EFI_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] PrepareFileSystemBlocks Failed with status: %r", __FUNCTION__, status));
        goto Exit;
    }
...
)
{
    ...
    if (!pCombinedFSHeaders)
    {
        DEBUG((EFI_D_ERROR, "[%a] File System Buffer is NULL!\n", __FUNCTION__));
        goto Exit;
    }
...
)
{
    ...
    if (EFI_ERROR(status))
    {
        ...
        {
            DEBUG((EFI_D_ERROR, "[%a] Retrieve file system headers failed. Returning %r\n", __FUNCTION__, status));
        }
    ...
    if (!pCombinedFSHeaders)
    {
        DEBUG((EFI_D_ERROR, "[%a] File System header is NULL!\n", __FUNCTION__));
        status = EFI_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] File does not exist in file system!\n", __FUNCTION__));
        status = EFI_NOT_FOUND;
        goto Exit;
    }
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] PrepareDeleteFileWriteBlocks failed. Returning %r\n", __FUNCTION__, status));
        goto Exit;
    }
...
)
{
    ...
    if (FileExistsInFileSystem(pCombinedFSHeaders, pNewFileInfo->pFSFilePath, &pNewFileInfo->FileEntryIdx) == TRUE)
    {
        ...
        else
        {
            ...
            */
            ...
            if (EFI_ERROR(status))
            {
                DEBUG((EFI_D_ERROR, "[%a] DeleteFileRawFileSystem failed. Returning %r\n", __FUNCTION__, status));
                goto Exit;
            }
            ...
            {
                status = EFI_UNSUPPORTED;
                DEBUG((EFI_D_ERROR, "[%a] ReadBlocksFromFileSystem produced null pBlockData\n", __FUNCTION__));
                goto Exit;
            }
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] GetFirstFreeFileEntry failed. Returning %r\n", __FUNCTION__, status));
        goto Exit;
    }
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] AllocateFromFreeContiguousMemory failed. Returning %r\n", __FUNCTION__, status));
        goto Exit;
    }
...
)
{
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] AllocateFromFreeContiguousMemory:File System Pointer is NULL!\n", __FUNCTION__));
        goto Exit;
    }
    ...
    {
        DEBUG((EFI_D_ERROR, "[%a] AllocateFromFreeContiguousMemory:File system does not have enough contiguous memory left\n", __FUNCTION__));
        status = EFI_OUT_OF_RESOURCES;
        goto Exit;
    }
...
)
{
    ...
    {
      DEBUG((EFI_D_ERROR, "[%a] One or more passed parameters is NULL\n", __FUNCTION__));
        status = EFI_INVALID_PARAMETER;
        goto Exit;
    }
...
)
{
    ...
    if (EFI_ERROR(status))
    {
        ...
        {
            DEBUG((EFI_D_ERROR, "[%a] Partition is not formated. Returning: %r", __FUNCTION__, status));
        }
        else
        {
            DEBUG((EFI_D_ERROR, "[%a] Retrieve file system headers failed. Returning: %r", __FUNCTION__, status));
        ...
        if (StriCmp(pFileEntries[fileEntryIdx].FolderName, pDirectoryName) == 0)
        {
            ...
            if (NULL == pFiletoRead || (inputFileidx != -1))
            {
                *pNextFile = AllocateZeroPool(StrSize(pFileEntries[fileEntryIdx].FileName));
                if (*pNextFile == NULL)
                {
                    DEBUG((EFI_D_WARN, "[%a] AllocateZeroPool error.\n", __FUNCTION__));
                    status = EFI_OUT_OF_RESOURCES;
                    goto GetNextFileNameInRawFSDirectoryExit;
                }
                CopyMem(*pNextFile,
                ...
                DEBUG((EFI_D_INFO, "[%a] NextFile: %s.\n", __FUNCTION__, *pNextFile));
                break;
            }
    ...
    if (isDirFound == FALSE)
    {
        DEBUG((EFI_D_WARN, "[%a] Directory with name %s not found!\n", __FUNCTION__, pDirectoryName));
        status = EFI_NOT_FOUND;
        goto GetNextFileNameInRawFSDirectoryExit;
    }
...
)
{
    ...
    DEBUG((EFI_D_INFO, "[%a] Fileread count: %d\n", __FUNCTION__, ++fReadCount));
...
)
{
    ...
    DEBUG((EFI_D_INFO, "[%a] FileWrite count = %d\n", __FUNCTION__, ++fWriteCount));
...
)
{
    ...
    DEBUG((EFI_D_INFO, "[%a] FileDelete count = %d\n", __FUNCTION__, ++fDelCount));
...
)
{
    ...
    if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "[%a] Failed to Install EfiRawFileSystem protocol \r\n", __FUNCTION__));
    }
