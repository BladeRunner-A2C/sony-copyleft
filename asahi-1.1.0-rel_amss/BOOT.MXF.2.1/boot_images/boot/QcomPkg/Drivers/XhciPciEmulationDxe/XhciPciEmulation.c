...
Copyright (c) 2010 - 2012, 2014-2015, 2017, 2021-2022 Qualcomm Technologies, Inc.	All Rights Reserved.
...
/*=============================================================================
 ...
 --------   ---      ---------------------------------------------------------
 06/16/2021 jay      Made changes to resolve misra issues
  ...
  )
{
  ...
  return EFI_UNSUPPORTED;        // added to resolve misra issue
}
  ...
  )
{
  ...
  //Check the resource, since it is optional
  if (Resources != NULL) {
    BarDesc = AllocateZeroPool (sizeof (EFI_ACPI_ADDRESS_SPACE_DESCRIPTOR));
    if (BarDesc == NULL) {
      DBG_PRINT(EFI_D_ERROR,"PciIoGetBarAttributes, Out of memory"); 
      return EFI_OUT_OF_RESOURCES;
    }
    BarDesc->ResType = ACPI_ADDRESS_SPACE_TYPE_MEM;
    *Resources = BarDesc;
    return EFI_SUCCESS;
  }
  return EFI_UNSUPPORTED;  
...
{
  //USB_ASSERT_RETURN (FALSE,EFI_UNSUPPORTED);
  DBG_PRINT(EFI_D_ERROR,"set bar not support idx %d", BarIndex); 
  return EFI_UNSUPPORTED;
}
  ...
  )
{
  ...
  /* 
   * don't free Private as it contains the functions supported by pciio protocol.
   * These functions (eg: Pci.Read) are used in XhcDriverBindingSupported, and would cause abort 
   * when accessed after freeing.
   */
  ...
  )
{
  ...
  {
    DEBUG((EFI_D_ERROR, "XhcPciEmulationDriverBindingStop: Error: PciIO == NULL\n"));
  }
