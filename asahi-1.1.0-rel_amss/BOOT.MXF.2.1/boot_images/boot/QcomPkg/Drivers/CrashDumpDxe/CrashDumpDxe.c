/** @file CrashDumpDxe.c
  ...
  Copyright (c) 2012-2016,2021,2023 Qualcomm Technologies, Inc. 
  All rights reserved.
...
/*=============================================================================
  ...
  --------    ---       ----------------------------------------------------------
  07/28/2021  ka        Added AddShmBridgeRegion
...
#include <Library/DbiDumpVariantLib.h>
									   ...
									   )
{
  ...
  AsciiStrCpyS(buffer_address, buffer_sz, pmic_register_name_string);    
...
InitializeResetDataBuffers ( EFI_OFFLINE_CRASHDUMP_CONFIG_TABLE *mOfflineCrashDumpConfigTable )
{
  ...
  const CHAR8 NormalBoot[]           = "Normal Boot=";
  ...
  Status = GetMemRegionInfoByName("Reset_Data", &ResetDataRegionInfo);
  if (Status != EFI_SUCCESS)
  {
  ...
  /* Pass a NULL pointer to prevent tzbsp_dump_tz_diag_for_hlos from performing 
   * a memscpy after an abnormal reset with crash dump disabled through UEFI menu. */
  TZResetDiagDataBufferAddressSysCall(NULL, QCDATA_1_SIZE);
  ...
  {
	  AsciiStrCpyS((CHAR8 *)tzBufferAddress, AsciiStrLen(NormalBoot)+1, NormalBoot);  
  }
  ...
  *((UINT64*)UefiResetBufferAddress) = RESET_DATA2_VERSION;
  ...
  AsciiStrCpyS(UefiResetBufferAddress, UefiResetDataSize - sizeof(UINT64), GccStr);
  ...
  )
{
   ...
   // SetupDBIDumpTable is assumed to have already been called in SEC and dump tables setup
   // Now only do remaining APREG SHM bridge creation
   //AddShmBridgeRegion();
   HandleCrashDump();
