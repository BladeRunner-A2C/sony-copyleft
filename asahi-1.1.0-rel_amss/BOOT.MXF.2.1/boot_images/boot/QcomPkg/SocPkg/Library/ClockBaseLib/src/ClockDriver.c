...
/*
 * Default XO Frequency
 */
#define DEFAULT_FREQ  19200  

/*===========================================================================
                      FUNCTION PROTOTYPES
===========================================================================*/
static const HAL_clk_SourceRegSettingsType *Clock_GetSourceRegSettings(ClockSourceNodeType *pSource);
...
)
{
  ...
  HAL_clk_SourceDescType nHALSource;
   ...
   */
  if (HAL_clk_IsSourceEnabled(&nHALSource))
  {
  ...
  if(pRegSettings)
  {
    HAL_clk_SetSourceRegSettings(&nHALSource, pRegSettings);
  }

  /* Enable any outputs for this PLL */
  HAL_clk_ConfigSource(&nHALSource, HAL_CLK_CONFIG_PLL_MAIN_OUTPUT_ENABLE);
  if (pSource->nConfigMask & CLOCK_CONFIG_PLL_EVEN_OUTPUT_ENABLE)
  {
    HAL_clk_ConfigSource(&nHALSource, HAL_CLK_CONFIG_PLL_EVEN_OUTPUT_ENABLE);
  }
  if (pSource->nConfigMask & CLOCK_CONFIG_PLL_ODD_OUTPUT_ENABLE)
  {
    HAL_clk_ConfigSource(&nHALSource, HAL_CLK_CONFIG_PLL_ODD_OUTPUT_ENABLE);
  }
  if (pSource->nConfigMask & CLOCK_CONFIG_PLL_EARLY_OUTPUT_ENABLE)
  {
    HAL_clk_ConfigSource(&nHALSource, HAL_CLK_CONFIG_PLL_EARLY_OUTPUT_ENABLE);
  }
  if (pSource->nConfigMask & CLOCK_CONFIG_PLL_MAIN_OUTPUT_HW_CTL)
  {
    HAL_clk_ConfigSource(&nHALSource, HAL_CLK_CONFIG_PLL_MAIN_OUTPUT_HW_CTL_ENABLE);
  }
  if (pSource->nConfigMask & CLOCK_CONFIG_PLL_EVEN_OUTPUT_HW_CTL)
  {
    HAL_clk_ConfigSource(&nHALSource, HAL_CLK_CONFIG_PLL_EVEN_OUTPUT_HW_CTL_ENABLE);
  }
  if (pSource->nConfigMask & CLOCK_CONFIG_PLL_ODD_OUTPUT_HW_CTL)
  {
    HAL_clk_ConfigSource(&nHALSource, HAL_CLK_CONFIG_PLL_ODD_OUTPUT_HW_CTL_ENABLE);
  }
  ...
  if (nVoteAddr != 0)
  {
    HAL_clk_ConfigSource(&nHALSource, HAL_CLK_CONFIG_PLL_FSM_MODE_ENABLE);
  }
  ...
  if(pCalConfig)
  {
    HAL_clk_SetSourceCalConfig(&nHALSource, &pCalConfig->HALConfig);
  }

  HAL_clk_ConfigPLL(&nHALSource, pHALConfig, HAL_CLK_SOURCE_CONFIG_MODE_NORMAL);
...
)
{
  ...
  if (nVoteAddr != 0)
  {
    if (!HAL_clk_EnableSourceVote(&nHAL))
    {
      return FALSE;
    }
  }
...
static const HAL_clk_SourceRegSettingsType *Clock_GetSourceRegSettings
(
  ClockSourceNodeType *pSource
)
{
...
static bool Clock_ConfigMux
(
  ClockDomainNodeType *pDomain, 
  const HAL_clk_ClockMuxConfigType *pConfig
)
{
  uintptr_t nCfgRCGRAddr, nCmdRCGRAddr;
  uint32_t nCmdRCGRVal,  nCfgRCGRVal, nDVal;
  ...
  /* Basic Mux */
  if(pDomain->nFlags & CLOCK_FLAG_DOMAIN_BASIC_MUX)
  {
    outpdw((uintptr_t)(pDomain->nCGRAddr), pConfig->nMuxSel);
    return TRUE;
  } 
  ...
  if (pConfig->nM != 0 && (pConfig->nM < pConfig->nN))
  {
    ...
    nDVal = pConfig->nN;
    if(nDVal > ((pConfig->nN - pConfig->nM) * 2))
      nDVal = (pConfig->nN - pConfig->nM) * 2;
    else if (nDVal < pConfig->nM)
      nDVal = pConfig->nM;
...
)
{
  ...
  Clock_ConfigMux(pDomain, &pConfig->HALConfig);
...
)
{
  ...
  nGDSCRAddr = pPowerDomain->nGDSCRAddr;
  nVal  = inpdw(nGDSCRAddr);
...
)
{
  ...
  for (i = 0; i < ClockBSP.nNumAliases; i++)
  {
    if (!strcmp(ClockBSP.aAliases->szFrom, szName))
    {
      szName = ClockBSP.aAliases->szTo;
      break;
    }
  }
...
)
{
  ...
  switch (CLOCK_GET_ID_CATEGORY(ClockId))
  {
    ...
    case CLOCK_ID_CATEGORY_CLOCK:
      ...
      /* 
       * Skip setting the frequency only when there is no
       * no domain pointer and the default 19.2MHz freq 
       * is requested. It is expected that clocks without
       * domain pointers only support one frequency and
       * are already defaulted to 19.2MHz.
       */   
      if (pClock->pDomain == NULL && nFreq == DEFAULT_FREQ)  
      {
        return CLOCK_SUCCESS;
      }
...
)
{
  ...
  Clock_ConfigMux(pClock->pDomain, &muxConfig);
...
)
{
  ...
  Clock_ConfigDivider(pDivider->nAddr, nDivider);
