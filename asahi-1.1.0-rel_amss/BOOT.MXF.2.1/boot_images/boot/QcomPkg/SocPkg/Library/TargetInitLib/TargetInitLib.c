/** @file TargetInitLib.c
  ...
  Copyright (c) 2013,2017 - 2018,2020 by Qualcomm Technologies, Inc. All Rights Reserved.
...
TargetConfigType SOCConfigParams = 
{
  ...
  .MpidrToCoreNumBits = 16,

  /* How many bits to Consider for masking in order to get the cluster# from MPIDR 
   * register value Check MPIDR registers for 2 aux cores to determine this
   * Usually affinity2 or affinity 3 */
  .MpidrToClustNumBits = 24,
  ...
  .MpidrToCoreNumShift = 8,

  /* How many bits to shift to get the cluster number from MPIDR register */
  .MpidrToClustNumShift = 16,
  /*MPIDR to MT Bit Shift*/
  .MpidrToMtBitShift = 24,
  .CoresPerCluster = FixedPcdGet64(PcdCorePerCluster), 
  .MaxCoreCnt = FixedPcdGet64(PcdMaxCoreCnt),                                    
  ...
  /* GIC params */
  .GicDistributorOffset = 0x17100000,     /* APSS_GICD_CTLR */
  .GicRedistributorOffset = 0x17180000,   /* APSS_ALIAS0_GICR_CTLR */
  .GicRedistributorStride  = 0x40000,

  .TimerFrameBaseAddr  = 0x17422000,      /* APSS_QTMR0_F0V2_QTMR_V2_CNTPCT_LO */
  .TimerFrameIntrNum  = 38,               /* QTMR_qgicFrm0VirtIrq[0] */
}; 
...
GetTargetBootCore (VOID)
{
  ...
  /* Aff1 shows the bootcore on current Wildcat arch */
  return((Mpidr & (MPIDR_AFFINITY_MASK))>> AFF1_BIT_SHIFT);
}
