...
DESCRIPTION: driver for the Atheros-based QSPI (quad SPI)
                       ...
                       Copyright (c) 2015, 2017 - 2018, 2020, 2022, 2023 Qualcomm Technologies, Incorporated
                                        All Rights Reserved
...
#include <Library/ArmLib.h>                 // ArmDataSynchronizationBarrier() 
#include <Library/CacheMaintenanceLib.h>    // *InvalidateDataCacheRange
...
#define QUAD_SPI_CLK        127
#define QUAD_SPI_CS_0       132
#define QUAD_SPI_CS_1       133
#define QUAD_SPI_DATA_0     128
#define QUAD_SPI_DATA_1     129
#define QUAD_SPI_DATA_2     130
#define QUAD_SPI_DATA_3     131
...
#define RD_FIFO                 0x50
#define SAMPLING_CLK_CONFIG     0x90
...
// SAMPLING_CLK_CONFIG register 
#define LANE0_CLK_CNTR_INIT_VAL_MASK  0x07
#define LANE0_CLK_CNTR_INIT_VAL_SHIFT 0x00
#define LANE1_CLK_CNTR_INIT_VAL_MASK  0x07
#define LANE1_CLK_CNTR_INIT_VAL_SHIFT 0x03
#define LANE2_CLK_CNTR_INIT_VAL_MASK  0x07
#define LANE2_CLK_CNTR_INIT_VAL_SHIFT 0x06
#define LANE3_CLK_CNTR_INIT_VAL_MASK  0x07
#define LANE3_CLK_CNTR_INIT_VAL_SHIFT 0x09
#define LANE4_CLK_CNTR_INIT_VAL_MASK  0x07
#define LANE4_CLK_CNTR_INIT_VAL_SHIFT 0x0F
#define LANE5_CLK_CNTR_INIT_VAL_MASK  0x07
#define LANE5_CLK_CNTR_INIT_VAL_SHIFT 0x12
#define LANE6_CLK_CNTR_INIT_VAL_MASK  0x07
#define LANE6_CLK_CNTR_INIT_VAL_SHIFT 0x15
#define LANE7_CLK_CNTR_INIT_VAL_MASK  0x07
#define LANE7_CLK_CNTR_INIT_VAL_SHIFT 0x18
...
//static QSPI_LANE_SAMPLING_CONFIG sequence;
...
static bool dlp_sequence_enabled = false;

static uint8_t *qspi_base = (void *)0x088DC000;

static uint32_t mstr_config_ddr = 0;
/*==================================================================================================
                                     LOCAL FUNCTION PROTOTYPES
==================================================================================================*/
static bool 			  set_qspi_frequency(uint32_t input_freq);
...
static void               reg_update_ram_mode(QSPI_RAM_MODES mode);
...
static bool set_qspi_frequency(uint32_t input_freq)
{
  ...
  res = Clock_GetId(clock_handle, "gcc_qspi_cnoc_periph_ahb_clk", &qup_id);
  if (res != CLOCK_SUCCESS) { 
    return FALSE;
  }
  ...
  res = Clock_GetId(clock_handle, "gcc_qspi_core_clk", &qup_id);
  if (res != CLOCK_SUCCESS) { 
    return FALSE;
  }
...
static DESCRIPTOR *allocate_descriptor(uint32_t bytes_needed, bool dma_mode)
{
   ...
   {
      if (dma_mode) {
         uint32_t len = (current - dma_chain + 1) * sizeof(DESCRIPTOR);
         WriteBackInvalidateDataCacheRange(dma_chain, len);
      }
   ...
   next->data_address    = bytes_needed ? (uint64_t)free_ptr : 0;
   ...
   if (current) { current->next_descriptor = (uint64_t)next; }
   else         { dma_chain = next; }
...
static uint32_t available_room(DESCRIPTOR *last)
{
   ...
   return (uint64_t)(end_of_buffer - (last->data_address + last->length));
}
...
static bool configure_clocks(QSPI_MASTER_CONFIG *cfg)
{
  if (get_platform_type() == QSPI_PLATFORM_RUMI)
  {
    ...
    uint8_t *gcc_qspi_base = (void *)0x0014B000;

    #define AHB_CBCR     0x4
    #define SER_CBCR     0x8
    #define SER_CMD_RCGR 0xC
  ...
  else
  {
    ...
    *(volatile uint32_t *)(gcc_qspi_base + SER_CMD_RCGR) = 0x00000003;     // ROOT_EN and UPDATE
	...
	if ( !set_qspi_frequency(cfg->clk_freq * 4)) { return false; }
...
static bool configure_gpios(void)
{
  ...
  else
  {
    ...
    {
       DAL_GPIO_CFG(QUAD_SPI_CLK, 1, DAL_GPIO_INPUT, DAL_GPIO_NO_PULL, DAL_GPIO_4MA),
       DAL_GPIO_CFG(QUAD_SPI_CS_0, 1, DAL_GPIO_OUTPUT, DAL_GPIO_PULL_UP, DAL_GPIO_4MA),
       DAL_GPIO_CFG(QUAD_SPI_CS_1, 1, DAL_GPIO_OUTPUT, DAL_GPIO_PULL_UP, DAL_GPIO_4MA),
       DAL_GPIO_CFG(QUAD_SPI_DATA_0, 1, DAL_GPIO_INPUT, DAL_GPIO_PULL_DOWN, DAL_GPIO_4MA),
       DAL_GPIO_CFG(QUAD_SPI_DATA_1, 1, DAL_GPIO_INPUT, DAL_GPIO_PULL_DOWN, DAL_GPIO_4MA),
       DAL_GPIO_CFG(QUAD_SPI_DATA_2, 1, DAL_GPIO_INPUT, DAL_GPIO_PULL_DOWN, DAL_GPIO_4MA),
       DAL_GPIO_CFG(QUAD_SPI_DATA_3, 1, DAL_GPIO_INPUT, DAL_GPIO_PULL_DOWN, DAL_GPIO_4MA),
    };
...
static void dma_transfer_chain(DESCRIPTOR *chain)
{
   ...
   REG_OUT(NEXT_DMA_DESC_ADDR, (uint64_t)chain);
   ...
   while (desc)
   {
      if (desc->direction == MASTER_READ) {
         if (dma_mode && desc->bounce_length == 0) {
            InvalidateDataCacheRange(INT_TO_PTR(desc->data_address),desc->length);
         }
         else {
            ...
            if ((dma_mode) && !(desc->bounce_length & 0x3) && !(desc->bounce_dst & 0x3)) {
               InvalidateDataCacheRange(src, desc->bounce_length);
...
static void queue_addr(uint32_t addr, uint8_t addr_bytes, QSPI_MODE addr_mode, bool dma_mode)
{
   ...
   if (dma_mode) { WriteBackInvalidateDataCacheRange(ptr_org, addr_bytes); }
                              ...
                              QSPI_MODE data_mode, bool write, bool dma_mode)
{
   ...
   if (write)
   {
      ...
      if (dma_mode) { WriteBackInvalidateDataCacheRange(ptr, data_bytes); }
   }
   else
   {
      desc->bounce_src = (uint64_t)ptr;
      desc->bounce_dst = (uint64_t)data;
                              ...
                              QSPI_MODE data_mode, bool write, bool dma_mode)
{
   ...
   while (data_bytes)
   {
      ...
      desc->direction = write;
      desc->multi_io_mode = data_mode;
      desc->data_address = (uint64_t)data;
      ...
      if (dma_mode) {
         if (write) {
            WriteBackInvalidateDataCacheRange(INT_TO_PTR(desc->data_address), desc->length);
         } else {
            InvalidateDataCacheRange(INT_TO_PTR(desc->data_address), desc->length);
         }
      }
...
static void queue_opcode(uint8_t opcode, QSPI_MODE cmd_mode, bool dma_mode)
{
   ...
   *ptr = opcode;
   if (dma_mode) { WriteBackInvalidateDataCacheRange(ptr, opcode_bytes); }
...
/*==================================================================================================
FUNCTION: reg_update_ram_mode

DESCRIPTION: for ddr mode configuring as mentioned below.

ddr_mode:
   --> FULL_CYCLE_MODE  0x00000008  to 0x00000000 
   --> tx_data_oe_delay = 0 set to 1
   --> tx_data_delay    = 0 set to 1
 
sdr_mode:
   --> FULL_CYCLE_MODE  0x00000001  to 0x00000008 
   --> tx_data_oe_delay = 1 set to 0
   --> tx_data_delay    = 1 set to 0
==================================================================================================*/
void  reg_update_ram_mode(QSPI_RAM_MODES mode)  
{  
  
  REG_IN(MSTR_CONFIG, &mstr_config_ddr);
  if(mode == QSPI_DDR_MODE)
  {
	 mstr_config_ddr &= ~(FULL_CYCLE_MODE);
     mstr_config_ddr |= (1 << TX_DATA_OE_DELAY_SHIFT) |(1 << TX_DATA_DELAY_SHIFT); 
  }
  else //sdr mode
  {
	 mstr_config_ddr |= (FULL_CYCLE_MODE);
  ...
  REG_OUT(MSTR_CONFIG,    mstr_config_ddr);  
}
...
bool quadspi_cmd(QSPI_CMD *cmd, uint32_t addr, uint8_t *data, uint32_t data_bytes, bool enable_dma)
{
   ...
   /*
       for ddr mode we are updating few bits,which are clearly mentioned in  reg_update_ddrmode  
   */
   if( (cmd->data_mode == DDR_4BIT )  && (cmd->addr_mode == DDR_4BIT) )
   {
	 reg_update_ram_mode(QSPI_DDR_MODE);
   }
   else
   {
	 reg_update_ram_mode(QSPI_SDR_MODE);  
   }
...
FUNCTION: quadspi_configuredlp

DESCRIPTION:
   Configure Phase sample for QSPI lane.

==================================================================================================*/
void quadspi_configure_dlp(QSPI_LANE_SAMPLING_CONFIG sequence, bool enable_dlp_mode)
{
   uint32_t mstr_config = 0;
   uint32_t sample_clk_config = 0;
   bool config_dlp = TRUE;

   if(!enable_dlp_mode && dlp_sequence_enabled)
   {
      REG_IN(MSTR_CONFIG, &mstr_config);
      mstr_config |= (FB_CLK_EN);
      REG_OUT(MSTR_CONFIG, mstr_config);
      config_dlp = FALSE;
      dlp_sequence_enabled = FALSE;
   }
   else if(enable_dlp_mode && !dlp_sequence_enabled)
   {
      REG_IN(MSTR_CONFIG, &mstr_config);
      mstr_config &= ~FB_CLK_EN;
      REG_OUT(MSTR_CONFIG, mstr_config);
      dlp_sequence_enabled = TRUE;
   }

   if(config_dlp){
         sample_clk_config = ((sequence.data_lane0_phase & LANE0_CLK_CNTR_INIT_VAL_MASK) << LANE0_CLK_CNTR_INIT_VAL_SHIFT) |
                             ((sequence.data_lane1_phase & LANE1_CLK_CNTR_INIT_VAL_MASK) << LANE1_CLK_CNTR_INIT_VAL_SHIFT) |
                             ((sequence.data_lane2_phase & LANE2_CLK_CNTR_INIT_VAL_MASK) << LANE2_CLK_CNTR_INIT_VAL_SHIFT) |
                             ((sequence.data_lane3_phase & LANE3_CLK_CNTR_INIT_VAL_MASK) << LANE3_CLK_CNTR_INIT_VAL_SHIFT) ;
         REG_OUT(SAMPLING_CLK_CONFIG,sample_clk_config);  
   }
}