...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/SocPkg/Library/RamdumpTableTargetLib/SDX/ramdump_table_target_sdx.c#1 $

when       who       what, where, why
--------   ---       ----------------------------------------------------------
03/02/23   krd       Added reset reason in rst_stat.bin
02/27/23   batta     Added support for sbl_pmic_dump.bin
02/15/23   ck        Renamed edk2 interfaces
12/27/22   batta     Added support for IPA dumps.
08/19/22   rhy       Added JUMP value to false for Sahara
...
static bl_error_boot_type _ramdump_add_ipa_mem_regions(boot_handle, ramdump_table_sdx_handle*, uint32 dload_cookie);
static bl_error_boot_type _ramdump_add_pmic_dump_data (boot_handle, ramdump_table_sdx_handle*, uint32 dload_cookie);
...
)
{
  ...
  do
  {
    ...
    /* Setting JUMP value to false for Sahara */
...
static bl_error_boot_type _ramdump_add_pmic_dump_data (boot_handle config_context_handle, ramdump_table_sdx_handle* handle, uint32 dload_cookie)
{
...
)
{
  ...
  boot_memory_mgr_if_type *mem_if = NULL;
 struct boot_shared_imem_cookie_type *boot_shared_imem_cookie_ptr = (struct boot_shared_imem_cookie_type *)SHARED_IMEM_BOOT_BASE;
...
static bl_error_boot_type _ramdump_add_ddr_regions (boot_handle config_context_handle, ramdump_table_sdx_handle* handle, uint32 dload_cookie)
{
  ...
  do
  {
    ...
    if (smem_if == NULL)
    {
      return_status = GET_ERROR_CODE(BL_ERROR_GROUP_SMEM, BL_ERR_INTERFACE_FAILURE);
      break;
    }
    
    /* Get the RAM partition table */
    pDDRRegionsData = smem_if->get_addr(DDRINFO_UEFI_SMEM_ID, (uint32*)&DDRInfoBuffSz);
