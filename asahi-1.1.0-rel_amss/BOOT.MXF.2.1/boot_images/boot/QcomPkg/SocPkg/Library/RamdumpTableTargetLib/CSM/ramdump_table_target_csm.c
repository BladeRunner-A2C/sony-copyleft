...
#  07/08/22   batta   Updated parameters to ramdump_table_add_restricted_region API.
#  04/28/22   rama    Update TARGET_DEPENDENT_RAMDUMP_ENTRIES
#  03/16/22   ck      Switched from IPA to eCPRI and commented out DDR during pre-sil
#  02/09/22   ck      Updated heap function names
#  01/19/22   ck      Initial creation from SM copy
#
=============================================================================*/
...
} ramdump_table_csm_handle;


static bl_error_boot_type _ramdump_add_ecpri_mem_regions (boot_handle, ramdump_table_csm_handle*, uint32 dload_cookie);
static bl_error_boot_type _ramdump_add_pmic_info (boot_handle, ramdump_table_csm_handle*, uint32 dload_cookie);
static bl_error_boot_type _ramdump_add_pmic_dump_data (boot_handle, ramdump_table_csm_handle*, uint32 dload_cookie);
static bl_error_boot_type _ramdump_add_shared_imem_region (boot_handle, ramdump_table_csm_handle*, uint32 dload_cookie);
static bl_error_boot_type _ramdump_add_smem_info_region (boot_handle, ramdump_table_csm_handle*, uint32 dload_cookie);
static bl_error_boot_type _ramdump_add_reset_status_reg (boot_handle, ramdump_table_csm_handle*, uint32 dload_cookie);
static bl_error_boot_type _ramdump_add_ddr_training_data (boot_handle, ramdump_table_csm_handle*, uint32 dload_cookie);
static bl_error_boot_type _ramdump_add_pimem_data( boot_handle, ramdump_table_csm_handle*, uint32 dload_cookie);
static bl_error_boot_type _ramdump_add_binning_info (boot_handle, ramdump_table_csm_handle*);
static bl_error_boot_type _ramdump_add_aoss_cc_regs (boot_handle, ramdump_table_csm_handle*, uint32 dload_cookie);
static bl_error_boot_type _ramdump_add_ddr_regions (boot_handle, ramdump_table_csm_handle*, uint32 dload_cookie);
...
static bl_error_boot_type _init_handle(const boot_handle config_context_handle,
                                       ramdump_table_csm_handle *handle,
...
*
* @brief
*   This function populates ramdump_table_info with its CSM specific regions
*
* @par Dependencies
*   None
*
...
)
{
  ...
  ramdump_table_csm_handle *handle = NULL;
  ...
  do
  {
    ...
    return_status = mem_if->malloc(sizeof(ramdump_table_csm_handle), (void **)&handle);
    ...
    /* Add eCPRI region */
    return_status = _ramdump_add_ecpri_mem_regions (config_context_handle, handle, dload_cookie);
    if (return_status != BL_ERR_NONE)
    {
      break;
    }
...
static bl_error_boot_type _ramdump_add_ecpri_mem_regions
(
  boot_handle config_context_handle,
  ramdump_table_csm_handle* handle,
  uint32 dload_cookie
...
{
  bl_error_boot_type return_status = BL_ERR_NONE;
  ramdump_table_memory_region dump_regions[] = {ECPRI_MEMORY_REGION_TABLE};
  uint32 index = 0;
...
static bl_error_boot_type _ramdump_add_pmic_dump_data (boot_handle config_context_handle, ramdump_table_csm_handle* handle, uint32 dload_cookie)
{
  boot_pmic_if_type *pmic_if = NULL;
  bl_error_boot_type return_status = BL_ERR_NONE;
...
static bl_error_boot_type _ramdump_add_ddr_training_data(boot_handle config_context_handle, ramdump_table_csm_handle* handle, uint32 dload_cookie)
{
...
#ifdef SCL_pIMEM_BASE    
static bl_error_boot_type _ramdump_add_pimem_data(boot_handle config_context_handle, ramdump_table_csm_handle* handle, uint32 dload_cookie)
{
  uint32 size_in_mb = 0;
  bl_error_boot_type return_status = BL_ERR_NONE;
...
static bl_error_boot_type _ramdump_add_ddr_regions (boot_handle config_context_handle, ramdump_table_csm_handle* handle, uint32 dload_cookie)
{
  bl_error_boot_type return_status = BL_ERR_NONE;
  uint64 start_addr =0;
  uint64 size =0;
...
)
{
  ...
  ramdump_table_csm_handle *handle = (ramdump_table_csm_handle *)handle_ptr;
...
bl_error_boot_type ramdump_table_target_unrecoverable_error_handler (void)
{
  ...
  do
  {
    ...
    return_status = boot_config_context_get_ptr(CONFIG_CONTEXT_CRT_HANDLE,
    ...
    cache_if->WriteBackInvalidateDataCacheRange((void *)SCL_RAMDUMP_DLOAD_BUFFERS_BASE,
                                                (uintnt)SCL_RAMDUMP_DLOAD_BUFFERS_SIZE);
    ...
    DALSYS_DeInitMod();
    cache_if->WriteBackInvalidateDataCacheRange((void *)Image$$RAMDUMP_DATA_RW$$Base,
                                                (uintnt)Image$$RAMDUMP_DATA_RW$$Length);

    cache_if->WriteBackInvalidateDataCacheRange((void *)Image$$STACK$$Base,
                                                (uintnt)Image$$STACK$$Length);
