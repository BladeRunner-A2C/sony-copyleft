...
#
#  $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/SocPkg/Library/RamdumpTableTargetLib/SM/ramdump_table_target_sm.c#1 $
#
#  when       who        what, where, why
#  --------   ---        ----------------------------------------------------------
#  03/30/23   ts         Added changes for DDR Training dump using restricted region api
#  02/15/23   ck         Renamed edk2 interfaces
#  02/06/23   aishwark   Updating parameters in Dump Reset Reason
...
static bl_error_boot_type _ramdump_add_dump_reason_regions (boot_handle, ramdump_table_sm_handle*, uint32 dload_cookie);
static bl_error_boot_type _ramdump_add_cpu_context_regions (boot_handle, ramdump_table_sm_handle*, uint32 dload_cookie);
...
)
{
  ...
  do
  {
    ...
    /* Add Dump Reason region */
    return_status = _ramdump_add_dump_reason_regions (config_context_handle, handle, dload_cookie);
    if (return_status != BL_ERR_NONE)
    {
      break;
    }
    ...
    /* Add CPU Context */
    return_status = _ramdump_add_cpu_context_regions (config_context_handle, handle, dload_cookie);
    if (return_status != BL_ERR_NONE)
    {
      break;
    }
...
bl_error_boot_type _ramdump_add_dump_reason_regions
(
  boot_handle config_context_handle,
  ramdump_table_sm_handle* handle,
  uint32 dload_cookie
...
#ifdef FEATURE_XBL_ADD_DUMP_REASON_REGION
#define TZ_OEM_RESET_REASON_IMEM_OFFSET		0x7A4
#define TME_RESET_REASON_IMEM_OFFSET		0x800
  
  bl_error_boot_type return_status = BL_ERR_NONE;
  boot_pmic_if_type *pmic_if = NULL;
  boot_memory_mgr_if_type *mem_if = NULL;
  raw_dump_dump_reason_t *raw_dump_reason = NULL;
  uint32 *tz_oem_reset_reason_p  = NULL;
  uint32 *tme_reset_reason_p  = NULL;
  uint32 poff_reason = 0;
  uint32 tz_oem_reset_reason = 0;
  uint32 tme_reset_reason = 0;
  ...
  do
  {
    /* read pmic reset reason and insert in table */
    return_status = boot_config_context_get_ptr(config_context_handle, CONFIG_CONTEXT_MEMORY_MGR_INTERFACE, (void **)&mem_if);
    if (return_status != BL_ERR_NONE)
    {
      break;
    ...
    /* Malloc memory for raw_dump_dump_reason */
    return_status = mem_if->malloc(sizeof(raw_dump_dump_reason_t), (void **)&raw_dump_reason);

    if ((return_status != BL_ERR_NONE) || raw_dump_reason == NULL)
    {
      break;
    }
    ...
    /* Zero init parameter struct */
    raw_dump_reason->parameter1 = 0;
    raw_dump_reason->parameter2 = 0;
    raw_dump_reason->parameter3 = 0;
    raw_dump_reason->parameter4 = 0;

    /* Get power on reason for pmic device 0 */
    poff_reason = 0;
    pmic_if->pm_pon_get_poff_reason(PMIC_A, (pm_pon_poff_reason_type*)&poff_reason);

    /* Get TZ oem reset reason */
    tz_oem_reset_reason_p  = (uint32*)((uint64)SHARED_IMEM_BOOT_BASE + (uint32)TZ_OEM_RESET_REASON_IMEM_OFFSET);
    tz_oem_reset_reason = 0;
    tz_oem_reset_reason = *tz_oem_reset_reason_p;
	
    /* Get TME reset reason */
    tme_reset_reason_p  = (uint32*)((uint64)SHARED_IMEM_BOOT_BASE + (uint32)TME_RESET_REASON_IMEM_OFFSET);
    tme_reset_reason = 0;
    tme_reset_reason = *tme_reset_reason_p;

    /* Update POFF reason,TZ oem and TME reset reason  parameters */
    raw_dump_reason->parameter1 = (uint64)poff_reason;
    raw_dump_reason->parameter2 = (uint64)tz_oem_reset_reason;
    raw_dump_reason->parameter3 = (uint64)tme_reset_reason;

    /* reserved parameters as zero */
    raw_dump_reason->parameter4 = 0;
    ...
    if (dload_cookie & SBL_DLOAD_MODE_BIT_MASK)
    {
      return_status = ramdump_table_add_entry(handle->ramdump_table_handle_ptr,
                                              RAMDUMP_TABLE_OPTIONAL_DEF_SAVE,
                                              (uintnt)(raw_dump_reason),
                                              sizeof(raw_dump_dump_reason_t),
                                              "Dump Reason Region",
                                              "DMP_RESN.BIN");
      if (return_status != BL_ERR_NONE)
      {
    ...
    if (dload_cookie & SBL_MINIDUMP_MODE_BIT_MASK)
    {
      if(!(boot_add_minidump_region(MD_SS_BOOT, "DMP_RESN", (uint64)(raw_dump_reason), sizeof(raw_dump_dump_reason_t))))
      {
        return GET_ERROR_CODE(BL_ERROR_GROUP_RAMDUMP_TABLE, BL_ERR_MD_ADD_REGION_FAIL);
      }
    }
...
#define CPU_CONTEXT_PTR_IMEM_OFFSET 0x800

bl_error_boot_type _ramdump_add_cpu_context_regions
(
  boot_handle config_context_handle,
  ramdump_table_sm_handle* handle,
  uint32 dload_cookie
...
{
#ifdef FEATURE_XBL_ADD_CPU_CONTEXT_REGION 
  bl_error_boot_type return_status = BL_ERR_NONE;

  struct boot_shared_imem_cookie_type *boot_shared_imem_cookie_ptr;
  
  typedef struct cpu_context_data
  {
    uintnt *cpu_context_base;
    uintnt cpu_context_size;
    uintnt *cpu_context_type_specific_info;
    uintnt cpu_context_type_specific_info_size;
  } cpu_context_data_t;


  cpu_context_data_t *cpu_context_data_ptr = NULL;

  boot_shared_imem_cookie_ptr = (struct boot_shared_imem_cookie_type *)SHARED_IMEM_BOOT_BASE+CPU_CONTEXT_PTR_IMEM_OFFSET;
  ...
  cpu_context_data_ptr = (cpu_context_data_t*)boot_shared_imem_cookie_ptr;
  ...
  do
  {
    if (dload_cookie & SBL_DLOAD_MODE_BIT_MASK)
    {
      return_status = ramdump_table_add_entry(handle->ramdump_table_handle_ptr,
                                              RAMDUMP_TABLE_OPTIONAL_DEF_SAVE,
                                              (uintnt)(cpu_context_data_ptr->cpu_context_base),
                                              cpu_context_data_ptr->cpu_context_size,
                                              "CPU Context Region",
                                              "CPU_CTXT.BIN");
      if (return_status != BL_ERR_NONE)
      {
      ...
      return_status = ramdump_table_add_entry(handle->ramdump_table_handle_ptr,
                                              RAMDUMP_TABLE_OPTIONAL_DEF_SAVE,
                                              (uintnt)(cpu_context_data_ptr->cpu_context_type_specific_info),
                                              cpu_context_data_ptr->cpu_context_type_specific_info_size,
                                              "CPU Context Info",
                                              "CXT_INFO.BIN");
      if (return_status != BL_ERR_NONE)
      {
    ...
    if (dload_cookie & SBL_MINIDUMP_MODE_BIT_MASK)
    {
      if(!(boot_add_minidump_region(MD_SS_BOOT, "CPU_CTXT", (uint64)(cpu_context_data_ptr->cpu_context_base), cpu_context_data_ptr->cpu_context_size)))
      {
        return GET_ERROR_CODE(BL_ERROR_GROUP_RAMDUMP_TABLE, BL_ERR_MD_ADD_REGION_FAIL);
      }
      if(!(boot_add_minidump_region(MD_SS_BOOT, "CXT_INFO", (uint64)(cpu_context_data_ptr->cpu_context_type_specific_info), cpu_context_data_ptr->cpu_context_type_specific_info_size)))
      {
        return GET_ERROR_CODE(BL_ERROR_GROUP_RAMDUMP_TABLE, BL_ERR_MD_ADD_REGION_FAIL);
      }
    }
...
static bl_error_boot_type _ramdump_add_ddr_training_data(boot_handle config_context_handle, ramdump_table_sm_handle* handle, uint32 dload_cookie)
{
  ...
  return ramdump_table_add_restricted_region(handle->ramdump_table_handle_ptr, 0,
                                             SCL_DDR_PARTITION_DATA_BUF_SIZE,
                                             "DDR Training Data",
                                             "DDR_DATA.BIN",
                                             (void*)&read_ddr_training_data_from_partition);
...
bl_error_boot_type ramdump_table_target_unrecoverable_error_handler (void)
{
  ...
  do
  {
    ...
    cache_if->WriteBackInvalidateDataCacheRange((void *)SCL_RAMDUMP_DLOAD_BUFFERS_BASE, (uintnt)SCL_RAMDUMP_DLOAD_BUFFERS_SIZE);

    external_cached_heap_base_addr = xbl_get_external_cached_heap_addr();
    cache_if->WriteBackInvalidateDataCacheRange((void *)external_cached_heap_base_addr,
    ...
    cache_if->WriteBackInvalidateDataCacheRange((void *)Image$$RAMDUMP_DATA_RW$$Base, (uintnt)Image$$RAMDUMP_DATA_RW$$Length);
    cache_if->WriteBackInvalidateDataCacheRange((void *)Image$$STACK$$Base, (uintnt)Image$$STACK$$Length);
