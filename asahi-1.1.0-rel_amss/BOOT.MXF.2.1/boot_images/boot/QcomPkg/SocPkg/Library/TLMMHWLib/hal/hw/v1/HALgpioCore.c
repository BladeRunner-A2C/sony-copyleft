...
===========================================================================
Copyright (c) 2017-2018,2022 QUALCOMM Technologies Incorporated.
All Rights Reserved.
...
$Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/SocPkg/Library/TLMMHWLib/hal/hw/v1/HALgpioCore.c#1 $
 ...
 */
#define HAL_GPIO_CFG_TO_REG_EX(func, dir, pull, drive, strongpull) \
              (((pull)  & 0x3)        | \
              (((func)  & 0xF) << 2)  | \
              (((dir)   & 0x1) << 9)  | \
              (((drive) & 0x7) << 6)  | \
...
==========================================================================*/
void HAL_gpio_ConfigPinStrongPull
(
...
void HAL_gpio_GetPinConfigStrongPull
(
  HALgpioBlockDescType *pBlock,
  uint32                nOffs,
  uint32                nGPIO,
...
/*
 * custom HAL configuration added to support I2C Strong Pull
 */
HAL_tlmm_GpioCoreControlType HAL_gpio_StrongPullControl = 
{
  .Init              = HAL_gpio_Init,
  .ConfigPin         = HAL_gpio_ConfigPinStrongPull,
  .ReadPin           = HAL_gpio_ReadPin,
  .WritePin          = HAL_gpio_WritePin,
  .GetTileIndex      = HAL_gpio_GetTileIndex,
  .ReadPinOutput     = HAL_gpio_ReadPinOutput,
  .GetPinConfig      = HAL_gpio_GetPinConfigStrongPull
};
...
/*===============================================================================
  ...
  FUNCTION
    void HAL_gpio_ConfigPinStrongPull (*pBlock, nOffs, nGPIO, Config)
...
)
{
  uint32 nRegVal = 0;
    ...
    nRegVal =
          HAL_GPIO_CFG_TO_REG_EX(Config.func,
                              Config.dir,
                              Config.pull,
                              HAL_gpio_DriveCvtToHW(Config.drive),
                              Config.strongpull);
...
} /* HAL_gpio_ConfigPinStrongPull */

/*===============================================================================

  FUNCTION
    void HAL_gpio_GetPinConfigStrongPull (pBlock, nOffs, nGPIO, *Config)
...
)
{
  ...
  Config->pull   = HAL_GPIO_GET_PULL(nRegVal);
  Config->drive  = HAL_gpio_DriveCvtToLogical(HAL_GPIO_GET_DRIVE(nRegVal));
  Config->strongpull  = HAL_GPIO_GET_STRONG_PULL(nRegVal);
