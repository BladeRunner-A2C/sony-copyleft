...
===========================================================================
             Copyright (c) 2010 - 2015, 2022, 2023 Qualcomm Technologies, Inc. All rights reserved.
...
static GPIOClientHandleType GPIOHandle;
...
static uint32 TLMM_DriveCvtToHW
(
  uint32 nLogicalVal
)
{
...
static uint32 TLMM_DriveCvtToLogical
(
  uint32 nHWVal
)
{
...
DALResult TLMM_DeviceInit(TLMMClientCtxt *pCtxt)
{
  ...
  if( !bInitialized )
  {
    if ( GPIO_SUCCESS != GPIO_DriverInit())
    {
      return DAL_ERROR;
    }
...
)
{
  ...
  if (GPIO_SUCCESS == GPIO_RegisterPinExplicit(GPIOHandle, DAL_GPIO_NUMBER(eGpioConfig), GPIO_ACCESS_SHARED, &GPIOKey))
  {
    ...
    Config.strongpull = DAL_GPIO_STRONG_PULL(eGpioConfig);
    ...
    case DAL_TLMM_GPIO_ENABLE:
      for( nIdx = 0; nIdx < nSize; nIdx++ )
      {
        if (GPIO_SUCCESS == GPIO_RegisterPinExplicit(GPIOHandle, DAL_GPIO_NUMBER(eGpioGroup[nIdx]), GPIO_ACCESS_SHARED, &GPIOKey))
        {
          ...
          Config.strongpull = DAL_GPIO_STRONG_PULL(eGpioGroup[nIdx]);
          Config.unused = 0;
