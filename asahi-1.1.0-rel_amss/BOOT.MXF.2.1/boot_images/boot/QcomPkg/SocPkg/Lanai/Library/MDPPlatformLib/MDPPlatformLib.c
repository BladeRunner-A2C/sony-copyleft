...
** Global variables
** ----------------------------------------------------------------------- */
static MDPPlatformContext *pgMDPPlatformContext = NULL;

/* Helper to get MDP platform params struct*/
#define MDP_GET_PLATFORMCONTEXT()  (pgMDPPlatformContext)
...
/**********************************************************************************************
*
* FUNCTION: MDPGetPanelFuntionTables()
*
* DESCRIPTION:
*   Get Panel function tables from platform context.
*
***********************************************************************************************/
MDPPlatformPanelFunctionTable *MDPGetPanelFuntionTables(MDP_Display_IDType eDisplayId)
{
  MDPPlatformPanelFunctionTable  *pPanelFunctionTables = NULL;
  MDPPlatformContext             *pMDPPlatformContext  = MDP_GET_PLATFORMCONTEXT();

  if (eDisplayId >= MDP_DISPLAY_MAX )
  {
    DEBUG ((EFI_D_ERROR, "DisplayDxe: Unsupported Display ID\n"));
  }
  else if (NULL != pMDPPlatformContext)
  {
    pPanelFunctionTables = &pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId];
  }

  return pPanelFunctionTables;
}
...
*
***********************************************************************************************/
static MDPPlatformConfigType PlatformConfigRemap(MDPPlatformContext *pMDPPlatformContext, MDPPlatformConfigType eConfig)
{
  MDPPlatformConfigType eRemappedConfig;

  // In SW render mode bypass all configuration that manipulate the hardware
  if ((NULL !=  pMDPPlatformContext) &&
      (TRUE == pMDPPlatformContext->sPlatformInfo.bSWRender))
  {
    switch (eConfig)
    {
...
*
****************************************************************************/
static uint32 AsciiStrnCat(CHAR8 *pDest, uint32 uLimit, CHAR8 *pSrc)
{
...
MDP_Status MDPPlatformConfigure(MDP_Display_IDType eDisplayId, MDPPlatformConfigType eConfig, MDPPlatformParams *pPlatformParams)
{
  ...
  ePlatformConfig = PlatformConfigRemap(pMDPPlatformContext, eConfig);

  switch (ePlatformConfig)
  {
  case MDPPLATFORM_CONFIG_INIT:

    if (NULL == pgMDPPlatformContext)
    {
      if (NULL == (pgMDPPlatformContext = (MDPPlatformContext *)MDP_OSAL_CALLOC(sizeof(MDPPlatformContext))))
      {
    ...
    if (NULL != pgMDPPlatformContext)
    {
      ...
      if (MDP_STATUS_OK != ReadPlatformIDAndChipID(&pgMDPPlatformContext->sPlatformInfo.sEFIPlatformType, &pgMDPPlatformContext->sPlatformInfo.sEFIChipSetId, &pgMDPPlatformContext->sPlatformInfo.sEFIChipSetFamily))
      {
        pgMDPPlatformContext->sPlatformInfo.bSWRender = TRUE;
      }
    }
    break;
  ...
  case MDPPLATFORM_CONFIG_SW_RENDERER:
    ...
    if ((NULL == pPlatformParams) ||
        (NULL == pMDPPlatformContext))
    {
      eStatus = MDP_STATUS_BAD_PARAM;
    }
    else if (EFI_PLATFORMINFO_TYPE_VIRTIO  == pMDPPlatformContext->sPlatformInfo.sEFIPlatformType.platform)
    {
      // Emulation platform
      pMDPPlatformContext->sPlatformInfo.bSWRender                  = FALSE;
    ...
    else if (TRUE == PcdGetBool(PcdDisplayForceSwRenderer))
    {
      ...
      pMDPPlatformContext->sPlatformInfo.bSWRenderOverrride         = pPlatformParams->sPlatformInfo.bSWRenderOverrride;
      // Cache that SW Rendering is enabled
      pMDPPlatformContext->sPlatformInfo.bSWRender                  = TRUE;
    }
    /* Setup display panel configurations according to CDT */
    else if (MDP_STATUS_OK != MDPPlatformSetupPanelConfig(eDisplayId, &pMDPPlatformContext->sPlatformPanel[eDisplayId], &pMDPPlatformContext->sPlatformInfo, &pPlatformParams->sPlatformInfo, &pMDPPlatformContext->asPanelConfig[eDisplayId]))
    {
      eStatus = MDP_STATUS_FAILED;
    }
    else if (TRUE == pMDPPlatformContext->sPlatformInfo.bSWRender)
    {
        ...
        case MDP_DISPLAY_SECONDARY:
          {
            ...
            MDPPlatformGetPanelList(pMDPPlatformContext->sPlatformPanel[eDisplayId].eAutoDetect, &pPanelList, &uPanelCnt, NULL);

            if (TRUE == pMDPPlatformContext->sPlatformInfo.bSWRender)
            {
            ...
            {
              switch (pMDPPlatformContext->sPlatformInfo.sEFIPlatformType.platform)
              {
#ifdef FEATURE_XBOOT
                ...
                {
                  if ((MDP_DISPLAY_AUTODETECT_DISABLE    != pMDPPlatformContext->sPlatformPanel[eDisplayId].eAutoDetect) &&
                      (MDP_STATUS_OK                     == DynamicDSIPanelDetection(eDisplayId, &pMDPPlatformContext->sPlatformPanel[eDisplayId], &uPanelID, pPanelList, uPanelCnt)))
                  {
#ifdef FEATURE_XBOOT
...
#endif /* FEATURE_XBOOT */
                    /* Detected */
                    pMDPPlatformContext->sPlatformPanel[eDisplayId].uPanelId       = uPanelID;
                    pMDPPlatformContext->sPlatformInfo.uPrimaryPanelId             = uPanelID;

                    /* Always re-setup panel config for the new detected panel */
                    if (NULL == MDPPlatformSelectPanelConfig(eDisplayId, pMDPPlatformContext->sPlatformPanel[eDisplayId].szName, &pMDPPlatformContext->sPlatformInfo.bSWRender, &pMDPPlatformContext->asPanelConfig[eDisplayId]))
                    {
              ...
              if ((FALSE == pMDPPlatformContext->sPlatformInfo.bSWRender) &&
                  (MDP_STATUS_OK == MDPPlatformGetPanelXmlConfig(eDisplayId, &pMDPPlatformContext->sPlatformPanel[eDisplayId], &pMDPPlatformContext->asPanelConfig[eDisplayId])))
              {
                /* Copy the panel configurations to pPlatformParams->sPlatformPanel */
                MDP_OSAL_MEMCPY(&pPlatformParams->sPlatformPanel, &pMDPPlatformContext->sPlatformPanel[eDisplayId], sizeof(MDPPlatformPanelInfo));
              }
              else
              {
                pPlatformParams->sPlatformPanel.pPanelXMLConfig = (int8 *)dummy_xmldata;
  ...
  case MDPPLATFORM_CONFIG_POWERUP:
    {
      ...
      case MDP_DISPLAY_SECONDARY:
        ...
        case EFI_PLATFORMINFO_TYPE_RCM:
        {
          ...
          {
            switch (pMDPPlatformContext->sPlatformPanel[eDisplayId].uPanelId)
            {
            default:
          ...
          else
          {
            ...
            if (NULL != pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pPanel_PowerUp)
            {
              if (MDP_STATUS_OK != (eStatus = pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pPanel_PowerUp(eDisplayId, &pMDPPlatformContext->sPanelPowerCtrl)))
              {
      ...
      case MDP_DISPLAY_EXTERNAL:
        ...
        case EFI_PLATFORMINFO_TYPE_RCM:
          ...
          {
            eStatus = ExternalPanel_Default_PowerUp(eDisplayId, &pMDPPlatformContext->sPanelPowerCtrl);
          }
          break;
  ...
  case MDPPLATFORM_CONFIG_POWERDOWN:
    {
      ...
      case MDP_DISPLAY_SECONDARY:
        ...
        case EFI_PLATFORMINFO_TYPE_RCM:
          ...
          {
            if (NULL != pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pPanel_PowerDown)
            {
              if (MDP_STATUS_OK != (eStatus = pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pPanel_PowerDown(eDisplayId, &pMDPPlatformContext->sPanelPowerCtrl)))
              {
                DEBUG((EFI_D_WARN, "DisplayDxe: Primary Power Down Sequence failed (%d)\n", eStatus));
              }
            }

            pMDPPlatformContext->sPanelPowerCtrl.bBacklightEnabled = FALSE; /* wled is turned off */
          }
          break;
      ...
      case MDP_DISPLAY_EXTERNAL:
        ...
        case EFI_PLATFORMINFO_TYPE_RCM:
          ...
          {
            eStatus = ExternalPanel_Default_PowerDown(eDisplayId, &pMDPPlatformContext->sPanelPowerCtrl);
          }
          break;
        default:
          break;
      ...
      else
      {
        ...
        case MDP_DISPLAY_SECONDARY:
          ...
          case EFI_PLATFORMINFO_TYPE_RCM:
            ...
            if (FALSE == pMDPPlatformContext->sPanelPowerCtrl.bBacklightEnabled)
            {
              if (NULL != pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pPanel_Brightness_Enable)
              {
                if (MDP_STATUS_OK != (eStatus = pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pPanel_Brightness_Enable(eDisplayId, &pMDPPlatformContext->sPanelPowerCtrl)))
                {
                  DEBUG((EFI_D_WARN, "MDPPlatformConfig: Backlight enabled failed\n"));
                }
              }

              pMDPPlatformContext->sPanelPowerCtrl.bBacklightEnabled = TRUE;
            }

            if (NULL != pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pPanel_Brightness_Level)
            {
              if (MDP_STATUS_OK != (eStatus = pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pPanel_Brightness_Level(eDisplayId, &pPlatformParams->sBacklightConfig)))
              {
  ...
  case MDPPLATFORM_CONFIG_GETPLATFORMINFO:
    {
      ...
      else
      {
        ...
        //
        MDP_OSAL_MEMCPY(&pPlatformParams->sPlatformInfo, &pMDPPlatformContext->sPlatformInfo, sizeof(MDPPlatformInfo));
      }
    }
    break;
  ...
  case MDPPLATFORM_CONFIG_RESETPANEL:
    {
      ...
      else
      {
         ...
         if ((MDP_RESET_STATE_INVALID == (pPlatformParams->sPlatformPanelReset.uResetGpio)) &&
             (NULL                    != pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pMDPPlatformPanelGpio))
         {
           pMDPPlatformContext->sPanelPowerCtrl.uResetGpio = pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pMDPPlatformPanelGpio->uDispResetGpio;
           pPlatformParams->sPlatformPanelReset.uResetGpio = pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pMDPPlatformPanelGpio->uDispResetGpio;
         }
         else
         {
           pMDPPlatformContext->sPanelPowerCtrl.uResetGpio = pPlatformParams->sPlatformPanelReset.uResetGpio;
         }
         ...
         case MDP_DISPLAY_SECONDARY:
           ...
           case EFI_PLATFORMINFO_TYPE_RCM:
             ...
             if (NULL != pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pPanel_Reset)
             {
               if (MDP_STATUS_OK != (eStatus = pMDPPlatformContext->asMDPSupportPanelFunctions[eDisplayId].pPanel_Reset(&pPlatformParams->sPlatformPanelReset)))
               {
  ...
  case MDPPLATFORM_CONFIG_GETPANELXMLINFO:
    {
      ...
      else
      {
        ...
        MDPPlatformGetPanelList(pMDPPlatformContext->sPlatformPanel[eDisplayId].eAutoDetect, &pUEFIPanelList, &uPanelListLen, NULL);
        ...
        else
        {
          ...
          if (MDP_STATUS_OK == MDPPlatformGetPanelXmlConfig(eDisplayId, &sPlatformPanelInfo, &pMDPPlatformContext->asPanelConfig[eDisplayId]))
          {
  ...
  case MDPPLATFORM_CONFIG_GETSPRXMLINFO:
    {
      ...
      {
        UINT8    *pSprFileBuffer                     = NULL;
        UINT32    uSprBufferSize                     = 0;
        UINT32    uSprRetSize                        = 0;
        CHAR8     szName[DISPLAY_PANEL_NAME_MAX_LEN] = "sprconfig_";

        /* Load SPR config file priority based on panel name, fallback to the default file sprconfig.xml if this is not found */
        if (NULL != pMDPPlatformContext->sPlatformPanel[eDisplayId].szName)
        {
          AsciiStrnCat(szName, DISPLAY_PANEL_NAME_MAX_LEN, pMDPPlatformContext->sPlatformPanel[eDisplayId].szName);
          AsciiStrnCat(szName, DISPLAY_PANEL_NAME_MAX_LEN, ".xml");

          if (EFI_SUCCESS != MDP_Display_LoadFile(szName,
                                                  &pSprFileBuffer,
                                                  &uSprBufferSize,
                                                  &uSprRetSize,
                                                  0))
          {
            if (EFI_SUCCESS != MDP_Display_LoadFile("sprconfig.xml",
                                                    &pSprFileBuffer,
                                                    &uSprBufferSize,
                                                    &uSprRetSize,
                                                    0))
        ...
        {
          /* The pSprXMLConfig will be freed in MDPExitBoot()  */
          pPlatformParams->sSprXMLInfo.pSprXMLConfig  = (int8 *)pSprFileBuffer;
          pPlatformParams->sSprXMLInfo.uSprConfigSize = (uint32)uSprRetSize;
        }
      }
    }
    break;
  ...
  case MDPPLATFORM_CONFIG_SELECTPANELCONFIG:
    {
      ...
      {
        pPlatformParams->sPanelConfigInfo.pPanelConfig = MDPPlatformSelectPanelConfig(eDisplayId, pPlatformParams->sPlatformPanel.szName, NULL, &pMDPPlatformContext->asPanelConfig[eDisplayId]);
        ...
        {
          MDPPlatformSetPanelName(pMDPPlatformContext->sPlatformPanel[eDisplayId].szName, pPlatformParams->sPlatformPanel.szName);
        }
      }
    }
    break;
  ...
  case MDPPLATFORM_CONFIG_GETPANELDTINFO:
    {
      ...
      {
        pPlatformParams->pPanelConfig = &pMDPPlatformContext->asPanelConfig[eDisplayId];
      }
    }
    break;
...
static uint32 CheckNumDisplayCores()
{
  BOOLEAN                 bDisplayCoreDisabled = FALSE;
  ...
  {
    DEBUG((EFI_D_INFO, "CheckNumDisplayCores(): failed to locate gEfiChipInfoProtocolGuid.\n"));
  }
  else if (EFI_SUCCESS != hChipInfoProtocol->IsPartDisabled(hChipInfoProtocol, EFICHIPINFO_PART_DISPLAY, 0, &bDisplayCoreDisabled))
  {
    DEBUG((EFI_D_INFO, "CheckNumDisplayCores(): hChipInfoProtocol->IsPartDisabled() failed. \n"));
  }
  else if (bDisplayCoreDisabled)
  {
    ...
    DEBUG((EFI_D_WARN, "CheckNumDisplayCores(): Display part defect detected\n"));
