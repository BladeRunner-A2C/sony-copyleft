 ...
 * @ref         
 *
 *              Copyright (c) 2013-2015,2018-2023 Qualcomm Technologies Incorporated.
 *              All Rights Reserved.
...
// 01-24-18   vk       Remove ArmLib dependency
...
#include <Library/ArmLib.h>       // Data Memory Barrier
#include "DALSys.h"
#include "DALHeap.h"
//----------------------------------------------------------------------------
...
typedef struct 
{
  ...
  /*ALIGN(16) __QUSB_DMA_ZI__  */ volatile qusb_trb_t      trb_list[DCI_MAX_ENDPOINTS][DCI_MAX_DIRECTION][DCI_MAX_TRBS+DCI_ZLP_TRB];  // 4 ep * 2 dir * TDs, extra one for TX ZLP
  /*__QUSB_DMA_ZI__*/ volatile qusb_trb_ring_t trb_ring[DCI_MAX_ENDPOINTS][DCI_MAX_DIRECTION];  // 2 ep * 2 dir
  /*__QUSB_DMA_ZI__*/ uint8 control_tx_buffer[QUSB_MAX_CTRL_PKT_SIZE];

  boolean                   cached_event_buffer;
  qusb_dci_callback_table_t *dci_callback_table_ptr;
  qusb_event_buffer_info_t  evt_buf_info;
  uint32                    dci_max_packet_size;
  uint32                    dci_ep0_max_packet_size;
  boolean                   set_stall_on_xfer_nrdy_wrong_dir;
}qusb_dci_ctx_t;

static qusb_dci_ctx_t* qusb_dci_ctx;

void qusb_dci_ctx_free(void)
{
  if (qusb_dci_ctx)
  {
     if (FALSE == usb_dci_mem_free((void*)qusb_dci_ctx))
     {
       qusb_error_log(DCI_CTX_FREE_FAIL_LOG, 0, (uint32)(uintptr_t)qusb_dci_ctx);
       return;
     }
     else
     {
       qusb_log(DCI_CTX_FREE_LOG, 0, (uint32)(uintptr_t)qusb_dci_ctx);
       qusb_dci_ctx =NULL;
     }
  }
  else
  {
     qusb_error_log(DCI_CTX_FREE_REDUNDANT_LOG, 0, 0);
  }
}

static boolean qusb_dci_ctx_init(void)
{
  if (NULL == qusb_dci_ctx)
  {
    if (FALSE == usb_dci_mem_allocate((void**)&qusb_dci_ctx, sizeof(qusb_dci_ctx_t)))
    {
      qusb_error_log(DCI_CTX_INIT_FAIL_LOG, 0, 0);
      return FALSE;
    }
    qusb_log(DCI_CTX_INIT_LOG, 0, (uint32)(uintptr_t)qusb_dci_ctx);
  }
  else
  {
    qusb_error_log(DCI_CTX_INIT_AGAIN_LOG, 0, (uint32)(uintptr_t)qusb_dci_ctx);
    memset(qusb_dci_ctx, 0, sizeof(qusb_dci_ctx_t));
  }
  memset(qusb_dci_ctx, 0, sizeof(qusb_dci_ctx_t));

  qusb_dci_ctx->cached_event_buffer              = FALSE;
  qusb_dci_ctx->dci_callback_table_ptr           = NULL;
  qusb_dci_ctx->evt_buf_info.buf                 = NULL;
  qusb_dci_ctx->evt_buf_info.cur_pos             = 0;
  qusb_dci_ctx->dci_max_packet_size              = 0;
  qusb_dci_ctx->dci_ep0_max_packet_size          = 0;
  qusb_dci_ctx->set_stall_on_xfer_nrdy_wrong_dir = TRUE;
  return TRUE;
}
...
static uint32 qusb_dci_init_rumi(struct qusb_dcd_dsc_device *dcd_context_ptr)
{
  ...
  usb_shared_hs_phy_rumi_init(0, 0);
                       ...
                       qusb_dci_callback_table_t *callback_table_ptr)
{
  ...
  usb_platform_ctx usb_platform = usb_shared_get_platform();
  ...
  if (qusb_dci_skip_enumeration())
  {
    qusb_uart_log("enum_carried_from_pbl", 0);
  ...
  //GPIO toggle to reset Harmonium chip
  usb_shared_wcd_usb_switch_reset(0);
 
  /** It is not guaranteed that the SNPS core is always power collapsed **/ 
  /** Put it to power collapse explicitly and then bring out of power collapse **/
  ...
  HWIO_VIOCTL_EUD_DETECT_OUTM(HWIO_VIOCTL_EUD_DETECT_EUD_DETECT_BMSK, 0x1 << HWIO_VIOCTL_EUD_DETECT_EUD_DETECT_SHFT);
  ...
  /** Bring the controller out of Power Collapse and turn on clock**/
  //qusb_dci_enable_usb30_power_collapse(FALSE);
  qusb_dci_config_usb_clock(TRUE); 
  ...
  qusb_dci_config_usb_clock(FALSE); 
  ...
  {
    // If the charger type is CDP, then DP needs to be toggled for super speed enumeration for 200ms
    qusb_dci_handle_charger_cdp();
  }
...
void qusb_dci_event_buffer_restore(void)
{
  ...
  qusb_dci_ctx->evt_buf_info.buf = (uint8 *)QUSB_QSRAM_EVENT_BUFFER;
  // Read the event buffer current position cookie
  qusb_dci_read_evt_buff_cur_pos_cookie(&(qusb_dci_ctx->evt_buf_info.cur_pos));

  qusb_log(DCI_EVENT_BUFFER_RESTORE_LOG, 0, qusb_dci_ctx->evt_buf_info.cur_pos);
...
static void qusb_dci_event_buffer_init(void)
{
  ...
  qusb_dci_ctx->evt_buf_info.buf = (uint8 *)QUSB_QSRAM_EVENT_BUFFER;
  qusb_dci_ctx->evt_buf_info.cur_pos = 0x0; 
  memset(qusb_dci_ctx->evt_buf_info.buf, 0, EVENT_BUFFER_SIZE);

  qusb_log(DCI_EVENT_BUFFER_INIT_LOG, 0, (uint32)(uintptr_t)qusb_dci_ctx->evt_buf_info.buf);

  HWIO_USB3_PRI_GEVNTADRLO_REGS_p_GEVNTADRLO_OUTMI(0, HWIO_USB3_PRI_GEVNTADRLO_REGS_p_GEVNTADRLO_EVNTADRLO_BMSK, 
                                               (((uint32)(uintptr_t)(qusb_dci_ctx->evt_buf_info.buf)) << HWIO_USB3_PRI_GEVNTADRLO_REGS_p_GEVNTADRLO_EVNTADRLO_SHFT));
...
{
#if !defined(BUILD_HOSTDL) && !defined(BUILD_EHOSTDL) && !defined(FEATURE_EMMCBLD) && !defined(FEATURE_DEVICEPROGRAMMER_IMAGE)
    // Create variable to prevent flooding of error logs
    static boolean is_pm_err_detected = FALSE;
  
    boolean is_vbus_high = FALSE;
    pm_err_flag_type pm_status = PM_ERR_FLAG_SUCCESS;
  
    if (is_pm_err_detected)
    {
#ifdef FEATURE_QUSB_COMP_TEST
      // then use CC OUT GPIO
      if (qusb_dci_gpio_cc_out_get_ss_lane() != USB_TYPE_C_NO_LANE)
      {
        return TRUE;
      }
#endif
      //If Pmic return false, assume alwaysconnect
      return TRUE;
    }
    
    // Read Vbus state using PMIC CHGPTH API.
    pm_status = pm_chg_is_vbus_present(&is_vbus_high);
    if(PM_ERR_FLAG_SUCCESS == pm_status)
    {
      return (is_vbus_high);
    }
    else
    {
      qusb_uart_log("vbus_det_pm_unavail", 0);
      qusb_error_log(DCI_PMIC_API_FAILURE_LOG, 0, pm_status);
      is_pm_err_detected = TRUE;
      //If Pmic api return false, assume always connect
      return TRUE;
    }
#else 
    return TRUE;
#endif // !defined(BUILD_HOSTDL) && !defined(BUILD_EHOSTDL) && !defined(FEATURE_EMMCBLD) && !defined(FEATURE_DEVICEPROGRAMMER_IMAGE)
}
...
void qusb_dci_poll_events(void)
{
  ...
  if(IS_USB_SILICON(usb_platform.platform))
  {
    ...
    if ( (QUSB_MAX_SPEED_SUPER == dcd_dev->speed_required)
      && (QUSB_UNKNOWN_SPEED == dcd_dev->speed) )
    {
      // Execute SS PHY specific handling
  ...
  while (evt_byte_count > 0) 
  {
    ...
    cur_buf_pos = qusb_dci_ctx->evt_buf_info.cur_pos;

    if(qusb_dci_ctx->cached_event_buffer)
    {
      qusb_dci_dcache_inval(((uint8*)qusb_dci_ctx->evt_buf_info.buf + cur_buf_pos), sizeof(union qusb_event));
    }

    /* Move the event buffer to the new location */
    event.generic = *(uint32 *)((uint8 *)qusb_dci_ctx->evt_buf_info.buf + cur_buf_pos );
    ...
    qusb_dci_ctx->evt_buf_info.cur_pos = (cur_buf_pos + QUSB_EVT_BUFFER_UNIT_SIZE ) % EVENT_BUFFER_SIZE ;
...
void qusb_dci_handle_ctrl_transfers(struct qusb_event_depevt *depevt)
{
  ...
  dcd_dev = dci_core_info[DCI_SNPS_CORE].dcd_context_ptr;
  urb_ptr = (qusb_dci_ctx->trb_ring[ep][dir].urb_ptr);
  ...
  switch (ep_evt_type) 
  {
    case QUSB_EVT_EP_XFER_COMPLETE:
      ...
      if (dcd_dev->ctrl_state == EP0_STATE_SETUP_TRB)
      {
        ...
        qusb_dci_handle_transfer_complete(depevt);
        qusb_dci_ctx->dci_callback_table_ptr->setup_callback_fn(dcd_dev, (void*)&(qusb_dci_ctx->trb_list[0][0][0]));
      }
      /* Data stage complete   */
      ...
      else if(dcd_dev->ctrl_state == EP0_STATE_DATA)
      {
        if (depevt->event_status == QUSB_DDEPEVT_STATUS_CONTROL_DATA)
        {
          ...
          else
          {
            ...
            if(qusb_dci_ctx->set_stall_on_xfer_nrdy_wrong_dir == TRUE)
            {
              dcd_dev->ctrl_state = EP0_STATE_SETUP_TRB;
              qusb_dci_stall_endpoint(ep, DIR_RX); 
            }
...
static void qusb_dci_handle_transfer_complete(struct qusb_event_depevt *depevt)
{
  ...
  // if urb is not active due to cancel transfer or error (disconnect or others),
  // we should not overwrite urb even if the transfer get complete afterward.
  if (urb_ptr->transfer_status != QUSB_URB_STATUS_ACTIVE)
  {
    qusb_error_log(DCI_HANDLE_TRANSFER_COMPLETE_URB_NOT_ACTIVE_ERROR_LOG, 0, phy_ep_num);
    return;
  }

  #define SNPS_DCD_TRB_STATUS_SETUP_PENDING 0x2
  
  if (qusb_dci_ctx->trb_list[ep][dir][0].trbsts & SNPS_DCD_TRB_STATUS_SETUP_PENDING)
  {
    qusb_uart_log("STATUS_SETUP_PENDING, cancel_transfer ", ep << 16 |dir);
  ...
  if(dir == DIR_RX)
  {
    ...
    {
      if(urb_ptr->transfer_length < qusb_dci_ctx->dci_max_packet_size)
      {
        /*buf_size is updated by h/w and is always less than max_packet size*/
        urb_ptr->actual_length = qusb_dci_ctx->dci_max_packet_size - qusb_dci_ctx->trb_list[ep][dir][0].buf_size;
      } 
      else
      {
        /*Buffer size would always be less than or equal to transfer_length*/
        urb_ptr->actual_length = urb_ptr->transfer_length - qusb_dci_ctx->trb_list[ep][dir][0].buf_size;
      }
    ...
    {
      if(urb_ptr->transfer_length < qusb_dci_ctx->dci_ep0_max_packet_size)
      {
        /*buf_size is updated by h/w and is always less than max_packet_size */
        urb_ptr->actual_length = qusb_dci_ctx->dci_ep0_max_packet_size - qusb_dci_ctx->trb_list[ep][dir][0].buf_size;
      } 
      else
      {
        /*Buffer size would always be less than or equal to transfer_length*/
...
static void qusb_dci_handle_usb_reset(void)
{
  ...
  {
#ifdef FEATURE_QUSB_USBCV_MODE
    qusb_uart_log("usbcv_ignore_disc_on_reset", 0);
#else
    qusb_dci_handle_usb_disconnect();
#endif
  }
  ...
  for (; ep < DCI_MAX_ENDPOINTS; ep++ )
  {
    for (dir = 0; dir < DCI_MAX_DIRECTION; dir++)
    {
      ...
      if(qusb_dci_ctx->trb_ring[ep][dir].urb_ptr != NULL)
      {
        qusb_dci_cancel_transfer(ep, dir);
      }

      /* Clear stall for any non-control halted EPs */
      if((ep != 0) && (qusb_dci_ctx->trb_ring[ep][dir].ep_state & QUSB_EP_HALTED))
      {
  ...
  qusb_dci_ctx->dci_callback_table_ptr->usb_reset_callback_fn(dcd_dev);
}
 ...
 *           based on speed.
 *       4. Depending on the connected speed, write to the other PHYs control register to suspend it. 
...
static void qusb_dci_handle_connect_done(void)
{
  ...
  if(dcd_dev->speed >= QUSB_SPEED_MAX)
  {
  ...
  for (dir =0 ; dir < DCI_MAX_DIRECTION; dir++) 
  {
    ...
    ep_cmd.set_ep_config.xfer_nrdy_en = QUSB_ENABLE_XFER_NOT_READY;
    ep_cmd.set_ep_config.xfer_cmpl_en = QUSB_ENABLE_XFER_COMPLETE;
    if((dcd_dev->speed == QUSB_SUPER_SPEED) || (dcd_dev->speed == QUSB_SUPER_SPEED_PLUS))
    {
      ep_cmd.set_ep_config.max_packet_sz = QUSB_EP0_MAX_PACKET_SIZE_SUPER;
    } 
...
static void qusb_dci_handle_usb_disconnect(void)
{
  ...
  qusb_dci_ctx->dci_callback_table_ptr->usb_disconnect_callback_fn(dcd_dev);
} 
...
void qusb_dci_handle_suspend_entry(void)
{
  ...
  */
  ...
  if(link_state == LS_U3)
  {
    ...
    {
      qusb_dci_ctx->dci_callback_table_ptr->usb_suspend_callback_fn(dcd_dev); 
...
uint32 qusb_dci_stall_endpoint(uint8 ep, uint8 dir)
{
  ...
  qusb_dci_ctx->trb_ring[ep][dir].ep_state |= QUSB_EP_HALTED;
...
static uint32 qusb_dci_submit_trb_data(uint8 ep, uint8 dir)
{
  ...
  start_cmd.start_xfer.td_address_lo  = (uint32)((uint64)(qusb_dci_ctx->trb_ring[ep][dir].buf) & 0xFFFFFFFF);
  start_cmd.start_xfer.td_address_hi  = (uint32)((uint64)(qusb_dci_ctx->trb_ring[ep][dir].buf) >> 32 & 0xFFFFFFFF);
...
static void qusb_dci_prepare_trbs(uint32 ep, uint32 dir, uint64 buf_addr, uint32 byte_count, uint8 send_zlp, uint8 trb_cntrl)
{
  ...
  volatile qusb_trb_t *trb;
  volatile qusb_trb_ring_t *trb_ring;
  uint32 i = 0 ;
  if (NULL == qusb_dci_ctx)
  {
    qusb_error_log(DCI_CTX_NULL_LOG, 0, __LINE__);
  ...
  trb_ring = &(qusb_dci_ctx->trb_ring[ep][dir]);

  /* Initialize TRB ring */
  trb_ring->buf = (void *) &(qusb_dci_ctx->trb_list[ep][dir][0]);
  ...
  do
  {
    ...
    memset((void*)trb, 0, sizeof(qusb_trb_t));
    ...
    {
      if((dir == DIR_RX) && (total_bytes_for_TRB < qusb_dci_ctx->dci_max_packet_size))
      {
        trb->buf_size = qusb_dci_ctx->dci_max_packet_size;
      }
      else
      {
    ...
    {
      trb->buf_size = qusb_dci_ctx->dci_ep0_max_packet_size;
    }
  ...
  if((send_zlp == 1) && ((((xfer_len % qusb_dci_get_max_packet_size()) == 0) && (xfer_len != 0))
         ||((ep == 0) && ( dir == DIR_TX)&& ((xfer_len % qusb_dci_ctx->dci_ep0_max_packet_size) == 0))))
  {
    trb = (qusb_trb_t *) &(qusb_dci_ctx->trb_list[ep][dir][i]);
    memset((void*)trb, 0, sizeof(qusb_trb_t));
...
static uint32 qusb_dci_transfer_data_request(uint8 ep, uint8 dir,void* buffer_ptr,uint32 byte_count, uint8 send_zlp, trb_ctl_type_t trb_ctrl)
{
  if (ep == 0)
  {
    qusb_bus_log(DCI_TRANSFER_DATA_REQUEST_LOG, 0, byte_count);
  }
  else
  {
    qusb_log(DCI_TRANSFER_DATA_REQUEST_LOG, 0, byte_count);
  }
...
static uint32 qusb_dci_submit_request(uint8 ep, uint8 dir)
{
  ...
  qusb_dci_ctx->trb_ring[ep][dir].urb_ptr->transfer_status = QUSB_URB_STATUS_ACTIVE;
...
uint32 qusb_dci_transfer_request(qusb_urb_t* urb_ptr)
{
  ...
  uint8   ep=0;
  boolean dir=FALSE;
  trb_ctl_type_t trb_ctl=QUSB_TRB_CTL_NORMAL;
  if (NULL == qusb_dci_ctx)
  {
    qusb_error_log(DCI_CTX_NULL_LOG, 0, __LINE__);
    return (DCI_ERROR);
  ...
  if (ep == 0)
  {
    ...
    if(DIR_TX == dir)
    {
      if (urb_ptr->transfer_length > QUSB_MAX_CTRL_PKT_SIZE)
      {
        qusb_error_log(DCI_MEMORY_OUT_OF_BOUNDS_ERROR_LOG, 0, __LINE__);
        return(QUSB_ERR_BAD_PARAM);
      }
      memscpy(qusb_dci_ctx->control_tx_buffer, QUSB_MAX_CTRL_PKT_SIZE,
            urb_ptr->transfer_buffer_ptr, urb_ptr->transfer_length);

      urb_ptr->transfer_buffer_ptr = qusb_dci_ctx->control_tx_buffer;
    }
  }
  else
  {
  ...
  qusb_dci_ctx->trb_ring[ep][dir].urb_ptr = urb_ptr ;
  ...
  {
    qusb_dci_ctx->trb_ring[ep][dir].urb_ptr = NULL ;
...
uint32 qusb_dci_cancel_ep0(void)
{
  ...
  if(DCI_OK != qusb_dci_write_ep_cmd(0, DIR_RX, &ep_cmd, NULL))
  {
...
uint32 qusb_dci_cancel_unstall_requeue(uint8 ep, uint8 dir)
{

  qusb_ep_cmd_t ep_cmd;
  uint8 res_index;
  qusb_urb_t *urb_ptr;
  uint8 requeue_flag = 0;
  uint32 status = 0;
  ...
  if(NULL == urb_ptr)
  {
    qusb_bus_log(DCI_CANCEL_TRANSFER_URB_NULL_ERROR_LOG, 0, 0);
    requeue_flag = 0;
  }
  else
  {
    res_index = urb_ptr->transfer_index;
    qusb_bus_log(DCI_CANCEL_TRANSFER_LOG, 0, ep*10+dir);
    if((urb_ptr->transfer_status == QUSB_URB_STATUS_ACTIVE)&& (res_index != QUSB_RSC_IDX_NOT_SET))
    {
      memset(&ep_cmd, 0, sizeof(ep_cmd));
  ...
  /* Clear stall for any non-control halted EPs */
  status = qusb_dci_unstall_endpoint(ep, dir);
  if (status)
  {
    qusb_uart_log("unstall ep failed", status);
  }

  if (requeue_flag)
  {
    qusb_uart_log("resubmit urb ", (ep<<4) + dir);
    status = qusb_submit_urb(urb_ptr);
    if (status)
    {
      qusb_uart_log("resubmit urb failed", status);
    }
  }
...
uint32 qusb_dci_shutdown(void)
{
  ...
  {
    qusb_dci_write_evt_buff_cur_pos_cookie(qusb_dci_ctx->evt_buf_info.cur_pos);
  }
  qusb_dci_ctx_free();
  return(DCI_OK);
}
...
void qusb_dci_set_max_packet_size(uint32 max_packet_size)
{
  ...
  qusb_dci_ctx->dci_max_packet_size = max_packet_size;
  qusb_dci_ctx->dci_ep0_max_packet_size =( max_packet_size == 1024 ) ? QUSB_EP0_MAX_PACKET_SIZE_SUPER :
...
static uint32 qusb_dci_get_max_packet_size(void)
{
  ...
  qusb_log(DCI_GET_MAX_PACKET_SIZE_LOG, 0, qusb_dci_ctx->dci_max_packet_size);
  return qusb_dci_ctx->dci_max_packet_size;
}
...
boolean qusb_dci_skip_enumeration(void)
{
  ...
  qusb_log(DCI_SKIP_ENUMERATION_LOG,0,data);
...
// ===========================================================================
/**
 * @function    qusb_dci_retry_highspeed
 * 
 * @brief   This function will retry enumerate in HighSpeed again
 *  
 * @return  TRUE retry in Highspeed, FALSE no retry.
 * 
 */
// ===========================================================================
boolean qusb_dci_retry_highspeed(void)
{
  uint32 timeout_cnt =0;
  
  qusb_uart_log("qusb_dci_retry_highspeed ", 
            HWIO_USB3_PRI_PORTSC_30_REGS_p_PORTSC_30_INMI(0,HWIO_USB3_PRI_PORTSC_30_REGS_p_PORTSC_30_PLS_BMSK)>>HWIO_USB3_PRI_PORTSC_30_REGS_p_PORTSC_30_PLS_SHFT);

  qusb_dci_set_run_stop(FALSE);
    
  while ((timeout_cnt < 100) && (1 == HWIO_USB3_PRI_DSTS_INM(HWIO_USB3_PRI_DSTS_DEVCTRLHLT_BMSK)))
  {
    qusb_dci_delay_ms(1);
    timeout_cnt++;
  }
  HWIO_USB3_PRI_DCFG_OUTM(HWIO_USB3_PRI_DCFG_DEVSPD_BMSK, (QUSB_MAX_SPEED_HIGH << HWIO_USB3_PRI_DCFG_DEVSPD_SHFT));
  qusb_dci_set_run_stop(TRUE);
  return TRUE;
}
...
#define USB_MAX_MEM_HANDLE 32
static usb_mem_handle_t usb_mem_handle_pool[USB_MAX_MEM_HANDLE];
#define USB_MEM_ALIGN (16)
static uint32 usb_alloated_size = 0;

#ifdef USB_UNCACHED_GLOBAL
#define USB_MAX_MEM_POOL   0x800
static uint32 usb_mem_pool_idx =0;
static uint8 usb_mem_pool[USB_MAX_MEM_POOL];
...
{
  uint32 mem_handle_idx = 0;
  for (mem_handle_idx=0;mem_handle_idx<USB_MAX_MEM_HANDLE;mem_handle_idx++)
  {
    if (usb_mem_handle_pool[mem_handle_idx].ptr == mem_ptr)
    {
      if (TRUE == usb_mem_handle_pool[mem_handle_idx].is_active)
      {
        usb_mem_handle_pool[mem_handle_idx].is_active = FALSE;
        qusb_log(USB_MEM_FREE_SUCCESS_LOG, 0, (uint32)mem_ptr);
        usb_alloated_size -= usb_mem_handle_pool[mem_handle_idx].size;
      }
      else
      {
        qusb_log(USB_MEM_ALREADY_FREE_LOG, 0, (uint32)mem_ptr);
      }
      return TRUE;
    }
  }
  qusb_error_log(USB_MEM_FREE_FAIL_LOG, 0, (uint32)mem_ptr);  
  return FALSE;
}
...
boolean usb_dci_mem_allocate(void** mem_ptr, uint32 size)
{
  ...
  for (mem_handle_idx=0;mem_handle_idx<USB_MAX_MEM_HANDLE;mem_handle_idx++)
  {
    if ((usb_mem_handle_pool[mem_handle_idx].size > 0) && (FALSE == usb_mem_handle_pool[mem_handle_idx].is_active))
    {
      //find the old memory allocation chunk that is free
      if (usb_mem_handle_pool[mem_handle_idx].size ==size)
      {
        *mem_ptr = usb_mem_handle_pool[mem_handle_idx].ptr;
        usb_mem_handle_pool[mem_handle_idx].is_active = TRUE;
        memset(*mem_ptr, 0, size);
        qusb_log(USB_MEM_ALLOC_SUCCESS_REUSE_LOG, 0, (uint32)*mem_ptr);
        return TRUE;
      }
    }
    // find empty handle
    else if (0 == usb_mem_handle_pool[mem_handle_idx].size)
    {
      align = ((uint32)&usb_mem_pool[usb_mem_pool_idx])%USB_MEM_ALIGN;
      if (align)
      {
        usb_mem_pool_idx = usb_mem_pool_idx + (USB_MEM_ALIGN - align);
      }
      new_size_idx = usb_mem_pool_idx+size;
      if ((new_size_idx) >= USB_MAX_MEM_POOL)
      {
        qusb_error_log(USB_MEM_POOL_FULL_LOG, 0, new_size_idx);
        return FALSE;
      }
      *mem_ptr = &usb_mem_pool[usb_mem_pool_idx];
      memset(*mem_ptr, 0, size);
      usb_mem_pool_idx = new_size_idx;// += size;
      usb_mem_handle_pool[mem_handle_idx].is_active = TRUE;
      usb_mem_handle_pool[mem_handle_idx].size = size;
      usb_mem_handle_pool[mem_handle_idx].ptr = *mem_ptr;
      usb_alloated_size += usb_mem_handle_pool[mem_handle_idx].size;
      qusb_log(USB_MEM_ALLOC_SUCCESS_LOG, 0, (uint32)*mem_ptr);
      return TRUE;
    }
  }
  qusb_error_log(USB_MEM_HANDLE_POOL_FULL_LOG, 0, mem_handle_idx);  
  return FALSE;
}
...
boolean usb_dci_mem_free(void* mem_ptr)
{
  ...
  uint32 mem_handle_idx =0;
	for (mem_handle_idx=0;mem_handle_idx<USB_MAX_MEM_HANDLE;mem_handle_idx++)
  {
  	if (usb_mem_handle_pool[mem_handle_idx].ptr == mem_ptr || usb_mem_handle_pool[mem_handle_idx].align_ptr == mem_ptr)
  	{
      status = DALSYS_Free(usb_mem_handle_pool[mem_handle_idx].ptr);
      if (DAL_SUCCESS !=status)
      {
        qusb_error_log(USB_MEM_FREE_FAIL_LOG, 0, (uint32)(uintptr_t)mem_ptr);  
        return FALSE;
      }
      qusb_log(USB_MEM_FREE_SUCCESS_LOG, 0, (uint32)(uintptr_t)usb_mem_handle_pool[mem_handle_idx].ptr);  
      qusb_log(USB_MEM_FREE_SUCCESS_LOG, 0, (uint32)(uintptr_t)usb_mem_handle_pool[mem_handle_idx].align_ptr);  
      usb_alloated_size -= usb_mem_handle_pool[mem_handle_idx].size;

		  usb_mem_handle_pool[mem_handle_idx].is_active = FALSE;
  		usb_mem_handle_pool[mem_handle_idx].size = 0;
		  usb_mem_handle_pool[mem_handle_idx].ptr = NULL;
 		  usb_mem_handle_pool[mem_handle_idx].align_ptr = NULL;
      return TRUE;
  	}
	}
  qusb_log(USB_MEM_ALREADY_FREE_LOG, 0, (uint32)(uintptr_t)mem_ptr);  
	return FALSE;
}
...
boolean usb_dci_mem_allocate(void** mem_ptr, uint32 size)
{
  DALResult status = DAL_SUCCESS;
  uint32 mem_handle_idx =0;
  uint32 align_size = size+USB_MEM_ALIGN;
  uint32 align =0;

	qusb_log(USB_MEM_SIZE_LOG, 0, size);	
	status =DALSYS_MallocType(align_size, (void**)mem_ptr, DALHEAP_UNCACHED);
	if (DAL_SUCCESS !=status)
  {
      qusb_error_log(USB_MEM_ALLOC_FAIL_LOG, 0, status);  
    return FALSE;
  }
  memset(*mem_ptr, 0, align_size);
	for (mem_handle_idx=0;mem_handle_idx<USB_MAX_MEM_HANDLE;mem_handle_idx++)
  {
  	if (usb_mem_handle_pool[mem_handle_idx].is_active == FALSE)
  	{
		  usb_mem_handle_pool[mem_handle_idx].is_active = TRUE;
  		usb_mem_handle_pool[mem_handle_idx].size = size;
		  usb_mem_handle_pool[mem_handle_idx].ptr = *mem_ptr;
  ...
  if (mem_handle_idx == USB_MAX_MEM_HANDLE)
  {
    qusb_error_log(USB_MEM_ALLOC_FAIL_LOG, 0, mem_handle_idx);  
    return FALSE;
  }
  qusb_log(USB_MEM_ALLOC_SUCCESS_LOG, 0, (uint32)(uintptr_t)*mem_ptr);  

  align = ((uint32)(uintptr_t)*mem_ptr)%USB_MEM_ALIGN;
  if (align)
  {
    *mem_ptr = *mem_ptr + (USB_MEM_ALIGN - align);
  }
  usb_mem_handle_pool[mem_handle_idx].align_ptr= *mem_ptr;
  qusb_log(USB_MEM_ALLOC_SUCCESS_LOG, 0, (uint32)(uintptr_t)*mem_ptr);  
    return TRUE;
}
...
{
  uint32 mem_handle_idx = 0;
  qusb_error_log(USB_MEM_POOL_IDX_LOG, 0, usb_alloated_size);
  for (mem_handle_idx=0;mem_handle_idx<USB_MAX_MEM_HANDLE;mem_handle_idx++)
  {
    if (0 == usb_mem_handle_pool[mem_handle_idx].size)
    {
      return;
    }
    qusb_log(USB_MEM_IDX_LOG, 0, mem_handle_idx);
    qusb_log(USB_MEM_PTR_LOG, 0, (uint32)(uintptr_t)usb_mem_handle_pool[mem_handle_idx].ptr);
    qusb_log(USB_MEM_SIZE_LOG, 0, usb_mem_handle_pool[mem_handle_idx].size);
    qusb_log(USB_MEM_IS_ACTIVE_LOG, 0, usb_mem_handle_pool[mem_handle_idx].is_active);
  }
}
...
boolean usb_dci_mem_handle_free(void)
{
  ...
  for (mem_handle_idx =0; mem_handle_idx < USB_MAX_MEM_HANDLE; mem_handle_idx++)
  {
    if (NULL != usb_mem_handle_pool[mem_handle_idx].ptr)
    {
      if (usb_dci_mem_free(usb_mem_handle_pool[mem_handle_idx].ptr))
      {
        qusb_error_log(USB_MEM_HANDLE_FREE_FAIL_LOG, 0, (uint32)(uintptr_t)usb_mem_handle_pool[mem_handle_idx].ptr);  
        return FALSE;
      }
      usb_mem_handle_pool[mem_handle_idx].ptr = NULL;
    }
  ...
  if (mem_handle_idx == USB_MAX_MEM_HANDLE)
  {
    qusb_error_log(USB_MEM_HANDLE_FREE_FAIL_LOG, 0, mem_handle_idx);  
    return FALSE;
  }
  qusb_error_log(USB_MEM_HANDLE_FREE_SUCCESS_LOG, 0, (uint32)(uintptr_t)usb_mem_handle_pool[mem_handle_idx].ptr);  
  return TRUE;
}
