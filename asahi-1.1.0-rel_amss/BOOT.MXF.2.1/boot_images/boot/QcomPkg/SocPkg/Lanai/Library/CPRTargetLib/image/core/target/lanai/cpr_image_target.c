 ...
 * $Header: //components/dev/core.boot/6.1/cvelempa.core.boot.6.1.Lanai_XBL_CPR_Changes_07_04_2022/QcomPkg/SocPkg/lanai/Library/CPRTargetLib/image/core/target/lanai/cpr_image_target.c#1 $
 * $Change: 38231469 $
 */
...
const cpr_settings* cpr_image_target_get_settings(void)
{
    ...
    CPR_ASSERT(cpr_settings_lanai);

    cpr_settings_lanai->railsCount = 2;
    cpr_settings_lanai->enablementsCount = 2;
    cpr_settings_lanai->controllersCount = 2;

    cpr_settings_lanai->railVVPs = (cpr_versioned_voltage_plan **) \
        cpr_image_malloc(sizeof(cpr_versioned_voltage_plan *) * \
                cpr_settings_lanai->railsCount);
    for (i = 0; i < cpr_settings_lanai->railsCount; i++) {
        cpr_settings_lanai->railVVPs[i] = (cpr_versioned_voltage_plan *) \
            cpr_image_malloc(sizeof(cpr_versioned_voltage_plan));
    }

    cpr_settings_lanai->controllers = (cpr_hal_controller **) \
        cpr_image_malloc(sizeof(cpr_hal_controller *) * \
                cpr_settings_lanai->controllersCount);
    for (i = 0; i < cpr_settings_lanai->controllersCount; i++) {
        cpr_settings_lanai->controllers[i] = (cpr_hal_controller *) \
            cpr_image_malloc(sizeof(cpr_hal_controller));
    }

    cpr_settings_lanai->rails = (cpr_rail **) cpr_image_malloc(
            sizeof(cpr_rail *) * cpr_settings_lanai->railsCount);
    for (i = 0; i < cpr_settings_lanai->railsCount; i++) {
        cpr_settings_lanai->rails[i] =
            (cpr_rail *) cpr_image_malloc(sizeof(cpr_rail));
    }

    cpr_settings_lanai->enablements = (cpr_enablement **) \
        cpr_image_malloc(sizeof(cpr_enablement *) * \
                cpr_settings_lanai->enablementsCount);
    for (i = 0; i < cpr_settings_lanai->enablementsCount; i++) {
        cpr_settings_lanai->enablements[i] =
            (cpr_enablement *)cpr_image_malloc(sizeof(cpr_enablement));
    }

    cpr_settings_lanai->miscCfg = (cpr_misc_cfg *)
        cpr_image_malloc(sizeof(cpr_misc_cfg));

    cpr_rail gfx_rail;
    gfx_rail.id = CPR_RAIL_GFX;
    cpr_smem_deserialize_settings(&gfx_rail, cpr_settings_lanai, /*idx*/ 0);

    cpr_rail mm_rail;
    mm_rail.id = CPR_RAIL_MM;
    cpr_smem_deserialize_settings(&mm_rail, cpr_settings_lanai, /*idx*/ 1);

    return cpr_settings_lanai;
}
