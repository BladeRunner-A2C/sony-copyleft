...
/*=============================================================================
 ...
 --------     ---     -----------------------------------------------------------
 08/21/23     SN      Do not vote PMIC resources during boot.
 05/12/23     SN      PIL spss region change to align with v5 memory map in lanai.
 02/16/23     eb      Add Support for sharing Keymint info with SPU
...
extern EFI_STATUS keymint_info_install_protocol_interface();
...
extern uint64_t pil_region_start_addr;
extern uint64_t pil_region_size;
/*===========================================================================*/
...
)
{
  ...
  { 
    pil_region_start_addr = spss_cfg.ResvRegionStart;
    pil_region_size = spss_cfg.ResvRegionSize;

    /* Now trigger PilDXE protocol to load SP subsystem */
    Status = SPSSLib_LoadSPSS(&spss_cfg);

    if (Status == EFI_SUCCESS) {
      /* Install protocol only if SPSS was loaded successfully*/
      Status = keymint_info_install_protocol_interface();
      if (Status != EFI_SUCCESS) {
        DEBUG((EFI_D_ERROR,"SPSSDxe:: %a - SPSSDxe_InstallProtocolInterface() failed, status = %r \r\n", __FUNCTION__, Status ));
        return Status;
      }
    }
  }
                                        ...
                                        BOOLEAN IsChipVersionRelevant )
{
  ...
  if (chipFamily == EFICHIPINFO_FAMILY_LANAI)
  {
    spss_cfg->ResvRegionStart = 0x9B0A0000;  /* Lanai */
    spss_cfg->ResvRegionSize = 0x1E0000;
  }
  else if (chipFamily == EFICHIPINFO_FAMILY_KAILUA)
  {
    spss_cfg->ResvRegionStart = 0x9B100000;  /* Kailua */
...
STATIC VOID SPSSLib_VotePMICResources ( VOID )
{
  ...
  npa_client_handle gpio_sp = NULL; /*  for generic GPIOSP */

  vreg_sp =  npa_create_sync_client("/pm/vreg.sp/en", "vreg_sp", NPA_CLIENT_REQUIRED);
  gpio_sp =  npa_create_sync_client("/pm/gpio.sp/en", "gpio_sp", NPA_CLIENT_REQUIRED);
  if (vreg_sp != NULL)
  {
  ...
  if (gpio_sp != NULL)
  {
    npa_issue_required_request(gpio_sp, 1); /* enable vote */
  }
#endif
  return;
