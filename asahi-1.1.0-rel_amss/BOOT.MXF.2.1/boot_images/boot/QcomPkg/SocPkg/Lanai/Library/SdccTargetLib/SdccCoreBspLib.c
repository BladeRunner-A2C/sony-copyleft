   ...
   switch(slot_num)
   {
      case SDCC_BSP_SLOT_1:
         ...
         if (NULL != DtbProtocol) 
         {
            ...
            {
               snprintf(FdtNodeStr, SDCC_BSP_DT_PATH_LEN, "/soc/storage/sdc%d/HsrSettings/SDR104", slot_num);
            }
            else if (SDCC_SDR50_MODE == speed_mode) 
            {
               snprintf(FdtNodeStr, SDCC_BSP_DT_PATH_LEN, "/soc/storage/sdc%d/HsrSettings/SDR50", slot_num);
            }
            else 
            {
...
{ 
   ...
   if (driveno == SDCC_BSP_SLOT_1) 
   {
      ...
      {
         vccb_ls_sdc_mV_handle = npa_create_sync_client("/pm/ldob8/mV", "vccb_ls_sdc", NPA_CLIENT_REQUIRED);
      }

      if (vccb_ls_sdc_en_handle == NULL) 
      {
         vccb_ls_sdc_en_handle = npa_create_sync_client("/pm/ldob8/en", "vccb_ls_sdc", NPA_CLIENT_REQUIRED);
      }

      if (vccb_ls_sdc_mode_handle == NULL) 
      {
         vccb_ls_sdc_mode_handle = npa_create_sync_client("/pm/ldob8/mode", "vccb_ls_sdc", NPA_CLIENT_REQUIRED);
      }
...
sdcc_bsp_err_type sdcc_bsp_volt_set (UINT32 driveno, sdcc_bsp_regulator_mode mode)
{
   ...
   if (driveno == SDCC_BSP_SLOT_1)
   {
      ...
      {
         sd_mmc_vdd_mV_handle = npa_create_sync_client("/pm/ldob9/mV", "sd_mmc_vdd", NPA_CLIENT_REQUIRED);
      }

      if (sd_mmc_vdd_en_handle == NULL) 
      {
         sd_mmc_vdd_en_handle = npa_create_sync_client("/pm/ldob9/en", "sd_mmc_vdd", NPA_CLIENT_REQUIRED);
      }

      if (sd_mmc_vdd_mode_handle == NULL) 
      {
         sd_mmc_vdd_mode_handle = npa_create_sync_client("/pm/ldob9/mode", "sd_mmc_vdd", NPA_CLIENT_REQUIRED);
      }
      ...
      {
         vcca_ls_sdc_mV_handle = npa_create_sync_client("/pm/ldoc3/mV", "vcca_ls_sdc", NPA_CLIENT_REQUIRED);
      }

      if (vcca_ls_sdc_en_handle == NULL) 
      {
         vcca_ls_sdc_en_handle = npa_create_sync_client("/pm/ldoc3/en", "vcca_ls_sdc", NPA_CLIENT_REQUIRED);
      }

      if (vcca_ls_sdc_mode_handle == NULL) 
      {
         vcca_ls_sdc_mode_handle = npa_create_sync_client("/pm/ldoc3/mode", "vcca_ls_sdc", NPA_CLIENT_REQUIRED);
      }
