...
{ 
   ...
   {
      snprintf(fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/soc/storage/sdc%d", slot_num);
                                             ...
                                             sdcc_bsp_hsr_settings_type *hsr_settings)
{
   ...
   switch(slot_num)
   {
      case SDCC_BSP_SLOT_1:
         ...
         snprintf(fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/soc/storage/sdc%d/HsrSettings", slot_num);
         ...
         {
            snprintf(fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/soc/storage/sdc%d/HsrSettings/SDR104", slot_num);
         }
         else if (SDCC_SDR50_MODE == speed_mode)
         {
            snprintf(fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/soc/storage/sdc%d/HsrSettings/SDR50", slot_num);
         }
         else
         {
      ...
      case SDCC_BSP_SLOT_1:
         ...
         if (FALSE == dtError)
         {
            ...
            if ( FDT_ERR_QC_NOERROR == fdt_get_node_handle (&node, NULL, fdt_node_str))
		    {
                ...
                if (FDT_ERR_QC_NOERROR == fdt_get_uint32_prop (&node, "DriveStrength", &DriveStrength))
                {
#ifndef FEATURE_DISABLE_DT_LOGGING
                   if (enable_dt_logging) 
                   {
...
sdcc_bsp_err_type sdcc_bsp_lower_volt_level (UINT32 driveno)
{
   if (SDCC_BSP_SLOT_1 == driveno)
   {
      ...
      if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_B, PM_LDO_8, 1800 * 1000, TRUE))
      {
   ...
   switch(driveno)
   {
      case SDCC_BSP_SLOT_1:
         ...
         else
         {
            if (FALSE == dtError)
            {
               snprintf (fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/soc/storage/sdc%d", driveno); 
...
sdcc_bsp_err_type sdcc_bsp_volt_set (UINT32 driveno, sdcc_bsp_regulator_mode mode)
{
   ...
   if (SDCC_BSP_SLOT_1 == driveno)
   {
      if (SDCC_BSP_REGULATORS_ON == mode)
      {
         ...
         if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_C, PM_LDO_3, 1200 * 1000, TRUE))
         {
            return SDCC_BSP_ERR_PMIC; 
         }

         if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_mode(PMIC_B, PM_LDO_9, PM_SW_MODE_NPM))
         {
            return SDCC_BSP_ERR_PMIC; 
         }

         if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_B, PM_LDO_9, PM_ON, TRUE))
         {
            return SDCC_BSP_ERR_PMIC; 
         }

         if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_B, PM_LDO_9, 2960 * 1000, TRUE))
         {
            return SDCC_BSP_ERR_PMIC; 
         }
         ...
         if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_B, PM_LDO_8, 2960 * 1000, TRUE))
         {
            return SDCC_BSP_ERR_PMIC; 
         }
      }
      ...
      {
         if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_B, PM_LDO_8, PM_OFF, TRUE))
         {
            return SDCC_BSP_ERR_PMIC; 
         }

         if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_B, PM_LDO_9, PM_OFF, TRUE))
         {
            return SDCC_BSP_ERR_PMIC; 
         }

         if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_C, PM_LDO_3, PM_OFF, TRUE))
         {
            return SDCC_BSP_ERR_PMIC; 
         }
      }
