...
void CaptureBootButtonsState(VOID)
{
   ...
   if ((ButtonDtGpioCfg != NULL) && (ButtonDtGpioCfg->ButtonGpioCfg != NULL) &&
      (ButtonDtGpioCfg->ButtonGpioCfgCount > 0))
   {
      GpioCfg.PmicIndex = ButtonDtGpioCfg->ButtonGpioCfg[VOL_DOWN_BTN_IDX * BTN_DT_GPIO_CONFIG_ITEM_SIZE + BTN_DT_PMIC_IDX];
      GpioCfg.GpioNumber = ButtonDtGpioCfg->ButtonGpioCfg[VOL_DOWN_BTN_IDX * BTN_DT_GPIO_CONFIG_ITEM_SIZE + BTN_DT_GPIO_NUM_IDX];
