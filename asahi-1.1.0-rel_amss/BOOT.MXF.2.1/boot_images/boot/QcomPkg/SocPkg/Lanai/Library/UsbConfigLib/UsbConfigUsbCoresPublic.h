...
/**
Polls Super Speed PHY training to allow maximum speed detect to 
Super Speed Plug

@param [in]  This      The USB core interface instance

@retval EFI_SUCCESS       Poll was sucessful
@retval EFI_UNSUPPORTED   Poll is not supported
@retval Others            Error encountered while polling
**/
typedef
EFI_STATUS
(*QCOM_USB_CORE_POLL_SS_PHY_TRAINING) (
  IN QCOM_USB_CORE_INTERFACE              *This
  );

/**
Advances Super Speed TX compliance pattern signal (CP0, CP1, CP2, etc.)
...
typedef
EFI_STATUS
(*QCOM_USB_CORE_ADVANCE_SS_CMPL_PATTERN) (
  IN QCOM_USB_CORE_INTERFACE              *This
  );
...
struct _QCOM_USB_CORE_INTERFACE {
  ...
  QCOM_USB_CORE_POLL_SS_PHY_TRAINING        PollSSPhyTraining;
  QCOM_USB_CORE_ADVANCE_SS_CMPL_PATTERN     AdvanceSSCmplPattern;
};
...
/**
Function used to hold USB2PHY of given core in reset state.

@param [in]  CoreNum         USB coreNum.
...
/**
Function used to turn off NPA resources for all the cores initialized during the
core initialization.

@param [in]  Index         USB coreNum.
