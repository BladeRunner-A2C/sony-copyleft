...
ufs_bsp_err_type ufs_bsp_volt_set(uint8_t hostid, ufs_bsp_regulator_mode mode)
{
   ...
   if (hostid == 0) {
      ...
      {
         vcc_mV_client_handle = npa_create_sync_client("/pm/ldob17/mV", "ufs0_vcc", NPA_CLIENT_REQUIRED); 
         if (vcc_mV_client_handle == NULL) {
      ...
      {
         vcc_en_client_handle = npa_create_sync_client("/pm/ldob17/en", "ufs0_vcc", NPA_CLIENT_REQUIRED); 
         if (vcc_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      ...
      {
         vcc_mode_client_handle = npa_create_sync_client("/pm/ldob17/mode", "ufs0_vcc", NPA_CLIENT_REQUIRED); 
         if (vcc_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      ...
      if (vccq_mV_client_handle == NULL) {
         vccq_mV_client_handle = npa_create_sync_client("/pm/ldoc1/mV", "ufs0_vccq", NPA_CLIENT_REQUIRED); 
         if (vccq_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vccq_en_client_handle == NULL) {
         vccq_en_client_handle = npa_create_sync_client("/pm/ldoc1/en", "ufs0_vccq", NPA_CLIENT_REQUIRED); 
         if (vccq_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vccq_mode_client_handle == NULL) {
         vccq_mode_client_handle = npa_create_sync_client("/pm/ldoc1/mode", "ufs0_vccq", NPA_CLIENT_REQUIRED); 
         if (vccq_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      ...
      if (vdda_mV_client_handle == NULL) {
         vdda_mV_client_handle = npa_create_sync_client("/pm/ldoi3/mV", "ufs0_vdda", NPA_CLIENT_REQUIRED); 
         if (vdda_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vdda_en_client_handle == NULL) {
         vdda_en_client_handle = npa_create_sync_client("/pm/ldoi3/en", "ufs0_vdda", NPA_CLIENT_REQUIRED); 
         if (vdda_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vdda_mode_client_handle == NULL) {
         vdda_mode_client_handle = npa_create_sync_client("/pm/ldoi3/mode", "ufs0_vdda", NPA_CLIENT_REQUIRED); 
         if (vdda_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      ...
      if (qref_mV_client_handle == NULL) {
           qref_mV_client_handle = npa_create_sync_client("/pm/ldoi1/mV", "ufs0_qref", NPA_CLIENT_REQUIRED); 
         if (qref_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (qref_en_client_handle == NULL) {
         qref_en_client_handle = npa_create_sync_client("/pm/ldoi1/en", "ufs0_qref", NPA_CLIENT_REQUIRED); 
         if (qref_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (qref_mode_client_handle == NULL) {
         qref_mode_client_handle = npa_create_sync_client("/pm/ldoi1/mode", "ufs0_qref", NPA_CLIENT_REQUIRED); 
         if (qref_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
