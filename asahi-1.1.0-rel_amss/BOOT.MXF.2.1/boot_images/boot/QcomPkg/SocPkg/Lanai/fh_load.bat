...
if /I %TEST_MODE%=="FALSE" (
  ...
  if /I "%VARIANT%"=="LAA" (
    ...
    \\stone\aswcrm\smart\nt\bin\findbuild.exe -in=location -la -ap BOOT.MXF.2.1-?????-LANAI-? > %TEMP%\boot_build.log
...
if /I %SEND_LITE%=="TRUE" (
  ...
  %boot_build_path%\boot_images\boot_tools\QSaharaServer\QSaharaServer.exe -b %boot_build_path%\boot_images\boot\QcomPkg\SocPkg\Lanai\Bin\%VARIANT%\DEBUG\ -s 13:xbl_s_devprg_ns.melf -p \\.\%port_number%
) 

if /I %SEND_LITE%=="FALSE" (
  echo Send device programmer DDR ...
  %boot_build_path%\boot_images\boot_tools\QSaharaServer\QSaharaServer.exe -v 3 -b %boot_build_path%\boot_images\boot\QcomPkg\SocPkg\Lanai\Bin\%VARIANT%\DEBUG\ -s 13:xbl_s_devprg_ns.melf -p \\.\%port_number%
)
...
if /I %FLASH_MODE%=="FALSE" (
  if /I "%VARIANT%"=="LAA" (
    ...
    \\stone\aswcrm\smart\nt\bin\findbuild.exe -in=location -la -ap Lanai.LA.1.0-?????-STD.INT-? > %TEMP%\meta_build.log
  ) else if /I "%VARIANT%"=="LAB" (
    echo Findbuild meta for variant [%VARIANT%]
    \\stone\aswcrm\smart\nt\bin\findbuild.exe -in=location -la -ap Lanai.LA.0.1-?????-STD.INT-? > %TEMP%\meta_build.log
  )
