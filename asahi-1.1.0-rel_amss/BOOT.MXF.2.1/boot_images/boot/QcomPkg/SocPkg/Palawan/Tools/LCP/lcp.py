...
LCPLibTargetSettings = {
    "SM8635" : {
        "ProfilePkg" : os.path.join("profiles","palawan","palawan_tme_security_profile.xml"),
        "OSPathDSFPkg" : os.path.join("SocPkg","Palawan"),
    },
    "SM7675" : {
        "ProfilePkg" : os.path.join("profiles","palawan","palawan_tme_security_profile.xml"),
        "OSPathDSFPkg" : os.path.join("SocPkg","Palawan"),
...
def main():
    ...
    r'   python <boot_build_path>\boot_images\boot\QcomPkg\SocPkg\Palawan\Tools\LCP\lcp.py --target=SM8635 -v DEBUG -s <meta_build_path>\common\sectoolsv2\ext -o <absolute_path>\output in -l <absolute_path>\LCP.bin -x <absolute_path>\xbl_config.elf'
    '\r\n' +
    '2, example to update DCB bins to include LCP bin:\r\n' +
    r'   python <boot_build_path>\boot_images\boot\QcomPkg\SocPkg\Palawan\Tools\LCP\lcp.py --target=SM8635 -v DEBUG -o <absolute_path>\output  in -l <absolute_path>\LCP.bin -x <absolute_path>\<dcb_bin_path>'
    '\r\n' +
    '3, example to convert lcp josn file to LCP region bin file:\r\n' +
    r'   python <boot_build_path>\boot_images\boot\QcomPkg\SocPkg\Palawan\Tools\LCP\lcp.py --target=SM8635 -v DEBUG -o <absolute_path>\output convert -d <absolute_path>\.dcb_config_SM8635.json -l <absolute_path>\SM8635_LCP_region.json'
    '\r\n' +
    '4, example to convert lcp bin file to LCP region json file:\r\n' +
    r'   python <boot_build_path>\boot_images\boot\QcomPkg\SocPkg\Palawan\Tools\LCP\lcp.py --target=SM8635 -v DEBUG -o <absolute_path>\output convert -d <absolute_path>\.dcb_config_SM8635.json -l <absolute_path>\SM8635_LCP_region.bin')
    ...
    parser_convert.add_argument("-d", "--lcp_define_file", required=True, help="lcp define file, .dcb_config_SM8635.json")
