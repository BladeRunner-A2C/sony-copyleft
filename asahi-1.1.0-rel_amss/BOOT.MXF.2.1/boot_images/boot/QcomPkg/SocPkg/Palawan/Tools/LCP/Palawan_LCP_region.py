...
config = {
            ...
            [
                ...
                },
                {
                    ...
                    'regionData': {
                        ...
                        'size': 0, # Size 28672KB, this region size can be edited by user
                        'attr': 0, # DARE type, region attributes(bit 0 - Encrypt, bit 1 - ECC, bit 2 - MTE, bit 3 - Meta Deta, bit 4 - DARE)
                        'arc_en': 0, # 1 --> DARE with ANTI-REPLY COUNTER, 0 --> DAR w/o ANTI-REPLY COUNTER
                    }
                },
                {
                ...
                },
                {
                    ...
                    'name': "TUI VM",
                    'regionData': {
                        'start': 0,  # 0xF380, this region start address can be edited by user
                        'size': 0, #Size 69632KB, this region size can be edited by user
                        'attr': 0, # Encrypt Type, region attributes(bit 0 - Encrypt, bit 1 - ECC, bit 2 - MTE, bit 3 - Meta Deta, bit 4 - DARE)
                        'arc_en': 0, # 1 --> DARE with ANTI-REPLY COUNTER, 0 --> DAR w/o ANTI-REPLY COUNTER
                    }
                },
                {
                    ...
                    'name': "OEM VM",
                    'regionData': {
                        'start': 0, # 0xF7C0, this region start address can be edited by user
                        'size': 0, #Size 77824KB, this region size can be edited by user
                        'attr': 0, #  Encrypt Type, region attributes(bit 0 - Encrypt, bit 1 - ECC, bit 2 - MTE, bit 3 - Meta Deta, bit 4 - DARE)
                        'arc_en': 0, # 1 --> DARE with ANTI-REPLY COUNTER, 0 --> DAR w/o ANTI-REPLY COUNTER
                    }
                },
    ...
    'configName': '.dcb_config_SM8635.json',
    'description':'DCB LCP entry file',
    'name': 'SM8635_LCP_region.json',
    'platform': 'v6',
    'platformCode': '6',
    'version': '1.0',
}
...
def main():
    ...
    r'   python <boot_build_path>\boot_images\boot\QcomPkg\SocPkg\Palawan\Tools\LCP\Palawan_LCP_region.py --target=SM8635 -o <absolute_path>\output -d <absolute_path>\.dcb_config_SM8635.json ')
    ...
    parser.add_argument("-d", "--lcp_define_file", required=True, help="lcp define file, .dcb_config_SM8635.json")
