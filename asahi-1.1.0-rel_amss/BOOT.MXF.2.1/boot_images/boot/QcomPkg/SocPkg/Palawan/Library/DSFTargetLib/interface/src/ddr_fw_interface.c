...
--------   ---       --------------------------------------------------------
01/21/22   mkma      added HAL_DDR_FW_SDI_Get_Version api
...
{
    return fptr_HAL_DDR_FW_SDI_Get_Version(dsf_version);
}
...
{
    return fptr_HAL_DDR_FW_eCDT_Get_Voltage_Override_Values(clk_idx, buffer_ptr, size);
}
...
{
    return fptr_HAL_DDR_FW_MC_MR_Read(channel, rank, address);
}
...
{
    return fptr_HAL_DDR_FW_IOCTL(cmd, arg, size);
}
