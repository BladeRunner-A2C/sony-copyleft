...
#endif    
#define LOGE(fmt, args...) DEBUG((EFI_D_INFO, "SPSSDxe [%a] E: "fmt"\r\n", __FUNCTION__, ##args));
...
// "asym","crypt","keym"
#define MAX_QCOM_UEFI_APPS  3

// Names unknown, we name them uefi0, uefi1, uefi2 ..
#define MAX_3RD_PARTY_UEFI_APPS  (MAX_SPU_UEFI_APPS-MAX_QCOM_UEFI_APPS)
...
static char m_app_names[MAX_QCOM_UEFI_APPS][MAX_UEFI_FILE_NAME] = {"asym","crypt","keym"};
...
EFI_STATUS spss_iar_main(PIL_SUBSYS_CFG_TYPE* spss_cfg)
{
    ...
    if (m_iar_state == IAR_STATE_ACTIVATED) {
        ...
        AsciiStrCatS(full_name_path, sizeof(full_name_path), ".b02");
