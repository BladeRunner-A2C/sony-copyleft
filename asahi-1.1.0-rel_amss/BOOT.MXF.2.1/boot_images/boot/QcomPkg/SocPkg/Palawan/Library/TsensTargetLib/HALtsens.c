/*============================================================================
  ...
  DEPENDENCIES: None
                ...
                Copyright (c) 2014-2019, 2022, 2023 Qualcomm Technologies, Inc.
                All Rights Reserved.
 ...
 * -------------------------------------------------------------------------*/
#define TSENS_HW_VER(major,minor,step) ( ((major) << 28) | ((minor) << 16) | (step) )

/* For use with array-style HWIO macros */
 ...
 * -------------------------------------------------------------------------*/
 static volatile boolean TsensEnable = TRUE;
...
{
   HWIO_OUTXV(pucTsensSROTAddr, TSENS_CTRL, PSHOLD_ARES_MODE_LEVEL, PULSE);
...
/* ============================================================================
**
**  HAL_tsens_srot_SetZeroCParameters
**
**  Description:
**    Configures the zero degree C threshold parameters
**
**  Parameters:
**    pucTsensSROTAddr - address of controller's SROT region
**    uChannelEnMask - which channels to monitor
**    nSetVal - set threshold
**    nResetVal - reset threshold
**
**  Return: None
**
**  Dependencies: None
**
** ========================================================================= */
void HAL_tsens_srot_SetZeroCParameters(uint8 *pucTsensSROTAddr, uint32 uChannelEnMask, int32 nSetVal, int32 nResetVal)
{
   uint32 uCtrl = HWIO_INX(pucTsensSROTAddr, TSENS_0C_CTRL);
   uint32 uThresh = HWIO_INX(pucTsensSROTAddr, TSENS_0C_THRESHOLDS);

   uCtrl &= ~(HWIO_FMSK(TSENS_0C_CTRL, TSENS_0C_SENSOR_EN) |
              HWIO_FMSK(TSENS_0C_CTRL, TSENS_0C_INTERRUPT_EN));

   if (uChannelEnMask != 0)
   {
      uThresh &= ~(HWIO_FMSK(TSENS_0C_THRESHOLDS, THRESHOLD_SET) |
                   HWIO_FMSK(TSENS_0C_THRESHOLDS, THRESHOLD_RESET));
      uThresh |= HWIO_FVAL(TSENS_0C_THRESHOLDS, THRESHOLD_SET, nSetVal);
      uThresh |= HWIO_FVAL(TSENS_0C_THRESHOLDS, THRESHOLD_RESET, nResetVal);
      HWIO_OUTX(pucTsensSROTAddr, TSENS_0C_THRESHOLDS, uThresh);

      uCtrl |= HWIO_FVAL(TSENS_0C_CTRL, TSENS_0C_SENSOR_EN, uChannelEnMask);
      uCtrl |= HWIO_FVAL(TSENS_0C_CTRL, TSENS_0C_INTERRUPT_EN, 1);
   }

   HWIO_OUTX(pucTsensSROTAddr, TSENS_0C_CTRL , uCtrl);
}
