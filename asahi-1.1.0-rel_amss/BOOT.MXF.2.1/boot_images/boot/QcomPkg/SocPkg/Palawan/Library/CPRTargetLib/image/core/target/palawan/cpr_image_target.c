 ...
 * $Header: //components/dev/core.boot/6.1/cvelempa.core.boot.6.1.palawan_XBL_CPR_Changes_07_04_2022/QcomPkg/SocPkg/palawan/Library/CPRTargetLib/image/core/target/palawan/cpr_image_target.c#1 $
 * $Change: 38231469 $
 */
...
const cpr_settings* cpr_image_target_get_settings(void)
{
    ...
    CPR_ASSERT(cpr_settings_palawan);

    cpr_settings_palawan->railsCount = 2;
    cpr_settings_palawan->enablementsCount = 2;
    cpr_settings_palawan->controllersCount = 2;

    cpr_settings_palawan->railVVPs = (cpr_versioned_voltage_plan **) \
        cpr_image_malloc(sizeof(cpr_versioned_voltage_plan *) * \
                cpr_settings_palawan->railsCount);
    for (i = 0; i < cpr_settings_palawan->railsCount; i++) {
        cpr_settings_palawan->railVVPs[i] = (cpr_versioned_voltage_plan *) \
            cpr_image_malloc(sizeof(cpr_versioned_voltage_plan));
    }

    cpr_settings_palawan->controllers = (cpr_hal_controller **) \
        cpr_image_malloc(sizeof(cpr_hal_controller *) * \
                cpr_settings_palawan->controllersCount);
    for (i = 0; i < cpr_settings_palawan->controllersCount; i++) {
        cpr_settings_palawan->controllers[i] = (cpr_hal_controller *) \
            cpr_image_malloc(sizeof(cpr_hal_controller));
    }

    cpr_settings_palawan->rails = (cpr_rail **) cpr_image_malloc(
            sizeof(cpr_rail *) * cpr_settings_palawan->railsCount);
    for (i = 0; i < cpr_settings_palawan->railsCount; i++) {
        cpr_settings_palawan->rails[i] =
            (cpr_rail *) cpr_image_malloc(sizeof(cpr_rail));
    }

    cpr_settings_palawan->enablements = (cpr_enablement **) \
        cpr_image_malloc(sizeof(cpr_enablement *) * \
                cpr_settings_palawan->enablementsCount);
    for (i = 0; i < cpr_settings_palawan->enablementsCount; i++) {
        cpr_settings_palawan->enablements[i] =
            (cpr_enablement *)cpr_image_malloc(sizeof(cpr_enablement));
    }

    cpr_settings_palawan->miscCfg = (cpr_misc_cfg *)
        cpr_image_malloc(sizeof(cpr_misc_cfg));

    cpr_rail gfx_rail;
    gfx_rail.id = CPR_RAIL_GFX;
    cpr_smem_deserialize_settings(&gfx_rail, cpr_settings_palawan, /*idx*/ 0);

    cpr_rail mm_rail;
    mm_rail.id = CPR_RAIL_MM;
    cpr_smem_deserialize_settings(&mm_rail, cpr_settings_palawan, /*idx*/ 1);

    return cpr_settings_palawan;
}
...
void cpr_image_target_cpr_to_arc_domain(cpr_domain_id railId, char **res_name)
{
    switch(railId)
    {
        ...
        case CPR_RAIL_NSP2   : *res_name = "nsp2.lvl";    break;
        default:
...
rail_voltage_level cpr_image_target_cpr_mode_to_vlvl(cpr_voltage_mode mode)
{
    switch (mode)
    {
		...
		case CPR_VOLTAGE_MODE_LOW_SVS_D2 : return  RAIL_VOLTAGE_LEVEL_LOW_SVS_D2;
        case CPR_VOLTAGE_MODE_LOW_SVS_D1 : return  RAIL_VOLTAGE_LEVEL_LOW_SVS_D1;
		case CPR_VOLTAGE_MODE_LOW_SVS_D0 : return  RAIL_VOLTAGE_LEVEL_LOW_SVS_D0;
        ...
        case CPR_VOLTAGE_MODE_TURBO_L0   : return  RAIL_VOLTAGE_LEVEL_TUR_L0;
		case CPR_VOLTAGE_MODE_TURBO_L1   : return  RAIL_VOLTAGE_LEVEL_TUR_L1;
		case CPR_VOLTAGE_MODE_TURBO_L2   : return  RAIL_VOLTAGE_LEVEL_TUR_L2;
		case CPR_VOLTAGE_MODE_TURBO_L3   : return  RAIL_VOLTAGE_LEVEL_TUR_L3;
        default:
