...
MDP_Status Panel_LCD_PowerUp(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  else
  {
    ...
    */
     ...
     if (EFI_SUCCESS != PmicGpioProtocol->CfgMode(PMIC_D, EFI_PM_GPIO_8, EFI_PM_GPIO_DIG_OUT))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Config Backlight enable GPIO failed!\n"));
    }
    if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_D, EFI_PM_GPIO_8, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set output for Backlight enable GPIO failed!\n"));
    }
    if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_D, EFI_PM_GPIO_8, EFI_PM_GPIO_OUT_DRV_STR_MEDIUM))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set drive strength for Backlight enable GPIO failed!\n"));
    }
    if (EFI_SUCCESS != PmicGpioProtocol->SetVoltageSource(PMIC_D, EFI_PM_GPIO_8, EFI_PM_GPIO_VIN0))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set voltage source for Backlight enable GPIO failed!\n"));
    }
    if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_D, EFI_PM_GPIO_8, EFI_PM_GPIO_SRC_GND))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set output source for Backlight enable GPIO failed!\n"));
    }
    if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_D, EFI_PM_GPIO_8, EFI_PM_GPIO_LEVEL_HIGH))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set output level for Backlight enable GPIO failed!\n"));
    }
    if (EFI_SUCCESS != PmicGpioProtocol->Enable(PMIC_D, EFI_PM_GPIO_8, TRUE))
    {
    ...
    if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_B, EFI_PM_GPIO_8, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set output for Backlight enable GPIO failed!\n"));
    }
    if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_B, EFI_PM_GPIO_8, EFI_PM_GPIO_OUT_DRV_STR_MEDIUM))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set drive strength for Backlight enable GPIO failed!\n"));
    }
    if (EFI_SUCCESS != PmicGpioProtocol->SetVoltageSource(PMIC_B, EFI_PM_GPIO_8, EFI_PM_GPIO_VIN0))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set voltage source for Backlight enable GPIO failed!\n"));
    }
    if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_B, EFI_PM_GPIO_8, EFI_PM_GPIO_SRC_GND))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set output source for Backlight enable GPIO failed!\n"));
    }
    if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_B, EFI_PM_GPIO_8, EFI_PM_GPIO_LEVEL_HIGH))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set output level for Backlight enable GPIO failed!\n"));
    }
    if (EFI_SUCCESS != PmicGpioProtocol->Enable(PMIC_B, EFI_PM_GPIO_8, TRUE))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Enable for Backlight enable GPIO failed!\n"));
    }
...
MDP_Status Panel_LCD_PowerDown(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  else
  {
    ...
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_D, EFI_PM_GPIO_8, EFI_PM_GPIO_LEVEL_LOW))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio8 out put level failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_D, EFI_PM_GPIO_8, EFI_PM_GPIO_SRC_GND))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio8 out src cfg failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_D, EFI_PM_GPIO_8, EFI_PM_GPIO_OUT_DRV_STR_LOW))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio8 outdrv str failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_D, EFI_PM_GPIO_8, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio8 out buf cfg failed!\n"));
    }
    ...
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_B, EFI_PM_GPIO_8, EFI_PM_GPIO_SRC_GND))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio10 out src cfg failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_B, EFI_PM_GPIO_8, EFI_PM_GPIO_OUT_DRV_STR_LOW))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio10 outdrv str failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_B, EFI_PM_GPIO_8, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
    {
    ...
    * OLEDB_EN signal to PMIC to enable IBB/LAB.
    */
    if (EFI_SUCCESS != PmicGpioProtocol->Enable(PMIC_D, EFI_PM_GPIO_8, FALSE))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Disable for EFI_PM_GPIO_8 failed!\n"));
    }

    if (EFI_SUCCESS != PmicGpioProtocol->Enable(PMIC_B, EFI_PM_GPIO_8, FALSE))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Disable for EFI_PM_GPIO_10 failed!\n"));
    }
