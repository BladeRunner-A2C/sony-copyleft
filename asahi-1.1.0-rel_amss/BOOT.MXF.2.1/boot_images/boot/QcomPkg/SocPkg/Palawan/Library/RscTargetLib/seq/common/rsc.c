...
#define RSC_NUM_TIMESTAMPS 4
#define RSC_DYN_INIT_MODE_ID 0xF
#define RSC_MAX_DYN_MODE_ID 0xFF
...
rsc_result_t rsc_instance_init(rsc_handle handle)
{
  ...
  /*
   * Lock to update PCIE RSC. No-op for other RSCs.
   */
//rsc_lock_for_seq_update(handle);
  ...
  /*
   * Unlock after seq update for PCIE RSC.
   * This will be called from the PCIE drivers.
   */
   ...
   * DRV2 = HLOS
   *
   * Passing the ownership to HLOS such that HLOS can configure
   * the next wake up time in hidden TCS which eventually transffered
   * to the PDC when the RSCp sequence is triggered.
   */
  rsc_conf_hidden_tcs_ownership(handle, handle->hidden_tcs_drv_owner);
...
rsc_result_t rsc_ts_init(rsc_handle handle)
{
  ...
  {
    mode = &handle->modes[n];
    DALSYS_Malloc(RSC_NUM_TIMESTAMPS * sizeof(rsc_hal_time_stamp_info_t),
		    (void **)&(mode->time_stamps));
  ...
  for(m = 0; m < handle->mode_count; m++)
  {
    ...
    {
      DALSYS_Malloc(num_ts_events * sizeof(unsigned long int),
		      (void **)&(mode->time_stamps[i].ts_val));
...
free_ts_events:
  for (; m > 0; m--, i = RSC_NUM_TIMESTAMPS)
  {
    ...
    for(; i > 0; i--)
    {
      DALSYS_Free(mode->time_stamps[i-1].ts_val);
    }
  }
free_num_timestamps:
  for(; n > 0; n--)
...
rsc_result_t rsc_set_low_power_mode(rsc_handle handle, uint8 mode_id)
{
  ...
  if (NULL != mode->branch_info)
  {
      for (i = 0; i < mode->br_count; i++)
      {
        if (0 != mode->branch_info[i].br_addr)
        {
          ...
          rsc_hal_config_br_addr(handle, i, addr);
        }
      }
  }
   ...
   */
  rsc_hal_set_seq_override_start_addr_valid(handle, 0x1);

  /**
   * Write the address of the lpm mode to be kicked
   */
  rsc_hal_set_seq_override_start_addr(handle, mode->start_addr);
                                      ...
                                      uint16 seq_length, uint32 *mode_id)
{
  ...
  cmds_copied = copy_cmd_seq(handle, handle->copied_cmds, seq, seq_length);
   ...
   */
  // Realloc ?
if( DAL_ERROR == DALSYS_Malloc( sizeof(rsc_hal_bsp_mode_t) * (handle->dyn_mode_count + 1), (void **)&(handle->dyn_modes))){
	result = RSC_MEM_ALLOC_FAIL;
	return result;
}
...
static void rsc_tigger_sw_seq(rsc_handle handle)
{
   ...
   *  - Write 0x1
   */
  rsc_hal_set_trigger_seq(handle, 0x0);
  rsc_hal_set_trigger_seq(handle, 0x1);
}
...
rsc_result_t rsc_trigger_seq(rsc_handle handle, uint32 mode_id)
{
   ...
   */
  rsc_hal_set_trigger_start_address(handle, start_addr);
...
rsc_result_t rsc_get_time_stamp(rsc_handle handle, uint8 mode_id)
{
  ...
  {
    ts_low = rsc_hal_get_ts_unit_l(handle, i);
    ts_high = rsc_hal_get_ts_unit_l(handle, i);
...
rsc_result_t rsc_conf_hidden_tcs_ownership(rsc_handle handle, uint8 drv_id)
{
  ...
  rsc_hal_conf_hidden_tcs_owner(handle, drv_id);
...
{
 if(RSC_VALID_HANDLE(handle))
    rsc_unlock_after_seq_update(handle);
}
