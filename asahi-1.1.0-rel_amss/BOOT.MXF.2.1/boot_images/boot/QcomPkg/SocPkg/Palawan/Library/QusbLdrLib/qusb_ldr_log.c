 ...
 * @note        
 *
 *              Copyright 2012, 2023 Qualcomm Technologies, Incorporated.
 *              All Rights Reserved.
...
#define QUSB_GENERIC_LOG_SIZE       (64)
#define QUSB_BUS_LOG_SIZE           (64)
#define QUSB_ERROR_LOG_SIZE         (32)
...
void qusb_log(qusb_log_enum id_0, uint8 offset, uint32 param)
{
  ...
  qusb_log_buffer.generic_buffer_index = (qusb_log_buffer.generic_buffer_index + 1) % qusb_generic_log_buf_size;
}
...
void qusb_bus_log(qusb_log_enum id_0, uint8 offset, uint32 param)
{
  ...
  qusb_log_buffer.bus_buffer_index = (qusb_log_buffer.bus_buffer_index + 1) % qusb_bus_log_buf_size;
}
...
void qusb_error_log(qusb_log_enum id_0, uint8 offset, uint32 param)
{
  ...
  qusb_log_buffer.error_buffer_index = (qusb_log_buffer.error_buffer_index + 1) % qusb_error_log_buf_size;
  qusb_uart_buffer_log(id_0+offset, param);  
}
