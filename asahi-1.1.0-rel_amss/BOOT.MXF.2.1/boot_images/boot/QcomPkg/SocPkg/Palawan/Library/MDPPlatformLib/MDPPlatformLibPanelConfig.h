...
/* Defines HW config with default gpio/reset function, power control/brightness level is dynamically defined
 */
 #define MDP_PLATFORM_CREATE_PANEL_HW_CONFIG_POWER(_power_up_, _power_down_) \
{\
  .pMDPPlatformPanelGpio    = &sDefaultGpioConfig,\
  .pPanel_PowerUp           = (_power_up_),\
  .pPanel_PowerDown         = (_power_down_),\
  .pPanel_Reset             = Panel_Default_Reset,\
  .pPanel_Brightness_Enable = NULL,\
  .pPanel_Brightness_Level  = Panel_Default_Brightness_Level\
}
