...
$Header: //components/dev/core.boot/6.1/mmohamme.core.boot.6.1.Ln_initialDSF/QcomPkg/SocPkg/Kailua/Library/DDRTargetLib/ddr_sdi_wrapper.c#1 $
...
================================================================================
                   Copyright 2018,2020-2022, 2023 Qualcomm Technologies Incorporated
                              All Rights Reserved
...
{ 
  ...
  {    
    ...
    if (ddr_shared_data_sdi->device_params.device_type == DDR_TYPE_LPDDR5)
    {
      if(!HWIO_DPCC_REG_DPCC_MC5_GDSCR_INM(HWIO_DPCC_REG_DPCC_MC5_GDSCR_PWR_ON_BMSK))
      //(!HWIO_DPC_REG_DPCC_PHY_GDSCR_INM(HWIO_DPC_REG_DPCC_PHY_GDSCR_PWR_ON_BMSK)) )
      {
          ddr_sdi_logs_ptr_sdi->pass1_enter_cookie->p_cookie_value = DPCC_MC5_GDSCR_PWR_ON_NOT_SET;
          return FALSE;
      }   
    }
    else
    ...
    if (!freeze_io_status) 
    {
        if (ddr_shared_data_sdi->device_params.device_type == DDR_TYPE_LPDDR5)
        {
          if (HWIO_INF(MC_BROADCAST_MC5_SHKE_SELF_REFRESH_STATUS, WDOG_SELF_REFRESH))
          {
