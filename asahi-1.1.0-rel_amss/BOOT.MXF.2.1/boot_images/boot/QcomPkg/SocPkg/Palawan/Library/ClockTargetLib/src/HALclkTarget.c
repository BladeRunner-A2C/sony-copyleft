...
void HAL_clk_PongoOlePLLPalawanSetRegSettings ( HAL_clk_SourceDescType *pSource, const HAL_clk_SourceRegSettingsType *pmSettings);
...
HAL_clk_SourceControlType HAL_clk_PongoolePalawanPLLControl =
{
  ...
  .SetRegSettings   = HAL_clk_PongoOlePLLPalawanSetRegSettings,
};
...
void HAL_clk_PongoOlePLLPalawanSetRegSettings
(
  HAL_clk_SourceDescType              *pSource,
...
// FV to follow up with PTE and PLL designer for Kailua.
  ...
  nK = HWIO_INF(QFPROM_CORR_CALIBRATION_ROW22_MSB, AOSS_K);
  ...
  if (nK)
  {
    ...
    HWIO_OUTF(AOSS_CC_PLL1_PLL_TEST_CTL_U1,
              FORCE_PCAL_VALUE,
              HWIO_INF(QFPROM_CORR_CALIBRATION_ROW23_LSB, AOSS_RDOPCODE) << 2);
    HWIO_OUTF(AOSS_CC_PLL1_PLL_TEST_CTL_U1,
              FORCE_TDO_PCAL_VALUE,
              HWIO_INF(QFPROM_CORR_CALIBRATION_ROW23_LSB, AOSS_TDOPCODE) << 2);
  }
