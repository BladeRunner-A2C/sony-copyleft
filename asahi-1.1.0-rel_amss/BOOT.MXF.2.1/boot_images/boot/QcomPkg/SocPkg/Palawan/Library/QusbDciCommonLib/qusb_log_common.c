 ...
 * @note        
 *
 *              Copyright 2012, 2017, 2023 Qualcomm Technologies, Incorporated.
 *              All Rights Reserved.
...
#define QUSB_UART_INT_STR_SIZE      (128)    //MAX int digits can appear on UART log
...
void qusb_uart_buffer_log(uint32 id, uint32 value)
{
  int len;
  char uart_int_log_buffer[QUSB_UART_INT_STR_SIZE];
  len = snprintf(uart_int_log_buffer, QUSB_UART_INT_STR_SIZE, "usb: ID:0x%x, value: 0x%x", id, value);
  if((len < 0) || (len >= QUSB_UART_INT_STR_SIZE))
  {
    boot_log_message("str_overflow");
  }
...
#define QUSB_LOG_TYPE_STR_SIZE (4)

char qusb_ldr_log_type_str[QUSB_LOG_TYPE_MAX][QUSB_LOG_TYPE_STR_SIZE] = 
{
  "ldr"
};


void qusb_uart_error_log(uint32 log_type, uint32 event)
{
  qusb_uart_log(qusb_ldr_log_type_str[log_type], event);  
}
