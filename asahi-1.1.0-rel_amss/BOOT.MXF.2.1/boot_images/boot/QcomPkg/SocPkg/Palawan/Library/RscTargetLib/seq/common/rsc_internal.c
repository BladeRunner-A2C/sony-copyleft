/*==============================================================================
  ...
  DEPENDENCIES: None
                ...
                Copyright (c) 2017, 2021, 2023 Qualcomm Technologies, Inc. (QTI).
                All Rights Reserved.
...
#define RSC_INSTANCE_MAX_NAME_LEN 6
 ...
 *        buffer size.
 *
 * @param entity: Name of the RSC handles are requested for (e.g apss).
...
void rsc_read_id_info(rsc_handle handle)
{
  ...
  rsc_id->step_ver = rsc_hal_get_step_ver(handle);
  rsc_id->minor_ver = rsc_hal_get_minor_ver(handle); 
  rsc_id->major_ver = rsc_hal_get_major_ver(handle);
}
...
void rsc_read_internal_config(rsc_handle handle)
{
  ...
  DALSYS_Malloc(sizeof(struct rsc_hal_param_info_s), (void **)&handle->param_cfg_info);
  
  if( NULL == handle->param_cfg_info)
	return;
  ...
  slvr_cfg->solver_cfg = rsc_hal_get_solver_cfg(handle);

  /* Reading parentchild configuration */
  parentchild_cfg = &param_cfg_info->parentchild_cfg;
  parentchild_cfg->parentchild_cfg = rsc_hal_get_parentchild_cfg(handle);

  /* Reading rsc configuration */
  rsc_cfg = &param_cfg_info->rsc_cfg;
  rsc_cfg->num_drv = rsc_hal_get_num_drvs(handle);
  rsc_cfg->num_seq_cmd_words = rsc_hal_get_num_seq_cmd_words(handle);
  rsc_cfg->num_ts_events = rsc_hal_get_num_ts_events(handle); 
  rsc_cfg->delay_cntr_bitwidth = rsc_hal_get_delay_cntr_bitwidth(handle);
}
...
rsc_result_t rsc_copy_cmd_seqs(rsc_handle handle)
{
  ...
  uint16 (*copy_cmd_seq)(rsc_handle,  uint16, rsc_cmd_t*, uint16);
  uint32 i;

if (handle->op_mode == SIMPLE_8BIT_MODE)
  copy_cmd_seq = rsc_hal_copy_simple_cmd_seq;
else   if(handle->op_mode == EXTENDED_16BIT_MODE)
  copy_cmd_seq = rsc_hal_copy_extended_cmd_seq;
else if(handle->op_mode == EXTENDED_32BIT_MODE)
  copy_cmd_seq = rsc_hal_copy_32_cmd_seq;
else
	return result = RSC_ERROR;
  /**
  ...
  for(i = 0; i < handle->mode_count; i++)
  {
    ...
    copied_cmds = copy_cmd_seq(handle, handle->copied_cmds,
...
uint32 rsc_mode_index(rsc_handle handle, uint8 lpm_id)
{
  ...
  {
    for(i = 0; i < handle->dyn_mode_count; i++)
    {
      mode = &handle->dyn_modes[i];
      if((NULL != mode) && (mode->mode_id == lpm_id))
        return i;
    }
  }
 ...
 *
 * @brief Enable/Disable time stamp unit associated with RSC instance(apss).
 *
...
rsc_result_t rsc_enable_ts_events(rsc_handle handle, boolean enable)
{
  ...
  {
      rsc_hal_enable_ts_unit(handle, i, enable);
      /*
      ...
      if (!handle->ts_init_done)
        rsc_hal_get_ts_unit_h(handle, i);
  }
...
{
  return !(strncmp(handle->name, "pcie", strlen("pcie")));
}

int rsc_is_cam(rsc_handle handle)
{
  return !(strncmp(handle->name, "cam", strlen("cam")));
}

/**
 * <!-- rsc_lock_for_seq_update -->
 *
 * @brief Prepare the RSC to receive the sequences. Needed for PCIE RSC.
 *
 * @param handle to RSC instance to which sequence is written to.
 *
 * @return None.
 */
 
 /*
void rsc_lock_for_seq_update(rsc_handle handle)
{
  if (rsc_is_pcie(handle))
    rsc_hal_pcie_rsc_force_wake(handle, TRUE);
}
*/

/**
 * <!-- rsc_unlock_after_seq_update -->
 *
 * @brief Allow RSC to function after seq update. Needed for PCIE RSC.
 *
 * @param handle to RSC instance to which sequence is written to.
 *
 * @return None.
 */
 /*
void rsc_unlock_after_seq_update(rsc_handle handle)
{
  if (rsc_is_pcie(handle))
    rsc_hal_pcie_rsc_force_wake(handle, FALSE);
}
*/