...
2022-03-03   jt      Initial Lanai changes 
...
{ 
  	...
  	{ VOLT_CORNER_LOWSVS,   UFS_GEAR_2,       { 100, 4,   100}},  
	{ VOLT_CORNER_SVS,      UFS_GEAR_3,       { 202, 8,   202}},
	{ VOLT_CORNER_NOM,      UFS_GEAR_5,       { 403, 16,  403}},
	{ VOLT_CORNER_TURBO,    UFS_GEAR_5,       { 403, 16,  403}}
};
...
{
   {
      {"gcc_ufs_phy_axi_clk", 0, 0, 0, 100000000, 201500000, 403000000, 403000000}, 
      {"gcc_ufs_phy_ice_core_clk", 0, 0, 0, 100000000, 201500000, 403000000, 403000000}, 
      {"gcc_ufs_phy_unipro_core_clk", 0, 0, 0, 100000000, 201500000, 403000000, 403000000},
...
ufs_bsp_err_type ufs_bsp_volt_set(uint8_t hostid, ufs_bsp_regulator_mode mode)
{
   ...
   if (hostid == 0) {
      ...
      {
         vcc_mV_client_handle = npa_create_sync_client("/pm/ldob12/mV", "ufs0_vcc", NPA_CLIENT_REQUIRED); 
         if (vcc_mV_client_handle == NULL) {
      ...
      {
         vcc_en_client_handle = npa_create_sync_client("/pm/ldob12/en", "ufs0_vcc", NPA_CLIENT_REQUIRED); 
         if (vcc_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      ...
      {
         vcc_mode_client_handle = npa_create_sync_client("/pm/ldob12/mode", "ufs0_vcc", NPA_CLIENT_REQUIRED); 
         if (vcc_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      ...
      if (vccq_mV_client_handle == NULL) {
         vccq_mV_client_handle = npa_create_sync_client("/pm/ldod3/mV", "ufs0_vccq", NPA_CLIENT_REQUIRED); 
         if (vccq_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vccq_en_client_handle == NULL) {
         vccq_en_client_handle = npa_create_sync_client("/pm/ldod3/en", "ufs0_vccq", NPA_CLIENT_REQUIRED); 
         if (vccq_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vccq_mode_client_handle == NULL) {
         vccq_mode_client_handle = npa_create_sync_client("/pm/ldod3/mode", "ufs0_vccq", NPA_CLIENT_REQUIRED); 
         if (vccq_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      ...
      if (vdda_mV_client_handle == NULL) {
         vdda_mV_client_handle = npa_create_sync_client("/pm/ldob4/mV", "ufs0_vdda", NPA_CLIENT_REQUIRED); 
         if (vdda_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vdda_en_client_handle == NULL) {
         vdda_en_client_handle = npa_create_sync_client("/pm/ldob4/en", "ufs0_vdda", NPA_CLIENT_REQUIRED); 
         if (vdda_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vdda_mode_client_handle == NULL) {
         vdda_mode_client_handle = npa_create_sync_client("/pm/ldob4/mode", "ufs0_vdda", NPA_CLIENT_REQUIRED); 
         if (vdda_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      ...
      if (vdda_core_mV_client_handle == NULL) {
         vdda_core_mV_client_handle = npa_create_sync_client("/pm/ldod1/mV", "ufs0_vdda_core", NPA_CLIENT_REQUIRED); 
         if (vdda_core_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vdda_core_en_client_handle == NULL) {
         vdda_core_en_client_handle = npa_create_sync_client("/pm/ldod1/en", "ufs0_vdda_core", NPA_CLIENT_REQUIRED); 
         if (vdda_core_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vdda_core_mode_client_handle == NULL) {
         vdda_core_mode_client_handle = npa_create_sync_client("/pm/ldod1/mode", "ufs0_vdda_core", NPA_CLIENT_REQUIRED); 
         if (vdda_core_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      ...
      if (qref_mV_client_handle == NULL) {
           qref_mV_client_handle = npa_create_sync_client("/pm/ldob2/mV", "ufs0_qref", NPA_CLIENT_REQUIRED); 
         if (qref_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (qref_en_client_handle == NULL) {
         qref_en_client_handle = npa_create_sync_client("/pm/ldob2/en", "ufs0_qref", NPA_CLIENT_REQUIRED); 
         if (qref_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (qref_mode_client_handle == NULL) {
         qref_mode_client_handle = npa_create_sync_client("/pm/ldob2/mode", "ufs0_qref", NPA_CLIENT_REQUIRED); 
         if (qref_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
   ...
   if (mode == UFS_BSP_REGULATORS_ON) {
      ...
      // LDO1D
      npa_issue_required_request(vdda_core_mV_client_handle, 912);
