...
/*=============================================================================
 ...
 05/28/21   jaychoi  Tear down XHCI stack upon exiting UEFI
 ...
 04/15/20   pw       Added enumerate fastboot in secondary port
 ...
 02/21/19   gk       Added Support for LA Flavor for MultiPort
 01/16/19   gk       Added Support to Turnoff NPA for all cores.
 11/13/18   gk       Added support for LPM in UEFI
 10/22/18   kameya   Added code for new PwrctrlDxe Api
 10/11/18   kameya   Support USB SLT loopback feature.
 05/03/18   pm       Fix UsbConfigInit issue for CLS Platform 
 04/19/18   amitg    Poipu Multi-Port Support, Vbus fixes
 04/10/18   amitg    Poipu BU Fixes
 04/06/18   amitg    Enabled Dual Role for Primary and Secondary Port
 04/03/18   psapin   For Poipu BU: primary to Host mode, add StartController for prim, other ConfigInit changes
...
BOOLEAN                       gHighSpeedOnlyPrimary = FALSE;

#if USB_SEC_FLAG
BOOLEAN                       gHighSpeedOnlySecondary = FALSE;
BOOLEAN                       gFastbootSecPort = FALSE;

#endif

#if USB_MP_FLAG
BOOLEAN                       gHighSpeedOnlyMP = FALSE;
#endif

//UEFI Variable Name and its default value for each core 
CHAR16                     
gVariableName[USB_CORE_MAX_NUM_SOC][MAX_VARIABLE_NAME_SIZE] = {L"UsbConfigPrimaryPort", 
#if USB_SEC_FLAG	
                                                                   L"UsbConfigSecondaryPort"
#endif
#if USB_MP_FLAG
                                                                   ,L"UsbConfigHostMultiPort"
#endif
};

UINT8 
gDefaultVariableVal[USB_CORE_MAX_NUM_SOC] =  {USB_DUAL_ROLE_SOC,  //Auto Detection For Primary port
#if USB_SEC_FLAG	
                                                   USB_DUAL_ROLE_SOC
#endif                                                   
#if USB_MP_FLAG
...
BOOLEAN 
gDualRoleEnable[USB_CORE_MAX_NUM_SOC]     =     {FALSE
#if USB_SEC_FLAG			
                                                , FALSE
#endif
#if USB_MP_FLAG
  ...
  )
{
  ...
  switch (CoreType)
  {
    ...
    default:
      ...
      *CoreNum = (QCOM_USB_CORE_NUM) USB_CORE_MAX_NUM_SOC;
    break;
  }
  ...
  )
{
  ...
  if (CoreNum >= (QCOM_USB_CORE_NUM)USB_CORE_MAX_NUM_SOC) {
    Status = EFI_INVALID_PARAMETER;
...
#if USB_MP_FLAG
  if ((CoreNum == USB_CORE_2_SOC) && (Mode != USB_HOST_MODE_XHCI))
  {
    Status = EFI_INVALID_PARAMETER;
    ERR_CHK ("ConfigUsb: Error - Invalid CoreNum %d doesnt support device mode", CoreNum);
  }
#endif
...
#if USB_MP_FLAG
  case USB_CORE_2:
    *CoreType = USB_CONFIG_SSUSB3;
    break;
#endif
  default: 
    Status = EFI_INVALID_PARAMETER;
  *CoreType = USB_CONFIG_SSUSB1;
    ERR_CHK("GetUsbHostConfig: Error - CoreNum is %d, but expect < %d", CoreNum, USB_CORE_MAX_NUM_SOC);
  }
  ...
  )
{
  ...
  if (gUsbConfigDevice[USB_CORE_MAX_NUM_SOC].PeripheralHandle != NULL)
  {
  ...
  DBG(EFI_D_WARN, "USB platform %d, ver %d, sub %d", PlatformInfo.platform, PlatformInfo.version, PlatformInfo.subtype);


  //Turn off NPA resources for all the Usb Cores.
  Status = UsbConfigLibCloseNPA(PMIC_NPA_MODE_ID_GENERIC_OFF, USB_CORE_0_SOC);
  if(Status != EFI_SUCCESS){
     DBG(EFI_D_INFO, "Failed to Disable clocks! Status =  (0x%x)", Status);
  }
  ...
  gUsbConfigDevice[USB_CORE_MAX_NUM_SOC].UsbConfig = gUsbConfigTemplate;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SOC].UsbConfig.coreNum = (QCOM_USB_CORE_NUM)USB_CORE_MAX_NUM_SOC;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SOC].UsbConfig.modeType = USB_INVALID_MODE;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SOC].HostHandle = NULL;
  gUsbConfigDevice[USB_CORE_MAX_NUM_SOC].PeripheralHandle = NULL;

  //Check if the UEFI variable is set to enable USBPriHighSpeedOnlySupported in UEFI. If so, 
  //enable gHighSpeedOnlyPrimary
  UefiVarSize = sizeof(UefiVar);
  Status = gRT->GetVariable(L"EnableHighSpeedOnlyForPri", &gQcomTokenSpaceGuid, NULL, &UefiVarSize, &UefiVar);
  if(!EFI_ERROR(Status)) {
    DBG(EFI_D_WARN, "USB force High Speed Only mode for Primary port via Menu HSOnly=%d", UefiVar);
    if (UefiVar != 0) {
      gHighSpeedOnlyPrimary = TRUE;
    }
  } else if (FeaturePcdGet(USBPriHighSpeedOnlySupported)) {
    DBG(EFI_D_WARN, "USB force High Speed only mode set for Primary port via PCD");
    gHighSpeedOnlyPrimary = TRUE;
  }
 #if USB_SEC_FLAG
  //Check if the UEFI variable is set to enable USBSecHighSpeedOnlySupported in UEFI. If so, 
  //enable gHighSpeedOnlySecondary
  UefiVar = 0;
  UefiVarSize = sizeof(UefiVar);
  Status = gRT->GetVariable(L"EnableHighSpeedOnlyForSec", &gQcomTokenSpaceGuid, NULL, &UefiVarSize, &UefiVar);
  if(!EFI_ERROR(Status)) {
    DBG(EFI_D_WARN, "USB force High Speed Only mode for Secondary port via Menu HSOnly=%d", UefiVar);
    if (UefiVar != 0) {
      gHighSpeedOnlySecondary = TRUE;
    }
  } else if (FeaturePcdGet(USBSecHighSpeedOnlySupported)) {
    DBG(EFI_D_WARN, "USB force High Speed only mode set for Secondary port via PCD");
    gHighSpeedOnlySecondary = TRUE;
  }
#endif
#if USB_MP_FLAG  
  //Check if the UEFI variable is set to enable USBMPHighSpeedOnlySupported in UEFI. If so, 
  //enable gHighSpeedOnlyMP
  UefiVar = 0;
  UefiVarSize = sizeof(UefiVar);
  Status = gRT->GetVariable(L"EnableHighSpeedOnlyForMP", &gQcomTokenSpaceGuid, NULL, &UefiVarSize, &UefiVar);
  if(!EFI_ERROR(Status)) {
    DBG(EFI_D_WARN, "USB force High Speed Only mode for MP port via Menu HSOnly=%d", UefiVar);
    if (UefiVar != 0) {
      gHighSpeedOnlyMP = TRUE;
    }
  } else if (FeaturePcdGet(USBMPHighSpeedOnlySupported)) {
    DBG(EFI_D_WARN, "USB force High Speed only mode set for MP port via PCD");
    gHighSpeedOnlyMP = TRUE;
  }
#endif
  ...
  Status = gBS->InstallMultipleProtocolInterfaces(&gUsbConfigDevice[USB_CORE_MAX_NUM_SOC].PeripheralHandle,
    &gQcomUsbConfigProtocolGuid, &gUsbConfigDevice[USB_CORE_MAX_NUM_SOC].UsbConfig,
    NULL);
  ...
  for (Index = 0; Index < USB_CORE_MAX_NUM_SOC; Index++) {
  ...
  if (NULL == gHalIommuProtocol) {
    ...
    if (EFI_ERROR(SmmuStatus))
    {
      ...
      // attach
      if (gSmmuIsSetup) {
        SmmuStatus = gSmmuTable->domain_attach_func(gUsbSmmuDomain, (void *)"USB3", 0x03000000, 0x0);
        SMMU_CHK("Failed to attach USB Arid 0x0 HAL IOMMU domain Result =  (0x%x)", SmmuStatus);
        gSmmuUsb30Attached = gSmmuIsSetup;
#if USB_SEC_FLAG	
        SmmuStatus = gSmmuTable->domain_attach_func(gUsbSmmuDomain, (void *)"USB1", 0x03000000, 0x0);
        SMMU_CHK("Failed to attach USB Arid 0x1 HAL IOMMU domain Result =  (0x%x)", SmmuStatus);
        gSmmuUsb31Attached = gSmmuIsSetup;
#endif
#if USB_MP_FLAG
        SmmuStatus = gSmmuTable->domain_attach_func(gUsbSmmuDomain, (void *)"\\_SB_.USB3-2", 0x0, 0x0);
        SMMU_CHK("Failed to attach USB3-2 HAL IOMMU domain Result =  (0x%x)", SmmuStatus);
        gSmmuUsb32Attached = gSmmuIsSetup;
#endif
        gSmmuIsSetup = TRUE;
      }
    }   
  ...
  if (PlatformInfo.platform == EFI_PLATFORMINFO_TYPE_CLS)
  {
    ...
    gDefaultVariableVal[USB_CORE_1_SOC] = USB_HOST_MODE_SOC;
  } 
  else
#endif		
  {
    for (Index = 0; Index < USB_CORE_MAX_NUM_SOC; Index++) 
    {
        ...
        { 
          ...
          case USB_DUAL_ROLE_SOC:
          // If the user plugs in the host cable during the boot time, configure the 
          // USB hardware  in the host mode
            DBG(EFI_D_INFO, "Dual Role Enabled on Port Number: %d", Index);
            gDualRoleEnable[Index] = TRUE;
            gUsbConfigDevice[Index].UsbCoreInterface->ModeType = USB_DUAL_ROLE_MODE;
            break;


      case USB_DISABLED_MODE_SOC:
        // if ENTPMGMT is set to Disable core 
        DBG(EFI_D_WARN, "Disable set on Port Number : %d", Index);
        gUsbConfigDevice[Index].EnableStaticCfg.Enabled = FALSE;
        gUsbConfigDevice[Index].EnableStaticCfg.UsbMode = USB_INVALID_MODE;
        gUsbConfigDevice[Index].UsbCoreInterface->ModeType = USB_INVALID_MODE;
        gDualRoleEnable[Index] = FALSE;
        break;
    ...
    if (PlatformInfo.platform != EFI_PLATFORMINFO_TYPE_ADP)
    {
        ...
        else if (((Index == USB_CORE_0_SOC)
          #if USB_SEC_FLAG
            ||(Index == USB_CORE_1_SOC)
          #endif
             ) && gUsbConfigDevice[Index].UsbCoreInterface->ModeType == USB_HOST_MODE)
        {
...
#if USB_MP_FLAG
        //Also Need to check Vbus enablement for multi core in the next round of changes.
        else if ((Index == USB_CORE_2_SOC) && gUsbConfigDevice[Index].UsbCoreInterface->ModeType == USB_HOST_MODE && FeaturePcdGet(USBMultiPortSupported))
        {
          // Start MP Controller in host mode always
          DBG(EFI_D_WARN, "Start MP SS Controller");
          Status = UsbStartController((QCOM_USB_CORE_NUM)Index, USB_HOST_MODE_XHCI);
          ERR_CHK("Start controller failed with error: %r on core %d", Status, USB_CORE_2_SOC);
        }
#endif
        else
        {
          DBG(EFI_D_INFO, "Not start on port: %d, mode %d", Index, gUsbConfigDevice[Index].UsbCoreInterface->ModeType);
        }
      }
    }
  }
  ...
  if(gPwrCtrlProtocol != NULL && gPwrCtrlProtocol->GetHwInfo!=NULL) {
  ...
  if (!EFI_ERROR(Status) && (UefiVar != 0)) 
  {
    ...
    if (EFI_ERROR (Status)) 
    {
        DEBUG((EFI_D_ERROR, "UsbConfigUpdateACPIEntry: Failed to Locate SDT protocol \r\n"));
        Status = EFI_SUCCESS;
        goto ON_EXIT;
     }
  }
  ...
  )
{
  ...
  if(gSmmuIsSetup)
  {
    if (FeaturePcdGet(USBSmmuStage1Supported))
    {
      ...
      {
        gSmmuTable->domain_detach_func(gUsbSmmuDomain, (void *)"USB3", 0x0, 0x0);
      }
#if USB_SEC_FLAG
      if (gSmmuUsb31Attached)
      {
        gSmmuTable->domain_detach_func(gUsbSmmuDomain, (void *)"USB1", 0x0, 0x0);
      }
#endif
#if USB_MP_FLAG
      if (gSmmuUsb32Attached) {
      	gSmmuTable->domain_detach_func(gUsbSmmuDomain, (void *)"\\_SB_.USB3-2", 0x0, 0x0);
      }
#endif
    }
  ...
  )
{
...
#if USB_MP_FLAG
  if ((CoreNum == USB_CORE_2_SOC) && (ModeType != USB_HOST_MODE_XHCI)) {
    Status = EFI_INVALID_PARAMETER;
    ERR_CHK ("CoreNum = %d,Error - Invalid ModeType %d", CoreNum, ModeType);
  }
#endif
  ...
  // check if the core is ever start, if not return
  if (gUsbConfigDevice[CoreNum].UsbConfig.modeType == USB_INVALID_MODE) {
    Status = EFI_NOT_STARTED;
    ERR_CHK("Error - Unable to stop core %d, never start (curr mode %d)",
      CoreNum,
  ...
  if (gUsbConfigDevice[CoreNum].UsbConfig.AlwaysConnected)
  { 
...
QCOM_USB_MODE_TYPE
EFIAPI UsbClientModeToQcomMode(QCOM_USB_MODE_TYPE_SOC usbmode)
{
  switch(usbmode)
  { 
    case USB_HOST_MODE_SOC:
      return USB_HOST_MODE_XHCI;
    case USB_DEVICE_MODE_SOC:
      return USB_DEVICE_MODE_SS;
    case USB_DUAL_ROLE_SOC:
      return USB_DUAL_ROLE_MODE;
    default:
      return USB_INVALID_MODE;
  }
}

EFI_STATUS
EFIAPI UsbStartDevice(void)
{
  EFI_STATUS Status = EFI_SUCCESS;
  QCOM_USB_CORE_NUM_SOC      DeviceCoreNum = USB_CORE_0_SOC; 
  QCOM_USB_MODE_TYPE         usb_mode = USB_DEVICE_MODE_SS;

  UINT8                      UefiVar = 0;
  UINTN                      UefiVarSize;

  UefiVarSize = sizeof(UefiVar);
  Status = gRT->GetVariable(L"UsbDeviceSecPort", &gQcomTokenSpaceGuid, NULL, &UefiVarSize, &UefiVar);
#if USB_SEC_FLAG
  if (!EFI_ERROR(Status) && (TRUE == UefiVar))
  {
    DBG(EFI_D_WARN, "USB use Secondary for Device via Menu UsbDeviceSecPort=%d", UefiVar);
    usb_mode = USB_DEVICE_MODE_SS;  //If uefi reading failed, assumed default dual_role
    DeviceCoreNum = USB_CORE_1_SOC;
  }
  else if (FeaturePcdGet(FastbootSecPort)) {
    DBG(EFI_D_WARN, "USB use Secondary as fastboot via PCD");
    usb_mode = USB_DEVICE_MODE_SS;  //If uefi reading failed, assumed default dual_role
    DeviceCoreNum = USB_CORE_1_SOC;
  }
  else
#endif
  {
    //DBG(EFI_D_WARN, "UsbStartDevice, Read failed, enumerate in Primary %d", Status);
    usb_mode = USB_DEVICE_MODE_SS;  //If uefi reading failed, assumed default dual_role
    DeviceCoreNum = USB_CORE_0_SOC;
  }

  if (gUsbConfigDevice[DeviceCoreNum].UsbConfig.modeType == USB_INVALID_MODE)
  {
    Status =UsbStartController((QCOM_USB_CORE_NUM)DeviceCoreNum, usb_mode);
  }
  else
  {
    DBG(EFI_D_ERROR, "UsbStartDevice, core %d, already enumerate in mode %d, skip", DeviceCoreNum, gUsbConfigDevice[DeviceCoreNum].UsbConfig.modeType);
  }
  return Status;
}

QCOM_USB_CORE_NUM
EFIAPI UsbStartLoopbackInit(void)
{
  QCOM_USB_CORE_NUM_SOC  CoreNum = USB_CORE_MAX_NUM_SOC;
#ifdef USB_LOOPBACK_FEATURE			
  EFI_STATUS Status = EFI_SUCCESS;
  QCOM_USB_MODE_TYPE         ModeType = USB_HOST_MODE_XHCI;
  
  QCOM_USB_MODE_TYPE         usb0_mode = USB_DUAL_ROLE_MODE;
  QCOM_USB_MODE_TYPE         usb1_mode = USB_DUAL_ROLE_MODE;
  //QCOM_USB_CONNECTOR_TYPE    ConnectorType;
  UINT8      SltLbEnabled = 0;
  UINTN      UefiVarSize;
  UINT8      UefiVar = 0;
  ...
  if (!EFI_ERROR(Status) && (SltLbEnabled != 0))
  {
    UefiVarSize = sizeof(UefiVar);
    Status = gRT->GetVariable(gVariableName[USB_CORE_0_SOC], &gQcomTokenSpaceGuid, NULL, &UefiVarSize, &UefiVar);
    if (EFI_SUCCESS == Status)
    {
      usb0_mode = UsbClientModeToQcomMode(UefiVar);
    }
    else
    {
      if (gUsbConfigDevice[USB_CORE_0_SOC].EnableStaticCfg.Enabled)
      {
        usb0_mode = gUsbConfigDevice[USB_CORE_0_SOC].EnableStaticCfg.UsbMode;
      }
    }
    UefiVar = 0;
    UefiVarSize = sizeof(UefiVar);

    Status = gRT->GetVariable(gVariableName[USB_CORE_1_SOC], &gQcomTokenSpaceGuid, NULL, &UefiVarSize, &UefiVar);
    if (EFI_SUCCESS == Status)
    {
      usb1_mode = UsbClientModeToQcomMode(UefiVar);
    }
    else
    {
      if (gUsbConfigDevice[USB_CORE_1_SOC].EnableStaticCfg.Enabled)
      {
        usb1_mode = gUsbConfigDevice[USB_CORE_1_SOC].EnableStaticCfg.UsbMode;
      }
    }
    DBG(EFI_D_WARN, "usb_loopback enable, uefi setting Pri-mode %d, Sec-mode %d", usb0_mode, usb1_mode);

    // ROLE for Primary role
    //      Sec:  HOST | DEVICE | DUAL  
    // PRI:     
    // HOST       HOST | HOST   | HOST
    // DEVICE     DEV  | DEV    | DEV
    // DUAL       DEV  | HOST   | DEV

    if ((usb0_mode == USB_HOST_MODE_XHCI) || ((usb0_mode == USB_DUAL_ROLE_MODE) && (usb1_mode == USB_DEVICE_MODE_SS)))
    {
      ModeType = USB_HOST_MODE_XHCI;
      CoreNum = USB_CORE_0_SM_SOC;
	  gUsbConfigDevice[CoreNum].EnableStaticCfg.Enabled = TRUE;
	  gUsbConfigDevice[CoreNum].EnableStaticCfg.UsbMode = ModeType;
	  gUsbConfigDevice[CoreNum].UsbCoreInterface->ModeType = USB_HOST_MODE;
			
	  gUsbConfigDevice[USB_CORE_1_SOC].EnableStaticCfg.Enabled = TRUE;
	  gUsbConfigDevice[USB_CORE_1_SOC].EnableStaticCfg.UsbMode = USB_DEVICE_MODE_SS;
	  gUsbConfigDevice[USB_CORE_1_SOC].UsbCoreInterface->ModeType = USB_DEVICE_MODE;
    }
    else
    {
      ModeType = USB_HOST_MODE_XHCI;
      CoreNum = USB_CORE_1_SOC;
	  gUsbConfigDevice[CoreNum].EnableStaticCfg.Enabled = TRUE;
	  gUsbConfigDevice[CoreNum].EnableStaticCfg.UsbMode = ModeType;
	  gUsbConfigDevice[CoreNum].UsbCoreInterface->ModeType = USB_HOST_MODE;
	  
	  gUsbConfigDevice[USB_CORE_0_SOC].EnableStaticCfg.Enabled = TRUE;
	  gUsbConfigDevice[USB_CORE_0_SOC].EnableStaticCfg.UsbMode = USB_DEVICE_MODE_SS;
	  gUsbConfigDevice[USB_CORE_0_SOC].UsbCoreInterface->ModeType = USB_DEVICE_MODE;
    } 

    DBG(EFI_D_WARN, "Init host mode for core %d", CoreNum);
    Status = UsbStartController((QCOM_USB_CORE_NUM)CoreNum, ModeType);
  }

#endif
  gUsbDeviceCnt++;

  //if the core number is maxium, the host didnt initalize
  if (USB_CORE_MAX_NUM_SOC == CoreNum)
  {
    return USB_CORE_MAX_NUM;
  }
  return (QCOM_USB_CORE_NUM)CoreNum;
}
  ...
  )
{
  ...
  //For Enterprise Mgmt core disablement, dont start controller in Invalid mode
  if((gUsbConfigDevice[CoreNum].UsbCoreInterface == NULL) || 
     (gUsbConfigDevice[CoreNum].UsbCoreInterface->ModeType == USB_INVALID_MODE ))
    {
      Status = EFI_INVALID_PARAMETER;
      ERR_CHK ("UsbStartController: Error - Core %d. ModeType %d.", CoreNum, ModeType);
    }
  ...
  if (((CoreNum == USB_CORE_0_SOC)
      ...
      || (CoreNum == USB_CORE_1_SOC)
     #endif
     ) && (ModeType != USB_HOST_MODE_XHCI) && (ModeType != USB_DEVICE_MODE_SS))
  {
    Status = EFI_INVALID_PARAMETER;
  ...
  if (gUsbConfigDevice[CoreNum].UsbConfig.modeType == ModeType)
  {
    if (gUsbConfigDevice[CoreNum].UsbConfig.AlwaysConnected)
    { 
      //in the loopback situation we might get the same test case,
      //  so we need to stop the duplicate request and start again.
      DBG(EFI_D_WARN, "CoreNum %d, ModeType %d", CoreNum, ModeType);
  ...
  WRN_CHK ("GetUsbAlwaysConnect failed on Core %d: %r", CoreNum, Status);
  ...
  if (ModeType == USB_DEVICE_MODE_SS)
  {
    if (gUsbConfigDevice[CoreNum].PeripheralHandle == NULL) {
      ...
      ERR_CHK("Core %d: Failed to install USB_CONFIG protocol (Peripheral Client Handle) with error: %r", CoreNum, Status);
    }
    if ((ConnectorType == USB_CONNECTOR_TYPE_C) 
          && gUsbConfigDevice[CoreNum].UsbConfig.AlwaysConnected)
    { 
    ...
    DBG(EFI_D_INFO,"CoreNum %d, Mode %d", CoreNum, USB_DEVICE_MODE_SS);
    Status = gBS->ConnectController(gUsbConfigDevice[CoreNum].PeripheralHandle, NULL, NULL, TRUE);
    ERR_CHK("Core %d: Failed to connect QcomUsbConfig protocol peripheral handle with error: %r", CoreNum, Status);
  ...
  else
  {
    if (gUsbConfigDevice[CoreNum].HostHandle == NULL) 
    {
      ...
      Status = gBS->InstallMultipleProtocolInterfaces(&gUsbConfigDevice[CoreNum].HostHandle,
        &gQcomUsbConfigProtocolGuid, &gUsbConfigDevice[CoreNum].UsbConfig, NULL);
      ERR_CHK("Core %d: Failed to install USB_CONFIG protocol (Host Client Handle) with error: %r", CoreNum, Status);
    }
    ...
    ERR_CHK("Core %d: Failed to connect QcomUsbConfig protocol host handle with error: %r", CoreNum, Status);
  ...
  )
{
  ...
  if (INVALID_CORE(CoreNum)) {
   Status = EFI_INVALID_PARAMETER;
   ERR_CHK ( "Error - Invalid CoreNum %d", CoreNum);
  }

  if (gUsbConfigDevice[CoreNum].UsbConfig.modeType == USB_INVALID_MODE) {
    Status = EFI_NOT_READY;
    ERR_CHK ( "Error - ModeType is USB_INVALID_MODE");
  }

  // store the current mode type. UsbStopController will set the
  // gUsbConfigDevice[CoreNum].modeType to USB_INVALID_MODE
  CurrModeType = gUsbConfigDevice[CoreNum].UsbConfig.modeType;
  ...
  switch (CurrModeType) {
    ...
    NextModeType = USB_DEVICE_MODE_SS;
    break;
  ...
  DBG(EFI_D_WARN, "Starting toggling controller mode on core %d from mode %d to %d", CoreNum, CurrModeType, NextModeType);

  Status = UsbStopController(CoreNum, CurrModeType);
  ERR_CHK ("Error - Unable to stop controller on core %d, Status =  (%r)", CoreNum, Status);
  if (NextModeType == USB_HOST_MODE_XHCI)
  {
    gUsbConfigDevice[CoreNum].EnableStaticCfg.Enabled = TRUE;
    gUsbConfigDevice[CoreNum].EnableStaticCfg.UsbMode = USB_HOST_MODE_XHCI;
    gUsbConfigDevice[CoreNum].UsbCoreInterface->ModeType = USB_HOST_MODE;
    usbmode = USB_HOST_MODE_SOC;
  }
  else
  {
    gUsbConfigDevice[CoreNum].EnableStaticCfg.Enabled = TRUE;
    ...
    usbmode = USB_DEVICE_MODE_SOC;
  }
  
  VarSize = sizeof(usbmode);
  Status = gRT->SetVariable (gVariableName[CoreNum], &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &usbmode);

  WRN_CHK ( "update uefi variable failed CoreNum %d mode %d", CoreNum);
  ...
  )
{
  ...
  *MaxCoreNum = USB_CORE_MAX_NUM_SOC;
  return EFI_SUCCESS;
}
...
#if USB_MP_FLAG
  if (CoreNum == USB_CORE_2_SOC)
  {
    *ModeType = USB_HOST_MODE;
  }
  else 
#endif
  {
  ...
  IN QCOM_USB_CORE_NUM    CoreNum)
{
  ...
  UINTN                   UefiVarSize;
  USB_TYPEC_CC_OUT_STATUS CcStatus=USB_TYPEC_CC_OUT_RESERVED;
  QCOM_USB_CORE_INTERFACE *UsbCoreIfc;
  ...
  if(CoreNum == USB_CORE_1_SOC
#if USB_MP_FLAG
  || CoreNum == USB_CORE_2_SOC
#endif
  )
  {
    Status = EFI_SUCCESS;
    DBG (EFI_D_WARN, "No Lane Number for Port %d", CoreNum);
    goto ON_EXIT;
  }
#endif
  ...
  {
    DBG (EFI_D_WARN, "Override CC core %d, lane %d", CoreNum, USB_SS_PHY_LANE_A);
    LaneNum = USB_SS_PHY_LANE_A;
  }
  else if (override_cc_lane==UEFI_VAR_CC_LANE_OVERRIDE_B)
  {
    DBG (EFI_D_WARN, "Override CC core %d, lane %d", CoreNum, USB_SS_PHY_LANE_B);
  ...
  DBG (EFI_D_INFO,"CoreNum %d, LaneNum = %d", CoreNum, LaneNum);
  ...
  IN USB_TYPEC_PORT_MODE  PortMode)
{
  ...
  if (Message == UsbCfgPortMsgPartnerAttach) { //Attach
    ...
    DBG(EFI_D_INFO, "Cable Attach core %d, portmode %d, dualmode %d", CoreNum, PortMode, gDualRoleEnable[CoreNum]);
    ...
    if(gDualRoleEnable[CoreNum] == TRUE)
    {
      switch (PortMode)
      {
        ...
        default:
          DBG(EFI_D_INFO, "Unknown Port Mode Detected core %d, %d", CoreNum, PortMode);
          goto ON_EXIT;
      }
    }
    ...
    { 
      ...
      if (EFI_ERROR(Status) || FeaturePcdGet(UsbNoPmic)) { // cannot get charger type, assume invalid
    ...
    {
        if(   !(CoreNum == USB_CORE_0_SOC && gHighSpeedOnlyPrimary)
#if USB_SEC_FLAG
           || !(CoreNum == USB_CORE_1_SOC && gHighSpeedOnlySecondary)
#endif
#if USB_MP_FLAG
           || !((CoreNum == USB_CORE_2_SOC)  && gHighSpeedOnlyMP)  
#endif
        )
      {
    ...
    DBG(EFI_D_WARN, "Cable Detach coreNum %d mode %d", CoreNum, gUsbConfigDevice[CoreNum].UsbConfig.modeType);
    ...
    {  
      ...
      if (gDualRoleEnable[CoreNum] == TRUE)
      {
        Status = UsbStopController(CoreNum,  USB_HOST_MODE_XHCI);
        DBG(EFI_D_WARN,"UsbStopController: on core %d, Status = (0x%x)", CoreNum, Status);
      }
    }   
...
#define USB_MSD_EVT_CNT_MAX   (10)
  
STATIC EFI_USB_MSD_PERIPHERAL_PROTOCOL *UsbMsdProtocol3 = NULL;
VOID
EFIAPI
UsbfnMsdEventHandler3(
  VOID 
)
{
  EFI_STATUS      Status;
  ...
  if (UsbMsdProtocol3 == NULL)
  {
    Status = gBS->LocateProtocol (&gEfiUsbMsdPeripheralProtocolGuid, 
                                  NULL,
                                  (VOID **) &UsbMsdProtocol3);
    if (EFI_ERROR (Status))
    {
        DEBUG ((EFI_D_ERROR, "UsbfnMsdEventHandler3: Error Locating EfiUsbMsdPeripheralProtocol\n"));
      return;
    }
  }
  UsbMsdProtocol3->EventHandler(UsbMsdProtocol3);
  i++;
  if ((i%20000)==0)
  {
    DEBUG ((EFI_D_ERROR, "UsbfnMsdEventHandler3: handle event\n"));
  }
  ...
  )
{
  EFI_STATUS Status;
  UINTN Index;
  USB_CFG_PORT_MESSAGE Message=UsbCfgPortMsgNone;
  UINTN PayloadSize;
  USB_TYPEC_PORT_MESSAGE_PAYLOAD Payload;
  QCOM_USB_CORE_NUM CoreNum=USB_CORE_MAX_NUM;
  QCOM_USB_CONNECTOR_TYPE ConnectorType=USB_CONNECTOR_INVALID;
  UINT8 PortIndex = 0;
  UINT8 Cnt1;
  FNC_ENTER_MSG();
  ...
  if (gUsbDeviceCnt >=1)
  {
    //DBG(EFI_D_WARN,"UsbPortStateChgCb event 0x%x", Event);

    for (Cnt1 = 0; Cnt1 < USB_MSD_EVT_CNT_MAX; Cnt1++) {
      UsbfnMsdEventHandler3();        
    }
  }
...
#endif
#if USB_MP_FLAG
  //Multi port support only Host Mode.
  if(CoreNum == USB_CORE_2)
  {
    Status       = EFI_INVALID_PARAMETER;
    goto ON_EXIT;
  }
...
#if FEATURE_QC_USB_WOL_EN

/**
@summary
Returns WakeOnLan state

@param [out] pWoLSatate     Pointer to WakeOnLan state
...
EFIAPI 
GetWoLState(
  OUT QCOM_USB_CONFIG_WOL_STATE_TYPE  *pWoLState
  )
{
  EFI_STATUS Status = EFI_SUCCESS;
  FNC_ENTER_MSG();

  *pWoLState = gWakeOnLanState;
...
/**
@summary
Sets WakeOnLan state

@param [in]  pWoLSatate     WakeOnLan state
...
EFIAPI 
SetWoLState(
  IN  QCOM_USB_CONFIG_WOL_STATE_TYPE   WoLState
  )
{
  ...
  switch (WoLState)
  {
    case USB_CONFIG_WOL_DISABLED:
    {
      gWakeOnLanState = WoLState;
      DEBUG((EFI_D_ERROR, "SetWoLState:: USB_CONFIG_WOL_DISABLED \r\n"));
    }
    break;

    case USB_CONFIG_WOL_EN_ETH_INIT:
    {
      gWakeOnLanState = WoLState;
      DEBUG((EFI_D_ERROR, "SetWoLState:: USB_CONFIG_WOL_EN_ETH_INIT \r\n"));
    }
    break;

    case USB_CONFIG_WOL_EN_ETH_ENUM:
    {
      gWakeOnLanState = WoLState;
      DEBUG((EFI_D_ERROR, "SetWoLState:: USB_CONFIG_WOL_EN_ETH_ENUM \r\n"));
    }
    break;

    case USB_CONFIG_WOL_EN_ETH_ARMED:
    {
      gWakeOnLanState = WoLState;
      DEBUG((EFI_D_ERROR, "SetWoLState:: USB_CONFIG_WOL_EN_ETH_ARMED \r\n"));
    }
    break;

    case USB_CONFIG_WOL_EN_ETH_TRIGGERED:
    {
      gWakeOnLanState = WoLState;
      DEBUG((EFI_D_ERROR, "SetWoLState:: USB_CONFIG_WOL_EN_ETH_TRIGGERED \r\n"));

      // Nofity Charger API to boot-up to RESET_COLD
      if (pQcomChargerProtocol == NULL)
      {
        ...
        {
          DEBUG((EFI_D_ERROR, "ERROR:  - Failed to locate pQcomChargerProtocol! Error Code = 0x%08X \r\n", Status));
        }
        else
        {
          Status = pQcomChargerProtocol->SetControlAction(EFI_CHARGER_CONTROL_ACTION_RESET_COLD);
          if (EFI_ERROR (Status))
          {
            DEBUG (( EFI_D_ERROR, "ERROR: UsbWoLRootHubBusChangeCb: failed to ChargerSetControlAction RESET_COLD \r\n"));
          }
          else
          {
            DEBUG (( EFI_D_ERROR, "UsbWoLRootHubBusChangeCb: Success ChargerSetControlAction RESET_COLD \r\n"));
          }
        }
      }
    }
    ...
    {
      // Invalid State
      DBG(EFI_D_ERROR, "SetWoLState:: Invalid State (%d)", WoLState);
      Status = EFI_UNSUPPORTED; 
    }
    break;
