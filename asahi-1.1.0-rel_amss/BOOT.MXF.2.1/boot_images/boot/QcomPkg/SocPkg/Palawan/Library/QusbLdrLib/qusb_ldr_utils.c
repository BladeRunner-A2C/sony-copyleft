...
//============================================================================
/**
  Copyright (c) 2015-2022, 2023 Qualcomm Technologies, Inc. All rights reserved.
...
#define QUSB_CHARGER_DETECT_RETRY_CNT             (40)
#define APSD_RERUN_DELAY_MS                       (50)
#define APSD_DONE_STS_DELAY_PER_LOOP_MS           (30)
...
#define PMIC_DEFAULT_OTP_VERSION                  (0x4)
...
// ===========================================================================
/**
 * @function     qusb_ldr_utils_rerun_apsd
 * 
 * @brief   This function will rerun APSD in xbl 
 *
 * 
 * @return  None
 * 
 */
// ===========================================================================
void qusb_ldr_utils_rerun_apsd(void)
{
  uint32 retry_cnt = 0;
  pm_err_flag_type pm_err = PM_ERR_FLAG_SUCCESS;;
  boolean apsd_done = FALSE;
  
  // disable eusb repeater
  pm_err = pm_chg_eusb_enable(FALSE);
  
  //enable apsd BC1P2_SRC_DETECT
  pm_err |= pm_chg_enable_apsd(7, TRUE);
  
  // apsd rerun cmd
  pm_err |= pm_chg_rerun_apsd(TRUE);

  if (PM_ERR_FLAG_SUCCESS != pm_err)
  { 
    qusb_uart_log("err_rerun_apsd",pm_err);
  }
  else
  {
    qusb_dci_delay_ms(APSD_RERUN_DELAY_MS);

    while(retry_cnt <  QUSB_CHARGER_DETECT_RETRY_CNT  && apsd_done == FALSE)
    {
      pm_err = pm_get_apsd_status(&apsd_done);
      if (PM_ERR_FLAG_SUCCESS != pm_err)
      { 
        qusb_uart_log("apsd not succesfull",0);
        break;
      }
      retry_cnt++;
      //qusb_uart_log("retry_cnt_wait_apsd", retry_cnt);
      qusb_dci_delay_ms(APSD_DONE_STS_DELAY_PER_LOOP_MS);
    }
    if(retry_cnt == QUSB_CHARGER_DETECT_RETRY_CNT)
    {
      qusb_uart_log("apsd_timeout",0);
    }
  }

  // disable apsd BC1P2_SRC_DETECT
  pm_err |= pm_chg_enable_apsd(7, FALSE);
  
  //enable eusb repeater
  pm_err |= pm_chg_eusb_enable(TRUE);
  
  if (PM_ERR_FLAG_SUCCESS != pm_err)
  { 
    qusb_uart_log("err_disable_apsd",pm_err);
  }
}
...
void qusb_ldr_utils_handle_charger(void)
{
  ...
  // get pmic otp version
  pm_err = pm_get_otp_version(&pmic_otp[1], &pmic_otp[0]);
  if (PM_ERR_FLAG_SUCCESS != pm_err)
  {
   qusb_uart_log("otp_ver_fail",pm_err);
  }

  //for old otp version, run APSD WA in XBL
  if(pm_err == PM_ERR_FLAG_SUCCESS && pmic_otp[0] <= PMIC_DEFAULT_OTP_VERSION)
  {   
    qusb_uart_log("otp_ver",pmic_otp[1]<<8|pmic_otp[0]);
    ...
    if (PM_ERR_FLAG_SUCCESS != pm_err)
    {
      qusb_uart_log("get_typec_sts_err", 0);
      qusb_error_log(LDR_VBUS_DET_ERROR_LOG, 0, pm_err);
    }
	// if controller is alredy running , dont re-run APSD
    else if (FALSE == is_runstop && TRUE == typec_status.vbus_sts && ((usb_charger_type == PM_CHARGER_PORT_SDP_CHARGER) || (usb_charger_type == PM_CHARGER_PORT_FLOAT_CHARGER)))
    {
      qusb_uart_log("re-run APSD", 0);
      qusb_ldr_utils_rerun_apsd();
    }
  }
  usb_charger_type = qusb_ldr_utils_get_chgr_type();  
  ...
  if (FALSE == is_eud_active && FALSE == is_runstop)
  {
    /* pwr down the eUSB2PHY if not in use by EUD or DWC3 core */
    qusb_dci_eusb2phy_enable(FALSE);

    /* reset eusb repeater for later use */
    pm_chg_eusb_enable(FALSE);
    pm_chg_eusb_enable(TRUE);
  }
}
