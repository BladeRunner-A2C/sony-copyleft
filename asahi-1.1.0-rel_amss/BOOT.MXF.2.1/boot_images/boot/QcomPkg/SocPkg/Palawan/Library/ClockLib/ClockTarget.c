...
$Header: //components/dev/core.boot/6.1/arajashe.core.boot.6.1.lanaiuefi/QcomPkg/SocPkg/Kailua/Library/ClockLib/ClockTarget.c#1 $
...
{
  HWIO_OUT(CAM_CC_CAMNOC_AXI_NRT_DIV_DCDR, 1);
  HWIO_OUT(VCODEC_VIDEO_CC_MVS0C_DIV_DCDR, 1);
  HWIO_OUT(VCODEC_VIDEO_CC_MVS1C_DIV_DCDR, 1);
} /* HAL_clk_EnableDCDs */
...
)
{
   ...
   * in GFX PMIC domain)   
   */
  HWIO_OUTF(GPUCC_GPU_CC_ACD_CXO_CBCR, CLK_DIS, 1);
  HWIO_OUT(GPUCC_GPU_CC_GX_BCR, 1);
  HWIO_OUT(GPUCC_GPU_CC_ACD_BCR, 1);
  HWIO_OUT(GPUCC_GPU_CC_ACD_MISC, 1);

  /*  
   * Make sure that above BLK_ARES is kept asserted for at least 100us before
   ...
   *  Enable ACD clocks
   */
  HWIO_OUTF(GPUCC_GPU_CC_ACD_CXO_CBCR, CLK_DIS, 0);
  
  
  HWIO_OUTF(GPUCC_GPU_CC_GX_GFX3D_CMD_RCGR, ROOT_EN, 1);
  ...
  HWIO_OUTF(GPUCC_GPU_CC_GX_GFX3D_CBCR, CLK_ENABLE, 1);
  /*
   * Wait for PD ON
   */
