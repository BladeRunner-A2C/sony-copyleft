  ...
  *Message = UsbCfgPortMsgNone;
  ...
  if (gUsbPortCfg[PortIndex].Type == USB_CONNECTOR_TYPE_C) { // For Type-C ports, primary
    ...
    if (EFI_ERROR (Status)) {
      ...
      DBG(EFI_D_ERROR, "GetTypeCPortStatus returned ERROR (0x%X), switching platform to UNKNOWN on port: %u", Status, PortIndex);
      PlatformInfo.platform = EFI_PLATFORMINFO_TYPE_UNKNOWN;
      // Suppress error until PMIC Charger API is implemented correctly
      Status = EFI_SUCCESS;
      goto ON_EXIT;
    ...
    ERR_CHK("Failed to get VbusStatus on port: %u", PortIndex);
    DBG(EFI_D_INFO, "VbusStatus: %d on port: %u", VbusStatus, PortIndex);
    ...
    {
      DBG(EFI_D_ERROR, "Vbus FALSE %d on port: %u", UsbPort->ConnectState, PortIndex);
  ...
  if (*Message == UsbCfgPortMsgPartnerAttach) {
    UsbPort->ConnectState = USBC_STATE_ATTACHED;
    DBG(EFI_D_INFO, "usbport->connectstate: ATT on Port: %u", PortIndex);
  }
  else {
    UsbPort->ConnectState = USBC_STATE_DETACHED;
    DBG(EFI_D_INFO, "usbport->connectstate: DEATT on Port: %u", PortIndex);

  }

  UsbConfigPortsPrintPortStatus(EFI_D_ERROR, UsbPort, *Message, PortIndex);
