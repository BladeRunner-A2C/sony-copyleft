...
--------   ---     --------------------------------------------------------
07/25/23   STR       Updated spi fw for Palawan
11/22/22   MP        Updated input parameter for spi_get_fw
01/20/22   bng       Initial revision
...
#define MAX_SPI_QUP_INST 3

uint8  __attribute__ ((aligned (4))) qup_v3_13_spi_fw[] =
{
...
char *spi_common_clocks[MAX_SPI_QUP_INST][MAX_SPI_COMMON_CLK_CNT] = {
	{NULL,NULL,NULL,NULL},
...
spi_plat_clock_config spi_clock_config[MAX_SPI_FREQ_CNT] = {
                                                        {19200000,0,0},
                                                        {32000000,0,0},
                                                        {38400000,0,0},
                                                        {48000000,0,0},
...
spi_status spi_fetch_common_clock(spi_plat_device_config *dcfg)
{
    ...
    if(dcfg->qupv3_instance < MAX_SPI_QUP_INST)
    {
        comm = spi_common_clocks[dcfg->qupv3_instance];

        while(i < MAX_SPI_COMMON_CLK_CNT)
       {
...
{
    for(int i=0; i<MAX_SPI_FREQ_CNT; i++)
    {
        dcfg->clock_config[i].se_clock_frequency_khz= spi_clock_config[i].se_clock_frequency_khz;
        dcfg->clock_config[i].bus_speed_khz=spi_clock_config[i].bus_speed_khz;
        dcfg->clock_config[i].clk_div=spi_clock_config[i].clk_div;
    }
}
