...
#
# OVERVIEW:     LCP automation script, create a LCP configure JSON file
#
...
# Regions have names
# All regions must be less than MAX_SIZE
# Encoded with starting address = 0 & ending address = size
# Metadata and metadata_size_in_kb are configured in DSF LCP itself, user doesn't need to configure it, just leave it to "0"
# User can configure ending address, its value should be euqal to size with unit KB
# User can configure region memory type via "attr" and "arc_en" field.
# Memory type, only Encrypt, DARE, DAR and plain DDR type are supported on SM8650
#   Encrypt:   attr bit0 = 1B, attr value: 1
#   ECC:       attr bit1 = 1B, attr value: 2
#   MTE:       attr bit2 = 1B, attr value: 4
#   Meta Deta: attr bit3 = 1B, attr value: 8
#   DARE:      attr bit4 = 1B and arc_en = 1, attr value: 16, arc_en value: 1
#   DAR:       attr bit4 = 1B and arc_en = 0, attr value: 16, arc_en value: 0
...
typedef struct
{
    ...
    uint32 end;                     // Upper 20 bits (35:16) of regino's ending address, it is equal to region's size with KB unit
    ...
    uint8  arc_en;                  // 1 --> DARE with ANTI-REPLY COUNTER, 0 --> DAR w/o ANTI-REPLY COUNTER
...
# Only "regions" field can be changed to configure LCP regions based on their own needs
# Region 0 is for internal use and cannot be edited by user
# Only field 'start', 'end',  'attr' and 'arc_en' can be edited by user
config = {
    ...
    [
        {
            ...
            [
                ...
                },
                {
                    ...
                    'name': "TUI VM",
                    'regionData': {
                        'start': 62336,  # 0xF380, this region start address can be edit by user
                        'end': 69632, #Size 69632KB
                        'metadata': 0,
                        'metadata_size_in_kb': 0,
                ...
                },
                {
                    ...
                    'name': "OEM VM",
                    'regionData': {
                        'start': 63424, # 0xF7C0, this region start address can be edit by user
                        'end': 77824, #Size 77824KB
                        'metadata': 0,
                        'metadata_size_in_kb': 0,
                        'attr': 1, #  Encrypt Type, region attributes(bit 0 - Encrypt, bit 1 - ECC, bit 2 - MTE, bit 3 - Meta Deta, bit 4 - DARE)
                        'arc_en': 0, # 1 --> DARE with ANTI-REPLY COUNTER, 0 --> DAR w/o ANTI-REPLY COUNTER
                ...
                },
                {
                    ...
                    'name': "OEM 1",
                    'regionData': {
                        'start': 0, #this region start address can be edit by user
                        'end': 0,
                        'metadata': 0,
                        'metadata_size_in_kb': 0,
                        'attr': 0,
                ...
                {
                    # Region 9 -- S2 PT, only 'attr' and 'arc_en' can be edit by user
                    'name': "S2 PT",
                    'regionData': {
                        'start': 0, #this region start address can not be edit by user
                        'end': 0,   #this region end address can not be edit by user
                        'metadata': 0,
                        'metadata_size_in_kb': 0,
                        'attr': 1,
                        'arc_en': 0,
...
class ConfigureLCPLib(object):
    ...
    def get_reservedRegions(self, memmap_xml):
        ...
        while i < len(memmap):
            ...
            if "Reserved" in region.regionName or "VM_KERNEL_MEM" in region.regionName or "DARE-TZ" in region.regionName or r'TA/Tags' in region.regionName:
                i = i + 1
                continue
        ...
        for map in self.memap:
            region = RegionSpace(map.regionName, map.addr, map.addr + map.size) 
        ...
        for space in self.lcp_config.ddr_spaces:
            ...
            for regionReserved in self.ReservedRegionSpaces:
                ...
                if regionReserved.start > spaceStartTemp and regionReserved.end < spaceEndTemp:
                    validSpace = RegionSpace("", spaceStartTemp, regionReserved.start)
                    spaceStartTemp = regionReserved.end
                    self.AvailableRegionSpaces.append(validSpace)
                else:
                    spaceStartTemp = regionReserved.end
            
            if spaceStartTemp >= space["start"] and spaceStartTemp <= space["end"]:
                validSpace = RegionSpace("", spaceStartTemp, space["end"])
                self.AvailableRegionSpaces.append(validSpace)
                    ...
                    if "TA S1" in region_name:
                        ...
                        for index, value in enumerate(ConfigureLCPLib.region_spaces):
                            if "TA" in value["name"]:
                                ...
                                region_end = ConfigureLCPLib.region_spaces[index]["end"] + region_end + 1
                                ConfigureLCPLib.region_spaces[index]["name"] = "TA S1 + TA S2" 
                                ConfigureLCPLib.region_spaces[index]["end"] = region_end 
            ...
            if "TA" not in region_name and "S2 PT" not in region_name:
                ...
                for space in ConfigureLCPLib.region_spaces:
                    if space["name"] != region_name:
                        if (region_end < space["end"] and region_end > space["start"]) or \
                            (region_start < space["end"] and region_start > space["start"]) or \
                            (region_start < space["start"] and region_end > space["end"]) or \
                            (region_start < space["end"] and region_end > space["start"]):  
                            self.logger.error("\n***ERROR***")
                            
                            if space["type"] == "DARE" or space["type"] == "DAE":
                                self.logger.error("'%s' region start: '0x%X' end(with tags): '0x%X'", space["name"], space["start"], space["end"])
                            else:
                                self.logger.error("'%s' region start: '0x%X' end: '0x%X'", space["name"], space["start"], space["end"])
    ...
    def region_type_check(self, region_name, region_types, region_attr, region_arc_en):    
        region_type = {}
        available_type = True
        ...
        if region_name == "TA S1" and available_type == False:
            for type in self.lcp_config.lcp["TYPE"]:
                if type["name"] == "DARE":
    ...
    def region_restriction_check(self, region, region_index):
        ...
        region_end = region["regionData"]["end"]
        region_metadata = region["regionData"]["metadata"]
        region_metadata_size_in_kb = region["regionData"]["metadata_size_in_kb"]
        ...
        self.logger.info("region_end: 0x%X", region_end)
        ...
        region_type = self.region_type_check(region_name, region_types, region_attr, region_arc_en)
        ...
        if region_metadata != 0 or region_metadata_size_in_kb != 0:
            self.logger.error("'%s' region's metadata and metadata_size_in_kb shoud always be '0', it cannot be configured by user!", region_name)
            return False
        ...
        if region_max_size != "no restriction" and region_end > region_max_size:
            self.logger.error("'%s' region's size(end) can not exceed %s", region_name, region_max_size)
            return False
        
        if False == self.region_size_check(region_name, region_end, region_type):
            return False
        
        if False == self.region_address_check(region_name, region_start, region_end, region_type):
            return False
        ...
        if region_type["name"] == "DARE":
            ConfigureLCPLib.total_dare_size += region_end
...
def main():
    ...
    r'   python <boot_build_path>\boot_images\boot\QcomPkg\SocPkg\Milos\Tools\LCP\SM8650_LCP_region.py -o <absolute_path>\output -d <absolute_path>\.dcb_config_SM8650.json ')
        ...
        for region in lcpRegion["regions"]:
            ...
            if region_index < NUM_RESERVED_FOR_INTERNAL_USE:
                if regionData["start"] != 0 or \
                    regionData["end"] != 0 or \
                    regionData["metadata"] != 0 or \
                    regionData["metadata_size_in_kb"] != 0 or \
                    regionData["attr"] != 0 or \
                    regionData["arc_en"] != 0 or \
                    regionData["reserved1"] != 0 or \
                    regionData["reserved2"] != 0 :
    ...
    try:
        if args.output_dir is not None:
            config_filename = os.path.join(args.output_dir, 'SM8650_LCP_region.json')
