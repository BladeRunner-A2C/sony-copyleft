...
LCPLibTargetSettings = {
    "SM8650" : {
        "ProfilePkg" : os.path.join("profiles","milos","milos_tme_security_profile.xml"),
        "OSPathDSFPkg" : os.path.join("SocPkg","milos"),
    },
}
...
class LCPLib(object):
                    ...
                    if "TA S1" in region_name:
                        ...
                        for index, value in enumerate(LCPLib.region_spaces):
                            if "TA" in value["name"]:
                                ...
                                region_end = LCPLib.region_spaces[index]["end"] + region_end + 1
                                LCPLib.region_spaces[index]["name"] = "TA S1 + TA S2" 
                                LCPLib.region_spaces[index]["end"] = region_end 
    ...
    def region_restriction_check(self, region, region_index):
        ...
        if region_type["name"] == "DARE":
            LCPLib.total_dare_size += region_end
        ...
        with open(os.path.join(self.output_dir, lcp_bin_file), "wb") as f:
            ...
            for lcpRegion in lcp_file.lcpRegions:
                ...
                for region in lcpRegion["regions"]:
                    ...
                    f.write(start)
                    end = struct.pack("<I", regionData["end"])
                    f.write(end)
                    metadata = struct.pack("<I", regionData["metadata"])
                    f.write(metadata)
                    metadata_size_in_kb = struct.pack("<I", regionData["metadata_size_in_kb"])
                    f.write(metadata_size_in_kb)
                    attr = struct.pack("<B", regionData["attr"])
                    f.write(attr)
                    arc_en = struct.pack("<B", regionData["arc_en"])
                    f.write(arc_en)
                    reserved1 = struct.pack("<B", regionData["reserved1"])
                    f.write(reserved1)
                    reserved2 = struct.pack("<B", regionData["reserved2"])
                    f.write(reserved2)
    ...
    def convert_bin2json(self, lcp_bin_file):
        ...
        if size % self.LCP_ADDR_REGIONS_STRUCT_SIZE != 0:
            self.logger.error("LCP bin file is broken, please provide a correct LCP bin file!")

        lcpRegions = []
        for lcpRegions_index in range(int(size / (self.LCP_ADDR_REGIONS_STRUCT_SIZE * 10))):
            lcpRegion = dict()
            #lcpRegion["header"] = NULL
            
            regions = []
            for region_index in range(int(size / self.LCP_ADDR_REGIONS_STRUCT_SIZE)):
                print("region_index: ", region_index)
                lcpBinFile.seek(lcpRegions_index * (self.LCP_ADDR_REGIONS_STRUCT_SIZE * 10) + region_index * self.LCP_ADDR_REGIONS_STRUCT_SIZE, 0)
                ...
                region["regionData"]["end"] = end
                region["regionData"]["metadata"] = metadata
                region["regionData"]["metadata_size_in_kb"] = metadata_size_in_kb
                region["regionData"]["attr"] = attr
                region["regionData"]["arc_en"] = arc_en
                region["regionData"]["reserved1"] = reserved1
                region["regionData"]["reserved2"] = reserved2
...
def main():
    ...
    r'   python <boot_build_path>\boot_images\boot\QcomPkg\SocPkg\Milos\Tools\LCP\lcp.py --target=SM8650 -v DEBUG -s <meta_build_path>\common\sectoolsv2\ext -o <absolute_path>\output in -l <absolute_path>\LCP.bin -x <absolute_path>\xbl_config.elf'
    '\r\n' +
    '2, example to update DCB bins to include LCP bin:\r\n' +
    r'   python <boot_build_path>\boot_images\boot\QcomPkg\SocPkg\Milos\Tools\LCP\lcp.py --target=SM8650 -v DEBUG -o <absolute_path>\output  in -l <absolute_path>\LCP.bin -x <absolute_path>\<dcb_bin_path>'
    '\r\n' +
    '3, example to convert lcp josn file to LCP region bin file:\r\n' +
    r'   python <boot_build_path>\boot_images\boot\QcomPkg\SocPkg\Milos\Tools\LCP\lcp.py --target=SM8650 -v DEBUG -o <absolute_path>\output convert -d <absolute_path>\.dcb_config_SM8650.json -l <absolute_path>\SM8650_LCP_region.json'
    '\r\n' +
    '4, example to convert lcp bin file to LCP region json file:\r\n' +
    r'   python <boot_build_path>\boot_images\boot\QcomPkg\SocPkg\Milos\Tools\LCP\lcp.py --target=SM8650 -v DEBUG -o <absolute_path>\output convert -d <absolute_path>\.dcb_config_SM8650.json -l <absolute_path>\SM8650_LCP_region.bin')
