...
uint32_t Clock_DCD_Cfg[] =
{
  ...
  HWIO_ADDR(GCC_SYS_NOC_GC_AXI_DCD_CDIV_DCDR),
...
boolean Boot_Clock_ODSC_Ctl = FALSE;
...
uint32_t Clock_ODSC_Ctl_Cfg[] =
{
  ...
  // Following list are highlighed ODSC which didn't work in the prev chip as per HPG 2.16 , 1.2  
  HWIO_ADDR(GCC_AHB_CONFIG_CBCR),
  HWIO_ADDR(GCC_AHB2PHY_0_CBCR),
  HWIO_ADDR(GCC_AHB2PHY_1_CBCR),
  ...
  HWIO_ADDR(GCC_TCU_CFG_QX_CBCR),
  HWIO_ADDR(GCC_TLMM_AHB_CBCR),
...
boolean Boot_Clock_DCG_Ctl = FALSE;

uint32_t Clock_DCG_Ctl_Cfg[] =
{
  HWIO_ADDR(GCC_AGGRE_NOC_PCIE_AXI_CLONE_CBCR),
  HWIO_ADDR(GCC_CNOC_PCIE_SF_AXI_CBCR),
  HWIO_ADDR(GCC_DDRSS_GIC_CBCR),
  HWIO_ADDR(GCC_CFG_NOC_PCIE_ANOC_AHB_CBCR),
  HWIO_ADDR(GCC_AGGRE_NOC_PCIE_AXI_CBCR),
  HWIO_ADDR(GCC_ANOC_PCIE_PWRCTL_CBCR),
  ...
  HWIO_ADDR(GCC_DDRSS_GPU_AXI_CBCR),
  HWIO_ADDR(GCC_DDRSS_PCIE_SF_QTB_CBCR),
  HWIO_ADDR(GCC_MEMNOC_CBCR),
  HWIO_ADDR(GCC_DDRSS_LPASS_SHUB_CBCR),
  HWIO_ADDR(GCC_DDRSS_PWRCTL_CBCR),
  HWIO_ADDR(GCC_DDRSS_WPSS_AXI_CBCR),
...
)
{
  ...
  HWIO_OUTF(GCC_PCIE_1_MSTR_AXI_CBCR , IGNORE_PMU_CLK_DIS, 1); /* QCTDD06646862 */
...
)
{
  ...
  if (!RUMI_CHECK())
  {
    while(!HWIO_INF(PHY_REFGEN_0_EAST_RESTRIM_CODE_STATUS, REFGEN_READY));
    while(!HWIO_INF(PHY_REFGEN_WEST_RESTRIM_CODE_STATUS, REFGEN_READY));
  }

  // Programs REFGEN to accept votes via Hardware pins.
  HWIO_OUT(PHY_REFGEN_0_EAST_CMN_REFGEN_CTRL1, 0x8);
  HWIO_OUT(PHY_REFGEN_WEST_CMN_REFGEN_CTRL1, 0x8);
  
  //Prepare for future analog rail collapse - clamp analog inputs to digital
  HWIO_OUT(PHY_REFGEN_0_EAST_CMN_REFGEN_CTRL2, 0x3);
  HWIO_OUT(PHY_REFGEN_WEST_CMN_REFGEN_CTRL2, 0x3);

  // Milos DACC IPTUNE Value to 0xF
  HWIO_OUTF(PHY_REFGEN_0_EAST_IP_TUNEH, CMN_IP_TUNE_CTRLH_4_0, 0xF);
  HWIO_OUTF(PHY_REFGEN_WEST_IP_TUNEH,  CMN_IP_TUNE_CTRLH_4_0, 0xF);
}
...
static void Clock_ApplyWorkarounds(void)
{
  ...
  HWIO_OUTF(GCC_CAMERA_SREG_MISC, HF_AXI_MICRO_FORCE_MEM_CORE_ON, 1);
  HWIO_OUTF(GCC_CAMERA_SREG_MISC, SF_AXI_MICRO_FORCE_MEM_CORE_ON, 1);
  HWIO_OUTF(GCC_DISP_SREG_MISC, HF_AXI_MICRO_FORCE_MEM_CORE_ON, 1);
  HWIO_OUTF(GCC_VIDEO_SREG_MISC, AXI_1_MICRO_FORCE_MEM_CORE_ON, 1);
  HWIO_OUTF(GCC_VIDEO_SREG_MISC, AXI_0_MICRO_FORCE_MEM_CORE_ON, 1);
  HWIO_OUTF(GCC_ANOC_QTB_SREG_MISC, QTB_1_MICRO_FORCE_MEM_CORE_ON, 1);
  HWIO_OUTF(GCC_ANOC_QTB_SREG_MISC, QTB_2_MICRO_FORCE_MEM_CORE_ON, 1);
  ...
  /* In PCIe reset mid-operation test cases, seeing the BDF memory is getting corrupted due to slp_nret_n & slp_ret_n 
   * signals going low. This behavior is observed due to the change of power-on reset value of 
   * GCC_PCIE_0_MSTR_AXI_CBCR[FORCE_MEM_CORE_ON] to 0x0 starting Milos (hence, inherited from Lahaina onwards). 
   * PCIe expects mem_core_on to be asserted (i.e. 0x1), so it is necessary for SW to set FORCE_MEM_CORE_ON back to 0x1
   */
  HWIO_OUTF(GCC_PCIE_0_MSTR_AXI_CBCR, FORCE_MEM_CORE_ON, 0x1);
  HWIO_OUTF(GCC_PCIE_1_MSTR_AXI_CBCR, FORCE_MEM_CORE_ON, 0x1);
}
...
)
{
   ...
   * Apply the workaround to swit ch the GFMUX to GPLL0
   */
  while((HWIO_INF(APSS_CC_SILVER_MUX_GFMUXR, STS_SEL_A) == 0x1) &&
        (HWIO_INF(APSS_CC_SILVER_MUX_GFMUXR, MUX_SEL) == 0x1))
  {
    Clock_ApplyGfMuxWA();
    nQdssApbResetCnt++;
