...
2024-01-22   pk      Milos reference clock update
2024-01-11   pk      Milos regulator configuration
2023-10-26   gk      Milos initial changes, Removed Rumi emulation dut register,used for reset.
2023-07-24   sa      Add the values for de-emphasis.
2023-06-02   sa      Set right value for MAX_CORE_CLK_1US_CYCLES for gear5.
2023-04-21   jt      Add Eye Monitor PHY settings 
2022-12-22   sa      Share info using UFS_MEM_DEBUG_SPARE_CFG register.
2022-12-05   sa      Enable LDO's and change the gear.
...
2022-03-03   jt      Initial Milos changes 
...
// Macro for voltage LDOs.
#define PMIC_LDO_UFS0_VCC              "ldob12"
#define PMIC_LDO_UFS0_VCCQ             "ldof5"
#define PMIC_LDO_UFS0_VCCQ2            "ldof4"  //Only required for Ufs 2.x for backward compatibility.
#define PMIC_LDO_UFS0_VDDA             "ldob4"
#define PMIC_LDO_UFS0_VDDA_CORE        "ldob2"
#define PMIC_LDO_UFS0_QREF             "ldob2"

#define LDO_ENABLE(arg)                "/pm/" arg "/en"
#define LDO_VOLT(arg)                  "/pm/" arg "/mV"
#define LDO_MODE(arg)                  "/pm/" arg "/mode"

#define UFS_REF_CLOCK                       19200000
...
static ufs_bsp_pm_info ufs_pm_params[] = 
{
    ...
    { VOLT_CORNER_NOM,      UFS_GEAR_4,       { 300, 12,  300}},
    { VOLT_CORNER_TURBO,    UFS_GEAR_4,       { 300, 12,  300}}
};
...
ufs_bsp_err_type ufs_bsp_volt_set(uint8_t hostid, ufs_bsp_regulator_mode mode)
{
   ...
   UINT32 voltage_req_rail;
   static npa_client_handle vcc_mV_client_handle = NULL;
   static npa_client_handle vcc_en_client_handle = NULL;
   static npa_client_handle vcc_mode_client_handle = NULL;
   static npa_client_handle vccq_mV_client_handle = NULL;
   static npa_client_handle vccq_en_client_handle = NULL;
   static npa_client_handle vccq_mode_client_handle = NULL;
   static npa_client_handle vccq2_mV_client_handle = NULL;
   static npa_client_handle vccq2_en_client_handle = NULL;
   static npa_client_handle vccq2_mode_client_handle = NULL;
   static npa_client_handle vdda_mV_client_handle = NULL;
   static npa_client_handle vdda_en_client_handle = NULL;
   static npa_client_handle vdda_mode_client_handle = NULL;
   static npa_client_handle vdda_core_mV_client_handle = NULL;
   static npa_client_handle vdda_core_en_client_handle = NULL;
   static npa_client_handle vdda_core_mode_client_handle = NULL;
   static npa_client_handle qref_mV_client_handle = NULL;
   static npa_client_handle qref_en_client_handle = NULL;
   static npa_client_handle qref_mode_client_handle = NULL;
   
   if (hostid == 0) {
      if (vcc_mV_client_handle == NULL) 
      {
         vcc_mV_client_handle = npa_create_sync_client(LDO_VOLT(PMIC_LDO_UFS0_VCC), "ufs0_vcc", NPA_CLIENT_REQUIRED); 
         if (vcc_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vcc_en_client_handle == NULL) 
      {
         vcc_en_client_handle = npa_create_sync_client(LDO_ENABLE(PMIC_LDO_UFS0_VCC), "ufs0_vcc", NPA_CLIENT_REQUIRED); 
         if (vcc_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vcc_mode_client_handle == NULL) 
      {
         vcc_mode_client_handle = npa_create_sync_client(LDO_MODE(PMIC_LDO_UFS0_VCC), "ufs0_vcc", NPA_CLIENT_REQUIRED); 
         if (vcc_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
  
      if (vccq_mV_client_handle == NULL) {
         vccq_mV_client_handle = npa_create_sync_client(LDO_VOLT(PMIC_LDO_UFS0_VCCQ), "ufs0_vccq", NPA_CLIENT_REQUIRED); 
         if (vccq_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vccq_en_client_handle == NULL) {
         vccq_en_client_handle = npa_create_sync_client(LDO_ENABLE(PMIC_LDO_UFS0_VCCQ), "ufs0_vccq", NPA_CLIENT_REQUIRED); 
         if (vccq_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vccq_mode_client_handle == NULL) {
         vccq_mode_client_handle = npa_create_sync_client(LDO_MODE(PMIC_LDO_UFS0_VCCQ), "ufs0_vccq", NPA_CLIENT_REQUIRED); 
         if (vccq_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
	  
      if (vccq2_mV_client_handle == NULL) {
         vccq2_mV_client_handle = npa_create_sync_client(LDO_VOLT(PMIC_LDO_UFS0_VCCQ2), "ufs0_vccq2", NPA_CLIENT_REQUIRED); 
         if (vccq2_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vccq2_en_client_handle == NULL) {
         vccq2_en_client_handle = npa_create_sync_client(LDO_ENABLE(PMIC_LDO_UFS0_VCCQ2), "ufs0_vccq2", NPA_CLIENT_REQUIRED); 
         if (vccq2_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vccq2_mode_client_handle == NULL) {
         vccq2_mode_client_handle = npa_create_sync_client(LDO_MODE(PMIC_LDO_UFS0_VCCQ2), "ufs0_vccq2", NPA_CLIENT_REQUIRED); 
         if (vccq2_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }	  
  
      if (vdda_mV_client_handle == NULL) {
         vdda_mV_client_handle = npa_create_sync_client(LDO_VOLT(PMIC_LDO_UFS0_VDDA), "ufs0_vdda", NPA_CLIENT_REQUIRED); 
         if (vdda_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vdda_en_client_handle == NULL) {
         vdda_en_client_handle = npa_create_sync_client(LDO_ENABLE(PMIC_LDO_UFS0_VDDA), "ufs0_vdda", NPA_CLIENT_REQUIRED); 
         if (vdda_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vdda_mode_client_handle == NULL) {
         vdda_mode_client_handle = npa_create_sync_client(LDO_MODE(PMIC_LDO_UFS0_VDDA), "ufs0_vdda", NPA_CLIENT_REQUIRED); 
         if (vdda_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
  
      if (vdda_core_mV_client_handle == NULL) {
         vdda_core_mV_client_handle = npa_create_sync_client(LDO_VOLT(PMIC_LDO_UFS0_VDDA_CORE), "ufs0_vdda_core", NPA_CLIENT_REQUIRED); 
         if (vdda_core_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vdda_core_en_client_handle == NULL) {
         vdda_core_en_client_handle = npa_create_sync_client(LDO_ENABLE(PMIC_LDO_UFS0_VDDA_CORE), "ufs0_vdda_core", NPA_CLIENT_REQUIRED); 
         if (vdda_core_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (vdda_core_mode_client_handle == NULL) {
         vdda_core_mode_client_handle = npa_create_sync_client(LDO_MODE(PMIC_LDO_UFS0_VDDA_CORE), "ufs0_vdda_core", NPA_CLIENT_REQUIRED); 
         if (vdda_core_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      
      if (qref_mV_client_handle == NULL) {
           qref_mV_client_handle = npa_create_sync_client(LDO_VOLT(PMIC_LDO_UFS0_QREF), "ufs0_qref", NPA_CLIENT_REQUIRED); 
         if (qref_mV_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (qref_en_client_handle == NULL) {
         qref_en_client_handle = npa_create_sync_client(LDO_ENABLE(PMIC_LDO_UFS0_QREF), "ufs0_qref", NPA_CLIENT_REQUIRED); 
         if (qref_en_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
      if (qref_mode_client_handle == NULL) {
         qref_mode_client_handle = npa_create_sync_client(LDO_MODE(PMIC_LDO_UFS0_QREF), "ufs0_qref", NPA_CLIENT_REQUIRED); 
         if (qref_mode_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
   ...
   if (mode == UFS_BSP_REGULATORS_ON) {
      ...
      Status=PmicVRegProtocol->VregGetLevel(0x1 /*Bonefish  B 1*/, EFI_PM_LDO_12, &voltage_req_rail);
      if (Status != EFI_SUCCESS) {
         return UFS_BSP_ERR_PMIC;
      }

       /* Based on JESD220E-UFS3.1 Table 6.3- UFS Power Supply Parameters */
      if(voltage_req_rail >= 2400000 && voltage_req_rail <= 2700000) {
    
        npa_issue_required_request(vcc_mV_client_handle, 2504);
        npa_issue_required_request(vcc_en_client_handle, 1);
        npa_issue_required_request(vcc_mode_client_handle, 7);

        npa_issue_required_request(vccq_mV_client_handle, 1200);
        npa_issue_required_request(vccq_en_client_handle, 1);
        npa_issue_required_request(vccq_mode_client_handle, 7);
      }
      else {
      
        npa_issue_required_request(vcc_mV_client_handle, 2952);
        npa_issue_required_request(vcc_en_client_handle, 1);
        npa_issue_required_request(vcc_mode_client_handle, 7);

      
        npa_issue_required_request(vccq2_mV_client_handle, 1800);
        npa_issue_required_request(vccq2_en_client_handle, 1);
        npa_issue_required_request(vccq2_mode_client_handle, 7);
      }


   
      npa_issue_required_request(qref_mV_client_handle, 880);
      npa_issue_required_request(qref_en_client_handle, 1);
      npa_issue_required_request(qref_mode_client_handle, 7);

   
      npa_issue_required_request(vdda_mV_client_handle, 1200);
      npa_issue_required_request(vdda_en_client_handle, 1);
      npa_issue_required_request(vdda_mode_client_handle, 7);

      
      npa_issue_required_request(vdda_core_mV_client_handle, 880);
      npa_issue_required_request(vdda_core_en_client_handle, 1);
      npa_issue_required_request(vdda_core_mode_client_handle, 7);

   }
   /* Lower Vcc to 0V for UFS device sleep */
   else if (mode == UFS_BSP_REGULATORS_SLEEP) {
      npa_issue_required_request(vcc_mV_client_handle, 0);
      npa_issue_required_request(vcc_en_client_handle, 0);
      npa_issue_required_request(vcc_mode_client_handle, 5);
   }
   else if (mode == UFS_BSP_REGULATORS_OFF) {
     npa_issue_required_request(vdda_core_mV_client_handle, 1200);
     npa_issue_required_request(vdda_core_en_client_handle, 1);
     npa_issue_required_request(vdda_core_mode_client_handle, 5);

     npa_issue_required_request(qref_mV_client_handle, 0);
     npa_issue_required_request(qref_en_client_handle, 0);
     npa_issue_required_request(qref_mode_client_handle, 5);

     npa_issue_required_request(vdda_mV_client_handle, 0);
     npa_issue_required_request(vdda_en_client_handle, 0);
     npa_issue_required_request(vdda_mode_client_handle, 5);

     npa_issue_required_request(vccq_mV_client_handle, 1200);
     npa_issue_required_request(vccq_en_client_handle, 1);
     npa_issue_required_request(vccq_mode_client_handle, 5);

     npa_issue_required_request(vccq2_mV_client_handle, 1200);
     npa_issue_required_request(vccq2_en_client_handle, 1);
     npa_issue_required_request(vccq2_mode_client_handle, 5);
...
{	//Milos is 3.1, This is to pass the Ufs version info to HLOS. Not required for 3.1 only required for Ufs4.0.
