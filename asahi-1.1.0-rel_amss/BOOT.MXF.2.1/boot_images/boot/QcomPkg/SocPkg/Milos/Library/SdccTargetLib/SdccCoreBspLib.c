...
----------   ---     ----------------------------------------------------------- 
2024-03-06   sb      Moved SD Card pins to single sdc2_active node
2024-03-05   sb      Enable SD Card muxed with GPIOs
2024-02-14   sb      Update SOC emulation check 
2022-11-29   sa      Changes to enable LDO's.
...
/* Handles for LDO's */
static npa_client_handle sd_mmc_vdd_mV_handle = NULL;
static npa_client_handle sd_mmc_vdd_en_handle = NULL;
static npa_client_handle sd_mmc_vdd_mode_handle = NULL;

static npa_client_handle vccb_ls_sdc_mV_handle = NULL;
static npa_client_handle vccb_ls_sdc_en_handle = NULL;
static npa_client_handle vccb_ls_sdc_mode_handle = NULL;

static npa_client_handle vcca_ls_sdc_mV_handle = NULL;
static npa_client_handle vcca_ls_sdc_en_handle = NULL;
static npa_client_handle vcca_ls_sdc_mode_handle = NULL;
   ...
   switch(slot_num)
   {
      case SDCC_BSP_SLOT_1:
         ...
         if (NULL != DtbProtocol) 
         {
            ...
            {
               snprintf(FdtNodeStr, SDCC_BSP_DT_PATH_LEN, "/soc/sdc%d/HsrSettings/SDR104", slot_num);
            }
...
{ 
   ...
   if (driveno == SDCC_BSP_SLOT_1) 
   {
      ...
      if (vccb_ls_sdc_mV_handle == NULL) 
      {
         vccb_ls_sdc_mV_handle = npa_create_sync_client("/pm/ldob23/mV", "vccb_ls_sdc", NPA_CLIENT_REQUIRED);
      }

      if (vccb_ls_sdc_en_handle == NULL) 
      {
         vccb_ls_sdc_en_handle = npa_create_sync_client("/pm/ldob23/en", "vccb_ls_sdc", NPA_CLIENT_REQUIRED);
      }

      if (vccb_ls_sdc_mode_handle == NULL) 
      {
         vccb_ls_sdc_mode_handle = npa_create_sync_client("/pm/ldob23/mode", "vccb_ls_sdc", NPA_CLIENT_REQUIRED);
      }

      if ((vccb_ls_sdc_mV_handle == NULL) || (vccb_ls_sdc_en_handle == NULL) 
            || (vccb_ls_sdc_mode_handle == NULL)) 
      {
   ...
   /* Enable and set mode */
   npa_issue_required_request(vccb_ls_sdc_en_handle, 1);
   npa_issue_required_request(vccb_ls_sdc_mode_handle, 7);
   npa_issue_required_request(vccb_ls_sdc_mV_handle, 1800);
...
sdcc_bsp_err_type sdcc_bsp_volt_set (UINT32 driveno, sdcc_bsp_regulator_mode mode)
{
   ...
   if (driveno == SDCC_BSP_SLOT_1)
   {
      if (sd_mmc_vdd_mV_handle == NULL) 
      {
         sd_mmc_vdd_mV_handle = npa_create_sync_client("/pm/ldob13/mV", "sd_mmc_vdd", NPA_CLIENT_REQUIRED);
      }

      if (sd_mmc_vdd_en_handle == NULL) 
      {
         sd_mmc_vdd_en_handle = npa_create_sync_client("/pm/ldob13/en", "sd_mmc_vdd", NPA_CLIENT_REQUIRED);
      }

      if (sd_mmc_vdd_mode_handle == NULL) 
      {
         sd_mmc_vdd_mode_handle = npa_create_sync_client("/pm/ldob13/mode", "sd_mmc_vdd", NPA_CLIENT_REQUIRED);
      }

      if ((sd_mmc_vdd_mV_handle == NULL) || (sd_mmc_vdd_en_handle == NULL) || (sd_mmc_vdd_mode_handle == NULL)) 
      {
        return SDCC_BSP_ERR_PMIC;
      }
      
      if (vcca_ls_sdc_mV_handle == NULL) 
      {
         vcca_ls_sdc_mV_handle = npa_create_sync_client("/pm/ldob5/mV", "vcca_ls_sdc", NPA_CLIENT_REQUIRED);
      }

      if (vcca_ls_sdc_en_handle == NULL) 
      {
         vcca_ls_sdc_en_handle = npa_create_sync_client("/pm/ldob5/en", "vcca_ls_sdc", NPA_CLIENT_REQUIRED);
      }

      if (vcca_ls_sdc_mode_handle == NULL) 
      {
         vcca_ls_sdc_mode_handle = npa_create_sync_client("/pm/ldob5/mode", "vcca_ls_sdc", NPA_CLIENT_REQUIRED);
      }

      if ((vcca_ls_sdc_mV_handle == NULL) || (vcca_ls_sdc_en_handle == NULL) || (vcca_ls_sdc_mode_handle == NULL)) 
      {
         return SDCC_BSP_ERR_PMIC;
      }
      ...
      if ((vccb_ls_sdc_mV_handle == NULL) || (vccb_ls_sdc_en_handle == NULL) || (vccb_ls_sdc_mode_handle == NULL)) 
      {
         return SDCC_BSP_ERR_PMIC;
      }
      ...
      {
         npa_issue_required_request(sd_mmc_vdd_mV_handle, 2960);
         npa_issue_required_request(sd_mmc_vdd_en_handle, 1);
         npa_issue_required_request(sd_mmc_vdd_mode_handle, 7);

         npa_issue_required_request(vccb_ls_sdc_mV_handle, 2960);
         npa_issue_required_request(vccb_ls_sdc_en_handle, 1);
         npa_issue_required_request(vccb_ls_sdc_mode_handle, 7);

         npa_issue_required_request(vcca_ls_sdc_mV_handle, 1200);
         npa_issue_required_request(vcca_ls_sdc_en_handle, 1);
         npa_issue_required_request(vcca_ls_sdc_mode_handle, 7);
      }
      else if (mode == SDCC_BSP_REGULATORS_OFF) 
      {
         npa_issue_required_request(sd_mmc_vdd_en_handle, 0);
         npa_issue_required_request(vccb_ls_sdc_en_handle, 0);
         npa_issue_required_request(vcca_ls_sdc_en_handle, 0);
      }
      else
      {
         return SDCC_BSP_ERR_INVALID_PARAM;
