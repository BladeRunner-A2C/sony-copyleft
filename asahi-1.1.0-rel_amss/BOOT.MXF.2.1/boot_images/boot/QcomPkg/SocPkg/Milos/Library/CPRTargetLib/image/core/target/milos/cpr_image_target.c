 ...
 * $Header: //components/dev/core.boot/6.1/cvelempa.core.boot.6.1.Milos_XBL_CPR_Changes_07_04_2022/QcomPkg/SocPkg/milos/Library/CPRTargetLib/image/core/target/milos/cpr_image_target.c#1 $
 * $Change: 38231469 $
 */
...
const cpr_settings* cpr_image_target_get_settings(void)
{
    ...
    CPR_ASSERT(cpr_settings_milos);

    cpr_settings_milos->railsCount = 1;
    cpr_settings_milos->enablementsCount = 1;
    cpr_settings_milos->controllersCount = 1;

    cpr_settings_milos->railVVPs = (cpr_versioned_voltage_plan **) \
        cpr_image_malloc(sizeof(cpr_versioned_voltage_plan *) * \
                cpr_settings_milos->railsCount);
    for (i = 0; i < cpr_settings_milos->railsCount; i++) {
        cpr_settings_milos->railVVPs[i] = (cpr_versioned_voltage_plan *) \
            cpr_image_malloc(sizeof(cpr_versioned_voltage_plan));
    }

    cpr_settings_milos->controllers = (cpr_hal_controller **) \
        cpr_image_malloc(sizeof(cpr_hal_controller *) * \
                cpr_settings_milos->controllersCount);
    for (i = 0; i < cpr_settings_milos->controllersCount; i++) {
        cpr_settings_milos->controllers[i] = (cpr_hal_controller *) \
            cpr_image_malloc(sizeof(cpr_hal_controller));
    }

    cpr_settings_milos->rails = (cpr_rail **) cpr_image_malloc(
            sizeof(cpr_rail *) * cpr_settings_milos->railsCount);
    for (i = 0; i < cpr_settings_milos->railsCount; i++) {
        cpr_settings_milos->rails[i] =
            (cpr_rail *) cpr_image_malloc(sizeof(cpr_rail));
    }

    cpr_settings_milos->enablements = (cpr_enablement **) \
        cpr_image_malloc(sizeof(cpr_enablement *) * \
                cpr_settings_milos->enablementsCount);
    for (i = 0; i < cpr_settings_milos->enablementsCount; i++) {
        cpr_settings_milos->enablements[i] =
            (cpr_enablement *)cpr_image_malloc(sizeof(cpr_enablement));
    }

    cpr_settings_milos->miscCfg = (cpr_misc_cfg *)
        cpr_image_malloc(sizeof(cpr_misc_cfg));

    cpr_rail gfx_rail;
    gfx_rail.id = CPR_RAIL_GFX;
    cpr_smem_deserialize_settings(&gfx_rail, cpr_settings_milos, /*idx*/ 0);
