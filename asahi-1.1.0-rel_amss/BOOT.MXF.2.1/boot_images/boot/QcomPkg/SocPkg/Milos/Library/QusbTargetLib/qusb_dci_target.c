...
//============================================================================
/**
  Copyright (c) 2013-2015, 2017-2023 Qualcomm Technologies, Inc. All rights reserved.
...
#include "pm_chg.h"
#include "Clock.h"
#include "pm_ldo.h"               // Turn off LDO for EUD recovery
#include "pm_version.h"           // Turn off LDO for EUD recovery
...
// delay_us_per_loop * loop_count.   Currently 400 milliseconds from VBUS detect
#define QUSB_DCI_SS_LANE_TIMEOUT_DELAY_US_PER_LOOP            (100)
#define QUSB_DCI_SS_LANE_TIMEOUT_LOOP_COUNT                   (4000)


// Total timeout for PMIC to finish for charger detection is
// delay_us_per_loop * loop_count.   Currently 1000 milliseconds from VBUS detect
#define QUSB_DCI_CHGDET_TIMEOUT_DELAY_US_PER_LOOP             (1000)
#define QUSB_DCI_CHGDET_TIMEOUT_LOOP_COUNT                    (1000)
#define QUSB_DCI_CHGDET_CDP_DP_TOGGLE_MS                      (200)
...
* Device has to honor hosts(receivers) NumP. Since Win7 xHCI advertises NumP=4, NumP > 4 does not seem to make much difference.
* Usage of TxFIFO:
...
// ===========================================================================
/**
 * @function    qusb_dci_config_usb_clock
 * 
 ...
 */
// ===========================================================================
static ClockHandle clock_handle = (unsigned long long) 0;
static ClockIdType clock_id = 0;
static boolean is_attached = FALSE;
boolean qusb_dci_attach_usb_clock(void)
{
...
boolean qusb_dci_config_usb_clock(boolean enable)
{  
  ClockResult eResult = CLOCK_SUCCESS;
  if (FALSE == is_attached)
  {
    if(FALSE == qusb_dci_attach_usb_clock())
    {
      return FALSE;
    }
  }    
...
boolean qusb_dci_is_usb30_sw_collapsed(void)
{
  ...
  //This API is removed, clock enable will take care the GSDC value
  ...
  eResult =  Clock_IsOn(clock_handle, clock_id, &clk_status);
  if ((eResult != CLOCK_SUCCESS) || (FALSE == clk_status))
  {
    return TRUE;
  }
  return FALSE;
...
void qusb_dci_configure_device_mode(qusb_max_speed_required_t speed_required)
{
  ...
  switch(speed_required)
  {
    ...
    {
#if 0 // Do not override speed for QRD since RamDump from HLOS is more common scenario
      if (DALPLATFORMINFO_TYPE_QRD == DalPlatformInfo_Platform())
      {
        qusb_uart_log("qrd_limit_usb31gen1", 0);
...
#if 0
// only USB3_PRI_LINK_REGS_p_LU1LFPSRXTIM present in ipcat
...
{
  //Dont need to reset usbdp mode, GCC reset take care of this
  //qusb_dci_usbdp_mode_reset(qusb_dci_ss_lane);
  usb_shared_redriver_set_usb_mode(0, USB4_MODE_TYPE_USB3, qusb_dci_ss_lane);
 ...
 *  
 * @details Trigger EUD reset using VIOCTL_EUD_DETECT
 * 
 * @param   None.
 * 
 * @return  None.
...
void qusb_dci_eud_recovery(void)
{
  ...
  if (is_eud_active)
  {
    // Toggling VIOCTL_EUD_DETECT input will reset EUD
    usb_shared_uart_log_error("qusb_dci_eud_recovery vioctl_eud_detect:", HWIO_VIOCTL_EUD_DETECT_IN);
    HWIO_VIOCTL_EUD_DETECT_OUTM(HWIO_VIOCTL_EUD_DETECT_EUD_DETECT_BMSK, 0x0);

    qusb_dci_delay_ms(10);

    HWIO_VIOCTL_EUD_DETECT_OUTM(HWIO_VIOCTL_EUD_DETECT_EUD_DETECT_BMSK, 0x1);
  }
}
 ...
 * 
 * @param   usb_ss_phy_lane: type-c cc lane
 * 
 * @return  None
 * 
 */
...
{
  qusb_uart_log("qusb_dci_usbdp_mode_reset: init", usb_ss_phy_lane);

  // Perform DP USB common block programming sequence here based upon mode selected here

  // usb3_dp_com_reset_ovrd_ctrl
  // USB3/DP Reset Override Control
  HWIO_USB3_DP_PHY_USB3_DP_COM_RESET_OVRD_CTRL_OUT(0xF);

  // usb3_dp_com_phy_mode_ctrl
  // 00 - Reserved
  // 01 - USB3 enabled, DP disabled
  // 10 - USB3 disabled, DP enabled
  // 11 - Both USB3 and DP enabled to operate concurrently
  HWIO_USB3_DP_PHY_USB3_DP_COM_PHY_MODE_CTRL_OUT(0x3);

  // usb3_dp_com_sw_reset   0x01 
  // Perform software reset of Phy Common logic
  HWIO_USB3_DP_PHY_USB3_DP_COM_SW_RESET_OUT(0x1);

  // usb3_dp_com_typec_ctrl '   If port_select is 1, value should be 0x03 else 0x02
  // Usb3 Type-C Control
  ...
  // usb3_dp_com_swi_ctrl  0x00   Leave it to default
  HWIO_USB3_DP_PHY_USB3_DP_COM_SWI_CTRL_OUT(0x00);

  // usb3_dp_com_sw_reset    0x00
  HWIO_USB3_DP_PHY_USB3_DP_COM_SW_RESET_OUT(0x0);

  //usb3_dp_com_power_down_ctrl    0x01
  HWIO_USB3_DP_PHY_USB3_DP_COM_POWER_DOWN_CTRL_OUT(0x01);

  // usb3_dp_com_reset_ovrd_ctrl   0x00
  HWIO_USB3_DP_PHY_USB3_DP_COM_RESET_OVRD_CTRL_OUT(0x00);
}

// ===========================================================================
/**
 * @function    qusb_dci_deinit_phy
 * 
 * @brief   API used to set the USB2 and USB3 state to default (disconnect) in 
 *          Controller Wrapper register.
 * 
 * @param   None.
 * 
 * @return  None.
...
void qusb_dci_deinit_phy(void)
{
  ...
  // Clear super speed lane to default in case it was set during init.
...
void qusb_dci_handle_charger_cdp(void)
{
  uint32                   loop_count       = 0;
  pm_chg_charger_port_type usb_charger_type = PM_CHARGER_PORT_TYPE_INVALID;  
  uint32                   pm_err           = PM_ERR_FLAG_SUCCESS;

  for (loop_count = 0; loop_count < QUSB_DCI_CHGDET_TIMEOUT_LOOP_COUNT; loop_count++)
  {
    pm_err = pm_chg_get_charger_port_type(&usb_charger_type);
    if (PM_ERR_FLAG_SUCCESS != pm_err)
    {
      qusb_uart_log("dci, chgr_type_det_err", 0);
    ...
    qusb_dci_delay_us(QUSB_DCI_CHGDET_TIMEOUT_DELAY_US_PER_LOOP);
  }
  
  qusb_uart_log("usb: chgr", usb_charger_type);

  if (PM_CHARGER_PORT_CDP_CHARGER == usb_charger_type)
  {
    // Toggle D+ for N interval to comply with CDP
    usb_shared_dp_toggle(QUSB_DCI_CHGDET_CDP_DP_TOGGLE_MS);
    qusb_uart_log("usb_dp_toggled_for_cdp", QUSB_DCI_CHGDET_CDP_DP_TOGGLE_MS);    
  }
}
...
void qusb_dci_typec_lane_detect(void)
{
  ...
  while (0 == typec_status.vbus_sts)
  {
    ...
    if (PM_ERR_FLAG_SUCCESS != pm_err)
    {
      qusb_uart_log("vbus_pm_err", pm_err);
      use_cc_out_gpio = TRUE;
      break;
    }
    qusb_dci_delay_us(QUSB_DCI_VBUS_HIGH_TIMEOUT_DELAY_US_PER_LOOP);
    pm_retry++;
    if (pm_retry>300000) //USB VBUS Timeout for 30 Sec (300000*100us), workaround for PMIC reading success but return wrong value
    {
      qusb_uart_log("Vbus_low_timeout", pm_retry);
      qusb_dci_ss_lane = 0;      
      return;
    }
  }
  ...
  if (use_cc_out_gpio)
  {
    ...
    while (TRUE)
    {
      ...
      {
        case USB_TYPE_C_LANE_A:
          qusb_uart_log("ss_lane_1st", 0);
          qusb_dci_ss_lane = 0;
          return;
        case USB_TYPE_C_LANE_B:
  ...
  {
    if (PM_ERR_FLAG_SUCCESS != pm_chg_get_typec_status(&typec_status))
    {
  ...
  else
  {
    ...
    {
      case PM_CHG_TYPEC_ORIENTATION_PLUG_UNFLIP:
      {
        // Select 1st lane
        qusb_uart_log("ss_lane_1st", 0);
        qusb_dci_ss_lane = 0;
      ...
      case PM_CHG_TYPEC_ORIENTATION_PLUG_FLIP:
      {
        // Select 2nd lane
        qusb_uart_log("ss_lane_2nd", 0);
        qusb_dci_ss_lane = 1;
...
{
  qusb_dci_type_c_lane_type type_c_lane = USB_TYPE_C_NO_LANE;
