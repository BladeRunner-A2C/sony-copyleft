...
MDP_Status Panel_Default_PowerUp(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  else
  {
    ...
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Failed to get panel function tables\n"));
    }
    else if (EFI_SUCCESS != TLMMProtocol->ConfigGpio(EFI_GPIO_CFG(pMDPSupportPanelFunctions->pMDPPlatformPanelGpio->uDispTeGpio,
