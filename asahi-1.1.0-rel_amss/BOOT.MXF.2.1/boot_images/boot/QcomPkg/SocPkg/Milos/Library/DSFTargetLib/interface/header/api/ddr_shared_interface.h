...
typedef struct
{
    ...
    uint8				                    	trn_idx;
	uint8										board_byte_swap;                // 0: No byte swap on Board, 1: Byte swap on Board
    uint8                                       secojr;                         // 0: select S3C as VDD2/VDD2H rail,  1: select S1G for VDD2/VDD2H rail //Milos specific
}
DDR_SHARED_STRUCT;

#define MAX_NUM_TRAININGS   36 //navbiswa: Changed as per Camano
