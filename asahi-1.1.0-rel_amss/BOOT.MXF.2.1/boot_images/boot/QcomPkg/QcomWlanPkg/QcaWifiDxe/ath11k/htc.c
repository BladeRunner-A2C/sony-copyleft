...
struct sk_buff *ath11k_htc_alloc_skb(struct ath11k_base *ab, int size)
{
	...
	skb = dev_alloc_skb(ab->dev, size + sizeof(struct ath11k_htc_hdr));
	if (!skb)
		return NULL;
...
static struct sk_buff *ath11k_htc_build_tx_ctrl_skb(struct ath11k_base *ab)
{
	struct sk_buff *skb;
	struct ath11k_skb_cb *skb_cb;

	skb = dev_alloc_skb(ab->dev, ATH11K_HTC_CONTROL_BUFFER_SIZE);
	if (!skb)
		return NULL;

	skb_reserve(skb, sizeof(struct ath11k_htc_hdr));
	WARN_ON_ONCE(!IS_ALIGNED((u64)skb->data, 4));

	skb_cb = ATH11K_SKB_CB(skb);
	memset(skb_cb, 0, sizeof(*skb_cb));

	ath11k_dbg(ab, ATH11K_DBG_HTC, "skb %pK\n", skb);
	return skb;
}
					     ...
					     struct sk_buff *skb)
{
	...
	dma_unmap_single(htc->ab->dev, skb_cb->paddr, skb_cb->paddr_mapping_addr, skb->len, DMA_TO_DEVICE);
		    ...
		    struct sk_buff *skb)
{
	...
	skb_cb->eid = eid;
	ret = dma_map_single(dev, skb->data, skb->len, DMA_TO_DEVICE, 
        &skb_cb->paddr, &skb_cb->paddr_mapping_addr);
	if (ret) {
		ret = -EIO;
		goto err_credits;
	}
...
err_unmap:
	dma_unmap_single(dev, skb_cb->paddr, skb_cb->paddr_mapping_addr,skb->len, DMA_TO_DEVICE);
err_credits:
	if (ep->tx_credit_flow_enabled) {
		spin_lock_bh(&htc->tx_lock);
		ep->tx_credits += credits;
				      ...
				      struct sk_buff *skb)
{
	...
	payload_len = (u16)FIELD_GET(HTC_HDR_PAYLOADLEN, hdr->htc_info);
...
static int ath11k_htc_setup_target_buffer_assignments(struct ath11k_htc *htc)
{
	...
	for (i = 0; i < htc->wmi_ep_count; i++) {
		serv_entry[i].service_id = (u16)svc_id[i];
		serv_entry[i].credit_allocation = (u8)credits;
	}
...
int ath11k_htc_wait_target(struct ath11k_htc *htc)
{
	...
	if (!time_left) {
		...
		for (i = 0; i < CE_COUNT; i++)
			ath11k_ce_per_engine_service(htc->ab, (u16)i);
			       ...
			       struct ath11k_htc_svc_conn_resp *conn_resp)
{
	...
	DEBUG_PARAMETER(service_id);

    ath11k_info(ab, "service_id 0x%x\n", conn_req->service_id);
	...
	if (!tx_alloc)
		ath11k_dbg(ab, ATH11K_DBG_BOOT,
			   "boot htc service %a does not allocate target credits\n",
	...
	message_id = (u16)FIELD_GET(HTC_MSG_MESSAGEID, resp_msg->msg_svc_id);
	service_id = (u16)FIELD_GET(HTC_SVC_RESP_MSG_SERVICEID,
			       resp_msg->msg_svc_id);
	...
	ath11k_dbg(ab, ATH11K_DBG_HTC,
		   "HTC Service %a connect response: status: 0x%lx, assigned ep: 0x%lx\n",
	...
	if (conn_resp->connect_resp_code != ATH11K_HTC_CONN_SVC_STATUS_SUCCESS) {
		ath11k_err(ab, "HTC Service %a connect request failed: 0x%x)\n",
			   htc_service_name(service_id),
...
setup:
	...
	if (assigned_eid >= ATH11K_HTC_EP_COUNT || assigned_eid == ATH11K_HTC_EP_UNUSED)
		return -EPROTO;
	...
	ath11k_dbg(ab, ATH11K_DBG_BOOT,
		   "boot htc service '%a' ul pipe %d dl pipe %d eid %d ready\n",
	...
	if (disable_credit_flow_ctrl && ep->tx_credit_flow_enabled) {
		...
		ath11k_dbg(ab, ATH11K_DBG_BOOT,
			   "boot htc service '%a' eid %d TX flow control disabled\n",
