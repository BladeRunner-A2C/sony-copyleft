...
static bool ath11k_regdom_changes(struct ath11k *ar, char *alpha2)
{
	...
	regd = ieee80211_hw_get_regdomain(ar->hw);
...
static void
ath11k_reg_notifier(struct ieee80211_hw *hw, struct regulatory_request *request)
{
	...
	ath11k_dbg(ar->ab, ATH11K_DBG_REG,
		   "Regulatory Notification received for 0x%p\n", hw);
...
int ath11k_reg_update_chan_list(struct ath11k *ar)
{
	...
	bands = ieee80211_hw_get_sbands(hw);
	...
	for (band = 0; band < NUM_NL80211_BANDS; band++) {
		...
		for (i = 0; i < bands[band]->n_channels; i++) {
			...
			ch->minpower = 0;
			ch->maxpower = (char)channel->max_power * 2;
			ch->maxregpower = (char)channel->max_reg_power * 2;
			ch->antennamax = (u8)channel->max_antenna_gain * 2;
			...
			if (channel->band == NL80211_BAND_6GHZ &&
			    net80211_channel_is_psc(channel))
				ch->psc_channel = true;
...
int ath11k_regd_update(struct ath11k *ar, bool init)
{
    ...
    ret = ieee80211_hw_set_regdomain(ar->hw, regd_copy);
	...
	//ret = regulatory_set_wiphy_regd_sync_rtnl(ar->hw->wiphy, regd_copy); //TODOTODO
	rtnl_unlock();
...
{
	u32 num_old_regd_rules, num_curr_regd_rules, num_new_regd_rules;
...
ath11k_reg_update_weather_radar_band(struct ath11k_base *ab,
				     ...
				     u32 *rule_idx, u32 flags, u16 max_bw)
{
	u16 end_freq;
	u16 bw;
	u32 i;
		      ...
		      struct cur_regulatory_info *reg_info, bool intersect)
{
	...
	ath11k_dbg(ab, ATH11K_DBG_REG,
		   "Country %a, CFG Regdomain %s FW Regdomain %d, num_reg_rules %d\n",
...
void ath11k_regd_update_work(void *context)
{
...
void ath11k_reg_init(struct ath11k *ar)
{
	...
	//ar->hw->wiphy->reg_notifier = ath11k_reg_notifier; //Will check
}
