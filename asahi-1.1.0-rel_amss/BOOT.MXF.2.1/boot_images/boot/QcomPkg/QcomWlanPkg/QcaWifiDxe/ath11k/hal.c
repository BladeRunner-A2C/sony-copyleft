...
static void ath11k_hal_free_cont_rdp(struct ath11k_base *ab)
{
	...
	dma_free_coherent(ab->dev, size,
			  hal->rdp.vaddr, hal->rdp.paddr,hal->rdp.dma_mapping_addr);
	hal->rdp.vaddr = NULL;
}
...
static void ath11k_hal_free_cont_wrp(struct ath11k_base *ab)
{
	...
	size = sizeof(u32) * HAL_SRNG_NUM_LMAC_RINGS;
	dma_free_coherent(ab->dev, size,
			  hal->wrp.vaddr, hal->wrp.paddr, hal->wrp.dma_mapping_addr);
	hal->wrp.vaddr = NULL;
}
...
{
	const struct hal_srng_config *srng_config = &ab->hal.srng_config[HAL_CE_DST];
	u32 addr;
	u32 val;
					...
					struct hal_srng *srng)
{
	...
	hp_addr = hal->rdp.paddr + 
        (u32)((u8*)srng->u.dst_ring.hp_addr - (u8*)hal->rdp.vaddr);
					...
					struct hal_srng *srng)
{
	...
	if (srng->ring_id != HAL_SRNG_RING_ID_WBM_IDLE_LINK) {
		tp_addr = hal->rdp.paddr + 
            (u32)((u8*)srng->u.src_ring.tp_addr - (u8*)hal->rdp.vaddr);
		ath11k_hif_write32(ab,
...
{
	const struct hal_srng_config *srng_config = &ab->hal.srng_config[type];
	int ring_id;
				       ...
				       struct hal_srng *srng)
{
    ...
    if (srng->ring_dir == HAL_SRNG_DIR_SRC) {
        return ab->hal.wrp.paddr + 
            (u32)((u8*)srng->u.src_ring.hp_addr - (u8*)ab->hal.wrp.vaddr);
    }
    else {
        return ab->hal.rdp.paddr +
            (u32)((u8*)srng->u.dst_ring.hp_addr - (u8*)ab->hal.rdp.vaddr);
    }
}
				       ...
				       struct hal_srng *srng)
{
    ...
    if (srng->ring_dir == HAL_SRNG_DIR_SRC) {
        return ab->hal.rdp.paddr +
            (u32)((u8*)srng->u.src_ring.tp_addr - (u8*)ab->hal.rdp.vaddr);
    }
    else {
        return ab->hal.wrp.paddr + 
            (u32)((u8*)srng->u.dst_ring.tp_addr - (u8*)ab->hal.wrp.vaddr);
    }
}
...
void ath11k_hal_srng_access_end(struct ath11k_base *ab, struct hal_srng *srng)
{
	...
	if (srng->flags & HAL_SRNG_FLAGS_LMAC_RING) {
		...
		if (srng->ring_dir == HAL_SRNG_DIR_SRC) {
			ath11k_hif_write32(ab,
					   (u32)((u8 *)srng->u.src_ring.hp_addr - (u8 *)ab->mem),
					   srng->u.src_ring.hp);
		} else {
			ath11k_hif_write32(ab,
					   (u32)((u8 *)srng->u.dst_ring.tp_addr - (u8 *)ab->mem),
					   srng->u.dst_ring.tp);
		}
	}
}
				     ...
				     u32 end_offset)
{
	...
	link_addr = (void *)((u8*)sbuf[0].vaddr +
	...
	for (i = 1; i < nsbufs; i++) {
		...
		link_addr = (void *)((u8*)sbuf[i].vaddr +
            HAL_WBM_IDLE_SCATTER_BUF_SIZE);
	}
			  ...
			  struct hal_srng_params *params)
{
	...
	ath11k_info(ab, "type:%d, ring_num:%d, ring_id:%d, lmac_ring:%d,msi_addr:0x%llx, msi_data:%d\n",
		    type, ring_num, srng->ring_id, srng_config->lmac_ring,
	...
	if (type == HAL_CE_DST) {
		srng->u.dst_ring.max_buffer_length = (u16)params->max_buffer_len;
...
static int ath11k_hal_srng_create_config(struct ath11k_base *ab)
{
	...
	s->reg_size[0] = (u16)(HAL_REO2_RING_BASE_LSB(ab) - HAL_REO1_RING_BASE_LSB(ab));
	s->reg_size[1] = (u16)(HAL_REO2_RING_HP(ab) - HAL_REO1_RING_HP(ab));
	...
	s->reg_size[0] = (u16)(HAL_TCL2_RING_BASE_LSB(ab) - HAL_TCL1_RING_BASE_LSB(ab));
	...
	s->reg_start[0] = HAL_CE0_SRC_REG_BASE(ab) + HAL_CE_DST_RING_BASE_LSB;
	s->reg_start[1] = HAL_CE0_SRC_REG_BASE(ab) + HAL_CE_DST_RING_HP;
	s->reg_size[0] = (u16)(HAL_CE1_SRC_REG_BASE(ab) - HAL_CE0_SRC_REG_BASE(ab));
	s->reg_size[1] = (u16)(HAL_CE1_SRC_REG_BASE(ab) - HAL_CE0_SRC_REG_BASE(ab));

	s = &hal->srng_config[HAL_CE_DST];
	s->reg_start[0] = HAL_CE0_DST_REG_BASE(ab) + HAL_CE_DST_RING_BASE_LSB;
	s->reg_start[1] = HAL_CE0_DST_REG_BASE(ab) + HAL_CE_DST_RING_HP;
	s->reg_size[0] = (u16)(HAL_CE1_DST_REG_BASE(ab) - HAL_CE0_DST_REG_BASE(ab));
	s->reg_size[1] = (u16)(HAL_CE1_DST_REG_BASE(ab) - HAL_CE0_DST_REG_BASE(ab));

	s = &hal->srng_config[HAL_CE_DST_STATUS];
	s->reg_start[0] = HAL_CE0_DST_REG_BASE(ab) + HAL_CE_DST_STATUS_RING_BASE_LSB;
	s->reg_start[1] = HAL_CE0_DST_REG_BASE(ab) + HAL_CE_DST_STATUS_RING_HP;
	s->reg_size[0] = (u16)(HAL_CE1_DST_REG_BASE(ab) - HAL_CE0_DST_REG_BASE(ab));
	s->reg_size[1] = (u16)(HAL_CE1_DST_REG_BASE(ab) - HAL_CE0_DST_REG_BASE(ab));

	s = &hal->srng_config[HAL_WBM_IDLE_LINK];
	s->reg_start[0] = HAL_WBM_IDLE_LINK_RING_BASE_LSB(ab)
		+ HAL_SEQ_WCSS_UMAC_WBM_REG;
	s->reg_start[1] = HAL_SEQ_WCSS_UMAC_WBM_REG + HAL_WBM_IDLE_LINK_RING_HP;

	s = &hal->srng_config[HAL_SW2WBM_RELEASE];
	s->reg_start[0] = HAL_WBM_RELEASE_RING_BASE_LSB(ab)
		+ HAL_SEQ_WCSS_UMAC_WBM_REG;
	s->reg_start[1] = HAL_SEQ_WCSS_UMAC_WBM_REG + HAL_WBM_RELEASE_RING_HP;

	s = &hal->srng_config[HAL_WBM2SW_RELEASE];
	s->reg_start[0] = HAL_WBM0_RELEASE_RING_BASE_LSB(ab)
		+ HAL_SEQ_WCSS_UMAC_WBM_REG;
	s->reg_start[1] = HAL_SEQ_WCSS_UMAC_WBM_REG + HAL_WBM0_RELEASE_RING_HP;
	s->reg_size[0] = (u16)(HAL_WBM1_RELEASE_RING_BASE_LSB(ab)
