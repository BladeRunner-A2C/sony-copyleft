...
ath11k_dp_tx_get_encap_type(struct ath11k_vif *arvif, struct sk_buff *skb)
{
	...
	if (tx_info->flags & IEEE80211_TX_CTRL_HW_80211_ENCAP)
...
static void ath11k_dp_tx_encap_nwifi(struct sk_buff *skb)
{
	...
	memmove(skb->data + IEEE80211_QOS_LEN,
		skb->data, (u8 *)qos_ctl - (u8 *)skb->data);
	skb_pull(skb, IEEE80211_QOS_LEN);
...
static u8 ath11k_dp_tx_get_tid(struct sk_buff *skb)
{
	...
	if (cb->flags & ATH11K_SKB_HW_80211_ENCAP)
		return skb->priority & IEEE80211_QOS_TID_MASK;
		 ...
		 struct sk_buff *skb)
{
	...
	/* TODO: wbm2sw1 doesn't work expectedly */
...
#if 0 // TODOTODO: Remove the code for mesh
    ...
    ti.paddr = skb_data_map_paddr(skb);
    if (!ti.paddr) {
        ath11k_warn(ab, "skb %p failed to DMA map data Tx buffer\n", skb);
        ret = -ENOMEM;
        goto fail_remove_idr;
    }
	...
	hal_ring_id = (u8)tx_ring->tcl_data_ring.ring_id;
	...
	ath11k_hal_tx_cmd_desc_setup(ab, (u8*)hal_tcl_desc +
				 ...
				 struct ath11k_dp_htt_wbm_tx_status *ts)
{
	...
	if (ts->acked) {
		if (!(info->flags & IEEE80211_TX_CTL_NO_ACK)) {
			...
			info->u.status.ack_signal = ATH11K_DEFAULT_NOISE_FLOOR +
						  ts->ack_rssi;
			info->u.status.is_valid_ack_signal = true;
		} else {
			info->flags |= IEEE80211_TX_STATUS_NOACK_TRANSMITTED;
		}
	}
				       ...
				       struct hal_tx_status *ts)
{
    ...
    ath11k_dbg(ab, ATH11K_DBG_DATA,
        "buf_rel_source %d, status %d, flags 0x%x\n", 
        ts->buf_rel_source, ts->status, ts->flags);
					     ...
					     struct hal_tx_status *ts)
{
	...
	ts->ppdu_id = FIELD_GET(HAL_WBM_RELEASE_INFO1_TQM_STATUS_NUMBER,
				desc->info1);
	ts->try_cnt = (u8)FIELD_GET(HAL_WBM_RELEASE_INFO1_TRANSMIT_COUNT,
				desc->info1);
	...
	ts->tid = (u8)FIELD_GET(HAL_WBM_RELEASE_INFO3_TID, desc->info3);
...
void ath11k_dp_tx_completion_handler(struct ath11k_base *ab, int ring_id)
{
	...
	while ((ATH11K_TX_COMPL_NEXT(ab, tx_ring->tx_status_head) !=
		tx_ring->tx_status_tail) &&
	       (desc = ath11k_hal_srng_dst_get_next_entry(ab, status_ring))!= NULL) {
				...
				int mac_id, enum hal_ring_type ring_type)
{
	...
	ath11k_info(ab, "msi_addr_lo:0x%x, msi_addr_hi:0x%x, msi_data:0x%x\n",
		    cmd->ring_msi_addr_lo, cmd->ring_msi_addr_hi,
		   cmd->msi_data);
	ath11k_info(ab, "ring_id:%d, ring_type:%d,intr_info:0x%x, flags:0x%x\n",
...
int ath11k_dp_tx_htt_h2t_ver_req_msg(struct ath11k_base *ab)
{
	...
	time_left = wait_for_completion_timeout(&dp->htt_tgt_version_received,
...
int ath11k_dp_tx_htt_h2t_ppdu_stats_req(struct ath11k *ar, u32 mask)
{
	...
	for (i = 0; i < MAX_MACS_PER_PDEV; i++) {
		...
		ath11k_info(ab, "%s pdev_mask:0x%x, stats_mask:0x%x\n",
			    __func__, pdev_mask, mask);
	}
