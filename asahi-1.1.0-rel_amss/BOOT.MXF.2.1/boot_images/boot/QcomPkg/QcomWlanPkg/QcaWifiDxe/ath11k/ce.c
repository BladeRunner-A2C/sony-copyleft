...
#define ATH11K_CE_RX_BUFFER_PENDING_SIZE 128

#define host_ce_config_wlan ab->hw_params.host_ce_config

const struct ce_attr ath11k_host_ce_config_qca6x90[] = {
...
static int ath11k_ce_rx_post_pipe(struct ath11k_ce_pipe *pipe)
{
	...
	while (pipe->rx_buf_needed) {
		skb = sk_buff_pool_alloc_skb(&pipe->rx_skb_pool, 4 /* alignment */);
		if (!skb) {
			ret = -ENOMEM;
			goto exit;
		}
        ...
        if (!paddr) {
            ath11k_warn(ab, "skb %p failed to dma map ce rx buf\n", skb);
...
static int ath11k_ce_recv_process_cb(struct ath11k_ce_pipe *pipe)
{
	struct ath11k_base *ab = pipe->ab;
	struct sk_buff *skb;
	...
	if (ret && ret != -ENOSPC) {
		ath11k_warn(ab, "failed to post rx buf to pipe: %d err: %d\n",
			    pipe->pipe_num, ret);
        queue_delayed_work(&ab->rx_replenish_retry,
...
static int ath11k_ce_send_done_cb(struct ath11k_ce_pipe *pipe)
{
	struct ath11k_base *ab = pipe->ab;
	struct sk_buff *skb;
    int skb_completed = 0;
	...
	while (!IS_ERR(skb = ath11k_ce_completed_send_next(pipe))) {
		...
		dma_unmap_single(ab->dev, ATH11K_SKB_CB(skb)->paddr, 
                 ATH11K_SKB_CB(skb)->paddr_mapping_addr,
                 skb->len, DMA_TO_DEVICE);
		dev_kfree_skb_any(skb);
        skb_completed++;
	}

    return skb_completed;
}
...
ath11k_ce_alloc_ring(struct ath11k_base *ab, int nentries, int desc_sz)
{
    ...
    ce_ring->base_addr_mapping_addr = base_mapping_addr;
...
int ath11k_ce_per_engine_service(struct ath11k_base *ab, u16 ce_id)
{
	struct ath11k_ce_pipe *pipe = &ab->ce.ce_pipe[ce_id];
    int handled = 0;

	if (pipe->send_cb)
        handled += pipe->send_cb(pipe);

	if (pipe->recv_cb)
        handled += ath11k_ce_recv_process_cb(pipe);
		   ...
		   u16 transfer_id)
{
	...
	if (pipe->attr_flags & CE_ATTR_DIS_INTR) {
		...
		if (num_used > ATH11K_CE_USAGE_THRESHOLD)
			ath11k_ce_poll_send_completed(ab, (u8)pipe->pipe_num);
	}
    ...
    ath11k_info(ab, "skb 0x%p: paddr=0x%x, len=%u\n", 
        skb, ATH11K_SKB_CB(skb)->paddr, skb->len);
...
void ath11k_ce_cleanup_pipes(struct ath11k_base *ab)
{
	...
	for (pipe_num = 0; pipe_num < CE_COUNT; pipe_num++) {
		...
		ath11k_ce_poll_send_completed(ab, (u8)pipe_num);
...
void ath11k_ce_rx_replenish_retry(void *ctx)
{
...
int ath11k_ce_init_pipes(struct ath11k_base *ab)
{
	...
	for (i = 0; i < CE_COUNT; i++) {
		...
		if (pipe->dest_ring) {
            ...
            /* addtional 32 skb for cache */
            sk_buff_pool_init(&pipe->rx_skb_pool, 
                pipe->dest_ring->nentries + ATH11K_CE_RX_BUFFER_PENDING_SIZE,
                pipe->buf_sz, ab->dev);
	...
	for (i = 0; i < CE_COUNT; i++) {
		...
		if (pipe->src_ring) {
			...
			dma_free_coherent(ab->dev,
					  ...
					  pipe->src_ring->base_addr_mapping_addr);
			kfree(pipe->src_ring);
			pipe->src_ring = NULL;
		}
		...
		if (pipe->dest_ring) {
			...
			dma_free_coherent(ab->dev,
					  ...
					  pipe->dest_ring->base_addr_mapping_addr);
            sk_buff_pool_purge(&pipe->rx_skb_pool);
		...
		if (pipe->status_ring) {
			...
			dma_free_coherent(ab->dev,
					  ...
					  pipe->status_ring->base_addr_mapping_addr);
