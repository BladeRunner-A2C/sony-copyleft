					  ...
					  struct ath11k_hal_reo_cmd *cmd)
{
	...
	u8 avail_slot = (u8)ffz(hal->avail_blk_resource);
				     ...
				     u32 *cookie, u8 *rbm)
{
	...
	*rbm = (u8)FIELD_GET(BUFFER_ADDR_INFO1_RET_BUF_MGR, binfo->info1);
}
				  ...
				  struct hal_rx_wbm_rel_info *rel_info)
{
	...
	rel_info->first_msdu = (bool)FIELD_GET(HAL_WBM_RELEASE_INFO2_FIRST_MSDU,
					 wbm_desc->info2);
	rel_info->last_msdu = (bool)FIELD_GET(HAL_WBM_RELEASE_INFO2_LAST_MSDU,
					wbm_desc->info2);
	return 0;
}
...
int ath11k_hal_reo_process_status(u8 *reo_desc, u8 *status)
{
	...
	*status = (u8)FIELD_GET(HAL_REO_STATUS_HDR_INFO0_EXEC_STATUS, hdr->info0);
				       ...
				       struct hal_reo_status *status)
{
	...
	status->u.flush_cache.cache_controller_flush_status_hit =
		(bool)FIELD_GET(HAL_REO_FLUSH_CACHE_STATUS_INFO0_FLUSH_STATUS_HIT,
			  desc->info0);

	status->u.flush_cache.cache_controller_flush_status_desc_type =
		(u8)FIELD_GET(HAL_REO_FLUSH_CACHE_STATUS_INFO0_FLUSH_DESC_TYPE,
			  desc->info0);
	status->u.flush_cache.cache_controller_flush_status_client_id =
		(u8)FIELD_GET(HAL_REO_FLUSH_CACHE_STATUS_INFO0_FLUSH_CLIENT_ID,
			  desc->info0);
	status->u.flush_cache.cache_controller_flush_status_err =
		(u8)FIELD_GET(HAL_REO_FLUSH_CACHE_STATUS_INFO0_FLUSH_ERR,
			  desc->info0);
	status->u.flush_cache.cache_controller_flush_status_cnt =
		(u8)FIELD_GET(HAL_REO_FLUSH_CACHE_STATUS_INFO0_FLUSH_COUNT,
			  desc->info0);
}
					      ...
					      struct hal_reo_status *status)
{
	...
	status->u.timeout_list.release_desc_cnt =
		(u16)FIELD_GET(HAL_REO_FLUSH_TIMEOUT_STATUS_INFO1_REL_DESC_COUNT,
			  desc->info1);
	status->u.timeout_list.fwd_buf_cnt =
		(u16)FIELD_GET(HAL_REO_FLUSH_TIMEOUT_STATUS_INFO1_FWD_BUF_COUNT,
			  desc->info1);
}
				   ...
				   u32 tlv_tag, u8 *tlv_data)
{
	...
	case HAL_RX_PPDU_START: {
		...
		ppdu_info->chan_num = (u16)__le32_to_cpu(ppdu_start->chan_num);
	...
	case HAL_RX_PPDU_END_USER_STATS: {
		...
		ppdu_info->tid =
			(u8)(ffs(FIELD_GET(HAL_RX_PPDU_END_USER_STATS_INFO6_TID_BITMAP,
				      __le32_to_cpu(eu_stats->info6))) - 1);
		ppdu_info->tcp_msdu_count =
			(u16)FIELD_GET(HAL_RX_PPDU_END_USER_STATS_INFO4_TCP_MSDU_CNT,
				  __le32_to_cpu(eu_stats->info4));
		ppdu_info->udp_msdu_count =
			(u16)FIELD_GET(HAL_RX_PPDU_END_USER_STATS_INFO4_UDP_MSDU_CNT,
				  __le32_to_cpu(eu_stats->info4));
		ppdu_info->other_msdu_count =
			(u16)FIELD_GET(HAL_RX_PPDU_END_USER_STATS_INFO5_OTHER_MSDU_CNT,
				  __le32_to_cpu(eu_stats->info5));
		ppdu_info->tcp_ack_msdu_count =
			(u16)FIELD_GET(HAL_RX_PPDU_END_USER_STATS_INFO5_TCP_ACK_MSDU_CNT,
				  __le32_to_cpu(eu_stats->info5));
	...
	case HAL_PHYRX_HE_SIG_A_SU: {
		...
		ppdu_info->mcs =
			FIELD_GET(HAL_RX_HE_SIG_A_SU_INFO_INFO0_TRANSMIT_MCS,
				  info0);
		ppdu_info->bw =
			(u8)FIELD_GET(HAL_RX_HE_SIG_A_SU_INFO_INFO0_TRANSMIT_BW,
				  info0);
		ppdu_info->ldpc = FIELD_GET(HAL_RX_HE_SIG_A_SU_INFO_INFO1_CODING, info0);
		ppdu_info->is_stbc = (u8)(info1 &
	...
	case HAL_PHYRX_HE_SIG_A_MU_DL: {
		...
		ppdu_info->bw =
			(u8)FIELD_GET(HAL_RX_HE_SIG_A_MU_DL_INFO_INFO0_TRANSMIT_BW,
				  info0);
	...
	case HAL_PHYRX_HE_SIG_B2_MU: {
		...
		ppdu_info->mcs =
            (u8)FIELD_GET(HAL_RX_HE_SIG_B2_MU_INFO_INFO0_STA_MCS,
				  info0);
		ppdu_info->nss =
            (u8)FIELD_GET(HAL_RX_HE_SIG_B2_MU_INFO_INFO0_STA_NSTS,
				  info0) + 1;
		ppdu_info->ldpc = (u8)FIELD_GET(HAL_RX_HE_SIG_B2_MU_INFO_INFO0_STA_CODING,
					    info0);
		break;
	}
	case HAL_PHYRX_HE_SIG_B2_OFDMA: {
		...
		ppdu_info->mcs =
            (u8)FIELD_GET(HAL_RX_HE_SIG_B2_OFDMA_INFO_INFO0_STA_MCS,
				  info0);
		ppdu_info->nss =
            (u8)FIELD_GET(HAL_RX_HE_SIG_B2_OFDMA_INFO_INFO0_STA_NSTS,
				  info0) + 1;
		ppdu_info->beamformed = (u8)(info0 &
			HAL_RX_HE_SIG_B2_OFDMA_INFO_INFO0_STA_TXBF);
		ppdu_info->ldpc = (u8)FIELD_GET(HAL_RX_HE_SIG_B2_OFDMA_INFO_INFO0_STA_CODING,
					    info0);
	...
	case HAL_PHYRX_RSSI_LEGACY: {
		 ...
		 */
		ppdu_info->rssi_comb =
            (u8)FIELD_GET(HAL_RX_PHYRX_RSSI_LEGACY_INFO_INFO1_RSSI_COMB,
				  __le32_to_cpu(rssi->info0));
		break;
	}
	case HAL_RX_MPDU_START: {
		u16 peer_id = ab->hw_params.hw_ops->mpdu_info_get_peerid(tlv_data);
		if (peer_id)
			       ...
			       struct sk_buff *skb)
{
	...
	do {
		...
		tlv_len = (u16)FIELD_GET(HAL_TLV_HDR_LEN, tlv->tl);
		ptr += sizeof(*tlv);
		...
		ptr += tlv_len;
		addr = (u64)ptr;
		ptr = PTR_ALIGN(addr, HAL_TLV_ALIGN);
					 ...
					 u8 *rbm, u32 *msdu_cnt)
{
	...
	*sw_cookie = FIELD_GET(BUFFER_ADDR_INFO1_SW_COOKIE,
			       buf_addr_info->info1);
	*rbm = (u8)FIELD_GET(BUFFER_ADDR_INFO1_RET_BUF_MGR,
			 buf_addr_info->info1);
