...
struct ath11k_hif_ops {
	...
	void (*get_msi_address)(struct ath11k_base *ab, u32 *msi_addr_lo,
				u32 *msi_addr_hi);
    int (*dump)(struct ath11k_base *sc);
	int (*dump_sram)(struct ath11k_base *sc);
};
...
static inline int ath11k_hif_dump_target(struct ath11k_base *ab)
{
    return ab->hif.ops->dump(ab);
}

static inline int ath11k_hif_dump_sram(struct ath11k_base *ab)
{
    return ab->hif.ops->dump_sram(ab);
}
