...
static int ath11k_qmi_respond_fw_mem_request(struct ath11k_base *ab)
{
	...
	if (ab->qmi.mem_seg_count <= 2) {
		...
		for (i = 0; i < (int)req->mem_seg_len; i++) {
...
static int ath11k_qmi_alloc_target_mem_chunk(struct ath11k_base *ab)
{
	...
	for (i = 0; i < (int)ab->qmi.mem_seg_count; i++) {
        mem_chunk = &ab->qmi.target_mem[i];

        WARN_ON(mem_chunk->vaddr != NULL);

        mem_chunk->alloc_size = mem_chunk->size + 
            ATH11K_QMI_WLFW_MM_DEFAULT_ALIGNMENT - 1;
        mem_chunk->vaddr = dma_alloc_coherent(ab->dev,
            mem_chunk->alloc_size,
            &mem_chunk->paddr, &mem_chunk->p_dma_mapping_addr);
		if (!mem_chunk->vaddr) {
        ...
        mem_chunk->aligned_paddr = 
            ALIGN(mem_chunk->paddr, ATH11K_QMI_WLFW_MM_DEFAULT_ALIGNMENT);
        mem_chunk->aligned_vaddr = (u8*)mem_chunk->vaddr +
            (mem_chunk->aligned_paddr - mem_chunk->paddr);
        ath11k_trace(ab, ATH11K_DBG_QMI,
            "qmi mem seg type %d size %d, aligned_vaddr=0x%p, aligned_paddr=0x%x, "
            "p_dma_mapping_addr=0x%p\n",
            mem_chunk->type, ab->qmi.target_mem[i].size,
            mem_chunk->aligned_vaddr,
            mem_chunk->aligned_paddr,
            mem_chunk->p_dma_mapping_addr);

        if (mem_chunk->type == QMI_WLFW_MEM_TYPE_DDR_V01) {
...
static int ath11k_qmi_request_target_cap(struct ath11k_base *ab)
{
	...
	ath11k_info(ab, "qmi fw_version: 0x%x fw_build_timestamp: %a fw_build_id: %a\n",
...
static int ath11k_target_match_expression(struct ath11k_base *ab, struct expression_iterator *it)
{
    ...
    if (!device_id) {
        ath11k_err(ab, "failed to get hardware id\n");
        return -ENODEV;
    }
    ...
    while (expression_iterator_next(it) == 0) {
        ...
        if (strcmp("bus", it->key) == 0) {
            ...
            ath11k_warn(ab, "unsupported expression: %a=%a\n", it->key, it->value);
            ret = -EBADF;
        }
    }
...
static int ath11k_firmware_metadata_match_target(const u8 *name, struct firmware_metadata *metadata, void *ctx)
{
    ...
    if (!metadata->description) {
        ath11k_info(ab, "match firmware %a(no description): 0\n", name);
        return 0;
    }
    ...
    while (description_iterator_next(&it) == 0) {
        ret = ath11k_target_match_expression(ab, &it.expression_it);
        if (!ret)
            break;
    }
    ...
    if (SUPPORT_DEBUG_PRINT) {
        ...
        ath11k_info(ab, "match firmware %a(description: %a): %d\n", name, description, ret);
    }

    return ret;
}

int ath11k_get_bdf(struct ath11k_base *ab, struct firmware *fw_entry)
{
    int ret;

    ath11k_info(ab, "try to get matched bdf from firmware\n");
    ret = request_firmware_fancily(fw_entry, BIN_BDF_FILE_NAME, ath11k_firmware_metadata_match_target, ab);
    if (ret) {
        ath11k_err(ab, "failed to load a matched bdf: %d\n", ret);
    }

    return ret;
}

int ath11k_get_regdb(struct ath11k_base *ab, struct firmware *fw_entry)
{
    int ret;

    ret = request_firmware_fancily(fw_entry, REGDB_FILE_NAME, ath11k_firmware_metadata_match_target, ab);
    if (ret) {
        ath11k_warn(ab, "qmi failed to load regdb: %d\n", ret);
    }

    return ret;
}

static int ath11k_qmi_load_bdf_target_mem(struct ath11k_base *ab, enum ath11k_bdf_type type)
{
    ...
    if (type == ATH11K_BDF_BIN) {
        ret = ath11k_get_bdf(ab, &fw_entry);
        if (ret) {
            ath11k_err(ab, "qmi failed to load bdf\n");
            goto out;
        }
    }
    else if (type == ATH11K_BDF_REGDB) {
        ret = ath11k_get_regdb(ab, &fw_entry);
        if (ret) {
            ath11k_err(ab, "qmi failed to load regdb\n");
            goto out;
        }
    }
    else {
        ret = -EINVAL;
        ath11k_err(ab, "unsupported bdf type %d\n", type);
        goto out;
    }
    ath11k_info(ab, "qmi BDF type %d downloading\n", type);
	...
	while (remaining) {
        ...
        ath11k_info(ab, "seg id %d len %d result %d error %d\n",
	...
	ath11k_info(ab, "qmi BDF type %d downloaded\n", type);
...
static int ath11k_load_m3_bin(struct ath11k_base *ab)
{
	...
	ret = request_firmware_fancily(&fw_entry, filename, ath11k_firmware_metadata_match_target, ab);
	if (ret) {
	...
	if (!m3_mem->vaddr) {
		ath11k_err(ab, "Failed to allocate memory for M3, size: 0x%x\n",
			    fw_entry.size);
		release_firmware(&fw_entry);
		return -ENOMEM;
	}

	ath11k_err(ab, "memory for M3, size: 0x%x\n", fw_entry.size);
...
static int ath11k_qmi_wlanfw_wlan_cfg_send(struct ath11k_base *ab)
{
	...
	for (pipe_num = 0; pipe_num < (int)req->tgt_cfg_len ; pipe_num++) {
	...
	for (pipe_num = 0; pipe_num < (int)req->svc_cfg_len; pipe_num++) {
...
static int ath11k_qmi_event_load_bdf(struct ath11k_qmi *qmi)
{
	...
	ret = ath11k_qmi_load_bdf_target_mem(ab, ATH11K_BDF_REGDB);
    if (ret < 0) {
        ath11k_warn(ab, "qmi failed to load regulatory database file:%d\n", ret);
		return ret;
    }

    ret = ath11k_qmi_load_bdf_target_mem(ab, ATH11K_BDF_BIN);
	if (ret < 0) {
					  ...
					  const void *data)
{
	...
	for (i = 0; i < (int)qmi->mem_seg_count; i++) {
				     ...
				     struct qmi_service *service)
{
	...
	if((service->service != ATH11K_QMI_WLFW_SERVICE_ID_V01) ||
		(service->version != ATH11K_QMI_WLFW_SERVICE_VERS_V01)) {
		ath11k_info(ab, "qmi wifi fw service not match: fw service: %u, version: %u,\
				expected service: %u, version: %u\n", service->service, service->version,
				ATH11K_QMI_WLFW_SERVICE_ID_V01, ATH11K_QMI_WLFW_SERVICE_VERS_V01);
		return 0;
	}
	...
	ath11k_dbg(ab, ATH11K_DBG_QMI, 
        "qmi wifi fw qmi service connected, qmi=0x%p, qmi_hdl=0x%p\n", qmi, qmi_hdl);
...
static void ath11k_qmi_driver_event_work(void *context)
{
    struct ath11k_qmi *qmi = (struct ath11k_qmi *)context;
	struct ath11k_qmi_driver_event *event;
	struct ath11k_base *ab = qmi->ab;
	int ret = 0;
	...
	while (!list_is_empty(&qmi->event_list)) {
		...
		case ATH11K_QMI_EVENT_FW_MEM_READY:
			...
			if (ret < 0) {
				...
				ath11k_err(ab, "Bdf/Regdb load fail, will try to reload again:");
		...
		case ATH11K_QMI_EVENT_FW_READY:
			...
			if (ret < 0) {
#if SUPPORT_SECURITY_BOOT
				ath11k_err(ab, "M3 load fail, will try to reload again");
				schedule_work(&ab->reset_work);
#endif
			}
...
#endif
    ath11k_warn(ab, "initialize the qmi: qmi=0x%p, qmi_hdl=0x%p\n",
        &ab->qmi, &ab->qmi.handle);
	list_init_head(&ab->qmi.event_list);
	spin_lock_init(&ab->qmi.event_lock);
    init_work(&ab->qmi.event_work, ath11k_qmi_driver_event_work, &ab->qmi);
...
{
    ath11k_warn(ab, "release the qmi handle: 0x%p\n", &ab->qmi.handle);
