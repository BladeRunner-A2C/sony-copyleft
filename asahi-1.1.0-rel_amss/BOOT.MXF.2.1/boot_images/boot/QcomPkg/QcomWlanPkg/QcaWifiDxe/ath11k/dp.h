...
struct dp_rx_tid {
    ...
    struct delayed_work frag_timer;
...
#define DP_TCL_DATA_RING_SIZE(ab)	((ab)->hw_params.srng_params->tcl_data_ring_size)
...
#define DP_REO_DST_RING_SIZE(ab)	((ab)->hw_params.srng_params->reo_dst_ring_size)
...
struct htt_tlv {
	u32 header;
    //u8 value[0]; /* Comment out to suppress the warning: error C2229: an illegal zero-sized array */
} __packed;
