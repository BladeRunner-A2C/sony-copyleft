				     ...
				     const u8 *addr)
{
	...
	list_for_each_entry(peer, &ab->peers, struct ath11k_peer, list) {
			   ...
			   u8 *mac_addr, u16 ast_hash)
{
	...
	if (!peer) {
		...
		list_add_tail(&peer->list, &ab->peers);
		wake_up(&ab->peer_mapping_wq);
	}
...
static int ath11k_peer_find_mapping(
    struct ath11k_base *ab, int vdev_id,
				       ...
				       const u8 *addr, bool expect_mapped)
{
	...
	wait_event_timeout(ab->peer_mapping_wq, 
        ((mapping_result = ath11k_peer_find_mapping(ab, vdev_id, addr, expect_mapped)) > 0), 
        3 * HZ, ret);
...
int ath11k_peer_delete(struct ath11k *ar, u32 vdev_id, const u8 *addr)
{
	...
	ret = ath11k_wmi_send_peer_delete_cmd(ar, addr, (u8)vdev_id);
	if (ret) {
		ath11k_warn(ar->ab,
