...
{
	return ab->hw_params.hw_ops->rxdesc_80211_hdr(desc);
}

static enum hal_encrypt_type ath11k_dp_rx_h_mpdu_start_enctype(struct ath11k_base *ab,
							       struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_mpdu_start_enctype(desc);
}

static u8 ath11k_dp_rx_h_msdu_start_decap_type(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_start_decap_type(desc);
}

static u8 ath11k_dp_rx_h_msdu_start_mesh_ctl_present(struct ath11k_base *ab,
						     struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_start_mesh_ctl_present(desc);
}

static bool ath11k_dp_rx_h_mpdu_start_seq_ctrl_valid(struct ath11k_base *ab,
						     struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_mpdu_start_seq_ctrl_valid(desc);
}

static bool ath11k_dp_rx_h_mpdu_start_fc_valid(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_mpdu_start_fc_valid(desc);
}

static bool ath11k_dp_rx_h_mpdu_start_more_frags(struct ath11k_base *ab, struct sk_buff *skb)
{
...
static u16 ath11k_dp_rx_h_mpdu_start_frag_no(struct ath11k_base *ab, struct sk_buff *skb)
{
	struct ieee80211_hdr *hdr;

	hdr = (struct ieee80211_hdr *)(skb->data + ab->hw_params.hal_desc_sz);
	return le16_to_cpu(hdr->seq_ctrl) & IEEE80211_SEQ_FRAG_MASK;
}

static u16 ath11k_dp_rx_h_mpdu_start_seq_no(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_mpdu_start_seq_no(desc);
}

static u8 *ath11k_dp_rx_h_mpdu_start_addr2(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_mpdu_start_addr2(desc);
}

static bool ath11k_dp_rx_h_attn_msdu_done(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_attn_msdu_done(desc);
}

static bool ath11k_dp_rx_h_attn_l4_cksum_fail(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_attn_l4_cksum_fail(desc);
}

static bool ath11k_dp_rx_h_attn_ip_cksum_fail(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_attn_ip_cksum_fail(desc);
}

static bool ath11k_dp_rx_h_attn_is_decrypted(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_attn_is_decrypted(desc);
}

static u32 ath11k_dp_rx_h_attn_mpdu_err(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	struct rx_attention *attn;
	u32 info, errmap = 0;

	attn = ab->hw_params.hw_ops->rxdesc_get_attn(desc);
	info = __le32_to_cpu(attn->info1);
...
static u16 ath11k_dp_rx_h_msdu_start_msdu_len(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_start_msdu_len(desc);
}

static u8 ath11k_dp_rx_h_msdu_start_sgi(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_start_sgi(desc);
}

static u8 ath11k_dp_rx_h_msdu_start_rate_mcs(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_start_rate_mcs(desc);
}

static u8 ath11k_dp_rx_h_msdu_start_rx_bw(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_start_rx_bw(desc);
}

static u32 ath11k_dp_rx_h_msdu_start_freq(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_start_freq(desc);
}

static u8 ath11k_dp_rx_h_msdu_start_pkt_type(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_start_pkt_type(desc);
}

static u8 ath11k_dp_rx_h_msdu_start_nss(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	u8 mimo_ss_bitmap;

	mimo_ss_bitmap = ab->hw_params.hw_ops->rxdesc_msdu_start_nss(desc);
	return hweight8(mimo_ss_bitmap);
}

static u8 ath11k_dp_rx_h_mpdu_start_tid(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_mpdu_start_tid(desc);
}

static u16 ath11k_dp_rx_h_mpdu_start_peer_id(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_mpdu_start_peer_id(desc);
}

static u8 ath11k_dp_rx_h_msdu_end_l3pad(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_end_l3pad(desc);
}

static bool ath11k_dp_rx_h_msdu_end_first_msdu(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_end_first_msdu(desc);
}

static bool ath11k_dp_rx_h_msdu_end_last_msdu(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_end_last_msdu(desc);
}

static void ath11k_dp_rxdesc_end_tlv_copy(struct ath11k_base *ab, struct hal_rx_desc *fdesc,
					  struct hal_rx_desc *ldesc)
{
	ab->hw_params.hw_ops->rxdesc_end_tlv_copy(fdesc, ldesc);
}

static u32 ath11k_dp_rxdesc_get_mpdulen_err(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_get_mpdulen_err(desc);
}

static u32 ath11k_dp_rxdesc_get_decap_format(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_get_decap_format(desc);
}

static u8 *ath11k_dp_rxdesc_get_80211hdr(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_get_80211hdr(desc);
}

static bool ath11k_dp_rxdesc_mpdu_valid(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_mpdu_valid(desc);
}

static u32 ath11k_dp_rxdesc_get_ppduid(struct ath11k_base *ab, struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_get_ppduid(desc);
}

static u32 *ath11k_dp_rx_h_msdu_start_msdu_len_offset(struct ath11k_base *ab,
						      struct hal_rx_desc *desc)
{
	return ab->hw_params.hw_ops->rxdesc_msdu_start_msdu_len_offset(desc);
}

#if ATH11K_SUPPORT_MONITOR
void ath11k_dp_service_mon_ring(struct ath11k_base *ab)
{
	...
	//	  msecs_to_jiffies(10));
    queue_delayed_work(&ab->mon_reap_work, msecs_to_jiffies(10));
}
			       ...
			       gfp_t gfp)
{
	...
	while (num_remain > 0) {
		skb = sk_buff_pool_alloc_skb(&rx_ring->skb_pool, 
            DP_RX_BUFFER_ALIGN_SIZE);
		if (!skb)
			break;

        paddr = skb_data_map_paddr(skb);
        if (!paddr)
            goto fail_free_skb;
					 ...
					 struct dp_rxdma_ring *rx_ring)
{
    ...
    idr_iter_t iter;
	spin_lock_bh(&rx_ring->idr_lock);
	idr_for_each_entry(&rx_ring->bufs_idr, iter, buf_id, skb) {
		idr_remove(&rx_ring->bufs_idr, buf_id);
	...
	spin_unlock_bh(&rx_ring->idr_lock);
	sk_buff_pool_purge(&rx_ring->skb_pool);
    ...
    idr_for_each_entry(&rx_ring->bufs_idr, iter, buf_id, skb) {
		 ...
		 * of rxdma_buffer.
		 */
		dma_unmap_single(ar->ab->dev, ATH11K_SKB_RXCB(skb)->paddr, ATH11K_SKB_RXCB(skb)->paddr_mapping_addr,
					  ...
					  u32 ringtype)
{
    ...
    /* additional DP_RXDMA_BUF_PENDING_SIZE skb for cache */
    sk_buff_pool_init(&rx_ring->skb_pool, 
	    num_entries + DP_RXDMA_BUF_PENDING_SIZE,
        DP_RX_BUFFER_SIZE + DP_RX_BUFFER_ALIGN_SIZE,
        ar->ab->dev);
...
static int ath11k_dp_rx_pdev_srng_alloc(struct ath11k *ar)
{
    ...
    if (!ab->hw_params.rxdma1_enable) {
        ...
        init_delayed_work(&ar->ab->mon_reap_work, ath11k_dp_service_mon_ring, ab);
		return 0;
    }
#endif
...
void ath11k_dp_reo_cmd_list_cleanup(struct ath11k_base *ab)
{
	...
	list_for_each_entry_safe(cmd, tmp, &dp->reo_cmd_list, struct dp_reo_cmd, list) {
		list_del(&cmd->list);
        dma_free_coherent(ab->dev, cmd->data.alloc_size, cmd->data.vaddr,
            cmd->data.paddr, cmd->data.paddr_mapping_addr);
		kfree(cmd);
	}

	list_for_each_entry_safe(cmd_cache, tmp_cache,
				 &dp->reo_cmd_cache_flush_list, struct dp_reo_cache_flush_elem, list) {
		list_del(&cmd_cache->list);
        dp->reo_cmd_cache_flush_count--;
        dma_free_coherent(ab->dev, cmd_cache->data.alloc_size, cmd_cache->data.vaddr,
            cmd_cache->data.paddr, cmd_cache->data.paddr_mapping_addr);
		kfree(cmd_cache);
	}
				   ...
				   enum hal_reo_cmd_status status)
{
    ...
    dma_free_coherent(dp->ab->dev, rx_tid->alloc_size, rx_tid->vaddr, 
        rx_tid->paddr, rx_tid->paddr_mapping_addr);
}
				      ...
				      struct dp_rx_tid *rx_tid)
{
	...
	while (tot_desc_sz > desc_sz) {
		tot_desc_sz -= desc_sz;
		cmd.addr_lo = lower_32_bits(rx_tid->paddr_aligned + tot_desc_sz);
		cmd.addr_hi = (u8)upper_32_bits(rx_tid->paddr_aligned);
	...
	cmd.addr_lo = lower_32_bits(rx_tid->paddr_aligned);
	cmd.addr_hi = (u8)upper_32_bits(rx_tid->paddr_aligned);
	...
	if (ret) {
		ath11k_err(ab, "failed to send HAL_REO_CMD_FLUSH_CACHE cmd, tid %d (%d)\n",
			   rx_tid->tid, ret);
        dma_free_coherent(ab->dev, rx_tid->alloc_size, rx_tid->vaddr, 
            rx_tid->paddr, rx_tid->paddr_mapping_addr);
	}
}
				      ...
				      enum hal_reo_cmd_status status)
{
	...
	spin_lock_bh(&dp->reo_cmd_lock);
	list_for_each_entry_safe(elem, tmp, &dp->reo_cmd_cache_flush_list, struct dp_reo_cache_flush_elem,
				 list) {
				      ...
				      struct ath11k_peer *peer, u8 tid)
{
	...
	if (ret) {
		ath11k_err(ar->ab, "failed to send HAL_REO_CMD_UPDATE_RX_QUEUE cmd, tid %d (%d)\n",
			   tid, ret);
        dma_free_coherent(ar->ab->dev, rx_tid->alloc_size, rx_tid->vaddr,
            rx_tid->paddr, rx_tid->paddr_mapping_addr);
	}
...
void ath11k_peer_frags_flush(struct ath11k *ar, struct ath11k_peer *peer)
{
	...
	for (i = 0; i <= IEEE80211_NUM_TIDS; i++) {
		...
		cancel_delayed_work_sync(&rx_tid->frag_timer);
			     ...
			     enum hal_pn_type pn_type)
{
	...
	u32 *addr_aligned;
	void *vaddr;
	dma_addr_t paddr, paddr_aligned;
	...
	if (rx_tid->active) {
        paddr_aligned = rx_tid->paddr_aligned;
    ...
    vaddr = dma_alloc_coherent(ab->dev, hw_desc_sz + HAL_LINK_DESC_ALIGN - 1, 
	...
	addr_aligned = (u32 *)PTR_ALIGN(vaddr, HAL_LINK_DESC_ALIGN);
    paddr_aligned = paddr + (u32)((u8*)addr_aligned - (u8*)vaddr);
	...
	rx_tid->vaddr = vaddr;
	rx_tid->paddr = paddr;
    rx_tid->paddr_mapping_addr = paddr_mapping_addr;
    rx_tid->alloc_size = hw_desc_sz + HAL_LINK_DESC_ALIGN - 1;
    rx_tid->paddr_aligned = paddr_aligned;
...
{
	struct ath11k_base *ab = ar->ab;
	struct ath11k_sta *arsta = ieee80211_sta_get_priv_data(params->sta);
	int vdev_id = arsta->arvif->vdev_id;
	int ret;

	ret = ath11k_peer_rx_tid_setup(ar, ieee80211_sta_addr(params->sta), vdev_id,
				       (u8)params->tid, params->buf_size,
			    ...
			    struct ieee80211_ampdu_params *params)
{
	...
	peer = ath11k_peer_find(ab, vdev_id, ieee80211_sta_addr(params->sta));
	if (!peer) {
	...
	ret = ath11k_wmi_peer_rx_reorder_queue_setup(ar, vdev_id,
        ieee80211_sta_addr(params->sta), paddr,
        (u8)params->tid, 1, 1);
	if (ret)
			   ...
			   void *data)
{
	...
	while (len > 0) {
		...
		tlv_tag = (u16)FIELD_GET(HTT_TLV_TAG, tlv->header);
		tlv_len = (u16)FIELD_GET(HTT_TLV_LEN, tlv->header);
...
static inline u8 ath11k_he_gi_to_nl80211_he_gi(u8 sgi)
{
	u8 ret = 0;
	switch (sgi) {
				...
				struct htt_ppdu_stats *ppdu_stats, u8 user)
{
	...
	if (usr_stats->tlv_flags &
		...
		succ_pkts = (u16)FIELD_GET(HTT_PPDU_STATS_ACK_BA_INFO_NUM_MSDU_M,
				      usr_stats->ack_ba.info);
		tid = (u8)FIELD_GET(HTT_PPDU_STATS_ACK_BA_INFO_TID_NUM,
				usr_stats->ack_ba.info);
	}
	...
	flags = (u8)HTT_USR_RATE_PREAMBLE(user_rate->rate_flags);
	bw = (u8)HTT_USR_RATE_BW(user_rate->rate_flags) - 2;
	nss = (u8)HTT_USR_RATE_NSS(user_rate->rate_flags) + 1;
	mcs = (u8)HTT_USR_RATE_MCS(user_rate->rate_flags);
	sgi = (u8)HTT_USR_RATE_GI(user_rate->rate_flags);
	dcm = (u8)HTT_USR_RATE_DCM(user_rate->rate_flags);
    ...
    arsta = ieee80211_sta_get_priv_data(peer->sta);
	...
	case WMI_RATE_PREAMBLE_HT:
		...
		arsta->txrate.flags = IEEE80211_RATE_INFO_FLAGS_MCS;
		if (sgi)
			arsta->txrate.flags |= IEEE80211_RATE_INFO_FLAGS_SHORT_GI;
		break;
	case WMI_RATE_PREAMBLE_VHT:
		arsta->txrate.mcs = mcs;
		arsta->txrate.flags = IEEE80211_RATE_INFO_FLAGS_VHT_MCS;
		if (sgi)
			arsta->txrate.flags |= IEEE80211_RATE_INFO_FLAGS_SHORT_GI;
		break;
	case WMI_RATE_PREAMBLE_HE:
		arsta->txrate.mcs = mcs;
		arsta->txrate.flags = IEEE80211_RATE_INFO_FLAGS_HE_MCS;
		arsta->txrate.he.dcm = dcm;
		arsta->txrate.he.gi = ath11k_he_gi_to_nl80211_he_gi(sgi);
		arsta->txrate.he.ru_alloc = ath11k_he_ru_tones_to_nl80211_he_ru_alloc(
						(user_rate->ru_end -
	...
	memcpy(&arsta->last_txrate, &arsta->txrate, sizeof(struct ieee80211_rate_info));
							...
							u32 ppdu_id)
{
	...
	if (!list_is_empty(&ar->ppdu_stats_info)) {
		list_for_each_entry(ppdu_info, &ar->ppdu_stats_info, struct htt_ppdu_stats_info, list) {
		...
		if (ar->ppdu_stat_list_depth > HTT_PPDU_DESC_MAX_DEPTH) {
			ppdu_info = BASE_CR(list_first_entry(&ar->ppdu_stats_info), struct htt_ppdu_stats_info, list);
				      ...
				      struct sk_buff *skb)
{
	...
	if (ath11k_debug_is_pktlog_lite_mode_enabled(ar)) ; //TODOTODO
						  ...
						  struct sk_buff *skb)
{
	...
	DEBUG_PARAMETER(pdev_id);
	DEBUG_PARAMETER(ring_type);
	DEBUG_PARAMETER(ring_id);
	DEBUG_PARAMETER(hp);
	DEBUG_PARAMETER(tp);
	DEBUG_PARAMETER(backpressure_time);

	pdev_id = (u8)FIELD_GET(HTT_BACKPRESSURE_EVENT_PDEV_ID_M, *data);
	ring_type = (u8)FIELD_GET(HTT_BACKPRESSURE_EVENT_RING_TYPE_M, *data);
	ring_id = (u8)FIELD_GET(HTT_BACKPRESSURE_EVENT_RING_ID_M, *data);
	++data;

	hp = (u16)FIELD_GET(HTT_BACKPRESSURE_EVENT_HP_M, *data);
	tp = (u16)FIELD_GET(HTT_BACKPRESSURE_EVENT_TP_M, *data);
	++data;
				       ...
				       struct sk_buff *skb)
{
	...
	case HTT_T2H_MSG_TYPE_VERSION_CONF:
		dp->htt_tgt_ver_major = (u8)FIELD_GET(HTT_T2H_VERSION_CONF_MAJOR,
						  resp->u.version_msg.version);
		dp->htt_tgt_ver_minor = (u8)FIELD_GET(HTT_T2H_VERSION_CONF_MINOR,
						  resp->u.version_msg.version);
		complete(&dp->htt_tgt_version_received);
		break;
	case HTT_T2H_MSG_TYPE_PEER_MAP:
		vdev_id = (u8)FIELD_GET(HTT_T2H_PEER_MAP_INFO_VDEV_ID,
				    resp->u.peer_map_ev.info);
		peer_id = (u16)FIELD_GET(HTT_T2H_PEER_MAP_INFO_PEER_ID,
				    resp->u.peer_map_ev.info);
		peer_mac_h16 = (u16)FIELD_GET(HTT_T2H_PEER_MAP_INFO1_MAC_ADDR_H16,
					 resp->u.peer_map_ev.info1);
		ath11k_dp_get_mac_addr(resp->u.peer_map_ev.mac_addr_l32,
	...
	case HTT_T2H_MSG_TYPE_PEER_MAP2:
		...
		ast_hash = FIELD_GET(HTT_T2H_PEER_MAP_INFO2_AST_HASH_VAL,
				     resp->u.peer_map_ev.info2);
		ath11k_peer_map_event(ab, vdev_id, peer_id, mac_addr, ast_hash);
		break;
	case HTT_T2H_MSG_TYPE_PEER_UNMAP:
	case HTT_T2H_MSG_TYPE_PEER_UNMAP2:
		peer_id = (u16)FIELD_GET(HTT_T2H_PEER_UNMAP_INFO_PEER_ID,
				    resp->u.peer_unmap_ev.info);
				      ...
				      u8 l3pad_bytes, int msdu_len)
{
	 ...
	 */
	buf_first_hdr_len = ab->hw_params.hal_desc_sz + l3pad_bytes;
	 ...
	 * exceeds DP_RX_BUFFER_SIZE - ab->hw_params.hal_desc_sz. So assume the data
	 * in the first buf is of length DP_RX_BUFFER_SIZE - ab->hw_params.hal_desc_sz.
	 */
	 ...
	 */
	ab->dp.rx_ops->rxdesc_end_tlv_copy(rxcb->rx_desc, ldesc);
	...
	while ((skb = __skb_dequeue(msdu_list)) != NULL && rem_len > 0) {
		rxcb = ATH11K_SKB_RXCB(skb);
		if (rxcb->is_continuation)
			buf_len = (int)(DP_RX_BUFFER_SIZE - ab->hw_params.hal_desc_sz);
		else
			buf_len = rem_len;

		if (buf_len > (int)(DP_RX_BUFFER_SIZE - ab->hw_params.hal_desc_sz)) {
			WARN_ON_ONCE(1);
			dev_kfree_skb_any(skb);
			return -EINVAL;
		}

		skb_put(skb, (u32)(buf_len + ab->hw_params.hal_desc_sz));
		skb_pull(skb, (u32)ab->hw_params.hal_desc_sz);
...
static void ath11k_dp_rx_h_csum_offload(struct ath11k_base *ab, struct sk_buff *msdu)
{
	struct ath11k_skb_rxcb *rxcb = ATH11K_SKB_RXCB(msdu);
	bool ip_csum_fail, l4_csum_fail;

	ip_csum_fail = ath11k_dp_rx_h_attn_ip_cksum_fail(ab, rxcb->rx_desc);
	l4_csum_fail = ath11k_dp_rx_h_attn_l4_cksum_fail(ab, rxcb->rx_desc);
				       ...
				       enum hal_encrypt_type enctype)
{
	...
	case HAL_ENCRYPT_TYPE_CCMP_128:
		return IEEE80211_CCMP_MIC_LENGTH;
	case HAL_ENCRYPT_TYPE_CCMP_256:
		return IEEE80211_CCMP_256_MIC_LENGTH;
	case HAL_ENCRYPT_TYPE_GCMP_128:
	case HAL_ENCRYPT_TYPE_AES_GCMP_256:
		return IEEE80211_GCMP_MIC_LENGTH;
					 ...
					 enum hal_encrypt_type enctype)
{
	...
	case HAL_ENCRYPT_TYPE_TKIP_NO_MIC:
	case HAL_ENCRYPT_TYPE_TKIP_MIC:
		return IEEE80211_TKIP_IV_LENGTH;
	case HAL_ENCRYPT_TYPE_CCMP_128:
		return IEEE80211_CCMP_HDR_LENGTH;
	case HAL_ENCRYPT_TYPE_CCMP_256:
		return IEEE80211_CCMP_256_HDR_LENGTH;
	case HAL_ENCRYPT_TYPE_GCMP_128:
	case HAL_ENCRYPT_TYPE_AES_GCMP_256:
		return IEEE80211_GCMP_HDR_LENGTH;
	case HAL_ENCRYPT_TYPE_WEP_40:
	case HAL_ENCRYPT_TYPE_WEP_104:
	case HAL_ENCRYPT_TYPE_WEP_128:
	case HAL_ENCRYPT_TYPE_WAPI_GCM_SM4:
				       ...
				       enum hal_encrypt_type enctype)
{
	...
	case HAL_ENCRYPT_TYPE_TKIP_NO_MIC:
	case HAL_ENCRYPT_TYPE_TKIP_MIC:
		return IEEE80211_TKIP_ICV_LENGTH;
	case HAL_ENCRYPT_TYPE_WEP_40:
	case HAL_ENCRYPT_TYPE_WEP_104:
	case HAL_ENCRYPT_TYPE_WEP_128:
	case HAL_ENCRYPT_TYPE_WAPI_GCM_SM4:
					 ...
					 struct ieee80211_rx_status *status)
{
	...
	if (rxcb->is_first_msdu) {
		...
		if (ieee80211_is_data_qos(hdr->frame_control)) {
			...
			qos[0] &= ~IEEE80211_QOS_A_MSDU_PRESENT;
		}
	} else {
		/*  Rebuild qos header if this is a middle/last msdu */
		hdr->frame_control |= __cpu_to_le16(IEEE80211_FC_SUBTYPE_QOS);

		/* Reset the order bit as the HT_Control header is stripped */
		hdr->frame_control &= ~(__cpu_to_le16(IEEE80211_FC_ORDER));

		qos_ctl = rxcb->tid;

		if (ath11k_dp_rx_h_msdu_start_mesh_ctl_present(ar->ab, rxcb->rx_desc))
			qos_ctl |= IEEE80211_QOS_MESH_CONTROL_PRESENT;
	...
	if (!(status->flag & IEEE80211_RX_FLAG_IV_STRIPPED)) {
	...
	if (!rxcb->is_first_msdu) {
		memcpy(skb_push(msdu,
				IEEE80211_QOS_LEN), &qos_ctl,
				IEEE80211_QOS_LEN);
		memcpy(skb_push(msdu, (u32)hdr_len), decap_hdr, (u32)hdr_len);
		return;
	}

	memcpy(skb_push(msdu, (u32)hdr_len), hdr, (u32)hdr_len);
				       ...
				       bool decrypted)
{
	...
	skb_trim(msdu, msdu->len - DP_RX_FCS_LEN);
	...
	if (status->flag & IEEE80211_RX_FLAG_IV_STRIPPED) {
		...
		/* MIC */
		if (status->flag & IEEE80211_RX_FLAG_MIC_STRIPPED)
			skb_trim(msdu, msdu->len -
				 ath11k_dp_rx_crypto_mic_len(ar, enctype));

		/* ICV */
		if (status->flag & IEEE80211_RX_FLAG_ICV_STRIPPED)
			skb_trim(msdu, msdu->len -
				 ath11k_dp_rx_crypto_icv_len(ar, enctype));
	}

	/* MMIC */
	if ((status->flag & IEEE80211_RX_FLAG_MMIC_STRIPPED) &&
	    !ieee80211_has_morefrags(hdr->frame_control) &&
	    enctype == HAL_ENCRYPT_TYPE_TKIP_MIC)
		skb_trim(msdu, msdu->len - IEEE80211_CCMP_MIC_LENGTH);
					 ...
					 enum hal_encrypt_type enctype)
{
	...
	if (rxcb->is_first_msdu) {
		...
		rfc1042 = (u8*)rfc1042 + hdr_len + crypto_len;
	}

	if (is_amsdu)
		rfc1042 = (u8*)rfc1042 + sizeof(struct ath11k_dp_amsdu_subframe_hdr);
...
#if 0 /* NO PN check for UEFI */
		/* PN for mcast packets will be validated in mac80211;
		 * remove eth header and add 802.11 header.
		 */
		if (ATH11K_SKB_RXCB(msdu)->is_mcbc) {
			status->flag |= IEEE80211_RX_FLAG_80211_MCAST;
			if (decrypted)
...
ath11k_dp_rx_h_find_peer(struct ath11k_base *ab, struct sk_buff *msdu) 
{
	...
	if (rxcb->peer_id) {
		...
		if (peer) {
            ath11k_info(ab, "peer_id %d peer mac: %02x-%02x-%02x-%02x-%02x-%02x\n", rxcb->peer_id,
	...
	peer = ath11k_peer_find_by_addr(ab, ath11k_dp_rx_h_mpdu_start_addr2(ab, rxcb->rx_desc));
				...
				struct ieee80211_rx_status *rx_status)
{
    ...
    hdr_eth = (struct ethhdr *)msdu->data;
	
	rxcb = ATH11K_SKB_RXCB(msdu);
	rxcb->is_mcbc = is_multicast_ether_addr(hdr_eth->h_dest);
	...
	err_bitmap = ath11k_dp_rx_h_attn_mpdu_err(ar->ab, rx_desc);
	is_decrypted = ath11k_dp_rx_h_attn_is_decrypted(ar->ab, rx_desc);
	
	/* Clear per-MPDU flags while leaving per-PPDU flags intact */
	rx_status->flag &= ~(IEEE80211_RX_FLAG_FAILED_FCS_CRC |
				 IEEE80211_RX_FLAG_MMIC_ERROR |
				 IEEE80211_RX_FLAG_DECRYPTED |
				 IEEE80211_RX_FLAG_IV_STRIPPED |
				 IEEE80211_RX_FLAG_MMIC_STRIPPED);

	if (err_bitmap & DP_RX_MPDU_ERR_FCS)
		rx_status->flag |= IEEE80211_RX_FLAG_FAILED_FCS_CRC;
	if (err_bitmap & DP_RX_MPDU_ERR_TKIP_MIC)
		rx_status->flag |= IEEE80211_RX_FLAG_MMIC_ERROR;
	if (err_bitmap & DP_RX_MPDU_ERR_MSDU_LEN)
		rx_status->flag |= IEEE80211_RX_FLAG_MSDU_ERR_LEN;

	if (is_decrypted) {
		rx_status->flag |= IEEE80211_RX_FLAG_DECRYPTED | IEEE80211_RX_FLAG_MMIC_STRIPPED;

		if (fill_crypto_hdr)
			rx_status->flag |= IEEE80211_RX_FLAG_MIC_STRIPPED |
					IEEE80211_RX_FLAG_ICV_STRIPPED;
		else
			rx_status->flag |= IEEE80211_RX_FLAG_IV_STRIPPED |
					   IEEE80211_RX_FLAG_PN_VALIDATED;
	}

	ath11k_dp_rx_h_csum_offload(ar->ab, msdu);
	...
	if (ath11k_dp_rx_h_msdu_start_decap_type(ar->ab, rx_desc) !=
		...
		hdr = (void *)msdu->data;
		hdr->frame_control &= ~__cpu_to_le16(IEEE80211_FC_WEP);
	}
}
				...
				struct ieee80211_rx_status *rx_status)
{
	...
	rx_status->bw = IEEE80211_RATE_INFO_BW_20;

	rx_status->flag |= IEEE80211_RX_FLAG_NO_SIGNAL_VAL;

	channel_num = ath11k_dp_rx_h_msdu_start_freq(ab, rx_desc);
	center_freq = ath11k_dp_rx_h_msdu_start_freq(ab, rx_desc) >> 16;
	...
	if (center_freq >= ATH11K_MIN_6G_FREQ && center_freq <= ATH11K_MAX_6G_FREQ) {
		...
		spin_unlock_bh(&ar->data_lock);
#if 0
		ath11k_dbg_dump(ab, ATH11K_DBG_DATA, "rx_desc: ",
				rx_desc, (u32)ab->hw_params.hal_desc_sz, 0);
#endif
	}

	rx_status->freq = (u16)ieee80211_channel_to_frequency((int)channel_num,
							 rx_status->band);

	if (rx_status->band == NL80211_BAND_6GHZ)
		rx_status->freq = (u16)center_freq;

    if (rx_status->band < NUM_NL80211_BANDS)
		ath11k_dp_rx_h_rate(ar, rx_desc, rx_status);
}

static int ath11k_dp_rx_deliver_msdu(struct ath11k *ar, struct napi_struct *napi,
				      struct sk_buff *msdu, struct ieee80211_rx_status *status)
{
    struct ath11k_base *ab = ar->ab;
    struct ieee80211_rx_status *rx_status;
	...
	DEBUG_PARAMETER(enctype);
	DEBUG_PARAMETER(is_amsdu);

	if (!(status->flag & IEEE80211_RX_FLAG_ONLY_MONITOR)) {
		decap = ath11k_dp_rx_h_msdu_start_decap_type(ab, rxcb->rx_desc);
	} else {
	...
	spin_lock_bh(&ab->base_lock);
	peer = ath11k_dp_rx_h_find_peer(ab, msdu);
	if (peer && peer->sta) {
		pubsta = peer->sta;
		enctype = peer->sec_type;
	}
	...
	ath11k_dbg(ab, ATH11K_DBG_DATA,
		   "is_amsdu %x is_mcbc %x enctype %d decap %d rx skb %pK len %u peer %pM tid %u sn %u %a%a%a%a%a%a%a %a rate_idx %u vht_nss %u freq %u band %u flag 0x%x fcs-err %x mic-err %x amsdu-more %x\n",
		   is_amsdu,
		   is_mcbc,
		   enctype,
           decap,
		   ...
		   (status->bw == IEEE80211_RATE_INFO_BW_40) ? "40" : "",
		   (status->bw == IEEE80211_RATE_INFO_BW_80) ? "80" : "",
		   (status->bw == IEEE80211_RATE_INFO_BW_160) ? "160" : "",
		   status->enc_flags & RX_ENC_FLAG_SHORT_GI ? "sgi " : "",
		   status->rate_idx,
		   status->nss,
		   status->freq,
		   status->band, status->flag,
		   !!(status->flag & IEEE80211_RX_FLAG_FAILED_FCS_CRC),
		   !!(status->flag & IEEE80211_RX_FLAG_MMIC_ERROR),
		   !!(status->flag & IEEE80211_RX_FLAG_AMSDU_MORE));

	if (status->flag & IEEE80211_RX_FLAG_MSDU_ERR_LEN) {
		ath11k_warn(ab, "Got msdu length error, drop it\n");
		return -EIO;
	}
	...
	*rx_status = *status;
	...
	if (decap != DP_RX_DECAP_TYPE_ETHERNET2_DIX ||
		((rx_status->flag & IEEE80211_RX_FLAG_80211_MCAST) &&
			is_mcbc && (rx_status->flag & IEEE80211_RX_FLAG_DECRYPTED))) {
		ath11k_dbg_dump(ab, ATH11K_DBG_PKT, "rx 802.11 data ", msdu->data, msdu->len, 0);
		ieee80211_rx_napi(ar->hw, NULL, msdu);
	}
	else {
		ath11k_dbg_dump(ab, ATH11K_DBG_PKT, "rx 802.3 data ", msdu->data, msdu->len, 0);
...
static int ath11k_dp_rx_process_msdu(struct ath11k *ar,
				     struct sk_buff *msdu,
				     struct sk_buff_head *msdu_list, struct ieee80211_rx_status *rx_status)
{
	void *rx_desc, *lrx_desc;
	struct ath11k_skb_rxcb *rxcb;
	...
	rx_desc = msdu->data;
	lrx_desc = last_buf->data;
	if (!ath11k_dp_rx_h_attn_msdu_done(ab, lrx_desc)) {
	...
	if (rxcb->is_frag) {
		skb_pull(msdu, (u32)ab->hw_params.hal_desc_sz);
	} else if (!rxcb->is_continuation) {
		if ((msdu_len + ab->hw_params.hal_desc_sz) > DP_RX_BUFFER_SIZE) {
			hdr_status = ath11k_dp_rx_h_80211_hdr(ab, rx_desc);
			ret = -EINVAL;
			ath11k_warn(ab, "invalid msdu len %u\n", msdu_len);
			ath11k_dbg_dump(ab, ATH11K_DBG_DATA, "", hdr_status,
					sizeof(struct ieee80211_hdr), 0);
			ath11k_dbg_dump(ab, ATH11K_DBG_DATA, "", rx_desc,
					(u32)ab->hw_params.hal_desc_sz, 0);
			goto free_out;
		}
		skb_put(msdu, (u32)(ab->hw_params.hal_desc_sz + l3_pad_bytes + msdu_len));
		skb_pull(msdu, (u32)(ab->hw_params.hal_desc_sz + l3_pad_bytes));
	} else {
	...
	rx_status->flag |= IEEE80211_RX_FLAG_SKIP_MONITOR | IEEE80211_RX_FLAG_DUP_VALIDATED;
 	return 0;
						  ...
						  int *quota, int ring_id)
{
	...
	while (*quota && (msdu = __skb_dequeue(msdu_list)) != NULL) {
		rxcb = ATH11K_SKB_RXCB(msdu);
		mac_id = rxcb->mac_id;
		...
		if (ath11k_dp_rx_deliver_msdu(ar, napi, msdu, &rx_status)) {
...
try_again:
	while ((rx_desc = ath11k_hal_srng_dst_get_next_entry(ab, srng)) != NULL) {
		...
		if (push_reason !=
			...
			dev_kfree_skb_any(msdu);
            if (dp->reo_dst_ring[ring_id].ring_id < ARRAY_LENGTH(ab->soc_stats.hal_reo_error))
		...
		rxcb->peer_id = (u16)FIELD_GET(RX_MPDU_DESC_META_DATA_PEER_ID,
					  desc.rx_mpdu_info.meta_data);
		rxcb->seq_no = (u16)FIELD_GET(RX_MPDU_DESC_INFO0_SEQ_NUM,
					 desc.rx_mpdu_info.info0);
		rxcb->tid = (u8)FIELD_GET(HAL_REO_DEST_RING_INFO0_RX_QUEUE_NUM,
				      desc.info0);
		rxcb->mac_id = (u8)mac_id;
							 ...
							 int *buf_id, gfp_t gfp)
{
	...
	*buf_id = idr_alloc(&rx_ring->bufs_idr, skb, 0,
	...
	ATH11K_SKB_RXCB(skb)->paddr = paddr;
    ATH11K_SKB_RXCB(skb)->paddr_mapping_addr = 0;
	return skb;
				    ...
				    struct napi_struct *napi, int budget)
{
	...
	while ((skb = __skb_dequeue(&skb_list)) != NULL) {
		...
		if (ath11k_debug_is_pktlog_rx_stats_enabled(ar)) ; //TODOTODO:
		...
		if (ath11k_debug_is_pktlog_peer_valid(ar, peer->addr)) ;//TODOTODO:
			//trace_ath11k_htt_rxdesc(ar, skb->data, DP_RX_BUFFER_SIZE);
...
static void ath11k_dp_rx_frag_timer(void *ctx)
{
	struct dp_rx_tid *rx_tid = ctx;
...
int ath11k_peer_rx_frag_setup(struct ath11k *ar, const u8 *peer_mac, int vdev_id)
{
	...
	for (i = 0; i <= IEEE80211_NUM_TIDS; i++) {
        ...
        init_delayed_work(&rx_tid->frag_timer, ath11k_dp_rx_frag_timer, rx_tid);
...
{
#if 0 // michael_mic is not supported
...
static int ath11k_dp_rx_h_verify_tkip_mic(struct ath11k *ar, struct ath11k_peer *peer,
					  struct sk_buff *msdu)
{
	void *rx_desc = msdu->data;
	struct ieee80211_rx_status *rxs = IEEE80211_SKB_RXCB(msdu);
	struct ieee80211_key_conf *key_conf;
	struct ieee80211_hdr *hdr;
	u8 mic[IEEE80211_CCMP_MIC_LENGTH];
	size_t head_len, tail_len, ret;
	size_t data_len;
	u32 hdr_len;
	u8 *key, *data;
	u8 key_idx = 0;
	...
	if (ath11k_dp_rx_h_mpdu_start_enctype(ab, rx_desc) != HAL_ENCRYPT_TYPE_TKIP_MIC)
		return 0;

	hdr = (struct ieee80211_hdr *)(msdu->data + ab->hw_params.hal_desc_sz);
	hdr_len = ieee80211_hdrlen(hdr->frame_control);
	head_len = hdr_len + ab->hw_params.hal_desc_sz + IEEE80211_TKIP_IV_LENGTH;
	tail_len = IEEE80211_CCMP_MIC_LENGTH + IEEE80211_TKIP_ICV_LENGTH + DP_RX_FCS_LEN;
	...
	if (ret || memcmp(mic, data + data_len, IEEE80211_CCMP_MIC_LENGTH))
		goto mic_fail;
...
mic_fail:
	...
	rxs->flag |= IEEE80211_RX_FLAG_MMIC_ERROR | IEEE80211_RX_FLAG_MMIC_STRIPPED |
		    IEEE80211_RX_FLAG_IV_STRIPPED | IEEE80211_RX_FLAG_DECRYPTED;
	skb_pull(msdu, (u32)ab->hw_params.hal_desc_sz);
					...
					enum hal_encrypt_type enctype, u32 flags)
{
	...
	if (flags & IEEE80211_RX_FLAG_MIC_STRIPPED)
		skb_trim(msdu, msdu->len -
			 ath11k_dp_rx_crypto_mic_len(ar, enctype));

	if (flags & IEEE80211_RX_FLAG_ICV_STRIPPED)
		skb_trim(msdu, msdu->len -
			 ath11k_dp_rx_crypto_icv_len(ar, enctype));

	if (flags & IEEE80211_RX_FLAG_IV_STRIPPED) {
		hdr_len = ieee80211_hdrlen(hdr->frame_control);
		crypto_len = ath11k_dp_rx_crypto_param_len(ar, enctype);

		memmove((u8 *)msdu->data + ab->hw_params.hal_desc_sz + (u32)crypto_len,
			(u8 *)msdu->data + ab->hw_params.hal_desc_sz, (u32)hdr_len);
		skb_pull(msdu, (u32)crypto_len);
	}
}
				 ...
				 struct sk_buff **defrag_skb)
{
	...
	skb_queue_walk(&rx_tid->rx_frags, skb) {
		...
		enctype = ath11k_dp_rx_h_mpdu_start_enctype(ab, rx_desc);
		if (enctype != HAL_ENCRYPT_TYPE_OPEN)
		    is_decrypted = ath11k_dp_rx_h_attn_is_decrypted(ab, rx_desc);

		if (is_decrypted) {
			if (skb != first_frag)
				flags |=  IEEE80211_RX_FLAG_IV_STRIPPED;
			if (skb != last_frag)
				flags |= IEEE80211_RX_FLAG_ICV_STRIPPED |
					 IEEE80211_RX_FLAG_MIC_STRIPPED;
		}

		/* RX fragments are always raw packets */
		if (skb != last_frag)
			skb_trim(skb, skb->len - DP_RX_FCS_LEN);
	...
	while ((skb = __skb_dequeue(&rx_tid->rx_frags)) != NULL) {
		skb_put_data(first_frag, skb->data, skb->len);
		dev_kfree_skb_any(skb);
	}

	hdr = (struct ieee80211_hdr*)(first_frag->data + ab->hw_params.hal_desc_sz);
	hdr->frame_control &= ~__cpu_to_le16(IEEE80211_FC_MORE_FRAG);
					      ...
					      struct sk_buff *defrag_skb)
{
	...
	msdu_info = FIELD_PREP(RX_MSDU_DESC_INFO0_FIRST_MSDU_IN_MPDU, 1) |
		    ...
		    FIELD_PREP(RX_MSDU_DESC_INFO0_MSDU_LENGTH,
			       defrag_skb->len - ab->hw_params.hal_desc_sz) |
	...
	msdu_len_offset = ath11k_dp_rx_h_msdu_start_msdu_len_offset(ab, rx_desc);
	*msdu_len_offset &= ~(RX_MSDU_START_INFO1_MSDU_LENGTH);
	*msdu_len_offset |= defrag_skb->len - ab->hw_params.hal_desc_sz;
...
static int ath11k_dp_rx_h_cmp_frags(struct ath11k_base *ab, struct sk_buff *a, struct sk_buff *b)
{
	int frag1, frag2;

	frag1 = ath11k_dp_rx_h_mpdu_start_frag_no(ab, a);
	frag2 = ath11k_dp_rx_h_mpdu_start_frag_no(ab, b);

	return frag1 - frag2;
}

static void ath11k_dp_rx_h_sort_frags(struct ath11k_base *ab, struct sk_buff_head *frag_list,
				      struct sk_buff *cur_frag)
{
	...
	skb_queue_walk(frag_list, skb) {
		cmp = ath11k_dp_rx_h_cmp_frags(ab, skb, cur_frag);
		if (cmp < 0)
			continue;
...
static u64 ath11k_dp_rx_h_get_pn(struct ath11k_base *ab, struct sk_buff *skb)
{
	struct ieee80211_hdr *hdr;
	u64 pn = 0;
	u8 *ehdr;
	...
	hdr = (struct ieee80211_hdr *)(skb->data + ab->hw_params.hal_desc_sz);
	ehdr = skb->data + ab->hw_params.hal_desc_sz + ieee80211_hdrlen(hdr->frame_control);
...
ath11k_dp_rx_h_defrag_validate_incr_pn(struct ath11k *ar, struct dp_rx_tid *rx_tid)
{
	...
	last_pn = ath11k_dp_rx_h_get_pn(ar->ab, first_frag);
	skb_queue_walk(&rx_tid->rx_frags, skb) {
		if (skb == first_frag)
			continue;

		cur_pn = ath11k_dp_rx_h_get_pn(ar->ab, skb);
		if (cur_pn != last_pn + 1)
			return false;
		last_pn = cur_pn;
	}
...
static bool
ath11k_dp_rx_h_defrag_validate_key(struct ath11k *ar,
	struct dp_rx_tid *rx_tid)
{
	struct sk_buff *first_frag, *skb;
	u32 peer_id;
	...
	peer_id = ath11k_dp_rx_h_mpdu_start_peer_id(ar->ab, (void*)first_frag->data);
	peer = ath11k_peer_find_by_id(ar->ab, peer_id);
	if (peer) {
		key_conf = peer->keys[peer->ucast_keyidx];

	if (key_conf)
		key_color = key_conf->color;
	else
		return true;
	}
	...
	skb_queue_walk(&rx_tid->rx_frags, skb) {
		...
		peer_id = ath11k_dp_rx_h_mpdu_start_peer_id(ar->ab, (void *)skb->data);
		peer = ath11k_peer_find_by_id(ar->ab, peer_id);

		if (peer) {
			key_conf = peer->keys[peer->ucast_keyidx];
			if (key_conf) {
				if (key_color != key_conf->color)
					return false;
			}
		}
	}
				    ...
				    u32 *ring_desc)
{
	...
	rx_desc = msdu->data;
	peer_id = ath11k_dp_rx_h_mpdu_start_peer_id(ab, rx_desc);
	tid = ath11k_dp_rx_h_mpdu_start_tid(ab, rx_desc);
	seqno = ath11k_dp_rx_h_mpdu_start_seq_no(ab, rx_desc);
	frag_no = ath11k_dp_rx_h_mpdu_start_frag_no(ab, msdu);
	more_frags = ath11k_dp_rx_h_mpdu_start_more_frags(ab, msdu);
	struct ath11k_skb_rxcb* rxcb = ATH11K_SKB_RXCB(msdu);
	/* Multicast/Broadcast fragments are not expected */
	if (rxcb && rxcb->is_mcbc) {
		return -EINVAL;
	}

	if (!ath11k_dp_rx_h_mpdu_start_seq_ctrl_valid(ab, rx_desc) ||
	    !ath11k_dp_rx_h_mpdu_start_fc_valid(ab, rx_desc) ||
	...
	if (frag_no > __fls(rx_tid->rx_frag_bitmap))
	    ...
	    ath11k_dp_rx_h_sort_frags(ar->ab, &rx_tid->rx_frags, msdu);
	...
	if (!rx_tid->last_frag_no ||
        ...
        queue_delayed_work(&rx_tid->frag_timer, ATH11K_DP_RX_FRAGMENT_TIMEOUT_MS);
		goto out_unlock;
	}
	...
	if (!ath11k_dp_rx_h_defrag_validate_key(ar, rx_tid))
		goto err_frags_cleanup;
...
ath11k_dp_process_rx_err_buf(struct ath11k *ar, u32 *ring_desc, int buf_id, bool drop)
{
	...
	msdu = idr_find(&rx_ring->bufs_idr, buf_id);
	if (!msdu) {
		ath11k_warn(ab, "rx err buf with invalid buf_id %d\n",
			    buf_id);
		spin_unlock_bh(&rx_ring->idr_lock);
		return -EINVAL;
	}
	...
	if ((msdu_len + ab->hw_params.hal_desc_sz) > DP_RX_BUFFER_SIZE) {
		hdr_status = ath11k_dp_rx_h_80211_hdr(ab, rx_desc);
		ath11k_warn(ab, "invalid msdu leng %u", msdu_len);
		ath11k_dbg_dump(ab, ATH11K_DBG_DATA, "", hdr_status,
				sizeof(struct ieee80211_hdr), 0);
		ath11k_dbg_dump(ab, ATH11K_DBG_DATA, "", rx_desc,
				(u32)ab->hw_params.hal_desc_sz, 0);
	...
	skb_put(msdu, (u32)(ab->hw_params.hal_desc_sz + msdu_len));
			     ...
			     int budget)
{
	...
	int tot_n_bufs_reaped, quota, ret;
	...
	while (budget &&
	       ((desc = ath11k_hal_srng_dst_get_next_entry(ab, srng)) != NULL)) {
					     ...
					     struct sk_buff_head *msdu_list)
{
	...
	n_buffs = (int)DIV_ROUND_UP(msdu_len,
			       (DP_RX_BUFFER_SIZE - ab->hw_params.hal_desc_sz));
				      ...
				      struct sk_buff_head *msdu_list)
{
	...
	msdu_len = ath11k_dp_rx_h_msdu_start_msdu_len(ab, desc);

	if (!rxcb->is_frag && ((msdu_len + ab->hw_params.hal_desc_sz) > DP_RX_BUFFER_SIZE)) {
		/* First buffer will be freed by the caller, so deduct it's length */
		msdu_len = msdu_len - (u16)(DP_RX_BUFFER_SIZE - ab->hw_params.hal_desc_sz);
		ath11k_dp_rx_null_q_desc_sg_drop(ar, msdu_len, msdu_list);
		return -EINVAL;
	}

	if (!ath11k_dp_rx_h_attn_msdu_done(ab, desc)) {
		ath11k_warn(ab,
	 ...
	 */
	rxcb->is_first_msdu = ath11k_dp_rx_h_msdu_end_first_msdu(ab, desc);
	rxcb->is_last_msdu = ath11k_dp_rx_h_msdu_end_last_msdu(ab, desc);

	if (rxcb->is_frag) {
		skb_pull(msdu, (u32)ab->hw_params.hal_desc_sz);
	} else {
		l3pad_bytes = ath11k_dp_rx_h_msdu_end_l3pad(ab, desc);

		if ((ab->hw_params.hal_desc_sz + l3pad_bytes + msdu_len) > DP_RX_BUFFER_SIZE)
			return -EINVAL;

		skb_put(msdu, (u32)(ab->hw_params.hal_desc_sz + l3pad_bytes + msdu_len));
		skb_pull(msdu, (u32)(ab->hw_params.hal_desc_sz + l3pad_bytes));
	}
	ath11k_dp_rx_h_ppdu(ar, desc, status);

	ath11k_dp_rx_h_mpdu(ar, msdu, desc, status);

	rxcb->tid = ath11k_dp_rx_h_mpdu_start_tid(ab, desc);
				   ...
				   struct sk_buff_head *msdu_list)
{
    ...
    if (rxcb->err_code < ARRAY_LENGTH(ab->soc_stats.reo_error))
        ab->soc_stats.reo_error[rxcb->err_code]++;
					...
					struct ieee80211_rx_status *status)
{
	...
	status->flag |= (IEEE80211_RX_FLAG_MMIC_STRIPPED | IEEE80211_RX_FLAG_MMIC_ERROR |
			 IEEE80211_RX_FLAG_DECRYPTED);
				     ...
				     struct ieee80211_rx_status *status)
{
    ...
    if (rxcb->err_code < ARRAY_LENGTH(ab->soc_stats.rxdma_error))
        ab->soc_stats.rxdma_error[rxcb->err_code]++;
				 ...
				 struct sk_buff_head *msdu_list)
{
	...
	if (ath11k_dp_rx_deliver_msdu(ar, napi, msdu, &rxs)) {
		__skb_queue_purge(msdu_list);
		dev_kfree_skb_any(msdu);
	}
}
				 ...
				 struct napi_struct *napi, int budget)
{
	...
	while (budget) {
        ...
        ath11k_dbg(ab, ATH11K_DBG_DATA, 
            "msdu %p, err_rel_src %d, push_reason %d, err_code %d\n",
            msdu, err_info.err_rel_src, err_info.push_reason, err_info.err_code);

		if (err_info.push_reason !=
		    HAL_REO_DEST_RING_PUSH_REASON_ERR_DETECTED) {
			ab->soc_stats.hal_wbm_rel_route++;
			dev_kfree_skb_any(msdu);
			continue;
		}

		rxcb->err_rel_src = err_info.err_rel_src;
		rxcb->err_code = (u8)err_info.err_code;
...
void ath11k_dp_process_reo_status(struct ath11k_base *ab)
{
	...
	while ((reo_desc = ath11k_hal_srng_dst_get_next_entry(ab, srng)) != NULL) {
...
int ath11k_dp_rx_pdev_alloc(struct ath11k_base *ab, int mac_id)
{
	...
	for (i = 0; i < MAX_MACS_PER_PDEV; i++) {
		...
		ret = ath11k_dp_tx_htt_srng_setup(ab, ring_id, mac_id + i, HAL_RXDMA_DST);
		if (ret) {
...
static void ath11k_dp_mon_set_frag_len(struct ath11k_base *ab, u32 *total_len, u32 *frag_len)
{
	if (*total_len >= (DP_RX_BUFFER_SIZE - ab->hw_params.hal_desc_sz)) {
		*frag_len = (u32)(DP_RX_BUFFER_SIZE - ab->hw_params.hal_desc_sz);
		*total_len -= *frag_len;
	} else {
		*frag_len = *total_len;
		*total_len = 0;
					...
					u16 *num_msdus)
{
	...
	for (i = 0; i < HAL_RX_NUM_MSDU_DESC; i++) {
		...
		msdu_list->msdu_info[i].msdu_len =
			 (u16)HAL_RX_MSDU_PKT_LENGTH_GET(msdu_desc_info->info0);
		msdu_list->sw_cookie[i] =
			FIELD_GET(BUFFER_ADDR_INFO1_SW_COOKIE,
				  msdu_details[i].buf_addr_info.info1);
		tmp =(u8)FIELD_GET(BUFFER_ADDR_INFO1_RET_BUF_MGR,
				msdu_details[i].buf_addr_info.info1);
		msdu_list->rbm[i] = tmp;
	}
	*num_msdus = (u16)i;
...
static void ath11k_dp_mon_get_buf_len(struct ath11k_base *ab, 
					  		struct hal_rx_msdu_desc_info *info,
...

{
	if (info->msdu_flags & RX_MSDU_DESC_INFO0_MSDU_CONTINUATION) {
		...
		ath11k_dp_mon_set_frag_len(ab, total_len,
					   frag_len);
	} else {
		if (*is_frag) {
			ath11k_dp_mon_set_frag_len(ab, total_len,
			  ...
			  u32 *ppdu_id)
{
	...
	do {
		...
		if (ab->hw_params.rxdma1_enable)
			...
			rx_msdu_link_desc =
				(u8 *)ab->dp.link_desc_banks[sw_cookie].vaddr +
				(paddr - ab->dp.link_desc_banks[sw_cookie].paddr);
		...
		for (i = 0; i < num_msdus; i++) {
			...
			rxcb = ATH11K_SKB_RXCB(msdu);
			if (!rxcb->unmapped) {
				dma_unmap_single(ab->dev, rxcb->paddr, rxcb->paddr_mapping_addr,
						 msdu->len +
						 skb_tailroom(msdu),
						 DMA_FROM_DEVICE);
				rxcb->unmapped = 1;
			...
			rx_pkt_offset = ab->hw_params.hal_desc_sz;
			l2_hdr_offset = ath11k_dp_rx_h_msdu_end_l3pad(ab, rx_desc);

			if (is_first_msdu) {
				if (!ath11k_dp_rxdesc_mpdu_valid(ab, rx_desc)) {
					drop_mpdu = true;
					dev_kfree_skb_any(msdu);
					msdu = NULL;
				...
				msdu_ppdu_id = ath11k_dp_rxdesc_get_ppduid(ab, rx_desc);
			...
			ath11k_dp_mon_get_buf_len(ab, &msdu_list.msdu_info[i],
						  &is_frag, &total_len,
						  &frag_len, &msdu_cnt);
...
next_msdu:
		...
		if (ab->hw_params.rxdma1_enable) {
			...
			ath11k_dp_rx_link_desc_return(ab, rx_link_buf_info,
						      HAL_WBM_REL_BM_ACT_PUT_IN_IDLE);
		}
...
static void ath11k_dp_rx_msdus_set_payload(struct ath11k_base *ab, struct sk_buff *msdu)
{
	u32 rx_pkt_offset, l2_hdr_offset;

	rx_pkt_offset = ab->hw_params.hal_desc_sz;
	l2_hdr_offset = ath11k_dp_rx_h_msdu_end_l3pad(ab, msdu->data);
			    ...
			    struct ieee80211_rx_status *rxs)
{
	...
	if (ath11k_dp_rxdesc_get_mpdulen_err(ar->ab, rx_desc))
		return NULL;

	decap_format = ath11k_dp_rxdesc_get_decap_format(ar->ab, rx_desc);

	ath11k_dp_rx_h_ppdu(ar, rx_desc, rxs);

	if (decap_format == DP_RX_DECAP_TYPE_RAW) {
		ath11k_dp_rx_msdus_set_payload(ar->ab, head_msdu);

		prev_buf = head_msdu;
		msdu = head_msdu->next;

		while (msdu) {
			ath11k_dp_rx_msdus_set_payload(ar->ab, msdu);
		...
		rx_desc = head_msdu->data;
		hdr_desc = ath11k_dp_rxdesc_get_80211hdr(ar->ab, rx_desc);
...
static int ath11k_dp_rx_mon_deliver(struct ath11k *ar, u32 mac_id,
				    ...
				    struct napi_struct *napi)
{
    struct sk_buff *mon_skb, *skb_next;
	struct ath11k_pdev_dp *dp = &ar->dp;
	...
	do {
		...
		if (!skb_next)
			rxs->flag &= ~IEEE80211_RX_FLAG_AMSDU_MORE;
		else
			rxs->flag |= IEEE80211_RX_FLAG_AMSDU_MORE;
		...
		rxs->flag |= IEEE80211_RX_FLAG_ONLY_MONITOR;
