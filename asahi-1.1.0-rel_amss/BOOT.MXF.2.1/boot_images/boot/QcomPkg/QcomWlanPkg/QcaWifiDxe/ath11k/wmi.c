...
static const struct wmi_tlv_policy wmi_tlv_policies[] = {
    ...
    [WMI_TAG_PDEV_SET_HW_MODE_RESPONSE_EVENT]
        = {.min_len = sizeof(struct wmi_pdev_set_hw_mode_response_event) },
...
static const char* cmd_id_to_string(u32 cmd_id)
{
#define CASE_CMD(_cmd)     case _cmd: return(#_cmd);

    switch (cmd_id) {
        CASE_CMD(WMI_INIT_CMDID)
        CASE_CMD(WMI_START_SCAN_CMDID)
        CASE_CMD(WMI_STOP_SCAN_CMDID)
        CASE_CMD(WMI_SCAN_CHAN_LIST_CMDID)
        CASE_CMD(WMI_SCAN_SCH_PRIO_TBL_CMDID)
        CASE_CMD(WMI_SCAN_UPDATE_REQUEST_CMDID)
        CASE_CMD(WMI_SCAN_PROB_REQ_OUI_CMDID)
        CASE_CMD(WMI_SCAN_ADAPTIVE_DWELL_CONFIG_CMDID)
        CASE_CMD(WMI_PDEV_SET_REGDOMAIN_CMDID)
        CASE_CMD(WMI_PDEV_SET_CHANNEL_CMDID)
        CASE_CMD(WMI_PDEV_SET_PARAM_CMDID)
        CASE_CMD(WMI_PDEV_PKTLOG_ENABLE_CMDID)
        CASE_CMD(WMI_PDEV_PKTLOG_DISABLE_CMDID)
        CASE_CMD(WMI_PDEV_SET_WMM_PARAMS_CMDID)
        CASE_CMD(WMI_PDEV_SET_HT_CAP_IE_CMDID)
        CASE_CMD(WMI_PDEV_SET_VHT_CAP_IE_CMDID)
        CASE_CMD(WMI_PDEV_SET_DSCP_TID_MAP_CMDID)
        CASE_CMD(WMI_PDEV_SET_QUIET_MODE_CMDID)
        CASE_CMD(WMI_PDEV_GREEN_AP_PS_ENABLE_CMDID)
        CASE_CMD(WMI_PDEV_GET_TPC_CONFIG_CMDID)
        CASE_CMD(WMI_PDEV_SET_BASE_MACADDR_CMDID)
        CASE_CMD(WMI_PDEV_DUMP_CMDID)
        CASE_CMD(WMI_PDEV_SET_LED_CONFIG_CMDID)
        CASE_CMD(WMI_PDEV_GET_TEMPERATURE_CMDID)
        CASE_CMD(WMI_PDEV_SET_LED_FLASHING_CMDID)
        CASE_CMD(WMI_PDEV_SMART_ANT_ENABLE_CMDID)
        CASE_CMD(WMI_PDEV_SMART_ANT_SET_RX_ANTENNA_CMDID)
        CASE_CMD(WMI_PDEV_SET_ANTENNA_SWITCH_TABLE_CMDID)
        CASE_CMD(WMI_PDEV_SET_CTL_TABLE_CMDID)
        CASE_CMD(WMI_PDEV_SET_MIMOGAIN_TABLE_CMDID)
        CASE_CMD(WMI_PDEV_FIPS_CMDID)
        CASE_CMD(WMI_PDEV_GET_ANI_CCK_CONFIG_CMDID)
        CASE_CMD(WMI_PDEV_GET_ANI_OFDM_CONFIG_CMDID)
        CASE_CMD(WMI_PDEV_GET_NFCAL_POWER_CMDID)
        CASE_CMD(WMI_PDEV_GET_TPC_CMDID)
        CASE_CMD(WMI_MIB_STATS_ENABLE_CMDID)
        CASE_CMD(WMI_PDEV_SET_PCL_CMDID)
        CASE_CMD(WMI_PDEV_SET_HW_MODE_CMDID)
        CASE_CMD(WMI_PDEV_SET_MAC_CONFIG_CMDID)
        CASE_CMD(WMI_PDEV_SET_ANTENNA_MODE_CMDID)
        CASE_CMD(WMI_SET_PERIODIC_CHANNEL_STATS_CONFIG_CMDID)
        CASE_CMD(WMI_PDEV_WAL_POWER_DEBUG_CMDID)
        CASE_CMD(WMI_PDEV_SET_REORDER_TIMEOUT_VAL_CMDID)
        CASE_CMD(WMI_PDEV_SET_WAKEUP_CONFIG_CMDID)
        CASE_CMD(WMI_PDEV_GET_ANTDIV_STATUS_CMDID)
        CASE_CMD(WMI_PDEV_GET_CHIP_POWER_STATS_CMDID)
        CASE_CMD(WMI_PDEV_SET_STATS_THRESHOLD_CMDID)
        CASE_CMD(WMI_PDEV_MULTIPLE_VDEV_RESTART_REQUEST_CMDID)
        CASE_CMD(WMI_PDEV_UPDATE_PKT_ROUTING_CMDID)
        CASE_CMD(WMI_PDEV_CHECK_CAL_VERSION_CMDID)
        CASE_CMD(WMI_PDEV_SET_DIVERSITY_GAIN_CMDID)
        CASE_CMD(WMI_PDEV_DIV_GET_RSSI_ANTID_CMDID)
        CASE_CMD(WMI_PDEV_BSS_CHAN_INFO_REQUEST_CMDID)
        CASE_CMD(WMI_PDEV_UPDATE_PMK_CACHE_CMDID)
        CASE_CMD(WMI_PDEV_UPDATE_FILS_HLP_PKT_CMDID)
        CASE_CMD(WMI_PDEV_UPDATE_CTLTABLE_REQUEST_CMDID)
        CASE_CMD(WMI_PDEV_CONFIG_VENDOR_OUI_ACTION_CMDID)
        CASE_CMD(WMI_PDEV_SET_AC_TX_QUEUE_OPTIMIZED_CMDID)
        CASE_CMD(WMI_PDEV_SET_RX_FILTER_PROMISCUOUS_CMDID)
        CASE_CMD(WMI_PDEV_DMA_RING_CFG_REQ_CMDID)
        CASE_CMD(WMI_PDEV_HE_TB_ACTION_FRM_CMDID)
        CASE_CMD(WMI_VDEV_CREATE_CMDID)
        CASE_CMD(WMI_VDEV_DELETE_CMDID)
        CASE_CMD(WMI_VDEV_START_REQUEST_CMDID)
        CASE_CMD(WMI_VDEV_RESTART_REQUEST_CMDID)
        CASE_CMD(WMI_VDEV_UP_CMDID)
        CASE_CMD(WMI_VDEV_STOP_CMDID)
        CASE_CMD(WMI_VDEV_DOWN_CMDID)
        CASE_CMD(WMI_VDEV_SET_PARAM_CMDID)
        CASE_CMD(WMI_VDEV_INSTALL_KEY_CMDID)
        CASE_CMD(WMI_VDEV_WNM_SLEEPMODE_CMDID)
        CASE_CMD(WMI_VDEV_WMM_ADDTS_CMDID)
        CASE_CMD(WMI_VDEV_WMM_DELTS_CMDID)
        CASE_CMD(WMI_VDEV_SET_WMM_PARAMS_CMDID)
        CASE_CMD(WMI_VDEV_SET_GTX_PARAMS_CMDID)
        CASE_CMD(WMI_VDEV_IPSEC_NATKEEPALIVE_FILTER_CMDID)
        CASE_CMD(WMI_VDEV_PLMREQ_START_CMDID)
        CASE_CMD(WMI_VDEV_PLMREQ_STOP_CMDID)
        CASE_CMD(WMI_VDEV_TSF_TSTAMP_ACTION_CMDID)
        CASE_CMD(WMI_VDEV_SET_IE_CMDID)
        CASE_CMD(WMI_VDEV_RATEMASK_CMDID)
        CASE_CMD(WMI_VDEV_ATF_REQUEST_CMDID)
        CASE_CMD(WMI_VDEV_SET_DSCP_TID_MAP_CMDID)
        CASE_CMD(WMI_VDEV_FILTER_NEIGHBOR_RX_PACKETS_CMDID)
        CASE_CMD(WMI_VDEV_SET_QUIET_MODE_CMDID)
        CASE_CMD(WMI_VDEV_SET_CUSTOM_AGGR_SIZE_CMDID)
        CASE_CMD(WMI_VDEV_ENCRYPT_DECRYPT_DATA_REQ_CMDID)
        CASE_CMD(WMI_VDEV_ADD_MAC_ADDR_TO_RX_FILTER_CMDID)
        CASE_CMD(WMI_PEER_CREATE_CMDID)
        CASE_CMD(WMI_PEER_DELETE_CMDID)
        CASE_CMD(WMI_PEER_FLUSH_TIDS_CMDID)
        CASE_CMD(WMI_PEER_SET_PARAM_CMDID)
        CASE_CMD(WMI_PEER_ASSOC_CMDID)
        CASE_CMD(WMI_PEER_ADD_WDS_ENTRY_CMDID)
        CASE_CMD(WMI_PEER_REMOVE_WDS_ENTRY_CMDID)
        CASE_CMD(WMI_PEER_MCAST_GROUP_CMDID)
        CASE_CMD(WMI_PEER_INFO_REQ_CMDID)
        CASE_CMD(WMI_PEER_GET_ESTIMATED_LINKSPEED_CMDID)
        CASE_CMD(WMI_PEER_SET_RATE_REPORT_CONDITION_CMDID)
        CASE_CMD(WMI_PEER_UPDATE_WDS_ENTRY_CMDID)
        CASE_CMD(WMI_PEER_ADD_PROXY_STA_ENTRY_CMDID)
        CASE_CMD(WMI_PEER_SMART_ANT_SET_TX_ANTENNA_CMDID)
        CASE_CMD(WMI_PEER_SMART_ANT_SET_TRAIN_INFO_CMDID)
        CASE_CMD(WMI_PEER_SMART_ANT_SET_NODE_CONFIG_OPS_CMDID)
        CASE_CMD(WMI_PEER_ATF_REQUEST_CMDID)
        CASE_CMD(WMI_PEER_BWF_REQUEST_CMDID)
        CASE_CMD(WMI_PEER_REORDER_QUEUE_SETUP_CMDID)
        CASE_CMD(WMI_PEER_REORDER_QUEUE_REMOVE_CMDID)
        CASE_CMD(WMI_PEER_SET_RX_BLOCKSIZE_CMDID)
        CASE_CMD(WMI_PEER_ANTDIV_INFO_REQ_CMDID)
        CASE_CMD(WMI_BCN_TX_CMDID)
        CASE_CMD(WMI_BCN_TMPL_CMDID)
        CASE_CMD(WMI_BCN_FILTER_RX_CMDID)
        CASE_CMD(WMI_PRB_REQ_FILTER_RX_CMDID)
        CASE_CMD(WMI_MGMT_TX_CMDID)
        CASE_CMD(WMI_PRB_TMPL_CMDID)
        CASE_CMD(WMI_MGMT_TX_SEND_CMDID)
        CASE_CMD(WMI_OFFCHAN_DATA_TX_SEND_CMDID)
        CASE_CMD(WMI_PDEV_SEND_FD_CMDID)
        CASE_CMD(WMI_BCN_OFFLOAD_CTRL_CMDID)
        CASE_CMD(WMI_BSS_COLOR_CHANGE_ENABLE_CMDID)
        CASE_CMD(WMI_VDEV_BCN_OFFLOAD_QUIET_CONFIG_CMDID)
        CASE_CMD(WMI_ADDBA_CLEAR_RESP_CMDID)
        CASE_CMD(WMI_ADDBA_SEND_CMDID)
        CASE_CMD(WMI_ADDBA_STATUS_CMDID)
        CASE_CMD(WMI_DELBA_SEND_CMDID)
        CASE_CMD(WMI_ADDBA_SET_RESP_CMDID)
        CASE_CMD(WMI_SEND_SINGLEAMSDU_CMDID)
        CASE_CMD(WMI_STA_POWERSAVE_MODE_CMDID)
        CASE_CMD(WMI_STA_POWERSAVE_PARAM_CMDID)
        CASE_CMD(WMI_STA_MIMO_PS_MODE_CMDID)
        CASE_CMD(WMI_PDEV_DFS_ENABLE_CMDID)
        CASE_CMD(WMI_PDEV_DFS_DISABLE_CMDID)
        CASE_CMD(WMI_DFS_PHYERR_FILTER_ENA_CMDID)
        CASE_CMD(WMI_DFS_PHYERR_FILTER_DIS_CMDID)
        CASE_CMD(WMI_PDEV_DFS_PHYERR_OFFLOAD_ENABLE_CMDID)
        CASE_CMD(WMI_PDEV_DFS_PHYERR_OFFLOAD_DISABLE_CMDID)
        CASE_CMD(WMI_VDEV_ADFS_CH_CFG_CMDID)
        CASE_CMD(WMI_VDEV_ADFS_OCAC_ABORT_CMDID)
        CASE_CMD(WMI_ROAM_SCAN_MODE)
        CASE_CMD(WMI_ROAM_SCAN_RSSI_THRESHOLD)
        CASE_CMD(WMI_ROAM_SCAN_PERIOD)
        CASE_CMD(WMI_ROAM_SCAN_RSSI_CHANGE_THRESHOLD)
        CASE_CMD(WMI_ROAM_AP_PROFILE)
        CASE_CMD(WMI_ROAM_CHAN_LIST)
        CASE_CMD(WMI_ROAM_SCAN_CMD)
        CASE_CMD(WMI_ROAM_SYNCH_COMPLETE)
        CASE_CMD(WMI_ROAM_SET_RIC_REQUEST_CMDID)
        CASE_CMD(WMI_ROAM_INVOKE_CMDID)
        CASE_CMD(WMI_ROAM_FILTER_CMDID)
        CASE_CMD(WMI_OFL_SCAN_REMOVE_AP_PROFILE)
        CASE_CMD(WMI_OFL_SCAN_PERIOD)
        CASE_CMD(WMI_P2P_DEV_SET_DEVICE_INFO)
        CASE_CMD(WMI_P2P_DEV_SET_DISCOVERABILITY)
        CASE_CMD(WMI_P2P_GO_SET_BEACON_IE)
        CASE_CMD(WMI_P2P_GO_SET_PROBE_RESP_IE)
        CASE_CMD(WMI_P2P_SET_VENDOR_IE_DATA_CMDID)
        CASE_CMD(WMI_P2P_DISC_OFFLOAD_CONFIG_CMDID)
        CASE_CMD(WMI_P2P_DISC_OFFLOAD_APPIE_CMDID)
        CASE_CMD(WMI_P2P_DISC_OFFLOAD_PATTERN_CMDID)
        CASE_CMD(WMI_P2P_SET_OPPPS_PARAM_CMDID)
        CASE_CMD(WMI_P2P_LISTEN_OFFLOAD_START_CMDID)
        CASE_CMD(WMI_P2P_LISTEN_OFFLOAD_STOP_CMDID)
        CASE_CMD(WMI_AP_PS_PEER_PARAM_CMDID)
        CASE_CMD(WMI_AP_PS_PEER_UAPSD_COEX_CMDID)
        CASE_CMD(WMI_AP_PS_EGAP_PARAM_CMDID)
        CASE_CMD(WMI_PEER_RATE_RETRY_SCHED_CMDID)
        CASE_CMD(WMI_WLAN_PROFILE_TRIGGER_CMDID)
        CASE_CMD(WMI_WLAN_PROFILE_SET_HIST_INTVL_CMDID)
        CASE_CMD(WMI_WLAN_PROFILE_GET_PROFILE_DATA_CMDID)
        CASE_CMD(WMI_WLAN_PROFILE_ENABLE_PROFILE_ID_CMDID)
        CASE_CMD(WMI_WLAN_PROFILE_LIST_PROFILE_ID_CMDID)
        CASE_CMD(WMI_PDEV_SUSPEND_CMDID)
        CASE_CMD(WMI_PDEV_RESUME_CMDID)
        CASE_CMD(WMI_ADD_BCN_FILTER_CMDID)
        CASE_CMD(WMI_RMV_BCN_FILTER_CMDID)
        CASE_CMD(WMI_WOW_ADD_WAKE_PATTERN_CMDID)
        CASE_CMD(WMI_WOW_DEL_WAKE_PATTERN_CMDID)
        CASE_CMD(WMI_WOW_ENABLE_DISABLE_WAKE_EVENT_CMDID)
        CASE_CMD(WMI_WOW_ENABLE_CMDID)
        CASE_CMD(WMI_WOW_HOSTWAKEUP_FROM_SLEEP_CMDID)
        CASE_CMD(WMI_WOW_IOAC_ADD_KEEPALIVE_CMDID)
        CASE_CMD(WMI_WOW_IOAC_DEL_KEEPALIVE_CMDID)
        CASE_CMD(WMI_WOW_IOAC_ADD_WAKE_PATTERN_CMDID)
        CASE_CMD(WMI_WOW_IOAC_DEL_WAKE_PATTERN_CMDID)
        CASE_CMD(WMI_D0_WOW_ENABLE_DISABLE_CMDID)
        CASE_CMD(WMI_EXTWOW_ENABLE_CMDID)
        CASE_CMD(WMI_EXTWOW_SET_APP_TYPE1_PARAMS_CMDID)
        CASE_CMD(WMI_EXTWOW_SET_APP_TYPE2_PARAMS_CMDID)
        CASE_CMD(WMI_WOW_ENABLE_ICMPV6_NA_FLT_CMDID)
        CASE_CMD(WMI_WOW_UDP_SVC_OFLD_CMDID)
        CASE_CMD(WMI_WOW_HOSTWAKEUP_GPIO_PIN_PATTERN_CONFIG_CMDID)
        CASE_CMD(WMI_WOW_SET_ACTION_WAKE_UP_CMDID)
        CASE_CMD(WMI_RTT_MEASREQ_CMDID)
        CASE_CMD(WMI_RTT_TSF_CMDID)
        CASE_CMD(WMI_VDEV_SPECTRAL_SCAN_CONFIGURE_CMDID)
        CASE_CMD(WMI_VDEV_SPECTRAL_SCAN_ENABLE_CMDID)
        CASE_CMD(WMI_REQUEST_STATS_CMDID)
        CASE_CMD(WMI_MCC_SCHED_TRAFFIC_STATS_CMDID)
        CASE_CMD(WMI_REQUEST_STATS_EXT_CMDID)
        CASE_CMD(WMI_REQUEST_LINK_STATS_CMDID)
        CASE_CMD(WMI_START_LINK_STATS_CMDID)
        CASE_CMD(WMI_CLEAR_LINK_STATS_CMDID)
        CASE_CMD(WMI_GET_FW_MEM_DUMP_CMDID)
        CASE_CMD(WMI_DEBUG_MESG_FLUSH_CMDID)
        CASE_CMD(WMI_DIAG_EVENT_LOG_CONFIG_CMDID)
        CASE_CMD(WMI_REQUEST_WLAN_STATS_CMDID)
        CASE_CMD(WMI_REQUEST_RCPI_CMDID)
        CASE_CMD(WMI_REQUEST_PEER_STATS_INFO_CMDID)
        CASE_CMD(WMI_REQUEST_RADIO_CHAN_STATS_CMDID)
        CASE_CMD(WMI_SET_ARP_NS_OFFLOAD_CMDID)
        CASE_CMD(WMI_ADD_PROACTIVE_ARP_RSP_PATTERN_CMDID)
        CASE_CMD(WMI_DEL_PROACTIVE_ARP_RSP_PATTERN_CMDID)
        CASE_CMD(WMI_NETWORK_LIST_OFFLOAD_CONFIG_CMDID)
        CASE_CMD(WMI_APFIND_CMDID)
        CASE_CMD(WMI_PASSPOINT_LIST_CONFIG_CMDID)
        CASE_CMD(WMI_NLO_CONFIGURE_MAWC_CMDID)
        CASE_CMD(WMI_GTK_OFFLOAD_CMDID)
        CASE_CMD(WMI_CSA_OFFLOAD_ENABLE_CMDID)
        CASE_CMD(WMI_CSA_OFFLOAD_CHANSWITCH_CMDID)
        CASE_CMD(WMI_CHATTER_SET_MODE_CMDID)
        CASE_CMD(WMI_CHATTER_ADD_COALESCING_FILTER_CMDID)
        CASE_CMD(WMI_CHATTER_DELETE_COALESCING_FILTER_CMDID)
        CASE_CMD(WMI_CHATTER_COALESCING_QUERY_CMDID)
        CASE_CMD(WMI_PEER_TID_ADDBA_CMDID)
        CASE_CMD(WMI_PEER_TID_DELBA_CMDID)
        CASE_CMD(WMI_STA_DTIM_PS_METHOD_CMDID)
        CASE_CMD(WMI_STA_UAPSD_AUTO_TRIG_CMDID)
        CASE_CMD(WMI_STA_KEEPALIVE_CMDID)
        CASE_CMD(WMI_BA_REQ_SSN_CMDID)
        CASE_CMD(WMI_ECHO_CMDID)

        CASE_CMD(WMI_PDEV_UTF_CMDID)

        CASE_CMD(WMI_DBGLOG_CFG_CMDID)
        CASE_CMD(WMI_PDEV_QVIT_CMDID)
        CASE_CMD(WMI_PDEV_FTM_INTG_CMDID)
        CASE_CMD(WMI_VDEV_SET_KEEPALIVE_CMDID)
        CASE_CMD(WMI_VDEV_GET_KEEPALIVE_CMDID)
        CASE_CMD(WMI_FORCE_FW_HANG_CMDID)
        CASE_CMD(WMI_SET_MCASTBCAST_FILTER_CMDID)
        CASE_CMD(WMI_THERMAL_MGMT_CMDID)
        CASE_CMD(WMI_HOST_AUTO_SHUTDOWN_CFG_CMDID)
        CASE_CMD(WMI_TPC_CHAINMASK_CONFIG_CMDID)
        CASE_CMD(WMI_SET_ANTENNA_DIVERSITY_CMDID)
        CASE_CMD(WMI_OCB_SET_SCHED_CMDID)
        CASE_CMD(WMI_RSSI_BREACH_MONITOR_CONFIG_CMDID)
        CASE_CMD(WMI_LRO_CONFIG_CMDID)
        CASE_CMD(WMI_TRANSFER_DATA_TO_FLASH_CMDID)
        CASE_CMD(WMI_CONFIG_ENHANCED_MCAST_FILTER_CMDID)
        CASE_CMD(WMI_VDEV_WISA_CMDID)
        CASE_CMD(WMI_DBGLOG_TIME_STAMP_SYNC_CMDID)
        CASE_CMD(WMI_SET_MULTIPLE_MCAST_FILTER_CMDID)

        CASE_CMD(WMI_GPIO_CONFIG_CMDID)
        CASE_CMD(WMI_GPIO_OUTPUT_CMDID)
        CASE_CMD(WMI_TXBF_CMDID)
        CASE_CMD(WMI_UNIT_TEST_CMDID)
        CASE_CMD(WMI_FWTEST_CMDID)
        CASE_CMD(WMI_STA_SMPS_FORCE_MODE_CMDID)
        CASE_CMD(WMI_STA_SMPS_PARAM_CMDID)
        CASE_CMD(WMI_RMC_SET_MODE_CMDID)
        CASE_CMD(WMI_RMC_SET_ACTION_PERIOD_CMDID)
        CASE_CMD(WMI_RMC_CONFIG_CMDID)
        CASE_CMD(WMI_RMC_SET_MANUAL_LEADER_CMDID)
        CASE_CMD(WMI_MHF_OFFLOAD_SET_MODE_CMDID)
        CASE_CMD(WMI_MHF_OFFLOAD_PLUMB_ROUTING_TBL_CMDID)
        CASE_CMD(WMI_BATCH_SCAN_ENABLE_CMDID)
        CASE_CMD(WMI_BATCH_SCAN_DISABLE_CMDID)
        CASE_CMD(WMI_BATCH_SCAN_TRIGGER_RESULT_CMDID)
        CASE_CMD(WMI_OEM_REQ_CMDID)
        CASE_CMD(WMI_OEM_REQUEST_CMDID)
        CASE_CMD(WMI_LPI_OEM_REQ_CMDID)
        CASE_CMD(WMI_CHAN_AVOID_UPDATE_CMDID)
        CASE_CMD(WMI_COEX_CONFIG_CMDID)
        CASE_CMD(WMI_CHAN_AVOID_RPT_ALLOW_CMDID)
        CASE_CMD(WMI_COEX_GET_ANTENNA_ISOLATION_CMDID)
        CASE_CMD(WMI_SAR_LIMITS_CMDID)

        CASE_CMD(WMI_OBSS_SCAN_ENABLE_CMDID)
        CASE_CMD(WMI_OBSS_SCAN_DISABLE_CMDID)
        CASE_CMD(WMI_LPI_MGMT_SNOOPING_CONFIG_CMDID)
        CASE_CMD(WMI_LPI_START_SCAN_CMDID)
        CASE_CMD(WMI_LPI_STOP_SCAN_CMDID)
        CASE_CMD(WMI_EXTSCAN_START_CMDID)
        CASE_CMD(WMI_EXTSCAN_STOP_CMDID)
        CASE_CMD(WMI_EXTSCAN_CONFIGURE_WLAN_CHANGE_MONITOR_CMDID)
        CASE_CMD(WMI_EXTSCAN_CONFIGURE_HOTLIST_MONITOR_CMDID)
        CASE_CMD(WMI_EXTSCAN_GET_CACHED_RESULTS_CMDID)
        CASE_CMD(WMI_EXTSCAN_GET_WLAN_CHANGE_RESULTS_CMDID)
        CASE_CMD(WMI_EXTSCAN_SET_CAPABILITIES_CMDID)
        CASE_CMD(WMI_EXTSCAN_GET_CAPABILITIES_CMDID)
        CASE_CMD(WMI_EXTSCAN_CONFIGURE_HOTLIST_SSID_MONITOR_CMDID)
        CASE_CMD(WMI_EXTSCAN_CONFIGURE_MAWC_CMDID)
        CASE_CMD(WMI_SET_DHCP_SERVER_OFFLOAD_CMDID)
        CASE_CMD(WMI_MDNS_OFFLOAD_ENABLE_CMDID)
        CASE_CMD(WMI_MDNS_SET_FQDN_CMDID)
        CASE_CMD(WMI_MDNS_SET_RESPONSE_CMDID)
        CASE_CMD(WMI_MDNS_GET_STATS_CMDID)
        CASE_CMD(WMI_SAP_OFL_ENABLE_CMDID)
        CASE_CMD(WMI_SAP_SET_BLACKLIST_PARAM_CMDID)
        CASE_CMD(WMI_OCB_SET_CONFIG_CMDID)
        CASE_CMD(WMI_OCB_SET_UTC_TIME_CMDID)
        CASE_CMD(WMI_OCB_START_TIMING_ADVERT_CMDID)
        CASE_CMD(WMI_OCB_STOP_TIMING_ADVERT_CMDID)
        CASE_CMD(WMI_OCB_GET_TSF_TIMER_CMDID)
        CASE_CMD(WMI_DCC_GET_STATS_CMDID)
        CASE_CMD(WMI_DCC_CLEAR_STATS_CMDID)
        CASE_CMD(WMI_DCC_UPDATE_NDL_CMDID)
        CASE_CMD(WMI_SOC_SET_PCL_CMDID)
        CASE_CMD(WMI_SOC_SET_HW_MODE_CMDID)
        CASE_CMD(WMI_SOC_SET_DUAL_MAC_CONFIG_CMDID)
        CASE_CMD(WMI_SOC_SET_ANTENNA_MODE_CMDID)
        CASE_CMD(WMI_PACKET_FILTER_CONFIG_CMDID)
        CASE_CMD(WMI_PACKET_FILTER_ENABLE_CMDID)
        CASE_CMD(WMI_MAWC_SENSOR_REPORT_IND_CMDID)
        CASE_CMD(WMI_MNT_FILTER_CMDID)
        CASE_CMD(WMI_SET_CURRENT_COUNTRY_CMDID)
        CASE_CMD(WMI_11D_SCAN_START_CMDID)
        CASE_CMD(WMI_11D_SCAN_STOP_CMDID)
        CASE_CMD(WMI_SET_INIT_COUNTRY_CMDID)
        CASE_CMD(WMI_HW_DATA_FILTER_CMDID)
        CASE_CMD(WMI_TWT_ENABLE_CMDID)
        CASE_CMD(WMI_TWT_DISABLE_CMDID)
        CASE_CMD(WMI_TWT_ADD_DIALOG_CMDID)
        CASE_CMD(WMI_TWT_DEL_DIALOG_CMDID)
        CASE_CMD(WMI_TWT_PAUSE_DIALOG_CMDID)
        CASE_CMD(WMI_TWT_RESUME_DIALOG_CMDID)

        CASE_CMD(WMI_PDEV_OBSS_PD_SPATIAL_REUSE_CMDID)
        CASE_CMD(WMI_PDEV_OBSS_PD_SPATIAL_REUSE_SET_DEF_OBSS_THRESH_CMDID)

    default:
        return "UNKNOWN_WMI_CMDID";
    }

#undef CASE_CMD
}

static const char* evt_id_to_string(u32 evt_id)
{
#define CASE_EVENT(_event)     case _event: return(#_event);

    switch (evt_id) {
        CASE_EVENT(WMI_SERVICE_READY_EVENTID)
        CASE_EVENT(WMI_READY_EVENTID)
        CASE_EVENT(WMI_SERVICE_AVAILABLE_EVENTID)
        CASE_EVENT(WMI_SCAN_EVENTID)
        CASE_EVENT(WMI_PDEV_TPC_CONFIG_EVENTID)
        CASE_EVENT(WMI_CHAN_INFO_EVENTID)
        CASE_EVENT(WMI_PHYERR_EVENTID)
        CASE_EVENT(WMI_PDEV_DUMP_EVENTID)
        CASE_EVENT(WMI_TX_PAUSE_EVENTID)
        CASE_EVENT(WMI_DFS_RADAR_EVENTID)
        CASE_EVENT(WMI_PDEV_L1SS_TRACK_EVENTID)
        CASE_EVENT(WMI_PDEV_TEMPERATURE_EVENTID)
        CASE_EVENT(WMI_SERVICE_READY_EXT_EVENTID)
        CASE_EVENT(WMI_PDEV_FIPS_EVENTID)
        CASE_EVENT(WMI_PDEV_CHANNEL_HOPPING_EVENTID)
        CASE_EVENT(WMI_PDEV_ANI_CCK_LEVEL_EVENTID)
        CASE_EVENT(WMI_PDEV_ANI_OFDM_LEVEL_EVENTID)
        CASE_EVENT(WMI_PDEV_TPC_EVENTID)
        CASE_EVENT(WMI_PDEV_NFCAL_POWER_ALL_CHANNELS_EVENTID)
        CASE_EVENT(WMI_PDEV_SET_HW_MODE_RESP_EVENTID)
        CASE_EVENT(WMI_PDEV_HW_MODE_TRANSITION_EVENTID)
        CASE_EVENT(WMI_PDEV_SET_MAC_CONFIG_RESP_EVENTID)
        CASE_EVENT(WMI_PDEV_ANTDIV_STATUS_EVENTID)
        CASE_EVENT(WMI_PDEV_CHIP_POWER_STATS_EVENTID)
        CASE_EVENT(WMI_PDEV_CHIP_POWER_SAVE_FAILURE_DETECTED_EVENTID)
        CASE_EVENT(WMI_PDEV_CSA_SWITCH_COUNT_STATUS_EVENTID)
        CASE_EVENT(WMI_PDEV_CHECK_CAL_VERSION_EVENTID)
        CASE_EVENT(WMI_PDEV_DIV_RSSI_ANTID_EVENTID)
        CASE_EVENT(WMI_PDEV_BSS_CHAN_INFO_EVENTID)
        CASE_EVENT(WMI_PDEV_UPDATE_CTLTABLE_EVENTID)
        CASE_EVENT(WMI_PDEV_DMA_RING_CFG_RSP_EVENTID)
        CASE_EVENT(WMI_PDEV_DMA_RING_BUF_RELEASE_EVENTID)
        CASE_EVENT(WMI_PDEV_CTL_FAILSAFE_CHECK_EVENTID)
        CASE_EVENT(WMI_VDEV_START_RESP_EVENTID)
        CASE_EVENT(WMI_VDEV_STOPPED_EVENTID)
        CASE_EVENT(WMI_VDEV_INSTALL_KEY_COMPLETE_EVENTID)
        CASE_EVENT(WMI_VDEV_MCC_BCN_INTERVAL_CHANGE_REQ_EVENTID)
        CASE_EVENT(WMI_VDEV_TSF_REPORT_EVENTID)
        CASE_EVENT(WMI_VDEV_DELETE_RESP_EVENTID)
        CASE_EVENT(WMI_VDEV_ENCRYPT_DECRYPT_DATA_RESP_EVENTID)
        CASE_EVENT(WMI_PEER_STA_KICKOUT_EVENTID)
        CASE_EVENT(WMI_PEER_INFO_EVENTID)
        CASE_EVENT(WMI_PEER_TX_FAIL_CNT_THR_EVENTID)
        CASE_EVENT(WMI_PEER_ESTIMATED_LINKSPEED_EVENTID)
        CASE_EVENT(WMI_PEER_STATE_EVENTID)
        CASE_EVENT(WMI_PEER_ASSOC_CONF_EVENTID)
        CASE_EVENT(WMI_PEER_DELETE_RESP_EVENTID)
        CASE_EVENT(WMI_PEER_RATECODE_LIST_EVENTID)
        CASE_EVENT(WMI_WDS_PEER_EVENTID)
        CASE_EVENT(WMI_PEER_STA_PS_STATECHG_EVENTID)
        CASE_EVENT(WMI_PEER_ANTDIV_INFO_EVENTID)
        CASE_EVENT(WMI_PEER_RESERVED0_EVENTID)
        CASE_EVENT(WMI_PEER_RESERVED1_EVENTID)
        CASE_EVENT(WMI_PEER_RESERVED2_EVENTID)
        CASE_EVENT(WMI_PEER_RESERVED3_EVENTID)
        CASE_EVENT(WMI_PEER_RESERVED4_EVENTID)
        CASE_EVENT(WMI_PEER_RESERVED5_EVENTID)
        CASE_EVENT(WMI_PEER_RESERVED6_EVENTID)
        CASE_EVENT(WMI_PEER_RESERVED7_EVENTID)
        CASE_EVENT(WMI_PEER_RESERVED8_EVENTID)
        CASE_EVENT(WMI_PEER_RESERVED9_EVENTID)
        CASE_EVENT(WMI_PEER_RESERVED10_EVENTID)
        CASE_EVENT(WMI_PEER_OPER_MODE_CHANGE_EVENTID)
        CASE_EVENT(WMI_MGMT_RX_EVENTID)
        CASE_EVENT(WMI_HOST_SWBA_EVENTID)
        CASE_EVENT(WMI_TBTTOFFSET_UPDATE_EVENTID)
        CASE_EVENT(WMI_OFFLOAD_BCN_TX_STATUS_EVENTID)
        CASE_EVENT(WMI_OFFLOAD_PROB_RESP_TX_STATUS_EVENTID)
        CASE_EVENT(WMI_MGMT_TX_COMPLETION_EVENTID)
        CASE_EVENT(WMI_MGMT_TX_BUNDLE_COMPLETION_EVENTID)
        CASE_EVENT(WMI_TBTTOFFSET_EXT_UPDATE_EVENTID)
        CASE_EVENT(WMI_TX_DELBA_COMPLETE_EVENTID)
        CASE_EVENT(WMI_TX_ADDBA_COMPLETE_EVENTID)
        CASE_EVENT(WMI_BA_RSP_SSN_EVENTID)
        CASE_EVENT(WMI_AGGR_STATE_TRIG_EVENTID)
        CASE_EVENT(WMI_ROAM_EVENTID)
        CASE_EVENT(WMI_AP_PS_EGAP_INFO_EVENTID)
        CASE_EVENT(WMI_PDEV_RESUME_EVENTID)
        CASE_EVENT(WMI_WOW_WAKEUP_HOST_EVENTID)
        CASE_EVENT(WMI_D0_WOW_DISABLE_ACK_EVENTID)
        CASE_EVENT(WMI_WOW_INITIAL_WAKEUP_EVENTID)
        CASE_EVENT(WMI_RTT_MEASUREMENT_REPORT_EVENTID)
        CASE_EVENT(WMI_TSF_MEASUREMENT_REPORT_EVENTID)
        CASE_EVENT(WMI_RTT_ERROR_REPORT_EVENTID)
        CASE_EVENT(WMI_STATS_EXT_EVENTID)
        CASE_EVENT(WMI_IFACE_LINK_STATS_EVENTID)
        CASE_EVENT(WMI_PEER_LINK_STATS_EVENTID)
        CASE_EVENT(WMI_RADIO_LINK_STATS_EVENTID)
        CASE_EVENT(WMI_UPDATE_FW_MEM_DUMP_EVENTID)
        CASE_EVENT(WMI_DIAG_EVENT_LOG_SUPPORTED_EVENTID)
        CASE_EVENT(WMI_INST_RSSI_STATS_EVENTID)
        CASE_EVENT(WMI_RADIO_TX_POWER_LEVEL_STATS_EVENTID)
        CASE_EVENT(WMI_REPORT_STATS_EVENTID)
        CASE_EVENT(WMI_UPDATE_RCPI_EVENTID)
        CASE_EVENT(WMI_PEER_STATS_INFO_EVENTID)
        CASE_EVENT(WMI_RADIO_CHAN_STATS_EVENTID)
        CASE_EVENT(WMI_NLO_MATCH_EVENTID)
        CASE_EVENT(WMI_NLO_SCAN_COMPLETE_EVENTID)
        CASE_EVENT(WMI_APFIND_EVENTID)
        CASE_EVENT(WMI_PASSPOINT_MATCH_EVENTID)
        CASE_EVENT(WMI_GTK_OFFLOAD_STATUS_EVENTID)
        CASE_EVENT(WMI_GTK_REKEY_FAIL_EVENTID)
        CASE_EVENT(WMI_CSA_HANDLING_EVENTID)
        CASE_EVENT(WMI_CHATTER_PC_QUERY_EVENTID)
        CASE_EVENT(WMI_PDEV_DFS_RADAR_DETECTION_EVENTID)
        CASE_EVENT(WMI_VDEV_DFS_CAC_COMPLETE_EVENTID)
        CASE_EVENT(WMI_VDEV_ADFS_OCAC_COMPLETE_EVENTID)
        CASE_EVENT(WMI_ECHO_EVENTID)

        CASE_EVENT(WMI_PDEV_UTF_EVENTID)

        CASE_EVENT(WMI_DEBUG_MESG_EVENTID)
        CASE_EVENT(WMI_UPDATE_STATS_EVENTID)
        CASE_EVENT(WMI_DEBUG_PRINT_EVENTID)
        CASE_EVENT(WMI_DCS_INTERFERENCE_EVENTID)
        CASE_EVENT(WMI_PDEV_QVIT_EVENTID)
        CASE_EVENT(WMI_WLAN_PROFILE_DATA_EVENTID)
        CASE_EVENT(WMI_PDEV_FTM_INTG_EVENTID)
        CASE_EVENT(WMI_WLAN_FREQ_AVOID_EVENTID)
        CASE_EVENT(WMI_VDEV_GET_KEEPALIVE_EVENTID)
        CASE_EVENT(WMI_THERMAL_MGMT_EVENTID)
        CASE_EVENT(WMI_DIAG_DATA_CONTAINER_EVENTID)
        CASE_EVENT(WMI_HOST_AUTO_SHUTDOWN_EVENTID)
        CASE_EVENT(WMI_UPDATE_WHAL_MIB_STATS_EVENTID)
        CASE_EVENT(WMI_UPDATE_VDEV_RATE_STATS_EVENTID)
        CASE_EVENT(WMI_DIAG_EVENTID)
        CASE_EVENT(WMI_OCB_SET_SCHED_EVENTID)
        CASE_EVENT(WMI_DEBUG_MESG_FLUSH_COMPLETE_EVENTID)
        CASE_EVENT(WMI_RSSI_BREACH_EVENTID)
        CASE_EVENT(WMI_TRANSFER_DATA_TO_FLASH_COMPLETE_EVENTID)
        CASE_EVENT(WMI_PDEV_UTF_SCPC_EVENTID)
        CASE_EVENT(WMI_READ_DATA_FROM_FLASH_EVENTID)
        CASE_EVENT(WMI_REPORT_RX_AGGR_FAILURE_EVENTID)
        CASE_EVENT(WMI_UPLOADH_EVENTID)
        CASE_EVENT(WMI_CAPTUREH_EVENTID)
        CASE_EVENT(WMI_RFKILL_STATE_CHANGE_EVENTID)
        CASE_EVENT(WMI_SOC_SET_HW_MODE_RESP_EVENTID)
        CASE_EVENT(WMI_SOC_HW_MODE_TRANSITION_EVENTID)
        CASE_EVENT(WMI_SOC_SET_DUAL_MAC_CONFIG_RESP_EVENTID)
        CASE_EVENT(WMI_REG_CHAN_LIST_CC_EVENTID)
        CASE_EVENT(WMI_11D_NEW_COUNTRY_EVENTID)

    default:
        return "UNKNOWN_WMI_EVENTID";
    }
		    ...
		    void *data)
{
	...
	DEBUG_PARAMETER(begin);

	while (len > 0) {
		if (len < sizeof(*tlv)) {
			ath11k_err(ab, "wmi tlv parse failure at byte %zd (%zu bytes left, %zu expected)\n",
				   (u8*)ptr - (u8*)begin, len, sizeof(*tlv));
			return -EINVAL;
		}

		tlv = ptr;
		tlv_tag = (u16)FIELD_GET(WMI_TLV_TAG, tlv->header);
		tlv_len = (u16)FIELD_GET(WMI_TLV_LEN, tlv->header);
		ptr = (u8*)ptr + sizeof(*tlv);
		len -= sizeof(*tlv);

		if (tlv_len > len) {
			ath11k_err(ab, "wmi tlv parse failure of tag %hhu at byte %zd (%zu bytes left, %hhu expected)\n",
				   tlv_tag, (u8*)ptr - (u8*)begin, len, tlv_len);
			return -EINVAL;
		}
		...
		if (tlv_tag < ARRAY_SIZE(wmi_tlv_policies) &&
			...
			ath11k_err(ab, "wmi tlv parse failure of tag %hhu at byte %zd (%hhu bytes is less than min length %zu)\n",
				   tlv_tag, (u8*)ptr - (u8*)begin, tlv_len,
				   wmi_tlv_policies[tlv_tag].min_len);
			return -EINVAL;
		}
			   ...
			   size_t len, gfp_t gfp)
{
	...
	ret = ath11k_wmi_tlv_parse(ab, (const void **)tb, ptr, len);
	if (ret) {
		kfree(tb);
		return ERR_PTR(ret);
	}
			...
			u32 cmd_id)
{
	...
	//might_sleep();
	ath11k_info(wmi_sc->ab, "skb 0x%p, cmd %a (0x%08x)\n",
        skb, cmd_id_to_string(cmd_id), cmd_id);
	
	wait_event_timeout(wmi_sc->tx_credits_wq, 
        ((ret = ath11k_wmi_cmd_send_nowait(wmi, skb, cmd_id)) != -EAGAIN), 
        WMI_SEND_TIMEOUT_HZ, time_left);
...
static int ath11k_service_ready_event(struct ath11k_base *ab, struct sk_buff *skb)
{
    ...
    memset(&svc_ready, 0, sizeof(struct wmi_tlv_svc_ready_parse));
...
int ath11k_wmi_vdev_create(struct ath11k *ar, const u8 *macaddr,
			   struct vdev_create_params *param)
{
	...
	ether_addr_copy(cmd->vdev_macaddr.u.addr, macaddr);
	...
	ath11k_dbg(ar->ab, ATH11K_DBG_WMI,
		   "WMI vdev create: id %d type %d subtype %d macaddr: %02X:%02X;%02X:%02X:%02X;%02X pdevid %d\n",
		   param->if_id, param->type, param->subtype,
		   macaddr[0], macaddr[1],macaddr[2],macaddr[3],macaddr[4],macaddr[5], param->pdev_id);

	return ret;
}

int ath11k_wmi_vdev_delete(struct ath11k *ar, u32 vdev_id)
{
	struct ath11k_pdev_wmi *wmi = ar->wmi;
...
int ath11k_wmi_vdev_stop(struct ath11k *ar, u32 vdev_id)
{
	struct ath11k_pdev_wmi *wmi = ar->wmi;
...
int ath11k_wmi_vdev_down(struct ath11k *ar, u32 vdev_id)
{
	struct ath11k_pdev_wmi *wmi = ar->wmi;
			  ...
			  bool restart)
{
	...
	if (arg->ssid_len > sizeof(cmd->ssid.ssid)) {
		WARN_ON(arg->ssid_len > sizeof(cmd->ssid.ssid));
		return -EINVAL;
	}
...
int ath11k_wmi_vdev_up(struct ath11k *ar, u32 vdev_id, u32 aid, const u8 *bssid)
{
	...
	ether_addr_copy(cmd->vdev_bssid.u.addr, bssid);
				    ...
				    struct peer_create_params *param)
{
	...
	ether_addr_copy(cmd->peer_macaddr.u.addr, param->peer_addr);
...
int ath11k_wmi_send_peer_delete_cmd(struct ath11k *ar,
				    const u8 *peer_addr, u32 vdev_id)
{
	struct ath11k_pdev_wmi *wmi = ar->wmi;
	...
	ether_addr_copy(cmd->peer_macaddr.u.addr, peer_addr);
	cmd->vdev_id = vdev_id;
					   ...
					   u32 ba_window_size)
{
	...
	ether_addr_copy(cmd->peer_macaddr.u.addr, addr);
	cmd->vdev_id = vdev_id;
	cmd->tid = tid;
				 ...
				 struct rx_reorder_queue_remove_params *param)
{
	...
	ether_addr_copy(cmd->peer_macaddr.u.addr, param->peer_macaddr);
	cmd->vdev_id = param->vdev_id;
...
int ath11k_wmi_send_set_ap_ps_param_cmd(struct ath11k *ar, const u8 *peer_addr,
				...
				u32 param, u32 param_value)
{
	...
	if (ret) {
		ath11k_warn(ar->ab, "failed to send WMI_STA_POWERSAVE_PARAM_CMDID: %d\n", ret);
		dev_kfree_skb(skb);
	}
			...
			struct sk_buff *bcn)
{
	...
	len = sizeof(*cmd) + sizeof(*bcn_prb_info) + TLV_HDR_SIZE + (int)aligned_len;
	...
	ptr = (u8*)ptr + sizeof(*bcn_prb_info);
				...
				struct wmi_vdev_install_key_arg *arg)
{
	...
	cmd->tlv_header = FIELD_PREP(WMI_TLV_TAG, WMI_TAG_VDEV_INSTALL_KEY_CMD) |
			  FIELD_PREP(WMI_TLV_LEN, sizeof(*cmd) - TLV_HDR_SIZE);
	cmd->vdev_id = arg->vdev_id;
	ether_addr_copy(cmd->peer_macaddr.u.addr, arg->macaddr);
				   ...
				   struct peer_assoc_params *param)
{
	...
	ether_addr_copy(cmd->peer_macaddr.u.addr, param->peer_mac);
	...
	ptr = (u8*)ptr + peer_legacy_rates_align;
	...
	/* VHT Rates */
	ptr = (u8*)ptr + peer_ht_rates_align;
	...
	for (i = 0; i < (int)param->peer_he_mcs_count; i++) {
		he_mcs = ptr;
				...
				struct scan_req_params *arg)
{
	...
	arg->scan_req_id = 1;
	arg->scan_priority = (enum scan_priority)WMI_SCAN_PRIORITY_LOW;
	...
	arg->u_scan_f.scan_flags |= WMI_SCAN_CHAN_STAT_EVENT;
	arg->num_bssid = 1;
	/* fill bssid_list[0] with 0xff */
	ether_addr_copy(arg->bssid_list[0].u.addr, (const u8 *)"\xFF\xFF\xFF\xFF\xFF\xFF");
				       ...
				       struct scan_req_params *param)
{
	...
	if (param->u_ev.scan_ev.scan_ev_started)
		cmd->notify_scan_events |=  WMI_SCAN_EVENT_STARTED;
	if (param->u_ev.scan_ev.scan_ev_completed)
		cmd->notify_scan_events |=  WMI_SCAN_EVENT_COMPLETED;
	if (param->u_ev.scan_ev.scan_ev_bss_chan)
		cmd->notify_scan_events |=  WMI_SCAN_EVENT_BSS_CHANNEL;
	if (param->u_ev.scan_ev.scan_ev_foreign_chan)
		cmd->notify_scan_events |=  WMI_SCAN_EVENT_FOREIGN_CHAN;
	if (param->u_ev.scan_ev.scan_ev_dequeued)
		cmd->notify_scan_events |=  WMI_SCAN_EVENT_DEQUEUED;
	if (param->u_ev.scan_ev.scan_ev_preempted)
		cmd->notify_scan_events |=  WMI_SCAN_EVENT_PREEMPTED;
	if (param->u_ev.scan_ev.scan_ev_start_failed)
		cmd->notify_scan_events |=  WMI_SCAN_EVENT_START_FAILED;
	if (param->u_ev.scan_ev.scan_ev_restarted)
		cmd->notify_scan_events |=  WMI_SCAN_EVENT_RESTARTED;
	if (param->u_ev.scan_ev.scan_ev_foreign_chn_exit)
		cmd->notify_scan_events |=  WMI_SCAN_EVENT_FOREIGN_CHAN_EXIT;
	if (param->u_ev.scan_ev.scan_ev_suspended)
		cmd->notify_scan_events |=  WMI_SCAN_EVENT_SUSPENDED;
	if (param->u_ev.scan_ev.scan_ev_resumed)
		cmd->notify_scan_events |=  WMI_SCAN_EVENT_RESUMED;

	/** Set scan control flags */
	cmd->scan_ctrl_flags = 0;
	if (param->u_scan_f.scan_f.scan_f_passive)
		cmd->scan_ctrl_flags |=  WMI_SCAN_FLAG_PASSIVE;
	if (param->u_scan_f.scan_f.scan_f_strict_passive_pch)
		cmd->scan_ctrl_flags |=  WMI_SCAN_FLAG_STRICT_PASSIVE_ON_PCHN;
	if (param->u_scan_f.scan_f.scan_f_promisc_mode)
		cmd->scan_ctrl_flags |=  WMI_SCAN_FILTER_PROMISCUOS;
	if (param->u_scan_f.scan_f.scan_f_capture_phy_err)
		cmd->scan_ctrl_flags |=  WMI_SCAN_CAPTURE_PHY_ERROR;
	if (param->u_scan_f.scan_f.scan_f_half_rate)
		cmd->scan_ctrl_flags |=  WMI_SCAN_FLAG_HALF_RATE_SUPPORT;
	if (param->u_scan_f.scan_f.scan_f_quarter_rate)
		cmd->scan_ctrl_flags |=  WMI_SCAN_FLAG_QUARTER_RATE_SUPPORT;
	if (param->u_scan_f.scan_f.scan_f_cck_rates)
		cmd->scan_ctrl_flags |=  WMI_SCAN_ADD_CCK_RATES;
	if (param->u_scan_f.scan_f.scan_f_ofdm_rates)
		cmd->scan_ctrl_flags |=  WMI_SCAN_ADD_OFDM_RATES;
	if (param->u_scan_f.scan_f.scan_f_chan_stat_evnt)
		cmd->scan_ctrl_flags |=  WMI_SCAN_CHAN_STAT_EVENT;
	if (param->u_scan_f.scan_f.scan_f_filter_prb_req)
		cmd->scan_ctrl_flags |=  WMI_SCAN_FILTER_PROBE_REQ;
	if (param->u_scan_f.scan_f.scan_f_bcast_probe)
		cmd->scan_ctrl_flags |=  WMI_SCAN_ADD_BCAST_PROBE_REQ;
	if (param->u_scan_f.scan_f.scan_f_offchan_mgmt_tx)
		cmd->scan_ctrl_flags |=  WMI_SCAN_OFFCHAN_MGMT_TX;
	if (param->u_scan_f.scan_f.scan_f_offchan_data_tx)
		cmd->scan_ctrl_flags |=  WMI_SCAN_OFFCHAN_DATA_TX;
	if (param->u_scan_f.scan_f.scan_f_force_active_dfs_chn)
		cmd->scan_ctrl_flags |=  WMI_SCAN_FLAG_FORCE_ACTIVE_ON_DFS;
	if (param->u_scan_f.scan_f.scan_f_add_tpc_ie_in_probe)
		cmd->scan_ctrl_flags |=  WMI_SCAN_ADD_TPC_IE_IN_PROBE_REQ;
	if (param->u_scan_f.scan_f.scan_f_add_ds_ie_in_probe)
		cmd->scan_ctrl_flags |=  WMI_SCAN_ADD_DS_IE_IN_PROBE_REQ;
	if (param->u_scan_f.scan_f.scan_f_add_spoofed_mac_in_probe)
		cmd->scan_ctrl_flags |=  WMI_SCAN_ADD_SPOOF_MAC_IN_PROBE_REQ;
	if (param->u_scan_f.scan_f.scan_f_add_rand_seq_in_probe)
		cmd->scan_ctrl_flags |=  WMI_SCAN_RANDOM_SEQ_NO_IN_PROBE_REQ;
	if (param->u_scan_f.scan_f.scan_f_en_ie_whitelist_in_probe)
		cmd->scan_ctrl_flags |=
				   ...
				   struct scan_req_params *params)
{
	...
	struct hint_short_ssid *s_ssid = NULL;  //TODOTODO
	struct hint_bssid *hint_bssid = NULL;   //TODOTODO
	...
	if (params->extraie.len)
		extraie_len_with_pad =
			roundup((u8)params->extraie.len, sizeof(u32));
	...
	ether_addr_copy(cmd->mac_addr.u.addr, params->mac_addr.u.addr);
	ether_addr_copy(cmd->mac_mask.u.addr, params->mac_mask.u.addr);
	...
	for (i = 0; i < (int)params->num_chan; ++i)
	...
	if (params->num_ssids) {
		ssid = ptr;
		for (i = 0; i < (int)params->num_ssids; ++i) {
	...
	ptr = (u8*)ptr + (params->num_ssids * sizeof(*ssid));
	...
	if (params->num_bssid) {
		for (i = 0; i < (int)params->num_bssid; ++i) {
			ether_addr_copy(bssid->u.addr,
					params->bssid_list[i].u.addr);
			bssid++;
		}
	}

	ptr = (u8*)ptr + params->num_bssid * sizeof(*bssid);
	...
	ptr = (u8*)ptr + extraie_len_with_pad;
    if (params->num_hint_s_ssid) {
		...
		ptr = (u8*)ptr + TLV_HDR_SIZE;
		s_ssid = ptr;
		for (i = 0; i < (int)params->num_hint_s_ssid; ++i) {
	...
	if (params->num_hint_bssid) {
		...
		ptr = (u8*)ptr + TLV_HDR_SIZE;
		hint_bssid = ptr;
		for (i = 0; i < (int)params->num_hint_bssid; ++i) {
			hint_bssid->freq_flags =
				params->hint_bssid[i].freq_flags;
			ether_addr_copy(&params->hint_bssid[i].bssid.u.addr[0],
					&hint_bssid->bssid.u.addr[0]);
			hint_bssid++;
		}
	}
				       ...
				       struct scan_chan_list_params *chan_list)
{
	...
	while (chan_list->nallchans) {
		len = sizeof(*cmd) + TLV_HDR_SIZE;
		max_chan_limit = (u16)(wmi->wmi_ab->max_msg_len[ar->pdev_idx] - len) /
			sizeof(*chan_info);
...
int ath11k_wmi_pdev_peer_pktlog_filter(struct ath11k *ar, u8 *addr, u8 enable)
{
	...
	ether_addr_copy(info->peer_macaddr.u.addr, addr);
...
int ath11k_wmi_send_sar_limit(struct ath11k *ar,
		struct wmi_sar_limit_cmd_params *sar_limit_params)
{
	struct sk_buff *skb;
	struct wmi_sar_limits_cmd *cmd;
	int i;
	struct wmi_sar_limit_cmd_row *wmi_sar_rows_list;
	struct sar_limit_cmd_row *sar_rows_list;
	uint32_t len = sizeof(*cmd) + TLV_HDR_SIZE;
	struct wmi_tlv *tlv;

	len += sizeof(struct wmi_sar_limit_cmd_row) * sar_limit_params->num_limit_rows;
	skb = ath11k_wmi_alloc_skb(ar->wmi->wmi_ab, len);
	if (!skb)
		return -ENOMEM;

	cmd = (struct wmi_sar_limits_cmd *)skb->data;
	cmd->tlv_header = FIELD_PREP(WMI_TLV_TAG, WMI_TAG_SAR_LIMITS_CMD) |
		FIELD_PREP(WMI_TLV_LEN, sizeof(*cmd) - TLV_HDR_SIZE);
	...
	ath11k_dbg(ar->ab, ATH11K_DBG_WMI, "no of sar rows = %d, len = %d",
			sar_limit_params->num_limit_rows, len);

	tlv = (struct wmi_tlv *)((u8 *)cmd + sizeof(*cmd));
	tlv->header = FIELD_PREP(WMI_TLV_TAG, WMI_TAG_ARRAY_STRUCT) |
		FIELD_PREP(WMI_TLV_LEN,
				sar_limit_params->num_limit_rows * sizeof(struct wmi_sar_limit_cmd_row));

	if (cmd->num_limit_rows == 0)
		goto send_sar_limits;

	wmi_sar_rows_list = (struct wmi_sar_limit_cmd_row *)
		((u8 *)tlv + TLV_HDR_SIZE);
	sar_rows_list = sar_limit_params->sar_limit_row_list;

	for (i = 0; i < (int)sar_limit_params->num_limit_rows; i++) {
		wmi_sar_rows_list->tlv_header = FIELD_PREP(WMI_TLV_TAG, WMI_TAG_SAR_LIMIT_CMD_ROW) |
			FIELD_PREP(WMI_TLV_LEN, sizeof(struct wmi_sar_limit_cmd_row) - TLV_HDR_SIZE);
		...
		wmi_sar_rows_list->u.limit_value = sar_rows_list->limit_value;
		wmi_sar_rows_list->validity_bitmap =
			sar_rows_list->validity_bitmap;
		ath11k_dbg(ar->ab, ATH11K_DBG_WMI, "row %d, band_id = %d, chain_id = %d, mod_id = %d, limit_value = %d, validity_bitmap = %d",
				i, wmi_sar_rows_list->band_id,
				wmi_sar_rows_list->chain_id,
				wmi_sar_rows_list->mod_id,
				wmi_sar_rows_list->u.limit_value,
				wmi_sar_rows_list->validity_bitmap);
		sar_rows_list++;
		wmi_sar_rows_list++;
	}
send_sar_limits:
	return ath11k_wmi_cmd_send(ar->wmi, skb, WMI_SAR_LIMITS_CMDID);
}
...
int ath11k_wmi_add_beacon_filter(struct ath11k *ar, u32 vdev_id, const u32 *ies, u32 ies_cnt)
{
#define WMI_BCN_FLT_MAX_SUPPORTED_IES    256
#define WMI_BCN_FLT_MAX_ELEMS_IE_LIST    (WMI_BCN_FLT_MAX_SUPPORTED_IES/32)

	struct ath11k_pdev_wmi *wmi = ar->wmi;
	struct wmi_add_bcn_filter_cmd *cmd;
	struct sk_buff *skb;
	int ret;
    struct wmi_tlv *tlv;
    void* ptr;
    u16 len = sizeof(struct wmi_add_bcn_filter_cmd) + WMI_TLV_HDR_SIZE + (sizeof(u32)*WMI_BCN_FLT_MAX_ELEMS_IE_LIST);
    u32 ie_map[WMI_BCN_FLT_MAX_ELEMS_IE_LIST] = {0};
    u32 *p_ie_val;
	...
	cmd = (struct wmi_add_bcn_filter_cmd *)skb->data;
	cmd->tlv_header =
		FIELD_PREP(WMI_TLV_TAG, WMI_TAG_ADD_BCN_FILTER_CMD) |
		FIELD_PREP(WMI_TLV_LEN, sizeof(*cmd) - TLV_HDR_SIZE);

	cmd->vdev_id = vdev_id;

	while (ies_cnt--) {
		ie_map[ies[ies_cnt] / 32] |= 1 << (ies[ies_cnt] % 32);
	}
	p_ie_val = ie_map;

	ptr = skb->data + sizeof(*cmd);
	len = WMI_BCN_FLT_MAX_ELEMS_IE_LIST * sizeof(u32);
	...
	ptr = (u8*)ptr + TLV_HDR_SIZE;
	memcpy(ptr, p_ie_val, sizeof(u32)*WMI_BCN_FLT_MAX_ELEMS_IE_LIST);

    ath11k_dbg_dump(ar->ab, ATH11K_DBG_DATA, "beacon filter: ",
				p_ie_val, sizeof(u32)*WMI_BCN_FLT_MAX_ELEMS_IE_LIST, 0);
	
	ret = ath11k_wmi_cmd_send(wmi, skb, WMI_ADD_BCN_FILTER_CMDID);

	ath11k_dbg(ar->ab, ATH11K_DBG_WMI,
		   "send add beacon filter vdev id %d\n",
		   cmd->vdev_id);

	if (ret) {
		ath11k_warn(ar->ab,
			    "failed to send WMI_ADD_BCN_FILTER_CMDID: %d\n", ret);
		dev_kfree_skb(skb);
	}
...
int ath11k_wmi_remove_beacon_filter(struct ath11k *ar, u32 vdev_id)
{
	struct ath11k_pdev_wmi *wmi = ar->wmi;
	struct wmi_rmv_bcn_filter_cmd *cmd;
	struct sk_buff *skb;
	int ret;
	...
	cmd = (struct wmi_rmv_bcn_filter_cmd *)skb->data;
	cmd->tlv_header =
		FIELD_PREP(WMI_TLV_TAG, WMI_TAG_RMV_BCN_FILTER_CMD) |
		FIELD_PREP(WMI_TLV_LEN, sizeof(*cmd) - TLV_HDR_SIZE);

	cmd->vdev_id = vdev_id;
	ret = ath11k_wmi_cmd_send(wmi, skb, WMI_RMV_BCN_FILTER_CMDID);

	ath11k_dbg(ar->ab, ATH11K_DBG_WMI,
		   "send remove beacon filter vdev id %d\n",
		   cmd->vdev_id);

	if (ret) {
		ath11k_warn(ar->ab,
			    "failed to send WMI_RMV_BCN_FILTER_CMDID: %d\n", ret);
		dev_kfree_skb(skb);
	}
...
int ath11k_wmi_set_beacon_filter(struct ath11k *ar, u32 vdev_id, const u32 *ies, u32 ies_cnt)
{
	int ret;

	if (ies == NULL || ies_cnt == 0) {
		ret = ath11k_wmi_remove_beacon_filter(ar, vdev_id);
	}
	else {
		ret = ath11k_wmi_add_beacon_filter(ar, vdev_id, ies, ies_cnt);
	}
	return ret;
}
				...
				struct wmi_init_cmd_param *param)
{
	...
	skb = ath11k_wmi_alloc_skb(wmi->wmi_ab, (u32)len);
	if (!skb)
		return -ENOMEM;
	...
	ptr = (u8*)ptr + sizeof(*cfg);
	host_mem_chunks = (struct wlan_host_mem_chunk *)((u8*)ptr + TLV_HDR_SIZE);
	...
	for (idx = 0; idx < param->num_mem_chunks; ++idx) {
		...
		host_mem_chunks[idx].ptr = (u32)param->mem_chunks[idx].paddr;
...
int ath11k_wmi_cmd_init(struct ath11k_base *ab)
{
    ...
    init_param.num_mem_chunks = (u8)wmi_sc->num_mem_chunks;
					     ...
					     const void *ptr, void *data)
{
	...
	hw_mode_cap = container_of((u8*)ptr, struct wmi_hw_mode_capabilities,
				   hw_mode_id);
					   ...
					   u16 len, const void *ptr, void *data)
{
	...
	for (i = 0; i < svc_rdy_ext->param.num_phy; i++) {
		ret = ath11k_pull_reg_cap_svc_rdy_ext(wmi_handle,
						      ...
						      svc_rdy_ext->ext_hal_reg_caps, (u8)i,
						      &reg_cap);
		if (ret) {
    ...
    void *data)
{
    struct ath11k_pdev_wmi *wmi_handle = &soc->wmi_ab.wmi[0];
    struct wmi_tlv_svc_rdy_ext_parse *svc_rdy_ext = data;
    u8 hw_mode_id = (u8)svc_rdy_ext->pref_hw_mode_caps.hw_mode_id;
    u32 phy_id_map;
    int pdev_index = 0;
    int ret;
...
static int ath11k_service_ready_ext_event(struct ath11k_base *ab,
					  struct sk_buff *skb)
{
	struct wmi_tlv_svc_rdy_ext_parse svc_rdy_ext = {0};
	int ret;
			   ...
			   struct wmi_regulatory_rule_struct *wmi_reg_rule)
{
	...
	for (count = 0; count < num_reg_rules; count++) {
		reg_rule_ptr[count].start_freq =
			(u16)FIELD_GET(REG_RULE_START_FREQ,
				  wmi_reg_rule[count].freq_info);
		reg_rule_ptr[count].end_freq =
			(u16)FIELD_GET(REG_RULE_END_FREQ,
				  wmi_reg_rule[count].freq_info);
		reg_rule_ptr[count].max_bw =
			(u16)FIELD_GET(REG_RULE_MAX_BW,
				  wmi_reg_rule[count].bw_pwr_info);
		reg_rule_ptr[count].reg_power =
			(u8)FIELD_GET(REG_RULE_REG_PWR,
				  wmi_reg_rule[count].bw_pwr_info);
		reg_rule_ptr[count].ant_gain =
			(u8)FIELD_GET(REG_RULE_ANT_GAIN,
				  wmi_reg_rule[count].bw_pwr_info);
		reg_rule_ptr[count].flags =
			(u16)FIELD_GET(REG_RULE_FLAGS,
					       ...
					       struct cur_regulatory_info *reg_info)
{
	...
	reg_info->num_phy = (u8)chan_list_event_hdr->num_phy;
	reg_info->phy_id = (u8)chan_list_event_hdr->phy_id;
	reg_info->ctry_code = (u16)chan_list_event_hdr->country_id;
	reg_info->reg_dmn_pair = (u16)chan_list_event_hdr->domain_code;
	...
	ath11k_dbg(ab, ATH11K_DBG_WMI,
		   "%a:cc %a dsf %d BW: min_2g %d max_2g %d min_5g %d max_5g %d\n",
		   __func__, reg_info->alpha2, reg_info->dfs_region,
		   reg_info->min_bw_2g, reg_info->max_bw_2g,
		   reg_info->min_bw_5g, reg_info->max_bw_5g);

	ath11k_dbg(ab, ATH11K_DBG_WMI,
		   "%a: num_2g_reg_rules %d num_5g_reg_rules %d\n", __func__,
					...
					struct wmi_peer_delete_resp_event *peer_del_resp)
{
	...
	ether_addr_copy(peer_del_resp->peer_macaddr.u.addr,
			ev->peer_macaddr.u.addr);
...
static int ath11k_pull_vdev_delete_resp_param_tlv(struct ath11k_base *ab, struct sk_buff *skb,
    u32 *vdev_id)
{
    void **tb;
    ...
    if (!ev) {
        ath11k_warn(ab, "failed to fetch vdev delete response ev");
        kfree(tb);
        return -EPROTO;
    }
					  ...
					  struct mgmt_rx_event_params *hdr)
{
	...
	for (int i = 0; i < ATH_MAX_ANTENNA; i++) {
		hdr->rssi_ctl[i] = (s8)ev->rssi_ctl[i];
	}
	...
	skb_trim(skb, 0);
	skb_put(skb, (u32)(frame - skb->data));
	skb_pull(skb, (u32)(frame - skb->data));
				    ...
				    u32 status)
{
	...
	dma_unmap_single(ar->ab->dev, skb_cb->paddr, skb_cb->paddr_mapping_addr, msdu->len, DMA_TO_DEVICE);

	info = IEEE80211_SKB_CB(msdu);

	if ((!(info->flags & IEEE80211_TX_CTL_NO_ACK)) && !status)
		info->flags |= IEEE80211_TX_STATUS_ACK;
...
static void ath11k_wmi_event_scan_foreign_chan(struct ath11k *ar, u32 freq)
{
	...
	case ATH11K_SCAN_RUNNING:
	case ATH11K_SCAN_ABORTING:
		ar->scan_channel = ieee80211_get_channel(ar->hw, freq);
		break;
	}
}
					   ...
					   struct wmi_peer_sta_kickout_arg *arg)
{
	...
	arg->mac_addr = ev->peer_macaddr.u.addr;
				      ...
				      struct wmi_vdev_install_key_complete_arg *arg)
{
	...
	arg->macaddr = ev->peer_macaddr.u.addr;
					  ...
					  struct wmi_peer_assoc_conf_arg *peer_assoc_conf)
{
	...
	peer_assoc_conf->macaddr = ev->peer_macaddr.u.addr;
...
size_t ath11k_wmi_fw_stats_num_vdevs(LIST_ENTRY *head)
{
	struct ath11k_fw_stats_vdev *i;
	size_t num = 0;

	list_for_each_entry(i, head, struct ath11k_fw_stats_vdev, list)
		++num;

	return num;
}

static size_t ath11k_wmi_fw_stats_num_bcn(LIST_ENTRY *head)
{
	struct ath11k_fw_stats_bcn *i;
	size_t num = 0;

	list_for_each_entry(i, head, struct ath11k_fw_stats_bcn, list)
		++num;
			      ...
			      char *buf, u32 *length)
{
	...
	vif_macaddr = ieee80211_vif_addr(arvif->vif);
			     ...
			     char *buf, u32 *length)
{
    ...
    vdev_macaddr = ieee80211_vif_addr(arvif->vif);
			      ...
			      u32 stats_id, char *buf)
{
	...
	if (stats_id == WMI_REQUEST_VDEV_STAT) {
		...
		list_for_each_entry(vdev, &fw_stats->vdevs, struct ath11k_fw_stats_vdev, list)
	...
	if (stats_id == WMI_REQUEST_BCN_STAT) {
		...
		list_for_each_entry(bcn, &fw_stats->bcn, struct ath11k_fw_stats_bcn, list)
...
static int ath11k_reg_11d_new_cc_event(struct ath11k_base *ab, struct sk_buff *skb)
{
	...
	schedule_work(&ab->update_11d_work);
				    ...
				    const void *ptr, void *data)
{
	...
	case WMI_TAG_READY_EVENT:
		...
		ether_addr_copy(ab->mac_addr, fixed_param->mac_addr.u.addr);
		ab->wmi_ready = true;
		break;
	case WMI_TAG_ARRAY_FIXED_STRUCT:
		...
		for (i = 0; i < ab->num_radios; i++) {
			pdev = &ab->pdevs[i];
			ether_addr_copy(pdev->mac_addr, addr_list[i].u.addr);
		}
...
static int ath11k_pdev_set_hw_mode_response_parse(struct ath11k_base *ab, u16 tag, u16 len,
    const void *ptr, void *data)
{
    struct wmi_pdev_set_hw_mode_response_event *fixed_param;

    switch (tag) {
    case WMI_TAG_PDEV_SET_HW_MODE_RESPONSE_EVENT:
        fixed_param = (struct wmi_pdev_set_hw_mode_response_event *)ptr;
        memcpy(data, fixed_param, sizeof(*fixed_param));
        break;
    case WMI_TAG_PDEV_SET_HW_MODE_RESPONSE_VDEV_MAC_ENTRY:
        ath11k_warn(ab, "VDEV MAC ENTRY IS IGNORED\n");
        break;
    default:
        break;
    }
...
static void ath11k_pdev_set_hw_mode_resp_event(struct ath11k_base *ab, struct sk_buff *skb)
{
    struct wmi_pdev_set_hw_mode_response_event set_hw_mode_resp;
    int ret;

    ret = ath11k_wmi_tlv_iter(ab, skb->data, skb->len,
        ath11k_pdev_set_hw_mode_response_parse, &set_hw_mode_resp);
    if (ret) {
        ath11k_warn(ab, "failed to parse tlv %d\n", ret);
        return;
    }

    ath11k_info(ab, "pdev_id %d, hw_mode_index %d, status %d\n", 
        set_hw_mode_resp.pdev_id, set_hw_mode_resp.cfgd_hw_mode_index, 
        set_hw_mode_resp.status);

    complete(&ab->hw_mode_ready);
}
...
static void ath11k_vdev_stopped_event(struct ath11k_base *ab, struct sk_buff *skb)
{
	...
	ath11k_dbg(ab, ATH11K_DBG_WMI, "vdev stopped for vdev id %d\n", vdev_id);
}
...
static void ath11k_vdev_delete_resp_event(struct ath11k_base *ab, struct sk_buff *skb)
{
    ...
    if (ath11k_pull_vdev_delete_resp_param_tlv(ab, skb, &vdev_id) != 0) {
        ath11k_warn(ab, "failed to extract vdev delete resp event\n");
        return;
    }

    rcu_read_lock();
    ar = ath11k_mac_get_ar_vdev_delete_status(ab, vdev_id);
    if (!ar) {
    ...
    ath11k_dbg(ab, ATH11K_DBG_WMI, "vdev deleted for vdev id %d\n", vdev_id);
}
...
static void ath11k_mgmt_rx_event(struct ath11k_base *ab, struct sk_buff *skb)
{
	...
	if (rx_ev.status & WMI_RX_STATUS_ERR_MIC)
		status->flag |= IEEE80211_RX_FLAG_MMIC_ERROR;
	...
	status->freq = (u16)ieee80211_channel_to_frequency(rx_ev.channel,
						      status->band);

	if (status->band == NL80211_BAND_6GHZ)
		status->freq = (u16)rx_ev.chan_freq;

	status->signal = (s8)rx_ev.snr + ATH11K_DEFAULT_NOISE_FLOOR;
	status->rate_idx = ath11k_mac_bitrate_to_idx(sband, rx_ev.rate / 100);

    status->chains = ar->num_rx_chains;
	memcpy(status->chain_signal, rx_ev.rssi_ctl, sizeof(status->chain_signal));
	 ...
	 */
	status->flag |= IEEE80211_RX_FLAG_SKIP_MONITOR;
	...
	if (ieee80211_has_protected(hdr->frame_control) &&
		...
		status->flag |= IEEE80211_RX_FLAG_DECRYPTED;

		if (!ieee80211_is_robust_mgmt_frame(skb)) {
			status->flag |= IEEE80211_RX_FLAG_IV_STRIPPED |
					IEEE80211_RX_FLAG_MMIC_STRIPPED;
	...
	ath11k_dbg(ab, ATH11K_DBG_MGMT,
		   ...
		   skb, skb->len,
		   IEEE80211_GET_FC_TYPE(fc), IEEE80211_GET_FC_STYPE(fc));
    ...
    ath11k_dbg_dump(ab, ATH11K_DBG_PKT, "rx 802.11 mgmt ", skb->data, skb->len, 0);
...
static void ath11k_scan_event(struct ath11k_base *ab, struct sk_buff *skb)
{
	...
	ath11k_dbg(ab, ATH11K_DBG_WMI,
		   "scan event %a type %d reason %d freq %d req_id %d scan_id %d vdev_id %d state %a (%d)\n",
...
static void ath11k_peer_sta_kickout_event(struct ath11k_base *ab, struct sk_buff *skb)
{
	...
	arvif = ath11k_mac_get_arvif_by_vdev_id(ab, peer->vdev_id);
	if (!arvif) {
		ath11k_warn(ab, "invalid vdev id in peer sta kickout ev %d\n",
			    peer->vdev_id);
		goto exit;
	}
	sta = ieee80211_find_sta(arvif->vif, arg.mac_addr);
	if (!sta) {
...
ath11k_pdev_bss_chan_info_event(struct ath11k_base *ab, struct sk_buff *skb)
{
	...
	DEBUG_PARAMETER(busy);
	DEBUG_PARAMETER(total);
	DEBUG_PARAMETER(tx);
	DEBUG_PARAMETER(rx);
	DEBUG_PARAMETER(rx_bss);
...
static void
ath11k_wmi_diag_event(struct ath11k_base *ab, struct sk_buff *skb)
{
	u32 len = __cpu_to_be32(skb->len);
	u8 *len_buf = (u8*)&len;
	
    /* Will use the WMI event to trace the fw log, first record the payload-len and
     * then record the payload body.
	 */ 
	wmi_local_write(ab->dev, len_buf, 4);
	wmi_local_write(ab->dev, skb->data, skb->len);
}

static void
ath11k_wmi_tlv_op_rx(struct ath11k_base *ab, struct sk_buff *skb)
{
	struct wmi_cmd_hdr *cmd_hdr;
	...
	ath11k_dbg(ab, ATH11K_DBG_WMI, "wmi tlv op rx 0x%x %a\n", id, evt_id_to_string(id));
    ...
    case WMI_PDEV_SET_HW_MODE_RESP_EVENTID:
        ath11k_pdev_set_hw_mode_resp_event(ab, skb);
        break;
	...
	case WMI_TWT_DISABLE_EVENTID:
		ath11k_dbg(ab, ATH11K_DBG_WMI,
			   "ignoring unsupported event 0x%08x\n", id);
		break;
    case WMI_TX_PAUSE_EVENTID:
        /* don't need to handle tx pause event */
        break;
	...
	default:
		ath11k_warn(ab, "unsupported event: 0x%08x\n", id);
		break;
	}
...
static int
ath11k_connect_pdev_htc_service(struct ath11k_base *ab,
					   u32 pdev_idx)
{
	int status;
	u16 svc_id[] = { ATH11K_HTC_SVC_ID_WMI_CONTROL,
