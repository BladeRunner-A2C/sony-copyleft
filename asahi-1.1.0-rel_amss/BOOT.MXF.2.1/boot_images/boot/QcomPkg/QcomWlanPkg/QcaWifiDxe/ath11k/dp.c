			 ...
			 int mac_id, int num_entries)
{
	...
	ring->paddr = ring->paddr_unaligned + ((u8*)ring->vaddr -
		      (u8*)ring->vaddr_unaligned);
...
static int ath11k_dp_srng_common_setup(struct ath11k_base *ab)
{
	 ...
	 */
	ab->hw_params.hw_ops->reo_setup(ab, ab->hw_params.ring_hmap);
						  ...
						  u32 last_bank_sz)
{
	...
	for (i = 0; i < n_link_desc_bank; i++) {
		align_bytes = (int)((u8*)link_desc_banks[i].vaddr -
	...
	end_offset = (u32)(((u8*)scatter_buf - (u8*)slist[scatter_idx].vaddr) *
			      ...
			      u32 n_link_desc)
{
	...
	for (i = 0; i < n_link_desc_bank; i++) {
		...
		paddr = link_desc_banks[i].paddr;
		while (n_entries &&
		       (desc = ath11k_hal_srng_src_get_next_entry(ab, srng)) != NULL) {
...
static int ath11k_dp_tx_pending_cleanup(int buf_id, void *skb, void *ctx)
{
	...
	dma_unmap_single(ab->dev, ATH11K_SKB_CB(msdu)->paddr, ATH11K_SKB_CB(msdu)->paddr_mapping_addr, msdu->len,
			 DMA_TO_DEVICE);
...
void ath11k_dp_free(struct ath11k_base *ab)
{
	...
	for (i = 0; i < DP_TCL_NUM_RING_MAX; i++) {
        ...
        idr_for_each_entry(&dp->tx_ring[i].txbuf_idr, iter, buf_id, skb) {
            ath11k_dp_tx_pending_cleanup((int)buf_id, skb, ab);
        }
