...
#define ath11k_info(ab, fmt, ...) do {                              \
        (void)ab;                                                   \
        pr_debug(fmt, ##__VA_ARGS__);                               \
    } while (0)

#define ath11k_err(ab, fmt, ...) do {                               \
        (void)ab;                                                   \
        pr_err(fmt, ##__VA_ARGS__);                                 \
    } while (0)

#define ath11k_warn(ab, fmt, ...) do {                              \
        (void)ab;                                                   \
...
#define ath11k_dbg(ab, dbg_mask, fmt, ...) do {       \
	    if (ath11k_debug_mask & dbg_mask)		      \
		    ath11k_info(ab, fmt, ##__VA_ARGS__);      \
    } while (0)


#define ath11k_dbg_dump(ab, dbg_mask, msg, buf, len, max_len) do {  \
	    if (ath11k_debug_mask & dbg_mask)                           \
           pr_array(msg, (u8 *)buf, len, max_len);                  \
    } while (0)

#define ath11k_trace(ab, dbg_mask, fmt, ...) (void)sizeof(u8)
