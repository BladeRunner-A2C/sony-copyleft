				...
				int level)
{
	...
	while (temp_ei->data_type != QMI_EOTI) {
		...
		if (temp_ei->data_type == QMI_DATA_LEN) {
			...
			if (level > 1)
				min_msg_len += temp_ei->elem_len <= 0xFF/*U8_MAX*/ ?
					sizeof(u8) : sizeof(u16);
				  ...
				  int enc_level)
{
	...
	for (i = 0; i < elem_len; i++) {
		...
		if (rc < 0) {
			pr_err("STRUCT Encode failure\n");
			return rc;
		}
		buf_dst = (u8 *)buf_dst + rc;
		buf_src = (u8 *)buf_src + temp_ei->elem_size;
		encoded_bytes += rc;
	}
				  ...
				  u32 out_buf_len, int enc_level)
{
	...
	string_len = (u32)strlen(buf_src);
	string_len_sz = temp_ei->elem_len <= 0xFF/*U8_MAX*/ ?
			sizeof(u8) : sizeof(u16);
	...
	rc = qmi_encode_basic_elem((u8 *)buf_dst + encoded_bytes, buf_src,
		      ...
		      int enc_level)
{
	...
	while (temp_ei->data_type != QMI_EOTI) {
		buf_src = (const u8 *)in_c_struct + temp_ei->offset;
		tlv_type = temp_ei->tlv_type;
				  ...
				  int dec_level)
{
	...
	for (i = 0; i < elem_len && decoded_bytes < tlv_len; i++) {
		...
		if (rc < 0)
			return rc;
		buf_src = (u8 *)buf_src + rc;
		buf_dst = (u8 *)buf_dst + temp_ei->elem_size;
		decoded_bytes += rc;
	}
				  ...
				  u32 tlv_len, int dec_level)
{
	...
	rc = qmi_decode_basic_elem(buf_dst, (u8 *)buf_src + decoded_bytes,
				   string_len, temp_ei->elem_size);
	*((u8 *)buf_dst + string_len) = '\0';
	decoded_bytes += rc;
		      ...
		      int dec_level)
{
	...
	while (decoded_bytes < in_buf_len) {
		...
		buf_dst = (u8 *)out_c_struct + temp_ei->offset;
		if (temp_ei->data_type == QMI_OPT_FLAG) {
			 ...
			 const void *c_struct)
{
	...
	if (c_struct) {
		msglen = qmi_encode(ei, msg + sizeof(*hdr), c_struct, (u32)*len, 1);
		if (msglen < 0) {
			kfree(msg);
	...
	hdr = (struct qmi_header *)msg;
	hdr->type = (u8)type;
	hdr->txn_id = (u16)txn_id;
	hdr->msg_id = (u16)msg_id;
	hdr->msg_len = (u16)msglen;
		       ...
		       struct qmi_elem_info *ei, void *c_struct)
{
	...
	return qmi_decode(ei, c_struct, (const u8 *)buf + sizeof(struct qmi_header),
			  (u32)(len - sizeof(struct qmi_header)), 1);
}
