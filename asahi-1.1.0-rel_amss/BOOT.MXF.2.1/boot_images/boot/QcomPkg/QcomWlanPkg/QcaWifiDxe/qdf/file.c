    ...
    IN CONST UINT64 OpenMode)
{
    ...
    if (EFI_ERROR(Status)) {
        DbgError("Failed to get simple file system volume: %r\n", Status);
        return NULL;
    }

    Status = Vol->OpenVolume(Vol, &RootDir);
    if (EFI_ERROR(Status)) {
        DbgError("Failed to open volume: %r\n", Status);
        return NULL;
    }
    ...
    OUT EFI_QDF_FILE_HANDLE *FileHandle)
{
    ...
    if (DevicePath && DevicePath[0] != 0) {
        ...
        UnicodeSPrint(FilePath, sizeof(FilePath), L"%s%s", DevicePath, FileName);
...
QdfReadFirmwareVolumeSection(
    ...
    OUT UINTN *Size)
{
    return GetSectionFromAnyFv(NameGuid, EFI_SECTION_RAW, 0, Buffer, Size);
}
...
QdfReadFromPartition(                          
    ...
    UINT32           Attrib)
{
    return ReadFromPartition(PartitionGuid, PartitionLabel, ReadBuffer, ImageSize, Attrib);
}
