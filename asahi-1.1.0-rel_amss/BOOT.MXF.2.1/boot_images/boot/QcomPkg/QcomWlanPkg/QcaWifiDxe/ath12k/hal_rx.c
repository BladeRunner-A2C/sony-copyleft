				     ...
				     u32 *cookie, u8 *rbm)
{
	...
	*rbm = (u8)le32_get_bits(binfo->info1, BUFFER_ADDR_INFO1_RET_BUF_MGR);
}
				  ...
				  struct hal_rx_wbm_rel_info *rel_info)
{
	...
	if (!hw_cc_done) {
		...
		rel_info->rx_desc =
			(struct ath12k_rx_desc_info *)(desc_va);
	}
				       ...
				       struct hal_reo_status *status)
{
	...
	status->uniform_hdr.cmd_num =
		(u16)le32_get_bits(desc->hdr.info0,
