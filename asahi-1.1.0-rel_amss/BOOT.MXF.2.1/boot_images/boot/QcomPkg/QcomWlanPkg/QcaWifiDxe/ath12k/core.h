...
struct ath12k_base {
	...
	struct fw_remote_mem remote_mem[ATH12K_QMI_WLANFW_MAX_NUM_MEM_SEG_V01];
...
static inline struct ath12k_skb_cb *ATH12K_SKB_CB(struct sk_buff *skb)
{
	...
	BUILD_BUG_ON(sizeof(struct ath12k_skb_cb) > sizeof(IEEE80211_SKB_CB(skb)->u.driver_data));
	return (struct ath12k_skb_cb *)&IEEE80211_SKB_CB(skb)->u.driver_data;
}
...
{
	return (struct ath12k_vif *)ieee80211_vif_get_priv_data(vif);
}
