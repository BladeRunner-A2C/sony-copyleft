...
static int ath12k_mhi_link_status(struct mhi_controller *mhi_ctrl)
{
	return 0;
}
...
{
	struct ath12k_pci *ab_pci = mhi_cntrl->priv_data;
	struct ath12k_base *ab = ab_pci->ab;

	ath12k_dbg(ab, ATH12K_DBG_BOOT, "mhi notify status reason %a\n",
	...
	case MHI_CB_EE_RDDM:
		if (!(test_bit(ATH12K_FLAG_UNREGISTERING, &ab->dev_flags))) {
			...
			schedule_work(&ab_pci->rddm_worker);
		}
		break;
	default:
		break;
...
int ath12k_mhi_register(struct ath12k_pci *ab_pci)
{
	...
	mhi_ctrl->fw_image = ATH12K_PCI_FW_FILE_NAME;
	...
	mhi_ctrl->link_status = ath12k_mhi_link_status;
				...
				enum ath12k_mhi_state mhi_state)
{
	...
	ath12k_dbg(ab, ATH12K_DBG_PCI, "setting mhi state: %a(%d)\n",
		   ath12k_mhi_state_to_str(mhi_state), mhi_state);
	...
	case ATH12K_MHI_POWER_ON:
		...
		if (ret) {
			ath12k_err(ab, "failed to power up ret = 0x%x\n", ret);
...
out:
	ath12k_err(ab, "failed to set mhi state: %a(%d)\n",
		   ath12k_mhi_state_to_str(mhi_state), mhi_state);
	return ret;
}
...
int ath12k_mhi_start(struct ath12k_pci *ab_pci)
{
	...
	if (ret) {
		...
		ath12k_err(ab, "power on fail and try again.\n");

		/* clean up the fw cache */
    	flush_firmware_cache();
		
		/* Fw auth fail, try to power on again.*/
		ret = ath12k_mhi_set_state(ab_pci, ATH12K_MHI_POWER_ON);
#else
		ath12k_err(ab, "power on fail ret = 0x%x.\n", ret);
		goto out;
#endif
	}
...
{
    return ath12k_mhi_set_state(ar_pci, ATH12K_MHI_TRIGGER_RDDM);
}
