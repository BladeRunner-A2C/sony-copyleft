/* SPDX-License-Identifier: BSD-3-Clause-Clear */
/*
 * Copyright (c) 2018-2021 The Linux Foundation. All rights reserved.
 * Copyright (c) 2021-2022 Qualcomm Innovation Center, Inc. All rights reserved.
 */

#ifndef _ATH12K_DEBUG_H_
#define _ATH12K_DEBUG_H_

#include <qdf/pr_debug.h>
#include "hal_tx.h"

enum ath12k_debug_mask {
	ATH12K_DBG_AHB		= 0x00000001,
	ATH12K_DBG_WMI		= 0x00000002,
	ATH12K_DBG_HTC		= 0x00000004,
	ATH12K_DBG_DP_HTT	= 0x00000008,
	ATH12K_DBG_MAC		= 0x00000010,
	ATH12K_DBG_BOOT		= 0x00000020,
	ATH12K_DBG_QMI		= 0x00000040,
	ATH12K_DBG_DATA		= 0x00000080,
	ATH12K_DBG_MGMT		= 0x00000100,
	ATH12K_DBG_REG		= 0x00000200,
	ATH12K_DBG_TESTMODE	= 0x00000400,
	ATH12K_DBG_HAL		= 0x00000800,
	ATH12K_DBG_PCI		= 0x00001000,
	ATH12K_DBG_PKT      = 0x00002000,
	ATH12K_DBG_ANY		= 0xffffffff,
};

/* htt_dbg_ext_stats_type */
enum ath12k_dbg_htt_ext_stats_type {
	ATH12K_DBG_HTT_EXT_STATS_RESET                      =  0,
	ATH12K_DBG_HTT_EXT_STATS_PDEV_TX                    =  1,
	ATH12K_DBG_HTT_EXT_STATS_PDEV_RX                    =  2,
	ATH12K_DBG_HTT_EXT_STATS_PDEV_TX_HWQ                =  3,
	ATH12K_DBG_HTT_EXT_STATS_PDEV_TX_SCHED              =  4,
	ATH12K_DBG_HTT_EXT_STATS_PDEV_ERROR                 =  5,
	ATH12K_DBG_HTT_EXT_STATS_PDEV_TQM                   =  6,
	ATH12K_DBG_HTT_EXT_STATS_TQM_CMDQ                   =  7,
	ATH12K_DBG_HTT_EXT_STATS_TX_DE_INFO                 =  8,
	ATH12K_DBG_HTT_EXT_STATS_PDEV_TX_RATE               =  9,
	ATH12K_DBG_HTT_EXT_STATS_PDEV_RX_RATE               =  10,
	ATH12K_DBG_HTT_EXT_STATS_PEER_INFO                  =  11,
	ATH12K_DBG_HTT_EXT_STATS_TX_SELFGEN_INFO            =  12,
	ATH12K_DBG_HTT_EXT_STATS_TX_MU_HWQ                  =  13,
	ATH12K_DBG_HTT_EXT_STATS_RING_IF_INFO               =  14,
	ATH12K_DBG_HTT_EXT_STATS_SRNG_INFO                  =  15,
	ATH12K_DBG_HTT_EXT_STATS_SFM_INFO                   =  16,
	ATH12K_DBG_HTT_EXT_STATS_PDEV_TX_MU                 =  17,
	ATH12K_DBG_HTT_EXT_STATS_ACTIVE_PEERS_LIST          =  18,
	ATH12K_DBG_HTT_EXT_STATS_PDEV_CCA_STATS             =  19,
	ATH12K_DBG_HTT_EXT_STATS_TWT_SESSIONS               =  20,
	ATH12K_DBG_HTT_EXT_STATS_REO_RESOURCE_STATS         =  21,
	ATH12K_DBG_HTT_EXT_STATS_TX_SOUNDING_INFO           =  22,

	/* keep this last */
	ATH12K_DBG_HTT_NUM_EXT_STATS,
};

#define ATH12K_HTT_STATS_BUF_SIZE (1024 * 512)

#define ATH12K_FW_STATS_BUF_SIZE (1024 * 1024)

#define ATH12K_HTT_PKTLOG_MAX_SIZE 2048

enum ath12k_pktlog_filter {
	ATH12K_PKTLOG_RX		= 0x000000001,
	ATH12K_PKTLOG_TX		= 0x000000002,
	ATH12K_PKTLOG_RCFIND		= 0x000000004,
	ATH12K_PKTLOG_RCUPDATE		= 0x000000008,
	ATH12K_PKTLOG_EVENT_SMART_ANT	= 0x000000020,
	ATH12K_PKTLOG_EVENT_SW		= 0x000000040,
	ATH12K_PKTLOG_ANY		= 0x00000006f,
};

enum ath12k_pktlog_mode {
	ATH12K_PKTLOG_MODE_LITE = 1,
	ATH12K_PKTLOG_MODE_FULL = 2,
};

enum ath12k_pktlog_enum {
	ATH12K_PKTLOG_TYPE_TX_CTRL      = 1,
	ATH12K_PKTLOG_TYPE_TX_STAT      = 2,
	ATH12K_PKTLOG_TYPE_TX_MSDU_ID   = 3,
	ATH12K_PKTLOG_TYPE_RX_STAT      = 5,
	ATH12K_PKTLOG_TYPE_RC_FIND      = 6,
	ATH12K_PKTLOG_TYPE_RC_UPDATE    = 7,
	ATH12K_PKTLOG_TYPE_TX_VIRT_ADDR = 8,
	ATH12K_PKTLOG_TYPE_RX_CBF       = 10,
	ATH12K_PKTLOG_TYPE_RX_STATBUF   = 22,
	ATH12K_PKTLOG_TYPE_PPDU_STATS   = 23,
	ATH12K_PKTLOG_TYPE_LITE_RX      = 24,
};

#if 0
__printf(2, 3) void ath12k_info(struct ath12k_base *ab, const char *fmt, ...);
__printf(2, 3) void ath12k_err(struct ath12k_base *ab, const char *fmt, ...);
__printf(2, 3) void ath12k_warn(struct ath12k_base *ab, const char *fmt, ...);
#else
#define ath12k_info(ab, fmt, ...) do {                              \
        (void)ab;                                                   \
        pr_debug(fmt, ##__VA_ARGS__);                               \
    } while (0)

#define ath12k_err(ab, fmt, ...) do {                               \
        (void)ab;                                                   \
        pr_err(fmt, ##__VA_ARGS__);                                 \
    } while (0)

#define ath12k_warn(ab, fmt, ...) do {                              \
        (void)ab;                                                   \
        pr_warn(fmt, ##__VA_ARGS__);                                \
    } while (0)
#endif

extern unsigned int ath12k_debug_mask;

#if 0
#ifdef CONFIG_ATH12K_DEBUG
__printf(3, 4) void __ath12k_dbg(struct ath12k_base *ab,
				 enum ath12k_debug_mask mask,
				 const char *fmt, ...);
void ath12k_dbg_dump(struct ath12k_base *ab,
		     enum ath12k_debug_mask mask,
		     const char *msg,
		     const void *buf, u32 len, u32 max_len);
#else /* CONFIG_ATH12K_DEBUG */
static inline int __ath12k_dbg(struct ath12k_base *ab,
			       enum ath12k_debug_mask dbg_mask,
			       const char *fmt, ...)
{
	return 0;
}

static inline void ath12k_dbg_dump(struct ath12k_base *ab,
				   enum ath12k_debug_mask mask,
				   const char *msg, 
				   const void *buf, u32 len, u32 max_len)
{
}
#endif /* CONFIG_ATH12K_DEBUG */
#endif

#ifdef CONFIG_ATH12K_DEBUGFS
int ath12k_debug_soc_create(struct ath12k_base *ab);
void ath12k_debug_soc_destroy(struct ath12k_base *ab);
int ath12k_debug_pdev_create(struct ath12k_base *ab);
void ath12k_debug_pdev_destroy(struct ath12k_base *ab);
int ath12k_debug_register(struct ath12k *ar);
void ath12k_debug_unregister(struct ath12k *ar);
void ath12k_dbg_htt_ext_stats_handler(struct ath12k_base *ab,
				      struct sk_buff *skb);
void ath12k_debug_fw_stats_process(struct ath12k_base *ab, struct sk_buff *skb);

void ath12k_debug_fw_stats_init(struct ath12k *ar);
int ath12k_dbg_htt_stats_req(struct ath12k *ar);

static inline bool ath12k_debug_is_pktlog_lite_mode_enabled(struct ath12k *ar)
{
	return (ar->debug.pktlog_mode == ATH12K_PKTLOG_MODE_LITE);
}

static inline bool ath12k_debug_is_pktlog_rx_stats_enabled(struct ath12k *ar)
{
	return (!ar->debug.pktlog_peer_valid && ar->debug.pktlog_mode);
}

static inline bool ath12k_debug_is_pktlog_peer_valid(struct ath12k *ar, u8 *addr)
{
	return (ar->debug.pktlog_peer_valid && ar->debug.pktlog_mode &&
		ether_addr_equal(addr, ar->debug.pktlog_peer_addr));
}

static inline int ath12k_debug_is_extd_tx_stats_enabled(struct ath12k *ar)
{
	return ar->debug.extd_tx_stats;
}

static inline int ath12k_debug_is_extd_rx_stats_enabled(struct ath12k *ar)
{
	return ar->debug.extd_rx_stats;
}

void ath12k_sta_add_debugfs(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			    struct ieee80211_sta *sta, struct dentry *dir);
void
ath12k_accumulate_per_peer_tx_stats(struct ath12k_sta *arsta,
				    struct ath12k_per_peer_tx_stats *peer_stats,
				    u8 legacy_rate_idx);
void ath12k_update_per_peer_stats_from_txcompl(struct ath12k *ar,
					       struct sk_buff *msdu,
					       struct hal_tx_status *ts);
#else
static inline int ath12k_debug_soc_create(struct ath12k_base *ab)
{
	return 0;
}

static inline void ath12k_debug_soc_destroy(struct ath12k_base *ab)
{
}

static inline int ath12k_debug_pdev_create(struct ath12k_base *ab)
{
	return 0;
}

static inline void ath12k_debug_pdev_destroy(struct ath12k_base *ab)
{
}

static inline int ath12k_debug_register(struct ath12k *ar)
{
	return 0;
}

static inline void ath12k_debug_unregister(struct ath12k *ar)
{
}

static inline void ath12k_dbg_htt_ext_stats_handler(struct ath12k_base *ab,
						    struct sk_buff *skb)
{
}

static inline void ath12k_debug_fw_stats_process(struct ath12k_base *ab,
						 struct sk_buff *skb)
{
}

static inline void ath12k_debug_fw_stats_init(struct ath12k *ar)
{
}

static inline int ath12k_debug_is_extd_tx_stats_enabled(struct ath12k *ar)
{
	return 0;
}

static inline int ath12k_debug_is_extd_rx_stats_enabled(struct ath12k *ar)
{
	return 0;
}

static inline int ath12k_dbg_htt_stats_req(struct ath12k *ar)
{
	return 0;
}

static inline bool ath12k_debug_is_pktlog_lite_mode_enabled(struct ath12k *ar)
{
	return false;
}

static inline bool ath12k_debug_is_pktlog_rx_stats_enabled(struct ath12k *ar)
{
	return false;
}

static inline bool ath12k_debug_is_pktlog_peer_valid(struct ath12k *ar, u8 *addr)
{
	return false;
}

/*
static inline void
ath12k_accumulate_per_peer_tx_stats(struct ath12k_sta *arsta,
				    struct ath12k_per_peer_tx_stats *peer_stats,
				    u8 legacy_rate_idx)
{
}
*/
static inline void
ath12k_update_per_peer_stats_from_txcompl(struct ath12k *ar,
					  struct sk_buff *msdu,
					  struct hal_tx_status *ts)
{
}

#endif /* CONFIG_MAC80211_DEBUGFS*/

#define ath12k_dbg(ab, dbg_mask, fmt, ...) do {       \
	    if (ath12k_debug_mask & dbg_mask)		      \
		    ath12k_info(ab, fmt, ##__VA_ARGS__);      \
    } while (0)


#define ath12k_dbg_dump(ab, dbg_mask, msg, buf, len, max_len) do {  \
	    if (ath12k_debug_mask & dbg_mask)                           \
           pr_array(msg, (u8 *)buf, len, max_len);                  \
    } while (0)

#define ath12k_trace(ab, dbg_mask, fmt, ...) (void)sizeof(u8)

#endif /* _ATH12K_DEBUG_H_ */
