					...
					gfp_t gfp)
{
	...
	ret = dma_map_single(ab->dev, ptr_aligned, ring->buf_sz,
			       DMA_FROM_DEVICE, &paddr, &paddr_mapping_addr);
	if (ret)
		goto err;

	spin_lock_bh(&ring->idr_lock);
	buf_id = idr_alloc(&ring->bufs_idr, buff, 0, ring->bufs_max);
	...
	buff->paddr = paddr;
	buff->paddr_mapping_addr = paddr_mapping_addr;
...
err_idr_remove:
	...
	spin_unlock_bh(&ring->idr_lock);
err_dma_unmap:
	dma_unmap_single(ab->dev, paddr, paddr_mapping_addr, ring->buf_sz,
			 DMA_FROM_DEVICE);
err:
	ath12k_hal_srng_access_end(ab, srng);
	return ret;
				       ...
				       struct ath12k_dbring_buf_release_event *ev)
{
	...
	pdev_idx = (u8)le32_to_cpu(ev->fixed.pdev_id);
	...
	while (num_buff_reaped < num_entry) {
		...
		dma_unmap_single(ab->dev, buff->paddr, buff->paddr_mapping_addr, ring->buf_sz,
				 DMA_FROM_DEVICE);
...
void ath12k_dbring_buf_cleanup(struct ath12k *ar, struct ath12k_dbring *ring)
{
	...
	spin_lock_bh(&ring->idr_lock);
	idr_for_each_entry(&ring->bufs_idr, iter, buf_id, buff) {
		idr_remove(&ring->bufs_idr, buf_id);
		dma_unmap_single(ar->ab->dev, buff->paddr, buff->paddr_mapping_addr,
