...
static int ath12k_dp_rx_pdev_srng_alloc(struct ath12k *ar)
{
	...
	if (!ar->ab->hw_params->rxdma1_enable) {
		...
		//init_delayed_work(&ar->ab->mon_reap_work, ath12k_dp_service_mon_ring, ab);
		return 0;
	}
...
void ath12k_dp_rx_reo_cmd_list_cleanup(struct ath12k_base *ab)
{
	...
	list_for_each_entry_safe(cmd, tmp, &dp->reo_cmd_list, struct ath12k_dp_rx_reo_cmd, list) {
		list_del(&cmd->list);
	...
	list_for_each_entry_safe(cmd_cache, tmp_cache,
				 &dp->reo_cmd_cache_flush_list, struct ath12k_dp_rx_reo_cache_flush_elem, list) {
				      ...
				      struct ath12k_dp_rx_tid *rx_tid)
{
	...
	while (tot_desc_sz > desc_sz) {
		...
		cmd.addr_hi = (u8)upper_32_bits(rx_tid->paddr);
				      ...
				      enum hal_reo_cmd_status status)
{
	...
	list_for_each_entry_safe(elem, tmp, &dp->reo_cmd_cache_flush_list,
				 struct ath12k_dp_rx_reo_cache_flush_elem, list) {
...
void ath12k_peer_frags_flush(struct ath12k *ar, struct ath12k_peer *peer)
{
	struct ath12k_dp_rx_tid *rx_tid;
	int i;
					 ...
					 bool update_ssn)
{
	...
	cmd.ba_window_size = (u16)ba_win_sz;
				...
				enum hal_pn_type pn_type)
{
	...
	addr_aligned = (struct hal_rx_reo_queue *)PTR_ALIGN(vaddr, HAL_LINK_DESC_ALIGN);
...
{
	struct ath12k_base *ab = ar->ab;
	struct ath12k_sta *arsta = ieee80211_sta_get_priv_data(params->sta);
	int vdev_id = arsta->arvif->vdev_id;
	int ret;

	ret = ath12k_dp_rx_peer_tid_setup(ar, ieee80211_sta_addr(params->sta), vdev_id,
			    ...
			    struct ieee80211_ampdu_params *params)
{
	...
	peer = ath12k_peer_find(ab, vdev_id, ieee80211_sta_addr(params->sta));
	if (!peer) {
			   ...
			   void *data)
{
	...
	while (len > 0) {
		...
		tlv_tag = (u16)u32_get_bits(tlv->header, HTT_TLV_TAG);
		tlv_len = (u16)u32_get_bits(tlv->header, HTT_TLV_LEN);
				...
				struct htt_ppdu_stats *ppdu_stats, u8 user)
{
	...
	if (usr_stats->tlv_flags &
		...
		succ_pkts = (u16)u32_get_bits(usr_stats->ack_ba.info,
					 HTT_PPDU_STATS_ACK_BA_INFO_NUM_MSDU_M);
		tid = (u8)u32_get_bits(usr_stats->ack_ba.info,
	...
	case WMI_RATE_PREAMBLE_HE:
		...
		arsta->txrate.he.gi = ath12k_he_gi_to_nl80211_he_gi(sgi);
		v = ath12k_he_ru_tones_to_nl80211_he_ru_alloc((user_rate->ru_end -
							       user_rate->ru_start) + 1);
		arsta->txrate.he.ru_alloc = v;
		break;
	}
	...
	if (tid < HTT_PPDU_STATS_NON_QOS_TID) {
		...
		peer_stats->ba_fails =
			(u8)(HTT_USR_CMPLTN_LONG_RETRY(usr_stats->cmpltn_cmn.flags) +
				      ...
				      struct sk_buff *skb)
{
	...
	pdev_id = (u8)u32_get_bits(msg->info, HTT_T2H_PPDU_STATS_INFO_PDEV_ID);
	ppdu_id = msg->ppdu_id;
...
static void ath12k_htt_pktlog(struct ath12k_base *ab, struct sk_buff *skb)
{
	...
	pdev_id = (u8)u32_get_bits(data->hdr, HTT_T2H_PPDU_STATS_INFO_PDEV_ID);
	ar = ath12k_mac_get_ar_by_pdev_id(ab, pdev_id);
	if (!ar) {
						  ...
						  struct sk_buff *skb)
{
	...
	pdev_id = (u8)u32_get_bits(*data, HTT_BACKPRESSURE_EVENT_PDEV_ID_M);
	ring_type = (u8)u32_get_bits(*data, HTT_BACKPRESSURE_EVENT_RING_TYPE_M);
	ring_id = (u8)u32_get_bits(*data, HTT_BACKPRESSURE_EVENT_RING_ID_M);
	++data;

	hp = (u16)u32_get_bits(*data, HTT_BACKPRESSURE_EVENT_HP_M);
	tp = (u16)u32_get_bits(*data, HTT_BACKPRESSURE_EVENT_TP_M);
	++data;
						...
						struct sk_buff *skb)
{
	...
	pdev_id = (u8)u32_get_bits(__le32_to_cpu(msg->info),
				       ...
				       struct sk_buff *skb)
{
	...
	case HTT_T2H_MSG_TYPE_VERSION_CONF:
		dp->htt_tgt_ver_major = (u8)u32_get_bits(resp->u.version_msg.version,
						     HTT_T2H_VERSION_CONF_MAJOR);
		dp->htt_tgt_ver_minor = (u8)u32_get_bits(resp->u.version_msg.version,
						     HTT_T2H_VERSION_CONF_MINOR);
		complete(&dp->htt_tgt_version_received);
		break;
	/* TODO: remove unused peer map versions after testing */
	case HTT_T2H_MSG_TYPE_PEER_MAP:
		vdev_id = (u8)u32_get_bits(resp->u.peer_map_ev.info,
				       HTT_T2H_PEER_MAP_INFO_VDEV_ID);
		peer_id = (u16)u32_get_bits(resp->u.peer_map_ev.info,
				       HTT_T2H_PEER_MAP_INFO_PEER_ID);
		peer_mac_h16 = (u16)u32_get_bits(resp->u.peer_map_ev.info1,
					    HTT_T2H_PEER_MAP_INFO1_MAC_ADDR_H16);
		ath12k_dp_get_mac_addr(resp->u.peer_map_ev.mac_addr_l32,
	...
	case HTT_T2H_MSG_TYPE_PEER_MAP2:
		...
		ath12k_dp_get_mac_addr(resp->u.peer_map_ev.mac_addr_l32,
				       peer_mac_h16, mac_addr);
		ast_hash = (u16)u32_get_bits(resp->u.peer_map_ev.info2,
					HTT_T2H_PEER_MAP_INFO2_AST_HASH_VAL);
		hw_peer_id = (u16)u32_get_bits(resp->u.peer_map_ev.info1,
	...
	case HTT_T2H_MSG_TYPE_PEER_UNMAP2:
		peer_id = (u16)u32_get_bits(resp->u.peer_unmap_ev.info,
				      ...
				      u8 l3pad_bytes, int msdu_len)
{
	...
	while ((skb = __skb_dequeue(msdu_list)) != NULL && rem_len > 0) {
		...
		if (buf_len > (int)(DP_RX_BUFFER_SIZE - hal_rx_desc_sz)) {
			WARN_ON_ONCE(1);
			dev_kfree_skb_any(skb);
			return -EINVAL;
		}
...
static u32 ath12k_dp_rx_crypto_param_len(struct ath12k *ar,
					 enum hal_encrypt_type enctype)
{
	switch (enctype) {
	case HAL_ENCRYPT_TYPE_OPEN:
					      ...
					      enum hal_encrypt_type enctype)
{
	...
	if (rxcb->is_mcbc)
		status->flag &= ~IEEE80211_RX_FLAG_PN_VALIDATED;
	...
	if (ieee80211_is_data_qos(hdr->frame_control)) {
		...
		/* TODO: Add other QoS ctl fields when required */
		memcpy(msdu->data + (hdr_len - IEEE80211_QOS_LEN),
		       &qos_ctl, IEEE80211_QOS_LEN);
	}
}
			 ...
			 struct ieee80211_rx_status *rx_status)
{
	...
	channel_num = (u8)meta_data;
	center_freq = (u16)(meta_data >> 16);

	if (center_freq >= ATH12K_MIN_6G_FREQ && center_freq <= ATH12K_MAX_6G_FREQ) {
		...
		spin_unlock_bh(&ar->data_lock);
		ath12k_dbg_dump(ar->ab, ATH12K_DBG_DATA, "rx_desc: ",
				rx_desc, sizeof(struct hal_rx_desc), 0);
	}

	if (rx_status->band != NL80211_BAND_6GHZ) {
		rx_status->freq = (u16)ieee80211_channel_to_frequency(channel_num,
								 rx_status->band);
	}
				      ...
				      struct ieee80211_rx_status *status)
{
	...
	ath12k_dbg(ab, ATH12K_DBG_DATA,
		   "is_amsdu %x is_mcbc %x enctype %d decap %d rx skb 0x%p len %u peer %p tid %d %a sn %u %a%a%a%a%a%a%a%a rate_idx %u vht_nss %u freq %u band %u flag 0x%x fcs-err %i mic-err %i amsdu-more %i\n",
		   is_amsdu,
		   is_mcbc,
		   enctype,
		   decap,
	...
	*rx_status = *status;
	...
	if (decap != DP_RX_DECAP_TYPE_ETHERNET2_DIX ||
		...
		ath12k_dbg_dump(ab, ATH12K_DBG_PKT, "rx 802.11 data ", msdu->data, msdu->len, 0);
		ieee80211_rx_napi(ar->hw, NULL, msdu);
	}
	else {
		ath12k_dbg_dump(ab, ATH12K_DBG_PKT, "rx 802.3 data ", msdu->data, msdu->len, 0);
				     ...
				     struct ieee80211_rx_status *rx_status)
{
	...
	if (rxcb->is_frag) {
		...
		if ((msdu_len + hal_rx_desc_sz) > DP_RX_BUFFER_SIZE) {
			...
			ath12k_dbg_dump(ab, ATH12K_DBG_DATA, "", rx_desc,
					sizeof(struct hal_rx_desc), 0);
			goto free_out;
		}
...
try_again:
	...
	while ((desc = ath12k_hal_srng_dst_get_next_entry(ab, srng)) != NULL) {
		...
		mac_id = (u8)le32_get_bits(desc->info0,
		...
		rxcb->mac_id = mac_id;
		rxcb->peer_id = (u16)le32_get_bits(desc->rx_mpdu_info.peer_meta_data,
					      RX_MPDU_DESC_META_DATA_PEER_ID);
		rxcb->tid = (u8)le32_get_bits(desc->rx_mpdu_info.info0,
...
static void ath12k_dp_rx_frag_timer(void *ctx)
{
	struct ath12k_dp_rx_tid *rx_tid = ctx;
...
int ath12k_dp_rx_peer_frag_setup(struct ath12k *ar, const u8 *peer_mac, int vdev_id)
{
	...
	for (i = 0; i <= IEEE80211_NUM_TIDS; i++) {
		...
		init_delayed_work(&rx_tid->frag_timer, ath12k_dp_rx_frag_timer, rx_tid);
					  ...
					  struct sk_buff *msdu)
{
	...
	hdr_len = ieee80211_hdrlen(hdr->frame_control);
	head_len = hdr_len + hal_rx_desc_sz + IEEE80211_TKIP_IV_LENGTH;
				 ...
				 struct sk_buff **defrag_skb)
{
	...
	if (first_frag == NULL) {
		ath12k_warn(ab, "first_frag is NULL\n");
		return 0;
	}
					      ...
					      struct sk_buff *defrag_skb)
{
	...
	dst_ind = (u8)u32_get_bits(msdu_ext_info, RX_MSDU_EXT_DESC_INFO0_REO_DEST_IND);
	...
	ath12k_dp_rxdesc_set_msdu_len(ab, rx_desc, (u16)(defrag_skb->len - hal_rx_desc_sz));
	...
	ath12k_hal_rx_buf_addr_info_set(&msdu0->buf_addr_info, paddr,
					desc_info->cookie,
	...
	ath12k_hal_rx_buf_addr_info_set(&reo_ent_ring->buf_addr_info, paddr,
					cookie,
				    ...
				    struct hal_reo_dest_ring *ring_desc)
{
	...
	if (!rx_tid->last_frag_no ||
		...
		queue_delayed_work(&rx_tid->frag_timer, ATH12K_DP_RX_FRAGMENT_TIMEOUT_MS);
		goto out_unlock;
	}
			     ...
			     int budget)
{
	...
	while (budget &&
	       ((reo_desc = ath12k_hal_srng_dst_get_next_entry(ab, srng)) != NULL)) {
		...
		link_desc_va = 
			(struct hal_rx_msdu_link *)((u8*)link_desc_banks[desc_bank].vaddr +
				(paddr - link_desc_banks[desc_bank].paddr));
				      ...
				      struct sk_buff_head *msdu_list)
{
	...
	if (!rxcb->is_frag && ((msdu_len + hal_rx_desc_sz) > DP_RX_BUFFER_SIZE)) {
		...
		msdu_len = (u16)(msdu_len - (DP_RX_BUFFER_SIZE - hal_rx_desc_sz));
...
void ath12k_dp_rx_process_reo_status(struct ath12k_base *ab)
{
	...
	while ((hdr = ath12k_hal_srng_dst_get_next_entry(ab, srng)) != NULL) {
		tag = (u16)u64_get_bits(hdr->tl, HAL_SRNG_TLV_HDR_TAG);
...
int ath12k_dp_rxdma_ring_sel_config_qcn9274(struct ath12k_base *ab)
{
	...
	tlv_filter.rx_packet_offset = (u16)hal_rx_desc_sz;

	tlv_filter.rx_mpdu_start_offset =
		(u16)(ab->hw_params->hal_ops->rx_desc_get_mpdu_start_offset());
	tlv_filter.rx_msdu_end_offset =
		(u16)(ab->hw_params->hal_ops->rx_desc_get_msdu_end_offset());
