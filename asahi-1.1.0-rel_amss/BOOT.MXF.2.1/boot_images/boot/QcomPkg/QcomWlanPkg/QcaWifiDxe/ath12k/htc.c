...
struct sk_buff *ath12k_htc_alloc_skb(struct ath12k_base *ab, int size)
{
	...
	skb = dev_alloc_skb(ab->dev, size + sizeof(struct ath12k_htc_hdr));
	if (!skb)
		return NULL;
...
static struct sk_buff *ath12k_htc_build_tx_ctrl_skb(struct ath12k_base *ab)
{
	struct sk_buff *skb;
	struct ath12k_skb_cb *skb_cb;

	skb = dev_alloc_skb(ab->dev, ATH12K_HTC_CONTROL_BUFFER_SIZE);
	if (!skb)
		return NULL;
				      ...
				      struct sk_buff *skb)
{
	...
	eid = (u8)le32_get_bits(hdr->htc_info, HTC_HDR_ENDPOINTID);
	...
	payload_len = (u16)le32_get_bits(hdr->htc_info, HTC_HDR_PAYLOADLEN);
...
int ath12k_htc_wait_target(struct ath12k_htc *htc)
{
	...
	if (!time_left) {
		...
		for (i = 0; i < ab->hw_params->ce_count; i++)
			ath12k_ce_per_engine_service(htc->ab, (u16)i);
	...
	message_id = (u16)le32_get_bits(ready->id_credit_count, HTC_MSG_MESSAGEID);
	credit_count = (u16)le32_get_bits(ready->id_credit_count,
				     HTC_READY_MSG_CREDITCOUNT);
	credit_size = (u16)le32_get_bits(ready->size_ep, HTC_READY_MSG_CREDITSIZE);
			       ...
			       struct ath12k_htc_svc_conn_resp *conn_resp)
{
	...
	message_id = (u16)le32_get_bits(resp_msg->msg_svc_id, HTC_MSG_MESSAGEID);
	service_id = (u16)le32_get_bits(resp_msg->msg_svc_id,
	...
	ath12k_dbg(ab, ATH12K_DBG_HTC,
		   "HTC Service %a connect response: status: %u, assigned ep: %u\n",
	...
	conn_resp->connect_resp_code = 
		(u8)le32_get_bits(resp_msg->flags_len,
