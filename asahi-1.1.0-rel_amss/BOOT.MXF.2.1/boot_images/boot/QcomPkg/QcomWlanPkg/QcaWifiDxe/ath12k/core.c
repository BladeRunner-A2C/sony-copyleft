...
unsigned int ath12k_debug_mask = (unsigned int)ATH12K_DBG_ANY;
...
static int ath12k_core_soc_create(struct ath12k_base *ab)
{
	...
	//Poll to trigger the interrupt
	time_left = wait_for_completion_timeout(&ab->hw_mode_ready, 30000);
	if (!time_left) {
        ath12k_err(ab, "wait HW mode ready timeout\n");
		ret = -ETIMEDOUT;
			     ...
			     enum ath12k_firmware_mode mode)
{
	...
	ret = ath12k_wmi_diag_connect(ab);
	if (ret) {
		ath12k_err(ab, "failed to connect diag: %d\n", ret);
		goto err_hif_stop;
	}
...
static int ath12k_core_send_default_sar_limit(struct ath12k_base *ab)
{
#define DEFAULT_SAR_LIMIT_NUM_ROWS	2
	...
	for (i = 0; i < (int)ab->num_radios; i++) {
		ar = ab->pdevs[i].ar;
		ret = ath12k_wmi_send_sar_limit(ar, &sar_limit);
	}
...
int ath12k_core_qmi_firmware_ready(struct ath12k_base *ab)
{
	...
	ret = ath12k_core_send_default_sar_limit(ab);
	if (ret) {
		ath12k_err(ab, "failed to send default sar limit: %d\n", ret);
		goto err_core_stop;
	}
...
static void ath12k_update_11d(void *context)
{
	struct ath12k_base *ab = context;
	struct ath12k *ar;
	struct ath12k_pdev *pdev;
	struct wmi_set_current_country_params set_current_param = { 0, };
	u8 i;
	int ret;
...
static void ath12k_core_restart(void *context)
{
	struct ath12k_base *ab = context;
	int ret;
...
static void ath12k_core_reset(void *context)
{
	struct ath12k_base *ab = context;
#if 0
...
struct ath12k_base *ath12k_core_alloc(pci_dev_handle dev, size_t priv_size,
				      enum ath12k_bus bus)
{
	...
	if (cfg_is_ath_debug_mask_valid())
		ath12k_debug_mask = cfg_get_ath_debug_mask();
	...
	init_work(&ab->restart_work, ath12k_core_restart, ab);
	init_work(&ab->update_11d_work, ath12k_update_11d, ab);
	init_work(&ab->reset_work, ath12k_core_reset, ab);
	//timer_setup(&ab->rx_replenish_retry, ath12k_ce_rx_replenish_retry, 0);
	init_delayed_work(&ab->rx_replenish_retry, ath12k_ce_rx_replenish_retry, ab);
