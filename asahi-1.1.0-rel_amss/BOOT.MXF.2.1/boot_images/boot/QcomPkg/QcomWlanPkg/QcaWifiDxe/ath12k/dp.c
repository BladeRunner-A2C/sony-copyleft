...
void ath12k_dp_srng_cleanup(struct ath12k_base *ab, struct dp_srng *ring)
{
	...
	dma_free_coherent(ab->dev, ring->size, ring->vaddr_unaligned,
			  ring->paddr_unaligned, ring->paddr_mapping_addr);
				     ...
				     enum hal_ring_type type, int ring_num)
{
	...
	if (msi_group_number > (int)msi_data_count) {
		ath12k_dbg(ab, ATH12K_DBG_PCI,
			 ...
			 int mac_id, int num_entries)
{
	...
	ring->vaddr_unaligned = dma_alloc_coherent(ab->dev, ring->size,
						   ...
						   &ring->paddr_mapping_addr);
	if (!ring->vaddr_unaligned)
		return -ENOMEM;

	ring->vaddr = (u32 *)PTR_ALIGN(ring->vaddr_unaligned, HAL_RING_BASE_ALIGN);
	ring->paddr = ring->paddr_unaligned + (dma_addr_t)(ring->vaddr -
...
inc_ref_and_return:
	...
	if (configure_register)
		ath12k_hal_tx_configure_bank_register(ab, bank_config, (u8)bank_id);
...
static int ath12k_dp_init_bank_profiles(struct ath12k_base *ab)
{
	...
	dp->num_bank_profiles = (u8)num_tcl_banks;
	dp->bank_profiles = kcalloc(num_tcl_banks,
...
static void ath12k_dp_scatter_idle_link_desc_cleanup(struct ath12k_base *ab)
{
	...
	for (i = 0; i < DP_IDLE_SCATTER_BUFS_MAX; i++) {
		...
		dma_free_coherent(ab->dev, HAL_WBM_IDLE_SCATTER_BUF_SIZE_MAX,
				  slist[i].vaddr, slist[i].paddr, slist[i].dma_mapping);
		slist[i].vaddr = NULL;
	}
}
						  ...
						  u32 last_bank_sz)
{
	...
	u32 num_scatter_buf, scatter_idx;
	...
	for (i = 0; i < num_scatter_buf; i++) {
		slist[i].vaddr = dma_alloc_coherent(ab->dev,
						    ...
						    &slist[i].paddr, &slist[i].dma_mapping);
		if (!slist[i].vaddr) {
			ret = -ENOMEM;
			goto err;
		}
	...
	for (i = 0; i < n_link_desc_bank; i++) {
		align_bytes = (u32)((u8*)link_desc_banks[i].vaddr -
			      (u8*)link_desc_banks[i].vaddr_unaligned);
	...
	end_offset = (u32)((scatter_buf - slist[scatter_idx].vaddr) *
	...
	for (i = 0; i < DP_LINK_DESC_BANKS_MAX; i++) {
		if (link_desc_banks[i].vaddr_unaligned) {
			dma_free_coherent(ab->dev,
					  ...
					  link_desc_banks[i].paddr_mapping_addr);
	...
	for (i = 0; i < n_link_desc_bank; i++) {
		...
		desc_bank[i].vaddr_unaligned =
					dma_alloc_coherent(ab->dev, desc_sz,
							   ...
							   &desc_bank[i].paddr_mapping_addr);
		...
		desc_bank[i].paddr = desc_bank[i].paddr_unaligned +
				     ((u8*)desc_bank[i].vaddr -
				      (u8*)desc_bank[i].vaddr_unaligned);
			      ...
			      u32 n_link_desc)
{
	...
	for (i = 0; i < n_link_desc_bank; i++) {
		...
		paddr = link_desc_banks[i].paddr;
		while (n_entries &&
		       ((desc = ath12k_hal_srng_src_get_next_entry(ab, srng)) != NULL)) {
			cookie = DP_LINK_DESC_COOKIE_SET(n_entries, i);
...
static void ath12k_dp_cc_cleanup(struct ath12k_base *ab)
{
	...
	list_for_each_entry_safe(desc_info, tmp, &dp->rx_desc_used_list, struct ath12k_rx_desc_info, list) {
	...
	for (i = 0; i < dp->num_spt_pages; i++) {
		...
		dma_free_coherent(ab->dev, ATH12K_PAGE_SIZE,
				  dp->spt_info[i].vaddr, dp->spt_info[i].paddr, dp->spt_info[i].paddr_mapping_addr);
...
static void ath12k_dp_reoq_lut_cleanup(struct ath12k_base *ab)
{
	...
	dma_free_coherent(ab->dev, DP_REOQ_LUT_SIZE,
			  dp->reoq_lut.vaddr, dp->reoq_lut.paddr, dp->reoq_lut.paddr_mapping_addr);
...
{
	u32 cmem_base = (u32)ab->qmi.dev_mem[ATH12K_QMI_DEVMEM_CMEM_INDEX].start;
						  ...
						  u32 cookie)
{
	...
	ppt_idx = (u16)u32_get_bits(cookie, ATH12K_DP_CC_COOKIE_PPT);
	spt_idx = (u16)u32_get_bits(cookie, ATH12k_DP_CC_COOKIE_SPT);
...
static int ath12k_dp_cc_init(struct ath12k_base *ab)
{
	...
	list_init_head(&dp->rx_desc_free_list);
	list_init_head(&dp->rx_desc_used_list);
	spin_lock_init(&dp->rx_desc_lock);

	for (i = 0; i < ATH12K_HW_MAX_QUEUES; i++) {
		list_init_head(&dp->tx_desc_free_list[i]);
		list_init_head(&dp->tx_desc_used_list[i]);
	...
	cmem_base = (u32)ab->qmi.dev_mem[ATH12K_QMI_DEVMEM_CMEM_INDEX].start;

	for (i = 0; i < dp->num_spt_pages; i++) {
		dp->spt_info[i].vaddr = dma_alloc_coherent(ab->dev,
							   ATH12K_PAGE_SIZE,
							   &dp->spt_info[i].paddr,
							    &dp->spt_info[i].paddr_mapping_addr);
		...
		ath12k_hif_write32(ab, cmem_base + ATH12K_PPT_ADDR_OFFSET(i),
				   (u32)(dp->spt_info[i].paddr >> ATH12K_SPT_4K_ALIGN_OFFSET));
	}
...
static int ath12k_dp_reoq_lut_setup(struct ath12k_base *ab)
{
	...
	dp->reoq_lut.vaddr = dma_alloc_coherent(ab->dev,
						DP_REOQ_LUT_SIZE,
						&dp->reoq_lut.paddr,
						&dp->reoq_lut.paddr_mapping_addr);
...
int ath12k_dp_alloc(struct ath12k_base *ab)
{
	...
	list_init_head(&dp->reo_cmd_list);
	list_init_head(&dp->reo_cmd_cache_flush_list);
