					     ...
					     struct hal_tx_info *ti)
{
	...
	tcl_ext_cmd->info0 = le32_encode_bits((u32)ti->paddr,
		 ...
		 struct sk_buff *skb)
{
	...
	dma_addr_t paddr_ext_desc = 0;
	void *paddr_mapping_ext_desc = NULL;
...
tcl_ring_sel:
    ...
    if (!ti.paddr) {
        ath12k_warn(ab, "skb %p failed to DMA map data Tx buffer\n", skb);
        ret = -ENOMEM;
        goto fail_remove_tx_buf;
    }
	...
	if (msdu_ext_desc) {
		skb_ext_desc = dev_alloc_skb(ab->dev, sizeof(struct hal_tx_msdu_ext_desc));
		if (!skb_ext_desc) {
			ret = -ENOMEM;
			goto fail_unmap_dma;
		}
		...
		ret = dma_map_single(ab->dev, skb_ext_desc->data,
					  skb_ext_desc->len, DMA_TO_DEVICE,
					  &paddr_ext_desc, &paddr_mapping_ext_desc);
		if (ret) {
			kfree(skb_ext_desc);
			goto fail_unmap_dma;
		}
		skb_cb->paddr_ext_desc = paddr_ext_desc;
		skb_cb->paddr_mapping_ext_desc = paddr_mapping_ext_desc;
...
fail_unmap_dma:
	dma_unmap_single(ab->dev, skb_cb->paddr_ext_desc, skb_cb->paddr_mapping_ext_desc,
				 ...
				 struct ath12k_dp_htt_wbm_tx_status *ts)
{
	...
	if (ts->acked) {
		if (!(info->flags & IEEE80211_TX_CTL_NO_ACK)) {
			...
			info->u.status.ack_signal = ATH12K_DEFAULT_NOISE_FLOOR +
						  ts->ack_rssi;
				     ...
				     struct dp_tx_ring *tx_ring)
{
	...
	status_desc = (struct htt_tx_wbm_completion *)
		((u8*)desc + HTT_TX_WBM_COMP_STATUS_OFFSET);
...
void ath12k_dp_tx_completion_handler(struct ath12k_base *ab, int ring_id)
{
	...
	while (ATH12K_TX_COMPL_NEXT(tx_ring->tx_status_tail) != tx_ring->tx_status_head) {
		...
		if (le32_get_bits(tx_status->info0, HAL_WBM_COMPL_TX_INFO0_CC_DONE)) {
			...
			tx_desc = (struct ath12k_tx_desc_info *)(desc_va);
		} else {
