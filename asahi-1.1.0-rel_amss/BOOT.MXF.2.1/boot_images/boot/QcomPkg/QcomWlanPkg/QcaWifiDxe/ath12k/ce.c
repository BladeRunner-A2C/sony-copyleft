...
#define ATH12K_CE_RX_BUFFER_PENDING_SIZE 128
...
static int ath12k_ce_rx_post_pipe(struct ath12k_ce_pipe *pipe)
{
	...
	while (pipe->rx_buf_needed) {
        ...
        if (!paddr) {
            ath12k_warn(ab, "skb %p failed to dma map ce rx buf\n", skb);
...
static int ath12k_ce_recv_process_cb(struct ath12k_ce_pipe *pipe)
{
	struct ath12k_base *ab = pipe->ab;
	struct sk_buff *skb;
...
static int ath12k_ce_send_done_cb(struct ath12k_ce_pipe *pipe)
{
	struct ath12k_base *ab = pipe->ab;
	struct sk_buff *skb;
	int skb_completed = 0;
	...
	while (!IS_ERR(skb = ath12k_ce_completed_send_next(pipe))) {
		...
		dma_unmap_single(ab->dev, ATH12K_SKB_CB(skb)->paddr, 
                 ATH12K_SKB_CB(skb)->paddr_mapping_addr,
...
int ath12k_ce_per_engine_service(struct ath12k_base *ab, u16 ce_id)
{
	struct ath12k_ce_pipe *pipe = &ab->ce.ce_pipe[ce_id];
	int handled = 0;
	...
	if (pipe->recv_cb)
		handled += ath12k_ce_recv_process_cb(pipe);

	return handled;
}

void ath12k_ce_poll_send_completed(struct ath12k_base *ab, u16 pipe_id)
{
...
void ath12k_ce_rx_replenish_retry(void *ctx)
{
	...
	for (i = 0; i < ab->hw_params->ce_count; i++) {
		...
		if (pipe->dest_ring) {
            ...
            sk_buff_pool_init(&pipe->rx_skb_pool, 
                pipe->dest_ring->nentries + ATH12K_CE_RX_BUFFER_PENDING_SIZE,
                pipe->buf_sz, ab->dev);
...
int ath12k_ce_get_attr_flags(struct ath12k_base *ab, u16 ce_id)
{
