...
struct ath12k_peer *ath12k_peer_find(struct ath12k_base *ab, u32 vdev_id,
				     const u8 *addr)
{
	...
	list_for_each_entry(peer, &ab->peers, struct ath12k_peer, list) {
...
static int ath12k_peer_find_mapping(
    struct ath12k_base *ab, int vdev_id,
				       ...
				       const u8 *addr, bool expect_mapped)
{
	...
	wait_event_timeout(ab->peer_mapping_wq, 
        ((mapping_result = ath12k_peer_find_mapping(ab, vdev_id, addr, expect_mapped)) > 0), 
        3 * HZ, ret);
