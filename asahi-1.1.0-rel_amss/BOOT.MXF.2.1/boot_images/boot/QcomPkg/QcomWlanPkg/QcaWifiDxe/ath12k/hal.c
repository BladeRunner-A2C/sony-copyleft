...
{
	return (u8)le16_get_bits(desc->u.qcn9274.msdu_end.info5,
...
{
	return (u8)le32_get_bits(desc->u.qcn9274.msdu_end.info11,
...
static int ath12k_hal_srng_create_config_qcn9274(struct ath12k_base *ab)
{
	...
	s->reg_size[0] = (u16)(HAL_WBM_SW1_RELEASE_RING_BASE_LSB(ab) -
			 HAL_WBM_SW_RELEASE_RING_BASE_LSB(ab));
...
{
	return (u8)le16_get_bits(desc->u.wcn7850.msdu_end.info5,
			    RX_MSDU_END_INFO5_L3_HDR_PADDING);
}
...
{
	return (u8)le32_get_bits(desc->u.wcn7850.msdu_end.info11,
			     RX_MSDU_END_INFO11_DECAP_FORMAT);
}
...
{
	return (u16)le32_get_bits(desc->u.wcn7850.mpdu_start.info4,
			     RX_MPDU_START_INFO4_MPDU_SEQ_NUM);
}

static u16 ath12k_hw_wcn7850_rx_desc_get_msdu_len(struct hal_rx_desc *desc)
{
	return (u16)le32_get_bits(desc->u.wcn7850.msdu_end.info10,
			     RX_MSDU_END_INFO10_MSDU_LENGTH);
}

static u8 ath12k_hw_wcn7850_rx_desc_get_msdu_sgi(struct hal_rx_desc *desc)
{
	return (u8)le32_get_bits(desc->u.wcn7850.msdu_end.info12,
			     RX_MSDU_END_INFO12_SGI);
}
...
{
	return (u32)le64_get_bits(desc->u.wcn7850.mpdu_start_tag,
			    HAL_TLV_HDR_TAG);
}
					...
					struct hal_srng *srng)
{
	...
	ath12k_hif_write32(ab, reg_base, (u32)(srng->ring_base_paddr));
				     ...
				     u32 end_offset)
{
	...
	link_addr = (void *)((u8*)sbuf[0].vaddr + HAL_WBM_IDLE_SCATTER_BUF_SIZE);
	...
	val = u32_encode_bits((u32)(sbuf[nsbufs - 1].paddr), BUFFER_ADDR_INFO0_ADDR);
	ath12k_hif_write32(ab,
			   HAL_SEQ_WCSS_UMAC_WBM_REG +
	...
	val = u32_encode_bits((u32)(sbuf[0].paddr), BUFFER_ADDR_INFO0_ADDR);
	ath12k_hif_write32(ab,
			   HAL_SEQ_WCSS_UMAC_WBM_REG +
			   HAL_WBM_SCATTERED_DESC_PTR_HEAD_INFO_IX0(ab),
			   val);
...
void ath12k_hal_dump_srng_stats(struct ath12k_base *ab)
{
	...
	DEBUG_PARAMETER(irq_grp);
	DEBUG_PARAMETER(ce_pipe);
	...
	for (i = 0; i < ab->hw_params->ce_count; i++) {
		...
		ath12k_err(ab, "CE_id %d pipe_num %d %u Jiffies before\n",
			   i, ce_pipe->pipe_num,
			   (jiffies - ce_pipe->timestamp));
	}

	ath12k_err(ab, "\nLast interrupt received for each group:\n");
	for (i = 0; i < ATH12K_EXT_IRQ_GRP_NUM_MAX; i++) {
		irq_grp = &ab->ext_irq_grp[i];
		ath12k_err(ab, "group_id %d %u Jiffies before\n",
			   irq_grp->grp_id,
			   (jiffies - irq_grp->timestamp));
	}
	...
	for (i = 0; i < HAL_SRNG_RING_ID_MAX; i++) {
		...
		if (srng->ring_dir == HAL_SRNG_DIR_SRC)
			ath12k_err(ab,
				   "src srng id %u hp %u, reap_hp %u, cur tp %u, cached tp %u last tp %u napi processed before %u Jiffies\n",
				   srng->ring_id, srng->u.src_ring.hp,
				   srng->u.src_ring.reap_hp,
				   *srng->u.src_ring.tp_addr, srng->u.src_ring.cached_tp,
				   srng->u.src_ring.last_tp,
				   (jiffies - srng->timestamp));
		else if (srng->ring_dir == HAL_SRNG_DIR_DST)
			ath12k_err(ab,
				   "dst srng id %u tp %u, cur hp %u, cached hp %u last hp %u napi processed before %u Jiffies\n",
