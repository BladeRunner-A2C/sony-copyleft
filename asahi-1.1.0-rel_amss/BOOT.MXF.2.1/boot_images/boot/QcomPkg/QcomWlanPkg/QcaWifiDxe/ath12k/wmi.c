			     ...
			     struct ath12k_wmi_resource_config_arg *config)
{
	...
	config->peer_map_unmap_version = 0;
		    ...
		    void *data)
{
	...
	while (len > 0) {
		...
		tlv = ptr;
		tlv_tag = (u16)le32_get_bits(tlv->header, WMI_TLV_TAG);
		tlv_len = (u16)le32_get_bits(tlv->header, WMI_TLV_LEN);
			...
			u32 cmd_id)
{
	...
	//might_sleep();
	ath12k_info(wmi_sc->ab, "skb 0x%p, cmd %a (0x%08x)\n",
        skb, cmd_id_to_string(cmd_id), cmd_id);
			
	wait_event_timeout(wmi_sc->tx_credits_wq, 
        ((ret = ath12k_wmi_cmd_send_nowait(wmi, skb, cmd_id)) != -EAGAIN), 
				      ...
				      struct ath12k_pdev *pdev)
{
	...
	//TODOTODO-zxue service ready ext2 support
	...
	ab->target_pdev_ids[ab->target_pdev_count].phy_id = mac_phy_caps->phy_id;
...
{
	struct ath12k_wmi_svc_ready_parse svc_ready = { 0, };
	int ret;
...
int ath12k_wmi_vdev_create(struct ath12k *ar, const u8 *macaddr,
...
int ath12k_wmi_vdev_delete(struct ath12k *ar, u32 vdev_id)
{
	struct ath12k_wmi_pdev *wmi = ar->wmi;
...
int ath12k_wmi_vdev_stop(struct ath12k *ar, u32 vdev_id)
{
	struct ath12k_wmi_pdev *wmi = ar->wmi;
...
int ath12k_wmi_vdev_down(struct ath12k *ar, u32 vdev_id)
{
	struct ath12k_wmi_pdev *wmi = ar->wmi;
			  ...
			  bool restart)
{
	...
	ath12k_dbg(ar->ab, ATH12K_DBG_WMI, "vdev %a id 0x%x freq 0x%x mode 0x%x\n",
...
int ath12k_wmi_pdev_set_ps_mode(struct ath12k *ar, u32 vdev_id, u32 enable)
{
	struct ath12k_wmi_pdev *wmi = ar->wmi;
			...
			struct sk_buff *bcn)
{
	...
	int ret;
	u32 len;
	u32 aligned_len = roundup(bcn->len, 4);
	...
	cmd->csa_switch_count_offset = cpu_to_le32(offs->csa_counter_offs[0]);
	cmd->ext_csa_switch_count_offset = cpu_to_le32(offs->csa_counter_offs[1]);
				...
				struct ath12k_wmi_scan_req_arg *arg)
{
	 ...
	 */
	ether_addr_copy(arg->bssid_list[0].addr, (const u8 *)"\xFF\xFF\xFF\xFF\xFF\xFF");
}
				   ...
				   struct ath12k_wmi_scan_req_arg *arg)
{
	...
	if (arg->extraie.len && arg->extraie.len <= 0xFFFF)
		extraie_len_with_pad =
			(u16)roundup(arg->extraie.len, sizeof(u32));
	...
	ptr = (u8*)ptr + (arg->num_ssids * sizeof(*ssid));
	...
	ptr = (u8*)ptr + arg->num_bssid * sizeof(*bssid);
...
int ath12k_wmi_send_sar_limit(struct ath12k *ar,
		struct wmi_sar_limit_cmd_params *sar_limit_params)
{
	...
	ath12k_dbg(ar->ab, ATH12K_DBG_WMI, "no of sar rows = %d, len = %d\n",
	...
	for (i = 0; i < (int)sar_limit_params->num_limit_rows; i++) {
		...
		ath12k_dbg(ar->ab, ATH12K_DBG_WMI, "row %d, band_id = %d, chain_id = %d, mod_id = %d, limit_value = %d, validity_bitmap = %d\n",
...
send_sar_limits:
	return ath12k_wmi_cmd_send(ar->wmi, skb, WMI_SAR_LIMITS_CMDID);
}
...
int ath12k_wmi_add_beacon_filter(struct ath12k *ar, u32 vdev_id, const u32 *ies, u32 ies_cnt)
{
...
#define WMI_BCN_FLT_MAX_ELEMS_IE_LIST    (WMI_BCN_FLT_MAX_SUPPORTED_IES/32)
    ...
    ath12k_dbg_dump(ar->ab, ATH12K_DBG_DATA, "beacon filter: ",
				p_ie_val, sizeof(u32)*WMI_BCN_FLT_MAX_ELEMS_IE_LIST, 0);
	
	ret = ath12k_wmi_cmd_send(wmi, skb, WMI_ADD_BCN_FILTER_CMDID);
...
int ath12k_wmi_remove_beacon_filter(struct ath12k *ar, u32 vdev_id)
{
	struct ath12k_wmi_pdev *wmi = ar->wmi;
	...
	cmd->vdev_id = vdev_id;
	ret = ath12k_wmi_cmd_send(wmi, skb, WMI_RMV_BCN_FILTER_CMDID);
...
int ath12k_wmi_set_beacon_filter(struct ath12k *ar, u32 vdev_id, const u32 *ies, u32 ies_cnt)
{
	int ret;

	if (ies == NULL || ies_cnt == 0) {
		ret = ath12k_wmi_remove_beacon_filter(ar, vdev_id);
	}
	else {
		ret = ath12k_wmi_add_beacon_filter(ar, vdev_id, ies, ies_cnt);
	}
	return ret;
}
			       ...
			       struct sk_buff *tmpl)
{
	...
	void *ptr;
	int ret;
	u32 len, aligned_len = roundup(tmpl->len, 4);
				...
				struct ath12k_wmi_init_cmd_arg *arg)
{
	...
	host_mem_chunks = 
		(struct ath12k_wmi_host_mem_chunk_params *)((u8*)ptr + TLV_HDR_SIZE);
...
int ath12k_wmi_cmd_init(struct ath12k_base *ab)
{
	...
	struct ath12k_wmi_init_cmd_arg arg = { 0, };

	ab->hw_params->wmi_init(ab, &arg.res_cfg);
	arg.num_mem_chunks = (u8)wmi_sc->num_mem_chunks;
...
static void ath12k_wmi_pdev_dma_ring_buf_release_event(struct ath12k_base *ab,
						       struct sk_buff *skb)
{
	struct ath12k_wmi_dma_buf_release_arg arg = { 0, };
						 ...
						 void *data)
{
	struct ath12k_wmi_pdev *wmi_handle = &soc->wmi_ab.wmi[0];
	struct ath12k_wmi_svc_rdy_ext_parse *svc_rdy_ext = data;
	u32 hw_mode_id = le32_to_cpu(svc_rdy_ext->pref_hw_mode_caps.hw_mode_id);
	u32 phy_id_map;
	int pdev_index = 0;
	int ret;
...
static int ath12k_service_ready_ext_event(struct ath12k_base *ab,
					  struct sk_buff *skb)
{
	struct ath12k_wmi_svc_rdy_ext_parse svc_rdy_ext = { 0, };
	int ret;
			       ...
			       struct ath12k_wmi_reg_rule_ext_params *wmi_reg_rule)
{
	...
	for (count = 0; count < num_reg_rules; count++) {
		reg_rule_ptr[count].start_freq =
			(u16)le32_get_bits(wmi_reg_rule[count].freq_info,
				      REG_RULE_START_FREQ);
		reg_rule_ptr[count].end_freq =
			(u16)le32_get_bits(wmi_reg_rule[count].freq_info,
				      REG_RULE_END_FREQ);
		reg_rule_ptr[count].max_bw =
			(u16)le32_get_bits(wmi_reg_rule[count].bw_pwr_info,
				      REG_RULE_MAX_BW);
		reg_rule_ptr[count].reg_power =
			(u8)le32_get_bits(wmi_reg_rule[count].bw_pwr_info,
				      REG_RULE_REG_PWR);
		reg_rule_ptr[count].ant_gain =
			(u8)le32_get_bits(wmi_reg_rule[count].bw_pwr_info,
				      REG_RULE_ANT_GAIN);
		reg_rule_ptr[count].flags =
			(u16)le32_get_bits(wmi_reg_rule[count].flag_info,
				      REG_RULE_FLAGS);
		reg_rule_ptr[count].psd_flag =
			!!le32_get_bits(wmi_reg_rule[count].psd_power_info,
				      REG_RULE_PSD_INFO);
		reg_rule_ptr[count].psd_eirp =
			(u16)le32_get_bits(wmi_reg_rule[count].psd_power_info,
				      REG_RULE_PSD_EIRP);
	}
						   ...
						   struct ath12k_reg_info *reg_info)
{
	...
	reg_info->num_phy = (u8)le32_to_cpu(ev->num_phy);
	reg_info->phy_id = (u8)le32_to_cpu(ev->phy_id);
	reg_info->ctry_code = (u16)le32_to_cpu(ev->country_id);
	reg_info->reg_dmn_pair = (u16)le32_to_cpu(ev->domain_code);
	...
	reg_info->domain_code_6g_ap[WMI_REG_INDOOR_AP] =
		(u8)le32_to_cpu(ev->domain_code_6g_ap_lpi);
	reg_info->domain_code_6g_ap[WMI_REG_STD_POWER_AP] =
		(u8)le32_to_cpu(ev->domain_code_6g_ap_sp);
	reg_info->domain_code_6g_ap[WMI_REG_VLP_AP] =
		(u8)le32_to_cpu(ev->domain_code_6g_ap_vlp);

	for (i = 0; i < WMI_REG_MAX_CLIENT_TYPE; i++) {
		reg_info->domain_code_6g_client[WMI_REG_INDOOR_AP][i] =
			(u8)le32_to_cpu(ev->domain_code_6g_client_lpi[i]);
		reg_info->domain_code_6g_client[WMI_REG_STD_POWER_AP][i] =
			(u8)le32_to_cpu(ev->domain_code_6g_client_sp[i]);
		reg_info->domain_code_6g_client[WMI_REG_VLP_AP][i] =
			(u8)le32_to_cpu(ev->domain_code_6g_client_vlp[i]);
	}
					  ...
					  struct ath12k_wmi_mgmt_rx_arg *hdr)
{
	...
	for (i = 0; i < ATH_MAX_ANTENNA; i++)
		hdr->rssi_ctl[i] = (u8)le32_to_cpu(ev->rssi_ctl[i]);
...
static int freq_to_idx(struct ath12k *ar, u32 freq)
{
...
{
	struct ath12k_wmi_rdy_parse rdy_parse = { 0, };
	int ret;
...
static int ath12k_pdev_set_hw_mode_response_parse(struct ath12k_base *ab, u16 tag, u16 len,
    const void *ptr, void *data)
{
    ...
    case WMI_TAG_PDEV_SET_HW_MODE_RESPONSE_VDEV_MAC_ENTRY:
        ath12k_warn(ab, "VDEV MAC ENTRY IS IGNORED\n");
        break;
    default:
        break;
    }
...
static void ath12k_pdev_set_hw_mode_resp_event(struct ath12k_base *ab, struct sk_buff *skb)
{
    struct wmi_pdev_set_hw_mode_response_event set_hw_mode_resp;
    int ret;

    ret = ath12k_wmi_tlv_iter(ab, skb->data, skb->len,
        ath12k_pdev_set_hw_mode_response_parse, &set_hw_mode_resp);
    if (ret) {
        ath12k_warn(ab, "failed to parse tlv %d\n", ret);
        return;
    }

    ath12k_info(ab, "pdev_id %d, hw_mode_index %d, status %d\n", 
...
static void ath12k_vdev_start_resp_event(struct ath12k_base *ab, struct sk_buff *skb)
{
	...
	ath12k_dbg(ab, ATH12K_DBG_WMI, "vdev start resp for vdev id %d done = %d",
		   vdev_start_resp.vdev_id, ar->vdev_setup_done.done);
}
...
static void ath12k_mgmt_rx_event(struct ath12k_base *ab, struct sk_buff *skb)
{
	...
	status->signal = (s8)(rx_ev.snr + ATH12K_DEFAULT_NOISE_FLOOR);
	...
	ath12k_dbg_dump(ab, ATH12K_DBG_PKT, "rx 802.11 mgmt ", skb->data, skb->len, 0);
...
{
	struct wmi_peer_sta_kickout_arg arg = { 0, };
...
{
	struct wmi_roam_event roam_ev = { 0, };
	struct ath12k *ar;
	...
	case WMI_ROAM_REASON_BEACON_MISS:
		ath12k_mac_handle_beacon_miss(ar, roam_ev.vdev_id);
		break;
...
{
	struct wmi_pdev_bss_chan_info_event bss_ch_info_ev = { 0, };
	struct ath12k *ar;
...
static void
ath12k_wmi_diag_event(struct ath12k_base *ab,
		      struct sk_buff *skb)
{
...
static void ath12k_wmi_op_rx(struct ath12k_base *ab, struct sk_buff *skb)
{
	...
	ath12k_dbg(ab, ATH12K_DBG_WMI, "wmi tlv op rx 0x%x %a\n", id, evt_id_to_string(id));
	...
	case WMI_PDEV_SET_HW_MODE_RESP_EVENTID:
        ath12k_pdev_set_hw_mode_resp_event(ab, skb);
        break;
...
int ath12k_wmi_diag_connect(struct ath12k_base *ab)
{
	...
	/* connect to control service */
	conn_req.service_id = ATH12K_HTC_SVC_ID_WMI_CONTROL_DIAG;

	status = ath12k_htc_connect_service(&ab->htc, &conn_req, &conn_resp);
	if (status) {
		ath12k_warn(ab, "failed to connect to WMI diag service status: %d\n",
			    status);
		return status;
	}
