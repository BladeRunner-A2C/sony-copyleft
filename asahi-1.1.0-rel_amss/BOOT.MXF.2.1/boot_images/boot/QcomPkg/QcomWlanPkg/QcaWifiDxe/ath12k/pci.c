...
static const struct pci_device_id ath12k_pci_id_table[] = {
	{ QCOM, WCN7850_DEVICE_ID, QCA_PCI_ANY_ID, QCA_PCI_ANY_ID,
...
static const struct ath12k_msi_config ath12k_msi_config[] = {
	{
		...
		.users = (struct ath12k_msi_user[]) {
			...
			{ .name = "DP", .num_vectors = 19, .base_vector = 13 },
		},
	},
};
...
static void ath12k_pci_select_window(struct ath12k_pci *ab_pci, u32 offset)
{
	...
	if (window != ab_pci->register_window) {
		pci_write32(ab_pci->pdev, PCI_BASE_OFFSET(ab_pci->mem) + WINDOW_REG_ADDRESS,
            WINDOW_ENABLE_BIT | window);
		pci_read32(ab_pci->pdev, PCI_BASE_OFFSET(ab_pci->mem) + WINDOW_REG_ADDRESS);
...
static void ath12k_pci_ce_irq_enable(struct ath12k_base *ab, u32 ce_id)
{
	u32 irq_idx;
...
static void ath12k_pci_ce_irq_disable(struct ath12k_base *ab, u32 ce_id)
{
	u32 irq_idx;
...
static int ath12k_pci_ce_interrupt_handler(int irq, int budget, void *arg)
{
	struct ath12k_ce_pipe *ce_pipe = (struct ath12k_ce_pipe *)arg;
	int handled;
	...
	//tasklet_schedule(&ce_pipe->intr_tq);
	 ...
	 handled = ath12k_ce_per_engine_service(ce_pipe->ab, ce_pipe->pipe_num);
...
static int ath12k_pci_ext_interrupt_handler(int irq, int budget, void *arg)
{
	...
	work_done = ath12k_pci_ext_grp_napi_poll(&irq_grp->napi, budget);
...
static int ath12k_pci_config_irq(struct ath12k_base *ab)
{
	...
	for (i = 0, msi_data_idx = 0; i < ab->hw_params->ce_count; i++) {
		...
		ath12k_info(ab, "irq:%d, irq_idx:%d\n", irq, irq_idx);
...
static int ath12k_pci_get_msi_assignment(struct ath12k_pci *ar_pci)
{
	ar_pci->msi_config = &ath12k_msi_config[0];

	return 0;
}

static int ath12k_pci_msi_enable(struct ath12k_pci *ar_pci)
{
    struct ath12k_base *ab = ar_pci->ab;
    const struct ath12k_msi_config *msi_config;
    int ret;

    ret = ath12k_pci_get_msi_assignment(ar_pci);
    if (ret) {
        ath12k_err(ab, "failed to get MSI assignment, err = %d\n", ret);
        goto out;
    }

    msi_config = ar_pci->msi_config;
    if (!msi_config) {
        ath12k_err(ab, "msi_config is NULL!\n");
        ret = -EINVAL;
        goto out;
    }
...
static void ath12k_pci_msi_disable(struct ath12k_pci *ar_pci)
{
    /* free dummy msi */
...
static int ath12k_pci_claim(struct ath12k_pci *ab_pci, pci_dev_handle pdev)
{
	struct ath12k_base *ab = ab_pci->ab;
	u16 device_id;
	int ret = 0;
	...
	pci_request_dma_pool(pdev, "ath12k_pci_reserved_8k", SZ_8K,
		&ab_pci->reserved_dma_pool);
...
static void ath12k_pci_free_region(struct ath12k_pci *ab_pci)
{
	...
	pci_free_dma_pool(&ab_pci->reserved_dma_pool);
				   ...
				   u8 *ul_pipe, u8 *dl_pipe)
{
	...
	/* for diag over CE7, only dl pipe is used */
...
int ath12k_pci_target_dump(struct ath12k_pci *ar_pci)
{
    return ath12k_mhi_force_rddm(ar_pci);
}

static int ath12k_pci_dump(struct ath12k_base *ab)
{
    int ret;
    struct ath12k_pci *ar_pci;

    ar_pci = ath12k_pci_priv(ab);

    ret = ath12k_pci_target_dump(ar_pci);

    return ret;
}

/* Provide an interface to dump the SRAM*/
int ath12k_sram_dump(struct ath12k_base *ab)
{
	...
	const u16 *file = ATH12K_SRAM_DUP_FILE_NAME;
	 ...
	 */
	wakeup_required = test_bit(ATH12K_PCI_FLAG_INIT_DONE, &ab_pci->flags) &&
			  end >= ACCESS_ALWAYS_OFF;
	if (wakeup_required) {
		ret = mhi_device_get_sync(ab_pci->mhi_ctrl->mhi_dev);
		if (ret)
			ath12k_err(ab, "failed to wakeup MHI: %d\n", ret);
	}

    ptr = buf;
	for (i = start; i < end; i += 4) {
		data = ath12k_hif_read32(ab, i);
		memcpy(ptr, &data, 4);
		ptr += 4;
	}
	...
	for (idx = 0; idx < msi_config->total_users; idx++) {
		if (strcmp(user_name, msi_config->users[idx].name) == 0) {
			...
			*base_vector = msi_config->users[idx].base_vector;
			...
			ath12k_dbg(ab, ATH12K_DBG_PCI, "Assign MSI to user: %a, num_vectors: %d, user_base_data: %u, base_vector: %u\n",
				...
				u32 *msi_addr_hi)
{
    ...
    *msi_addr_lo = (u32)(ab_pci->dummy_msi.paddr & 0xFFFFFFFF);
    *msi_addr_hi = (u32)(ab_pci->dummy_msi.paddr >> 32);
}
...
u32 ath12k_pci_do_read32(struct ath12k_base *ab, u32 offset)
{
	struct ath12k_pci *ab_pci = ath12k_pci_priv(ab);
...
u32 ath12k_pci_read32(struct ath12k_base *ab, u32 offset)
{
	...
	val = ath12k_pci_do_read32(ab, offset);
...
void ath12k_pci_do_write32(struct ath12k_base *ab, u32 offset, u32 value)
{
	struct ath12k_pci *ab_pci = ath12k_pci_priv(ab);
	u32 window_start;
...
void ath12k_pci_write32(struct ath12k_base *ab, u32 offset, u32 value)
{
	...
	ath12k_pci_do_write32(ab, offset, value);
...
static const struct ath12k_hif_ops ath12k_pci_hif_ops = {
	...
	.dump = ath12k_pci_dump,
	.dump_sram = ath12k_sram_dump,
};

static int ath12k_pci_probe(pci_dev_handle pdev)
{
	struct ath12k_base *ab;
	struct ath12k_pci *ab_pci;
	int ret;

	ab = ath12k_core_alloc(pdev, sizeof(*ab_pci), ATH12K_BUS_PCI);
	if (!ab) {
		ath12k_err(ab, "failed to allocate ath12k base\n");
		return -ENOMEM;
	}

	ab->pdev = pdev;
	ab_pci = ath12k_pci_priv(ab);
	ab_pci->dev_id = pci_get_hardware_dev_id(pdev);
	ab_pci->ab = ab;
	ab_pci->pdev = pdev;
	...
	default:
		ath12k_err(ab, "Unknown PCI device found: 0x%x\n",
			ab_pci->dev_id);
		ret = -EOPNOTSUPP;
	...
	init_work(&ab_pci->rddm_worker, ath12k_mhi_pm_rddm_worker, ab_pci);
...
static void ath12k_pci_remove(pci_dev_handle pdev)
{
...
struct pci_driver ath12k_pci_driver = {
    .name = "ath12k_pci",
