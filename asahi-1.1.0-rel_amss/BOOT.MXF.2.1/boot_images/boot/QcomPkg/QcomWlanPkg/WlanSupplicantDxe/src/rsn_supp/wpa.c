				      ...
				      const u8 *key_rsc, int wnm_sleep)
{
	...
	if ((sm->gtk.gtk_len == (size_t) gd->gtk_len &&
		...
		wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
			"WPA: (UEFI)reinstalling already in-use GTK to the driver (keyidx=%d tx=%d len=%d)",
			gd->keyidx, gd->tx, gd->gtk_len);
        /* NOTE: UEFI reinstall key to update state machine state */
	}
				       ...
				       int wnm_sleep)
{
	...
	if ((sm->igtk.igtk_len == len &&
		...
		wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
			"WPA: (UEFI)reinstalling already in-use IGTK to the driver (keyidx=%d)",
			keyidx);
        /* NOTE: UEFI reinstall key to update state machine state */
	}
				       ...
				       int wnm_sleep)
{
	...
	if ((sm->bigtk.bigtk_len == len &&
		...
		wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
			"WPA: (UEFI)reinstalling already in-use BIGTK to the driver (keyidx=%d)",
			keyidx);
        /* NOTE: UEFI reinstall key to update state machine state */
	}
