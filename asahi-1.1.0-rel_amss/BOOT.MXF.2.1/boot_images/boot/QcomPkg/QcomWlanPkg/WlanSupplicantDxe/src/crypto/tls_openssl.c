...
static int tls_cryptoapi_ca_cert(SSL_CTX *ssl_ctx, SSL *ssl, const char *name)
{
	...
	if (wstore == NULL)
		return -1;
	swprintf_s(wstore, (os_strlen(store) + 1), L"%S", store);
...
#endif /* OPENSSL_NO_ENGINE */
#define TLS_DEFAULT_CIPHERS "ALL:!EXPORT:!LOW:!aNULL:!eNULL:!SSLv2"
...
static int tls_use_private_key_blob(struct tls_data *data, SSL *ssl,
    ...
    const char *private_key_passwd)
{
    ...
    if (!bio)
        return -1;
    if (BIO_write(bio, private_key_blob, (int)private_key_blob_len) <= 0) {
        BIO_free(bio);
        return -1;
    }
				      ...
				      size_t private_key_blob_len)
{
	...
	while (private_key_blob) {
        ...
        if (tls_use_private_key_blob(data, conn->ssl,
            private_key_blob, private_key_blob_len, private_key_passwd) == 0) {
            wpa_printf(MSG_DEBUG, "OpenSSL: tls_use_private_key_blob"
                "--> OK");
            ok = 1;
            break;
        }
