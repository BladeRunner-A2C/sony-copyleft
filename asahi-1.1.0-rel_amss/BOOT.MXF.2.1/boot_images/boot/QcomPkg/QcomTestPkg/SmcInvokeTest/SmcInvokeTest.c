/** @file SmcInvokeTest.c
  ...
  Copyright (c) 2020-2023 Copyright Qualcomm Technologies, Inc. All Rights Reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 08/31/21   shl     Modified app ID check
 12/20/22   sourav  Changes to search the TA across FVs. 
...
#define TOTAL_TEST_FILE_PATHS    5
...
static CHAR8  TzTestAppFile[] = "smcinvoketestapp.mbn";
...
EFI_STATUS ReadImage(void)
{
   ...
   //searching the file from fv2 to fv5
   for(UINT32 i= 2; i<=TOTAL_TEST_FILE_PATHS; i++)
   {
      snprintf(FilePath, sizeof(FilePath),"fv%d:\\%s",i, TzTestAppFile);
      AsciiPrint("Searching in filePath: %a \r\n",FilePath);

      File = EfiOpen ((CHAR8*)FilePath, EFI_FILE_MODE_READ, EFI_SECTION_RAW);

      if (File != NULL)
      {
         AsciiPrint("Trusted app file is found.\r\n");
         break;
      }
   }
