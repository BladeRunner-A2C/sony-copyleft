  ...
  do //For Easy error handling
  {
    ...
    else//EFI_SUCCESS 
    {
      ...
      if (0 == AsciiStrCmp(ULogTestCmdLineParams.cmd,"dump")) 
      {
        ...
        {
          //Concat
          AsciiStrCatS(ULogTestCmdLineParams.LogName, ULOG_MAX_NAME_SIZE, argv[cnt]);
          if (cnt < argc-1)//We concat spaces except at the end. - Like "Clock Log" has only one Space and no space at the end.
          {
            AsciiStrCatS(ULogTestCmdLineParams.LogName, ULOG_MAX_NAME_SIZE, " ");
          }
        }
      }
...
ULogResult ULogFileTest_MakePathFromName(CONST CHAR8 *LogName, CHAR8 *FilePath)
{
  ...
  AsciiStrCpyS(PathStr, max_length, sTestLogDirectory);
  ...
  AsciiStrCatS(PathStr, max_length, LogName);
  if (AsciiStrLen(PathStr) < length) {
  ...
  AsciiStrCatS(PathStr, max_length, ULOGFILE_EXTENSION);
  if (AsciiStrLen(PathStr) < length) {
  ...
  StrnCpyS((CHAR16 *) FilePath, MAX_PATHNAME, (CONST CHAR16 *) PathStr, length);
