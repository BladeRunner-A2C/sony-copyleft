...
EFI_STATUS clear_counter2(VOID);
EFI_STATUS set_counter2(UINTN increment);
UINTN get_counter2(VOID);
...
EFI_STATUS EFIAPI CPT_MpTest2Main ( IN EFI_HANDLE ImageHandle,
   ...
   while (index < argc)
   {
      ...
      switch (CmdOp)
      {
         ...
         case TEST_SUITE:
            ...
            {
               //tc_config.suite = "cpt";
               AsciiStrnCpyS(tc_config.suite, TEST_CONFIG_LEN2, "cpt", TEST_CONFIG_LEN2 - 1);
            }
            else {
               //tc_config.suite = "dev";
               AsciiStrnCpyS(tc_config.suite, TEST_CONFIG_LEN2, "dev", TEST_CONFIG_LEN2 - 1);
            }
            index++;
            break;
