...
/*=============================================================================
 ...
 4/07/23    kac     Replace ASSERT with Invalid return error status  
  ...
  )
{
  ...
  */
  ...
  if (Status != EFI_SUCCESS){
    ...
    if (BltY > VerticalResolution)
    {
      AsciiPrint("ERROR: Font size is greater than the display height\n");
      AsciiPrint(" Exiting the test...\n");
    }
  ...
  )
{
  ...
  UINTN            AsciiFontSize = 32;
  CHAR8            AsciiFontName[AsciiFontSize];

  for (Index = 0; Index < ARRAY_SIZE(TestFonts); Index++) {
    UnicodeStrToAsciiStrS(TestFonts[Index], AsciiFontName, AsciiFontSize); 
