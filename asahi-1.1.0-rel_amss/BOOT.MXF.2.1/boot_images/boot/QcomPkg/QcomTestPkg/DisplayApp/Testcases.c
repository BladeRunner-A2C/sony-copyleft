...
#include "Utils.h"
#include "Testcases.h"
#include "DisplayDemuraPattern.h"
...
#define PRIMARY_DISPLAYID                   0
#define SECONDARY_DISPLAYID                 1
#define EXTERNAL_DISPLAYID                  2
#define DEFAULT_FPS_WAITTIME                5
#define CONVERT_S_TO_US(x)                  (x * 1000000)    //convert seconds to microseconds

#define LIGHT_GREY                          0xA0A0A0FF
#define DARK_GREY                           0x1A1A1AFF

#define DSI_CMDMODE_DT_DSCLONGWRITE         0x39
#define DSI_CMDMODE_DT_DSCREAD0PARA         0x06
#define DSI_DISPLAY_CMDFLAG_MAXPACKETSIZE   0x10
...
/* List of all mdss clocks that is always on
 */
ClockListEntry AlwaysOnClockList[] = 
{
   {"gcc_disp_ahb_clk"},
   {"disp_cc_xo_clk"},
};

/* List of mdss core clocks
 */
ClockListEntry CoreClockList[] =
{
...
static UINT32 DumpAndValidateClocks(ClockListEntry *pClockList, UINT32 uNumberofClocks, BOOLEAN bExpectedClockOn);
static BOOLEAN ValidateClockState(BOOLEAN bPowerOn);
static EFI_STATUS DisplayFillBackground(UINT32 uColor, UINTN uDstX, UINTN uDstY, UINTN uWidth, UINTN uHeight);
...
static EFI_STATUS DisplayBltPerformanceTest(void)
{
  ...
  else
  {
      ...
      UINTN                                   BufferLen;
      UINT32                                  mode          = 0;
      UINT32                                  uCurrentMode  = pGfxProtocol->Mode->Mode;
      UINT32                                  inplace;
      ...
      else
      {
          ...
          if (EFI_SUCCESS != (eStatus = pGfxProtocol->SetMode(pGfxProtocol, uCurrentMode)))
          {
            AsciiPrint("DisplayBltOperationTest: SetMode(%d) failed.\n", uCurrentMode);
          }
      }
  }
...
static EFI_STATUS DisplayBltOperationTest(void)
{
  ...
  else
  {
    ...
    UINTN                                   centerY;
    UINT32                                  mode                  = 0;
    uint32                                  uHorizontalResolution = 0;
    uint32                                  uVerticalResolution   = 0;
    uint32                                  uModeIndex            = 0;

    /* Loop through all display modes to find max resolution, so that BLT test has room to move around */
    for (uModeIndex = 0; uModeIndex < pGfxProtocol->Mode->MaxMode; uModeIndex++)
    {
      if (EFI_SUCCESS != (eStatus = pGfxProtocol->QueryMode(pGfxProtocol, uModeIndex, &nModeInfoSize, &pModeInfo)))
      {
        AsciiPrint("DisplayBltOperationTest: QueryMode() failed.\n");
        break;
      }
      ...
      {
        if ((pModeInfo->HorizontalResolution >  uHorizontalResolution) ||
           ((pModeInfo->HorizontalResolution == uHorizontalResolution) &&
            (pModeInfo->VerticalResolution   >  uVerticalResolution)))
        {
          uHorizontalResolution = pModeInfo->HorizontalResolution;
          uVerticalResolution   = pModeInfo->VerticalResolution;
          mode = uModeIndex;
        }
      }
    }
    ...
    centerX = (uHorizontalResolution > BITMAP_IMAGE_WIDTH)  ? ((uHorizontalResolution - BITMAP_IMAGE_WIDTH )>>1) : 0;
    centerY = (uVerticalResolution   > BITMAP_IMAGE_HEIGHT) ? ((uVerticalResolution   - BITMAP_IMAGE_HEIGHT)>>1) : 0;
...
static EFI_STATUS DisplayPanelPowerControlTest(void)
{
  ...
  else
  {
    ...
    for (cntr = 0; cntr < 2; cntr++)
    {
      ...
      /* Dump and Validate Clock Status */
      if (FALSE == ValidateClockState(FALSE))
      {
        uNumErrors++;
      }
      ...
      /* Dump and Validate Clock Status */
      if (FALSE == ValidateClockState(TRUE))
      {
        uNumErrors++;
      }
  ...
  return (uNumErrors==0)?EFI_SUCCESS:EFI_UNSUPPORTED;
}
...
static EFI_STATUS DisplaySetModeTest()
{
  ...
  else
  {
     ...
     for (uModeIndex = 0; uModeIndex < pGfxProtocol->Mode->MaxMode; uModeIndex++)
     {
       ...
       {
         AsciiPrint("DisplaySetModeTest : Mode#%d successfully changed (Current Mode %dx%d)\n",
                (UINT32)uModeIndex,
     ...
     // Reset back to original display mode at the end
     pGfxProtocol->SetMode(pGfxProtocol, uCurrentMode);
  }
...
static EFI_STATUS DisplaySetModeExtTest(void)
{
  ...
  UINT32                            DisplayId          = EXTERNAL_DISPLAYID; /* external display */
  UINT32                            ModeNUm            = 0;
  UINT32                            Flags              = 0;
  ...
  else 
  {
      ...
      else if (EFI_UNSUPPORTED == eStatus)
      {
          AsciiPrint("DisplaySetModeExtTest: Skip to set external display mode\n");
          eStatus = EFI_SUCCESS;
      }
      else
      {
 ...
 */
static TestCaseInfo DisplayPatternDescription =
{
    "Display Pattern Test",
    "Test pattern for demura verification."
};

/* DisplayFillBackground()
 * Helper function to fill the background.
 * @param[in]  uColor          The color data saved as RGBA format.
 * @param[in]  uDstX           X coordinate of destination for the BltBuffer.
 * @param[in]  uDstY           Y coordinate of destination for the BltBuffer.
 * @param[in]  uWidth          Width of rectangle to be filled in BltBuffer in pixels.
 * @param[in]  uHeight         Hight of rectangle to be filled in BltBuffer in pixels.
 */
static EFI_STATUS DisplayFillBackground(UINT32 uColor, UINTN uDstX, UINTN uDstY, UINTN uWidth, UINTN uHeight)
{
  EFI_STATUS                              eStatus         = EFI_SUCCESS;
  ...
  if (EFI_SUCCESS != (eStatus = gBS->LocateProtocol (&sGfxOutputGUID,
                                                     ...
                                                     (VOID **)&pGfxProtocol)))
  {
    AsciiPrint ("DisplayFillBackground: LocateProtocol() failed.\n");
  }
  else
  {
    /* Fill background */
    bgPixel.Red      = (uColor>>24) & 0xFF;
    bgPixel.Green    = (uColor>>16) & 0xFF;
    bgPixel.Blue     = (uColor>>8)  & 0xFF;
    bgPixel.Reserved = uColor       & 0xFF;
    ...
    if (EFI_SUCCESS != (eStatus = pGfxProtocol->Blt(pGfxProtocol, 
                                                    ...
                                                    0)))
    {
      AsciiPrint ("DisplayFillBackground: Blt(EfiBltVideoFill) failed.\n");
    }
  }
...
/* DisplayDrawPattern()
 * Helper function to draw the test pattern.
 */
static EFI_STATUS DisplayDrawPattern(DisplayPatternInfoType *pPatternInfo)
{
  EFI_STATUS                          eStatus               = EFI_SUCCESS;
  EFI_QCOM_DISPLAY_UTILS_PROTOCOL    *pDisplayUtilsProtocol = NULL;
  ...
  if (NULL == pPatternInfo)
  {
    AsciiPrint("DisplayDrawPattern: Invalid parameter for pattern info!\n");
  }
  else if ((EFI_SUCCESS != eStatus) ||
           (NULL        == pDisplayUtilsProtocol))
  {
    AsciiPrint("DisplayDrawPattern: Locate display utils protocol failed!\n");
  }
  else
  {
    EFI_DISPLAY_UTILS_RENDERLOGO_CONFIG RenderLogoCfg;

    ZeroMem(&RenderLogoCfg, sizeof(RenderLogoCfg));

    RenderLogoCfg.BGRTImage     = pPatternInfo->pBitmap;
    RenderLogoCfg.BGRTImageSize = pPatternInfo->uBitmapSize;
    RenderLogoCfg.uFlags        = DISPLAY_UTILS_RENDERLOGO_RENDER_BGRT_IMAGE;

    if (EFI_SUCCESS != (eStatus = pDisplayUtilsProtocol->DisplayUtilsRenderLogo(&RenderLogoCfg)))
    {
      AsciiPrint("DisplayDrawPattern: Render splash screen failed!\n");
    }
    else
    {
      gBS->Stall(DEFAULT_LONG_WAIT);
...
/* DisplayPatternTest()
 * Function to render test patterns.
 */
static EFI_STATUS DisplayPatternTest()
{
  EFI_STATUS                              eStatus        = EFI_SUCCESS;
  EFI_GUID                                sGfxOutputGUID = EFI_GRAPHICS_OUTPUT_PROTOCOL_GUID;
  ...
  // Display Pattern Info List
  DisplayPatternInfoType aPatternList[DISPLAY_PATTERN_MAX] =
  {
    {&aDemuraTestPattern[0], sizeof(aDemuraTestPattern)/sizeof(aDemuraTestPattern[0])},
  };
  ...
  else
  {
    ...
    /* Loop through all display modes to find max resolution, so that background color can be filled to show demura pattern*/
    for (uModeIndex = 0; uModeIndex < pGfxProtocol->Mode->MaxMode; uModeIndex++) 
    {
      if (EFI_SUCCESS != (eStatus = pGfxProtocol->QueryMode(pGfxProtocol, uModeIndex, &nModeInfoSize, &pModeInfo)))
      {
        AsciiPrint("DisplayPatternTest: QueryMode() failed.\n");
        break;
      }
      else
      {
  ...
  AsciiPrint("DisplayPatternTest: QueryMode() [%d]<%d-%d>.\n", uMode, uHorizontalResolution, uVerticalResolution);
  if (EFI_SUCCESS != eStatus)
  {
    AsciiPrint("DisplayPatternTest: failed to get mode.\n");
  }
  else if (EFI_SUCCESS != (eStatus = pGfxProtocol->SetMode(pGfxProtocol, uMode)))
  {
    AsciiPrint("DisplayFillBackground: SetMode() failed.\n");
  }
  else if (EFI_SUCCESS != (eStatus = DisplayFillBackground(LIGHT_GREY, 0, 0, uHorizontalResolution, uVerticalResolution)))
  {
    AsciiPrint("DisplayPatternTest: DisplayFillBackground() failed.\n");
  }
  else if (EFI_SUCCESS != (eStatus = DisplayFillBackground(DARK_GREY, uHorizontalResolution / 2, 0, uVerticalResolution / 2, uVerticalResolution)))
  {
    AsciiPrint("DisplayPatternTest: DisplayFillBackground() failed.\n");
  }
  else if (EFI_SUCCESS != (eStatus = DisplayDrawPattern(&aPatternList[DISPLAY_PATTERN_DEMURA])))
  {
    AsciiPrint("DisplayPatternTest: DisplayDrawPattern() failed.\n");
  }

  return eStatus;
}

/* DisplayRefreshRateDescription()
 */
static TestCaseInfo DisplayRefreshRateDescription =
{
    "Display RefreshRate Test",
    "This test measures the actual refresh rate the display controller on the primary display."
};

static EFI_STATUS DisplayRefreshRateTest()
{
  EFI_STATUS                                 eStatus                        = EFI_SUCCESS;
  EFI_GUID                                   sDisplayUtilGUID               = EFI_DISPLAYUTILS_PROTOCOL_GUID;
  EFI_QCOM_DISPLAY_UTILS_PROTOCOL            *pDisplayUtilProtocol;

  if (EFI_SUCCESS != (eStatus = gBS->LocateProtocol(&sDisplayUtilGUID, NULL, (VOID **)&pDisplayUtilProtocol)))
  {
    AsciiPrint("DisplayRefreshTest: LocateProtocol() failed.\n");
    eStatus = EFI_NOT_FOUND;
  }
  else
  {
    UINT32                                     uRefreshRate                 = 0;
    UINT32                                     uRefreshRateF                = 0;
    UINT64                                     uStartTimeUS                 = 0;
    UINT64                                     uEndTimeUS                   = 0;
    UINT64                                     uDeltaTimeUS                 = 0;
    UINT32                                     uStartFrame                  = 0;
    UINT32                                     uEndFrame                    = 0;
    UINT32                                     uDeltaFrame                  = 0;
    EFI_DISPLAY_UTILS_STATS_INFO               sDisplayStateInfo;
    UINT32                                     uParamSize                   = sizeof(sDisplayStateInfo);

    ZeroMem(&sDisplayStateInfo,  sizeof(sDisplayStateInfo));

    sDisplayStateInfo.uDisplayId = PRIMARY_DISPLAYID;

    if (EFI_SUCCESS != (eStatus = pDisplayUtilProtocol->DisplayUtilsGetProperty(EFI_DISPLAY_UTILS_STATS, (VOID *)&sDisplayStateInfo, &uParamSize)))
    {
      AsciiPrint("DisplayRefreshTest: Failed to get display statistics\n");
    }
    else
    {
      uStartTimeUS  = GetTimerCountus();
      uStartFrame   = sDisplayStateInfo.uFrameCount;

      AsciiPrint("DisplayRefreshTest: Calculating refresh rate for %ds..\n", DEFAULT_FPS_WAITTIME);
      gBS->Stall(CONVERT_S_TO_US(DEFAULT_FPS_WAITTIME));
      ...
      if (EFI_SUCCESS != (eStatus = pDisplayUtilProtocol->DisplayUtilsGetProperty(EFI_DISPLAY_UTILS_STATS, (VOID *)&sDisplayStateInfo, &uParamSize)))
      {
        AsciiPrint("DisplayRefreshTest: Get display frame statistics failed\n");
      }
      else
      {
        uEndTimeUS   = GetTimerCountus();
        uEndFrame    = sDisplayStateInfo.uFrameCount;

        if (uEndFrame < uStartFrame)
        {
          AsciiPrint("DisplayRefreshTest: Get end frame statistics unexpect, less than start frame\n");
          eStatus = EFI_UNSUPPORTED;
        }
        else if (uEndTimeUS <= uStartTimeUS)
        {
          AsciiPrint("DisplayRefreshTest: Get system time statistics unexpect\n");
          eStatus = EFI_UNSUPPORTED;
        }
        else
        {
          uDeltaFrame   = uEndFrame - uStartFrame;
          uDeltaTimeUS  = uEndTimeUS - uStartTimeUS;

          /* Convert microseconds to seconds: 1s = 1000000us */
          uRefreshRate  = uDeltaFrame * 1000000 / uDeltaTimeUS;
          uRefreshRateF = uDeltaFrame * 1000000 % uDeltaTimeUS;

          AsciiPrint("DisplayRefreshTest: DisplayID:%d, refresh rate: %d.%d\n", sDisplayStateInfo.uDisplayId, uRefreshRate, uRefreshRateF);
        }
      }
    }
  }
...
static TestCaseInfo DisplayPanelEdidDescription =
{
  "Display Get Panel Edid Tests",
  "This test show the generate EDID from panel"
};

static EFI_STATUS DisplayPanelEdidTest()
{
  EFI_STATUS                        eStatus           = EFI_SUCCESS;
  EFI_GUID                          sDisplayEdidGUID  = EFI_EDID_ACTIVE_PROTOCOL_GUID;
  EFI_GUID                          sDiscovedEdidGUID = EFI_EDID_DISCOVERED_PROTOCOL_GUID;
  EFI_EDID_ACTIVE_PROTOCOL         *pDisplayEdidActiveProtocol;
  EFI_EDID_DISCOVERED_PROTOCOL     *pDisplayEdidDiscoveredProtocol;


  if (EFI_SUCCESS != (eStatus = gBS->LocateProtocol(&sDisplayEdidGUID,
                                                    NULL,
                                                    (VOID **)&pDisplayEdidActiveProtocol)))
  {
    AsciiPrint("DisplayPanelEdidTest: LocateProtocol() failed for EDI_EDID_ACTIVE_PROTOCOL.\n");
    eStatus = EFI_NOT_FOUND;
  }
  else
  {
    if (NULL == pDisplayEdidActiveProtocol->Edid)
    {
      AsciiPrint("DisplayPanelEdidTest: EdidActive Protocol return an NULL EDID\n");
      eStatus = EFI_UNSUPPORTED;
    }
    else if (0 == pDisplayEdidActiveProtocol->SizeOfEdid)
    {
      AsciiPrint("DisplayPanelEdidTest: EdidActive Protocol return a zero sized EDID\n");
      eStatus = EFI_UNSUPPORTED;
    }
    else
    {
      ...
      AsciiPrint("\n DisplayPanelEdidTest: EdidActive Protocol EDID value: \n");

      for (i = 0; i < pDisplayEdidActiveProtocol->SizeOfEdid; i++)
      {
        AsciiPrint("%02x ", pDisplayEdidActiveProtocol->Edid[i]);
        if ((i + 1) % 16 == 0)
          AsciiPrint("\n");
      }
    }
  ...
  /* Check for EDI_EDID_DISCOVERED_PROTOCOL */
  if (EFI_SUCCESS != (eStatus = gBS->LocateProtocol(&sDiscovedEdidGUID,
                                                    NULL,
                                                    (VOID **)&pDisplayEdidDiscoveredProtocol)))
  {
    AsciiPrint("DisplayPanelEdidTest: LocateProtocol() failed for EDI_EDID_DISCOVERED_PROTOCOL.\n");
    eStatus = EFI_NOT_FOUND;
  }
  else
  {
    if (NULL == pDisplayEdidDiscoveredProtocol->Edid)
    {
      AsciiPrint("DisplayPanelEdidTest: EdidDiscovered Protocol return an NULL EDID\n");
      eStatus = EFI_UNSUPPORTED;
    }
    else if (0 == pDisplayEdidDiscoveredProtocol->SizeOfEdid)
    {
      AsciiPrint("DisplayPanelEdidTest: EdidDiscovered Protocol return a zero sized EDID\n");
      eStatus = EFI_UNSUPPORTED;
    }
    else
    {
      ...
      AsciiPrint("\n DisplayPanelEdidTest: EdidDiscovered Protocol EDID value: \n");

      for (i = 0; i < pDisplayEdidDiscoveredProtocol->SizeOfEdid; i++)
      {
        AsciiPrint("%02x ", pDisplayEdidDiscoveredProtocol->Edid[i]);
        if ((i + 1) % 16 == 0)
          AsciiPrint("\n");
      }
    }
...
EFI_STATUS DisplayVerifyRegTest(void)
{
  ...
  /* Always try free the allocated buffer in the end */
  if (NULL != pGoldenSettingBuf)
  {
    gBS->FreePool(pGoldenSettingBuf);
  }
...
/*
  Get Display Interface Configuration
*/
static EFI_STATUS DisplayGetDisplayInterface(EFI_DISPLAY_UTILS_SUPPORTED_PANELS_PARAMS  *psPanelParams, UINT32 uSize)
{
  EFI_STATUS                                 eStatus               = EFI_SUCCESS;
  EFI_GUID                                   sDisplayUtilGUID      = EFI_DISPLAYUTILS_PROTOCOL_GUID;
  EFI_QCOM_DISPLAY_UTILS_PROTOCOL           *pDisplayUtilProtocol;
  ...
  if (EFI_SUCCESS != (eStatus = gBS->LocateProtocol(&sDisplayUtilGUID, NULL, (VOID **)&pDisplayUtilProtocol)))
  {
    AsciiPrint("DisplayGetDisplayInterface: LocateProtocol() failed.\n");
  }
  else
  {
    if (EFI_SUCCESS == (eStatus = pDisplayUtilProtocol->DisplayUtilsGetProperty(EFI_DISPLAY_UTILS_DISPLAY_CONFIGURATION, (VOID *)psPanelParams, &uSize)))
    {
      switch (psPanelParams->eInterfaceType)
      {
        case EFI_DISPLAY_UTILS_INTERFACE_DSI:
        {
          if (psPanelParams->uNumInterfaces < EFI_DISPLAY_UTILS_MAX_INTERFACES)
          {
            if (psPanelParams->uIntefaceList[0] == psPanelParams->uIntefaceList[1])
            {
              AsciiPrint("DisplayGetDisplayInterface: Display interface use DSI0\n");
            }
            else
            {
              AsciiPrint("DisplayGetDisplayInterface: Display interface use DSI1\n");
            }
          }
          else
          {
            AsciiPrint("DisplayGetDisplayInterface: Display interface use both DSI0 and DSI1\n");
          }
        }
        break;

        case EFI_DISPLAY_UTILS_INTERFACE_DP:
        {
          AsciiPrint("DisplayGetDisplayInterface: Display Interface DP\n");
        }
        break;

        default:
          AsciiPrint("DisplayGetDisplayInterface: Display Interface not support\n");
        break;
      }
    }
  }
...
/*
  Display Write data to panel
*/
static EFI_STATUS DisplayInterfaceSetRegister(EFI_DISPLAY_UTILS_INTERFACE_TYPE eInterfaceType, EFI_DISPLAY_UTILS_READWRITE_PARAMS *psWiteParams, UINT32 uSize)
{
  EFI_STATUS                                 eStatus               = EFI_SUCCESS;
  ...
  switch (eInterfaceType)
  {
    case EFI_DISPLAY_UTILS_INTERFACE_DSI:
    {
      ...
      if (EFI_SUCCESS != (eStatus = gBS->LocateProtocol(&sDisplayUtilGUID, NULL, (VOID **)&pDisplayUtilProtocol)))
      {
        AsciiPrint("DisplayInterfaceSetRegister: LocateProtocol() failed.\n");
      }
      else
      {
        if (EFI_SUCCESS == (eStatus = pDisplayUtilProtocol->DisplayUtilsSetProperty(EFI_DISPLAY_UTILS_DISPLAY_WRITE, (VOID *)psWiteParams, uSize)))
        {
          AsciiPrint("DisplayInterfaceSetRegister: Display write %d bytes successful\n", psWiteParams->uPayloadSize);
        }
      }
    }
    break;
...
/*
  Display Read data from panel
*/
static EFI_STATUS DisplayInterfaceGetRegister(EFI_DISPLAY_UTILS_INTERFACE_TYPE eInterfaceType, EFI_DISPLAY_UTILS_READWRITE_PARAMS *psReadParams, UINT32 uSize)
{
  EFI_STATUS                                 eStatus               = EFI_SUCCESS;
  ...
  switch (eInterfaceType)
  {
    case EFI_DISPLAY_UTILS_INTERFACE_DSI:
    {
      ...
      if (EFI_SUCCESS != (eStatus = gBS->LocateProtocol(&sDisplayUtilGUID, NULL, (VOID **)&pDisplayUtilProtocol)))
      {
        AsciiPrint("DisplayInterfaceGetRegister: LocateProtocol() failed.\n");
      }
      else
      {
        if (EFI_SUCCESS == (eStatus = pDisplayUtilProtocol->DisplayUtilsGetProperty(EFI_DISPLAY_UTILS_DISPLAY_READ, (VOID *)psReadParams, &uSize)))
        {
          UINT32 i;

          AsciiPrint("DisplayInterfaceGetRegister: Display dsi read payload:");
          for (i = 0; i < psReadParams->uPayloadSize; i++)
          {
            AsciiPrint("%02x ", psReadParams->uPayload[i]);
          }
          AsciiPrint("\n");
        }
      }
...
static TestCaseInfo DisplayReadWriteDescription =
{
  "Display Read write Tests",
  "This test verify read write function"
};

static EFI_STATUS DisplayReadWriteTest(void)
{
  EFI_STATUS                                 eStatus               = EFI_SUCCESS;
  EFI_DISPLAY_UTILS_INTERFACE_TYPE           eInterfaceType        = EFI_DISPLAY_UTILS_INTERFACE_DSI;
  EFI_DISPLAY_UTILS_SUPPORTED_PANELS_PARAMS  sPanelParams;
  UINT32                                     uSize                 = sizeof(UINT32);
  EFI_DISPLAY_UTILS_READWRITE_PARAMS         sWriteParams;
  EFI_DISPLAY_UTILS_READWRITE_PARAMS         sReadParams;
  UINT32                                     uSizeRW               = sizeof(sWriteParams);
  const uint8                                uWriteData[]          = {0x51, 0x1F, 0xFE};

  ZeroMem(&sPanelParams, sizeof(sPanelParams));
  ZeroMem(&sWriteParams, sizeof(sWriteParams));
  ZeroMem(&sReadParams, sizeof(sReadParams));

  sPanelParams.uDisplayId = 0;
  if (EFI_SUCCESS == (eStatus = DisplayGetDisplayInterface(&sPanelParams, uSize)))
  {
    UINT32      uIndex;

    sWriteParams.uDisplayId   = sPanelParams.uDisplayId;
    sWriteParams.uPayloadSize = sizeof(uWriteData);
    sWriteParams.uCmdType     = DSI_CMDMODE_DT_DSCLONGWRITE;

    for (uIndex = 0; uIndex < sizeof(uWriteData); uIndex++)
    {
      sWriteParams.uPayload[uIndex] = uWriteData[uIndex];
    }

    if (EFI_SUCCESS != (eStatus = DisplayInterfaceSetRegister(eInterfaceType, &sWriteParams, uSizeRW)))
    {
      AsciiPrint("DisplayReadWriteTest: Display%d set register failed\n", sWriteParams.uDisplayId);
    }
    else
    {
      sReadParams.uDisplayId   = sPanelParams.uDisplayId;
      sReadParams.uAddress     = uWriteData[0];
      sReadParams.uCmdType     = DSI_CMDMODE_DT_DSCREAD0PARA;
      sReadParams.uFlags       = DSI_DISPLAY_CMDFLAG_MAXPACKETSIZE;
      sReadParams.uPayloadSize = 2;

      if (EFI_SUCCESS != (eStatus = DisplayInterfaceGetRegister(eInterfaceType, &sReadParams, uSizeRW)))
      {
        AsciiPrint("DisplayReadWriteTest: Display%d read register failed\n", sReadParams.uDisplayId);
      }
    }
  }
  else if (EFI_UNSUPPORTED == eStatus)
  {
    AsciiPrint("DisplayReadWriteTest: Display%d not support, skip to get info\n", sPanelParams.uDisplayId);
    eStatus = EFI_SUCCESS;
  }

  sPanelParams.uDisplayId = 1;
  if (EFI_SUCCESS == (eStatus = DisplayGetDisplayInterface(&sPanelParams, uSize)))
  {
    UINT32      uIndex;
...
TestCaseEntry TestCaseList[] = 
{
  ...
  {&DisplayPatternDescription,                         &DisplayPatternTest},
  {&DisplayRefreshRateDescription,                     &DisplayRefreshRateTest},
  {&DisplayPanelEdidDescription,                       &DisplayPanelEdidTest},
  {&DisplayReadWriteDescription,                       &DisplayReadWriteTest},
};
...
/*
 * Helper to dump clocks and verify whether the clock is in expected state
 */
static UINT32 DumpAndValidateClocks(ClockListEntry *pClockList, UINT32 uNumberofClocks, BOOLEAN bExpectedClockOn)
{
  UINT32  uNumPass = 0;
  ...
  else
  {
    ...
    for (i = 0; i < uNumberofClocks; i++)
    {
      CHAR8 *pClockName = pClockList[i].sClockName;
      ...
      else 
      {
        ...
        BOOLEAN bClockEnabled = FALSE;
        UINT32  uFreq         = 0;
        
        if (EFI_SUCCESS != mClockProtocol->IsClockOn(mClockProtocol, uClockId, &bClockOn))
        {
          AsciiPrint("ERROR: Failed to query clock on/off status of %a\n", pClockName);    
        }
        else if (bClockOn == bExpectedClockOn)
        {
          uNumPass++;
        }
        else if (EFI_SUCCESS != mClockProtocol->IsClockEnabled(mClockProtocol, uClockId, &bClockEnabled))
        {
          AsciiPrint("ERROR: Failed to query clock enable status of %a\n", pClockName);
        }
        else if (bClockEnabled == bExpectedClockOn)
        {
          // Some clocks are ODSC (On Demand Slave Clocking), even clock is enabled, if no slave access it,
          // the clock will be self gating by NOC. In this case, test will continue tracking the clock enable 
          // status set by SW, instead of on/off status.
          uNumPass++;
          bClockOn = bClockEnabled;
        }
        ...
        AsciiPrint("%-30a %4a %11a   %9dHz %8a\n", pClockName, (bClockOn)?"ON":"OFF",(bExpectedClockOn)?"ON":"OFF", (bClockOn)?uFreq:0, (bClockOn==bExpectedClockOn)?"Pass":"Fail");  
      }
    }
  }
...
/* 
 * Validate the clock state after power on/off
 * Return TRUE if all clocks are in expected state
 * Otherwise return FALSE
 */
static BOOLEAN ValidateClockState(BOOLEAN bPowerOn)
{
  BOOLEAN                                    bExpectedState = bPowerOn;
  UINT32                                     uPassClockNum  = 0;
  EFI_DISPLAY_UTILS_SUPPORTED_PANELS_PARAMS  sPanelParams;
  UINT32                                     uTotalClockNum = ARRAY_SIZE(AlwaysOnClockList) +
                                                              ARRAY_SIZE(CoreClockList);

  AsciiPrint("%-30a %4a %11a %12a %8a\n", "Clock Name", "State", "ExpectState", "Frequency", "Result");
  
  uPassClockNum  += DumpAndValidateClocks(AlwaysOnClockList, ARRAY_SIZE(AlwaysOnClockList), TRUE);
  uPassClockNum  += DumpAndValidateClocks(CoreClockList,     ARRAY_SIZE(CoreClockList),     bExpectedState);

  sPanelParams.uDisplayId = 0;

  if (EFI_SUCCESS == DisplayGetDisplayInterface(&sPanelParams, sizeof(UINT32)))
  {
    switch (sPanelParams.eInterfaceType)
    {
      case EFI_DISPLAY_UTILS_INTERFACE_DSI:
      {
        uPassClockNum  += DumpAndValidateClocks(DSI0ClockList,     ARRAY_SIZE(DSI0ClockList),     bExpectedState);
        uTotalClockNum += ARRAY_SIZE(DSI0ClockList);

        // In case of power off, also check DSI1 clock state
        if (FALSE == bPowerOn)
        {
          uPassClockNum  += DumpAndValidateClocks(DSI1ClockList,   ARRAY_SIZE(DSI1ClockList),     FALSE);
          uTotalClockNum += ARRAY_SIZE(DSI1ClockList);
        }
        break;
      }
      case EFI_DISPLAY_UTILS_INTERFACE_DP:
      {
        uPassClockNum  += DumpAndValidateClocks(EDPClockList,     ARRAY_SIZE(EDPClockList),       bExpectedState);
        uTotalClockNum += ARRAY_SIZE(EDPClockList);
        break;
      }
      default:
      {
  ...
  AsciiPrint("%d of %d clocks have passed\n", uPassClockNum, uTotalClockNum);

  return (uPassClockNum == uTotalClockNum) ? TRUE : FALSE;
}
