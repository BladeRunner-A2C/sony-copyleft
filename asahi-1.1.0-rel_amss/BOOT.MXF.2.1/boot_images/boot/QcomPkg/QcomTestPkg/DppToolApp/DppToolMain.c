/** @file DppToolMain.c
  ...
  Copyright (c) 2010-2013, 2022 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 11/25/22   dnkg    Disabled unsupported commands 
...
#define ENABLE_ERASEALL_LIST_COMMAND 0
...
typedef enum
{
   ...
   ARG_WRITE,
   ARG_READ,
#if ENABLE_ERASEALL_LIST_COMMAND
   ARG_ERASE_ALL,
   ARG_LIST_ALL
#endif
} ArgTypeEnum;
...
const ArgMap argumentList[] =
{
   ...
   {ARG_WRITE, L"/Write", 6, Process_Write, Help_Write},
...
static void PrintShortUsage(void)
{
    ...
    AsciiPrint("DppTool.efi /List  \r\n\
          ...
          | /Write <DPP item type> <Dpp item name> <File contents> <Size of file in bytes>\r\n\
          | /Read <DPP item type> <Dpp item name> \r\n\
          \r\n");
#else
    AsciiPrint("DppTool.efi /Delete <DPP item type> <Dpp item name> \r\n\
          | /Write <DPP item type> <Dpp item name> <File contents> <Size of file in bytes>\r\n\
          | /Read <DPP item type> <Dpp item name> \r\n\
          \r\n");
#endif
...
OPTIONS:\r\n\
    ...
    /Write    - Write a file located in the root of partition to DPP\r\n\
    /Read     - Read a DPP item out to a file in provdump folder of partition\r\n\
    \r\n");
#else
    AsciiPrint("DESCRIPTION:\r\n\
OPTIONS:\r\n\
...
{
   PrintVersionBanner();
   AsciiPrint("DppTool.efi /Write <item_type> <item_name> <file_contents> <file_size_in_bytes>\n");
}
...
static EFI_STATUS Process_Write(IN UINTN argc, IN CHAR16* argv[])
{
  ...
  CHAR16                          *Type                         = argv[2];
  CHAR16                          *DPPName                      = argv[3];
  CHAR16                          *pBuffer											= argv[4];
	CHAR16                          *nBufferSize									= argv[5];

  if ((Type == NULL) || (DPPName == NULL) || (pBuffer == NULL) || (nBufferSize == 0) || (argc != 6) )
  {   
    AsciiPrint("  EFI_INVALID_PARAMETER\n\n");
    return EFI_INVALID_PARAMETER;
  }
  ...
  Status = DppItemWrite ( Type, 
                        DPPName, 
                        (UINT8*)pBuffer,
                        (UINT32)(*nBufferSize) * sizeof(CHAR16));
...
static EFI_STATUS Process_Read(IN UINTN argc, IN CHAR16* argv[])
{
  ...
  HANDLE_ERROR_LABEL(Status, DppItemRead, Process_ReadExit);  
	...
	/* Write to screen */
	PrintDataInHexAndChar((UINT8*)Buffer, BufferSize);
  ...
  )
{
  ...
  if ((*Walker)[0] == L'\"') {
    ...
    if (NextDelim - ((*Walker)+1) == 0) {
      ...
      // found ""
      //
      StrCpyS(*TempParameter, TempParameterSize, L"");
      *Walker = NextDelim + 1;
    } else if (NextDelim != NULL) {
      StrnCpyS(*TempParameter, TempParameterSize, (*Walker)+1, NextDelim - ((*Walker) + 1));
      *Walker = NextDelim + 1;
    } else {
      //
      // last one... someone forgot the training quote!
      //
      StrCpyS(*TempParameter, TempParameterSize, *Walker);
      *Walker = NULL;
    }
    ...
    if (NextDelim != NULL) {
      StrnCpyS(*TempParameter, TempParameterSize, *Walker, NextDelim - (*Walker));
  ...
  )
{
  ...
  for ( Count = 0
      ...
      ; GetNextParameter(&Walker, &TempParameter, Size/ sizeof (CHAR16))
      , Count++
     );
...
*/
  ...
  *Argc = 0;
  ...
  while(Walker != NULL && *Walker != CHAR_NULL) {
    ...
    StrCpyS(NewParam, Size/ sizeof (CHAR16), TempParameter);
