...
/*******************************************************************************
 *               Palawan port configurations for public
 ******************************************************************************/
extern const pcie_port_config_t palawan_port_0_cfg[];

extern const uint8_t palawan_port_0_cfg_len;

pcie_port_options_config_t palawan_target_port_options[] =
{
   {
      ...
      .ports_config_cnt = &palawan_port_0_cfg_len,
      .ports_config_list = palawan_port_0_cfg,
   },
};

static mem_region_t palawan_target_common_mem_regions[] =
{
  /* TCSR register space */
...
pcie_target_config_t palawan_pcie_rp_cfg_data =
{
//    .config_data_version = PCIE_CONFIG_DATA_VERSION,
    .target_chip_id = PCIE_TARGET_CHIP_ID_IGNORE,
    .target_chip_family = EFICHIPINFO_FAMILY_PALAWAN,    //  palawan family
    .target_platform_type = 0, // EFI_PLATFORMINFO_TYPE_CDP,  //   CDP

    .target_port_option_cnt = ARRAY_LENGTH(palawan_target_port_options),
    .target_port_option_list = palawan_target_port_options,
    .entp_mgmt_addr = 0, //(const uint32*)HWIO_ENTP_MGMT2_PCIE_EN_ADDR,
    .max_port_index = TARGET_MAX_PORT_INDEX,
    .common_mem_regions = palawan_target_common_mem_regions,
};

pcie_status_t palawan_pcie_rp_set_hw_port_lanes_config (uint32 port, uint32 option)
{
   return PCIE_SUCCESS;
}

pcie_status_t palawan_pcie_rp_set_default_target_config (uint32 option)
{
   return PCIE_SUCCESS;
}
