...
#
#  Copyright (c) 2020 - 2024 Qualcomm Technologies, Inc. All rights reserved.
...
{
   {.vend_id = PCIE_VENDOR_ID_QCOM, .dev_id = PCIE_DEV_ID_HSP, .reg_offset = 0x10, .size = 0x8, .type_of_data = reg_type_addr},
   {.vend_id = PCIE_VENDOR_ID_QCOM, .dev_id = PCIE_DEV_ID_HMT, .reg_offset = 0x60, .size = 0x8, .type_of_data = reg_type_addr},
   {.vend_id = PCIE_VENDOR_ID_QCOM, .dev_id = PCIE_DEV_ID_MOSELLE, .reg_offset = 0x10, .size = 0x8},
   {.vend_id = PCIE_NVME_VENDOR_ID_PHISON, .dev_id = PCIE_NVME_DEV_ID_PHISON, .reg_offset = 0x2C, .size = 0x4},  // Phison NVMe device
   {.vend_id = PCIE_KIOXIA_NVME_VENDOR_ID, .dev_id = PCIE_KIOXIA_NVME_DEV_ID, .reg_offset = 0x2c, .size = 0x4},
...
//           AsciiPrint ("Speed set to %d\n", req_speed);
   ...
   if (Status == EFI_SUCCESS)
   {
     ...
     if (pcie_disable_loopback (port_handle, LoopBackOption) != PCIE_SUCCESS)
