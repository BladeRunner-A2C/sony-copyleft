  ...
  )
{
  ...
  if ((*Walker)[0] == L'\"') {
    ...
    if (NextDelim - ((*Walker)+1) == 0) {
      ...
      // found ""
      //
      StrCpyS(*TempParameter, StrSize(*TempParameter)/sizeof(CHAR16), L"");
      *Walker = NextDelim + 1;
    } else if (NextDelim != NULL) {
      StrnCpyS(*TempParameter, StrSize(*TempParameter), (*Walker)+1, NextDelim - ((*Walker)+1));
      *Walker = NextDelim + 1;
    } else {
      //
      // last one... someone forgot the training quote!
      //
      StrCpyS(*TempParameter, StrSize(*TempParameter)/sizeof(CHAR16), *Walker);
      *Walker = NULL;
    }
    ...
    if (NextDelim != NULL) {
      StrnCpyS(*TempParameter, StrSize(*TempParameter)/sizeof(CHAR16), *Walker, NextDelim - (*Walker));
...
*/
  ...
  *Argc = 0;
  ...
  while(Walker != NULL && *Walker != CHAR_NULL) {
    ...
    StrCpyS(NewParam, StrSize(TempParameter)/sizeof(CHAR16), TempParameter);
...
SaveProvItems()
{
  ...
  for (Index = 0; Index < NumOfEntriesUsed; Index++) 
  {
    if (pDppDataEntry && pDppDataEntry->Valid) 
    {
      ...
      UINT32 FilePathLen = StrSize(pDppDataEntry->Type) + StrSize(pDppDataEntry->Name) + StrSize(OUTPUT_DIR_NAME) + 2 * StrSize(L"\\") ;
      FilePath = AllocateZeroPool(FilePathLen);
      if(FilePath == NULL)
      {
        Status = EFI_OUT_OF_RESOURCES;
        goto SaveProvItemsExit;
      ...
      StrCatS(FilePath, FilePathLen, L"\\");
      StrCatS(FilePath, FilePathLen, OUTPUT_DIR_NAME);
      StrCatS(FilePath, FilePathLen, L"\\");
      StrCatS(FilePath, FilePathLen, pDppDataEntry->Type);
      StrCatS(FilePath, FilePathLen, L"__");
      StrCatS(FilePath, FilePathLen, pDppDataEntry->Name);
