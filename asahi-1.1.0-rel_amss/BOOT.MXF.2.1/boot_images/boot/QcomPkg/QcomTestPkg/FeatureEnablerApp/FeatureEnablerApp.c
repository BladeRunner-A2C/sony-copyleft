...
static EFI_STATUS FeatureEnablerQueryStatusTest(void)
{
  ...
  else if (NULL != pFeatureEnablerProtocol->FeatureEnablerQueryStatus)
  {
    ...
    else
    {
      ...
      UINT32               uFailCount    = 0;
      volatile UINT32      uUninitCount  = 0;
...
static UINTN FeatureEnablerParseCommand(CHAR8 *pCommandLine, CHAR8 *pParam, CHAR8 *pValue)
{
      ...
      (strlength > 2))
  {
    ...
    // Copy the paramter
    AsciiStrnCpyS(pParam, MAX_STRING, &pCommandLine[1], uCount - 1);
    pParam[uCount-1] = '\0';
      ...
      ((uCount + 1)<strlength))
    {
      ...
      AsciiStrnCpyS(pValue, MAX_STRING, &pCommandLine[uCount + 1], strlength - uCount - 1);
