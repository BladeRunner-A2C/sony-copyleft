 ...
 *
 * Release Version: 21.06.23.10.30 (Jun 23 2021 @ 10:30)
 *
...
=============================================================================
 ...
 $Header: //service/BOOT/BOOT.MXF.2.1-01839-LANAI-2.65818/boot_images/boot/QcomPkg/Tools/storage/fh_loader/fh_loader/fh_loader.c#1 $
...
char *CommandLineOptions[] = {  "benchmarkdigestperformance", "benchmarkreads", "benchmarkwrites",
                                ...
                                "slot=", "start_sector=",
                                "verbose", "verify_programming_getsha", "verify_programming",
                                "zlpawarehost="
                             };
...
int main (int argc, char *argv[])
{
    ...
    */
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        {
            dbg (LOG_WARN, "--createconfigxml parameter is deprecated and scheduled to be removed");
            createconfigxml = 1;
            Simulate = 1;
        }
        ...
        {
            dbg (LOG_WARN, "--erasefirst parameter is deprecated and scheduled to be removed");
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        else if (strncmp(MyOpt, "port_type=", MAX(strlen(MyOpt), strlen("port_type="))) == 0)
        {
          ...
          if (i == num_port_types)
          {
            dbg(LOG_INFO, "Invalid port_type specified, possible values are:");
            for (i = 0; i < num_port_types; i++)
        ...
        {
            dbg (LOG_WARN, "--interactive= parameter is deprecated and scheduled to be removed");
        ...
        {
            dbg (LOG_WARN, "--dontsorttags parameter is deprecated and scheduled to be removed");
        ...
        {
            dbg (LOG_WARN, "--showdigestperpacket parameter is deprecated and scheduled to be removed");
        ...
        {
            dbg (LOG_WARN, "--showdigestperfile parameter is deprecated and scheduled to be removed");
        ...
        {
            dbg (LOG_WARN, "--createcommandtrace parameter is deprecated and scheduled to be removed");
        ...
        else if (strncmp (MyOpt, "createdigests", MAX (strlen (MyOpt), strlen ("createdigests") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--createdigests parameter is deprecated and scheduled to be removed, use --createvipdigests= instead.");
            CreateDigests   = 1;
            Simulate      = 1;
        ...
        else if (strncmp (MyOpt, "createvipdigests=", MAX (strlen (MyOpt), strlen ("createvipdigests") ) ) == 0)
        {
          ...
          if (i == num_types)
          {
            dbg(LOG_INFO, "Invalid VIP digest specified in createvipdigests, possible values are:");
            for (i = 0; i < num_types; i++)
        ...
        else if (strncmp (MyOpt, "skipstorageinit", MAX (strlen (MyOpt), strlen ("skipstorageinit") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--skipstorageinit parameter is no longer needed for targets after MSM8998");
        ...
        else if (strncmp (MyOpt, "readbogusdata", MAX (strlen (MyOpt), strlen ("readbogusdata") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--readbogusdata parameter is deprecated and scheduled to be removed");
        ...
        else if (strncmp (MyOpt, "comportopentimeout=", MAX (strlen (MyOpt), strlen ("comportopentimeout=") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--comportopentimeout= parameter is deprecated and scheduled to be removed");
            continue;
        }
        else if (strncmp (MyOpt, "maxdigesttablesizeinbytes=", MAX (strlen (MyOpt), strlen ("maxdigesttablesizeinbytes=") ) ) == 0)
        {
            dbg (LOG_WARN, "--maxdigesttablesizeinbytes= parameter is deprecated and scheduled to be removed");
        ...
        {
          dbg (LOG_WARN, "--forceoverwrite parameter is deprecated and scheduled to be removed");
          ForceOverwrite = 1;
        }
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        {
            dbg (LOG_WARN, "--getgptmainbackup= parameter is deprecated and scheduled to be removed");
            InitBufferWithXMLHeader (tx_buffer, sizeof (tx_buffer) );
            AppendToBuffer (tx_buffer, "<data>\n", FIREHOSE_TX_BUFFER_SIZE);
        ...
        {
            dbg (LOG_WARN, "--verify_build parameter is deprecated and scheduled to be removed");
        ...
        else if (strncmp (MyOpt, "stresstest", MAX (strlen (MyOpt), strlen ("stresstest") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--stresstest= parameter is deprecated and scheduled to be removed");
...
unsigned int sendTransmitBuffer (void)
{
  ...
  if (tx_res == SUCCESS)
  {
    dbg (LOG_DEBUG, "CHANNEL DATA (P%"SIZE_T_FORMAT4") (H%"SIZE_T_FORMAT5") (%7lld bytes) - HOST TO TARGET -->\n"
         "%s",
...
void time_throughput_calculate (struct timeval *start_time, struct timeval *end_time, uint64 size_bytes, double NetworkElapsed, enum fh_log_lvl ThisLogType)
{
    ...
    {
      dbg (ThisLogType, "Time measurement too small --- Overall to target 0.0 seconds (0Bps)");
      return;
    }
