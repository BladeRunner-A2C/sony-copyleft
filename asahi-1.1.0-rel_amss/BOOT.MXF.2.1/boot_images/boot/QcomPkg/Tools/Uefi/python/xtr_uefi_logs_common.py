...
# --------   ---     --------------------------------------------------------
# 04/24/23   bxr     Add minidump extract                                    
...
def get_ddr_baseaddr_and_filename(dump_path, infoblk_base_addr):
  ...
  try:
    ...
    with open(os.path.join(dump_path, dumpinfo_txt), 'r') as dumpinfo_file:
      length = 0
      base_addr = 0
      ddr_base_addr = 0
...
##############################################################################
# extract_uefi_log_from_minidump
##############################################################################
def extract_uefi_log_from_minidump(dump_path, path_to_save_log):
  ...
  try:
    ...
    with open(os.path.join(dump_path, dumpinfo_txt), 'r') as dumpinfo_file:
      uefilog_length = 0
      base_addr = 0
      uefilog_dump_filename = ''
      for cur_line in dumpinfo_file:
        cur_line = cur_line.strip()
        file_name = cur_line.split()[-1]
        #capture the line which include the filename of "md_UEFI_LOG.BIN"
        if re.match("^(md_UEFI_LOG)(\S+)(BIN)$", file_name):
          uefilog_dump_filename = file_name
          base_addr = int(cur_line.split()[1], 16)
          uefilog_length = int(cur_line.split()[2], 16)
          break
      dumpinfo_file.close()
      #read raw data from uefilog dump file and store to output file
      if uefilog_dump_filename not in os.listdir(dump_path):
        print("Failed to get uefilog dump file %s from %s" % (uefilog_dump_filename, dump_path))
        sys.exit()
      uefilog_dump_file = open(os.path.join(dump_path, uefilog_dump_filename), "rb")
      if uefilog_length:
        uart_log = uefilog_dump_file.read(uefilog_length)
        uefilog_file = open(path_to_save_log, "wb")
        uefilog_file.write(uart_log)
        uefilog_file.close()
        uefilog_size = os.path.getsize(path_to_save_log)
      uefilog_dump_file.close()
  except Exception as err:
    print(err)
    sys.exit("ERROR. Failed to extract uefilog from minidump.")

  return hex(uefilog_size)


##############################################################################
# extract_uefi_log_from_fulldump
##############################################################################
def extract_uefi_log_from_fulldump(dump_path, target_tool_path, qcompkg_tool_path,\
...
def extract_uefilog_and_size(dump_path, target_tool_path, qcompkg_tool_path,\
  ...
  if dumpinfo_txt not in os.listdir(dump_path):
    print("Failed to get dump.txt from dump path %s" % dump_path)
    sys.exit(0)
  ...
  try:
    ...
    with open(os.path.join(dump_path, dumpinfo_txt), 'r') as dumpinfo_file:
      for cur_line in dumpinfo_file:
        cur_line = cur_line.strip()
        file_name = cur_line.split()[-1]
        if file_name.startswith("md_"):
          dump_flag = MINIDUMP
          break;
      dumpinfo_file.close()
  except Exception as err:
    print("Failed to read file %s" % os.path.join(dump_path, dumpinfo_txt))
    sys.exit()
  
  if (dump_flag == FULLDUMP):
    print("Extract uefilog from fulldump.")
    uefilog_size = extract_uefi_log_from_fulldump(dump_path, target_tool_path, qcompkg_tool_path,\
                                                  path_to_save_log, infoblk_addr)
  else:
    print("Extract uefilog from minidump.")
    uefilog_size = extract_uefi_log_from_minidump(dump_path, path_to_save_log)
