#
#  This is a python script for plotting the UFS eye 
#
...
#  ----------  -----   -----------------------------------------------------------
#  2023-08-31   rp     X and Y axis as per M-PHY and support for EYE-Mask
#  2023-07-26   rp     Initial version
...
class ufs_eye_monitor_plot(object):
    ...
    def plot_eye(self, lane_no):
        ...
        if self.ufs_device_mfr_name != '':
            title_1 = lane_title + ' : ' + self.ufs_device_mfr_name + ', ' + self.ufs_device_mfr_id + ' ' + self.ufs_device_size + ' ' + self.ufs_version_no + ' '  + self.ufs_gear
        else:
            title_1 = lane_title + ' : < UFS Device information missing in the log > '
        ...
        title = title_1 + '\n' + title_2 + '\n' + title_3
        ax2.set_title(title, fontsize=20, fontweight='bold', loc='left')
...
def print_usage():
    print('-' * 60)
    print('{:s} <abc.csv>'.format(sys.argv[0]))
    print('-' * 60)
