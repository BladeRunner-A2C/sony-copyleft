...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 01/24/23   pw      Added Tertiary Port, MP ports
 07/21/21   vpal    Added USB3,USB4 HLOS config for ports
 06/09/21   pw      Added usb4 enable flag  
 06/09/21   pw      Added enumerate fastboot in secondary port
 06/09/21   pw      Added force primary and secondary to enumerate in HS  
 05/28/21   jaychoi Added force Retimer to DP4LANE mode to unit test retimer
 03/26/21   jaychoi Fixed Wake On Lan not working during Offmode Charging
 10/28/21   sch     Add support for boot Cfg selection
 05/08/20   ns      Added support for ChargerPD logging level and timer
 02/14/20   am      Added support for Toggle MemMap
...
#define DEFAULT_MDMCRASHRECOVERY              (0)
/* BDS nCPU ID type */
typedef enum {
  ///
  /// CPU Clock ID
...
ToggleEnableMemMap (CHAR8* Arg)
{
  ...
  if (QueryToggleFlag ("Memory Map Display", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"EnableMemMap", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
ToggleDGPU (CHAR8* Arg)
{
  ...
  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"EnableDGPUBoot", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);

  if(Status != EFI_SUCCESS)
    Flag = 0;

  if (QueryToggleFlag ("Toggle DGPU", &Flag, FALSE) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"EnableDGPUBoot", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
MountFvCommand (UINTN  Argc, CHAR8  **Argv)
{
  ...
  switch (Argc)
  {
    ...
    case 2:
        ...
        {
            AsciiStrnCpyS(GuidStr, GUID_STRING_LENGTH + 1, Argv[2], GUID_STRING_LENGTH+1);
            Ch = &GuidStr[0];
            while (*Ch != '\0') {
                if (*Ch == '-')
                    *Ch = ' ';
...
/*
 * Handler to set ChargerPDLogLevel 
 */
EFI_STATUS
SetChargerPDLogLevel(CHAR8 *Arg)
{
  EFI_STATUS    Status          = EFI_SUCCESS;
  INT8          Flag            = 0;
  UINTN         VarSize         = sizeof(Flag);
  CONST UINT8   DefaultLogLevel = 4;
  CONST UINT8   MinVal          = 0;
  CONST UINT8   MaxVal          = 5;
  CONST CHAR8   *FeatureString  = "Set Charger PD Log Level";
  
  // Query ChargerPDLogLevel variable
  ...
  if (Status != EFI_SUCCESS) { Flag = DefaultLogLevel; }

  // allow the user to set the new Log Level
  Status = QuerySetFlag(FeatureString, NULL, &Flag, MinVal, MaxVal);

  if (Status != EFI_SUCCESS) { return EFI_SUCCESS; }

  // store the change for ChargerPDLogLevel variable
  VarSize = sizeof(Flag);
  Status = gRT->SetVariable(L"ChargerPDLogLevel", &gQcomTokenSpaceGuid,
                            (EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE),
                            VarSize, &Flag);
                            
  if (Status == EFI_SUCCESS)
  {
    AsciiPrint("Updated ChargerPD Log Level successfully.\r\n");
  }
  else
  {
    AsciiPrint("ERROR: Unable to change/store setting\r\n");
...
/*
 * Handler to set ChargerPDLogTimer
 */
EFI_STATUS
SetChargerPDLogTimer(CHAR8 *Arg)
{
  EFI_STATUS    Status          = EFI_SUCCESS;
  INT8          Flag            = 0;
  UINTN         VarSize         = sizeof(Flag);
  CONST UINT8   DefaultLogTimer = 10; //in seconds
  CONST UINT8   MinVal          = 0;
  CONST UINT8   MaxVal          = 60;  //in seconds
  CONST CHAR8   *FeatureString  = "Set Charger PD Log Timer";
  
  // Query ChargerPDLogTimer variable
  Status = gRT->GetVariable(L"ChargerPDLogTimer", &gQcomTokenSpaceGuid, NULL, &VarSize, &Flag);
  
  // if the variable does not exit, use the default value
  if (Status != EFI_SUCCESS) { Flag = DefaultLogTimer; }

  // allow the user to set the new Log Timer
  Status = QuerySetFlag(FeatureString, NULL, &Flag, MinVal, MaxVal);
 
  if (Status != EFI_SUCCESS) { return EFI_SUCCESS; }

  // store the change for ChargerPDLogTimer variable
  VarSize = sizeof(Flag);
  Status = gRT->SetVariable(L"ChargerPDLogTimer", &gQcomTokenSpaceGuid,
                            (EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE),
                            VarSize, &Flag);

  if (Status == EFI_SUCCESS)
  {
    AsciiPrint("Updated ChargerPD Log Timer successfully.\r\n");
  }
  else
  {
    AsciiPrint("ERROR: Unable to change/store setting\r\n");
...
ToggleChargingLimit(VOID)
{
    ...
    UINT8 Mask = 0x1;
    UINTN VarSize = 0;
    UINT8 ChargingLimit = 0;

    VarSize = sizeof(VarValue);

    Status = gRT->GetVariable(L"EnableChargingLimit", &gOSAVendorGuid,
        NULL, &VarSize, &VarValue);
    ...
    if (Status == EFI_NOT_FOUND) 
        ...
        // bit 0 of charging limit represents if charge limiting is enabled or not
        ChargingLimit = (VarValue & Mask);

    if (QueryToggleFlag("Toggle Charging Limit", &ChargingLimit, NON_INVERTED_MEANING) != EFI_SUCCESS)
        return EFI_SUCCESS;

    VarSize = sizeof(ChargingLimit);


    Status = gRT->SetVariable(L"EnableChargingLimit", &gOSAVendorGuid,
        EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
        VarSize, &ChargingLimit);

    if (Status == EFI_SUCCESS)
    {
        AsciiPrint("Charging Limit");
        if (ChargingLimit == 0)
            AsciiPrint("Disable(Deleted). \r\n");
        else
            AsciiPrint("Enabled(Set). \r\n");
...
EFI_STATUS
ToggleBatteryAuthentication(VOID)
{
    // create the variable if it does not exist
    EFI_STATUS Status;
    UINT8 VarValue = 0;
    ...
    Status = gRT->GetVariable(L"EnableBatteryAuthentication", &gOSAVendorGuid,
        NULL, &VarSize, &VarValue);
    ...
    if (Status == EFI_NOT_FOUND) 
        ...
        BattAuth = (VarValue & Mask);

    if (QueryToggleFlag("Toggle Battery Authentication", &BattAuth, NON_INVERTED_MEANING) != EFI_SUCCESS)
        return EFI_SUCCESS;

    VarSize = sizeof(BattAuth);


    Status = gRT->SetVariable(L"EnableBatteryAuthentication", &gOSAVendorGuid,
        EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
        VarSize, &BattAuth);

    if (Status == EFI_SUCCESS)
    {
        AsciiPrint("Battery Authentication");
        if (BattAuth == 0)
            AsciiPrint("Disable(Deleted). \r\n");
        else
            AsciiPrint("Enabled(Set). \r\n");
...
EFI_STATUS
ToggleUnauthenticatedBatteryCharging(VOID)
{
    // create the variable if it does not exist
    EFI_STATUS Status;
    UINT8 VarValue = 0;
    UINT8 Mask = 0x1;
    UINTN VarSize = 0;
    UINT8 ChargingEnable = 0;

    VarSize = sizeof(VarValue);

    Status = gRT->GetVariable(L"UnauthenticatedBatteryChargingEnable", &gOSAVendorGuid,
        NULL, &VarSize, &VarValue);
    ...
    if (Status == EFI_NOT_FOUND) 
        ...
        ChargingEnable = (VarValue & Mask);

    if (QueryToggleFlag("Toggle Unauthenticated Battery Charging", &ChargingEnable, NON_INVERTED_MEANING) != EFI_SUCCESS)
        return EFI_SUCCESS;

    VarSize = sizeof(ChargingEnable);


    Status = gRT->SetVariable(L"UnauthenticatedBatteryChargingEnable", &gOSAVendorGuid,
        EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
        VarSize, &ChargingEnable);

    if (Status == EFI_SUCCESS)
    {
        AsciiPrint("Unauthenticated Battery Charging");
        if (ChargingEnable == 0)
            AsciiPrint("Disable(Deleted). \r\n");
        else
            AsciiPrint("Enabled(Set). \r\n");
...
/*
 * Handler to set Boot Cfg
 */
EFI_STATUS
SetBootThresholdCfgOption (CHAR8 *Arg)
{
  EFI_STATUS    Status          = EFI_SUCCESS;
  INT8          Flag            = 0;
  UINTN         VarSize         = sizeof(Flag);
  CONST UINT8   DefaultBootCfg  = 0;
  CONST UINT8   MinVal          = 0;  // vbatt based cfg
  CONST UINT8   MaxVal          = 1;  //soc based Cfg
  CONST CHAR8   *FeatureString  = "Enable/Disable SOC based boot Cfg";
  
  // Query Boot Cfg
  Status = gRT->GetVariable(L"SocBasedBoot", &gQcomTokenSpaceGuid, NULL, &VarSize, &Flag);
  
  // if the variable does not exit, use the default value
  if (Status != EFI_SUCCESS) { Flag = DefaultBootCfg; }

  // allow the user to set the new cfg
  Status = QuerySetFlag(FeatureString, NULL, &Flag, MinVal, MaxVal);
 
  if (Status != EFI_SUCCESS) { return EFI_SUCCESS; }
  VarSize = sizeof(Flag);
  Status = gRT->SetVariable(L"SocBasedBoot", &gQcomTokenSpaceGuid,
                            (EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE),
                            VarSize, &Flag);

  if (Status == EFI_SUCCESS)
  {
    AsciiPrint("Updated boot cfg successfully.\r\n");
  }
  else
  {
    AsciiPrint("ERROR: Unable to change Boot Cfg\r\n");
  }
...
ToggleMDMCrashRecovery(VOID)
{
    ...
    UINT8 Mask = 0x1;
    UINTN VarSize = 0;
    UINT8 MDMCrashRecovery = DEFAULT_MDMCRASHRECOVERY;

    VarSize = sizeof(VarValue);

    Status = gRT->GetVariable (L"MDMCrashRecovery", &gOSAVendorGuid,
               NULL, &VarSize, &VarValue);

    if (Status == EFI_NOT_FOUND)
      MDMCrashRecovery = 0;
    else
      // bit 0 of MDMCrashRecovery NV represents if mdm crash recovery is enabled or not
      MDMCrashRecovery = (VarValue & Mask);

    if (QueryToggleFlag ("MDM SDX65", &MDMCrashRecovery, INVERTED_MEANING) != EFI_SUCCESS)
      return EFI_SUCCESS;

    VarSize = sizeof(MDMCrashRecovery);
    
    Status = gRT->SetVariable (L"MDMCrashRecovery", &gOSAVendorGuid,
                 EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                 VarSize, &MDMCrashRecovery);

  if (Status == EFI_SUCCESS)
  {
      AsciiPrint("MDM ");
    if (MDMCrashRecovery == 0)
      AsciiPrint("Enabled(Set). \r\n");
    else
      AsciiPrint("Disable(Deleted). \r\n");
  }
...
// USB Variable Default Value
#define DEFAULT_USBCONFIGPRIMARYPORT          (3)
#define DEFAULT_USBCONFIGSECONDARYPORT        (3)
#define DEFAULT_USBCONFIGTERTIARYPORT         (3)
#define DEFAULT_USBCONFIGMULTIPORT            (1)
#define DEFAULT_USBDEVICEPORT                 (0)
#define DEFAULT_USBFNMAXSPEED                 (0)
#define DEFAULT_FORCEUSBDEVICEMODECOMPLIANCE  (0)
#define DEFAULT_FORCEQCUSBFNHLOS              (0)
#define DEFAULT_ENABLESLTLOOPBACK             (0)
#define DEFAULT_USBCCLANEOVERRIDE             (0)
#define DEFAULT_BOOTUSBFIRSTDELAYVALUE        (30)
#define DEFAULT_HIGHSPEEDONLYFORPRIMARY       (0)
#define DEFAULT_HIGHSPEEDONLYFORSECONDARY     (0)
#define DEFAULT_HIGHSPEEDONLYFORTERTIARY      (0)
#define DEFAULT_HIGHSPEEDONLYFORMP            (0)
#define DEFAULT_ENABLEUSB4UEFI                (0)
#define DEFAULT_WAKEONLAN                     (0)
#define DEFAULT_FORCEUSBWAKEONLANINUEFI       (0)
#define DEFAULT_FORCEUSB4RETIMERDP4LANE       (0)
#define DEFAULT_USB3HLOSCONFIGPRIMARYPORT     (1)
#define DEFAULT_USB3HLOSCONFIGSECONDARYPORT   (1)
#define DEFAULT_USB3HLOSCONFIGTERTIARYPORT    (1)
#define DEFAULT_USB4HLOSCONFIGPRIMARYPORT     (1)
#define DEFAULT_USB4HLOSCONFIGSECONDARYPORT   (1)
#define DEFAULT_USB4HLOSCONFIGTERTIARYPORT    (1)
#define DEFAULT_USBMULTIPORTDISABLEBITFIELD   (0)
#define DEFAULT_USBDPMODETERTIARY             (0)


/*
 * Handler to Set SetUsbConfigPrimaryPort variable to modify
...
SetUsbConfigPrimaryPort (CHAR8* Arg)
{
  ...
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USBCONFIGPRIMARYPORT;
  CONST UINT8   MinVal     = 1;
  CONST UINT8   MaxVal     = 4;
...
/*
 * Handler to Set SetUsbConfigSecondaryPort variable to modify
...
SetUsbConfigSecondaryPort (CHAR8* Arg)
{
  ...
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USBCONFIGSECONDARYPORT;
  CONST UINT8   MinVal     = 1;
  CONST UINT8   MaxVal     = 4;
...
/*
 * Handler to Set SetUsbConfigTertiaryPort variable to modify
 * the configuration of the USB tertiary cores.
 */
#define  USB_CONFIG_TERTIARY_PORT_STRING    " 1 = Host Mode"      "\r\n" \
                                            " 2 = Device mode (FFU, MassStorage)"     "\r\n" \
                                            " 3 = Dual Role (Based on type of cable) \r\n" \
                                            " 4 = Disabled"
EFI_STATUS
SetUsbConfigTertiaryPort (CHAR8* Arg)
{
  EFI_STATUS    Status;
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USBCONFIGTERTIARYPORT;
  CONST UINT8   MinVal     = 1;
  CONST UINT8   MaxVal     = 4;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"UsbConfigTertiaryPort", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QuerySetFlag ("USB Configuration on Tertiary Port", USB_CONFIG_TERTIARY_PORT_STRING, &Flag, MinVal, MaxVal) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"UsbConfigTertiaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler to Set SetUsbConfigMultiPort variable to modify
 * the configuration of the USB tertiary cores.
 */
#define  USB_CONFIG_MP_PORT_STRING          " 1 = Host Mode"      "\r\n" \
                                            " 2 = INVALID"        "\r\n" \
                                            " 3 = INVALID"        "\r\n" \
                                            " 4 = Disabled"
EFI_STATUS
SetUsbConfigMultiPort (CHAR8* Arg)
{
  EFI_STATUS    Status;
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USBCONFIGMULTIPORT;
  CONST UINT8   MinVal     = 1;
  CONST UINT8   MaxVal     = 4;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"UsbConfigMultiPort", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QuerySetFlag ("USB Configuration on Multi Port", USB_CONFIG_MP_PORT_STRING, &Flag, MinVal, MaxVal) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"UsbConfigMultiPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler to SetUsbDevicePort variable
 * To enable fastboot/MSD on secondary Port.
 */
#define  USB_CONFIG_DEVICE_PORT_STRING      " 0 = Primary"      "\r\n" \
                                            " 1 = Secondary"     "\r\n" \
                                            " 2 = Tertiary"
EFI_STATUS
SetUsbDevicePort (CHAR8* Arg)
{
  EFI_STATUS    Status;
  INT8          Flag = 0;
  CONST UINT8   DefaultVal = DEFAULT_USBDEVICEPORT;
  CONST UINT8   MinVal     = 0;
  CONST UINT8   MaxVal     = 2;
  UINTN         VarSize;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"UsbDevicePort", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QuerySetFlag ("Force Fastboot or Masstorage Enumerate In Port", USB_CONFIG_DEVICE_PORT_STRING, &Flag, MinVal, MaxVal) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"UsbDevicePort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
SetUsbfnMaxSpeed(CHAR8* Arg)
{
  ...
  INT8          MaxSpeed;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USBFNMAXSPEED;
  CONST UINT8   MinVal     = 0;
  CONST UINT8   MaxVal     = 5;
...
{
  EFI_STATUS    Status;
  UINT8         Flag = DEFAULT_FORCEUSBDEVICEMODECOMPLIANCE;
  UINTN         VarSize;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"ForceUsbDeviceModeCompliance", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"ForceUsbDeviceModeCompliance", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
{
  EFI_STATUS    Status;
  UINT8         Flag = DEFAULT_FORCEQCUSBFNHLOS;
  UINTN         VarSize;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"ForceQcUsbFnHlos", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"ForceQcUsbFnHlos", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
{
  EFI_STATUS    Status;
  UINT8         Flag = DEFAULT_ENABLESLTLOOPBACK;
  UINTN         VarSize;
...
SetUsbCCLaneOverride (CHAR8* Arg)
{
  ...
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USBCCLANEOVERRIDE;
  CONST UINT8   MinVal     = 0;
  CONST UINT8   MaxVal     = 2;
...
/*
 * Handler for EnableHighSpeedOnlyForPrimary Command for Primary Port only.
 */
EFI_STATUS
EnableHighSpeedOnlyForPrimary (CHAR8* Arg)
{
  EFI_STATUS    Status  = 0;
  UINT8         Flag    = DEFAULT_HIGHSPEEDONLYFORPRIMARY;
  UINTN         VarSize = 0;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"HighSpeedOnlyForPrimary", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QueryToggleFlag ("Force Usbfn HighSpeed Enumeration for Primary Port", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"HighSpeedOnlyForPrimary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler for EnableHighSpeedOnlyForSecondary Command for Secondary Port only.
 */
EFI_STATUS
EnableHighSpeedOnlyForSecondary (CHAR8* Arg)
{
  EFI_STATUS    Status  = 0;
  UINT8         Flag    = DEFAULT_HIGHSPEEDONLYFORSECONDARY;
  UINTN         VarSize = 0;


  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"HighSpeedOnlyForSecondary", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QueryToggleFlag ("Force HighSpeed Enumeration for Secondary Port", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"HighSpeedOnlyForSecondary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler for EnableHighSpeedOnlyForTertiary Command for Tertiary Port only.
 */
EFI_STATUS
EnableHighSpeedOnlyForTertiary (CHAR8* Arg)
{
  EFI_STATUS    Status;
  UINT8         Flag = DEFAULT_HIGHSPEEDONLYFORTERTIARY;
  UINTN         VarSize;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"HighSpeedOnlyForTertiary", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QueryToggleFlag ("Force HighSpeed Enumeration for Tertiary Port", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"HighSpeedOnlyForTertiary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler for EnableHighSpeedOnlyForMP Command for Multi Port only.
 */
EFI_STATUS
EnableHighSpeedOnlyForMP (CHAR8* Arg)
{
  EFI_STATUS    Status;
  UINT8         Flag = DEFAULT_HIGHSPEEDONLYFORMP;
  UINTN         VarSize;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"HighSpeedOnlyForMP", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QueryToggleFlag ("Force HighSpeed Enumeration for Multi Port", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"HighSpeedOnlyForMP", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler for ToggleEnableUsb4Uefi Command in UEFI
 */
EFI_STATUS
ToggleEnableUsb4Uefi (CHAR8* Arg)
{
  EFI_STATUS    Status;
  UINT8         Flag = DEFAULT_ENABLEUSB4UEFI;
  UINTN         VarSize;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"EnableUsb4Uefi", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QueryToggleFlag ("Enable usb4 in Uefi", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"EnableUsb4Uefi", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
ToggleWakeOnLAN(VOID)
{
  // create the variable if it does not exist
    ...
    UINT8 Mask = 0x1;
    UINTN VarSize = 0;
    UINT8 WakeOnLAN = DEFAULT_WAKEONLAN;

    VarSize = sizeof(VarValue);

    Status = gRT->GetVariable (L"WakeOnLAN", &gOSAVendorGuid,
               NULL, &VarSize, &VarValue);

    if (Status == EFI_NOT_FOUND)
      WakeOnLAN = 0;
    else
      // bit 0 of WakeOnLAN NV represents if wake on lan is enabled or not
      WakeOnLAN = (VarValue & Mask);

    if (QueryToggleFlag ("Toggle Wake on LAN enable", &WakeOnLAN, NON_INVERTED_MEANING) != EFI_SUCCESS)
      return EFI_SUCCESS;

    VarSize = sizeof(WakeOnLAN);
    

    Status = gRT->SetVariable (L"WakeOnLAN", &gOSAVendorGuid,
                 EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                 VarSize, &WakeOnLAN);

  if (Status == EFI_SUCCESS)
  {
      AsciiPrint("Wake on LAN ");
    if (WakeOnLAN == 0)
      AsciiPrint("Disable(Deleted). \r\n");
    else
      AsciiPrint("Enabled(Set). \r\n");
...
/*
 * Handler for Toggle forcing Wake On Lan in UEFI to unit test the feature in UEFI menu
 */
EFI_STATUS
ToggleForceUsbWakeOnLanInUefi(VOID)
{
  // create the variable if it does not exist
  EFI_STATUS Status;
  UINT8 VarValue = 0;    
  UINT8 Mask = 0x1;
  UINTN VarSize = 0;
  UINT8 ForceWakeOnLAN = DEFAULT_FORCEUSBWAKEONLANINUEFI;

  VarSize = sizeof(VarValue);

  Status = gRT->GetVariable (L"ForceUsbWakeOnLanInUefi", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &VarValue);

  if (Status == EFI_NOT_FOUND)
    ForceWakeOnLAN = DEFAULT_FORCEUSBWAKEONLANINUEFI;
  else
    // bit 0 of ForceUsbWakeOnLanInUefi NV represents if wake on lan is enabled or not
    ForceWakeOnLAN = (VarValue & Mask);

  if (QueryToggleFlag ("Toggle Force Wake on LAN in UEFI", &ForceWakeOnLAN, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(ForceWakeOnLAN);

  Status = gRT->SetVariable (L"ForceUsbWakeOnLanInUefi", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &ForceWakeOnLAN);

  if (Status == EFI_SUCCESS)
  {
    AsciiPrint("Force Wake on LAN in UEFI ");
    if (ForceWakeOnLAN == 0)
      AsciiPrint("Disable(Deleted). \r\n");
    else
      AsciiPrint("Enabled(Set). \r\n");
...
/*
 * Handler for Toggle forcing USB4 Retimer to DP4LANE Mode in Windows in case Windows does not have Retimer driver
 */
EFI_STATUS
ToggleForceUsb4RetimerDP4LANE(VOID)
{
  // create the variable if it does not exist
  EFI_STATUS Status;
  UINT8 VarValue = 0;    
  UINT8 Mask = 0x1;
  UINTN VarSize = 0;
  UINT8 ForceDP4LANE = DEFAULT_FORCEUSB4RETIMERDP4LANE;

  VarSize = sizeof(VarValue);

  Status = gRT->GetVariable (L"ForceUsb4RetimerDP4LANE", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &VarValue);

  if (Status == EFI_NOT_FOUND)
    ForceDP4LANE = DEFAULT_FORCEUSB4RETIMERDP4LANE;
  else
    // bit 0 of ForceUsb4RetimerDP4LANE NV represents if enabled or not
    ForceDP4LANE = (VarValue & Mask);

  if (QueryToggleFlag ("Toggle Force USB4 Retimer to DP4LANE Mode", &ForceDP4LANE, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(ForceDP4LANE);

  Status = gRT->SetVariable (L"ForceUsb4RetimerDP4LANE", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &ForceDP4LANE);

  if (Status == EFI_SUCCESS)
  {
      AsciiPrint("Force USB4 Retimer to DP4LANE Mode");
    if (ForceDP4LANE == 0)
      AsciiPrint("Disable(Deleted). \r\n");
    else
      AsciiPrint("Enabled(Set). \r\n");
...
/*
 * Handler to Set SetUsb3HLOSConfigPrimaryPort variable to modify
 * the configuration of the USB Primary core _STA in ACPI.
 */
#define  USB3_HLOS_CONFIG_PRIMARY_PORT_STRING   " 1 = ACPI Default"      "\r\n" \
                                                " 2 = Force Enable"     "\r\n" \
                                                " 3 = Force Disable      \r\n"

EFI_STATUS
SetUsb3HLOSConfigPrimaryPort (CHAR8* Arg)
{
  EFI_STATUS    Status;
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USB3HLOSCONFIGPRIMARYPORT;
  CONST UINT8   MinVal     = 1;
  CONST UINT8   MaxVal     = 3;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"Usb3HLOSConfigPrimaryPort", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QuerySetFlag ("USB3 HLOS Configuration on Primary Port", USB3_HLOS_CONFIG_PRIMARY_PORT_STRING, &Flag, MinVal, MaxVal) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"Usb3HLOSConfigPrimaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler to Set SetUsb3HLOSConfigSecondaryPort variable to modify
 * the configuration of the USB Secondary core device _STA in ACPI.
 */
#define  USB3_HLOS_CONFIG_SECONDARY_PORT_STRING   " 1 = ACPI Default"      "\r\n" \
                                                  " 2 = Force Enable"     "\r\n" \
                                                  " 3 = Force Disable      \r\n"

EFI_STATUS
SetUsb3HLOSConfigSecondaryPort (CHAR8* Arg)
{
  EFI_STATUS    Status;
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USB3HLOSCONFIGSECONDARYPORT;
  CONST UINT8   MinVal     = 1;
  CONST UINT8   MaxVal     = 3;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"Usb3HLOSConfigSecondaryPort", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QuerySetFlag ("USB3 HLOS Configuration on Secondary Port", USB3_HLOS_CONFIG_SECONDARY_PORT_STRING, &Flag, MinVal, MaxVal) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"Usb3HLOSConfigSecondaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler to Set SetUsb3HLOSConfigTertiaryPort variable to modify
 * the configuration of the USB Tertiary core device _STA in ACPI.
 */
#define  USB3_HLOS_CONFIG_TERTIARY_PORT_STRING   " 1 = ACPI Default"      "\r\n" \
                                                 " 2 = Force Enable"     "\r\n" \
                                                 " 3 = Force Disable      \r\n"

EFI_STATUS
SetUsb3HLOSConfigTertiaryPort (CHAR8* Arg)
{
  EFI_STATUS    Status;
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USB3HLOSCONFIGTERTIARYPORT;
  CONST UINT8   MinVal     = 1;
  CONST UINT8   MaxVal     = 3;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"Usb3HLOSConfigTertiaryPort", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QuerySetFlag ("USB3 HLOS Configuration on Tertiary Port", USB3_HLOS_CONFIG_TERTIARY_PORT_STRING, &Flag, MinVal, MaxVal) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"Usb3HLOSConfigTertiaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler to Set SetUsb4HLOSConfigPrimaryPort variable to modify
 * the configuration of the USB Primary core _STA in ACPI.
 */
#define  USB4_HLOS_CONFIG_PRIMARY_PORT_STRING   " 1 = Platform Default \r\n" \
                                                " 2 = Force Enable USB4 and TBT3 \r\n" \
                                                " 3 = Force Disable USB4 and TBT3 \r\n" \
                                                " 4 = Force Enable TBT3 Only \r\n"


EFI_STATUS
SetUsb4HLOSConfigPrimaryPort (CHAR8* Arg)
{
  EFI_STATUS    Status;
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USB4HLOSCONFIGPRIMARYPORT;
  CONST UINT8   MinVal     = 1;
  CONST UINT8   MaxVal     = 4;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"Usb4HLOSConfigPrimaryPort", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QuerySetFlag ("USB4 HLOS Configuration on Primary Port", USB4_HLOS_CONFIG_PRIMARY_PORT_STRING, &Flag, MinVal, MaxVal) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"Usb4HLOSConfigPrimaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler to Set SetUsb4HLOSConfigSecondaryPort variable to modify
 * the configuration of the USB Secondary core device _STA in ACPI.
 */
#define  USB4_HLOS_CONFIG_SECONDARY_PORT_STRING   " 1 = Platform Default  \r\n" \
                                                  " 2 = Force Enable USB4 and TBT3 \r\n" \
                                                  " 3 = Force Disable USB4 and TBT3 \r\n" \
                                                  " 4 = Force Enable TBT3 Only \r\n"
...
EFI_STATUS
SetUsb4HLOSConfigSecondaryPort (CHAR8* Arg)
{
  EFI_STATUS    Status;
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USB4HLOSCONFIGSECONDARYPORT;
  CONST UINT8   MinVal     = 1;
  CONST UINT8   MaxVal     = 4;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"Usb4HLOSConfigSecondaryPort", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QuerySetFlag ("USB4 HLOS Configuration on Secondary Port", USB4_HLOS_CONFIG_SECONDARY_PORT_STRING, &Flag, MinVal, MaxVal) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"Usb4HLOSConfigSecondaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler to Set SetUsb4HLOSConfigTertiaryPort variable to modify
 * the configuration of the USB Tertiary core device _STA in ACPI.
 */
#define  USB4_HLOS_CONFIG_TERTIARY_PORT_STRING   " 1 = Platform Default  \r\n" \
                                                  " 2 = Force Enable USB4 and TBT3 \r\n" \
                                                  " 3 = Force Disable USB4 and TBT3 \r\n" \
                                                  " 4 = Force Enable TBT3 Only \r\n"
...
EFI_STATUS
SetUsb4HLOSConfigTertiaryPort (CHAR8* Arg)
{
  EFI_STATUS    Status;
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USB4HLOSCONFIGTERTIARYPORT;
  CONST UINT8   MinVal     = 1;
  CONST UINT8   MaxVal     = 4;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"Usb4HLOSConfigTertiaryPort", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QuerySetFlag ("USB4 HLOS Configuration on Tertiary Port", USB4_HLOS_CONFIG_TERTIARY_PORT_STRING, &Flag, MinVal, MaxVal) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"Usb4HLOSConfigTertiaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
#define  USB_MP_PORT_DISABLE_BITFIELD_STRING        " 0 = 0x0 = Enable all MP ports(Default)"  "\r\n" \
                                                    " 1 = 0x1 = Disable MP port 0"             "\r\n" \
                                                    " 2 = 0x2 = Disable MP port 1"             "\r\n" \
                                                    " 3 = 0x3 = Disable MP port 1, 0"          "\r\n" \
                                                    " ports Mapping:"                          "\r\n" \
                                                    "Bits:      1    |   0   |"                "\r\n" \
                                                    "MP port: port-1 | port-0|"                "\r\n" \
                                                    "  Example:"                               "\r\n" \
                                                    "To disable MP ports 1 and 0, set value as = 3 (0x3 == 0011)\r\n"



EFI_STATUS
SetUsbMultiPortDisableBitfield(CHAR8* Arg)
{
  EFI_STATUS    Status;
  INT8          Flag = 0;
  UINTN         VarSize;
  CONST UINT8   DefaultVal = DEFAULT_USBMULTIPORTDISABLEBITFIELD;
  CONST UINT8   MinVal     = 0;
  CONST UINT8   MaxVal     = 3;

  VarSize = sizeof(Flag);
  Status = gRT->GetVariable (L"UsbMultiPortDisableBitfield", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &Flag);
  ...
  if (QuerySetFlag ("USB Multiport Disable Bitfield", USB_MP_PORT_DISABLE_BITFIELD_STRING, &Flag, MinVal, MaxVal) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(Flag);
  Status = gRT->SetVariable (L"UsbMultiPortDisableBitfield", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
...
/*
 * Handler Reset all USB variable to default value
 */
EFI_STATUS
ResetUsbUefiVariable()
{
  // create the variable if it does not exist
  EFI_STATUS Status;
  UINT8 Flag;
  ...
  if (QueryToggleFlag ("To reset all usb variable", &Flag, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  Flag = DEFAULT_USBCONFIGPRIMARYPORT;
  Status = gRT->SetVariable (L"UsbConfigPrimaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  {
    AsciiPrint("ERROR: Unable to reset variable %d\r\n", 1);
  }

  Flag = DEFAULT_USBCONFIGSECONDARYPORT;
  Status = gRT->SetVariable (L"UsbConfigSecondaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USBCONFIGTERTIARYPORT;
  Status = gRT->SetVariable (L"UsbConfigTertiaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USBCONFIGMULTIPORT;
  Status = gRT->SetVariable (L"UsbConfigMultiPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USBCCLANEOVERRIDE;
  Status = gRT->SetVariable (L"UsbCCLaneOverride", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_FORCEUSBDEVICEMODECOMPLIANCE;
  Status = gRT->SetVariable (L"ForceUsbDeviceModeCompliance", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_BOOTUSBFIRSTDELAYVALUE;
  Status = gRT->SetVariable (L"BootUsbFirstDelayValue", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  {
  ...
  Flag = DEFAULT_HIGHSPEEDONLYFORPRIMARY;
  Status = gRT->SetVariable (L"HighSpeedOnlyForPrimary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_HIGHSPEEDONLYFORSECONDARY;
  Status = gRT->SetVariable (L"HighSpeedOnlyForSecondary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_HIGHSPEEDONLYFORTERTIARY;
  Status = gRT->SetVariable (L"HighSpeedOnlyForTertiary", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_HIGHSPEEDONLYFORMP;
  Status = gRT->SetVariable (L"HighSpeedOnlyForMP", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USBDPMODETERTIARY;
  Status = gRT->SetVariable (L"DPControllerModeType", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  {
  ...
  Flag = DEFAULT_ENABLESLTLOOPBACK;
  Status = gRT->SetVariable (L"EnableSLTLoopback", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  if (Status != EFI_SUCCESS)
  {
    ...
    Flag = DEFAULT_FORCEUSBWAKEONLANINUEFI;
  Status = gRT->SetVariable (L"ForceUsbWakeOnLanInUefi", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_FORCEUSB4RETIMERDP4LANE;
  Status = gRT->SetVariable (L"ForceUsb4RetimerDP4LANE", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USB3HLOSCONFIGPRIMARYPORT;
  Status = gRT->SetVariable (L"Usb3HLOSConfigPrimaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USB3HLOSCONFIGSECONDARYPORT;
  Status = gRT->SetVariable (L"Usb3HLOSConfigSecondaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USB3HLOSCONFIGTERTIARYPORT;
  Status = gRT->SetVariable (L"Usb3HLOSConfigTertiaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USB4HLOSCONFIGPRIMARYPORT;
  Status = gRT->SetVariable (L"Usb4HLOSConfigPrimaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USB4HLOSCONFIGSECONDARYPORT;
  Status = gRT->SetVariable (L"Usb4HLOSConfigSecondaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USB4HLOSCONFIGTERTIARYPORT;
  Status = gRT->SetVariable (L"Usb4HLOSConfigTertiaryPort", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
  ...
  Flag = DEFAULT_USBMULTIPORTDISABLEBITFIELD;
  Status = gRT->SetVariable (L"UsbMultiPortDisableBitfield", &gQcomTokenSpaceGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                             VarSize, &Flag);
  if (Status != EFI_SUCCESS)
...
ToggleForcePowerTesting(VOID)
{
  ...
  UINT8 Mask = 0x1;
  UINTN VarSize = 0;
  UINT8 ForcePowerTesting = 0;

  VarSize = sizeof(VarValue);

  Status = gRT->GetVariable (L"ForcePowerTesting", &gOSAVendorGuid,
               NULL, &VarSize, &VarValue);

  if (Status == EFI_NOT_FOUND)
    ForcePowerTesting = 0;
  else
    // bit 0 of force power testing represents if force power testing is enabled or not
    ForcePowerTesting = (VarValue & Mask);

  if (QueryToggleFlag ("Toggle force power testing", &ForcePowerTesting, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(ForcePowerTesting);
    

  Status = gRT->SetVariable (L"ForcePowerTesting", &gOSAVendorGuid,
               EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
               VarSize, &ForcePowerTesting);

  if (Status == EFI_SUCCESS)
  {
    AsciiPrint("Force power testing ");
    if (ForcePowerTesting == 0)
      AsciiPrint("Disable(Deleted). \r\n");
    else
      AsciiPrint("Enabled(Set). \r\n");
  }
...
/*
 * Configure DP mode on Tertiary Port (USB-DP <-> DP Only)
*/
EFI_STATUS
SetDPModeConfigTertiaryPort (VOID)
{
  EFI_STATUS    Status;
  ...
  VarSize = sizeof(CurrentFlag);
  Status = gRT->GetVariable (L"DPControllerModeType", &gQcomTokenSpaceGuid,
                             NULL, &VarSize, &CurrentFlag);

  if(Status != EFI_SUCCESS)
    CurrentFlag = 0;

  // Get the mode for the tertiary DP Controller
  NewFlag = ((CurrentFlag >> 4) & 0x3);

  if (QueryToggleFlag ("Configure DP mode on Tertiary Port", &NewFlag, FALSE) != EFI_SUCCESS)
    return EFI_SUCCESS;

  // Update the mode for the tertiary DP Controller
  CurrentFlag = ((CurrentFlag & 0xFFFFFFCF)| ((NewFlag & 0x3)<<4));
  ...
  if(Status == EFI_SUCCESS)
    ...
    AsciiPrint("ERROR: Unable to change/store DP controller#2 mode setting\r\n");
  return Status;
}

EFI_STATUS
ToggleChargerAppOffmodeCharging(VOID)
{
  // create the variable if it does not exist
  EFI_STATUS Status;
  UINT8 VarValue = 0;    
  UINT8 Mask = 0x1;
  UINTN VarSize = 0;
  UINT8 ChargerAppOffmodeCharging = 0;

  VarSize = sizeof(VarValue);

  Status = gRT->GetVariable (L"ChargerAppOffmode", &gOSAVendorGuid,
               NULL, &VarSize, &VarValue);

  if (Status == EFI_NOT_FOUND)
    ChargerAppOffmodeCharging = 0;
  else
    // bit 0 of force power testing represents if force power testing is enabled or not
    ChargerAppOffmodeCharging = (VarValue & Mask);

  if (QueryToggleFlag ("Toggle chargerapp offmode charging", &ChargerAppOffmodeCharging, NON_INVERTED_MEANING) != EFI_SUCCESS)
    return EFI_SUCCESS;

  VarSize = sizeof(ChargerAppOffmodeCharging);
    

  Status = gRT->SetVariable (L"ChargerAppOffmode", &gOSAVendorGuid,
               EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
               VarSize, &ChargerAppOffmodeCharging);

  if (Status == EFI_SUCCESS)
  {
    AsciiPrint("Force go to chargerApp for offmode charging");
    if (ChargerAppOffmodeCharging == 0)
      AsciiPrint("Disabled (offmode charging in Dxe). \r\n");
    else
      AsciiPrint("Enabled(offmode charging in chargerApp). \r\n");
  }
  else
  {
    AsciiPrint("ERROR: Unable to store setting\r\n");
...
EFI_STATUS
SetOffmodeChargingSleepTime (CHAR8 *Arg)
{
  EFI_STATUS    Status          = EFI_SUCCESS;
  INT8          Flag            = 0;
  UINTN         VarSize         = sizeof(Flag);
  CONST UINT8   DefaultSleepTime = 0;
  CONST UINT8   MinVal          = 0;
  CONST UINT8   MaxVal          = 6;
  CONST CHAR8   *FeatureString  = "Set offmode sleep time: 0: 600 sec, 1: 300 sec; 2: 60 sec; 3: 30 sec; 4: 6 sec; 5: 3 sec; 6: 500 msec";
  
  // Query offmodeSleepTime
  Status = gRT->GetVariable(L"offmodeSleepTime", &gQcomTokenSpaceGuid, NULL, &VarSize, &Flag);
  
  // if the variable does not exit, use the default value
  if (Status != EFI_SUCCESS) { Flag = DefaultSleepTime; }
  ...
  // store the change for ChargerPDLogLevel variable
  VarSize = sizeof(Flag);
  Status = gRT->SetVariable(L"offmodeSleepTime", &gQcomTokenSpaceGuid,
                            (EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE),
                            VarSize, &Flag);
                            
  if (Status == EFI_SUCCESS)
  {
    AsciiPrint("Updated offmode sleep time successfully.\r\n");
  }
  else
  {
    AsciiPrint("ERROR: Unable to change/store setting\r\n");
