/** @file
  ...
  Copyright (c) 2017 - 2019, 2022-2023 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---      -----------------------------------------------------------
 03/02/23   bkulkarn Port test hook support changes for EC Firmware update
...
#include <Include/Library/ECFwUpdateLib.h>
...
/**
  Read and set the reset data for Device (EC) FW update phase data
**/
EFI_STATUS
ReadAndSetEcFwPhaseData (
);
  ...
  )
{
  ...
  Print(L"  CapsuleApp -ecRP <PR1/FU1/US1/UF1/RD1/RF1/V>\n");
  ...
  Print(L"  -ecRP/ecResetPhase: Used to reset the device at different phase of EC firmware update \n");
  Print(L"       Sub params:\n");
  Print(L"           PR: To reset the device before processing the prepare-update phase of the current FW entry\n");
  Print(L"           FU: To reset the device before processing the flash-update phase of the current FW entry\n");
  Print(L"           US: To reset the device before processing update-success of the current FW entry\n");
  Print(L"           UF: To reset the device before processing update-fail phase of the current FW entry\n");
  Print(L"           RD: To reset the device before processing rollback-done phase of the current FW entry\n");
  Print(L"           RF: To reset the device before processing rollback-fail phase of the current FW entry\n");
  Print(L"           V : To view the current configuration of reset phase test hook\n");
  Print(L"       Notes:\n");
  Print(L"           - Currently only '1' is the valid entry.Any other values are treated as invalid entries\n");
  Print(L"           - 'Empty' will clear the respective reset phase variable\n");
  Print(L"           - Reset phases can be set individual or as group except option 'V' which shall be \n");
  Print(L"             individual to display all the phase value configured. All literals are case sensitive \n");
  Print(L"       Eg: CapsuleApp.efi -ecRP PR1\n");
  Print(L"           CapsuleApp.efi -ecRP FU1\n");
  Print(L"           CapsuleApp.efi -ecRP US1\n");
  Print(L"           CapsuleApp.efi -ecRP PR1 UF1 US1\n");
  Print(L"           CapsuleApp.efi -ecRP PR FU US\n");
  Print(L"           CapsuleApp.efi -ecRP V\n");
  ...
  )
{
  ...
  if ((StrCmp(Argv[1], L"-ecRP") == 0) ||
      ((StrCmp(Argv[1], L"-ecResetPhase") == 0))) {
    ...
    Status = ReadAndSetEcFwPhaseData();
    return Status;
  }
...
)
{
  ...
  else
  {
    for (ArgumentIndex = 2; ArgumentIndex < Argc; ArgumentIndex++)
    {
      FwEntryToResetStrPtr = &Argv[ArgumentIndex][2];
      if (*FwEntryToResetStrPtr == L'\0')
      {
        /*
         * No Fw entry mentioned for the current phase.
         * Assume it a request to clear the setting for the current phase.
         */
        FwEntryToReset = NO_RESET_IN_CURRENT_EC_FW_UPDATE_PHASE;
      }
      else
      {
        Status = ConvertStringToUInt(
           FwEntryToResetStrPtr,
           &FwEntryToReset);
        if (EFI_ERROR(Status))
        {
          Print(L"ConvertStringToUInt() failed for FwEntryToReset: %d with %r\n", FwEntryToReset, Status);
          goto Exit;
        }

        if (FwEntryToReset > MAX_ALLOWED_EC_FW_ENTRIES)
        {
          Print(L"EC device firmware entry count to reset exceeded maximum allowed value, aborting.\n");
          Status = EFI_INVALID_PARAMETER;
          goto Exit;
        }
      }

      if (StrnCmp(Argv[ArgumentIndex], L"PR", 2) == 0)
      {
        ResetPhaseData[EC_FW_TESTHOOK_PREPARE_UPDATE_PHASE] = FwEntryToReset;
      }
      else if (StrnCmp(Argv[ArgumentIndex], L"FU", 2) == 0)
      {
        ResetPhaseData[EC_FW_TESTHOOK_FLASH_UPDATE_PHASE] = FwEntryToReset;
      }
      else if (StrnCmp(Argv[ArgumentIndex], L"US", 2) == 0)
      {
        ResetPhaseData[EC_FW_TESTHOOK_UPDATE_SUCCESS_PHASE] = FwEntryToReset;
      }
      else if (StrnCmp(Argv[ArgumentIndex], L"UF", 2) == 0)
      {
        ResetPhaseData[EC_FW_TESTHOOK_UPDATE_FAIL_PHASE] = FwEntryToReset;
      }
      else if (StrnCmp(Argv[ArgumentIndex], L"RD", 2) == 0)
      {
        ResetPhaseData[EC_FW_TESTHOOK_ROLLBACK_DONE_PHASE] = FwEntryToReset;
      }
      else if (StrnCmp(Argv[ArgumentIndex], L"RF", 2) == 0)
      {
        ResetPhaseData[EC_FW_TESTHOOK_ROLLBACK_FAIL_PHASE] = FwEntryToReset;
      }
      else
      {
        Status = EFI_UNSUPPORTED;
        Print(L"Supplied input parameters are unsupported (%s), aborting.\n", Argv[ArgumentIndex]);
        goto Exit;
      }
    }
  ...
  if (EFI_ERROR(Status))
  {
    Print(L"CapsuleApp failed to record new reset phase data with %r\n", Status);
    goto Exit;
  }

  Print(L"\n");
  Print(L"################################################\n");
  Print(L"####      EC FW Update Reset phase data     ####\n");
  Print(L"################################################\n");
  Print(L"Fw entry count for Prepare-Update Phase : %d\n",   ResetPhaseData[EC_FW_TESTHOOK_PREPARE_UPDATE_PHASE]);
  Print(L"Fw entry count for Flash-Update Phase   : %d\n",   ResetPhaseData[EC_FW_TESTHOOK_FLASH_UPDATE_PHASE]);
  Print(L"Fw entry count for Update-Success Phase : %d\n",   ResetPhaseData[EC_FW_TESTHOOK_UPDATE_SUCCESS_PHASE]);
  Print(L"Fw entry count for Update-Fail Phase    : %d\n", ResetPhaseData[EC_FW_TESTHOOK_UPDATE_FAIL_PHASE]);
  Print(L"Fw entry count for Rollback-Done Phase  : %d\n", ResetPhaseData[EC_FW_TESTHOOK_ROLLBACK_DONE_PHASE]);
  Print(L"Fw entry count for Rollback-Fail Phase  : %d\n", ResetPhaseData[EC_FW_TESTHOOK_ROLLBACK_FAIL_PHASE]);
