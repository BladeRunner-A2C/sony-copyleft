...
# --------   ---     -----------------------------------------------------------
# 02/28/22   bxr     added buildoption -O0 check
# 12/22/21   bxr     added reletive path check
# 04/01/20   yps     Update the path of QcomCopyright.py 
...
RELETIVE_PATH_CHECK_ERROR_CODE = 13
BUILDOPTION_O0_CHECK_ERROR_CODE = 14

# build_files_sha1 structure, <File Name>, <Hash>, <mandatory Existance>
build_files_sha1=[["buildex.py", "0000000000000000000000000000000000000000", True]]
...
["EFIChipInfoTypes.h", "b1dfde82a26e5b8f28f2627da0ac525e988bfac3", True],
["EFICipher.h", "f53ecb0cf2ad28320182cd360a3fe141cc4af616", True],
["EFIClock.h", "ac68155bc2ee10e9966b4a540c013800ae366b0f", True],
...
["EFISmp2p.h", "31d2812f43b942448186e8e31153d8b6", True],
["EFIGLink.h", "18086914b7c98b2a48c672d4f5124d1ad21144ac", True],
["EFIQSocketQMI.h", "aba06e4dd4ff459ea7ed7d7be602591f", True],
...
["EFIListenerDef.h", "a693a5635b5122d4eaa8c0275105fd74138e066d", True],
...
["EFIPmicVersion.h", "68481131e455d0b4ad5b2f446193f7e9c2631d62", True],
...
["EFIQdss.h", "cd3d72b0e080d81240b0958035e74e4869d733ff", True],
...
["EFIScm.h", "f2e547e962fc8a5b163bd303166952119da2e009", True],
...
["EFITpm.h", "5a4c4f02d173e5a775905fa1937f6cb167be10c1", True],
["EFIToolSupport.h", "64f7e314e9b2b0d9a207120c5e16baefd9098d06", True],
["EFITrEE.h", "2529eb1476f4c344f82827b58b8c5b48a8190b55", True],
...
'EFISmp2p.h','EFIGLink.h', 'EFIGpiProtocol.h', 'EFIHALIOMMUProtocol.h', 'EFIHWIO.h', 'EFII2C.h', 'EFIIci.h',
'EFIKernelInterface.h', 'EFILimits.h', 'EFIListenerDef.h','EFIMac.h', 'EFIMeasureBoot.h', 'EFIMemoryMap.h', 'EFIMorPpi.h', 'EFIMpIoService.h', 
...
'EFISPI.h', 'EFISPMI.h', 'EFIStorageWriteProtect.h', 'EFITlmm.h', 'EFITpm.h', 'EFIToolSupport.h', 'EFITrEE.h', 'EFITsens.h', 'EFITzeLoader.h',
...
def main():
  ...
  parser.add_option("--rpcheck-threshold",
                    action="store",
                    type = "int",
                    dest="relative_path_check_threshold",
                    default="0",
                    help="Numbers of reletive path check file allowed before fault " + \
                         "is triggered.")
                         
  parser.add_option("--rpcheck-dir",
                    action="store",
                    type="string",
                    dest="relative_path_check_dir",
                    default="boot_images/boot/QcomPkg",
                    help="Base directory of where to start check. " + \
                         "Default is QcomPkg")

  parser.add_option("--relative-path-check",
                    action="store_true",
                    dest="relative_path_check",
                    default=False,
                    help="Enables the relative path check")

  parser.add_option("--O0-check-threshold",
                    action="store",
                    type = "int",
                    dest="O0_check_threshold",
                    default="0",
                    help="Numbers of buildoption -O0 check file allowed before fault " + \
                         "is triggered.")
                         
  parser.add_option("--O0-check-dir",
                    action="store",
                    type="string",
                    dest="O0_check_dir",
                    default="boot_images/boot/QcomPkg",
  ...
  parser.add_option("--O0-check",
                    action="store_true",
                    dest="O0_check_enable",
                    default=False,
                    help="Enables O0 check")
  ...
  if (options.enable_generate_watchit_hash_value):
    try:
      print ("WatchIt hash value" + ("." * 4), end='')
  ...
  if (options.watchit_hash_value):
    try:
      print("Executing watchit self check" + ("." * 27), end='')
  ...
  if (options.enable_all_features or options.enable_build_hash_check):
    try:
      print("Executing build scripts check" + ("." * 26), end='')
  ...
  if (options.enable_all_features or options.enable_Folder_hash_check):
    try:
      print("Executing folder check : Checking if made updates any in intended folder"\
            + ("." * 12), end='')
  ...
  if (options.enable_all_features or options.enable_cross_target_check):
    try:
      print("Executing cross target check" + ("." * 27), end='')
  ...
  if (options.enable_all_features or options.enable_warning_as_error_check):
    try:
      print("Executing warning as error disable check" + ("." * 15), end='')
  ...
  if (options.enable_all_features or options.enable_dal_target_check):
    try:
      print("Executing DAL target check" + ("." * 29), end='')
      dal_target_check()
      print("PASSED")
    except Exception as error:
      print(error)
  ...
  if (options.enable_all_features or options.enable_target_drivers_check):
    try:
      print("Executing target driver reference common code check" + ("." * 4), end='')
    ...
    try:
      print("Executing target specific common code check" + ("." * 12), end='')
  ...
  if (options.enable_all_features or options.enable_one_comdef_check):
    try:
      print("Executing one comdef.h check" + ("." * 4), end='')
  ...
  if (len(options.qcom_copyright_dir)):
    try:
      print("Executing Qcom Copyright check starting from: " + \
            options.qcom_copyright_dir + ("." * 6), end='')
  ...
  if (options.relative_path_check):
    try:
      print ("Executing the relative path check" + ("." * 5), end='')
      file_nums = rpcheck.relative_path_check(options.relative_path_check_dir,
                                              options.relative_path_check_threshold)
    except Exception as error:
      print(error)
      sys.exit(RELETIVE_PATH_CHECK_ERROR_CODE)
    
    if (file_nums == 0):
      print("PASSED")
    elif (file_nums > 0) and (file_nums <= options.relative_path_check_threshold):
      print("WARNING. Input threshold is %d. %d INFs need to be removed the relative path." \
           % (options.relative_path_check_threshold, file_nums))
      print("Please go through output file <boot_images/boot/relative_path_check_output.txt>")
    elif (file_nums > options.relative_path_check_threshold):
      print("\nERROR: watchit::relative_path_check: %d INFs need to be removed relative path, exceeding input threshold %d."\
             % (file_nums, options.relative_path_check_threshold) + \
             "\nPlease go through output file <boot_images/boot/relative_path_check_output.txt>")
      raise UserWarning("Check the new promoted *.inf file and remove the relative path from [Sources] item.")     
      
  #Check the buildoption -O0
  if options.O0_check_enable:
    try:
      print ("Executing the buildoption -O0 check" + ("." * 5), end = '')
      file_nums = O0_check.buildoption_O0_check(options.O0_check_dir,
                                               options.O0_check_threshold)
    except Exception as error:
      print(error)
      sys.exit(BUILDOPTION_O0_CHECK_ERROR_CODE)
    
    if (file_nums == 0):
      print("PASSED")
    elif (file_nums > 0) and (file_nums <= options.O0_check_threshold):
      print("WARNING. Input threshold is %d. %d INFs/DSCs need to be removed build flag \"-O0\"." \
           % (options.O0_check_threshold, file_nums))
      print("Please go through output file <boot_images/boot/O0_check_output.txt>")
    elif (file_nums > options.O0_check_threshold):
      print("\nERROR: watchit::buildoption_O0_check: %d INFs/DSCs need to be removed build flag \"-O0\", exceeding input threshold %d"\
             % (file_nums, options.O0_check_threshold) + \
             "\nPlease go through output file <boot_images/boot/O0_check_output.txt>")
      raise UserWarning("Check the new promoted *.inf/*.dsc file and remove \"-O0\" from [BuildOptions] item.")
...
def calculate_sha1(openssl_path,
  ...
  openssl_result = str(openssl_result, encoding = "utf8")
...
def qcom_copyright_check(qcom_copyright_dir,
  ...
  if int(qcom_copyright_threshold) < 0:
    raise NameError("ERROR: watchit::qcom_copyright_check: " + \
  ...
  script_result = script_result.decode()
