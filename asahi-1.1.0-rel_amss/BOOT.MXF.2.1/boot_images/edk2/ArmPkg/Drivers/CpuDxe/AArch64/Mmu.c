  ...
  )
{
  ...
  switch (EfiAttributes & EFI_MEMORY_CACHETYPE_MASK) {
    ...
    default:
#if defined (QCOM_EDK2_PATCH)
    /*EfiAttributes should be one among 
    EFI_MEMORY_CACHETYPE_MASK or 
    EFI_MEMORY_PROTECTION_MASK, if not Assert*/
    ASSERT ((EfiAttributes & (EFI_MEMORY_CACHETYPE_MASK | EFI_MEMORY_PROTECTION_MASK)) != 0 );
