...
/**
  Schedule to Locks all the UEFI FMP Variables on event based on passed LockGuid.
**/
EFI_STATUS
LockAllFmpVariablesOnEvent (
  EFI_GUID *LockGuid,
...
/**
  Schedule to Locks all the UEFI FMP Variables on event based on passed LockGuid.
  This function definition present in QcomPkg/Library/FmpDeviceLib
**/
EFI_STATUS
FmpDeviceVarLockOnEvent (
  EFI_GUID *LockGuid,
  const CHAR16 **mFmpVariableList,
  UINT32 size
  );
