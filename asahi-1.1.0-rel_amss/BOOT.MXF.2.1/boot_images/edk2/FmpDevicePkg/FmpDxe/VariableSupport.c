...
#if defined (QCOM_EDK2_PATCH)
/**
  Locks all the UEFI FMP Variables used by this module at particular passed event guid.

  @retval  EFI_SUCCESS      All UEFI FMP variables are scheduled to be locked.
  @retval  Other            One of the UEFI variables could not be locked.

**/
EFI_STATUS
LockAllFmpVariablesOnEvent (EFI_GUID *LockGuid, FIRMWARE_MANAGEMENT_PRIVATE_DATA  *Private)
{
  EFI_STATUS Status = EFI_SUCCESS;
  UINT32 size = ARRAY_SIZE (mFmpVariableLockList);

  mFmpVariableLockList[0] = Private->VersionVariableName;
  mFmpVariableLockList[1] = Private->LsvVariableName;
  mFmpVariableLockList[2] = Private->LastAttemptStatusVariableName;
  mFmpVariableLockList[3] = Private->LastAttemptVersionVariableName;
  mFmpVariableLockList[4] = Private->FmpStateVariableName;

  Status = FmpDeviceVarLockOnEvent(LockGuid,mFmpVariableLockList,size);
  DEBUG ((DEBUG_INFO, "[LockAllFmpVariablesOnEvent]: FmpDeviceVarLockOnEvent Status = %r\n",Status));
