  ...
  )
{
  ...
  if (IsLockFmpDeviceAtLockEventGuidRequired ()) {
    ...
    Status = LockAllFmpVariablesOnEvent(LockGuid, Private);
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "FmpDxe: Failed to schedule for lock variables.  Status = %r.\n", Status));
    } else {
      DEBUG ((DEBUG_INFO, "FmpDxe: All variables are schedule to be locked\n"));
    }
#else
    //
  ...
  )
{
  ...
  DetectTestKey ();
#if defined (QCOM_EDK2_PATCH)
  Status = FmpDeviceInit();
  if (EFI_ERROR(Status)) {
    DEBUG ((DEBUG_ERROR, "FmpDxe: Failed to initialize FMP Device.\n"));
    ASSERT (FALSE);
    return Status;
  }
