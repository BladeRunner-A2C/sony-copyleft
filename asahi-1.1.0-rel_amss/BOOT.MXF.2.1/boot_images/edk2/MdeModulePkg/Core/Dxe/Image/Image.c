...
Copyright (c) 2012-2017, 2020, 2022 Qualcomm Technologies, Inc.  All Rights Reserved.
  ...
  )
{
  ...
  if (PdbFileName == NULL) {
    AsciiStrCpyS (GaugeString, AsciiStrLen(GaugeString) + 1, " ");
  } else {
    StartIndex = 0;
  ...
  )
{
  ...
  if (Image->JumpBuffer == NULL) {
    ...
    //
#if defined (QCOM_EDK2_PATCH)
   // DEBUG ((EFI_D_INFO, "StartImage: %s ", Module));
    PERF_START_IMAGE_END (Module);
#else
...
#if defined(QCOM_EDK2_PATCH)
//  CoreExit already calls RestoreTPL by default, maintain proper nesting of Raise/Restore
#else
