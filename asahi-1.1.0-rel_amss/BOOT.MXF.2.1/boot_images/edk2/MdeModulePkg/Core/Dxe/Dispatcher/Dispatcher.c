...
Copyright (c) 2012 - 2013, 2016, 2018, 2020, 2022 Qualcomm Technologies, Inc. All rights reserved.
...
#if !defined (QCOM_EDK2_PATCH)  // Reduce dispatcher overhead
  ...
  while (TRUE) {
    ...
    if (gSecurity != NULL) {
      ...
      if (SecurityStatus != EFI_SUCCESS) {
        ...
        // Security check failed. The firmware volume should not be used for any purpose.
        //
		DEBUG ((DEBUG_ERROR, "Dispatcher - Security check failed!\n"));
        continue;
      }
    }
#endif	
