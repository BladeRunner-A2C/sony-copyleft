...
Copyright (c) 2011-2014, 2016-2018, 2020, 2021 Qualcomm Technologies, Inc. All rights reserved.
...
#if defined (QCOM_EDK2_PATCH)
  /* If Lock lib functional then don't install the exception handlers */
  if (InitLockLib ())
  {
    /* Use the interfaces with locks */
    mEfiSystemTableTemplate.BootServices = &mBootServicesLk;
    gDxeCoreDS = &mDxeServicesLk;
	goto SkipExceptionHandlerInstall;
  }
#endif
  //
...
#if defined (QCOM_EDK2_PATCH)
  // Initialize and start using Dxe locks now
  DxeLockInit ();
...
#if defined (QCOM_EDK2_PATCH)
  DEBUG ((EFI_D_ERROR, "Start EBS        [%5d] \n", GetTimerCountms ()));
#endif
  //
  // Disable Timer
  //
...
#if defined (QCOM_EDK2_PATCH)
  DEBUG ((EFI_D_ERROR, "Exit EBS        [%5d] UEFI End\n", GetTimerCountms ()));
  SerialPortFlush ();
  SerialPortControl (SIO_CONTROL_POWER,0);
#endif                                                                                   
  return Status;
}
