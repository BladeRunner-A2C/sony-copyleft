...
Copyright (c) 2013-2021, 2022 Qualcomm Technologies, Inc. All rights reserved.
...
#if defined (QCOM_EDK2_PATCH)
///
/// gYieldEvent - Event which is signalled when we need to yield without blocking
///
EFI_EVENT       gYieldEvent = NULL;
#endif
  ...
  )
{
  ...
  while (!IsListEmpty (Head)) {
...
#if defined (QCOM_EDK2_PATCH) && defined (ENABLE_FINE_GRAINED_LOCKING)
    DxeUnlock ();
#endif
    ...
    {  
      PrintNotifyFnFileNameEx((UINTN)Event->NotifyFunction, &Event->EventGroup,
                             CALLBACK_PERF_IGNORE);
    }
...
#if defined (QCOM_EDK2_PATCH)
    DeltaLog = GetTimerCountms() - logVarSt;
    if(DeltaLog > CALLBACK_PERF_THRESHOLD)
    {
      PrintNotifyFnFileNameEx((UINTN)Event->NotifyFunction, &Event->EventGroup,
                             DeltaLog);
    }
...
#if defined (QCOM_EDK2_PATCH)
  // If event is not signalled yet, yield for the event to occur just in case
  if (Event->SignalCount == 0)
    CoreSignalEvent (gYieldEvent);
#endif
