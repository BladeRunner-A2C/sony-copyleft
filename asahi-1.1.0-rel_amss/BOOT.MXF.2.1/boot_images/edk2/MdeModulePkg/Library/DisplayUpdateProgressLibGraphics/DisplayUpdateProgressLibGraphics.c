  ...
  )
{
...
#if defined(QCOM_EDK2_PATCH)
  BOOLEAN                              EnableScreenWideProgressBar = 0;
  UINTN                                NvVarSize = sizeof (EnableScreenWideProgressBar);
#endif // #if defined(QCOM_EDK2_PATCH)
  ...
  Status = gRT->GetVariable(
    ...
    NULL,
    &NvVarSize,
    &EnableScreenWideProgressBar
    );
  if ((EFI_ERROR(Status) ||
      ((sizeof (EnableScreenWideProgressBar) != NvVarSize)))) {
    EnableScreenWideProgressBar = PcdGetBool (PcdEnableScreenWideProgressBar);
  }

  if(EnableScreenWideProgressBar == TRUE)
  {
    //
    // Compute mBlockWidth using half of screen width.
    //
    mBlockWidth = (mGop->Mode->Info->HorizontalResolution / 2)  / 100;
  
    //
    // Adjust mStartX based on block width so it is centered in the middle of the screen
    //
    mStartX = mGop->Mode->Info->HorizontalResolution / 4;
	goto SkipLogoParamsInit;
  }
#endif
