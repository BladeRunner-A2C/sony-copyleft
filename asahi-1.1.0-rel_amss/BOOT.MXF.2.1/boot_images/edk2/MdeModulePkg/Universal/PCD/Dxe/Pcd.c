...
/// PCD database lock.
///
/* Running at TPL_NOTIFY creating CoreRaiseTPL assert while GetWorker-> RaiseTPL to 16 ->GetHiiVariable->GetVariable->Raise TPL to 8 ,whereas GetVariable which runs at TPL level TPL_CALLBACK,
  so raising TPL from NOTIFY to CALLBACK(16 to 8 degrading) is not allowed*/
