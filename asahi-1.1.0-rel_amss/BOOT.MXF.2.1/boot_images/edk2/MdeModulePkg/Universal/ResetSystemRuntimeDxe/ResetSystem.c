  ...
  )
{
  ...
  /*CR - 3437616
   * The drivers registering for the notification in order a->b>c
   *wil need to get notified in order of c->b->a
   */
  InsertHeadList (&Instance->ResetNotifies, &Entry->Link);
#else
  ...
  )
{
  ...
  mResetNotifyDepth++;
#if defined(QCOM_EDK2_PATCH)
  //Stub out Debug print since uefi's uart driver resources will not be 
  //available post ExitBootServices
#else  
  DEBUG ((
    DEBUG_INFO,
