  ...
  )
{
  ...
  if (Index == MAX_MBR_PARTITIONS) {
#if defined (QCOM_EDK2_PATCH)
    //
    // For removable media, skip non-GPT formatted devices
    //
    ...
    //
    // Otherwise, validate the GPT despite MBR validation failure.
    // Update MBR if GPT validation passes.
    //
  	DEBUG ((EFI_D_ERROR, " Protective MBR validation might be needed. \n"));
  ...
  if (!PartitionValidGptTable (BlockIo, DiskIo, PRIMARY_PART_HEADER_LBA, PrimaryHeader)) {
    ...
    if (!PartitionValidGptTable (BlockIo, DiskIo, LastBlock, BackupHeader)) {
      ...
      //
      // Skip updating primary table if RemovableMedia is TRUE
      //
      if (!BlockIo->Media->RemovableMedia) {
...
#if defined (QCOM_EDK2_PATCH)
  DEBUG ((DEBUG_INFO, "SizeOfPartitionEntry = %d\n", PrimaryHeader->SizeOfPartitionEntry));
#endif //QCOM_EDK2_PATCH
  ...
  if (ProtectiveMbrValidStatus == EFI_NOT_FOUND) {
    ...
    //
    DEBUG ((EFI_D_ERROR, "Write valid Protective MBR\n"));
    ...
    ProtectiveMbr->Partition[0].StartingLBA[1] = 0;
    ProtectiveMbr->Partition[0].StartingLBA[2] = 0;
    ProtectiveMbr->Partition[0].StartingLBA[3] = 0;
...
#if defined (QCOM_EDK2_PATCH)
    // Skip GPT fix-up for removable media
    if (BlockIo->Media->RemovableMedia) {
      DEBUG((EFI_D_INFO, " Skipping GPT fix for removable media\n"));
      return TRUE;
    }
#endif
  PartHdr = NULL;
