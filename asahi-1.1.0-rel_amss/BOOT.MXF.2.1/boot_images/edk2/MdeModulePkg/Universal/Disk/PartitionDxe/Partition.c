  ...
  )
{
  ...
  // Perf improvement to avoid reading first block of every logical partition.
  // This disables support of GPT in GPT.
  ...
  )
{
  ...
  if (Private->ParentEraseBlock)
  {
    return Private->ParentEraseBlock->NonSecureEraseBlocks (
        Private->ParentBlockIo,
        Private->Media.MediaId,
        Offset,
        Size);
...
/**
  purge by using BLock purge protocol on the parent device.
  ...
  )
{
  ...
  if (Private->ParentEraseBlock)
  {
    return Private->ParentEraseBlock->PurgeBlocks (
        Private->ParentBlockIo,
        Token);
  }
  else
...
#ifdef FEATURE_XBOOT
    Private->BlockErase.NonSecureEraseBlocks  = PartitionNonSecureEraseBlocks;
    Private->BlockErase.PurgeBlocks  = PartitionPurgeBlocks;
#endif /* FEATURE_XBOOT */
  }
  ...
  if (!EFI_ERROR (Status)) {
      if (Private->ParentEraseBlock)
      {
        Status = gBS->InstallProtocolInterface (
                      ...
                      &Private->BlockErase);
        if (EFI_ERROR(Status))
          DEBUG((EFI_D_ERROR, "Could not install erase block protocol for partition\n"));
      }
    } 
#endif // QCOM_EDK2_PATCH
  } else {
