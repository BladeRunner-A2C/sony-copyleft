...
#if defined (QCOM_EDK2_PATCH)
  Instance->SharedWorkingBufferSize = PcdGet32 (PcdDiskIoDataBufferBlockNum) * Instance->BlockIo->Media->BlockSize;
  Instance->SharedWorkingBuffer = AllocateAlignedPages (
                                    EFI_SIZE_TO_PAGES (Instance->SharedWorkingBufferSize),
                                    Instance->BlockIo->Media->IoAlign
                                    );
  if (Instance->SharedWorkingBuffer == NULL) {
    Instance->SharedWorkingBufferSize = 0;
    Status = EFI_OUT_OF_RESOURCES;
    goto ErrorExit;
  }
#else
...
#if defined (QCOM_EDK2_PATCH)
        UINTN SharedWorkingBlockCnt;

#define MAX_WORKING_BUFFER_SIZE    (1024 * 1024)

         if ((Instance->SharedWorkingBufferSize < MAX_WORKING_BUFFER_SIZE) &&
             (BufferSize >= MAX_WORKING_BUFFER_SIZE))
        {
          VOID * NewBufferPtr = AllocateAlignedPages (
                                    EFI_SIZE_TO_PAGES (MAX_WORKING_BUFFER_SIZE),
                                    Instance->BlockIo->Media->IoAlign);

          if (NewBufferPtr != NULL)
          {
            if (Instance->SharedWorkingBuffer)
              FreeAlignedPages (Instance->SharedWorkingBuffer,
                EFI_SIZE_TO_PAGES (Instance->SharedWorkingBufferSize));

            Instance->SharedWorkingBufferSize = MAX_WORKING_BUFFER_SIZE;
            Instance->SharedWorkingBuffer = NewBufferPtr;
            SharedWorkingBuffer = Instance->SharedWorkingBuffer;
          }
        }

        SharedWorkingBlockCnt = Instance->SharedWorkingBufferSize / BlockSize;
        for (; Lba < OverRunLba; Lba += SharedWorkingBlockCnt) {
          DataBufferSize = MIN (BufferSize, Instance->SharedWorkingBufferSize);
...
#if defined (QCOM_EDK2_PATCH)
/*FR:75405
  To support UFS Write booster feature, FUA attribute is set to 0. hence writes 
  in the storage layer are only to the internal cache and needs to be flushed. */
...
/*FR:75405
  To support UFS Write booster feature, FUA attribute is set to 0. hence writes 
  in the storage layer are only to the internal cache and needs to be flushed. */
        BlockIo2->FlushBlocksEx (BlockIo2, &Subtask->BlockIo2Token);
#endif
      }
    } else {
      //
  ...
  )
{
  ...
  //
  BufferAlignment = (UINTN) WorkingBuffer & (UINTN) (Media->IoAlign - 1);

//  if (BufferAlignment) 
//    DEBUG ((EFI_D_ERROR, "Slow write perf due to misaligned buffer %X  %X  %X\n", Offset, BufferSize, Buffer));

  Lba  = DivU64x32Remainder (Offset, BlockSize, &UnderRun);
  DivU64x32Remainder (BufferSize, BlockSize, &OverRun);
  if (UnderRun == 0 && OverRun == 0 && BufferAlignment == 0) {
    AcquireLock (DiskIoLock);
...
#if defined (QCOM_EDK2_PATCH)
  InitRecursiveLock (NULL, &DiskIoLock);
#endif
