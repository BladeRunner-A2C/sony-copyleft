...
#include <Library/StorageGUIDs.h>

typedef struct {
  VENDOR_DEVICE_PATH Nvme; 
  EFI_DEVICE_PATH    End; 
} QCNVME_DEVICE_PATH; 

static VENDOR_DEVICE_PATH NvmePath = {
  {
    HARDWARE_DEVICE_PATH, HW_VENDOR_DP,
    {sizeof(VENDOR_DEVICE_PATH),(sizeof(VENDOR_DEVICE_PATH) >> 8)}
  },
  {0}
};

static EFI_DEVICE_PATH NvmeEndPath = {
  ...
  )
{
  ...
  QCNVME_DEVICE_PATH                    NvmeDevicePath; 
#endif 
  ...
  if (NamespaceData->Ncap == 0) {
    ...
    //
#if defined (QCOM_EDK2_PATCH)
    Device->Media.MediaId        = SIGNATURE_32('n','v','m','e');
#else 
...
#if defined (QCOM_EDK2_PATCH)
    if (DevicePath != NULL) {
      NvmeDevicePath.Nvme = NvmePath; 
      NvmeDevicePath.Nvme.Guid = (EFI_GUID)EFI_NVME_0_GUID; 
      NvmeDevicePath.End = NvmeEndPath; 

      TempDevicePath = DevicePath; 
      DevicePath = AppendDevicePath((EFI_DEVICE_PATH_PROTOCOL *)&NvmeDevicePath, TempDevicePath); 
    }
#endif 
