  ...
  )
{
  ...
  if (read_count == 1)
  {
    UINT32 total_time, end_time = 0;
    end_time = GetTimerCountus();
    total_time = (end_time - start_time);
    /* Only print the warning if the first read took longer than 8ms. On a trial run all reads
     * of 1 sector took less than 2.2ms. */
    if (total_time >= 8000)
    {
      DEBUG ((EFI_D_WARN, "NvmeRead first read %d ms @ %d ms\r\n", total_time / 1000, start_time / 1000));
    }
  }
#endif
