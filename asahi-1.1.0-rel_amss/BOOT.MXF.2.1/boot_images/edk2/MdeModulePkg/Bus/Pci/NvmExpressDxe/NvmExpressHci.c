  ...
  )
{
  ...
  {
    UINT8 fw_ver[9];
    CopyMem (fw_ver, Private->ControllerData->Fr, sizeof(Private->ControllerData->Fr));
    fw_ver[8] = 0;
    DEBUG ((EFI_D_WARN, "NVMe %a FW:%a SN:%a PCI VID 0x%x 0x%x\n", Mn, fw_ver, Sn,
            Private->ControllerData->Vid, Private->ControllerData->Ssvid));
  }
#endif
