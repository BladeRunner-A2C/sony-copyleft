  ...
  do {
    ...
    if (Update != (UINTN) -1) {
      ...
      if ((Key.UnicodeChar == CHAR_BACKSPACE) || (Key.UnicodeChar == 0 && Key.ScanCode == SCAN_DELETE)) {
        ShellPrintEx ((INT32)Column+1, (INT32)Row, L"%s%.*s", CurrentString + Update, Delete, L"\b");
        ShellPrintEx ((INT32)Column+1, (INT32)Row, L"%s%.*s", CurrentString + Update, Delete, L"");
        ShellPrintEx ((INT32)Column+1, (INT32)Row, L"%s%.*s", CurrentString + Update, Delete, L"\b");
      }
      else {
