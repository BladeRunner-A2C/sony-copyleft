/** @file
  ...
  Copyright (c) 2019, 2021, 2022 Qualcomm Technologies, Inc. All rights reserved.
  ...
  )
{
  ...
  /*
  Its possible that USB cable could be disconnected while read operation was going on
  and all related data structures are freed
  Exit gracefully during USB cable removal detection
  CR 2503748
  */
  if(((VOID *)Private == NULL) || 
  ...
  */
  if(((VOID *)PxeBc == NULL) ||
     ((UINT32)PxeBc->Revision != EFI_PXE_BASE_CODE_PROTOCOL_REVISION))
     return;
#endif
