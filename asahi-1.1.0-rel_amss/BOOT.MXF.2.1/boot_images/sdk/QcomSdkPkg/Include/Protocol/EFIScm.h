...
*/
/*=============================================================================
  Copyright (c) 2012-2014,2018-2020,2023 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 12/10/23   shl      Added cache property on Permission word
 08/18/23   shl      Added create memory object API
...
/** @ingroup QCOM_SCM_PROTOCOL_data
  Protocol version.
*/
#define QCOM_SCM_PROTOCOL_REVISION 0x0000000000050002
...
// permission = cache attrib | perm attrib;
#define PERM_MASK  UINT32_C(0x0000000F)
#define CACHE_MASK UINT32_C(0xF0000000)

// permit attribs ( can select a combo ):
#define PERM_X  UINT32_C(1) // excute
#define PERM_W  UINT32_C(2) // write
#define PERM_R  UINT32_C(4) // read 
#define PERM_RW UINT32_C(6) // combo of read/write 
#define PERM_RX UINT32_C(5) // combo of read/excute

// cache attribs ( only select one ): 
#define CACHED    UINT32_C(0x10000000) // mem is allocated from cached pool
#define UNCACHED  UINT32_C(0x20000000) // mem is allocated from uncached pool

/* QCOM_SCM_CREATE_MEM_OBJ */ 
/** @ingroup QCOM_SCM_PROTOCOL_apis
  @par Summary
  This API will create a memory object based on input memory info.
   
  @param[in]     This       Pointer to the QCOM_SCM_PROTOCOL instance.
  @param[in]     Buf        Memory address.
  @param[in]     BufSize    Memory size.
  @param[in]     Permission See permission definition.
  @param[out]    MemObject  Memory Object created.
...
typedef
EFI_STATUS
(EFIAPI *QCOM_SCM_CREATE_MEM_OBJ)(
  IN  QCOM_SCM_PROTOCOL      *This, 
  IN  VOID                   *Buf, 
  IN  UINTN                   BufSize, 
  IN  UINT32                  Permission,
  OUT VOID                   *MemObject
);
...
struct _QCOM_SCM_PROTOCOL {
  ...
  QCOM_SCM_CREATE_MEM_OBJ        ScmCreateMemObj;
};
