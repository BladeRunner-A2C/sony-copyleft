...
#
# This section contains comments describing changes made to the modules.
# Notice that changes are listed in reverse chronological order.
#
# when       who     what, where, why
# --------   ---     -----------------------------------------------------------
# 04/24/23   yps      Support to copy ignore list in all sub folders
# 03/21/23   yps      Corrected regex for searching macro definination in inf
# 01/05/23   fi       Added subprocess call to PackedBuildValidator.py
...
#regex for searching macro define in inf
REGEX_OF_DEFINE = '^(\s*DEFINE\s+)([a-zA-Z0-9_]+)(\s*=\s*)([a-zA-Z0-9_./]+)'
  ...
  def copytree_ignore_evaluation(eval_directory, names):
   ...
   # logger.debug("ignore eval_directory:%s",eval_directory)
    if eval_directory in exclude_path:
      ...
      #list = or path in exlude_path if path.endwith('*') if re.serach(path.strip('*'),eval_directory) or path.strip('*') = eval_directory break
      exclude_path_list =  list(filter(lambda x:x.endswith('*'),exclude_path))
      for path in exclude_path_list:
        if re.search(path.strip('*'),eval_directory) or path.strip('/*')==eval_directory :
          logger.debug("Bingo ***************%s in  :%s",eval_directory,path)
          ignore = set(name for pattern in patterns for name in fnmatch.filter(names,pattern))
          logger.debug("ignore %s", ignore)
          return ignore
    return []
...
class packit(object):
                ...
                with open(original_inf_path) as inf_file:
                    ...
                    for line in inf_file:
                        ...
                        if line.strip().startswith("#"):
                            continue
                        m = re.search(REGEX_OF_DEFINE,line)
                        if m!=None:
              ...
              for line in inf_file:
                ...
                if release_entry == True:
                  if line == "" or "[" in line:
                    ...
                    if len(assembled_line) > 0:
                      if release_entry == True:
                        release_list.append(self.replace_arguments(assembled_line))
  ...
  def copy_release_binaries(self,
    ...
    for release_list_file in release_list:
      if ".c" in release_list_file.lower() or \
         ".asl" in release_list_file.lower() or \
  ...
  def cleanse_pack_dir(self,
    ...
    logger.info("[packout list]:%s","\n[packout list]:".join(packout_list))
  ...
  def post_cleanse_pack_dir(self,base_directory):
      ...
      logger.info("[%s] Post cleansing pack directory",self.task_name)
      logger.info("[release list]:%s","\n[release list]".join(release_list))
      try:
          for root, directories, files in os.walk(base_directory,
                                                  topdown=False):
              for current_file in files:
                  ...
                  if current_file_path not in release_list:
                      ...
                      if current_file_path.endswith(('.c', '.cpp', '.s', '.S', '.asm','.asl')):
                          try:
                              os.chmod(current_file_path, stat.S_IWRITE)
                              os.remove(current_file_path)
                              logger.debug("Post cleanse to delete %s", current_file_path)
                          except:
              ...
              for line in inf_file:
                ...
                if packout_entry == True:
                  if line == "" or "[" in line:
                    ...
                    if len(assembled_line) > 0:
                      if packout_entry == True:
                        postbuild_packout_list.append(self.replace_arguments(assembled_line))
  ...
  def process (self):
    ...
    # Execute external PackedBuildValidator.py which is used to validate packed build created by
    # packit against manifest files that define what is allowed to be shipped.
    # This catches any file that should not be shipped.
    if (options.run_validator):
      print("Running PackedBuildValidator.py...")

      general_manifest_path = os.path.join(os.getcwd(), "boot_images", "boot",
                                           "Manifest.txt")
      target_manifest_path =  os.path.join(os.getcwd(), "boot_images", "boot", "QcomPkg", "SocPkg", 
                                           options.target_name, (options.target_name + "_Manifest.txt"))
      
      try:
        subprocess.check_call(['python',
                               os.path.join("boot_images", "boot_tools", "PackedBuildValidator.py"),
                               '-b', os.path.join(os.getcwd(), self.options.packdir_name),
                               '-f', general_manifest_path + ',' + target_manifest_path])
...
def main():
  ...
  parser.add_option('-a', '--architectures',
                     type='string', action='callback', callback=architecture_parser_callback,
                     default=["aarch64","ipf"],
  ...
  parser.add_option("--boot-flow",
                     type="string", action="store", dest="boot_flow", 
                     default="SM",
                     help="Boot Flow.  I.E. SM or SDX.  Default is SM")
  ...
  parser.add_option("--run-validator",
                     action="store_true", dest="run_validator", default=False,
                     help = "Run ShippedFileValidator.py.")
