...
# --------   ---     -----------------------------------------------------------
# 09/21/22   rama    Changes to support absolute paths in input json file
# 09/14/22   rama    Added logic to copy raw binary to raw folder in the output folder
# 09/08/22   ck      Added --integrity-image-no-metadata option for sectools v2
# 06/01/22   ck      Added --integrity-image option for sectools v2
# 05/02/22   rama    Python version check to 3.7.4
# 03/16/22   rama    Add --align option to align individual config item
# 03/02/22   rama    Fix logic for fixing behavior if sectools directory is not provided
# 02/16/22   rama    reconcile versions across PWs
# 02/14/22   rama    handle compilation in case of sectools option not provided
# 11/16/21   yps     Support python 3.8.2 or later only
# 09/14/21   yps     Support to generate single segment for multi cfg sections
# 07/29/21   yps     Add signing mode option
# 04/06/21   tmt     Additional python v3 change & version support
# 12/01/20   ck      Port unsigned image support from SBL 4.1
# 09/30/20   gt      Support for Sectools V2
# 06/30/20   yps     Support python3.8.3
# 05/14/20   ck      Port ELF32 support from SBL 4.1
# 02/27/20   yps     Make scripts compatible with python2 and python3
# 10/05/18   kpa     Fix sectools config path command-line option
# 10/12/17   vk      Add hash algorithm parameter
# 08/22/17   aus     Added support for clean option
...
def create_elf(base_directory,
  ...
  if not single_segment:
    ...
    for CFG_section in json_data:
      ...
      current_offset += os.path.getsize(os.path.join(output_directory, correct_path(CFG_section + ".bin")))
