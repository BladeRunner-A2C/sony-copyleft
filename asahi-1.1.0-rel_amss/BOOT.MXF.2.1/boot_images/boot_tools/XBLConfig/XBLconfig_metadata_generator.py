...
def create_new_cfg_item(input_cfg_item, base_directory, align):
  config_file = None
  cfg_name = None
  cfg_entry = json.loads(cfg_item_json, object_pairs_hook=OrderedDict)
...
def create_meta_data_table(json_input_cfg_items, xbl_cfg_value, fp_cfg_meta_file, base_directory, align):
  ...
  for input_cfg_item_key in json_input_cfg_items:
    new_cfg_entry = create_new_cfg_item(json_input_cfg_items[input_cfg_item_key], base_directory, align)
...
def generate_xbl_config_metadata(json_config_path, base_directory, output_directory, align):
  ...
  for xcfg_meta_item in input_json_xbl_cfgs:
    ...
    create_meta_data_table(input_json_xbl_cfgs[xcfg_meta_item], str(xcfg_meta_item), fp_cfg_meta_file, base_directory, align)
  ...
  for cfg_entry in meta_cfg_entries:
    ...
    if number_of_segments == 3 or number_of_segments == 1:
      ...
      try:
        ...
        sys.exit(-1)
      print('offset is'+cfg_entry['offset_from_meta_start']['value'])
      print(int(cfg_entry['offset_from_meta_start']['value'],0))
      try:
...
if __name__ == "__main__":
  ...
  if not options.disassembled_xcfg_info_json:
    ...
    generate_xbl_config_metadata(options.json_config_path, options.base_directory, options.output_directory, options.align)
  else:
