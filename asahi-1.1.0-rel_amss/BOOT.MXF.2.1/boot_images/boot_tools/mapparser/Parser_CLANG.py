...
PADDING_NAME='PADDING_ALIGNMENT'
Debug_flag=False
################################################################################
# MapScannerTracker
################################################################################
...
def get_next_map_token(scanner):
    ...
    for line in scanner.fh:
        ...
        m = re.search('^([0-9_A-Z]+)(\s+(0x[0-9a-fA-F]+)\s+(0x[0-9a-fA-F]+)\s+(#)\s+([a-zA-Z]+:)\s+(0x[0-9a-fA-F,]+)\s+([a-zA-Z:]+)\s+(0x[0-9a-fA-F]+)(,\s+([a-zA-Z:]+)\s+(0x[0-9a-fA-F]+))?([,0-9a-zA-Z_:\s\[\]]+)?)?\s*$',
                      line)
        if m:
            if m.group(2) != None:
                symbol = None
