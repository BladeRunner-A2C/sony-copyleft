...
def main():
    ...
    if 'PADDING_NAME' in dir(compiler):
        PADDING_NAME = compiler.PADDING_NAME
...
def calc_lib_sizes(symbols, regions, lib_sizes):
    ...
    for symbol in symbols:
        ...
        # caculate padding size of each lib.
        if PADDING_NAME != None:            
            if symbol.name == PADDING_NAME:
              if "PADDING" not in lib_sizes[symbol.lib]:
                lib_sizes[symbol.lib]["PADDING"] = 0
              lib_sizes[symbol.lib]["PADDING"] += symbol.size
...
def print_libs(lib_sizes):
    ...
    for key in sorted(segments):
        print(key, column_padding(key, col_width), end=' ')
    print("Total", column_padding("Total", col_width), end=' ')
    if PADDING_NAME != None:
        print("Totalnopad", column_padding("Totalnopad", col_width), end=' ')
        ...
        for seg in sorted(segments):
            if seg in lib_sizes[lib]:
                ...
                if (seg == "CODE" or seg == "RO" or seg == "RW"):
                    rom += lib_sizes[lib][seg]
                print(lib_sizes[lib][seg], end=' ') 
                print(column_padding(str(lib_sizes[lib][seg]), col_width), end=' ')
            else: 
                print(0, column_padding('0', col_width), end=' ')
        print(total, column_padding(str(total), col_width), end=' ')
        if PADDING_NAME != None:
            if "PADDING" in lib_sizes[lib]:
                print(total-lib_sizes[lib]["PADDING"], column_padding(str(total-lib_sizes[lib]["PADDING"]), col_width), end=' ')
            else:
                print(total, column_padding(str(total), col_width), end=' ')
    ...
    rom = 0
    for seg in sorted(segments):
        print(segments[seg], column_padding(str(segments[seg]), col_width), end=' ')
        if seg != 'PADDING':
            total += segments[seg]
        if (seg == "CODE" or seg == "RO" or seg == "RW"):
            rom += segments[seg]
    print(total, column_padding(str(total), col_width), end=' ')
    if PADDING_NAME != None: 
        print(total-segments['PADDING'],column_padding(str(total-segments['PADDING']), col_width), end=' ')
...
def print_map(m, header):
    ...
    for key1 in m:
        if len(m[key1]):
            print(key1)
            for key2 in m[key1]:
                print(key2, "\t Entity \t Address \t Size \t Obj \t Lib \t Segment")
