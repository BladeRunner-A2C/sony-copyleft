...
#
# Copyright 2017 - 2023 by Qualcomm Technologies, Inc.
#                 All Rights Reserved.
...
# --------   ---     ----------------------------------------------------------
# 04/21/23   yps      Exact match "errno"
# 02/15/23   ck       Added environment modification to missed subprocess call 
# 01/23/23   vk       Pass -j flag to BaseTools compilation
# 01/10/23   ck       Added environment modification to subprocess calls
...
def main():
  ...
  parser.add_option('-l', '--logdirpath',
                    action="store",default=os.path.join(os.path.dirname(os.path.dirname(__file__)),"BuildLogs"),
                      help="log_dir_path" + \
                            "Example: -l D:\workspace\boot_images\BuildLogs",)

  (options, args) = parser.parse_args()
  init_work_path(options.imgrootpath)
  init_conf_folder(os.environ["BUILDROOT"])

  ARGUMENTS["$LOGDIR"]=options.logdirpath

  if not os.path.isdir(options.logdirpath):
     os.makedirs(options.logdirpath)

  logging.basicConfig(filename=os.path.join(options.logdirpath,"buildex.log"),filemode='w',format='%(message)s',level = logging.DEBUG)
  ...
  for i in range(len(build_config['CreateImages'])):
    if build_config['CreateImages'][i]['Name'] == str(variant_flavor.image):
      ...
      for j in range(len(build_config['CreateImages'][i]['Process'])):
        ...
        constructed_environment = construct_environment(process_action)
  ...
  if needs_compile:
    for i in range(len(component_config['Process'])):
      ...
      if build_cmd[0] == "build" or "build.py" in list(build_cmd[0].split(os.sep)) or "build.py" == build_cmd[0]:
        ...
        constructed_environment = construct_environment(cprocess_config)
        try:
            ...
            while proc.poll() is None:
                  ...
                  if(len(line)):
                     logger.info(line)
                  if line!='':
                      if 'Error:' in line or 'ERROR:' in line or 'error:' in line or re.search('(^Errno[:]?)|([ ]+Errno[:]?)',line):
                          hit_error = True
            if hit_error:
...
#####################################################################
def construct_environment(process_action):
  constructed_environment = os.environ.copy()

  if 'Export' in process_action:
    for key in process_action['Export']:
      constructed_environment[key] = replace_arguments(process_action['Export'][key])

  return constructed_environment
  ...
  if 'Path' in tool_config:
      ...
      if isinstance(tool_config['Path'],type([])):
        ...
        if 'arm-elfcopy' in tool_config['Path']:
          tool_config['Path'] = os.getenv("CLANG1602LINUX_ELFCOPY")
...
def set_compiler_path(tool_config,tool_name):
   ...
   if os.path.exists(os.getenv("CLANG1602LINUX_BIN")):
     os.environ[COMPILER_BIN_NAME] = os.getenv("CLANG1602LINUX_BIN") + "/"
   else:
     logger.error("[buildex.py] ERROR: Tool path {} does not exist!".format( tool_config['Path']))
     logger.error("[buildex.py] ERROR: build_single_flavor: " + \
                        "Missing LLVM tool chain." + \
                        "Expected location " + os.environ[COMPILER_BIN_NAME])
     sys.exit(-1)

   if os.path.exists(os.getenv("CLANG1602LINUX_TOOLS_BIN")):
     os.environ[COMPILER_PREFIX] = os.getenv("CLANG1602LINUX_TOOLS_BIN") + "/"
   else:
     logger.error("[buildex.py] ERROR: {} does not exist".format(os.getenv("CLANG1602LINUX_TOOLS_BIN")))
     logger.error("[buildex.py] ERROR: build_single_flavor: " + \
                        "Missing LLVM tool chain." + \
...
def make_edk_tools():
  ...
  subprocess.call(["make", "-j", "9", "-C", tools_path])
