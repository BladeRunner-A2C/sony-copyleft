 ...
 *  Copyright (C) 2012,2018,2021,2023 Qualcomm Technologies, Inc. All rights reserved.
 *
 ...
 *  YYYY-MM-DD		who		why
 *  -----------------------------------------------------------------------------
 *  2023-03-17      batta  Updated open_file API.
...
int open_file(const char* file_name, bool for_reading)
{
	...
	char mem_debug_filename[MAX_FILENAME_LENGTH] = { 0 };
	char final_filename[MAX_FILENAME_LENGTH] = { 0 };
	...
	if (file_name == NULL || strlen(file_name) == 0 || strlen(file_name) >= MAX_FILENAME_LENGTH) {
		dbg(LOG_ERROR, "Invalid file name");
		return fd;
	}

	if (strlcpy(final_filename, file_name, sizeof(final_filename)) >= sizeof(final_filename)) {
		dbg(LOG_ERROR, "Buffer overflow occured");
		return fd;
	}

	if (for_reading) {
#if defined(WINDOWSPC) || defined(WIN32) 
		_sopen_s(&fd, final_filename, O_RDONLY | _O_BINARY, _SH_DENYNO, 0);
#else
		...
		if (strstr(final_filename, "..") != NULL)
		{
			dbg(LOG_ERROR, "\"..\" or Slash was found in file path. Files cannot modify path.");
			return fd;
		}

		if (is_mem_debug_mode) {
			if (strlcpy(mem_debug_filename, kickstart_options.path_to_save_files, sizeof(mem_debug_filename)) >= sizeof(mem_debug_filename) ||
				strlcat(mem_debug_filename, kickstart_options.saved_file_prefix, sizeof(mem_debug_filename)) >= sizeof(mem_debug_filename) ||
				strlcat(mem_debug_filename, file_name, sizeof(mem_debug_filename)) >= sizeof(mem_debug_filename)) {
				dbg(LOG_ERROR, "String was truncated while concatenating");
				return fd;
			}
			if (strlcpy(final_filename, mem_debug_filename, sizeof(final_filename)) >= sizeof(final_filename)) {
				dbg(LOG_ERROR, "Buffer overflow occured");
				return fd;
			}
		...
		dbg(LOG_EVENT, "Saving '%s'", final_filename);

#if defined(WINDOWSPC) || defined(WIN32) 
		_sopen_s(&fd, final_filename, write_flags | _O_BINARY, _SH_DENYNO, _S_IREAD | _S_IWRITE);
#else
#if !defined(LINUXPC) && !defined(linux)
		if (stat(final_filename, &fileInfo) == 0 && S_ISBLK(fileInfo.st_mode)) {
			// If the file is a block device
			write_flags = write_flags | O_DIRECT;
	    }
#endif
		fd = open(final_filename,
			write_flags,
