 ...
 *
 *  Copyright (C) 2010,2016, 2018, 2023 Qualcomm Technologies, Inc. All rights reserved.
 *
 ...
 * ==========================================================================================
 *   $Header: //depot/HTE/QDART/QDART_MFG/BSP_Storage/QSaharaServer/QSaharaServer/sahara_protocol.h#6 $
 *   $DateTime: 2019/07/02 00:05:44 $
 *   $Author: anushkas $
 *
 *  Edit History:
 *  YYYY-MM-DD		who		why
 *  -----------------------------------------------------------------------------
 *  2023-05-04       batta   Added support for Sahara write and added MAX_WRITE_MAPPINGS macro.
...
typedef enum
{
  ...
  SAHARA_WRITE_DATA_ID              = 0x14, // sent from target to host
...
// WRITE_DATA command packet type - sent from target to host
//   sends data segment offset and length to be written to current host
//   image file
typedef struct
{
    sahara_packet_header header;
    uint64_t data_offset;           // offset into image file to write data to
    uint32_t image_id;              // ID of image to be transferred.
    uint32_t data_length;           // length of data segment to be written
                                    // to image file.
} sahara_packet_write_data;
