 ...
 *
 *  Copyright (c) 2010,2016, 2018-20, 2022, 2023 Qualcomm Technologies Inc. All rights reserved.
 ...
 * ==========================================================================================
 *   $Header: //depot/HTE/QDART/QDART_MFG/BSP_Storage/QSaharaServer/QSaharaServer/sahara_protocol.c#10 $
 ...
 *  YYYY-MM-DD       who     why
 *  -----------------------------------------------------------------------------
 *  2023-10-16       batta   Fix for security CR 2363095.
 *  2023-05-04       batta   Added support for Sahara write.
 *  2023-03-30       batta   Added a variable which will be set when the device is in crash mode.
 *  2022-12-27       batta   Updated sahara_data structure from the mode received from Hello packet.
 *  2022-04-22       ck      Added fd check before attempting to close file in SAHARA_WAIT_DONE_RESP
 *  2022-04-13       batta   Added a check to handle zero sized dump regions.
 *  2021-09-16       rhy     Added check for ".." in sahara memory table filenames.
...
static int num_write_mappings = 0;
...
static write_mapping_t write_mappings[MAX_WRITE_MAPPINGS];
static int num_cmd_mappings = 0;
static uint64_t maximum_size_of_file = 0;
bool is_mem_debug_mode = false;
...
bool add_write_mapping(int id, const char* name, int *max_size,
    write_mapping_t mapping_info[], int* num_mappings) {
    int i;

    if (name == NULL || max_size == NULL || num_mappings == NULL || mapping_info == NULL) {
        dbg(LOG_ERROR, "Arguments passed to %s are invalid", __func__);
        return false;
    }

    if (MAX_WRITE_MAPPINGS <= *num_mappings) {
        dbg(LOG_ERROR, "Cannot accept more than %d arguments", MAX_WRITE_MAPPINGS);
        return false;
    }

    dbg(LOG_INFO, "in add_mapping(%d,'%s, %d')", id, name, *max_size);

    /* Below logic first checks if the mapping already exists in the mapping_list based on image ID.
    *  If the mapping exist, return true to avoid adding duplicate mappings.
    *  Otherwise, add a new mapping.
    */ 
    for (i = 0; i < *num_mappings; i++) {	
        if (id == mapping_info[i].id) {
            mapping_info[i].name = name;
            mapping_info[i].max_size = *max_size;
            dbg(LOG_INFO, "Just mapped '%s' to '%d' (0x%X), note that i=%d and num_mappings=%d and max_size = %d", name, id, id, i, *num_mappings, *max_size);
            return true;
        }
    }
    ...
    mapping_info[*num_mappings].id = id;
    mapping_info[*num_mappings].name = name;
    mapping_info[*num_mappings].max_size = *max_size;
    *num_mappings += 1;
    dbg(LOG_INFO, "Just mapped '%s' to '%d' (0x%X), note that i=%d and num_mappings=%d and max_size = %d", name, id, id, i, *num_mappings, *max_size);
...
bool add_input_to_write_mapping_list(const char* arg, bool is_writeable_img) {
    int image_id = 0;
    int max_size = 0;
    int status = 0;
    char* path = NULL;
    char* image_id_str = NULL;

    /* Logic : Find the location of first colon first and then last colon.
    * User might provide a path like this - 00:C:\path\filename.txt:20
    * In this scenario, the colon ':' after "C" should not be considered.
    */

    char* delim1 = strchr(arg, ':');

    // Find the first colon
    if (delim1 != NULL) {
        // Find the last colon
        char* delim2 = strrchr(arg, ':');
        if (delim1 == delim2) {
            dbg(LOG_ERROR, "Input is not in the form <image_id:image_file_name:max_size_of_file>");
            return false;
        }
        if (delim2 != NULL) {
            // Extract the path between first and last colon.
            size_t path_len = delim2 - delim1 - 1;
            
            // Validate path_len.
            if (path_len >= MAX_FILE_NAME_SIZE) {
                dbg(LOG_ERROR, "Length of the path given has execeeded.");
                return false;
            }

            path = (char*)malloc((path_len + 1) * sizeof(char));
            if (path == NULL) {
                dbg(LOG_ERROR, "malloc failed : %s", strerror(errno));
                return false;
            }

            strlcpy(path, delim1 + 1, path_len + 1);
            path[path_len] = '\0';

            // Convert first integer substring to integer.
            image_id_str = (char*)malloc(sizeof(char) * (delim1 - arg + 1));
            if (image_id_str == NULL) {
                dbg(LOG_ERROR, "malloc failed : %s", strerror(errno));
                return false;
            }
            strlcpy(image_id_str, arg, delim1 - arg + 1);
            image_id_str[delim1 - arg] = '\0';
            image_id = atoi(image_id_str);

            // Check if the image ID exceeds the maximum supported image ID limit.
            if (image_id > MAX_IMAGE_IDS) {
                dbg(LOG_ERROR, "The image ID provided exceeds the maximum supported image IDs.");
                return false;
            }

            // Store the max size of file using second colon pointer.
            max_size = atoi(delim2 + 1);
        }
        else {
            dbg(LOG_ERROR, "Input is not in the form <image_id:image_file_name:max_size_of_file>");
            return false;
    ...
    if (is_writeable_img) {
        status = add_write_mapping(image_id, path, &max_size,
            write_mappings, &num_write_mappings);
        if (!status) {
            return status;
        }
        /* If the file is writeable, then it should be readable as well.
        *  Add the mapping in Sahara mappings
        */
        status = add_mapping(image_id, path, sahara_mappings, &num_sahara_mappings);
    }
...
const char* get_file_from_write_mapping_list(int image_id) {
    int i;
    const char* filename = NULL;

    dbg(LOG_INFO, "num_write_mappings = %d", num_write_mappings);

    for (i = 0; i < num_write_mappings; i++)
    {
        dbg(LOG_INFO, "(%d) Looking for image %d, write_mappings[%d].id = %d", i, image_id, i, write_mappings[i].id);

        if (image_id == write_mappings[i].id)
        {
            dbg(LOG_INFO, "Matched!!! - '%s'", write_mappings[i].name);
            maximum_size_of_file = write_mappings[i].max_size;
            // It is expected that for the first time the file will not be present at Host.
            //Open the file if it doesn't exist. 
            sahara_data.fd = open_file(write_mappings[i].name, false);
            if (sahara_data.fd <= -1) {
                return filename;
            }
            if (close_file(sahara_data.fd) == false) {
                dbg(LOG_ERROR, "Error closing file - %s", strerror(errno));
                return false;
            }
            filename =  find_file(write_mappings[i].name);
        }
    }
    return filename;
}
...
void show_write_mapping_list() {
    int i;
    printf("Write mappings:\n");
    for (i = 0; i < num_write_mappings; i++) {
        printf("%d : %s :: %d KB\n", write_mappings[i].id, write_mappings[i].name, write_mappings[i].max_size);
    }
}
...
bool sahara_start() {
    ...
    sahara_packet_write_data* sahara_write_data = (sahara_packet_write_data*)sahara_data.rx_buffer;
...
*/
              ...
              if ((sahara_data.mode < SAHARA_MODE_LAST) && (cmds_executed == false)) {
                  ...
                  sahara_data.mode = sahara_hello->mode;
              }
        ...
        case SAHARA_WAIT_COMMAND:
            ...
            if (true == is_end_of_image_rx (((sahara_packet_header *)sahara_data.rx_buffer)->command)) {
            ...
            else if (SAHARA_WRITE_DATA_ID == ((sahara_packet_header*)sahara_data.rx_buffer)->command) {
                dbg(LOG_EVENT, "RECEIVED <-- SAHARA_WRITE_DATA");
                if (false == started_raw_image_transfer) {
                    id_mapped_file = get_file_from_write_mapping_list(sahara_write_data->image_id);
                    if (NULL == id_mapped_file)
                    {
                        dbg(LOG_ERROR, "Matching input image for ID %d not found",  sahara_write_data->image_id);
                        return false;
                    }
                    dbg(LOG_STATUS, "Requested ID %d, file: \"%s\"", sahara_write_data->image_id, id_mapped_file);
                    CurrentImageId = sahara_write_data->image_id;
                    sahara_data.fd = open_file(id_mapped_file, false);
                    if (sahara_data.fd <= -1) {
                        dbg(LOG_ERROR, "Unable to open file : %s. Error %s", id_mapped_file, strerror(errno));
                        exit(EXIT_FAILURE);
                    }
                    started_raw_image_transfer = true;
                    sahara_data.timed_data_size = 0;
                ...
                /* Start the raw data transfer */
                if (NULL != id_mapped_file) {
                    // If there is a failure in writing data to the file then close the file.
                    if (false == store_write_data()) {
                        if (close_file(sahara_data.fd) == false) {
                            dbg(LOG_ERROR, "File closing failed : %s", strerror(errno));
                            exit(EXIT_FAILURE);
                        }
                        sahara_data.fd = -1;
                        return false;
        ...
        case SAHARA_WAIT_MEMORY_TABLE:
            if (memory_table_length > 0) {
				...
				if (true == sahara_data.ram_dump_64bit) {
                    ...
                    // Overflow check.
                    if(memory_table_length > (INT_MAX * sizeof(dload_debug_type))) {
                       // Set to maximum possible value.
                       num_debug_entries = INT_MAX;
                    } else {
					...
					for (i = 0; i < num_debug_entries; ++i) 
					{
                        ...
                        if (strstr(sahara_memory_table[i].filename, "..") != NULL)
                        {
						...
						if(FindLastSlash(sahara_memory_table[i].filename)>0)
						{
                            ...
                            if (stat(MyNewPath, &status_buf) != 0)
                            {
                            ...
                            {
                                if ((status_buf.st_mode & S_IFDIR) != S_IFDIR)
                                {
                                    dbg(LOG_ERROR, "Not a directory '%s'. %s.\n", MyNewPath, strerror(errno));
                                    return false;
                                }
                                printf("Path %s already exists", MyNewPath);
                            }
            ...
            if (sahara_data.state == SAHARA_WAIT_MEMORY_REGION)
            {
                ...
                if (false == sahara_rx_data((size_t)memory_table_length)) {
                    ...
                    if (!is_file_opened) {
                        ...
                        if (sahara_data.fd == -1) {
                            dbg(LOG_ERROR, "Unable to open the file : %s. Error %s", sahara_memory_table[num_debug_entries].filename, strerror(errno));
                            exit(EXIT_FAILURE);
                        }
                        is_file_opened = true;
                    }
                    ...
                    if (sahara_memory_table[num_debug_entries].length == 0)
                    {
                        ...
                        if (num_debug_entries >= 0 && sahara_memory_table[num_debug_entries].length > 0) {
			                sahara_data.fd = open_file(sahara_memory_table[num_debug_entries].filename, false);
		                    if (sahara_data.fd == -1) {
			                 dbg(LOG_ERROR, "Unable to open the file : %s. Error %s", sahara_memory_table[num_debug_entries].filename, strerror(errno));
			                 exit(EXIT_FAILURE);
                            ...
                            // If there is one or more than one consecutive zero sized dump regions, skip all of them.
                            while (num_debug_entries >= 0 && sahara_memory_table[num_debug_entries].length == 0) {
                               dbg(LOG_INFO, "Skipping zero sized region : 0x%x and 0x%x\n", sahara_memory_table[num_debug_entries].mem_base, sahara_memory_table[num_debug_entries].length);
                               num_debug_entries--;
                            }
                         }
                    }
...
static bool store_write_data()
{
    ...
    * This is to distinguish files from directories, symbolic links, etc.,
    * The read and write operations will vary based on the file's type.
    */
    int is_reg_file = is_regular_file(sahara_data.fd);
    uint32_t bytes_written = 0, bytes_to_write_next;
    uint64_t DataOffset = 0;
    uint64_t DataLength = 0;
    uint64_t position = 0;

    /* If the file is not regular, return an error */
    if (is_reg_file == -1) {
        dbg(LOG_ERROR, "Could not get file info");
        return false;
    }

    DataOffset = (uint64_t)sahara_write_data->data_offset;
    DataLength = (uint64_t)sahara_write_data->data_length;

    dbg(LOG_INFO, "store_write_data: Offset %ld", DataOffset);
    dbg(LOG_INFO, "store_write_data: Offset 0x%X", DataOffset);
    dbg(LOG_INFO, "store_write_data: Length %ld", DataLength);
    dbg(LOG_INFO, "store_write_data: Length 0x%X", DataLength);

    if (DataLength == 0) {
        dbg(LOG_ERROR, "Invalid length %ld bytes request to write", DataLength);
        return false;
    }

    if (DataOffset >= maximum_size_of_file * CONVERT_TO_KB) {
        dbg(LOG_ERROR, "Offset exceeds the maximum limit");
        return false;
    }

    if (DataOffset + DataLength > maximum_size_of_file * CONVERT_TO_KB) {
        dbg(LOG_ERROR, "File limit execeeded");
        return false;
    }

    if (lseek(sahara_data.fd, (long)DataOffset, SEEK_SET) <= -1) {
        dbg(LOG_ERROR, "lseek failed : %s", strerror(errno));
        return false;
    }

    while (bytes_written < (uint32_t)DataLength) {
        bytes_to_write_next = MIN((uint32_t)DataLength - bytes_written, SAHARA_RAW_BUFFER_SIZE);
        if (false == sahara_rx_data(bytes_to_write_next)) {
            dbg(LOG_ERROR, "Rx Sahara Image Failed");
            return false;
        }
        retval = write(sahara_data.fd, sahara_data.rx_buffer, bytes_to_write_next);
        if ((uint32_t)retval != bytes_to_write_next) {
            dbg(LOG_ERROR, "Written %d bytes, but was asked for %ld bytes", retval, DataLength);
            return false;
        }

        bytes_written += bytes_to_write_next;
    }
    sahara_data.timed_data_size += (uint32_t)DataLength;
    return true;
}
