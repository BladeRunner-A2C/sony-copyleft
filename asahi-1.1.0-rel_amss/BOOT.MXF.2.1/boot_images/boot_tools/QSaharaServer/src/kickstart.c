 ...
 *
 * Copyright (C) 2010,2016, 2018, 2020, 2022, 2023 Qualcomm Technologies, Inc. All rights reserved.
 ...
 *  YYYY-MM-DD       who     why
 *  -----------------------------------------------------------------------------
 *  2023-10-05       batta   Added a command line option - W.
 *  2022-03-29       ck      Added ethernet support
 *  2019-11-12       rrawat  Added function table to separate transport mediums
...
/*Contains the current medium information*/
function_table Medium;

/*
 Map the generic functions to current medium
*/
void map_function_table()
{
  switch(com_port.transport_medium)
  {
  case USING_UART:
    uart_comm_populate_medium_interface(&Medium);
    break;
    
  case USING_ETHERNET:
    ethernet_comm_populate_medium_interface(&Medium);
    break;
    
  default:   
    usb_comm_populate_medium_interface(&Medium);
    break;
  }
}
...
void usage(char *program_name)
{
  ...
  printf(
         ...
         " -U                     --UART                      Sahara protocol over UART"
         " -e                     --ethernet                  Sahara protocol over ethernet"
         " -n                     --ethernet_port             Ethernet/Network port number to listen for connections on when using ethernet interface"
         " -W <cmd_id:path:size>  --write                     Sahara write to file. Size will be in KB."
...
int main (int argc, char *argv[])
{
    ...
    const char *const short_options = "hp:v:c:mis:g:w:r:lt:j:b:ku:xa:o:U:en:W:";  // possible cmd line short options
    ...
    {
      strlcpy(TempSz+i,argv[option],strlen(argv[option]) + 1);
    ...
    do {
      ...
      case 'W':
        if (false == add_input_to_write_mapping_list(optarg, true)) {
           dbg(LOG_ERROR, "Failed to add file to write list");
           return EXIT_FAILURE;
        }
        enable_sahara_transfer = true;
        break;
      ...
      case 'e':
        com_port.transport_medium = USING_ETHERNET;
        break;
        
      case 'n':
        com_port.ethernet_port = atoi(optarg);
        break;
    ...
    if (true == enable_sahara_transfer){
	  if (com_port.transport_medium == USING_ETHERNET)
	  {
	    /* Verify ethernet port has been set */
		if (com_port.ethernet_port <= 0)
		{
		  dbg(LOG_ERROR, "Ethernet port not provided; use -n option.");
		  return EXIT_FAILURE;
		}
	  }
	  else
      ...
      for (i = 0; i < kickstart_options.port_connect_retries; i++) {
        ...
        if (true == Medium.port_connect(com_port.port_name))
          break;
      }
