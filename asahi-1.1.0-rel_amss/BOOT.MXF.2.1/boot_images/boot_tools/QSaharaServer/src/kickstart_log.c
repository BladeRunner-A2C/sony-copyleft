/*================================================================================================
 *
 * FILE:        kickstart_log.c
 *
 * DESCRIPTION:
 *    This module implements the logic for logging EVENT, INFO, WARNING, ERROR messages
 *
 *
 *        Copyright © 2009-2012,2018, 2023 Qualcomm Technologies, Inc.
 *               All Rights Reserved.
 *===============================================================================================
 *
 *
 *  kickstart_log.c : This module implements the logic for logging EVENT, INFO, WARNING, ERROR
 *  messages
 * ==========================================================================================
 *   $Header: //depot/HTE/QDART/QDART_MFG/BSP_Storage/QSaharaServer/QSaharaServer/kickstart_log.c#3 $
 *   $DateTime: 2019/07/02 00:05:44 $
 *   $Author: anushkas $
 *
 *  Edit History:
 *  YYYY-MM-DD		who		why
 *  -----------------------------------------------------------------------------
 *  2023-04-19       batta   Added timestamp support for LOG_EVENT.
 *  2010-09-28       ng      Added command mode support
 *
 *
 *==========================================================================================
 */
#include "common_protocol_defs.h"
#include "kickstart_log.h"
#if defined(LINUXPC) || defined(linux)
#include <stdarg.h>
#endif

/*===========================================================================
 *  FUNCTION:  kickstart_log
 *
 *  DESCRIPTION
 *  Kickstart logger, prints the function name, line number and the logs
 *
 *  PARAMETERS
 *  func name      - function name
 *  line_number    - line number
 *  format         - variable list of arguments
 *
 *  RETURN VALUE
 *  NONE
 *
 *  SIDE EFFECTS
 *  NONE
 *  ===========================================================================*/
void kickstart_log (int log_level, const char *func_name, int line_number, const char *format, ...)
{
   va_list args;
   char log[LOG_BUFFER_SIZE];
   char date_time_buff[DATE_TIME_BUFFER_SIZE];

   time_t current_time;
   struct tm *local_time;

   current_time = time(NULL);
   local_time = localtime(&current_time);

   va_start (args, format);
   vsnprintf(log, sizeof(log), format, args);
   va_end (args);

    if ((log_level == LOG_ERROR) ||
        log_level == LOG_WARN ||
        log_level == LOG_STATUS ||
        log_level == LOG_EVENT ||
        kickstart_options.verbose > 0) {

        strftime(date_time_buff, DATE_TIME_BUFFER_SIZE, "%m.%d.%Y-%H.%M.%S", local_time);
        printf("\n%s : ", date_time_buff);

        switch (log_level) {
        case LOG_ERROR:
            LOGE("ERROR: function: %s:%d %s\n", func_name, line_number, log);
            break;

        case LOG_INFO:
            LOGI("INFO: function: %s:%d %s\n", func_name, line_number, log);
            break;

        case LOG_WARN:
            LOGI("WARNING: function: %s:%d %s\n", func_name, line_number, log);
            break;

        case LOG_EVENT:
            LOGI("EVENT: %s\n", log);
            break;

        case LOG_STATUS:
            LOGE("%s\n", log);
            break;
        }
    }
}
