...
from qc_dt_schema_common import Handle_DT_VALIDATE, \
								Handle_DT_EXIT, \
								Get_tools_path, \
								Exec_command, \
								Create_log_file, \
...
OVERLAY_FILENAME_STRING = "overlay"

# Default DT verification tools
DT_DOC_VALIDATE_EXEC="qc-dt-doc-validate"
DT_EXTRACT_EXAMPLE_EXEC="qc-dt-extract-example"
DT_MK_SCHEMA_EXEC="qc-dt-mk-schema"
DT_VALIDATE_EXEC="qc-dt-validate"
...
QC_DT_SCHEMA_CHECK_TOOL = "qc_dt_schema_check.log"

FINAL_OVERLAY_FILENAME_STRING = "_tmp_overlay_validation"

##############################################################################
# Get_list_dts_files
##############################################################################

def Get_list_dts_files(args, dtsfilespath):
	...
	for file in os.listdir(dtsfilespath):
		if(file.endswith(DTS_PP_EXTN)):
			...
			if OVERLAY_FILENAME_STRING in file:
				if FINAL_OVERLAY_FILENAME_STRING not in file:
					list_overlay_files.append(filename)
			else:
				list_dts_files.append(filename)
	
	# override overlay list if overlayspath arg is available
	if args.overlayspath:
		list_overlay_files = []
		overlayspaths = (args.overlayspath).split()
		for op in overlayspaths:
			if op.endswith(DTBO_EXTN):
				filename = op.replace(DTBO_EXTN, "")
				list_overlay_files.append(filename)
	
	return list_dts_files, list_overlay_files
...
def Generate_overlay_file(overlay_dts_files, list_dts_files, output_directory_path, dtsfilespath):
	
	main_filename = ""
	if overlay_dts_files:
		output_filename = FINAL_OVERLAY_FILENAME_STRING + DTS_PP_EXTN
		output_filename = os.path.join(dtsfilespath, output_filename)
		for files in list_dts_files:
			if "pre" not in os.path.basename(files):
				main_filename = files + DTS_PP_EXTN
		...
		with open(output_filename, "w") as output_file:
			...
			for files in overlay_dts_files:
				files += DTS_PP_EXTN
		...
		if args.verbose:
			print("Concatenating overlay files ")
			
		list_dts_files.append(output_filename.replace(DTS_PP_EXTN, ""))
...
def main():
	...
	parser.add_argument("-l", "--overlayspath", help="Include Path(s) for all overlay output ")
	parser.add_argument("-m", "--arch", help="Architecture can be arm|hexagon")
	...
	if args.verbose:
		print("DTS files are located in: [%s]" % (dtsfilespath))
	...
	#######################################################################
	if os.path.exists(CPP) != True:
		error_msg = "Compiler [" + CPP + "] does not exist"
		Handle_DT_EXIT(error_msg, args.warning)
	
	if os.path.exists(DTC) != True:
		error_msg = "Compiler [" + DTC + "] does not exist"
		Handle_DT_EXIT(error_msg, args.warning)
	
	# Step 4: Make list of all *.dts files and a delete file list
	#######################################################################
	list_dts_files, list_overlay_files = Get_list_dts_files(args, dtsfilespath)
	
	list_dts_files = Generate_overlay_file(list_overlay_files, list_dts_files, output_directory_path, dtsfilespath)
	...
	if len(compatible_string_undefined):
		error_msg = ("**** WARNING: %s Discrepancies found ****" % (str(len(compatible_string_undefined))))
