...
#
#
#  Copyright (c) 2014, 2018, 2020, 2022-23 Qualcomm Technologies, Inc.  All Rights Reserved.
...
# --------   ---       ------------------------------------------------------
#03/28/23 aishwark     Updating IMAGE_VERSION_LENGTH
# 04/20/21   yps       Copy manifest.xml to boot_tools folder 
...
def find_xml():
  ...
  manifest_dest = os.path.join(base_directory,'boot_tools','manifest','build')
  if not os.path.exists(os.path.join(manifest_dest,'manifest.xml')):
      ...
      for current_file in files:
       if current_file == 'manifest.xml':
         ...
         if os.path.basename(os.path.normpath(root)).lower() == 'build':
           manifest_path = os.path.join(root,
                               'manifest.xml')
           break;
    #Copy manifest.xml to boot_tools\manifest\build
    if manifest_path!='':
      try:
  ...
  manifest_path = os.path.join(manifest_dest,'manifest.xml')
...
def qc_version_builder(target, source):
  ...
  # or singles, or mix (common use case: one target multiple sources).
   ...
   qc_version_str = "".join([
       "const char QC_IMAGE_VERSION_STRING_AUTO_UPDATED[" + str(IMAGE_VERSION_LENGTH) +"]=", 
...
def oem_version_builder(target,
   ...
   variant_version_str = "".join([ "const char IMAGE_VARIANT_STRING_AUTO_UPDATED[" + str(IMAGE_VERSION_LENGTH) +"]=\"IMAGE_VARIANT_STRING=", build_type, "\";\n"])

   # Get XML data for oem_version informaiton
   XmlCfg = et.ElementTree(file=source_full).getroot()

   XmlImageTree = XmlCfg.find('image_tree')
   if XmlImageTree is not None:
      XmlImageTreeOemVersion = XmlImageTree.find('oem_version')

   if XmlImageTreeOemVersion is not None:
      GlobalenVersionInfo = XmlImageTreeOemVersion.text.strip()
      if GlobalenVersionInfo is not None:
         oem_version_str = "".join([ "const char OEM_IMAGE_VERSION_STRING_AUTO_UPDATED[" + str(IMAGE_VERSION_LENGTH) +"]=\"OEM_IMAGE_VERSION_STRING=", GlobalenVersionInfo, "\";\n"])
      else:
         oem_version_str = "".join([ "const char OEM_IMAGE_VERSION_STRING_AUTO_UPDATED[" + str(IMAGE_VERSION_LENGTH) +"]=\"OEM_IMAGE_VERSION_STRING=", oem_environ, "\";\n"])
   
   if XmlImageTreeOemVersion is None :
      if oem_environ == "NULL":
         oem_ver = get_oem_version_string()
         oem_version_str = "".join([ "const char OEM_IMAGE_VERSION_STRING_AUTO_UPDATED[" + str(IMAGE_VERSION_LENGTH) +"]=\"OEM_IMAGE_VERSION_STRING=", oem_ver, "\";\n"])
      else :
         oem_version_str = "".join([ "const char OEM_IMAGE_VERSION_STRING_AUTO_UPDATED[" + str(IMAGE_VERSION_LENGTH) +"]=\"OEM_IMAGE_VERSION_STRING=", oem_environ, "\";\n"])
...
def oem_uuid_builder(target):
   ...
   oem_uuid_str = "".join([
      "const char OEM_IMAGE_UUID_STRING_AUTO_UPDATED[" + str(IMAGE_VERSION_LENGTH) +"]=\"OEM_IMAGE_UUID_STRING=",
      oem_uuid, "\";\n"])
...
def main():
  ...
  oem_version_builder([os.path.join(options.targetdir,
                                    'oem_version.c')],
                      options.build_type,[options.xml],options.oem_environ)
