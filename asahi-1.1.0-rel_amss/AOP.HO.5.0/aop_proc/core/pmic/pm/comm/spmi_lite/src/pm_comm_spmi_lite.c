...
$Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/core/pmic/pm/comm/spmi_lite/src/pm_comm_spmi_lite.c#1 $
...
boolean spmi_initialized = FALSE;
...
pm_err_flag_type pm_comm_read_byte(uint32 slave_id, uint16 addr, uint8* data, uint8 priority)
{
    ...
    if(TRUE == spmi_initialized)
    {
        ...
        {
            spmi_result = SpmiBus_ReadLong(0, slave_id, (Spmi_AccessPriority)priority, addr, data, 1, &dataReadLength);
...
pm_err_flag_type pm_comm_write_byte(uint32 slave_id, uint16 addr, uint8 data, uint8 priority)
{
    ...
    if(TRUE == spmi_initialized)
    {
        ...
        {
            spmi_result = SpmiBus_WriteLong(0, slave_id, (Spmi_AccessPriority)priority, addr, &data, 1);
...
pm_err_flag_type pm_comm_write_byte_array(uint32 slave_id, uint16 addr, uint32 num_bytes, uint8* data, uint8 priority) 
{
    ...
    if(TRUE == spmi_initialized)
    {
        ...
        {
            spmi_result = SpmiBus_WriteLong(0, slave_id, (Spmi_AccessPriority)priority, addr, data, num_bytes);
...
pm_err_flag_type pm_comm_read_byte_array (uint32 slave_id, uint16 addr, uint32 num_bytes, uint8* data, uint8 priority) 
{
    ...
    if(TRUE == spmi_initialized)
    {
        ...
        {
            spmi_result = SpmiBus_ReadLong(0, slave_id, (Spmi_AccessPriority)priority, addr, data, num_bytes, &dataReadLength);
