...
$Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/core/pmic/pm/rpmh/src/pm_rpmh.c#1 $
...
rpmh_client_handle pm_rpmh_h = NULL; 
...
void 
pm_rpmh_create_cmd_set(vrm_rpmh_vote_data *active_set, 
...
void  
pm_rpmh_post_set(vrm_rpmh_vote_data *active_set,
...
pm_rpmh_notify_completion_all(uint32 msg_id)
{
    ...
    rpmh_notify_completion_all(pm_rpmh_h, msg_id, NULL);
...
{
    // No msg_id check here so that call back is always called (even for msg_id 0)
    rpmh_notify_completion_single(pm_rpmh_h, msg_id, NULL);
