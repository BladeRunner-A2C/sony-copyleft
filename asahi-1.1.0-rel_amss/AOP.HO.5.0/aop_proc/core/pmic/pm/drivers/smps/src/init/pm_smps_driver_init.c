...
$Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/core/pmic/pm/drivers/smps/src/init/pm_smps_driver_init.c#1 $
...
#define SMPS_AHC_CTL1_OFFSET  0x3D
#define SMPS_AHC_CTL1_OFFSET_FTS520  0x31
#define SMPS_AHC_CTL3_OFFSET_FTS520  0x33
...
static pm_pwr_register_info_type smps_reg = 
{
  ...
  .AHC_CTL               = 0x03F,
  .PBS_VOTE_CTL          = 0x049,
  .PMOS_FORCE_SOFT_START = 0x00,
};

/* extern global variable to store the SMPS driver data */
extern pm_smps_data_type *pm_smps_data_arr[PM_MAX_NUM_PMICS];
             ...
             (peripheral_info->peripheral_subtype >= PM_HW_MODULE_FTS__FTS426_CTL)))
        {
           /* Flag this rail peripheral as unified register map peripheral */
            ...
            /* Store AHC Enablement Status */
              if((peripheral_info->peripheral_subtype >= PM_HW_MODULE_FTS__FTS520_CTL) || 
                 ((peripheral_info->peripheral_type == PM_HW_MODULE_HF_BUCK) && (peripheral_info->digital_major_version > 4)))
            {
              reg = (pm_register_address_type)(peripheral_info->base_address + SMPS_AHC_CTL3_OFFSET_FTS520);
            }
            else
            {
              reg = (pm_register_address_type)(peripheral_info->base_address + smps_ptr->pm_pwr_data.pwr_reg_table->AHC_CTL);
            }
            err_flag |= pm_comm_read_byte(smps_ptr->comm_ptr->slave_id, reg, &data , 0);
            ahc_status = (data & AHC_EN_CTL_MASK) != 0 ? TRUE : FALSE;
            smps_ptr->pm_pwr_data.pwr_specific_info[smps_index].is_ahc_enabled = ahc_status;
            ...
            if(ahc_status)
            {
              ...
              if((peripheral_info->peripheral_subtype >= PM_HW_MODULE_FTS__FTS520_CTL) || 
                 ((peripheral_info->peripheral_type == PM_HW_MODULE_HF_BUCK) && (peripheral_info->digital_major_version > 4)))
              {
                reg = (pm_register_address_type)(peripheral_info->base_address + SMPS_AHC_CTL1_OFFSET_FTS520);
              }
              else
              {
                reg = (pm_register_address_type)(peripheral_info->base_address + SMPS_AHC_CTL1_OFFSET);
              }

              err_flag |= pm_comm_read_byte(smps_ptr->comm_ptr->slave_id, reg, &data, 0);
              smps_ptr->pm_pwr_data.pwr_specific_info[smps_index].ahc_reserved_hr = ((data & AHC_RESERVED_HR_SEL_MASK) >> 4) * 8;
            }
        }
        else
        {
