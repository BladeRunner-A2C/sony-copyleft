...
$Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/core/pmic/pm/vrm/src/init/pm_vrm_vreg.c#1 $  
...
#define PM_MIN_STEPPER_RATE 1200
...
void pm_vrm_vreg_register_resources(uint8 pmic_index, pm_vrm_periph_type vreg_type, uint32 num_rails)
{
   ...
   uint8 pshold_status = 0;
   uint8 access_allowed = 0;
   uint32 drv_owner_mask = 0;
   boolean is_ahc_enabled = FALSE;
   boolean pbs_vote_status = FALSE;
   ...
   if(num_rails > 0)
   {
      ...
      for( rail_index = 0; rail_index < num_rails; rail_index++)
      {
         ...
         if((access_allowed) && (pm_rail_info != NULL) && (pm_rsrc_info != NULL) && (pm_dep_info != NULL))
         {
            ...
            pmic_rsrc_data.rsrc_category = pm_rail_info[rail_index].RsrcCategory;
            ...
            // headroom info
            pmic_rsrc_data.hr_min_state = pm_rail_info[rail_index].SafetyHR;
         ...
         if(access_allowed)
         {
            ...
            //AHC and headroom info
            is_ahc_enabled = pwr_info.pwr_data->pwr_specific_info[rail_index].is_ahc_enabled;
            //update ahc enable status to the resource struct.
            pmic_rsrc_data.ahc_en = is_ahc_enabled;
            //AHC reserved headroom is only populated for SMPS and is 0 for LDO, BOB etc 
            //If AHC is enabled then populate min_hr with reserved headroom
            pmic_rsrc_data.hr_min_state = 
              is_ahc_enabled ?
              pwr_info.pwr_data->pwr_specific_info[rail_index].ahc_reserved_hr :
              pm_rail_info[rail_index].SafetyHR;
            ...
            //get clx arb id for smps
            if (vreg_type == PM_VRM_PERIPH_SMPS)
            {
               base_addr = base_addr + 0x100;
               err_flag = pm_comm_get_vrm_id (slave_id, base_addr, &pmic_rsrc_data.vrm_clx_arb_id); 
               //CORE_VERIFY (err_flag == PM_ERR_FLAG_SUCCESS);
            }
            ...
            if((pmic_rsrc_data.volt_info.current_state > pmic_rsrc_data.volt_max_state) &&
               (pmic_rsrc_data.rsrc_category == VRM_RSRC_REGULATOR || pmic_rsrc_data.rsrc_category == VRM_RSRC_XO_BUFFER))
            {
            ...
            {
               pmic_rsrc_data.enable_stepper_rate_uv_us = PM_MIN_STEPPER_RATE;
               pmic_rsrc_data.volt_stepper_rate_uv_us = PM_MIN_STEPPER_RATE;
            }
            ...
            if(vreg_type != PM_VRM_PERIPH_BOB)
            {
               ...
               err_flag = pm_pwr_pbs_vote_en_status_alg ( pwr_info.pwr_data, pwr_info.comm_ptr, rail_index, &pbs_vote_status);
               CORE_VERIFY (err_flag == PM_ERR_FLAG_SUCCESS);

               if (select_pin != 0 || pbs_vote_status)
               {
            ...
            {
               if(pmic_rsrc_data.rsrc_category == VRM_RSRC_XO_BUFFER)
               {
                  pmic_rsrc_data.enable_info.fixed_time_usec = 
                     (pmic_rsrc_data.enable_info.fixed_time_usec * (1 << (pmic_rsrc_data.settling_wdog_timer)));
               }
               ...
               if((pmic_rsrc_data.enable_info.current_state == PM_ON) && (settle_flag == TRUE))
               {
                  ...
                  err_flag = pm_app_vrm_vreg_settle_status(&pmic_rsrc_data.periph_info,
                                                           pmic_rsrc_data.enable_info.fixed_time_usec, &settle_status, &pshold_status);
                  CORE_VERIFY (err_flag == PM_ERR_FLAG_SUCCESS);
