...
$Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/core/pmic/pm/vrm/src/init/pm_vrm_clk.c#1 $  
   ...
   if(num_clks > 0)
   {
      ...
      for( clk_index = 0; clk_index < num_clks; clk_index++)
      {
         ...
         if((access_allowed) && (pm_clk_info != NULL) && (pm_rsrc_info != NULL) && (pm_dep_info != NULL))
         {
            ...
            pmic_rsrc_data.rsrc_category = pm_clk_info[clk_index].RsrcCategory;
            ...
            // clk_ready info
            pmic_rsrc_data.settling_en = pm_clk_info[clk_index].ClkReadyEn;
            pmic_rsrc_data.settling_err_en = pm_clk_info[clk_index].ClkReadyErrEn;
            pmic_rsrc_data.settling_wdog_timer = (vrm_settling_wdog_timer)pm_clk_info[clk_index].ClkWdogTimer;
         ...
         if(access_allowed)
         {
            ...
            base_addr = clk_ptr->base_address + (clk_ptr->clk_reg_table->peripheral_offset*(clk_index+1));
            ...
            //enable interrupts if clk_ready is used
            if(pmic_rsrc_data.settling_en)
            {
               err_flag = pm_clk_irq_enable(pmic_index, clk_ptr->clk_common[clk_index+1], TRUE);
               CORE_VERIFY (err_flag == PM_ERR_FLAG_SUCCESS);
            }
