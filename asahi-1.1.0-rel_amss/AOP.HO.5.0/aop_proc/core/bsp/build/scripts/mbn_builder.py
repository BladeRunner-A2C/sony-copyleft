...
#
# Copyright 2011,2023 by Qualcomm Technologies, Incorporated.
# All Rights Reserved.
# QUALCOMM Proprietary/GTDR
#
#----------------------------------------------------------------------------
#
#  $Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/core/bsp/build/scripts/mbn_builder.py#1 $
...
def generate(env):
   ...
   if os.path.exists(env.subst('${BUILD_ROOT}/tools/build/scons/sectools/sectools/features/isc/parsegen')):
        env.LoadToolScript('mbn_tools', toolpath = ['${BUILD_ROOT}/tools/build/scons/sectools/sectools/features/isc/parsegen'])
        env.PrintInfo('Using mbn_tools.py from local sectools.int source component.')
   elif "SECTOOLS_DIR" in os.environ:
        env.LoadToolScript('mbn_tools', toolpath = [os.path.join(os.environ["SECTOOLS_DIR"], 'sectools', 'features', 'isc', 'parsegen')])
        env.PrintInfo('Using mbn_tools.py from SECTOOLS_DIR environment key.')
   else:
        env.PrintError('Unable to locate mbn_tools.py from either a local sectools.int source component, or from the SECTOOLS_DIR environment key.')
        sys.exit(1)
   mbn_generate(env)
