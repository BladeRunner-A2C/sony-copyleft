        ...
        for seg in range (elf_header.e_phnum):
            ...
            if (curr_phdr.p_type == self.LOAD_TYPE) and \
                ...
                for attr in self.attribute_list:
                  ...
                  segments = [int (i) for i in str.split (attr[2], ",")]
...
def aop_elf_builder (target, source, env):
   ...
   if env['MSM_ID'] in ['55','8250', 'Saipan', '8350', 'bitra', 'Chitwan', 'makena', 'cedros', 'Olympic','kodiak','makenaau']:
   ...
   elif env['MSM_ID'] in ['waipio','fillmore','kailua','aurora','lassen','netrani','kapiti','hamoa', 'pinnacle','camano']:
      re = AopElfBuilder (env, source_str, target_str, [
            ('segment.paddr', '+0x0B000000', '0,1'),
            ('segment.vaddr', '+0x0B000000', '0,1'),
            ('segment.paddr', '-0x60000000', '5,6'),
            ('segment.vaddr', '-0x60000000', '5,6'),
            ('entry_point', '+0x0B000000', '')
            ])
      ...
      re = AopElfBuilder (env, source_str, target_str, [
            ('segment.paddr', '+0x0B000000', '0,1'),
            ('segment.vaddr', '+0x0B000000', '0,1'),
            #offset is added such that this segment get loaded to ddr address AOPSS_DDR_CODE_SDI in aop.scl file(AOPSS_SYSDBG_START + 0xDB8F000 = AOPSS_DDR_CODE_SDI)
            ('segment.paddr', '+0xDB8F000', '5'),
            ('segment.vaddr', '+0xDB8F000', '5'),
            ('entry_point', '+0x0B000000', '')
            ])
   elif env['MSM_ID'] in ['lanai', 'pakala', 'palawan', 'milos', 'kaanapali']:
      re = AopElfBuilder (env, source_str, target_str, [
            ('segment.paddr', '+0x02E00000', '0'),
            ('segment.vaddr', '+0x02E00000', '0'),
            ('segment.paddr', '+0x02CE0000', '1'),
            ('segment.vaddr', '+0x02CE0000', '1'),
            ('segment.paddr', '-0x60000000', '5,6,7,8'),
            ('segment.vaddr', '-0x60000000', '5,6,7,8'),
            ('entry_point', '+0x0B000000', '')
            ])
