...
extern uint32 lanai_humu_rev_id;
...
void pmic_spmi_hiz_wa(void)
{
    /* QFPROM_CORR_PTE_ROW0_MSB | 0x221C209C has the MINOR_REV (29:28) fuse */
    uint32_t pte_row0_msb;
    uint8_t minor_rev;

    pte_row0_msb = HWIO_IN(QFPROM_CORR_PTE_ROW0_MSB);
    minor_rev = (pte_row0_msb >> 28);

    /* MINOR_REV would be 0x1 for 1.0.2 parts and 0 for 1.0 and 1.0.1 parts */
    /* On some parts PMIC_OTP_VER is blown and anything less that 0x5A
     * (or) if parts fused was 1.0 and 1.0.1 needs this WA */  
    if((minor_rev < 1))
    {
        if (lanai_humu_rev_id < PMIC_OPT_VER)
        {
            out_dword(0x670010, 0x5); //Vote for MXA Nominal using DBG DRV ID
            out_dword(0x5f4824, 0x5); //GRP0 PDC TCS0 COMMAND2(MXA) to Nominal
            out_dword(0x5f48ec, 0x5); //GRP0 PDC TCS1 COMMAND2(MXA) to Nominal
            out_dword(0x5f49b4, 0x5); //GRP0 PDC TCS2 COMMAND2(MXA) to Nominal
            out_dword(0x5f4c0c, 0x5); //GRP0 PDC TCS2 COMMAND2(MXA) to Nominal
        }
    }
} 
...
void init_phase_2(void)
{
  ...
  if ((ChipInfo_GetChipFamily() == CHIPINFO_FAMILY_LANAI) &&
