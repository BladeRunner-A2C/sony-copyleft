#  ===========================================================================
#
#  Copyright (c) 2011-2016, 2023 Qualcomm Technologies, Inc.
#  All Rights Reserved.
...
commands_lib.install_lib_modules(['pandas', 'pyelftools'])
# Create output directories
binary_output=os.path.join(args.output, "binaries")
...
pretty = PrettyPrint()
pretty.set_log_output(os.path.join(args.output,"hansei_log.txt"))
...
if args.elf == '':
  ...
  elf_dir = os.path.join(os.path.realpath(__file__).split('aop_proc')[0],'aop_proc','core','bsp','aop','build')
  elf_file = '';
  for file in os.listdir(elf_dir):
    if file.endswith(".elf"):
      elf_file = file
  if elf_file != '':
    args.elf = os.path.join(elf_dir,elf_file)
...
if args.elf == '':
  ...
  pretty.p("",fill="-")                           # "#------------------------------------------------------------------------------#"
  sys.exit(-1)
shutil.copy(args.elf,os.path.join(binary_output, os.path.basename(args.elf)))
args.elf = os.path.join(binary_output, os.path.basename(args.elf))
...
if args.target == 'pinnacles':
  args.target = 'pinnacle'
...
for arg in args.files :
  if os.path.isdir(arg):
    if os.path.exists(os.path.join(arg,"load.cmm")):
      with open(os.path.join(arg,"load.cmm"),'r') as f :
        for line in f.readlines():  
           if search(r"(DDRCS\S*)\s+(0x\S)",line):
              bin_name = search(r"(DDRCS\S*)\s+(0x\S+)",line).group(1)
              bin_base_addr = search(r"(DDRCS\S*)\s+(0x\S+)",line).group(2)
              ddr_base_addr[bin_name] = int(bin_base_addr,16)
...
if os.name == 'posix':
  temp_files = temp_files + [os.path.join(f, '*.BIN') for f in args.files if os.path.isdir(f)]
...
for dft in dump_file_types:
    ...
    if type(dft['base'][target_family])==dict:
        ...
        if(args.target in ['lanai','pakala','palawan']):
          if(dft['display_name'] == 'CODERAM'):
            required.add(0x8200000)
          else:
            if(dft['display_name'] == 'DATARAM'):
              required.add(0x8400000)
            else:
  ...
  for dft in dump_file_types:
    ...
    if (dfn in dft['name']) or ((dfe.upper() in dft['ext']) and (dfne.upper() in dft['basename'])):
      if dft.get('no_load',False) == True:
        ...
        if(args.target in ['lanai','pakala','palawan'] and dft['display_name'] == 'CODERAM'):
            base = 0x8200000
        if(args.target in ['lanai','pakala','palawan'] and dft['display_name'] == 'DATARAM'):
            base = 0x8400000
        if base not in read_bases:
          dumpfile_local = os.path.join(binary_output,dfn)
          shutil.copy(dumpfile, dumpfile_local)
          with open(dumpfile_local,'rb') as data:
            read_bases.add(base)
            system.load_hunk(base, data.read())
            required -= {base}
            #shutil.copy(dumpfile, os.path.join(binary_output,dfn))
            dft['size'] = os.path.getsize(dumpfile_local)
            store_bases[dft.get('display_name',"None")] = base
            pretty.p("\t\tLoading {0} at 0x{1:0>8x} from {2} ".format(dft['display_name'], base, dumpfile_local))
            try:
              logger.debug("Reading %s into 0x%0.8x" % (dumpfile_local, base))
            except:
              logger.debug("Reading %s into a variable address" % (dumpfile_local))
...
while args.no_ddr==0 and args.use_recovery==0:
  ...
  for dft in dump_file_types:
    if dft.get('display_name',"None") == "OCIMEM" or dft.get('display_name',"None") == "md_SHRDIMEM":
      if dft['path'] != '':
        with open(dft['path'], 'rb') as data:
          ...
          loaded=1
          if dft['display_name'] == 'md_SHRDIMEM':
            minidump = 1
            pretty.p("\tRecovering minidump RPMH Binary!...")
        break
  if loaded:
    try:
        ...
        for dft in dump_file_types:
          if dft.get('no_load',False) == True:
            #print("stage_4")
            if minidump:
              #pretty.p("Py3 minidump loading...")
              if "md" in dft.get('display_name',''):
                if dft['path'] != '':
                  path = dft['path']
                  ddr_base = dt_addr
                  size = 0x80000000
                  pretty.p("\t\tMinidump Loading {0} at {1} from {2} ".format(dft['display_name'], hex(ddr_base), path))
                  ddr.add_ddr(path, ddr_base, size)
                else:
                  pretty.p("\t\tWARNING: Minidump Missing {0} file".format(dft.get('display_name', 'unknown')))
            else:
              if "DDRCS" in dft.get('display_name',''):
                #import pdb; pdb.set_trace()
                #pretty.p("Py3 fulldump loading...")
                if dft['path'] != '':
                  path = dft['path']
                  size = dft['size']
                  if ddr_base_addr == {}:
                    ddr_base = dft['base'][target_family][dft['size']]
                    pretty.p("\t\tLoading {0} at {1} from {2} ".format(dft['display_name'], hex(ddr_base), path))
                    ddr.add_ddr(path, ddr_base, size)
                  for bin_name in ddr_base_addr :
                    if bin_name in dft['name']:
                      ddr_base = ddr_base_addr[bin_name]
                      pretty.p("\t\tFulldump Loading {0} at {1} from {2} ".format(dft['display_name'], hex(ddr_base), path))
                      ddr.add_ddr(path, ddr_base, size)
                else:
                  pretty.p("\t\tWARNING: Fulldump Missing {0} file".format(dft.get('display_name', 'unknown')))
        ...
        if cr_struct_addr == 0x0:
          ...
          pretty.p("\t\tWARNING: the magic identifier not correct")
          break
        else:
        ...
        pretty.p("\t\tRPMH Binary Recovered by {0}".format(cr_name.decode('utf-8').strip('\x00')))
        
        # Write recovered rpmh binary to 'RPMH_BINARY_RECOVERED.BIN'
        with open(os.path.join(binary_output,"RPMH_BINARY_RECOVERED.BIN"), 'wb') as cr_recover:
          #import pdb; pdb.set_trace()
    ...
    try:
        ...
        #import pdb; pdb.set_trace()
        if hansei_target in ['24','55']:
            cmd_db_addr = 0x8fe20000
            cmd_db_load_addr = 0x6fe20000
            cmd_db_size = 0x2000
        elif hansei_target in ['Olympic']:
            cmd_db_addr = 0x8fee0000
            cmd_db_load_addr = 0x6fee0000
            cmd_db_size = 0x2000
        elif hansei_target in ['855','6150','7150','1000']:
            cmd_db_addr = 0x85f20000
            cmd_db_load_addr = 0x65f20000
            cmd_db_size = 0x2000
        elif hansei_target in ['8250','Saipan', 'lahaina','cedros','waipio','fillmore','aurora']:
            cmd_db_addr = 0x80860000
            cmd_db_load_addr = 0x60860000
            cmd_db_size = 0x2000
        elif hansei_target in ['kailua','lanai','hamoa','pakala','palawan']:
            cmd_db_addr = 0x81C60000
            cmd_db_load_addr = 0x61C60000
            cmd_db_size = 0x2000
        elif hansei_target in [ 'kuno' ]:
            cmd_db_addr = 0x82220000
            cmd_db_load_addr = 0x62220000
            cmd_db_size = 0x20000
        elif hansei_target in ['pinnacle']:
            cmd_db_addr = 0x87EE0000
            cmd_db_load_addr = 0x67EE0000
            cmd_db_size = 0x2000
        elif hansei_target in [ 'camano' ]:
            cmd_db_addr = 0x81c60000
            cmd_db_load_addr = 0x61c60000
            cmd_db_size = 0x20000
        else:
...
if args.no_mangle==0:
  ...
  for rgn_name in rgn_names:
      ...
      for i in range(int(size or 0)):
          if data[i] != int.from_bytes(system.read(addr+i,1),"big"):
            match=0
...
with open(os.path.join(binary_output, "load_core_rpm_binaries.cmm"), 'w') as cmm:
  ...
  try:
    ...
    print("; failed to find remaining msgrams", file=cmm)
...
cmm_re = open(os.path.join(binary_output, "load_core_rpm_binaries_recovered.cmm"), 'w')
with open(os.path.join(binary_output, "load_core_rpm_binaries.cmm"), 'r') as cmm:
...
if (args.parser == ''):
    args.parser = os.path.join(os.path.realpath(__file__).split('aop_proc')[0], 'aop_proc','core','bsp','aop','scripts_py3')
    ...
    if(os.path.isfile(os.path.join(args.parser, 'aop_log_hfam.py'))==False):
      #print(os.path.join(args.parser, 'aop_log_hfam.py'))
      args.parser=os.path.join(os.path.realpath(__file__).split(os.path.basename(__file__))[0], '..', 'hfam_log_parser')
    # if (os.path.isfile(args.parser+'aop_log_hfam.py')==False):
    if(os.path.join(args.parser, 'aop_log_hfam.py')==False):
...
debug_data['parser_tbl'] = os.path.join(os.path.realpath(__file__).split('aop_proc')[0],'aop_proc','core','api','debugtrace')
debug_data['bcm_json_dir'] =  os.path.join(os.path.realpath(__file__).split('aop_proc')[0]+'aop_proc','core','bsp','aop','scripts_py3','hansei','clock_plan')
debug_data['bcm_csv_dir'] =  os.path.join(os.path.realpath(__file__).split('aop_proc')[0]+'aop_proc','core','power','cmd_db','scripts',hansei_target)
debug_data['ddr_mgr_log_parser'] =  os.path.join(os.path.realpath(__file__).split('aop_proc')[0]+'aop_proc','core','power','ddr_mgr','scripts')
debug_data['aop_serv_log_parser'] =  os.path.join(os.path.realpath(__file__).split('aop_proc')[0]+'aop_proc','core','power','utils','scripts')

[addr, data, size] = di.get_section_data("PM_CRAM_RECLAIM_POOL")
debug_data['reclaim_pool'] = {'start_addr':0, 'memory':0, 'size':0}
if addr != None:
  reclaim_mem = Memory()
...
for file in os.listdir(os.path.join(os.path.dirname(os.path.abspath(__file__)),'dumpers')):
  if not ('init' in file) and not (file in dumps_run) and not ('.pyc' in file) and not (file in dumps_skip) and not('pycache' in file) and not ('.bak' in file):
    dumps_run += [file]
    mod = import_module('.'+file.split('.')[0], 'dumpers')
    try:      
      mod.dump(debug_data)
...
#os.system("python 3" + os.path.dirname(os.path.abspath(__file__)) + os.sep + "tcs_content_dump" + os.sep + "printTcs.py " + os.path.dirname(list(args.files)[0]) + " " + args.output)
python_version = commands_lib.get_python_version()
command = f'{python_version} {os.path.join(os.path.dirname(__file__), "tcs_content_dump", "printTcs.py")} {os.path.dirname(list(args.files)[0])} {args.output}'
commands_lib.run_shell_cmd(command)
for dumpfile in args.files:
  dfn = os.path.basename(dumpfile)                        # File name + extension
  dfs = os.path.getsize(dumpfile)                         # File size
  dfe = os.path.splitext(dumpfile)[1]                     # File extension
  ...
  for dft in dump_file_types:
    ...
    if (dfn in dft['name']) or ((dfe.upper() in dft['ext']) and (dfne.upper() in dft['basename'])):
        dft['path'] = dumpfile
        if hansei_target in ['24','55','Olympic']:
          if dft.get('display_name',"None") == "MSGRAM2":
            os.system("perl " + debug_data['ddr_mgr_log_parser'] + os.sep + "MDMSCRIPTS" + os.sep + "ddr_log.pl " + dumpfile + "  > " + args.output + os.sep + "ddr_mgr_msgram_parse.txt" )
          ...
          if dft.get('display_name',"None") == "MSGRAM2":
            os.system("perl " + debug_data['ddr_mgr_log_parser'] + os.sep + "ddr_log.pl " + dumpfile + "  > " + args.output + os.sep + "ddr_mgr_msgram_parse.txt" )
          elif dft.get('display_name',"None") == "MSGRAM3":
            os.system("perl " + debug_data['aop_serv_log_parser'] + os.sep + "wa_log.pl " + dumpfile  + "  > " + args.output + os.sep + "aop_serv_msgram_parse.txt" )
          elif dft.get('display_name',"None") == "MSGRAM5":
            os.system(python_version + " " + debug_data['ddr_mgr_log_parser'] + os.sep + args.target + os.sep + "decode_msg11.py " + dumpfile + "  > " + args.output + os.sep + "aop_ddr_logs_msgram11.txt" )
        elif hansei_target in ['pinnacle']:
          if dft.get('display_name',"None") == "MSGRAM2":
            os.system("perl " + debug_data['ddr_mgr_log_parser'] + os.sep + "ddr_log.pl " + dumpfile + "  > " + args.output + os.sep + "ddr_mgr_msgram_parse.txt" )
          elif dft.get('display_name',"None") == "MSGRAM3":
            os.system("perl " + debug_data['aop_serv_log_parser'] + os.sep + "wa_log_mdm.pl " + dumpfile  + "  > " + args.output + os.sep + "aop_serv_msgram_parse.txt" )
          elif dft.get('display_name',"None") == "MSGRAM5":
            os.system(python_version + " " + debug_data['ddr_mgr_log_parser'] + os.sep + args.target + os.sep + "decode_msg5.py " + dumpfile + "  > " + args.output + os.sep + "aop_ddr_logs_msgram5.txt" )
        else:
          if dft.get('display_name',"None") == "MSGRAM6":
            ...
            #import pdb;pdb.set_trace()
            os.system("perl " + debug_data['aop_serv_log_parser'] + os.sep + "wa_log.pl " + dumpfile  + "  > " + args.output + os.sep + "aop_serv_msgram_parse.txt" )
          elif dft.get('display_name',"None") == "MSGRAM11":
            os.system(python_version + " " + debug_data['ddr_mgr_log_parser'] + os.sep + args.target + os.sep + "decode_msg11.py " + dumpfile + "  > " + args.output + os.sep + "aop_ddr_logs_msgram11.txt" )
