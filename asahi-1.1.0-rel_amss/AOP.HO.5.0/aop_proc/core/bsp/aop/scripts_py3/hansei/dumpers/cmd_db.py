...
def dump(debug_data): #dump_path, memory, debug_info, fill_char):
    ...
    try:
        if target in ['24', '55']:
            ver_data = memory.read(0x6FE20000, 4)
        elif target in ['855','6150','7150','1000']:
            ver_data = memory.read(0x65F20000, 4)
        elif target in ['8250','Saipan','lahaina','cedros','waipio','fillmore','aurora']:
            ver_data = memory.read(0x80860000, 4)
        elif target in ['kailua','lanai','hamoa', 'camano', 'pakala','palawan']:
            ver_data = memory.read(0x81C60000, 4)
        elif target in [ 'kuno' ]:
            ver_data = memory.read(0x82220000, 4)
        elif target in ['pinnacle']:
            ver_data = memory.read(0x87EE0000, 4)
        else:
            ver_data = memory.read(0x65FE0000, 4)
    except:
        debug_data['printer'].p("\tCMD_DB bin is not recovered...")
        return

    #import pdb; pdb.set_trace()
    smem_db_addr_die  = debug_info.variables[b'smem_db'].die
    smem_db_addr_addr = debug_info.variables[b'smem_db'].address
    ...
    with open(cmddb_file_name, 'w') as cmd_db_file:
        ...
        if target in ['24', '55']:
            smem_db_data_addr = 0x6FE20090
        elif target in ['855','6150','7150','1000']:
            smem_db_data_addr = 0x65F20090
        elif target in ['8250','Saipan', 'lahaina','cedros','waipio','fillmore','aurora']:
            smem_db_data_addr = 0x80860090
        elif target in ['kailua','lanai','hamoa','pakala','palawan']:
            smem_db_data_addr = 0x81C60090
        elif target in [ 'kuno' ]:
            smem_db_data_addr = 0x82220090
        elif target in ['pinnacle']:
            smem_db_data_addr = 0x87EE0090
        elif target in [ 'camano' ]:
            smem_db_data_addr = 0x81c60090
        else:
        ...
        for x in range(0,8):
            if smem_db.slv_id_info[x].slv_id != 0:
                if smem_db.slv_id_info[x].slv_id == 3:
                    ...
                    cmd_db_header_data = decode_object('bcm_db_headers', cmd_db_header_addr, None, memory, debug_info, die=cmd_db_header_die)
                    ...
                    bcm_data_die  = debug_info.variables[b'bcm_aux'].die
                    bcm_data_addr = smem_db_data_addr + smem_db.slv_id_info[x].data_offset
                    bcm_data = decode_object('bcm_db_headers', bcm_data_addr, None, memory, debug_info, die=bcm_data_die)
                    ...
                    for cnt in range(0, smem_db.slv_id_info[x].cnt):
                        ...
                        bcm_idx = int((res_addr & 0xffff)/ 0x4)
