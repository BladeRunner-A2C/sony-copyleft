...
from dumpers.arc import *
from dumpers.vrm_vt import *
from dumpers.bcm import *
...
def dump_drv_vts(dump_path, memory, target, debug_info, fill_char):
    ...
    for vrm_drv in filter((lambda vrm_drv: vrm_drv not in ignore_drv), range(0, no_vrm_drv-1)):
        ...
        with open(ss_file_name, 'a') as ss_vt_file:
            ...
            #print("\t\t~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", file=ss_vt_file)      
            print("\t\t~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", file=ss_vt_file)
            print("\t\t|      Resource      | VRM# | Vote | ENB | MODE |", file=ss_vt_file)
            print("\t\t~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", file=ss_vt_file)      
            for reg in range(0,no_vrm_reg):
                vrm = VRM_votetable[reg][vrm_drv]
                if vrm != 0:
                    print("\t\t|{0:>20s}|{1:>6d}|{2:>6d}|{3:>5d}|{4:>6d}| ".format( vrm_map[reg], reg, vrm[0], vrm[1], vrm[2] ), file=ss_vt_file)
