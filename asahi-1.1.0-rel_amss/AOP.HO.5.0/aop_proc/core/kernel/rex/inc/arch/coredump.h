...
typedef struct __attribute__((packed, aligned(4)))
{
  uint32_t gp_regs[27]; // x5 - x31

  uint32_t RA; // x1 -- Return Address
  uint32_t SP_main; //x2 -- Stack Pointer
  uint32_t GP_main; //x3 -- Global Pointer
  uint32_t TP_main; //x4 -- Thread Pointer
