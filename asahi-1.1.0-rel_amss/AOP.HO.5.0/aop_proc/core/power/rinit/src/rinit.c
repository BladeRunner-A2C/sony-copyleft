...
enum
{
  ...
  EXT_CMD_DB_GET_MOL = 0x12,
  EXT_GET_WLAN_VOLT  = 0x13,
  EXT_GET_VRM_ID = 0x14,
  EXT_GET_ARC_ID = 0x15,
  EXT_GET_RPID = 0x16,
  EXT_MOD_HDR = 0x100
    ...
    for (ext_processed = 0 ; ext_processed < ext_count; ext_processed++)
    {
      ...
      case EXT_GET_ARC_ID:
       ...
       case EXT_GET_RPID:
        if (!post_write) /*pre write extension*/
        {
          #define RPID_CMD_DB_AUX_DATA_LEN 4
          uint8 res_len = RPID_CMD_DB_AUX_DATA_LEN;
          uint8 res_data[RPID_CMD_DB_AUX_DATA_LEN] = {0};

          RINIT_INTS_TO_STR (buffer, args, args+1);
          CORE_VERIFY(CMD_DB_SUCCESS ==
                  cmd_db_query_aux_data (buffer, &res_len, res_data));
        ...
        case EXT_CMD_DB_GET_MOL:
          if (!post_write)          /* Pre write extension */
          {
            RINIT_INTS_TO_STR (buffer, args, args+1);
            ext_res = ((pwr_utils_mol_hlvl_named_resource(buffer) )<< 16) + args[2];
            init_val_update = TRUE;
          }
          ext_index += 4;    /* ext and three args */
          break;
        ...
        case EXT_GET_WLAN_VOLT:
          if (!post_write)          /* Pre write extension */
          {
            RINIT_INTS_TO_STR (buffer, args, args+1);
#if !defined(bitra_STUBS) && !defined(Olympic_STUBS) && !defined(WAIPIO_TARGET_CONFIG) && !defined(NETRANI_TARGET_CONFIG) && !defined(LANAI_TARGET_CONFIG) && !defined(HAMOA_TARGET_CONFIG)  &&!defined(PINNACLE_TARGET_CONFIG) && !defined(KUNO_TARGET_CONFIG) && !defined(MILOS_TARGET_CONFIG) 
            ext_res = cpr_get_wlan_volt(args[0]) ;
#endif
            init_val_update = TRUE;
          }
          ext_index += 2;    /* ext and one args */
          break;
 ...
 * rinit_module_init
 */
//TODO: __attribute__((section("rinit_code_sec_pool1")))
