...
typedef enum {
  ...
  CMD_DB_ERR_NODE_NOT_FOUND = CMD_DB_ERR_NOT_FOUND,
  CMD_DB_ERR_PROP_NOT_FOUND,
  CMD_DB_ERR_INVALID_SIZE,
} cmd_db_err_type;
...
typedef enum {
  ...
  CMD_DB_QUERY_RES_ID_MODIFY_DATA,
 ...
 */
uint32_t cmd_db_query_aux_len(const char *res_id);

// w
#define cmd_db_query_len cmd_db_query_aux_len
 ...
 * <!-- cmd_db_query_prop -->
 *
 * @brief Query a single property at the specified node
 *
 *  This is used to retrieve a single, full property.
 *
 *  @param path : Full node path to find node at
 *  @param prop_name: Name of the property to query
 *  @param prop_vals: Buffer to fill with property data
 *  @param prop_size: Size of the buffer in bytes
 *
 *  @return CMD_DB_SLV_ID_INVALID for failure, otherwise slv_id
 *          type.
 */

cmd_db_err_type cmd_db_query_prop(char *path, const char *prop_name, uint32_t *prop_vals, uint32_t prop_size);
