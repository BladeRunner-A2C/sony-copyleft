...
#-------------------------------------------------------------------------------
#
#  $Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/tools/build/scons/build/logger.py#1 $
...
class Logger:
    ...
    def __init__(self, lg_file_name, prefix=None, log_to_stdout=True, \
        ...
        self.log_file = open(lg_file_name + '.log', access_mode)
    ...
    def log_exec (self, cmd, shell=False, stdin = sys.stdin, env=None, \
        ...
        if sys.version_info.major == 2:
            ...
            for curr_line in task.stdout:
                if VERBOSE_LEVEL >= verbose:
                    decoded_line = curr_line.decode('utf-8-sig')
                    if self.log_file:
                        self.log_file.write(decoded_line)
                    if self.log_to_stdout:
