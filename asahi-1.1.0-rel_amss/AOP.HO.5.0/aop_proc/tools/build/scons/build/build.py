...
#-------------------------------------------------------------------------------
#
#  $Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/tools/build/scons/build/build.py#1 $
...
def build(lg, image_cfg_str, args):
    ...
    qcscons_cmd       = os.path.realpath(os.path.join(qcscons_root, "SCons/scripts/scons"))
    qcscons_lib_root  = os.path.realpath(os.path.join(qcscons_root, "SCons/lib"))
    if not on_linux:
        qcscons_cmd += '.bat'
    qcscons_script    = os.path.realpath(os.path.join( \
                            qcscons_root, "build/start.scons"))
    qcscons_build    = os.path.realpath(os.path.join( \
                            qcscons_root, "build"))
    changed_file_json = os.path.join(targ_root, "changed_files.json")
    ...
    # ================================
    # Add path to SCons in PYTHONPATH
    # ================================
    if 'PYTHONPATH' in os.environ:
        os.environ['PYTHONPATH'] += os.pathsep + qcscons_lib_root
    else:
        os.environ['PYTHONPATH'] = qcscons_lib_root
    ...
    run_swift = False
    swift_build_in_args = '--swift_build' in cleaned_args

    for bid in build_id_list:
        if swift_build_in_args and os.path.exists(os.path.join(qcscons_root, "scripts", "swift_build_utils.py")):
            sys.path.append(os.path.join(qcscons_root, "scripts"))
            import swift_build_utils
            swift_scons_script = os.path.join(qcscons_build, "swift_" + bid + ".scons")
            swift_details_json = os.path.join(qcscons_build, "swift_details_" + bid +".json")
            run_swift = swift_build_utils.is_swift_eligible(changed_file_json, swift_details_json, bid)
        if run_swift:
            lg.log("This Build met all the conditions that are needed to run SWIFT Incremental Build, "
                  "hence running SWIFT BUILD....")
            build_cmd_line = [qcscons_cmd, "-f", swift_scons_script, "swift_build"]
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        LG.log("Running $Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/tools/build/scons/build/build.py#1 $")
