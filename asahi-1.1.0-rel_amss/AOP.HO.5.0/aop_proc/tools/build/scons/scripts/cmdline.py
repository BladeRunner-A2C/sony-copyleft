...
#------------------------------------------------------------------------------
#
#  $Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/tools/build/scons/scripts/cmdline.py#1 $
...
def generate(env):
    ...
    # Build from declassified paths, even if red-ip synced components are present
    SCons.Script.AddOption('--build_from_artifacts', \
        dest='build_from_artifacts', \
        type='string', \
        default='', \
        help='Build from the specified comma-separated list of artifacts folders.'
             'Set to "all" to build from all declassified paths.'
    )

    # add SwiftBuild option
    SCons.Script.AddOption('--swift_build', \
                           dest='swift_build', \
                           action="store_true", \
                           default=False, \
                           help='Generates the info of every scons node required for doing swift build' \
                           )
...
#2345678901234567890123456789012345678901234567890123456789012345678901234567890
    ...
    old_cflags = env.get('CFLAGS', SCons.Util.CLVar())
    old_lflags = env.get('LFLAGS', SCons.Util.CLVar())
    ...
    # Convert some parameters to SCons.Util.CLVar
    if 'CFLAGS' in env and type(env['CFLAGS']) is str:
        new_cflags = env['CFLAGS']
        env['CFLAGS'] = old_cflags
        new_cflags = [x.strip() for x in new_cflags.split(',') if len(x) > 0]
        env.Append(CFLAGS=new_cflags)
    if 'LFLAGS' in env and type(env['LFLAGS']) is str:
        new_lflags = env['LFLAGS']
        env['LFLAGS'] = old_lflags
        new_lflags = [x.strip() for x in new_lflags.split(',') if len(x) > 0]
        env.Append(LFLAGS=new_lflags)
    ...
    env.Replace(VARIANT=env.get('BUILD_ID'))
    env.Replace(ARTIFACTS_R="${TARGET_ROOT}/artifacts_r")
    env.Replace(ARTIFACTS="${TARGET_ROOT}/artifacts")
...
#2345678901234567890123456789012345678901234567890123456789012345678901234567890
    ...
    build_from_artifacts = env.GetOption('build_from_artifacts').split(',')
    env.Replace(BUILD_FROM_ARTIFACTS=[x for x in build_from_artifacts if x])
    ...
    # Addressing request to enable time-actions by default for all non-Cirrus
    # CRM builds. Done for better metrics and tracking a given CRM build
    env['IS_ACTION_TIMING_OUTPUT_ENABLED'] = True
    if 'CRM_BUILDID' in os.environ and 'time-actions' not in debuginfo_opt:
        build_manifest = BuildManifest()
        if not build_manifest.is_service_build():
            env['IS_ACTION_TIMING_OUTPUT_ENABLED'] = False
            debuginfo_opt.append('time-actions')
...
Local Options:
  ...
  --debuginfo=<opt1,opt2,...> Debug Information, options are:
                                 ...
                                 time-actions, Log action timing and summary table
  ...
  --build_from_artifacts=<dir1,dir2,...>
                              Build from the specified artifact folders, even if
                              the corresponding sync folders are present.  Specify
                              "all" to build from all artifact folders.  Note that
                              you must specify the name of the artifact folder, not
                              that of the sync folder.
...
class BuildManifest:
    """
    Class definition that abstracts away the build_root/build/manifest.xml file
    """
    def __init__(self):
        self.si_name = ''
        self.build_id = ''
        self.branch_point_revision = ''
        self.revision = ''
        self.spin = ''
        self.client = ''

        build_root = os.environ.get('BUILD_ROOT')
        self.file_path = os.path.join(build_root, "build", "manifest.xml")
        if os.path.exists(self.file_path):
            manifest_etree = et.ElementTree(file=self.file_path)
            self.manifest_root = manifest_etree.getroot()
            self.image_tree = self.manifest_root.find('image_tree')
            self.si_name = self.get_element_text(self.image_tree.find('name'))
            self.build_id = self.get_element_text(self.image_tree.find('build_id'))
            self.branch_point_revision = self.get_element_text(
                self.image_tree.find('branch_point_revision'))
            self.revision = self.get_element_text(self.image_tree.find('revision'))
            self.spin = self.get_element_text(self.image_tree.find('spin'))
            self.client = self.get_element_text(self.image_tree.find('client'))

    def is_service_build(self):
        return bool(re.match(r'^[\d\.]+$', self.branch_point_revision))
    ...
    def get_element_text(element, default=''):
        ...
        :param default: Default value to use if element or element text field
        does not exist
        :return: Text field of element, or default value
        """
