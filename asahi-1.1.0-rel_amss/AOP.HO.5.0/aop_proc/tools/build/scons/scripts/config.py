...
#    Common utilities used across the build system
#
# Copyright (c) 2009-2018,2023 by Qualcomm Technologies, Incorporated.
# All Rights Reserved.
# QUALCOMM Proprietary/GTDR
#
#-------------------------------------------------------------------------------
#
#  $Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/tools/build/scons/scripts/config.py#1 $
...
def pack_out_files(env):
    ...
    if not test_pack_cleaned:
        ...
        clean_files += env.FindFiles("hexagon_obfuscator.py", "${BUILD_ROOT}/tools/build/scons/scripts")
        clean_files += env.FindFiles("hexagon_obf_decoder_builder.py", "${BUILD_ROOT}/tools/build/scons/scripts")
        clean_files += env.FindFiles("swift_build_utils.py", "${BUILD_ROOT}/tools/build/scons/scripts")
...
def list_apis(env):
    ...
    for au_item in au_list:
        au_pattern_list.append([au_item, [str.replace \
    ...
    for au_item in au_pattern_list:
        ...
        env.PrintDebugInfo('any', "Public APIs for AU: %s" % \
            str.upper(au_item[au_name_index]))
...
def init_incpaths(env):
    ...
    # get valid SCons AU names and create the list of paths
    au_list = env.GetAreaSoftwareUnitNames(include_api_aus=True)
    if type(au_list) is not list:
        au_list = [au_list]
...
def print_api(env, api_name, paths, area=None):
    ...
    if area is None:
        area = str.upper(env.get('AU_NAME', 'UNKNOWN'))
    api_name = "".join \
        ([area, "_", api_name])
    env.PrintDebugInfo('any', "---------------------------------------------" \
        "-------------------")
...
def require_public_api(env, api_names, area=None):
    ...
    if len(api_names) > 0:
        if area is None:
            ...
            api_prefix = str.upper(env.get('AU_NAME', 'CORE'))

        else:
            api_prefix = str.upper(area)
...
def require_au_restricted_api(env, api_names):
    ...
    api_prefix = "".join([str.upper(env.get('AU_NAME', 'UNKNOWN')), "_"])
    api_env = env['AU_INCPATHS_ENV']
    require_api(env, api_env, api_prefix, api_names, api_suffix="_RESTRICTED", \
        dbg_str="Required Restricted APIs", fname="internal_restricted")
    return None
...
def init_image_vars(env, alias_list, proc, config='none', plat='l4', \
    ...
    if buildpath is None:
      ...
      if env['MSM_ID'] in ['waipio']:
        qc_buildpath = env.subst("%s/${PROC}/${BUILD_ID}/${CHIPSET_PMIC}" % image_name.lower())
      else:
    ...
    if config == 'modem':
        ...
        env.Replace(CUST_H=str.lower(env.subst('CUST${BUILD_ID}.H')))
