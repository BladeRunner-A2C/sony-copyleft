...
#-------------------------------------------------------------------------------
#
#  $Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/tools/build/scons/scripts/hexagon.py#1 $
...
def old_hexagon_obf_files_preserver(obf_file_path, obf_file_prefix):
    current_working_dir = os.getcwd()
    os.chdir(obf_file_path)
    if os.path.exists(obf_file_prefix + '.elf' + '.obf'):
        ref_count = 1
        while os.path.exists(obf_file_prefix + '.elf' + '_%.2d.obf' % (ref_count)):
            ref_count += 1
        os.rename(obf_file_prefix + '.elf' + '.obf',
                  obf_file_prefix + '.elf' + '_%.2d.obf' % (ref_count))
...
def C_static_obj_emitter(target, source, env):
    ...
    if 'USES_COMPILATION_DB' in env:
        env.Clean(tgt, "".join([str(tgt[0]), ".json"]))
...
def add_program(env, target=None, source=None, **kargs):
    ...
    if (('CONFIG_ENV' in env and 'IS_OBFUSCATION_ENABLED' in env['CONFIG_ENV'])
            and env['CONFIG_ENV']['IS_OBFUSCATION_ENABLED']
            and 'RED_SOURCE_COMPONENT_LIST' in env and len(env['RED_SOURCE_COMPONENT_LIST'])>0):
        env.Replace(HEXAGONLD_OBF_CMD="-show-unobfuscated --od")
        variant_specific_filename = os.path.basename(target_base)

        # Obfuscated dictionary generation.
        variant_obf_file = env.subst("%s.elf.obf" % variant_specific_filename)
        obf_file_artifact_path = os.path.join(env.subst("${ARTIFACTS_R}"), variant_obf_file)
        if os.path.exists(env.subst("${ARTIFACTS_R}")):
            old_hexagon_obf_files_preserver(env.subst("${ARTIFACTS_R}"), variant_specific_filename)
        env.Append(LINKFLAGS="${HEXAGONLD_OBF_CMD}" + " " + obf_file_artifact_path)
        hexagon_obfuscated_dict = env.SideEffect(obf_file_artifact_path, target_elf)
        env["HexagonObfuscatedDict"] = hexagon_obfuscated_dict
        env.Clean(target_elf, obf_file_artifact_path)
...
def add_bin_shared_library(env, targets, library, sources, add_to_list=True, \
    ...
    builds.
    '''
    return add_shared_library(env, targets, library, sources, pack=True, add_to_list=add_to_list, \
...
def generate(env):
    ...
    # Try to parse-out the major.minor.spin revs if needed
    # Only captures the purely numeric pieces, and defaults to -1
    hexverlist = re.split(r'\D+',hexagon_rtos_release)
    hexagon_ver_major = int(hexverlist[0]) if len(hexverlist) > 0 else -1
    hexagon_ver_minor = int(hexverlist[1]) if len(hexverlist) > 1 else -1
    hexagon_ver_spin  = int(hexverlist[2]) if len(hexverlist) > 2 else -1
    ...
    if env['PLATFORM'] in ['cygwin']:
        qdsp6bin = str.replace("/cygdrive/%s" % qdsp6bin, ":", "")
    ...
    if 'USES_NO_DEBUG' in env:
        ...
        # Turn on appropriate DWARF debug flags, per compiler version
        if ((hexagon_ver_major >= 8 and hexagon_ver_minor >= 4)
            or hexagon_ver_major > 8):
            debugflag = '-g -gpubnames'  #hex tools >= 8.4.x incl 9+
        else:
            debugflag = '-g'  #default

        env.Replace(HEXAGON_DBG=debugflag)
        env.PrintInfo("DWARF debug enabled via {} for Hexagon {}.{}.{}".format(debugflag,hexagon_ver_major,hexagon_ver_minor,hexagon_ver_spin))
    ...
    #---------------------------------------------------------------------------
    # HEXAGON Lang independent options - optional extension
    #---------------------------------------------------------------------------

    # this one generated object level compile db
    if 'USES_COMPILATION_DB' in env:
      env.Append(HEXAGON_LANIND = "-MJ ${TARGET.posix}.json ")
    ...
    if 'USES_STANDARD_LINK_GROUPS' not in env:
        env.Append(QDSP6OBJS1=[
            '${QDSP6_LIBDIR}/crt0.o'
        ])
    ...
    if 'USES_LIBC_PLUS_PLUS' in env:
        # libc++ is a newer standard that supports up to c++17
        env.Append(QDSP6OBJS3=[
            '${QDSP6_RELEASE_LIB_DIR}/libc++.a',
            '${QDSP6_RELEASE_LIB_DIR}/libc++abi.a',
            '${QDSP6_RELEASE_LIB_DIR}/libc_eh.a',
        ])
    else:
        # libstdc++ is deprecated but still supported in hexagon 8.5.
        # It supports up to c++03
        env.Append(QDSP6OBJS3=[
    ...
    if 'USES_STANDARD_LINK_GROUPS' not in env:
        ...
        #---------------------------------------------------------------------------
        # Link groups are use to help the linker traverse trough libraries
        # more then once if a symbol is not found from the first pass.
        # The groups are not require for code location or initialization
        # and thus the previous defines (legacy) are really not needed,
        # nor qurt has to be treaded differently. Below is the standard way
        # the groups should be initialized.
        #
        # Basically
        #     init.o start-group <all libs and objs> end-group  fini.o
        #---------------------------------------------------------------------------
        env.Replace(
            LINKOBJGRP="${QDSP6OBJS1} "
    ...
    # Obfuscation OPTIONS
    #===========================================================================
    obfuscator = "${BUILD_SCRIPTS_ROOT}/hexagon_obfuscator.py"

    if env.PathExists(obfuscator):
        env.LoadToolScript(obfuscator)

    # ===========================================================================
    # Obfuscation OPTIONS
    # ===========================================================================
    hexagon_obf_decoder_builder_script = "${BUILD_SCRIPTS_ROOT}/hexagon_obf_decoder_builder.py"

    if env.PathExists(hexagon_obf_decoder_builder_script):
        env.LoadToolScript(hexagon_obf_decoder_builder_script)
