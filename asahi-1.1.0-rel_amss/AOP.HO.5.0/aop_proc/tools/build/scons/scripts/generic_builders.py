...
#------------------------------------------------------------------------------
#
#  $Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/tools/build/scons/scripts/generic_builders.py#1 $
...
def list_file_builder_generate(env):
    ...
    list_file_bld = env.Builder(
        action=list_file_act,
        emitter=list_file_emitter)
...
def list_file_emitter(target, source, env):
    '''
        list_file_emitter

        List file emitter, adds missed dependencies
    '''

    # for list files the location of the file are important too,
    # SCons does an MD5 check on the source and if MD5 is same
    # it will decare list file as "up to date". On the emitter
    # will add the file paths as dependencies as well

    source_list = env.Flatten(source)
    data = [str(x) for x in source_list]

    env.Depends(target, env.Value(data))
...
#===============================================================================
def list_file_extractor_get_nodes(env, lf_list, exec_dir=None):
    '''
       list_file_extractor
        ...
        if env.PathExists(lf_fname):
            ...
            for fname in lf_files_raw:
                if re.match(re_nonrel_match, fname):
                    ...
                    if exec_dir:
                        nodes[env.RealPath("%s/%s" % (exec_dir, fname))] = True
                    else:
...
def list_file_extractor_generate(env):
    ...
    '''
    list_file_ex_scn = env.Scanner(function=list_file_scanner, skeys=['.txt'])
    list_file_ex_act = env.GetBuilderAction(list_file_extractor)
    list_file_ex_bld = env.Builder(action=list_file_ex_act, source_scanner=list_file_ex_scn)
    env.Append(BUILDERS={'ListFileExtractor' : list_file_ex_bld})


#    The scons documentation says there should be 4 parameters, but I'm not
#    seeing the 'path' parameter when I'm running the build, so I'm leaving
#    it out here.
def list_file_scanner(node, env, arg):
    '''
        Parse the file listed in the node parameter and return the files listed
        there as dependencies.
    '''
    exec_dir = env.Dir('#').get_abspath()
    return [x for x in list_file_extractor_get_nodes(env, [node], exec_dir=exec_dir)
              if os.path.isfile(x)]
# End of list_file_scanner()
...
def replace_text_file_builder_generate(env):
    ...
    replace_file_bld = env.Builder(
        action=replace_file_act,
        emitter=replace_text_file_emitter)

    env.Append(BUILDERS={'ReplaceTextInFileBuilder' : replace_file_bld})


def replace_text_file_emitter(target, source, env):
    '''
        replace_text_file_emitter

        Emitter adds missing dependencies
    '''

    # The data contain in the replace dictionary should force a rebuild
    # thus adding that and th header data as dependency
    replace_dict = env.get('REPLACE_DICT')
    header_style = env.get('HEADER_STYLE')

    env.Depends(target, env.Value(replace_dict))
    env.Depends(target, env.Value(header_style))
...
def start_timing_action(target, source, env):
    timingNodeLogTimeObj = source[0].value
