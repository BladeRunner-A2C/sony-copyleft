...
#-------------------------------------------------------------------------------
#
#  $Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/tools/build/scons/tcfg/target_config.py#1 $
...
TCFG_FILE_NAME_TEMPLATE = "tcfg_%s.py"      # New data file for target config
                                            # (mostly USES_* flags)
SYMBOL_MAP_FILE_NAME_TEMPLATE = "config_symbol_map_%s.json"
                                            # JSON map mapping config file names
                                            # to the symbols they define.
        ...
        if not check_bid_products(env, build_id.lower()):
            ...
            if gen_result:
                ...
                if tcfg_bid_data:
                    ...
                    WriteSymbolMap(env, tcfg_bid_data, build_id)
                else:
                    gen_result = False
...
#------------------------------------------------------------------------
# Function: WriteSymbolMap
#
# Write a json file containing a map of configuration file names to the
...
#   Updates environment with Build ID config data
#------------------------------------------------------------------------
def WriteSymbolMap(env, tcfg_bid_data, build_id):
    '''
       Function: WriteSymbolMap
    ...
    with open(TARGET_CONFIG_PATH + SYMBOL_MAP_FILE_NAME_TEMPLATE
              % build_id.lower(), 'w') as sym_map_file:
        json.dump(tcfg_bid_data.symbol_map, sym_map_file, sort_keys=True,
                  indent=4, separators=(',', ': '))

# End of WriteSymbolMap()
