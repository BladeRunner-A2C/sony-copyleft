...
#-------------------------------------------------------------------------------
#
#  $Header: //service/AOP/AOP.HO.5.0-00688-LANAI_E-1.65547/aop_proc/tools/build/scons/tcfg/tcfgparser.py#1 $
...
class Printable:
   ...
   def DumpObjToStr (self, obj, pad=0):
      ...
      if ty == str:
      ...
      elif ty in small_types or obj is None:
         ...
         for key in obj.keys():
            ...
            ty2 = type(obj[key])
            if ty2 in small_types or obj[key] is None:
         ...
         for field in fields:
            ...
            ty2 = type(value)
            if ty2 in small_types or value is None:
...
class TargetConfigurationDb(Printable):
         ...
         while token != build_file.eof:
            ...
            if token == 'define':
               ...
               if pl_to_include == '' or pl_to_include == pl:
                  ...
                  build_file.push_source(hook[1], hook[0])
            elif token == 'tag_map':
   ...
   def GenConfigData (self, build_id, pl = None):
      ...
      try:
         ...
         return_data.symbol_map = self.CreateSymbolMap(bid_config_data)
   ...
   def ExtractBidConfigData (self, bid_config_data):
      ...
      for tag in self.TagMap.Elements:
         print("tcfgparser: Visiting " + tag.name + ": " + tag.bid_val.upper())
         BEType = self.BuildElementTypes[tag.name]
         BE     = BEType.Elements[tag.bid_val]
         ...
         if attr.image in bid_config_data.images_allowed:
            if attr.type == 'define':
               ...
               if attr.value in ['make', 'makeonly']:
                  ...
                  attr.name = "USES_" + attr.name.upper()
...
""" % (type.upper() + build_id.upper(), date.today().year))
   ...
   # CreateSymbolMap
   #
   # Create a dictionary mapping symbols to the name of the .builds files
   ...
   #
   # Returns:
   #   Dictionary mapping symbols to the files in which they were defined.
   # -------------------------------------------------------------------
   def CreateSymbolMap(self, bid_config_data):

      symbol_map = dict()
      attrs = []
      attrs += bid_config_data.hattrs.defines
      attrs += bid_config_data.hattrs.undefs
      attrs += bid_config_data.hattrs.undef_firsts
      # attrs += bid_config_data.hattrs.includes
      attrs += bid_config_data.sattrs.defines
      attrs += bid_config_data.sattrs.undefs
      attrs += bid_config_data.sattrs.undef_firsts
      # attrs += bid_config_data.sattrs.includes
      # attrs += bid_config_data.unuses
      attrs += bid_config_data.mod_uses
      attrs += bid_config_data.mod_tcfg

      for attr in attrs:
         if attr.SourceFileName not in symbol_map:
            symbol_map[attr.SourceFileName] = []
         symbol_map[attr.SourceFileName].append(attr.name)

      return symbol_map

   # End of TargetConfigurationDb.CreateSymbolMap()

# End of class TargetConfigurationDb
...
class TagElement(Printable):
   ...
   def __init__ (self, token, build_file):
      ...
      if self.start >= 0:
         ...
         elif index == '-':   # Only supports exactly one more build ID letter
            self.end = -1
         elif index == '+':   # Goes to end of build ID.
            self.end = None
      else:
...
class AttributeParent(Printable):
   ...
   # Parameters:
   #   self:              Self reference
   #   source_file_name:  Name of file that defined this attribute
   # 
   # Return Value:
   #   No return value.
   #------------------------------------------------------------------
...
class AttributeDefine(AttributeParent):
   ...
   #------------------------------------------------------------------
   def __init__ (self, build_file, GlobalDefines):
      AttributeParent.__init__(self, build_file.infile)
      self.type = 'define'
...
class Tokenizer(shlex.shlex):
   ...
   #   No return value.
   #------------------------------------------------------------------
   def __init__ (self, instream, stream_name):
      shlex.shlex.__init__(self, instream, stream_name)
      self.whitespace = ''
   ...
   def get_multi_token (self, terminators, preserve_case=False, expand_dict=None, see_whitespace = False):
      ...
      result = ""
      add_space = False
      while token not in terminators and token != self.eof:
