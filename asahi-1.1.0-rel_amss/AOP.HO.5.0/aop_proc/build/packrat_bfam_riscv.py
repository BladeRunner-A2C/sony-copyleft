...
def execfile(file, g=None, l=None):
    with open(file, "rb") as src:
        code = compile(src.read(), file, "exec")
    exec(code, g, l)
...
def running_from_aop_build():
  ...
  return os.path.exists('build/build_common_riscv.py')
...
def configure_no_strip_no_debug(msm):
  ...
  # Update build_common_riscv to default to external variant BUILD_ID
  move_file('build/build_common_riscv.py', 'build/build_common_riscv.py.orig')
  orig = open('build/build_common_riscv.py.orig', 'r')
  new = open('build/build_common_riscv.py', 'w')
  ...
  orig.close()
  new.close()
  delete_file('build/build_common_riscv.py.orig')
...
def check_elf_exists(msm):
  ...
  elif msm in ['6150','7150','cedros','8350', 'makena','kodiak','fillmore', 'waipio','aurora','palima','lassen','netrani','kapiti','kailua', 'lanai', 'pakala','palawan']:
    return os.path.exists(msm_path + '/' + msm + '/aop.mbn')
  elif msm in ['ivi']:
    return os.path.exists(msm_path + '/' + 'makenaau' + '/' 'IVI' + '/aop.mbn')
  elif msm in ['adas']:
    return os.path.exists(msm_path + '/' + 'makenaau' + '/' 'ADAS' + '/aop.mbn')
  else:
...
def create_profiled_scons():
  if platform.system() == "Linux":
    move_file('tools/build/scons/SCons/scripts/scons', 'tools/build/scons/SCons/scripts/scons.sh.normal')
    move_file('tools/build/scons/SCons/scripts/profiled_scons.sh', 'tools/build/scons/SCons/scripts/scons')
  else:
    move_file('tools/build/scons/SCons/scripts/scons.bat', 'tools/build/scons/SCons/scripts/scons.bat.normal')
    move_file('tools/build/scons/SCons/scripts/profiled_scons.bat', 'tools/build/scons/SCons/scripts/scons.bat')
  return True

@describe_func('Restore the normal scons.bat')
def restore_regular_scons():
  if platform.system() == "Linux":
    move_file('tools/build/scons/SCons/scripts/scons', 'tools/build/scons/SCons/scripts/profiled_scons.sh')
    move_file('tools/build/scons/SCons/scripts/scons.sh.normal', 'tools/build/scons/SCons/scripts/scons')
  else:
    move_file('tools/build/scons/SCons/scripts/scons.bat', 'tools/build/scons/SCons/scripts/profiled_scons.bat')
    move_file('tools/build/scons/SCons/scripts/scons.bat.normal', 'tools/build/scons/SCons/scripts/scons.bat')
  return True
...
def parse_files_used(file_list,msm_id):
  ...
  profiler_used_files = re.findall(cwd + '[^:]+', stats.decode('utf-8'))
...
def match_whitelist(all_files, whitelist_files, msm):
  ...
  if msm == '670':
    ...
    execfile(packrat_path + '/whitelist_6150.py', whitelist)
  elif msm == '7150':
    execfile(packrat_path + '/whitelist_7150.py', whitelist)
  elif msm == 'TalosAU':
    execfile(packrat_path + '/whitelist_TalosAU.py', whitelist)
  elif msm == 'Saipan':
    execfile(packrat_path + '/whitelist_Saipan.py', whitelist)
  elif msm == '55':
    execfile(packrat_path + '/whitelist_55.py', whitelist)
  elif msm == 'bitra':
    execfile(packrat_path + '/whitelist_bitra.py', whitelist)
  elif msm == 'cedros':
    execfile(packrat_path + '/whitelist_cedros.py', whitelist)
  elif msm == 'kodiak':
    execfile(packrat_path + '/whitelist_kodiak.py', whitelist)
  elif msm == 'fillmore':
    execfile(packrat_path + '/whitelist_fillmore.py', whitelist)
  elif msm == 'waipio':
    execfile(packrat_path + '/whitelist_waipio.py', whitelist)
  elif msm == 'kailua':
    execfile(packrat_path + '/whitelist_kailua.py', whitelist)
  elif msm == 'kapiti':
    execfile(packrat_path + '/whitelist_kapiti.py', whitelist)
  elif msm == 'palima':
    execfile(packrat_path + '/whitelist_palima.py', whitelist)
  elif msm == 'aurora':
    execfile(packrat_path + '/whitelist_aurora.py', whitelist)
  elif msm == 'lassen':
    execfile(packrat_path + '/whitelist_lassen.py', whitelist)
  elif msm == 'netrani':
    execfile(packrat_path + '/whitelist_netrani.py', whitelist)
  elif msm == 'lanai':
    execfile(packrat_path + '/whitelist_lanai.py', whitelist)
  elif msm == 'palawan':
    execfile(packrat_path + '/whitelist_palawan.py', whitelist)
  elif msm == 'pakala':
    execfile(packrat_path + '/whitelist_pakala.py', whitelist)
  else:
...
def main(argv):
  ...
  if msm == '670':
    ...
    supported_targets = ['pakala']
  elif msm == 'kailua':
    supported_targets = ['kailua']#['kailua','kapiti'] kapiti_todo for SPF support enable, changes already in
  elif msm == 'waipio':
    supported_targets = ['waipio','palima']
  elif msm == 'aurora':
    supported_targets = ['aurora']
  elif msm == 'fillmore':
    supported_targets = ['fillmore']
  elif msm == 'lassen':
    supported_targets = ['lassen']
  elif msm == 'netrani':
    supported_targets = ['netrani']
  elif msm == 'palawan':
    supported_targets = ['palawan']
  else:
