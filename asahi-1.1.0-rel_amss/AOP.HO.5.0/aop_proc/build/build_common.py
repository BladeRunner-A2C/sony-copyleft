...
def get_clang_version(path):
    ...
    m = search(r'(Snapdragon LLVM ARM Compiler\s(?P<major>\d+).(?P<minor>\d+).(?P<build>\d+))', output.decode('utf-8').strip('\x00'))
    if not m:
...
def create_target_specific_build_artifacts():
  ...
  elif os.environ['MSM_ID'] in ['pinnacle'] and os.environ.get('BIN_FOLDER_NAME') is not None:
     target_artifact_dir = '../core/bsp/aop/build/'+os.environ['BIN_FOLDER_NAME']+'/'
  else:
  ...
  artifacts = [ # Target Artifact                           # Source Artifact
                ...
                [target_artifact + '_unsigned.mbn',         source_artifact + '_unsigned.mbn'],
                [target_artifact + '.elf',                  source_artifact + '.elf'],
                [target_artifact + '_devcfg.mbn',           source_artifact + '_devcfg.mbn'],
                [target_artifact + '_devcfg_unsigned.mbn',  source_artifact + '_devcfg_unsigned.mbn'],
...
def start_build():
  ...
  while c:
     log.write(c.decode('utf-8','ignore').strip('\x00'))
     sys.stdout.write(c.decode('utf-8','ignore').strip('\x00'))
     c = the_build.stdout.read(1)
  exit_code = the_build.wait()

  create_target_specific_build_artifacts()
  if os.environ['MSM_ID'] in ['pinnacle'] and os.environ.get('BIN_FOLDER_NAME') is not None:
    GenerateXML("build-cfg-%s.xml" %os.environ['BIN_FOLDER_NAME'])
  else:
    chipset = os.environ.get('CHIPSET')
    GenerateXML("build-cfg-%s.xml" %os.environ['CHIPSET'])

  if exit_code == 0:
    if "GENERATE_DEVCFG_MBN_ONLY" not in os.environ and "-c" not in passed_args:
      generate_memsizes()
  sys.exit(exit_code)
...
def generate_memsizes():
    ...
    try:
        os.system('python3 mapread_hfam_splitmem.py ')
    except:
...
if __name__ == '__main__':
  ...
  DEFAULT_BUILD_ID = 'AAAAANAAO'

  os.environ['BUILD_ID']  = os.environ.get('BUILD_ID', DEFAULT_BUILD_ID)
