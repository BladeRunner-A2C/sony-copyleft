/*
 * Copyright 2016, 2021, 2022, 2023 Sony Corporation
...
#elif defined(FEATURE_SM8550) || defined(FEATURE_SM8650) || defined(FEATURE_SM6450)
			result = bootconfig_append_color_id(boot_context, ta_unit, ta_unit_sz);
#elif defined(FEATURE_SM4350)
// NOT support color_id in sm4350
#else /* !(FEATURE_SM6375 || FEATURE_SM8550 || FEATURE_SM8650 || FEATURE_SM6450) */
#error "NOT support platform"
#endif /* !(FEATURE_SM6375 || FEATURE_SM8550 || FEATURE_SM8650 || FEATURE_SM6450) */
			free(ta_unit);
		} else {
			FCS_LOG_ERR("MiscTA%d is invalid strings\n", TA_HW_COLOR_ID);
		}
...
static void sony_boot_populate_commandline(struct xboot_ctx *boot_context)
{
	...
	uint32_t cnt = 0;
	char *buf;
#if !defined(FEATURE_SM8550) && !defined(FEATURE_SM8450) && !defined(FEATURE_SM6450)
	int ret;
	uint32_t unitSize = 0;
	uint8_t *ta_buf = NULL;
#endif /* !defined(FEATURE_SM8550) && !defined(FEATURE_SM8450) && !defined(FEATURE_SM6450) */
...
#if !defined(FEATURE_SM8550) && !defined(FEATURE_SM8450) && !defined(FEATURE_SM6450)
	/* Add androidboot.serialconsole=0 to bootconfig if miscTA2473 is 0 (including read error) */
	ret = sony_boot_ta_simple_read(TA_ENABLE_CONSOLE, &ta_buf, &unitSize);
	if (!((ret == FCSERR_OK) && unitSize && (*ta_buf&0x01))){
		boot_context->bootconfig->taCommandLineArgs = sony_alloc_strcat(
		boot_context->bootconfig->taCommandLineArgs, " androidboot.serialconsole=0");
	}
