...
static ssize_t usb2_nvm_reg_addr_store(struct class *c,
				       struct class_attribute *attr,
				       const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
	...
	somc_bcext_set_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB2_NVM_ADDR,
			    reg_addr);

	return count;
}
static ssize_t usb2_nvm_reg_addr_show(struct class *c,
				      struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
							struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB2_NVM_ADDR,
			    &reg_addr);

	return scnprintf(buf, PAGE_SIZE, "0x%02X\n", (int)reg_addr);
}
static CLASS_ATTR_RW(usb2_nvm_reg_addr);

static ssize_t usb2_nvm_reg_data_show(struct class *c,
				      struct class_attribute *attr,char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
							struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB2_NVM_DATA,
			    &reg_data);

	return scnprintf(buf, PAGE_SIZE, "0x%02X\n", (int)reg_data);
}
static CLASS_ATTR_RO(usb2_nvm_reg_data);

static ssize_t usb2_cc1stat_show(struct class *c, struct class_attribute *attr,
				 char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
							struct somc_bcext_dev, bcext_class);
	u32 cc1_stat;
	char *cc1_power_status;

	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB2_CC1_STATUS,
			    &cc1_stat);
	switch (cc1_stat) {
	case USB2_SNK_POWER_NONE:
		cc1_power_status = "None";
		break;
	case USB2_SNK_POWER_DEFAULT:
		cc1_power_status = "SNK Default";
		break;
	case USB2_SNK_POWER_15:
		cc1_power_status = "SNK Power1.5";
		break;
	case USB2_SNK_POWER_30:
		cc1_power_status = "SNK Power3.0";
		break;
	default:
		cc1_power_status = "Unknown";
	};

	return scnprintf(buf, PAGE_SIZE, "%s\n", cc1_power_status);
}
static CLASS_ATTR_RO(usb2_cc1stat);

static ssize_t usb2_cc2stat_show(struct class *c, struct class_attribute *attr,
				 char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
							struct somc_bcext_dev, bcext_class);
	u32 cc2_stat;
	char *cc2_power_status;

	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB2_CC2_STATUS,
			    &cc2_stat);
	switch (cc2_stat) {
	case USB2_SNK_POWER_NONE:
		cc2_power_status = "None";
		break;
	case USB2_SNK_POWER_DEFAULT:
		cc2_power_status = "SNK Default";
		break;
	case USB2_SNK_POWER_15:
		cc2_power_status = "SNK Power1.5";
		break;
	case USB2_SNK_POWER_30:
		cc2_power_status = "SNK Power3.0";
		break;
	default:
		cc2_power_status = "Unknown";
	};

	return scnprintf(buf, PAGE_SIZE, "%s\n", cc2_power_status);
}
static CLASS_ATTR_RO(usb2_cc2stat);

static ssize_t usb2_plugged_show(struct class *c, struct class_attribute *attr,
				      char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
							struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB2_PLUGGED,
			    &plugged);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)plugged);
}
static CLASS_ATTR_RO(usb2_plugged);

static ssize_t usb2_snk_ready_show(struct class *c, struct class_attribute *attr,
				 char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
							struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB2_SNK_READY,
			    &snk_ready);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)snk_ready);
}
static CLASS_ATTR_RO(usb2_snk_ready);

static ssize_t usb2_voltage_show(struct class *c, struct class_attribute *attr,
				   char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
							struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB2_IN_V,
			    &voltage);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)voltage);
}
static CLASS_ATTR_RO(usb2_voltage);

static ssize_t usb_plugged_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
							struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB_PLUGGED,
			    &plugged);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)plugged);
}
static CLASS_ATTR_RO(usb_plugged);

static ssize_t usb_voltage_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
							struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB_IN_V,
			    &voltage);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)voltage);
}
static CLASS_ATTR_RO(usb_voltage);
...
static struct attribute *somc_bcext_class_attrs[] = {
	...
	&class_attr_usb2_cc1stat.attr,
	&class_attr_usb2_cc2stat.attr,
	&class_attr_usb2_plugged.attr,
	&class_attr_usb2_snk_ready.attr,
	&class_attr_usb2_voltage.attr,
	&class_attr_usb_plugged.attr,
	&class_attr_usb_voltage.attr,
	...
	&class_attr_usb2_nvm_reg_addr.attr,
	&class_attr_usb2_nvm_reg_data.attr,
