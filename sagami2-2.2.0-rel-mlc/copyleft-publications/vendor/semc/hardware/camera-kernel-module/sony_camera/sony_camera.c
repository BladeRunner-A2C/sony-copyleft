...
#define SONY_CAMERA_MAX_IDLE_RETRY_COUNT			50
...
struct sony_camera_data {
	...
	struct regulator					*cam_clk2;
	struct regulator					*cam_clk3;
	struct regulator					*cam_vmdr;
	struct regulator					*cam_vdig2;
	struct regulator					*cam_vdig3;
	struct regulator					*cam_vdig4;
	struct regulator					*pmic_vin;
	struct regulator					*cam_pmic_cs0;
	struct regulator					*cam_pmic_cs1;
	struct regulator					*cam_pmic_vdd0;
	struct regulator					*cam_pmic_vdd1;
	struct regulator					*cam_gyro;
	struct regulator					*cam_isp1;
	struct regulator					*cam_isp2;
	struct regulator					*cam_isp3;
	struct regulator					*cam_isp4;
	struct regulator					*cam_isp5;
	struct regulator					*cam_extio;
	...
	unsigned int flags, const char *label)
{
	...
	LOGD("regist gpio %d for %s", irq_gpio->gpio, label);
	return 0;
}
	...
	enum sony_camera_cmd cmd, int level, int op_mode)
{
	...
	if (cmd == SONY_CAM_VDIG) {
	...
	} else if (cmd == SONY_CAM_CLK2) {
		if (data->cam_clk2) {
			vreg = data->cam_clk2;
		} else {
			vreg = regulator_get(dev, "cam_clk2");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_clk2, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
	...
	} else if (cmd == SONY_CAM_CLK3) {
		if (data->cam_clk3) {
			vreg = data->cam_clk3;
		} else {
			vreg = regulator_get(dev, "cam_clk3");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_clk3, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
	...
	} else if (cmd == SONY_CAM_VDIG3) {
		if (data->cam_vdig3) {
			vreg = data->cam_vdig3;
		} else {
			vreg = regulator_get(dev, "cam_vdig3");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_vdig3, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
	...
	} else if (cmd == SONY_CAM_VDIG4) {
		if (data->cam_vdig4) {
			vreg = data->cam_vdig4;
		} else {
			vreg = regulator_get(dev, "cam_vdig4");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_vdig4, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
		...
		if (data->cam_pmic_cs0) {
			vreg = data->cam_pmic_cs0;
		} else {
			vreg = regulator_get(dev, "cam_pmic_cs0");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_pmic_cs0, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
			data->cam_pmic_cs0 = vreg;
		}
	} else if (cmd == SONY_PMIC_CS1) {
		if (data->cam_pmic_cs1) {
			vreg = data->cam_pmic_cs1;
		} else {
			vreg = regulator_get(dev, "cam_pmic_cs1");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_pmic_cs1, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
			data->cam_pmic_cs1 = vreg;
		}
	} else if (cmd == SONY_PMIC_VDD0) {
		if (data->cam_pmic_vdd0) {
			vreg = data->cam_pmic_vdd0;
		} else {
			vreg = regulator_get(dev, "cam_pmic_vdd0");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_pmic_vdd0, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
			data->cam_pmic_vdd0 = vreg;
		}
	} else if (cmd == SONY_PMIC_VDD1) {
		if (data->cam_pmic_vdd1) {
			vreg = data->cam_pmic_vdd1;
		} else {
			vreg = regulator_get(dev, "cam_pmic_vdd1");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_pmic_vdd1, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
			data->cam_pmic_vdd1 = vreg;
		}
	} else if (cmd == SONY_CAM_ISP1) {
		if (data->cam_isp1) {
			vreg = data->cam_isp1;
		} else {
			vreg = regulator_get(dev, "cam_isp1");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_isp1, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
	...
	} else if (cmd == SONY_CAM_ISP2) {
		if (data->cam_isp2) {
			vreg = data->cam_isp2;
		} else {
			vreg = regulator_get(dev, "cam_isp2");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_isp2, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
	...
	} else if (cmd == SONY_CAM_ISP3) {
		if (data->cam_isp3) {
			vreg = data->cam_isp3;
		} else {
			vreg = regulator_get(dev, "cam_isp3");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_isp3, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
	...
	} else if (cmd == SONY_CAM_ISP4) {
		if (data->cam_isp4) {
			vreg = data->cam_isp4;
		} else {
			vreg = regulator_get(dev, "cam_isp4");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_isp4, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
	...
	} else if (cmd == SONY_CAM_ISP5) {
		if (data->cam_isp5) {
			vreg = data->cam_isp5;
		} else {
			vreg = regulator_get(dev, "cam_isp5");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_isp5, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
	...
	} else if (cmd == SONY_CAM_EXTIO) {
		if (data->cam_extio) {
			vreg = data->cam_extio;
		} else {
			vreg = regulator_get(dev, "cam_extio");
			if (IS_ERR(vreg)) {
				LOGE("could not get cam_extio, vreg = %ld",
					PTR_ERR(vreg));
				rc = -ENODEV;
				goto exit;
			}
...
exit:
	if (rc < 0 || level < 0) {
		if (vreg == data->cam_vdig)
			...
			data->cam_vdig2 = NULL;
		else if (vreg == data->cam_vdig3)
			data->cam_vdig3 = NULL;
		else if (vreg == data->cam_vdig4)
			data->cam_vdig4 = NULL;
		else if (vreg == data->cam_pmic_cs0)
			data->cam_pmic_cs0 = NULL;
		else if (vreg == data->cam_pmic_cs1)
			data->cam_pmic_cs1 = NULL;
		else if (vreg == data->cam_pmic_vdd0)
			data->cam_pmic_vdd0 = NULL;
		else if (vreg == data->cam_pmic_vdd1)
			data->cam_pmic_vdd1 = NULL;
		else if (vreg == data->cam_gyro)
			data->cam_gyro = NULL;
		else if (vreg == data->cam_clk)
			data->cam_clk = NULL;
		else if (vreg == data->cam_clk2)
			data->cam_clk2 = NULL;
		else if (vreg == data->cam_clk3)
			data->cam_clk3 = NULL;
		else if (vreg == data->cam_vmdr)
			data->cam_vmdr = NULL;
		else if (vreg == data->cam_isp1)
			data->cam_isp1 = NULL;
		else if (vreg == data->cam_isp2)
			data->cam_isp2 = NULL;
		else if (vreg == data->cam_isp3)
			data->cam_isp3 = NULL;
		else if (vreg == data->cam_isp4)
			data->cam_isp4 = NULL;
		else if (vreg == data->cam_isp5)
			data->cam_isp5 = NULL;
		else if (vreg == data->cam_extio)
			data->cam_extio = NULL;
	}
	...
	uint8_t ctrl)
{
	...
	if (ctrl == 1) {
		if (camera_data->has_hw_sof) {
			rc = sony_camera_regist_gpio_irq(camera_data,
				sony_camera_irq_handler,
				// TODO: Make dtsi to change FALLING and RISING
...
static void __exit sony_camera_exit_module(void)
{
	...
	sony_camera_platform_cleanup();
	for (i = 0; i < g_sensor_num; i++) {
		dev_t dn = MKDEV(MAJOR(g_dev_id), i);
		device_destroy(g_c, dn);
